...
Motivation: 解决文本离散性导致的不可微分计算问题，以及检测器与攻击者之间的tokenizer不匹配问题。

Method: 引入加权嵌入、暖启动攻击者方法、tokenizer推理和模型提取技术。

Result: 在多个数据集和语言模型上表现优异，成功攻击商业检测器，并发现训练数据表达风格差异是主要漏洞。

Conclusion: 开源GradEscape以促进更鲁棒的检测器开发，并提出潜在防御策略。
...
<details>
  <summary>Details</summary>
Motivation: 研究目的是通过分析代理在模拟网络防御挑战中的表现，提高对代理行为的可解释性，并识别其成功与失败的模式。

Method: 通过简化复杂的状态和动作空间，跟踪关键事件（如渗透和清除事件），分析环境状态转换以及评估诱饵服务的效果。

Result: 防御代理通常在主机被利用后1-2个时间步内清除渗透；某些重要动作的无效性高达40%-99%；诱饵服务可阻止高达94%的直接特权访问攻击。

Conclusion: 研究揭示了代理行为的细节，并提出了对CAGE Challenge 4的改进建议，以增强模拟环境的真实性。
...
<details>
  <summary>Details</summary>
Motivation: Docker镜像的透明性和信任机制被广泛依赖，但gh0stEdit漏洞暴露了其安全缺陷。

Method: 通过gh0stEdit工具，攻击者可以隐蔽地修改镜像，绕过历史记录、层级和签名验证。

Result: 攻击案例展示了漏洞的实际危害，且现有静态和动态扫描工具无法检测。

Conclusion: 当前Docker镜像的安全机制存在严重漏洞，亟需改进。
...
<details>
  <summary>Details</summary>
Motivation: 传统防护措施（如掩码）存在集成复杂、面积开销大等问题，PoSyn旨在解决这些问题并提升安全性。

Method: 采用最优二分映射和成本函数，在不改变功能的情况下优化RTL到网表的转换。

Result: 实验显示DPA和CPA攻击成功率分别降至3%和6%，面积效率提升3.79倍。

Conclusion: PoSyn显著提升了密码硬件的安全性和效率，优于传统防护措施。
...
<details>
  <summary>Details</summary>
Motivation: LLMs在自然语言处理中表现卓越，但其输出的不一致性和不可预测性在安全关键领域（如访问控制）中带来挑战。

Method: 采用两种方法：1）仅通过自然语言提示生成配置；2）结合官方文档生成配置，并评估其准确性。

Result: 发现当前LLMs在生成配置时存在显著挑战。

Conclusion: 研究为LLMs在访问控制系统中的优化部署提供了重要见解。
...
TLDR: 论文提出了一种用户中心的搜索隐私保护方法Distortion Search，通过混淆查询关键词和k-匿名化点击行为来保护用户意图隐私。


<details>
  <summary>Details</summary>
Motivation: 搜索引擎的搜索日志可能泄露用户意图，现有隐私工具依赖第三方，用户无法自主控制隐私。

Method: 通过混淆查询关键词类别和k-匿名化点击行为生成扭曲的用户画像。

Result: 初步实验表明，该方法能在不依赖搜索引擎或第三方的情况下保护用户意图隐私。

Conclusion: Distortion Search提供了一种用户自主保护搜索隐私的有效方法。
...
Method: ReAgent采用两级方法：执行层验证代理思考与行动的一致性；规划层通过代理重建指令检查与用户指令的一致性。

Result: ReAgent显著降低攻击成功率（如数据库操作任务中降低90%），优于现有防御方法。

Conclusion: 研究表明可以利用被攻击代理自身来减轻后门风险。
...
<details>
  <summary>Details</summary>
Motivation: 尽管DRL在无人机导航系统中得到广泛应用，但其安全漏洞尚未得到系统研究。本文旨在填补这一空白，重点关注GPS欺骗攻击。

Method: 提出了一个攻击模型，通过GPS欺骗攻击模拟PX4自动驾驶仪的EKF传感器融合，并将其与DRL系统结合，设计更真实的攻击场景。

Result: 实验证明，攻击不仅在基础DRL系统中可行，在结合PX4自动驾驶仪的DRL系统中同样有效。

Conclusion: 本文揭示了DRL导航系统在GPS欺骗攻击下的脆弱性，强调了安全研究的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有PAP仅支持单一固定攻击目标，无法适应动态环境（如自动驾驶）的需求。

Method: 通过预定义条件（如投射不同颜色光线）动态切换攻击目标，保持隐蔽性。

Result: 在交通标志识别和深度估计任务中验证了可行性和有效性，并通过户外实验确认其鲁棒性。

Conclusion: SwitchPatch提供了一种灵活且实用的对抗策略，适用于多样化任务和现实条件。
...
<details>
  <summary>Details</summary>
Motivation: 现有GNN所有权验证方法（指纹和黑盒水印）存在计算成本高、后门风险或无法传递额外信息的问题。

Method: WGLE利用LDDE（层间距离差异）在边上的量化差异，预定义正负值嵌入水印，避免影响主任务。

Result: 在六个数据集和GNN架构上，WGLE实现了100%验证准确率、0.85%平均保真度下降，且嵌入开销低。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是评估攻击性安全研究中AI伦理的沟通方式，揭示趋势和最佳实践。

Method: 分析15篇利用LLMs进行攻击性安全研究的论文，关注伦理表达和论证。

Result: 86.6%的研究提及伦理考量，主要动机是普及渗透测试和防御AI攻击。

Conclusion: 学术社区在创新与伦理责任间取得平衡，但需进一步填补伦理讨论的空白。
...
<details>
  <summary>Details</summary>
Motivation: IoT设备资源有限且攻击面扩大，现有加密日志方案效率低，需平衡安全性与性能。

Method: 通过高效随机性管理、灵活聚合和多种算法实现，结合GPU加速批量验证框架。

Result: POSLO在中等GPU上每秒可验证231条日志，比现有技术更紧凑且高效。

Conclusion: POSLO适合IoT-Cold-STaaS生态系统，显著节省IoT设备电量，提升日志审计效率。
...
<details>
  <summary>Details</summary>
Motivation: 数字恐怖主义对患者和医疗提供者的数据安全构成严重威胁，现有方法存在漏洞。

Method: 结合对称加密（数据加密）、非对称加密（访问控制和签名）及Lesamnta-LW签名技术。

Result: 实验证明该方法在安全性和性能上优于现有方法。

Conclusion: 该方法有效提升了医疗数据的安全性，适用于无线传感器网络环境。
...
<details>
  <summary>Details</summary>
Motivation: 传统污点分析存在可扩展性问题，而现有的模糊测试驱动污点推断方法运行时开销较大。

Method: 提出ZTaint-Havoc，利用havoc变异模式进行轻量级污点推断，并设计有效的变异算法。

Result: 在AFL++中实现ZTaint-Havoc，边缘覆盖率提升最高达33.71%（FuzzBench）和51.12%（UniBench），平均提升2.97%和6.12%。

Conclusion: ZTaint-Havoc是一种高效且低开销的污点推断方法，显著提升了模糊测试的效果。
...
<details>
  <summary>Details</summary>
Motivation: 研究智能手表作为超声波隐蔽通信的未充分探索但有效的攻击媒介。

Method: 利用智能手表内置麦克风实时捕获18-22 kHz超声波信号，评估不同环境条件下的攻击可行性。

Result: 实验验证了智能手表在超声波隐蔽通信中的有效性，并分析了影响因素。

Conclusion: 智能手表在高安全环境中存在安全风险，需制定缓解策略应对这一新兴威胁。
...
<details>
  <summary>Details</summary>
Motivation: 混合网络的复杂性使得长期隐私侵蚀难以量化，需要更准确的方法评估隐私保护效果。

Method: 训练大型语言模型分析混合网络流量，测量不同设置下的发送者-消息不可链接性。

Result: 发现不同混合策略的隐私水平差异显著，传统隐私指标（如熵和似然）无法全面捕捉多观测合成的隐私风险。

Conclusion: 更大的模型能更高效地评估隐私，未来Transformer的进步将进一步提升隐私评估的准确性。
...
<details>
  <summary>Details</summary>
Motivation: 攻击者和渗透测试者常使用现成的C2软件（如Cobalt Strike）伪装网络流量，增加了检测难度。现有方法难以有效识别加密的C2流量。

Method: 利用网络流量元数据，通过机器学习方法自适应调整模型，优化检测性能。

Result: 该方法在性能上优于或等同于现有技术，同时更易于在生产环境中部署和解释。

Conclusion: 该方法为检测C2活动提供了高效且实用的解决方案，适用于实际安全运营环境。
...
<details>
  <summary>Details</summary>
Motivation: 尽管cookie横幅广泛使用，但其合规性未得到充分保障，用户拒绝后仍可能被放置cookie，侵犯用户隐私。

Method: 提出ConsentChk系统，通过形式化模型检测和分类cookie同意违规行为，分析全球8个英语地区的1,793个网站。

Result: 发现cookie行为和违规率因地区而异，CMP和开发者可能基于对隐私法律的错误解读配置横幅。

Conclusion: cookie横幅实施不一致，需改进合规性以确保用户隐私。
...
<details>
  <summary>Details</summary>
Motivation: KV Cache的高内存需求限制了LLM在资源受限平台上的部署，现有方法无法动态优化精度分配。

Method: KVmix利用梯度重要性分析分配层特定比特宽度，动态优化长上下文任务中的KV精度，并提供高效低比特量化和CUDA内核。

Result: 在Llama和Mistral等LLM上，KVmix实现了近无损推理性能，内存压缩4.9倍，推理速度提升5.3倍。

Conclusion: KVmix通过动态混合精度量化，有效平衡了内存、精度和吞吐量，为LLM部署提供了高效解决方案。
...
Result: 成功将1000多万难民观测数据分配到网格单元，识别出局部流离失所模式。

Conclusion: 高分辨率数据集为深入研究流离失所驱动因素提供了基础。
...
### [21] [Bi-level Unbalanced Optimal Transport for Partial Domain Adaptation](https://arxiv.org/abs/2506.08020)
...
<details>
  <summary>Details</summary>
Motivation: 部分域适应问题需要对齐跨域样本并识别异常类，现有加权框架仅关注样本级关系，忽略了聚类结构且对预测误差敏感。

Method: 提出BUOT模型，结合样本级和类级传输，利用标签感知传输成本确保局部结构，并提高计算效率。

Result: 在基准数据集上的实验验证了BUOT的竞争力。

Conclusion: BUOT通过双层次传输框架有效解决了部分域适应问题，提升了异常类识别和跨域对齐的准确性。
...
Method: 集成POD降维与LLM微调，设计流体动力学文本模板提升预测性能。

Result: 在少样本学习中优于传统Transformer模型，预测时间从小时级降至秒级，保持90%以上准确率。

Conclusion: 为快速流体动力学预测开辟新方向，适用于气动优化、流动控制等工程领域。
...
<details>
  <summary>Details</summary>
Motivation: 现有偏好优化方法未能有效抑制大型语言模型（LLM）的内部偏见，且依赖离线数据，无法适应动态分布变化。MBPO旨在解决这些问题。

Method: MBPO通过对抗性扰动生成硬负样本，并结合在线验证奖励，利用GRPO方法训练模型。

Result: 实验表明，MBPO能显著提升LMM在视觉语言任务中的性能，并减少幻觉现象。

Conclusion: MBPO为解决LMM模态不平衡问题提供了有效方案，并展示了其在任务适应和泛化能力上的优势。
...
<details>
  <summary>Details</summary>
Motivation: 研究MX格式在预训练中的数值稳定性问题，特别是其舍入模式可能导致模型发散。

Method: 提出了一种改进的舍入模式（round-to-infinity），用于计算MXFP8格式的缩放因子。

Result: 改进后的方法成功地在15T tokens上预训练了一个8B参数的LLM。

Conclusion: 改进的舍入模式解决了MX格式在预训练中的发散问题，证明了其有效性。
...
Method: 使用细粒度和粗粒度空间图（基于H3六边形空间索引）构建模型，结合多模态数据（如文本嵌入）和多种GNN架构（如GCN、GAT、DSTGCN）。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在块状缺失数据场景中特征提取效果差，静态图结构限制了模型对非平稳交通数据的适应性。

Method: 提出混合专家框架捕获时空特征及其权重，设计低秩引导采样图注意力机制生成动态图。

Result: 在四个交通数据集上实验表明，STAMImputer显著优于现有方法。

Conclusion: STAMImputer有效解决了交通数据填补中的块状缺失和非平稳性问题。
...
<details>
  <summary>Details</summary>
Motivation: 减少监督微调的计算成本，探索在有限资源和部分数据集访问下的高效模型部署。

Method: 利用上下文学习（ICL）技术，在推理时模拟监督微调的行为，理论分析不同任务的数据集规模需求。

Result: 在文本生成和线性分类任务中，推导出近似微调行为所需的最小数据集规模，并扩展到实际有限上下文场景。

Conclusion: 为大型语言模型的高效部署提供理论基础，实际技术（如检索增强生成）可弥合理想到现实的差距。
...
<details>
  <summary>Details</summary>
Motivation: 线性标量化在多目标强化学习中无法捕捉公平性目标（如Nash社会福利或最大最小公平），而现有方法在离线设置中缺乏统一解决方案。

Method: FairDICE利用分布校正估计，联合优化福利最大化和分布正则化，实现稳定高效的离线学习。
...
Method: Lite-RVFL通过指数加权新样本的目标函数，强调近期数据并实现及时适应，理论分析验证了其可行性。

Result: 在真实世界安全评估任务中验证了Lite-RVFL的高效性、适应漂移的有效性及捕捉时间模式的能力。

Conclusion: Lite-RVFL是一种无需漂移检测和重新训练的轻量级解决方案，适用于实时应用。
...
<details>
  <summary>Details</summary>
Motivation: 解决机器学习中数据增长带来的标注和训练效率问题，避免传统方法的数据冗余和主动学习的复杂性。

Method: 利用任务特定模型和开源模型，选择性标注并整合在线和网络数据，实现高效数据集优化。

Result: 在ImageNet-1K等数据集上，标注和训练成本降低32%，且无需性能损失；结合半监督学习可进一步降低标注比例至50%。

Conclusion: Info-Coevolution是一种高效、无偏的数据与模型协同进化框架，显著提升数据集构建和训练效率。
...
TLDR: 论文比较了多种预训练时间序列模型与传统方法在电力价格预测中的表现，发现传统模型MSTL表现最佳。


<details>
  <summary>Details</summary>
Motivation: 电力价格预测对电力交易决策至关重要，但现有生成式AI和时间序列基础模型在电力价格预测中的效果尚不明确。

Method: 使用德国、法国等国家的2024年日前拍卖电价数据，对比了多种预训练模型与传统统计和机器学习方法。

Result: Chronos-Bolt和Time-MoE表现最佳，但传统MSTL模型在一致性和统计性能上优于所有预训练模型。

Conclusion: 尽管预训练模型表现良好，但传统MSTL模型在电力价格预测中仍是最优选择。
...
Result: 实验表明，Bingo在多个推理基准测试中优于其他方法，实现了准确性与效率的良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 传统算法（如k-NN）在监督微调（SFT）后的特征提取中表现优于神经网络本身，但因其不可微分性难以直接整合到神经网络中。

Method: 提出NONA回归层，利用注意力机制和学习掩码模拟k-NN回归，使其可微分并适用于神经网络训练。

Result: 在多个非结构化数据集上，NONA的性能优于传统的密集层预测和k-NN方法。

Conclusion: NONA为将传统算法整合到神经网络提供了一种可行方案，显著提升了回归任务的性能。
...
<details>
  <summary>Details</summary>
Motivation: 加速科学发现需要高质量数据，但现有数据稀缺，AutoSDT旨在解决这一问题。

Method: 利用LLMs的编码能力和参数知识，自动搜索、筛选和合成任务及代码解决方案，构建AutoSDT-5K数据集。

Result: AutoSDT-5K包含5,404个任务，93%有效，92.2%代码正确。AutoSDT-Coder模型在基准测试中表现优异，接近GPT-4o水平。

Conclusion: AutoSDT有效解决了数据稀缺问题，提升了AI在科学发现中的表现。
...
Method: 将公平谱聚类问题转化为凸差函数框架，采用变量增强策略和优化的交替方向乘子法（ADMM）求解子问题。

Result: 数值实验表明，该方法在合成和真实数据集上均有效，计算时间显著优于现有方法，尤其在大规模问题上。
...
<details>
  <summary>Details</summary>
Motivation: 在敏感领域的关系和网络结构数据学习中，保护个体隐私至关重要，但直接应用DP-SGD存在高敏感性和耦合采样问题。

Method: 提出了一个框架，包括严格的敏感性分析、基于实体出现频率的自适应梯度裁剪方案，并扩展了隐私放大分析到耦合采样子类。

Result: 实验表明，该方法在文本属性网络关系数据上实现了良好的隐私-效用权衡。

Conclusion: 该工作为关系数据提供了具有可证明隐私保证的DP-SGD变体，解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法在处理异质材料时难以捕捉其复杂性质，需要一种更灵活且物理约束的方法。

Method: 结合傅里叶特征的神经网络近似应变场，利用NODE框架发现本构方程，并通过超网络处理异质性。

Result: 数值实验表明该方法在噪声和实验数据中均表现稳健，适用于多种异质材料。

Conclusion: 该方法为异质材料力学性质识别提供了一种通用且高效的替代方案。
...
<details>
  <summary>Details</summary>
Motivation: 确保LLM符合数据法规和伦理实践，需要有效遗忘训练中获得的知识和能力，但现有方法存在性能下降问题。

Method: 采用双层优化框架，下层最小化遗忘损失，上层保持模型性能，提出BLUR算法。

Result: BLUR在多种任务、模型和指标上优于现有方法。

Conclusion: BLUR通过分层优化解决了遗忘与保留的权衡问题，具有理论和实践优势。
...
TLDR: 本文通过实证研究证明，即使在现实的联邦学习环境中，客户端数据仍可被有效重构，并提出FedLeak方法解决梯度匹配问题。


<details>
  <summary>Details</summary>
Motivation: 探讨联邦学习中的隐私风险，尤其是梯度泄漏攻击（GLAs）在实际环境中的有效性，以填补现有研究的空白。

Method: 提出FedLeak方法，包含部分梯度匹配和梯度正则化两种新技术，并设计了一个基于文献和行业实践的实际评估协议。

Result: 在现实联邦学习环境中，FedLeak仍能实现高保真数据重构，揭示了FL系统的重大漏洞。
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习在保护用户隐私的同时，由于本地数据的潜在噪声（如测量误差或人为错误）可能导致模型性能下降，因此需要一种能够估计数据真实状态并量化噪声的方法。

Method: 在联邦学习框架中引入随机神经网络作为本地模型，利用其估计数据真实状态和量化噪声的能力。

Result: 数值实验表明，该方法在处理非独立同分布数据时表现优异。

Conclusion: 提出的Federated stochastic neural networks方法有效解决了联邦学习中的噪声问题，提升了模型性能。
...
Result: 实验表明，该方法优于仅使用本地数据训练的决策树和基准算法。
...
<details>
  <summary>Details</summary>
Motivation: 传统DP机制在随机梯度下降（SGD）学习算法的每一步注入独立噪声以保护训练数据隐私，但研究表明引入（反）相关噪声可通过抵消部分先前噪声显著优化隐私-效用权衡。

Method: 采用相关噪声机制，如矩阵机制、因子分解机制和DP-FTRL（应用于学习算法时），通过精心设计噪声相关性来改进隐私保护效果。

Result: 相关噪声机制在实践中具有重要影响，已在全球范围内实现工业级部署。

Conclusion: 相关噪声机制为差分隐私提供了更优的隐私-效用平衡，并在实际应用中展现出显著潜力。
...
Method: 构建大量过程模拟数据集，基于U-Net架构训练ML模型，通过超参数调优学习应力分布结构。
...
Result: 发现解释集成模型的复杂性受多种因素影响，例如基础模型数量和小规模决策树集成可高效解释，而线性模型集成则难以解释。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于Transformer的算子模型在高分辨率多尺度域中面临注意力计算成本高和与离散化耦合的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 研究自动驾驶领域中运动预测与规划模型的性能扩展规律，以优化模型开发与性能提升。

Method: 使用50万小时驾驶数据集，分析模型性能与计算预算的关系，并研究模型参数与数据规模的优化比例。

Result: 模型性能随计算预算呈幂律增长，训练时模型规模需以数据规模的1.5倍速度扩展；推理时小模型通过采样与聚类可媲美大模型。

Conclusion: 优化训练与推理的扩展策略是提升自动驾驶模型性能的关键，同时利用其他车辆的驾驶数据可缓解机器人数据稀缺问题。
...
Result: 框架能识别需改进变量、系统性检测潜在错误并确认数据集适用性。
...
### [51] [Dealing with the Evil Twins: Improving Random Augmentation by Addressing Catastrophic Forgetting of Diverse Augmentations](https://arxiv.org/abs/2506.08240)
*Dongkyu Cho,Rumi Chunara*

Main category: cs.LG

TLDR: 本文探讨了随机数据增强的局限性，并提出了一种简单方法来解决其导致的特征扭曲问题，从而提升泛化性能。


<details>
  <summary>Details</summary>
Motivation: 随机数据增强成本低但效果有限，而目标数据增强虽效果好但成本高。本文旨在改进随机增强的泛化能力。

Method: 通过分析随机增强的随机性导致特征扭曲的问题，提出了一种解决遗忘现象的简单方法。
...
### [52] [Temporalizing Confidence: Evaluation of Chain-of-Thought Reasoning with Signal Temporal Logic](https://arxiv.org/abs/2506.08243)
*Zhenjiang Mao,Artem Bisliouk,Rohith Reddy Nama,Ivan Ruchkin*

Main category: cs.LG

TLDR: 提出了一种基于信号时序逻辑（STL）的结构化框架，用于评估大语言模型（LLMs）在数学推理任务中的逐步置信度，并通过不确定性重塑策略提升校准效果。


<details>
  <summary>Details</summary>
Motivation: LLMs在数学推理任务中表现出高置信度但错误的输出，可能对缺乏专业知识的用户造成风险。

Method: 使用STL定义时序约束，计算稳健性评分作为置信度估计，并引入不确定性重塑策略。

Result: 实验表明，该方法显著改善了校准指标，提供了比传统方法更可靠的不确定性估计。

Conclusion: 提出的框架有效提升了LLMs在数学推理任务中的置信度校准和可靠性。
...
### [53] [Parameter-free approximate equivariance for tasks with finite group symmetry](https://arxiv.org/abs/2506.08244)
*Riccardo Ali,Pietro Liò,Jamie Vicary*

Main category: cs.LG

TLDR: 提出一种零参数的近似等变方法，通过在损失函数中添加额外项，在潜在空间中实现有限群的近似等变性，实验表明其性能优于现有方法且参数更少。
...
Method: 在潜在空间中通过损失函数的额外项实现近似等变性，并让网络学习群的表示。

Result: 实验表明，该方法在多个数据集上性能与现有方法相当或更好，且参数更少。

Conclusion: 提出的零参数方法简单高效，适用于多种任务，性能优于现有方法。
...
### [54] [SHIELD: Secure Hypernetworks for Incremental Expansion Learning Defense](https://arxiv.org/abs/2506.08255)
*Patryk Krukowski,Łukasz Gorczyca,Piotr Helm,Kamil Książek,Przemysław Spurek*

Main category: cs.LG

TLDR: SHIELD提出了一种结合超网络和区间算术的新方法，同时解决灾难性遗忘和对抗攻击问题。
...
Result: SHIELD能够动态生成子任务网络，同时保证输入数据区间内的安全性，提升网络适应性。

Conclusion: SHIELD在持续学习中兼顾安全性和适应性，解决了现有模型的局限性。
...
### [55] [Reinforcement Learning from Human Feedback with High-Confidence Safety Constraints](https://arxiv.org/abs/2506.08266)
*Yaswanth Chittepu,Blossom Metevier,Will Schwarzer,Austin Hoag,Scott Niekum,Philip S. Thomas*

Main category: cs.LG

TLDR: HC-RLHF是一种高置信度安全强化学习方法，通过解耦人类偏好为有用性和无害性，提供安全保证并最大化有用性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在语言模型对齐中将安全性与有用性对立，导致敏感领域出现不可接受的响应。

Method: HC-RLHF训练奖励模型和成本模型，分两步优化：在悲观成本约束下优化奖励，并通过安全测试验证性能。

Result: 实验表明，HC-RLHF能以高概率生成安全模型，并在无害性和有用性上优于现有方法。

Conclusion: HC-RLHF提供理论安全保证，并在实践中提升模型性能。
...
### [56] [Sparse Interpretable Deep Learning with LIES Networks for Symbolic Regression](https://arxiv.org/abs/2506.08267)
*Mansooreh Montazerin,Majd Al Aawar,Antonio Ortega,Ajitesh Srivastava*

Main category: cs.LG

TLDR: LIES框架通过固定神经网络架构和优化策略，实现了高效且可解释的符号回归，优于现有方法。
...
Method: 采用LIES神经网络架构，结合过采样策略和定制损失函数，训练后通过剪枝策略简化表达式。
...
### [57] [SWAT-NN: Simultaneous Weights and Architecture Training for Neural Networks in a Latent Space](https://arxiv.org/abs/2506.08270)
*Zitong Huang,Mansooreh Montazerin,Ajitesh Srivastava*

Main category: cs.LG

TLDR: 提出一种同时优化神经网络结构和权重的新方法，通过连续潜在空间嵌入架构和参数信息，实现高效模型发现。
...
Method: 训练通用多尺度自编码器，将架构和参数信息嵌入连续潜在空间，通过梯度下降联合优化结构和权重。

Result: 在合成回归任务中，方法成功发现性能优越的稀疏紧凑神经网络。
...
### [58] [Universal Differential Equations for Scientific Machine Learning of Node-Wise Battery Dynamics in Smart Grids](https://arxiv.org/abs/2506.08272)
*Tarushri N. S.*

Main category: cs.LG

TLDR: UDEs结合神经网络与物理微分方程，用于智能电网中电池动态建模，通过神经残差学习未建模动态，实验显示其准确性高且稳定。


<details>
  <summary>Details</summary>
Motivation: 智能电网中电池动态建模因太阳能输入随机性和家庭负载变化而复杂，传统方法泛化能力不足。

Method: 提出UDE框架，将神经残差嵌入物理电池ODE中，利用合成数据模拟电池动态。

Result: UDE模型与真实电池轨迹高度吻合，收敛平滑，长期预测稳定。

Conclusion: UDE方法适用于去中心化能源网络的电池建模，对实时控制和优化有广泛意义。
...
### [59] [The Impact of Feature Scaling In Machine Learning: Effects on Regression and Classification Tasks](https://arxiv.org/abs/2506.08274)
...
TLDR: 该研究系统评估了12种特征缩放技术对14种机器学习算法和16个数据集的影响，发现集成方法对缩放不敏感，而其他模型（如逻辑回归、SVM等）性能受缩放选择影响显著。
...
Result: 集成方法（如随机森林、XGBoost等）对缩放不敏感，而逻辑回归、SVM等模型性能受缩放选择影响显著。

Conclusion: 研究为实践者提供了模型特定的缩放技术选择建议，并公开了所有代码和结果以确保透明性和可重复性。
...
### [60] [From Debate to Equilibrium: Belief-Driven Multi-Agent LLM Reasoning via Bayesian Nash Equilibrium](https://arxiv.org/abs/2506.08292)
*Xie Yi,Zhanke Zhou,Chentao Cao,Qiyu Niu,Tongliang Liu,Bo Han*

Main category: cs.LG

TLDR: ECON提出了一种基于贝叶斯纳什均衡的多LLM协调框架，通过分层强化学习实现高效推理，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 解决多智能体框架在LLM中计算成本高且缺乏收敛保证的问题。

Method: 将多LLM协调建模为不完全信息博弈，采用贝叶斯纳什均衡策略，结合分层强化学习。

Result: ECON在六个基准测试中平均性能提升11.2%，并具有可扩展性。

Conclusion: ECON为大规模多LLM集成提供了高效且可扩展的解决方案。
...
### [61] [From Passive to Active Reasoning: Can Large Language Models Ask the Right Questions under Incomplete Information?](https://arxiv.org/abs/2506.08295)
*Zhanke Zhou,Xiao Feng,Zhaocheng Zhu,Jiangchao Yao,Sanmi Koyejo,Bo Han*

Main category: cs.LG

TLDR: AR-Bench是一个新的基准测试，旨在评估大型语言模型（LLM）的主动推理能力，发现当前模型在主动推理方面表现较差。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要评估被动推理能力，而主动推理（需要与外部系统交互获取信息）缺乏系统性研究。

Method: 设计了AR-Bench，包含三类任务（侦探案例、情境谜题和猜数字），模拟真实场景并测试常识、逻辑和符号推理能力。

Result: 当代LLM在主动推理中表现不佳，难以获取或利用必要信息，且现有改进策略效果有限。

Conclusion: 需开发新方法（如交互式学习、实时反馈）提升主动推理能力，AR-Bench已公开。
...
### [62] [H$^2$GFM: Towards unifying Homogeneity and Heterogeneity on Text-Attributed Graphs](https://arxiv.org/abs/2506.08298)
*Trung-Kien Nguyen,Heng Ping,Shixuan Li,Peiyu Zhang,Nikos Kanakaris,Nicholas Kotov,Paul Bogdan*

Main category: cs.LG

TLDR: 论文提出了一种名为H$^2$GFM的框架，旨在统一处理同质和异质文本属性图（TAGs），通过上下文编码和新型图变换器（CGT）提升图基础模型（GFM）的能力。
...
Method: 提出H$^2$GFM框架，利用统一的文本空间投影多样元关系，并采用上下文编码捕捉空间和高阶语义关系。引入上下文自适应图变换器（CGT）和专家混合机制，以处理图类型的结构异质性。

Result: 在多种HoTAGs和HeTAGs及学习场景下的实验验证了模型的有效性。

Conclusion: H$^2$GFM成功扩展了GFM的能力，为处理异质图提供了新思路。
...
### [63] [Learnable Spatial-Temporal Positional Encoding for Link Prediction](https://arxiv.org/abs/2506.08309)
*Katherine Tieu,Dongqi Fu,Zihao Li,Ross Maciejewski,Jingrui He*

Main category: cs.LG

TLDR: 论文提出了一种可学习的时空位置编码方法L-STEP，解决了现有位置编码在适应性、动态性和效率上的不足，并在多个数据集和算法中验证了其优越性。


<details>
  <summary>Details</summary>
Motivation: 现有图深度学习中的位置编码方法多为预定义且固定，无法适应复杂属性图或动态拓扑变化，且计算效率低。

Method: 提出L-STEP方法，通过可学习的时空位置编码，结合MLP的简洁性，替代复杂的注意力机制。

Result: 在13个经典数据集和10种算法中验证了L-STEP的优越性，并在大规模TGB基准测试中表现领先。

Conclusion: L-STEP是一种高效且适应性强的时空位置编码方法，显著提升了图深度学习模型的性能。
...
### [64] [Why Masking Diffusion Works: Condition on the Jump Schedule for Improved Discrete Diffusion](https://arxiv.org/abs/2506.08316)
*Alan N. Amin,Nate Gruver,Andrew Gordon Wilson*

Main category: cs.LG

TLDR: 离散扩散模型通过逐步去噪生成高质量样本，但实践中表现最佳的是掩码扩散模型。研究发现，掩码扩散利用了离散马尔可夫过程的跳跃特性，通过固定跳跃时间分布提升性能。提出的SCUD模型将这一特性推广到其他离散扩散模型，并在图像、文本和蛋白质数据上表现优于掩码扩散。


<details>
  <summary>Details</summary>
Motivation: 研究离散扩散模型的性能差异，尤其是掩码扩散模型的优越性，并提出改进方法。

Method: 提出SCUD模型，将已知跳跃时间分布融入离散扩散模型，结合不同领域的归纳偏置。

Result: SCUD模型在图像、文本和蛋白质数据上表现优于传统掩码扩散模型。

Conclusion: 通过利用离散马尔可夫过程的跳跃特性，SCUD模型为离散扩散提供了更优的解决方案。
...
### [65] [Graph Prompting for Graph Learning Models: Recent Advances and Future Directions](https://arxiv.org/abs/2506.08326)
*Xingbo Fu,Zehong Wang,Zihan Chen,Jiazheng Li,Yaochen Zhu,Zhenyu Lei,Cong Shen,Yanfang Ye,Chuxu Zhang,Jundong Li*

Main category: cs.LG

TLDR: 本文系统综述了图提示学习的最新进展，包括预训练方法、主流技术和实际应用，并讨论了未来研究方向。
...
Method: 通过自监督预训练图学习模型，随后设计可学习的提示（prompts）进行任务适应，保持预训练模型不变。

Result: 总结了图提示学习的代表性预训练方法、主流技术及其在不同领域的实际应用。

Conclusion: 图提示学习是一个有前景的方向，但仍面临挑战，未来需进一步研究。
...
### [66] [A Simple Analysis of Discretization Error in Diffusion Models](https://arxiv.org/abs/2506.08337)
*Juhyeok Choi,Chenglin Fan*

Main category: cs.LG

TLDR: 本文提出了一种简化的理论框架，用于分析DDPM中VP-SDE的Euler--Maruyama离散化误差，并证明了离散噪声可替代高斯噪声而不影响收敛性。


<details>
  <summary>Details</summary>
Motivation: 现有对扩散模型离散化误差的分析依赖复杂概率工具，本文旨在简化分析并提供实用见解。

Method: 利用Grönwall不等式推导收敛速率，并验证离散噪声（如Rademacher或均匀噪声）的可行性。

Result: 实验验证了误差按预测比例缩放，离散噪声与高斯噪声质量相当，错误噪声缩放会降低性能。
...
### [67] [Dynamical System Optimization](https://arxiv.org/abs/2506.08340)
*Emo Todorov*

Main category: cs.LG

TLDR: 该论文提出了一种基于参数化策略的优化框架，将控制权转移给策略，从而在自治动态系统层面简化算法，避免直接使用动态规划和强化学习的复杂方法。


<details>
  <summary>Details</summary>
Motivation: 动机是避免传统动态规划和强化学习的复杂性，通过将控制权完全交给参数化策略，直接在自治系统层面进行优化。

Method: 方法是通过参数化策略将控制权转移给策略，形成自治动态系统，并在此层面设计优化算法，避免直接使用动态规划和强化学习。

Result: 结果表明，这些算法能够计算与策略梯度、Hessian矩阵、自然梯度等相同的量，并适用于行为克隆、机制设计、系统识别等多种任务。

Conclusion: 结论是该框架不仅简化了优化过程，还扩展了应用范围，包括生成式AI模型的调优，且概念上更接近自治系统而非强化学习。
...
### [68] [An Adaptive Method Stabilizing Activations for Enhanced Generalization](https://arxiv.org/abs/2506.08353)
*Hyunseok Seung,Jaewoo Lee,Hyunsuk Ko*

Main category: cs.LG

TLDR: AdaAct是一种新颖的优化算法，通过根据激活方差调整学习率，提升神经元输出的稳定性，从而改善泛化能力。
...
Method: AdaAct在训练过程中动态调整学习率，结合激活方差信息，优化神经元输出稳定性。

Result: 在CIFAR和ImageNet等基准测试中，AdaAct表现优异，同时平衡了Adam的收敛速度和SGD的泛化能力。

Conclusion: AdaAct是一种高效的优化算法，兼具快速收敛和强泛化能力，代码已开源。
...
### [69] [NysAct: A Scalable Preconditioned Gradient Descent using Nystrom Approximation](https://arxiv.org/abs/2506.08360)
*Hyunseok Seung,Jaewoo Lee,Hyunsuk Ko*

Main category: cs.LG

TLDR: NysAct是一种可扩展的一阶梯度预处理方法，平衡了一阶和二阶优化方法的优点，通过近似激活协方差矩阵显著降低计算和内存成本。


<details>
  <summary>Details</summary>
Motivation: 自适应梯度方法计算高效但泛化性能差，二阶方法泛化好但计算成本高，NysAct旨在平衡两者。

Method: 利用特征值偏移的Nystrom方法近似激活协方差矩阵作为预处理矩阵。

Result: 实验表明NysAct在测试准确率上优于一阶和二阶方法，同时计算资源需求更低。

Conclusion: NysAct在保持高效计算的同时提升了泛化性能，是一种实用的优化方法。
...
### [70] [AlphaFold Database Debiasing for Robust Inverse Folding](https://arxiv.org/abs/2506.08365)
*Cheng Tan,Zhenxiao Cao,Zhangyang Gao,Siyuan Li,Yufei Huang,Stan Z. Li*

Main category: cs.LG

TLDR: AlphaFold数据库（AFDB）在蛋白质结构预测中表现出高精度，但其用于训练对原子几何敏感的深度学习模型时存在系统偏差。作者提出了一种去偏自编码器（DeSAE）来纠正这种偏差，显著提升了逆折叠等任务的性能。


<details>
  <summary>Details</summary>
Motivation: AFDB的结构虽然高精度，但存在系统性几何偏差，与实验结构（PDB）的多样性不符，影响了逆折叠等任务的泛化能力。

Method: 提出DeSAE模型，通过从故意损坏的主链几何中重建自然构象，学习更鲁棒的结构流形。

Result: DeSAE显著改善了AFDB结构的去偏效果，提升了逆折叠任务在多个基准上的性能。

Conclusion: 研究揭示了预测结构中的系统性偏差对任务性能的影响，并提出了一种有效的去偏框架。
...
### [71] [Reinforce LLM Reasoning through Multi-Agent Reflection](https://arxiv.org/abs/2506.08379)
*Yurun Yuan,Tengyang Xie*

Main category: cs.LG

TLDR: 论文提出DPSDP算法，通过强化学习训练LLM系统迭代优化答案，提升推理能力。
...
Method: 将多轮优化建模为马尔可夫决策过程，引入DPSDP算法，基于自生成数据进行直接偏好学习。

Result: 在MATH 500基准上，五步优化将准确率从58.2%提升至63.2%。

Conclusion: DPSDP通过多智能体协作和分布外泛化显著提升LLM推理性能。
...
### [72] [Reinforcement Learning Teachers of Test Time Scaling](https://arxiv.org/abs/2506.08388)
*Edoardo Cetin,Tianyu Zhao,Yujin Tang*

Main category: cs.LG

TLDR: 提出了一种新的框架Reinforcement-Learned Teachers (RLTs)，通过密集奖励训练教师模型，专注于生成适合学生模型蒸馏的详细解释，避免了传统强化学习的探索挑战。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习训练推理语言模型需要模型在初始化时具备一定的探索能力，而RLTs框架旨在通过教师模型生成详细解释，直接优化学生模型的学习效果。

Method: RLTs框架通过输入问题和解决方案，要求教师模型生成详细的解释，并通过学生模型对这些解释的理解程度来提供密集奖励。

Result: 实验表明，7B规模的RLT模型在竞赛和研究生级任务上的表现优于现有蒸馏和冷启动方法，且能有效训练更大的学生模型并适用于零样本任务。

Conclusion: RLTs框架显著提升了推理语言模型的效率和可重用性，为强化学习推理提供了新的可能性。
...
### [73] [Spatiotemporal deep learning models for detection of rapid intensification in cyclones](https://arxiv.org/abs/2506.08397)
*Vamshika Sutar,Amandeep Singh,Rohitash Chandra*

Main category: cs.LG

TLDR: 本文提出了一种结合深度学习和数据增强的方法，用于检测气旋快速增强事件，解决了数据集类别不平衡问题。
...
### [74] [FUSE: Measure-Theoretic Compact Fuzzy Set Representation for Taxonomy Expansion](https://arxiv.org/abs/2506.08409)
*Fred Xu,Song Jiang,Zijie Huang,Xiao Luo,Shichang Zhang,Adrian Chen,Yizhou Sun*

Main category: cs.LG

TLDR: 论文提出了一种基于模糊集的集合表示学习方法FUSE，用于分类学扩展任务，相比现有方法提升高达23%。
...
Method: 提出FUSE框架，通过体积近似模糊集实现集合表示学习，满足所有集合操作，同时高效且信息保留完整。
...
### [75] [Learning to Hear Broken Motors: Signature-Guided Data Augmentation for Induction-Motor Diagnostics](https://arxiv.org/abs/2506.08412)
*Saraa Ali,Aleksandr Khizhik,Stepan Svirin,Artem Ryzhikov,Denis Derkach*

Main category: cs.LG

TLDR: 论文提出了一种结合机器学习与无监督异常生成方法的新框架SGDA，用于三相电机的智能诊断，显著提升了诊断性能。
...
Method: 提出SGDA框架，结合无监督异常生成与电机物理模型，在健康电流信号的频域中合成物理合理的故障。

Result: 该方法结合监督ML与无监督签名分析，实现了更高的诊断准确性和可靠性。

Conclusion: SGDA为电机诊断领域提供了高效、鲁棒的解决方案，具有广泛工业应用潜力。
...
### [76] [Improved Scaling Laws in Linear Regression via Data Reuse](https://arxiv.org/abs/2506.08415)
*Licong Lin,Jingfeng Wu,Peter L. Bartlett*

Main category: cs.LG

TLDR: 论文研究了数据重用如何在线性回归中改进现有的缩放定律，通过多轮随机梯度下降（SGD）在数据受限情况下实现更优的测试误差。


<details>
  <summary>Details</summary>
Motivation: 探索在数据有限的情况下，如何通过数据重用改进模型性能，避免因数据不足导致的缩放不可持续问题。

Method: 使用多轮随机梯度下降（multi-pass SGD）在具有特征草图的数据上训练M维线性模型，假设数据协方差和真实参数具有特定的幂律谱。

Result: 多轮SGD实现了测试误差的改进缩放定律，优于单轮SGD，数值模拟验证了理论结果。

Conclusion: 数据重用可以在数据受限的情况下显著改进模型性能，为实际应用提供了理论支持。
...
### [77] [Offline RL with Smooth OOD Generalization in Convex Hull and its Neighborhood](https://arxiv.org/abs/2506.08417)
*Qingmao Yao,Zhichao Lei,Tianyuan Chen,Ziyue Yuan,Xuefan Chen,Jianxiang Liu,Faguo Wu,Xiao Zhang*

Main category: cs.LG

TLDR: 论文提出了一种新方法SQOG，通过平滑贝尔曼算子（SBO）在凸包及其邻域（CHN）内提升Q函数泛化能力，解决了离线强化学习中Q值高估问题。


<details>
  <summary>Details</summary>
Motivation: 离线强化学习中，分布偏移导致Q值对分布外（OOD）动作的高估，现有方法因约束过度而泛化能力受限。

Method: 提出平滑贝尔曼算子（SBO），在CHN内通过平滑OOD Q值与邻近样本Q值来更新Q值，理论证明其能近似真实Q值。

Result: SQOG算法在D4RL基准测试中表现优于现有方法，计算效率更高。

Conclusion: SQOG通过提升Q函数泛化能力，有效解决了离线强化学习中的Q值高估问题。
...
### [78] [Online Learning-guided Learning Rate Adaptation via Gradient Alignment](https://arxiv.org/abs/2506.08419)
*Ruichen Jiang,Ali Kavis,Aryan Mokhtari*

Main category: cs.LG

TLDR: GALA框架通过动态调整学习率，基于梯度对齐和局部曲率估计，减少了对学习率调优的依赖。


<details>
  <summary>Details</summary>
Motivation: 大规模深度学习模型中，学习率的调优通常需要大量网格搜索，耗时且低效。

Method: 提出GALA框架，通过跟踪连续梯度的对齐和局部曲率估计，动态调整学习率，并将其建模为一维在线学习问题。

Result: 实验表明，结合GALA的SGD和Adam优化器在不同初始学习率下表现稳健，且无需调优。

Conclusion: GALA提供了一种自适应学习率调整方法，显著减少了调优需求，提升了优化器的鲁棒性。
...
TLDR: HASFL框架通过自适应控制批量大小和模型分割，解决了分片联邦学习中的设备异构性问题，提升了训练效率和收敛性。


<details>
  <summary>Details</summary>
Motivation: 现有分片联邦学习方法因边缘设备能力异构性导致严重的拖尾效应，亟需解决方案。

Method: 提出HASFL框架，基于收敛边界分析，自适应调整批量大小和模型分割，平衡通信-计算延迟与训练收敛。

Result: 实验验证HASFL在多种数据集上优于现有基准方法。

Conclusion: HASFL有效解决了异构边缘网络中的训练效率问题，具有实际应用潜力。
...
### [80] [Learning to Lead: Incentivizing Strategic Agents in the Dark](https://arxiv.org/abs/2506.08438)
*Yuchen Wu,Xinyi Zhong,Zhuoran Yang*

Main category: cs.LG

TLDR: 论文研究了一种在线学习的广义委托-代理模型，提出了首个样本高效的算法，通过延迟机制、奖励角度估计框架和悲观-乐观LinUCB算法，实现了接近最优的遗憾界。


<details>
  <summary>Details</summary>
Motivation: 解决委托人在与具有私有类型和奖励的战略代理人重复交互时，如何学习最优协调机制的问题。

Method: 结合延迟机制、奖励角度估计框架和悲观-乐观LinUCB算法，激励代理人近似短视行为并高效探索。

Result: 提出了首个样本高效的算法，实现了接近最优的$\tilde{O}(\sqrt{T})$遗憾界。

Conclusion: 该研究为涉及私有类型和战略代理人的博弈论场景设计鲁棒在线学习算法开辟了新途径。
...
### [81] [Time-Aware World Model for Adaptive Prediction and Control](https://arxiv.org/abs/2506.08441)
*Anh N. Nhu,Sanghyun Son,Ming Lin*

Main category: cs.LG

TLDR: TAWM是一种基于时间感知的世界模型，通过动态调整时间步长提升任务性能和数据效率。


<details>
  <summary>Details</summary>
Motivation: 传统模型固定时间步长的采样方式无法适应不同频率的任务动态，限制了性能和数据效率。

Method: TAWM通过条件化时间步长Δt，并在多样化的Δt值上训练，学习高低频任务动态。

Result: 实验表明，TAWM在多种控制任务中优于传统模型，尤其是在不同观测率下表现更优。

Conclusion: TAWM的时间感知设计显著提升了模型性能和数据效率，适用于多样化控制问题。
...
### [82] [MOBODY: Model Based Off-Dynamics Offline Reinforcement Learning](https://arxiv.org/abs/2506.08460)
*Yihong Guo,Yu Yang,Pan Xu,Anqi Liu*

Main category: cs.LG

TLDR: MOBODY是一种基于模型的离线强化学习算法，通过生成目标域的新合成数据来解决源域和目标域动态不匹配的问题，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有离线强化学习方法在源域和目标域动态不匹配时表现受限，无法有效探索目标域。

Method: MOBODY通过表示学习共享潜在状态和动态，结合行为克隆损失和Q值加权策略，生成目标域数据增强学习。

Result: 在MuJoCo基准测试中，MOBODY显著优于现有方法，尤其在复杂场景中表现突出。

Conclusion: MOBODY通过动态学习和数据增强解决了动态不匹配问题，为离线强化学习提供了新思路。
...
### [83] [How to Provably Improve Return Conditioned Supervised Learning?](https://arxiv.org/abs/2506.08463)
*Zhishuai Liu,Yu Yang,Ruhan Wang,Pan Xu,Dongruo Zhou*

Main category: cs.LG

TLDR: Reinforced RCSL通过引入“in-distribution optimal return-to-go”机制，解决了标准RCSL缺乏stitching能力的问题，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 标准RCSL因依赖离线数据集的质量而性能受限，缺乏stitching能力。
...
Conclusion: Reinforced RCSL是一种简单且有效的改进方法，解决了RCSL的关键局限性。
...
### [84] [MAC: An Efficient Gradient Preconditioning using Mean Activation Approximated Curvature](https://arxiv.org/abs/2506.08464)
*Hyunseok Seung,Jaewoo Lee,Hyunsuk Ko*

Main category: cs.LG

TLDR: 论文提出了一种名为MAC的高效二阶优化方法，通过近似Kronecker因子降低计算负担，并在Transformer中首次应用Kronecker分解。


<details>
  <summary>Details</summary>
Motivation: KFAC等二阶优化方法虽然收敛性好，但计算成本高，因此需要更高效的近似方法。

Method: 分析KFAC中Fisher信息矩阵的Kronecker因子，提出高效近似，并首次在Transformer中应用Kronecker分解。
...
### [85] [AsFT: Anchoring Safety During LLM Fine-Tuning Within Narrow Safety Basin](https://arxiv.org/abs/2506.08473)
...
TLDR: 论文提出了一种名为AsFT的安全微调方法，通过正则化项抑制有害方向的更新，确保微调在安全范围内进行。实验表明，AsFT优于Safe LoRA，减少了有害行为并提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在微调过程中容易受到安全风险的影响，少量恶意或无害数据可能破坏模型的安全保障。

Method: 基于对齐方向的概念，提出AsFT方法，通过正则化项将微调约束在安全范围内。

Result: AsFT在多个数据集上表现优异，有害行为减少7.60%，模型性能提升3.44%。

Conclusion: AsFT通过锚定对齐方向，有效抑制有害更新，为LLMs的安全微调提供了可行方案。
...
### [86] [Thermodynamically Consistent Latent Dynamics Identification for Parametric Systems](https://arxiv.org/abs/2506.08475)
*Xiaolong He,Yeonjong Shin,Anthony Gruber,Sohyeon Jung,Kookjin Lee,Youngsoo Choi*

Main category: cs.LG

TLDR: 提出了一种基于热力学的潜在空间动力学识别框架（tLaSDI），用于参数化非线性动力系统的降阶建模，结合自编码器和参数化GENERIC形式神经网络，显著提升了计算效率和精度。


<details>
  <summary>Details</summary>
Motivation: 解决参数化非线性动力系统建模中计算效率低和热力学原理保持的问题。

Method: 结合自编码器降维与参数化GENERIC形式神经网络（pGFINNs），并采用物理信息主动学习策略优化数据采样。

Result: 在Burgers方程和1D/1V Vlasov-Poisson方程实验中，实现了最高3,528倍加速，相对误差1-3%，训练和推理成本显著降低。

Conclusion: tLaSDI框架高效且精确，揭示了系统的热力学行为，为物理空间动力学提供了新见解。
...
### [87] [Explaining, Fast and Slow: Abstraction and Refinement of Provable Explanations](https://arxiv.org/abs/2506.08505)
*Shahaf Bassan,Yizhak Yisrael Elboher,Tobias Ladner,Matthias Althoff,Guy Katz*

Main category: cs.LG

TLDR: 提出了一种基于抽象-精炼技术的高效计算神经网络预测的可证明充分解释的方法，显著提升了验证效率。
...
Method: 通过构建一个大幅简化的网络抽象原始网络，逐步精炼网络规模直至收敛，以高效计算可证明充分的解释。
...
### [88] [NeurIPS 2024 ML4CFD Competition: Results and Retrospective Analysis](https://arxiv.org/abs/2506.08516)
...
TLDR: ML4CFD竞赛通过替代建模评估机器学习在CFD中的潜力，展示了ML模型在特定条件下超越传统求解器的能力。
...
Method: 组织ML4CFD竞赛，提供OpenFOAM生成的数据集，评估预测精度、物理保真度、计算效率和泛化能力。

Result: 顶级参赛模型在综合指标上超越了OpenFOAM求解器。

Conclusion: ML替代模型在科学计算中具有潜力，未来需优化设计原则和评估框架。
...
### [89] [Leveraging chaos in the training of artificial neural networks](https://arxiv.org/abs/2506.08523)
*Pedro Jiménez-González,Miguel C. Soriano,Lucas Lacasa*

Main category: cs.LG

TLDR: 论文探讨了在超常学习率下梯度下降（GD）优化神经网络时的动态特性，发现学习率在某一区间时，GD会从纯利用型转向探索-利用平衡态，且训练时间最短。


<details>
  <summary>Details</summary>
Motivation: 研究传统GD算法在超常学习率下的动态行为，探索如何通过调整学习率加速神经网络训练。

Method: 通过分析神经网络轨迹的最大Lyapunov指数，研究GD在不同学习率下的动态特性，并在MNIST分类任务中验证。

Result: 在特定学习率区间，GD表现出探索-利用平衡态，训练时间最短，且结果适用于多种学习任务和架构。

Conclusion: 超常学习率下GD的混沌动态对神经网络训练具有建设性作用，可加速训练过程。
...
### [90] [Robust Evolutionary Multi-Objective Network Architecture Search for Reinforcement Learning (EMNAS-RL)](https://arxiv.org/abs/2506.08533)
*Nihal Acharya Adde,Alexandra Gianzina,Hanno Gottschalk,Andreas Ebert*

Main category: cs.LG

TLDR: EMNAS首次提出用于自动驾驶强化学习的多目标网络架构搜索，通过遗传算法优化网络设计，提升奖励并减少模型大小，同时采用并行化和师生方法加速搜索。实验表明其优于人工设计模型。
...
### [91] [DeepForm: Reasoning Large Language Model for Communication System Formulation](https://arxiv.org/abs/2506.08551)
*Panlong Wu,Ting Wang,Yifei Zhong,Haoqi Zhang,Zitong Wang,Fangxin Wang*

Main category: cs.LG

TLDR: DeepForm是首个专为通信系统设计而优化的推理型LLM，通过两阶段训练策略和专用数据集CSFRC，显著优于现有通用LLM。


<details>
  <summary>Details</summary>
Motivation: 解决通用LLM在通信系统设计领域缺乏专业知识和高质量数据的问题。

Method: 采用两阶段训练：SFT结合CoT数据，以及基于ReMax的C-ReMax强化学习算法。
...
### [92] [The Geometries of Truth Are Orthogonal Across Tasks](https://arxiv.org/abs/2506.08572)
*Waiss Azizian,Michael Kirchhof,Eugene Ndiaye,Louis Bethune,Michal Klein,Pierre Ablin,Marco Cuturi*

Main category: cs.LG

TLDR: 研究发现，尽管大语言模型（LLMs）在多个任务中表现优异，但其可靠性仍存疑。通过分析模型激活状态来判断答案正确性的方法存在局限性，即“真理几何”无法跨任务迁移。


<details>
  <summary>Details</summary>
Motivation: 探讨大语言模型在任务间激活状态的差异，揭示现有方法在跨任务可靠性评估中的局限性。

Method: 通过训练线性分类器分析不同任务下的激活状态，并验证其跨任务迁移能力。

Result: 发现不同任务的“真理几何”几乎不重叠，且复杂方法（如混合探测）也无法解决此问题。

Conclusion: 激活状态在不同任务间差异显著，现有方法难以实现跨任务可靠性评估。
...
### [93] [SLEEPYLAND: trust begins with fair evaluation of automatic sleep staging models](https://arxiv.org/abs/2506.08574)
...
TLDR: SLEEPYLAND是一个开源睡眠分期评估框架，旨在解决模型评估、泛化、偏见和标注差异问题。其包含大量ID和OOD数据，并提出了SOMNUS集成模型，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 临床采用深度学习睡眠分期受限，因模型评估、泛化、偏见和标注差异问题。

Method: 提出SLEEPYLAND框架，包含大量ID/OOD数据，预训练模型，并引入SOMNUS集成模型。

Result: SOMNUS在24个数据集中表现优异，超越人类评分者，且能预测标注分歧区域。

Conclusion: SLEEPYLAND和SOMNUS显著提升睡眠分期性能，但仍需解决模型偏见问题。
...
### [94] [Diffusion-based Time Series Forecasting for Sewerage Systems](https://arxiv.org/abs/2506.08577)
*Nicholas A. Pearson,Francesca Cairoli,Luca Bortolussi,Davide Russo,Francesca Zanello*

Main category: cs.LG

TLDR: 提出了一种基于生成式人工智能的深度学习模型，用于提升污水系统上下文预测的准确性，结合扩散模型和保形推理技术，确保预测结果的统计可靠性。


<details>
  <summary>Details</summary>
Motivation: 解决污水系统在极端天气条件下预测准确性不足的问题，捕捉复杂的环境信号相关性。

Method: 开发扩散模型处理多元时间序列数据，结合保形推理技术校准预测区间。

Result: 模型在真实数据测试中表现出色，即使在极端天气下也能保持高准确性。

Conclusion: 该方法为污水系统预测提供了可靠且统计有效的解决方案。
...
### [95] [CALT: A Library for Computer Algebra with Transformer](https://arxiv.org/abs/2506.08600)
*Hiroshi Kera,Shun Arakawa,Yuta Sato*

Main category: cs.LG

TLDR: Transformer模型通过深度学习学习符号计算，CALT库为非专家提供训练模型的工具。
...
### [96] [Flow Matching Meets PDEs: A Unified Framework for Physics-Constrained Generation](https://arxiv.org/abs/2506.08604)
*Giacomo Baldan,Qiang Liu,Alberto Guardone,Nils Thuerey*

Main category: cs.LG

TLDR: 提出了一种基于物理约束的生成框架PBFM，显式嵌入PDE残差和代数关系，结合流匹配损失和物理残差损失，无需调参，显著提升物理残差精度和分布准确性。
...
Method: 提出PBFM框架，显式嵌入物理约束（PDE残差和代数关系），结合流匹配损失和物理残差损失，引入时间展开训练和随机采样策略。

Result: 在三个典型PDE问题上，PBFM的物理残差精度比FM高8倍，分布准确性优于现有算法。
...
### [97] [Sample Efficient Demonstration Selection for In-Context Learning](https://arxiv.org/abs/2506.08607)
*Kiran Purohit,V Venktesh,Sourangshu Bhattacharya,Avishek Anand*

Main category: cs.LG

TLDR: 论文提出了一种名为CASE的高效示例选择方法，通过将示例选择问题建模为多臂老虎机问题，显著减少了LLM的调用次数和运行时间。
...
Method: 将示例选择问题建模为多臂老虎机问题，提出CASE方法，通过维护一个“挑战者”候选集，减少样本复杂性和LLM调用次数。

Result: CASE在运行时间上实现了7倍加速，LLM调用次数减少了87%，且性能不下降。
...
### [98] [HSG-12M: A Large-Scale Spatial Multigraph Dataset](https://arxiv.org/abs/2506.08618)
*Xianquan Yan,Hakan Akgün,Kenji Kawaguchi,N. Duane Loh,Ching Hua Lee*

Main category: cs.LG

TLDR: HSG-12M是首个大规模空间多重图数据集，包含静态和动态的哈密顿谱图，用于几何感知的图学习。
...
Method: 通过Poly2Graph管道将一维晶体哈密顿量映射为谱图，生成多样化的物理基础拓扑。
...
### [99] [Time Series Representations for Classification Lie Hidden in Pretrained Vision Transformers](https://arxiv.org/abs/2506.08641)
*Simon Roschmann,Quentin Bouniot,Vasilii Feofanov,Ievgen Redko,Zeynep Akata*

Main category: cs.LG

TLDR: TiViT框架将时间序列转换为图像，利用预训练的视觉Transformer（ViT）提升时间序列分类性能。
...
Method: 将时间序列转换为图像，利用预训练的ViT模型提取特征，并结合OpenCLIP模型。

Result: TiViT在标准时间序列分类基准上达到最优性能，并与时间序列基础模型特征互补。

Conclusion: TiViT展示了视觉表示在非视觉领域重用的潜力。
...
### [100] [Semi-gradient DICE for Offline Constrained Reinforcement Learning](https://arxiv.org/abs/2506.08644)
*Woosung Kim,JunHo Seo,Jongmin Lee,Byung-Jun Lee*

Main category: cs.LG

TLDR: DICE框架用于解决策略诱导的稳态分布与目标分布不匹配问题，但近期改进方法削弱了其离线策略评估能力。本文提出新方法，通过半梯度DICE实现准确成本估计和约束强化学习。


<details>
  <summary>Details</summary>
Motivation: 解决DICE框架在改进离线强化学习性能时，意外削弱其离线策略评估能力的问题，使其适用于约束强化学习场景。

Method: 提出一种新方法，通过半梯度DICE优化，确保准确成本估计，并在DSRL基准测试中实现最优性能。

Result: 新方法在DSRL基准测试中表现优异，同时解决了成本估计问题。

Conclusion: 通过半梯度DICE优化，成功解决了DICE框架在约束强化学习中的局限性，并提升了性能。
...
### [101] [Fusing Cross-modal and Uni-modal Representations: A Kronecker Product Approach](https://arxiv.org/abs/2506.08645)
*Youqi Wu,Jingwei Zhang,Farzan Farnia*

Main category: cs.LG

TLDR: RP-KrossFuse方法通过随机投影和Kronecker乘积整合跨模态和单模态嵌入，既保留了跨模态对齐能力，又提升了单模态任务性能。


<details>
  <summary>Details</summary>
Motivation: 解决跨模态嵌入在单模态任务中表现不佳，而单模态嵌入缺乏跨模态对齐能力的问题。

Method: 提出RP-KrossFuse，利用随机投影和Kronecker乘积融合跨模态与单模态嵌入，在核空间中高效操作。

Result: 实验表明，RP-KrossFuse在单模态任务中表现优异，同时保持了跨模态对齐能力。
...
### [102] [JoFormer (Journey-based Transformer): Theory and Empirical Analysis on the Tiny Shakespeare Dataset](https://arxiv.org/abs/2506.08652)
*Mahesh Godavarti*

Main category: cs.LG

TLDR: JoFormer是一种基于非交换代数的Transformer架构，通过可学习的定向变换表示相对位置，优于RoFormer在语言建模任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 解决Transformer中有效融入位置信息的挑战。

Method: 提出JoFormer架构，利用非交换代数生成可学习的定向变换，扩展相对位置表示方法。

Result: 在Tiny Shakespeare任务中，JoFormer表现优于RoFormer，困惑度更低且收敛更快。

Conclusion: JoFormer为Transformer中位置信息的整合提供了理论支持，展示了其潜力。
...
### [103] [When Simple Model Just Works: Is Network Traffic Classification in Crisis?](https://arxiv.org/abs/2506.08655)
*Kamil Jerabek,Jan Luxemburk,Richard Plny,Josef Koumar,Jaroslav Pesek,Karel Hynek*

Main category: cs.LG

TLDR: 研究发现简单k-NN基线方法在网络流量分类中表现优异，原因是数据集中冗余样本过多，导致复杂模型性能被高估。


<details>
  <summary>Details</summary>
Motivation: 探讨为何简单k-NN方法在网络流量分类中能与复杂神经网络媲美，并揭示数据集冗余问题。

Method: 评估k-NN基线在12个数据集和15个任务中的表现，分析数据冗余及其影响。

Result: 多数数据集含50%以上冗余样本，导致模型性能被高估，且标准机器学习方法可能不适用于流量分类。

Conclusion: 提出新的任务制定和评估方向，以解决数据冗余问题并推动领域发展。
...
### [104] [Towards Robust Real-World Multivariate Time Series Forecasting: A Unified Framework for Dependency, Asynchrony, and Missingness](https://arxiv.org/abs/2506.08660)
*Jinkwan Jang,Hyungjin Park,Jinmyeong Choi,Taesup Kim*

Main category: cs.LG

TLDR: ChannelTokenFormer是一种基于Transformer的预测模型，旨在解决多变量时间序列数据中的通道依赖、采样异步和缺失值问题，并在实验中表现出优越的鲁棒性和准确性。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的多变量时间序列数据通常存在通道依赖、采样异步和缺失值问题，而现有模型大多基于过于简化的假设，无法有效处理这些挑战。
...
Conclusion: ChannelTokenFormer为多变量时间序列预测提供了一种有效的解决方案，能够应对现实世界中的复杂挑战。
...
### [105] [Optimizing Learned Image Compression on Scalar and Entropy-Constraint Quantization](https://arxiv.org/abs/2506.08662)
*Florian Borzechowski,Michael Schäfer,Heiko Schwarz,Jonathan Pfaff,Detlev Marpe,Thomas Wiegand*

Main category: cs.LG

TLDR: 论文提出了一种针对变分自编码器图像压缩的额外微调训练步骤，以解决量化噪声建模不准确的问题，从而提升编码效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在训练过程中无法准确建模量化噪声，导致网络性能次优。

Method: 在传统端到端训练后，对网络部分进行基于量化潜在表示的微调训练。

Result: 实验表明，该方法在均匀标量量化和熵约束量化中均能带来额外的编码增益，节省比特率。

Conclusion: 通过正确量化数据的微调训练，可以显著提升压缩效率，且不增加推理复杂度。
...
### [106] [Enhancing Reasoning Capabilities of Small Language Models with Blueprints and Prompt Template Search](https://arxiv.org/abs/2506.08669)
...
TLDR: 提出了一种通过LLM生成蓝图增强SLM推理能力的框架，并优化提示模板以减少SLM对提示变化的敏感性。
...
Method: 利用LLM生成结构化蓝图指导SLM推理，并结合提示模板搜索机制。
...
### [107] [Towards Fair Representation: Clustering and Consensus](https://arxiv.org/abs/2506.08673)
*Diptarka Chakraborty,Kushagra Chatterjee,Debarati Das,Tien Long Nguyen,Romina Nobahari*

Main category: cs.LG

TLDR: 该论文研究了共识聚类问题，首次结合公平性约束，提出了一个常数因子近似算法，并探讨了如何最小化修改现有聚类以实现公平性。


<details>
  <summary>Details</summary>
Motivation: 共识聚类是机器学习和数据分析中的基础任务，但现有研究未考虑公平性。本文旨在填补这一空白，确保聚类结果对特定保护属性公平。

Method: 通过引入公平性约束，提出了一种最优算法（适用于等比例群体）和近似算法（适用于不等比例群体），并证明了问题的NP难度。

Result: 提出了第一个常数因子近似算法，并展示了在不等比例群体情况下的高效近似解法。

Conclusion: 本文的研究为公平共识聚类提供了理论基础和实用算法，可能对其他聚类问题的公平性研究产生广泛影响。
...
### [108] [Mitigating Reward Over-optimization in Direct Alignment Algorithms with Importance Sampling](https://arxiv.org/abs/2506.08681)
*Phuc Minh Nguyen,Ngoc-Hieu Nguyen,Duy H. M. Nguyen,Anji Liu,An Mai,Binh T. Nguyen,Daniel Sonntag,Khoa D. Doan*

Main category: cs.LG

TLDR: 本文提出了一种基于重要性采样的方法（IS-DAAs）来解决直接对齐算法（DAAs）中的过优化问题，并通过实验验证其有效性。
...
Conclusion: IS-DAAs是一种有效的解决方案，能够提升直接对齐算法的性能。
...
### [109] [Variational Autoencoder-Based Approach to Latent Feature Analysis on Efficient Representation of Power Load Monitoring Data](https://arxiv.org/abs/2506.08698)
*Boyu Xie,Tangtang Xie*

Main category: cs.LG

TLDR: 提出基于变分自编码器（VAE）的VAE-LF模型，用于高效表示和补全高维不完整（HDI）电力负荷监测（PLM）数据，提升电力负荷预测（PLF）性能。


<details>
  <summary>Details</summary>
Motivation: 智能电网中高维不完整的PLM数据对PLF模型性能提出挑战，需有效补全缺失数据。

Method: 采用Encoder-Decoder结构的VAE-LF模型，将HDI PLM数据分向量输入，学习低维潜在表示并生成补全数据。

Result: 在UK-DALE数据集上，VAE-LF在5%和10%稀疏度测试中表现优于基准模型，RMSE和MAE显著更低，尤其低稀疏度数据表现突出。
...
### [110] [Breaking the ICE: Exploring promises and challenges of benchmarks for Inference Carbon & Energy estimation for LLMs](https://arxiv.org/abs/2506.08727)
...
TLDR: 论文探讨了生成式AI和大型语言模型（LLM）对能源和环境的影响，提出了利用现有基准数据估算碳排放的框架R-ICE，以解决当前工具的不足。


<details>
  <summary>Details</summary>
Motivation: 生成式AI的快速普及对能源和环境造成负担，现有工具在监测和估算碳排放方面存在数据需求高、侵入性强、误差大等问题。

Method: 提出框架R-ICE，利用现有SOTA基准数据估算LLM推理碳排放，提供非侵入性解决方案。

Result: 验证结果表明基于基准的建模在碳排放估算方面具有潜力。

Conclusion: R-ICE框架为动态LLM路由和碳核算等新兴用例提供了实用方法，值得进一步研究。
...
### [111] [Exploration by Random Reward Perturbation](https://arxiv.org/abs/2506.08737)
...
### [112] [Urban Incident Prediction with Graph Neural Networks: Integrating Government Ratings and Crowdsourced Reports](https://arxiv.org/abs/2506.08740)
*Sidhika Balachandar,Shuvom Sadhuka,Bonnie Berger,Emma Pierson,Nikhil Garg*

Main category: cs.LG

TLDR: 提出了一种多视图、多输出的GNN模型，结合政府评级和众包报告数据预测城市事件的真实状态，并在纽约市案例中验证其优越性。


<details>
  <summary>Details</summary>
Motivation: 政府评级数据稀疏且众包报告存在偏差，需要一种方法结合两者以更准确地预测城市事件的真实状态。

Method: 采用多视图、多输出的GNN模型，整合无偏的政府评级数据和有偏的众包报告数据。

Result: 模型在真实和半合成数据上优于仅使用单一数据源的模型，尤其是在评级数据稀疏时。同时量化了众包报告的 demographic 偏差。

Conclusion: 该方法为利用异构、稀疏且有偏数据进行潜在状态预测提供了广泛适用的解决方案。
...
### [113] [On the Stability of the Jacobian Matrix in Deep Neural Networks](https://arxiv.org/abs/2506.08764)
*Benjamin Dadoun,Soufiane Hayou,Hanan Salam,Mohamed El Amine Seddik,Pierre Youssef*

Main category: cs.LG

TLDR: 论文提出了一种适用于稀疏和非独立同分布权重的深度神经网络稳定性理论，扩展了初始化方案的理论基础。
...
Method: 利用随机矩阵理论的最新进展，建立了一个适用于更广泛网络模型的稳定性定理。
...
### [114] [Design Patterns for Securing LLM Agents against Prompt Injections](https://arxiv.org/abs/2506.08837)
...
TLDR: 提出了一套设计模式，用于构建具有可证明抗提示注入攻击能力的AI代理，并分析了其效用与安全性的权衡。
...
Method: 提出了一套设计模式，系统分析其效用与安全性，并通过案例研究展示实际应用。

Result: 设计模式能够有效抵抗提示注入攻击，同时考虑了实用性与安全性的平衡。

Conclusion: 该研究为构建安全的AI代理提供了理论基础和实用方法。
...
### [115] [IMAGIC-500: IMputation benchmark on A Generative Imaginary Country (500k samples)](https://arxiv.org/abs/2506.08844)
*Siyi Sun,David Antony Selby,Yunchuan Huang,Sebastian Vollmer,Seth Flaxman,Anisoara Calinescu*

Main category: cs.LG

TLDR: 研究利用公开的合成数据集IMAGIC-500，系统评估了不同缺失机制和比例下的数据填补方法，填补了社会经济数据填补领域的基准空白。
...
Method: 基于世界银行的合成数据集，构建了IMAGIC-500数据集，并在不同缺失机制（MCAR、MAR、MNAR）和比例下评估了多种填补方法。

Result: 结果揭示了统计、传统机器学习和深度学习填补方法的优缺点，包括扩散模型等新方法。

Conclusion: IMAGIC-500数据集和基准为填补算法的开发和可重复的社会科学研究提供了支持。
...
### [116] [Agile Reinforcement Learning for Real-Time Task Scheduling in Edge Computing](https://arxiv.org/abs/2506.08850)
*Amin Avan,Akramul Azim,Qusay Mahmoud*

Main category: cs.LG

TLDR: 论文提出了一种敏捷强化学习（aRL）方法，通过智能探索和动作屏蔽技术优化边缘计算中软实时应用的任务调度，显著提高了命中率和收敛速度。


<details>
  <summary>Details</summary>
Motivation: 边缘计算环境中任务调度的复杂性、动态性和多目标性导致传统启发式算法难以生成最优调度方案，而强化学习算法又因学习时间长和无效探索受限。

Method: 提出敏捷强化学习（aRL），结合智能探索和动作屏蔽技术，仅执行相关动作，减少无效探索，提升预测性和适应性。

Result: 实验表明，aRL在命中率和收敛速度上优于基线方法。

Conclusion: aRL通过优化探索和动作选择，成为边缘计算中软实时应用任务调度的有效解决方案。
...
### [117] [Adapting to Heterophilic Graph Data with Structure-Guided Neighbor Discovery](https://arxiv.org/abs/2506.08871)
*Victor M. Tenorio,Madeline Navarro,Samuel Rey,Santiago Segarra,Antonio G. Marques*

Main category: cs.LG

TLDR: 提出了一种基于结构引导的图神经网络（SG-GNN），通过构建具有更高标签同质性的新图结构，解决GNN在异质性数据上的性能问题。


<details>
  <summary>Details</summary>
Motivation: 传统GNN假设同质性，依赖局部消息传递，但在异质性数据（连接节点标签不同）上表现不佳。

Method: 通过链接具有相似结构属性的节点创建新图结构，提出SG-GNN，自适应学习原始图与新图的权重。
...
### [118] [Filling in the Blanks: Applying Data Imputation in incomplete Water Metering Data](https://arxiv.org/abs/2506.08882)
*Dimitrios Amaxilatis,Themistoklis Sarantakos,Ioannis Chatzigiannakis,Georgios Mylonas*

Main category: cs.LG

TLDR: 论文探讨了数据插补技术在水分配网络监测中的应用，通过比较多种方法（如k-NN、MissForest、Transformer和RNN），发现有效的数据插补能显著提升水质数据的准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 智能水表数据因技术问题存在缺失，影响运营决策和效率，需通过数据插补技术解决。

Method: 比较了k-Nearest Neighbors、MissForest、Transformer和Recurrent Neural Networks等多种数据插补方法。

Result: 有效的数据插补显著提升了水质数据的准确性和可靠性，支持漏损检测和预测性维护等应用。

Conclusion: 数据插补技术能显著改善智能水表数据的质量，为水分配网络管理提供更可靠的解决方案。
...
### [119] [InfoDPCCA: Information-Theoretic Dynamic Probabilistic Canonical Correlation Analysis](https://arxiv.org/abs/2506.08884)
*Shiqin Tang,Shujian Yu*

Main category: cs.LG

TLDR: InfoDPCCA是一种动态概率CCA框架，用于提取两个相关观测序列的共享潜在表示，同时学习序列特定信息。
...
Method: 采用信息论目标提取共享潜在表示，并通过两步训练方案和残差连接机制提升模型稳定性。
...
### [120] [SeerAttention-R: Sparse Attention Adaptation for Long Reasoning](https://arxiv.org/abs/2506.08889)
...
TLDR: SeerAttention-R是一种稀疏注意力框架，专为推理模型的长解码设计，保留了自蒸馏门控机制，移除查询池化以适应自回归解码，训练仅需0.4B token，在4K token预算下保持近乎无损的推理精度，并实现高达9倍的理论加速。


<details>
  <summary>Details</summary>
Motivation: 为推理模型的长解码设计一种高效且灵活的稀疏注意力框架，同时保持高精度和性能。

Method: 扩展SeerAttention，移除查询池化，引入轻量级门控机制，开发优化的稀疏解码内核。

Result: 在AIME基准测试中，4K token预算下保持近乎无损的推理精度，稀疏解码内核在H100 GPU上实现高达9倍的理论加速。
...
### [121] [SwS: Self-aware Weakness-driven Problem Synthesis in Reinforcement Learning for LLM Reasoning](https://arxiv.org/abs/2506.08989)
*Xiao Liang,Zhong-Zhi Li,Yeyun Gong,Yang Wang,Hengyuan Zhang,Yelong Shen,Ying Nian Wu,Weizhu Chen*

Main category: cs.LG

TLDR: RLVR训练LLMs在复杂推理任务中有效，但高质量问题集稀缺。SwS框架通过自我识别弱点并针对性生成问题，提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有问题集质量不足且合成策略效率低，限制了RLVR的可扩展性。

Method: 提出SwS框架，识别模型弱点并基于失败案例合成新问题，针对性增强训练。

Result: 在7B和32B模型上，平均性能提升10.0%和7.7%。

Conclusion: SwS通过自我识别和解决弱点，无需外部知识蒸馏，显著提升模型推理能力。
...
### [122] [Intention-Conditioned Flow Occupancy Models](https://arxiv.org/abs/2506.08902)
*Chongyi Zheng,Seohong Park,Sergey Levine,Benjamin Eysenbach*

Main category: cs.LG

TLDR: 论文提出了一种基于流匹配的意图条件流占用模型（InFOM），用于预测智能体在长期未来可能访问的状态，并通过潜在变量捕捉用户意图，显著提升了强化学习预训练的效果。


<details>
  <summary>Details</summary>
Motivation: 大规模预训练在机器学习中取得了显著成功，但在强化学习（RL）中仍面临长期依赖的挑战。论文旨在通过生成式AI的新工具，构建一个能够预测长期状态的模型，以提升RL的样本效率和鲁棒性。

Method: 提出了一种基于流匹配的概率模型（InFOM），通过潜在变量捕捉用户意图，预测智能体在长期未来的状态占用情况，并利用广义策略改进进行适应。

Result: 在36个基于状态和4个基于图像的基准任务上，InFOM相比其他预训练方法，回报中位数提升了1.8倍，成功率提高了36%。

Conclusion: InFOM通过结合流匹配和意图建模，显著提升了强化学习的预训练效果，为RL的长期依赖问题提供了有效解决方案。
...
### [123] [e3: Learning to Explore Enables Extrapolation of Test-Time Compute for LLMs](https://arxiv.org/abs/2506.09026)
*Amrith Setlur,Matthew Y. R. Yang,Charlie Snell,Jeremy Greer,Ian Wu,Virginia Smith,Max Simchowitz,Aviral Kumar*

Main category: cs.LG

TLDR: 论文提出了一种名为e3的方法，通过训练LLM在推理时进行上下文探索，以提升其推理能力，并在超出训练预算时仍能保持性能。


<details>
  <summary>Details</summary>
Motivation: 现有推理模型在超出训练预算时性能不佳，希望通过改进推理时的计算利用来提升性能。

Method: 通过训练LLM进行上下文探索，包括链式操作（如生成、验证、精炼等）、利用错误轨迹的负梯度增强探索，以及设计训练课程。

Result: e3方法在1.7B模型上取得了AIME'25和HMMT'25的最佳成绩，并能扩展到2倍训练预算。

Conclusion: e3方法显著提升了LLM的推理能力和性能扩展性。
...
### [124] [Enhancing generalizability of model discovery across parameter space with multi-experiment equation learning (ME-EQL)](https://arxiv.org/abs/2506.08916)
*Maria-Veronica Ciocanel,John T. Nardini,Kevin B. Flores,Erica M. Rutter,Suzanne S. Sindi,Alexandria Volkening*

Main category: cs.LG

TLDR: 论文提出两种多实验方程学习方法（ME-EQL），通过插值或统一模型库提升ABM数据的泛化性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 解决传统方程学习（EQL）方法因需大量模拟而泛化性不足的问题。

Method: 提出OAT ME-EQL（逐个参数集学习并插值）和ES ME-EQL（统一模型库）两种方法。
...
### [125] [Local MDI+: Local Feature Importances for Tree-Based Models](https://arxiv.org/abs/2506.08928)
*Zhongyuan Liang,Zachary T. Rewolinski,Abhineet Agarwal,Tiffany M. Tang,Bin Yu*

Main category: cs.LG

TLDR: 论文提出了一种名为LMDI+的新方法，用于解决现有局部特征重要性方法（如LIME和TreeSHAP）的不足，通过扩展MDI+框架到样本特定场景，显著提升了性能和稳定性。


<details>
  <summary>Details</summary>
Motivation: 由于树集成模型在高风险领域广泛应用，其可解释性至关重要。现有局部特征重要性方法依赖近似和扰动，忽略了模型内部结构，导致不稳定。

Method: 提出LMDI+方法，将MDI+框架扩展到样本特定场景，利用决策树与线性模型在节点基础上的等价性。

Result: LMDI+在12个真实数据集上平均提升下游任务性能10%，并表现出更高的稳定性。

Conclusion: LMDI+不仅优于现有基线方法，还支持局部可解释性用例，如识别反事实和发现同质子组。
...
### [126] [BioLangFusion: Multimodal Fusion of DNA, mRNA, and Protein Language Models](https://arxiv.org/abs/2506.08936)
*Amina Mollaysa,Artem Moskale,Pushpak Pati,Tommaso Mansi,Mangal Prakash,Rui Liao*

Main category: cs.LG

TLDR: BioLangFusion通过整合DNA、mRNA和蛋白质的预训练语言模型，提供统一的分子表示，并在分子属性预测任务中优于单模态基线。


<details>
  <summary>Details</summary>
Motivation: 受分子生物学中心法则（基因到转录到蛋白质的信息流）启发，研究如何整合不同模态的分子信息。

Method: 采用三种融合技术：密码子级嵌入拼接、熵正则化注意力池化和跨模态多头注意力，无需额外预训练。

Result: 在五个分子属性预测任务中表现优于单模态基线。

Conclusion: 简单融合预训练模型即可高效捕获多组学信息。
...
### [127] [KARMA: A Multilevel Decomposition Hybrid Mamba Framework for Multivariate Long-Term Time Series Forecasting](https://arxiv.org/abs/2506.08939)
*Hang Ye,Gaoxiang Duan,Haoran Zeng,Yangxin Zhu,Lingxue Meng,Xiaoying Zheng,Yongxin Zhu*

Main category: cs.LG

TLDR: KARMA提出了一种自适应时间通道分解模块（ATCD）和混合频率-时间分解模块（HFTD），结合多尺度Mamba-based KarmaBlock，显著提升了多变量长期时间序列预测的准确性和计算效率。


<details>
  <summary>Details</summary>
Motivation: 传统时间序列分解方法单一且依赖固定规则，无法挖掘复杂序列的动态特性；而基于Transformer的模型因计算复杂度高难以有效建模长序列和复杂动态关系。

Method: KARMA通过ATCD动态提取趋势和季节性成分，并通过HFTD进一步分解序列为频域和时域，结合多尺度KarmaBlock协调处理全局和局部信息。

Result: 在八个真实数据集上的实验表明，KARMA在预测准确性和计算效率上显著优于主流基线方法。

Conclusion: KARMA通过创新的分解和建模方法，有效解决了复杂时间序列预测的挑战，具有实际应用潜力。
...
### [128] [Towards Robust Deep Reinforcement Learning against Environmental State Perturbation](https://arxiv.org/abs/2506.08961)
*Chenxu Wang,Huaping Liu*

Main category: cs.LG

TLDR: 论文研究了深度强化学习（DRL）中环境状态扰动的对抗攻击与防御，提出了一种名为BAT的防御框架，显著提升了代理的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有研究较少关注环境状态扰动，而这是实际场景中的常见问题。论文旨在提升DRL代理对此类扰动的鲁棒性。

Method: 首先提出一种非目标攻击方法作为校准对手，随后设计BAT框架，结合监督学习和对抗训练强化代理。

Result: 实验表明主流代理易受环境状态扰动影响，BAT框架能显著提升鲁棒性。

Conclusion: BAT框架在多种情境下有效增强代理对环境状态扰动的抵抗能力，优于现有方法。
...
### [129] [GFRIEND: Generative Few-shot Reward Inference through EfficieNt DPO](https://arxiv.org/abs/2506.08965)
*Yiyang Zhao,Huiyu Bai,Xuejiao Zhao*

Main category: cs.LG

TLDR: 提出了一种数据增强与扩展框架，通过偏好细化和多级直接偏好优化（M-DPO），使小数据集训练的生成奖励模型性能媲美大数据集训练的模型。
...
Method: 采用偏好细化（Chain-of-Thought采样）和多级直接偏好优化（M-DPO），结合困惑度评分机制。

Result: 实验表明，该方法显著提升数据效率和模型性能，小数据集训练的模型性能媲美大数据集训练的模型。

Conclusion: 数据高效策略在奖励模型优化中潜力巨大，为低资源RLHF应用提供了稳健解决方案。
...
### [130] [Tailored Architectures for Time Series Forecasting: Evaluating Deep Learning Models on Gaussian Process-Generated Data](https://arxiv.org/abs/2506.08977)
*Victoria Hankemeier,Malte Schilling*

Main category: cs.LG

TLDR: 论文提出了一种基于高斯过程生成的新数据集，用于评估模型对不同时间序列特征的适应性，并提出了模块化架构模型TimeFlex。


<details>
  <summary>Details</summary>
Motivation: 揭示时间序列特征与特定模型之间的明确联系，填补现有研究中对模型适应性的评估不足。

Method: 使用高斯过程生成具有已知特征的数据集，并开发模块化模型TimeFlex以处理多样化的时间动态。

Result: TimeFlex在多样化时间序列条件下表现优于现有最先进模型。

Conclusion: 研究为时间序列模型的选择和设计提供了更清晰的指导，展示了模型适应性的重要性。
...
### [131] [Propositional Logic for Probing Generalization in Neural Networks](https://arxiv.org/abs/2506.08978)
*Anna Langedijk,Jaap Jumelet,Willem Zuidema*

Main category: cs.LG

TLDR: 论文研究了三种神经网络架构（Transformers、GCNs、LSTMs）在命题逻辑任务中的泛化能力，发现其对未见过模式（特别是涉及否定的情况）的泛化能力有限。


<details>
  <summary>Details</summary>
Motivation: 探讨神经网络是否能学习和表示符号规则，尤其是在逻辑推理任务中的表现。

Method: 使用扩展的数据集评估三种架构在命题逻辑任务中的泛化能力，特别是对未见过运算符组合的表现。

Result: 所有模型在训练分布内表现良好，但对未见过模式（尤其是涉及否定）的泛化能力较差，Transformers需要结构偏置才能组合性应用否定。

Conclusion: 标准架构在系统学习逻辑运算符表示方面存在局限性，需要更强的归纳偏置以支持基于规则的推理。
...
### [132] [On Finetuning Tabular Foundation Models](https://arxiv.org/abs/2506.08982)
*Ivan Rubachev,Akim Kotelnikov,Nikolay Kartashev*

Main category: cs.LG

TLDR: TabPFNv2的微调策略研究表明，完整微调在时间效率和效果上表现最佳，揭示了微调通过改进相似性度量提升性能的机制。


<details>
  <summary>Details</summary>
Motivation: 探索TabPFNv2这种表格基础模型的最佳微调方法及其内部机制变化，填补现有研究的空白。

Method: 系统评估多种微调策略，分析微调对模型内部机制的影响，类比检索增强模型。

Result: 完整微调在大多数任务中表现最佳，尤其在学术数据集上达到SOTA，但在时序变化和复杂特征的数据集上表现不稳定。

Conclusion: 完整微调是TabPFNv2的实用解决方案，但其适用性受数据集特性影响。
...
### [133] [Branched Schrödinger Bridge Matching](https://arxiv.org/abs/2506.09007)
*Sophia Tang,Yinuo Zhang,Alexander Tong,Pranam Chatterjee*

Main category: cs.LG

TLDR: 论文提出了一种名为BranchSBM的新框架，用于学习分支的Schrödinger桥，解决了现有方法无法捕捉多路径分支演化的问题。
...
Conclusion: BranchSBM为解决多路径分支演化问题提供了有效工具，适用于复杂动态系统的建模。
...
### [134] [Effective Data Pruning through Score Extrapolation](https://arxiv.org/abs/2506.09010)
*Sebastian Schmidt,Prasanga Dhungel,Christoffer Löffler,Björn Nieth,Stephan Günnemann,Leo Schwinn*

Main category: cs.LG

TLDR: 提出了一种新的重要性分数外推框架，仅需少量数据训练即可预测样本重要性，显著降低计算成本。
...
Method: 引入基于k近邻和图神经网络的重要性分数外推框架，通过少量数据训练预测全数据集样本重要性。

Result: 在多种剪枝方法、数据集和训练范式下验证了框架的有效性。
...
### [135] [SPEED-RL: Faster Training of Reasoning Models via Online Curriculum Learning](https://arxiv.org/abs/2506.09016)
*Ruiqi Zhang,Daman Arora,Song Mei,Andrea Zanette*

Main category: cs.LG

TLDR: SPEED是一种自适应在线RL课程，通过选择性选择中等难度的训练示例，显著提高学习效率，实现2x到6x的加速训练。
...
Result: 实验显示SPEED实现2x到6x训练加速，且不影响准确性。

Conclusion: SPEED无需手动调参，可无缝集成到标准RL算法中，显著提升效率。
...
### [136] [Edit Flows: Flow Matching with Edit Operations](https://arxiv.org/abs/2506.09018)
*Marton Havasi,Brian Karrer,Itai Gat,Ricky T. Q. Chen*

Main category: cs.LG

TLDR: Edit Flows是一种非自回归模型，通过编辑操作（插入、删除、替换）在序列空间上定义离散流，克服了传统非自回归模型的局限性，实现了更灵活的序列生成。


<details>
  <summary>Details</summary>
Motivation: 传统非自回归模型在生成变长序列时表现不佳，通常需要固定的标记结构。Edit Flows旨在通过编辑操作实现更灵活的序列生成。

Method: Edit Flows通过在序列空间上定义连续时间马尔可夫链，结合插入、删除和替换操作，并利用扩展状态空间和辅助变量进行高效训练。

Result: 实验结果表明，Edit Flows在图像描述生成任务上优于自回归和掩码模型，在文本和代码生成任务上显著优于掩码构造方法。

Conclusion: Edit Flows通过编辑操作实现了灵活且高效的序列生成，为非自回归模型提供了新的可能性。
...
### [137] [FZOO: Fast Zeroth-Order Optimizer for Fine-Tuning Large Language Models towards Adam-Scale Speed](https://arxiv.org/abs/2506.09034)
*Sizhe Dang,Yangyang Guo,Yanjun Zhao,Haishan Ye,Xiaodong Zheng,Guang Dai,Ivor Tsang*

Main category: cs.LG

TLDR: FZOO是一种快速零阶优化器，通过减少收敛所需的前向传递次数和利用并行处理，显著提高了优化速度和内存效率。
...
Result: FZOO在11个任务中平均比MeZO准确率高3%，前向传递次数减少3倍；RoBERTa-large上准确率提高5.6%，前向传递减少18倍。
...
### [138] [The Decoupled Risk Landscape in Performative Prediction](https://arxiv.org/abs/2506.09044)
*Javier Sanguino,Thomas Kehrenberg,Jose A. Lozano,Novi Quadrianto*

Main category: cs.LG

TLDR: 论文提出了一种可视化方法，用于补充Performative Prediction的理论研究，并引入扩展的Performative Prediction设置。


<details>
  <summary>Details</summary>
Motivation: 通过可视化损失景观，为Performative Prediction的理论研究提供实践见解。

Method: 1. 引入解耦风险可视化方法，展示模型参数和数据参数的风险景观；2. 提出扩展的Performative Prediction设置，反映现实场景。

Result: 可视化方法揭示了兴趣点的新特性，并分析了现有算法在更现实条件下的表现。

Conclusion: 可视化方法为Performative Prediction提供了实践补充，扩展设置更贴近现实场景。
...
### [139] [Agentic Neural Networks: Self-Evolving Multi-Agent Systems via Textual Backpropagation](https://arxiv.org/abs/2506.09046)
*Xiaowen Ma,Chenyang Lin,Yao Zhang,Volker Tresp,Yunpu Ma*

Main category: cs.LG

TLDR: 提出Agentic Neural Network（ANN）框架，将多智能体协作建模为分层神经网络架构，通过前向和后向两阶段优化提升任务分解与协作效率，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前多智能体系统依赖静态配置，难以适应复杂任务，需动态、可扩展的协作框架。

Method: ANN将智能体作为节点，分层协作；前向阶段动态分解任务并构建协作团队，后向阶段通过反馈优化协作。

Result: 在四个基准数据集上，ANN优于现有方法，表现更优。

Conclusion: ANN结合LLMs协作能力与神经网络效率，提供可扩展的数据驱动框架，计划开源。
...
### [140] [Understanding Task Vectors in In-Context Learning: Emergence, Functionality, and Limitations](https://arxiv.org/abs/2506.09048)
*Yuxin Dong,Jiachen Jiang,Zhihui Zhu,Xia Ning*

Main category: cs.LG

TLDR: 论文提出了线性组合猜想，认为任务向量是通过原始演示的线性组合形成的单一上下文演示，并通过理论和实验验证了这一猜想。


<details>
  <summary>Details</summary>
Motivation: 任务向量在上下文学习中加速推理的机制尚不明确，研究旨在揭示其工作原理。

Method: 通过损失景观分析、理论推导和实验验证（包括显著性分析和参数可视化）支持线性组合猜想。

Result: 任务向量在线性变换器中自然出现，但在表示高秩映射时会失效，实验验证了这一点。

Conclusion: 研究深化了对任务向量的理解，揭示了基于变换器的模型中上下文学习的机制。
...
### [141] [Conservative Bias in Large Language Models: Measuring Relation Predictions](https://arxiv.org/abs/2506.08120)
*Toyin Aguda,Erik Wilson,Allan Anzagira,Simerjot Kaur,Charese Smiley*

Main category: cs.CL

TLDR: LLMs在关系提取任务中表现出保守偏差，倾向于选择No_Relation标签，导致信息丢失。研究发现保守偏差比幻觉更常见，并通过SBERT和LLM提示量化其影响。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在关系提取任务中的保守偏差行为及其对信息完整性的影响。

Method: 通过多提示、数据集和关系类型评估保守偏差，引入Hobson's choice概念，并使用SBERT和LLM提示量化语义相似性。

Result: 保守偏差发生频率是幻觉的两倍，且导致显著信息丢失。

Conclusion: LLMs的保守偏差虽避免错误分配，但牺牲信息完整性，需进一步优化。
...
### [142] [QA-LIGN: Aligning LLMs through Constitutionally Decomposed QA](https://arxiv.org/abs/2506.08123)
*Jacob Dineen,Aswin RRV,Qin Liu,Zhikun Xu,Xiao Ye,Ming Shen,Zhaonan Li,Shijie Lu,Chitta Baral,Muhao Chen,Ben Zhou*

Main category: cs.CL

TLDR: QA-LIGN是一种自动符号奖励分解方法，通过为每个原则设计特定评估问题，实现透明且可控的语言模型对齐。
...
Method: QA-LIGN为每个宪法原则设计评估问题，生成独立奖励组件，替代传统黑盒奖励模型。

Result: 实验表明QA-LIGN在透明度和适应性上优于基线，性能不逊于或优于DPO方法。

Conclusion: QA-LIGN为语言模型对齐提供了更可解释且可控的解决方案，同时保持任务性能。
...
### [143] [EconWebArena: Benchmarking Autonomous Agents on Economic Tasks in Realistic Web Environments](https://arxiv.org/abs/2506.08136)
*Zefang Liu,Yinzhu Quan*

Main category: cs.CL

TLDR: EconWebArena是一个用于评估自主代理在复杂、多模态经济任务中表现的基准测试，包含360个任务，覆盖多个经济领域，强调权威数据源和基于网络的经济推理。
...
Result: 评估显示多模态LLM在导航、多模态理解和数据提取方面存在显著性能差距。
...
### [144] [Multilingual Hate Speech Detection in Social Media Using Translation-Based Approaches with Large Language Models](https://arxiv.org/abs/2506.08147)
*Muhammad Usman,Muhammad Ahmad,M. Shahiki Tash,Irina Gelbukh,Rolando Quintero Tellez,Grigori Sidorov*

Main category: cs.CL

TLDR: 该论文提出了一种基于注意力层和大型语言模型的多语言仇恨言论检测方法，填补了乌尔都语研究的空白，并在多语言数据集上取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 社交媒体上的仇恨言论威胁在线安全与包容性，但乌尔都语的研究较少，尤其是基于翻译的方法。

Method: 使用注意力层增强特征提取，结合GPT-3.5 Turbo和Qwen 2.5 72B等模型，同时对比传统方法如SVM。

Result: 在多语言数据集上，模型性能显著提升，F1分数最高达0.88。
...
### [145] [ETT-CKGE: Efficient Task-driven Tokens for Continual Knowledge Graph Embedding](https://arxiv.org/abs/2506.08158)
*Lijing Zhu,Qizhen Lan,Qing Tian,Wenbo Sun,Li Yang,Lu Xia,Yixin Xie,Xi Xiao,Tiehang Duan,Cui Tao,Shuteng Niu*

Main category: cs.CL

TLDR: ETT-CKGE提出了一种高效的任务驱动令牌方法，解决了现有CKGE方法在知识保留和计算效率上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有CKGE方法在知识保留和计算效率上存在局限性，需要改进。

Method: 引入可学习的任务驱动令牌，避免显式节点评分或遍历，通过矩阵操作实现知识转移。

Result: 在六个基准数据集上表现优异，显著提升了训练效率和可扩展性。

Conclusion: ETT-CKGE在性能和效率上均优于现有方法。
...
### [146] [Can Artificial Intelligence Write Like Borges? An Evaluation Protocol for Spanish Microfiction](https://arxiv.org/abs/2506.08172)
*Gerardo Aleman Manzanarez,Nora de la Cruz Arana,Jorge Garcia Flores,Yobany Garcia Medina,Raul Monroy,Nathalie Pernelle*

Main category: cs.CL

TLDR: 论文提出GrAImes协议，基于文学理论评估AI生成微小说的文学价值，并验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有AI生成故事缺乏对文学价值的严格评估，尤其是美学品质。

Method: 提出GrAImes协议，结合文学理论，从主题连贯性、文本清晰度、解释深度和美学质量等方面评估。

Result: 验证了GrAImes协议的有效性，得到文学专家和爱好者的认可。

Conclusion: GrAImes协议为评估AI生成微小说的文学价值提供了客观框架。
...
### [147] [LLM-BT: Back-Translation as a Framework for Terminology Standardization and Dynamic Semantic Embedding](https://arxiv.org/abs/2506.08174)
*Li Weigang,Pedro Carvalho Brom*

Main category: cs.CL

TLDR: LLM-BT是一种基于大语言模型的回译框架，用于自动化术语验证和标准化，通过跨语言语义对齐实现高一致性。


<details>
  <summary>Details</summary>
Motivation: 传统的人工标准化方法难以应对快速发展的技术术语，尤其是在AI和量子计算等领域，需要一种自动化解决方案。

Method: LLM-BT采用回译框架，结合多路径验证工作流（检索-生成-验证-优化），并通过动态语义嵌入实现术语一致性。

Result: 实验显示，LLM-BT在术语一致性上表现优异（90%以上匹配），跨语言鲁棒性强（BLEU>0.45，葡萄牙语准确率100%）。
...
### [148] [Unable to forget: Proactive lnterference Reveals Working Memory Limits in LLMs Beyond Context Length](https://arxiv.org/abs/2506.08184)
*Chupei Wang,Jiaqiu Vince Sun*

Main category: cs.CL

TLDR: 研究探讨了大型语言模型（LLMs）在信息检索中因上下文干扰导致的性能下降问题，并提出了一种评估方法PI-LLM。


<details>
  <summary>Details</summary>
Motivation: 长上下文通常被认为能提升检索能力，但内部干扰的影响尚未充分研究。

Method: 采用认知科学中的主动干扰（PI）范式，设计PI-LLM评估方法，测试LLMs在语义相关更新流中的检索表现。

Result: 随着干扰累积，LLM检索准确率呈对数线性下降至零，提示工程缓解效果有限。

Conclusion: LLMs在抑制无关内容方面存在瓶颈，需改进模型以增强信息检索的灵活性。
...
### [149] ["I Wrote, I Paused, I Rewrote" Teaching LLMs to Read Between the Lines of Student Writing](https://arxiv.org/abs/2506.08221)
*Samra Zafar,Shaheer Minhas,Syed Ali Hassan Zaidi,Arfa Naeem,Zahra Ali*

Main category: cs.CL

TLDR: 研究探讨了利用写作过程数据（如击键记录和定期快照）改进大型语言模型（LLM）对学生的写作反馈，结果显示学生更偏好基于过程的反馈。
...
Method: 开发了一个数字写作工具，记录学生的写作过程和最终文本，并让20名学生使用时限写作任务，比较基于最终文本和基于写作过程的LLM反馈效果。
...
### [150] [Compound AI Systems Optimization: A Survey of Methods, Challenges, and Future Directions](https://arxiv.org/abs/2506.08234)
*Yu-Ang Lee,Guan-Ting Yi,Mei-Yi Liu,Jui-Chao Lu,Guan-Bo Yang,Yun-Nung Chen*

Main category: cs.CL

TLDR: 本文综述了复合AI系统优化的最新进展，包括数值和基于语言的技术，并提出了未来研究方向。
...
### [151] [Can AI Validate Science? Benchmarking LLMs for Accurate Scientific Claim $\rightarrow$ Evidence Reasoning](https://arxiv.org/abs/2506.08235)
*Shashidhar Reddy Javaji,Yupeng Cao,Haohang Li,Yangyang Yu,Nikhil Muralidhar,Zining Zhu*

Main category: cs.CL

TLDR: CLAIM-BENCH是一个评估大语言模型（LLMs）在科学论文中提取和验证声明与证据能力的基准测试，揭示了LLMs在处理复杂科学内容时的局限性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索LLMs是否能够理解和处理科学论文中声明与证据之间的复杂逻辑关系。

Method: 通过系统比较三种启发自分治策略的方法，在六种LLMs上进行评估，涉及300多个声明-证据对。

Result: 结果显示，闭源模型（如GPT-4和Claude）在精确率和召回率上优于开源模型，且三遍和逐一提示方法显著提升了性能。
...
### [152] [Automatic Generation of Inference Making Questions for Reading Comprehension Assessments](https://arxiv.org/abs/2506.08260)
*Wanjing Anya Ma,Michael Flor,Zuowei Wang*

Main category: cs.CL

TLDR: 论文探讨了阅读理解中推理技能的复杂性，提出了一种推理类型分类法，并利用GPT-4o生成诊断性问题，实验表明自动生成结合人工判断具有潜力。


<details>
  <summary>Details</summary>
Motivation: 解决阅读理解中推理技能的复杂性，为教育者提供更有效的诊断工具。

Method: 提出推理类型分类法，使用GPT-4o生成推理问题，比较不同提示条件下的效果。

Result: GPT-4o生成的问题93.8%质量良好，但仅42.6%准确匹配目标推理类型。

Conclusion: 自动生成结合人工判断是高质量诊断评估的可扩展路径。
...
### [153] [Institutional Books 1.0: A 242B token dataset from Harvard Library's collections, refined for accuracy and usability](https://arxiv.org/abs/2506.08300)
...
TLDR: 该技术报告介绍了Institutional Books 1.0数据集，旨在解决高质量公开训练数据的稀缺问题，通过哈佛图书馆的公共领域书籍提供丰富的历史文本资源。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）的训练数据质量直接影响其性能，但目前高质量公开数据稀缺，需要可持续的数据管理实践。

Method: 与哈佛图书馆合作，提取、分析并处理了Google Books项目中扫描的公共领域书籍，形成包含OCR文本和元数据的数据集。

Result: 发布了983,004卷公共领域书籍的文本和元数据，总计242B tokens，覆盖多种语言和历史时期。

Conclusion: 该数据集为研究者和开发者提供了高质量的历史文本资源，支持更易访问和使用的数据管理实践。
...
### [154] [Wait, We Don't Need to "Wait"! Removing Thinking Tokens Improves Reasoning Efficiency](https://arxiv.org/abs/2506.08343)
*Chenlong Wang,Yuanning Feng,Dongping Chen,Zhaoyang Chu,Ranjay Krishna,Tianyi Zhou*

Main category: cs.CL

TLDR: NoWait方法通过抑制显式自我反思标记（如“Wait”和“Hmm”）来减少推理过程中的冗余输出，提高效率，同时保持模型性能。
...
### [155] [Evaluating LLMs Across Multi-Cognitive Levels: From Medical Knowledge Mastery to Scenario-Based Problem Solving](https://arxiv.org/abs/2506.08349)
*Yuxuan Zhou,Xien Liu,Chenwei Yan,Chen Ning,Xiao Zhang,Boxun Li,Xiangling Fu,Shijin Wang,Guoping Hu,Yu Wang,Ji Wu*

Main category: cs.CL

TLDR: 论文提出了一种基于Bloom分类法的多认知层次评估框架，用于评估大语言模型（LLMs）在医学领域的表现，发现模型性能随认知复杂度增加而显著下降。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在不同认知层次上的能力，尤其是在医学领域的表现，填补现有研究的空白。

Method: 提出多认知层次评估框架，整合现有医学数据集，设计针对三个认知层次的任务（基础知识掌握、综合知识应用、场景问题解决），并评估六类主流LLMs。

Result: 模型性能随认知复杂度增加而显著下降，模型规模在高认知层次表现中起关键作用。

Conclusion: 研究强调需提升LLMs在高认知层次的医学能力，为开发适用于实际医学应用的LLMs提供参考。
...
### [156] [Text Embeddings Should Capture Implicit Semantics, Not Just Surface Meaning](https://arxiv.org/abs/2506.08354)
*Yiqun Sun,Qiang Huang,Anthony K. H. Tung,Jun Yu*

Main category: cs.CL

TLDR: 该立场论文主张文本嵌入研究应超越表层语义，将隐含语义作为核心建模目标。


<details>
  <summary>Details</summary>
Motivation: 当前文本嵌入模型主要关注表层语义，而忽略了隐含语义（如语用、说话者意图和社会文化背景），导致其在需要解释性推理的任务中表现不佳。

Method: 通过试点研究展示现有模型在隐含语义任务上的局限性，并提出改进方向。

Result: 试点研究表明，即使是先进模型在隐含语义任务上表现仅略优于简单基线。

Conclusion: 呼吁研究社区转向更丰富、语言学基础更强的训练数据，设计评估深层语义理解的基准，并将隐含语义明确作为建模目标。
...
### [157] [DEAL: Disentangling Transformer Head Activations for LLM Steering](https://arxiv.org/abs/2506.08359)
*Li-Ming Zhan,Bo Liu,Zexin Lu,Chengqiang Xie,Jiannong Cao,Xiao-Ming Wu*

Main category: cs.CL

TLDR: 提出了一种基于因果归因的框架，用于识别Transformer中与行为相关的注意力头，通过VQ-AE量化注意力激活，评估行为相关性，并在实验中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 当前方法依赖表面线索或启发式规则，导致模块选择不准确或效果不佳，需要一种更原则性的方法。

Method: 训练VQ-AE量化注意力头的激活，划分行为相关与无关子空间，通过二元分类指标评估行为相关性。

Result: 在七个LLM和五个行为数据集上验证，方法在真实性干预任务中表现优越，且所选头部具有零样本泛化能力。
...
### [158] [CC-RAG: Structured Multi-Hop Reasoning via Theme-Based Causal Graphs](https://arxiv.org/abs/2506.08364)
*Jash Rajesh Parekh,Pengcheng Jiang,Jiawei Han*

Main category: cs.CL

TLDR: CC-RAG通过整合零样本三元组提取和主题感知图链，改进了RAG管道，以建模因果关系，提升LLM在专业领域中的因果推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有RAG管道将证据视为扁平上下文，缺乏建模真实因果依赖的结构，导致LLM在专业领域的因果推理能力不足。

Method: 提出CC-RAG方法，结合零样本三元组提取和主题感知图链，构建DAG并利用前向/后向链指导结构化答案生成。

Result: 在比特币价格波动和高雪氏病两个领域，CC-RAG在链相似性、信息密度和词汇多样性上优于标准RAG和零样本LLM。

Conclusion: 显式建模因果结构使LLM能生成更准确和可解释的响应，尤其在扁平检索失败的专业领域。
...
### [159] [Mitigating Posterior Salience Attenuation in Long-Context LLMs with Positional Contrastive Decoding](https://arxiv.org/abs/2506.08371)
*Zikai Xiao,Ziyang Wang,Wen Ma,Yan Zhang,Wei Shen,Yan Wang,Luqi Gong,Zuozhu Liu*

Main category: cs.CL

TLDR: 论文提出了一种无需训练的Positional Contrastive Decoding（PCD）方法，通过对比长文本和局部注意力生成的logits，缓解LLMs在长上下文中的性能退化问题，并在实验中取得了最优表现。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在长上下文窗口中存在性能退化问题，现有解决方案训练成本高，且统计行为和成本效益方法研究不足。

Method: 提出PCD方法，对比长文本注意力和局部注意力生成的logits，利用大规模短到长训练的优势。

Result: PCD有效缓解了注意力分数退化，在长上下文基准测试中达到最优性能。
...
### [160] [Draft-based Approximate Inference for LLMs](https://arxiv.org/abs/2506.08373)
*Kevin Galim,Ethan Ewer,Wonjun Kang,Minjae Lee,Hyung Il Koo,Kangwook Lee*

Main category: cs.CL

TLDR: 提出了一种利用小型草稿模型优化长上下文大语言模型（LLM）推理的新框架，包括SpecKV和SpecPC两种实现，显著提高了准确性和效率。
...
Method: 利用草稿模型预测token和KV对的重要性，提出SpecKV（优化KV缓存丢弃）和SpecPC（丢弃不重要提示token）。

Result: 实验表明，该方法在长上下文基准测试中准确性更高，同时保持内存、延迟和吞吐量的改进。

Conclusion: 草稿模型首次用于近似LLM推理加速，扩展了其传统用途，为高效推理提供了新方向。
...
### [161] [EIFBENCH: Extremely Complex Instruction Following Benchmark for Large Language Models](https://arxiv.org/abs/2506.08375)
*Tao Zou,Xinghua Zhang,Haiyang Yu,Minzheng Wang,Fei Huang,Yongbin Li*

Main category: cs.CL

TLDR: 论文提出了一个名为EIFBENCH的复杂指令遵循基准测试，用于更真实地评估大语言模型在多任务和约束环境下的表现，并提出了SegPO算法以优化模型性能。
...
Method: 设计了EIFBENCH基准测试，包含多任务场景和多种约束，并提出SegPO算法以提升模型的多任务处理能力。

Result: 评估显示现有大语言模型在复杂指令下表现差异显著，表明需要进一步优化。

Conclusion: EIFBENCH和SegPO为复杂场景下的模型评估和优化提供了有效工具，强调了持续改进的必要性。
...
### [162] [mSTEB: Massively Multilingual Evaluation of LLMs on Speech and Text Tasks](https://arxiv.org/abs/2506.08400)
...
TLDR: 论文提出了mSTEB基准，用于评估LLMs在多语言和多模态任务中的表现，发现低资源语言性能显著落后。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs在低资源语言中缺乏标准化评估基准的问题。

Method: 引入mSTEB基准，涵盖语言识别、文本分类、问答和翻译任务，评估多种LLMs。

Result: 高资源与低资源语言性能差距显著，非洲及美洲/大洋洲语言表现不佳。

Conclusion: 需更多投资以改善LLMs对低资源语言的覆盖和表现。
...
### [163] [TACTIC: Translation Agents with Cognitive-Theoretic Interactive Collaboration](https://arxiv.org/abs/2506.08403)
...
TLDR: 论文提出了一种名为TACTIC的认知理论驱动的多智能体翻译框架，通过模拟人类翻译的认知过程提升翻译质量，实验证明其性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在机器翻译中取得进展，但现有多智能体框架忽视了认知翻译研究的核心见解，限制了翻译潜力的发挥。

Method: TACTIC框架包含六个功能不同的智能体，分别模拟人类翻译的认知过程，如起草、优化、评估等，通过协作提升翻译质量。

Result: 在FLORES-200和WMT24基准测试中，TACTIC表现优于GPT-4.1和DeepSeek-R1，平均提升显著。

Conclusion: TACTIC通过认知理论驱动的多智能体协作，有效释放了大语言模型的翻译潜力，实现了最先进的性能。
...
### [164] [Large Language Models Have Intrinsic Meta-Cognition, but Need a Good Lens](https://arxiv.org/abs/2506.08410)
*Ziyang Ma,Qingyue Yuan,Zhenglin Wang,Deyu Zhou*

Main category: cs.CL

TLDR: 本文研究了LLM的元认知能力评估，提出AutoMeco框架和MIRA策略，以改进现有评估方法。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注LLM的认知错误检测能力，而对其元认知能力（如自我意识）的研究较少，这对LLM的可靠性至关重要。

Method: 提出AutoMeco框架评估现有元认知方法，并引入训练免费的MIRA策略优化评估。

Result: 在三个数学推理数据集和三个LLM上的实验表明，AutoMeco与Best-of-N验证相比具有合理性，MIRA能更好地评估LLM的元认知能力。
...
### [165] [Know-MRI: A Knowledge Mechanisms Revealer&Interpreter for Large Language Models](https://arxiv.org/abs/2506.08427)
...
TLDR: 论文提出了一种开源工具Know-MRI，用于系统分析大语言模型（LLMs）的内部知识机制，解决了现有解释方法输入输出格式不统一的问题。
...
Conclusion: Know-MRI为LLMs的知识机制分析提供了系统化解决方案，具有实际应用价值。
...
### [166] [CAF-I: A Collaborative Multi-Agent Framework for Enhanced Irony Detection with Large Language Models](https://arxiv.org/abs/2506.08430)
*Ziqi. Liu,Ziyang. Zhou,Mingxuan. Hu*

Main category: cs.CL

TLDR: CAF-I是一种基于LLM的多代理系统，通过多维分析和协作优化解决讽刺检测中的单视角限制、理解不足和可解释性差问题，实验表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM方法在讽刺检测中存在单视角限制、理解不足和可解释性差的问题，需要一种更全面的解决方案。

Method: CAF-I采用多代理系统（Context、Semantics、Rhetoric代理）进行多维分析，并通过决策代理和优化评估代理实现协作优化。

Result: CAF-I在基准数据集上实现了零样本SOTA性能，Macro-F1平均76.31，比之前最佳基线提升4.98。
...
### [167] [Low-resource domain adaptation while minimizing energy and hardware resource consumption](https://arxiv.org/abs/2506.08433)
*Hernán Maina,Nicolás Wolovick,Luciana Benotti*

Main category: cs.CL

TLDR: 研究探讨了不同数值精度和数据并行化策略对LLM训练速度和准确性的影响，旨在降低领域适应的计算成本。


<details>
  <summary>Details</summary>
Motivation: 降低领域适应的计算成本，使其在资源有限的环境中更可行。

Method: 评估不同数值精度和数据并行化策略对训练速度和模型准确性的影响。

Result: 研究结果为能源效率、可访问性或硬件资源有限的环境提供了实用指导。

Conclusion: 该研究为资源有限环境中的领域适应提供了可行的优化策略。
...
### [168] [Olica: Efficient Structured Pruning of Large Language Models without Retraining](https://arxiv.org/abs/2506.08436)
*Jiujun He,Huazhen Lin*

Main category: cs.CL

TLDR: 提出了一种无需重训练的LLM剪枝框架Olica，通过正交分解和线性校准压缩模型，保持精度。


<details>
  <summary>Details</summary>
Motivation: 现有剪枝方法需要大量计算和数据资源进行重训练，成本高昂。

Method: 利用PCA处理多头注意力层的矩阵乘积，并引入线性校准减少误差累积。

Result: 实验表明Olica在数据使用、GPU内存和运行时间上高效，性能优越。

Conclusion: Olica无需重训练即可高效压缩LLM，适用于实际应用。
...
### [169] [Detecting Harmful Memes with Decoupled Understanding and Guided CoT Reasoning](https://arxiv.org/abs/2506.08477)
*Fengjun Pan,Anh Tuan Luu,Xiaobao Wu*

Main category: cs.CL

TLDR: 论文提出U-CoT+框架，通过将视觉模因转换为文本描述，结合零样本思维链提示，实现高效、灵活且可解释的有害模因检测。
...
Method: 开发高保真模因转文本管道，结合人类制定的可解释指南，利用零样本思维链提示进行分类。

Result: 在七个基准数据集上验证了框架的有效性，展示了其在小规模LLM上的低资源需求和高可解释性。
...
### [170] [Efficient Context Selection for Long-Context QA: No Tuning, No Iteration, Just Adaptive-$k$](https://arxiv.org/abs/2506.08479)
*Chihiro Taguchi,Seiji Maekawa,Nikita Bhutani*

Main category: cs.CL

TLDR: 论文提出了一种自适应检索方法Adaptive-$k$，通过动态调整检索段落数量，解决了固定检索规模在开放领域问答中的效率问题，显著减少了计算资源消耗并保持了高准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如Self-RAG和Self-Route）在事实性问答中表现良好，但在聚合问答中因固定检索规模而效率低下或遗漏关键信息。需要一种无需额外调优或推理的自适应方法。

Method: 提出Adaptive-$k$检索方法，基于查询与候选段落相似度分数的分布动态选择段落数量，无需模型微调或额外推理。

Result: 在事实性和聚合问答基准测试中，Adaptive-$k$性能优于固定规模检索，减少90%的token消耗，同时保留70%的相关段落，提升了五种LCLM和两种嵌入模型的准确性。

Conclusion: 动态调整上下文规模能显著提升问答系统的效率和准确性，Adaptive-$k$为开放领域问答提供了一种简单有效的解决方案。
...
### [171] [Re-Thinking the Automatic Evaluation of Image-Text Alignment in Text-to-Image Models](https://arxiv.org/abs/2506.08480)
*Huixuan Zhang,Xiaojun Wan*

Main category: cs.CL

TLDR: 本文指出现有文本到图像生成评估框架的不足，提出改进建议。


<details>
  <summary>Details</summary>
Motivation: 研究文本到图像生成评估框架的可靠性，发现现有方法未能全面满足关键评估属性。

Method: 通过实证分析，检验主流评估框架在多样指标和模型中的表现。

Result: 发现现有评估框架未能完全满足可靠评估的关键属性。
...
### [172] [Fairness is Not Silence: Unmasking Vacuous Neutrality in Small Language Models](https://arxiv.org/abs/2506.08487)
*Sumanth Manduru,Carlotta Domeniconi*

Main category: cs.CL

TLDR: 该论文首次大规模审计了参数规模在0.5至50亿之间的指令调优小型语言模型（SLMs），揭示了其公平性与性能的关系、架构对偏见的影响以及量化压缩的权衡。


<details>
  <summary>Details</summary>
Motivation: 随着小型语言模型（SLMs）在资源受限设备上的快速部署，其伦理风险尚未被充分理解，因此需要对其公平性和性能进行系统评估。

Method: 使用BBQ基准测试，在零样本提示下评估了9个开源SLMs（来自Qwen 2.5、LLaMA 3.2、Gemma 3和Phi系列），分析其在模糊和明确上下文中的效用和公平性。

Result: 研究发现：1）性能与公平性可以共存（如Phi模型）；2）架构显著影响偏见（Qwen 2.5表现中性但可能虚假，LLaMA 3.2偏见更强）；3）量化压缩带来复杂权衡（如4-bit AWQ提升性能但增加偏见）。

Conclusion: 研究为资源受限环境中SLMs的负责任部署提供了实用指导，强调了公平性与效率的平衡对小企业和资源受限环境的重要性。
...
### [173] [EtiCor++: Towards Understanding Etiquettical Bias in LLMs](https://arxiv.org/abs/2506.08488)
*Ashutosh Dwivedi,Siddhant Shivdutt Singh,Ashutosh Modi*

Main category: cs.CL

TLDR: 论文介绍了EtiCor++语料库，用于评估LLMs对全球礼仪的理解和偏见，并揭示了LLMs对某些地区的固有偏见。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs的文化敏感性，特别是礼仪方面的理解和偏见，目前缺乏相关资源。

Method: 提出EtiCor++语料库，设计任务和指标评估LLMs的礼仪知识和偏见。
...
### [174] [Integration of Old and New Knowledge for Generalized Intent Discovery: A Consistency-driven Prototype-Prompting Framework](https://arxiv.org/abs/2506.08490)
*Xiao Wei,Xiaobao Wang,Ning Zhuang,Chenyang Wang,Longbiao Wang,Jianwu dang*

Main category: cs.CL

TLDR: 提出了一种基于一致性驱动的原型提示框架（GID），用于整合新旧知识，显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅关注无监督数据聚类，忽略了领域适应问题。

Method: 包括原型提示框架（转移旧知识）和分层一致性约束（学习新知识）。

Result: 实验表明，该方法显著优于基线，达到SOTA。
...
### [175] [DRAGged into Conflicts: Detecting and Addressing Conflicting Sources in Search-Augmented LLMs](https://arxiv.org/abs/2506.08500)
*Arie Cattan,Alon Jacovi,Ori Ram,Jonathan Herzig,Roee Aharoni,Sasha Goldshtein,Eran Ofek,Idan Szpektor,Avi Caciularu*

Main category: cs.CL

TLDR: 论文提出了一种新的知识冲突分类法，并引入了CONFLICTS基准，用于评估大语言模型在检索增强生成中处理冲突的能力。实验表明，现有模型表现不佳，但通过提示模型显式推理可改善结果。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成（RAG）中检索到的信息可能存在冲突，目前缺乏对模型如何处理这些冲突的系统研究。

Method: 提出知识冲突分类法，构建CONFLICTS基准，并通过实验评估模型表现。

Result: 大语言模型在处理冲突时表现不佳，但显式推理提示能显著提升结果质量。
...
### [176] [Werewolf: A Straightforward Game Framework with TTS for Improved User Engagement](https://arxiv.org/abs/2506.00160)
*Qihui Fan,Enfu Nan,Wenbo Li,Lei Lu,Pu Zhao,Yanzhi Wang*

Main category: cs.CL

TLDR: 本文提出了一种基于LLM的狼人杀游戏系统，通过优化的TTS模型提升兼容性和用户体验，认为随着LLM推理能力的增强，额外组件将变得不必要。


<details>
  <summary>Details</summary>
Motivation: 随着LLM推理和说服能力的提升，尤其是DeepSeek R1和V3模型的兴起，LLM代理在社交推理游戏（如狼人杀）中的应用潜力增强。

Method: 提出了一种新颖且简单的基于LLM的狼人杀系统，结合优化的TTS模型，以提升与多种LLM模型的兼容性和用户参与度。

Result: 系统通过优化的TTS模型和LLM推理能力，实现了更高的兼容性和用户体验。

Conclusion: 随着LLM推理能力的持续提升，未来在狼人杀等游戏中，额外的组件可能不再需要。
...
### [177] [CoMuMDR: Code-mixed Multi-modal Multi-domain corpus for Discourse paRsing in conversations](https://arxiv.org/abs/2506.08504)
*Divyaksh Shukla,Ritesh Baviskar,Dwijesh Gohil,Aniket Tiwari,Atul Shree,Ashutosh Modi*

Main category: cs.CL

TLDR: 论文介绍了CoMuMDR语料库，用于多模态多领域代码混合（印地语和英语）对话的话语解析，并展示了现有模型的性能不足。


<details>
  <summary>Details</summary>
Motivation: 当前话语解析数据集局限于单一领域的英语对话，缺乏多模态和代码混合的真实场景数据。

Method: 构建了包含音频和文本的多模态代码混合语料库CoMuMDR，并标注了九种话语关系。

Result: 现有SoTA模型在该语料库上表现不佳，凸显了多领域代码混合数据的挑战。

Conclusion: 需要开发更好的模型以应对多模态代码混合话语解析的现实需求。
...
### [178] [Efficient Post-Training Refinement of Latent Reasoning in Large Language Models](https://arxiv.org/abs/2506.08552)
*Xinyuan Wang,Dongjie Wang,Wangyang Ying,Haoyue Bai,Nanxu Gong,Sixun Dong,Kunpeng Liu,Yanjie Fu*

Main category: cs.CL

TLDR: 提出了一种轻量级后训练框架，通过对比推理反馈和残差嵌入细化来优化潜在推理轨迹，显著提升了推理准确性。


<details>
  <summary>Details</summary>
Motivation: 现有推理方法（如Chain-of-Thought）存在固定推理轨迹和额外标记开销的问题，而潜在推理虽能避免这些问题，但缺乏有效的后训练更新机制。

Method: 采用两种策略：1）对比推理反馈，通过比较强/弱基线优化嵌入方向；2）残差嵌入细化，通过梯度渐进整合实现稳定更新。

Result: 在五个推理基准测试中验证了框架有效性，MathQA上实现了5%的准确率提升。

Conclusion: 该框架为优化潜在推理提供了一种高效且可控的后训练方法。
...
### [179] [Being Strong Progressively! Enhancing Knowledge Distillation of Large Language Models through a Curriculum Learning Framework](https://arxiv.org/abs/2506.05695)
*Lingyuan Liu,Mengxiang Zhang*

Main category: cs.CL

TLDR: 提出了一种基于渐进式过载原则的课程学习框架（POCL），用于改进大型语言模型的知识蒸馏（KD），通过逐步增加训练样本难度，提升学生模型的稳定性和性能。


<details>
  <summary>Details</summary>
Motivation: 现有KD方法在训练过程中容易导致学生模型分布偏移，引发灾难性遗忘、模式崩溃等问题，需一种更稳定的训练框架。

Method: POCL框架包含难度测量器和训练调度器，逐步引入从易到难的样本，并应用温度逐渐升高的损失函数。

Result: 实验表明，POCL能显著提升学生模型性能，适用于多种KD方法和模型家族。

Conclusion: 通过结构化训练数据，POCL有效提升了KD过程的稳定性和模型性能。
...
### [180] [Neighbors and relatives: How do speech embeddings reflect linguistic connections across the world?](https://arxiv.org/abs/2506.08564)
*Tuukka Törö,Antti Suni,Juraj Šimko*

Main category: cs.CL

TLDR: 该研究利用机器学习方法分析语音嵌入，探索106种语言之间的关系，发现嵌入距离与传统语言距离测量方法一致，为大规模语言分析提供了新视角。


<details>
  <summary>Details</summary>
Motivation: 研究旨在通过机器学习方法补充传统的历史和类型学方法，以更高效地分析全球语言的多样性及其关系。

Method: 使用XLS-R自监督语言识别模型生成语音嵌入，并通过线性判别分析（LDA）聚类语言嵌入，与传统语言距离（如谱系、词汇和地理距离）进行比较。

Result: 嵌入距离与传统测量方法高度一致，能够捕捉全局和局部类型学模式，但也揭示了语言变化的动态性。

Conclusion: 该方法为语言变异的可扩展分析提供了潜力，未来可扩展至低资源语言并整合社会语言学变异，以更全面地理解语言多样性。
...
### [181] [CounselBench: A Large-Scale Expert Evaluation and Adversarial Benchmark of Large Language Models in Mental Health Counseling](https://arxiv.org/abs/2506.08584)
*Yahan Li,Jifan Yao,John Bosco S. Bunyi,Adam C. Frank,Angel Hwang,Ruishan Liu*

Main category: cs.CL

TLDR: CounselBench是一个用于评估大型语言模型（LLMs）在心理健康咨询场景中的表现的基准测试，包含专家评价和对抗性问题，发现LLMs在质量上优于人类治疗师，但存在安全隐患。
...
Method: 开发CounselBench基准，包含专家评价（CounselBench-EVAL）和对抗性问题（CounselBench-Adv），评估LLMs在多个维度的表现。
...
### [182] [Dense Retrievers Can Fail on Simple Queries: Revealing The Granularity Dilemma of Embeddings](https://arxiv.org/abs/2506.08592)
*Liyan Xu,Zhenlin Su,Mo Yu,Jiangnan Li,Fandong Meng,Jie Zhou*

Main category: cs.CL

TLDR: 本文研究了文本编码器在识别细粒度实体或事件时的局限性，提出了中文评估数据集CapRetrieval，并通过微调编码器提升了性能。
...
Result: 微调后编码器在CapRetrieval上表现最佳，但发现粒度困境问题。

Conclusion: 细粒度语义匹配仍需改进，公开了数据集、代码和模型。
...
### [183] [Hateful Person or Hateful Model? Investigating the Role of Personas in Hate Speech Detection by Large Language Models](https://arxiv.org/abs/2506.08593)
*Shuzhou Yuan,Ercong Nie,Mario Tawfelis,Helmut Schmid,Hinrich Schütze,Michael Färber*

Main category: cs.CL

TLDR: 本文研究了MBTI人格特质对大型语言模型（LLMs）在仇恨言论检测中的影响，发现人格提示会导致显著差异，需谨慎设计提示以确保公平性。
...
Method: 通过人类标注调查验证MBTI对标注行为的影响，并在四个开源LLMs中使用MBTI人格提示，评估其在三个仇恨言论数据集上的表现。

Result: 发现人格提示导致显著差异，包括与真实标签的不一致、人格间分歧以及逻辑层面的偏见。

Conclusion: 强调在设计LLM标注流程时需谨慎定义人格提示，以确保公平性和与人类价值观的一致性。
...
### [184] [RAISE: Enhancing Scientific Reasoning in LLMs via Step-by-Step Retrieval](https://arxiv.org/abs/2506.08625)
*Minhae Oh,Jeonghye Kim,Nakyung Lee,Donggeon Seo,Taeuk Kim,Jungwoo Lee*

Main category: cs.CL

TLDR: RAISE是一个分步检索增强框架，用于科学推理，通过问题分解、逻辑查询生成和逻辑检索三个步骤，显著优于其他基线方法。
...
Method: RAISE分为三个步骤：问题分解、逻辑查询生成和逻辑检索，从广泛语料库中检索逻辑相关文档。

Result: RAISE在科学推理基准测试中表现优于其他基线方法，检索的文档不仅领域知识相似，逻辑相关性也更强。

Conclusion: RAISE通过逻辑检索显著提升了科学推理的效果，优于传统方法。
...
### [185] [MEMETRON: Metaheuristic Mechanisms for Test-time Response Optimization of Large Language Models](https://arxiv.org/abs/2506.08643)
*Son The Nguyen,Theja Tulabandhula*

Main category: cs.CL

TLDR: MEMETRON是一个任务无关的框架，将LLM解码视为离散黑盒优化问题，通过混合元启发式算法高效发现高奖励响应。


<details>
  <summary>Details</summary>
Motivation: 现有LLM的解码策略（如贪婪搜索、采样或重排序）缺乏控制且未明确优化任务目标，需要一种更灵活的方法。

Method: MEMETRON利用混合元启发式算法（GENETRON和ANNETRON），结合奖励模型和LLM的上下文操作，无需模型重训练或梯度访问。

Result: 在人类偏好对齐任务中，MEMETRON显著优于标准解码和重排序方法。

Conclusion: MEMETRON提供了一种无需模型重训练即可改进对齐的通用框架。
...
### [186] [TableDreamer: Progressive and Weakness-guided Data Synthesis from Scratch for Table Instruction Tuning](https://arxiv.org/abs/2506.08646)
*Mingyu Zheng,Zhifan Feng,Jia Wang,Lanrui Wang,Zheng Lin,Yang Hao,Weiping Wang*

Main category: cs.CL

TLDR: TableDreamer是一个渐进式、弱点指导的数据合成框架，用于解决LLM在表格指令调优中数据多样性和效率不足的问题。
...
Method: 首先生成多样化的种子数据，然后根据新发现的弱点数据迭代探索输入空间，最终用于目标LLM的微调。

Result: 在10个表格基准测试中，TableDreamer将Llama3.1-8B-instruct的平均准确率提升11.62%，优于现有方法。

Conclusion: TableDreamer通过弱点指导的数据合成，显著提升了表格指令调优的效果。
...
### [187] [Summarization for Generative Relation Extraction in the Microbiome Domain](https://arxiv.org/abs/2506.08647)
*Oumaima El Khettari,Solen Quiniou,Samuel Chaffron*

Main category: cs.CL

TLDR: 生成式关系提取方法在肠道微生物组研究中表现初步成功，但BERT方法仍占优。
...
### [188] [RuleReasoner: Reinforced Rule-based Reasoning via Domain-aware Dynamic Sampling](https://arxiv.org/abs/2506.08672)
*Yang Liu,Jiaqi Li,Zilong Zheng*

Main category: cs.CL

TLDR: RuleReasoner是一种通过动态采样增强小推理模型（SRMs）在规则推理任务中表现的方法，显著优于大型推理模型（LRMs）。
...
Method: 提出RuleReasoner方法，利用动态采样更新不同领域的权重，结合强化学习进行训练。

Result: 在分布内和分布外任务中显著优于前沿大型模型，计算效率更高。

Conclusion: RuleReasoner证明了小模型在规则推理任务中的潜力，同时提高了效率和泛化能力。
...
### [189] [Brevity is the soul of sustainability: Characterizing LLM response lengths](https://arxiv.org/abs/2506.08686)
*Soham Poddar,Paramita Koley,Janardan Misra,Sanjay Podder,Navveen Balani,Niloy Ganguly,Saptarshi Ghosh*

Main category: cs.CL

TLDR: 论文研究了大型语言模型（LLMs）推理过程中的能源消耗问题，提出通过输出压缩和提示工程策略减少响应长度，实现25-60%的能源优化。
...
Method: 首先对12种解码器LLMs在5个数据集上进行基准测试，分析响应长度；然后定义LLM响应中的六类信息，并通过提示工程策略优化响应长度。

Result: 实验表明，通过针对性提示策略可减少响应长度25-60%，同时保持响应质量。

Conclusion: 提示工程策略能有效优化LLMs的能源消耗，为未来研究提供了实用方向。
...
### [190] [ClimateViz: A Benchmark for Statistical Reasoning and Fact Verification on Scientific Charts](https://arxiv.org/abs/2506.08700)
*Ruiran Su,Jiasheng Si,Zhijiang Guo,Janet B. Pierrehumbert*

Main category: cs.CL

TLDR: ClimateViz是首个大规模科学图表事实核查基准，包含49,862个与2,896个可视化图表关联的声明，标注为支持、反驳或信息不足。评估显示当前多模态模型在图表推理上表现不佳，最佳模型准确率仅76.2-77.8%，远低于人类水平（89.3-92.7%）。


<details>
  <summary>Details</summary>
Motivation: 科学事实核查主要关注文本和表格，忽略了科学图表这一关键定量证据展示方式。

Method: 引入ClimateViz数据集，包含专家标注的图表和声明，评估多模态语言模型在零样本和少样本设置下的表现。

Result: 当前模型在图表推理上表现不佳，最佳模型准确率远低于人类水平，但解释增强输出能提升部分模型性能。

Conclusion: 科学图表事实核查具有挑战性，当前模型仍需改进。数据集和代码已公开。
...
### [191] [ConfPO: Exploiting Policy Model Confidence for Critical Token Selection in Large Language Model Preference Optimization](https://arxiv.org/abs/2506.08712)
*Hee Suk Yoon,Eunseop Yoon,Mark A. Hasegawa-Johnson,Sungwoong Kim,Chang D. Yoo*

Main category: cs.CL

TLDR: ConfPO是一种专注于偏好关键令牌的轻量级偏好学习方法，无需辅助模型或额外计算，优于传统均匀调整方法。


<details>
  <summary>Details</summary>
Motivation: 传统直接对齐算法（如DPO）对所有令牌进行均匀调整，可能导致效率低下或奖励滥用问题，而ConfPO旨在通过针对性优化提升对齐质量。

Method: ConfPO基于训练策略的置信度识别偏好关键令牌，并仅优化这些令牌，避免使用额外模型或计算资源。

Result: 在AlpacaEval 2和Arena-Hard等基准测试中，ConfPO表现优于传统方法，且无额外计算开销。

Conclusion: ConfPO是一种高效、轻量的偏好学习方法，显著提升对齐质量并避免过优化问题。
...
### [192] [Explainable Compliance Detection with Multi-Hop Natural Language Inference on Assurance Case Structure](https://arxiv.org/abs/2506.08713)
*Fariz Ikhwantri,Dusica Marijan*

Main category: cs.CL

TLDR: 提出了一种基于自然语言推理（NLI）的合规检测方法EXCLAIM，通过多跳推理实现可解释和可追溯的合规检测，并利用大型语言模型生成保证案例。


<details>
  <summary>Details</summary>
Motivation: 解决保证案例验证中的复杂法律和技术文本、模型解释需求以及数据访问限制等问题。

Method: 将保证案例的声明-论证-证据结构建模为多跳推理，利用大型语言模型生成案例，并引入覆盖率和结构一致性指标。

Result: 通过GDPR要求的案例研究验证了方法的有效性，展示了NLI在自动化合规检测中的潜力。

Conclusion: NLI方法为自动化合规检测提供了可行且高效的解决方案。
...
### [193] [Multi-Teacher Language-Aware Knowledge Distillation for Multilingual Speech Emotion Recognition](https://arxiv.org/abs/2506.08717)
*Mehedi Hasan Bijoy,Dejan Porjazovski,Tamás Grósz,Mikko Kurimo*

Main category: cs.CL

TLDR: 提出了一种语言感知的多教师知识蒸馏方法，用于多语言语音情感识别（SER），在英语、芬兰语和法语上取得了先进性能。


<details>
  <summary>Details</summary>
Motivation: 尽管单语言SER取得了进展，但构建多语言系统仍具挑战性。目标是训练一个能处理多语言SER的单一模型。

Method: 利用Wav2Vec2.0作为单语言教师模型的基础，通过知识蒸馏将其整合到一个多语言学生模型中。
...
### [194] [Improved LLM Agents for Financial Document Question Answering](https://arxiv.org/abs/2506.08726)
*Nelvin Tan,Zian Seng,Liang Zhang,Yu-Ching Shih,Dong Yang,Amol Salunkhe*

Main category: cs.CL

TLDR: 论文研究了大型语言模型（LLMs）在金融文档数值问答任务中的表现，提出改进的批评代理和计算器代理，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: LLMs在金融文档数值问答任务中表现不佳，尤其是缺乏真实标签时，批评代理性能下降。

Method: 提出改进的批评代理和计算器代理，分析其交互对性能的影响。

Result: 新方法优于现有技术（程序思维），且更安全。

Conclusion: 改进的批评代理和计算器代理在无真实标签时表现更优，交互分析揭示了性能提升的关键因素。
...
### [195] [Societal AI Research Has Become Less Interdisciplinary](https://arxiv.org/abs/2506.08738)
*Dror Kris Markus,Fabrizio Gilardi,Daria Stetsenko*

Main category: cs.CL

TLDR: 研究分析了2014-2024年间ArXiv上的10万篇AI论文，发现跨学科团队虽更倾向关注社会伦理问题，但纯计算机科学团队在此领域的贡献正快速增长。
...
Method: 开发分类器识别论文中的社会内容，分析跨学科与纯技术团队对社会伦理问题的关注程度。

Result: 纯计算机科学团队越来越多地整合社会伦理问题，涉足公平、安全、医疗等多个领域。

Conclusion: 研究挑战了跨学科团队主导社会伦理AI的假设，并引发对技术团队主导的伦理研究及其对AI安全与治理影响的思考。
...
### [196] [Towards Secure and Private Language Models for Nuclear Power Plants](https://arxiv.org/abs/2506.08746)
*Muhammad Anwar,Mishca de Costa,Issam Hammad,Daniel Lau*

Main category: cs.CL

TLDR: 本文介绍了一种专用于核能领域的语言模型，基于公开的CANDU教材构建，采用紧凑的Transformer架构，单GPU训练以保护敏感数据。模型在专业词汇捕捉上表现良好，但生成文本有时缺乏语法连贯性。


<details>
  <summary>Details</summary>
Motivation: 开发一种符合核能领域严格数据安全和保密标准的内部语言模型解决方案。

Method: 基于公开的Essential CANDU教材，采用紧凑的Transformer架构，单GPU训练。

Result: 模型能捕捉专业核能词汇，但生成文本的语法连贯性有待提升。
...
### [197] [Unlocking the Potential of Large Language Models in the Nuclear Industry with Synthetic Data](https://arxiv.org/abs/2506.08750)
*Muhammad Anwar,Daniel Lau,Mishca de Costa,Issam Hammad*

Main category: cs.CL

TLDR: 论文探讨了如何通过合成数据生成解决核工业中非结构化文本数据的可用性问题，以支持大型语言模型的应用。


<details>
  <summary>Details</summary>
Motivation: 核工业中存在大量非结构化文本数据，但这些数据无法直接用于需要结构化问答对的LLM任务。合成数据可以填补这一空白。

Method: 利用LLM分析文本、提取关键信息、生成相关问题，并评估合成数据集的质量。

Result: 合成数据能够将现有文本转化为可用的问答对，支持LLM在核工业中的应用。

Conclusion: 合成数据为核工业提供了改进信息检索、知识共享和决策支持的新途径。
...
### [198] [Factors affecting the in-context learning abilities of LLMs for dialogue state tracking](https://arxiv.org/abs/2506.08753)
*Pradyoth Hegde,Santosh Kesiraju,Jan Švec,Šimon Sedláček,Bolaji Yusuf,Oldřich Plchot,Deepak K T,Jan Černocký*

Main category: cs.CL

TLDR: 研究探讨了上下文学习（ICL）在对话状态跟踪（DST）中的应用，并分析了影响其效果的因素。


<details>
  <summary>Details</summary>
Motivation: 探索ICL在DST中的适用性，并研究如何通过演示选择和提示上下文优化性能。

Method: 使用基于句子嵌入的k近邻方法选择演示，结合模板输入到LLM中，系统分析演示选择和提示上下文的影响。

Result: 在MultiWoZ2.4数据集上测试，发现ICL对DST任务的有效性，并提供了相关洞察。
...
### [199] [Enhancing Accuracy and Maintainability in Nuclear Plant Data Retrieval: A Function-Calling LLM Approach Over NL-to-SQL](https://arxiv.org/abs/2506.08757)
*Mishca de Costa,Muhammad Anwar,Dave Mercier,Mark Randall,Issam Hammad*

Main category: cs.CL

TLDR: 论文提出了一种基于函数调用的大型语言模型（LLM）替代传统自然语言到SQL（NL-to-SQL）的方法，以提高核电厂数据查询的准确性和安全性。


<details>
  <summary>Details</summary>
Motivation: 传统NL-to-SQL方法在核电厂等关键系统中存在验证困难和查询生成复杂的问题，可能导致不准确和信任缺失。

Method: 通过预定义一组经过审核的、特定用途的函数来封装SQL逻辑，避免直接生成SQL查询，确保查询经过专家验证和优化。

Result: 研究表明，函数调用方法在准确性和可维护性上优于直接NL-to-SQL生成。

Conclusion: 该方法在用户易用性和操作安全性之间取得了平衡，为关键系统提供了稳健的数据检索框架。
...
### [200] [AraReasoner: Evaluating Reasoning-Based LLMs for Arabic NLP](https://arxiv.org/abs/2506.08768)
*Ahmed Hasanaath,Aisha Alansari,Ahmed Ashraf,Chafik Salmane,Hamzah Luqman,Saad Ezzini*

Main category: cs.CL

TLDR: 该论文对大型语言模型（LLMs）在阿拉伯语数据上的性能进行了全面评估，重点关注DeepSeek模型，发现少量示例和微调策略显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语因其丰富的形态、多样的方言和复杂脚本，LLMs在该语言上的表现尚未充分研究，论文旨在填补这一空白。

Method: 通过零样本、少样本和微调策略，在15个阿拉伯语NLP任务上评估LLMs，特别关注DeepSeek模型。

Result: 少量示例显著提升分类任务性能；DeepSeek在复杂推理任务中优于GPT o4-mini；LoRA微调进一步提高了模型表现。

Conclusion: 论文展示了LLMs在阿拉伯语任务中的潜力，并提供了优化性能的有效策略。
...
### [201] [The impact of fine tuning in LLaMA on hallucinations for named entity extraction in legal documentation](https://arxiv.org/abs/2506.08827)
*Francisco Vargas,Alejandro González Coene,Gaston Escalante,Exequiel Lobón,Manuel Pulido*

Main category: cs.CL

TLDR: 论文提出了一种从法律文档中提取交通事故信息的两步法：文本分割和实体提取，比较了传统方法和基于语义搜索的方法，并展示了微调后LLaMA模型性能的提升。


<details>
  <summary>Details</summary>
Motivation: 从法律文档中提取交通事故信息（如伤残百分比和赔偿金额）对保险公司成本量化至关重要，但现有方法效率低且易出错。

Method: 采用两步法：1) 文本分割（正则表达式或语义搜索）；2) 实体提取（使用LLaMA-2、LLaMA-3和GPT-4 Turbo等模型，部分模型通过LoRA微调）。

Result: 基于语义搜索和LLMs的方法显著优于传统方法（39.5%准确率）。微调后LLaMA-2 70B达到79.4%，LLaMA-3 8B未微调即达76.6%，GPT-4 Turbo最高（86.1%）。

Conclusion: 语义搜索结合LLMs显著提升信息提取准确率，模型微调和新型模型（如LLaMA-3）进一步优化性能。
...
### [202] [Advancing STT for Low-Resource Real-World Speech](https://arxiv.org/abs/2506.08836)
*Flavio D'Intino,Hans-Peter Hutter*

Main category: cs.CL

TLDR: 论文介绍了SRB-300数据集，一个300小时的瑞士德语语音语料库，用于改进低资源语言的语音转文本模型。通过微调Whisper模型，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 瑞士德语缺乏标准化书写形式，现有数据集在真实对话场景中表现不佳，需要改进语音转文本模型的鲁棒性。

Method: 使用SRB-300数据集微调多个OpenAI Whisper模型。

Result: 微调后模型性能显著提升，WER降低19%-33%，BLEU分数提高8%-40%，最佳模型WER为17.1%，BLEU为74.8。

Conclusion: SRB-300数据集和微调方法为瑞士德语及其他低资源语言的语音转文本系统提供了有效解决方案。
...
### [203] [AdversariaL attacK sAfety aLIgnment(ALKALI): Safeguarding LLMs through GRACE: Geometric Representation-Aware Contrastive Enhancement- Introducing Adversarial Vulnerability Quality Index (AVQI)](https://arxiv.org/abs/2506.08885)
*Danush Khanna,Krishna Kumar,Basab Ghosh,Vinija Jain,Vasu Sharma,Aman Chadha,Amitava Das*

Main category: cs.CL

TLDR: 论文揭示了LLM对抗性威胁的几何盲点，提出了ALKALI基准和GRACE防御框架，显著降低了攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 当前防御措施无法快速适应LLM对抗性威胁的升级，尤其是对抗性提示利用潜在伪装绕过表面防御。

Method: 提出ALKALI基准评估21种LLM，并设计GRACE框架，通过潜在空间正则化增强对齐。

Result: GRACE实现了高达39%的攻击成功率降低，并引入AVQI指标量化潜在对齐失败。

Conclusion: GRACE和AVQI为LLM安全提供了新的防御和评估工具，揭示了潜在伪装的结构性漏洞。
...
### [204] [PlantBert: An Open Source Language Model for Plant Science](https://arxiv.org/abs/2506.08897)
*Hiba Khey,Amine Lakhder,Salma Rouichi,Imane El Ghabi,Kamal Hejjaoui,Younes En-nahli,Fahd Kalloubi,Moez Amri*

Main category: cs.CL

TLDR: PlantBert是一个基于DeBERTa架构的高性能开源语言模型，专为植物胁迫响应文献设计，结合了规则增强的语言后处理和本体基础实体规范化，填补了农业NLP的空白。
...
Method: 基于DeBERTa架构，结合专家标注的摘要语料库进行微调，并采用规则增强的语言后处理和本体基础实体规范化。

Result: PlantBert在实体类型上表现出强大的泛化能力，适用于低资源科学领域的稳健领域适应。

Conclusion: PlantBert为农业NLP提供了可扩展且可复制的框架，推动了植物科学领域的数据驱动系统发展。
...
### [205] [From Legal Texts to Defeasible Deontic Logic via LLMs: A Study in Automated Semantic Analysis](https://arxiv.org/abs/2506.08899)
*Elias Horner,Cristinel Mateis,Guido Governatori,Agata Ciabattoni*

Main category: cs.CL

TLDR: 提出了一种利用大语言模型（LLMs）自动化分析法律文本语义的新方法，将其转化为可辩解的规范逻辑（DDL）形式表示。


<details>
  <summary>Details</summary>
Motivation: 旨在通过自动化方法解决法律文本语义分析的复杂性问题，提高法律信息处理的效率和可扩展性。

Method: 采用结构化流程，将复杂的规范性语言分解为原子片段，提取规范规则，并评估其语法和语义一致性。评估了多种LLM配置，包括提示工程、微调模型和多阶段流程。

Result: 实验结果表明，机器生成的规范化与专家手工制作的规范化具有良好的一致性，特别是通过有效提示的LLMs在可扩展法律信息学中表现突出。

Conclusion: LLMs在有效提示下能够显著提升法律信息学的可扩展性，为自动化法律分析提供了有前景的解决方案。
...
### [206] [Dialect Normalization using Large Language Models and Morphological Rules](https://arxiv.org/abs/2506.08907)
*Antonios Dimakis,John Pavlopoulos,Antonios Anastasopoulos*

Main category: cs.CL

TLDR: 论文提出了一种结合规则和大型语言模型的方法，用于低资源语言的方言标准化，无需平行数据，并在希腊方言上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 解决低资源语言（包括高资源语言的方言）在自然语言理解系统中的困难，通过方言标准化提升下游工具的使用效果。

Method: 结合基于规则的语言学转换和大型语言模型（LLMs）的少样本提示，无需平行数据。

Result: 在希腊方言数据集上验证了方法的有效性，发现之前的研究仅依赖表层语言信息，而新方法能保留更多语义信息。

Conclusion: 提出的方法为方言标准化提供了一种有效途径，同时揭示了语义信息在方言研究中的重要性。
...
### [207] [PropMEND: Hypernetworks for Knowledge Propagation in LLMs](https://arxiv.org/abs/2506.08920)
*Zeyu Leo Liu,Greg Durrett,Eunsol Choi*

Main category: cs.CL

TLDR: PropMEND是一种基于超网络的方法，通过元学习修改梯度以促进知识传播，解决了大语言模型知识编辑后无法推理的问题。
...
Method: 扩展MEND的元目标，通过超网络修改梯度更新，使模型能回答涉及注入知识的多跳问题。

Result: 在RippleEdit数据集上表现优异，多跳问题准确率提升近2倍；在Controlled RippleEdit数据集上仍优于现有方法，但性能差距缩小。

Conclusion: PropMEND在知识传播方面表现优越，但需进一步研究以扩展到更广泛的关系类型。
...
### [208] [Can A Gamer Train A Mathematical Reasoning Model?](https://arxiv.org/abs/2506.08935)
*Andrew Shin*

Main category: cs.CL

TLDR: 论文提出了一种在单张普通游戏GPU上训练高性能数学推理模型的方法，通过结合强化学习和内存优化技术，显著降低了计算资源需求。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型（LLMs）在数学推理等任务中表现出色，但其训练通常需要昂贵的计算资源。本文旨在证明，通过优化技术，可以在资源有限的环境中实现高性能。

Method: 结合强化学习和内存优化技术，在单张RTX 3080 Ti（16GB内存）上训练一个1.5B参数的数学推理模型。

Result: 该模型在数学推理基准测试中表现优于或接近更大规模的模型，且资源需求显著降低。

Conclusion: 研究结果表明，高性能数学推理模型无需依赖大规模基础设施，为AI研究提供了更广泛的可行性。
...
### [209] [FaithfulRAG: Fact-Level Conflict Modeling for Context-Faithful Retrieval-Augmented Generation](https://arxiv.org/abs/2506.08938)
*Qinggang Zhang,Zhishang Xiang,Yilin Xiao,Le Wang,Junhui Li,Xinrun Wang,Jinsong Su*

Main category: cs.CL

TLDR: FaithfulRAG提出了一种新框架，通过显式建模LLM参数知识与检索上下文之间的差异，解决知识冲突问题，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强LLM在知识冲突时生成不忠实输出，现有方法通过强制抑制模型知识实现忠实性，但损害了模型内部知识结构。

Method: FaithfulRAG在事实层面识别冲突知识，设计自思考过程，让LLM推理并整合冲突事实后再生成响应。
...
### [210] [Can LLMs Ground when they (Don't) Know: A Study on Direct and Loaded Political Questions](https://arxiv.org/abs/2506.08952)
*Clara Lachenmaier,Judith Sieker,Sina Zarrieß*

Main category: cs.CL

TLDR: 论文研究了大型语言模型（LLMs）在政治领域中如何管理共同知识，特别是在处理直接知识问题和预设错误信息的引导性问题时的表现。


<details>
  <summary>Details</summary>
Motivation: 探讨LLMs在面对错误信息时是否能主动纠正用户信念，以及其政治偏见如何影响这一能力。

Method: 通过评估LLMs回答直接知识问题和引导性问题的能力，分析其是否主动纠正错误信念。

Result: 研究发现LLMs在纠正错误用户信念和进行共同知识管理方面存在显著挑战。

Conclusion: LLMs在政治领域的信息纠错能力不足，可能加剧错误信息的传播。
...
### [211] [Pre-trained Language Models Learn Remarkably Accurate Representations of Numbers](https://arxiv.org/abs/2506.08966)
*Marek Kadlčík,Michal Štefánik,Timothee Mickus,Michal Spiegel,Josef Kuchař*

Main category: cs.CL

TLDR: 预训练语言模型（LMs）在算术运算中容易出错，现有方法难以准确探测数值嵌入。本文提出一种新探测技术，能近乎完美地从嵌入中解码数值，证明LMs在预训练后能高精度表示数字，并通过对齐嵌入模式减少算术错误。


<details>
  <summary>Details</summary>
Motivation: 现有方法未能有效探测LMs中数值嵌入的正弦模式结构，导致算术错误。本文旨在改进探测技术，验证LMs是否能在预训练后精确表示数字。

Method: 提出一种新型探测技术，从输入嵌入中解码数值，并验证其准确性。通过分析嵌入精度与算术错误的关系，进一步调整嵌入模式以减少错误。

Result: 新探测技术能近乎完美地从多种开源LMs的嵌入中解码数值，证明LMs能高精度表示数字。嵌入精度与算术错误高度相关，调整嵌入模式可减少错误。

Conclusion: LMs在预训练后能精确表示数字，改进的探测技术揭示了嵌入精度与算术错误的关系，调整嵌入模式可有效减少错误。
...
### [212] [Atomic-to-Compositional Generalization for Mobile Agents with A New Benchmark and Scheduling System](https://arxiv.org/abs/2506.08972)
*Yuan Guo,Tingjia Miao,Zheng Wu,Pengzhou Cheng,Ming Zhou,Zhuosheng Zhang*

Main category: cs.CL

TLDR: UI-NEXUS是一个评估移动代理在组合任务上表现的基准，提出了AGENT-NEXUS系统以提升任务成功率。
...
Method: 提出UI-NEXUS基准，评估三类组合操作；开发AGENT-NEXUS系统，动态分解长任务为原子子任务。

Result: 现有代理在组合任务上表现不佳，AGENT-NEXUS提升任务成功率24%至40%。

Conclusion: AGENT-NEXUS有效提升移动代理在组合任务上的表现，且不显著增加计算开销。
...
### [213] [FROST-EMA: Finnish and Russian Oral Speech Dataset of Electromagnetic Articulography Measurements with L1, L2 and Imitated L2 Accents](https://arxiv.org/abs/2506.08981)
*Satu Hopponen,Tomi Kinnunen,Alexandre Nikolaev,Rosa González Hautamäki,Lauri Tavi,Einar Meister*

Main category: cs.CL

TLDR: 介绍了FROST-EMA语料库，包含18名双语者的母语、第二语言及模仿第二语言的语音数据，支持语音和技术研究。
...
Method: 构建FROST-EMA语料库，并进行两个案例研究：自动说话人验证系统的性能分析和发音模式分析。

Result: 语料库为研究提供了数据支持，案例研究展示了语言变异性对技术和语音的影响。

Conclusion: FROST-EMA语料库为语言变异性研究提供了新工具，案例研究验证了其应用潜力。
...
### [214] [Naturalistic Language-related Movie-Watching fMRI Task for Detecting Neurocognitive Decline and Disorder](https://arxiv.org/abs/2506.08986)
*Yuejiao Wang,Xianmin Gong,Xixin Wu,Patrick Wong,Hoi-lam Helene Fung,Man Wai Mak,Helen Meng*

Main category: cs.CL

TLDR: 该论文提出了一种基于语言相关fMRI任务的新方法，用于早期检测老年认知衰退和神经认知障碍（NCD），并在97名非痴呆中国老年人中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 早期检测对预防和减缓老年人群中常见的神经认知障碍（NCD）进展至关重要，语言相关fMRI可能是一种有前景的检测方法。

Method: 研究设计了一种自然语言相关的fMRI任务，并结合机器学习和人口统计学特征（年龄、性别、教育年限）进行分类分析。

Result: 基于fMRI特征和人口统计学的机器学习模型在分类认知状态（正常vs衰退）时，平均AUC达到0.86，特征定位显示关键脑区与语言处理相关。
...
### [215] [Employing self-supervised learning models for cross-linguistic child speech maturity classification](https://arxiv.org/abs/2506.08999)
*Theo Zhang,Madurya Suresh,Anne S. Warlaumont,Kasia Hitczenko,Alejandrina Cristia,Margaret Cychosz*

Main category: cs.CL

TLDR: 论文提出了一种名为SpeechMaturity的新数据集，用于改进儿童语音分类任务，通过大规模多语言样本训练模型，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 儿童语音识别系统因训练数据少和儿童语音特殊性而表现不佳，需更有效的数据集和方法。

Method: 使用SpeechMaturity数据集（包含242,004个标注语音样本）训练Transformer模型，分类儿童发声类型（如哭、笑、成熟/不成熟语音）。

Result: 模型性能优于现有方法，分类准确率接近人类水平，且在不同环境中表现稳健。
...
### [216] [SWE-Flow: Synthesizing Software Engineering Data in a Test-Driven Manner](https://arxiv.org/abs/2506.09003)
*Lei Zhang,Jiaxi Yang,Min Yang,Jian Yang,Mouxiang Chen,Jiajun Zhang,Zeyu Cui,Binyuan Hui,Junyang Lin*

Main category: cs.CL

TLDR: SWE-Flow是一个基于测试驱动开发（TDD）的数据合成框架，通过单元测试自动推断增量开发步骤，生成结构化开发计划和可验证的TDD任务。
...
Method: 构建运行时依赖图（RDG）捕捉函数交互，生成逐步开发计划，包括部分代码库、单元测试和代码修改。

Result: 从GitHub项目中生成16,061个训练实例和2,020个测试实例，实验表明该数据集显著提升TDD编码性能。

Conclusion: SWE-Flow为TDD研究提供了高质量数据和工具，代码和数据集已开源。
...
### [217] [UD-KSL Treebank v1.3: A semi-automated framework for aligning XPOS-extracted units with UPOS tags](https://arxiv.org/abs/2506.09009)
*Hakyung Sung,Gyu-Ho Shin,Chanyoung Lee,You Kyung Sung,Boo Kyung Jung*

Main category: cs.CL

TLDR: 研究提出了一种半自动化框架，用于将XPOS序列与UPOS类别对齐，扩展了L2韩语语料库，并验证了对齐数据对形态句法分析的积极影响。


<details>
  <summary>Details</summary>
Motivation: 扩展L2韩语的通用依存标注研究，提高标注一致性和分析准确性。

Method: 引入半自动化框架对齐XPOS与UPOS，扩展语料库，并通过实验验证对齐效果。

Result: 对齐数据提高了标注一致性，并提升了形态句法标注和依存分析的准确性。

Conclusion: XPOS-UPOS对齐对L2韩语形态句法分析具有显著改进作用。
...
### [218] [Learning to Reason Across Parallel Samples for LLM Reasoning](https://arxiv.org/abs/2506.09014)
*Jianing Qi,Xi Ye,Hao Tang,Zhigang Zhu,Eunsol Choi*

Main category: cs.CL

TLDR: 提出了一种新方法SSA，通过训练一个小型LLM聚合多个样本答案，优于其他测试时扩展方法。


<details>
  <summary>Details</summary>
Motivation: 利用多样本答案的聚合提升大型语言模型在数学领域的性能。

Method: 训练一个紧凑型LLM（SSA），通过强化学习优化答案准确性，聚合多个样本答案。

Result: 在多个推理数据集上，SSA优于基于奖励模型的重新排序等方法，并展现出良好的泛化能力。

Conclusion: SSA方法能高效利用黑盒模型输出，提升性能且具有通用性。
...
### [219] [Comparing human and LLM proofreading in L2 writing: Impact on lexical and syntactic features](https://arxiv.org/abs/2506.09021)
*Hakyung Sung,Karla Csuros,Min-Chang Sung*

Main category: cs.CL

TLDR: 研究比较了人类和LLM校对在第二语言写作中的词汇和句法干预效果，发现两者均能提升词汇连贯性，但LLM更倾向于生成性修改，且三种LLM模型结果高度一致。


<details>
  <summary>Details</summary>
Motivation: 探讨人类与LLM校对在提升第二语言写作可读性方面的差异及LLM模型间的一致性。

Method: 比较人类和三种LLM（ChatGPT-4o、Llama3.1-8b、Deepseek-r1-8b）对相同文本的校对效果，分析词汇和句法变化。

Result: 人类和LLM校对均提升词汇连贯性，但LLM更倾向于生成多样词汇和复杂句法；三种LLM结果高度一致。

Conclusion: LLM校对在词汇和句法修改上更具生成性，且不同模型间表现一致，可能为第二语言写作提供高效支持。
...
### [220] [Router-R1: Teaching LLMs Multi-Round Routing and Aggregation via Reinforcement Learning](https://arxiv.org/abs/2506.09033)
*Haozhen Zhang,Tao Feng,Jiaxuan You*

Main category: cs.CL

TLDR: Router-R1是一个基于强化学习的框架，通过动态路由和聚合多个LLM来解决复杂任务，优于现有基线。
...
Method: Router-R1将路由和聚合建模为序列决策过程，利用LLM的推理能力交替进行内部思考和动态模型调用。

Result: 在七个基准测试中，Router-R1表现优于基线，同时保持泛化能力和成本管理。

Conclusion: Router-R1为多LLM路由和性能-成本权衡优化提供了有效解决方案。
...
### [221] [Same Task, Different Circuits: Disentangling Modality-Specific Mechanisms in VLMs](https://arxiv.org/abs/2506.09047)
*Yaniv Nikankin,Dana Arad,Yossi Gandelsman,Yonatan Belinkov*

Main category: cs.CL

TLDR: 研究发现视觉语言模型（VLMs）在文本任务上表现优于视觉任务，通过分析任务特定计算子图（circuits）发现模态间差异主要在于数据处理位置。通过将视觉数据表征从后期层修补到早期层，性能差距缩小了三分之一。
...
Method: 识别并比较不同模态的任务特定计算子图（circuits），分析数据处理位置差异，并提出将后期层视觉表征修补到早期层的干预方法。
...
### [222] [TIP-Search: Time-Predictable Inference Scheduling for Market Prediction under Uncertain Load](https://arxiv.org/abs/2506.08026)
*Xibai Wang*

Main category: cs.AI

TLDR: TIP-Search是一种时间可预测的推理调度框架，用于在不确定工作负载下进行实时市场预测，旨在满足高延迟需求并提高预测准确性。


<details>
  <summary>Details</summary>
Motivation: 高频率金融系统对延迟有严格要求，需要一种能够在动态工作负载下满足任务截止时间并最大化预测准确性的方法。

Method: TIP-Search通过离线分析延迟和泛化性能，动态选择深度学习模型，无需依赖显式输入域标签。

Result: 在三个真实数据集上测试，TIP-Search比静态基线方法准确率提升8.5%，且100%满足截止时间。

Conclusion: TIP-Search在不确定环境下实现了高效、低延迟的金融推理。
...
### [223] [Cognitive Weave: Synthesizing Abstracted Knowledge with a Spatio-Temporal Resonance Graph](https://arxiv.org/abs/2506.08098)
*Akash Vishwakarma,Hojin Lee,Mohith Suresh,Priyam Shankar Sharma,Rahul Vishwakarma,Sparsh Gupta,Yuvraj Anupam Chauhan*

Main category: cs.AI

TLDR: Cognitive Weave是一种新型记忆框架，通过多层时空共振图（STRG）管理信息，显著提升了任务完成率和查询效率。


<details>
  <summary>Details</summary>
Motivation: 现有记忆系统在结构灵活性、时间感知和高级洞察合成方面存在局限，需要更先进的记忆架构。

Method: 采用多层的时空共振图（STRG）和语义丰富的洞察粒子（IPs），通过语义预言接口（SOI）动态丰富信息。

Result: 实验显示，Cognitive Weave在任务完成率上平均提升34%，查询延迟降低42%。

Conclusion: Cognitive Weave为LLM记忆系统提供了高效解决方案，并探讨了其伦理问题和未来研究方向。
...
### [224] [SOP-Bench: Complex Industrial SOPs for Evaluating LLM Agents](https://arxiv.org/abs/2506.08119)
...
TLDR: 论文提出了SOP-Bench基准测试，用于评估LLM在复杂工业流程中的表现，发现当前LLM代理能力与真实需求存在显著差距。


<details>
  <summary>Details</summary>
Motivation: LLM在复杂工业流程中表现不佳，缺乏公开的基准测试来评估其能力。

Method: 提出合成数据生成框架，创建SOP-Bench基准测试，并评估两种代理架构。

Result: Function-Calling和ReAct代理的平均成功率分别为27%和48%，工具调用错误率高。

Conclusion: 当前LLM代理能力不足以满足工业自动化需求，需领域特定优化。
...
### [225] [The AI Imperative: Scaling High-Quality Peer Review in Machine Learning](https://arxiv.org/abs/2506.08134)
*Qiyao Wei,Samuel Holt,Jing Yang,Markus Wulfmeier,Mihaela van der Schaar*

Main category: cs.AI

TLDR: 论文主张利用AI辅助同行评审以应对机器学习领域评审规模危机，提出LLMs作为协作工具而非替代方案，并呼吁建立相关研究议程。
...
Method: 提出利用大型语言模型（LLMs）辅助作者、评审人和领域主席，包括事实核查、评审指导、质量改进和决策支持。

Result: 论文呼吁开发基于结构化、伦理数据的AI辅助系统，并提出了研究议程和技术挑战。

Conclusion: 机器学习社区需主动构建AI辅助评审系统，以维持科学验证的完整性和可扩展性。
...
### [226] [Compiling Metric Temporal Answer Set Programming](https://arxiv.org/abs/2506.08150)
*Arvid Becker,Pedro Cabalar,Martin Diéguez,Javier Romero,Susana Hahn,Torsten Schaub*

Main category: cs.AI

TLDR: 提出了一种计算方法来处理度量答案集编程（ASP）中的定量时间约束，如持续时间和截止时间，通过扩展ASP的差异约束来避免时间粒度对性能的影响。


<details>
  <summary>Details</summary>
Motivation: 解决在ASP中处理细粒度时间约束时可能导致的性能瓶颈问题。

Method: 利用ASP的差异约束扩展，将时间相关部分外部化处理，从而解耦时间粒度与度量ASP的关系。

Result: 实现了不受时间精度影响的解决方案，保持了系统的可扩展性。

Conclusion: 该方法有效解决了ASP中时间约束的粒度问题，提升了处理定量时间约束的能力。
...
### [227] [AstroCompress: A benchmark dataset for multi-purpose compression of astronomical data](https://arxiv.org/abs/2506.08306)
*Tuan Truong,Rithwik Sudharsan,Yibo Yang,Peter Xiangyuan Ma,Ruihan Yang,Stephan Mandt,Joshua S. Bloom*

Main category: cs.AI

TLDR: 论文提出AstroCompress，一种用于天体物理数据的神经压缩方法，通过端到端学习提升数据压缩效率，优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 天文台因环境限制导致数据传输能力受限，改进无损压缩技术可显著提升科学数据采集效率。

Method: 引入AstroCompress挑战，提供五种数据集，并对比七种无损压缩方法（三种神经方法，四种传统方法）。

Result: 神经压缩技术可提升天文台数据采集效率，并为科学应用中的神经压缩采用提供指导。

Conclusion: 神经压缩技术在天文数据中表现优异，未来可探索有损压缩方法。
...
### [228] [LeanTutor: A Formally-Verified AI Tutor for Mathematical Proofs](https://arxiv.org/abs/2506.08321)
*Manooshree Patel,Rayna Bhattacharyya,Thomas Lu,Arnav Mehta,Niels Voss,Narges Norouzi,Gireeja Ranade*

Main category: cs.AI

TLDR: LeanTutor是一个基于大型语言模型（LLM）的数学证明辅导系统，通过自然语言交互、形式化验证和生成下一步指导来辅助学生。
...
Result: 自动形式化模块在正确证明中准确形式化57%的步骤，在错误证明中准确识别30%的错误步骤；反馈生成器在准确性和相关性上优于基线。

Conclusion: LeanTutor展示了LLM在数学辅导中的潜力，尤其在形式化验证和自然语言反馈方面表现良好。
...
### [229] [ORFS-agent: Tool-Using Agents for Chip Design Optimization](https://arxiv.org/abs/2506.08332)
*Amur Ghose,Andrew B. Kahng,Sayak Kundu,Zhiang Wang*

Main category: cs.AI

TLDR: ORFS-agent是一种基于大型语言模型（LLM）的迭代优化代理，用于自动化集成电路设计流程中的参数调优，显著优于传统贝叶斯优化方法。


<details>
  <summary>Details</summary>
Motivation: 集成电路设计流程涉及大量参数配置，微小变化可能对性能、功耗和面积产生重大影响，LLM为高维优化任务提供了新机会。

Method: ORFS-agent通过自适应探索参数配置，结合自然语言目标进行多目标优化，无需额外微调即可应用于前沿LLM。

Result: 在两个技术节点和多个电路基准测试中，ORFS-agent将布线长度和有效时钟周期改善超过13%，同时减少40%的优化迭代次数。

Conclusion: ORFS-agent提供了一种灵活、可解释且高效的参数优化框架，适用于多目标优化任务。
...
### [230] [FloorplanMAE:A self-supervised framework for complete floorplan generation from partial inputs](https://arxiv.org/abs/2506.08363)
*Jun Yin,Jing Zhong,Pengyu Zeng,Peilin Li,Miao Zhang,Ran Luo,Shuai Lu*

Main category: cs.AI

TLDR: FloorplanMAE是一个自监督学习框架，用于从不完整的地板平面图中恢复完整的地板平面图，提高设计效率。


<details>
  <summary>Details</summary>
Motivation: 在地板平面图设计过程中，预测完整平面图的能力对快速生成初步设计和减少重复修改的工作量具有重要价值。

Method: 提出基于Masked Autoencoders (MAE)的方法，通过掩码部分平面图并训练轻量级Vision Transformer (ViT)来重建缺失部分。

Result: 实验结果表明，FloorplanMAE能从不完整平面图中生成高质量的完整平面图。

Conclusion: 该框架为地板平面图生成提供了可扩展的解决方案，具有广泛的应用前景。
...
### [231] [On Reasoning Strength Planning in Large Reasoning Models](https://arxiv.org/abs/2506.08390)
*Leheng Sheng,An Zhang,Zijian Wu,Weixiang Zhao,Changshuo Shen,Yi Zhang,Xiang Wang,Tat-Seng Chua*

Main category: cs.AI

TLDR: 大型推理模型（LRMs）能根据问题难度自动分配推理强度（推理标记数量），其机制与模型激活中的预分配方向向量相关。


<details>
  <summary>Details</summary>
Motivation: 探索LRMs自动分配推理强度的内部机制。

Method: 通过模型激活分析，发现推理强度由预分配方向向量的幅度控制，并通过线性探针预测推理标记数量。

Result: 方向向量的幅度调节推理强度，调整该向量可改变推理标记数量和性能。
...
### [232] [SafeCoT: Improving VLM Safety with Minimal Reasoning](https://arxiv.org/abs/2506.08399)
*Jiachen Ma,Zhanhui Zhou,Chao Yang,Chaochao Lu*

Main category: cs.AI

TLDR: SafeCoT是一个轻量级、可解释的框架，通过规则引导的思维链监督改进视觉语言模型（VLM）的拒绝行为，减少过度拒绝并提升泛化能力。


<details>
  <summary>Details</summary>
Motivation: 确保视觉语言模型在高风险或模糊场景下生成安全且合适的响应是一个关键挑战。

Method: SafeCoT利用规则引导的思维链监督，无需大规模安全标注或复杂建模，仅需少量监督即可帮助模型推理安全风险并做出上下文感知的拒绝。

Result: 在多个基准测试中，SafeCoT显著减少了过度拒绝并提升了泛化能力，即使训练数据有限。

Conclusion: SafeCoT为视觉语言模型的安全关键目标对齐提供了一种可扩展的解决方案。
...
### [233] [Single-Node Trigger Backdoor Attacks in Graph-Based Recommendation Systems](https://arxiv.org/abs/2506.08401)
*Runze Li,Di Jin,Xiaobao Wang,Dongxiao He,Bingdao Feng,Zhen Wang*

Main category: cs.AI

TLDR: 本文提出了一种新的图后门攻击方法，通过插入单个虚假用户节点，隐蔽地提升目标物品对目标用户的曝光率，同时减少对推荐系统性能的影响。
...
Method: 设计了一个单节点触发器生成器，仅插入一个虚假用户节点，并引入目标节点与无关节点之间的约束条件。

Result: 目标物品对目标用户的曝光率达到50%以上（99%的用户），对推荐系统性能的影响控制在约5%。

Conclusion: 该方法在隐蔽性和破坏性之间取得了平衡，有效提升了目标物品的曝光率。
...
### [234] [Transforming Expert Knowledge into Scalable Ontology via Large Language Models](https://arxiv.org/abs/2506.08422)
...
TLDR: 提出了一种结合大语言模型（LLMs）与专家校准的自动化分类法对齐框架，显著提升了分类法对齐的效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 传统手动分类法对齐方法成本高且主观性强，现有自动化方法在处理复杂语义关系和跨领域一致性方面存在不足。

Method: 结合LLMs、专家校准和迭代提示优化，通过专家标注示例、多阶段提示工程和人工验证生成分类法链接及支持理由。
...
### [235] [SHIELD: Multi-task Multi-distribution Vehicle Routing Solver with Sparsity and Hierarchy](https://arxiv.org/abs/2506.08424)
*Yong Liang Goh,Zhiguang Cao,Yining Ma,Jianan Zhou,Mohammad Haroon Dupty,Wee Sun Lee*

Main category: cs.AI

TLDR: 论文提出了一种名为SHIELD的新模型，用于解决多任务多分布车辆路径问题（MTMDVRP），通过稀疏性和层次性原则提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型忽略了现实世界中复杂的客户分布，因此需要更适应多任务和多分布场景的解决方案。

Method: 结合了Mixture-of-Depths（MoD）技术和上下文聚类层，动态选择节点并利用层次结构优化表示。
...
### [236] [A Survey on Large Language Models for Mathematical Reasoning](https://arxiv.org/abs/2506.08446)
...
TLDR: 该论文综述了大型语言模型（LLMs）在数学推理能力上的发展，分为理解与答案生成两个阶段，并探讨了提升方法、挑战及未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 数学推理是人工智能研究中的核心挑战，LLMs在此领域取得显著进展，但仍有改进空间。

Method: 通过预训练策略和逐步推理（如Chain-of-Thought）提升数学推理能力，方法包括训练无关提示和微调技术。

Result: LLMs在数学推理上取得进展，但仍面临容量、效率和泛化等挑战。

Conclusion: 未来研究方向包括高级预训练、知识增强、形式化推理框架和元泛化，为LLMs推理能力提升提供指导。
...
### [237] [Hybrid Reasoning for Perception, Explanation, and Autonomous Action in Manufacturing](https://arxiv.org/abs/2506.08462)
*Christos Margadji,Sebastian W. Pattinson*

Main category: cs.AI

TLDR: CIPHER是一种结合视觉-语言-动作（VLA）模型的框架，旨在通过混合专家知识和推理实现工业控制的自主化，适用于数据稀缺的工业环境。


<details>
  <summary>Details</summary>
Motivation: 工业过程需要鲁棒性和适应性，但传统AI控制依赖有监督学习，难以泛化到数据稀缺的场景。

Method: CIPHER结合过程专家、回归模型和检索增强生成技术，支持物理信息驱动的链式推理。

Result: CIPHER在分布外任务中表现出强泛化能力，能解释决策并生成精确指令，无需显式标注。

Conclusion: CIPHER为工业环境中的自主系统提供了精确、透明且可信的解决方案。
...
### [238] [RHealthTwin: Towards Responsible and Multimodal Digital Twins for Personalized Well-being](https://arxiv.org/abs/2506.08486)
*Rahatara Ferdousi,M Anwar Hossain*

Main category: cs.AI

TLDR: 论文提出Responsible Health Twin (RHealthTwin)框架，通过Responsible Prompt Engine (RPE)解决大语言模型在医疗健康领域中的幻觉、偏见等问题，实现安全、可解释的响应。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在医疗健康领域的应用存在幻觉、偏见、透明度和伦理问题，需要一种负责任的方法来构建和治理AI驱动的数字孪生。

Method: 提出RHealthTwin框架，核心是RPE，动态提取预定义槽位以结构化输入，结合反馈循环优化提示结构。

Result: 在四个健康领域（心理支持、症状分诊、营养规划、活动指导）中，RPE取得BLEU=0.41、ROUGE-L=0.63、BERTScore=0.89的先进结果，伦理合规和指令遵循指标超过90%。

Conclusion: RHealthTwin为医疗健康领域负责任的大语言模型应用提供了前瞻性基础。
...
### [239] [FEDTAIL: Federated Long-Tailed Domain Generalization with Sharpness-Guided Gradient Matching](https://arxiv.org/abs/2506.08518)
*Sunny Gupta,Nikita Jangid,Shounak Das,Amit Sethi*

Main category: cs.AI

TLDR: FedTAIL是一个联邦领域泛化框架，通过锐度引导的梯度对齐优化解决长尾分布和优化冲突问题，结合梯度一致性正则化和动态加权方案，提升模型在未见目标域上的性能。


<details>
  <summary>Details</summary>
Motivation: 解决领域泛化中长尾类分布和优化目标冲突的问题，提升模型在未见目标域上的鲁棒性。
...
### [240] [Safe and Economical UAV Trajectory Planning in Low-Altitude Airspace: A Hybrid DRL-LLM Approach with Compliance Awareness](https://arxiv.org/abs/2506.08532)
*Yanwei Gong,Xiaolin Chang*

Main category: cs.AI

TLDR: 提出了一种结合深度强化学习（DRL）和大语言模型（LLM）的新型无人机轨迹规划框架，显著提升了复杂城市环境中的路径规划性能。
...
Method: 结合DRL与LLM推理，设计了一种新型无人机轨迹规划框架。

Result: 实验表明，该方法在数据收集率、避障、成功着陆、法规合规性和能源效率等多方面显著优于现有基线。
...
### [241] [HGFormer: A Hierarchical Graph Transformer Framework for Two-Stage Colonel Blotto Games via Reinforcement Learning](https://arxiv.org/abs/2506.08580)
*Yang Lv,Jinlong Lei,Peng Yi*

Main category: cs.AI

TLDR: HGformer框架通过层次化图Transformer和反馈强化学习算法，解决了两阶段Colonel Blotto游戏中资源分配的全局优化问题，显著提升了效率和对抗收益。


<details>
  <summary>Details</summary>
Motivation: 传统方法难以处理两阶段Colonel Blotto游戏中阶段依赖和网络拓扑约束带来的复杂性，需要一种新方法以实现全局最优策略。

Method: 提出HGformer框架，结合增强的图Transformer编码器和两代理层次决策模型，设计分层反馈强化学习算法优化策略。

Result: 实验显示HGformer在资源分配效率和对抗收益上显著优于现有方法。

Conclusion: HGformer在复杂动态游戏场景中表现优异，为类似问题提供了有效解决方案。
...
### [242] [FoldA: Computing Partial-Order Alignments Using Directed Net Unfoldings](https://arxiv.org/abs/2506.08627)
*Douwe Geurtjens,Xixi Lu*

Main category: cs.AI

TLDR: 本文提出了一种基于Petri网展开的新技术FoldA，用于计算部分顺序对齐，解决了现有方法在并发行为表示和状态空间爆炸上的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的一致性检查方法在处理高选择性和并发性模型时存在状态空间爆炸问题，且无法充分表示并发行为。

Method: 使用定向Petri网展开技术动态计算部分顺序对齐。

Result: 在485个合成模型-日志对和19个真实/基准模型-日志对上评估，FoldA减少了排队状态数量并更准确地表示并发，但计算时间更长。

Conclusion: FoldA技术虽增加计算时间，但显著提升了并发行为表示的准确性并缓解了状态空间爆炸问题。
...
### [243] [Modular Recurrence in Contextual MDPs for Universal Morphology Control](https://arxiv.org/abs/2506.08630)
*Laurens Engwegen,Daan Brinks,Wendelin Böhmer*

Main category: cs.AI

TLDR: 提出了一种模块化循环架构，通过推断部分可观测的上下文信息，提升未见机器人的泛化性能。
...
Method: 采用模块化循环架构，通过交互推断上下文信息，并在MuJoCo机器人上评估。

Result: 在未见动态、运动学和拓扑的机器人上，四种环境中表现显著提升。

Conclusion: 模块化循环架构能有效推断上下文信息，提升泛化性能。
...
### [244] [Consistent Paths Lead to Truth: Self-Rewarding Reinforcement Learning for LLM Reasoning](https://arxiv.org/abs/2506.08745)
*Kongcheng Zhang,Qi Yao,Shunyu Liu,Yingjie Wang,Baisheng Lai,Jieping Ye,Mingli Song,Dacheng Tao*

Main category: cs.AI

TLDR: 提出了一种自奖励强化学习框架CoVo，通过利用中间推理状态的一致性来增强大型语言模型的推理能力，无需外部监督即可实现与监督强化学习相当的性能。
...
Result: 在多个推理基准测试中，CoVo的性能达到或超过监督强化学习。
...
### [245] [A Sample Efficient Conditional Independence Test in the Presence of Discretization](https://arxiv.org/abs/2506.08747)
*Boyang Sun,Yu Yao,Xinshuai Dong,Zongfang Liu,Tongliang Liu,Yumou Qiu,Kun Zhang*

Main category: cs.AI

TLDR: 论文提出了一种不依赖二值化的样本高效条件独立性检验方法，通过广义矩方法（GMM）解决过度识别限制问题，并利用节点回归建立检验统计量。


<details>
  <summary>Details</summary>
Motivation: 由于测量限制，实际场景中的变量常被离散化，直接应用条件独立性检验会导致错误结论。现有方法通过二值化处理会丢失信息，影响检验性能。

Method: 通过广义矩方法（GMM）解决过度识别限制问题，利用节点回归建立检验统计量，并推导其渐近分布。

Result: 理论和实证结果表明，该方法在多种数据集上表现出优越性和有效性。

Conclusion: 提出的方法避免了二值化过程的信息损失，提供了一种更准确的检验条件独立性的途径。
...
### [246] [Paths to Causality: Finding Informative Subgraphs Within Knowledge Graphs for Knowledge-Based Causal Discovery](https://arxiv.org/abs/2506.08771)
*Yuni Susanti,Michael Färber*

Main category: cs.AI

TLDR: 论文提出了一种结合知识图谱（KGs）和大语言模型（LLMs）的新方法，用于提升基于知识的因果发现，解决了现有LLM方法的不稳定性和不一致性问题。


<details>
  <summary>Details</summary>
Motivation: 传统基于观测数据的因果发现方法存在局限性，而基于知识的因果发现（利用变量元数据）虽具潜力，但现有LLM方法结果不稳定。

Method: 通过知识图谱识别信息丰富的元路径子图，并利用学习排序模型优化子图选择，将其融入零样本提示中以提升LLM的因果推理效果。

Result: 在生物医学和开放领域数据集上的实验表明，该方法在F1分数上比基线方法最高提升44.4分。

Conclusion: 结合KGs和LLMs的方法显著提升了基于知识的因果发现的可靠性和效果。
...
### [247] [Measuring Data Science Automation: A Survey of Evaluation Tools for AI Assistants and Agents](https://arxiv.org/abs/2506.08800)
*Irene Testini,José Hernández-Orallo,Lorenzo Pacchiardi*

Main category: cs.AI

TLDR: 本文综述了大型语言模型（LLM）助手和代理在数据科学中的评估，发现研究集中在少数目标导向活动，忽视了数据管理和探索性活动，且缺乏对中间人机协作和任务转换的关注。


<details>
  <summary>Details</summary>
Motivation: 探讨LLM助手和代理在数据科学中的应用现状及其局限性，为未来研究提供方向。

Method: 通过文献综述分析LLM助手和代理在数据科学中的评估现状。

Result: 研究发现主要集中在目标导向活动，忽视了数据管理和探索性活动，且缺乏中间协作和任务转换的探讨。

Conclusion: 未来研究应更全面地评估LLM助手和代理，包括数据管理、探索性活动及人机协作的中间层次。
...
### [248] [Your Brain on ChatGPT: Accumulation of Cognitive Debt when Using an AI Assistant for Essay Writing Task](https://arxiv.org/abs/2506.08872)
...
TLDR: 研究探讨了LLM辅助写作对神经和行为的影响，发现LLM使用者在认知、语言和行为层面表现较差，引发对AI在教育中长期影响的担忧。


<details>
  <summary>Details</summary>
Motivation: 探索LLM辅助写作对认知和行为的潜在影响，评估AI工具在教育中的长期效果。

Method: 将54名参与者分为LLM、搜索引擎和无工具组，通过EEG和NLP分析认知负荷和写作质量，并在第四阶段交换工具使用。

Result: LLM使用者脑连接最弱，认知活动减少，写作所有权感最低；无工具组表现最佳。

Conclusion: LLM虽便捷，但可能带来认知成本，需深入研究AI在教育中的角色。
...
### [249] [Preference-Driven Multi-Objective Combinatorial Optimization with Conditional Computation](https://arxiv.org/abs/2506.08898)
*Mingfeng Fan,Jianan Zhou,Yifeng Zhang,Yaoxin Wu,Jinbiao Chen,Guillaume Adrien Sartoretti*

Main category: cs.AI

TLDR: POCCO是一个新的框架，通过自适应选择子问题的模型结构，并基于偏好信号优化，显著提升了多目标组合优化问题的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将所有子问题平等对待并使用单一模型，限制了解决方案空间的探索，导致性能不佳。

Method: 提出POCCO框架，设计条件计算块将子问题路由到专用神经网络架构，并提出偏好驱动的优化算法。

Result: 在四个经典MOCOP基准测试中，POCCO表现出显著优势和强泛化能力。

Conclusion: POCCO通过自适应模型选择和偏好驱动优化，有效解决了现有方法的局限性。
...
### [250] [IntTrajSim: Trajectory Prediction for Simulating Multi-Vehicle driving at Signalized Intersections](https://arxiv.org/abs/2506.08957)
*Yash Ranjan,Rahul Sengupta,Anand Rangarajan,Sanjay Ranka*

Main category: cs.AI

TLDR: 提出了一种基于深度生成建模的交通模拟器，用于模拟交叉路口的驾驶行为，并引入交通工程相关指标进行评估。


<details>
  <summary>Details</summary>
Motivation: 现有基于规则的交通模拟器难以真实模拟驾驶行为，而交叉路口是交通安全和效率的关键。

Method: 提出多头部自注意力轨迹预测模型，结合信号信息，并通过模拟闭环管道评估。

Result: 新模型在评估指标上优于先前模型。

Conclusion: 数据驱动的模拟器结合交通工程指标，能更真实地模拟交叉路口行为。
...
### [251] [Evaluating Generative Vehicle Trajectory Models for Traffic Intersection Dynamics](https://arxiv.org/abs/2506.08963)
*Yash Ranjan,Rahul Sengupta,Anand Rangarajan,Sanjay Ranka*

Main category: cs.AI

TLDR: 论文提出了一种基于深度生成模型的交通路口动态分析方法，通过在线微模拟和新的评估指标，从交通工程角度更全面地评估模型性能。


<details>
  <summary>Details</summary>
Motivation: 交通路口是城市路网的关键，但易发生事故。现有模型仅关注轨迹重建误差，未考虑交通工程问题（如闯红灯、违规停车等）。

Method: 训练了一个多车辆轨迹预测模型，并在真实世界路口数据上进行校准，通过在线微模拟评估模型在未知交通条件下的表现。

Result: 尽管输入理想轨迹且轨迹重建误差低，生成的轨迹仍违反交通规则。论文提出了新指标评估此类不良行为。

Conclusion: 论文提供了一种更全面的模型评估工具，揭示了现有模型的局限性，并提出了改进方向。
...
### [252] [A Survey of Link Prediction in N-ary Knowledge Graphs](https://arxiv.org/abs/2506.08970)
*Jiyao Wei,Saiping Guan,Da Li,Xiaolong Jin,Jiafeng Guo,Xueqi Cheng*

Main category: cs.AI

TLDR: 本文是关于N元知识图谱（NKG）中链接预测的首个全面综述，总结了现有方法、性能和应用场景，并展望了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: N元知识图谱能更高效地表示复杂事实，但链接预测任务尚未得到系统总结，本文旨在填补这一空白。

Method: 通过系统分类和分析现有方法，评估其性能和应用场景。

Result: 总结了NKG链接预测的现状，提出了未来研究的潜在方向。

Conclusion: 本文为NKG链接预测领域提供了系统综述，并指出了未来的研究机会。
...
### [253] [AbstentionBench: Reasoning LLMs Fail on Unanswerable Questions](https://arxiv.org/abs/2506.09038)
*Polina Kirichenko,Mark Ibrahim,Kamalika Chaudhuri,Samuel J. Bell*

Main category: cs.AI

TLDR: AbstentionBench是一个大规模基准测试，用于全面评估大型语言模型（LLMs）在20个多样化数据集上的拒绝回答能力，发现拒绝回答仍是一个未解决的问题，且模型扩展对此帮助有限。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在现实世界查询中拒绝回答的能力，以提升其可靠性。

Method: 引入AbstentionBench基准，评估20个前沿LLMs在多样化数据集上的拒绝回答表现。

Result: 发现拒绝回答仍是一个未解决的问题，推理微调反而降低拒绝回答能力（平均24%）。

Conclusion: 精心设计的系统提示可以提升拒绝回答能力，但无法解决模型对不确定性推理的根本问题，呼吁进一步研究提升LLM可靠性。
...
### [254] [VIKI-R: Coordinating Embodied Multi-Agent Cooperation via Reinforcement Learning](https://arxiv.org/abs/2506.09049)
*Li Kang,Xiufeng Song,Heng Zhou,Yiran Qin,Jie Yang,Xiaohong Liu,Philip Torr,Lei Bai,Zhenfei Yin*

Main category: cs.AI

TLDR: VIKI-Bench是一个针对多智能体协作的层次化基准测试，结合了视觉输入和多样化机器人形态。VIKI-R框架通过两阶段方法显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 解决多智能体在动态环境中协作的挑战，尤其是视觉驱动的推理和多样化形态支持不足的问题。
...
### [255] [ALE-Bench: A Benchmark for Long-Horizon Objective-Driven Algorithm Engineering](https://arxiv.org/abs/2506.09050)
*Yuki Imajuku,Kohki Horie,Yoichi Iwata,Kensho Aoki,Naohiro Takahashi,Takuya Akiba*

Main category: cs.AI

TLDR: ALE-Bench是一个新的基准测试，用于评估AI系统在基于分数的算法编程竞赛中的表现，专注于解决复杂优化问题。


<details>
  <summary>Details</summary>
Motivation: 评估AI系统在算法工程中的表现，尤其是在复杂优化问题（如物流、调度和生产规划）上的能力。

Method: 基于AtCoder Heuristic Contests的真实任务，设计了一个支持迭代优化和交互式代理架构的软件框架。
...
### [256] [Towards Reliable AR-Guided Surgical Navigation: Interactive Deformation Modeling with Data-Driven Biomechanics and Prompts](https://arxiv.org/abs/2506.08048)
*Zheng Han,Jun Zhou,Jialun Pei,Jing Qin,Yingfang Fan,Qi Dou*

Main category: cs.CV

TLDR: 提出了一种数据驱动的生物力学算法，结合人机交互机制，用于AR手术导航中的变形建模，提高了计算效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在计算效率和处理大解剖变化方面存在不足，导致AR导航不准确。

Method: 结合数据驱动的生物力学算法和人机交互机制，动态调整模型以适应手术场景。

Result: 算法平均目标配准误差为3.42 mm，结合交互提示后降至2.78 mm，优于现有方法。

Conclusion: 该框架实现了高效准确的变形建模，提升了手术导航的可靠性。
...
### [257] [ReCogDrive: A Reinforced Cognitive Framework for End-to-End Autonomous Driving](https://arxiv.org/abs/2506.08052)
...
TLDR: ReCogDrive通过结合视觉语言模型（VLMs）和扩散规划器，解决了自动驾驶在长尾场景中的性能问题，并通过三阶段训练方法提升了性能。


<details>
  <summary>Details</summary>
Motivation: 解决现有自动驾驶系统在罕见和长尾场景中性能下降的问题，并克服视觉语言模型在领域差距、维度不匹配和模仿学习局限性上的挑战。

Method: 采用三阶段训练：1）用大规模驾驶问答数据集训练VLMs；2）用扩散规划器进行模仿学习；3）用强化学习微调规划器。

Result: 在NAVSIM基准测试中达到89.6 PDMS，比之前最佳视觉模型提升5.6 PDMS。

Conclusion: ReCogDrive通过多阶段训练和扩散规划器，显著提升了自动驾驶在复杂场景中的性能，成为新的SOTA。
...
### [258] [CuRe: Cultural Gaps in the Long Tail of Text-to-Image Systems](https://arxiv.org/abs/2506.08071)
*Aniket Rege,Zinnia Nie,Mahesh Ramesh,Unmesh Raskar,Zhuoran Yu,Aditya Kusupati,Yong Jae Lee,Ramya Korlakai Vinayak*

Main category: cs.CV

TLDR: CuRe是一个用于评估文本到图像（T2I）系统文化代表性的新基准，通过属性规范的边际效用作为人类判断的代理。


<details>
  <summary>Details</summary>
Motivation: 当前T2I系统训练数据偏向欧美文化，忽视全球南方文化，需量化这种偏见。

Method: 利用Wikimedia知识图谱构建包含300个文化实体的数据集，分为6大类，通过文本条件的信息量评估T2I系统。

Result: CuRe评分与人类在感知相似性、图文对齐和文化多样性上的判断高度相关，适用于多种图像编码器和T2I系统。

Conclusion: CuRe为文化代表性提供了可扩展的评估工具，数据集和代码已开源。
...
### [259] [IGraSS: Learning to Identify Infrastructure Networks from Satellite Imagery by Iterative Graph-constrained Semantic Segmentation](https://arxiv.org/abs/2506.08137)
...
TLDR: IGraSS是一个结合语义分割和图优化的迭代框架，用于改进不完整的地面真实数据，显著提升运河和道路网络的映射准确性。


<details>
  <summary>Details</summary>
Motivation: 现有语义分割模型依赖高质量标注数据，但地面真实数据的不完整性限制了其性能。基础设施网络（如运河和道路）的图级特性（如可达性或连通性）可用于优化地面真实数据。

Method: IGraSS结合了多模态（RGB、NDWI、DEM）的语义分割模块和图优化的地面真实数据细化模块，通过迭代处理提升网络映射准确性。

Result: 实验表明，IGraSS将不可达运河段从18%降至3%，且使用细化后的地面真实数据显著提升了运河识别性能。该方法也适用于道路网络。

Conclusion: IGraSS是一个鲁棒的框架，可用于优化噪声地面真实数据并从遥感图像中映射基础设施网络，具有广泛适用性。
...
### [260] [Spectral Domain Neural Reconstruction for Passband FMCW Radars](https://arxiv.org/abs/2506.08163)
*Harshvardhan Takawale,Nirupam Roy*

Main category: cs.CV

TLDR: SpINRv2是一个基于神经网络的框架，用于通过FMCW雷达实现高保真体积重建，改进了前作SpINR，解决了高频下的相位混叠和子箱模糊问题。


<details>
  <summary>Details</summary>
Motivation: 高频FMCW雷达在体积重建中面临相位混叠和子箱模糊的挑战，现有方法难以准确建模。

Method: 提出了一种完全可微的频域前向模型，结合隐式神经表示（INR）进行连续体积建模，并引入稀疏性和平滑性正则化。

Result: SpINRv2在实验中显著优于经典和基于学习的方法，尤其是在高频条件下。
...
### [261] [Surgeon Style Fingerprinting and Privacy Risk Quantification via Discrete Diffusion Models in a Vision-Language-Action Framework](https://arxiv.org/abs/2506.08185)
*Huixin Zhan,Jason H. Moore*

Main category: cs.CV

TLDR: 论文提出了一种基于离散扩散框架和视觉-语言-动作（VLA）管道的方法，用于建模外科医生在机器人手术中的个性化操作风格，同时关注隐私保护。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统常忽略外科医生的个性化操作风格，而手术风格因训练、经验和行为差异而显著不同。

Method: 通过离散扩散框架结合VLA管道，将手势预测建模为结构化序列去噪任务，输入包括内窥镜视频、手术意图语言和隐私感知的外科医生身份与技能嵌入。

Result: 在JIGSAWS数据集上验证，方法能准确重建手势序列并学习到每位外科医生独特的运动指纹。但更个性化的嵌入会增加身份泄露风险。
...
### [262] [Open World Scene Graph Generation using Vision Language Models](https://arxiv.org/abs/2506.08189)
...
TLDR: 提出了一种无需训练、高效的Open-World SGG框架，利用预训练的视觉语言模型（VLMs）直接生成场景图，支持零样本学习。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖数据集特定监督或微调，限制了其在开放世界（涉及新对象或关系）中的适用性。

Method: 将SGG视为零样本结构化推理问题，结合多模态提示、嵌入对齐和轻量级对优化策略。

Result: 在Visual Genome、Open Images V6和PSG数据集上验证了预训练VLMs无需任务级训练即可进行关系理解的能力。

Conclusion: Open-World SGG框架展示了预训练VLMs在开放世界场景中的潜力，无需额外学习即可生成场景图。
...
### [263] [Generative Learning of Differentiable Object Models for Compositional Interpretation of Complex Scenes](https://arxiv.org/abs/2506.08191)
*Antoni Nowinowski,Krzysztof Krawiec*

Main category: cs.CV

TLDR: 扩展了DVP架构以处理多对象场景，利用潜在空间损失函数改进训练，并在新基准测试中表现优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 解决DVP在处理多对象场景时的局限性，并通过潜在空间损失函数优化训练过程。

Method: 扩展DVP架构以支持多对象，利用解码器生成额外训练样本，并在图像和潜在空间定义损失函数。

Result: 在新基准测试中，扩展后的DVP在重建质量和对象分解能力上优于MONet和LIVE。

Conclusion: 潜在空间损失函数显著提升训练效果，但可微分渲染在自编码器中仍有局限性。
...
### [264] [GIQ: Benchmarking 3D Geometric Reasoning of Vision Foundation Models with Simulated and Real Polyhedra](https://arxiv.org/abs/2506.08194)
...
TLDR: GIQ是一个评估视觉和视觉语言基础模型几何推理能力的综合基准，揭示了当前模型在几何任务中的显著不足。


<details>
  <summary>Details</summary>
Motivation: 评估现有模型对几何属性的真实理解能力，填补几何推理评估的空白。

Method: 通过合成和真实图像，设计包含224种多面体的数据集，进行3D重建、对称性检测、心理旋转测试和零样本分类等实验。

Result: 当前模型在基本几何形状重建、详细几何区分和复杂多面体理解上表现不佳。

Conclusion: GIQ为几何智能研究提供了结构化平台，促进未来几何感知表示学习的进展。
...
### [265] [A Comprehensive Study of Decoder-Only LLMs for Text-to-Image Generation](https://arxiv.org/abs/2506.08210)
*Andrew Z. Wang,Songwei Ge,Tero Karras,Ming-Yu Liu,Yogesh Balaji*

Main category: cs.CV

TLDR: 研究探讨了使用现代仅解码器LLM作为文本编码器在文本到图像扩散模型中的效果，发现多层归一化平均嵌入优于传统方法，显著提升性能。
...
Result: 多层归一化平均嵌入优于传统最后一层嵌入，LLM在复杂提示对齐和视觉语言推理上表现更佳。

Conclusion: 现代LLM作为文本编码器能显著提升文本到图像生成性能，尤其是多层嵌入方法。
...
### [266] [Using Satellite Images And Self-supervised Machine Learning Networks To Detect Water Hidden Under Vegetation](https://arxiv.org/abs/2506.08214)
*Ioannis Iakovidis,Zahra Kalantari,Amir Hossein Payberah,Fernando Jaramillo,Francisco Pena Escobar*

Main category: cs.CV

TLDR: 论文提出了一种结合深度聚类和负采样的自监督方法，用于雷达卫星图像的水陆分割，无需人工标注，并通过集成模型提升性能。
...
Method: 采用深度聚类和负采样技术训练模型，并实现集成模型以减少方差并提高性能。

Result: 自监督集成模型在测试数据集上的IOU指标比全监督单模型提高了0.02。

Conclusion: 自监督方法在减少人工标注需求的同时，能够有效提升模型性能。
...
### [267] [Jamais Vu: Exposing the Generalization Gap in Supervised Semantic Correspondence](https://arxiv.org/abs/2506.08220)
*Octave Mariotti,Zhipeng Du,Yash Bhalgat,Oisin Mac Aodha,Hakan Bilen*

Main category: cs.CV

TLDR: 提出了一种通过单目深度估计将2D关键点提升到3D规范空间的新方法，以学习密集对应关系，无需显式3D监督或相机注释。


<details>
  <summary>Details</summary>
Motivation: 现有监督语义对应方法局限于稀疏标注的训练关键点，泛化能力不足。

Method: 利用单目深度估计构建连续规范流形，捕捉对象几何形状。

Result: 在未见关键点上显著优于监督基线，无监督基线在跨数据集泛化中表现更优。

Conclusion: 新方法能学习鲁棒对应关系，无监督方法在泛化性上优于监督方法。
...
### [268] [A Good CREPE needs more than just Sugar: Investigating Biases in Compositional Vision-Language Benchmarks](https://arxiv.org/abs/2506.08227)
*Vishaal Udandarao,Mehdi Cherti,Shyamgopal Karthik,Jenia Jitsev,Samuel Albanie,Matthias Bethge*

Main category: cs.CV

TLDR: 论文分析了17个用于评估视觉语言模型组合理解能力的基准测试，揭示了其设计中的偏见问题，并提出改进建议。


<details>
  <summary>Details</summary>
Motivation: 研究旨在揭示现有视觉语言模型基准测试中的设计缺陷，特别是组合理解能力的评估是否有效。

Method: 通过分析17个基准测试的数据来源和构建过程，识别其中的偏见，并比较简单启发式方法与CLIP模型的表现。

Result: 发现基准测试存在分布不对称问题，导致简单启发式方法与CLIP模型表现相当，表明其未能有效评估组合理解能力。

Conclusion: 提出了构建更稳健基准测试的建议，以减少设计偏见和简单攻击的影响。
...
### [269] [Highly Compressed Tokenizer Can Generate Without Training](https://arxiv.org/abs/2506.08257)
*L. Lao Beyer,T. Li,X. Chen,S. Karaman,K. He*

Main category: cs.CV

TLDR: 1D图像分词器通过高度压缩的一维序列实现图像编辑和生成能力，无需训练生成模型。
...
Conclusion: 1D图像分词器在高度压缩下仍能实现精细的图像编辑和生成，展示了其潜在空间的强大表达能力。
...
### [270] [Seeing Voices: Generating A-Roll Video from Audio with Mirage](https://arxiv.org/abs/2506.08279)
...
TLDR: Mirage是一种音频到视频的基础模型，能够根据音频输入生成逼真、富有表现力的视频，尤其擅长处理语音内容。


<details>
  <summary>Details</summary>
Motivation: 视频的音频和视觉元素的和谐整合对视频效果至关重要，但现有方法要么忽略音频，要么局限于特定领域。

Method: 提出了一种统一的训练方法，基于自注意力机制，支持从头训练或利用现有权重。

Result: Mirage生成的视频在主观质量上优于其他方法，且具有通用性。

Conclusion: Mirage为音频到视频生成提供了一种通用且高质量的解决方案。
...
### [271] [SEMA: a Scalable and Efficient Mamba like Attention via Token Localization and Averaging](https://arxiv.org/abs/2506.08297)
*Nhat Thanh Tran,Fanghui Xue,Shuai Zhang,Jiancheng Lyu,Yunling Zheng,Yingyong Qi,Jack Xin*

Main category: cs.CV

TLDR: 论文提出了一种名为SEMA的新型注意力机制，解决了传统注意力计算复杂度高和线性注意力无法聚焦的问题，并在图像分类任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 传统注意力机制在视觉任务中存在计算复杂度高和线性注意力无法聚焦的问题，需要一种更高效的替代方案。

Method: 通过数学定义广义注意力，提出SEMA机制，利用令牌定位避免分散并保持聚焦，同时通过算术平均捕捉全局注意力。

Result: 在Imagenet-1k分类任务中，SEMA优于线性注意力和近期视觉Mamba模型，尤其在更大规模图像上表现更佳。

Conclusion: SEMA是一种可扩展且高效的注意力机制，适合处理大规模视觉任务。
...
### [272] [OpenRR-1k: A Scalable Dataset for Real-World Reflection Removal](https://arxiv.org/abs/2506.08299)
*Kangning Yang,Ling Ouyang,Huiming Sun,Jie Cai,Lan Fu,Jiaming Ding,Chiu Man Ho,Zibo Meng*

Main category: cs.CV

TLDR: 提出了一种新的反射数据集收集方法，并发布了OpenRR-1k数据集，包含1000对高质量图像对，提升了反射去除技术的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有反射去除技术因缺乏高质量野外数据集而受限，需要一种便捷、经济且可扩展的数据收集方法。

Method: 提出了一种新颖的数据收集范式，确保数据高质量、对齐且多样化，并基于此收集了OpenRR-1k数据集。

Result: OpenRR-1k数据集通过实验验证了其在提升反射去除方法鲁棒性方面的有效性。

Conclusion: OpenRR-1k数据集为反射去除技术提供了高质量、多样化的数据支持，推动了该领域的发展。
...
### [273] [Hyperspectral Image Classification via Transformer-based Spectral-Spatial Attention Decoupling and Adaptive Gating](https://arxiv.org/abs/2506.08324)
*Guandong Li,Mengxia Ye*

Main category: cs.CV

TLDR: STNet是一种新型网络架构，通过空间-光谱Transformer模块的创新设计，有效解决了高光谱图像分类中的过拟合和泛化能力问题。
...
### [274] [Locating Tennis Ball Impact on the Racket in Real Time Using an Event Camera](https://arxiv.org/abs/2506.08327)
*Yuto Kase,Kai Ishibe,Ryoma Yasuda,Yudai Washida,Sakiko Hashimoto*

Main category: cs.CV

TLDR: 提出了一种使用事件相机实时定位网球拍击球位置的方法，解决了高速相机内存消耗大和手动数字化耗时的问题。


<details>
  <summary>Details</summary>
Motivation: 在网球等球拍运动中，击球位置的定位对个性化装备设计和性能分析至关重要，但现有方法存在内存消耗大和人工误差问题。

Method: 通过事件相机高效捕捉亮度变化，结合传统计算机视觉技术和原创事件处理（PATS）分三步识别击球位置。

Result: 实验结果在测量网球运动员性能的允许范围内，计算时间满足实时应用需求。

Conclusion: 该方法能有效实时定位击球位置，为运动员性能分析提供支持。
...
### [275] [How Much To Guide: Revisiting Adaptive Guidance in Classifier-Free Guidance Text-to-Vision Diffusion Models](https://arxiv.org/abs/2506.08351)
*Huixuan Zhang,Junzhe Zhang,Xiaojun Wan*

Main category: cs.CV

TLDR: 论文提出了一种名为Step AG的自适应引导策略，通过限制分类器自由引导在前几个去噪步骤中，显著提升了生成效率，同时保持图像质量和文本对齐。


<details>
  <summary>Details</summary>
Motivation: 分类器自由引导方法虽然流行，但计算成本高，且现有自适应引导方法缺乏分析和通用性。

Method: 提出Step AG策略，仅在前几个去噪步骤中应用分类器自由引导。

Result: 实验表明，该方法在图像质量和文本对齐上表现良好，平均提速20%-30%，且适用于多种模型和设置。

Conclusion: Step AG是一种简单、通用的自适应引导策略，显著提升了生成效率，适用于广泛的扩散模型。
...
### [276] [MedMoE: Modality-Specialized Mixture of Experts for Medical Vision-Language Understanding](https://arxiv.org/abs/2506.08356)
*Shivang Chopra,Lingchao Mao,Gabriela Sanchez-Rodriguez,Andrew J Feola,Jing Li,Zsolt Kira*

Main category: cs.CV

TLDR: MedMoE提出了一种动态适应医学影像模态的视觉语言处理框架，通过Mixture-of-Experts模块和多尺度特征提取，提升跨模态对齐和检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有医学视觉语言框架采用统一特征提取策略，忽视了不同模态的特定需求，导致诊断信息提取不足。

Method: MedMoE基于Swin Transformer提取多尺度特征，通过条件MoE模块动态路由到模态专家分支，实现空间自适应注意力。

Result: 实验表明，MedMoE在多种医学基准测试中提升了跨模态对齐和检索性能。
...
### [277] [Do Concept Replacement Techniques Really Erase Unacceptable Concepts?](https://arxiv.org/abs/2506.08991)
*Anudeep Das,Gurjot Singh,Prach Chantasantitam,N. Asokan*

Main category: cs.CV

TLDR: 本文探讨了生成模型中概念替换技术（CRTs）的局限性，特别是在图像到图像（I2I）场景中的失效问题，并提出了一种新的技术AntiMirror以同时实现有效性和保真度。


<details>
  <summary>Details</summary>
Motivation: 生成模型在避免生成不可接受内容（如冒犯性或受版权保护的内容）方面仍面临挑战，现有CRTs在I2I场景中表现不佳，需要新的解决方案。

Method: 通过I2I模型实证分析现有CRTs的不足，提出并验证了AntiMirror技术，该技术结合了目标图像编辑方法。

Result: 实验表明现有CRTs在I2I场景中无法有效替换不可接受概念，而AntiMirror在替换的同时保持了输入的其他概念（保真度）。

Conclusion: 需要针对I2I场景改进CRTs，AntiMirror展示了同时实现有效性和保真度的可行性。
...
### [278] [Image Demoiréing Using Dual Camera Fusion on Mobile Phones](https://arxiv.org/abs/2506.08361)
*Yanting Mei,Zhilu Zhang,Xiaohe Wu,Wangmeng Zuo*

Main category: cs.CV

TLDR: 论文提出了一种利用双摄像头融合（DCID）去除电子屏幕拍摄中摩尔纹的方法，通过超广角（UW）图像辅助广角（W）图像去摩尔纹，效果优于现有方法。
...
Result: 在包含约9,000样本的真实数据集上实验表明，该方法优于现有技术。

Conclusion: DCID方法有效解决了大范围摩尔纹去除问题，代码和数据集已开源。
...
### [279] [SECOND: Mitigating Perceptual Hallucination in Vision-Language Models via Selective and Contrastive Decoding](https://arxiv.org/abs/2506.08391)
*Woohyeon Park,Woojin Kim,Jaeik Kim,Jaeyoung Do*

Main category: cs.CV

TLDR: SECOND是一种新方法，通过选择性对比解码和多尺度视觉信息整合，显著减少视觉语言模型中的物体幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型因物体幻觉问题导致性能受限，需要更精确的视觉理解方法。

Method: 提出SECOND方法，通过选择性对比解码和多尺度视觉信息整合，以对象为中心逐步优化图像理解。

Result: SECOND显著减少幻觉问题，并在多个基准测试中表现优异。

Conclusion: 多尺度视觉信息的优先级和对比应用在视觉语言模型中有巨大潜力，优于现有方法。
...
### [280] [RadioDUN: A Physics-Inspired Deep Unfolding Network for Radio Map Estimation](https://arxiv.org/abs/2506.08418)
*Taiqin Chen,Zikun Zhou,Zheng Fang,Wenzhen Zou,Kanjun Liu,Ke Chen,Yongbing Zhang,Yaowei Wang*

Main category: cs.CV

TLDR: 论文提出了一种基于稀疏信号恢复和物理传播模型的无线电地图估计方法，通过深度展开网络（RadioDUN）和动态重加权模块（DRM）优化问题，并结合障碍物相关因素提升性能。
...
Conclusion: 通过结合物理特性和深度学习方法，显著提升了无线电地图估计的性能。
...
### [281] [Better Reasoning with Less Data: Enhancing VLMs Through Unified Modality Scoring](https://arxiv.org/abs/2506.08429)
*Mingjie Xu,Andrew Estornell,Hongzheng Yang,Yuzhi Zhao,Zhaowei Zhu,Qi Xuan,Jiaheng Wei*

Main category: cs.CV

TLDR: SCALE提出了一种基于数据质量驱动的视觉语言模型（VLM）指令调优数据集选择方法，解决了图像与文本对齐噪声和文本模糊问题。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型的效果依赖于高质量数据集，但存在图像与文本对齐噪声和文本模糊两大挑战，影响了模型的准确性和鲁棒性。

Method: SCALE通过跨模态评估框架，为数据条目分配任务、生成多类型描述，并评估对齐性、清晰度、任务稀有性、文本连贯性和图像清晰度。

Result: 研究发现，现有单模态质量评估方法低估了特定任务关键样本，而生成的图像描述能有效将多模态任务转化为统一文本模态。

Conclusion: SCALE为提升VLM性能提供了高效的数据选择方法，强调了跨模态评估的重要性。
...
### [282] [Enhancing Motion Dynamics of Image-to-Video Models via Adaptive Low-Pass Guidance](https://arxiv.org/abs/2506.08456)
*June Suk Choi,Kyungmin Lee,Sihyun Yu,Yisol Choi,Jinwoo Shin,Kimin Lee*

Main category: cs.CV

TLDR: 论文提出自适应低通引导（ALG）方法，解决图像到视频（I2V）生成中动态性不足的问题，显著提升视频动态性。
...
Result: ALG在VBench-I2V测试中动态性平均提升36%，且不影响视频质量或图像保真度。

Conclusion: ALG是一种简单有效的方法，显著提升I2V生成的动态性。
...
### [283] [MARMOT: Masked Autoencoder for Modeling Transient Imaging](https://arxiv.org/abs/2506.08470)
*Siyuan Shen,Ziheng Wang,Xingyue Peng,Suan Xia,Ruiqian Li,Shiying Li,Jingyi Yu*

Main category: cs.CV

TLDR: 提出了一种名为MARMOT的掩码自编码器，用于非视距（NLOS）瞬态成像任务，通过自监督预训练在大规模数据集上学习特征，并在下游任务中表现出高效性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在NLOS场景中主要优化体积密度或表面重建，而未利用数据集学习的先验知识，因此提出MARMOT以填补这一空白。

Method: 采用基于Transformer的编码器-解码器结构，通过扫描模式掩码（SPM）从部分掩码的瞬态数据中学习特征，并预测完整测量。

Result: 在TransVerse数据集上预训练的MARMOT，通过直接特征迁移或解码器微调，在下游任务中表现优于现有方法。
...
### [284] [Context-aware TFL: A Universal Context-aware Contrastive Learning Framework for Temporal Forgery Localization](https://arxiv.org/abs/2506.08493)
*Qilin Yin,Wei Lu,Xiangyang Luo,Xiaochun Cao*

Main category: cs.CV

TLDR: 该论文提出了一种通用的上下文感知对比学习框架（UniCaCLF），用于解决视频中部分片段被篡改的时域伪造定位（TFL）问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究多将深度伪造检测视为分类任务，忽略了视频中部分片段被篡改的情况，而TFL更符合实际应用场景。

Method: 采用监督对比学习，结合上下文感知感知层和自适应上下文更新器，构建对比目标以增强伪造特征的区分性。

Result: 在五个公开数据集上的实验表明，UniCaCLF显著优于现有算法。

Conclusion: UniCaCLF通过上下文感知对比学习，有效提升了时域伪造定位的性能。
...
### [285] [MLVTG: Mamba-Based Feature Alignment and LLM-Driven Purification for Multi-Modal Video Temporal Grounding](https://arxiv.org/abs/2506.08512)
*Zhiyi Zhu,Xiaoyu Wu,Zihao Liu,Linlin Yang*

Main category: cs.CV

TLDR: MLVTG提出了一种新框架，通过MambaAligner和LLMRefiner模块解决视频时间定位任务中的冗余注意力和多模态对齐问题，实现了更精确的定位。
...
Method: MLVTG结合MambaAligner（使用Vision Mamba块建模时间依赖）和LLMRefiner（利用预训练LLM的语义先验），实现双对齐策略。

Result: 在QVHighlights、Charades-STA和TVSum数据集上，MLVTG表现优于现有基线，达到SOTA性能。

Conclusion: MLVTG通过双对齐策略显著提升了视频时间定位任务的性能。
...
### [286] [Robust Visual Localization via Semantic-Guided Multi-Scale Transformer](https://arxiv.org/abs/2506.08526)
*Zhongtao Tian,Wenhao Huang,Zhidong Chen,Xiao Wei Sun*

Main category: cs.CV

TLDR: 提出了一种结合多尺度特征学习和语义场景理解的框架，用于动态环境中的视觉定位，通过层次化Transformer和语义监督提升性能。
...
Method: 采用层次化Transformer和跨尺度注意力融合几何细节与上下文信息，结合语义监督训练网络学习视角不变特征。
...
### [287] [LiftVSR: Lifting Image Diffusion to Video Super-Resolution via Hybrid Temporal Modeling with Only 4$\times$RTX 4090s](https://arxiv.org/abs/2506.08529)
*Xijun Wang,Xin Li,Bingchen Li,Zhibo Chen*

Main category: cs.CV

TLDR: LiftVSR提出了一种高效的视频超分辨率框架，通过结合动态时间注意力和注意力记忆缓存，显著降低了计算成本并提升了长期一致性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在视频超分辨率中存在时间一致性不足和高计算成本的问题，尤其是对长视频处理。

Method: LiftVSR采用混合时间建模机制，包括动态时间注意力（DTA）和注意力记忆缓存（AMC），并通过非对称采样策略优化缓存交互。

Result: 在多个VSR基准测试中，LiftVSR表现出色，计算成本显著降低。

Conclusion: LiftVSR在保持高效的同时，实现了视频超分辨率的长期一致性和高质量结果。
...
### [288] [TrajFlow: Multi-modal Motion Prediction via Flow Matching](https://arxiv.org/abs/2506.08541)
...
TLDR: TrajFlow是一个基于流匹配的运动预测框架，通过单次推理预测多模态轨迹，显著降低计算开销，并在WOMD数据集上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中高效准确的运动预测对安全和决策至关重要，尤其是在动态多模态场景下。

Method: 提出TrajFlow框架，结合流匹配和Plackett-Luce分布排序损失，并采用自条件训练技术。

Result: 在Waymo Open Motion Dataset上表现优异，达到最先进水平。

Conclusion: TrajFlow在计算效率和预测准确性上均表现出色，适用于安全关键的自动驾驶应用。
...
### [289] [Convergence of Spectral Principal Paths: How Deep Networks Distill Linear Representations from Noisy Inputs](https://arxiv.org/abs/2506.08543)
*Bowei Tian,Xuntao Lyu,Meng Liu,Hongyi Wang,Ang Li*

Main category: cs.CV

TLDR: 论文提出输入空间线性假设（ISLH），并引入谱主路径（SPP）框架，揭示深度网络如何沿主导谱方向逐步提取线性表示，验证其在视觉语言模型中的多模态鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 基于线性表示假设（LRH），研究概念对齐方向在输入空间中的起源及其在深度网络中的选择性放大，以增强AI的透明性和控制性。

Method: 提出ISLH假设和SPP框架，分析深度网络沿主导谱方向提取线性表示的过程，并在视觉语言模型中验证其多模态鲁棒性。

Result: SPP框架揭示了深度网络中线性表示的逐步提取机制，并证明其在多模态任务中的鲁棒性。

Conclusion: 该研究为深度网络表示形成提供了结构化理论，有助于提升AI的鲁棒性、公平性和透明性。
...
### [290] [From Pixels to Graphs: using Scene and Knowledge Graphs for HD-EPIC VQA Challenge](https://arxiv.org/abs/2506.08553)
*Agnese Taluzzi,Davide Gesualdi,Riccardo Santambrogio,Chiara Plizzari,Francesca Palermo,Simone Mentasti,Matteo Matteucci*

Main category: cs.CV

TLDR: SceneNet和KnowledgeNet用于HD-EPIC VQA挑战赛，分别利用场景图和外部知识提升视觉问答性能，组合后准确率达44.21%。


<details>
  <summary>Details</summary>
Motivation: 解决复杂自我中心视觉问答任务中细粒度对象交互和语义推理的需求。

Method: SceneNet通过多模态大语言模型生成场景图捕捉对象交互和时空关系；KnowledgeNet引入ConceptNet的外部常识知识增强语义推理。

Result: 在HD-EPIC基准测试的七类任务中表现优异，组合后准确率为44.21%。
...
### [291] [Towards Cross-Subject EMG Pattern Recognition via Dual-Branch Adversarial Feature Disentanglement](https://arxiv.org/abs/2506.08555)
*Xinyue Niu,Akira Furui*

Main category: cs.CV

TLDR: 提出一种基于特征解耦的双分支对抗神经网络，无需校准即可实现跨被试EMG模式识别。


<details>
  <summary>Details</summary>
Motivation: 跨被试EMG模式识别因个体差异面临挑战，传统方法依赖校准数据，耗时且不实用。

Method: 使用双分支对抗神经网络，将EMG特征解耦为模式相关和个体相关部分。

Result: 模型在未见用户数据上表现优异，优于基线方法。

Conclusion: 该方法为无校准跨被试EMG识别提供了新思路，并具有广泛应用的潜力。
...
### [292] [Hierarchical Neural Collapse Detection Transformer for Class Incremental Object Detection](https://arxiv.org/abs/2506.08562)
*Duc Thanh Pham,Hong Dang Nguyen,Nhat Minh Nguyen Quoc,Linh Ngo Van,Sang Dinh Viet,Duc Anh Nguyen*

Main category: cs.CV

TLDR: 提出了一种名为Hier-DETR的新框架，通过利用神经崩溃和类标签的层次关系，解决了增量目标检测中的效率和性能问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界中新物体不断出现，需要检测模型持续学习而不受灾难性遗忘的影响，现有增量目标检测模型性能有限且推理时间长。
...
### [293] [Generating Vision-Language Navigation Instructions Incorporated Fine-Grained Alignment Annotations](https://arxiv.org/abs/2506.08566)
*Yibo Cui,Liang Xie,Yu Zhao,Jiawei Sun,Erwei Yin*

Main category: cs.CV

TLDR: 论文提出FCA-NIG框架，自动生成具有细粒度跨模态标注的导航指令，显著提升VLN智能体的导航性能。


<details>
  <summary>Details</summary>
Motivation: 现有数据集缺乏细粒度的跨模态对齐标注，限制了导航动作决策的准确性。

Method: 通过分割轨迹、地标检测、指令生成和实体选择，生成具有子指令-轨迹和实体-地标对齐的FCA-R2R数据集。

Result: 实验表明，FCA-R2R显著提升了多个VLN智能体的性能，增强了状态感知和导航泛化能力。
...
### [294] [Diversity-Guided MLP Reduction for Efficient Large Vision Transformers](https://arxiv.org/abs/2506.08591)
*Chengchao Shen,Hourun Zhu,Gongfan Fang,Jianxin Wang,Xinchao Wang*

Main category: cs.CV

TLDR: 论文提出了一种多样性引导的MLP压缩方法（DGMR），显著减少大型视觉Transformer的参数和计算量，同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 大型Transformer模型的参数过多导致计算和内存成本高昂，MLP模块占据大部分参数，因此需要高效压缩方法。

Method: 采用Gram-Schmidt权重剪枝策略消除MLP隐藏层的冗余神经元，同时保留权重多样性以在蒸馏过程中恢复性能。

Result: 在多个大型视觉Transformer上，DGMR实现了57%以上的参数和FLOPs减少，且性能几乎无损。对于EVA-CLIP-E（4.4B），参数和FLOPs减少71.5%且无性能下降。

Conclusion: DGMR是一种高效的模型压缩方法，显著减少资源消耗，同时保持性能，适用于大规模视觉Transformer。
...
### [295] [Transformers Meet Hyperspectral Imaging: A Comprehensive Study of Models, Challenges and Open Problems](https://arxiv.org/abs/2506.08596)
*Guyang Zhang,Waleed Abdulla*

Main category: cs.CV

TLDR: 本文是第一篇专注于基于Transformer的高光谱图像（HSI）分类的端到端综述，总结了300多篇论文，分析了Transformer在HSI中的应用及其设计选择。
...
Result: 总结了HSI领域的主要挑战，如标注数据稀缺、高光谱维度、计算开销和模型可解释性不足。

Conclusion: 提出了未来研究方向，包括公共数据集、轻量化模型、鲁棒性和可解释性机制，以推动下一代HSI应用的发展。
...
### [296] [Towards Class-wise Fair Adversarial Training via Anti-Bias Soft Label Distillation](https://arxiv.org/abs/2506.08611)
*Shiji Zhao,Chi Chen,Ranjie Duan,Xizhe Wang,Xingxing Wei*

Main category: cs.CV

TLDR: 本文提出了一种名为ABSLD的方法，通过调整教师模型软标签的平滑度来提升对抗鲁棒性的公平性。


<details>
  <summary>Details</summary>
Motivation: AT和ARD方法存在对抗鲁棒性公平性问题，即模型对某些类别表现强鲁棒性，而对其他类别表现弱鲁棒性。本文旨在解决这一问题。

Method: 提出ABSLD方法，通过为不同类别分配不同温度参数，调整教师模型软标签的平滑度，从而减少学生模型在不同类别上的误差风险差距。

Result: 实验表明，ABSLD在鲁棒性和公平性的综合表现上优于现有方法。

Conclusion: ABSLD是一种有效提升对抗鲁棒性公平性的方法，且易于与其他样本方法结合。
...
### [297] [Data-Efficient Challenges in Visual Inductive Priors: A Retrospective](https://arxiv.org/abs/2506.08612)
*Robert-Jan Bruintjes,Attila Lengyel,Osman Semih Kayhan,Davide Zambrano,Nergis Tömen,Hadi Jamali-Rad,Jan van Gemert*

Main category: cs.CV

TLDR: 论文探讨了在数据不足情况下提升深度学习模型性能的方法，通过组织数据受限挑战赛，鼓励开发结合先验知识的新方法。


<details>
  <summary>Details</summary>
Motivation: 解决深度学习在数据不足时性能下降的问题，探索如何利用先验知识提升数据效率。

Method: 组织数据受限挑战赛，限制参与者使用少量样本从头训练模型，禁止迁移学习，鼓励开发新方法。

Result: 成功参赛方案结合了Transformer与CNN的大型模型集成、大量数据增强及基于先验知识的方法。
...
### [298] [SAMSelect: A Spectral Index Search for Marine Debris Visualization using Segment Anything](https://arxiv.org/abs/2506.08613)
*Joost van Dalen,Yuki M. Asano,Marc Russwurm*

Main category: cs.CV

TLDR: SAMSelect是一种算法，用于从多光谱图像中选择最佳三通道可视化组合，以帮助海洋科学家更直观地识别海洋漂浮物。


<details>
  <summary>Details</summary>
Motivation: 海洋漂浮物因其成分异质性在中等分辨率图像中难以可视化，而专家通常依赖经验和启发式方法手动选择波段或光谱指数。

Method: SAMSelect通过Segment Anything Model在小标注数据集上选择分类准确率最高的波段或指数组合，假设最佳分割结果的三通道可视化也能提供良好的视觉信息。

Result: 在加纳阿克拉和南非德班的Sentinel-2场景中测试，发现新的未使用波段组合（如B8和B2的归一化差异指数）优于文献中的指数。

Conclusion: SAMSelect为海洋领域的视觉解译提供了有效工具，并开源了代码库。
...
### [299] [A Probability-guided Sampler for Neural Implicit Surface Rendering](https://arxiv.org/abs/2506.08619)
*Gonçalo Dias Pais,Valter Piedade,Moitreya Chatterjee,Marcus Greiff,Pedro Miraldo*

Main category: cs.CV

TLDR: 该论文提出了一种改进的NeRF方法，通过隐式表面表示和3D图像投影空间中的概率密度函数，实现了更精准的光线采样，并结合新的表面重建损失，提升了3D重建和图像渲染的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有NeRF方法因可扩展性问题无法对所有输入数据（如像素和3D点）进行训练，导致采样效率低。本文旨在通过针对性采样和新型损失函数优化渲染效果。

Method: 利用隐式表面表示和3D图像投影空间中的概率密度函数实现目标区域的光线采样，并引入结合近表面和空白空间分量的新型表面重建损失。

Result: 通过整合新采样策略和损失函数，显著提升了3D重建和图像渲染的准确性，特别是在目标区域。
...
### [300] [ECMNet:Lightweight Semantic Segmentation with Efficient CNN-Mamba Network](https://arxiv.org/abs/2506.08629)
*Feixiang Du,Shengkun Wu*

Main category: cs.CV

TLDR: 提出了一种轻量级的CNN-Mamba网络（ECMNet）用于语义分割，结合CNN和Mamba的优势，通过增强的双注意力块和多尺度注意力单元提升性能。
...
Method: 设计了增强的双注意力块（EDAB）作为轻量级瓶颈，多尺度注意力单元（MSAU）整合多尺度特征，以及Mamba增强的特征融合模块（FFM）。

Result: 在Cityscapes和CamVid数据集上分别达到70.6%和73.6%的mIoU，参数量为0.87M，计算量为8.27G FLOPs。

Conclusion: ECMNet在准确性和效率之间取得了良好平衡，验证了CNN与Mamba结合的潜力。
...
### [301] [RoboSwap: A GAN-driven Video Diffusion Framework For Unsupervised Robot Arm Swapping](https://arxiv.org/abs/2506.08632)
*Yang Bai,Liudi Yang,George Eskandar,Fengyi Shen,Dong Chen,Mohammad Altillawi,Ziyuan Liu,Gitta Kutyniok*

Main category: cs.CV

TLDR: RoboSwap是一种新型视频编辑框架，通过结合GAN和扩散模型，实现机器人手臂的无配对数据跨环境替换，提升跨平台机器人学习的泛化能力。
...
Method: 提出RoboSwap框架，结合GAN和扩散模型，先分割机器人手臂并翻译，再通过扩散模型增强视频连贯性和运动真实性。

Result: 在三个基准测试中表现优于现有视频和图像编辑模型，结构连贯性和运动一致性更优。
...
### [302] [SurfR: Surface Reconstruction with Multi-scale Attention](https://arxiv.org/abs/2506.08635)
*Siddhant Ranade,Gonçalo Dias Pais,Ross Tyler Whitaker,Jacinto C. Nascimento,Pedro Miraldo,Srikumar Ramalingam*

Main category: cs.CV

TLDR: 提出了一种基于隐式表示的快速、准确的无组织点云表面重建算法，通过三个关键贡献实现了最佳精度与速度的平衡。


<details>
  <summary>Details</summary>
Motivation: 现有学习方法要么需要针对单个对象训练的小模型（细节丰富但泛化性差），要么是泛化性强但细节不足的大模型（推理速度慢）。本文旨在解决这一矛盾。

Method: 1. 延迟查询（lazy query）加速重建；2. 并行多尺度网格表示处理噪声和分辨率；3. 跨尺度注意力提升重建效果。

Result: 提出的隐式表示在速度上优于所有基线方法，性能仅略低于最先进方法。

Conclusion: 该方法在精度与速度之间取得了最佳平衡，适用于通用3D形状重建。
...
### [303] [Orientation Matters: Making 3D Generative Models Orientation-Aligned](https://arxiv.org/abs/2506.08640)
*Yichong Lu,Yuzhuo Tian,Zijin Jiang,Yikun Zhao,Yuanbo Yang,Hao Ouyang,Haoji Hu,Huimin Yu,Yujun Shen,Yiyi Liao*

Main category: cs.CV

TLDR: 论文提出了一种方向对齐的3D物体生成任务，并构建了Objaverse-OA数据集，通过微调两种3D生成模型，实现了跨类别的一致方向生成。


<details>
  <summary>Details</summary>
Motivation: 现有3D生成模型因训练数据不一致导致方向错位，限制了其在下游任务中的应用。

Method: 构建Objaverse-OA数据集，微调多视图扩散和3D变分自编码器模型。

Result: 实验表明，该方法优于后处理对齐方法，并展示了零样本方向估计和高效旋转操作等下游应用。
...
### [304] [Enhancing Video Memorability Prediction with Text-Motion Cross-modal Contrastive Loss and Its Application in Video Summarization](https://arxiv.org/abs/2506.08649)
*Zhiyi Zhu,Xiaoyu Wu,Youwei Lu*

Main category: cs.CV

TLDR: 本文提出了一种新的多模态视频记忆性预测模型TMCCL，通过文本-运动跨模态对比损失增强运动特征表示，并在两个数据集上取得最佳性能。同时，提出MWCVS方法，利用视频记忆性预测减少视频摘要标注的主观性。


<details>
  <summary>Details</summary>
Motivation: 现有模型未能充分利用运动线索，且运动特征表示在微调阶段因缺乏标注数据而受限。

Method: 引入TMCCL模型，利用文本描述相似性构建正负运动样本集，提升运动特征表示。

Result: 在两个视频记忆性预测数据集上取得最优性能，并验证了MWCVS在视频摘要中的有效性。
...
### [305] [Beyond Calibration: Physically Informed Learning for Raw-to-Raw Mapping](https://arxiv.org/abs/2506.08650)
*Peter Grönquist,Stepan Tulyakov,Dengxin Dai*

Main category: cs.CV

TLDR: 论文提出了一种轻量级的神经物理模型（NPM），用于解决多相机间颜色一致性问题，适应性强且计算高效。


<details>
  <summary>Details</summary>
Motivation: 多相机系统中因传感器和光学差异导致颜色一致性难以实现，现有方法在适应性、计算成本或实用性上存在不足。

Method: 提出NPM，通过模拟指定光照下的原始图像来估计设备间的转换，支持物理测量初始化，并可无需配对数据训练。
...
### [306] [LLaVA-c: Continual Improved Visual Instruction Tuning](https://arxiv.org/abs/2506.08666)
*Wenzhuo Liu,Fei Zhu,Haiyang Guo,Longhui Wei,Cheng-Lin Liu*

Main category: cs.CV

TLDR: LLaVA-c通过光谱感知巩固和无监督询问正则化改进LLaVA-1.5，解决多任务学习中的任务平衡和基础模型退化问题，在持续学习中表现优异。
...
Method: 在LLaVA-1.5基础上引入光谱感知巩固和无监督询问正则化。

Result: LLaVA-c在持续预训练和微调中提升基准性能并保持通用能力，首次实现任务级持续学习超越多任务联合学习。

Conclusion: LLaVA-c为持续学习提供有效解决方案，代码将开源。
...
### [307] [ATAS: Any-to-Any Self-Distillation for Enhanced Open-Vocabulary Dense Prediction](https://arxiv.org/abs/2506.08678)
*Juan Yeo,Soonwoo Cha,Jiwoo Song,Hyunbin Jin,Taesup Kim*

Main category: cs.CV

TLDR: 论文提出了一种名为ATAS的新方法，通过自蒸馏技术提升CLIP模型在细粒度视觉语言对齐和语义一致性上的表现，无需额外模块或有监督微调。


<details>
  <summary>Details</summary>
Motivation: CLIP模型在细粒度和区域级理解上表现不足，影响了其在密集预测任务中的效果。

Method: 提出Any-to-Any Self-Distillation (ATAS)，利用无标注图像和内部自蒸馏过程优化CLIP视觉编码器的表示。
...
### [308] [CanadaFireSat: Toward high-resolution wildfire forecasting with multiple modalities](https://arxiv.org/abs/2506.08690)
*Hugo Porta,Emanuele Dalsasso,Jessica L. McCarty,Devis Tuia*

Main category: cs.CV

TLDR: 加拿大2023年经历了近年来最严重的野火季节，凸显气候变化对火灾季节的影响。本文提出高分辨率野火预测方法，利用多模态数据提升预测精度。


<details>
  <summary>Details</summary>
Motivation: 气候变化导致火灾季节延长和加剧，亟需更好的野火管理工具。高分辨率预测模型可弥补现有方法的不足。

Method: 利用多模态数据（Sentinel-2、MODIS、ERA5）和深度学习模型，开发100米分辨率的野火预测基准数据集CanadaFireSat。

Result: 多模态输入优于单模态输入，F1分数达60.3%，适用于未见过的2023年野火季节。
...
### [309] [VReST: Enhancing Reasoning in Large Vision-Language Models through Tree Search and Self-Reward Mechanism](https://arxiv.org/abs/2506.08691)
*Congzhi Zhang,Jiawei Peng,Zhenglin Wang,Yilong Lai,Haowen Sun,Heng Chang,Fei Ma,Weijiang Yu*

Main category: cs.CV

TLDR: VReST是一种无需训练的方法，通过蒙特卡洛树搜索和自奖励机制提升大型视觉语言模型（LVLM）的推理能力，在多项基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型在复杂视觉推理任务中表现受限，尤其是使用思维链提示技术时。

Method: 提出VReST方法，利用蒙特卡洛树搜索构建推理路径，并通过自奖励机制评估推理步骤质量。

Result: VReST在三个多模态数学推理基准测试中超越现有方法，达到最优性能。

Conclusion: VReST展示了测试时间扩展定律在多模态任务中的有效性，为未来研究提供了新方向。
...
### [310] [MoSiC: Optimal-Transport Motion Trajectory for Dense Self-Supervised Learning](https://arxiv.org/abs/2506.08694)
*Mohammadreza Salehi,Shashanka Venkataramanan,Ioana Simion,Efstratios Gavves,Cees G. M. Snoek,Yuki M Asano*

Main category: cs.CV

TLDR: 提出了一种基于运动引导的自监督学习框架，通过聚类密集点轨迹学习时空一致的表示，提升了动态场景中的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖静态增强，难以处理视频中的物体变形、遮挡和相机运动，导致特征学习不一致。
...
### [311] [ArrowPose: Segmentation, Detection, and 5 DoF Pose Estimation Network for Colorless Point Clouds](https://arxiv.org/abs/2506.08699)
*Frederik Hagelskjaer*

Main category: cs.CV

TLDR: 提出了一种快速检测和5自由度姿态估计网络，适用于无色点云。
...
Method: 通过神经网络预测物体的中心和顶部点，计算姿态。训练使用合成数据，测试在基准数据集上进行。

Result: 在基准测试中表现最优，速度快（250毫秒），适用于多种场景。

Conclusion: 该方法在无色点云姿态估计中实现了高效和高精度。
...
### [312] [TraGraph-GS: Trajectory Graph-based Gaussian Splatting for Arbitrary Large-Scale Scene Rendering](https://arxiv.org/abs/2506.08704)
*Xiaohan Zhang,Sitong Wang,Yushen Yan,Yi Yang,Mingda Xu,Qi Liu*

Main category: cs.CV

TLDR: 论文提出TraGraph-GS方法，通过轨迹图和空间分区技术解决大规模场景中高质量新视角合成的挑战，显著提升了渲染精度和效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在大规模场景中难以泛化，主要因刚性空间分区和区域合并导致的高斯重叠问题。

Method: 提出基于轨迹图的空间分区方法，结合正则化约束和渐进渲染策略，减少高斯重叠并提升纹理渲染质量。

Result: 实验表明，TraGraph-GS在四个空中和四个地面数据集上PSNR平均提升1.86 dB和1.62 dB，优于现有方法。

Conclusion: TraGraph-GS有效解决了大规模场景渲染中的泛化和细节失真问题，具有显著性能优势。
...
### [313] [SceneSplat++: A Large Dataset and Comprehensive Benchmark for Language Gaussian Splatting](https://arxiv.org/abs/2506.08710)
...
TLDR: 论文提出了首个大规模基准测试，用于系统评估3D高斯泼溅（3DGS）在语言理解中的三种方法，并引入了一个包含49K场景的数据集GaussianWorld-49K。


<details>
  <summary>Details</summary>
Motivation: 当前3DGS语言理解方法主要在少量场景和视角下评估，缺乏对整体3D理解的深入分析。

Method: 提出大规模基准测试，评估1060个场景，并引入GaussianWorld-49K数据集。

Result: 通用性方法表现最佳，能快速推理新场景并实现优越的分割性能。

Conclusion: 通用性方法在3DGS场景理解中具有优势，数据集和代码将公开以推动研究。
...
### [314] [Geometric deep learning for local growth prediction on abdominal aortic aneurysm surfaces](https://arxiv.org/abs/2506.08729)
*Dieuwertje Alblas,Patryk Rygiel,Julian Suk,Kaj O. Kappe,Marieke Hofman,Christoph Brune,Kak Khee Yeung,Jelmer M. Wolterink*

Main category: cs.CV

TLDR: 论文提出了一种基于SE(3)-对称变换器模型的方法，用于预测腹主动脉瘤（AAA）的生长，通过保留血管表面的解剖结构和几何保真度，实现了个性化监测策略。


<details>
  <summary>Details</summary>
Motivation: 当前AAA监测策略仅基于最大直径，忽略了3D形状与生长的复杂关系，可能导致监测间隔不准确。个性化生长预测可优化监测。

Method: 使用SE(3)-对称变换器模型，直接在血管模型表面结合局部多物理特征预测AAA生长。训练数据为24名患者的113次CTA扫描。

Result: 模型预测AAA生长的中位直径误差为1.18 mm，并能以0.93的准确率识别患者是否在两年内需手术修复。外部验证集结果也支持模型的泛化能力。

Conclusion: 局部方向性AAA生长预测可行，有望为个性化监测策略提供支持。
...
### [315] [InceptionMamba: An Efficient Hybrid Network with Large Band Convolution and Bottleneck Mamba](https://arxiv.org/abs/2506.08735)
*Yuhang Wang,Jun Li,Zhijian Wu,Jianhua Xu*

Main category: cs.CV

TLDR: InceptionMamba是一种新型的卷积神经网络架构，通过正交带卷积和瓶颈Mamba模块解决了InceptionNeXt在空间依赖性和全局上下文建模上的不足，实现了卓越的分类和下游任务性能。


<details>
  <summary>Details</summary>
Motivation: InceptionNeXt在图像分类和下游任务中表现优异，但其一维条带卷积限制了空间依赖性的捕获和局部邻域的空间建模能力，且卷积操作的局部性约束不利于全局上下文建模。

Method: 提出InceptionMamba架构，用正交带卷积替代传统一维条带卷积以实现空间建模，并通过瓶颈Mamba模块实现全局上下文建模，增强跨通道信息融合和扩大感受野。

Result: 在分类和多种下游任务上的广泛评估表明，InceptionMamba实现了最先进的性能，同时具有优越的参数和计算效率。

Conclusion: InceptionMamba通过改进空间建模和全局上下文建模能力，显著提升了性能，代码将开源。
...
### [316] [RS-MTDF: Multi-Teacher Distillation and Fusion for Remote Sensing Semi-Supervised Semantic Segmentation](https://arxiv.org/abs/2506.08772)
*Jiayi Song,Kaiyu Li,Xiangyong Cao,Deyu Meng*

Main category: cs.CV

TLDR: 论文提出了一种基于视觉基础模型（VFMs）的半监督语义分割框架RS-MTDF，通过多教师蒸馏与融合技术，显著提升了遥感图像分割的性能。


<details>
  <summary>Details</summary>
Motivation: 遥感图像的语义分割依赖大量标注数据，但标注成本高昂。半监督学习（SSS）可缓解这一问题，但现有方法因数据分布不匹配而泛化能力不足。

Method: 利用预训练的VFMs（如DINOv2和CLIP）作为教师模型，通过特征级蒸馏将学生模型的特征与教师模型对齐，并将知识融合到学生解码器中。

Result: 在ISPRS Potsdam、LoveDA和DeepGlobe数据集上，RS-MTDF表现优异，尤其在LoveDA上以不同标注比例超越现有方法，并在多数语义类别中取得最高IoU。

Conclusion: 多教师VFM指导显著提升了遥感分割的泛化能力和语义理解，各模块的贡献通过消融实验得到验证。
...
### [317] [Gaussian2Scene: 3D Scene Representation Learning via Self-supervised Learning with 3D Gaussian Splatting](https://arxiv.org/abs/2506.08777)
*Keyi Liu,Weidong Yang,Ben Fei,Ying He*

Main category: cs.CV

TLDR: 论文提出了一种名为Gaussian2Scene的新型自监督学习框架，利用3D高斯泼溅（3DGS）进行点云预训练，解决了现有方法依赖隐式场景表示和高内存需求的问题。


<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法在场景级别依赖体积渲染和RGB-D图像，存在内存需求高且难以捕捉3D几何结构的问题。

Method: 采用3DGS进行预训练，分两阶段：第一阶段通过双分支掩码自编码器学习2D和3D场景表示；第二阶段利用重建点云和高斯基元的几何位置进行监督学习。

Result: 在多个下游3D目标检测任务中，Gaussian2Scene表现优于现有预训练方法。

Conclusion: Gaussian2Scene通过显式3D场景重建和渐进式训练策略，有效提升了模型的几何理解和跨模态学习能力。
...
### [318] [Landsat-Bench: Datasets and Benchmarks for Landsat Foundation Models](https://arxiv.org/abs/2506.08780)
*Isaac Corley,Lakshay Sharma,Ruth Crasto*

Main category: cs.CV

TLDR: Landsat-Bench是一套基于Landsat影像的基准测试，用于评估地理空间基础模型（GFM）的性能，并证明SSL4EO-L预训练模型优于ImageNet。
...
Conclusion: Landsat-Bench为Landsat数据提供了标准化评估工具，并验证了SSL4EO-L预训练模型的有效性。
...
### [319] [HomographyAD: Deep Anomaly Detection Using Self Homography Learning](https://arxiv.org/abs/2506.08784)
*Jongyub Seok,Chanjin Kang*

Main category: cs.CV

TLDR: 提出了一种基于ImageNet预训练网络的新方法HomographyAD，用于解决工业环境中数据未对齐的异常检测问题。


<details>
  <summary>Details</summary>
Motivation: 现有异常检测方法仅适用于完全对齐的数据集，而真实工业环境中的数据往往未对齐，限制了其应用。

Method: 通过深度单应性估计方法对齐输入前景，并通过自单应性学习微调模型以学习正常样本的形状信息，最后基于测试样本特征与正常特征分布的距离进行异常检测。

Result: 实验表明，该方法能显著提升现有异常检测方法的性能。

Conclusion: HomographyAD为解决工业环境中数据未对齐的异常检测问题提供了一种有效方法。
...
### [320] [A PDE-Based Image Dehazing Method via Atmospheric Scattering Theory](https://arxiv.org/abs/2506.08793)
*Zhuoran Zheng*

Main category: cs.CV

TLDR: 本文提出了一种基于偏微分方程（PDE）的单幅图像去雾方法，结合大气散射模型、非局部正则化和暗通道先验，改进了PDE框架，并通过实验验证了其有效性。
...
Method: 提出改进的PDE方程，结合边缘保持扩散系数、高斯卷积算子和自适应正则化参数，利用Lax-Milgram定理证明解的存在唯一性，并通过PyTorch GPU加速实现固定点迭代。

Result: 实验结果表明该方法是一种有效的去雾解决方案，并可推广至深度学习模型。

Conclusion: 该PDE框架为单幅图像去雾提供了新的解决方案，具有理论和实践意义。
...
### [321] [Flow Diverse and Efficient: Learning Momentum Flow Matching via Stochastic Velocity Field Sampling](https://arxiv.org/abs/2506.08796)
*Zhiyuan Ma,Ruixun Liu,Sixian Liu,Jianjun Li,Bowen Zhou*

Main category: cs.CV

TLDR: Discretized-RF通过将直线路径离散化为可变速度子路径，提升多样性和多尺度噪声建模能力。


<details>
  <summary>Details</summary>
Motivation: 解决传统直线路径在多样性和多尺度噪声建模上的局限性。

Method: 引入动量流模型，离散化路径并随机化速度方向。

Result: 实验表明，该方法能高效生成多样且高质量的样本。

Conclusion: Discretized-RF在多样性和效率上优于传统直线路径方法。
...
### [322] [HunyuanVideo-HOMA: Generic Human-Object Interaction in Multimodal Driven Human Animation](https://arxiv.org/abs/2506.08797)
...
TLDR: HunyuanVideo-HOMA是一个弱条件多模态驱动框架，用于提升人-物交互（HOI）视频生成的泛化性和可控性，减少对精确输入的依赖。


<details>
  <summary>Details</summary>
Motivation: 解决HOI视频生成中对精选运动数据的依赖、对新物体/场景的泛化能力有限以及可访问性受限等问题。

Method: 通过稀疏解耦的运动引导，将外观和运动信号编码到多模态扩散变换器（MMDiT）的双输入空间，并在共享上下文空间中融合。结合参数空间HOI适配器和面部交叉注意力适配器优化训练。

Result: 在弱监督下实现了交互自然性和泛化性的最先进性能，支持文本条件生成和交互式物体操作。

Conclusion: HunyuanVideo-HOMA展示了在HOI视频生成中的多功能性和高效性，提供了一个用户友好的演示界面。
...
### [323] [HiSin: Efficient High-Resolution Sinogram Inpainting via Resolution-Guided Progressive Inference](https://arxiv.org/abs/2506.08809)
*Jiaze E,Srutarshi Banerjee,Tekin Bicer,Guannan Wang,Yanfu Zhang,Bin Ren*

Main category: cs.CV

TLDR: HiSin是一种基于扩散模型的高效正弦图修复框架，通过分辨率引导的渐进推理减少内存和计算需求，同时保持修复精度。
...
Conclusion: HiSin通过高效推理策略解决了高分辨率正弦图修复的内存和计算问题，具有实际应用潜力。
...
### [324] [Video-CoT: A Comprehensive Dataset for Spatiotemporal Understanding of Videos Based on Chain-of-Thought](https://arxiv.org/abs/2506.08817)
*Shuyi Zhang,Xiaoshuai Hao,Yingbo Tang,Lingfeng Zhang,Pengwei Wang,Zhongyuan Wang,Hongxuan Ma,Shanghang Zhang*

Main category: cs.CV

TLDR: Video-CoT是一个新数据集，旨在通过Chain-of-Thought方法提升视频时空理解能力，包含大量标注数据，并揭示了当前视觉语言模型在此任务上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在捕捉视频时空细节方面表现不足，需要更精细的数据集和方法来提升理解能力。

Method: 引入Video-CoT数据集，包含192,000个时空问答对和23,000个CoT标注样本，并提供评估基准。

Result: 实验显示当前模型在时空理解任务上表现不佳，凸显了任务的挑战性。

Conclusion: Video-CoT为视频理解研究提供了新方向，支持未来智能系统的创新。
...
### [325] [CulturalFrames: Assessing Cultural Expectation Alignment in Text-to-Image Models and Evaluation Metrics](https://arxiv.org/abs/2506.08835)
...
TLDR: 该研究首次系统量化了文本到图像（T2I）模型在显性和隐性文化期望上的表现，发现模型在44%的情况下未能满足文化期望，且现有评估指标与人类判断相关性低。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决T2I模型在多样化文化背景下的表现不足问题，尤其是其对显性和隐性文化期望的忽视。

Method: 研究引入了CulturalFrames基准，涵盖10个国家、5个社会文化领域，包含983个提示、3637张图像和超过10k条人类标注。

Result: 结果显示，T2I模型在显性期望上的失败率为68%，隐性期望为49%，且现有评估指标与人类判断相关性差。
...
### [326] [Adapting Vision-Language Foundation Model for Next Generation Medical Ultrasound Image Analysis](https://arxiv.org/abs/2506.08849)
...
TLDR: 本文提出了一种针对医学超声图像的视觉-语言基础模型领域适应方法，通过大语言模型作为文本精炼器和任务驱动头，显著提升了超声图像分割和分类的性能。


<details>
  <summary>Details</summary>
Motivation: 医学超声图像分析中手动标注区域耗时且存在不一致性，视觉-语言基础模型在医学领域的性能受限于与自然图像的差异。

Method: 采用大语言模型作为文本精炼器，结合特殊设计的适应策略和任务驱动头，对视觉-语言基础模型进行微调。

Result: 在六个超声数据集上的实验表明，该方法显著提升了模型性能，优于现有视觉-语言和纯基础模型。

Conclusion: 该方法为医学超声图像分析提供了一种高效的解决方案，代码已开源。
...
### [327] [Spatial Transcriptomics Expression Prediction from Histopathology Based on Cross-Modal Mask Reconstruction and Contrastive Learning](https://arxiv.org/abs/2506.08854)
*Junzhuo Liu,Markus Eckstein,Zhixiang Wang,Friedrich Feuerhake,Dorit Merhof*

Main category: cs.CV

TLDR: 提出一种基于对比学习的深度学习方法，从全切片图像预测空间转录组数据，显著提升了高表达基因、高变异基因和标记基因的预测准确性。
...
### [328] [StreamSplat: Towards Online Dynamic 3D Reconstruction from Uncalibrated Video Streams](https://arxiv.org/abs/2506.08862)
*Zike Wu,Qi Yan,Xuanyu Yi,Lele Wang,Renjie Liao*

Main category: cs.CV

TLDR: StreamSplat是一个实时处理未校准视频流并重建动态3D场景的框架，解决了实时性、动态建模和长期稳定性的挑战。
...
Conclusion: StreamSplat是首个完全前馈的框架，能够高效处理未校准视频流并重建动态3D场景。
...
### [329] [DiscoVLA: Discrepancy Reduction in Vision, Language, and Alignment for Parameter-Efficient Video-Text Retrieval](https://arxiv.org/abs/2506.08887)
*Leqi Shen,Guoqiang Gong,Tianxiang Hao,Tao He,Yifeng Zhang,Pengzhang Liu,Sicheng Zhao,Jungong Han,Guiguang Ding*

Main category: cs.CV

TLDR: DiscoVLA通过减少视觉、语言和对齐的差异，提升了CLIP模型在视频-文本检索中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注视觉差异，忽视了语言和对齐的差异，导致视频-文本检索效果不佳。

Method: 提出Image-Video Features Fusion整合图像和视频特征，生成伪图像标题学习细粒度对齐，并通过Image-to-Video Alignment Distillation增强视频级对齐。

Result: 在MSRVTT数据集上，DiscoVLA以50.5%的R@1得分优于之前方法1.5%。

Conclusion: DiscoVLA通过同时解决视觉、语言和对齐的差异，显著提升了视频-文本检索性能。
...
TLDR: 提出了一种基于Product of Experts（PoE）框架的推理时知识组合方法，通过Annealed Importance Sampling（AIS）从异构模型中采样，提升图像和视频合成的可控性和灵活性。


<details>
  <summary>Details</summary>
Motivation: 现代神经模型在共享数据领域（如图像和视频）具有丰富的先验知识和互补性，但如何整合来自不同来源（如视觉生成模型、视觉语言模型、图形引擎和物理模拟器）的多样化知识尚未充分探索。

Method: 采用Product of Experts（PoE）框架，通过Annealed Importance Sampling（AIS）从异构模型的乘积分布中采样，实现无需训练的推理时知识组合。

Result: 在图像和视频合成任务中表现出实用优势，比单一方法具有更好的可控性，并提供灵活的用户界面以指定视觉生成目标。

Conclusion: 提出的PoE框架为异构模型的知识组合提供了一种有效且灵活的方法，显著提升了视觉生成任务的可控性和用户交互体验。
...
### [331] [WetCat: Automating Skill Assessment in Wetlab Cataract Surgery Videos](https://arxiv.org/abs/2506.08896)
*Negin Ghamsarian,Raphael Sznitman,Klaus Schoeffmann,Jens Kowal*

Main category: cs.CV

TLDR: WetCat是首个专为自动化技能评估设计的湿实验室白内障手术视频数据集，解决了传统手动评估效率低、主观性强的问题。
...
Result: WetCat支持开发基于AI的客观评估工具，提升手术培训的可扩展性和标准化。

Conclusion: WetCat为眼科手术培训的自动化技能评估奠定了基础，并设定了新标准。
...
### [332] [MIRAGE: Multimodal foundation model and benchmark for comprehensive retinal OCT image analysis](https://arxiv.org/abs/2506.08900)
...
TLDR: MIRAGE是一种新型多模态基础模型，用于分析OCT和SLO图像，并在分类和分割任务中表现优越。


<details>
  <summary>Details</summary>
Motivation: 现有AI模型在眼科图像分析中依赖大量标注且泛化能力不足，基础模型虽有潜力但缺乏验证。

Method: 提出MIRAGE多模态基础模型，并设计新的评估基准，涵盖OCT/SLO分类和分割任务。

Result: MIRAGE在分类和分割任务中优于通用和专业模型，适合作为视网膜OCT图像分析的AI基础。

Conclusion: MIRAGE为眼科图像分析提供了强大的多模态基础模型，其评估基准和模型已开源。
...
### [333] [Hyperbolic Dual Feature Augmentation for Open-Environment](https://arxiv.org/abs/2506.08906)
*Peilin Yu,Yuwei Wu,Zhi Gao,Xiaomeng Fan,Shuo Yang,Yunde Jia*

Main category: cs.CV

TLDR: 提出了一种双曲双特征增强方法，用于开放环境中的特征增强，包括已知和未知类别。
...
Method: 1. 使用神经ODE模块估计特征分布；2. 引入正则化器保持层次结构；3. 推导损失上界以支持无限增强。
...
### [334] [SkipVAR: Accelerating Visual Autoregressive Modeling via Adaptive Frequency-Aware Skipping](https://arxiv.org/abs/2506.08908)
*Jiajun Li,Yue Ma,Xinyu Zhang,Qingyan Wei,Songhua Liu,Linfeng Zhang*

Main category: cs.CV

TLDR: 论文提出SkipVAR框架，通过自动跳过冗余步骤和替换无条件分支来加速视觉自回归模型，动态选择策略提升效率，实验显示显著加速且保持质量。


<details>
  <summary>Details</summary>
Motivation: 高频分量或后期步骤在生成过程中导致推理延迟，但相关计算冗余尚未深入研究。

Method: 分析VAR推理过程，提出自动跳过冗余步骤和替换无条件分支的策略，并引入样本自适应框架SkipVAR。

Result: SkipVAR在GenEval基准上实现1.81倍加速和2.62倍速度提升，平均SSIM达0.88。

Conclusion: 频率感知的无训练自适应加速方法有效，适用于可扩展自回归图像生成。
...
### [335] [Inherently Faithful Attention Maps for Vision Transformers](https://arxiv.org/abs/2506.08915)
*Ananthu Aniraj,Cassio F. Dantas,Dino Ienco,Diego Marcos*

Main category: cs.CV

TLDR: 提出了一种基于注意力的两阶段方法，通过学习二进制注意力掩码来限制预测仅受相关图像区域影响，从而提高对虚假相关性和分布外背景的鲁棒性。
...
Method: 两阶段框架：第一阶段处理全图以发现对象部分和任务相关区域；第二阶段使用注意力掩码限制感受野，进行聚焦分析。

Result: 在多个基准测试中显著提升了对虚假相关性和分布外背景的鲁棒性。

Conclusion: 该方法通过联合训练两阶段，有效平衡了上下文需求和对象聚焦，提升了模型性能。
...
### [336] [Socratic-MCTS: Test-Time Visual Reasoning by Asking the Right Questions](https://arxiv.org/abs/2506.08927)
*David Acuna,Ximing Lu,Jaehun Jung,Hyunwoo Kim,Amlan Kar,Sanja Fidler,Yejin Choi*

Main category: cs.CV

TLDR: 本文探讨了如何通过蒙特卡洛树搜索（MCTS）算法，在不额外训练或监督的情况下，从非推理视觉语言模型中提取隐藏知识并生成长推理链。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索是否可以通过搜索机制从已部署的非推理模型中提取隐藏知识，而无需额外训练。

Method: 采用蒙特卡洛树搜索（MCTS）算法，通过注入子问题-子答案对来引导模型生成长推理链。

Result: 在三个基准测试中表现一致提升，MMMU-PRO上整体提升2%，其中文科领域显著提升9%。

Conclusion: 结论表明，将推理视为搜索过程可以有效帮助非推理模型连接碎片化知识并生成长推理链。
...
### [337] [What Limits Virtual Agent Application? OmniBench: A Scalable Multi-Dimensional Benchmark for Essential Virtual Agent Capabilities](https://arxiv.org/abs/2506.08933)
...
TLDR: OmniBench是一个自生成、跨平台的图基准，通过子任务组合控制任务复杂度，OmniEval是多维评估框架，评估虚拟代理的10种能力。数据集包含36k图结构任务，人工接受率达91%。


<details>
  <summary>Details</summary>
Motivation: 现有基准存在任务复杂度不可控、人工标注场景有限和多维评估缺乏的问题。

Method: 提出OmniBench（自生成、跨平台的图基准）和OmniEval（多维评估框架），通过子任务组合生成可控复杂度的任务。

Result: 数据集包含36k图结构任务，人工接受率91%，训练效率优于人工标注数据。开源和闭源模型在多维评估中表现各异。
...
### [338] [SSS: Semi-Supervised SAM-2 with Efficient Prompting for Medical Imaging Segmentation](https://arxiv.org/abs/2506.08949)
*Hongjie Zhu,Xiwei Liu,Rundong Xue,Zeyu Zhang,Yong Xu,Daji Ergu,Ying Cai,Yang Zhao*

Main category: cs.CV

TLDR: 论文提出了一种名为SSS的半监督学习方法，利用SAM-2的强大特征提取能力增强医学图像分割性能，通过多视图特征差异优化和提示生成器设计，显著提升了分割效果。


<details>
  <summary>Details</summary>
Motivation: 在医学图像分析中，如何高效利用大规模未标注数据并减少对高质量标注的依赖是一个关键挑战。半监督学习（SSL）通过知识转移提升未标注数据的利用率，成为重要研究方向。

Method: 基于单流'弱到强'一致性正则化框架，引入判别性特征增强（DFE）机制，探索多视图数据增强带来的特征差异。同时设计提示生成器，结合物理约束和滑动窗口机制（PCSW）生成未标注数据的输入提示。

Result: 在ACDC和BHSD数据集上的实验表明，SSS方法显著优于现有方法，BHSD上的平均Dice分数达到53.15，比之前最优方法提升+3.65。

Conclusion: SSS方法通过结合SAM-2的特征提取能力和创新的半监督学习策略，有效提升了医学图像分割性能，为半监督学习在医学图像分析中的应用提供了新思路。
...
### [339] [Cross-Spectral Body Recognition with Side Information Embedding: Benchmarks on LLCM and Analyzing Range-Induced Occlusions on IJB-MDF](https://arxiv.org/abs/2506.08953)
*Anirudh Nanduri,Siyuan Huang,Rama Chellappa*

Main category: cs.CV

TLDR: ViT模型通过Side Information Embedding（SIE）在跨光谱人体识别中表现优异，仅编码相机信息即可达到SOTA。同时，研究填补了可见-红外Re-ID中遮挡问题的空白。


<details>
  <summary>Details</summary>
Motivation: 探索ViT在跨光谱人体识别中的应用，并解决现有数据集中遮挡问题未被充分研究的情况。

Method: 使用预训练的ViT模型，结合SIE编码相机信息，并在IJB-MDF数据集上分析遮挡影响。

Result: 仅编码相机信息即可在LLCM数据集上达到SOTA性能。

Conclusion: ViT结合SIE在跨光谱识别中有效，且遮挡问题需进一步研究。
...
### [340] [Segment Concealed Objects with Incomplete Supervision](https://arxiv.org/abs/2506.08955)
...
TLDR: 论文提出了一种名为SEE的统一方法，用于解决不完全监督的隐蔽物体分割（ISCOS）问题，通过结合SAM模型生成伪标签和混合粒度特征分组模块，显著提升了分割性能。
...
TLDR: 提出了一种基于Fast AutoAugment的数据增强方法，显著提升了小目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 尽管目标检测性能近年有显著提升，但小目标检测性能仍远低于大目标，这是计算机视觉中的重要挑战。

Method: 使用Fast AutoAugment快速找到最优的数据增强策略，以克服小目标检测的退化问题。
...
### [342] [ORIDa: Object-centric Real-world Image Composition Dataset](https://arxiv.org/abs/2506.08964)
*Jinwoo Kim,Sangmin Han,Jinho Jeong,Jiwoo Choi,Dongyoung Kim,Seon Joo Kim*

Main category: cs.CV

TLDR: ORIDa是一个大规模的真实世界图像合成数据集，包含30,000多张图像和200个独特对象，旨在解决现有数据集在多样性和规模上的不足。
...
Method: ORIDa包含两种数据类型：事实-反事实集（每组5张图像）和仅事实场景（单张图像），覆盖多样化的环境和对象位置。

Result: ORIDa是首个公开的具有如此规模和复杂性的真实世界图像合成数据集。

Conclusion: ORIDa为对象合成研究的进一步推进提供了重要资源。
...
### [343] [ADAM: Autonomous Discovery and Annotation Model using LLMs for Context-Aware Annotations](https://arxiv.org/abs/2506.08968)
*Amirreza Rouhi,Solmaz Arezoomandan,Knut Peterson,Joseph T. Woods,David K. Han*

Main category: cs.CV

TLDR: ADAM是一种无需训练的自发现与标注模型，利用LLM和CLIP在开放世界中标注未知物体。
...
Method: 结合LLM生成候选标签和CLIP视觉嵌入构建ELR，通过检索、投票和重排序标注新物体，并引入自优化循环提升一致性。
...
### [344] [Rethinking Range-View LiDAR Segmentation in Adverse Weather](https://arxiv.org/abs/2506.08979)
*Longyu Yang,Ping Hu,Lu Zhang,Jun Liu,Yap-Peng Tan,Heng Tao Shen,Xiaofeng Zhu*

Main category: cs.CV

TLDR: 论文提出了一种轻量级模块化框架，提升LiDAR分割在恶劣天气下的泛化性能，通过几何异常抑制和反射率校准模块优化现有模型。


<details>
  <summary>Details</summary>
Motivation: 现有基于范围视图的LiDAR分割方法在恶劣天气下泛化性能不足，限制了实际应用可靠性。

Method: 将标准范围视图网络的初始块重构为两个分支，分别处理几何属性和反射率强度，引入GAS和RDC模块抑制噪声和校准失真。
...
### [345] [Efficient Medical Vision-Language Alignment Through Adapting Masked Vision Models](https://arxiv.org/abs/2506.08990)
*Chenyu Lian,Hong-Yu Zhou,Dongyun Liang,Jing Qin,Liansheng Wang*

Main category: cs.CV

TLDR: ALTA是一种高效的医学视觉-语言对齐方法，通过适应预训练的视觉模型，显著提升了检索和零样本分类任务的性能。
...
### [346] [SDTagNet: Leveraging Text-Annotated Navigation Maps for Online HD Map Construction](https://arxiv.org/abs/2506.08997)
*Fabian Immel,Jan-Hendrik Pauls,Richard Fehler,Frank Bieder,Jonas Merkert,Christoph Stiller*

Main category: cs.CV

TLDR: SDTagNet是一种利用标准定义（SD）地图（如OpenStreetMap）在线构建高清晰度（HD）地图的方法，通过结合多段线数据和文本注释提升远距离检测精度。


<details>
  <summary>Details</summary>
Motivation: 高维护成本的HD地图限制了自动驾驶车辆的规模化部署，而在线HD地图构建方法受限于车载传感器的短感知范围。

Method: SDTagNet结合多段线SD地图数据和文本注释，引入点级SD地图编码器和正交元素标识符，统一整合所有类型的地图元素。

Result: 在Argoverse 2和nuScenes数据集上，SDTagNet比无先验方法提升5.9 mAP（45%），比现有SD先验方法提升3.2 mAP（20%）。

Conclusion: SDTagNet通过充分利用SD地图信息，显著提升了在线HD地图构建的性能和远距离检测精度。
...
### [347] [Do MIL Models Transfer?](https://arxiv.org/abs/2506.09022)
*Daniel Shao,Richard J. Chen,Andrew H. Song,Joel Runevic,Ming Y. Lu,Tong Ding,Faisal Mahmood*

Main category: cs.CV

TLDR: 本文研究了多实例学习（MIL）模型在计算病理学中的迁移学习能力，发现预训练MIL模型在不同器官和任务上表现优于从头训练的模型，且泛化能力强。


<details>
  <summary>Details</summary>
Motivation: 计算病理学中MIL模型在弱监督小数据集上表现不佳，而迁移学习的潜力尚未充分探索。

Method: 系统评估了11个预训练MIL模型在21个预训练任务中的表现，涵盖形态学和分子亚型预测。

Result: 预训练MIL模型在不同器官和任务上表现优于从头训练模型，且泛化能力强。

Conclusion: MIL模型具有强大的适应性，迁移学习可显著提升计算病理学任务性能，并提供了标准化实现资源。
...
### [348] [DIsoN: Decentralized Isolation Networks for Out-of-Distribution Detection in Medical Imaging](https://arxiv.org/abs/2506.09024)
*Felix Wagner,Pramit Saha,Harry Anthony,J. Alison Noble,Konstantinos Kamnitsas*

Main category: cs.CV

TLDR: 论文提出了一种名为DIsoN的去中心化OOD检测框架，解决了训练数据无法共享的问题，通过交换模型参数实现训练和测试数据的比较。
...
Method: 提出Isolation Network和DIsoN框架，通过解决二分类任务量化测试样本与训练数据的分离难度，支持无数据共享的OOD检测。

Result: 在四个医学影像数据集上的12个OOD检测任务中，DIsoN表现优于现有方法，同时保护数据隐私。

Conclusion: DIsoN为ML开发者提供了一种新的服务模式，支持远程、安全地利用训练数据进行OOD检测。
...
### [349] [Diffuse and Disperse: Image Generation with Representation Regularization](https://arxiv.org/abs/2506.09027)
*Runqian Wang,Kaiming He*

Main category: cs.CV

TLDR: 提出了一种名为Dispersive Loss的简单正则化方法，用于改进扩散生成模型，无需额外数据或参数。


<details>
  <summary>Details</summary>
Motivation: 扩散生成模型通常缺乏显式正则化，且与表示学习进展独立发展。本文旨在通过新损失函数弥合这一差距。
...
### [350] [Princeton365: A Diverse Dataset with Accurate Camera Pose](https://arxiv.org/abs/2506.09035)
*Karhan Kayan,Stamatis Alexandropoulos,Rishabh Jain,Yiming Zuo,Erich Liang,Jia Deng*

Main category: cs.CV

TLDR: Princeton365是一个大规模多样化数据集，包含365个带有精确相机姿态的视频，填补了当前SLAM基准在精度和数据多样性之间的空白。
...
Method: 通过校准板和360度相机收集室内、室外和物体扫描视频，同步单目、立体RGB视频及IMU数据，并提出新的场景尺度感知评估指标。

Result: 数据集支持多场景SLAM评估，并提出新的光学流评估指标和挑战性新视角合成基准。
...
### [351] [Autoregressive Semantic Visual Reconstruction Helps VLMs Understand Better](https://arxiv.org/abs/2506.09040)
*Dianyi Wang,Wei Song,Yikun Wang,Siyuan Wang,Kaicheng Yu,Zhongyu Wei,Jiaqi Wang*

Main category: cs.CV

TLDR: 论文提出了一种名为ASVR的方法，通过自回归框架联合学习视觉和文本模态，解决了现有大型视觉语言模型（LVLMs）在视觉模态学习中的不足。


<details>
  <summary>Details</summary>
Motivation: 现有LVLMs仅对文本序列进行自回归监督，未充分利用视觉模态，导致无法处理无标注图像、遗漏视觉细节及难以表达视觉中心内容。

Method: 提出ASVR方法，通过自回归语义视觉重建，联合学习视觉和文本模态。

Result: 实验表明，ASVR在多种多模态理解基准上表现优异，显著提升了模型性能（如LLaVA-1.5平均提升5%）。

Conclusion: ASVR通过自回归语义重建有效提升了多模态理解能力，为视觉语言模型提供了新的学习框架。
...
### [352] [Cosmos-Drive-Dreams: Scalable Synthetic Driving Data Generation with World Foundation Models](https://arxiv.org/abs/2506.09042)
...
TLDR: Cosmos-Drive-Dreams是一个合成数据生成（SDG）管道，旨在生成具有挑战性的场景，以支持自动驾驶系统的感知和驾驶策略训练。


<details>
  <summary>Details</summary>
Motivation: 收集和标注现实世界数据用于自动驾驶等安全关键AI系统成本高且耗时，尤其是难以捕捉对训练和测试至关重要的罕见边缘案例。

Method: 基于NVIDIA Cosmos世界基础模型，开发了Cosmos-Drive模型套件，能够生成可控、高保真、多视角且时空一致的驾驶视频。

Result: 实验表明，生成的数据有助于缓解长尾分布问题，并提升3D车道检测、3D物体检测和驾驶策略学习等下游任务的泛化能力。
...
### [353] [MagCache: Fast Video Generation with Magnitude-Aware Cache](https://arxiv.org/abs/2506.09045)
*Zehong Ma,Longhui Wei,Feng Wang,Shiliang Zhang,Qi Tian*

Main category: cs.CV

TLDR: 论文提出了一种基于统一幅度规律的视频扩散模型加速技术MagCache，通过自适应跳过不重要时间步和缓存策略，显著提升速度并保持视觉质量。


<details>
  <summary>Details</summary>
Motivation: 现有加速技术依赖统一启发式方法或时间嵌入变体，需大量校准且易因提示过拟合导致输出不一致。

Method: 发现统一幅度规律，提出MagCache，利用误差建模机制和自适应缓存策略跳过不重要时间步。

Result: MagCache在Open-Sora和Wan 2.1上分别实现2.1倍和2.68倍加速，且在LPIPS、SSIM和PSNR上优于现有方法。

Conclusion: MagCache是一种高效且鲁棒的加速技术，仅需单样本校准，显著优于现有方法。
...
### [354] [UAVs Meet Agentic AI: A Multidomain Survey of Autonomous Aerial Intelligence and Agentic UAVs](https://arxiv.org/abs/2506.08045)
*Ranjan Sapkota,Konstantinos I. Roumeliotis,Manoj Karkee*

Main category: cs.RO

TLDR: Agentic UAVs integrate AI for adaptive, goal-driven behavior in complex environments, surpassing traditional UAVs. The paper covers their architecture, applications, challenges, and future directions.


<details>
  <summary>Details</summary>
Motivation: To advance autonomous UAVs by integrating Agentic AI for improved adaptability, reasoning, and collaboration in real-world scenarios.

Method: Comprehensive analysis of architectural components, enabling technologies, and comparative advancements in autonomy, AI, and mission flexibility.

Result: Identifies seven high-impact applications and key challenges, proposing solutions in hardware, learning, and human-AI interaction.

Conclusion: A foundational framework for future development and governance of Agentic UAVs, with a roadmap for self-evolving ecosystems and equitable deployment.
...
### [355] [Ego-centric Learning of Communicative World Models for Autonomous Driving](https://arxiv.org/abs/2506.08149)
*Hang Wang,Dechen Gao,Junshan Zhang*

Main category: cs.RO

TLDR: 提出了一种名为CALL的通信世界模型，用于解决多智能体强化学习中的部分可观测性和非平稳性问题，通过轻量级信息共享提升预测精度和性能。


<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习在复杂高维环境（如自动驾驶）中存在部分可观测性和非平稳性问题，信息共享虽常用但面临通信开销和可扩展性挑战。

Method: 利用生成式AI的世界模型及其潜在表示，开发CALL模型：1）每个智能体学习其世界模型，将状态和意图编码为低维潜在表示；2）通过轻量级通信共享信息，提升世界模型的泛化能力。

Result: 实验在CARLA平台上进行，展示了CALL在局部轨迹规划任务中的性能提升。

Conclusion: CALL通过轻量级信息共享有效解决了多智能体强化学习中的挑战，提升了预测精度和任务性能。
...
### [356] [Re4MPC: Reactive Nonlinear MPC for Multi-model Motion Planning via Deep Reinforcement Learning](https://arxiv.org/abs/2506.08344)
*Neşet Ünver Akmandor,Sarvesh Prajapati,Mark Zolotas,Taşkın Padır*

Main category: cs.RO

TLDR: 提出了一种名为Re4MPC的多模型运动规划方法，结合非线性模型预测控制（NMPC）和深度强化学习（DRL），以提高计算效率和任务成功率。
...
### [357] [Diffusion Models for Safety Validation of Autonomous Driving Systems](https://arxiv.org/abs/2506.08459)
*Juanran Wang,Marc R. Schlichting,Harrison Delecki,Mykel J. Kochenderfer*

Main category: cs.RO

TLDR: 使用去噪扩散模型生成自动驾驶系统的潜在故障案例，无需外部数据集，适用于交通路口的安全验证。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统的安全验证成本高、风险大，且潜在故障罕见且多样，需高效方法生成故障案例。

Method: 训练去噪扩散模型，基于初始交通状态生成潜在故障案例。

Result: 在四路交叉口实验中，模型能生成多样且真实的故障样本。
...
### [358] [PhyBlock: A Progressive Benchmark for Physical Understanding and Planning via 3D Block Assembly](https://arxiv.org/abs/2506.08708)
...
TLDR: PhyBlock是一个用于评估视觉语言模型（VLMs）在物理理解和规划能力的渐进式基准测试，通过3D积木组装任务和VQA任务揭示了VLMs在高级规划和空间推理上的局限性。


<details>
  <summary>Details</summary>
Motivation: 尽管VLMs在推理和规划方面表现出潜力，但其对物理现象的理解能力（尤其在3D环境中）仍有限，因此需要开发新的评估工具。

Method: PhyBlock包含2600个任务（400个组装任务和2200个VQA任务），通过四个认知层次的任务和三个关键维度（部分完成、失败诊断和规划鲁棒性）评估VLMs。

Result: 21个先进VLMs的测试结果显示，其在高级规划和空间推理能力上存在显著不足，任务复杂度增加时性能下降明显。

Conclusion: PhyBlock为提升VLMs在物理问题解决和空间推理能力提供了统一的测试平台。
...
### [359] [Bayesian Inverse Physics for Neuro-Symbolic Robot Learning](https://arxiv.org/abs/2506.08756)
*Octavio Arriaga,Rebecca Adam,Melvin Laux,Lisa Gutzeit,Marco Ragni,Jan Peters,Frank Kirchner*

Main category: cs.RO

TLDR: 论文提出了一种结合数据驱动学习和结构化推理的混合神经符号架构，以提高机器人在未知动态环境中的适应性和效率。


<details>
  <summary>Details</summary>
Motivation: 现实世界的机器人应用需要自适应、可解释且数据高效的学习范式，但现有深度学习方法在未知和动态环境中表现不足。

Method: 结合可微分物理进行高效世界建模、贝叶斯推理实现不确定性感知决策，以及元学习快速适应新任务。

Result: 通过将物理符号推理嵌入神经模型，机器人可以超越训练数据泛化、推理新情境并持续扩展知识。
...
### [360] [Towards Biosignals-Free Autonomous Prosthetic Hand Control via Imitation Learning](https://arxiv.org/abs/2506.08795)
*Kaijie Shi,Wanglong Lu,Hanli Zhao,Vinicius Prado da Fonseca,Ting Zou,Xianta Jiang*

Main category: cs.RO

TLDR: 开发了一种基于摄像头和模仿学习的全自主假肢控制系统，能够自动抓取和释放不同形状的物体，减少用户的心理负担。


<details>
  <summary>Details</summary>
Motivation: 传统肌电信号控制方法对用户要求高且繁琐，本研究旨在提供一种更简单、更自主的假肢控制方案。

Method: 通过摄像头采集环境数据，利用模仿学习训练模型，模仿人类动作实现自主控制。

Result: 模型仅需少量数据即可实现高成功率，并能泛化到不同用户和未见过的物体。

Conclusion: 该系统为肢体缺失者提供了易用的假肢控制接口，显著降低了使用时的心理负担。
...
### [361] [FreqPolicy: Efficient Flow-based Visuomotor Policy via Frequency Consistency](https://arxiv.org/abs/2506.08822)
*Yifei Su,Ning Liu,Dong Chen,Zhen Zhao,Kun Wu,Meng Li,Zhiyuan Xu,Zhengping Che,Jian Tang*

Main category: cs.RO

TLDR: FreqPolicy通过频率一致性约束提升基于生成模型的视觉运动策略的实时性，支持高效的一步动作生成。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型在机器人操作中因多步采样推理成本高而难以实时应用，且现有加速方法未考虑时间依赖性。

Method: 提出FreqPolicy，通过频率一致性约束和自适应一致性损失，捕捉时间结构并优化一步动作生成。

Result: 在3个仿真基准的53个任务中表现优异，集成到VLA模型后在40个任务中实现加速且性能无损，实际机器人场景中推理频率达93.5Hz。

Conclusion: FreqPolicy有效解决了时间依赖性问题，提升了生成模型在实时机器人操作中的适用性。
...
### [362] [Solving excited states for long-range interacting trapped ions with neural networks](https://arxiv.org/abs/2506.08594)
*Yixuan Ma,Chang Liu,Weikang Li,Shun-Yao Zhang,L. -M. Duan,Yukai Wu,Dong-Ling Deng*

Main category: quant-ph

TLDR: 提出了一种基于神经网络的算法（NQES），用于高效计算量子多体系统的低激发态，无需显式正交化，适用于高维系统。


<details>
  <summary>Details</summary>
Motivation: 强相互作用量子多体系统的激发态计算具有基础重要性，但由于希尔伯特空间维度随系统尺寸指数增长，计算极具挑战性。

Method: 开发了NQES算法，通过神经网络同时输出多个低激发态，适用于高维系统，并在Haldane-Shastry模型和二维Wigner晶体中验证。

Result: NQES算法能高效计算多个激发态及其可观测量，并成功揭示了300离子系统的能隙标度和关联特征。
...
### [363] [Systematic and Efficient Construction of Quadratic Unconstrained Binary Optimization Forms for High-order and Dense Interactions](https://arxiv.org/abs/2506.08448)
*Hyakka Nakada,Shu Tanaka*

Main category: quant-ph

TLDR: 论文提出了一种基于量子退火（QA）和二次化方法的新黑盒优化方案，适用于复杂机器学习问题。


<details>
  <summary>Details</summary>
Motivation: 传统二次化方法在处理强非线性和密集交互的机器学习问题时效果不佳，因此需要一种新方法。

Method: 使用修正线性单元基的和建模目标函数，并将其转化为等效二次多项式表示。

Result: 通过数值和解析验证了方法的可行性，并设计了结合QA的新优化方案。

Conclusion: 该方法为复杂机器学习问题的二次化提供了新思路，扩展了QA的应用范围。
...
### [364] [The interplay of robustness and generalization in quantum machine learning](https://arxiv.org/abs/2506.08455)
*Julian Berberich,Tobias Fellner,Christian Holm*

Main category: quant-ph

TLDR: 探讨量子机器学习中对抗鲁棒性与泛化性的关系，提出基于Lipschitz界的正则化训练方法。


<details>
  <summary>Details</summary>
Motivation: 研究量子机器学习中对抗鲁棒性与泛化性的相互作用，填补现有文献空白。

Method: 利用Lipschitz界量化鲁棒性和泛化性，提出正则化训练方法，强调可训练数据编码策略。

Result: 理论结果在时间序列分析中得到验证，展示了实用性。
...
### [365] [Quantum Adiabatic Generation of Human-Like Passwords](https://arxiv.org/abs/2506.08917)
*Sascha Mücke,Raoul Heese,Thore Gerlach,David Biesner,Loong Kuan Lee,Nico Piatkowski*

Main category: quant-ph

TLDR: 研究探讨了量子计算在生成短语义结构（如密码）中的潜力，提出基于QUBO和UD-MIS问题的新方法，并在256量子位的中性原子量子计算机上生成了类似人类行为的密码。


<details>
  <summary>Details</summary>
Motivation: 探索量子计算是否能减少生成式AI模型的资源需求，尤其是在生成短语义结构（如密码）方面的应用。

Method: 采用QUBO和UD-MIS问题编码令牌字符串，通过量子态制备和测量生成密码。

Result: 在256量子位的量子计算机上生成了128个类似人类行为的密码样本。
...
### [366] [Superposed Parameterised Quantum Circuits](https://arxiv.org/abs/2506.08749)
*Viktoria Patapovich,Mo Kordzanganeh,Alexey Melnikov*

Main category: quant-ph

TLDR: 论文提出了一种叠加参数化量子电路（superposed parameterised quantum circuits），通过结合翻转量子随机存取存储器和重复直至成功协议，提升了量子机器学习的表达能力和可扩展性。
...
Method: 采用叠加参数化量子电路，结合翻转量子随机存取存储器和重复直至成功协议，嵌入指数级数量的参数化子模型，并通过振幅变换和后选择实现多项式激活函数。

Result: 数值实验表明，该方法在1D阶跃函数回归中，均方误差比基线降低了三个数量级；在2D星形分类任务中，准确率提升至81.4%，且运行方差降低三倍。

Conclusion: 叠加参数化量子电路为构建更深层、更通用的参数化量子电路提供了一条硬件高效的路径，能够学习复杂决策边界。
...
### [367] [syren-baryon: Analytic emulators for the impact of baryons on the matter power spectrum](https://arxiv.org/abs/2506.08783)
*Lukas Kammerer,Deaglan J. Bartlett,Gabriel Kronberger,Harry Desmond,Pedro G. Ferreira*

Main category: astro-ph.CO

TLDR: 论文提出了一种符号回归方法，用于参数化重子物理对物质功率谱的影响，并提供了不同重子物理模型的解析近似及其不确定性。


<details>
  <summary>Details</summary>
Motivation: 重子物理对宇宙物质分布有显著影响，是当前和未来宇宙学调查的关键系统误差来源。研究旨在找到简单且物理合理的参数化方法。

Method: 使用符号回归从CAMELS流体动力学模拟中构建解析近似，描述重子物理对物质功率谱的影响，并分析其不确定性。

Result: 解析近似的误差与样本方差相当，且在小尺度上误差增加。不同模型的解析形式可用于区分重子物理模型。

Conclusion: 提出的解析形式具有物理正确性，可用于直接解释宇宙学和反馈参数的影响，并区分不同重子物理模型。
...
### [368] [TS-PIELM: Time-Stepping Physics-Informed Extreme Learning Machine Facilitates Soil Consolidation Analyses](https://arxiv.org/abs/2506.08381)
*He Yang,Fei Ren,Hai-Sui Yu,Xueyu Geng,Pei-Zhi Zhuang*

Main category: physics.geo-ph

TLDR: 论文提出了一种名为TS-PIELM的高效物理信息机器学习方法，用于改进传统PINN在土壤固结分析中的精度和效率。
...
Method: 通过时间步进物理信息极限学习机（TS-PIELM）框架，将固结过程分为多个时间间隔，并使用单层前馈极限学习机（ELM）加速训练。

Result: TS-PIELM在一维案例中的计算效率和精度分别提高了1000倍和100倍。

Conclusion: PIML在计算岩土工程中具有强大潜力。
...
### [369] [Generalizing while preserving monotonicity in comparison-based preference learning models](https://arxiv.org/abs/2506.08616)
*Julien Fageot,Peva Blanchard,Gilles Bareilles,Lê-Nguyên Hoang*

Main category: math.ST

TLDR: 论文提出了一种新的线性广义Bradley-Terry模型，结合扩散先验，解决了现有偏好学习模型缺乏单调性保证的问题，并在有限数据集上提高了准确性。


<details>
  <summary>Details</summary>
Motivation: 现有基于比较的偏好学习模型（如大型语言模型）缺乏单调性保证，且唯一已知的单调模型（广义Bradley-Terry模型）无法泛化到未比较数据。

Method: 提出线性广义Bradley-Terry模型与扩散先验结合的新方法，并确定嵌入条件的充分性以保证单调性。

Result: 实验表明，新模型在有限数据集上显著提高了准确性，且单调性并非普遍保证。

Conclusion: 新模型解决了单调性与泛化能力的矛盾，为偏好学习提供了更可靠的解决方案。
...
### [370] [SakugaFlow: A Stagewise Illustration Framework Emulating the Human Drawing Process and Providing Interactive Tutoring for Novice Drawing Skills](https://arxiv.org/abs/2506.08443)
*Kazuki Kawamura,Jun Rekimoto*

Main category: cs.HC

TLDR: SakugaFlow是一个四阶段流程，结合扩散模型和大型语言模型，为初学者提供实时反馈，支持非线性修改和多版本探索，将黑盒生成器转化为学习工具。
...
Method: 采用四阶段流程，结合扩散模型生成图像，并通过语言模型提供实时反馈，支持非线性修改和多版本分支。

Result: SakugaFlow成功将黑盒生成器转化为支持创意探索和技能学习的环境。

Conclusion: SakugaFlow通过暴露中间输出和嵌入教学对话，为初学者提供了有效的学习工具。
...
### [371] [MOSAIC-F: A Framework for Enhancing Students' Oral Presentation Skills through Personalized Feedback](https://arxiv.org/abs/2506.08634)
*Alvaro Becerra,Daniel Andres,Pablo Villegas,Roberto Daza,Ruth Cobos*

Main category: cs.HC

TLDR: MOSAIC-F是一个多模态反馈框架，结合人类评估和多模态数据分析，为学生提供个性化反馈。


<details>
  <summary>Details</summary>
Motivation: 旨在通过整合多模态数据和人类评估，提供更准确、个性化的学习反馈。

Method: 框架包括四步：标准化评估、多模态数据收集、AI生成反馈、学生自我评估与可视化。

Result: 在提升口头表达能力的情境中测试了该框架。

Conclusion: 结合人类与数据评估，MOSAIC-F能提供更精准、可操作的反馈。
...
### [372] [Stop Misusing t-SNE and UMAP for Visual Analytics](https://arxiv.org/abs/2506.08725)
*Hyeon Jeon,Jeongin Park,Sungbok Shin,Jinwook Seo*

Main category: cs.HC

TLDR: 论文指出t-SNE和UMAP在可视化分析中的误用现象普遍，并探讨其原因及预防措施。


<details>
  <summary>Details</summary>
Motivation: 揭示t-SNE和UMAP误用的普遍性及其背后的原因，以促进更合理的降维技术使用。

Method: 通过文献综述（114篇论文）和访谈研究，分析误用现象及实践者的动机。

Result: 误用主要源于可视化分析中对这些技术适用性的讨论不足。

Conclusion: 提出未来方向和具体措施，以推动降维技术的合理使用。
...
### [373] [Implementing Keyword Spotting on the MCUX947 Microcontroller with Integrated NPU](https://arxiv.org/abs/2506.08911)
*Petar Jakuš,Hrvoje Džapo*

Main category: cs.HC

TLDR: 论文提出了一种在NXP MCXN947微控制器上实现的关键词检测系统，利用集成的NPU实现实时语音交互，通过MFCC特征提取和CNN分类器结合量化感知训练优化模型。
...
Result: 实验结果显示，使用NPU相比仅用CPU推理速度提升59倍，模型大小为30.58 KB，准确率达97.06%。
...
### [374] [SPBA: Utilizing Speech Large Language Model for Backdoor Attacks on Speech Classification Models](https://arxiv.org/abs/2506.08346)
*Wenhan Yao,Fen Xiao,Xiarun Chen,Jia Liu,YongQiang He,Weiping Wen*

Main category: cs.SD

TLDR: 论文提出一种基于语音大语言模型（SLLM）的语音后门攻击方法（SPBA），通过多样化的触发机制提高攻击效果，并引入多梯度下降算法（MGDA）作为缓解策略。


<details>
  <summary>Details</summary>
Motivation: 语音分类任务（如关键词识别和说话人验证）的安全性易受后门攻击，现有方法因触发机制限制仅能生成有限数量的后门。

Method: 利用语音元素（如音色和情感）和SLLM生成多样化触发机制，提出SPBA攻击方法，并采用MGDA作为缓解策略。

Result: 实验表明，SPBA在两种语音分类任务中表现出显著的触发效果，攻击指标优异。

Conclusion: SPBA通过多样化触发机制显著提升攻击效果，为语音分类任务的安全性提供了新的研究方向。
...
### [375] [MD-ViSCo: A Unified Model for Multi-Directional Vital Sign Waveform Conversion](https://arxiv.org/abs/2506.08357)
*Franck Meyer,Kyunghoon Hur,Edward Choi*

Main category: cs.SD

TLDR: 提出MD-ViSCo框架，统一处理多种生命体征波形转换，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有模型仅针对特定波形转换，需多模型，临床实用性差。

Method: 结合1D U-Net和Swin Transformer，利用AdaIN捕捉波形风格。

Result: MAE降低8.8%，PC提升4.9%，ABP波形符合AAMI和BHS标准。

Conclusion: MD-ViSCo提供统一框架，简化临床监测中的波形转换。
...
### [376] [Step-Audio-AQAA: a Fully End-to-End Expressive Large Audio Language Model](https://arxiv.org/abs/2506.08967)
...
TLDR: Step-Audio-AQAA是一种端到端的LALM，通过双码本音频标记器和1300亿参数LLM实现音频查询-音频回答任务，结合DPO和模型合并提升性能。
...
Method: 采用双码本音频标记器提取特征，结合1300亿参数LLM和神经声码器，通过DPO和模型合并优化性能。

Result: 在StepEval-Audio-360基准测试中表现优异，尤其在语音控制方面超越现有LALM。

Conclusion: Step-Audio-AQAA为端到端LALM提供了有效解决方案，并展示了基于标记的声码器在AQAA任务中的重要性。
...
TLDR: ACORN框架通过声音教会大型语言模型（LLM）物理感知，利用物理模拟器生成训练数据，构建AQA-PHY数据集，并连接音频编码器与LLM，在模拟和现实任务中取得合理结果。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在文本和多模态处理中表现出色，但缺乏对现实世界物理现象的理解。ACORN旨在填补这一空白，通过声音教会LLM物理感知。

Method: ACORN引入基于物理的模拟器生成多样化训练数据，构建AQA-PHY数据集，并提出处理幅度和相位信息的音频编码器，将其与先进LLM连接。

Result: 在模拟和现实任务（如视线检测、多普勒效应估计和到达方向估计）中取得合理结果。

Conclusion: ACORN为LLM理解物理世界铺平了道路，展示了通过声音实现物理感知的潜力。
...
### [378] [Auto-Regressive vs Flow-Matching: a Comparative Study of Modeling Paradigms for Text-to-Music Generation](https://arxiv.org/abs/2506.08570)
*Or Tal,Felix Kreuk,Yossi Adi*

Main category: cs.SD

TLDR: 论文对文本到音乐生成的两种常见建模范式（自回归解码和条件流匹配）进行了系统比较，揭示了各自的优缺点。
...
Result: 研究比较了生成质量、推理配置鲁棒性、可扩展性、条件遵循能力和编辑能力，揭示了各自的优势与局限。
...
### [379] [PerfTracker: Online Performance Troubleshooting for Large-scale Model Training in Production](https://arxiv.org/abs/2506.08528)
*Yu Guan,Zhiyu Yin,Haoyu Chen,Sheng Cheng,Chaojie Yang,Tianyin Xu,Yang Zhang,Hanyu Zhao,Yong Li,Dennis Cai,Ennan Zhai*

Main category: cs.DC

TLDR: PerfTracker是一个在线故障排除系统，用于诊断大规模模型训练中的性能问题，通过细粒度分析和差分可观测性定位问题。
...
Method: PerfTracker利用细粒度分析和在线分析，结合差分可观测性，定位硬件和软件问题。
...
### [380] [A Systematic Literature Review on Continuous Integration and Deployment (CI/CD) for Secure Cloud Computing](https://arxiv.org/abs/2506.08055)
*Sabbir M. Saleh,Nazim Madhavji,John Steinbacher*

Main category: cs.SE

TLDR: 本文通过系统文献综述（SLR）分析了云环境中CI/CD的安全问题，总结了相关工具、方法和挑战，并指出了研究空白。


<details>
  <summary>Details</summary>
Motivation: 随着云环境的普及，网络安全成为关键问题，特别是在CI/CD流程中，亟需系统研究以填补安全漏洞。

Method: 对66篇论文进行系统文献综述，总结云环境中CI/CD安全的工具、方法和挑战。

Result: 识别了Harbor、SonarQube等工具，并揭示了镜像篡改、未授权访问等安全挑战。

Conclusion: 研究发现现有工具和实践在解决CI/CD安全问题上存在不足，需进一步研究以提升云安全解决方案。
...
### [381] [A Metrics-Oriented Architectural Model to Characterize Complexity on Machine Learning-Enabled Systems](https://arxiv.org/abs/2506.08153)
*Renato Cordeiro Ferreira*

Main category: cs.SE

TLDR: 研究提出了一种基于指标的架构模型，用于管理ML-enabled系统的复杂性，并展示了其初步实现。


<details>
  <summary>Details</summary>
Motivation: 探讨复杂性如何影响ML-enabled系统（MLES），并支持架构决策。

Method: 扩展参考架构以描述MLES并收集指标。

Result: 初步实现了基于指标的架构模型。

Conclusion: 为MLES的复杂性管理提供了初步框架。
...
### [382] [Worst-Case Symbolic Constraints Analysis and Generalisation with Large Language Models](https://arxiv.org/abs/2506.08171)
*Daniel Koh,Yannic Noller,Corina S. Pasareanu,Adrians Skapars,Youcheng Sun*

Main category: cs.SE

TLDR: 论文研究了大型语言模型（LLMs）在符号约束分析中的能力，特别是针对程序的最坏情况执行。通过SMT约束求解和专门设计的数据集，改进LLMs的符号推理能力，实验表明3B规模的模型WARP-1.0-3B表现优于基线。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在复杂符号推理任务中的潜力，尤其是程序的最坏情况执行分析，以连接神经网络学习和形式化方法。

Method: 定义最坏情况符号约束分析任务，通过SMT约束求解和专门数据集进行微调，结合强化学习方法。

Result: WARP-1.0-3B模型在符号约束分析任务中表现优于基线，验证了LLMs在符号推理中的潜力。

Conclusion: LLMs能够进行更深层次的符号推理，为神经网络学习与形式化方法的结合提供了支持。
...
### [383] [Repeton: Structured Bug Repair with ReAct-Guided Patch-and-Test Cycles](https://arxiv.org/abs/2506.08173)
*Nguyen Phu Vinh,Anh Chung Hoang,Chris Ngo,Truong-Son Hy*

Main category: cs.SE

TLDR: Repeton是一个开源框架，利用LLMs通过结构化补丁测试流程实现精确的代码操作，提升复杂软件工程任务的性能和可解释性。


<details>
  <summary>Details</summary>
Motivation: LLMs在代码生成和理解方面表现强大，但在复杂软件工程任务中精度低且可解释性差。

Method: Repeton采用结构化补丁测试流程，迭代诊断问题、提出代码变更并通过自动化测试验证，避免依赖嵌入检索系统。

Result: 在SWE-bench Lite基准测试中，Repeton在补丁有效性和可解释性上优于RAG方法。

Conclusion: 通过模块化、可验证的阶段分解任务，Repeton为可扩展且透明的自主调试提供了实用路径。
...
TLDR: 本文系统研究了软件工程代理（SWE代理）的决策行为，提出首个分类法，并分析了核心组件及其对代理成功的影响。


<details>
  <summary>Details</summary>
Motivation: 尽管SWE代理在软件任务中表现优异，但其内部决策机制尚不清晰，研究旨在提升代理的可靠性和效率。

Method: 通过执行轨迹分析，提出分类法并深入研究核心组件（如错误定位、补丁生成和测试生成）。

Result: 研究发现测试生成对补丁成功至关重要，并揭示了代理生成补丁与开发者补丁的结构和风格差异。
...
### [385] [Do Generative AI Tools Ensure Green Code? An Investigative Study](https://arxiv.org/abs/2506.08790)
*Samarth Sikand,Rohit Mehra,Vibhu Saujanya Sharma,Vikrant Kaulgud,Sanjay Podder,Adam P. Burden*

Main category: cs.SE

TLDR: 论文研究了AI生成代码的可持续性，发现主流工具（ChatGPT、BARD、Copilot）生成的代码普遍不符合绿色编码实践，需进一步研究改进。


<details>
  <summary>Details</summary>
Motivation: 软件可持续性日益重要，但缺乏对AI生成代码环保性的研究，需评估其是否符合绿色编码实践。

Method: 对三种主流生成式AI工具（ChatGPT、BARD、Copilot）生成的代码进行可持续性分析。

Result: 结果显示这些工具生成的代码普遍不符合绿色编码规则，存在环保问题。

Conclusion: 需深入研究并制定有效策略，提升AI生成代码的可持续性。
...
### [386] [On The Impact of Merge Request Deviations on Code Review Practices](https://arxiv.org/abs/2506.08860)
*Samah Kansab,Francis Bordeleau,Ali Tizghadam*

Main category: cs.SE

TLDR: 论文研究了代码审查中的非标准合并请求（MR）现象，提出了一种检测方法，并证明排除这些异常情况能提升机器学习模型的性能。


<details>
  <summary>Details</summary>
Motivation: 工业中的合并请求流程常偏离标准审查过程，导致分析和机器学习模型产生偏差。

Method: 识别了七种异常类别，并提出了一种少样本学习检测方法。

Result: 异常检测准确率达91%，排除异常后ML模型性能提升53.33%，特征重要性显著变化。
...
### [387] [Dynamic Diffusion Schrödinger Bridge in Astrophysical Observational Inversions](https://arxiv.org/abs/2506.08065)
*Ye Zhu,Duo Xu,Zhiwei Deng,Jonathon C. Tan,Olga Russakovsky*

Main category: astro-ph.IM

TLDR: 研究Diffusion Schr\"odinger Bridge (DSB)模型在星际分子云（GMCs）恒星形成观测逆预测任务中的应用，提出Astro-DSB模型，提升了解释性、学习效率和预测性能。


<details>
  <summary>Details</summary>
Motivation: 解决传统天体统计和机器学习方法在GMCs观测逆预测任务中的局限性，探索扩散模型在天体物理动态系统中的应用。

Method: 提出Astro-DSB模型，基于配对域假设，结合物理模拟和实际观测数据（Taurus B213）进行学习和预测。

Result: Astro-DSB在解释性、学习效率和预测性能上优于传统方法，且在OOD测试中表现更优。

Conclusion: 扩散模型在天体物理动态系统中具有潜力，为未来物理感知生成模型的发展提供了方向。
...
### [388] [midr: Learning from Black-Box Models by Maximum Interpretation Decomposition](https://arxiv.org/abs/2506.08338)
*Ryoichi Asashiba,Reiji Kozuma,Hirokazu Iwasawa*

Main category: stat.ME

TLDR: 论文介绍了R包midr，用于实现最大解释分解（MID），以解释黑盒模型。
...
### [389] [The World of AI: A Novel Approach to AI Literacy for First-year Engineering Students](https://arxiv.org/abs/2506.08041)
*Siddharth Siddharth,Brainerd Prince,Amol Harsh,Shreyas Ramachandran*

Main category: cs.CY

TLDR: 介绍了一门名为《AI世界》的新课程，面向大一工科生，旨在填补他们对AI基础知识和其社会影响的认知空白。课程分为三个模块，结合工程与人文视角，结果显示学生认知显著提升。


<details>
  <summary>Details</summary>
Motivation: 工科生通常缺乏对AI及其社会影响的初步了解，课程旨在通过跨学科设计填补这一空白。

Method: 课程分为三个模块（行星、社会影响、职场），由工程和人文教师共同授课，内容涵盖AI的环境影响、偏见、隐私及职场变革。

Result: 学生认知显著提升，包括对AI环境影响和公平性问题的理解加深，以及对AI变革作用的重新认识。

Conclusion: 该课程的跨学科设计和教学方法有效提升了学生对AI及其社会影响的理解。
...
### [390] [Evaluation of Machine Learning Models in Student Academic Performance Prediction](https://arxiv.org/abs/2506.08047)
*A. G. R. Sandeepa,Sanka Mohottala*

Main category: cs.CY

TLDR: 该研究探讨了使用机器学习方法预测学生学业表现，多层感知机分类器（MLPC）表现最佳，测试集准确率达86.46%。


<details>
  <summary>Details</summary>
Motivation: 研究旨在利用机器学习方法预测学生学业表现，以帮助教育者更好地理解和支持学生。

Method: 使用多层感知机分类器（MLPC）等标准机器学习模型，结合行为、学术和人口统计数据，并通过特征选择提升性能。

Result: MLPC在测试集上最高准确率达86.46%，10折交叉验证中平均测试集准确率为79.58%。

Conclusion: 研究表明神经网络是高效的数据模型，特征选择和可解释性方法对模型性能提升至关重要。
...
### [391] [Surgeons Awareness, Expectations, and Involvement with Artificial Intelligence: a Survey Pre and Post the GPT Era](https://arxiv.org/abs/2506.08258)
...
TLDR: 研究通过2021年和2024年的全球调查，分析了外科医生对AI在手术中的认知、期望和参与情况，发现AI课程意识和参与度提升，但基础概念理解仍有限，伦理问题凸显，基础设施是主要障碍。


<details>
  <summary>Details</summary>
Motivation: 探讨AI在手术中的应用潜力及外科医生对其的认知变化，尤其是生成式AI（如ChatGPT）的影响。

Method: 2021年和2024年分别进行两次全球横断面调查，评估外科医生的AI意识、期望、参与度和伦理问题。

Result: AI课程意识从14.5%升至44.6%，参与度从12.9%升至23%；伦理问题受关注（87.2%）；基础设施是主要障碍；79.9%认为AI对手术有积极影响。

Conclusion: 外科医生对AI的认知在演变，但知识缺口和基础设施问题仍需通过教育、伦理框架和基础设施发展解决。
...
### [392] [WIP: Large Language Model-Enhanced Smart Tutor for Undergraduate Circuit Analysis](https://arxiv.org/abs/2506.08962)
*Liangliang Chen,Huiru Xie,Jacqueline Rohde,Ying Zhang*

Main category: cs.CY

TLDR: 本文介绍了一款AI驱动的智能导师，用于为本科生电路分析课程提供作业评估和反馈，目前已在实践中取得90.9%的学生满意度。


<details>
  <summary>Details</summary>
Motivation: 旨在通过AI技术优化学生的作业反馈和学习体验，同时为教师提供实时数据支持。

Method: 设计了智能导师的核心组件，包括开放式问题回答和作业反馈生成，并在Microsoft Azure平台上部署。

Result: 学生满意度达90.9%，数据帮助教师识别学生常见问题并优化教学。

Conclusion: 智能导师在电路分析课程中表现良好，未来将扩展至更多工程学科并优化技术。
...
### [393] [Instruction-Tuned Video-Audio Models Elucidate Functional Specialization in the Brain](https://arxiv.org/abs/2506.08277)
...
TLDR: 研究发现，指令调优的多模态大语言模型（MLLMs）在视频和音频任务中显著优于非指令调优模型，且其层次结构与大脑活动对齐。


<details>
  <summary>Details</summary>
Motivation: 填补先前研究在评估MLLMs大脑对齐性时未关注指令调优多模态模型的空白。

Method: 利用指令调优的MLLMs生成任务特定表示，测量其对自然电影观看时神经活动的预测能力。

Result: 指令调优视频MLLMs比非指令调优模型表现提升15%-20%，且其层次结构与大脑区域对齐。

Conclusion: 任务特定指令显著提升MLLMs与大脑活动的对齐性，为研究多模态信息处理提供新方向。
...
### [394] [Physics-Informed Teleconnection-Aware Transformer for Global Subseasonal-to-Seasonal Forecasting](https://arxiv.org/abs/2506.08049)
*Tengfei Lyu,Weijia Zhang,Hao Liu*

Main category: stat.ML

TLDR: TelePiT是一种新型深度学习架构，通过多尺度物理和遥相关感知提升全球S2S预测性能。
...
Method: 结合球形谐波嵌入、多尺度物理信息神经ODE和遥相关感知Transformer，显式建模大气物理过程和全球气候交互。

Result: TelePiT显著优于现有数据驱动模型和数值天气预报系统，如2米温度RMSE降低57.7%。

Conclusion: TelePiT通过物理和遥相关建模，为S2S预测提供了更优解决方案。
...
### [395] [WWAggr: A Window Wasserstein-based Aggregation for Ensemble Change Point Detection](https://arxiv.org/abs/2506.08066)
*Alexander Stepikin,Evgenia Romanenkova,Alexey Zaytsev*

Main category: stat.ML

TLDR: 本文提出了一种基于Wasserstein距离的新型集成聚合方法WWAggr，用于提升高维数据流中的变化点检测性能，并解决了决策阈值选择的长期问题。


<details>
  <summary>Details</summary>
Motivation: 高维数据流中的变化点检测（CPD）因数据模式复杂性和常见假设的违反而具有挑战性，现有深度神经网络检测器性能仍有提升空间。

Method: 引入WWAggr，一种基于Wasserstein距离的任务特定集成聚合方法，适用于多种深度CPD模型集成。

Result: WWAggr有效提升了变化点检测的性能，并解决了决策阈值选择的难题。

Conclusion: WWAggr为高维数据流中的变化点检测提供了一种更鲁棒的解决方案，具有广泛适用性。
...
### [396] [Constrained Pareto Set Identification with Bandit Feedback](https://arxiv.org/abs/2506.08127)
*Cyrille Kone,Emilie Kaufmann,Laura Richert*

Main category: stat.ML

TLDR: 本文提出了一种在多臂老虎机设置下识别Pareto集的方法，考虑了可行性约束，并证明了其样本复杂度的近优性。


<details>
  <summary>Details</summary>
Motivation: 解决在多目标优化中识别Pareto集的问题，同时满足线性约束条件。

Method: 提出了一种固定置信度识别算法，优于传统的racing-like算法和两阶段方法。

Result: 算法在样本复杂度上表现优异，并通过理论证明和实验验证了其有效性。
...
### [397] [Model-Free Kernel Conformal Depth Measures Algorithm for Uncertainty Quantification in Regression Models in Separable Hilbert Spaces](https://arxiv.org/abs/2506.08325)
*Marcos Matabuena,Rahul Ghosal,Pavlo Mozharovskyi,Oscar Hernan Madrid Padilla,Jukka-Pekka Onnela*

Main category: stat.ML

TLDR: 提出了一种基于条件深度度量的新型模型无关不确定性量化算法，用于在可分希尔伯特空间中定义预测区域。
...
Result: 算法在模拟研究和数字健康应用中表现良好，具有快速收敛率和一致性保证。

Conclusion: 该方法为复杂数据提供了有效的预测区域定义工具，具有理论和实际意义。
...
### [398] [Asymptotic Normality of Infinite Centered Random Forests -Application to Imbalanced Classification](https://arxiv.org/abs/2506.08548)
*Moria Mayala,Erwan Scornet,Charles Tillier,Olivier Wintenberger*

Main category: stat.ML

TLDR: 本文研究了在类别不平衡数据上训练中心随机森林（CRF）的理论，提出了一种基于重要性采样（IS）的去偏方法（IS-ICRF），并证明了其在方差减少方面的优势。


<details>
  <summary>Details</summary>
Motivation: 类别不平衡数据在分类任务中常见，但现有方法缺乏理论支持。本文旨在填补这一空白，分析重平衡数据集对CRF的影响。

Method: 通过理论分析，建立了无限CRF的中心极限定理（CLT），并提出了基于重要性采样的去偏方法（IS-ICRF）。

Result: IS-ICRF在预测函数值上满足CLT，且在高度不平衡情况下方差减少。实验验证了理论结果对Breiman随机森林的有效性。

Conclusion: 重平衡数据集结合去偏方法优于直接使用原始数据，理论分析支持了这一结论。
...
### [399] [Flexible and Efficient Drift Detection without Labels](https://arxiv.org/abs/2506.08734)
*Nelvin Tan,Yu-Ching Shih,Dong Yang,Amol Salunkhe*

Main category: stat.ML

TLDR: 提出了一种无标签环境下的灵活高效概念漂移检测算法，基于经典统计过程控制，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型在自动化决策中广泛应用，确保其性能至关重要，但传统方法依赖即时标签，难以应对大规模数据集的无标签场景。

Method: 使用经典统计过程控制方法，设计无标签的概念漂移检测算法，并引入新框架结合历史检测结果。

Result: 在计算约束下，算法统计功效优于现有方法，数值模拟显示良好性能。

Conclusion: 该算法在无标签场景下高效检测概念漂移，为大规模数据集的模型监控提供实用解决方案。
...
### [400] [ChemGraph: An Agentic Framework for Computational Chemistry Workflows](https://arxiv.org/abs/2506.06363)
*Thang D. Pham,Aditya Tanikanti,Murat Keçeli*

Main category: physics.chem-ph

TLDR: ChemGraph是一个基于AI的框架，用于简化和自动化计算化学与材料科学的工作流程，结合图神经网络和大型语言模型，支持多种任务和方法。


<details>
  <summary>Details</summary>
Motivation: 原子模拟在化学和材料科学中至关重要，但设置和运行这些模拟需要专业知识且耗时。ChemGraph旨在解决这些问题。

Method: ChemGraph利用图神经网络基础模型进行高效计算，并结合大型语言模型（LLM）实现自然语言理解和任务规划。支持从简单到复杂的多种计算方法。

Result: 在13个基准任务中，较小的LLM在简单任务上表现良好，而复杂任务需要更大的模型。多代理框架使小模型在特定场景下能匹配或超越大模型性能。

Conclusion: ChemGraph通过AI和自动化显著提升了原子模拟的效率，证明了多代理框架在处理复杂任务时的潜力。
...
### [401] [A Privacy-Preserving Federated Learning Framework for Generalizable CBCT to Synthetic CT Translation in Head and Neck](https://arxiv.org/abs/2506.08654)
*Ciro Benito Raggio,Paolo Zaffino,Maria Francesca Spadea*

Main category: physics.med-ph

TLDR: 论文提出了一种基于联邦学习的CBCT-to-sCT生成方法，解决了多中心数据共享的隐私问题，并在头颈部区域验证了其泛化能力。
...
Method: 采用跨机构的水平联邦学习方法（FedSynthCT框架），结合条件生成对抗网络，在多中心数据上协作训练。

Result: 联邦模型在多中心数据上表现出色（MAE: 64.38-85.90 HU, SSIM: 0.882-0.922, PSNR: 32.86-34.91 dB），外部验证集也验证了其泛化能力。

Conclusion: 联邦学习为CBCT-to-sCT生成提供了可行的隐私保护解决方案，无需集中数据共享或站点微调。
...
### [402] [Approaching Dialogue State Tracking via Aligning Speech Encoders and LLMs](https://arxiv.org/abs/2506.08633)
*Šimon Sedláček,Bolaji Yusuf,Ján Švec,Pradyoth Hegde,Santosh Kesiraju,Oldřich Plchot,Jan Černocký*

Main category: eess.AS

TLDR: 该论文通过连接语音编码器和LLM的表示空间，提出了一种用于口语对话状态跟踪（DST）的方法，并在开源数据和组件上进行了实验。


<details>
  <summary>Details</summary>
Motivation: 研究旨在通过结合语音编码器和LLM的能力，提升口语对话状态跟踪的性能，同时关注开源数据和组件的使用。

Method: 使用小型连接模块桥接语音编码器（WavLM-large）和LLM（OLMo），并研究了不同系统配置（如LoRA适配器微调、对话历史中的代理轮次影响）以及模糊匹配后处理的效果。

Result: 在SpokenWOZ数据集上，最佳模型（WavLM + connector + OLMo-1B）达到了34.66%的JGA，而使用Gemma-2-9B-instruct的模型进一步提升至42.17% JGA。

Conclusion: 该方法在口语对话状态跟踪任务中取得了最先进的性能，证明了语音编码器与LLM结合的有效性。
...
### [403] [Normalized Radon Cumulative Distribution Transforms for Invariance and Robustness in Optimal Transport Based Image Classification](https://arxiv.org/abs/2506.08761)
*Matthias Beckmann,Robert Beinert,Jonas Bresch*

Main category: math.NA

TLDR: 本文研究了max-normalized R-CDT的鲁棒性，并引入mean-normalized R-CDT以应对非仿射变形和脉冲噪声。


<details>
  <summary>Details</summary>
Motivation: 解决图像分类中因测量过程导致的仿射变换问题，并进一步研究非仿射变形的鲁棒性。

Method: 提出max-normalized和mean-normalized R-CDT，分析其在水无穷距离和水二距离下的稳定性。

Result: 理论分析和数值实验表明，新特征提取器对局部非仿射变形和脉冲噪声具有鲁棒性。

Conclusion: max-normalized和mean-normalized R-CDT在图像分类中具有鲁棒性和有效性。
...
### [404] [Structured Variational $D$-Decomposition for Accurate and Stable Low-Rank Approximation](https://arxiv.org/abs/2506.08535)
*Ronald Katende*

Main category: math.NA

TLDR: 论文提出了一种名为$D$-分解的非正交矩阵分解方法，形式为$A \approx P D Q$，通过最小化正则化Frobenius损失实现，支持控制秩、稀疏性和条件数。


<details>
  <summary>Details</summary>
Motivation: 传统代数分解方法（如LU或SVD）缺乏灵活性，无法直接控制稀疏性和条件数，因此需要一种更灵活的分解方法。

Method: 采用交替最小化方法计算$D$-分解，每一步更新的复杂度为$\mathcal{O}(n^2k)$。

Result: 在MovieLens、MNIST、Olivetti Faces和基因表达矩阵上，$D$-分解相比截断SVD、CUR和非负矩阵分解具有更高的重建精度，尤其在稀疏和噪声条件下表现更优。

Conclusion: $D$-分解是一种灵活且高效的矩阵分解方法，适用于需要控制稀疏性和条件数的场景。
...
### [405] [sparseGeoHOPCA: A Geometric Solution to Sparse Higher-Order PCA Without Covariance Estimation](https://arxiv.org/abs/2506.08670)
*Renjie Xu,Chong Wu,Maolin Che,Zhuoheng Ran,Yimin Wei,Hong Yan*

Main category: math.NA

TLDR: 提出了一种名为sparseGeoHOPCA的新框架，通过几何视角解决高维张量分解问题，显著提升计算效率和可解释性。


<details>
  <summary>Details</summary>
Motivation: 解决传统稀疏高阶主成分分析（SHOPCA）在高维和不平衡数据场景中的非凸性和计算效率问题。

Method: 通过将输入张量沿每个模式展开，并将子问题转化为结构化二元线性优化问题，将原始非凸稀疏目标转化为可处理的几何形式。

Result: 理论证明了几何子问题与原始SHOPCA的等价性，并提供了数据依赖的性能保证。实验表明，该方法在合成数据中准确恢复稀疏支持，在10倍压缩下保持分类性能，并在ImageNet上实现高质量图像重建。

Conclusion: sparseGeoHOPCA在计算效率和可解释性方面具有显著优势，适用于高维和不平衡数据场景。
...
### [406] [Exp4Fuse: A Rank Fusion Framework for Enhanced Sparse Retrieval using Large Language Model-based Query Expansion](https://arxiv.org/abs/2506.04760)
*Lingyuan Liu,Mengxiang Zhang*

Main category: cs.IR

TLDR: Exp4Fuse是一种新型融合排名框架，通过零样本LLM查询扩展提升稀疏检索性能。


<details>
  <summary>Details</summary>
Motivation: 解决LLM生成文档质量依赖复杂提示策略和高计算成本的问题。

Method: 提出Exp4Fuse框架，结合原始查询和LLM增强查询的两种检索路径，使用改进的互惠排名融合方法。

Result: 在多个数据集上超越现有LLM查询扩展方法，结合先进稀疏检索器达到SOTA性能。

Conclusion: Exp4Fuse显著提升了稀疏检索的查询扩展效果。
...
### [407] [Hierarchical Lexical Graph for Enhanced Multi-Hop Retrieval](https://arxiv.org/abs/2506.08074)
...
TLDR: 论文提出了一种名为HLG的分层词汇图索引，结合两种检索器（StatementGraphRAG和TopicGraphRAG），显著提升了多文档检索性能，并在实验中表现优于传统RAG方法。


<details>
  <summary>Details</summary>
Motivation: 传统检索增强生成（RAG）在处理跨文档语义分散信息时表现不佳，需要更高效的索引和检索方法。

Method: 设计了HLG三层索引结构，并开发了两种互补的检索器，同时引入合成数据集生成流程以评估多跳检索系统。

Result: 实验表明，该方法在五个数据集上平均检索召回率和正确率相对提升23.1%。

Conclusion: HLG和配套检索器有效解决了跨文档信息整合问题，开源工具包可供使用。
...
### [408] [Bridging RDF Knowledge Graphs with Graph Neural Networks for Semantically-Rich Recommender Systems](https://arxiv.org/abs/2506.08743)
*Michael Färber,David Lamprecht,Yuni Susanti*

Main category: cs.IR

TLDR: 该论文提出了一种将RDF知识图谱（KGs）与图神经网络（GNNs）全面集成的方法，以充分利用RDF的语义信息，提升推荐系统的性能。
...
Result: 实验表明，利用RDF KGs的语义丰富性显著提高了推荐系统的性能。
...
### [409] [Multimodal Representation Alignment for Cross-modal Information Retrieval](https://arxiv.org/abs/2506.08774)
*Fan Xu,Luis A. Leiva*

Main category: cs.IR

TLDR: 该论文研究了多模态检索中的特征对齐问题，探讨了视觉和文本嵌入的几何关系，并比较了多种相似性度量方法。研究发现Wasserstein距离可用于衡量模态差距，而余弦相似度在特征对齐任务中表现最佳。


<details>
  <summary>Details</summary>
Motivation: 多模态检索中，不同机器学习模型对同一概念的表示方式不同，这为跨模态特征对齐带来了挑战。研究旨在探索视觉和文本嵌入的几何关系，并找到有效的对齐方法。

Method: 研究首先分析了视觉和文本嵌入的几何关系，随后使用四种标准相似性度量和两种基于神经网络的学习度量进行特征对齐。

Result: Wasserstein距离可作为模态差距的衡量指标，而余弦相似度在特征对齐任务中表现最优。此外，传统的多层感知机架构不足以捕捉图像和文本表示之间的复杂交互。

Conclusion: 该研究为多模态信息检索提供了新的见解和实践建议，特别是在现实世界的跨模态应用中。
...
### [410] [QUITE: A Query Rewrite System Beyond Rules with LLM Agents](https://arxiv.org/abs/2506.07675)
*Yuyang Song,Hanxu Yan,Jiale Lao,Yibo Wang,Yufei Li,Yuanchun Zhou,Jianguo Wang,Mingjie Tang*

Main category: cs.DB

TLDR: 论文提出了一种基于LLM的SQL查询重写方法QUITE，解决了规则方法的局限性，通过多智能体框架和实时反馈显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 规则方法在SQL查询重写中存在局限性，而人类专家和LLM的语义推理能力为解决这一问题提供了新思路。

Method: 设计了基于有限状态机的多智能体框架，结合外部工具和实时反馈，并开发了重写中间件和提示注入技术。

Result: QUITE将查询执行时间减少35.8%，重写数量增加24.1%，覆盖更多查询模式。

Conclusion: QUITE是一种高效且通用的查询重写方法，优于现有规则方法。
...
### [411] [RADAR: Benchmarking Language Models on Imperfect Tabular Data](https://arxiv.org/abs/2506.08249)
...
TLDR: RADAR是一个用于评估语言模型在表格数据中处理数据异常能力的基准测试，揭示了前沿模型在数据异常下的性能下降。


<details>
  <summary>Details</summary>
Motivation: 语言模型在自主数据分析中的部署日益增多，但其对数据异常（如缺失值、离群值等）的处理能力尚未充分研究，这些异常可能严重影响分析结论的有效性。

Method: 开发了RADAR基准测试，通过程序化扰动模拟数据异常，包含2980个表格查询对，涵盖9个领域和5种数据异常类型，并系统性地研究表格大小对推理性能的影响。

Result: 前沿模型在无数据异常时表现尚可，但在数据异常引入后性能显著下降，暴露了其在稳健数据分析能力上的不足。
...
### [412] [LEANN: A Low-Storage Vector Index](https://arxiv.org/abs/2506.08276)
...
TLDR: LEANN是一种高效的近似最近邻搜索索引，专为资源受限的个人设备设计，显著减少存储开销，同时保持搜索性能。


<details>
  <summary>Details</summary>
Motivation: 随着对本地个人数据嵌入搜索需求的增长，传统方法的高存储开销使其不适用于本地部署，因此需要一种存储高效的解决方案。

Method: LEANN结合了紧凑的基于图的结构和动态重计算策略，以减少存储需求并保持搜索速度和准确性。

Result: LEANN将索引大小降至原始数据的5%以下，存储需求比标准索引小50倍，同时在真实问答基准测试中保持90%的top-3召回率，延迟低于2秒。

Conclusion: LEANN为资源受限设备提供了一种高效的嵌入搜索解决方案，显著降低了存储开销，同时保持了良好的搜索性能。
...
### [413] [Inverse Design in Distributed Circuits Using Single-Step Reinforcement Learning](https://arxiv.org/abs/2506.08029)
*Jiayu Li,Masood Mortazavi,Ning Yan,Yihong Ma,Reza Zafarani*

Main category: eess.SY

TLDR: DCIDA是一种分布式电路逆向设计框架，通过联合训练的采样策略生成近最优设计，显著降低设计误差。


<details>
  <summary>Details</summary>
Motivation: 现有设计方法依赖人工网格、可微分评估和固定拓扑，而实际需求涉及非可微分评估、多变拓扑和连续布局空间。

Method: DCIDA通过联合训练的采样策略生成设计决策，利用注入式映射将原始设计动作转化为物理表示。

Result: 实验表明，DCIDA基于Transformer的策略网络显著降低了设计误差，尤其适用于复杂传递函数。

Conclusion: DCIDA为分布式电路设计提供了更灵活、高效的解决方案。
...
### [414] [Feasibility Study of CNNs and MLPs for Radiation Heat Transfer in 2-D Furnaces with Spectrally Participative Gases](https://arxiv.org/abs/2506.08033)
*Axel TahmasebiMoradi,Vincent Ren,Benjamin Le-Creurer,Chetra Mang*

Main category: eess.SY

TLDR: 论文提出了一种基于CNN和MLP的替代模型，用于近似2D壁域中参与性气体的辐射热传递解，以减少计算成本。CNN在精度和鲁棒性上优于MLP。
...
Method: 使用CNN和MLP构建替代模型，输入问题参数（气体和壁属性）以适应CNN架构。数据集由经典求解器ICARUS2D生成，并通过Optuna优化超参数。

Result: CNN和MLP均显著加速计算且误差在工业可接受范围内，CNN在精度和鲁棒性上优于MLP。

Conclusion: CNN是更优的替代模型选择，适用于辐射热传递的高效近似。
...
### [415] [Learning-Based Multiuser Scheduling in MIMO-OFDM Systems with Hybrid Beamforming](https://arxiv.org/abs/2506.08263)
*Pouya Agheli,Tugce Kobal,François Durand,Matthew Andrews*

Main category: cs.IT

TLDR: 论文研究了在毫米波信道下，基于混合波束成形的MIMO-OFDM系统中的多用户调度问题，旨在通过优化模拟和数字预编码器设计以及用户选择，最大化比例公平性。


<details>
  <summary>Details</summary>
Motivation: 由于混合波束成形系统的复用增益有限，改进调度对提升频谱效率和系统长期性能至关重要。

Method: 采用两时间尺度协议：长时尺度为每个用户分配模拟波束，短时尺度进行用户调度和数字预编码器设计。提出了组合算法（贪婪和排序算法）及机器学习方法。

Result: 数值结果展示了性能与复杂度之间的权衡，表明方法选择需根据具体场景标准。

Conclusion: 研究表明，不同方法在不同场景下各有优劣，需根据实际需求选择合适方案。
...
### [416] [Flow-Lenia: Emergent evolutionary dynamics in mass conservative continuous cellular automata](https://arxiv.org/abs/2506.08569)
...
TLDR: Flow-Lenia是Lenia的质量守恒扩展，用于生成具有复杂行为的空间局部化模式（SLPs），并展示了参数优化和多物种模拟的能力。


<details>
  <summary>Details</summary>
Motivation: 研究人工生命系统中自生、自复制、进化和开放性等生命特性的自发生成。

Method: 提出Flow-Lenia，一种基于Lenia的连续细胞自动机模型，通过参数优化和多物种嵌入实现复杂行为模拟。

Result: 实验证明Flow-Lenia能生成复杂行为的SLPs，并支持多物种模拟和进化动态分析。

Conclusion: Flow-Lenia为人工生命研究提供了新的工具，展示了其在模拟复杂生命行为和进化动态中的潜力。
...
### [417] [MOSS: Multi-Objective Optimization for Stable Rule Sets](https://arxiv.org/abs/2506.08030)
*Brian Liu,Rahul Mazumder*

Main category: math.OC

TLDR: MOSS是一个多目标优化框架，用于构建稳定的决策规则集，同时考虑稀疏性、准确性和稳定性。


<details>
  <summary>Details</summary>
Motivation: 解决决策规则集中准确性与稳定性之间的权衡问题，提供可解释的模型选择。

Method: 开发了专门的切割平面算法，快速计算Pareto前沿，适用于大规模问题。

Result: MOSS在预测性能和稳定性上优于现有规则集成方法。

Conclusion: MOSS为多目标优化提供了一种高效且可扩展的解决方案。
...
### [418] [Continuous Policy and Value Iteration for Stochastic Control Problems and Its Convergence](https://arxiv.org/abs/2506.08121)
*Qi Feng,Gu Wang*

Main category: math.OC

TLDR: 提出了一种连续策略-值迭代算法，通过Langevin型动力学同时更新随机控制问题的值函数近似和最优控制。


<details>
  <summary>Details</summary>
Motivation: 解决熵正则化松弛控制问题和经典控制问题中的无限时域优化问题，利用Langevin型随机微分方程实现连续更新。

Method: 结合策略迭代方向的Langevin型随机微分方程，利用分布采样和非凸学习技术优化值函数和最优控制。

Result: 在哈密顿单调性条件下，证明了策略改进和收敛到最优控制。

Conclusion: 该方法为机器学习的分布采样和非凸优化技术提供了新的应用场景。
...
### [419] [Solving Convex-Concave Problems with $\tilde{\mathcal{O}}(ε^{-4/7})$ Second-Order Oracle Complexity](https://arxiv.org/abs/2506.08362)
...
### [420] [Sharper Convergence Rates for Nonconvex Optimisation via Reduction Mappings](https://arxiv.org/abs/2506.08428)
*Evan Markou,Thalaiyasingam Ajanthan,Stephen Gould*

Main category: math.OC

TLDR: 论文提出了一种利用降维映射优化高维问题的方法，通过利用解流形的几何结构改善目标函数的曲率，从而加速梯度下降法的收敛。


<details>
  <summary>Details</summary>
Motivation: 高维优化问题的解集通常具有丰富的几何结构（如流形），若能利用这些结构，可以通过降维映射去除冗余方向，改善优化问题的条件数。

Method: 提出一个通用框架，分析降维映射如何影响优化问题的曲率，并通过理论证明其能加速梯度下降法的收敛。

Result: 研究表明，设计良好的降维映射能改善目标函数的曲率，从而提升优化问题的条件数，理论上加速收敛。
...
### [421] [Optimization over Sparse Support-Preserving Sets: Two-Step Projection with Global Optimality Guarantees](https://arxiv.org/abs/2506.08558)
*William de Vazelhes,Xiao-Tong Yuan,Bin Gu*

Main category: math.OC

TLDR: 论文提出了一种新的迭代硬阈值算法，用于解决稀疏优化中带有额外支持保持约束的问题，并提供了全局收敛性保证。


<details>
  <summary>Details</summary>
Motivation: 稀疏优化中，硬约束（如ℓ0伪范数）能更好地控制稀疏性，但实际应用常需额外约束。现有方法通常需要闭式投影或仅提供局部收敛保证，无法满足全局需求。

Method: 提出了一种带有两步连续投影算子的迭代硬阈值算法，用于处理混合约束，并通过稀疏松弛和次优性权衡提供全局收敛性。

Result: 在确定性、随机和零阶设置下，算法在受限强凸/光滑假设下提供了全局目标值保证，并在零阶情况下改进了现有结果。

Conclusion: 新算法填补了稀疏优化中混合约束处理的空白，并通过创新的证明技术提供了更优的收敛分析。
...
TLDR: MasHost是一个基于强化学习的框架，用于自主设计多智能体系统（MAS），通过图搜索问题和统一概率采样机制优化智能体角色和交互。


<details>
  <summary>Details</summary>
Motivation: 现有MAS构建方法依赖人工设计的交互机制或启发式规则，限制了自主性并引入人为偏见。

Method: 提出MasHost框架，将MAS构建视为图搜索问题，使用统一概率采样机制和分层相对策略优化（HRPO）实现多目标优化。

Result: 在六个基准测试中，MasHost表现优于现有基线，验证了其有效性、效率和结构合理性。

Conclusion: MasHost是首个基于强化学习的自主MAS图构建框架，具有显著优势。
...
### [423] [KP-PINNs: Kernel Packet Accelerated Physics Informed Neural Networks](https://arxiv.org/abs/2506.08563)
*Siyuan Yang,Cheng Song,Zhilu Lai,Wenjia Wang*

Main category: cs.CE

TLDR: KP-PINNs提出了一种新的PINNs框架，通过RKHS范数重新定义损失函数，并利用KP方法加速计算，显著提升了求解微分方程的稳定性和效率。


<details>
  <summary>Details</summary>
Motivation: 传统PINNs使用L2损失函数时，对复杂微分方程的求解结果不稳定且不准确，需要改进。

Method: 提出KP-PINNs框架，利用RKHS范数定义新损失函数，并采用KP方法加速计算。
...
### [424] [A System for Accurate Tracking and Video Recordings of Rodent Eye Movements using Convolutional Neural Networks for Biomedical Image Segmentation](https://arxiv.org/abs/2506.08183)
*Isha Puri,David Cox*

Main category: eess.IV

TLDR: 提出了一种基于卷积神经网络的灵活、鲁棒且高精度的啮齿动物瞳孔和角膜反射识别方法，解决了现有技术未考虑啮齿动物眼睛独特特性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有眼动追踪技术主要针对人眼，未考虑啮齿动物眼睛的特殊性（如参数变异、周围毛发多、尺寸小），需要一种适应这些特性的方法。

Method: 采用可增量训练的卷积神经网络架构，结合自动化红外视频眼动记录系统，实现高精度的瞳孔和角膜反射识别。

Result: 该方法首次展示了高精度且实用的生物医学图像分割技术，为啮齿动物眼动追踪提供了先进解决方案。

Conclusion: 结合自动化红外视频系统，该方法为神经科学和视觉科学研究提供了最先进的啮齿动物眼动追踪技术。
...
### [425] [Snap-and-tune: combining deep learning and test-time optimization for high-fidelity cardiovascular volumetric meshing](https://arxiv.org/abs/2506.08280)
*Daniel H. Pak,Shubh Thaker,Kyle Baylous,Xiaoran Zhang,Danny Bluestein,James S. Duncan*

Main category: eess.IV

TLDR: 提出了一种结合深度学习和测试时优化的“snap-and-tune”策略，用于医学图像的高质量体积网格生成，显著提高了空间精度和网格质量。
...
### [426] [Plug-and-Play Linear Attention for Pre-trained Image and Video Restoration Models](https://arxiv.org/abs/2506.08520)
*Srinivasan Kidambi,Pravin Nair*

Main category: eess.IV

TLDR: PnP-Nystra是一种基于Nyström的线性自注意力近似方法，作为即插即用模块，无需重新训练即可加速预训练模型，显著提升计算效率。
...
Method: 提出PnP-Nystra，一种基于Nyström的线性自注意力近似方法，可直接替换MHSA模块，适用于多种窗口式Transformer架构。
...
### [427] [DCD: A Semantic Segmentation Model for Fetal Ultrasound Four-Chamber View](https://arxiv.org/abs/2506.08534)
*Donglian Li,Hui Guo,Minglang Chen,Huizhen Chen,Jialing Chen,Bocheng Liang,Pengchen Liang,Ying Tan*

Main category: eess.IV

TLDR: 提出了一种基于深度学习的模型DCD，用于自动分割胎儿心超A4C视图中的关键解剖结构，以提高分割精度并减少工作量。


<details>
  <summary>Details</summary>
Motivation: 胎儿心超A4C视图的精确分割对先天性心脏病早期诊断至关重要，但超声伪影、噪声和解剖变异性使其具有挑战性。

Method: 结合Dense ASPP模块进行多尺度特征提取，并使用CBAM增强自适应特征表示。

Result: DCD模型实现了精确且鲁棒的分割，提升了产前心脏评估效果。

Conclusion: DCD模型为胎儿心脏解剖结构的自动分割提供了高效解决方案。
...
### [428] [Biologically Inspired Deep Learning Approaches for Fetal Ultrasound Image Classification](https://arxiv.org/abs/2506.08623)
*Rinat Prochii,Elizaveta Dakhova,Pavel Birulin,Maxim Sharaev*

Main category: eess.IV

TLDR: 提出了一种生物启发的深度学习集成框架，用于同时分类16种胎儿超声图像结构，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决胎儿超声图像分类中因低质量、高类内变异性和类别不平衡带来的挑战。
...
### [429] [MAMBO: High-Resolution Generative Approach for Mammography Images](https://arxiv.org/abs/2506.08677)
*Milica Škipina,Nikola Jovišić,Nicola Dall'Asen,Vanja Švenda,Anil Osman Tur,Slobodan Ilić,Elisa Ricci,Dubravko Ćulibrk*

Main category: eess.IV

TLDR: 论文提出了一种名为MAMBO的基于扩散模型的乳腺X光图像生成方法，旨在解决训练AI系统时数据不足的问题，并成功生成高分辨率图像。
...
Method: MAMBO采用基于块的扩散方法，结合局部和全局上下文信息，生成高达3840x3840像素的高分辨率乳腺X光图像。

Result: 实验证明MAMBO能生成高度逼真的图像，并可用于分类模型训练和异常检测。
...
### [430] [Neural-Augmented Kelvinlet: Real-Time Soft Tissue Deformation with Multiple Graspers](https://arxiv.org/abs/2506.08043)
...
TLDR: 提出了一种基于物理信息的神经模拟器，用于实时、高精度模拟软组织变形，结合Kelvinlet先验和大规模FEM模拟，提升预测准确性和物理一致性。


<details>
  <summary>Details</summary>
Motivation: 手术机器人和医学训练需要快速、准确的软组织变形模拟。

Method: 将Kelvinlet先验集成到神经模拟器中，结合线性与非线性FEM模拟，通过残差学习和正则化改进预测。

Result: 实现了高保真度的实时软组织模拟，适用于腹腔镜手术工具操作。
...
### [431] [A Real-time 3D Desktop Display](https://arxiv.org/abs/2506.08064)
*Livio Tenze,Enrique Canessa*

Main category: cs.GR

TLDR: 扩展版altiro3D库，支持从2D图像或视频流实时生成3D光场，利用AI技术提升性能，并新增多平台GUI。


<details>
  <summary>Details</summary>
Motivation: 为玻璃全息显示提供更灵活的3D体验，支持多种输入源（如2D摄像头、视频文件或桌面应用）。

Method: 使用MiDaS CNN从单张2D图像提取深度图，结合AI技术优化处理，新增GUI简化操作。

Result: 实现实时3D光场合成，支持多种输入源，并兼容光场3D设备（如Looking Glass）。

Conclusion: 扩展版altiro3D库提升了3D内容生成的灵活性和性能，适用于多样化应用场景。
...
### [432] [Generalizable Articulated Object Reconstruction from Casually Captured RGBD Videos](https://arxiv.org/abs/2506.08334)
*Weikun Peng,Jun Lv,Cewu Lu,Manolis Savva*

Main category: cs.GR

TLDR: 论文提出了一种从手持相机拍摄的RGBD视频中重建关节物体的方法，解决了现有方法对数据要求高的问题。
...
Method: 采用由粗到细的框架，从动态RGBD视频中推断关节参数并分割可移动部分。

Result: 在合成和真实数据集上，该方法显著优于现有方法。

Conclusion: 该方法能够从动态RGBD视频中高效重建关节物体，具有广泛的应用潜力。
...
### [433] [Complex-Valued Holographic Radiance Fields](https://arxiv.org/abs/2506.08350)
*Yicheng Zhan,Dong-Ha Shin,Seung-Hwan Baek,Kaan Akşit*

Main category: cs.GR

TLDR: 提出了一种基于复值高斯基元的3D全息场景表示方法，显著提升了渲染速度。
...
Result: 相比现有方法，实现了30倍至10,000倍的加速，同时保持图像质量。
...
### [434] [Fine-Grained Spatially Varying Material Selection in Images](https://arxiv.org/abs/2506.09023)
*Julia Guerrero-Viu,Michael Fischer,Iliyan Georgiev,Elena Garces,Diego Gutierrez,Belen Masia,Valentin Deschaintre*

Main category: cs.GR

TLDR: 提出了一种基于视觉Transformer（ViT）的材料选择方法，通过多分辨率处理策略，实现了对光照和反射变化鲁棒的材料选择，并支持纹理和子纹理两级选择。


<details>
  <summary>Details</summary>
Motivation: 图像编辑中材料选择是第一步，但现有方法对光照和反射变化敏感，且缺乏多级选择能力。

Method: 利用ViT模型特征，采用多分辨率处理策略，结合新构建的DuMaS数据集（包含80万张合成图像的两级标注）。

Result: 相比现有方法，实现了更精细和稳定的选择结果，支持纹理和子纹理两级选择。

Conclusion: 该方法为图像编辑提供了鲁棒且多级的材料选择工具，推动了相关领域的发展。
...
### [435] [CaliciBoost: Performance-Driven Evaluation of Molecular Representations for Caco-2 Permeability Prediction](https://arxiv.org/abs/2506.08059)
...
TLDR: 研究通过结合多种分子特征表示和AutoML技术，提升了Caco-2渗透性的预测准确性，发现3D描述符能显著降低误差。
...
Result: PaDEL、Mordred和RDKit描述符表现最佳，AutoML模型CaliciBoost的MAE最优，3D描述符比2D降低15.73%误差。
...
### [436] [Protriever: End-to-End Differentiable Protein Homology Search for Fitness Prediction](https://arxiv.org/abs/2506.08954)
*Ruben Weitzman,Peter Mørch Groth,Lood Van Niekerk,Aoi Otani,Yarin Gal,Debora Marks,Pascal Notin*

Main category: q-bio.QM

TLDR: Protriever是一个端到端的可微分框架，用于检索同源蛋白序列并同时训练目标任务，相比传统MSA方法更快且性能更优。


<details>
  <summary>Details</summary>
Motivation: 传统基于MSA的同源序列检索方法计算成本高，且难以处理高度分化的序列或复杂插入删除模式。

Method: Protriever通过可微分框架学习检索相关同源序列，同时训练目标任务，支持高效向量搜索。

Result: 在蛋白质适应性预测任务中，Protriever性能优于传统MSA方法，且速度快两个数量级。

Conclusion: Protriever提供了一种可扩展的替代方案，适用于不同架构和任务。
...
### [437] [EDINET-Bench: Evaluating LLMs on Complex Financial Tasks using Japanese Financial Statements](https://arxiv.org/abs/2506.08762)
*Issa Sugiura,Takashi Ishida,Taro Makino,Chieko Tazuke,Takanori Nakagawa,Kosuke Nakago,David Ha*

Main category: q-fin.ST

TLDR: EDINET-Bench是一个开源日本金融基准数据集，用于评估大语言模型在金融任务（如会计欺诈检测、盈利预测等）中的表现，结果显示现有模型表现不佳，需领域特定优化。


<details>
  <summary>Details</summary>
Motivation: 日本金融数据稀缺，阻碍了金融分析领域的学术创新和大语言模型的发展与评估。

Method: 通过从EDINET下载过去10年的年度报告，自动标注任务标签，构建EDINET-Bench数据集。
...
### [438] [ABC-FHE : A Resource-Efficient Accelerator Enabling Bootstrappable Parameters for Client-Side Fully Homomorphic Encryption](https://arxiv.org/abs/2506.08461)
*Sungwoong Yune,Hyojeong Lee,Adiwena Putra,Hyunjun Cho,Cuong Duong Manh,Jaeho Jeon,Joo-Young Kim*

Main category: cs.AR

TLDR: ABC-FHE是一种高效的全同态加密加速器，针对客户端计算瓶颈提出，通过流式架构和关键创新显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 全同态加密（FHE）因计算开销大而难以广泛应用，尤其是客户端计算成为瓶颈。

Method: 采用流式架构、可重构傅里叶引擎、片上伪随机数生成器和优化的任务调度。

Result: 在28纳米工艺下，ABC-FHE面积为28.638 mm²，功耗5.654 W，性能提升显著（编码加密速度提升1112倍，解码解密速度提升963倍）。

Conclusion: ABC-FHE有效解决了客户端FHE计算瓶颈，为隐私保护计算提供了高效解决方案。
...
### [439] [POLARON: Precision-aware On-device Learning and Adaptive Runtime-cONfigurable AI acceleration](https://arxiv.org/abs/2506.08785)
*Mukul Lokhande,Santosh Kumar Vishvakarma*

Main category: cs.AR

TLDR: PARV-CE是一种支持多精度格式的SIMD MAC引擎，通过统一数据路径和自适应精度策略优化性能和能耗，适用于边缘AI加速。


<details>
  <summary>Details</summary>
Motivation: AI模型复杂度增加，边缘平台需要灵活支持多精度格式的硬件以降低能耗。

Method: 设计PARV-CE引擎，结合统一数据路径、自适应精度策略和硬件-软件协同设计，支持多种精度格式。

Result: 相比现有设计，PDP提升2倍，资源使用减少3倍，精度损失在1.8%以内。

Conclusion: PARV-CE是一种高效、可扩展的边缘AI加速解决方案。
...
### [440] [Aligning Proteins and Language: A Foundation Model for Protein Retrieval](https://arxiv.org/abs/2506.08023)
*Qifeng Wu,Zhengzhe Liu,Han Zhu,Yizhou Zhao,Daisuke Kihara,Min Xu*

Main category: q-bio.BM

TLDR: 提出了一种基于CLIP风格的多模态框架，用于对齐3D蛋白质结构与功能注释，通过对比学习实现蛋白质检索。


<details>
  <summary>Details</summary>
Motivation: 利用视觉-语言模型的最新进展，促进从大规模蛋白质数据集中检索具有相似结构和语义的蛋白质，以支持功能解释。

Method: 采用CLIP风格的框架，通过对比学习对齐3D蛋白质结构与功能注释，并使用约20万蛋白质-描述对进行训练。

Result: 在PDB和EMDB数据集上展示了良好的零样本检索性能，验证了多模态基础模型在蛋白质结构-功能理解中的潜力。

Conclusion: 该方法为蛋白质生物学中的结构-功能理解提供了新的可能性，展示了多模态模型的优势。
...
### [441] [Real-Time Cascade Mitigation in Power Systems Using Influence Graph Improved by Reinforcement Learning](https://arxiv.org/abs/2506.08893)
*Kai Zhou,Youbiao He,Chong Zhong,Yifu Wu*

Main category: physics.soc-ph

TLDR: 论文提出了一种基于马尔可夫决策过程（MDP）和强化学习的方法，用于实时缓解电力传输系统中的级联停电风险，考虑了发电、负载和初始故障的不确定性。


<details>
  <summary>Details</summary>
Motivation: 现代电力系统虽然可靠性高，但随着可再生能源渗透率的增加，级联停电风险上升，需要快速、复杂的实时决策。

Method: 将影响图扩展为MDP模型，结合强化学习（特别是策略梯度算法）进行求解，设计了保守的奖励机制。

Result: 在IEEE 14总线和118总线系统上验证，主动断开某些线路可有效降低级联风险，并识别出关键线路。
...
### [442] [Domain Switching on the Pareto Front: Multi-Objective Deep Kernel Learning in Automated Piezoresponse Force Microscopy](https://arxiv.org/abs/2506.08073)
*Yu Liu,Utkarsh Pratiush,Kamyar Barakati,Hiroshi Funakubo,Ching-Che Lin,Jaegyu Kim,Lane W. Martin,Sergei V. Kalinin*

Main category: cond-mat.mtrl-sci

TLDR: 提出了一种多目标核学习工作流，直接从高分辨率成像数据中推断微观结构规则，用于高效分析铁电极化切换行为。


<details>
  <summary>Details</summary>
Motivation: 铁电极化切换行为受复杂微观结构特征影响，传统手动或网格测量方法难以系统研究。

Method: 采用多目标核学习工作流，结合自动化压电力显微镜实验，分析域壁构型与局部切换动力学关系。

Result: 揭示了特定壁几何和缺陷分布如何调控极化反转，并将抽象奖励函数映射到物理可解释描述符。

Conclusion: 该方法不仅适用于铁电域切换，还可推广至复杂非可微设计空间的探索，如分子发现中的结构-性质关联。
...
### [443] [Mic-hackathon 2024: Hackathon on Machine Learning for Electron and Scanning Probe Microscopy](https://arxiv.org/abs/2506.08423)
...
TLDR: 显微镜数据管理存在标准化不足问题，机器学习与显微镜社区合作通过黑客松活动推动解决方案。


<details>
  <summary>Details</summary>
Motivation: 解决显微镜数据管理中的标准化和机器学习应用不足问题，提升数据分析效率和材料科学研究。

Method: 通过黑客松活动促进机器学习研究者与显微镜专家合作，开发标准化代码和数字孪生显微镜。

Result: 生成了基准数据集和数字孪生显微镜，支持社区发展和标准化工作流程。

Conclusion: 黑客松活动有效推动了机器学习在显微镜领域的应用，为未来研究奠定了基础。
