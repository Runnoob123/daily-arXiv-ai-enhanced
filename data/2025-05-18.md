...
Motivation: 现代移动平台依赖多种定位技术，但这些技术易受攻击，导致位置服务功能被破坏，亟需有效的防御方法。

Method: 提出扩展的RAIM框架，结合多种定位信息（如传感器、地面基础设施和GNSS）以提高抗欺骗能力。

Result: 扩展RAIM框架显著提高了检测准确性（提升24-58%），并在5秒内检测到攻击，且误报率低。

Conclusion: 扩展RAIM框架为智能手机定位系统提供了一种有效的抗欺骗解决方案，弥补了现有防御方法的不足。
...
Result: 系统在真实SOC环境中表现优异，减少61%的警报展示，误报率仅为1.36%。

Conclusion: AACT系统显著提升SOC效率，减轻分析师负担，适用于大规模网络安全环境。
...
<details>
  <summary>Details</summary>
Motivation: 以太坊混币服务（如Tornado Cash）的交易不可追踪性对区块链安全和金融监管构成挑战，现有方法因标签数据有限和噪声标注问题而受限。

Method: 提出StealthLink框架，通过MixFusion模块构建混币子图捕获交易模式，并引入知识转移机制通过对抗性差异最小化对齐跨域特征。

Result: 在真实混币交易数据集上，StealthLink在10-shot学习场景中达到96.98%的F1分数，且在数据不平衡条件下优于传统监督方法。

Conclusion: StealthLink是区块链取证中首个跨域知识转移的系统性方法，为打击去中心化生态中的隐私增强金融犯罪提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在多个领域表现出色，但存在隐私泄露风险。现有评估方法容易被对齐模型阻止，而越狱攻击在隐私场景中的作用尚未充分研究。

Method: 提出PIG框架，通过识别PII实体、构建隐私上下文，并采用三种梯度策略迭代更新，以提取目标PII。

Result: 在四种白盒和两种黑盒LLMs上的实验表明，PIG优于基线方法，达到SoTA效果。

Conclusion: 研究揭示了LLMs的重大隐私风险，强调了加强保护措施的必要性。
...
Method: 设计DeFeed协议，包含多种功能（如池功能、缓存功能、订阅功能等），优化gas成本并简化操作。

Result: 在以太坊测试网络上实现，显著提升合约交互效率，降低计算复杂性和gas成本。
...
<details>
  <summary>Details</summary>
Motivation: 中国在消费物联网产业中占比较大，但相关研究较少，现有分析多集中于欧美和澳大利亚，因此填补中国地区的研究空白。

Method: 提出细粒度流量收集指南，覆盖设备全生命周期，收集了70个设备（36品牌、8类别）的流量数据。

Result: 中国设备更依赖国内服务，加密表现优于其他地区，但20/35设备证书验证不当，5/70设备使用不安全加密协议。

Conclusion: 研究填补了中国消费物联网流量研究的空白，开源了数据集和收集指南，为未来研究提供支持。
...
<details>
  <summary>Details</summary>
Motivation: 探讨LLM在网络安全应用中的潜力与风险，如威胁分析和恶意软件检测，同时关注数据泄露和恶意软件生成的隐患。

Method: 基于OWASP Top 10框架评估7个开源LLM，提出并测试了一种通过重写指令-响应对的安全对齐方法。

Result: 微调降低了所有测试LLM的安全韧性（如Llama 3.1 8B的提示注入安全分从0.95降至0.15），但安全对齐方法可维持或提升安全性。

Conclusion: 研究为LLM的安全风险提供了系统评估，推动了生成式AI在敏感领域的安全应用，促进了可信赖和伦理对齐LLM的发展。
...
<details>
  <summary>Details</summary>
Motivation: PRAC框架虽能缓解RowHammer漏洞，但其引入的定时通道可能被攻击者利用，泄露敏感数据。

Method: 提出TPRAC方案，采用定时刷新管理命令，独立于内存活动，消除定时通道。

Result: TPRAC成功关闭定时通道，性能开销仅为3.4%。

Conclusion: TPRAC是一种高效且兼容现有标准的防御方案，能有效消除PRAC框架的定时通道漏洞。
...
Method: 使用多头Transformer编码器处理运动学数据，区分正常行为和攻击模式。

Result: 实验显示AttentionGuard在攻击检测中F1分数达0.95，延迟低至100ms，适用于实时应用。
...
<details>
  <summary>Details</summary>
Motivation: Transformer模型在MLaaS中的集中处理引发隐私担忧，需要保护用户数据和模型隐私。

Method: 利用安全多方计算和同态加密等密码学技术实现PTI。

Result: 提出了PTI的分类和评估框架，平衡资源效率与隐私。

Conclusion: PTI在高效推理与数据隐私之间架起桥梁，但仍需解决挑战。
...
<details>
  <summary>Details</summary>
Motivation: BLS聚合签名虽然被广泛使用，但其使用复杂且容易因未满足安全要求而被攻击，需要形式化验证工具的支持。

Method: 设计了基于通用聚合签名密码学定义的模型，并特别针对BLS聚合签名建模，使用Tamarin工具分析SANA协议。

Result: 通过Tamarin发现SANA协议中未记录的假设可能导致攻击。

Conclusion: 形式化验证工具能有效发现聚合签名协议中的潜在漏洞，提升安全性。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在通过LLM降低渗透测试成本并提高频率，探索自动化测试的可行性。

Method: 基于GPT-4o和LangChain开发AutoPentest工具，支持多步骤任务，并通过外部工具增强功能。

Result: 在Hack The Box实验中，AutoPentest和ChatGPT完成15-25%的子任务，前者略优，成本为96.20美元。

Conclusion: 未来通过优化和更强大的LLM，自动化渗透测试可能成为漏洞管理的可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有本地差分隐私（LDP）机制在隐私保护与数据效用之间存在固有权衡，强隐私常以降低数据效用为代价。频率估计是LDP的基础，但传统RR机制在精度上存在不足。

Method: JRR将数据贡献者分为两人一组，组内成员联合扰动二进制数据，通过隐藏组信息实现隐私保护，同时提高频率估计精度。

Result: 理论分析和仿真实验表明，JRR在保持与RR相同隐私水平的同时，将频率估计精度提高了多达两个数量级。
...
<details>
  <summary>Details</summary>
Motivation: 尽管深度学习在金融领域广泛应用，但其安全性问题在算法交易系统中尚未得到充分研究，尤其是针对实际约束下的对抗攻击。

Method: 作者提出了短暂扰动（EP）的概念，并针对一个盈利的算法交易系统进行了端到端评估。

Result: 实验表明，即使对输入股票价格进行微小改动，也能导致深度学习模型行为异常，并使整个交易系统做出次优决策，影响财务表现。

Conclusion: 该研究揭示了深度学习在金融领域的潜在安全风险，强调了对抗攻击对算法交易系统的实际威胁。
...
<details>
  <summary>Details</summary>
Motivation: 软件供应链的脆弱性导致网络攻击激增，威胁到从企业到个人开发者的安全，因此需要行业和政府共同关注和改进。

Method: 通过组织Secure Software Supply Chain Summit，汇集12名来自9家公司的实践者，讨论六个关键主题，分享经验和挑战。

Result: 峰会促进了跨公司经验分享、新合作的形成，并为未来研究方向提供了实践者的实际挑战。

Conclusion: 软件供应链安全需要行业协作和持续研究，峰会为这一目标提供了重要平台。
...
TLDR: 提出了一种基于Sybil节点的虚拟数据投毒攻击方法，通过梯度匹配降低计算复杂度，并在非独立同分布数据下优于现有攻击算法。
...
Method: 利用Sybil节点生成虚拟数据放大攻击效果，基于梯度匹配降低计算复杂度，设计三种目标模型获取方案。

Result: 在非独立同分布数据下，该方法优于其他攻击算法，能获取全局目标模型。

Conclusion: 提出的Sybil虚拟数据投毒攻击高效且成本低，适用于多种联邦学习场景。
...
<details>
  <summary>Details</summary>
Motivation: 现有ANN-to-SNN转换方法在处理ANN-based LLMs时存在激活异常和非线性操作不兼容的问题，LAS旨在解决这些问题。

Method: LAS引入两种新型神经元转换ANN-based LLMs的激活异常和非线性操作，并定制了脉冲等效的Transformer组件。

Result: 在六种语言模型和两种视觉语言模型上，LAS实现了无损转换，并在OPT-66B上提升了2%的准确率。

Conclusion: LAS通过创新的神经元设计和组件定制，实现了无损且高效的ANN-SNN转换，验证了其有效性。
...
<details>
  <summary>Details</summary>
Motivation: 模拟内存计算（AIMC）在提升神经网络推理速度和能效方面具有潜力，但其噪声和量化限制导致现有LLM无法在4位精度下高效运行。

Method: 引入了一种鲁棒性方法，适配LLM以适应低精度模拟硬件的噪声和量化约束。

Result: 该方法使Phi-3-mini-4k-instruct和Llama-3.2-1B-Instruct等模型在模拟噪声和量化限制下仍能保持与4位权重、8位激活基线相当的性能。
...
<details>
  <summary>Details</summary>
Motivation: 图遗忘方法在保护隐私时可能引入预测偏差，影响不同敏感群体的公平性，因此需要研究公平的图遗忘方法。

Method: FGU方法通过分片训练子图模型、删除请求数据后重新训练，并采用双层去偏技术（分片级公平正则化和全局级对齐）确保公平性。

Result: 实验表明FGU在保持隐私和准确性的同时实现了优越的公平性，且对多样化的遗忘请求具有鲁棒性。

Conclusion: FGU是一种有效的公平图遗忘方法，解决了隐私保护与公平性的平衡问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有文献中，联邦学习在AIoT中的能源影响常被忽视，研究旨在填补这一空白。

Method: 提出两种聚类方法，将具有相似标签分布的设备分组，以减少异构性。

Result: 实验表明，所提方法在保持低能耗的同时，实现了较高的收敛速度。

Conclusion: 聚类策略能有效解决分布式学习中的异构性问题，同时优化能源效率。
...
Method: 提出几种新架构，分别对参数施加不同约束，并验证其可训练性。

Result: DMNNs可成功训练且更易剪枝，但泛化能力有限；混合架构显著加速大批次梯度下降收敛。

Conclusion: DMNNs在特定约束下可训练，混合架构能提升训练效率。
...
<details>
  <summary>Details</summary>
Motivation: 研究如何通过组合特征实现OOD泛化，验证算法是否真正学习到组合结构。

Method: 比较MLP、CNN和Transformer在OOD任务中的表现，并提出两种具有特定偏置的新网络架构。

Result: 常见神经网络在OOD任务中表现不佳，新架构虽能提升OOD性能，但仍可能未学习到正确的组合特征。

Conclusion: 仅依赖OOD测试不足以验证组合性，需进一步确认特征组合的正确性。
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习在保护数据隐私的同时面临数据质量问题（如噪声标签、类别缺失和不平衡分布），影响其有效性。

Method: 采用自适应噪声清理、协作条件GAN生成合成数据和鲁棒联邦模型训练，系统提升数据完整性。

Result: 在MNIST和Fashion-MNIST数据集上的实验表明，模型性能（尤其是macro-F1分数）在噪声和类别不平衡条件下显著提升。

Conclusion: 该方法有效缓解了数据质量挑战，提供了一种鲁棒、可扩展且符合隐私要求的解决方案，适用于实际联邦学习场景。
...
Method: 借鉴退火算法思想，将黑盒目标视为能量函数，训练神经网络建模Boltzmann分布，通过温度调节分布形态，从均匀分布到集中于全局最优解。

Result: 在有限和无限查询预算下，验证了该方法在复杂组合任务上的竞争力。

Conclusion: 该方法通过学习能量景观结构，提升了样本效率和求解质量，适用于不同查询成本的场景。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于邻居或固定交互图的方法无法捕捉灵活的协作模式，因此需要一种能够支持重叠社区和个性化成员权重的框架。

Method: 设计了一种基于社区的框架，每个智能体属于多个重叠社区，共享策略和价值函数，并通过个性化权重聚合。采用actor-critic算法，利用社区级估计进行策略更新和价值学习。

Result: 理论证明了在线性函数逼近下actor和critic更新的收敛性，支持迁移学习和主动学习。
...
TLDR: 研究生成模型在噪声和对抗性数据筛选下的自循环训练行为，分析其影响并提出攻击算法。


<details>
  <summary>Details</summary>
Motivation: 生成模型的自循环训练可能导致模型崩溃或不稳定，且数据筛选可能受到噪声或对抗性干扰。

Method: 理论分析噪声数据筛选对模型的影响，设计对抗性攻击算法。

Result: 实验证明算法在合成和真实数据集上的有效性。

Conclusion: 研究揭示了对抗性数据筛选对生成模型的威胁，并提出了应对策略。
...
<details>
  <summary>Details</summary>
Motivation: 在LLMs训练中，检查点数据量大且传输耗时，需优化以减少数据量和时间开销。

Method: 分析了张量数据的可压缩性，结合字节分组和增量增量压缩技术，开发了基于字节分组和霍夫曼编码的LMC压缩方案。

Result: LMC在压缩性能上优于BZ2，且压缩时间大幅减少，16核并行实现可达2.78 GiB/s压缩和3.76 GiB/s解压吞吐量。

Conclusion: LMC显著减少了CPU资源需求，支持更高频率的检查点，提升了训练效率。
...
<details>
  <summary>Details</summary>
Motivation: 中风是全球第二大死因和第三大致残原因，需更可靠的预测方法。

Method: 使用人口统计、临床和生活方式数据，评估逻辑回归、随机森林和XGBoost等模型，解决类别不平衡和缺失数据问题。

Result: 模型精度高但灵敏度不足，识别了关键预测特征。

Conclusion: 研究为开发更可靠、可解释的中风风险早期评估模型提供了基础。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM的广泛应用，理解其漏洞并提升安全性至关重要。现有方法在离散或连续空间优化时存在效率或效果问题，需要一种更优的解决方案。

Method: 采用指数梯度下降和Bregman投影方法，优化token嵌入空间，确保优化结果始终位于概率单纯形内。

Result: 在五个开源LLM和四个公开数据集上验证，该方法比现有三种越狱技术具有更高的成功率和效率。

Conclusion: 该方法有效解决了现有技术的不足，为LLM安全性研究提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 随着多路数据的普及，乘积图成为建模依赖结构的自然工具，但现有方法能学习的图类型有限，尤其是Kronecker乘积图因其复杂依赖关系而难以学习。

Method: 提出了一种交替优化方案，分别优化每个因子图，并提供了理论保证其渐近收敛性。算法还扩展到强乘积图的因子图学习。

Result: 在合成和真实世界图上进行了实验，证明了方法的有效性和优于现有方法的性能。

Conclusion: Kronecker乘积图能更精细地建模依赖关系，提出的算法为解决这一非凸问题提供了有效途径。
...
<details>
  <summary>Details</summary>
Motivation: 整合影像和基因组数据可以揭示疾病复杂性的新见解，尤其是在阿尔茨海默病研究中。

Method: 使用异质二分图表示学习框架，包含基因和图像两种节点类型，对阿尔茨海默病进行三阶段分类。

Result: 方法在小数据集上表现良好，能识别关键基因，并通过分类准确率、召回率等指标验证性能。

Conclusion: 该技术有潜力扩展到其他疾病的放射基因组分类。
...
Method: 提出ZENN神经网络，同时学习能量和内在熵分量，并重新设计网络架构以适应异构数据。

Result: ZENN在分类任务和能量景观重建中表现出优异的泛化能力和鲁棒性，成功应用于Fe3Pt材料的Helmholtz能量景观重建。

Conclusion: ZENN为复杂异构数据的机器学习提供了一种新方法，展示了其在科学问题中的广泛适用性。
...
Method: 提出Chisme协议套件，包含同步DFL（Chisme-DFL）和异步GL（Chisme-GL）变体，利用数据相似性启发式优化模型聚合与合并机制。

Result: Chisme方法在异构数据分布和不同网络条件下优于标准方法，资源需求更低且性能更优。

Conclusion: Chisme为边缘网络中的分布式学习提供了高效且适应性强的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有联邦学习中的数据重建攻击存在局限性，如依赖数据分布假设或在小批量数据上效率低下。本文旨在解决这些问题。

Method: 利用全连接层的新几何视角，设计恶意模型参数，无需客户端数据先验知识即可完美恢复大规模数据批次。

Result: 实验表明，该方法在图像和表格数据上均优于现有方法，能完美恢复比现有技术大两个数量级的数据批次。

Conclusion: 该方法显著提升了数据重建攻击的能力，揭示了联邦学习中更严重的隐私风险。
...
TLDR: 论文研究了Transformer模型的两种学习模式（IWL和ICL）及其与进化生物学中遗传编码和表型可塑性的类比，探讨了环境可预测性对两种模式平衡的影响。


<details>
  <summary>Details</summary>
Motivation: 理解Transformer模型中IWL（权重内学习）和ICL（上下文学习）的交互机制，并从进化生物学的角度寻找类比。

Method: 通过实验操作环境可预测性（稳定性和线索可靠性），在回归和分类任务中系统研究其对IWL/ICL平衡的影响。

Result: 高环境稳定性显著偏好IWL，而高线索可靠性增强ICL效果；学习动态显示任务依赖的时序演化。

Conclusion: 可预测性是决定Transformer学习模式的关键因素，研究结果支持相对成本假说，为理解ICL和优化训练方法提供了新视角。
...
Method: 利用跨客户端注意力机制和特征表示重建误差计算异常分数，区分恶意与良性客户端。
...
Conclusion: 该方法对营销和广告技术领域具有广泛适用性，并分享了相关经验和见解。
...
Method: 结合Kernel-QuantTree直方图和EWMA统计，实现非参数监控，适用于任意平稳分布。

Result: 实验表明，KQT-EWMA能有效控制ARL0，且检测延迟优于或等同于同类方法。

Conclusion: KQT-EWMA是一种高效且实用的非参数变化检测算法。
...
<details>
  <summary>Details</summary>
Motivation: 大型模型的高昂训练和运营成本限制了非富裕个体和公司的参与，同时增加了AI产品的价格。现有剪枝方法计算量大且不环保。

Method: BINGO在训练过程中评估神经网络中每个权重的重要性，生成显著性分数，一次性剪枝不重要的权重。

Result: BINGO提供了一种计算量较小且能保持准确性的剪枝技术。

Conclusion: BINGO为AI发展提供了一种不依赖模型规模增长的可持续方案。
...
<details>
  <summary>Details</summary>
Motivation: 多模态模型虽推动了AI发展，但也继承了多种模态的脆弱性，对抗性威胁加剧。现有研究缺乏针对实践者的攻击类型概述。

Method: 通过调查针对文本、图像、视频和音频四种模态的对抗性攻击，分析威胁态势及其演变。

Result: 提供了多模态对抗性攻击的全面总结，揭示了威胁的演变趋势。

Conclusion: 填补了多模态领域对抗性攻击研究的空白，为实践者提供了预防措施的依据。
...
Method: 使用多臂老虎机（MAB）任务，结合可解释的选择模型，比较LLMs、人类和MAB算法的E&E策略，并研究推理能力对LLM决策的影响。
...
<details>
  <summary>Details</summary>
Motivation: 随着健康食品需求增长，农产品价格预测变得重要。哈斯鳄梨价格受季节、地区和天气等多因素影响，传统模型难以处理非线性动态数据。

Method: 使用TCN提取时序特征，MLP处理非线性交互，注意力机制动态加权特征。数据包括2015-2018年美国5万条哈斯鳄梨销售记录，经过预处理后训练模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有异常检测方法多为离线或需周期性重新训练，难以适应流数据场景。

Method: 提出Online-iForest方法，动态跟踪数据生成过程的变化。

Result: 在真实数据集上验证，性能与离线方法相当，效率优于所有竞争对手。

Conclusion: Online-iForest是流数据异常检测的高效解决方案，适用于网络安全、欺诈检测等场景。
...
Conclusion: 提出的方法有效解决了得分函数奇异性问题，提升了流形约束数据的生成质量。
...
TLDR: 本文提出了一种交互式持续学习范式RiCL，通过实时人类反馈动态学习新技能，同时保留旧知识，解决了传统持续学习的两大限制。


<details>
  <summary>Details</summary>
Motivation: 传统持续学习依赖静态数据集和干净标签，而实际应用中数据是动态且标签可能噪声较大。
...
<details>
  <summary>Details</summary>
Motivation: 传统统计方法和机器学习难以捕捉事故复杂因素，需更高效的分析工具。

Method: 基于226项研究构建数据集，微调Llama3 8B模型，通过零样本分类识别事故原因。

Result: 模型有效识别酒驾、超速等主因，与专家问卷结果一致率达88.89%。

Conclusion: LLM为事故分析提供新思路，助力制定更有效的交通安全措施。
...
<details>
  <summary>Details</summary>
Motivation: 解决长期持续学习（Long-CL）中因任务数量庞大导致的灾难性遗忘问题，并评估现有方法在此场景下的表现。

Method: 提出任务核心记忆管理策略和长期记忆巩固机制，构建多模态和文本基准（MMLongCL-Bench和TextLongCL-Bench）。

Result: Long-CL在两个基准上的性能分别比现有最优方法提高7.4%和6.5% AP。

Conclusion: Long-CL框架通过模仿人类记忆机制，显著提升了长期持续学习的性能，为未来研究提供了新方向。
...
Method: TransPL通过向量量化（VQ）生成时间序列片段的代码，构建源域的类和通道级代码转移矩阵，并利用贝叶斯规则进行目标域适应。

Result: 在四个时间序列UDA基准测试中，TransPL显著优于现有伪标签方法（准确率提升6.1%，F1分数提升4.9%）。
...
<details>
  <summary>Details</summary>
Motivation: 联邦强化学习中，如何在保护隐私的同时提升性能是一个关键问题。

Method: 提出FedRAG框架，学习每个客户端的实用状态投影函数，并在中央服务器聚合参数。

Result: 实验表明，FedRAG在不共享敏感信息的情况下提升了性能。

Conclusion: FedRAG为联邦强化学习提供了一种高效且隐私保护的解决方案。
...
Method: 研究采用数据预处理、模型训练和评估三个步骤，使用了逻辑回归、K近邻和随机森林三种分类器，并通过GridSearchCV和RandomizedSearchCV进行超参数调优。

Result: 随机森林分类器表现最优，准确率为91%，F1分数为0.89。其他评估指标（如精确率、召回率和混淆矩阵）显示模型在各类别间表现均衡。

Conclusion: 该模型在心脏病预测中表现出色，具有临床决策辅助潜力。但数据集规模和泛化性存在局限，未来需更大规模、更多样化的数据验证。
...
TLDR: 提出了一种面向6G的多模态通用模型AI2MMUM，通过LLM骨干和任务指令灵活执行多种物理层任务，性能优异。
...
Method: 采用LLM骨干增强上下文理解，结合微调引入领域知识，任务指令包含固定关键词和可学习前缀提示，使用冻结的无线电模态编码器和适配器层。

Result: 在WAIR-D和DeepMIMO数据集上，AI2MMUM在五项代表性任务中达到SOTA性能。

Conclusion: AI2MMUM展示了在6G无线系统中处理多模态任务的潜力，为未来研究提供了新方向。
...
TLDR: 论文研究了分布鲁棒（DR）平均奖励强化学习问题，提出了两种算法，分别通过DR折扣MDP和锚定状态方法实现近最优样本复杂度。


<details>
  <summary>Details</summary>
Motivation: 实际应用中（如机器人、运筹学和医疗保健）需要稳定的长期性能，因此研究DR平均奖励强化学习问题。

Method: 提出两种算法：1）将问题简化为DR折扣MDP；2）引入锚定状态稳定控制转移核。假设名义MDP均匀遍历，算法在KL和f_k-散度不确定性集下达到样本复杂度。

Result: 两种算法在不确定性半径足够小时，达到样本复杂度Õ(|S||A|t_mix²ε⁻²)，首次为DR平均奖励强化学习提供有限样本收敛保证。

Conclusion: 通过数值实验验证了算法的收敛速率，为DR平均奖励强化学习提供了理论支持。
...
TLDR: 论文介绍了ImagineBench，首个用于评估结合真实数据和LLM生成合成数据的离线强化学习算法的基准测试。


<details>
  <summary>Details</summary>
Motivation: 解决强化学习依赖大量真实交互数据的局限性，通过LLM生成合成数据，但缺乏标准基准测试阻碍进展。

Method: 提出ImagineBench基准，包含真实和LLM生成的合成数据，覆盖多领域任务和复杂语言指令。

Result: 现有离线强化学习算法在未见任务上表现不佳（35.44%成功率），需改进以更好利用LLM生成数据。

Conclusion: 需算法改进以优化LLM生成数据的利用，并探索快速在线适应、持续学习及多模态任务扩展。
...
TLDR: 量子-经典混合机器学习（QHML）模型在小数据集上表现优异，但在数据编码上敏感。提出一种基于修正梯度tanh的归一化方法，显著提升QHML在抗癌药物响应预测中的性能。


<details>
  <summary>Details</summary>
Motivation: 抗癌药物响应预测中样本量通常较小，QHML模型因其高性能和泛化能力具有优势，但对数据编码敏感，需解决稳定性问题。

Method: 提出基于修正梯度tanh的归一化方法，优化神经网络输出，避免极端值集中。

Result: 在基因表达和药物响应数据集上，QHML模型性能优于经典深度学习模型。

Conclusion: 该方法为生物医学数据分析开辟了新途径，展示了量子计算机的潜力。
...
TLDR: 论文提出了一种基于逻辑门（AND、OR、ADDER）的电路发现框架，通过结合噪声和去噪干预，确保电路的完整性和忠实性，并验证了其有效性。
...
Method: 引入三种逻辑门，将电路分解为这些门的组合，并提出结合噪声和去噪干预的框架，以完全识别逻辑门。

Result: 实验验证了框架在恢复电路的忠实性、完整性和稀疏性方面的能力，并揭示了逻辑门的基本特性及其在语言模型中的行为。

Conclusion: 提出的框架能有效解决电路完整性问题，为机制解释性研究提供了新工具。
...
TLDR: 论文提出了一种新的非示例持续图学习方法IPAL，通过原型对比学习和拓扑集成高斯原型解决特征漂移问题，并在多个数据集上验证了其优越性。


<details>
  <summary>Details</summary>
Motivation: 图神经网络（GNN）在持续学习中存在灾难性遗忘问题，现有方法如重放技术和非示例方法各有局限性，需解决特征漂移和隐私问题。

Method: 提出Instance-Prototype Affinity Learning (IPAL)，结合原型对比学习（PCL）和拓扑集成高斯原型（TIGP），并引入决策边界感知（DBP）机制。

Result: 在四个节点分类基准数据集上，IPAL优于现有方法，实现了塑性和稳定性的更好平衡。

Conclusion: IPAL通过结合结构信息和原型学习，有效缓解了持续学习中的特征漂移问题，提升了模型性能。
...
TLDR: 提出了一种结合梯度提升模型和可解释AI技术的欺诈检测框架，实现了高准确性和透明性。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习模型在追求预测准确性的同时牺牲了透明性和可解释性，导致难以满足监管要求和获得利益相关者信任。
...
Conclusion: 高准确性与透明可解释性可结合，为金融欺诈检测提供更道德、可信的解决方案。
...
TLDR: 论文提出了一种自适应的多任务蒸馏方法，动态调整教师模型的知识传递量，并设计了知识轨迹以避免知识遗忘，显著提升了深度估计和场景分割的统一建模效果。


<details>
  <summary>Details</summary>
Motivation: 深度估计和场景分割在智能交通系统中至关重要，联合建模可减少存储和训练成本。现有方法静态传递知识，无法动态适应学生学习能力。

Method: 提出自适应的蒸馏方法动态调整知识传递量，并设计知识轨迹记录关键信息，通过轨迹蒸馏损失指导学生高效学习。

Result: 在Cityscapes和NYU-v2等数据集上，方法明显优于现有技术。

Conclusion: 自适应蒸馏和知识轨迹设计有效提升了多任务联合建模的性能，代码已开源。
...
TLDR: 论文提出了一种名为ChronoSteer的多模态时间序列预测模型，结合了大型语言模型（LLM）和时间序列基础模型（TSFM）的优势，通过文本修订指令指导预测，显著提升了预测精度。
...
Method: 提出解耦框架：LLM将文本事件转化为修订指令，指导TSFM输出。引入ChronoSteer模型，并通过基于合成数据的两阶段训练策略解决跨模态数据稀缺问题。
...
TLDR: 本文提出了一种名为MiCo的分层语言代理框架，利用大语言模型（LLM）设计启发式方法，解决云服务中的动态多维装箱问题（ODMBP），并在大规模场景中表现出色。
...
Result: 在超过10,000个虚拟机的场景中，MiCo实现了96.9%的竞争比，且在非平稳请求流和多样化配置下表现优异。
...
TLDR: 提出了一种联合训练策略，通过将XGBoost模型生成的合成任务作为辅助任务，提升图Transformer在多任务分子属性预测中的性能，无需特征注入或预训练。


<details>
  <summary>Details</summary>
Motivation: 解决将规则模型（如随机森林）注入可微分神经网络框架的挑战，并提升多任务分子属性预测的性能。

Method: 联合训练一个图Transformer神经网络，结合稀疏多任务分子属性实验目标和XGBoost模型生成的合成目标。

Result: 在19个分子属性预测任务中均表现显著提升，其中16个任务优于单任务XGBoost模型。

Conclusion: 合成任务增强是一种有效方法，可提升神经网络在多任务分子属性预测中的性能，无需额外特征或预训练。
...
TLDR: 本文提出了一种通过提升本地模型的适应性来优化联邦学习中全局模型性能的方法。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中，由于客户端数据分布不均和数据隐私问题，训练出高性能的全局模型具有挑战性。
...
TLDR: 本文提出了一种名为FedAPC的新型联邦学习框架，通过原型增强解决领域异质性导致的全局模型收敛问题。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在隐私敏感应用中受欢迎，但领域异质性阻碍了全局模型的收敛。

Method: FedAPC利用增强数据的均值特征生成原型，通过对比学习增强特征多样性和模型鲁棒性。
...
TLDR: 论文研究了多智能体多臂老虎机问题中的最佳臂识别问题，提出了两种算法（Cl-BAI和BAI-Cl），并验证了其高效性和准确性。
...
Result: 两种算法均具有δ-PC保证，样本复杂度有界，且在小M情况下BAI-Cl变体达到极小极大最优。实验验证了算法在样本和通信效率上的优势。

Conclusion: 提出的算法在多智能体多臂老虎机问题中表现优异，尤其在M远小于N时效率显著。
...
TLDR: 论文提出QuXAI框架，基于Q-MEDLEY解释器，用于提升混合量子-经典机器学习模型的透明度和可靠性。


<details>
  <summary>Details</summary>
Motivation: 混合量子-经典机器学习模型（HQML）的复杂性导致黑箱行为，缺乏透明度和可靠性，当前XAI方法在此领域的研究尚不成熟。

Method: 引入QuXAI框架，结合量子特征映射和Q-MEDLEY解释器，保留量子变换阶段并可视化特征重要性。

Result: Q-MEDLEY能有效识别HQML模型中的关键经典特征并分离噪声，性能优于传统XAI方法。

Conclusion: QuXAI框架显著提升了HQML模型的解释性和可靠性，为量子增强AI技术的安全应用提供了支持。
...
TLDR: Alinear是一种超轻量级时间序列预测模型，仅需k级参数即可实现竞争性性能，挑战了模型规模扩展的必要性。


<details>
  <summary>Details</summary>
Motivation: 质疑时间序列预测中模型规模扩展的适用性，探索更高效的轻量级解决方案。

Method: 提出Alinear模型，结合了水平感知自适应分解机制和渐进频率衰减策略，避免注意力机制的计算开销。
...
TLDR: 论文提出了一种通过合成SEM图像解决半导体制造中EUV光刻缺陷检测数据不足问题的方法，并验证了YOLOv8在检测小缺陷上的优越性能。


<details>
  <summary>Details</summary>
Motivation: 由于EUV光刻中缺陷极小且缺乏标注数据，阻碍了深度学习模型在生产线中的应用，因此需要解决数据不足问题。

Method: 通过人工生成带有已知缺陷分布的SEM图像并自动标注，利用YOLOv8、EfficientNet和SSD等模型进行缺陷检测性能比较。

Result: YOLOv8表现最佳，平均精度达96%，能检测比间距更小的缺陷，并在真实SEM数据中检测到84.6%的桥接缺陷和78.3%的断裂缺陷。
...
### [69] [ComplexFormer: Disruptively Advancing Transformer Inference Ability via Head-Specific Complex Vector Attention](https://arxiv.org/abs/2505.10222)
*Jintian Shao,Hongyi Huang,Jiayi Wu,Beiwen Zhang,ZhiYu Wu,You Shan,MingKai Zheng*

Main category: cs.LG

TLDR: ComplexFormer提出了一种新的复杂多头注意力机制（CMHA），通过将语义和位置信息统一在复数平面中建模，提升了Transformer模型的表达能力和性能。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer模型在处理位置信息和语义信息时存在局限性，无法灵活地为每个注意力头分配不同的位置调整策略。

Method: ComplexFormer引入了CMHA，包括每头的欧拉变换和自适应差分旋转机制，将查询和键投影为复数向量，并在复数子空间中进行操作。
...
### [70] [A multi-head deep fusion model for recognition of cattle foraging events using sound and movement signals](https://arxiv.org/abs/2505.10198)
...
TLDR: 该论文提出了一种基于声学和惯性信号融合的深度神经网络模型，用于自动识别牛的进食行为，提高了监测精度。


<details>
  <summary>Details</summary>
Motivation: 通过自动识别牛的进食行为，可以改善饮食配方、早期发现代谢问题和动物不适症状，从而提高牧场管理效率。

Method: 采用卷积、循环和密集层的深度神经网络，融合声学和惯性信号，自动提取特征，并通过不同层次的信息融合进行比较。

Result: 提出的模型F1分数为0.802，比现有方法提高了14%，特征级融合表现最佳。

Conclusion: 多传感器信号融合的深度神经网络显著提升了牛进食行为监测的精度，为牧场管理提供了有效工具。
...
### [71] [Informed Forecasting: Leveraging Auxiliary Knowledge to Boost LLM Performance on Time Series Forecasting](https://arxiv.org/abs/2505.10213)
*Mohammadmahdi Ghasemloo,Alireza Moradi*

Main category: cs.LG

TLDR: 提出了一种跨领域知识转移框架，用于提升大语言模型（LLMs）在时间序列预测中的表现，显著优于无辅助信息的基线方法。
...
Method: 通过系统性地将结构化时间信息注入LLMs，提升其预测准确性，并在真实数据集上与无辅助信息的基线进行比较。

Result: 实验表明，知识引导的预测方法在预测准确性和泛化能力上显著优于基线方法。
...
### [72] [Superposition Yields Robust Neural Scaling](https://arxiv.org/abs/2505.10465)
*Yizhou liu,Ziming Liu,Jeff Gore*

Main category: cs.LG

TLDR: 论文研究了大型语言模型（LLM）性能随模型规模增长的规律，发现表征叠加是神经缩放定律的关键机制。


<details>
  <summary>Details</summary>
Motivation: 探索神经缩放定律（模型性能随规模增长）的起源，以理解为何更大的模型表现更好。

Method: 基于两个经验原则构建玩具模型，分析损失随模型规模的变化，并在开源LLM家族中验证。

Result: 发现弱叠加时损失与特征频率相关，强叠加时损失与模型维度成反比，且开源LLM符合强叠加预测。

Conclusion: 表征叠加是神经缩放定律的核心机制，未来可基于此优化训练策略和模型架构。
...
### [73] [SpecOffload: Unlocking Latent GPU Capacity for LLM Inference on Resource-Constrained Devices](https://arxiv.org/abs/2505.10259)
*Xiangwen Zhuge,Xu Shen,Zeyu Wang,Fan Dang,Xuan Ding,Danyang Li,Yahui Han,Tianxiang Hao,Zheng Yang*

Main category: cs.LG

TLDR: SpecOffload是一种高效推理引擎，通过将推测解码嵌入卸载技术，提升资源受限设备上LLM推理的性能。


<details>
  <summary>Details</summary>
Motivation: 在资源受限设备上进行高效LLM推理面临计算和内存利用的挑战，现有系统因卸载模型权重到CPU内存导致GPU核心利用率低和内存性能影响小。
...
Conclusion: SpecOffload通过推测解码与卸载技术的结合，显著提升了LLM推理的效率，且成本几乎为零。
...
### [74] [Parallel Scaling Law for Language Models](https://arxiv.org/abs/2505.10475)
*Mouxiang Chen,Binyuan Hui,Zeyu Cui,Jiaxi Yang,Dayiheng Liu,Jianling Sun,Junyang Lin,Zhongxin Liu*

Main category: cs.LG

TLDR: 论文提出了一种新的并行计算扩展方法（ParScale），通过并行处理输入和动态聚合输出，显著提升了推理效率，同时减少了内存和延迟开销。
...
Method: 引入并行扩展（ParScale），通过并行执行模型前向传递和动态聚合输出，复用现有参数，适用于任何模型结构或任务。

Result: 实验表明，并行扩展在性能提升的同时，内存和延迟开销显著低于参数扩展（22倍内存减少和6倍延迟减少）。
...
### [75] [Electric Bus Charging Schedules Relying on Real Data-Driven Targets Based on Hierarchical Deep Reinforcement Learning](https://arxiv.org/abs/2505.10262)
*Jiaju Qi,Lei Lei,Thorsteinn Jonsson,Lajos Hanzo*

Main category: cs.LG

TLDR: 论文提出了一种基于分层深度强化学习（HDRL）的电动公交车充电调度方法，通过将问题分解为高层和低层决策，分别优化充电目标和充电功率，以最小化成本。
...
Method: 采用分层双深度Q网络（HDDQN）和事后经验回放（HER）算法，将原始MDP分解为高层SMDP和低层MDP，分两阶段学习。

Result: 实验证明，叠加最优高层和低层策略的扁平策略性能与原始MDP最优策略相当。

Conclusion: 所提算法能有效优化充电调度，降低充电成本。
...
### [76] [RouteNator: A Router-Based Multi-Modal Architecture for Generating Synthetic Training Data for Function Calling LLMs](https://arxiv.org/abs/2505.10495)
*Vibha Belavadi,Tushar Vatsa,Dewang Sultania,Suhas Suresha,Ishita Verma,Cheng Chen,Tracy Holloway King,Michael Friedrich*

Main category: cs.LG

TLDR: 论文提出了一种基于路由器的架构，用于生成高质量合成数据以微调大型语言模型（LLMs），解决了传统方法在多样性和复杂性上的不足，显著提升了功能分类和API参数选择的准确性。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏真实用户交互数据和隐私限制，传统合成数据生成方法无法模拟真实数据分布，导致LLM微调效果不佳。

Method: 采用基于路由器的架构，结合领域资源（如内容元数据和知识图谱）及文本到文本、视觉到文本的语言模型，生成高质量合成数据。

Result: 在真实用户查询上的评估显示，功能分类和API参数选择的准确性显著提升，模型性能优于传统方法。

Conclusion: 该架构为功能调用任务设定了新基准，证明了合成数据生成方法的有效性。
...
### [77] [MASSV: Multimodal Adaptation and Self-Data Distillation for Speculative Decoding of Vision-Language Models](https://arxiv.org/abs/2505.10526)
*Mugilan Ganesan,Shane Segal,Ankur Aggarwal,Nish Sinnadurai,Sean Lie,Vithursan Thangarasa*

Main category: cs.LG

TLDR: MASSV提出了一种通过两阶段方法将小型语言模型转化为高效多模态草稿模型的技术，显著加速了视觉语言模型的推理速度。


<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型（VLMs）在推测解码中面临的两个主要挑战：小型语言模型无法处理视觉输入，且其预测与考虑视觉上下文的VLM目标模型不匹配。

Method: MASSV通过轻量级可训练投影器将目标VLM的视觉编码器连接到草稿模型，并利用目标VLM生成的响应进行自蒸馏视觉指令调优。

Result: 实验表明，MASSV在视觉任务中使接受长度增加30%，推理速度提升1.46倍。

Conclusion: MASSV为加速当前和未来的VLMs提供了一种可扩展且架构兼容的方法。
...
### [78] [RainPro-8: An Efficient Deep Learning Model to Estimate Rainfall Probabilities Over 8 Hours](https://arxiv.org/abs/2505.10271)
*Rafael Pablos Sarabia,Joachim Nyborg,Morten Birk,Jeppe Liborius Sjørup,Anders Lillevang Vesterholt,Ira Assent*

Main category: cs.LG

TLDR: 提出一种深度学习模型，用于欧洲8小时高分辨率降水概率预报，结合雷达、卫星和数值天气预报数据，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决雷达深度学习模型预报时间短的问题，提升降水预报的准确性和不确定性量化。

Method: 整合雷达、卫星和数值天气预报数据，设计紧凑架构，捕捉长程相互作用。

Result: 模型在实验中优于现有数值天气预报系统和深度学习模型，实现高精度预报。
...
### [79] [Spike-timing-dependent Hebbian learning as noisy gradient descent](https://arxiv.org/abs/2505.10272)
*Niklas Dexheimer,Sascha Gaudlitz,Johannes Schmidt-Hieber*

Main category: cs.LG

TLDR: 论文将Hebbian突触可塑性与噪声梯度下降联系起来，证明该学习规则能识别最高活动的前突触神经元，并发现其与噪声镜像下降的内在联系。
...
### [80] [Optimizing Electric Bus Charging Scheduling with Uncertainties Using Hierarchical Deep Reinforcement Learning](https://arxiv.org/abs/2505.10296)
*Jiaju Qi,Lei Lei,Thorsteinn Jonsson,Dusit Niyato*

Main category: cs.LG

TLDR: 本文提出了一种分层深度强化学习（HDRL）方法DAC-MAPPO-E，用于优化电动公交车（EB）的充电调度，解决了多时间尺度决策和大规模车队的可扩展性问题。


<details>
  <summary>Details</summary>
Motivation: 电动公交车的普及对可持续发展至关重要，但其充电调度面临不确定性（如行驶时间、能耗和电价波动）和复杂性问题，需要高效且可扩展的解决方案。

Method: 通过将原始马尔可夫决策过程（MDP）重构为两个增强的MDP，并引入DAC-MAPPO-E算法，结合注意力机制和MAPPO算法，实现多时间尺度决策和可扩展性。

Result: 实验表明，DAC-MAPPO-E在优化大规模EB车队充电调度方面表现出色，计算复杂度低且收敛速度快。

Conclusion: DAC-MAPPO-E是一种高效且可扩展的解决方案，适用于电动公交车充电调度的优化。
...
TLDR: 提出了一种新的负度量学习增强的图对比学习方法（NML-GCL），通过可学习的负度量网络（NMN）区分假负样本，并采用联合训练方案优化模型。


<details>
  <summary>Details</summary>
Motivation: 解决图对比学习中假负样本问题，现有方法依赖先验知识导致性能不佳。

Method: 使用负度量网络构建负度量空间，通过双层次优化目标联合训练编码器和负度量网络。

Result: 理论分析和广泛实验验证了方法的优越性。

Conclusion: NML-GCL有效解决了假负样本问题，提升了图对比学习性能。
...
TLDR: 本文提出了一种改进的异步去中心化随机梯度下降（ADSGD）模型，解决了计算速度和通信延迟的异质性问题，并在实际假设下证明了其收敛性。
...
Method: 通过分析异步随机块坐标下降（ASBCD）作为工具，提出ADSGD模型，并证明其在计算延迟无关步长下的收敛性。
...
<details>
  <summary>Details</summary>
Motivation: AI在下一代无线网络中是核心，但特征分布变化可能导致模型性能下降，需要一种方法来检测和应对这种变化。

Method: ALERT包括表示学习、统计测试和效用评估三个组件，分别使用MLP、Kolmogorov-Smirnov和Population Stability Index测试以及新设计的效用评估函数。
...
TLDR: 论文探讨了如何在动态环境中实现深度强化学习（RL）的高效在线适应，提出了优先探索和选择性知识保留的关键能力。


<details>
  <summary>Details</summary>
Motivation: 现实世界的决策系统需要在动态环境中运行，传统RL方法难以适应环境变化，容易遗忘先验知识。

Method: 提出两种关键能力：优先探索和采样策略，以及通过结构化表示选择性保留先验知识。

Result: 展示了这两种能力能帮助RL代理在部署时高效适应新环境变化。

Conclusion: 高效在线适应需要优先探索和选择性知识保留，以解决动态环境中的RL挑战。
...
TLDR: 论文研究了随机分类器集合中集体行为的涌现现象，通过Gibbs测度加权，发现存在一个最优温度参数使分类效果最佳，且该参数具有普适性。
...
Method: 引入理论模型，采用Gibbs测度加权分类器集合，以分类损失作为能量函数，寻找最优温度参数。

Result: 在特定条件下，最优温度参数与教师分类器和分类器数量无关，表现出普适性；MNIST实验验证了现象的有效性。

Conclusion: 研究揭示了随机分类器集合中的自组织现象，为理解集体行为的涌现提供了理论支持。
...
TLDR: 本文介绍了离散变分自编码器（Discrete VAEs）的实用教程，重点讨论了使用分类分布作为潜在空间的VAEs。


<details>
  <summary>Details</summary>
Motivation: 离散潜在空间可能更适合某些数据类型（如文本），因此需要系统性地介绍离散VAEs的原理和实现。

Method: 通过数学推导从基本原理出发，逐步构建离散VAEs的模型，并提供具体的训练方法和实现示例。

Result: 提供了一个实用的训练框架和开源实现（https://github.com/alanjeffares/discreteVAE）。

Conclusion: 离散VAEs是处理特定数据模态的有效工具，本文为读者提供了从理论到实践的完整指南。
...
TLDR: SMTOs与均匀加权任务在多任务学习中的表现对比研究，发现两者在某些情况下表现相似。


<details>
  <summary>Details</summary>
Motivation: 探讨SMTOs是否真的优于均匀加权任务，以及其性能差异的原因。

Method: 通过广泛的实证评估，比较SMTOs和均匀加权任务在复杂多任务问题中的表现。

Result: SMTOs表现良好，但均匀加权任务也能达到竞争性性能，且在某些情况下两者表现相似。

Conclusion: SMTOs和均匀加权任务在多任务学习中各有优势，具体表现取决于问题背景。
...
### [88] [PIF: Anomaly detection via preference embedding](https://arxiv.org/abs/2505.10441)
*Filippo Leveni,Luca Magri,Giacomo Boracchi,Cesare Alippi*

Main category: cs.LG

TLDR: 提出了一种名为PIF的新型异常检测方法，结合了自适应隔离方法和偏好嵌入的优势，通过高维空间嵌入和PI-Forest树方法计算异常分数。
...
Method: 提出PIF方法，结合自适应隔离和偏好嵌入，使用PI-Forest树在高维空间中计算异常分数。

Result: 在合成和真实数据集上的实验表明，PIF优于现有技术，PI-Forest在测量任意距离和隔离偏好空间中的点方面表现更好。
...
### [89] [FactsR: A Safer Method for Producing High Quality Healthcare Documentation](https://arxiv.org/abs/2505.10360)
*Victor Petrén Bach Hansen,Lasse Krogsbøll,Jonas Lyngsø,Mathias Baltzersen,Andreas Motzfeldt,Kevin Pelgrims,Lars Maaløe*

Main category: cs.LG

TLDR: 提出了一种实时提取临床信息并递归生成最终笔记的方法FactsR，以提高准确性和简洁性。


<details>
  <summary>Details</summary>
Motivation: 现有AI医疗记录工具依赖一次性或少量提示生成笔记，易产生冗长、错误或误解，影响患者安全。

Method: 实时提取临床关键信息（Facts），并递归利用这些信息生成最终笔记。

Result: FactsR方法生成更准确、简洁的笔记，同时支持实时决策。

Conclusion: FactsR方法通过实时信息提取和递归生成，提升了医疗笔记的质量和实用性。
...
### [90] [SEAL: Searching Expandable Architectures for Incremental Learning](https://arxiv.org/abs/2505.10457)
*Matteo Gambella,Vicente Javier Castro Solar,Manuel Roveri*

Main category: cs.LG

TLDR: SEAL是一个基于NAS的框架，用于数据增量学习，动态调整模型结构以减少遗忘并提升准确性，同时保持较小的模型规模。


<details>
  <summary>Details</summary>
Motivation: 解决增量学习中平衡新任务学习和保留旧知识的挑战，避免现有NAS方法因频繁扩展模型而导致的资源浪费。

Method: 通过容量估计指标动态扩展模型结构，结合交叉蒸馏训练保持稳定性，NAS组件联合搜索架构和最优扩展策略。

Result: 实验表明，SEAL在多个基准测试中有效减少遗忘、提升准确性，并保持较小的模型规模。
...
### [91] [Schreier-Coset Graph Propagation](https://arxiv.org/abs/2505.10392)
*Aryan Mishra,Lizhen Lin*

Main category: cs.LG

TLDR: 提出了一种名为SCGP的图神经网络增强方法，通过Schreier-coset嵌入在不改变输入图拓扑的情况下提升信息传递能力，解决了现有方法在可扩展性和内存占用上的问题。


<details>
  <summary>Details</summary>
Motivation: 现有图神经网络（GNNs）在信息传递中存在过压缩问题，且现有解决方案（如图重连和Cayley图）存在可扩展性瓶颈。

Method: 引入Schreier-Coset Graph Propagation（SCGP），通过Schreier-coset嵌入增强节点特征，保持输入图拓扑不变。

Result: SCGP在标准节点和图分类任务中表现优于或与现有方法相当，尤其在处理分层和模块化图结构时表现突出。

Conclusion: SCGP在提升性能的同时，降低了推理延迟和内存占用，适用于实时和资源受限的应用场景。
...
TLDR: AneuG是一种基于变分自编码器（VAE）的两阶段颅内动脉瘤（IA）网格生成器，能够生成具有特定形态学特征的IA形状，并考虑动脉瘤囊与母血管的关系。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏大型IA图像数据集，现有形状生成方法难以捕捉真实IA特征且无法控制生成形状的形态学测量，因此需要一种更有效的生成模型。

Method: AneuG采用两阶段方法：第一阶段通过图谐波变形（GHD）编码和重建动脉瘤囊形状；第二阶段基于GHD生成母血管，并可进一步控制生成形状的临床相关形态学测量。

Result: AneuG能够生成更准确的IA形状，并支持特定形态学特征的定制，适用于临床研究和血流模拟。

Conclusion: AneuG为IA形状生成提供了一种高效且可控的方法，有助于临床研究和流体动力学分析。
...
TLDR: 论文探讨了人类学习的高效性是否源于多机制协同，并通过实验验证了多机制学习方法在数据效率上的优势。


<details>
  <summary>Details</summary>
Motivation: 现代神经网络依赖单一梯度下降机制，而人类学习通过多机制协同实现高效学习，研究旨在验证多机制方法是否能提升机器学习的数据效率。

Method: 通过在线辅导环境中的模拟实验，对比强化学习与三机制符号规则归纳方法，分析多机制对学习效率的影响。

Result: 多机制分解显著提升数据效率，接近人类学习水平，且其效果优于符号与亚符号学习的单一区分。
...
TLDR: 论文研究了基于梯度的参数模型优化与随机特征线性组合优化的关系，发现无分布学习的局限性。


<details>
  <summary>Details</summary>
Motivation: 探讨神经网络中梯度下降训练的无分布学习的局限性，揭示数据分布假设的重要性。

Method: 通过mini-batch随机梯度下降（bSGD）和随机特征组合的理论分析，引入平均概率维度复杂性（adc）框架。

Result: 证明目标函数可通过多项式大小的随机特征组合近似，adc与统计查询维度存在多项式关系。
...
TLDR: 论文提出了一种名为L2T的强化微调框架，旨在优化大型语言模型的推理效率，减少不必要的计算资源浪费。
...
Result: 理论分析和实验结果表明，L2T显著降低了计算复杂度并提高了推理效果和效率。
...
TLDR: 论文探讨了使用基于分数的扩散模型（如Diff、CorrDiff和LDM）进行云和降水的临近预报，发现CorrDiff方法表现最佳，优于传统U-Net和持续性预报。


<details>
  <summary>Details</summary>
Motivation: 传统数值天气预报在模拟云和降水时因需要次网格参数化而具有挑战性，早期机器学习方法又常产生模糊预报。因此，探索新的基于分数的扩散模型以提高预报精度。

Method: 研究了三种扩散模型：标准分数扩散模型（Diff）、残差校正扩散模型（CorrDiff）和潜在扩散模型（LDM），用于临近预报（0-3小时）云和降水。

Result: 扩散模型不仅能平流现有云层，还能生成和消散云层，包括对流启动。CorrDiff方法表现最佳，优于其他模型和传统方法。
...
TLDR: 论文提出了一种基于数据驱动的非线性动态识别方法，用于燃气涡轮发动机建模与控制，通过Koopman特征空间优化设计控制器，性能优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 传统燃气涡轮发动机建模方法依赖物理特性且需简化假设，数据驱动方法可弥补这些不足。

Method: 采用稀疏识别非线性动态技术估计转子动力学，通过Koopman特征空间优化构建模型，并设计全局非线性反馈控制器。

Result: Koopman控制器在参考跟踪和干扰抑制方面优于传统比例积分控制器和增益调度方法。

Conclusion: 基于Koopman特征空间的数据驱动方法为复杂非线性系统建模与控制提供了更优解决方案。
...
TLDR: NCDPO是一种新框架，通过将扩散策略重新表述为噪声条件确定性策略，解决了扩散策略在强化学习微调中的计算难题，显著提升了样本效率和性能。
...
TLDR: 论文提出了一种新的价值函数分解方法QFIX，通过简单的‘修复’层扩展了现有模型的表示能力，提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如VDN、QMIX）在表示IGM值类时能力有限，而QPLEX虽无限制但过于复杂。

Method: 提出QFIX，通过薄层修复扩展表示能力，并在多智能体框架中实现三种变体。

Result: 在SMACv2和Overcooked环境中，QFIX提升了性能，表现优于QPLEX，且模型更简单。

Conclusion: QFIX是一种简单有效的价值函数分解方法，优于现有技术。
...
<details>
  <summary>Details</summary>
Motivation: 现有XAI方法因架构和模态限制、缺乏灵活性和优化阶段而难以在实际应用中推广，PnPXAI旨在解决这些问题。

Method: PnPXAI以即插即用方式支持多种模型和数据模态，自动检测架构、推荐解释方法并优化超参数。

Result: 通过用户调查验证了PnPXAI的有效性，并在医学和金融等领域展示了其多功能性。

Conclusion: PnPXAI解决了现有XAI方法的局限性，为实际应用提供了更灵活和优化的解释框架。
...
<details>
  <summary>Details</summary>
Motivation: 开发针对缺乏结构或功能数据的新靶点的生物活性分子是药物发现中的主要挑战。药效团建模提供了一种替代方法。
...
Conclusion: PharmaDiff通过结合药效团建模和3D生成技术，为理性药物设计提供了强大而灵活的框架。
...
TLDR: 提出了一种基于可穿戴设备和实时环境数据的个性化健康响应预测工作流，利用对抗自编码器神经网络模型捕捉非线性健康信号。


<details>
  <summary>Details</summary>
Motivation: 空气污染和极端天气事件加剧健康风险，结合个人传感和AI技术改善健康监测。

Method: 整合可穿戴设备生理数据与实时环境暴露数据，训练对抗自编码器模型预测健康响应。

Result: 模型能准确重构时间依赖性健康信号并捕捉非线性污染响应，迁移学习提升泛化能力。

Conclusion: 该方法为个性化健康监测提供了有效工具，适用于真实世界用户数据。
...
TLDR: 论文提出了一种名为“神经热力学定律”（NTL）的新框架，用于理解大语言模型（LLM）的训练动态，揭示了热力学原理在其中的自然涌现，并提供了学习率调度的实用指南。
...
Method: 提出NTL框架，基于河流谷损失景观假设，展示热力学量（如温度、熵）和经典热力学原理在LLM训练中的自然出现。

Result: 理论证明了热力学原理在LLM训练中的适用性，并提供了学习率调度的实用建议。

Conclusion: NTL框架为LLM训练提供了新的科学视角和实用指导。
...
TLDR: 论文提出了一种结合图卷积网络（GNN）和LSTM的方法，用于语言建模中的上下文嵌入任务，并在资源有限的情况下验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型需要巨额计算资源和数据，本文旨在探索一种资源高效的上下文嵌入方法。
...
TLDR: 论文提出了一种名为DRA的方法，通过引入语义多样性到奖励计算中，解决了GRPO在语言模型后训练中的多样性-质量不一致问题。
...
Conclusion: DRA方法有效提升了语言模型后训练的多样性探索和高质量样本利用，实现了最优性能。
...
TLDR: LLM（Claude Sonnet 3.5）在实时对话测试中比激励人类更具说服力，无论是引导正确还是错误答案。


<details>
  <summary>Details</summary>
Motivation: 比较前沿LLM与激励人类在说服能力上的差异，探讨AI的说服潜力及其潜在影响。

Method: 通过预注册的大规模激励实验，让LLM和人类在在线测试中尝试说服参与者选择特定答案。

Result: LLM在说服效果上显著优于人类，既能提高正确率（增加收益），也能降低正确率（减少收益）。
...
TLDR: 提出了双层系统提示优化问题，通过元学习框架优化系统提示，使其适用于多样化用户提示和未见任务。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注任务特定的用户提示优化，而忽略了通用的系统提示优化，限制了LLM的跨任务性能。

Method: 提出元学习框架，通过迭代优化系统提示和用户提示，确保二者协同作用。

Result: 在14个未见数据集上验证，优化后的系统提示能快速适应多样化用户提示和未见任务，提升性能。

Conclusion: 双层系统提示优化能显著提升LLM的通用性和适应性。
...
TLDR: VeriFact是一个新的事实性评估框架，旨在通过识别和解决不完整或缺失的事实来提高长文本生成的事实性评估准确性。同时，FactRBench是一个新的基准测试，专注于评估长文本响应的精确性和召回率。


<details>
  <summary>Details</summary>
Motivation: 现有的事实性评估方法难以捕捉复杂的句子间依赖关系和关键关系事实，导致评估结果不准确。

Method: 提出VeriFact框架，通过增强事实提取和解决不完整事实来改进评估；同时引入FactRBench基准测试，评估精确性和召回率。

Result: VeriFact显著提高了事实完整性和复杂关系的保留，FactRBench显示大模型在同一家族内表现更好，但高精确性不一定与高召回率相关。

Conclusion: VeriFact和FactRBench为长文本生成的事实性评估提供了更全面的方法，强调了精确性和召回率的重要性。
...
TLDR: 论文介绍了如何利用LLMs高效开发、测试和应用分类法分析非结构化数据，并通过迭代协作过程实现高编码一致性。


<details>
  <summary>Details</summary>
Motivation: 分析开放式文本耗时且易受偏见影响，LLMs为解决这一问题提供了潜力。

Method: 通过迭代协作过程，结合预定义和数据驱动的分类法，编写提示词并评估分类法。

Result: 展示了如何生成生活领域分类法，并通过修改和测试实现高编码一致性。
...
TLDR: 论文提出了TokenAdapt框架，通过模型无关的tokenizer移植方法和多词Supertokens预分词学习，解决固定tokenization方案的效率与性能问题。
...
Method: 引入TokenAdapt（混合启发式初始化新token嵌入）和Supertokens（增强压缩和减少碎片化）。

Result: TokenAdapt显著优于基线方法，Supertokens实现压缩增益，零样本困惑度结果表现优异。

Conclusion: TokenAdapt框架在保留语义的同时显著减少再训练需求，性能优于现有方法。
...
TLDR: 该研究利用NLP技术（特别是NER）自动从电子健康记录中提取肺癌和乳腺癌的关键临床信息，使用uQuery工具和RoBERTa模型，取得了较高的准确性。


<details>
  <summary>Details</summary>
Motivation: 手动提取临床报告信息耗时且易错，限制了医疗数据驱动方法的效率，NLP提供了一种自动化解决方案。

Method: 使用GMV的uQuery工具和基于RoBERTa的bsc-bio-ehr-en3模型，对200份乳腺癌和400份肺癌报告进行NER任务。

Result: 模型在识别常见实体（如MET和PAT）上表现良好，但对低频实体（如EVOL）仍有挑战。

Conclusion: NLP技术能有效提升临床数据提取的准确性和效率，尤其在肺癌和乳腺癌领域，但需进一步优化低频实体的识别。
...
TLDR: 研究发现LLM中存在通用真相方向，但该方向在不同对话格式中泛化能力有限，尤其是长对话。通过添加固定关键词可显著改善泛化能力。


<details>
  <summary>Details</summary>
Motivation: 探索LLM中真相方向在不同对话格式中的泛化能力，以提升其作为谎言检测工具的可靠性。

Method: 训练线性探针分析隐藏状态，测试其在短对话和长对话中的泛化能力，并提出添加固定关键词的解决方案。

Result: 短对话中真相方向泛化良好，长对话中表现较差。添加关键词后泛化能力显著提升。

Conclusion: LLM作为谎言检测工具面临泛化挑战，但通过简单调整可部分解决。
...
TLDR: KRISTEVA是首个评估大语言模型（LLMs）在文学细读能力上的基准测试，包含1331道选择题，测试LLMs在风格提取、上下文检索和多跳推理上的表现。结果显示，LLMs虽有一定能力，但仍落后于人类评估者。


<details>
  <summary>Details</summary>
Motivation: 填补LLMs在文学细读能力评估上的空白，因为现有基准测试（如MMLU）未涵盖文学领域。

Method: 设计KRISTEVA基准测试，包含三个逐步复杂的任务：风格特征提取、上下文信息检索和多跳推理。

Result: LLMs在细读任务上的准确率为49.7% - 69.7%，但在11项任务中有10项表现不及人类评估者。

Conclusion: LLMs在文学细读上具备一定能力，但与人类仍有差距，需进一步改进。
...
Method: 通过两部分评估框架，分别在参数化（仅依赖预训练知识）和非参数化（结合外部冲突数据集和新闻）设置下，预测冲突趋势和死亡人数。

Result: 研究发现LLMs在冲突预测中具有潜力，但结合外部结构化知识能显著提升性能。

Conclusion: LLMs可用于冲突预测，但需结合外部信息以弥补预训练知识的不足。
...
### [115] [Crossing Borders Without Crossing Boundaries: How Sociolinguistic Awareness Can Optimize User Engagement with Localized Spanish AI Models Across Hispanophone Countries](https://arxiv.org/abs/2505.09902)
...
<details>
  <summary>Details</summary>
Motivation: 研究西班牙语在拉丁美洲和西班牙的变体差异，以支持本地化AI模型的开发，促进包容性和用户增长。

Method: 通过社会文化和语言学分析，比较不同地区的西班牙语变体，提出五种子变体的本地化策略。

Result: 研究表明，本地化模型能有效减少社会语言差异，提升用户信任和AI模型的国际适应性。

Conclusion: 实施本地化语言模型有助于文化包容性、用户增长和国际战略的成功。
...
### [116] [From Trade-off to Synergy: A Versatile Symbiotic Watermarking Framework for Large Language Models](https://arxiv.org/abs/2505.09924)
*Yidan Wang,Yubing Ren,Yanan Cao,Binxing Fang*

Main category: cs.CL

TLDR: 论文提出了一种结合基于logits和基于采样的水印方案的混合框架，通过三种策略（串行、并行和混合）优化水印的检测性、鲁棒性、文本质量和安全性。实验表明该方法优于现有基线，达到SOTA性能。
...
Method: 提出了一种混合水印框架，结合基于logits和基于采样的方案，采用串行、并行和混合三种策略，并利用token熵和语义熵自适应嵌入水印。

Result: 实验验证了该方法在多个数据集和模型上的优越性，性能优于现有基线，达到SOTA水平。
...
### [117] [Rethinking Prompt Optimizers: From Prompt Merits to Optimization](https://arxiv.org/abs/2505.09930)
*Zixiao Zhu,Hanzhang Zhou,Zijian Feng,Tianjiao Li,Chua Jia Jim Deryl,Mak Lee Onn,Gee Wah Ng,Kezhi Mao*

Main category: cs.CL

TLDR: 论文提出了一种基于可解释设计的提示优化方法MePO，通过轻量级LLM生成优化的提示，避免了依赖高级LLM的局限性，提升了提示和响应质量。


<details>
  <summary>Details</summary>
Motivation: 现有提示优化方法依赖高级LLM（如GPT-4），但其生成的冗长提示可能不适用于轻量级推理模型，导致响应质量下降。

Method: 提出MePO方法，通过定义模型无关的提示质量指标，并基于轻量级LLM生成的偏好数据集训练优化器。

Result: 实验证明MePO在多种任务和模型类型中表现优异，具有可扩展性和鲁棒性。

Conclusion: MePO提供了一种高效、可解释且适用于实际部署的提示优化解决方案。
...
### [118] [Interim Report on Human-Guided Adaptive Hyperparameter Optimization with Multi-Fidelity Sprints](https://arxiv.org/abs/2505.09792)
*Michael Kamfonas*

Main category: cs.CL

TLDR: 本文通过分阶段超参数优化方法比较多任务自然语言模型变体，结合多阶段学习率调度和优化器参数分组，利用贝叶斯优化和元学习器调优阈值。


<details>
  <summary>Details</summary>
Motivation: 研究如何通过高效的超参数优化方法提升多任务自然语言模型的性能，并解决分类概率的阈值调优问题。

Method: 采用分阶段贝叶斯优化，结合多保真度、超参数空间剪枝和渐进减半策略，使用Optuna TPE采样器和Hyperband剪枝器，以及Scikit-Learn高斯过程最小化。

Result: 在Eberts和Ulges提出的2021年联合实体和关系提取模型变体上验证了方法的有效性。

Conclusion: 分阶段超参数优化结合元学习器调优阈值是一种高效的方法，适用于多任务自然语言模型。
...
### [119] [Personalizing Large Language Models using Retrieval Augmented Generation and Knowledge Graph](https://arxiv.org/abs/2505.09945)
*Deeksha Prahlad,Chanhee Lee,Dongha Kim,Hokeun Kim*

Main category: cs.CL

TLDR: 论文提出了一种基于知识图谱（KGs）的检索增强生成（RAG）方法，以解决大型语言模型（LLMs）在生成个性化响应时的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: LLMs在生成响应时容易因过拟合而产生错误或多余信息（幻觉），原因是缺乏及时、真实和个性化的输入数据。

Method: 通过结合知识图谱（KGs）的检索增强生成（RAG）方法，为LLMs提供结构化且持续更新的个性化数据（如日历数据）。

Result: 实验表明，该方法在理解个性化信息和生成准确响应方面显著优于基线LLMs，且响应时间略有减少。

Conclusion: 结合KGs的RAG方法能有效提升LLMs生成个性化响应的准确性和可靠性。
...
### [120] [DIF: A Framework for Benchmarking and Verifying Implicit Bias in LLMs](https://arxiv.org/abs/2505.10013)
*Lake Yin,Fan Huang*

Main category: cs.CL

TLDR: 论文提出了一种衡量大语言模型（LLM）隐性偏见的方法DIF（人口统计隐性公平性），通过评估现有逻辑和数学问题数据集与社会人口角色，验证了LLM行为中的隐性偏见及其与回答准确性的负相关关系。


<details>
  <summary>Details</summary>
Motivation: LLM的训练数据可能带来隐性偏见，这不仅是一个伦理问题，也暴露了LLM在适应额外信息方面的技术缺陷。目前缺乏衡量此类偏见的标准方法。

Method: 开发了DIF方法，通过结合社会人口角色评估LLM的逻辑和数学问题数据集，以量化隐性偏见。

Result: DIF方法统计验证了LLM行为中的隐性偏见，并发现回答准确性与隐性偏见呈负相关。

Conclusion: DIF为衡量LLM隐性偏见提供了标准化方法，揭示了偏见与模型性能的关系，为未来研究提供了方向。
...
### [121] [CAFE: Retrieval Head-based Coarse-to-Fine Information Seeking to Enhance Multi-Document QA Capability](https://arxiv.org/abs/2505.10063)
*Han Peng,Jinhao Jiang,Zican Dong,Wayne Xin Zhao,Lei Fang*

Main category: cs.CL

TLDR: CAFE是一种两阶段粗到细的方法，用于增强多文档问答能力，通过逐步消除背景和干扰文档的负面影响，使回答更依赖证据文档。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在长上下文输入中的检索和推理能力仍有限，现有方法在平衡检索精度和召回率方面存在挑战。

Method: CAFE采用两阶段方法：粗粒度过滤和细粒度引导，分别用于识别相关文档和聚焦最相关内容。

Result: 在多个基准测试中，CAFE优于基线方法，在Mistral模型上分别比SFT和RAG方法提高了22.1%和13.7%的SubEM分数。

Conclusion: CAFE通过两阶段方法显著提升了多文档问答的效果，解决了现有方法在检索和推理中的局限性。
...
### [122] [Dark LLMs: The Growing Threat of Unaligned AI Models](https://arxiv.org/abs/2505.10066)
*Michael Fire,Yitzhak Elbazis,Adi Wasenstein,Lior Rokach*

Main category: cs.CL

TLDR: 论文探讨了大型语言模型（LLMs）因训练数据问题导致的越狱攻击漏洞，揭示了一种通用攻击方法，并指出行业应对不足的风险。


<details>
  <summary>Details</summary>
Motivation: LLMs的广泛应用伴随安全隐患，尤其是越狱攻击的威胁，源于训练数据中的未过滤内容。

Method: 研究识别了一种通用越狱攻击方法，测试了多个先进模型并验证其漏洞。

Result: 发现许多LLMs仍易受攻击，行业应对措施不足，存在广泛滥用风险。

Conclusion: 需采取果断措施防止LLMs被滥用，避免危险知识的普及。
...
TLDR: 本文系统研究了非洲语言预训练模型（PLMs）的语言知识分布，发现针对非洲语言优化的PLMs比多语言PLMs更能编码目标语言的语言信息。


<details>
  <summary>Details</summary>
Motivation: 探讨非洲语言PLMs改进的原因，填补对非洲语言PLMs语言知识分布的研究空白。

Method: 训练分层探针分析六种非洲语言的语言特征分布，设计控制任务验证探针性能。

Result: 优化的PLMs编码更多目标语言信息；句法信息集中在中后层，语义信息分布全层；性能反映PLMs内部知识。

Conclusion: 研究揭示了非洲语言PLMs的成功机制，支持主动学习和多语言适应策略的有效性。
...
Method: 构建XRAG基准，基于新闻文章生成需要外部知识回答的问题，涵盖单语和多语检索场景，并提供相关性标注。

Result: 实验发现两个新挑战：单语检索中模型难以保证回答语言正确性；多语检索中推理跨语言信息是主要难点。

Conclusion: XRAG为研究LLM推理能力提供了有价值的基准，揭示了跨语言RAG中的关键挑战。
...
TLDR: S-MedQA是一个英语医学问答数据集，用于评估大型语言模型在细粒度临床专业中的表现。研究发现，专业数据训练不一定带来最佳性能，且临床相关术语的概率普遍增加，表明改进主要来自领域转换而非知识注入。


<details>
  <summary>Details</summary>
Motivation: 研究旨在验证知识注入假设在医学问答中的适用性，并探讨微调数据的作用。

Method: 使用S-MedQA数据集，分析不同专业数据训练对模型性能的影响，并观察临床术语概率变化。

Result: 1) 专业数据训练不一定提升该专业性能；2) 所有专业的临床术语概率均增加。
...
TLDR: 论文提出GE-Chat框架，通过知识图谱增强检索生成，提升LLM生成答案的可信度。


<details>
  <summary>Details</summary>
Motivation: LLM生成的答案可能存在错误或幻觉，用户需手动验证，导致信任问题。

Method: 结合知识图谱、检索增强生成、CoT逻辑生成、n-hop子图搜索和蕴含句子生成，实现精确证据检索。

Result: 方法提升了现有模型在自由上下文中识别准确证据的能力，增强了LLM结论的可信度。

Conclusion: GE-Chat为LLM生成答案提供了可靠的证据支持，有助于用户判断其可信度。
...
TLDR: 论文探讨了通过合成数据改进大型语言模型（LLMs）推理能力的持续预训练方法（Reasoning CPT），并在多领域展示了性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如监督微调和强化学习）在训练推理模型时局限于特定领域，限制了数据的广度和可扩展性。Reasoning CPT 通过合成数据模拟作者的思考过程，试图解决这一问题。

Method: 采用 Reasoning CPT 方法，使用 STEM 和法律领域的合成数据（包含隐藏思考过程）对 Gemma2-9B 模型进行训练，并在 MMLU 基准上与标准 CPT 对比。

Result: Reasoning CPT 在所有评估领域均表现更优，推理能力可跨领域迁移，且在难题上性能提升高达 8 分。模型还能根据问题难度调整推理深度。

Conclusion: Reasoning CPT 是一种有效的通用推理能力提升方法，尤其在复杂问题上表现突出，展示了跨领域迁移潜力。
...
TLDR: 论文提出了一种名为CoT Encyclopedia的框架，用于分析和引导模型的推理策略，通过自动提取、聚类和解释推理行为，提升了模型的理解和性能。


<details>
  <summary>Details</summary>
Motivation: 理解大型语言模型的长链推理策略（CoT）的多样性，并超越基于人类直觉的分类方法。

Method: 自动提取模型生成的CoT中的推理标准，嵌入语义空间，聚类成代表性类别，并生成对比性解释。

Result: 框架比现有方法更具解释性和全面性，并能预测和优化模型推理策略，提升性能。
...
TLDR: VQ-Logits利用向量量化（VQ）减少LLM输出层的参数和计算开销，显著提升效率。
...
Method: 用小型共享码本替代大输出嵌入矩阵，预测紧凑码本的logits并映射到完整词汇空间。
...
TLDR: RAIDEN-R1是一种强化学习框架，通过VRAR奖励机制提升角色扮演对话代理的角色一致性，实验表明其在多个指标上优于基线模型。
...
TLDR: 研究评估了五种大型语言模型（GPT-3.5、GPT-4、LLAMA3、Mistral 7B和Claude-2）在零样本和少样本条件下对俄语和乌克兰语社交媒体帖子中人权侵犯内容的二元分类任务表现。


<details>
  <summary>Details</summary>
Motivation: 探讨大型语言模型在多语言复杂文本任务中的表现，尤其是在敏感领域（如人权侵犯）的可靠性。

Method: 比较模型标注与人工双标注的黄金标准，分析不同提示条件下的性能，并探讨模型错误模式。

Result: 研究揭示了模型在跨语言适应性和主观判断任务中的优缺点。

Conclusion: 为大型语言模型在多语言敏感任务中的实际应用提供了重要参考。
...
TLDR: 生成式大语言模型（LLMs）在开放性任务中表现优异，而传统NLP在信息提取和分析任务中占优。
...
Result: 生成式LLMs在开放性任务中表现更好，传统NLP在信息提取和分析任务中更优。

Conclusion: 随着技术进步，确保伦理使用对医疗应用至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 临床实践中整合科学证据面临工作量大、流程复杂和时间限制等挑战，需要自动化工具支持高效决策。

Method: Quicker采用全自动化流程，覆盖从问题到临床建议的所有阶段，并通过交互界面支持定制化决策。

Result: 实验显示Quicker在问题分解、文献筛选和推荐逻辑上表现优异，与人类专家协作可将推荐时间缩短至20-40分钟。
...
TLDR: 论文提出了一种名为J1的强化学习方法，用于训练LLM-as-a-Judge模型，通过可验证奖励提升判断能力，并在多个基准测试中表现优于现有模型。


<details>
  <summary>Details</summary>
Motivation: AI评估质量的瓶颈促使开发更强大的LLM-as-a-Judge模型，而链式思维推理能力的提升是关键。

Method: 采用强化学习训练J1模型，将可验证和不可验证提示转化为带可验证奖励的判断任务，以减少偏见并激励思考。

Result: J1在8B和70B规模下表现优于其他模型，甚至在某些基准上超越更大的模型。

Conclusion: J1通过生成评估标准、自我参考答案和重新评估响应正确性，显著提升了判断能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本嵌入方法（如SimCSE和LLM2Vec）性能优秀但难以解释，而经典稀疏嵌入（如词袋模型）性能较差。需要一种兼具性能和可解释性的方法。

Method: 通过最远点采样生成低维密集嵌入，数值表示与不同锚文本的语义相关性。

Result: 在多个语义文本相似性、检索和聚类任务中验证，LDIR性能接近黑盒基线，优于其他可解释嵌入方法。

Conclusion: LDIR提供了一种低维、密集且可解释的文本嵌入解决方案，兼具性能和可解释性。
...
TLDR: 提出一种多模态框架，通过视觉语言模型解码大脑活动中的语言信息，支持视觉、听觉和文本输入。


<details>
  <summary>Details</summary>
Motivation: 人类思维本质上是多模态的，而现有研究多限于单模态输入，限制了语言重建的生态效度和泛化能力。

Method: 利用视觉语言模型（VLMs），结合模态特定的专家，共同解析多模态信息。

Result: 实验表明，该方法性能与最先进系统相当，同时更具适应性和扩展性。

Conclusion: 该研究推动了更具生态效度和泛化能力的大脑解码技术。
...
TLDR: 本文探讨了基于大语言模型（LLMs）的方面情感分析系统设计，重点研究了四元组意见提取，并验证了单一模型在多领域任务中的有效性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是开发一个能够同时处理多领域和多语言数据的通用模型，以减少操作复杂性并提高效率。

Method: 方法包括使用内部数据集对单一模型进行微调，以同时处理多个领域特定的分类任务。
...
<details>
  <summary>Details</summary>
Motivation: 代码生成中结构重复问题普遍且具有挑战性，现有研究多关注内容重复，而忽略了结构重复。
...
TLDR: LLM生成的简明语言摘要（PLS）在主观评价中与人工编写的PLS难以区分，但人工编写的PLS显著提高了读者理解力。自动评估指标与人类判断不一致。


<details>
  <summary>Details</summary>
Motivation: 解决LLM生成PLS的有效性评估问题，填补现有方法依赖自动化评分或主观评分的不足。

Method: 通过大规模众包评估（150名参与者），结合主观评分（简洁性、信息性、连贯性、忠实性）和客观理解测试（多选题）。

Result: LLM生成的PLS在主观评价中表现良好，但人工编写的PLS在理解力上更优；自动评估指标与人类判断不一致。

Conclusion: 需要开发超越表面质量的评估框架，并优化生成方法以提升非专业人士的理解力。
...
TLDR: LongRefiner是一种高效的插件式优化器，用于处理长文本RAG应用中的冗余和噪声问题，通过双级查询分析、分层文档结构和自适应优化，显著降低计算成本和延迟。
...
TLDR: DCoLT是一种基于扩散语言模型的推理框架，通过强化学习优化推理轨迹，实现双向非线性推理，显著提升数学和代码生成任务的性能。


<details>
  <summary>Details</summary>
Motivation: 传统Chain-of-Thought方法局限于线性因果推理，DCoLT旨在通过扩散模型的中间步骤实现更灵活的非线性推理。

Method: DCoLT利用反向扩散过程的中间步骤作为潜在“思考”动作，通过基于结果的强化学习优化推理轨迹。在两种扩散语言模型（SEDD和LLaDA）上实现。

Result: 实验显示，DCoLT增强的扩散语言模型在数学和代码生成任务中表现优于其他方法，LLaDA在多个任务中准确率提升显著（如HumanEval提升19.5%）。

Conclusion: DCoLT为扩散语言模型提供了一种高效的推理框架，支持非线性推理，显著提升了任务性能。
...
TLDR: CL-RAG框架通过多阶段课程学习优化RAG系统，提升检索器和生成器的性能，在多个QA数据集上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统直接使用top-k检索文档，但文档有效性差异大，影响模型训练效果。受人类认知学习启发，引入课程学习以提升泛化能力。

Method: 构建多难度训练数据，分阶段训练检索器和生成器，采用课程学习优化性能。

Result: 在四个开放域QA数据集上表现优于现有方法，性能提升2%至4%。

Conclusion: CL-RAG框架有效提升RAG系统的性能和泛化能力，验证了课程学习的适用性。
...
TLDR: 该论文提出了CoV-Eval多任务基准和VC-Judge改进模型，用于全面评估LLM代码安全性，发现多数LLM在生成安全代码和修复漏洞方面仍有不足。


<details>
  <summary>Details</summary>
Motivation: 当前代码安全基准仅关注单一任务，缺乏对代码生成、漏洞修复等多维度的综合评估，需更全面的评测工具。
...
TLDR: 论文探讨了基于翻译的跨语言迁移（XLT）策略，重点研究了标签投影中的设计决策，并提出了一种新的集成策略，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 研究动机是系统调查低层次设计决策对基于翻译的XLT性能的影响，并探索如何优化标签投影方法。

Method: 方法包括系统研究标签投影中的设计决策（如算法、过滤策略和预标记化），并提出一种集成翻译训练和翻译测试预测的新策略。

Result: 结果表明，优化后的基于词对齐器（WA）的方法与标记方法性能相当，而新提出的集成策略显著优于标记方法，并降低了对WA设计决策的敏感性。

Conclusion: 结论是优化后的WA方法和集成策略能有效提升跨语言迁移的性能和鲁棒性。
...
TLDR: MuToR是一种简单有效的多令牌预测方法，通过插入可学习的寄存器令牌来预测未来目标，兼容现有预训练语言模型，适用于监督微调。


<details>
  <summary>Details</summary>
Motivation: 多令牌预测在语言模型预训练中表现优异，但在其他场景（如微调）中效果不一致，因此需要一种更通用的方法。

Method: 提出MuToR方法，插入可学习的寄存器令牌到输入序列中，每个令牌负责预测未来目标，无需额外参数或架构改动。

Result: MuToR在监督微调、参数高效微调（PEFT）和预训练中均表现优异，适用于语言和视觉任务。

Conclusion: MuToR是一种高效且通用的多令牌预测方法，适用于多种任务和领域。
...
TLDR: 论文发现偏好建模中存在类似语言模型的扩展规律，提出WorldPM框架，验证其在大规模数据和模型下的有效性，显著提升偏好微调性能。


<details>
  <summary>Details</summary>
Motivation: 受语言模型中测试损失与模型及数据集规模呈幂律关系的启发，探索偏好建模中的类似规律，并提出WorldPM框架以统一表示人类偏好。

Method: 从公共论坛收集多样化偏好数据，使用1.5B至72B参数的模型在15M规模数据上进行训练，分析不同评估指标的表现。

Result: 发现对抗性指标随数据和模型规模提升，客观指标在大模型中显现涌现行为，主观指标无扩展趋势。WorldPM显著提升泛化性能，在多个基准任务中表现优异。

Conclusion: WorldPM作为偏好微调的基础框架具有显著潜力，实验验证其在内部和公开评估集上的性能提升。
...
TLDR: 论文提出了一种显式对齐大型推理模型的元能力（演绎、归纳和溯因）的方法，通过自动生成的自验证任务，显著提升了推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有大型推理模型的推理行为（如自我修正、回溯和验证）虽然存在，但不可预测且不可控，限制了其可扩展性和可靠性。

Method: 采用三阶段流程：个体对齐、参数空间合并和领域特定的强化学习，显式对齐模型的元能力。

Result: 性能比指令调优基线提升10%以上，领域特定强化学习进一步带来2%的平均增益。
...
TLDR: 研究评估了通用和医学大语言模型（LLMs）在生成准确、安全且易懂的癌症信息方面的表现，发现通用模型在语言质量和情感表达上更优，而医学模型在信息可及性上更强，但存在更高的潜在危害和偏见。


<details>
  <summary>Details</summary>
Motivation: 乳腺癌和宫颈癌的沟通存在显著差距，可能导致诊断延迟和治疗不足，研究旨在评估LLMs在支持患者理解方面的能力与局限。

Method: 采用混合方法评估框架，结合定量指标、定性专家评分及Welch's ANOVA、Games-Howell和Hedges' g统计方法，对五种通用和三种医学LLMs进行评估。

Result: 通用LLMs在语言质量和情感表达上表现更好，医学LLMs在信息可及性上更优，但潜在危害、毒性和偏见更高，影响其安全性和可信度。

Conclusion: 研究揭示了领域专业知识与安全性之间的权衡，强调需针对性改进模型设计以减少危害和偏见，提升安全性和情感表达，为未来开发准确、安全且可及的数字健康工具提供关键见解。
...
TLDR: 论文提出了一种新的搜索算法，用于双人零和完美信息游戏，并在短时间和中等搜索时间下表现优于现有算法。
...
Conclusion: 新算法在双人零和完美信息游戏中具有优越性能，填补了现有研究的空白。
...
TLDR: 本文改进了现有技术，提出了高效算法来识别分类模型中的相关和必要特征，并引入全局有用性概念，验证其在决策树和复杂模型中的实用性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在识别特征重要性时存在效率问题，尤其是在复杂模型中。本文旨在改进这些方法，并扩展相关概念。

Method: 基于命题逻辑和充分理由概念，提出新算法来检测相关和必要特征，并引入全局有用性概念及其高效检测方法。

Result: 实验证明，新算法能高效检测必要特征，全局有用性与相关性和必要性相关，并在决策树和复杂模型中具有实用性。

Conclusion: 本文提出的方法改进了特征重要性分析，扩展了相关理论，并在实践中验证了其有效性。
...
TLDR: 论文提出了一种基于无模型目标条件强化学习的方法，用于动态环境中的目标识别问题。


<details>
  <summary>Details</summary>
Motivation: 在动态环境中，传统目标识别方法难以适应不断变化的目标，因此需要一种更通用的解决方案。

Method: 采用无模型目标条件强化学习方法，实现快速适应不同任务的目标识别。
...
TLDR: XpertXAI是一种基于专家指导的可解释AI模型，用于多肺部病理检测，优于现有方法并提供更符合专家推理的解释。
...
Method: 扩展ClinicXAI，采用InceptionV3分类器和公共数据集，与后验解释方法和无监督CBM（XCBs）对比。

Result: XpertXAI在预测准确性和解释临床意义方面优于基线方法。

Conclusion: 人本模型设计可扩展到更广泛的诊断场景，为医学诊断提供可解释AI的可行路径。
...
TLDR: 提出了一种多模态多代理框架，用于放射学报告生成（RRG），通过任务特定代理模拟临床推理流程，显著提升了报告的准确性、结构化和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在事实不一致、幻觉和跨模态对齐问题，需要一种更可靠且符合临床推理流程的解决方案。

Method: 采用多模态多代理框架，包括检索、草稿生成、视觉分析、精炼和合成等任务特定代理。
...
TLDR: 研究探讨了离线强化学习算法在微电网电压调节中的应用，特别是在太阳能渗透情况下，通过离线训练获得适用模型。


<details>
  <summary>Details</summary>
Motivation: 由于技术或安全原因无法进行在线环境交互时，离线强化学习能通过已有数据集训练模型，减少负面影响。

Method: 使用不同离线强化学习算法在IEEE 33-bus系统上进行实验。

Result: 实验证明该方法在不同质量离线数据集上均可行且有效。

Conclusion: 离线强化学习在微电网电压调节中具有潜力，尤其在缺乏在线交互时。
...
TLDR: 该研究提出了一个系统性评估问题质量的框架，定义了好问题的标准，并通过半自适应标准实现了灵活性与结构性的平衡。


<details>
  <summary>Details</summary>
Motivation: 由于问题质量评估的研究有限，该研究旨在填补这一空白，为人类和人工智能提供更全面的问题评估方法。

Method: 提出了两个关键评估维度：适当性（语境中的社会语言能力）和有效性（目标实现的策略能力），并开发了基于评分标准的系统。

Result: 在CAUS和SQUARE数据集上验证了框架的有效性，能够评估结构良好和有问题的提问，并适应不同语境。

Conclusion: 该研究为问题评估提供了一个灵活且全面的框架，推动了问题行为与结构化分析方法的结合。
...
TLDR: 本文回顾了AI从基础规则系统到复杂自主代理的发展历程，探讨了关键技术里程碑，并认为当前AI（如ChatGPT和Grok）可能是AI发展的最终阶段。


<details>
  <summary>Details</summary>
Motivation: 记录AI技术的快速演进及其对社会的影响，强调智慧与远见的重要性。

Method: 通过分析关键技术进展（如提示、训练方法、硬件和架构创新）和实际案例（如ChatGPT和Grok），探讨AI能力。

Result: 当前AI代表可能的技术顶峰，智能每六个月翻倍，带来深远社会影响。
...
TLDR: Pre-Act是一种通过多步执行计划和详细推理提升LLM代理性能的新方法，优于ReAct，并在小模型上通过微调实现显著改进。


<details>
  <summary>Details</summary>
Motivation: 提升语言模型代理的性能，特别是在复杂推理任务中，同时解决小模型在推理能力上的局限性。

Method: 提出Pre-Act方法，生成多步执行计划并逐步优化，结合微调小模型（如Llama 3.1）。

Result: Pre-Act在Almita数据集上优于ReAct（动作召回率提高70%），微调后的70B模型在动作准确率和目标完成率上显著优于GPT-4。

Conclusion: Pre-Act有效提升代理性能，尤其在小模型上通过微调实现突破，为实际应用提供可行方案。
...
TLDR: 论文提出了一种多模态个性化抑郁症检测方法，针对不同年龄段（老年人和年轻人）设计了两条研究路径，旨在填补现有抑郁症检测方法的不足。


<details>
  <summary>Details</summary>
Motivation: 现有抑郁症检测方法主要关注年轻人，忽视了不同年龄段的个体差异和抑郁症表现的多样性。

Method: 使用MPDD-Elderly和MPDD-Young数据集，结合多模态数据（音频和视频）及个体差异信息，开发基线模型。

Result: 提出了一种融合多模态数据和个体差异的抑郁症检测方法，适用于不同人群。

Conclusion: 该挑战旨在推动个性化抑郁症检测方法的发展，促进心理健康研究和包容性检测系统的进步。
...
TLDR: 论文提出了一种基于图RAG的管道，利用教育知识图谱和个人知识图谱，为MOOC学习者提供个性化学习支持，解决了LLMs的幻觉问题和RAG在MOOC中的局限性。


<details>
  <summary>Details</summary>
Motivation: MOOC中学习者与教师互动不足，LLMs存在幻觉问题，且RAG在MOOC中的应用受限于非结构化学习材料和缺乏主动引导。

Method: 提出Graph RAG管道，结合EduKGs和PKGs，实现个性化问题生成和基于知识图谱的问题回答。

Result: 在CourseMapper平台上对3门MOOC进行评估，结果表明Graph RAG能有效支持学习者理解新知识概念。

Conclusion: Graph RAG为MOOC学习者提供了个性化学习体验，展示了其在知识理解方面的潜力。
...
TLDR: 该研究提出了一种基于生成式AI和大型语言模型的方法，将台湾中国研究领域的非结构化学术文本转化为结构化的知识图谱，以揭示研究趋势和空白。


<details>
  <summary>Details</summary>
Motivation: 回应系统性整理台湾中国研究领域学术成果的需求，利用AI技术提升知识获取效率。

Method: 应用生成式AI和大型语言模型，从1367篇同行评审文章中提取实体关系三元组，并通过D3.js可视化构建知识图谱。

Result: 构建了一个领域特定的知识图谱和向量数据库，支持用户探索概念节点和语义关系，发现新的研究轨迹和主题集群。

Conclusion: 展示了生成式AI在区域知识系统中的应用潜力，为学术基础设施提供了数据驱动的替代方案。
...
TLDR: 研究探讨了在医疗AI中如何通过不同类型的解释增强透明度和信任，并通过医生调查和访谈评估了最有效的解释方式。


<details>
  <summary>Details</summary>
Motivation: 随着AI在医疗中的应用增加，理解哪些解释能增强透明度和信任，尤其是在医生与AI共同决策的场景中，至关重要。

Method: 通过用户研究（医生调查和后续访谈）评估不同类型的AI生成解释在诊断决策支持中的效果。
...
TLDR: MASS提出了一种基于LLM的多智能体投资组合构建方法，通过动态扩展智能体数量和反向优化实现稳定超额收益。
...
Conclusion: MASS范式可推广至类似任务，代码已开源。
...
Result: 大多数LLMs的常识能力低于人类中位数，且与人类相关性有限；较小开源模型表现优于大型专有模型。

Conclusion: 研究呼吁AI模型应适应不同文化背景的人类群体，强调常识评估需考虑文化多样性。
...
TLDR: 本文研究了SMT和MILP在医疗人员排班问题中的应用，比较了Z3和Gurobi求解器的性能，发现SMT在多样化排班中表现更优。


<details>
  <summary>Details</summary>
Motivation: 医疗排班问题复杂且约束多样，传统方法效果有限，SMT在形式验证领域的成功激发了其在排班中的应用探索。

Method: 提出通用约束模型，分别用SMT和MILP求解，并在学术和实际问题上比较Z3和Gurobi的性能。

Result: MILP在高度约束或无解问题中表现更好，而SMT在多样化排班中更优，但对约束表达更敏感。
...
TLDR: 论文提出了‘可塑性’作为衡量智能体受过去观察影响程度的概念，并揭示了其与‘赋能’的对称关系。


<details>
  <summary>Details</summary>
Motivation: 探讨智能体如何受其观察影响，填补了现有理论中对这一基础能力的量化空白。

Method: 通过引入广义定向信息这一新的信息论量来定义可塑性，并验证其性质。

Result: 发现可塑性与赋能是对称的，且两者之间存在张力，需在智能体设计中平衡。

Conclusion: 可塑性和赋能及其关系对理解智能体行为至关重要。
...
TLDR: 提出了一个不依赖真实解释或模型敏感性的解释评估框架AXE，用于评估局部特征重要性解释。


<details>
  <summary>Details</summary>
Motivation: 现有解释评估方法依赖真实解释或模型敏感性，存在局限性，需要更独立的质量评估策略。

Method: 提出AXE框架，基于三个原则设计，无需真实解释或模型敏感性。

Result: AXE验证了其有效性，可用于检测解释公平性。

Conclusion: AXE为解释评估提供了一种独立且有效的方法，未来可进一步发展。
...
TLDR: 本文区分了AI Agents和Agentic AI，提出了概念分类、应用映射和挑战分析，比较了两者的设计理念和能力。


<details>
  <summary>Details</summary>
Motivation: 澄清AI Agents和Agentic AI的区别，为开发更健壮、可扩展和可解释的系统提供指导。

Method: 通过定义、架构演变、操作机制、交互风格和自主性水平的比较分析。

Result: 展示了两种范式在不同应用领域的表现，并提出了针对挑战的解决方案。
...
TLDR: 研究评估了大语言模型在动态环境中的自适应能力，发现战略提示可以缩小模型间的性能差距，但高级推理方法效果不稳定，且当前模型在规划、推理等方面仍有局限。


<details>
  <summary>Details</summary>
Motivation: 探索大语言模型作为自学习和推理智能体在动态环境中的潜力，评估其自适应能力。

Method: 通过自反思、启发式变异和规划等提示技术，在动态环境中测试不同开源语言模型的性能。

Result: 大模型表现优于小模型，但战略提示可缩小差距；高级提示对小模型在复杂任务中更有效，但对大模型提升有限；推理方法效果不稳定。

Conclusion: 当前大语言模型在规划、推理等方面仍有根本性不足，需超越静态基准以全面评估推理能力。
...
TLDR: 本文介绍了一种针对左心房（LA）4D Flow MRI分析的开源计算框架，解决了传统超声分析的局限性，并首次全面评估了能量、涡度和压力参数作为预后生物标志物的潜力。


<details>
  <summary>Details</summary>
Motivation: 左心房在调节左心室充盈中起关键作用，但传统超声分析对其血流动力学的理解有限。4D Flow MRI虽有望提升理解，但低流速和空间分辨率限制以及缺乏专用计算框架，使得分析复杂化。

Method: 研究开发了首个开源计算框架，支持对4D Flow MRI数据进行定性和定量分析，并验证了其在不同中心数据中的鲁棒性。

Result: 框架实现了高精度自动分割（Dice > 0.9，Hausdorff 95 < 3 mm），并首次全面评估了LA中的能量、涡度和压力参数。

Conclusion: 该框架为研究LA血流动力学提供了强大工具，并展示了这些参数作为预后生物标志物的潜力。
...
TLDR: Dyadic Mamba利用状态空间模型（SSM）生成任意长度的高质量双人运动，解决了传统Transformer方法在长序列生成中的局限性。
...
Method: 提出Dyadic Mamba，通过简单的架构设计（如序列拼接）促进信息流动，避免复杂的跨注意力机制。

Result: 在短序列基准上表现优异，长序列生成显著优于Transformer方法，并提出了新的长序列评估基准。

Conclusion: 基于SSM的架构为长序列双人运动生成提供了有前景的方向。
...
TLDR: 提出了一种仅利用现有标注的医学图像分割方法BoundarySeg，通过多任务框架结合器官边界预测提升分割精度，无需未标注数据。
...
Result: 在低数据情况下性能优于或媲美半监督方法，且不增加计算负担。
...
TLDR: 提出一种基于文本条件扩散的两阶段方法，生成高质量手术视频以解决数据不平衡问题，并通过拒绝采样策略优化合成样本，显著提升下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 手术视频数据集中严重的数据不平衡阻碍了高性能模型的开发，因此需要合成手术视频以解决这一问题。
...
TLDR: 论文提出了一种名为概率模式归纳（PSI）的原型模型，通过深度学习在少量结构化示例上进行类比映射，形成组合概念。PSI在人类类似的学习表现上优于传统模型。
...
Method: 使用PSI模型，结合对象级相似性和关系相似性，并通过选择性注意力机制放大相关关系。

Result: PSI在人类类似的学习任务中表现优异，优于传统非结构化特征向量模型。

Conclusion: 结构化表示和类比映射是快速学习组合视觉概念的关键，深度学习可用于构建心理学模型。
...
TLDR: LSG-SLAM是一种基于3D高斯泼溅的大规模视觉SLAM方法，采用多模态策略和特征对齐扭曲约束，适用于户外场景，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有NeRF和3DGS方法依赖RGBD传感器且仅适用于室内环境，大规模户外场景的鲁棒性重建尚未解决。

Method: 采用多模态策略估计初始位姿，引入特征对齐扭曲约束优化跟踪，使用连续高斯泼溅子图处理大规模场景，并通过位姿优化和结构细化提升重建质量。

Result: 在EuRoc和KITTI数据集上表现优于现有神经、3DGS和传统方法。

Conclusion: LSG-SLAM为大规模户外场景提供了一种高效、鲁棒的视觉SLAM解决方案。
...
TLDR: AdaptCLIP是一种基于CLIP模型的视觉异常检测方法，通过交替学习视觉和文本表示，结合上下文和对齐残差特征，显著提升了跨域零/少样本泛化能力。
...
Method: 提出AdaptCLIP方法，通过视觉适配器、文本适配器和提示-查询适配器，交替学习自适应表示，并结合上下文和对齐残差特征进行对比学习。

Result: 在12个工业和医学领域的异常检测基准测试中，AdaptCLIP取得了最先进的性能，显著优于现有方法。

Conclusion: AdaptCLIP是一种简单有效的方法，无需目标域微调即可实现跨域零/少样本泛化，为视觉异常检测提供了新思路。
...
TLDR: 提出了一种新的源自由域适应（SFDA）框架，通过预适应生成高质量伪标签和数据依赖的频率提示，结合风格相关层微调策略，显著提升了跨模态分割任务的性能。


<details>
  <summary>Details</summary>
Motivation: 解决现有SFDA方法在图像风格转换和伪标签质量上的不足，以及训练效率低下的问题。

Method: 引入预适应生成预适应模型，提出数据依赖的频率提示优化图像风格转换，并采用风格相关层微调策略。

Result: 在跨模态腹部和心脏分割任务中，性能优于现有最先进方法。

Conclusion: 所提框架有效解决了SFDA中的关键挑战，为跨域分割任务提供了高效解决方案。
...
TLDR: VRU-CIPI框架通过GRU和Transformer自注意力机制预测VRU的过街意图，准确率96.45%，实时性33帧/秒，结合I2V通信提升路口安全。


<details>
  <summary>Details</summary>
Motivation: 理解和预测人类行为（尤其是路口VRU的过街意图）对提升道路交互安全至关重要。

Method: 提出VRU-CIPI框架，结合GRU捕捉时序动态和Transformer自注意力机制编码上下文与空间依赖。

Result: 在UCF-VRU数据集上达到96.45%准确率，实时性33帧/秒。

Conclusion: 结合I2V通信，可提前激活过街信号并预警车辆，提升路口安全性。
...
TLDR: 提出了一种新的遥感变化检测任务——非配准变化检测，以应对自然灾害、人为事故和军事打击等紧急情况。


<details>
  <summary>Details</summary>
Motivation: 解决非配准变化检测问题，填补现有研究的空白，并应对现实中的紧急需求。

Method: 系统提出八种现实场景，并针对不同场景开发图像转换方案，将现有配准数据集转换为非配准版本。

Result: 非配准变化检测对现有最先进方法造成灾难性影响。
...
TLDR: 提出了一种用于红外小目标检测的轮廓感知和显著性先验嵌入网络（CSPENet），通过提取目标轮廓特征和显著性先验，结合双分支嵌入架构和注意力机制，显著提升了检测性能。
...
Method: 设计了轮廓收敛先验提取模块（SCPEM）捕获目标轮廓特征，提出双分支先验嵌入架构（DBPEA）融合特征，并开发注意力引导特征增强模块（AGFEM）优化特征表示。
...
TLDR: MambaControl是一个结合选择性状态空间建模和扩散过程的新框架，用于高保真预测医学图像轨迹，特别针对阿尔茨海默病预测。


<details>
  <summary>Details</summary>
Motivation: 现有方法在捕捉疾病进展中的时空动态和保持解剖结构一致性方面存在困难。
...
TLDR: 提出了一种基于纹理关键驱动因素（TKDF）的新框架，用于野生环境下的面部表情识别（FER），通过纹理感知特征提取器（TAFE）和双重上下文信息过滤（DCIF）提升性能。
...
Method: 提出TKDF框架，结合TAFE（基于ResNet的多分支注意力提取器）和DCIF（自适应池化和注意力机制过滤上下文）。

Result: 在RAF-DB和KDEF数据集上达到最先进性能。

Conclusion: TKDF框架有效且鲁棒，提升了FER性能。
...
<details>
  <summary>Details</summary>
Motivation: ALS点云分割在现实应用中面临领域偏移问题，导致模型性能下降，而现有CTTA方法在ALS点云上的研究有限。

Method: 提出动态可训练层选择模块、熵一致性损失和随机参数插值机制，并构建了两个基准数据集ISPRSC和H3DC。

Result: APCoTTA在两个基准数据集上表现最佳，mIoU分别提升约9%和14%。
...
TLDR: HQUIC是一种新型水下图像压缩算法，通过自适应预测和动态加权多尺度频率组件，显著提升压缩效率。
...
Method: HQUIC采用ALTC模块预测衰减系数和全局光信息，利用辅助分支提取常见对象，并动态加权多尺度频率组件。
...
TLDR: PointArena是一个评估多模态指向能力的综合平台，包括数据集、交互式竞技场和机器人系统，用于测试模型在多样化推理场景中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有基准仅关注对象定位任务，缺乏对多模态指向能力的全面评估，因此需要开发更全面的平台。

Method: PointArena包含三个组件：Point-Bench（数据集）、Point-Battle（交互式竞技场）和Point-Act（机器人系统），用于多阶段评估。

Result: Molmo-72B表现最佳，专有模型逐渐接近其性能，监督训练显著提升模型表现。

Conclusion: 精确的指向能力对多模态模型连接抽象推理与具体行动至关重要，PointArena为评估提供了有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法采用稀疏二元监督，忽略了图像与文本之间固有的多对多关系以及从一般到具体描述的隐含连接。

Method: DITM利用句子描述性分数（基于TF-IDF）动态调整正负样本对的连接性，并按照从通用到具体的顺序对齐相关句子。

Result: 在MS-COCO、Flickr30K和CxC数据集上的实验表明，DITM在表示复杂图像-文本关系方面优于现有方法，并增强了模型的分层推理能力。

Conclusion: DITM通过超越刚性二元监督，能够更有效地发现最优匹配和潜在正样本对，提升了图像-文本匹配的性能。
...
TLDR: 提出了一种基于3D草图的3D服装生成框架，通过简单的手绘输入为普通用户生成高质量数字服装。


<details>
  <summary>Details</summary>
Motivation: 现有3D服装设计工具技术门槛高且数据有限，普通用户难以使用。

Method: 结合条件扩散模型、共享潜在空间训练的草图编码器和自适应课程学习策略，处理自由手绘输入。

Result: 实验和用户研究表明，该方法在逼真度和可用性上显著优于现有基线。

Conclusion: 该方法有望推动下一代消费平台的民主化时尚设计。
...
Conclusion: 该模型在自动驾驶竞赛（如FSAC）中具有实际应用潜力，特别适用于单目标和小目标检测场景。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在处理复杂场景和保留图像细节方面存在局限性，需要更有效的超分辨率方法。
...
Method: 提出DeepSeqCoco模型，测试了SGD、Adam及混合优化器配置，以平衡准确性、损失最小化和计算成本。

Result: 模型准确率达99.5%，混合SGD-Adam验证损失最低（2.81%），训练时间减少18%，预测时间减少85%。
...
### [190] [Exploring the Deep Fusion of Large Language Models and Diffusion Transformers for Text-to-Image Synthesis](https://arxiv.org/abs/2505.10046)
*Bingda Tang,Boyang Zheng,Xichen Pan,Sayak Paul,Saining Xie*

Main category: cs.CV

TLDR: 本文深入探讨了文本到图像合成中LLMs与DiTs深度融合的设计空间，填补了现有研究的空白。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注整体系统性能，缺乏详细比较和设计细节，导致方法潜力不明。

Method: 通过实证研究，进行对照比较，分析设计选择，并提供可复现的大规模训练方案。
...
### [191] [UOD: Universal One-shot Detection of Anatomical Landmarks](https://arxiv.org/abs/2306.07615)
*Heqin Zhu,Quan Quan,Qingsong Yao,Zaiyi Liu,S. Kevin Zhou*

Main category: cs.CV

TLDR: 论文提出了一种名为UOD的领域自适应一次性医学标志物检测框架，用于处理多领域医学图像，解决了现有方法在跨领域数据中的性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 现有一次性学习方法在跨领域未标记数据中表现不佳，且对标注图像质量敏感。UOD旨在解决这些问题，提升多领域医学图像的标志物检测性能。

Method: UOD采用两阶段框架：第一阶段通过自监督学习生成伪标签；第二阶段使用领域自适应Transformer消除领域偏好并建立全局上下文。

Result: 在三个不同解剖领域的X射线数据集上，UOD取得了最先进的性能。

Conclusion: UOD通过领域共享模块和自适应设计，显著提升了一次性医学标志物检测的鲁棒性和准确性。
...
TLDR: 本文综述了动态场景表示与重建领域的最新进展，重点分析了200多篇基于辐射场的论文，涵盖从隐式神经表示到显式高斯基元的技术。


<details>
  <summary>Details</summary>
Motivation: 动态场景重建在计算机视觉和图形学中具有重要意义，近年来通过神经辐射场和3D高斯泼溅技术的突破取得了显著进展。本文旨在系统梳理相关研究，为研究者提供全面参考。

Method: 通过分类和评估200多篇论文，从运动表示范式、重建技术、辅助信息整合策略和时间一致性正则化等多个角度进行分析。

Result: 总结了动态场景表示与重建的多样化方法，并提出了统一的表示框架。

Conclusion: 本文为动态场景重建领域的研究者提供了系统性参考，同时指出了未来研究的方向和挑战。
...
TLDR: 论文评估了大型多模态模型（LMMs）在低资源普什图语OCR任务中的表现，开发了合成数据集PsOCR，并比较了开源和闭源模型的性能。


<details>
  <summary>Details</summary>
Motivation: 解决普什图语OCR面临的挑战，如草书字体和数据集稀缺，推动类似脚本（如阿拉伯语、波斯语、乌尔都语）的研究。

Method: 开发合成数据集PsOCR（100万张图像），涵盖多种字体、颜色和布局，并评估7种开源和4种闭源LMMs的性能。

Result: Gemini在闭源模型中表现最佳，Qwen-7B在开源模型中领先。

Conclusion: 研究为普什图语OCR提供了基准，并为类似脚本的OCR任务奠定了基础。
...
TLDR: ToonifyGB是一个两阶段框架，用于生成多样化的3D风格化头部虚拟形象。第一阶段通过改进的StyleGAN生成风格化视频，第二阶段利用高斯混合形状合成风格化虚拟形象。


<details>
  <summary>Details</summary>
Motivation: 扩展Toonify框架，以支持基于高斯混合形状的多样化3D风格化头部虚拟形象生成。

Method: 采用两阶段框架：1）改进的StyleGAN生成风格化视频；2）从视频中学习高斯混合形状，合成虚拟形象。

Result: 在Arcane和Pixar两种风格上验证了ToonifyGB的有效性，能够高效渲染任意表情的风格化虚拟形象。

Conclusion: ToonifyGB成功实现了高质量、多样化的3D风格化头部虚拟形象生成。
...
TLDR: 论文提出MMRL和MMRL++方法，通过共享模态无关表示空间和优化表示令牌，解决小样本数据下视觉语言模型的过拟合问题，提升泛化能力。
...
Method: MMRL引入共享表示空间，优化表示令牌和类令牌；MMRL++进一步减少参数并增强模态内交互。

Result: 在15个数据集上优于现有方法，平衡任务适应和泛化。

Conclusion: MMRL和MMRL++有效提升小样本学习的泛化性能。
...
TLDR: 论文提出了一种动态视觉令牌剪枝方法MoB，通过双目标覆盖问题优化性能与效率。


<details>
  <summary>Details</summary>
Motivation: 现有静态剪枝方法忽视任务间目标重要性差异，导致性能不稳定。

Method: 基于Hausdorff距离推导误差界，提出MoB框架，通过贪婪半径分配平衡目标。

Result: MoB在LLaVA-1.5-7B上保留96.4%性能，仅用11.1%令牌，加速LLaVA-Next-7B 1.3-1.5倍。

Conclusion: MoB适用于先进MLLM及多样化视觉语言任务，性能与效率兼具。
...
TLDR: 提出一种基于条件U-Net架构的图像配准方法，用于估计未知条件下的图像，应用于放疗中的呼吸运动肿瘤定位，解决了传统方法的重建伪影问题。


<details>
  <summary>Details</summary>
Motivation: 解决放疗中因呼吸运动导致的4D-CT图像重建伪影问题，提高肿瘤定位精度。

Method: 使用条件U-Net架构，无需固定图像，直接利用已知图像及其条件信息进行建模。

Result: 在临床4D-CT数据上实现了无伪影的实时图像重建。

Conclusion: 该方法有效解决了呼吸运动导致的图像重建问题，代码已开源。
...
TLDR: 提出了一种多源协作风格增强和域不变学习方法（MCSAD），用于联邦域泛化，通过扩展风格空间和跨域特征对齐提升模型泛化能力。
...
Method: 提出多源协作风格增强模块和域不变学习方法，通过风格空间扩展和特征对齐学习域不变模型。
...
TLDR: 论文提出了一种新架构，通过少量数据集特定参数解决跨数据集显著性预测的性能下降问题，显著提升了泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有显著性预测模型在跨数据集时性能显著下降（约40%），且数据集多样性无法解决这一问题，主要原因是数据集特定偏差。

Method: 提出了一种新架构，扩展了数据集无关的编码器-解码器结构，仅使用少于20个数据集特定参数控制可解释机制（如多尺度结构、中心偏差和注视扩散）。

Result: 模型在MIT/Tuebingen显著性基准测试（MIT300、CAT2000和COCO-Freeview）上达到新最优性能，泛化能力提升75%以上，仅需50个样本即可显著改进。

Conclusion: 新模型不仅显著提升了跨数据集性能，还揭示了空间显著性的复杂多尺度效应，为理解显著性机制提供了新视角。
...
### [200] [VolE: A Point-cloud Framework for Food 3D Reconstruction and Volume Estimation](https://arxiv.org/abs/2505.10205)
...
Method: 利用移动设备捕获图像和相机位置，通过AR技术生成3D模型，并结合食物视频分割生成食物掩模。

Result: 实验表明，VolE在多个数据集上的平均绝对百分比误差（MAPE）为2.22%，优于现有技术。

Conclusion: VolE是一种高效且无需额外硬件的食物体积估计解决方案，适用于医疗营养管理和健康监测。
...
### [201] [Unlocking Location Intelligence: A Survey from Deep Learning to The LLM Era](https://arxiv.org/abs/2505.09651)
*Xixuan Hao,Yutian Jiang,Xingchen Zou,Jiabo Liu,Yifang Yin,Yuxuan Liang*

Main category: cs.CV

TLDR: 本文综述了地理空间表示学习在深度学习和大型语言模型（LLM）时代的发展，提出了基于数据、方法和应用视角的分类，并探讨了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 地理空间数据转化为可操作知识的需求推动了位置智能（LI）的发展，深度学习和LLM的进步为地理空间表示学习带来了新的机遇和挑战。

Method: 通过系统分类法，从数据、方法和应用三个视角，全面回顾了地理空间表示学习的技术发展。

Result: 总结了当前进展，指出了现有局限性，并提出了LLM时代的潜在研究方向。

Conclusion: 本文为位置智能领域的进一步创新提供了路线图，并持续更新相关资源。
...
TLDR: 论文探讨了医学图像分割模型在真实临床场景中性能下降的问题，提出MixUp和Auxiliary Fourier Augmentation两种数据增强方法，显著提升了模型的泛化能力和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割模型在真实临床场景中因训练与测试数据分布不匹配导致性能下降，传统数据增强方法难以应对多样性。
...
Conclusion: MixUp和Auxiliary Fourier Augmentation为医学分割模型提供了一种易于实现且有效的解决方案，提升了其在真实应用中的可靠性。
...
TLDR: 论文探讨了在医学影像中通过人类与AI对齐减少偏见的方法，结果显示适度对齐能提升公平性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决医学影像AI中存在的偏见问题，探索人类与AI对齐对公平性和泛化能力的影响。

Method: 系统研究人类与AI对齐在医学影像中的应用，结合人类洞察力调整模型。

Result: 适度对齐显著减少公平性差距并提升跨领域泛化能力，但过度对齐可能导致性能下降。

Conclusion: 人类与AI对齐是开发公平、鲁棒且泛化能力强的医学AI系统的有效方法，需平衡专家指导与自动化效率。
...
TLDR: MTVCrafter提出了一种基于4D运动序列的人类图像动画框架，通过4DMoT和MV-DiT技术，显著提升了动画的灵活性和表现力。
...
Method: 使用4DMoT将3D运动序列量化为4D运动令牌，并设计MV-DiT模型，利用运动令牌进行动画生成。

Result: 实验显示MTVCrafter的FID-VID为6.98，优于第二名65%，且在多样场景中表现优异。

Conclusion: MTVCrafter为人类视频生成开辟了新方向，具有广泛的应用潜力。
...
TLDR: ADHMR通过偏好优化对齐扩散模型，提升单图像人体网格恢复的鲁棒性和对齐性。
...
Method: 训练HMR-Scorer评估预测质量，构建偏好数据集，并用直接偏好优化微调基础模型。

Result: ADHMR在实验中优于现有方法。

Conclusion: ADHMR通过偏好优化和数据清理显著提升了人体网格恢复性能。
...
TLDR: 本文提出了一种名为SAGE DeeR的智能驾驶座舱代理，具备超级对齐、通用性和自我启发能力，并通过大规模基准测试验证其性能。
...
Method: SAGE DeeR通过超级对齐（根据用户偏好调整反应）、通用性（多模态输入理解）和自我启发（语言空间隐式思维链）实现目标，并构建了大规模基准测试。

Result: SAGE DeeR能够理解多模态输入并推理用户状态，同时通过基准测试验证了其感知决策能力和超级对齐的准确性。

Conclusion: SAGE DeeR为智能驾驶座舱提供了一种高度适应性和智能化的解决方案，具有广泛的应用潜力。
...
TLDR: 本文提出了一种新颖的离线地图构建方法，通过整合非正式驾驶路线（trails），利用Transformer模型构建全局地图，优于现有在线地图方法。


<details>
  <summary>Details</summary>
Motivation: 高精地图对自动驾驶规划至关重要，但传统在线地图方法在时间一致性、传感器遮挡等方面存在挑战。

Method: 整合车辆和其他交通参与者的trail数据，使用Transformer模型构建全局地图，支持持续更新且传感器无关。

Result: 在两个基准数据集上验证，表现优于现有在线地图方法，泛化能力更强。

Conclusion: 该方法为自动驾驶系统提供了更高效、鲁棒的地图构建解决方案。
...
TLDR: 本文介绍了HandReader，一种用于手语拼写识别的架构，包含三种模型（RGB、KP、RGB+KP），在多个数据集上取得先进成果，并发布了新的俄语手语数据集Znaki。


<details>
  <summary>Details</summary>
Motivation: 手语拼写识别在现有方法中仍有提升空间，尤其是处理视频时序信息时。

Method: 提出三种架构：HandReader$_{RGB}$（使用TSAM处理RGB特征）、HandReader$_{KP}$（基于TPE处理关键点）、HandReader$_{RGB+KP}$（结合RGB和关键点）。

Result: 在ChicagoFSWild、ChicagoFSWild+和Znaki数据集上取得先进成果。

Conclusion: HandReader模型在手语拼写识别任务中表现优异，并公开了Znaki数据集和预训练模型。
...
TLDR: 论文介绍了MFogHub，首个多区域、多卫星的海洋雾数据集，用于解决现有数据集的局限性，提升海洋雾检测和预测的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有海洋雾数据集多为单一区域或卫星数据，限制了模型在不同条件下的评估和海洋雾特性的研究。

Method: 构建了包含15个沿海雾区和6颗卫星的68,000多个高分辨率样本的MFogHub数据集，并测试了16个基线模型。

Result: 实验表明MFogHub能揭示区域和卫星差异导致的泛化波动，并为针对性预测技术提供资源。

Conclusion: MFogHub有助于全球海洋雾动态的监测和科学研究，数据集和代码已开源。
...
TLDR: 提出了一种多阶段跨模态交互（MSCI）模型，通过利用CLIP视觉编码器的中间层信息，增强对细粒度局部特征的感知能力，解决了现有CZSL方法在局部特征捕捉上的不足。
...
Conclusion: MSCI模型通过动态调整注意力权重，灵活适应不同场景，显著提升了CZSL任务的性能。
...
TLDR: 论文提出StoryReasoning数据集和Qwen Storyteller模型，通过视觉实体关联和多帧关系建模，减少视觉叙事中的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 解决视觉叙事系统中角色身份不一致和动作与主体关联错误的问题。

Method: 使用跨帧目标重识别、思维链推理和视觉实体关联技术，构建结构化场景分析和故事数据集。

Result: 微调后的Qwen Storyteller模型将幻觉问题平均减少12.3%。

Conclusion: StoryReasoning数据集和Qwen Storyteller模型有效提升了视觉叙事的一致性和准确性。
...
TLDR: MIPHEI是一种基于U-Net和ViT的模型，用于从H&E染色图像预测多路免疫荧光（mIF）信号，以低成本实现细胞类型分类。


<details>
  <summary>Details</summary>
Motivation: 解决mIF技术因成本和复杂性难以普及的问题，通过H&E图像预测mIF信号，实现细胞类型识别。

Method: 采用U-Net架构结合ViT编码器，训练于ORION数据集，并在两个独立数据集上验证。

Result: 模型在多个细胞标记物上表现优异（如Pan-CK F1=0.88），显著优于基线模型。
...
TLDR: 本文提出了一种名为PartCrop的统一成员推理方法，用于攻击视觉自监督模型，验证了其有效性和泛化性，并提出了防御策略。


<details>
  <summary>Details</summary>
Motivation: 自监督学习在处理大量无标签数据时面临隐私问题，尤其是在视觉领域。本文旨在在更现实的攻击场景下（攻击者对模型训练方法和细节未知）研究成员推理攻击。

Method: 提出PartCrop方法，通过裁剪图像中的部分对象并查询其在表示空间中的响应，利用模型对训练数据的部分感知能力进行攻击。

Result: 实验表明PartCrop对多种自监督模型和数据集有效。防御实验中，早期停止、差分隐私和裁剪尺度范围缩小等方法均有效。

Conclusion: PartCrop是一种有效的成员推理攻击方法，同时提出了防御策略，并通过改进提出了可扩展的PartCrop-v2。
...
TLDR: SpikeVideoFormer是一种高效的脉冲驱动视频Transformer，通过线性时间复杂度和优化的空间-时间注意力设计，在视频任务中表现出色，同时显著提升能效。


<details>
  <summary>Details</summary>
Motivation: 现有基于SNN的Transformer主要关注单图像任务，未能充分利用SNN在视频任务中的高效性。

Method: 设计了脉冲驱动的Hamming注意力（SDHA），并优化了空间-时间注意力方案，保持线性时间复杂度。

Result: 在视频分类、姿态跟踪和语义分割任务中达到SOTA性能，能效显著优于ANN方法。

Conclusion: SpikeVideoFormer展示了SNN在视频任务中的高效性和性能潜力。
...
<details>
  <summary>Details</summary>
Motivation: 开发学习型ISP时，获取像素对齐的配对数据成本高且困难，需解决此问题。

Method: 采用无配对训练方法，利用多损失函数和对抗训练，结合预训练网络特征图保持内容结构。

Result: 在Zurich RAW to RGB和Fujifilm UltraISP数据集上表现优异，评估指标显示高保真度。

Conclusion: 无配对学习策略在轻量级ISP训练中具有潜力，代码和模型已开源。
...
TLDR: ControlGS是一种3D高斯溅射优化方法，支持用户直观调整数量-质量权衡，适用于多样化场景。


<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏用户直观调整数量-质量权衡的能力，无法满足实际需求。

Method: 通过单次训练和用户指定的超参数，自动找到理想的权衡点。

Result: 在多样化场景中表现优异，渲染质量更高且高斯数量更少。

Conclusion: ControlGS提供无缝调整范围，支持广泛的实际应用。
...
TLDR: 本文研究了孤立手语识别（ISLR）中的两个问题：跨语言数据稀缺和相似手语的语义歧义，提出了Logos数据集和跨语言迁移学习方法，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 解决ISLR任务中数据稀缺和相似手语标注模糊的问题，推动跨语言手语识别的发展。

Method: 提出Logos数据集，采用跨语言迁移学习和多分类头联合训练方法，并标注视觉相似手语组。

Result: 在WLASL数据集上超越现有最佳结果，在AUTSL数据集上取得竞争性表现。

Conclusion: Logos数据集和跨语言迁移学习方法有效提升了ISLR任务性能，标注视觉相似手语组进一步优化了模型表现。
...
TLDR: UniEval是一个用于统一多模态模型评估的新框架，无需额外模型、图像或标注，简化了评估过程。


<details>
  <summary>Details</summary>
Motivation: 当前多模态模型的评估缺乏统一框架，存在冗余、依赖标注数据等问题。

Method: 提出UniEval框架，包含UniBench基准和UniScore指标，支持多样化的任务评估。

Result: UniBench更具挑战性，UniScore与人工评估一致，优于现有指标。

Conclusion: UniEval为多模态模型提供了高效、统一的评估方案，并揭示了新见解。
...
<details>
  <summary>Details</summary>
Motivation: 现有医学领域生成AI评估存在方法不一致、架构比较过时和评估标准脱节的问题，无法体现合成样本的实际临床价值。
...
TLDR: 提出一种双分支分类策略，增强深度网络对人脸融合攻击的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 人脸识别技术易受融合攻击威胁，需提升系统安全性。

Method: 通过双分支分类策略处理融合图像的标签模糊性，将其纳入训练过程。

Result: 在公开基准测试中验证了方法的有效性，提升了对抗融合攻击的能力。

Conclusion: 该方法通用性强，可集成至现有训练流程，提升分类性能。
...
TLDR: 提出了一种基于子模子集选择技术的检索框架增强方法，用于提升多图像问答任务中的检索性能。
...
TLDR: 论文提出了隐式视觉误解（IVM）问题，并引入注意力准确率作为评估指标，通过解耦视觉和文本模态揭示了模型对视觉输入的理解不足。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估答案正确性，忽略了模型是否真正理解视觉输入，因此需要更可靠的评估方法。

Method: 通过解耦视觉和文本模态的因果注意力模块，分析注意力分布，并提出注意力准确率指标和新基准。

Result: 注意力分布在深层网络逐渐聚焦于正确答案相关的图像，注意力准确率能可靠评估视觉理解。

Conclusion: 提出的方法不仅适用于多模态场景，还能扩展到单模态，具有通用性和泛化性。
...
TLDR: 研究了合成图像中可行性（feasibility）对CLIP分类器性能的影响，发现可行性对性能影响极小，且混合可行与不可行图像训练无显著差异。


<details>
  <summary>Details</summary>
Motivation: 探索合成图像中可行性是否影响CLIP分类器的性能，以优化合成数据的使用。

Method: 提出VariReal管道，通过最小编辑生成可行或不可行图像，并在三个细粒度数据集上测试LoRA微调CLIP的性能。

Result: 可行性对性能影响极小（<0.3%差异），且混合可行与不可行图像训练无显著影响。

Conclusion: 合成图像的可行性对CLIP分类器性能影响有限，无需严格区分可行与不可行数据。
...
TLDR: 论文提出利用代码作为跨模态对齐的监督信号，开发了图像到代码模型FigCodifier和数据集ImgCode-8.6M，并构建了高质量多模态数学指令数据集MM-MathInstruct-3M，最终训练出MathCoder-VL模型，在多项指标上达到开源SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有自然语言图像描述数据集忽视数学图形的细节，限制了多模态数学推理的发展。

Method: 利用代码作为监督信号开发图像到代码模型和数据集，并构建多模态数学指令数据集进行微调。
...
TLDR: ETT是一种端到端的视觉标记器调优方法，通过联合优化视觉标记化和目标自回归任务，显著提升多模态任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉标记器与下游任务解耦，导致标记化损失成为目标任务的表示瓶颈，影响性能。

Method: ETT利用视觉嵌入和代码本，联合优化重建和标题目标，无需修改原始代码本或架构。

Result: 实验表明，ETT在多模态理解和视觉生成任务中性能提升2-6%，同时保持重建能力。
...
TLDR: Prior Depth Anything框架结合不完整但精确的深度测量信息与相对但完整的几何结构，生成准确、密集且详细的深度图。


<details>
  <summary>Details</summary>
Motivation: 解决现有深度预测方法在精度和完整性上的不足，通过结合两种互补的深度数据源提升性能。

Method: 采用从粗到细的流程，先通过像素级度量对齐和距离感知加权预填充先验，再用条件单目深度估计模型细化噪声。

Result: 在7个真实数据集上表现出色，零样本泛化能力强，甚至超越任务专用方法。

Conclusion: 该框架灵活适应不同场景，支持测试时改进，为深度估计提供了高效且准确的解决方案。
...
TLDR: 3D-Fixup是一个基于3D先验的2D图像编辑框架，支持复杂编辑如平移和3D旋转，通过结合扩散模型和视频数据生成训练对，并利用Image-to-3D模型提供3D指导。


<details>
  <summary>Details</summary>
Motivation: 尽管扩散模型在图像先验建模方面取得进展，但基于单张图像的3D感知编辑仍具挑战性，因此提出3D-Fixup框架以解决这一问题。

Method: 结合扩散模型的生成能力和视频数据的物理动态，利用Image-to-3D模型提供3D指导，设计数据生成管道确保高质量训练。

Result: 3D-Fixup实现了复杂且身份一致的3D感知编辑，生成高质量结果，推动了扩散模型在图像处理中的应用。

Conclusion: 3D-Fixup通过整合3D先验和扩散模型，成功解决了复杂3D感知编辑的挑战，为图像处理提供了新方法。
...
### [229] [Quantized Approximate Signal Processing (QASP): Towards Homomorphic Encryption for audio](https://arxiv.org/abs/2505.10500)
*Tu Duyen Nguyen,Adrien Lesage,Clotilde Cantini,Rachid Riad*

Main category: eess.AS

TLDR: 论文提出了一种基于全同态加密（FHE）的安全音频处理管道，支持时间频率表示和CNN分类器的隐私计算，并通过近似STFT算法优化性能。


<details>
  <summary>Details</summary>
Motivation: 音频数据的被动收集引发隐私问题，FHE虽能保护隐私，但在音频处理中面临挑战，尤其是时间频率表示的安全计算。

Method: 引入安全管道，支持STFT、Mel滤波器组、MFCC和gammatone滤波器的隐私计算，并提出近似STFT算法以减少计算负担。

Result: 实验显示，近似STFT在隐私统计分析和CNN分类中显著降低错误率，并实现了基于原始音频的隐私分类。

Conclusion: 该方法为敏感音频数据的隐私保护提供了实用解决方案，并通过参数选择启发式方法提升了可操作性。
...
### [230] [Who Said What WSW 2.0? Enhanced Automated Analysis of Preschool Classroom Speech](https://arxiv.org/abs/2505.09972)
...
TLDR: WSW2.0是一个自动化框架，用于分析幼儿园课堂中的语音互动，结合wav2vec2和Whisper技术，提高了准确性和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 通过深度学习技术改进教育研究，提供更准确的幼儿园课堂语音特征分析，以支持早期语言发展和干预策略。

Method: 集成wav2vec2进行说话者分类，Whisper进行语音转录，并对比人工标注验证系统性能。

Result: 说话者分类F1分数0.845，转录错误率较低（教师0.119，儿童0.238），与专家标注的一致性高（ICC 0.64-0.98）。

Conclusion: WSW2.0展示了深度学习在教育研究中的潜力，能够大规模应用于真实课堂数据，支持语言发展研究。
...
### [231] [Neural models for prediction of spatially patterned phase transitions: methods and challenges](https://arxiv.org/abs/2505.09718)
*Daniel Dylewsky,Sonia Kéfi,Madhur Anand,Chris T. Bauch*

Main category: physics.comp-ph

TLDR: 本文探讨了神经网络在早期预警信号（EWS）检测中对空间模式相变的成功与不足，并揭示了模型泛化的新标准。


<details>
  <summary>Details</summary>
Motivation: 研究神经网络在空间模式相变中的EWS检测能力，填补高维相变中模型泛化研究的空白。

Method: 使用合成数据训练神经网络模型，并在几种典型测试系统中评估其性能，重点关注统计指标的表现和区分突变与连续相变的能力。

Result: 模型性能在训练和测试数据源互换时显著变化，揭示了模型泛化的新标准。

Conclusion: 神经网络EWS检测在空间模式相变中具有潜力，但模型泛化能力需进一步研究。
...
### [232] [Uncovering Magnetic Phases with Synthetic Data and Physics-Informed Training](https://arxiv.org/abs/2505.10393)
*Agustin Medina,Marcelo Arlego,Carlos A. Lamas*

Main category: cond-mat.str-el

TLDR: 论文研究了利用人工神经网络从合成数据中高效学习磁性相的方法，结合计算简单性和物理信息策略，探索了监督分类和无监督检测相变的互补方法。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于解决稀释伊辛模型缺乏精确解析解的问题，探索机器学习在复杂系统中识别相边界的潜力。

Method: 采用两种方法：监督分类（密集神经网络）和无监督检测（卷积自编码器），并结合物理信息指导（如对称性破缺特征增强）。

Result: 结果表明，合成数据和结构化训练方案能有效揭示物理相边界，验证了机器学习预测的准确性。

Conclusion: 该框架为传统方法提供了一种低成本、稳健的替代方案，适用于更广泛的凝聚态和统计物理问题。
...
### [233] [AI Greenferencing: Routing AI Inferencing to Green Modular Data Centers with Heron](https://arxiv.org/abs/2505.09989)
...
TLDR: 论文提出将AI负载部署到风电场中的模块化计算集群，利用Heron软件路由器优化跨风电场的电力互补性，提升AI计算效率。


<details>
  <summary>Details</summary>
Motivation: AI电力需求激增，而风电资源丰富但接入电网受限，因此探索将AI计算与风电直接结合的经济可行性。

Method: 提出部署策略和Heron软件路由器，利用风电互补性动态路由AI推理负载，避免电力波动影响。

Result: Heron在1周的实际数据测试中，将AI计算的总吞吐量提升了80%。

Conclusion: 将AI负载与风电结合是可行的，Heron能显著提升计算效率，为绿色AI提供新思路。
...
### [234] [KAITIAN: A Unified Communication Framework for Enabling Efficient Collaboration Across Heterogeneous Accelerators in Embodied AI Systems](https://arxiv.org/abs/2505.10183)
*Jieke Lin,Wanyu Wang,Longxiang Yin,Yinhe Han*

Main category: cs.DC

TLDR: KAITIAN是一个分布式通信框架，旨在解决异构加速器间的互操作性问题，提升资源利用率和性能。


<details>
  <summary>Details</summary>
Motivation: 异构加速器的增多导致专有通信库互操作性差，影响分布式AI任务的效率和性能。

Method: KAITIAN提供统一抽象层，结合专有库和通用协议，并采用负载自适应调度机制。

Result: 实验显示KAITIAN将训练时间缩短42%，通信开销仅2.8-4.3%，且保持模型精度。
...
### [235] [AI and Generative AI Transforming Disaster Management: A Survey of Damage Assessment and Response Techniques](https://arxiv.org/abs/2505.08202)
*Aman Raj,Lakshit Arora,Sanjay Surendranath Girija,Shashank Kapoor,Dipen Pradhan,Ankit Shetgaonkar*

Main category: cs.CY

TLDR: 本文综述了AI和GenAI在自然灾害损害评估中的应用，探讨了其优势、局限性及未来研究方向。
...
Result: AI和GenAI在灾害评估中展现出高效性和潜力，但也存在数据隐私、安全和伦理风险，以及GenAI被滥用的威胁。

Conclusion: 未来需开发安全、可靠且符合伦理的GenAI系统，以支持灾害管理。本文是首个全面综述GenAI在灾害评估与响应中应用的文献。
...
### [236] [Healthy Distrust in AI systems](https://arxiv.org/abs/2505.09747)
*Benjamin Paaßen,Suzana Alpsancar,Tobias Matzner,Ingrid Scharlau*

Main category: cs.CY

TLDR: 论文提出“健康的不信任”概念，强调在某些AI使用场景中，合理的不信任是必要的，以保护人类自主权。


<details>
  <summary>Details</summary>
Motivation: 当代AI研究强调设计可信赖的AI系统以促进人类信任，但忽视了社会背景下AI可能被滥用的情况。

Method: 通过跨学科（计算机科学、社会学、心理学等）分析信任与不信任的概念，填补现有研究空白。

Result: 提出“健康的不信任”作为尊重人类自主权的AI使用关键组成部分。

Conclusion: 合理的不信任是构建真正信任的基础，尤其在AI可能损害人类利益的场景中。
...
### [237] [Which Demographic Features Are Relevant for Individual Fairness Evaluation of U.S. Recidivism Risk Assessment Tools?](https://arxiv.org/abs/2505.09868)
*Tin Trung Nguyen,Jiannan Xu,Phuong-Anh Nguyen-Le,Jonathan Lazar,Donald Braman,Hal Daumé III,Zubin Jelveh*

Main category: cs.CY

TLDR: 论文探讨了如何在实际法规中应用个体公平性标准，并通过实验确定了在评估再犯风险评估工具时应考虑年龄和性别，但忽略种族。


<details>
  <summary>Details</summary>
Motivation: 尽管个体公平性标准在美国宪法中有基础，但尚未在州或联邦法规中具体实施。研究旨在填补这一空白。

Method: 通过人类受试者实验，评估了哪些人口特征与个体公平性评估相关。

Result: 分析表明，个体相似性函数应考虑年龄和性别，但忽略种族。

Conclusion: 研究为个体公平性标准的实际应用提供了具体指导，强调了年龄和性别的重要性，同时排除了种族因素。
...
### [238] [Predictive Models for Chronic Heart Failure](https://arxiv.org/abs/2505.09619)
...
TLDR: 本文提出了一种基于机器学习的预测模型，用于识别心力衰竭高风险患者，采用集成学习方法，结合临床和超声心动图特征，表现出较高的敏感性和中等准确性。


<details>
  <summary>Details</summary>
Motivation: 慢性心力衰竭管理面临挑战，需要持续监测和个性化治疗策略，机器学习模型可帮助早期识别高风险患者。

Method: 采用改进的堆叠技术，结合两个专门模型（临床和超声心动图特征）和一个元模型，形成集成学习模型。

Result: 模型敏感性达95%，准确性84%，优于基线模型，适合高风险患者识别。

Conclusion: 机器学习风险分层模型可作为决策支持工具，助力早期干预和个性化管理，具有潜在应用价值。
...
### [239] [Differentiable Quantum Architecture Search in Quantum-Enhanced Neural Network Parameter Generation](https://arxiv.org/abs/2505.09653)
*Samuel Yen-Chi Chen,Chen-Yu Liu,Kuan-Cheng Chen,Wei-Jia Huang,Yen-Jui Chang,Wei-Hao Huang*

Main category: quant-ph

TLDR: 论文提出了一种自动化方法，通过可微分优化联合优化量子电路参数和架构参数，解决了量子神经网络（QNNs）设计中依赖专家知识的问题，并在多个任务中验证了其性能。


<details>
  <summary>Details</summary>
Motivation: 量子机器学习（QML）中的变分量子电路（VQCs）在推理时依赖量子硬件，限制了其广泛应用。现有方法如Quantum-Train（QT）框架虽能生成经典神经网络参数，但设计有效的量子电路架构仍需专业知识。

Method: 提出了一种基于可微分优化的自动化方法，通过自动微分端到端联合优化电路参数和架构参数。

Result: 在分类、时间序列预测和强化学习任务中，该方法匹配或优于人工设计的QNN架构。

Conclusion: 该工作为设计可生成经典神经网络参数的QNN提供了一种可扩展且自动化的途径，适用于多样化应用。
...
### [240] [Quantum Computing and AI: Perspectives on Advanced Automation in Science and Engineering](https://arxiv.org/abs/2505.10012)
*Tadashi Kadowaki*

Main category: quant-ph

TLDR: 论文提出Quantum CAE框架，结合量子计算与AI，用于工程设计的模拟、优化和机器学习，并通过案例研究展示其应用。


<details>
  <summary>Details</summary>
Motivation: 探讨AI与量子计算如何加速科研自动化，并借鉴传统CAE实践，提出Quantum CAE框架以推动工程设计的革新。

Method: 引入Quantum CAE框架，利用量子算法进行工程设计中的模拟、优化和机器学习，并通过案例研究验证其可行性。

Result: 案例研究表明Quantum CAE在组合优化问题中具有实际应用潜力。

Conclusion: Quantum CAE结合AI与量子计算，为自动化发现和创新提供了变革性前景，同时引发了对人机协作动态的思考。
...
### [241] [Role of scrambling and noise in temporal information processing with quantum systems](https://arxiv.org/abs/2505.10080)
*Weijie Xiong,Zoë Holmes,Armando Angrisani,Yudai Suzuki,Thiparat Chotibut,Supanut Thanasilp*

Main category: quant-ph

TLDR: 论文研究了量子系统在时间信息处理中的表现，重点分析了量子储备池处理框架的可扩展性和记忆保持能力。在无噪声和有噪声条件下，证明了记忆衰减的指数性质。


<details>
  <summary>Details</summary>
Motivation: 量子系统在时间信息处理中的潜力尚未被充分理解，尤其是在可扩展性和记忆保持方面的理论分析不足。

Method: 采用量子储备池处理框架，模拟高阶酉设计的量子系统，分析其在无噪声和有噪声条件下的表现。

Result: 无噪声条件下，测量读数随储备池规模指数集中，但迭代不恶化；记忆随规模和迭代指数衰减。有噪声条件下，记忆同样指数衰减。

Conclusion: 量子储备池处理框架在时间任务中表现有限，需权衡规模与泛化能力，噪声进一步加剧记忆衰减。
...
### [242] [Deconstructing Subset Construction -- Reducing While Determinizing](https://arxiv.org/abs/2505.10319)
*John Nicol,Markus Frohme*

Main category: cs.FL

TLDR: 提出了一种新的NFA规范化方法，通过引入中间最小化步骤和等价注册表来优化性能，适用于经典子集构造和Brzozowski方法。
...
Method: 使用中间最小化步骤和等价注册表管理等价状态信息，结合凸性闭包或模拟等优化技术。

Result: 在真实世界的自动序列案例中表现优异，显著改善了最坏情况下的性能。
...
### [243] [Virtual Dosimetrists: A Radiotherapy Training "Flight Simulator"](https://arxiv.org/abs/2505.09796)
...
TLDR: 开发了'Virtual Dosimetrist'模型，通过自然语言提示生成和优化放疗计划，解决了当前临床教育模式的不足。


<details>
  <summary>Details</summary>
Motivation: 当前临床教育模式无法满足放疗计划质量教育的需求，缺乏灵活性和多样化的示例。

Method: 结合剂量分布预测与自然语言处理，开发'Virtual Dosimetrist'模型，生成次优训练计划并支持自然语言优化。

Result: 模型能快速、准确地生成和修改剂量分布，资源需求低。

Conclusion: 该模型为放疗计划质量教育提供了高效、灵活的解决方案，填补了当前临床模式的不足。
...
TLDR: RXTX算法通过结合机器学习和组合优化，减少了矩阵乘法的计算量，比现有技术节省5%的乘法和加法运算。
...
Method: 结合机器学习搜索方法和组合优化技术，设计新的算法RXTX。

Result: RXTX比现有技术节省5%的乘法和加法运算，对小规模矩阵也能实现加速。

Conclusion: RXTX是一种高效的矩阵乘法算法，适用于不同规模的矩阵计算。
...
TLDR: 提出一种算法，用于采样低秩随机矩阵Q，以最优方式近似目标矩阵P，满足无偏性、秩约束和最小化Frobenius范数误差。


<details>
  <summary>Details</summary>
Motivation: 解决在低秩约束下如何最优近似目标矩阵的问题，同时保证无偏性和最小误差。

Method: 算法基于矩阵P的奇异分量，类似于向量高效无偏稀疏化问题的解法。

Result: 证明了算法的最优性，匹配了现有下界的误差。

Conclusion: 该算法在低秩近似问题中实现了最优性能。
...
### [246] [A Fine-Grained Complexity View on Propositional Abduction -- Algorithms and Lower Bounds](https://arxiv.org/abs/2505.10201)
*Victor Lagerkvist,Mohamed Maizia,Johannes Schmidt*

Main category: cs.CC

TLDR: 该论文分析了非单调推理（如溯因推理）的复杂性，重点关注知识库中变量数量作为参数的影响，并首次展示了在Σ²P完全问题中超越穷举搜索的成果。


<details>
  <summary>Details</summary>
Motivation: 填补单调推理与非单调推理之间的复杂性研究空白，尤其是针对溯因推理的复杂性分析。

Method: 以知识库中的变量数量n为参数，分析不同复杂性类别（如Σ²P、NP和coNP完全）的溯因推理问题。

Result: 获得了多个积极结果，首次在Σ²P完全问题中实现了超越穷举搜索的突破，并提供了基于强指数时间假设的下界。

Conclusion: 该研究为非单调推理的复杂性分析提供了新视角，并展示了参数化方法在解决高复杂性推理问题中的潜力。
...
### [247] [Inferring entropy production in many-body systems using nonequilibrium MaxEnt](https://arxiv.org/abs/2505.10444)
*Miguel Aguilera,Sosuke Ito,Artemy Kolchinsky*

Main category: cond-mat.stat-mech

TLDR: 提出了一种推断高维随机系统中熵产生（EP）的方法，适用于多体系统和非马尔可夫系统。通过非平衡最大熵原理和凸对偶性，仅需轨迹观测样本即可推断轨迹级EP和平均EP的下界。


<details>
  <summary>Details</summary>
Motivation: 标准EP估计方法在高维系统中因计算和统计限制难以应用，需一种更高效的方法。

Method: 利用非平衡最大熵原理和凸对偶性，通过轨迹观测样本推断EP，无需重构高维概率分布或速率矩阵。

Result: 方法在1000自旋无序非平衡自旋模型和大型神经尖峰数据集上表现良好。

Conclusion: 该方法提供了一种高效且直观的EP推断方式，适用于复杂系统。
...
### [248] [LanTu: Dynamics-Enhanced Deep Learning for Eddy-Resolving Ocean Forecasting](https://arxiv.org/abs/2505.10191)
*Qingyu Zheng,Qi Shao,Guijun Han,Wei Li,Hong Li,Xuan Wang*

Main category: physics.ao-ph

TLDR: LanTu是一种基于动力学增强深度学习的区域涡旋分辨海洋预报系统，优于现有数值和AI预报系统。


<details>
  <summary>Details</summary>
Motivation: 中尺度涡旋对海洋能量级联有重要影响，但传统数值模型计算成本高，AI模型在涡旋预报中面临挑战。

Method: 开发了LanTu系统，结合跨尺度相互作用和多尺度物理约束，优化涡旋动力学预报。

Result: LanTu在温度、盐度、海平面异常和洋流预报上优于现有系统，预报时效超过10天。
...
### [249] [Tales of the 2025 Los Angeles Fire: Hotwash for Public Health Concerns in Reddit via LLM-Enhanced Topic Modeling](https://arxiv.org/abs/2505.09665)
*Sulong Zhou,Qunying Huang,Shaoheng Zhou,Yun Hang,Xinyue Ye,Aodong Mei,Kathryn Phung,Yuning Ye,Uma Govindswamy,Zehan Li*

Main category: cs.SI

TLDR: 该研究通过分析2025年洛杉矶野火期间的Reddit讨论，利用主题建模和分层框架，揭示了公众对灾害的情境意识和危机叙事，为灾害响应和公共卫生策略提供依据。


<details>
  <summary>Details</summary>
Motivation: 理解受灾人群在野火危机中的感知和反应，以提供及时和同理心的灾害响应。

Method: 收集385篇帖子和114,879条评论，采用主题建模方法（结合LLMs和HITL）和分层框架（SA和CN）进行分析。

Result: SA类别与火灾进展紧密相关，CN类别中60%为悲伤信号，40%为心理健康风险。

Conclusion: 研究提供了首个标注的社交媒体数据集，并提出了一种可扩展的多层框架，为灾害响应和公共卫生策略提供了新见解。
...
### [250] [Advancing Community Detection with Graph Convolutional Neural Networks: Bridging Topological and Attributive Cohesion](https://arxiv.org/abs/2505.10197)
*Anjali de Silva,Gang Chen,Hui Ma,Seyed Mohammad Nekooei,Xingquan Zuo*

Main category: cs.SI

TLDR: TAS-Com提出了一种基于拓扑和属性相似性的社区检测方法，通过结合Leiden算法优化模块化和人类标注的社区，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有GCN方法在最大化模块化时易陷入次优解，而直接使用人类标注的社区可能破坏拓扑连贯性。

Method: TAS-Com引入了一种新的损失函数，利用Leiden算法优化模块化，并修正人类标注社区以确保连通性。

Result: 在多个基准网络上，TAS-Com显著优于其他先进算法。

Conclusion: TAS-Com在模块化和人类标注社区之间取得了理想平衡，是一种高效的社区检测方法。
...
### [251] [On Measuring Intrinsic Causal Attributions in Deep Neural Networks](https://arxiv.org/abs/2505.09660)
*Saptarshi Saha,Dhruv Vansraj Rathore,Soumadeep Saha,Utpal Garain,David Doermann*

Main category: stat.ML

TLDR: 该论文提出了一种量化神经网络中特征因果影响的新方法，重点关注内在因果贡献（ICC），并通过实验验证其优于现有全局解释技术。


<details>
  <summary>Details</summary>
Motivation: 研究神经网络中特征的因果影响量化问题，现有方法通常关注直接、间接和总因果效应，但缺乏对内在因果贡献的深入探讨。

Method: 将神经网络视为结构因果模型（SCMs），提出了一种可识别的生成后验框架来量化ICC，并将其与Sobol指数关联。

Result: 在合成和真实数据集上的实验表明，ICC生成的解释比现有全局解释技术更直观可靠。

Conclusion: ICC为神经网络特征因果影响的量化提供了更有效的方法，具有更高的解释性和可靠性。
...
### [252] [Learning Multi-Attribute Differential Graphs with Non-Convex Penalties](https://arxiv.org/abs/2505.09748)
*Jitendra K Tugnait*

Main category: stat.ML

TLDR: 论文提出了一种基于非凸惩罚的D-trace损失函数方法，用于估计两个多属性高斯图模型的差异，并提供了理论分析和数值实验验证。
...
### [253] [LatticeVision: Image to Image Networks for Modeling Non-Stationary Spatial Data](https://arxiv.org/abs/2505.09803)
...
<details>
  <summary>Details</summary>
Motivation: 在科学和工业应用中，获取大量空间分布数据（如气候模型）成本高昂，而统计模拟提供了一种替代方案。传统MLE方法计算量大，尤其是对非平稳大尺度场。

Method: 将SAR模型的参数和输入的空间场视为图像，利用图像到图像（I2I）网络进行参数估计。

Result: 该方法在非平稳SAR模型的参数估计中实现了更快、更准确的结果，且适用于前所未有的复杂模型。

Conclusion: I2I网络为SAR模型参数估计提供了一种高效替代方案，尤其适用于计算成本高的场景。
...
### [254] [A Scalable Gradient-Based Optimization Framework for Sparse Minimum-Variance Portfolio Selection](https://arxiv.org/abs/2505.10099)
*Sarat Moka,Matias Quiroz,Vali Asimit,Samuel Muller*

Main category: stat.ML

TLDR: 论文提出了一种基于梯度的快速稀疏投资组合选择方法，通过布尔松弛将组合问题转化为连续优化任务，解决了传统混合整数二次规划方法计算成本高的问题。
...
Method: 通过布尔松弛将组合问题转化为连续优化任务，使用可调参数将辅助目标从凸函数转变为凹函数，逐步逼近稀疏二进制解。
...
### [255] [Path Gradients after Flow Matching](https://arxiv.org/abs/2505.10139)
*Lorenz Vaitl,Leon Klein*

Main category: stat.ML

TLDR: 结合路径梯度微调Flow Matching训练的CNFs，显著提升分子系统采样效率。


<details>
  <summary>Details</summary>
Motivation: 探索路径梯度在已知目标能量的情况下对CNFs的微调效果，以提高采样效率。

Method: 使用路径梯度微调Flow Matching训练的CNFs，测量流轨迹长度变化。

Result: 采样效率提升三倍，流结构保持良好。
...
### [256] [One-Stage Top-$k$ Learning-to-Defer: Score-Based Surrogates with Theoretical Guarantees](https://arxiv.org/abs/2505.10160)
*Yannis Montreuil,Axel Carlier,Lai Xing Ng,Wei Tsang Ooi*

Main category: stat.ML

TLDR: 提出了一种一阶段Top-$k$学习延迟框架，统一预测和延迟，通过共享的基于分数的模型选择$k$个最具成本效益的实体或专家。


<details>
  <summary>Details</summary>
Motivation: 现有的一阶段L2D方法仅限于延迟到单个专家，无法适应多实体场景。

Method: 定义了一个成本敏感损失，并推导出一个与$k$无关的凸代理，支持跨Top-$k$场景的泛化。

Result: 在CIFAR-10和SVHN上的实验表明，Top-$k$方法优于Top-1延迟，Top-$k(x)$通过动态调整实现了更好的精度-成本权衡。

Conclusion: 该框架在多实体延迟场景中表现优异，且具有理论保证和实际优势。
...
### [257] [Efficient MCMC Sampling with Expensive-to-Compute and Irregular Likelihoods](https://arxiv.org/abs/2505.10448)
*Conor Rosato,Harvinder Lehal,Simon Maskell,Lee Devlin,Malcolm Strens*

Main category: stat.ML

TLDR: 论文探讨了在似然函数不规则且计算昂贵时，如何通过子集采样算法和自适应控制器改进MCMC采样效率。


<details>
  <summary>Details</summary>
Motivation: 在似然函数不规则且计算成本高的情况下，传统MCMC采样效率低下，需要更高效的采样方法。

Method: 引入数据驱动的代理模型替代泰勒展开，并设计计算成本感知的自适应控制器，改进HINTS算法。

Result: 改进后的HINTS算法在固定计算预算下实现了最佳采样误差。

Conclusion: 子集采样和数据驱动代理模型的结合，通过分层延迟接受实现了高效且精确的采样。
...
### [258] [FlowVAT: Normalizing Flow Variational Inference with Affine-Invariant Tempering](https://arxiv.org/abs/2505.10466)
*Juehang Qin,Shixiao Liang,Christopher Tunnell*

Main category: stat.ML

TLDR: FlowVAT是一种基于条件调温的归一化流变分推断方法，解决了多模态高维后验分布中的模式塌缩问题，无需调温计划和超参数优化。


<details>
  <summary>Details</summary>
Motivation: 多模态高维后验分布对变分推断提出了挑战，传统退火方法需要调温计划和超参数优化，未能实现真正的黑盒推断。

Method: FlowVAT通过同时对基分布和目标分布进行调温，并利用归一化流的温度条件化，训练单一流表示不同温度下的后验分布。

Result: 在2、10和20维多模态分布实验中，FlowVAT优于传统和自适应退火方法，找到更多模式且ELBO值更高。

Conclusion: FlowVAT无需调温计划和超参数优化，为复杂后验分布提供了全自动黑盒变分推断的解决方案。
...
### [259] [Batched Nonparametric Bandits via k-Nearest Neighbor UCB](https://arxiv.org/abs/2505.10498)
*Sakshi Arya*

Main category: stat.ML

TLDR: 提出了一种非参数化的批处理上下文赌博算法BaNk-UCB，结合k-NN回归和UCB原则，适用于有限反馈场景。


<details>
  <summary>Details</summary>
Motivation: 医学和营销等领域中在线反馈有限，需要一种适应性强且易于实现的非参数化方法。

Method: 结合自适应k-NN回归和UCB原则，利用局部几何估计奖励并平衡探索与利用。

Result: 在标准Lipschitz平滑和边界假设下，提供了接近最优的遗憾保证，并在实验中优于基于分箱的基线方法。

Conclusion: BaNk-UCB是一种高效的非参数化算法，适用于有限反馈的批处理决策场景。
...
### [260] [Risk-Aware Safe Reinforcement Learning for Control of Stochastic Linear Systems](https://arxiv.org/abs/2505.09734)
*Babak Esmaeili,Nariman Niknejad,Hamidreza Modares*

Main category: eess.SY

TLDR: 提出了一种风险感知的安全强化学习控制设计，结合RL控制器和安全控制器，通过数据驱动方法优化安全性和性能。


<details>
  <summary>Details</summary>
Motivation: 解决传统安全强化学习中依赖高精度模型和短视干预的问题，提供高效且安全的控制方案。

Method: 学习分段仿射控制器，通过决策变量和线性规划优化安全违规概率，结合RL与安全控制器。

Result: 减少了数据需求和安全违规的方差，验证了方法的有效性。

Conclusion: 该方法在噪声环境中实现了安全与性能的平衡，并通过仿真验证了理论结果。
...
### [261] [A Hybrid Strategy for Aggregated Probabilistic Forecasting and Energy Trading in HEFTCom2024](https://arxiv.org/abs/2505.10367)
*Chuanqing Pu,Feilong Fan,Nengling Tai,Songyuan Liu,Jinming Yu*

Main category: eess.SY

TLDR: 团队GEB在IEEE HEFTCom2024竞赛中表现出色，提出了一种结合多种方法的解决方案，包括基于堆叠的风电预测、太阳能后处理模型、概率聚合方法和随机交易策略，以实现高精度概率预测和最大化交易收益。


<details>
  <summary>Details</summary>
Motivation: 未来能源系统中，准确的概率能量预测和应对多样化的不确定性是常见挑战。团队GEB旨在通过综合方法解决这些问题，并在竞赛中验证其有效性。

Method: 方法包括：(1)基于堆叠的风电预测，(2)太阳能后处理模型，(3)概率聚合方法，(4)随机交易策略，并探索端到端学习的潜力。

Result: 团队在竞赛中表现优异，风电预测准确，交易收益显著，并通过案例研究验证了方法的有效性。

Conclusion: 该解决方案为能源预测和交易提供了实用工具，代码开源以促进进一步研究和应用。
...
### [262] [Inconsistency Handling in DatalogMTL](https://arxiv.org/abs/2505.10394)
*Meghyn Bienvenu,Camille Bourgaux,Atefe Khodadaditaghanaki*

Main category: cs.LO

TLDR: 论文探讨了DatalogMTL中不一致性处理的问题，定义了冲突和修复的概念，并分析了其属性和数据复杂性。
...
Method: 定义了冲突（最小不一致解释）和修复（恢复一致性的方法）的概念，并研究了其属性和语义。
...
### [263] [VRSplat: Fast and Robust Gaussian Splatting for Virtual Reality](https://arxiv.org/abs/2505.10144)
*Xuechang Tu,Lukas Radl,Michael Steiner,Markus Steinberger,Bernhard Kerbl,Fernando de la Torre*

Main category: cs.GR

TLDR: VRSplat通过结合和改进3DGS技术，解决了VR中的时间伪影、投影失真和帧率问题，实现了72+ FPS的高性能表现。


<details>
  <summary>Details</summary>
Motivation: 3DGS在VR中面临时间伪影、投影失真和帧率下降等问题，这些问题在头戴显示器（HMDs）中尤为突出。

Method: 结合Mini-Splatting、StopThePop和Optimal Projection技术，改进3DGS光栅化器，提出高效的注视点光栅化器，并通过微调优化高斯参数。

Result: 用户研究表明，VRSplat优于其他配置，能够消除伪影和浮动物，实现72+ FPS。
...
### [264] [Style Customization of Text-to-Vector Generation with Image Diffusion Priors](https://arxiv.org/abs/2505.10558)
*Peiying Zhang,Nanxuan Zhao,Jing Liao*

Main category: cs.GR

TLDR: 论文提出了一种两阶段的SVG风格定制生成方法，结合了前馈T2V模型和T2I先验，解决了现有方法在风格定制和结构一致性上的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有T2V生成方法缺乏风格定制能力，无法满足实际应用中一致的视觉风格需求。

Method: 两阶段方法：1）训练路径级表示的T2V扩散模型以保持结构一致性；2）通过蒸馏定制T2I模型实现风格定制。

Result: 实验验证了方法能高效生成高质量、多样化的定制风格SVG。

Conclusion: 提出的方法有效解决了SVG风格定制问题，结合了结构一致性和风格多样性。
...
### [265] [A Survey on Large Language Models in Multimodal Recommender Systems](https://arxiv.org/abs/2505.09777)
*Alejo Lopez-Avila,Jinhua Du*

Main category: cs.IR

TLDR: 综述探讨了大型语言模型（LLMs）在多模态推荐系统（MRS）中的应用，分析了其优势、挑战及未来方向。


<details>
  <summary>Details</summary>
Motivation: 利用LLMs的语义推理和动态处理能力提升MRS性能，同时解决其可扩展性和可访问性问题。

Method: 通过分类整合模式、借鉴相关推荐技术，并总结评估指标和数据集。

Result: 提出了新的分类法，明确了LLMs在MRS中的角色，为未来研究提供方向。

Conclusion: LLMs为MRS带来新机遇，但仍需解决技术挑战，未来研究应关注其实际应用。
...
### [266] [Boosting Text-to-Chart Retrieval through Training with Synthesized Semantic Insights](https://arxiv.org/abs/2505.10043)
*Yifan Wu,Lutao Yan,Yizhang Zhu,Yinan Mei,Jiannan Wang,Nan Tang,Yuyu Luo*

Main category: cs.IR

TLDR: 论文提出了一种自动生成图表语义洞察的训练数据开发流程，并基于此训练了CLIP模型ChartFinder，显著提升了文本到图表的检索性能。
...
Method: 开发了一个自动生成层次化语义洞察的流程，并训练了CLIP模型ChartFinder，结合视觉和语义信息。

Result: ChartFinder在精确查询中NDCG@10达到66.9%，比现有模型高11.58%；在模糊查询中平均提升5%。

Conclusion: 提出的方法通过丰富语义洞察显著提升了文本到图表检索的性能，并建立了首个基准CRBench。
...
### [267] [Do LLMs Memorize Recommendation Datasets? A Preliminary Study on MovieLens-1M](https://arxiv.org/abs/2505.10212)
*Dario Di Palma,Felice Antonio Merra,Maurizio Sfilio,Vito Walter Anelli,Fedelucio Narducci,Tommaso Di Noia*

Main category: cs.IR

TLDR: 研究探讨大型语言模型（LLMs）是否记忆了公开推荐数据集（如MovieLens-1M），并分析了记忆对推荐性能的影响。
...
Method: 通过定义数据集记忆（包括项目属性、用户画像和用户-项目交互的可检索性），分析记忆对推荐性能的影响，并比较不同模型家族和规模的记忆差异。
...
### [268] [ILIF: Temporal Inhibitory Leaky Integrate-and-Fire Neuron for Overactivation in Spiking Neural Networks](https://arxiv.org/abs/2505.10371)
*Kai Sun,Peibo Duan,Levin Kuhlmann,Beilun Wang,Bin Zhang*

Main category: cs.NE

TLDR: 论文提出了一种基于生物抑制机制的ILIF神经元模型，解决了SNN训练中gamma困境（过大导致过度激活，过小导致梯度消失）的问题。


<details>
  <summary>Details</summary>
Motivation: SNN训练中，替代梯度支持宽度gamma的选择存在困境：过大导致神经元过度激活，增加能耗；过小导致梯度消失，削弱时间依赖性。

Method: 提出了一种时间抑制性漏积分发放（ILIF）神经元模型，通过引入相互连接的抑制单元来调节膜电位和电流，有效缓解过度激活并保持梯度传播。

Result: 理论分析和多数据集实验表明，ILIF模型能有效克服gamma困境，降低发放率、稳定训练并提高准确性。

Conclusion: ILIF模型在提升SNN能效和性能方面具有潜力，代码已开源。
...
### [269] [Pure Component Property Estimation Framework Using Explainable Machine Learning Methods](https://arxiv.org/abs/2505.09783)
*Jianfeng Jiao,Xi Gao,Jie Li*

Main category: stat.AP

TLDR: 提出了一种基于可解释机器学习的纯组分物性预测框架，通过分子表示方法和特征分析提升预测精度和可解释性。
...
Method: 采用基于连通矩阵的分子表示方法生成特征，结合随机森林进行特征排序和池化，引入调整R2评估特征贡献，使用ANN和GPR模型验证。

Result: 预测结果显著优于传统方法，测试集均方根误差降低达83.8%，特征池化将特征数从13316降至100。

Conclusion: 框架可行，为混合物组分重构和过程集成建模提供了坚实基础。
...
TLDR: DiD-BCF是一种新颖的非参数模型，解决了DiD估计中的关键挑战，如交错采用和异质性处理效应，提供统一的框架估计ATE、GATE和CATE。


<details>
  <summary>Details</summary>
Motivation: 解决传统DiD方法在复杂面板数据设置中的局限性，特别是交错采用和异质性处理效应问题。

Method: 提出基于平行趋势假设（PTA）的重新参数化方法，增强估计的准确性和稳定性。

Result: 模拟实验显示DiD-BCF在非线性、选择偏差和效应异质性下优于现有基准。应用于美国最低工资政策，揭示了传统方法忽略的条件处理效应异质性。
...
### [271] [Estimating the number of household TV profiles based in customer behaviour using Gaussian mixture model averaging](https://arxiv.org/abs/2505.10279)
*Gabriel R. Palma,Sally McClean,Brahim Allan,Zeeshan Tariq,Rafael A. Moral*

Main category: stat.ME

TLDR: 论文提出了一种基于高斯混合模型和贝叶斯随机游走的新框架，用于解决家庭电视观看行为中多用户识别和个性化推荐的问题。


<details>
  <summary>Details</summary>
Motivation: 电视用户面临众多频道和点播服务，个性化推荐对提升用户体验至关重要，但多用户观看行为难以识别和建模。

Method: 采用高斯混合模型平均估计家庭电视观看者数量，结合贝叶斯随机游走模型引入不确定性。

Result: 在50万条真实用户数据上验证，框架能有效估计家庭观看者数量及其特征变化，并量化不确定性。

Conclusion: 该框架为家庭多用户观看行为的建模和个性化推荐提供了可行解决方案。
...
### [272] [The Tangent Space Attack](https://arxiv.org/abs/2505.10184)
*Axel Lemoine*

Main category: cs.IT

TLDR: 提出了一种新方法，用于在满足特定条件时从任意生成矩阵中恢复通用交替码的代数结构，并讨论了其对McEliece密码系统安全性的挑战。


<details>
  <summary>Details</summary>
Motivation: 研究交替码的代数结构及其对密码系统安全性的影响，特别是McEliece密码系统的实例化。

Method: 通过定义线性码的二次壳（quadratic hull），利用其几何性质揭示码的内部结构，并结合Weil限制概念，展示其对广义Reed-Solomon码的继承性。

Result: 在交替码速率足够高时，提出了多项式时间算法，可从交替码恢复其基础广义Reed-Solomon码，从而实现对McEliece密码系统的高效密钥恢复攻击。
...
### [273] [Large Wireless Localization Model (LWLM): A Foundation Model for Positioning in 6G Networks](https://arxiv.org/abs/2505.10134)
*Guangjin Pan,Kaixuan Huang,Hui Chen,Shunqing Zhang,Christian Häger,Henk Wymeersch*

Main category: eess.SP

TLDR: 论文提出了一种基于基础模型的无线定位解决方案（LWLM），通过自监督学习框架优化三个互补目标，显著提升了定位精度和泛化能力。
...
Method: 设计了一个自监督学习框架，联合优化空间-频率掩码信道建模（SF-MCM）、域变换不变性（DTI）和位置不变对比学习（PICL）。

Result: LWLM在所有定位任务中均优于基于模型和监督学习的基线方法，尤其在无预训练的Transformer模型上提升了26.0%--87.5%。

Conclusion: LWLM作为一种无线定位的基础模型，展现了强大的泛化能力和潜力。
...
### [274] [Neurophysiologically Realistic Environment for Comparing Adaptive Deep Brain Stimulation Algorithms in Parkinson Disease](https://arxiv.org/abs/2505.09624)
*Ekaterina Kuzmina,Dmitrii Kriukov,Mikhail Lebedev,Dmitry V. Dylov*

Main category: q-bio.NC

TLDR: 该论文提出了首个神经生理学上真实的基准，用于比较帕金森病（PD）的合成模型和控制算法，并引入了一个结构化框架以优化自适应深部脑刺激（aDBS）控制策略。


<details>
  <summary>Details</summary>
Motivation: 现有aDBS技术因侵入性设备限制数据收集，导致需要合成模型优化控制算法，但缺乏真实神经生理学基准。

Method: 论文方法涵盖基底节回路动态、病理振荡及15种先前忽视的生理特征，通过β波段活动和反馈建模为时空分布特征，并构建结构化框架用于深度强化学习算法训练与评估。

Result: 提出了一个全面的神经生理学基准，为aDBS控制策略优化提供了新工具，并邀请机器学习社区参与智能神经刺激接口研究。

Conclusion: 该研究为aDBS控制策略优化提供了首个真实基准，推动了智能神经刺激接口领域的发展。
...
### [275] [Temporal Interception and Present Reconstruction: A Cognitive-Signal Model for Human and AI Decision Making](https://arxiv.org/abs/2505.09646)
*Carmel Mary Esther A*

Main category: q-bio.NC

TLDR: 提出了一种新理论模型，解释人类心智与人工智能如何通过减少感知延迟实现实时意识。


<details>
  <summary>Details</summary>
Motivation: 探索如何从被动感知转向与近未来的意识交互，结合宇宙信号延迟、神经反应时间和古老的静止认知状态。
...
### [276] [A Computational Approach to Epilepsy Treatment: An AI-optimized Global Natural Product Prescription System](https://arxiv.org/abs/2505.09643)
*Zhixuan Wang*

Main category: q-bio.NC

TLDR: AI驱动的计算方法优化癫痫草药治疗，结合机器学习、贝叶斯优化和RCT荟萃分析，显著减少癫痫发作频率。
...
Method: 结合机器学习算法、贝叶斯优化和RCT荟萃分析，分析1872种天然化合物和48项RCT数据。

Result: 识别17种高效草药，验证试验显示AI优化处方比传统方法减少28.5%癫痫发作。

Conclusion: AI驱动的草药优化系统为癫痫治疗提供更有效、个性化的解决方案。
...
TLDR: 提出ManipBench基准，用于评估视觉语言模型（VLMs）在机器人低层推理能力中的表现，测试了33种代表性VLMs，发现其性能差异显著且与真实任务相关，但仍与人类水平有差距。
...
Method: 设计ManipBench基准，测试VLMs在物体交互和可变形物体操作等维度的表现，评估33种VLMs。

Result: VLMs性能在不同任务中差异显著，与真实任务表现相关，但仍远低于人类水平。

Conclusion: ManipBench为评估VLMs低层推理能力提供了有效工具，揭示了其当前局限性。
...
TLDR: FlowDreamer提出了一种基于3D场景流的RGB-D世界模型，通过显式运动表示提升机器人操作中的视觉预测性能。


<details>
  <summary>Details</summary>
Motivation: 研究如何通过显式运动表示（3D场景流）改进机器人操作中的视觉世界模型，以更准确地预测未来视觉观察。

Method: FlowDreamer采用U-Net预测3D场景流，并结合扩散模型生成未来帧，实现端到端训练。
...
TLDR: 提出了一种结合多机器人任务分配（MRTA）和碰撞避免的新框架，通过空间聚类划分工作区，显著提升效率和安全性。


<details>
  <summary>Details</summary>
Motivation: 解决工业环境中多机器人任务分配与碰撞避免的联合问题，提高计算效率和操作安全性。
...
TLDR: EmbodiedMAE是一种统一的三维多模态表示方法，用于机器人操作，通过结合RGB、深度和点云数据，显著提升了训练效率和性能。
...
TLDR: 论文提出了一种在非结构化环境中通过机器人操纵器重新定位障碍物而非避障的移动导航方法，以提高多智能体长期路径效率。


<details>
  <summary>Details</summary>
Motivation: 传统避障方法在重复路径上效率低下，长期依赖主动路径规划系统。

Method: 结合外感知和内感知反馈评估障碍物的可推动性，利用机器人操纵器重新定位障碍物。

Result: 初步实验表明，该方法能减少多智能体在长期使用路径上的时间消耗。

Conclusion: 该方法适用于需要自主基础设施开发的环境（如月球或火星表面），提高了导航效率。
...
TLDR: IN-RIL结合模仿学习（IL）和强化学习（RL），通过交替更新策略，提升机器人策略学习的稳定性和样本效率。


<details>
  <summary>Details</summary>
Motivation: 现有IL预训练后RL微调的方法在微调阶段存在不稳定和样本效率低的问题。

Method: 提出IN-RIL，交替进行RL和IL更新，并通过梯度分离机制避免目标冲突。

Result: 在14个机器人任务中显著提升样本效率和性能，例如Robomimic Transport任务成功率从12%提升至88%。
...
TLDR: 评估基于深度强化学习（DRL）的自主水面车辆（ASV）在外部干扰下的鲁棒性，通过域随机化训练并在真实环境中测试。
...
Method: 使用域随机化训练DRL代理，并在模拟和真实环境中测试其应对不对称拖曳和偏心载荷等干扰的能力。
...
TLDR: 论文提出了一种名为KCAC的框架，通过跨任务课程学习将知识转移系统化地整合到强化学习中，显著提高了训练效率和任务成功率。


<details>
  <summary>Details</summary>
Motivation: 强化学习在机器人操作中表现出潜力，但存在样本效率低和可解释性差的问题，限制了其实际应用。通过知识捕获、适应和组合，可以提升智能体对多样化工作场景的理解和适应能力。

Method: 提出KCAC框架，通过重新设计奖励函数、定义子任务和实施结构化跨任务课程学习，优化学习效率。

Result: KCAC方法在CausalWorld基准测试中，训练时间减少40%，任务成功率提高10%。

Conclusion: KCAC框架为基于课程的强化学习提供了设计指导，提升了机器人学习的效率和效果。
...
TLDR: FORTRESS框架通过结合多模态推理和动态感知规划，实时生成安全后备策略，避免OOD故障，无需人工干预。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖手动定义的后备策略，无法实现通用的语义安全运动规划。

Method: FORTRESS使用多模态推理器识别目标并预测故障模式，实时合成后备计划并避免不安全区域。

Result: 在合成基准和真实机器人数据上，FORTRESS在安全分类准确性和系统安全性方面表现优于现有方法。

Conclusion: FORTRESS通过开放世界推理和动态规划的结合，显著提升了机器人系统的安全性和规划成功率。
...
Result: 用户研究显示，系统保持99.84%的显著特征可见性，姿态误差为4.36±2.11度和1.95±5.66毫米，计算效率高。

Conclusion: AutoCam为RAMIS中的多摄像头可视化方法奠定了基础，证明辅助摄像头可自主控制以跟踪显著特征。
...
TLDR: 提出了一种利用堆外探测器实时测量数据重建核反应堆中子通量空间分布的方法，基于基尔霍夫-亥姆霍兹方程作为数学框架。


<details>
  <summary>Details</summary>
Motivation: 解决复杂异质域（如核反应堆）中格林函数数值逼近的挑战，验证数据驱动方法的适定性。

Method: 通过单速中子扩散模型推导K-H方程，建立格林函数的对称性，并设计算法实现中子通量重建。

Result: 证明了从采样数据推断的格林函数的存在性和唯一性，验证了方法的可靠性。

Conclusion: 该方法为核反应堆中子通量分布的重建提供了可靠且数学严谨的解决方案。
...
### [288] [SpecWav-Attack: Leveraging Spectrogram Resizing and Wav2Vec 2.0 for Attacking Anonymized Speech](https://arxiv.org/abs/2505.09616)
*Yuqi Li,Yuanzhong Zheng,Zhongtian Guo,Yaoxuan Wang,Jianjun Yin,Haojun Fei*

Main category: cs.SD

TLDR: SpecWav-Attack是一种用于检测匿名语音中说话者的对抗模型，利用Wav2Vec2提取特征，结合频谱图调整和增量训练提升性能，在librispeech数据集上表现优于传统攻击方法。
...
Conclusion: SpecWav-Attack揭示了匿名语音系统的脆弱性，呼吁更强的防御措施，并参考ICASSP 2025 Attacker Challenge进行基准测试。
...
### [289] [Introducing voice timbre attribute detection](https://arxiv.org/abs/2505.09661)
*Jinghao He,Zhengyan Sheng,Liping Chen,Kong Aik Lee,Zhen-Hua Ling*

Main category: cs.SD

TLDR: 论文提出了一种称为声音音色属性检测（vTAD）的任务，通过感官属性描述语音信号中的音色，并比较语音对在特定音色描述符中的强度。基于说话人嵌入的框架在VCTK-RVA数据集上测试，结果显示ECAPA-TDNN在已知说话人场景表现更好，而FACodec在未知说话人场景中泛化能力更强。


<details>
  <summary>Details</summary>
Motivation: 研究语音信号中音色的感知特性，提出一种新的任务vTAD，以更系统地描述和比较语音音色。

Method: 使用说话人嵌入（ECAPA-TDNN和FACodec）构建框架，在VCTK-RVA数据集上比较语音对的音色强度。

Result: ECAPA-TDNN在已知说话人场景表现更优，FACodec在未知说话人场景中泛化能力更强。

Conclusion: vTAD任务和提出的框架为语音音色分析提供了有效工具，不同编码器在不同场景下各有优势。
...
### [290] [LAV: Audio-Driven Dynamic Visual Generation with Neural Compression and StyleGAN2](https://arxiv.org/abs/2505.10101)
...
Result: 保留了语义丰富性，实现细腻且语义一致的音频-视觉转换。
...
### [291] [Detecting Musical Deepfakes](https://arxiv.org/abs/2505.09633)
*Nick Sunday*

Main category: cs.SD

TLDR: 该研究通过分类音频（AI生成或人工）检测AI生成的歌曲，使用FakeMusicCaps数据集，并模拟对抗条件。采用卷积神经网络分析Mel频谱图，同时探讨了TTM平台的伦理和社会影响。


<details>
  <summary>Details</summary>
Motivation: 随着Text-to-Music平台的普及，音乐创作变得便捷，但也带来新挑战。研究旨在检测AI生成的歌曲，保护艺术家并发挥生成式AI的潜力。

Method: 使用FakeMusicCaps数据集，应用节奏拉伸和音高变换模拟对抗条件，生成Mel频谱图，并用卷积神经网络训练和评估。

Result: 通过技术手段成功分类AI生成和人工音乐，同时强调了检测系统的重要性。

Conclusion: 精心设计的检测系统对保护艺术家和发挥生成式AI的积极潜力至关重要。
...
### [292] [Learning Nonlinear Dynamics in Physical Modelling Synthesis using Neural Ordinary Differential Equations](https://arxiv.org/abs/2505.10511)
*Victor Zheleznov,Stefan Bilbao,Alec Wright,Simon King*

Main category: cs.SD

TLDR: 论文探讨了如何将模态分解与神经常微分方程结合，用于建模分布式音乐系统，并通过非线性横向弦的合成数据验证了模型的有效性。


<details>
  <summary>Details</summary>
Motivation: 研究旨在解决高振幅振动下弦的几何非线性效应（如音高滑移和亮度依赖）的建模问题，结合机器学习的优势。

Method: 利用模态分解将系统转化为非线性常微分方程组，并引入神经常微分方程处理非线性动态行为，同时保留物理参数的可访问性。

Result: 模型成功训练并再现了非线性弦的动态行为，生成了合成数据并提供了声音示例。

Conclusion: 该方法为分布式音乐系统的非线性建模提供了一种有效途径，且物理参数易于访问，无需网络架构中的参数编码器。
...
### [293] [Trustless Autonomy: Understanding Motivations, Benefits and Governance Dilemma in Self-Sovereign Decentralized AI Agents](https://arxiv.org/abs/2505.09757)
*Botao Amber Hu,Yuhan Liu,Helena Rong*

Main category: cs.HC

TLDR: 论文探讨了结合LLM的去中心化AI代理（DeAgents）的动机、优势及治理困境，通过访谈研究填补了信任与不可靠自主性之间的研究空白。


<details>
  <summary>Details</summary>
Motivation: 解决集中式AI系统的信任问题，同时探索去中心化技术（如区块链和TEE）如何赋予AI代理自主权。

Method: 通过访谈DeAgents的利益相关者（专家、创始人和开发者）来研究其动机、优势和治理挑战。

Result: 研究结果将为未来DeAgents系统和协议设计提供指导，并推动AI系统治理的讨论。

Conclusion: DeAgents在消除集中控制的同时，需平衡信任与自主性的矛盾，未来需进一步探索治理机制。
...
### [294] [Visual Feedback of Pattern Separability Improves Myoelectric Decoding Performance of Upper Limb Prostheses](https://arxiv.org/abs/2505.09819)
...
TLDR: 论文提出了一种名为Reviewer的3D视觉界面，通过实时反馈优化肌电假肢的控制性能，显著提高了用户的操作效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有肌电假肢的控制系统依赖用户通过试错调整解码器边界，导致认知负担高且效果不稳定。

Method: 研究通过10次实验，比较了使用Reviewer界面与传统虚拟手臂可视化训练的效果，评估了任务完成率、路径效率等指标。

Result: 使用Reviewer的参与者表现出更高的任务完成率、更少的过冲和更好的路径效率。

Conclusion: 3D视觉反馈通过结构化训练显著提升了新手操作者的控制性能，减少了试错调整的依赖。
...
### [295] [SOS: A Shuffle Order Strategy for Data Augmentation in Industrial Human Activity Recognition](https://arxiv.org/abs/2505.10312)
*Anh Tuan Ha,Hoang Khang Phan,Thai Minh Tien Ngo,Anh Phan Truong,Nhat Tan Le*

Main category: cs.HC

TLDR: 论文提出了一种通过深度学习方法（注意力自编码器和条件生成对抗网络）生成高质量HAR数据的方法，并通过随机序列策略解决了数据异构性问题，显著提升了分类性能。


<details>
  <summary>Details</summary>
Motivation: 在HAR领域，获取高质量且多样化的数据成本高且困难，数据异构性也是一个关键挑战。

Method: 使用注意力自编码器和条件生成对抗网络生成数据，并通过随机序列策略打乱数据分布。
...
### [296] [AI LEGO: Scaffolding Cross-Functional Collaboration in Industrial Responsible AI Practices during Early Design Stages](https://arxiv.org/abs/2505.10300)
*Muzhe Wu,Yanzhi Zhao,Shuyi Han,Michael Xieyang Liu,Hong Shen*

Main category: cs.HC

TLDR: 论文探讨了跨职能团队在AI开发中知识传递的挑战，并提出了一种名为AI LEGO的工具，以支持早期设计阶段的伦理评估和危害识别。


<details>
  <summary>Details</summary>
Motivation: 解决跨职能团队中技术专家与非技术角色之间知识传递的困难，以促进早期AI设计中的伦理评估和危害识别。

Method: 通过文献综述和与8名从业者的共同设计研究，开发了AI LEGO原型工具，并在18名跨职能从业者中进行了测试。

Result: AI LEGO显著提高了危害识别的数量和可能性，其模块化结构和角色模拟使危害识别更易进行。

Conclusion: AI LEGO为早期AI设计提供了更清晰、协作性更强的负责任AI实践方法。
...
### [297] [Evaluating Large Language Models for the Generation of Unit Tests with Equivalence Partitions and Boundary Values](https://arxiv.org/abs/2505.09830)
*Martín Rodríguez,Gustavo Rossi,Alejandro Fernandez*

Main category: cs.SE

TLDR: 研究评估了大型语言模型（LLMs）在自动生成测试用例中的潜力，并与手动测试进行了比较。结果表明，LLMs的效果依赖于精心设计的提示、稳健的实现和精确的需求。


<details>
  <summary>Details</summary>
Motivation: 探讨LLMs在自动生成测试用例中的潜力，以解决程序员忽视单元测试设计的问题。

Method: 开发了优化的提示，整合代码和需求，覆盖关键测试场景（如等价类和边界值），并通过定量指标和手动定性分析比较LLMs与人工测试的效果。

Result: LLMs的效果取决于提示设计、实现质量和需求精确性，虽然灵活且有潜力，但仍需人工监督。
...
### [298] [Are Sparse Autoencoders Useful for Java Function Bug Detection?](https://arxiv.org/abs/2505.10375)
*Rui Melo,Claudia Mamede,Andre Catarino,Rui Abreu,Henrique Lopes Cardoso*

Main category: cs.SE

TLDR: 论文探讨了稀疏自编码器（SAE）作为轻量级、可解释的替代方案，用于检测Java函数中的漏洞，无需微调预训练的大型语言模型（LLM）。
...
Method: 研究使用稀疏自编码器（SAE）从GPT-2 Small和Gemma 2B的内部表示中提取特征，检测漏洞。

Result: SAE在漏洞检测中F1分数高达89%，优于微调的Transformer编码器基线。

Conclusion: SAE可直接从预训练LLM的内部表示中检测漏洞，无需微调或任务特定监督，首次提供了实证支持。
...
### [299] [Are Large Language Models Robust in Understanding Code Against Semantics-Preserving Mutations?](https://arxiv.org/abs/2505.10443)
*Pedro Orvalho,Marta Kwiatkowska*

Main category: cs.SE

TLDR: 评估大型语言模型（LLMs）在Python程序推理中的表现，发现部分模型基于错误逻辑得出正确答案，且对代码变异的鲁棒性有限。


<details>
  <summary>Details</summary>
Motivation: 理解LLMs的推理能力和鲁棒性对其在编程任务中的可靠应用至关重要，现有研究多关注预测准确性而非推理逻辑。

Method: 应用五种语义保留的代码变异（如变量重命名、循环转换等），评估六种LLMs的推理能力，并结合人类专家分析。

Result: 部分模型（如Llama3.2）在61%的情况下基于错误逻辑得出正确答案，且对代码变异的预测不稳定。

Conclusion: LLMs在代码理解中的推理能力和鲁棒性仍需改进，需进一步研究其逻辑可靠性。
...
### [300] [ImplicitStainer: Data-Efficient Medical Image Translation for Virtual Antibody-based Tissue Staining Using Local Implicit Functions](https://arxiv.org/abs/2505.09831)
*Tushar Kataria,Beatrice Knudsen,Shireen Y. Elhabian*

Main category: eess.IV

TLDR: ImplicitStainer是一种基于局部隐式函数的新方法，用于从H&E染色图像生成虚拟IHC染色，解决了传统GAN和扩散模型的数据需求问题。


<details>
  <summary>Details</summary>
Motivation: H&E染色无法提供所有诊断信息，而IHC染色耗时且难以获取。虚拟染色技术通过深度学习提供替代方案，但现有方法数据需求高。
...
### [301] [Ordered-subsets Multi-diffusion Model for Sparse-view CT Reconstruction](https://arxiv.org/abs/2505.09985)
*Pengfei Yu,Bin Huang,Minghui Zhang,Weiwen Wu,Shaoyu Wang,Qiegen Liu*

Main category: eess.IV

TLDR: 提出了一种名为OSMM的有序子集多扩散模型，用于稀疏视图CT重建，通过分块学习和全局约束提升图像细节和噪声鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统扩散模型在处理未处理的稀疏视图CT投影数据时学习效率低且难以恢复精细细节。

Method: 将CT投影数据分成等量子集，使用多子集扩散模型（MSDM）独立学习每个子集，并结合完整数据的全局扩散模型（OWDM）作为约束。

Result: OSMM在图像质量和噪声鲁棒性上优于传统扩散模型。

Conclusion: OSMM为稀疏视图CT重建提供了一种高效且通用的解决方案。
...
### [302] [Visual Fidelity Index for Generative Semantic Communications with Critical Information Embedding](https://arxiv.org/abs/2505.10405)
...
Method: 开发了CIE框架，提取文本提示和语义关键特征；提出语义过滤方法选择关键特征；设计GVIF指标量化视觉质量；优化系统自适应控制特征量和压缩率。

Result: GVIF指标对视觉保真度敏感，与PSNR和关键信息量相关；优化系统在PSNR和FID分数上优于基准方案。

Conclusion: 混合Gen-SemCom系统通过CIE框架和GVIF指标，显著提升了生成图像的视觉质量和通信效率。
...
### [303] [HWA-UNETR: Hierarchical Window Aggregate UNETR for 3D Multimodal Gastric Lesion Segmentation](https://arxiv.org/abs/2505.10464)
...
<details>
  <summary>Details</summary>
Motivation: 胃癌多模态图像分割面临数据集稀缺和模态对齐问题，导致资源消耗大且准确性下降。

Method: 提出HWA-UNETR框架，采用HWA块和三元融合机制，动态对齐多模态特征并建模长距离依赖。

Result: 在GCM 2025和BraTS 2021数据集上验证，Dice分数提升1.68%，且鲁棒性强。

Conclusion: HWA-UNETR和GCM 2025数据集有效解决了多模态分割问题，性能显著提升。
...
### [304] [Multi-contrast laser endoscopy for in vivo gastrointestinal imaging](https://arxiv.org/abs/2505.10492)
*Taylor L. Bobrow,Mayank Golhar,Suchapa Arayakarnkul,Anthony A. Song,Saowanee Ngamruengphong,Nicholas J. Durr*

Main category: eess.IV

TLDR: 多对比激光内窥镜（MLE）通过可调谐光谱、相干和定向照明，显著提升胃肠道疾病检测的对比度和色彩差异。


<details>
  <summary>Details</summary>
Motivation: 白光内窥镜在检测胃肠道疾病时，由于特征对比度低，许多临床相关病例难以发现。

Method: MLE平台结合多光谱漫反射、激光散斑对比成像和光度立体技术，增强组织对比度、量化血流并表征黏膜地形。

Result: MLE在31个息肉样本中表现出对比度提升约3倍，色彩差异提升5倍。

Conclusion: MLE作为一种新型工具，有望改善胃肠道成像，并易于融入临床环境。
...
### [305] [Ontology-Based Structuring and Analysis of North Macedonian Public Procurement Contracts](https://arxiv.org/abs/2505.09798)
*Bojan Ristov,Stefan Eftimov,Milena Trajanoska,Dimitar Trajanov*

Main category: cs.DB

TLDR: 该研究提出了一种将结构化采购数据转换为语义知识图谱的方法，利用本体建模和自动化数据转换技术，提升数据的可访问性和可解释性，并结合机器学习预测模型，为北马其顿的公共采购提供透明度和分析支持。


<details>
  <summary>Details</summary>
Motivation: 传统采购数据以表格形式存储，限制了分析潜力和透明度，因此需要一种更高效的方法来提升数据的可用性和分析能力。

Method: 采用本体建模和自动化数据转换技术，将结构化数据转换为语义知识图谱，并结合RDF和SPARQL查询以及机器学习预测模型。

Result: 系统提升了采购数据的可访问性和可解释性，支持复杂语义查询和高级分析，并提供了采购趋势和风险评估的预测能力。

Conclusion: 该研究为公共采购智能领域提供了透明度和分析支持，有助于基于证据的决策制定和深入分析北马其顿的采购活动。
...
### [306] [Multi-Agent Path Finding For Large Agents Is Intractable](https://arxiv.org/abs/2505.10387)
*Artem Agafonov,Konstantin Yakovlev*

Main category: cs.MA

TLDR: 论文研究了多智能体路径规划（MAPF）问题中考虑智能体尺寸时的复杂性，证明了该问题是NP-hard。


<details>
  <summary>Details</summary>
Motivation: 在实际应用中（如机器人领域），智能体的尺寸不可忽略，但现有研究未解决其复杂性。

Method: 通过将经典的3SAT问题（NP完全问题）归约到MAPF问题，构造特定图例证明其NP-hard性。

Result: 证明了考虑智能体尺寸的MAPF问题是NP-hard，不存在多项式时间算法（除非P=NP）。

Conclusion: 首次确立了考虑智能体尺寸的MAPF问题的NP-hard性，为后续研究提供了理论基础。
...
TLDR: 本文提出了一种基于dApps和LibIQ库的实时RF频谱分类方法，解决了O-RAN架构中RIC的延迟和数据隐私问题，实现了高效的信号分类。


<details>
  <summary>Details</summary>
Motivation: O-RAN架构中的RIC在实时监控和数据隐私方面存在局限性，阻碍了如波束成形和频谱分类等应用。本文旨在通过dApps和LibIQ库解决这些问题。

Method: 利用LibIQ库处理I/Q样本，通过CNN进行信号分类，并在5G部署和OTA测试平台上创建数据集进行验证。

Result: 在实时分析中，模型对信号类型的分类平均准确率达到97.8%。

Conclusion: LibIQ和数据集将作为公开框架发布，为实时频谱分析提供了高效解决方案。
...
### [308] [Contextual Phenotyping of Pediatric Sepsis Cohort Using Large Language Models](https://arxiv.org/abs/2505.09805)
...
TLDR: 该研究评估了基于大型语言模型（LLM）的聚类方法在儿科败血症数据集上的表现，发现其优于传统方法，能更好地捕捉上下文信息并识别具有不同特征的亚组。


<details>
  <summary>Details</summary>
Motivation: 传统聚类方法在处理高维、异构的医疗数据时效果不佳，缺乏上下文理解。本研究旨在探索LLM在聚类任务中的潜力。

Method: 使用LLAMA 3.1 8B、DeepSeek-R1-Distill-Llama-8B和Stella-En-400M-V5模型生成嵌入，并应用K-means聚类。与传统方法（如K-Medoids）进行比较。

Result: Stella-En-400M-V5的轮廓分数最高（0.86）。LLAMA 3.1 8B在聚类目标下表现更好，能识别具有不同特征的亚组。LLM方法优于传统技术。
...
### [309] [Generative diffusion model surrogates for mechanistic agent-based biological models](https://arxiv.org/abs/2505.09630)
*Tien Comlekoglu,J. Quetzalcóatl Toledo-Marín,Douglas W. DeSimone,Shayn M. Peirce,Geoffrey Fox,James A. Glazier*

Main category: q-bio.QM

TLDR: 利用去噪扩散概率模型（DDPM）训练生成式AI替代模型，加速细胞-波特模型（CPM）的计算，减少计算时间约22倍。


<details>
  <summary>Details</summary>
Motivation: CPM在大规模时空尺度下计算成本高，需开发替代模型以加速复杂生物系统的模拟。

Method: 使用DDPM训练生成式AI替代模型，结合图像分类器辅助选择和验证模型。

Result: 替代模型能提前生成20,000时间步的配置，计算时间减少22倍。
