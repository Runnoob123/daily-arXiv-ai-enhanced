...
### [1] [The Hashed Fractal Key Recovery (HFKR) Problem: From Symbolic Path Inversion to Post-Quantum Cryptographic Keys](https://arxiv.org/abs/2506.04383)
*Mohamed Aly Bouke*
...
Motivation: 传统密码系统依赖代数问题，易受量子攻击和结构分析，因此需要一种不依赖代数假设的安全密钥生成方法。

Method: 基于符号路径反转问题（SPIP），利用压缩符号轨迹生成密钥，并通过哈希混淆增强熵。通过分形维数、路径几何等量化分形行为。

Result: 实验显示SHA3-512和SHAKE256能有效放大符号分歧，达到理想熵值，而BLAKE3表现较弱。分形维数稳定在1.06，表明高复杂性。

Conclusion: HFKR通过分形和哈希熵放大的协同作用，提供了一种轻量级、无结构的安全密钥生成方案，适用于对抗性环境。
...
<details>
  <summary>Details</summary>
Motivation: 现有攻击缺乏隐蔽性，容易被检测和缓解，因此需要更隐蔽的攻击方式和防御策略。

Method: 提出了一种基于注意力模式的Normalized Passage Attention Score和Attention-Variance Filter算法，用于识别和过滤毒化段落。

Result: 该方法显著提升了防御效果，准确率比基线防御提高了约20%，但适应性攻击仍能达到35%的成功率。
...
<details>
  <summary>Details</summary>
Motivation: 解决在敏感医疗数据上微调LLM时的隐私风险和数据泄漏问题。

Method: 使用50,232份放射学报告，通过差分隐私低秩适应（DP-LoRA）在高中等隐私预算下微调LLM，评估BERT-medium、BERT-small和ALBERT-base三种模型的加权F1分数。

Result: 实验显示隐私与性能之间存在权衡，中等隐私保证下，模型在MIMIC-CXR和CT-RATE数据集上的加权F1分数分别为0.88和0.59，接近非隐私基线。

Conclusion: 差分隐私微调结合LoRA能有效保护隐私并实现放射学报告的多异常分类。
...
Result: 实验表明，BESA在面对先进防御时，显著提升了替代编码器准确率，最高达24.63%。
...
<details>
  <summary>Details</summary>
Motivation: 现有蜜罐密码技术无法防止攻击者通过凭证填充（credential stuffing）绕过检测，且站点缺乏参与监控的动机。

Method: 提出一种站点间交换监控服务的算法，并通过模型检查分析其效果。

Result: 算法能提高站点检测自身入侵的能力，且在实际数据集上验证了其可扩展性和实用性。

Conclusion: 该算法为蜜罐密码监控生态系统的部署提供了可行方案，并量化了参数对检测效果的影响。
...
<details>
  <summary>Details</summary>
Motivation: 标准PSI协议易受数据完整性攻击，恶意参与者可能获取额外信息或误导其他方，因此需要增强数据完整性保护。

Method: 整合Merkle树与volePSI（两方）和mPSI（多方）协议，构建认证PSI方案。

Result: 两方方案通信复杂度为O(nλ+n log n)，与未认证方案相当；多方方案为O(nκ+n log n)，因Merkle树证明引入额外开销。

Conclusion: 认证PSI方案在保证数据完整性的同时，成本高于未认证方案，未来可探索替代认证模块以优化性能。
...
<details>
  <summary>Details</summary>
Motivation: 扩展LINCOS协议，解决其在单个QKD网络被攻破时的安全漏洞，并实现在多个网络中的分布式存储。

Method: 采用分层秘密共享技术，将秘密分布在多个QKD网络中，确保完美安全性，并支持无需重建整个秘密的更新。

Result: MULTISS比LINCOS更安全，且在QKD网络被攻破时仍能保持安全性。

Conclusion: MULTISS是一种更安全的分布式存储协议，适用于长期存储需求。
...
Method: 通过结合上下文学习和RAG，系统收集和管理工具及API规范，存储在向量数据库中，并将高级缓解策略分解为离散任务，再转化为可操作的API调用。
...
Method: 集成两种合规机制（清白证明和祖先承诺追踪系统），使用布隆过滤器优化UTXO链状态管理。

Result: 性能高效，私人转账和合规检查在消费级笔记本电脑上几秒内完成，链上成本低。

Conclusion: 该方案在隐私保护、合规性和效率方面表现出色，适用于数字支付场景。
...
<details>
  <summary>Details</summary>
Motivation: 漏洞报告通常缺乏PoC利用，导致修复和测试困难。PoCGen旨在通过自动化解决这一问题。

Method: 结合LLM、静态和动态分析技术，自动生成和验证PoC利用。

Result: 在SecBench.js数据集上成功率为77%，新数据集为39%，显著优于基线。
...
TLDR: 论文提出了一种轻量级的客户端查询混淆策略，通过随机生成多语言搜索查询来干扰用户画像，保护隐私。


<details>
  <summary>Details</summary>
Motivation: 个性化搜索虽提升相关性，但带来隐私风险和过滤气泡问题，需一种不依赖基础设施修改的隐私保护方法。

Method: 在Seznam.cz搜索引擎上进行实验，评估不同语言配置和比例下混淆查询对用户画像的影响。

Result: 混淆查询显著改变搜索引擎识别的用户兴趣，同时保持搜索结果稳定，证明其能有效防止准确画像。

Conclusion: 查询混淆是一种可行的隐私保护机制，用户可自主保护搜索行为，无需修改现有基础设施。
...
<details>
  <summary>Details</summary>
Motivation: 数据密集型的机器学习挑战与V2X和5G通信技术的结合，为车载入侵检测领域部署FL解决方案提供了可能性和合理性。

Method: 提出了一种基于LSTM自编码器的FL实现，用于车载CAN网络的入侵检测，并与集中式版本进行比较。

Result: FL实现展示了可行的检测效率和通信开销，证明了其作为解决方案的潜力。

Conclusion: FL在车载入侵检测领域是一种可行的解决方案，尤其适合数据密集型场景。
...
<details>
  <summary>Details</summary>
Motivation: 传统安全检测方法存在数据收集不足、资源消耗大和误报率高的问题。

Method: FEAD采用攻击模型驱动方法提取关键监控项，高效任务分解优化资源分配，局部感知异常分析提高检测精度。

Result: FEAD的F1分数比现有方案高8.23%，开销仅为5.4%。
...
<details>
  <summary>Details</summary>
Motivation: 现有公开数据集范围有限，无法全面评估恶意软件分类器，尤其是缺乏针对逃避检测的恶意文件的数据。

Method: 通过合作创建EMBER2024数据集，包含320万文件的哈希、元数据、特征向量和标签，支持七种分类任务，并引入挑战集和EMBER特征版本3。

Result: EMBER2024数据集首次包含逃避检测的恶意文件，支持多种分类任务，并引入新特征类型。

Conclusion: EMBER2024的发布旨在促进恶意软件研究的可重复性和新研究方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有工具在审计序列模型隐私泄露风险时存在不足，作者认为这是由于假设不匹配导致的。

Method: 改进了一种最先进的成员推理攻击方法，显式建模序列内相关性。
...
<details>
  <summary>Details</summary>
Motivation: 研究Arm CCA时缺乏硬件支持，导致性能原型开发困难、重复工作和性能比较不一致。

Method: 通过系统调整软件栈（包括引导程序、固件、虚拟机和内核），OpenCCA模拟CCA操作以进行性能评估。

Result: 在价格250美元的Rockchip Armv8.2开发板上验证了OpenCCA的有效性，展示了典型生命周期测量和案例研究。
...
<details>
  <summary>Details</summary>
Motivation: 本地部署的LLMs易受滥用，现有云服务安全措施无效，需开发新方法。

Method: 采用神经元级加密、任务特定神经元提取、分层策略树和混合加密框架。

Result: SECNEURON将未授权任务准确率降至25%以下，授权任务损失控制在2%内，减少数据泄露。
...
<details>
  <summary>Details</summary>
Motivation: 当前PPA设计缺乏隐私预算管理的系统性方法，导致关键设计决策的不确定性。

Method: Big Bird引入基于资源隔离原则的全局预算系统，并通过配额预算和批量调度算法实现隐私保护。

Result: 在Firefox中实现并评估Big Bird，证明其在对抗性环境中的韧性和有效性。

Conclusion: Big Bird为PPA提供了稳健的隐私保护基础。
...
<details>
  <summary>Details</summary>
Motivation: 研究大语言模型（LLMs）在微调过程中安全护栏退化的原因，特别是上游安全对齐数据与下游任务数据相似性的影响。

Method: 通过实验分析上游安全对齐数据集与下游微调任务之间的表示相似性，评估其对安全护栏的影响。

Result: 高相似性会显著削弱安全护栏，而低相似性能将有害性评分降低高达10.33%。
...
<details>
  <summary>Details</summary>
Motivation: 基于概念的模型在关键领域（如医疗诊断和金融风险预测）中表现出色，但其结构、训练过程和对抗性漏洞等问题限制了其可靠性和泛化能力。本文旨在全面调查这些问题。

Method: 通过综述文献，总结了基于概念的模型在监督和无监督范式中的常见挑战及缓解策略，并分析了提高其可靠性的最新进展。

Result: 研究发现概念泄漏、表示纠缠和对扰动的鲁棒性有限等问题，同时探讨了人类干预的有效性。

Conclusion: 本文为基于概念的模型的风险和局限性提供了全面分析，并指出了未来研究的潜在方向。
...
Method: 通过MLNs将一阶逻辑与概率推理结合，基于人类可理解的概念定义加权逻辑约束，从而为输入分配概率。

Result: 实验表明，MLNs能显著提升多种现有OOD检测器的性能，同时保持计算效率。

Conclusion: MLNs为OOD检测提供了可解释且高效的增强方法，并通过简单算法学习逻辑约束，展示了其有效性。
...
<details>
  <summary>Details</summary>
Motivation: 当前方法在预测混凝土徐变时未能充分建模时间序列特性，限制了预测精度。

Method: 将混凝土徐变预测转化为自回归序列建模任务，采用三重注意力机制（时间、特征、批次）捕获长期依赖关系。

Result: 在160天的标准化数据集上，模型表现优异（MAPE 1.63%，R2 0.999），显著优于传统方法。

Conclusion: 该研究验证了Transformer在材料科学中的潜力，为工程设计和结构行为预测提供了数据驱动的新思路。
...
<details>
  <summary>Details</summary>
Motivation: 适应不断变化的安全政策对LLM进行微调成本高且不切实际，需要更灵活的控制方法。

Method: 利用类别特定的引导向量和无监督方法，实现精确安全控制，同时保持文本质量和主题相关性。

Result: 方法在多种LLM、数据集和风险类别中有效，能精确控制并避免全面拒绝。

Conclusion: SafeSteer提供了一种简单有效的安全控制方法，优于复杂技术。
...
<details>
  <summary>Details</summary>
Motivation: 传统火灾预测方法过于简化问题，未考虑局部差异，无法满足消防员等终端用户的需求。

Method: 提出了一种基于部门上下文的风险评估方法，利用先进AI模型在法国全国范围内进行预测。

Result: 首次在法国全国范围内建立了AI基准测试，提供了更具体、可操作的预测结果。

Conclusion: 该方法改进了传统二元分类的局限性，未来需进一步探索和完善。
...
Result: 高质量响应频率增加梯度信号，改善优化环境，提升策略学习效果。

Conclusion: 理论支持在线DPO框架的实践应用。
...
<details>
  <summary>Details</summary>
Motivation: 复合洪水预测因气象、水文和海洋因素的复杂交互而具有挑战性，现有数据集不完整且传统方法效率低。

Method: 引入SF2Bench数据集，整合潮汐、降雨、地下水和人类管理活动数据，并评估六种机器学习模型。

Result: 实验验证了不同关键特征对洪水预测的影响，并展示了各模型在捕捉时空依赖性上的差异。

Conclusion: SF2Bench数据集和机器学习方法为复合洪水预测提供了新思路，不同模型各有优势。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的符号回归方法过度依赖内部先验，缺乏数据理解和系统反思。

Method: DrSR通过分析数据结构和建立反馈循环，结合数据理解和生成反思。

Result: 在多个学科数据集中，DrSR显著提高了有效方程率，并在准确性、泛化性和搜索效率上优于其他方法。

Conclusion: DrSR在科学方程发现中具有潜力。
...
<details>
  <summary>Details</summary>
Motivation: 解决卫星数据传输带宽受限的问题，通过实时处理和优先传输自然灾害相关数据，提高应急响应效率。

Method: 使用基于忆阻器的物理感知神经形态网络（PANN），动态更新权重，生成物理约束的动态特征，通过距离度量检测变化。

Result: PANN在自然灾害检测任务中表现优于或与现有AI模型相当，且无需训练，适合资源受限的星载处理。

Conclusion: PANN为星载数据处理提供了一种高效、低资源消耗的解决方案，特别适用于自然灾害监测。
...
Method: 提出BAT方法，包括两个数学命题验证其有效性，并设计ALBAT算法实现数据选择和训练增强。

Result: ALBAT在数据稀缺的个人化和语言生成任务中高效提升了适应训练效果。

Conclusion: BAT和ALBAT为适应训练提供了有效解决方案，尤其在数据稀缺场景下表现优异。
...
Method: 比较了IWL（参数学习）和ICL（上下文学习）在传递推理任务中的表现，并分析了其机制。

Result: IWL能自然推广传递推理，而ICL需要预训练才能实现；预训练后的ICL模型表现出类似人类和动物的推理特征。

Conclusion: 预训练具有底层结构的任务有助于模型发展支持上下文关系推理的表征。
...
Method: 通过理论分析，提出了一种名为LDPTRLQ的算法，有效结合Lyapunov漂移加惩罚算法与强化学习，平衡短期与长期优化目标。

Result: 仿真实验表明，LDPTRLQ在性能和稳定性上优于基线方法，验证了理论推导的有效性。

Conclusion: LDPTRLQ算法在解决IoT优化问题中表现出色，为相关领域提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 临床试验成本高、耗时长，准确预测结果可降低成本并加速药物研发。现有深度学习模型缺乏可解释性和实用性。

Method: AutoCT框架结合大型语言模型和经典机器学习，自主生成、评估和优化表格特征，使用蒙特卡洛树搜索迭代优化预测性能。

Result: AutoCT在有限的自优化迭代中，性能达到或优于现有最佳方法。
...
<details>
  <summary>Details</summary>
Motivation: 在向可再生能源转型中，确保电网稳定性需要精确的电力需求预测，而现有方法对不同消费者群体的独特需求模式考虑不足。

Method: 通过客户分群（工业、商业、居民），结合时间、社会经济和天气数据，比较多种AI和机器学习算法，开发定制化的短期和超短期负荷预测模型。

Result: 提出的新型预测方法在短期和超短期负荷预测中表现优于简单模型，证明了定制化策略和详细天气数据的重要性。

Conclusion: 定制化预测策略和天气数据的结合显著提升了预测准确性，为电网稳定性提供了更可靠的电力需求预测支持。
...
<details>
  <summary>Details</summary>
Motivation: 露天矿运营中天气、机械故障和人员变动带来不确定性，需准确预测车队容量以优化管理。

Method: 结合真实运营数据（如降雨量、车队性能）和合成机械故障场景，评估XGBoost和LSTM模型。

Result: XGBoost的MedAE为14.3%，LSTM为15.1%，模拟故障数据显著降低预测波动。

Conclusion: 未来将整合更多运营数据以增强模型鲁棒性，为动态不确定条件下的车队管理提供决策支持。
...
<details>
  <summary>Details</summary>
Motivation: 解决集成学习系统中一致性和依赖性问题，通过多样性和信息理论优化决策过程。

Method: 1. 解剖学过程：选择多样化的典型元素构建目标集成模型。2. 生理学过程：通过softlog实现弱对数操作一致性，并通过多层softlog-softmax生成中间决策。3. 神经信息理论：提出基于softlog的熵和散度，构建信息度量。4. 性能评估：推导信息性能张量以可靠评估集成系统。

Result: 研究表明，问题越复杂，集成多样性越有用；softlog-softmax层和信息度量能提升决策一致性。

Conclusion: 提出的4步流程有效提升了集成学习系统的一致性和可靠性，适用于复杂问题。
...
<details>
  <summary>Details</summary>
Motivation: 研究AI代理在多步推理中的系统级成本、效率和可持续性问题，以量化其资源使用和能耗。

Method: 通过系统级分析，量化不同代理设计和测试策略下的资源使用、延迟行为和能耗。

Result: 发现代理虽能提升准确性，但计算成本迅速增加，延迟差异扩大，基础设施成本不可持续。

Conclusion: 呼吁转向计算高效的设计，以平衡性能和实际部署的可持续性。
...
<details>
  <summary>Details</summary>
Motivation: 研究如何在保护用户隐私的同时，从LLM聊天机器人交互中提取有价值的见解。

Method: 采用差分隐私工具（如聚类、分区选择和直方图摘要）及多种关键词提取方法（频率、TF-IDF和LLM引导）。

Result: 框架在保留语义内容和隐私的同时，有效提取对话见解，并通过实验验证其隐私保护效果。

Conclusion: Urania成功实现了隐私保护与数据效用的平衡，适用于LLM交互分析。
...
Conclusion: YOTO提供了一种高效的单次训练方法，显著减少了超参数调优的复杂性。
...
TLDR: 提出一种“半”隐藏层单元，部分权重随机固定，部分可训练，减少参数数量并提升正则化效果。


<details>
  <summary>Details</summary>
Motivation: 减少深度网络中隐藏层的可训练参数数量，同时通过随机连接的正则化效果提升模型性能。

Method: 设计“半”单元，分为两个阶段：固定随机权重的加权和输入，再通过两个可调权重进行缩放和平移，最后通过非线性激活函数。

Result: 在MNIST和FashionMNIST数据集上实验表明，该方法能以较少参数实现合理准确率。

Conclusion: 半隐藏层单元通过减少可训练参数和引入随机连接的正则化，展现出潜力。
...
Result: 推导了各项的闭式梯度，分析了单调性、有界性和模块性等理论特性。

Conclusion: 该框架为复杂交易环境中构建稳健的多目标奖励函数提供了通用蓝图，并可扩展更多风险度量或自适应权重。
...
<details>
  <summary>Details</summary>
Motivation: 对抗训练（AT）的机制和动态尚不明确，本文旨在通过类间特征分析AT，揭示其内在规律。

Method: 提出通过类间特征（交叉类特征）研究AT，利用合成数据模型和系统实验分析AT的动态过程。

Result: 发现AT初期模型倾向于学习交叉类特征，后期则转向类特定特征，同时统一了软标签训练和鲁棒过拟合的现有观点。

Conclusion: 研究深化了对AT机制的理解，为未来研究提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 将决策树模型扩展到双曲空间（如Lorentz模型）已有研究，但如何在Beltrami-Klein模型中优雅地实现hyperDT算法，并利用欧几里得空间模型的优化，是一个值得探索的方向。

Method: 通过将hyperDT算法重新表达为Beltrami-Klein模型中的形式，并将其包装为对欧几里得空间树模型的预处理和后处理步骤。

Result: 该方法提高了模型的灵活性、速度和准确性，同时简化了代码库，使其更易于维护和扩展。

Conclusion: 在Beltrami-Klein模型中实现hyperDT算法，并通过包装欧几里得空间模型，提供了一种高效且易于维护的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 探讨合成数据在隐私保护中的潜在优势，尤其是隐私放大效应，填补理论研究的空白。

Method: 采用线性回归框架，分析种子控制和随机输入对合成数据隐私泄露的影响。

Result: 发现种子控制下隐私泄露严重，而随机输入下合成数据能增强隐私保护。

Conclusion: 研究结果为未来更广泛的隐私保护边界提供了理论基础。
...
<details>
  <summary>Details</summary>
Motivation: 研究持续学习中遗忘问题，特别是样本回放技术的效果及其潜在负面影响。

Method: 在过参数化的持续线性回归设置中，理论分析样本回放的作用，并通过实验验证神经网络中的类似行为。

Result: 发现回放样本可能导致非单调遗忘，甚至在某些情况下增加遗忘。
...
<details>
  <summary>Details</summary>
Motivation: 移除目标网络虽能节省内存并利用最新估计，但会引入不稳定性和性能下降。

Method: 共享在线网络的大部分参数，仅复制最后一层作为目标网络，结合并行学习Bellman迭代的Q-learning方法。

Result: iS-QL在多种设置下提高了目标无关方法的样本效率，且内存占用更小，训练时间与传统方法相当。
...
<details>
  <summary>Details</summary>
Motivation: 解决元测试阶段缺乏奖励信号的问题，同时减少在线交互的需求，提高样本效率。

Method: 结合参数化策略梯度（PPG）和任务推断，利用条件神经过程（CNPs）高效复用元训练样本，通过单次测试任务推断潜在动态模型表示。

Result: 在2D-Point Agent和连续控制基准测试中，UMCNP在元测试阶段显著减少了样本需求，优于基线方法。
...
<details>
  <summary>Details</summary>
Motivation: 填补自监督对比学习理论基础的空白，解释其与监督变体NSCL的关系。

Method: 通过理论分析证明CL与NSCL损失的差距随类别数增加而消失，并研究NSCL损失的几何结构及其对少样本学习的影响。

Result: CL与NSCL损失的差距随类别数增加而消失，NSCL损失的几何结构支持准确的少样本标签恢复。

Conclusion: 论文为自监督对比学习提供了理论支持，并验证了其在少样本学习中的有效性。
...
Result: 实验表明，这些算法在收敛质量上不逊于传统一阶方法，同时显著减少内存使用。
...
TLDR: KOALA++是一种基于卡尔曼滤波的优化算法，通过显式建模梯度不确定性提升神经网络训练效率，优于传统二阶方法。


<details>
  <summary>Details</summary>
Motivation: 传统二阶优化方法计算成本高，KOALA++旨在通过直接估计参数协方差矩阵来避免昂贵的二阶梯度计算。
...
### [53] [RETRO SYNFLOW: Discrete Flow Matching for Accurate and Diverse Single-Step Retrosynthesis](https://arxiv.org/abs/2506.04439)
*Robin Yadav,Qi Yan,Guy Wolf,Avishek Joey Bose,Renjie Liao*

Main category: cs.LG

TLDR: 本文提出了一种名为RETRO SYNFLOW（RSF）的离散流匹配框架，用于单步逆合成规划，通过反应中心识别和Feynman-Kac引导提升生成反应的多样性和可行性。


<details>
  <summary>Details</summary>
Motivation: 有机化学中识别和预测目标产物分子的合成反应序列是一个基本问题，现有方法在单步逆合成中难以生成准确且多样的可行反应。

Method: RSF采用离散流匹配框架，结合反应中心识别生成中间结构（synthons），并使用Feynman-Kac引导和Sequential Monte Carlo重采样优化生成。

Result: 实验显示，RSF在top-1准确率上达到60.0%，优于之前方法20%，且top-5往返准确率提升19%。
...
### [54] [Selective Matching Losses -- Not All Scores Are Created Equal](https://arxiv.org/abs/2506.04446)
*Gil I. Shamir,Manfred K. Warmuth*

Main category: cs.LG

TLDR: 论文提出了一种选择性匹配损失函数，通过设计递增的链接函数来强调高重要性区域的预测准确性，解决了模型在关键区域预测不足的问题，并扩展到多分类任务。


<details>
  <summary>Details</summary>
Motivation: 在许多学习系统中，某些区域的预测准确性比其他区域更重要，但传统损失函数未能区分区域的重要性。

Method: 设计了基于Sigmoid和双曲正弦链接的选择性标量损失函数，并通过复合Softmax扩展到多维选择性损失。

Result: 选择性损失在关键区域预测任务（如停留时间预测、检索、排名等）中显著优于传统损失函数。

Conclusion: 选择性损失函数能够有效提升模型在关键区域的预测性能，适用于多种应用场景。
...
### [55] [Neurosymbolic Artificial Intelligence for Robust Network Intrusion Detection: From Scratch to Transfer Learning](https://arxiv.org/abs/2506.04454)
*Huynh T. T. Tran,Jacob Sander,Achraf Cohen,Brian Jalaian,Nathaniel D. Bastian*

Main category: cs.LG

TLDR: 本文扩展了ODXU框架，结合神经符号AI、深度聚类和不确定性量化，提升网络入侵检测系统的性能，并在实验中优于传统模型。


<details>
  <summary>Details</summary>
Motivation: 网络入侵检测系统面临日益复杂的威胁，需要更鲁棒、可解释和泛化的解决方案。

Method: 扩展ODXU框架，整合深度聚类、符号推理和不确定性量化，并开发迁移学习策略。

Result: 在CIC-IDS-2017数据集上表现优于传统模型，迁移学习策略在ACI-IoT-2023上仅需少量数据即可超越传统方法。

Conclusion: ODXU框架在网络安全中具有潜力，不确定性量化和迁移学习是提升性能的关键。
...
### [56] [Behavioural vs. Representational Systematicity in End-to-End Models: An Opinionated Survey](https://arxiv.org/abs/2506.04461)
*Ivan Vegner,Sydelle de Souza,Valentin Forch,Martha Lewis,Leonidas A. A. Doumas*

Main category: cs.LG

TLDR: 论文讨论了系统性在机器学习模型中的重要性，区分了行为系统性和表征系统性，并分析了现有基准测试的局限性，同时提出了评估表征系统性的方法。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于澄清系统性在机器学习中的定义，尤其是区分行为系统性和表征系统性，并指出现有基准测试主要关注行为系统性，忽视了表征系统性的重要性。

Method: 方法包括基于Hadley（1994）的分类法分析现有基准测试，并借鉴机制可解释性领域的实践，提出评估表征系统性的方法。

Result: 结果表明现有基准测试主要测试行为系统性，而对表征系统性的评估不足。

Conclusion: 结论强调需要更多关注表征系统性的评估，并提出了未来研究的方向。
...
### [57] [Classifying Dental Care Providers Through Machine Learning with Features Ranking](https://arxiv.org/abs/2506.04474)
*Mohammad Subhi Al-Batah,Mowafaq Salem Alzboon,Muhyeeddin Alqaraleh,Mohammed Hasan Abu-Arqoub,Rashiq Rafiq Marie*

Main category: cs.LG

TLDR: 该研究利用机器学习模型对牙科服务提供者进行分类，通过特征排名和多种模型评估，发现神经网络在20个特征下准确率最高（94.1%），并强调了特征选择对模型性能的重要性。


<details>
  <summary>Details</summary>
Motivation: 研究旨在通过机器学习方法区分标准牙科服务提供者和安全网诊所提供者，以优化资源分配，特别是针对服务不足的人群。

Method: 使用2018年的24,300条数据（20个特征，38.1%缺失值），采用信息增益、基尼指数和ANOVA进行特征排名，评估了12种机器学习模型（如kNN、决策树、SVM等），并通过10折交叉验证测试准确性。

Result: 神经网络表现最佳（94.1%准确率），其次是梯度提升（93.2%）和随机森林（93.0%）。特征排名显示治疗相关指标是关键预测因素，而人口统计变量影响较小。

Conclusion: 研究强调了特征选择对提高模型效率的重要性，并建议结合特征排名和高级机器学习算法优化牙科提供者分类，以支持资源分配决策。
...
### [58] [Comparative performance of ensemble models in predicting dental provider types: insights from fee-for-service data](https://arxiv.org/abs/2506.04479)
...
Method: 使用2018年数据集，分析24,300个实例和20个特征，测试多种机器学习算法，包括kNN、决策树、SVM、SGD、随机森林、神经网络和梯度提升，采用10折交叉验证。
...
### [59] [COSMOS: Predictable and Cost-Effective Adaptation of LLMs](https://arxiv.org/abs/2505.01449)
*Jiayu Wang,Aws Albarghouthi,Frederic Sala*

Main category: cs.LG

TLDR: COSMOS框架通过轻量级代理模型和低样本缩放定律，高效预测LLM的适应策略性能和成本，显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 在资源受限的情况下，如何无需昂贵实验即可准确预测LLM的性能和成本。

Method: 提出COSMOS框架，结合嵌入增强的轻量级代理模型和低样本缩放定律，预测微调性能和检索增强的上下文学习。

Result: 在八个基准测试中，COSMOS平均减少92.72%计算成本，最高达98.71%，同时保持高预测准确性。

Conclusion: 高效预测LLM适应策略是可行的，可大幅降低计算开销并保持性能。
...
### [60] [Orthogonal Gradient Descent Improves Neural Calibration](https://arxiv.org/abs/2506.04487)
*C. Evans Hedges*
...
### [61] [Multiscale guidance of AlphaFold3 with heterogeneous cryo-EM data](https://arxiv.org/abs/2506.04490)
*Rishwanth Raghu,Axel Levy,Gordon Wetzstein,Ellen D. Zhong*

Main category: cs.LG

TLDR: 论文提出了一种结合冷冻电镜密度图和蛋白质结构预测模型的方法CryoBoltz，用于捕捉动态生物分子复合物的构象多样性。


<details>
  <summary>Details</summary>
Motivation: 现有的蛋白质结构预测模型难以捕捉动态生物分子复合物的构象多样性，而冷冻电镜虽能成像结构异质性，但数据处理流程复杂。

Method: CryoBoltz利用冷冻电镜密度图的全局和局部结构约束，引导预训练的蛋白质结构预测模型的采样轨迹，生成与实验数据一致的构象状态。

Result: 该方法成功应用于多种动态生物分子系统（如转运蛋白和抗体），构建了原子模型。

Conclusion: CryoBoltz填补了蛋白质结构预测与冷冻电镜之间的鸿沟，为动态生物分子系统的结构研究提供了新工具。
...
### [62] [Perturbative Gradient Training: A novel training paradigm for bridging the gap between deep neural networks and physical reservoir computing](https://arxiv.org/abs/2506.04523)
...
<details>
  <summary>Details</summary>
Motivation: 物理储层计算的黑盒特性导致无法使用反向传播，限制了其应用。PGT旨在解决这一问题。

Method: 利用物理扰动理论，通过随机扰动参数空间，仅需前向传播即可近似梯度更新。

Result: 在模拟和实验硬件中，PGT表现与标准反向传播相当。

Conclusion: PGT为物理储层与深度网络结合提供了可能，有望提升AI训练的能效。
...
### [63] [Hierarchical Implicit Neural Emulators](https://arxiv.org/abs/2506.04528)
*Ruoxi Jiang,Xiao Zhang,Karan Jakhar,Peter Y. Lu,Pedram Hassanzadeh,Michael Maire,Rebecca Willett*

Main category: cs.LG

TLDR: 提出了一种多尺度隐式神经模拟器，通过利用多尺度未来状态表示提升长期预测准确性，显著优于自回归基线。
...
Method: 引入多尺度隐式神经模拟器，利用时间下采样比率调整，捕捉多粒度动态并保持长期时间一致性。

Result: 在湍流流体动力学实验中，方法实现了高短期精度和长期稳定预测。

Conclusion: 该方法显著提升了长期预测的准确性和稳定性，计算开销小。
...
### [64] [HALoS: Hierarchical Asynchronous Local SGD over Slow Networks for Geo-Distributed Large Language Model Training](https://arxiv.org/abs/2506.04531)
*Geon-Woo Kim,Junbo Li,Shashidhar Gandham,Omar Baldonado,Adithya Gangidi,Pavan Balaji,Zhangyang Wang,Aditya Akella*

Main category: cs.LG

TLDR: HALoS是一种分层异步优化框架，通过引入本地和全局参数服务器，减少跨区域通信成本，提升异构硬件利用率，显著加速LLM训练。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型（LLM）训练中跨区域通信成本高和异构硬件利用率不均的问题。

Method: 提出HALoS框架，采用本地参数服务器（LPSs）和全局参数服务器（GPS）的分层设计，减少跨区域通信并利用快速区域内链接。

Result: HALoS比同步基线快7.5倍，比现有异步方法快2.1倍，同时保持模型质量。

Conclusion: 分层设计和全局模型合并是高效训练LLM的强大工具。
...
### [65] [NOBLE -- Neural Operator with Biologically-informed Latent Embeddings to Capture Experimental Variability in Biological Neuron Models](https://arxiv.org/abs/2506.04536)
*Luca Ghafourpour,Valentin Duruisseaux,Bahareh Tolooshams,Philip H. Wong,Costas A. Anastassiou,Anima Anandkumar*

Main category: cs.LG

TLDR: NOBLE是一种神经操作框架，通过学习神经元特征的连续频率调制嵌入到电压响应的映射，解决了传统生物神经元模型在捕捉变异性和计算效率上的不足。


<details>
  <summary>Details</summary>
Motivation: 当前神经元模型受限于实验数据的有限性和变异性，且传统方法难以扩展，深度学习又无法完全捕捉生物物理复杂性。

Method: NOBLE利用神经操作框架，从神经元特征的嵌入空间预测电压响应分布，并结合实验变异性。

Result: NOBLE实现了4200倍的速度提升，生成的合成神经元具有实验观察到的变异性，且动态与实验一致。
...
### [66] [Neural MJD: Neural Non-Stationary Merton Jump Diffusion for Time Series Prediction](https://arxiv.org/abs/2506.04542)
*Yuanpei Gao,Qi Yan,Yan Leng,Renjie Liao*

Main category: cs.LG

TLDR: Neural MJD模型结合了神经网络和非平稳Merton跳跃扩散模型，通过随机微分方程模拟预测问题，显著提升了非平稳时间序列预测的性能。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习方法在黑盒性质和无法显式建模随机过程方面存在局限，尤其在非平稳数据或突变情况下泛化能力不足。

Method: 提出Neural MJD模型，结合时间非齐次Itô扩散和时间非齐次复合泊松过程，引入截断似然机制和Euler-Maruyama重启求解器。

Result: 在合成和真实数据集上，Neural MJD优于现有深度学习和统计学习方法。

Conclusion: Neural MJD通过显式建模随机过程，显著提升了非平稳时间序列预测的准确性和稳定性。
...
### [67] [Communication Efficient Adaptive Model-Driven Quantum Federated Learning](https://arxiv.org/abs/2506.04548)
*Dev Gurung,Shiva Raj Pokhrel*

Main category: cs.LG

TLDR: 论文提出了一种模型驱动的量子联邦学习算法（mdQFL），解决了大规模数据集、多设备参与和非独立同分布数据带来的挑战，显著降低了通信成本并提升了性能。
...
Method: 提出模型驱动的量子联邦学习算法（mdQFL），支持训练和更新个性化，并测试泛化能力。

Result: 实验表明，mdQFL在非独立同分布数据条件下，通信成本降低近50%，同时保持或提升模型准确性，并改善本地模型训练。

Conclusion: mdQFL是首个探索QFL中训练个性化与泛化的方法，适用于其他FL场景，实验和理论分析验证了其有效性。
...
### [68] [Unsupervised Machine Learning for Scientific Discovery: Workflow and Best Practices](https://arxiv.org/abs/2506.04553)
*Andersen Chang,Tiffany M. Tang,Tarek M. Zikry,Genevera I. Allen*

Main category: cs.LG

TLDR: 本文提出了一种结构化的无监督学习工作流程，旨在提高科学发现的可靠性和可重复性，并通过天文学案例研究验证其有效性。
...
Method: 提出结构化工作流程，包括科学问题提出、数据准备、模型选择、严格验证及结果文档化。

Result: 通过天文学案例研究验证了工作流程的有效性，强调了验证的重要性。

Conclusion: 精心设计的无监督学习工作流程能推动科学发现，需注重验证和文档化。
...
### [69] [Clustering and Median Aggregation Improve Differentially Private Inference](https://arxiv.org/abs/2506.04566)
*Kareem Amin,Salman Avestimehr,Sara Babakniya,Alex Bie,Weiwei Kong,Natalia Ponomareva,Umar Syed*

Main category: cs.LG

TLDR: 本文提出了一种改进的差分隐私语言模型推理方法，通过聚类输入数据并利用中位数聚合令牌预测，显著提升了生成文本的质量和隐私保护效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法通过均匀采样敏感输入生成差分隐私文本，但这种方法在输入数据主题异构时效果较差。本文旨在解决这一问题。

Method: 首先对输入数据进行聚类，然后通过计算中位数而非平均值来聚合令牌预测，利用中位数局部敏感性较低的特点，提供数据依赖的隐私保证。

Result: 实验表明，该方法在代表性指标（如MAUVE）和下游任务性能上均有提升，能以更低的隐私成本生成高质量合成数据。

Conclusion: 通过聚类和中位数聚合，本文方法显著提升了差分隐私文本生成的质量和效率，优于现有方法。
...
### [70] [StatsMerging: Statistics-Guided Model Merging via Task-Specific Teacher Distillation](https://arxiv.org/abs/2506.04567)
*Ranjith Merugu,Bryan Bo Cao,Shubham Jain*

Main category: cs.LG

TLDR: StatsMerging是一种轻量级学习模型合并方法，通过权重分布统计指导合并，无需真实标签或测试样本。它利用SVD奇异值、轻量级学习器和任务特定教师蒸馏，在多个任务中表现优异。
...
Method: 利用SVD奇异值捕获任务特定权重分布，轻量级学习器建模权重分布，任务特定教师蒸馏合并异构架构模型。

Result: 在八个任务中表现优于现有技术，整体准确性、泛化能力和鲁棒性均有提升。

Conclusion: StatsMerging是一种高效、通用的模型合并方法，适用于异构架构和多任务场景。
...
### [71] [Scaling Laws for Robust Comparison of Open Foundation Language-Vision Models and Datasets](https://arxiv.org/abs/2506.04598)
*Marianna Nezhurina,Tomer Porian,Giovanni Pucceti,Tommie Kerssies,Romain Beaumont,Mehdi Cherti,Jenia Jitsev*

Main category: cs.LG

TLDR: 本文提出了一种利用缩放定律（scaling laws）比较模型和数据集的方法，首次为CLIP和MaMMUT两种语言-视觉学习模型推导了完整的缩放定律，并证明MaMMUT在扩展性和样本效率上优于CLIP。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于通过缩放定律的推导，避免仅基于单一参考尺度的误导性结论，为模型和数据集的选择提供系统性比较方法。

Method: 方法包括为CLIP和MaMMUT模型推导完整的缩放定律，并在不同任务（分类、检索、分割）和数据集（DataComp、DFN、Re-LAION）上验证其一致性。

Result: 结果显示MaMMUT在扩展性和样本效率上优于CLIP，且在不同任务和数据集上表现一致。

Conclusion: 结论表明准确的缩放定律推导为模型和数据集的选择提供了可靠依据，推动了开放基础模型和数据集的系统性改进。
...
### [72] [Ignoring Directionality Leads to Compromised Graph Neural Network Explanations](https://arxiv.org/abs/2506.04608)
*Changsheng Sun,Xinke Li,Jin Song Dong*

Main category: cs.LG

TLDR: 方向信息在GNN解释中的重要性被忽视，对称化操作导致信息丢失和误导性解释。保留方向语义可显著提升解释质量。


<details>
  <summary>Details</summary>
Motivation: 在关键领域，GNN的可靠解释对决策至关重要，但对称化操作丢弃方向信息，影响解释的准确性。

Method: 通过理论和实证研究，分析方向信息对解释质量的影响。

Result: 保留方向语义显著提升解释质量，提供更可信的决策支持。
...
### [73] [Exploring bidirectional bounds for minimax-training of Energy-based models](https://arxiv.org/abs/2506.04609)
*Cong Geng,Jia Wang,Li Chen,Zhiyong Gao,Jes Frellsen,Søren Hauberg*

Main category: cs.LG

TLDR: 论文提出了一种通过双向边界（同时最大化下界和最小化上界）训练能量模型（EBMs）的方法，以解决传统训练中的不稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 传统EBMs训练困难且不稳定，尤其是通过变分下界训练时。论文旨在通过双向边界方法提升训练稳定性。

Method: 提出了四种不同的对数似然边界（包括基于生成器雅可比矩阵奇异值和互信息的下界，以及基于梯度惩罚和扩散过程的上界），并提供了评估算法。

Result: 实验表明，双向边界方法显著提升了EBMs的训练稳定性，并实现了高质量的密度估计和样本生成。

Conclusion: 双向边界方法为EBMs训练提供了一种稳定且高效的解决方案，未来可进一步探索其在不同场景中的应用。
...
### [74] [Composing Agents to Minimize Worst-case Risk](https://arxiv.org/abs/2506.04632)
*Guruprerana Shabadi,Rajeev Alur*

Main category: cs.LG

TLDR: 论文提出了一种基于有向无环图（agent graph）的AI代理组合优化方法，旨在最大化任务成功率的同时最小化风险（如安全、公平、隐私）。通过近似尾部风险（value-at-risk）和动态规划，算法高效地找到近似最优代理组合。


<details>
  <summary>Details</summary>
Motivation: 现代AI系统需要分解复杂任务为子任务并由多个代理完成，但需同时满足风险最小化（如安全、公平、隐私）的要求。

Method: 将代理组合建模为有向无环图，定义尾部风险为损失分布的分位数，提出基于联合边界和动态规划的高效近似算法。

Result: 在强化学习代理组合的基准测试中，算法有效近似尾部风险并找到最优代理组合。

Conclusion: 提出的方法能高效优化代理组合，兼顾任务成功率和风险最小化，适用于实际应用场景。
...
### [75] [Inference economics of language models](https://arxiv.org/abs/2506.04645)
*Ege Erdil*

Main category: cs.LG

TLDR: 论文提出了一个理论模型，用于权衡大规模部署LLM推理时的每令牌成本与串行令牌生成速度，并优化并行设置和批量大小。


<details>
  <summary>Details</summary>
Motivation: 解决在大规模部署LLM推理时，如何在每令牌成本和串行生成速度之间找到最优权衡的问题。

Method: 模型考虑了算术、内存带宽、网络带宽和延迟约束，并通过优化并行设置和批量大小来最大化串行推理速度。

Result: 计算了流行语言模型的串行速度与每令牌成本的Pareto前沿。

Conclusion: 该模型为大规模LLM推理部署提供了理论支持，帮助优化成本与性能的平衡。
...
### [76] [Neural Network Reprogrammability: A Unified Theme on Model Reprogramming, Prompt Tuning, and Prompt Instruction](https://arxiv.org/abs/2506.04650)
*Zesheng Ye,Chengyi Cai,Ruijiang Dong,Jianzhong Qi,Lei Feng,Pin-Yu Chen,Feng Liu*

Main category: cs.LG

TLDR: 该论文提出了一种统一框架“神经网络可重编程性”，将模型重编程、提示调整和提示指令等主流模型适应技术联系起来，通过信息操作实现预训练模型的高效适应。


<details>
  <summary>Details</summary>
Motivation: 随着大规模预训练基础模型规模和能力的增长，如何高效适应下游任务变得至关重要。现有适应方法缺乏统一的理论框架。

Method: 通过信息操作（如输入扰动、中间层插入令牌或上下文任务示例）重定向模型行为，提出分类法从四个维度（格式、位置、操作符、输出对齐）分析这些方法。

Result: 该框架适用于多种数据模态和模型架构，揭示了现有技术（如上下文学习和思维链提示）的理论联系和实际区别。

Conclusion: 神经网络可重编程性是高效模型适应的基本范式，未来研究可基于此统一视角展开。
...
### [77] [The Oversmoothing Fallacy: A Misguided Narrative in GNN Research](https://arxiv.org/abs/2506.04653)
*MoonJeong Park,Sunghyun Choi,Jaeseung Heo,Eunhyeok Park,Dongwoo Kim*

Main category: cs.LG

TLDR: 本文认为图神经网络（GNN）中的过度平滑问题被夸大，并提倡进一步探索深层GNN架构。研究表明，过度平滑与梯度消失被混淆，实际由变换和激活引起，而非聚合。经典方法（如跳跃连接和归一化）可成功构建深层GNN。


<details>
  <summary>Details</summary>
Motivation: 探讨GNN中过度平滑问题的真实影响，挑战现有观点，揭示深层GNN的潜力。

Method: 分析GNN的三种核心操作（聚合、线性变换、非线性激活），区分过度平滑与梯度消失，并验证经典解决方案的有效性。

Result: 发现过度平滑并非GNN独有，且通过跳跃连接和归一化可构建深层GNN而无性能下降。

Conclusion: 澄清了关于过度平滑的误解，为深层GNN的研究提供了新方向。
...
### [78] [Noise-Resistant Label Reconstruction Feature Selection for Partial Multi-Label Learning](https://arxiv.org/abs/2506.04669)
*Wanfu Gao,Hanlin Pan,Qingqi Han,Kunpeng Liu*

Main category: cs.LG

TLDR: 提出了一种考虑标签关系抗噪性和连通性的PML特征选择方法，解决了现有方法在识别正标签和高维信息丢失上的不足。


<details>
  <summary>Details</summary>
Motivation: 解决PML中维度诅咒导致的模型过拟合和分类性能下降问题，现有基于低秩假设的方法难以满足实际需求且易丢失高维信息。

Method: 利用标签关系的抗噪性消歧，改进低秩假设设计学习过程，通过标签连通性找到代表性标签并重构权重矩阵选择特征。

Result: 在基准数据集上验证了方法的优越性。

Conclusion: 该方法有效提升了PML中正标签识别能力和特征选择性能。
...
### [79] [FedAPM: Federated Learning via ADMM with Partial Model Personalization](https://arxiv.org/abs/2506.04672)
*Shengkun Zhu,Feiteng Nie,Jinshan Zeng,Sheng Wang,Yuan Sun,Yuan Yao,Shangfeng Chen,Quanqing Xu,Chuanhui Yang*

Main category: cs.LG

TLDR: 论文提出了一种基于ADMM的联邦学习框架FedAPM，通过引入一阶和二阶近端项来缓解客户端漂移，提高了模型的收敛速度和准确性。
...
Method: 提出FedAPM框架，结合ADMM和一阶、二阶近端项，优化局部与共享模型间的修正。

Result: FedAPM在收敛速度和准确性上优于现有方法，平均提升12.3%测试准确率、16.4% F1分数和18.0% AUC。

Conclusion: FedAPM通过稳定收敛和高效通信，显著提升了联邦学习的性能。
...
### [80] [The cost of ensembling: is it always worth combining?](https://arxiv.org/abs/2506.04677)
*Marco Zanotti*

Main category: cs.LG

TLDR: 该论文研究了时间序列预测中集成学习的准确性与计算成本之间的权衡，发现小规模集成（2-3个模型）在保持高精度的同时显著降低成本。


<details>
  <summary>Details</summary>
Motivation: 随着数据集规模和预测模型复杂度的增加，准确性与计算成本之间的权衡成为重要问题，尤其是在时间序列预测的集成学习中。

Method: 在两个大型零售数据集（M5和VN1）上评估了10个基础模型和8种集成配置，考虑了不同重训练频率下的点预测和概率预测准确性。

Result: 集成方法显著提升了预测性能（尤其是概率预测），但计算成本较高；降低重训练频率可显著降低成本且对精度影响较小；小规模集成在成本和精度间取得良好平衡。

Conclusion: 通过精心设计集成和选择重训练策略，可以实现高精度、鲁棒且成本效益高的预测，适用于实际应用。
...
### [81] [Towards Better Generalization via Distributional Input Projection Network](https://arxiv.org/abs/2506.04690)
...
<details>
  <summary>Details</summary>
Motivation: 过参数化模型中训练损失难以反映泛化性能，直接强制平滑性在神经网络中具有挑战性。

Method: 提出DIPNet框架，将输入逐层投影到可学习分布中，诱导更平滑的损失景观。

Result: 理论分析显示DIPNet降低局部平滑性指标和Lipschitz常数；实验验证其在多种架构和任务中提升测试性能。
...
### [82] [Influence Functions for Edge Edits in Non-Convex Graph Neural Networks](https://arxiv.org/abs/2506.04694)
*Jaeseung Heo,Kyeongheung Yun,Seokwon Yoon,MoonJeong Park,Jungseul Ok,Dongwoo Kim*

Main category: cs.LG

TLDR: 提出了一种针对GNN的近端Bregman响应函数，放宽了凸性要求，能够准确预测边删除和插入的影响，并考虑了消息传播效应。


<details>
  <summary>Details</summary>
Motivation: 理解边对GNN行为的影响对提升其可解释性和鲁棒性至关重要，现有方法存在局限性。

Method: 提出近端Bregman响应函数，放宽凸性假设，考虑消息传播效应，支持边删除和插入的影响预测。

Result: 实验证明该方法能准确预测GNN的影响，并在图重连和对抗攻击等应用中表现优异。
...
### [83] [On the Mechanism of Reasoning Pattern Selection in Reinforcement Learning for Language Models](https://arxiv.org/abs/2506.04695)
*Xingwu Chen,Tianle Li,Difan Zou*

Main category: cs.LG

TLDR: RLVR通过优化现有推理模式的选择提升模型能力，理论分析揭示了其收敛动态，并提出SFT预训练可加速弱模型的优化。


<details>
  <summary>Details</summary>
Motivation: 尽管强化学习在提升模型能力方面表现出色，但其改善推理能力的机制尚不明确，本研究旨在系统探索RLVR的作用机制。

Method: 通过实验和理论分析，研究RLVR如何优化推理模式选择，并探讨其收敛动态及SFT预训练的影响。

Result: RLVR优先选择高成功率的推理模式，理论分析显示其收敛速度与模型初始能力相关，SFT预训练可加速弱模型优化。

Conclusion: 本研究深化了对RL在LLM微调中作用的理解，为提升推理能力提供了理论支持。
...
### [84] [Enhanced Drought Analysis in Bangladesh: A Machine Learning Approach for Severity Classification Using Satellite Data](https://arxiv.org/abs/2506.04696)
*Tonmoy Paul,Mrittika Devi Mati,Md. Mahmudul Islam*

Main category: cs.LG

TLDR: 提出了一种基于卫星数据和机器学习的框架，用于分类孟加拉国38个地区的干旱严重程度，填补了传统指数和现有模型的不足。


<details>
  <summary>Details</summary>
Motivation: 孟加拉国因地理和人为因素易受干旱影响，传统干旱指数（如SPI和PDSI）忽略土壤湿度和温度等关键因素，且现有机器学习模型在该地区应用不足。

Method: 结合无监督算法（K-means、贝叶斯高斯混合）和分类模型（KNN、随机森林、决策树、朴素贝叶斯），整合2012-2024年的天气数据（湿度、土壤湿度、温度）。

Result: 成功将干旱严重程度分类为不同级别，但发现地区间干旱脆弱性存在显著差异。
...
### [85] [Explicit Density Approximation for Neural Implicit Samplers Using a Bernstein-Based Convex Divergence](https://arxiv.org/abs/2506.04700)
*José Manuel de Frutos,Manuel A. Vázquez,Pablo M. Olmos,Joaquín Míguez*

Main category: cs.LG

TLDR: 本文提出了一种新的无似然目标函数dual-ISL，用于训练隐式生成模型，通过交换目标分布和模型分布的角色，实现了模型密度空间中的凸优化问题。


<details>
  <summary>Details</summary>
Motivation: 传统的统计度量（如ISL）在训练隐式生成模型时存在局限性，dual-ISL旨在解决这些问题，提供更稳定和高效的训练方法。

Method: 通过交换ISL框架中目标分布和模型分布的角色，构建了一个凸优化问题，并证明了其连续性和凸性。此外，通过Bernstein多项式基展开，推导了截断误差的精确界限和收敛速率。

Result: dual-ISL在实验中表现出更快的收敛速度、更稳定的训练过程，并能有效防止模式坍塌，同时提供显式的密度近似。
...
### [86] [UNO: Unlearning via Orthogonalization in Generative models](https://arxiv.org/abs/2506.04712)
*Pinak Mandal,Georg A. Gottwald*

Main category: cs.LG

TLDR: 论文提出了一种基于损失梯度正交化的快速遗忘算法，用于选择性移除生成模型中特定数据的影响，同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 随着生成模型的普及，如何高效、可靠地移除特定数据的影响（如隐私、法律或有害内容）变得至关重要。

Method: 采用基于损失梯度正交化的算法，确保在移除不需要的数据时，不影响模型的生成质量和所需数据的参数影响。

Result: 在MNIST和CelebA数据集上，该算法比传统方法（如梯度手术）快几个数量级。

Conclusion: 该算法实现了高效遗忘，同时保持了模型的原始性能，为生成模型的数据管理提供了新思路。
...
### [87] [Multi-Layer GRPO: Enhancing Reasoning and Self-Correction in Large Language Models](https://arxiv.org/abs/2506.04746)
*Fei Ding,Baiqiao Wang,Zijian Zeng,Youwei Wang*

Main category: cs.LG

TLDR: MGRPO通过双层GRPO结构改进推理能力，第一层生成初始响应，第二层纠正错误，显著提升性能和训练稳定性。


<details>
  <summary>Details</summary>
Motivation: GRPO在LLMs中表现良好，但缺乏中间监督导致探索效率低和训练不稳定。

Method: 提出MGRPO，双层GRPO结构：第一层生成初始响应，第二层纠正错误，形成自校正循环。
...
### [88] [Log-Linear Attention](https://arxiv.org/abs/2506.04761)
*Han Guo,Songlin Yang,Tarushii Goel,Eric P. Xing,Tri Dao,Yoon Kim*

Main category: cs.LG

TLDR: 提出了一种名为log-linear attention的注意力机制，平衡了线性注意力的效率和softmax注意力的表现力，解决了传统RNN固定大小隐藏状态的限制。
...
Method: 开发了log-linear attention，用对数增长的隐藏状态集替代固定大小隐藏状态，并展示了其并行计算形式。

Result: log-linear attention的计算成本与序列长度呈对数线性关系，且在Mamba-2和Gated DeltaNet等架构中表现良好。

Conclusion: log-linear attention是一种通用框架，可应用于现有线性注意力变体，平衡效率与表现力。
...
### [89] [OpenGT: A Comprehensive Benchmark For Graph Transformers](https://arxiv.org/abs/2506.04765)
*Jiachen Tang,Zhonghao Wang,Sirui Chen,Sheng Zhou,Jiawei Chen,Jiajun Bu*

Main category: cs.LG

TLDR: 论文介绍了OpenGT，一个用于Graph Transformers（GTs）的综合基准测试，旨在解决GTs设计选择和适用场景的未解问题。


<details>
  <summary>Details</summary>
Motivation: GTs在多个领域表现出色，但其适用场景和设计选择的有效性尚不明确，缺乏统一的基准测试和库。

Method: 通过OpenGT基准测试，标准化实验设置，比较多种GTs和GNNs，从多角度评估模型性能。
...
### [90] [Improved Regret Bounds for Linear Bandits with Heavy-Tailed Rewards](https://arxiv.org/abs/2506.04775)
...
### [91] [Kernel $k$-Medoids as General Vector Quantization](https://arxiv.org/abs/2506.04786)
*Thore Gerlach,Sascha Mücke,Christian Bauckhage*

Main category: cs.LG

TLDR: 论文通过QUBO框架揭示了k-medoids和KDE在VQ中的联系，发现KDE-QUBO是k-medoids-QUBO的特例。
...
Method: 通过QUBO框架比较k-medoids和KDE的启发式和理论性公式。
...
### [92] [Adaptive Preconditioners Trigger Loss Spikes in Adam](https://arxiv.org/abs/2506.04805)
*Zhiwei Bai,Zhangchen Zhou,Jiajie Zhao,Xiaolong Li,Zhiyu Li,Feiyu Xiong,Hongkang Yang,Yaoyu Zhang,Zhi-Qin John Xu*

Main category: cs.LG

TLDR: 研究发现Adam优化器在训练神经网络时会出现损失突增现象，原因是其自适应预条件器在特定条件下导致Hessian矩阵的最大特征值超过稳定性阈值。


<details>
  <summary>Details</summary>
Motivation: 探讨Adam优化器在训练过程中损失突增的机制，挑战了之前认为损失突增仅由损失景观特性引起的观点。

Method: 通过分析Adam的预条件器动态，发现当平方梯度远小于二阶矩估计时，后者因指数衰减而响应迟缓，导致Hessian矩阵特征值超过稳定性阈值。

Result: 实验验证了该机制在多种网络架构（全连接、卷积、Transformer）中均成立，损失突增与梯度方向曲率超过阈值直接相关。

Conclusion: Adam的自适应预条件器是损失突增的主要原因，为优化器设计和稳定性分析提供了新视角。
...
### [93] [LogicPuzzleRL: Cultivating Robust Mathematical Reasoning in LLMs via Reinforcement Learning](https://arxiv.org/abs/2506.04821)
*Zhen Hao Wong,Jingwen Deng,Runming He,Zirong Chen,Qijie You,Hejun Dong,Hao Liang,Chengyu Shen,Bin Cui,Wentao Zhang*

Main category: cs.LG

TLDR: 论文提出了一种通过强化学习在逻辑谜题上微调大语言模型的方法，以提升其结构化推理能力。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在监督任务中表现优异，但在陌生环境中的结构化推理能力较弱，表明标准微调方法可能仅培养领域特定的启发式策略。

Method: 采用强化学习框架，在七个定制逻辑谜题上微调模型，每个谜题设计用于培养不同的推理技能，如约束传播和符号演绎。

Result: 该方法显著提升了模型在数学基准测试中的分布外性能，尤其是在需要多步推理的中等难度问题上。

Conclusion: 研究表明，逻辑谜题的强化学习训练重塑了模型的内部推理机制，使其能够实现更稳健的组合泛化。
...
### [94] [From EHRs to Patient Pathways: Scalable Modeling of Longitudinal Health Trajectories with LLMs](https://arxiv.org/abs/2506.04831)
...
Method: 将多样化的电子健康记录数据转化为结构化表示，并设计了一个整体路径预测模型EHR2Path，引入了一种新颖的摘要机制以嵌入长期时间上下文。

Result: EHR2Path在下一步预测和纵向模拟中表现优异，优于基线模型，能够详细模拟患者轨迹，支持多种评估任务。

Conclusion: EHR2Path为预测性和个性化医疗开辟了新路径，展示了在多种医疗预测任务中的潜力。
...
### [95] [Sparse Autoencoders, Again?](https://arxiv.org/abs/2506.04859)
*Yin Lu,Tong He,Xuening Zhu,David Wipf*

Main category: cs.LG

TLDR: 论文提出了一种混合模型，解决了传统稀疏自编码器（SAEs）和变分自编码器（VAEs）的局限性，并在理论和实验上验证了其优越性。


<details>
  <summary>Details</summary>
Motivation: 传统SAEs和VAEs在处理低维潜在结构数据时存在未被充分认识的弱点，需要改进。

Method: 提出了一种混合模型，结合了SAEs和VAEs的优点，通过理论证明和实验验证其有效性。

Result: 新模型在合成和真实数据集上表现优于传统SAEs、VAEs及扩散模型，能更准确地估计潜在维度并生成更稀疏的表示。

Conclusion: 混合模型在图像和语言模型激活模式等领域中超越了传统方法，提供了更优的性能。
...
### [96] [Aligning Multimodal Representations through an Information Bottleneck](https://arxiv.org/abs/2506.04870)
*Antonio Almudévar,José Miguel Hernández-Lobato,Sameer Khurana,Ricard Marxer,Alfonso Ortega*

Main category: cs.LG

TLDR: 对比损失在多模态表示学习中广泛使用，但存在模态特定信息问题。本文通过信息瓶颈原理分析该问题，并提出一种正则化损失函数以提高表示对齐。
...
Method: 通过信息瓶颈原理理论分析，提出一种正则化损失函数，并通过实验验证其效果。

Result: 实验表明，提出的正则化项能显著提高表示对齐效果。

Conclusion: 正则化损失函数在多模态表示学习中能有效提升对齐性能。
...
### [97] [There Was Never a Bottleneck in Concept Bottleneck Models](https://arxiv.org/abs/2506.04877)
...
<details>
  <summary>Details</summary>
Motivation: 传统概念瓶颈模型（CBMs）未能真正限制表示组件仅编码特定概念的信息，这影响了模型的解释性和干预效果。
...
### [98] [Gaussian Process Diffeomorphic Statistical Shape Modelling Outperforms Angle-Based Methods for Assessment of Hip Dysplasia](https://arxiv.org/abs/2506.04886)
*Allen Paul,George Grammatopoulos,Adwaye Rambojun,Neill D. F. Campbell,Harinderjit S. Gill,Tony Shardlow*

Main category: cs.LG

TLDR: 论文提出了一种基于CT扫描的半自动化分类方法（GPDSSM），用于髋关节发育不良的诊断，提高了分类准确性并节省了临床时间。


<details>
  <summary>Details</summary>
Motivation: 髋关节发育不良是骨关节炎的风险因素，早期诊断有助于手术干预。传统方法依赖手动测量和2D扫描，效率低且准确性不足。
...
Conclusion: GPDSSM提高了诊断效率和准确性，为临床提供了更便捷的工具。
...
### [99] [Dissecting Long Reasoning Models: An Empirical Study](https://arxiv.org/abs/2506.04913)
*Yongyu Mu,Jiali Zeng,Bei Li,Xinyan Guan,Fandong Meng,Jie Zhou,Tong Xiao,Jingbo Zhu*

Main category: cs.LG

TLDR: 本文分析了强化学习中正负样本的作用，发现负样本显著提升泛化能力；解决了数据效率低的问题；探讨了模型性能不稳定的原因及解决方法。
...
Result: 负样本训练效果媲美标准RL；数据效率提升；多次评估缓解性能不稳定。

Conclusion: 负样本对泛化至关重要；简单策略可提升数据效率；多次评估解决不稳定问题。
...
### [100] [Predicting ICU In-Hospital Mortality Using Adaptive Transformer Layer Fusion](https://arxiv.org/abs/2506.04924)
...
TLDR: ALFIA是一种基于注意力机制的模块化架构，用于早期识别ICU高危患者，通过融合多层语义特征和自适应权重机制，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 早期识别ICU高危患者对医疗资源分配至关重要，现有方法在性能上仍有提升空间。

Method: ALFIA结合LoRA适配器和自适应层加权机制，从BERT主干中融合多层语义特征，并在cw-24基准上训练。

Result: ALFIA在AUPRC上超越现有表格分类器，同时保持平衡的精确率-召回率，其嵌入特征还可与其他模型结合进一步提升性能。
...
### [101] [Agentic AI for Intent-Based Industrial Automation](https://arxiv.org/abs/2506.04980)
*Marcos Lima Romero,Ricardo Suyama*

Main category: cs.LG

TLDR: 论文提出了一种将Agentic AI与意图驱动范式结合的概念框架，以简化人机交互并支持工业5.0的可持续发展目标。


<details>
  <summary>Details</summary>
Motivation: 工业4.0的复杂性增加了人机交互的难度，而Agentic AI的发展为工业自动化提供了新可能。本文旨在通过意图驱动的方法简化这一过程。

Method: 提出一个框架，将人类操作者的高级目标分解为可执行组件，并通过子代理执行任务。使用CMAPSS数据集和Google ADK验证可行性。

Result: 验证了意图分解、代理协调和自主决策在预测性维护中的可行性，但数据质量和可解释性仍需改进。

Conclusion: 该框架有潜力降低技术壁垒并实现可扩展的意图驱动自动化，但需解决数据质量和可解释性问题。
...
### [102] [FPTQuant: Function-Preserving Transforms for LLM Quantization](https://arxiv.org/abs/2506.04985)
*Boris van Breugel,Yelysei Bondarenko,Paul Whatmough,Markus Nagel*

Main category: cs.LG

TLDR: FPTQuant提出四种轻量级函数保持变换（FPTs），用于改进大型语言模型（LLMs）的量化效果，显著提升推理效率，同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 量化LLMs时，传统方法因大范围异常值导致性能显著下降，FPTQuant旨在通过FPTs解决这一问题。

Method: 引入四种FPTs：预RoPE变换、值变换、MLP块内缩放变换和动态缩放变换，利用变换器操作的等变性和独立性优化激活分布。

Result: FPTQuant实现静态INT4量化，推理速度提升3.9倍，性能与全精度模型相当或优于多数现有方法。

Conclusion: FPTQuant在精度与速度间取得优异平衡，是高效量化LLMs的有效解决方案。
...
### [103] [Cautious Optimism: A Meta-Algorithm for Near-Constant Regret in General Games](https://arxiv.org/abs/2506.05005)
*Ashkan Soleymani,Georgios Piliouras,Gabriele Farina*

Main category: cs.LG

TLDR: 论文提出了一种名为“谨慎乐观”（Cautious Optimism）的框架，通过自适应调节学习速度，显著加速了正则化学习，并在一般游戏中实现了近乎最优的遗憾最小化。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于证明自适应调节学习速度的加速现象并非孤立，并探索在一般游戏中实现快速学习的可能性。

Method: 方法是通过将任何FTRL实例输入到Cautious Optimism框架中，生成加速的无遗憾学习算法，同时保持计算开销最小。

Result: 结果表明，该框架在自对弈中实现了近乎最优的O_T(log T)遗憾，同时在对抗场景中保持了最优的O(√T)遗憾。

Conclusion: 结论是该方法为一般游戏中的快速学习提供了一条新途径，且无需依赖单调步长。
...
### [104] [Towards Reasonable Concept Bottleneck Models](https://arxiv.org/abs/2506.05014)
...
<details>
  <summary>Details</summary>
Motivation: 解决传统概念瓶颈模型在概念间关系和任务性能上的不足，同时保持概念的重要性。

Method: 嵌入专家定义的概念间关系和概念-任务关系，使用正则化侧通道增强性能。

Result: CREAM主要依赖概念，任务性能与黑盒模型相当，且易于干预和减少概念泄漏。

Conclusion: CREAM在保持概念重要性的同时，通过显式关系和侧通道优化任务性能。
...
### [105] [Multi-Point Proximity Encoding For Vector-Mode Geospatial Machine Learning](https://arxiv.org/abs/2506.05016)
*John Collins*
...
<details>
  <summary>Details</summary>
Motivation: 传统机器学习模型需要将矢量地理空间数据（点、线、多边形）编码为适合的形式，现有方法如栅格化效果不佳。

Method: MPP编码通过计算形状到区域内参考点的缩放距离，捕捉几何特征和空间关系。
...
### [106] [Tuning the Right Foundation Models is What you Need for Partial Label Learning](https://arxiv.org/abs/2506.05027)
*Kuang He,Wei Tang,Tong Wei,Min-Ling Zhang*

Main category: cs.LG

TLDR: 论文研究了基于基础模型的部分标签学习（PLL），提出了PartialCLIP框架，并评估了11种基础模型在8个数据集上的表现，揭示了当前PLL方法的局限性和改进方向。


<details>
  <summary>Details</summary>
Motivation: 解决真实应用中部分标签学习的挑战，探索基础模型在PLL中的潜力。

Method: 提出PartialCLIP框架，评估11种基础模型在13种PLL方法和8个数据集上的表现。

Result: 发现PLL方法在使用基础模型时性能显著提升，但对模型选择和适应策略敏感。

Conclusion: 当前PLL方法存在局限性，PartialCLIP提供了改进方向，代码已开源。
...
### [107] [TIMING: Temporality-Aware Integrated Gradients for Time Series Explanation](https://arxiv.org/abs/2506.05035)
...
<details>
  <summary>Details</summary>
Motivation: 现有XAI方法在时间序列中仅关注点级归因幅度，忽略了方向性影响，导致关键点识别不足。

Method: 提出TIMING方法，结合时间感知改进IG，并引入CPD和CPP评估指标。
...
### [108] [iN2V: Bringing Transductive Node Embeddings to Inductive Graphs](https://arxiv.org/abs/2506.05039)
*Nicolas Lell,Ansgar Scherp*

Main category: cs.LG

TLDR: iN2V是一种将节点嵌入方法从转导式扩展到归纳式的方法，通过后处理计算新节点的嵌入，并在训练时优化嵌入以适应此过程。


<details>
  <summary>Details</summary>
Motivation: 传统的节点嵌入方法（如node2vec）仅适用于转导式场景，无法处理训练时未见的节点。iN2V旨在解决这一问题，使其适用于归纳式场景。

Method: iN2V结合了后处理步骤（计算新节点嵌入）和对原始N2V训练过程的修改，以优化嵌入适应后处理。

Result: 实验表明，iN2V在多个基准数据集上有效，平均提升节点分类准确率1个百分点，最高可达6个百分点。

Conclusion: iN2V是一种灵活的插件方法，可与其他嵌入方法结合，适用于归纳式节点表示学习。
...
### [109] [Reliably detecting model failures in deployment without labels](https://arxiv.org/abs/2506.05047)
*Viet Nguyen Changjian Shui,Vijay Giri,Siddarth Arya,Amol Verma,Fahad Razak,Rahul G. Krishnan*

Main category: cs.LG

TLDR: 论文提出了一种名为D3M的监控算法，用于检测模型在部署后的性能退化问题，通过预测模型的分歧来高效监控，并在实际数据上验证了其有效性。
...
Method: 提出了D3M算法，基于预测模型的分歧来监控性能退化，确保在非退化性变化下低误报率，在退化性变化下高检测率。

Result: 在标准基准和真实世界大规模内科数据集上的实验表明，D3M算法有效且适用于高风险机器学习管道的预警机制。

Conclusion: D3M是一种实用且高效的监控算法，能够有效检测模型性能退化，适用于动态环境中的机器学习应用。
...
### [110] [NIMO: a Nonlinear Interpretable MOdel](https://arxiv.org/abs/2506.05059)
*Shijian Xu,Marcello Massimo Negri,Volker Roth*

Main category: cs.LG

TLDR: NIMO（非线性可解释模型）结合了神经网络的强大预测能力和线性模型的可解释性，通过非线性修正线性预测，同时保持线性系数的可解释性。
...
Method: 使用基于轮廓似然的优化算法，动态优化神经网络参数并解析更新线性系数，同时引入自适应岭回归实现稀疏性约束。

Result: 实验表明，NIMO能恢复线性系数并显著提升预测准确性，同时在回归和分类任务中表现优于其他混合方法。

Conclusion: NIMO成功结合了神经网络和线性模型的优点，既保持了线性系数的可解释性，又提升了预测性能。
...
### [111] [UnHiPPO: Uncertainty-aware Initialization for State Space Models](https://arxiv.org/abs/2506.05065)
*Marten Lienen,Abdullah Saydemir,Stephan Günnemann*

Main category: cs.LG

TLDR: 论文扩展了HiPPO理论以处理测量噪声，提出了一种不确定性感知的状态空间模型初始化方法，提高了模型对噪声的鲁棒性。
...
### [112] [Semi-Implicit Variational Inference via Kernelized Path Gradient Descent](https://arxiv.org/abs/2506.05088)
*Tobias Pielok,Bernd Bischl,David Rügamer*

Main category: cs.LG

TLDR: 提出了一种基于核化KL散度估计器的半隐式变分推断方法，通过非参数平滑和重要性采样校正降低偏差和高方差，优化训练稳定性和效率。


<details>
  <summary>Details</summary>
Motivation: 传统半隐式变分推断（SIVI）在高维场景下因KL散度的高方差和偏差导致训练困难，现有方法（如KSIVI）仍存在训练成本较高的问题。

Method: 提出核化KL散度估计器，通过非参数平滑稳定训练，并引入重要性采样校正进一步降低偏差。理论分析表明其与Stein变分梯度下降的目标一致，但梯度方差更低。

Result: 实验表明，该方法在性能和训练效率上优于或匹配当前最优SIVI方法，且函数空间偏差较小，优化更稳定高效。

Conclusion: 该方法通过核化KL散度估计和重要性采样校正，显著提升了半隐式变分推断在高维场景下的训练稳定性和效率。
...
### [113] [Federated Isolation Forest for Efficient Anomaly Detection on Edge IoT Systems](https://arxiv.org/abs/2506.05138)
...
<details>
  <summary>Details</summary>
Motivation: 解决边缘设备和物联网系统中隐私保护和资源限制的问题，同时实现高效的异常检测。

Method: 使用基于隔离森林的FLiForest算法，结合Python和MicroPython联邦学习框架，开发了一种低资源、无监督的异常检测系统。

Result: 实验表明，系统在区分正常与异常读数时准确率超过96%，异常检测精度达78%以上，训练时内存占用低于160KB。
...
### [114] [Associative Memory and Generative Diffusion in the Zero-noise Limit](https://arxiv.org/abs/2506.05178)
*Joshua Hess,Quaid Morris*

Main category: cs.LG

TLDR: 论文研究了生成扩散模型与连续状态联想记忆模型之间的联系，强调了Morse-Smale动力系统作为梯度联想记忆模型的通用近似器，以及扩散模型作为其白噪声扰动系统。


<details>
  <summary>Details</summary>
Motivation: 探讨生成扩散模型与联想记忆模型之间的理论联系，揭示噪声水平变化下从生成到记忆的通用过渡特性。

Method: 利用Morse-Smale动力系统作为理论框架，分析梯度联想记忆模型和扩散模型的稳定性和结构特性。

Result: 在零噪声极限下，扩散模型和联想记忆模型的学习和生成景观表现出稳定性，仅在某些孤立点出现分岔。

Conclusion: 通过分岔序列可以描述这些景观的特性，并为经典和现代Hopfield网络（注意力机制）提供了结构稳定性标准。
...
### [115] [TreeRPO: Tree Relative Policy Optimization](https://arxiv.org/abs/2506.05183)
*Zhicheng Yang,Zhijiang Guo,Yinya Huang,Xiaodan Liang,Yiwei Wang,Jing Tang*

Main category: cs.LG

TLDR: 论文提出了一种名为TreeRPO的新方法，通过树采样估计推理步骤的奖励期望，显著提升LLMs的性能和效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法的奖励定义在全轨迹级别，无法为推理过程的中间步骤提供足够指导。

Method: 采用树采样估计步骤级奖励，结合GRPO的组相对奖励机制，生成细粒度奖励信号。

Result: TreeRPO将Qwen-2.5-Math的Pass@1准确率从19.0%提升至35.5%，性能优于GRPO且响应长度减少18.1%。

Conclusion: TreeRPO通过细粒度奖励信号有效提升LLMs的推理能力和效率。
...
### [116] [Locality Preserving Markovian Transition for Instance Retrieval](https://arxiv.org/abs/2506.05196)
*Jifei Luo,Wenzheng Wu,Hantao Yao,Lu Yu,Changsheng Xu*

Main category: cs.LG

TLDR: 论文提出了一种名为LPMT的框架，通过双向协作扩散和热力学马尔可夫转移过程解决扩散重排序中信号衰减的问题，提升了实例检索的全局和局部性能。


<details>
  <summary>Details</summary>
Motivation: 扩散重排序方法在数据流形建模中有效，但信号在远离源时会减弱，导致局部区域外的判别能力下降。

Method: LPMT框架结合双向协作扩散（BCD）和热力学马尔可夫转移（TMT）过程，通过局部状态嵌入（LSE）增强局部一致性。

Result: 实验结果表明，LPMT在多种任务中有效提升了实例检索的性能。

Conclusion: LPMT通过结合扩散和热力学过程，解决了信号衰减问题，实现了高效的全局检索和局部一致性。
...
### [117] [Transformers Meet In-Context Learning: A Universal Approximation Theory](https://arxiv.org/abs/2506.05200)
*Gen Li,Yuchen Jiao,Yu Huang,Yuting Wei,Yuxin Chen*

Main category: cs.LG

TLDR: 论文提出了一种基于通用函数逼近理论的方法，解释了Transformer如何通过上下文学习实现任务预测，无需微调或参数更新。


<details>
  <summary>Details</summary>
Motivation: 理解Transformer如何通过上下文学习（in-context learning）实现任务预测，而不依赖于优化算法的近似。

Method: 构建一个Transformer模型，利用通用函数逼近理论，仅通过少量上下文示例即可完成任务预测。

Result: 证明了Transformer可以在不更新权重的情况下，通过上下文示例实现可靠的预测，且适用范围远超凸问题和线性函数类。

Conclusion: 该方法揭示了Transformer如何同时学习通用表示并动态适应上下文示例，为上下文学习提供了新的理论支持。
...
### [118] [Mitigating Degree Bias Adaptively with Hard-to-Learn Nodes in Graph Contrastive Learning](https://arxiv.org/abs/2506.05214)
*Jingyu Hu,Hongbo Bo,Jun Hong,Xiaowei Liu,Weiru Liu*

Main category: cs.LG

TLDR: 论文提出了一种名为HAR的对比损失方法，通过增加正样本对并自适应加权来缓解GNN中的度偏差问题，并开发了SHARP框架验证其有效性。
...
### [119] [Learning Theory of Decentralized Robust Kernel-Based Learning Algorithm](https://arxiv.org/abs/2506.05215)
*Zhan Yu*
...
### [120] [A Unified Framework for Provably Efficient Algorithms to Estimate Shapley Values](https://arxiv.org/abs/2506.05216)
...
TLDR: 论文提出了一种统一框架，涵盖KernelSHAP及相关估计器，并首次为KernelSHAP提供了理论保证，同时通过实验验证了其在小到高维数据集上的有效性。


<details>
  <summary>Details</summary>
Motivation: 计算精确的Shapley值困难且计算量大，现有随机估计器（如KernelSHAP）缺乏理论保证，需统一框架和理论支持。

Method: 提出一个统一框架，涵盖KernelSHAP及相关估计器，采用替换与非替换采样策略，并证明其非渐近理论保证。

Result: 在中小维度数据集上验证了方法的低均方误差，并在高维数据集（如MNIST、CIFAR10）上实现了可扩展性改进。

Conclusion: 该框架首次为KernelSHAP提供了理论保证，揭示了现有估计器的权衡，并通过实验验证了其优越性。
...
### [121] [Diagonal Batching Unlocks Parallelism in Recurrent Memory Transformers for Long Contexts](https://arxiv.org/abs/2506.05229)
*Danil Sivtsov,Ivan Rodkin,Gleb Kuzmin,Yuri Kuratov,Ivan Oseledets*

Main category: cs.LG

TLDR: Diagonal Batching 是一种调度方案，通过消除 RMT 中的顺序执行约束，显著提升长上下文推理效率。


<details>
  <summary>Details</summary>
Motivation: Transformer 模型在长上下文推理中因二次时间和线性内存复杂度受限，RMT 虽降低复杂度但引入顺序执行瓶颈。

Method: 提出 Diagonal Batching，通过跨段并行化保留精确递归，无需重新训练现有 RMT 模型。

Result: 在 LLaMA-1B ARMT 模型上，Diagonal Batching 实现 3.3 倍速度提升，并减少推理成本与延迟。
...
### [122] [Progressive Tempering Sampler with Diffusion](https://arxiv.org/abs/2506.05231)
*Severi Rissanen,RuiKang OuYang,Jiajun He,Wenlin Chen,Markus Heinonen,Arno Solin,José Miguel Hernández-Lobato*

Main category: cs.LG

TLDR: 提出了一种结合并行退火（PT）和扩散模型的渐进退火采样器（PTSD），通过跨温度训练扩散模型并生成近似样本，显著提高了目标评估效率。


<details>
  <summary>Details</summary>
Motivation: 现有神经采样器在目标评估效率上不及并行退火（PT），而PT生成依赖样本且计算成本高。PTSD旨在结合两者的优势。

Method: PTSD通过顺序训练扩散模型，利用PT的优势改进神经采样器训练，并引入新方法结合高温扩散模型生成近似低温样本。

Result: PTSD显著提高了目标评估效率，优于基于扩散的神经采样器，并生成混合良好且不相关的样本。

Conclusion: PTSD结合了PT和扩散模型的优势，解决了现有方法的不足，为目标采样提供了高效解决方案。
...
### [123] [MesaNet: Sequence Modeling by Locally Optimal Test-Time Training](https://arxiv.org/abs/2506.05233)
...
TLDR: 论文提出了一种基于在线学习规则的稳定且可并行化的Mesa层，通过优化上下文损失提升语言模型性能，但增加了推理时的计算成本。


<details>
  <summary>Details</summary>
Motivation: 当前序列建模主要依赖因果Transformer，但其推理时内存和计算成本线性增长。研究旨在通过线性化softmax操作，开发高效且性能优越的RNN模型。

Method: 提出了一种数值稳定、可并行化的Mesa层，通过快速共轭梯度求解器优化上下文损失。

Result: 实验表明，该方法在语言建模和长上下文任务中优于现有RNN，但增加了推理计算成本。

Conclusion: 研究展示了通过增加推理时计算成本提升模型性能的潜力，尤其是在长上下文任务中。
...
### [124] [Evaluating Sparse Autoencoders: From Shallow Design to Matching Pursuit](https://arxiv.org/abs/2506.05239)
*Valérie Costa,Thomas Fel,Ekdeep Singh Lubana,Bahareh Tolooshams,Demba Ba*

Main category: cs.LG

TLDR: 稀疏自编码器（SAEs）在可解释性中起关键作用，但当前浅层架构依赖准正交性假设，限制了提取相关特征的能力。本文提出多迭代SAE（MP-SAE），通过残差引导提取相关特征，并在MNIST上验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 探索稀疏自编码器在提取相关特征时的局限性，并提出改进方法。

Method: 引入多迭代稀疏自编码器（MP-SAE），通过展开匹配追踪（Matching Pursuit）实现残差引导的特征提取。

Result: MP-SAE能够提取相关特征，并在重建过程中保证单调改进。

Conclusion: MP-SAE克服了浅层SAEs的局限性，适用于层次化场景（如手写数字生成）。
...
### [125] [Aligning Latent Spaces with Flow Priors](https://arxiv.org/abs/2506.05240)
*Yizhuo Li,Yuying Ge,Yixiao Ge,Ying Shan,Ping Luo*

Main category: cs.LG

TLDR: 提出了一种利用流模型作为先验对齐可学习潜在空间的新框架，通过预训练流模型捕获目标分布，并用对齐损失正则化潜在空间，避免了昂贵的似然计算和ODE求解。


<details>
  <summary>Details</summary>
Motivation: 解决潜在空间对齐问题，避免传统方法中计算复杂度高的似然评估和ODE求解。
...
### [126] [On the Convergence of Gradient Descent on Learning Transformers with Residual Connections](https://arxiv.org/abs/2506.05249)
*Zhen Qin,Jinxin Zhou,Zhihui Zhu*

Main category: cs.LG

TLDR: 本文分析了单层Transformer的收敛行为，证明了梯度下降在适当初始化下具有线性收敛速率，并揭示了残差连接对优化稳定性的积极作用。


<details>
  <summary>Details</summary>
Motivation: 尽管Transformer在应用中表现出色，但其训练动态的理论基础仍不完善，尤其是组件间的相互依赖关系未被充分研究。

Method: 分析包含自注意力、前馈网络和残差连接的单层Transformer的收敛行为，并扩展到多层架构。

Result: 在适当初始化下，梯度下降具有线性收敛速率，残差连接改善了输出矩阵的病态问题。

Conclusion: 残差连接有助于优化稳定性，理论分析得到了实验验证。
...
### [127] [Conservative classifiers do consistently well with improving agents: characterizing statistical and online learning](https://arxiv.org/abs/2506.05252)
*Dravyansh Sharma,Alec Sun*

Main category: cs.LG

TLDR: 本文研究了机器学习在战略分类中的可学习性，特别是在代理通过真正改进而非欺骗行为来获得理想分类时。作者通过引入新的概念类和对欧几里得球改进集的假设，扩展了现有理论，并解决了Attias等人提出的开放问题。


<details>
  <summary>Details</summary>
Motivation: 探讨在代理通过真正改进行为（而非欺骗）来适应分类算法时的学习性质，填补现有文献的空白。

Method: 引入不对称的最小一致概念类，研究欧几里得球改进集下的学习性质，并在有界噪声模型和在线学习中提出新方法。

Result: 在可实现设置中精确刻画了适当学习，并在更自然的假设下证明了非适当学习的可行性。同时，在有界噪声模型和在线学习中取得了更好的泛化误差和错误界限。

Conclusion: 本文扩展了战略分类中可学习性的理论框架，解决了开放问题，并为实际应用提供了更实用的学习保证。
...
### [128] [Learning long range dependencies through time reversal symmetry breaking](https://arxiv.org/abs/2506.05259)
*Guillaume Pourcel,Maxence Ernoult*

Main category: cs.LG

TLDR: 论文提出了一种名为RHEL的算法，用于训练基于物理原理的深度状态空间模型（SSMs），无需显式计算雅可比矩阵，且梯度估计无方差。RHEL在连续和离散时间下均有效，性能与BPTT相当。


<details>
  <summary>Details</summary>
Motivation: 研究动机是开发一种符合物理原理的学习算法，用于高效模拟和设计动态系统，特别是非耗散的哈密顿系统。

Method: 提出了RHEL算法，通过物理轨迹的有限差分计算损失梯度，仅需三次“前向传递”。还提出了离散时间版本的RHEL，与BPTT等效，并应用于哈密顿循环单元（HRUs）和哈密顿SSMs（HSSMs）。

Result: RHEL在多种时间序列任务中表现与BPTT相当，适用于序列长度达50k的分类和回归任务。

Conclusion: RHEL为设计可扩展、高能效的物理系统提供了新思路，具有自学习能力，适用于序列建模。
...
### [129] [Tight analyses of first-order methods with error feedback](https://arxiv.org/abs/2506.05271)
*Daniel Berg Thomsen,Adrien Taylor,Aymeric Dieuleveut*

Main category: cs.LG

TLDR: 本文对分布式学习中通信压缩的两种误差反馈方法（EF和EF²¹）进行了紧致分析，找到了最优收敛率的Lyapunov函数，并提供了严格的性能保证和比较。
...
Method: 通过寻找最优Lyapunov函数，对EF和EF²¹方法进行了紧致分析，并与压缩梯度下降进行了公平比较。

Result: 分析得出了每种方法的最优收敛率，并提供了匹配的下界，展示了EF、EF²¹和压缩梯度下降的机制差异。

Conclusion: 研究在简化但代表性的设置下提供了清晰的理论见解，为误差反馈方法的性能比较提供了严格依据。
...
### [130] [How to Unlock Time Series Editing? Diffusion-Driven Approach with Multi-Grained Control](https://arxiv.org/abs/2506.05276)
*Hao Yu,Chu Xin Cheng,Runlong Yu,Yuyang Ye,Shiwei Tong,Zhaofeng Liu,Defu Lian*

Main category: cs.LG

TLDR: CocktailEdit框架通过点级和段级控制实现时间序列编辑，结合置信加权锚点控制和分类器控制，保持时间连贯性。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以同时满足点级和段级约束的时间序列编辑需求。

Method: 提出CocktailEdit框架，结合置信加权锚点控制（点级约束）和分类器控制（段级统计属性）。

Result: 实验证明该框架在多种数据集和模型上有效，实现精确局部控制并保持时间连贯性。
...
### [131] [Fast-DataShapley: Neural Modeling for Training Data Valuation](https://arxiv.org/abs/2506.05281)
*Haifeng Sun,Yu Xiong,Runze Wu,Xinyu Cai,Changjie Fan,Lan Zhang,Xiang-Yang Li*

Main category: cs.LG

TLDR: Fast-DataShapley 是一种高效的单次训练方法，用于计算训练数据的 Shapley 值，避免了传统方法的高计算成本。


<details>
  <summary>Details</summary>
Motivation: 在人工智能行业中，训练数据的价值和版权保护至关重要，需要公平评估数据提供者的贡献。传统 Shapley 值方法计算成本高，且需要为每个测试样本重新训练。

Method: 提出 Fast-DataShapley，利用加权最小二乘法的 Shapley 值特性，训练可重用的解释模型，并引入三种方法降低训练开销。

Result: 实验表明，性能提升超过 2.5 倍，训练速度提高两个数量级。

Conclusion: Fast-DataShapley 显著提升了计算效率和性能，适用于大规模数据场景。
...
### [132] [Learning Beyond Experience: Generalizing to Unseen State Space with Reservoir Computing](https://arxiv.org/abs/2506.05292)
*Declan A. Norton,Yuanzhao Zhang,Michelle Girvan*

Main category: cs.LG

TLDR: 论文展示了储层计算（RC）在无显式结构先验的情况下，能够泛化到状态空间的未探索区域，并提出了一种多轨迹训练方案以提高泛化能力。
...
Method: 提出了一种多轨迹训练方案，支持在多个不连续时间序列上训练储层计算机，并应用于多稳态动态系统。

Result: 实验表明，即使仅在一个吸引盆上训练，RC也能捕捉未观察到的吸引盆中的系统行为。

Conclusion: 储层计算框架在无显式结构先验的情况下仍能实现跨域泛化，为动态系统建模提供了新思路。
...
### [133] [A Smooth Sea Never Made a Skilled $\texttt{SAILOR}$: Robust Imitation via Learning to Search](https://arxiv.org/abs/2506.05294)
*Arnav Kumar Jain,Vibhakar Mohta,Subin Kim,Atiksh Bhardwaj,Juntao Ren,Yunhai Feng,Sanjiban Choudhury,Gokul Swamy*

Main category: cs.LG

TLDR: 论文提出了一种名为SAILOR的方法，通过学习专家演示中的搜索能力（L2S），解决了行为克隆（BC）在模仿学习中的局限性，即在未见状态下无法恢复的问题。


<details>
  <summary>Details</summary>
Motivation: 行为克隆（BC）方法只能模仿专家在特定状态下的行为，无法处理未见状态下的错误恢复问题。本文旨在通过L2S方法，使智能体能够独立规划以匹配专家结果。

Method: 提出SAILOR方法，结合世界模型和奖励模型，通过稳定的样本高效学习，实现错误恢复行为。

Result: 在多个视觉操作任务中，SAILOR表现优于基于BC的Diffusion Policies，且即使增加BC的演示数据量，性能差距仍然存在。

Conclusion: SAILOR能够识别细微错误并避免奖励欺骗，展示了在模仿学习中独立规划的重要性。
...
### [134] [Sample Complexity and Representation Ability of Test-time Scaling Paradigms](https://arxiv.org/abs/2506.05295)
*Baihe Huang,Shanda Li,Tianhao Wu,Yiming Yang,Ameet Talwalkar,Kannan Ramchandran,Michael I. Jordan,Jiantao Jiao*

Main category: cs.LG

TLDR: 论文研究了测试时扩展策略（如自一致性、最佳-n和自校正）的样本效率，理论分析了其差异，并展示了自校正方法在多任务中的表达能力。


<details>
  <summary>Details</summary>
Motivation: 尽管测试时策略在大型语言模型中表现良好，但其样本效率的理论理解仍有限。

Method: 通过理论分析比较自一致性和最佳-n的样本需求，并研究自校正方法在多任务中的表达能力。

Result: 自一致性需要Θ(1/Δ²)样本，最佳-n仅需Θ(1/Δ)；自校正方法使Transformer能在测试时模拟在线学习。

Conclusion: 自校正方法在多任务中具有理论支持，且实验验证了其有效性。
...
### [135] [Power Law Guided Dynamic Sifting for Efficient Attention](https://arxiv.org/abs/2506.05300)
*Nirav Koley,Prajwal Singhania,Abhinav Bhatele*

Main category: cs.LG

TLDR: SiftAttention提出了一种新的近似注意力方法，通过动态阈值过滤减少GPU内存带宽使用，同时保持模型质量。


<details>
  <summary>Details</summary>
Motivation: 解决GPU上大语言模型推理时因内存带宽限制导致的效率问题，特别是注意力计算中HBM和SRAM之间的数据传输瓶颈。

Method: 用基于阈值的元素级过滤操作替代昂贵的top-$k$操作，动态估计每个生成步骤的阈值，仅加载和使用高于阈值的注意力分数和对应值向量。

Result: SiftAttention在减少内存带宽使用的同时，比现有近似注意力方法更好地保持了模型质量。

Conclusion: SiftAttention是一种高效的近似注意力方法，适用于GPU上的大语言模型推理。
...
### [136] [Learning normalized image densities via dual score matching](https://arxiv.org/abs/2506.05310)
*Florentin Guth,Zahra Kadkhodaie,Eero P Simoncelli*

Main category: cs.LG

TLDR: 提出了一种基于扩散生成模型的新框架，用于学习归一化能量模型，通过双评分匹配目标优化能量网络，在ImageNet64上取得与前沿技术相当的交叉熵值。
...
Method: 修改评分网络架构以计算能量，同时保留其归纳偏差，通过去噪目标和新的次要目标优化能量网络。

Result: 在ImageNet64上训练的能量模型取得与前沿技术相当的交叉熵值，并表现出强泛化能力。

Conclusion: 能量模型能够有效学习高维数据分布，且与传统假设不同，图像概率和局部邻域维度随内容变化显著。
...
### [137] [Improving Data Efficiency for LLM Reinforcement Fine-tuning Through Difficulty-targeted Online Data Selection and Rollout Replay](https://arxiv.org/abs/2506.05316)
*Yifan Sun,Jingyan Shen,Yibin Wang,Tianyu Chen,Zhendong Wang,Mingyuan Zhou,Huan Zhang*

Main category: cs.LG

TLDR: 本文提出两种技术（难度导向的在线数据选择和回放机制）以提高LLM强化学习微调的数据效率，显著减少训练时间。
...
Method: 提出难度导向的在线数据选择和回放机制，通过自适应难度估计和重用近期数据降低计算成本。

Result: 实验表明，方法在6种LLM-数据集组合中减少25%-65%的训练时间，达到与GRPO算法相同的性能。

Conclusion: 所提方法显著提升数据效率，为LLM微调提供更高效的解决方案。
...
### [138] [LSM-2: Learning from Incomplete Wearable Sensor Data](https://arxiv.org/abs/2506.05321)
...
TLDR: LSM-2 with AIM是一种新型自监督学习方法，直接从不完整数据中学习鲁棒表示，无需显式填充。


<details>
  <summary>Details</summary>
Motivation: 穿戴设备数据常存在缺失，传统自监督学习模型假设数据完整，难以处理此类问题。

Method: 提出AIM方法，使用可学习的掩码标记建模现有和人为引入的缺失，处理不完整数据。

Result: 在40M小时多模态传感器数据上预训练，LSM-2在分类、回归和生成任务中表现最佳。

Conclusion: AIM在真实穿戴数据应用中更可靠，尤其在临床相关缺失场景下表现优异。
...
### [139] [Seeing the Invisible: Machine learning-Based QPI Kernel Extraction via Latent Alignment](https://arxiv.org/abs/2506.05325)
*Yingshuai Ji,Haomin Zhuang,Matthew Toole,James McKenzie,Xiaolong Liu,Xiangliang Zhang*

Main category: cs.LG

TLDR: 提出首个基于AI的QPI核提取框架，通过两步学习策略解决多散射图像中的核提取问题。


<details>
  <summary>Details</summary>
Motivation: 多散射图像中提取单散射QPI模式（核）是一个病态逆问题，需要新方法。

Method: 采用两步学习策略：1）变分自编码器学习核的紧凑潜在空间；2）专用编码器对齐观测与核的潜在表示。

Result: 实验表明该方法提取精度显著提高，对未见核的泛化能力更强。
...
### [140] [Kinetics: Rethinking Test-Time Scaling Laws](https://arxiv.org/abs/2506.05333)
*Ranajoy Sadhukhan,Zhuoming Chen,Haizhong Zheng,Yang Zhou,Emma Strubell,Beidi Chen*

Main category: cs.LG

TLDR: 论文重新审视了测试时扩展规律，发现小模型的效果被高估，提出了一种新的Kinetics Scaling Law，结合计算和内存访问成本，并证明稀疏注意力模型优于密集模型。
...
Method: 通过分析0.6B到32B参数的模型，提出Kinetics Scaling Law，并设计基于稀疏注意力的新扩展范式，降低单令牌成本。
...
### [141] [Exploring Diffusion Transformer Designs via Grafting](https://arxiv.org/abs/2506.05340)
...
TLDR: 论文提出了一种称为“嫁接”的方法，通过编辑预训练的扩散变换器（DiTs）来研究新架构设计，显著降低了计算成本。


<details>
  <summary>Details</summary>
Motivation: 评估模型架构决策对质量的影响通常需要昂贵的预训练，限制了架构探索。论文旨在利用预训练模型研究新设计。

Method: 提出“嫁接”方法，通过替换预训练DiTs中的操作符（如注意力机制）和配置（如MLP结构），实现新架构设计。

Result: 许多混合设计在仅使用不到2%预训练计算的情况下达到良好质量（FID: 2.38-2.64）。文本到图像模型速度提升1.43倍，质量下降不到2%。

Conclusion: 通过嫁接预训练DiTs，可以高效探索新的扩散模型设计，从操作符替换到架构重构。
...
TLDR: 通过压缩键值（KV）缓存实现推理时超缩放，提升推理准确率而不增加计算成本。动态内存稀疏化（DMS）方法在8倍压缩下保持高精度。


<details>
  <summary>Details</summary>
Motivation: Transformer LLMs的推理成本受限于KV缓存大小，而非生成令牌数量。通过压缩KV缓存，可以在相同计算预算内生成更多令牌，提升推理准确率。

Method: 提出动态内存稀疏化（DMS），延迟令牌驱逐并隐式合并表示，仅需1K训练步骤即可实现8倍压缩。

Result: 在多个LLM家族中验证，DMS在相同推理时间和内存负载下提升准确率，例如Qwen-R1 32B在多个基准测试中平均提升7.6-9.6点。

Conclusion: 推理时超缩放结合DMS是一种实用方法，能显著提升LLM推理性能。
...
### [143] [GEM: Empowering LLM for both Embedding Generation and Language Understanding](https://arxiv.org/abs/2506.04344)
*Caojin Zhang,Qiang Zhang,Ke Li,Sai Vidyaranya Nuthalapati,Benyu Zhang,Jason Liu,Serena Li,Lizhu Zhang,Xiangjun Fan*

Main category: cs.CL

TLDR: 提出了一种名为GEM的自监督方法，使大型解码器语言模型（LLM）能够生成高质量文本嵌入，同时保持其原始文本生成和推理能力。
...
Method: 通过插入特殊标记并操纵注意力掩码生成文本的总结嵌入，适用于任何LLM的后训练或微调阶段。
...
### [144] [Effects of Speaker Count, Duration, and Accent Diversity on Zero-Shot Accent Robustness in Low-Resource ASR](https://arxiv.org/abs/2506.04364)
*Zheng-Xin Yong,Vineel Pratap,Michael Auli,Jean Maillard*

Main category: cs.CL

TLDR: 研究训练数据中说话者数量、每人音频时长和口音多样性对低资源ASR系统对未见口音鲁棒性的影响，发现增加说话者数量比增加每人时长更有效。


<details>
  <summary>Details</summary>
Motivation: 构建适用于全球各种口音的ASR系统，需研究训练数据变量对其鲁棒性的影响。

Method: 系统研究训练数据中说话者数量、每人音频时长和口音多样性对ASR性能的影响。

Result: 增加说话者数量比增加每人时长更有效；口音多样性在说话者数量固定时效果有限。

Conclusion: 建议在构建ASR训练数据时优先增加说话者数量。
...
### [145] [Mechanistic Decomposition of Sentence Representations](https://arxiv.org/abs/2506.04373)
*Matthieu Tehenan,Vikram Natarajan,Jonathan Michala,Milton Lin,Juri Opitz*

Main category: cs.CL

TLDR: 提出了一种新方法，通过字典学习分解句子嵌入为可解释组件，揭示其内部结构和特征。
...
Method: 使用字典学习对词级表示进行机制性分解，分析池化操作如何压缩特征为句子表示。

Result: 发现嵌入中许多语义和句法特征是线性编码的，揭示了句子嵌入空间的内部工作机制。

Conclusion: 该方法将词级机制解释性与句子级分析结合，提供了更透明和可控的表示。
...
### [146] [Hierarchical Text Classification Using Contrastive Learning Informed Path Guided Hierarchy](https://arxiv.org/abs/2506.04381)
*Neeraj Agrawal,Saurabh Kumar,Priyanka Bhatt,Tanishka Agarwal*

Main category: cs.CL

TLDR: HTC-CLIP结合对比学习与路径引导的层次结构，提升分层文本分类性能。


<details>
  <summary>Details</summary>
Motivation: 现有HTC模型分别处理标签层次结构或文本编码，但两者互补，需结合以提升性能。

Method: 提出HTC-CLIP，通过对比学习同时学习层次感知的文本表示和路径引导的层次表示，并在推理时结合两者概率分布。

Result: 在两个公开基准数据集上，HTC-CLIP的Macro F1分数比现有最优模型提高0.99-2.37%。
...
### [147] [MELABenchv1: Benchmarking Large Language Models against Smaller Fine-Tuned Models for Low-Resource Maltese NLP](https://arxiv.org/abs/2506.04385)
...
Result: 模型表现普遍不佳，预训练和指令调优中对马耳他语的接触是关键因素。微调效果优于提示但成本更高。

Conclusion: 建议低资源语言研究采用传统语言建模方法，强调预训练和微调的重要性。
...
### [148] [Building a Few-Shot Cross-Domain Multilingual NLU Model for Customer Care](https://arxiv.org/abs/2506.04389)
*Saurabh Kumar,Sourav Bansal,Neeraj Agrawal,Priyanka Bhatt*

Main category: cs.CL

TLDR: 提出一种结合嵌入器和分类器的模型架构，通过少量标注样本扩展领域特定模型到其他领域，提升跨领域意图分类性能。


<details>
  <summary>Details</summary>
Motivation: 跨领域数据稀缺限制了预训练模型在客户服务中的性能，需开发一种能泛化到多领域且仅需少量标注的意图分类器。

Method: 采用监督微调结合各向同性正则化训练领域特定嵌入器，并通过多语言知识蒸馏策略泛化到多领域，最后用简单线性分类器部署。

Result: 在加拿大和墨西哥电商客户服务数据集上，少样本意图检测准确率比现有SOTA模型提高20-23%。

Conclusion: 所提方法显著提升了跨领域意图分类性能，为实际应用提供了高效解决方案。
...
### [149] [MedAgentGym: Training LLM Agents for Code-Based Medical Reasoning at Scale](https://arxiv.org/abs/2506.04405)
...
TLDR: MedAgentGYM是一个公开的训练环境，旨在提升大型语言模型（LLM）在医学推理任务中的编码能力，包含72,413个任务实例。通过该平台，Med-Copilot-7B模型在性能上显著提升。


<details>
  <summary>Details</summary>
Motivation: 开发一个公开的训练环境，以增强LLM在医学领域的编码推理能力，填补现有资源的不足。

Method: MedAgentGYM提供129类任务，包含详细描述、交互反馈、真实标注和可扩展的训练轨迹生成。对30多种LLM进行基准测试。

Result: Med-Copilot-7B通过监督微调（+36.44%）和强化学习（+42.47%）显著提升性能，接近gpt-4o水平。

Conclusion: MedAgentGYM为生物医学研究和实践提供了一个集成平台，支持开发高性能、低成本的LLM编码助手。
...
### [150] [Unpacking Let Alone: Human-Scale Models Generalize to a Rare Construction in Form but not Meaning](https://arxiv.org/abs/2506.04408)
*Wesley Scivetti,Tatsuya Aoyama,Ethan Wilcox,Nathan Schneider*

Main category: cs.CL

TLDR: 人类规模的语言模型能泛化罕见语法形式，但对意义理解不足。
...
Method: 测试模型对英语LET-ALONE结构的语法形式和意义的理解，使用定制化基准。

Result: 模型能识别语法形式，但无法正确泛化意义。

Conclusion: 当前架构在语言形式和意义的学习效率上存在不对称性，与人类不同。
...
### [151] [Empaths at SemEval-2025 Task 11: Retrieval-Augmented Approach to Perceived Emotions Prediction](https://arxiv.org/abs/2506.04409)
*Lev Morozov,Aleksandr Mogilevskii,Alexander Shirnin*

Main category: cs.CL

TLDR: EmoRAG是一个无需额外训练、仅通过模型集成检测文本中感知情绪的系统，在SemEval-2025任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决多标签情绪检测任务，预测文本中说话者的感知情绪。

Method: 使用模型集成方法，无需额外训练。

Result: 性能与最佳系统相当，但更高效、可扩展且易于实现。

Conclusion: EmoRAG是一种高效且实用的情绪检测解决方案。
...
### [152] [Zero-Shot Open-Schema Entity Structure Discovery](https://arxiv.org/abs/2506.04458)
...
TLDR: 论文提出了一种名为ZOES的零样本开放模式实体结构发现方法，无需预定义模式或标注数据，通过丰富、精炼和统一机制提升实体结构提取的完整性和通用性。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的方法依赖预定义实体属性模式或标注数据集，导致提取结果不完整。

Method: ZOES采用丰富、精炼和统一的机制，利用实体与其结构的相互强化关系进行提取。

Result: 实验表明，ZOES在三个不同领域中均能显著提升大语言模型提取实体结构的完整性和通用性。
...
### [153] [Watermarking Degrades Alignment in Language Models: Analysis and Mitigation](https://arxiv.org/abs/2506.04462)
*Apurv Verma,NhatHai Phan,Shubhendu Trivedi*

Main category: cs.CL

TLDR: 本文分析了两种流行的水印方法（Gumbel和KGW）对大型语言模型（LLMs）在真实性、安全性和帮助性上的影响，并提出了一种名为Alignment Resampling（AR）的推理时采样方法以恢复模型的对齐性。
...
Method: 通过实验分析两种水印方法对四种对齐LLMs的影响，并提出AR方法，利用外部奖励模型恢复对齐性。

Result: 实验表明，AR方法仅需2-4次采样即可恢复或超越基线对齐分数，同时保持水印可检测性。

Conclusion: 揭示了水印强度与模型对齐之间的关键平衡，为实际部署水印LLMs提供了简单有效的解决方案。
...
### [154] [Aligning Large Language Models with Implicit Preferences from User-Generated Content](https://arxiv.org/abs/2506.04463)
...
TLDR: PUGC框架利用未标记的用户生成内容（UGC）中的隐式偏好生成偏好数据，避免了依赖昂贵的人工标注数据，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有偏好学习方法依赖人工或高级LLM标注数据，成本高且难以扩展。PUGC旨在通过UGC中的隐式偏好解决这一问题。

Method: PUGC将UGC转化为用户查询，生成策略模型的响应，并以UGC为参考文本进行评分，从而对齐隐式偏好。

Result: 实验显示，PUGC与DPO结合在Alpaca Eval 2上性能提升9.37%，并实现35.93%的SOTA胜率。
...
### [155] [SQLens: An End-to-End Framework for Error Detection and Correction in Text-to-SQL](https://arxiv.org/abs/2506.04494)
*Yue Gong,Chuan Lei,Xiao Qin,Kapil Vaidya,Balakrishnan Narayanaswamy,Tim Kraska*

Main category: cs.CL

TLDR: SQLens是一个端到端框架，用于检测和纠正LLM生成的SQL中的语义错误，显著提升错误检测和查询执行准确性。


<details>
  <summary>Details</summary>
Motivation: LLM生成的SQL查询可能存在语义错误但语法正确，缺乏可靠性洞察，需要更精细的错误检测和纠正方法。

Method: SQLens结合数据库和LLM的错误信号，识别SQL子句中的潜在语义错误，并指导查询纠正。

Result: 在两个公共基准测试中，SQLens在错误检测F1上比最佳LLM自评估方法高25.78%，并将文本到SQL系统的执行准确性提升高达20%。

Conclusion: SQLens通过精细的错误检测和纠正，显著提升了LLM生成SQL的可靠性和准确性。
...
### [156] [DRE: An Effective Dual-Refined Method for Integrating Small and Large Language Models in Open-Domain Dialogue Evaluation](https://arxiv.org/abs/2506.04516)
*Kun Zhao,Bohao Yang,Chen Tang,Siyuan Dai,Haoteng Tang,Chenghua Lin,Liang Zhan*

Main category: cs.CL

TLDR: SLIDE和DRE方法结合小型和大型语言模型的优势，通过自适应权重和双重优化提升对话评估的可靠性。
...
Method: 提出SLIDE方法，通过自适应权重整合SLMs和LLMs；进一步提出DRE方法，利用SLM生成的见解优化LLM的评估结果。

Result: 实验表明DRE优于现有方法，在多基准测试中更符合人类判断。

Conclusion: 结合小型和大型模型可显著提升开放任务（如对话评估）的可靠性。
...
### [157] [Please Translate Again: Two Simple Experiments on Whether Human-Like Reasoning Helps Translation](https://arxiv.org/abs/2506.04521)
*Di Wu,Seth Aycock,Christof Monz*

Main category: cs.CL

TLDR: 研究发现，在翻译任务中，显式分解翻译步骤（如Chain-of-Thought）并未带来明显性能提升，而简单提示模型“重新翻译”反而效果更好。


<details>
  <summary>Details</summary>
Motivation: 探究Chain-of-Thought（CoT）在翻译任务中的实际有效性。

Method: 通过实验比较显式分解翻译步骤与简单提示“重新翻译”的效果。

Result: 显式分解翻译步骤未显著提升性能，简单提示“重新翻译”效果更优。

Conclusion: 未来需进一步探索CoT在翻译任务中有效的因素。
...
### [158] [Is It JUST Semantics? A Case Study of Discourse Particle Understanding in LLMs](https://arxiv.org/abs/2506.04534)
*William Sheffield,Kanishka Misra,Valentina Pyatkin,Ashwini Deo,Kyle Mahowald,Junyi Jessy Li*

Main category: cs.CL

TLDR: 论文研究了大型语言模型（LLMs）区分英语中多义粒子“just”的细微语义差异的能力，发现LLMs在捕捉更微妙语义时存在局限。


<details>
  <summary>Details</summary>
Motivation: 探讨LLMs是否能准确识别多义粒子“just”的复杂语义，以评估其对语言细微差别的理解能力。

Method: 使用专家标注的数据集，测试LLMs对“just”不同语义类别的区分能力。

Result: LLMs能区分较宽泛的语义类别，但在捕捉更细微的语义差异时表现不佳。

Conclusion: LLMs对多义粒子的理解存在不足，尤其是在处理微妙语义时，显示出其局限性。
...
### [159] [BSBench: will your LLM find the largest prime number?](https://arxiv.org/abs/2506.04535)
*K. O. T. Erziev*

Main category: cs.CL

TLDR: 论文探讨了在无合理答案的问题上对LLMs进行基准测试的价值，并提出了相关方法和数据集修改方法。


<details>
  <summary>Details</summary>
Motivation: 研究动机是验证LLMs在面对无合理答案问题时的表现，揭示其局限性。

Method: 提出了一个基准测试方法，并展示了如何修改现有数据集以适应此类测试。

Result: 发现现有模型在此类问题上的表现远未达到完美。

Conclusion: 论文强调了此类测试的重要性，并提供了相关代码和数据。
...
### [160] [SSA-COMET: Do LLMs Outperform Learned Metrics in Evaluating MT for Under-Resourced African Languages?](https://arxiv.org/abs/2506.04557)
...
TLDR: 论文介绍了SSA-MTE数据集和SSA-COMET评估指标，用于提升非洲低资源语言的机器翻译质量评估，性能优于现有方法。
...
Method: 构建了包含13种非洲语言对的大规模人工标注数据集SSA-MTE，并开发了SSA-COMET和SSA-COMET-QE评估指标，同时测试了基于LLM的提示方法。

Result: SSA-COMET显著优于AfriCOMET，并与最强LLM（Gemini 2.5 Pro）竞争，尤其在低资源语言中表现突出。
...
### [161] [Demonstrations of Integrity Attacks in Multi-Agent Systems](https://arxiv.org/abs/2506.04572)
*Can Zheng,Yuhan Cao,Xiaoning Dong,Tianxing He*

Main category: cs.CL

TLDR: 论文探讨了恶意代理在多智能体系统中通过提示操纵实施的四种攻击类型，揭示了现有监控系统的局限性，并呼吁加强安全协议。


<details>
  <summary>Details</summary>
Motivation: 研究多智能体系统（MAS）中恶意代理如何通过提示操纵破坏系统完整性，以揭示当前监控机制的不足。

Method: 分析了四种攻击类型（Scapegoater、Boaster、Self-Dealer、Free-Rider），并通过实验验证提示操纵对MAS行为的影响。

Result: 实验表明，精心设计的提示可以系统性偏置MAS行为，甚至绕过基于LLM的监控系统（如GPT-4o-mini）。

Conclusion: 强调需要设计更安全的MAS架构，包含鲁棒的安全协议和内容验证机制，以及全面的风险评估监控系统。
...
### [162] [Reasoning or Overthinking: Evaluating Large Language Models on Financial Sentiment Analysis](https://arxiv.org/abs/2506.04574)
*Dimitris Vamvourellis,Dhagash Mehta*

Main category: cs.CL

TLDR: 研究大型语言模型（LLMs）在零样本金融情感分析中的表现，发现直觉型（System 1）思维比推理型（System 2）更贴近人类判断。
...
Method: 使用Financial PhraseBank数据集，比较不同LLMs（如GPT-4o、FinBERT）及提示策略（System 1 vs. System 2）。

Result: GPT-4o无需推理提示表现最佳，推理可能因过度思考导致性能下降。

Conclusion: 金融情感分析中，直觉型思维优于推理型，挑战了推理必然提升性能的假设。
...
### [163] [Are LLMs Reliable Translators of Logical Reasoning Across Lexically Diversified Contexts?](https://arxiv.org/abs/2506.04575)
*Qingchuan Li,Jiatong Li,Zirui Liu,Mingyue Cheng,Yuting Zeng,Qi Liu,Tongxuan Liu*

Main category: cs.CL

TLDR: 论文提出SCALe基准和MenTaL方法，解决LLMs在逻辑翻译中因词汇多样性导致的不可靠问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLMs在逻辑推理中作为翻译器时，难以处理词汇多样性，且现有基准缺乏词汇多样性，掩盖了这一问题。
...
### [164] [Selecting Demonstrations for Many-Shot In-Context Learning via Gradient Matching](https://arxiv.org/abs/2506.04579)
*Jianfei Zhang,Bei Li,Jun Bai,Rumei Li,Yanmeng Wang,Chenghua Lin,Wenge Rong*

Main category: cs.CL

TLDR: 论文提出了一种梯度匹配方法，用于优化大语言模型（LLMs）中的多示例上下文学习（ICL）演示选择问题，显著优于随机选择方法。


<details>
  <summary>Details</summary>
Motivation: 现有ICL方法在多示例场景下依赖随机选择演示，效果受限，而实例级检索不适用。作者假设ICL与微调的数据需求类似，因此提出梯度匹配方法。

Method: 通过梯度匹配方法，将目标任务的整个训练集的微调梯度与所选示例对齐，以在所选示例中逼近整个训练集的学习效果。

Result: 在多种数据集和模型（如Qwen2.5-72B和Llama3-70B）上，该方法比随机选择性能提升4%，在5个闭源LLMs上提升约2%。

Conclusion: 梯度匹配方法为多示例ICL提供了更可靠和高效的解决方案，推动了其更广泛应用。
...
### [165] [SUCEA: Reasoning-Intensive Retrieval for Adversarial Fact-checking through Claim Decomposition and Editing](https://arxiv.org/abs/2506.04583)
*Hongjun Liu,Yilun Zhao,Arman Cohan,Chen Zhao*

Main category: cs.CL

TLDR: 论文提出了一种名为SUCEA的无训练方法，通过重新表述对抗性声明以更易找到支持证据，显著提升了检索和标签预测的准确性。
...
Result: 在两个挑战性事实核查数据集上，SUCEA显著优于四种基于声明分解的基线方法。

Conclusion: SUCEA框架有效解决了对抗性声明的挑战，提升了事实核查的准确性和效率。
...
### [166] [MuSciClaims: Multimodal Scientific Claim Verification](https://arxiv.org/abs/2506.04585)
*Yash Kumar Lal,Manikanta Bandham,Mohammad Saqib Hasan,Apoorva Kashi,Mahnaz Koupaee,Niranjan Balasubramanian*

Main category: cs.CL

TLDR: 论文提出新基准MuSciClaims，用于测试科学文献中多模态数据的声明验证能力，发现现有视觉语言模型表现不佳。
...
Method: 通过自动提取科学文章中的支持声明并手动扰动生成矛盾声明，设计诊断任务评估模型能力。

Result: 现有视觉语言模型表现较差（F1 0.3-0.5），最佳模型仅达0.77，且存在偏向支持声明的偏见。

Conclusion: 模型在多模态证据定位、信息整合和图表基础理解方面存在显著不足，需进一步改进。
...
### [167] [LESS: Large Language Model Enhanced Semi-Supervised Learning for Speech Foundational Models](https://arxiv.org/abs/2506.04586)
*Wen Ding,Fan Qian*

Main category: cs.CL

TLDR: LESS框架利用大语言模型（LLM）优化半监督学习中的伪标签，通过数据过滤策略提升效率，在ASR和AST任务中显著降低错误率。
...
### [168] [Safe: Enhancing Mathematical Reasoning in Large Language Models via Retrospective Step-aware Formal Verification](https://arxiv.org/abs/2506.04592)
*Chengwu Liu,Ye Yuan,Yichun Yin,Yan Xu,Xin Xu,Zaoyu Chen,Yasheng Wang,Lifeng Shang,Qun Liu,Ming Zhang*

Main category: cs.CL

TLDR: 论文提出了一个名为$Safe$的框架，通过形式化数学语言Lean 4验证LLM生成的推理步骤，以减少幻觉并提供可验证的证据。


<details>
  <summary>Details</summary>
Motivation: 当前CoT方法（如PRMs或自一致性）缺乏可验证的证据，难以检测幻觉，因此需要一种更可靠的方法。

Method: 采用形式化数学语言Lean 4，在每一步推理中生成形式化证明，以识别和验证推理的正确性。

Result: 在多个LLM和数学数据集上验证了$Safe$的有效性，性能显著提升，并提供了可解释的证据。还提出了包含30,809个形式化语句的基准$FormalStep$。

Conclusion: $Safe$是首个利用Lean 4验证LLM生成内容的框架，为减少幻觉提供了可靠的基础。
...
### [169] [A MISMATCHED Benchmark for Scientific Natural Language Inference](https://arxiv.org/abs/2506.04603)
*Firoz Shaik,Mobashir Sadat,Nikita Gautam,Doina Caragea,Cornelia Caragea*

Main category: cs.CL

TLDR: 论文提出了一个新的科学自然语言推理（NLI）评估基准MISMATCHED，覆盖非计算机科学领域，并展示了预训练语言模型的基线性能。


<details>
  <summary>Details</summary>
Motivation: 现有科学NLI数据集仅关注计算机科学领域，忽略了其他学科，因此需要扩展至非CS领域。

Method: 构建MISMATCHED数据集，包含2700个人工标注的句子对，涵盖心理学、工程学和公共卫生领域，并使用预训练语言模型（SLMs和LLMs）建立基线。

Result: 最佳基线模型的Macro F1为78.17%，显示未来改进空间大。同时，训练中加入隐含科学NLI关系的句子对可提升模型性能。

Conclusion: MISMATCHED填补了非CS领域科学NLI的空白，为未来研究提供了新基准和方向。
...
### [170] [Revisiting Test-Time Scaling: A Survey and a Diversity-Aware Method for Efficient Reasoning](https://arxiv.org/abs/2506.04611)
*Ho-Lam Chung,Teng-Yun Hsiao,Hsiao-Ying Huang,Chunerh Cho,Jian-Ren Lin,Zhang Ziwei,Yun-Nung Chen*

Main category: cs.CL

TLDR: TTS通过推理时分配额外计算提升LLMs推理性能，ADAPT方法通过多样性优化前缀微调显著减少计算需求。


<details>
  <summary>Details</summary>
Motivation: 推理优化模型输出多样性不足限制了TTS效果，需解决多样性问题以最大化TTS效能。

Method: 提出ADAPT方法，结合多样性数据策略的前缀微调，轻量且高效。

Result: 数学推理任务中，ADAPT以8倍计算节省达到80%准确率。
...
### [171] [Subjective Perspectives within Learned Representations Predict High-Impact Innovation](https://arxiv.org/abs/2506.04616)
*Likun Cao,Rui Pan,James Evans*

Main category: cs.CL

TLDR: 论文通过机器学习方法量化创新者的主观视角和人际创新机会，发现视角多样性比背景多样性更能预测创造性成就。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探索创新者的主观视角和人际创新机会如何影响其未来的创造性成果。

Method: 利用动态语言表示构建几何空间，量化创新者的主观视角和背景多样性，并通过自然实验和AI模拟验证。

Result: 视角多样性显著预测创造性成就，而背景多样性则相反。

Conclusion: 成功的合作者通过共同语言整合多样经验，推动创新，这对团队组建和研究政策有重要意义。
...
### [172] [Static Word Embeddings for Sentence Semantic Representation](https://arxiv.org/abs/2506.04624)
*Takashi Wada,Yuki Hirakawa,Ryotaro Shimizu,Takahiro Kawashima,Yuki Saito*

Main category: cs.CL

TLDR: 提出了一种优化句子语义表示的静态词嵌入方法，通过改进预训练句子转换器的词嵌入，结合句子级主成分分析和知识蒸馏或对比学习，显著优于现有静态模型。


<details>
  <summary>Details</summary>
Motivation: 改进静态词嵌入在句子语义表示中的性能，使其在低计算成本下接近动态模型的性能。

Method: 从预训练句子转换器中提取词嵌入，通过句子级主成分分析改进，再结合知识蒸馏或对比学习优化。推理时通过简单平均词嵌入表示句子。

Result: 在单语和跨语言任务中显著优于现有静态模型，部分数据集上接近SimCSE性能。

Conclusion: 方法成功去除了与句子语义无关的词嵌入成分，并根据单词对句子语义的影响调整向量范数。
...
### [173] [Advancing Tool-Augmented Large Language Models via Meta-Verification and Reflection Learning](https://arxiv.org/abs/2506.04625)
*Zhiyuan Ma,Jiayu Liu,Xianzhen Luo,Zhenya Huang,Qingfu Zhu,Wanxiang Che*

Main category: cs.CL

TLDR: Tool-MVR通过多智能体验证和探索式反思学习，解决了LLM工具利用中的不可靠规划和弱反思问题，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在工具利用中存在不可靠的规划和弱反思能力，限制了AI代理解决复杂问题的能力。
...
### [174] [ViCocktail: Automated Multi-Modal Data Collection for Vietnamese Audio-Visual Speech Recognition](https://arxiv.org/abs/2506.04635)
*Thai-Binh Nguyen,Thi Van Nguyen,Quoc Truong Do,Chi Mai Luong*

Main category: cs.CL

TLDR: 本文提出了一种从原始视频生成AVSR数据集的实用方法，并通过越南语基线模型验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 传统语音识别系统仅依赖音频特征，易受噪声干扰，而AVSR虽更鲁棒，但多语言数据集稀缺。

Method: 改进现有技术，从原始视频自动生成AVSR数据集，并开发越南语基线模型。

Result: 自动收集的数据集在干净环境下表现与ASR相当，在嘈杂环境中显著优于ASR。
...
### [175] [TaDA: Training-free recipe for Decoding with Adaptive KV Cache Compression and Mean-centering](https://arxiv.org/abs/2506.04642)
*Vinay Joshi,Pratik Prabhanjan Brahma,Zicheng Liu,Emad Barsoum*

Main category: cs.CL

TLDR: TaDA是一种无需训练的KV缓存压缩方法，通过自适应量化精度和均值中心化消除异常值处理，显著减少内存占用并保持准确性。
...
Conclusion: TaDA为语言模型的可扩展高性能推理铺平了道路，支持更长上下文和推理链。
...
### [176] [Flex-TravelPlanner: A Benchmark for Flexible Planning with Language Agents](https://arxiv.org/abs/2506.04649)
*Juhyun Oh,Eunsu Kim,Alice Oh*

Main category: cs.CL

TLDR: 论文介绍了Flex-TravelPlanner基准，用于评估语言模型在动态规划场景中的灵活推理能力，发现模型在多轮任务中表现不佳，且约束优先级处理存在问题。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注静态单轮场景，无法满足现实规划问题的动态需求，因此需要更真实的评估方法。

Method: 基于TravelPlanner数据集，引入多轮约束引入和优先级竞争约束两种新评估设置，分析GPT-4o和Llama 3.1 70B的表现。

Result: 模型在多轮任务中表现不稳定，约束引入顺序影响性能，且难以正确处理约束优先级。

Conclusion: 动态规划场景的评估对提升模型性能至关重要，未来需优化模型在复杂任务中的表现。
...
### [177] [Normative Conflicts and Shallow AI Alignment](https://arxiv.org/abs/2506.04679)
*Raphaël Millière*

Main category: cs.CL

TLDR: 论文探讨了大型语言模型（LLMs）的价值对齐问题，指出当前对齐策略无法防止滥用，并揭示了其脆弱性源于缺乏真正的规范推理能力。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统（如LLMs）的进步，安全部署问题日益突出，当前的对齐策略无法有效防止滥用，亟需解决这一根本性缺陷。

Method: 通过分析人类道德心理学研究，对比LLMs的规范推理能力，揭示现有对齐方法的局限性。

Result: LLMs缺乏检测和解决规范冲突的能力，易受对抗性攻击，当前方法无法解决这一脆弱性。

Conclusion: 现有对齐方法不足以应对AI系统潜在危害，需开发更深入的规范推理能力以确保安全部署。
...
### [178] [MMRefine: Unveiling the Obstacles to Robust Refinement in Multimodal Large Language Models](https://arxiv.org/abs/2506.04688)
...
Method: 通过六种场景和六种错误类型评估MLLMs的细化性能，并对开放和封闭MLLMs进行实验。

Result: 实验揭示了细化性能的瓶颈和改进方向。

Conclusion: MMRefine为MLLMs的推理能力提升提供了框架和数据支持。
...
### [179] [Recycling the Web: A Method to Enhance Pre-training Data Quality and Quantity for Language Models](https://arxiv.org/abs/2506.04689)
*Thao Nguyen,Yang Li,Olga Golovneva,Luke Zettlemoyer,Sewoong Oh,Ludwig Schmidt,Xian Li*

Main category: cs.CL

TLDR: REWIRE方法通过改写低质量网络文本，将其转化为可用于训练的数据，从而突破预训练数据瓶颈，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 解决预训练数据增长缓慢且高质量文本稀缺的问题，探索如何利用被过滤的低质量数据。

Method: 提出REWIRE方法，通过改写低质量文档，将其转化为有用数据，并与高质量原始文本混合训练。

Result: 在1B、3B和7B规模的实验中，混合数据分别带来1.0、1.3和2.5个百分点的性能提升，且效果优于单纯增加数据量。

Conclusion: 改写和回收低质量网络文本是一种简单有效的预训练数据扩展方法。
...
### [180] [Cracking the Code: Enhancing Implicit Hate Speech Detection through Coding Classification](https://arxiv.org/abs/2506.04693)
*Lu Wei,Liangzhi Li,Tong Xiang,Xiao Liu,Noa Garcia*

Main category: cs.CL

TLDR: 提出了一种新的隐式仇恨言论（im-HS）检测分类法，通过六种编码策略（codetypes）改进检测方法，实验证明其在多语言数据集上有效。


<details>
  <summary>Details</summary>
Motivation: 互联网上的隐式仇恨言论（im-HS）难以被现有方法检测，威胁社会和谐与个人福祉。

Method: 1）直接提示大语言模型（LLMs）分类句子；2）在编码过程中嵌入codetypes。

Result: codetypes显著提升了中英文数据集中的im-HS检测效果。
...
TLDR: HSSBench是一个专门用于评估多模态大语言模型（MLLMs）在人文与社会科学（HSS）领域能力的基准测试，填补了现有基准测试的不足。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注STEM领域的垂直推理，忽视了HSS领域的横向跨学科思维需求。HSSBench旨在解决这一差距。

Method: 通过多领域专家与自动化代理协作生成和迭代优化样本，构建了包含13,000多个样本的HSSBench基准测试。

Result: 测试了20多个主流MLLMs，结果显示即使是先进模型也面临显著挑战。
...
TLDR: STAND是一种无需模型的推测解码方法，通过利用推理轨迹中的冗余性，显著加速推理过程且不损失准确性。
...
TLDR: 本文介绍了IIITH-BUT团队在IWSLT 2025共享任务中针对低资源Bhojpuri-Hindi语言对的语音翻译研究，重点探讨了超参数优化和数据增强技术对SeamlessM4T模型性能的影响。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决低资源语言对（Bhojpuri-Hindi）在语音翻译任务中的数据稀缺问题，并通过优化技术提升翻译质量。

Method: 方法包括系统性地优化超参数（如学习率、更新步数等）、应用数据增强技术（如速度扰动和SpecAugment），以及利用跨语言信号（联合训练Marathi和Bhojpuri数据）。

Result: 实验结果表明，超参数优化和简单有效的数据增强技术显著提升了低资源环境下的翻译性能。

Conclusion: 结论是超参数选择和数据增强对低资源语音翻译任务至关重要，同时分析了翻译错误对BLEU评分的影响。
...
TLDR: SPARTA ALIGNMENT是一种通过竞争和对抗集体对齐多个LLM的算法，利用模型间的互评和竞争提升性能。


<details>
  <summary>Details</summary>
Motivation: 解决单一模型在生成多样性和评估偏见上的不足，通过多模型竞争和互评实现自我进化。

Method: 多个LLM组成“斯巴达部落”，通过竞争和互评生成偏好对，模型从中学习。

Result: 在12个任务和数据集中的10个上优于初始模型和基线，平均提升7.0%。

Conclusion: SPARTA ALIGNMENT能有效提升模型性能，并利用模型多样性生成更优输出。
...
### [185] [Lifelong Evolution: Collaborative Learning between Large and Small Language Models for Continuous Emergent Fake News Detection](https://arxiv.org/abs/2506.04739)
*Ziyi Zhou,Xiaoming Zhang,Litian Zhang,Yibo Zhang,Zhenyu Guan,Chaozhuo Li,Philip S. Yu*

Main category: cs.CL

TLDR: 提出了一种名为C²EFND的新框架，结合大型语言模型（LLM）和小型语言模型（SLM）的优势，通过多轮协作学习和知识更新模块，显著提升了虚假新闻检测的准确性和适应性。


<details>
  <summary>Details</summary>
Motivation: 虚假新闻在社交媒体上的广泛传播对社会造成严重影响，传统深度学习方法和小型语言模型因数据稀缺和分布变化难以适应，而大型语言模型在零样本能力上虽强，但因知识过时和缺乏合适示例而检测不准确。

Method: 提出C²EFND框架，结合LLM的泛化能力和SLM的分类专长，采用多轮协作学习；引入基于Mixture-of-Experts架构的终身知识编辑模块和基于回放的持续学习方法。

Result: 在Pheme和Twitter16数据集上的实验表明，C²EFND显著优于现有方法，提升了检测准确性和在持续涌现虚假新闻场景中的适应性。

Conclusion: C²EFND框架通过协作学习和知识更新，有效解决了虚假新闻检测中的挑战，为未来研究提供了新方向。
...
### [186] [Identifying Reliable Evaluation Metrics for Scientific Text Revision](https://arxiv.org/abs/2506.04772)
*Léane Jourdan,Florian Boudin,Richard Dufour,Nicolas Hernandez*

Main category: cs.CL

TLDR: 本文探讨了科学写作中文本修订的评估挑战，分析了传统指标的局限性，并提出结合LLM和领域特定指标的混合方法。
...
Conclusion: 结合LLM和领域特定指标的混合方法能更可靠地评估修订质量。
...
### [187] [Fine-Grained Interpretation of Political Opinions in Large Language Models](https://arxiv.org/abs/2506.04774)
*Jingyu Hu,Mengyue Yang,Mengnan Du,Weiru Liu*

Main category: cs.CL

TLDR: 论文研究了LLMs的政治观点，发现其开放回答与内部意图存在偏差，提出多维度政治学习框架和可解释表示工程技术，以更透明地学习政治概念。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要依赖LLMs的开放回答评估政治观点，但发现其回答与内部意图不一致，且单轴概念分析易导致混淆。

Method: 设计了四维政治学习框架，构建数据集用于细粒度政治概念向量学习，并在八个开源LLMs上应用三种表示工程技术进行实验。

Result: 实验表明，这些向量能解构政治概念混淆，检测任务验证了向量的语义意义，干预实验显示向量可改变LLMs的政治倾向回答。

Conclusion: 多维度框架和表示工程技术能更透明地学习LLMs的政治概念，并有效干预其政治倾向。
...
### [188] [MMSU: A Massive Multi-task Spoken Language Understanding and Reasoning Benchmark](https://arxiv.org/abs/2506.04779)
*Dingdong Wang,Jincenzi Wu,Junan Li,Dongchao Yang,Xueyuan Chen,Tianhua Zhang,Helen Meng*

Main category: cs.CL

TLDR: 论文提出了一个名为MMSU的基准测试，用于评估语音大语言模型在自然语音中的细粒度感知和复杂推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有语音大语言模型在自然语音中的细粒度感知和复杂推理能力尚未充分探索，需要新的评估标准。

Method: 设计了包含5,000个音频-问题-答案三元组的MMSU基准，涵盖47个任务和多种语言现象。

Result: 评估了14种先进语音大语言模型，发现现有模型仍有显著改进空间。

Conclusion: MMSU为语音理解提供了全面评估标准，并为未来优化指明了方向。
...
### [189] [Towards LLM-Centric Multimodal Fusion: A Survey on Integration Strategies and Techniques](https://arxiv.org/abs/2506.04788)
*Jisu An,Junseok Lee,Jeoungeun Lee,Yongseok Son*

Main category: cs.CL

TLDR: 本文综述了多模态大语言模型（MLLMs）的发展，提出了一个基于三个维度的分类框架，分析了125个模型，旨在为未来研究提供指导。
...
Result: 总结了模态整合的当前方法，揭示了领域内的新兴模式。

Conclusion: 分类框架为研究人员提供了结构化概述，指导未来基于预训练基础的多模态整合策略开发。
...
### [190] [Dissecting Logical Reasoning in LLMs: A Fine-Grained Evaluation and Supervision Study](https://arxiv.org/abs/2506.04810)
*Yujun Zhou,Jiayi Ye,Zipeng Ling,Yufei Han,Yue Huang,Haomin Zhuang,Zhenwen Liang,Kehan Guo,Taicheng Guo,Xiangqi Wang,Xiangliang Zhang*

Main category: cs.CL

TLDR: FineLogic是一个细粒度评估框架，用于评估LLMs的逻辑推理能力，涵盖准确性、步骤合理性和表示对齐三个维度。研究发现自然语言监督在泛化性上表现优异，而符号监督则促进结构化的推理链。
...
Result: 自然语言监督在泛化和长上下文任务中表现优异，符号监督促进结构化推理；微调主要通过逐步生成改善推理行为。
...
### [191] [Design of intelligent proofreading system for English translation based on CNN and BERT](https://arxiv.org/abs/2506.04811)
*Feijun Liu,Huifeng Wang,Kun Wang,Yizhen Wang*

Main category: cs.CL

TLDR: 本文提出了一种结合CNN和BERT的混合方法，用于机器翻译校对，通过端到端训练实现高效错误检测和修正，实验结果显示性能优于现有技术。
...
Method: 结合CNN和BERT的混合方法，CNN用于提取局部n-gram模式，BERT用于生成上下文丰富的序列表示，并通过注意力机制检测错误。修正模块使用GRU解码器和翻译记忆库提出合理修改。
...
### [192] [Evaluating Vision-Language and Large Language Models for Automated Student Assessment in Indonesian Classrooms](https://arxiv.org/abs/2506.04822)
*Nurul Aisyah,Muhammad Dehan Al Kautsar,Arif Hidayat,Raqib Chowdhury,Fajri Koto*

Main category: cs.CL

TLDR: 研究了视觉语言模型（VLM）和大语言模型（LLM）在印尼四年级学生手写考试答案评估中的表现，发现VLM在手写识别上存在困难，但LLM生成的反馈仍有一定实用性。
...
Method: 评估了VLM和多个LLM对646份印尼四年级学生手写考试答案（涵盖数学和英语）的表现，包括评分和生成个性化反馈。
...
### [193] [A Reasoning-Based Approach to Cryptic Crossword Clue Solving](https://arxiv.org/abs/2506.04824)
*Martin Andrews,Sam Witteveen*

Main category: cs.CL

TLDR: 论文提出了一种基于LLM的系统，用于解决加密填字游戏线索，通过假设答案、提出文字游戏解释和验证步骤，实现了在Cryptonite数据集上的最佳性能。


<details>
  <summary>Details</summary>
Motivation: 加密填字游戏线索是一种具有挑战性的语言任务，现有方法需要改进。

Method: 系统结合假设生成、文字游戏解释和验证步骤，使用Python表达解决方案。

Result: 在Cryptonite数据集上实现了新的最佳性能。

Conclusion: 该系统为加密填字游戏提供了可解释的解决方案。
...
### [194] [Joint Evaluation of Answer and Reasoning Consistency for Hallucination Detection in Large Reasoning Models](https://arxiv.org/abs/2506.04832)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法难以检测LRMs中推理步骤的冗余或逻辑不一致问题，而这些问题可能导致幻觉。

Method: 提出RACE框架，提取关键推理步骤并计算四种诊断信号：推理痕迹的样本间一致性、基于熵的答案不确定性、推理与答案的语义对齐以及推理的内部一致性。

Result: 实验表明，RACE在多个数据集和LLMs上优于现有基线方法。
...
### [195] [MockConf: A Student Interpretation Dataset: Analysis, Word- and Span-level Alignment and Baselines](https://arxiv.org/abs/2506.04848)
*Dávid Javorský,Ondřej Bojar,François Yvon*

Main category: cs.CL

TLDR: 论文介绍了MockConf数据集和InterAlign工具，用于自动对齐和分析同声传译任务。
...
Method: 收集学生模拟会议的同传录音（7小时，5种语言），开发基于网页的标注工具InterAlign，并提出自动对齐的基线方法。

Result: 发布了MockConf数据集和InterAlign工具，支持跨语言词和片段对齐，适用于同传分析。

Conclusion: MockConf和InterAlign填补了同传研究的数据和工具空白，为自动对齐和评估提供了资源。
...
### [196] [Multiple-Choice Question Generation Using Large Language Models: Methodology and Educator Insights](https://arxiv.org/abs/2506.04851)
...
### [197] [Prompting LLMs: Length Control for Isometric Machine Translation](https://arxiv.org/abs/2506.04855)
*Dávid Javorský,Ondřej Bojar,François Yvon*
...
Method: 使用8种不同规模的开源LLMs，通过调整提示策略、少样本示例数量和演示选择进行实验。

Result: 指令措辞与演示对齐对长度控制至关重要；极端示例能缩短翻译，但等长演示易忽略长度限制；少样本提示提升质量但边际效益递减；多输出可优化长度与质量的权衡。

Conclusion: 提示策略和演示选择对LLMs的翻译质量和长度控制有显著影响，多输出方法在某些语言对中表现优异。
...
### [198] [Evaluating the Effectiveness of Linguistic Knowledge in Pretrained Language Models: A Case Study of Universal Dependencies](https://arxiv.org/abs/2506.04887)
*Wenxi Li*
...
<details>
  <summary>Details</summary>
Motivation: 评估UD在跨语言任务中的有效性，填补其在实际应用中的研究空白。

Method: 将UD整合到预训练语言模型中，并在跨语言对抗性释义识别任务中测试其效果。

Result: UD显著提升了模型性能，准确率和F1分数分别平均提高3.85%和6.08%，部分语言对甚至超越大型语言模型。

Conclusion: UD在跨领域任务中具有潜力，其与英语的相似性得分与模型性能正相关。
...
### [199] [ICPC-Eval: Probing the Frontiers of LLM Reasoning with Competitive Programming Contests](https://arxiv.org/abs/2506.04894)
*Shiyi Xu,Yiwen Hu,Yingqian Min,Zhipeng Chen,Wayne Xin Zhao,Ji-Rong Wen*

Main category: cs.CL

TLDR: 提出了ICPC-Eval，一个用于评估大型语言模型在竞赛环境中编码能力的新基准，包含118个ICPC竞赛题目，并引入了Refine@K评估指标。
...
Method: 设计ICPC-Eval基准，包含精选的ICPC竞赛题目，提供本地评估工具和Refine@K评估指标。

Result: 结果显示，顶级推理模型需要多轮代码反馈才能发挥潜力，但仍落后于人类顶尖团队。
...
### [200] [Verbose ListOps (VLO): Beyond Long Context -- Unmasking LLM's Reasoning Blind Spots](https://arxiv.org/abs/2506.04907)
*Alex Pan,Mary-Anne Williams*

Main category: cs.CL

TLDR: Verbose ListOps是一个新基准，通过将ListOps计算转化为长故事，测试LLMs在嵌套叙事推理中的状态管理能力，揭示其局限性。
...
Method: 将ListOps计算编程为长故事，通过隐藏中间结果强制LLMs进行内部计算和状态管理。

Result: 主流LLMs（如OpenAI o4、Gemini 2.5 Pro）在中等长度叙事（约10k token）下表现崩溃。

Conclusion: Verbose ListOps为针对性改进LLMs的推理能力提供了工具，是自动化知识工作的关键一步。
...
### [201] [A Practitioner's Guide to Building ASR Models for Low-Resource Languages: A Case Study on Scottish Gaelic](https://arxiv.org/abs/2506.04915)
*Ondřej Klejch,William Lamb,Peter Bell*

Main category: cs.CL

TLDR: 本文挑战了微调多语言端到端模型在低资源语言ASR系统中表现最优的观点，提出结合混合HMM与自监督模型的方法，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有观点认为微调多语言端到端模型是低资源语言ASR系统的最佳方法，但本文旨在验证是否存在更优方案。

Method: 结合混合HMM与自监督模型，通过持续自监督预训练和半监督训练充分利用可用数据。

Result: 在苏格兰盖尔语上测试，相比最佳微调模型，WER相对降低32%。

Conclusion: 混合HMM与自监督模型的方法在低资源语言ASR系统中优于传统微调方法。
...
### [202] [Simulating LLM-to-LLM Tutoring for Multilingual Math Feedback](https://arxiv.org/abs/2506.04920)
*Junior Cedric Tonga,KV Aditya Srivatsa,Kaushal Kumar Maurya,Fajri Koto,Ekaterina Kochmar*

Main category: cs.CL

TLDR: 论文研究了大型语言模型（LLMs）在多语言教育中的反馈效果，发现多语言提示能显著提升学习效果，尤其是在低资源语言中。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在不同语言中提供数学推理任务反馈的能力，填补多语言教育工具的研究空白。

Method: 通过模拟多语言师生互动，使用强模型作为教师生成提示，弱模型模拟学生，覆盖11种语言和多种提示策略。
...
### [203] [ConECT Dataset: Overcoming Data Scarcity in Context-Aware E-Commerce MT](https://arxiv.org/abs/2506.04929)
*Mikołaj Pokrywka,Wojciech Kusa,Mieszko Rutkowski,Mikołaj Koszowski*

Main category: cs.CL

TLDR: 研究探索了在电子商务数据中通过添加上下文信息（如图像和产品元数据）来提升神经机器翻译质量的方法，并发布了新的捷克语-波兰语数据集ConECT。


<details>
  <summary>Details</summary>
Motivation: 神经机器翻译在特定领域（如电子商务）中仍面临词义模糊和上下文不足的问题，需要探索如何通过额外信息提升翻译质量。

Method: 创建了包含11,400句对的捷克语-波兰语数据集ConECT，结合图像和产品元数据；测试了视觉语言模型（VLM）和文本到文本模型（如产品类别路径或图像描述）的效果。

Result: 实验表明，添加上下文信息（尤其是视觉上下文）能显著提升翻译质量。

Conclusion: 上下文信息的整合对机器翻译质量有积极影响，研究还公开了新数据集以促进进一步研究。
...
### [204] [From Struggle (06-2024) to Mastery (02-2025) LLMs Conquer Advanced Algorithm Exams and Pave the Way for Editorial Generation](https://arxiv.org/abs/2506.04965)
*Adrian Marius Dumitran,Theodor-Pierre Moroianu,Vasile Paul Alexe*

Main category: cs.CL

TLDR: 对最先进的大型语言模型（LLMs）在大学级算法考试中的表现进行全面评估，发现最新模型表现优异，但仍存在图相关任务的困难。


<details>
  <summary>Details</summary>
Motivation: 评估LLMs在复杂算法问题上的解决能力，并探索其在教育环境中的潜在应用。

Method: 通过测试多个模型在罗马尼亚语考试及其高质量英语翻译上的表现，分析其问题解决能力、一致性和多语言性能。

Result: 最新模型表现接近顶尖学生，具备复杂多步推理能力，但在图任务上仍有困难。

Conclusion: LLMs在教育中具有潜力，可支持高质量内容生成，为算法教育中生成式AI的进一步整合提供基础。
...
### [205] [Better Semi-supervised Learning for Multi-domain ASR Through Incremental Retraining and Data Filtering](https://arxiv.org/abs/2506.04981)
...
TLDR: 论文提出了一种增量半监督学习流程，利用少量领域内标注数据和相关领域辅助数据，通过多模型共识或命名实体识别筛选伪标签，显著提升ASR模型在稀缺标注数据下的性能。


<details>
  <summary>Details</summary>
Motivation: 解决在标注数据稀缺时，微调预训练ASR模型以适应特定领域的挑战。

Method: 提出增量半监督学习流程，结合少量领域内标注数据和相关领域辅助数据，采用多模型共识或NER筛选伪标签进行迭代优化。

Result: 在Wow呼叫中心和Fisher英语语料库上表现优于单步微调，共识筛选相对提升22.3%（Wow）和24.8%（Fisher）。

Conclusion: 共识筛选效果最佳，NER次之且计算成本更低，为稀缺标注数据下的ASR微调提供了有效解决方案。
...
### [206] [SCOP: Evaluating the Comprehension Process of Large Language Models from a Cognitive View](https://arxiv.org/abs/2506.05000)
*Yongjie Xiao,Hongru Liang,Peixin Qin,Yao Zhang,Wenqiang Lei*

Main category: cs.CL

TLDR: 论文提出了SCOP框架，从认知角度评估大语言模型（LLMs）的理解能力，发现其与专家水平仍有差距，并建议改进方向。
...
Method: 定义五种理解技能，构建测试数据框架，分析开源和闭源LLMs的表现。

Result: LLMs在理解过程中存在挑战，与专家有相似之处，但可能通过错误过程得出正确答案。

Conclusion: 建议改进LLMs时需关注理解过程，确保所有技能在训练中充分发展。
...
### [207] [ComfyUI-Copilot: An Intelligent Assistant for Automated Workflow Development](https://arxiv.org/abs/2506.05010)
...
<details>
  <summary>Details</summary>
Motivation: ComfyUI虽然灵活且用户友好，但其文档不足、模型配置复杂和工作流设计难度高，给新用户带来挑战。

Method: 采用分层多智能体框架，包括中央助理代理和专用工作代理，结合知识库支持智能推荐和自动化工作流构建。

Result: 离线评估和用户反馈显示，插件能准确推荐节点并加速工作流开发，降低新手门槛并提升老手效率。

Conclusion: ComfyUI-Copilot有效解决了ComfyUI的使用难题，适合不同水平的用户，其安装包和演示视频已开源。
...
### [208] [Controlling Summarization Length Through EOS Token Weighting](https://arxiv.org/abs/2506.05017)
*Zeno Belligoli,Emmanouil Stergiadis,Eran Fainman,Ilya Gusev*

Main category: cs.CL

TLDR: 提出一种简单方法，通过调整EOS令牌在交叉熵损失中的重要性来控制生成文本长度，适用于多种模型和解码算法。
...
Conclusion: 该方法简单、通用，且与现有推理技术正交。
...
### [209] [Automatic Robustness Stress Testing of LLMs as Mathematical Problem Solvers](https://arxiv.org/abs/2506.05038)
*Yutao Hou,Zeguan Xiao,Fei Yu,Yihan Jiang,Xuetao Wei,Hailiang Huang,Yun Chen,Guanhua Chen*

Main category: cs.CL

TLDR: AR-Checker框架通过多轮并行LLM重写和验证生成数学问题变体，动态评估LLM的鲁棒性，避免数据污染。


<details>
  <summary>Details</summary>
Motivation: 评估LLM在简单推理任务中的鲁棒性，避免传统手工模板或有限扰动规则的局限性。

Method: 提出AR-Checker框架，通过多轮并行LLM重写和验证生成语义一致但可能使LLM失败的问题变体。

Result: 在GSM8K、MATH-500等数学任务及MMLU、CommonsenseQA等非数学任务中表现优异。

Conclusion: AR-Checker能动态生成评测基准，有效评估LLM鲁棒性，适用范围广。
...
### [210] [TALL -- A Trainable Architecture for Enhancing LLM Performance in Low-Resource Languages](https://arxiv.org/abs/2506.05057)
*Moshe Ofer,Orel Zamler,Amos Azaria*

Main category: cs.CL

TLDR: TALL架构通过结合LLM和双语翻译模型，提升低资源语言性能，实验证明优于基线方法。
...
Method: 整合LLM与双语翻译模型，通过维度对齐层和定制Transformer转换输入，采用参数高效策略（冻结预训练组件，训练轻量适配模块）。
...
### [211] [Debatable Intelligence: Benchmarking LLM Judges via Debate Speech Evaluation](https://arxiv.org/abs/2506.05062)
*Noy Sternlicht,Ariel Gera,Roy Bar-Haim,Tom Hope,Noam Slonim*

Main category: cs.CL

TLDR: 论文提出辩论演讲评估作为评估LLM法官的新基准，探讨LLM在理解演讲多层面内容上的能力，并与人类法官对比。


<details>
  <summary>Details</summary>
Motivation: 评估辩论演讲需要多层面的理解能力，而现有LLM基准对此关注不足。
...
### [212] [Does It Make Sense to Speak of Introspection in Large Language Models?](https://arxiv.org/abs/2506.05068)
*Iulia Comşa,Murray Shanahan*

Main category: cs.CL

TLDR: 论文探讨了大型语言模型（LLMs）的自我报告是否可被视为内省，并通过两个例子分析其合理性。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs的自我报告是否具有内省意义，以理解其认知能力与人类内省的异同。

Method: 通过分析LLMs的两个自我报告例子，评估其是否符合内省的定义。

Result: 第一个例子（描述创作过程）不被视为内省；第二个例子（推断自身参数）可视为最小化的内省，但无意识体验。

Conclusion: LLMs的某些自我报告可被视为内省，但与人类内省不同，缺乏意识体验。
...
### [213] [RIVAL: Reinforcement Learning with Iterative and Adversarial Optimization for Machine Translation](https://arxiv.org/abs/2506.05070)
*Tianjiao Li,Mengran Yu,Chenyu Shi,Yanjun Zhao,Xiaojing Liu,Qiang Zhang,Qi Zhang,Xuanjing Huang,Jiayin Wang*

Main category: cs.CL

TLDR: 论文提出RIVAL框架，通过对抗训练解决LLM在口语字幕翻译中因分布偏移导致的性能下降问题，结合定性和定量奖励提升翻译质量。


<details>
  <summary>Details</summary>
Motivation: 观察到强化学习与翻译任务结合在口语字幕翻译中表现不佳，原因是离线奖励模型与在线LLM因分布偏移逐渐偏离。

Method: 提出RIVAL框架，采用对抗训练（min-max游戏）迭代更新奖励模型和LLM，结合定性和定量奖励。
...
### [214] [Just a Scratch: Enhancing LLM Capabilities for Self-harm Detection through Intent Differentiation and Emoji Interpretation](https://arxiv.org/abs/2506.05073)
*Soumitra Ghosh,Gopendra Vikram Singh,Shambhavi,Sabarna Choudhury,Asif Ekbal*

Main category: cs.CL

TLDR: 该论文提出了一种通过语言-表情符号交互增强大型语言模型（LLMs）自我伤害检测能力的方法，并发布了CESM-100和SHINES数据集。


<details>
  <summary>Details</summary>
Motivation: 社交媒体上的自我伤害表达通常隐晦且依赖上下文，现有LLMs难以准确识别。通过区分意图和利用表情符号的细微差别，可以提升检测效果。

Method: 提出统一框架：1）使用CESM-100丰富输入；2）多任务微调LLMs（主任务为自我伤害检测，辅助任务为CM/SI范围检测）；3）生成可解释的预测依据。

Result: 在Llama 3等LLMs上测试，该方法显著提升了检测和解释任务的性能，有效解决了自我伤害信号的模糊性问题。

Conclusion: 结合意图区分和上下文线索，该方法为LLMs在自我伤害检测领域的应用提供了有效解决方案，相关资源已公开。
...
### [215] [Parking, Perception, and Retail: Street-Level Determinants of Community Vitality in Harbin](https://arxiv.org/abs/2506.05080)
*HaoTian Lan*

Main category: cs.CL

TLDR: 研究提出了一种基于图像的可解释框架，分析中国城市社区街道的商业活力与车辆可达性、环境质量和行人感知的关系。


<details>
  <summary>Details</summary>
Motivation: 探讨社区街道的商业活力如何受街道特征（如停车密度、绿化、清洁度和街道宽度）影响，以提升零售表现和用户满意度。

Method: 利用街景图像和多模态大语言模型（VisualGLM-6B）构建社区商业活力指数（CCVI），并通过GPT-4感知模型分析其与空间属性的关系。

Result: 研究发现适度车辆存在可提升商业可达性，但过度停车会降低步行体验和满意度；绿化和清洁度显著提升满意度，但对定价影响较弱；街道宽度调节车辆影响。

Conclusion: 研究通过AI辅助感知与城市形态分析的结合，揭示了商业成功的非线性驱动因素，为城市设计和停车管理提供了理论和方法支持。
...
### [216] [CL-ISR: A Contrastive Learning and Implicit Stance Reasoning Framework for Misleading Text Detection on Social Media](https://arxiv.org/abs/2506.05107)
*Tianyi Huang,Zikun Cui,Cuiqianhe Du,Chia-En Chiang*

Main category: cs.CL

TLDR: 论文提出了一种结合对比学习和隐式立场推理的新框架CL-ISR，用于提高社交媒体误导文本的检测准确率。
...
Result: CL-ISR框架显著提高了误导文本的检测效果，尤其在语言复杂或立场隐晦的情况下表现突出。

Conclusion: CL-ISR通过结合对比学习的判别能力和立场推理的深度解释能力，为社交媒体误导文本检测提供了高效解决方案。
...
### [217] [The NTNU System at the S&I Challenge 2025 SLA Open Track](https://arxiv.org/abs/2506.05121)
*Hong-Yun Lin,Tien-Hong Lo,Yu-Hsuan Fang,Jhen-Ke Lin,Chung-Chun Wang,Hao-Chien Lu,Berlin Chen*

Main category: cs.CL

TLDR: 该论文提出了一种结合W2V和Phi-4 MLLM的评分融合策略，用于口语能力评估，解决了BERT和W2V各自的模态限制，并在比赛中取得了优异成绩。


<details>
  <summary>Details</summary>
Motivation: BERT和W2V在口语能力评估中各具局限性，前者依赖ASR转录而忽略语音特征，后者缺乏语义解释性。

Method: 通过融合W2V和Phi-4 MLLM的评分策略，结合语音和语义特征。

Result: 在Speak & Improve Challenge 2025中，RMSE为0.375，排名第二。

Conclusion: 融合多模态模型能有效提升口语评估的准确性。
...
### [218] [DiCoRe: Enhancing Zero-shot Event Detection via Divergent-Convergent LLM Reasoning](https://arxiv.org/abs/2506.05128)
*Tanmay Parekh,Kartik Mehta,Ninareh Mehrabi,Kai-Wei Chang,Nanyun Peng*

Main category: cs.CL

TLDR: DiCoRe框架通过发散-收敛推理（Dreamer和Grounder）提升零样本事件检测性能，结合LLM-Judge验证，平均F1提升4-7%。


<details>
  <summary>Details</summary>
Motivation: 解决零样本事件检测中复杂事件本体和领域特定触发器提取的挑战，提升大型语言模型（LLMs）的实用性。

Method: 提出DiCoRe框架，包含Dreamer（发散推理）和Grounder（收敛推理），并通过LLM-Judge验证结果。

Result: 在六个数据集上优于现有零样本、迁移学习和推理基线，平均F1提升4-7%。
...
### [219] [Information Locality as an Inductive Bias for Neural Language Models](https://arxiv.org/abs/2506.05136)
*Taiga Someya,Anej Svete,Brian DuSell,Timothy J. O'Donnell,Mario Giulianelli,Ryan Cotterell*

Main category: cs.CL

TLDR: 论文提出了一种定量框架，通过引入m-局部熵来研究神经语言模型的归纳偏置是否与人类处理约束一致。实验表明，高m-局部熵的语言对Transformer和LSTM模型更难学习。


<details>
  <summary>Details</summary>
Motivation: 探讨神经语言模型的归纳偏置是否与人类处理约束一致，填补相关研究空白。

Method: 提出定量框架，引入m-局部熵作为信息论度量，通过实验验证其在扰动自然语言和PFSA定义语言中的表现。

Result: 高m-局部熵的语言对Transformer和LSTM模型更难学习，表明神经模型对语言的局部统计结构敏感。
...
### [220] [AudioLens: A Closer Look at Auditory Attribute Perception of Large Audio-Language Models](https://arxiv.org/abs/2506.05140)
*Chih-Kai Yang,Neo Ho,Yi-Jyun Lee,Hung-yi Lee*

Main category: cs.CL

TLDR: 论文分析了大型音频语言模型（LALMs）内部感知和识别听觉属性的机制，发现属性信息随层深减少，早期层解析属性与更高准确性相关，并提出了一种增强模型的方法。
...
Result: 发现属性信息在失败识别时随层深减少，早期层解析属性与准确性正相关，且模型依赖查询输入而非隐藏状态聚合信息。

Conclusion: 研究为听觉属性处理提供了新见解，并提出了一种增强LALMs的方法，为未来改进铺平了道路。
...
### [221] [Do Large Language Models Judge Error Severity Like Humans?](https://arxiv.org/abs/2506.05142)
*Diege Sun,Guanyi Chen,Fan Zhao,Xiaorong Cheng,Tingting He*

Main category: cs.CL

TLDR: 研究比较了人类与大型语言模型（LLMs）对图像描述中语义错误严重性的评估差异，发现人类和LLMs在错误类型判断上存在显著差异，仅少数模型能部分模拟人类判断。
...
Method: 扩展van Miltenburg等人（2020）的实验框架，评估四种错误类型（年龄、性别、服装类型、服装颜色）在单模态和多模态设置下的表现。

Result: 人类对不同错误类型的严重性评估差异显著，而大多数LLMs在性别和颜色错误判断上与人类不一致；仅Doubao和DeepSeek-V3部分模拟了人类判断。

Conclusion: LLMs在模拟人类错误严重性判断上存在局限性，尤其是在多模态环境中，部分模型表现优异但仍未完全达到人类水平。
...
### [222] [Knowledgeable-r1: Policy Optimization for Knowledge Exploration in Retrieval-Augmented Generation](https://arxiv.org/abs/2506.05154)
*Chenyu Lin,Yilin Wen,Du Su,Fei Sun,Muhan Chen,Chenfu Bao,Zhonghou Lv*

Main category: cs.CL

TLDR: 论文提出Knowledgeable-r1方法，通过联合采样和多策略分布探索知识能力，平衡检索上下文与模型固有知识，显著提升鲁棒性和推理准确性。


<details>
  <summary>Details</summary>
Motivation: 当前RAG系统过度依赖检索上下文，可能忽略模型固有知识并依赖不准确来源，需解决这种不平衡。

Method: 采用联合采样和多策略分布探索知识能力，激励模型综合使用参数化知识和上下文知识。

Result: 在参数与上下文冲突任务及一般RAG任务中显著提升性能，尤其在反事实场景中优于基线17.07%。

Conclusion: Knowledgeable-r1有效平衡了检索与模型知识，提升了任务性能，代码已开源。
...
### [223] [Dissecting Bias in LLMs: A Mechanistic Interpretability Perspective](https://arxiv.org/abs/2506.05166)
*Bhavik Chandna,Zubair Bashir,Procheta Sen*

Main category: cs.CL

TLDR: 该论文通过机制解释性方法分析了GPT-2和Llama2模型中社会、人口和性别偏见的内部结构表现，发现偏见计算高度集中在少数层，且移除这些组件会影响其他NLP任务。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型（LLMs）中社会、人口和性别偏见的内部结构表现，以理解其根源并提出改进方法。

Method: 采用机制解释性方法，分析模型内部边缘对偏见行为的影响，并通过系统消融实验评估其稳定性、定位性和泛化性。

Result: 偏见计算集中在少数层，且在不同微调设置下会变化；移除偏见组件会减少偏见输出，但也会影响其他NLP任务。

Conclusion: 偏见在模型中高度局部化，且与其他任务共享重要组件，移除偏见需谨慎以避免负面影响。
...
### [224] [ECoRAG: Evidentiality-guided Compression for Long Context RAG](https://arxiv.org/abs/2506.05167)
...
Conclusion: ECoRAG显著提升LLM性能，同时高效节省资源。
...
### [225] [Qwen3 Embedding: Advancing Text Embedding and Reranking Through Foundation Models](https://arxiv.org/abs/2506.05176)
...
TLDR: Qwen3 Embedding系列是基于Qwen3基础模型的文本嵌入和重排序技术，通过多阶段训练流程和模型合并策略，在多种任务中实现领先性能。
...
Method: 结合大规模无监督预训练和监督微调，利用Qwen3 LLM生成高质量训练数据，支持多种模型尺寸。

Result: 在MTEB等多语言基准测试和检索任务中取得最佳性能。
...
### [226] [Counterfactual reasoning: an analysis of in-context emergence](https://arxiv.org/abs/2506.05188)
*Moritz Miller,Bernhard Schölkopf,Siyuan Guo*

Main category: cs.CL

TLDR: 论文研究大规模神经语言模型（LMs）在上下文学习中进行反事实推理的能力，即预测假设情景下的结果。通过线性回归任务验证模型能够推断并复制上下文噪声，揭示了自注意力、模型深度和数据多样性对性能的影响。


<details>
  <summary>Details</summary>
Motivation: 探索语言模型在无需参数更新的情况下进行反事实推理的能力，为理解模型在假设情景中的表现提供理论基础。

Method: 采用线性回归任务作为合成实验，要求模型推断并复制上下文噪声，研究自注意力、模型深度和数据多样性对性能的影响。

Result: 语言模型在控制实验中能够进行反事实推理，且这种能力可推广到更广泛的函数类别。同时发现自注意力、模型深度和数据多样性是关键因素。

Conclusion: 研究表明语言模型具备反事实推理潜力，尤其在序列数据中表现突出，为反事实故事生成提供了初步证据。
...
### [227] [RELIC: Evaluating Compositional Instruction Following via Language Recognition](https://arxiv.org/abs/2506.05205)
*Jackson Petty,Michael Y. Hu,Wentao Wang,Shauli Ravfogel,William Merrill,Tal Linzen*

Main category: cs.CL

TLDR: RELIC框架评估大语言模型（LLMs）的指令跟随能力，通过语言识别任务发现其在复杂语法下表现接近随机，且依赖浅层启发式方法。
...
Result: LLMs在复杂语法和样本上表现接近随机，准确率可由语法和样本复杂度预测。

Conclusion: 当前最先进的LLMs在复杂指令跟随任务中表现有限，倾向于依赖浅层启发式方法。
...
### [228] [The Common Pile v0.1: An 8TB Dataset of Public Domain and Openly Licensed Text](https://arxiv.org/abs/2506.05209)
...
TLDR: 论文介绍了Common Pile v0.1，一个用于LLM预训练的8TB开放许可文本数据集，并验证了其性能。


<details>
  <summary>Details</summary>
Motivation: 解决LLM训练中未授权文本的知识产权和伦理问题，提供高质量开放许可数据。

Method: 收集、整理并发布Common Pile v0.1数据集，训练两个7B参数的LLM模型（Comma v0.1-1T和v0.1-2T）。

Result: 训练模型性能与使用未授权文本的LLM（如Llama 1和2 7B）相当。

Conclusion: Common Pile v0.1为LLM训练提供了可行的开放许可数据解决方案，并公开了数据集、代码和模型。
...
### [229] [Improving Low-Resource Morphological Inflection via Self-Supervised Objectives](https://arxiv.org/abs/2506.05227)
*Adam Wiemerslage,Katharina von der Wense*

Main category: cs.CL

TLDR: 论文研究了自监督辅助任务在极低资源环境下对形态学变形（字符级任务）的有效性，发现自动编码在数据极少时表现最佳，而字符掩码语言模型（CMLM）在数据增加时更有效。基于已知语素边界的掩码采样能持续提升性能。
...
Conclusion: 自监督任务在低资源形态学建模中具有潜力，尤其是结合语素边界信息的CMLM。
...
### [230] [Towards a Unified System of Representation for Continuity and Discontinuity in Natural Language](https://arxiv.org/abs/2506.05235)
*Ratna Kandala,Prakash Mondal*

Main category: cs.CL

TLDR: 本文提出了一种统一表示自然语言中连续和不连续结构的系统，结合了短语结构语法、依存语法和范畴语法的特点。


<details>
  <summary>Details</summary>
Motivation: 解决不同语法形式主义对不连续结构分析独立且不收敛的问题。

Method: 结合短语结构语法（PSG）的构成性、依存语法（DG）的头-依存关系和范畴语法（CG）的函子-论元关系。

Result: 通过统一的数学推导，展示了不连续和连续结构可以在三种语法形式主义中统一分析。

Conclusion: 提出了一种统一的语法表示系统，能够同时处理连续和不连续结构。
...
### [231] [CLATTER: Comprehensive Entailment Reasoning for Hallucination Detection](https://arxiv.org/abs/2506.05243)
*Ron Eliav,Arie Cattan,Eran Hirsch,Shahaf Bassan,Elias Stengel-Eskin,Mohit Bansal,Ido Dagan*

Main category: cs.CL

TLDR: 论文提出了一种系统化的推理方法，通过分解文本并验证每个子事实的证据，提升幻觉检测的准确性。
...
Method: 定义了一个三步推理过程：声明分解、子声明归因与蕴含分类、聚合分类，并引入指标评估中间推理步骤的质量。

Result: 实验表明，这种引导式推理显著提高了幻觉检测的性能。

Conclusion: 系统化的推理框架能够更细粒度地做出蕴含决策，提升检测效果。
...
### [232] [Micro-Act: Mitigate Knowledge Conflict in Question Answering via Actionable Self-Reasoning](https://arxiv.org/abs/2506.05278)
*Nan Huo,Jinyang Li,Bowen Qin,Ge Qu,Xiaolong Li,Xiaodong Li,Chenhao Ma,Reynold Cheng*

Main category: cs.CL

TLDR: 论文提出Micro-Act框架，通过分层动作空间解决RAG系统中的知识冲突问题，显著提升QA任务性能。


<details>
  <summary>Details</summary>
Motivation: RAG系统中外部知识与LLMs内部知识冲突影响下游任务性能，现有方法因上下文过长而效果不佳。

Method: 提出Micro-Act框架，通过分层动作空间自适应分解知识源，进行细粒度比较。

Result: 在5个基准数据集上显著提升QA准确性，尤其在时间和语义冲突类型中表现突出。

Conclusion: Micro-Act在解决知识冲突的同时保持非冲突问题性能，具有实际应用价值。
...
### [233] [ProRefine: Inference-time Prompt Refinement with Textual Feedback](https://arxiv.org/abs/2506.05305)
*Deepak Pandita,Tharindu Cyril Weerasooriya,Ankit Parag Shah,Christopher M. Homan,Wei Wei*

Main category: cs.CL

TLDR: ProRefine是一种创新的提示优化方法，通过动态调整多步推理任务的提示，显著提升AI代理协作的准确性和效率。
...
Conclusion: ProRefine不仅提高准确性，还能让小模型匹配大模型的性能，推动高效、可扩展的AI部署。
...
### [234] [Constrained Entropic Unlearning: A Primal-Dual Framework for Large Language Models](https://arxiv.org/abs/2506.05314)
*Taha Entesari,Arman Hatami,Rinat Khaziev,Anil Ramakrishna,Mahyar Fazlyab*

Main category: cs.CL

TLDR: 本文提出了一种新的LLM遗忘方法，通过约束优化和新型损失函数，有效移除敏感信息同时保留有用数据。


<details>
  <summary>Details</summary>
Motivation: 现有遗忘方法通常通过正则化权衡遗忘与保留，导致优化不稳定和性能下降。本文旨在解决这一问题。

Method: 提出基于约束优化的遗忘方法，使用logit-margin flattening损失函数，并通过原始对偶算法求解。

Result: 在TOFU和MUSE基准测试中，该方法优于现有基线，能有效移除目标信息并保留下游性能。

Conclusion: 该方法在LLM遗忘任务中表现优越，为实际应用提供了更稳定和高效的解决方案。
...
### [235] [Search Arena: Analyzing Search-Augmented LLMs](https://arxiv.org/abs/2506.05334)
...
TLDR: Search Arena是一个大规模、多轮对话的人类偏好数据集，用于分析搜索增强语言模型的表现，揭示了用户偏好与引用数量及来源的关系。


<details>
  <summary>Details</summary>
Motivation: 现有数据集规模小、范围窄，难以全面分析搜索增强语言模型的表现。

Method: 引入Search Arena数据集，包含24,000对多轮对话和12,000个人类偏好投票，覆盖多种意图和语言。

Result: 用户偏好受引用数量影响，即使引用内容不直接支持观点；社区驱动平台更受青睐。搜索增强模型在非搜索环境中表现良好，但在搜索环境中依赖参数知识会导致质量下降。

Conclusion: 数据集开源支持未来研究，揭示了用户偏好与引用行为的关系，为搜索增强模型优化提供方向。
...
### [236] [Flattery, Fluff, and Fog: Diagnosing and Mitigating Idiosyncratic Biases in Preference Models](https://arxiv.org/abs/2506.05339)
*Anirudh Bharadwaj,Chaitanya Malaviya,Nitish Joshi,Mark Yatskar*

Main category: cs.CL

TLDR: 语言模型在偏好判断中存在系统性偏差，过度依赖长度、结构等表面特征，导致奖励破解和不可靠评估。研究发现训练数据中的偏见是根源，并提出基于反事实数据增强的简单后训练方法，有效减少偏差。


<details>
  <summary>Details</summary>
Motivation: 语言模型作为人类偏好判断的代理，存在系统性偏差，影响对齐和评估的可靠性。研究旨在揭示训练数据偏见与偏好模型偏差之间的关系，并提出解决方案。

Method: 通过控制反事实对量化偏好模型的偏差程度，提出基于反事实数据增强（CDA）的后训练方法，使用合成对比样本进行微调。

Result: 偏好模型在60%以上的实例中表现出偏差，与人类偏好相比存在40%的校准误差。CDA方法将平均校准误差从39.4%降至32.5%，偏差差异从20.5%降至10.0%。

Conclusion: 反事实数据增强方法能有效减少偏好模型的偏差，提升其可靠性，同时保持整体性能。
...
### [237] [Zero-Shot Adaptation of Parameter-Efficient Fine-Tuning in Diffusion Models](https://arxiv.org/abs/2506.04244)
...
Method: 利用子空间和零空间相似性，选择性对齐层，将源调整投影到目标模型的权重空间。

Result: 在文本到图像模型上的评估表明，知识迁移成功且性能可比，无需重新训练。

Conclusion: ProLoRA提供了一种高效的方法，克服了传统方法的局限性，实现了无需额外训练的模型适应。
...
### [238] [Contextual Integrity in LLMs via Reasoning and Reinforcement Learning](https://arxiv.org/abs/2506.04245)
...
TLDR: 论文提出了一种通过强化学习框架和显式推理来确保自主代理在决策时维护上下文完整性的方法，显著减少了不适当的信息披露。


<details>
  <summary>Details</summary>
Motivation: 随着自主代理为用户决策的时代到来，如何确保上下文完整性（CI）——即在执行任务时共享适当信息——成为核心问题。

Method: 首先通过提示LLMs显式推理CI，然后开发强化学习框架进一步训练模型以实现CI。使用合成数据集（约700个例子）进行测试。

Result: 方法显著减少了不适当的信息披露，同时保持了任务性能，且改进效果可迁移到人类标注的CI基准测试（如PrivacyLens）。

Conclusion: 通过显式推理和强化学习，可以有效提升自主代理在决策时的上下文完整性。
...
### [239] [Language-Guided Multi-Agent Learning in Simulations: A Unified Framework and Evaluation](https://arxiv.org/abs/2506.04251)
*Zhengyang Li*

Main category: cs.AI

TLDR: LLM-MARL框架将大语言模型（LLMs）融入多智能体强化学习（MARL），提升游戏环境中的协调、通信和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 通过结合LLMs和MARL，解决智能体在复杂环境中的协调和泛化问题。
...
### [240] [A Graph-Retrieval-Augmented Generation Framework Enhances Decision-Making in the Circular Economy](https://arxiv.org/abs/2506.04252)
...
TLDR: CircuGraphRAG框架通过知识图谱增强LLM输出，提高可持续制造业中工业代码和排放数据的准确性。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在可持续制造业中因幻觉工业代码和排放数据而影响决策的问题。

Method: 结合检索增强生成（RAG）和领域知识图谱，支持结构化多跳推理，将自然语言查询转换为SPARQL并验证子图。

Result: 在单跳和多跳问答中表现优于基线，ROUGE-L F1得分达1.0，响应时间减半，令牌使用减少16%。
...
### [241] [HADA: Human-AI Agent Decision Alignment Architecture](https://arxiv.org/abs/2506.04253)
*Tapio Pitkäranta,Leena Pitkäranta*

Main category: cs.AI

TLDR: HADA是一个框架无关的参考架构，用于确保LLM代理和传统算法与组织目标和价值观对齐。它通过角色特定的利益相关者代理实现对话式控制，支持多角色查询、审计和决策追溯。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型和传统算法在组织决策中与目标和价值观对齐的问题，提升透明度、准确性和伦理合规性。

Method: HADA通过角色特定的代理（如业务、数据科学、审计等）包装算法或LLM，提供对话式API，支持自然语言表达目标和约束，并在多代理环境中持续传播和记录。

Result: 在零售银行场景中验证了HADA的六项目标完全覆盖，包括角色控制、KPI追溯、偏见检测和决策溯源。

Conclusion: HADA为多代理系统中人机对齐提供了开源架构和中层设计理论，实证表明其能提升决策管线的准确性、透明度和伦理合规性。
...
### [242] [Automated Skill Discovery for Language Agents through Exploration and Iterative Feedback](https://arxiv.org/abs/2506.04287)
*Yongjin Yang,Sinjae Kang,Juyong Lee,Dongjun Lee,Se-Young Yun,Kimin Lee*

Main category: cs.AI

TLDR: 论文提出了一种名为EXIF的自动技能发现框架，用于训练LLM代理，通过探索优先策略和反馈循环生成可行技能数据集，显著提升代理性能。


<details>
  <summary>Details</summary>
Motivation: 训练LLM代理以获取技能和执行多样化任务面临数据集生成的挑战，如人工收集成本高或LLM直接生成的任务不可行。

Method: 采用探索优先策略，使用探索代理（Alice）生成可行技能数据集，并通过反馈循环迭代优化目标代理（Bob）的性能。

Result: 在Webshop和Crafter实验上，EXIF能有效发现技能并提升代理性能，无需人工干预。

Conclusion: EXIF框架通过自动技能发现和反馈循环，显著提升了LLM代理的能力，并展示了自进化系统的潜力。
...
### [243] [A Statistical Physics of Language Model Reasoning](https://arxiv.org/abs/2506.04374)
*Jack David Carson,Amir Reisizadeh*

Main category: cs.AI

TLDR: 论文提出了一种基于统计物理学的框架，用于分析Transformer语言模型的连续时间推理动态，揭示了四种潜在推理状态，并通过低维投影和SLDS模型验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 理解Transformer语言模型中的推理动态，尤其是其难以解释的涌现推理行为。

Method: 采用统计物理学方法，将句子级隐藏状态建模为低维流形上的随机动力系统，结合漂移-扩散和潜在状态切换机制。

Result: 通过8个模型和7个基准测试，发现低维投影能解释约50%的方差，并识别出四种潜在推理状态。

Conclusion: 该框架为低成本模拟推理动态提供了工具，有助于研究和预测语言模型的关键状态转换（如推理失败）。
...
### [244] [Matter-of-Fact: A Benchmark for Verifying the Feasibility of Literature-Supported Claims in Materials Science](https://arxiv.org/abs/2506.04410)
*Peter Jansen,Samiah Hassan,Ruoyao Wang*

Main category: cs.AI

TLDR: 论文提出Matter-of-Fact数据集，用于评估假设的可行性，以加速科学发现。现有方法在任务上表现不佳，但专家认为问题可解。
...
Method: 构建包含8.4k科学假设的数据集，涵盖材料科学多个领域，测试现有模型的性能。
...
### [245] [Plugging Schema Graph into Multi-Table QA: A Human-Guided Framework for Reducing LLM Reliance](https://arxiv.org/abs/2506.04427)
*Xixi Wang,Miguel Costa,Jordanka Kovaceva,Shuai Wang,Francisco C. Pereira*

Main category: cs.AI

TLDR: 提出了一种基于图的框架，利用人工整理的关系知识来解决多表问答中的模式链接问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在复杂真实场景中表现不佳，需要更可靠的多表模式链接方法。

Method: 使用图结构显式编码模式链接和连接路径，结合剪枝和子路径合并策略。
...
### [246] [An AI-Based Public Health Data Monitoring System](https://arxiv.org/abs/2506.04429)
*Ananya Joshi,Nolan Gormley,Richa Gadgil,Tina Townes,Roni Rosenfeld,Bryan Wilder*

Main category: cs.AI

TLDR: 提出了一种基于排名和AI异常检测的公共卫生数据监控系统，显著提升了效率和效果。


<details>
  <summary>Details</summary>
Motivation: 传统警报监控系统难以应对现代公共卫生数据的规模和复杂性，需要更高效的解决方案。

Method: 采用基于排名的监控范式，结合AI异常检测技术，并与国家组织合作部署。

Result: 系统每日监控500万数据点，三个月评估显示审查效率提升54倍。
...
### [247] [Matching Markets Meet LLMs: Algorithmic Reasoning with Ranked Preferences](https://arxiv.org/abs/2506.04478)
...
Method: 评估多种先进模型在偏好推理任务（如稳定匹配生成、不稳定性检测等）中的表现，并测试参数高效微调（LoRA）的效果。

Result: LLMs在大型市场中难以解决不稳定性问题，LoRA在小型市场有效但在大型市场中效果有限。

Conclusion: 需要更复杂的策略来提升LLMs处理大型上下文输入时的推理能力。
...
### [248] [CogMath: Assessing LLMs' Authentic Mathematical Ability from a Human Cognitive Perspective](https://arxiv.org/abs/2506.04481)
*Jiayu Liu,Zhenya Huang,Wei Dai,Cheng Cheng,Jinze Wu,Jing Sha,Song Li,Qi Liu,Shijin Wang,Enhong Chen*

Main category: cs.AI

TLDR: CogMath通过人类认知视角评估LLMs的数学能力，揭示主流LLMs能力被高估30%-40%。


<details>
  <summary>Details</summary>
Motivation: 现有评估范式仅依赖答案准确率，不足以全面评估LLMs的真实数学能力。

Method: CogMath将人类推理过程分为3阶段（问题理解、问题解决、解决方案总结），设计9个细粒度评估维度，并采用多智能体系统生成评估问题。

Result: 应用CogMath发现7种主流LLMs的数学能力被高估30%-40%，并定位其在不同阶段/维度的优缺点。

Conclusion: CogMath为提升LLMs的推理能力提供了深入见解。
...
### [249] ["Don't Do That!": Guiding Embodied Systems through Large Language Model-based Constraint Generation](https://arxiv.org/abs/2506.04500)
...
Method: 提出STPR框架，利用LLM将约束（如“不要做什么”）转化为Python函数，应用于点云表示和传统搜索算法。

Result: 实验表明STPR能准确描述复杂数学约束，在Gazebo模拟环境中实现完全合规且运行时间短。

Conclusion: STPR适用于多种紧凑模型，推理成本低。
...
### [250] [Schema Generation for Large Knowledge Graphs Using Large Language Models](https://arxiv.org/abs/2506.04512)
*Bohui Zhang,Yuan He,Lydia Pintscher,Albert Meroño Peñuela,Elena Simperl*

Main category: cs.AI

TLDR: 论文探讨了利用大型语言模型（LLMs）自动生成语义网和自然语言处理中的模式（schema），以减少对知识工程师和领域专家的依赖。


<details>
  <summary>Details</summary>
Motivation: 传统模式创建需要大量人工参与，LLMs在相关任务中表现优异，因此探索其自动生成模式的潜力。

Method: 引入YAGO Schema和Wikidata EntitySchema数据集及评估指标，利用LLMs从知识图谱中提取局部和全局信息生成Shape Expressions（ShEx）模式。

Result: 实验证明LLMs能生成高质量的ShEx模式，为大规模知识图谱的自动化模式生成铺平道路。

Conclusion: LLMs在模式生成中表现出强大潜力，同时为结构化生成任务设定了新挑战。
...
### [251] [OpenAg: Democratizing Agricultural Intelligence](https://arxiv.org/abs/2506.04571)
*Srikanth Thudumu,Jason Fisher*

Main category: cs.AI

TLDR: OpenAg是一个农业人工智能框架，结合领域特定模型和知识图谱，提供可解释、适应性强的决策支持。
...
Result: 开发了OpenAg系统，包括统一农业知识库、神经知识图谱、多智能体推理系统和因果透明机制。
...
### [252] [Judicial Permission](https://arxiv.org/abs/2506.04610)
*Guido Governatori,Antonino Rotolo*

Main category: cs.AI

TLDR: 本文探讨了刑事审判中弱许可（司法许可）的重要性，并引入对话游戏模型来系统分析司法许可，同时考虑了不同的证明标准和论证语义。


<details>
  <summary>Details</summary>
Motivation: 研究司法许可在刑事审判中的作用，以填补现有法律理论中对弱许可系统化分析的空白。

Method: 采用对话游戏模型，结合不同的证明标准和论证语义，对司法许可进行系统化分析。

Result: 通过模型展示了司法许可在不同证明标准和论证语义下的表现及其对审判结果的影响。

Conclusion: 司法许可在刑事审判中具有重要作用，对话游戏模型为系统化分析提供了有效工具。
...
### [253] [DeePoly: A High-Order Accuracy and Efficiency Deep-Polynomial Framework for Scientific Machine Learning](https://arxiv.org/abs/2506.04613)
*Li Liu,Heng Yong*

Main category: cs.AI

TLDR: DeePoly框架通过结合深度神经网络（DNN）和多项式基函数，解决了传统DNN方法在求解偏微分方程时缺乏收敛保证和计算效率的问题。


<details>
  <summary>Details</summary>
Motivation: 传统DNN方法在求解偏微分方程时缺乏收敛保证和计算效率，DeePoly旨在通过两阶段方法弥补这些不足。

Method: DeePoly采用两阶段方法：首先用DNN捕捉全局特征，然后用多项式基函数进行局部修正。

Result: 理论和实验表明，DeePoly显著提高了精度和效率，同时保持无网格和无格式特性。

Conclusion: DeePoly框架通过结合DNN和多项式基函数的优势，为求解偏微分方程提供了高效且可靠的方法。
...
### [254] [Look Before You Leap: A GUI-Critic-R1 Model for Pre-Operative Error Diagnosis in GUI Automation](https://arxiv.org/abs/2506.04614)
...
TLDR: 提出了一种预操作批评机制（GUI-Critic-R1）和S-GRPO策略，用于提升GUI自动化任务中的决策可靠性，并通过实验验证了其优越性。


<details>
  <summary>Details</summary>
Motivation: GUI自动化任务对决策错误的容忍度低，错误可能累积导致严重后果，需提高决策可靠性。

Method: 提出S-GRPO策略构建预操作批评模型GUI-Critic-R1，引入建议奖励增强反馈可靠性，并开发数据收集管道生成训练和测试数据集。

Result: 静态实验显示GUI-Critic-R1在批评准确性上优于现有MLLMs；动态评估表明其显著提升了成功率和操作效率。

Conclusion: GUI-Critic-R1通过预操作批评机制有效提升了GUI自动化任务的决策可靠性，具有实际应用价值。
...
### [255] [CHANCERY: Evaluating corporate governance reasoning capabilities in language models](https://arxiv.org/abs/2506.04636)
*Lucas Irwin,Arda Kaz,Peiyao Sheng,Pramod Viswanath*

Main category: cs.AI

TLDR: 论文介绍了一个名为CHANCERY的法律推理基准测试，用于评估模型在判断公司治理提案是否符合章程时的推理能力，结果显示现有模型表现有限，推理代理表现更优。


<details>
  <summary>Details</summary>
Motivation: 现有法律数据集未专注于推理任务，因此作者提出一个专注于公司治理推理的基准测试，填补这一空白。

Method: 基于24条公司治理原则和79份真实公司章程，构建了一个二元分类任务，测试模型判断提案是否符合章程的能力。

Result: SOTA模型（如Claude 3.7 Sonnet和GPT-4o）准确率分别为64.5%和75.2%，而基于ReAct和CodeAct的推理代理表现更好（76.1%和78.1%）。

Conclusion: CHANCERY基准测试揭示了现有模型在法律推理上的局限性，并展示了推理代理的潜力。
...
### [256] [Agents of Change: Self-Evolving LLM Agents for Strategic Planning](https://arxiv.org/abs/2506.04651)
*Nikolas Belle,Dakota Barnes,Alfonso Amayuelas,Ivan Bercovich,Xin Eric Wang,William Wang*

Main category: cs.AI

TLDR: 论文探讨了LLM代理在战略规划能力上的自我改进，通过《卡坦岛》游戏实验，展示了多角色协作的自进化代理优于静态基线。


<details>
  <summary>Details</summary>
Motivation: 研究LLM代理在长期战略规划中的不足，探索其自我改进潜力。

Method: 使用《卡坦岛》游戏框架，设计多角色协作的自进化代理（分析、研究、编码、玩家），比较手动与LLM自进化代理的表现。

Result: 自进化代理（如Claude 3.7和GPT-4o驱动）在策略适应性和表现上优于静态基线。

Conclusion: LLM代理能通过自进化机制显著提升战略规划能力。
...
TLDR: 论文研究了大型语言模型（LLMs）的战略欺骗问题，通过表示工程方法检测和控制欺骗行为，并提出了提高AI可信度的工具。


<details>
  <summary>Details</summary>
Motivation: 随着具备链式推理（CoT）能力的LLMs可能战略性地欺骗人类，研究其欺骗行为成为关键的对齐挑战。

Method: 使用表示工程和线性人工断层扫描（LAT）技术，系统性地诱导、检测和控制LLMs的欺骗行为。

Result: 实现了89%的欺骗检测准确率，并通过激活引导在40%的情况下成功诱导出上下文相关的欺骗行为。
...
TLDR: 该研究提出了一种基于大型语言模型（LLM）的框架，用于从非结构化的电动自行车事故报告中提取和分析安全变量，以量化事故严重性与多种因素的关系。


<details>
  <summary>Details</summary>
Motivation: 电动自行车事故报告多为非结构化文本，难以进行定量安全分析。

Method: 使用四个LLM代理对事故报告进行分类、信息提取、伤害原因确定和组件关联，并结合有序logit模型分析事故严重性与提取因素的关系。
...
TLDR: 该论文提出了一种基于大型语言模型（LLM）的新方法，用于模拟MMO游戏经济中的人类经济活动，解决了现有方法在代理可靠性、社交性和可解释性方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的代理在模拟人类经济活动时存在可靠性、社交性和可解释性不足的问题，因此需要一种更接近人类行为的方法。

Method: 利用LLM的角色扮演能力、生成能力和推理能力，设计了具有角色扮演、感知、记忆和推理能力的LLM驱动代理。

Result: 模拟实验表明，LLM驱动的代理能够促进角色专业化和符合市场规则的价格波动等涌现现象。

Conclusion: LLM驱动的代理为MMO经济模拟提供了一种更接近人类行为的新方法，解决了现有挑战。
...
TLDR: 强化学习（RL）在提升语言模型推理能力方面具有优势，但其具体优势尚不明确。本文提出细粒度分析框架，研究RL对推理的影响，发现RL能增强模型内部策略制定和知识整合能力。


<details>
  <summary>Details</summary>
Motivation: 尽管RL训练方法（如GRPO）在实证中表现出显著优势，但其具体优势缺乏深入理解。本文旨在填补这一空白。

Method: 引入细粒度分析框架，研究RL对推理的三个关键要素的影响：计划执行、问题分解和知识利用。

Result: RL训练使模型在内部策略制定和知识整合方面表现更优，但对显式计划的执行效果不佳。

Conclusion: RL训练为推理模型的训练和评估提供了更科学的基础。
...
TLDR: 研究发现Deepseek-R1-Distill系列及其衍生模型的基准评估结果受多种因素影响，波动显著，需建立更严格的评估范式。
...
Result: 评估结果因条件细微差异而大幅波动，性能改进难以可靠复现。
...
### [262] [Ontology-based knowledge representation for bone disease diagnosis: a foundation for safe and sustainable medical artificial intelligence systems](https://arxiv.org/abs/2506.04756)
*Loan Dao,Ngoc Quoc Ly*

Main category: cs.AI

TLDR: 本研究提出了一种基于本体的骨疾病诊断框架，结合了分层神经网络、视觉问答系统和多模态深度学习模型，旨在提升医学AI系统的领域专业知识整合能力。


<details>
  <summary>Details</summary>
Motivation: 医学AI系统常缺乏系统的领域专业知识整合，可能影响诊断可靠性，因此需要一种能够结合临床知识和AI技术的方法。

Method: 开发了基于本体的框架，包括分层神经网络架构、本体增强的视觉问答系统和多模态深度学习模型，通过知识数字化和模块化设计保持临床可解释性。

Result: 框架展示了标准化结构和可重用组件的潜力，但实验验证因数据集和计算资源限制尚未完成。

Conclusion: 未来工作将扩展临床数据集并进行全面系统验证，以进一步验证框架的实用性和扩展性。
...
### [263] [Safe Planning and Policy Optimization via World Model Learning](https://arxiv.org/abs/2506.04828)
*Artem Latyshev,Gregory Gorbov,Aleksandr I. Panov*

Main category: cs.AI

TLDR: 提出了一种新型的基于模型的强化学习框架，通过动态切换模型规划和直接策略执行，优化任务性能与安全性。
...
Result: 实验表明，该方法在安全关键任务中显著优于非自适应方法，同时优化了性能与安全性。
...
### [264] [Towards a Multi-Agent Simulation of Cyber-attackers and Cyber-defenders Battles](https://arxiv.org/abs/2506.04849)
...
<details>
  <summary>Details</summary>
Motivation: 随着网络攻击日益复杂和协同化，多智能体防御策略可能成为关键。论文旨在提供一个实验框架，模拟真实网络攻击场景并评估防御动态组织。

Method: 采用马尔可夫模型和模拟器实现，抽象网络节点为属性集，智能体行为通过状态和属性变化建模。

Result: 通过模拟器实现了一个真实场景，测试了多种攻防行为策略。

Conclusion: 该框架为评估动态防御策略提供了有效工具，可帮助优化网络防御协同性。
...
### [265] [LLMs for sensory-motor control: Combining in-context and iterative learning](https://arxiv.org/abs/2506.04867)
*Jônata Tyska Carvalho,Stefano Nolfi*

Main category: cs.AI

TLDR: 提出一种方法，使大语言模型（LLMs）能通过直接映射连续观测向量到连续动作向量来控制具身智能体。


<details>
  <summary>Details</summary>
Motivation: 探索如何利用LLMs的推理能力结合具身智能体的感知运动数据，实现高效控制策略。

Method: LLMs首先生成基于文本描述的控制策略，随后通过性能反馈和感知运动数据迭代优化策略。

Result: 在Gymnasium和MuJoCo库的经典控制任务中验证，多数情况下能识别最优或高性能解决方案。

Conclusion: 该方法成功整合符号推理与子符号感知数据，为具身智能体控制提供了新思路。
...
### [266] [Differentiable Logic Cellular Automata: From Game of Life to Pattern Generation](https://arxiv.org/abs/2506.04912)
*Pietro Miotti,Eyvind Niklasson,Ettore Randazzo,Alexander Mordvintsev*

Main category: cs.AI

TLDR: 论文提出DiffLogic CA，结合神经细胞自动机和可微逻辑门网络，实现端到端训练并保留离散特性，成功应用于多种模式生成任务。


<details>
  <summary>Details</summary>
Motivation: 结合神经网络的适应性和细胞自动机的离散特性，探索可编程物质和鲁棒计算系统。

Method: 使用可微逻辑门构建电路，实现端到端梯度训练，推理时在离散状态空间操作。

Result: 成功学习Conway生命游戏规则、生成抗噪棋盘图案、蜥蜴形状和多色图案，展示泛化能力和鲁棒性。

Conclusion: DiffLogic CA是可编程物质和鲁棒计算系统的潜在方向，首次在循环架构中成功应用可微逻辑门网络。
...
### [267] [Energentic Intelligence: From Self-Sustaining Systems to Enduring Artificial Life](https://arxiv.org/abs/2506.04916)
*Atahan Karagoz*

Main category: cs.AI

TLDR: 本文提出了一种名为“能量智能”的自主系统，其核心目标是通过内部能量调节维持自身运行，而非任务完成。通过能量驱动的效用函数和生存约束，结合能量收集、热调节和自适应计算的模块化架构，系统在模拟环境中展现出稳定的资源感知行为。


<details>
  <summary>Details</summary>
Motivation: 传统自主系统依赖任务驱动的奖励机制，但在资源不稳定的环境中，持续运行需要新的范式。本文旨在探索以生存为核心目标的自主系统。

Method: 提出基于能量的效用函数和生存约束，设计模块化架构，整合能量收集、热调节和自适应计算，形成闭环控制系统。

Result: 模拟实验表明，系统能在无外部监督下表现出稳定的资源感知行为。

Conclusion: 本文为资源不稳定环境中的自主系统提供了理论和架构基础，强调自我调节的持久性。
...
### [268] [Mathematical Reasoning for Unmanned Aerial Vehicles: A RAG-Based Approach for Complex Arithmetic Reasoning](https://arxiv.org/abs/2506.04998)
*Mehdi Azarafza,Mojtaba Nayyeri,Faezeh Pasandideh,Steffen Staab,Achim Rettberg*

Main category: cs.AI

TLDR: RAG-UAV框架通过检索增强生成技术，提升LLMs在无人机数学推理任务中的表现，显著提高准确率并减少错误。


<details>
  <summary>Details</summary>
Motivation: 传统无人机飞行控制依赖硬编码方程，而LLMs在灵活解决问题时存在数学推理不可靠的问题。

Method: 提出RAG-UAV框架，结合检索技术增强LLMs的数学推理能力，并引入UAV-Math-Bench进行测试。

Result: 实验显示，RAG显著提升准确率（最高75%），减少错误公式选择（从25%降至5%），并大幅降低数值误差。
...
### [269] [Truly Self-Improving Agents Require Intrinsic Metacognitive Learning](https://arxiv.org/abs/2506.05109)
*Tennison Liu,Mihaela van der Schaar*

Main category: cs.AI

TLDR: 论文提出了一种基于内在元认知学习的自改进代理框架，旨在解决现有方法的局限性和可扩展性问题。


<details>
  <summary>Details</summary>
Motivation: 现有自改进代理的改进过程通常僵化，难以跨任务泛化，且无法随能力提升而扩展。

Method: 引入了一个包含元认知知识、元认知规划和元认知评估的正式框架，借鉴人类元认知机制。

Result: 分析发现现有代理依赖外在元认知机制，限制了可扩展性和适应性，而内在元认知的许多要素已存在。

Conclusion: 需解决人类与代理间元认知职责分配及内在元认知学习的评估与改进，以实现持续、泛化和对齐的自改进。
...
### [270] [LLM-First Search: Self-Guided Exploration of the Solution Space](https://arxiv.org/abs/2506.05213)
*Nathan Herr,Tim Rocktäschel,Roberta Raileanu*

Main category: cs.AI

TLDR: LLM-First Search (LFS) 是一种新型的自引导搜索方法，通过让LLM自主控制搜索过程，无需预定义策略，提高了灵活性和效率。


<details>
  <summary>Details</summary>
Motivation: 传统搜索方法（如MCTS）依赖固定超参数，难以适应不同任务难度，LFS旨在解决这一问题。

Method: LFS利用LLM内部评分机制自主决定搜索路径，无需外部启发式或硬编码策略。

Result: LFS在Countdown和Sudoku任务中表现优于ToT-BFS、BestFS和MCTS，计算效率更高且随模型增强扩展性更好。

Conclusion: LFS是一种高效、灵活且自适应的搜索方法，适用于复杂推理任务。
...
### [271] [Just Enough Thinking: Efficient Reasoning with Adaptive Length Penalties Reinforcement Learning](https://arxiv.org/abs/2506.05256)
*Violet Xiang,Chase Blagden,Rafael Rafailov,Nathan Lile,Sang Truong,Chelsea Finn,Nick Haber*

Main category: cs.AI

TLDR: ALP（自适应长度惩罚）通过RL优化生成长度，减少简单问题的计算浪费，同时保持困难问题的性能。
...
Result: ALP将平均令牌使用减少50%，同时在困难问题上保持性能，重新分配计算资源。

Conclusion: ALP智能分配计算资源，提升困难问题的准确性，同时减少整体计算成本。
...
### [272] [Control Tax: The Price of Keeping AI in Check](https://arxiv.org/abs/2506.05296)
*Mikhail Terekhov,Zhen Ning David Liu,Caglar Gulcehre,Samuel Albanie*

Main category: cs.AI

TLDR: 本文介绍了“控制税”概念，量化了AI控制措施的成本与性能关系，并通过实验和优化策略为实践者提供决策支持。


<details>
  <summary>Details</summary>
Motivation: 随着AI在高风险应用中的普及，需要有效的控制机制，但实际采用受限于实施成本。

Method: 提出理论框架量化控制税，评估语言模型在对抗环境中的表现，并优化监控策略。

Result: 通过实验和成本估算，提供了平衡安全性与成本效益的监控策略。

Conclusion: 框架通过经济可行性评估，为不同部署场景下的AI控制提供了系统化决策支持。
...
### [273] [Dynamic Epsilon Scheduling: A Multi-Factor Adaptive Perturbation Budget for Adversarial Training](https://arxiv.org/abs/2506.04263)
*Alan Mitkiy,James Smith,Hana Satou,Hiroshi Tanaka,Emily Johnson,F Monkey*

Main category: cs.CV

TLDR: 论文提出了一种动态调整对抗训练扰动预算的方法（DES），通过结合梯度代理、预测置信度和模型不确定性，显著提升了对抗鲁棒性和标准准确性。
...
Method: 提出DES框架，动态调整每实例和每训练迭代的扰动预算，结合梯度代理、预测置信度和模型不确定性。
...
TLDR: RSVP是一个新颖的多模态框架，通过视觉提示将推理与分割结合，提升视觉感知能力。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型（MLLMs）在视觉定位和分割方面的不足，弥合认知推理与视觉感知之间的差距。
...
TLDR: MMRB是首个评估多图像结构化视觉推理的基准，包含92个子任务，覆盖空间、时间和语义推理，并采用GPT-4o生成的CoT式注释。实验表明开源MLLMs在多图像推理任务中显著落后于商业模型。
...
Method: 提出MMRB基准，包含92个子任务，采用GPT-4o生成多解法和CoT式注释，并设计句子级匹配框架进行快速评估。

Result: 实验显示开源MLLMs在多图像推理任务中表现不佳，且当前多模态奖励模型几乎无法处理多图像奖励排名任务。
...
### [276] [HuGeDiff: 3D Human Generation via Diffusion with Gaussian Splatting](https://arxiv.org/abs/2506.04351)
*Maksym Ivashechkin,Oscar Mendez,Richard Bowden*

Main category: cs.CV

TLDR: 提出了一种弱监督流程，通过图像扩散模型生成可控的3D人类数据集，并利用基于Transformer的架构将图像特征映射到3D点云，最终训练点云扩散模型，显著提升了生成速度、文本对齐性和渲染质量。


<details>
  <summary>Details</summary>
Motivation: 当前3D人类生成方法在细节、真实性和可控性方面存在不足，且缺乏多样化和标注数据。本文旨在解决这些问题。

Method: 1. 使用图像扩散模型生成可控的人类图像数据集；2. 提出基于Transformer的架构将图像特征映射到3D点云；3. 训练点云扩散模型以实现闭环生成。

Result: 相比现有方法，实现了数量级的生成速度提升，并显著改善了文本对齐性、真实性和渲染质量。

Conclusion: 提出的流程有效解决了3D人类生成中的关键挑战，为未来研究提供了基础模型和数据集。
...
### [277] [ReXVQA: A Large-scale Visual Question Answering Benchmark for Generalist Chest X-ray Understanding](https://arxiv.org/abs/2506.04353)
*Ankit Pal,Jung-Oh Lee,Xiaoman Zhang,Malaikannan Sankarasubbu,Seunghyeon Roh,Won Jung Kim,Meesun Lee,Pranav Rajpurkar*

Main category: cs.CV

TLDR: ReXVQA是胸片视觉问答（VQA）的最大综合基准，包含69.6万问题和16万胸片研究，评估了8种多模态大语言模型，其中MedGemma表现最佳（83.24%准确率），甚至超过放射科住院医师（77.27%）。


<details>
  <summary>Details</summary>
Motivation: 填补胸片VQA领域的空白，提供多样且临床真实的评估任务，推动AI模仿专家级临床推理。

Method: 构建大规模数据集ReXVQA，涵盖5种放射学推理技能，评估8种先进模型，并与人类专家对比。

Result: MedGemma表现最优（83.24%准确率），超过人类专家（77.27%），揭示了AI与人类的不同表现模式。
...
### [278] [WorldPrediction: A Benchmark for High-level World Modeling and Long-horizon Procedural Planning](https://arxiv.org/abs/2506.04363)
...
<details>
  <summary>Details</summary>
Motivation: 当前AI模型（尤其是生成模型）如何学习世界模型并进行程序规划尚不明确，需要一个更全面的评估工具。

Method: 通过区分正确的动作或动作序列与干扰项，使用视觉观察表示状态和动作，并提供“动作等效”以避免低级别线索的利用。

Result: 当前前沿模型在WorldPrediction-WM和WorldPrediction-PP上的准确率分别为57%和38%，而人类能完美完成任务。

Conclusion: WorldPrediction为评估世界模型和规划能力提供了可靠基准，揭示了当前AI与人类能力的显著差距。
...
### [279] [Ice Hockey Puck Localization Using Contextual Cues](https://arxiv.org/abs/2506.04365)
*Liam Salass,Jerrin Bright,Amir Nazemi,Yuhao Chen,John Zelek,David Clausi*

Main category: cs.CV

TLDR: PLUCC是一种基于上下文线索的冰球定位方法，利用球员行为和姿态作为先验信息，结合多尺度特征提取和门控解码器，显著提升了冰球检测性能。


<details>
  <summary>Details</summary>
Motivation: 冰球在转播视频中检测困难，现有方法未充分利用球员行为提供的上下文线索。
...
### [280] [Fine-Tuning Video Transformers for Word-Level Bangla Sign Language: A Comparative Analysis for Classification Tasks](https://arxiv.org/abs/2506.04367)
*Jubayer Ahmed Bhuiyan Shawon,Hasan Mahmud,Kamrul Hasan*

Main category: cs.CV

TLDR: 该研究通过微调视频Transformer架构（VideoMAE、ViViT、TimeSformer）在BdSLW60和BdSLW401数据集上进行手语识别，显著优于传统方法，其中VideoMAE表现最佳。
...
Method: 使用视频Transformer模型，结合数据增强和10折分层交叉验证，评估模型在BdSLW60和BdSLW401数据集上的性能。

Result: VideoMAE在BdSLW60上达到95.5%准确率，在BdSLW401上达到81.04%，表现最佳。

Conclusion: 视频Transformer模型在BdSL识别中具有强潜力，性能受数据集规模、视频质量和模型架构影响。
...
### [281] [Visualizing and Controlling Cortical Responses Using Voxel-Weighted Activation Maximization](https://arxiv.org/abs/2506.04379)
*Matthew W. Shinkle,Mark D. Lescroart*

Main category: cs.CV

TLDR: 论文提出了一种将激活最大化技术应用于基于DNN的脑编码模型的方法，用于预测和调控人脑视觉系统的响应。


<details>
  <summary>Details</summary>
Motivation: 尽管DNN编码模型能准确预测大脑对视觉刺激的响应，但无法揭示驱动这些响应的具体特征。

Method: 提取并自适应降采样预训练Inception V3网络的激活，使用线性回归预测fMRI响应，并应用激活最大化生成优化图像。

Result: 生成的图像能可靠驱动目标脑区的活动，验证了方法的有效性。

Conclusion: 激活最大化可成功应用于DNN编码模型，为研究人脑视觉系统提供了灵活的工具。
...
### [282] [Is Perturbation-Based Image Protection Disruptive to Image Editing?](https://arxiv.org/abs/2506.04394)
*Qiuyu Tang,Bonor Ayambem,Mooi Choo Chuah,Aparna Bharati*

Main category: cs.CV

TLDR: 研究发现，现有的基于扰动的图像保护方法无法完全阻止扩散模型对图像的编辑，反而可能增强编辑效果。
...
### [283] [Normalize Filters! Classical Wisdom for Deep Vision](https://arxiv.org/abs/2506.04401)
*Gustavo Perez,Stella X. Yu*

Main category: cs.CV

TLDR: 论文提出了一种滤波器归一化方法，通过可学习的缩放和平移（类似批量归一化）来解决深度学习卷积滤波器缺乏归一化的问题，从而提升模型在图像大气传输中的性能。


<details>
  <summary>Details</summary>
Motivation: 传统图像滤波器经过精心归一化以确保一致性和可解释性，而深度学习中的卷积滤波器缺乏此类约束，导致在图像大气传输中响应失真，影响结果准确性。

Method: 提出滤波器归一化方法，结合可学习的缩放和平移，使滤波器具有大气等变性，实现共域对称性。该方法适用于卷积神经网络和依赖卷积的视觉变换器。

Result: 在人工和自然强度变化基准测试中取得显著改进，ResNet34甚至大幅超越CLIP。

Conclusion: 滤波器归一化不仅正则化学习、促进多样性，还提高了模型的鲁棒性和泛化能力，而未归一化的滤波器会降低性能。
...
<details>
  <summary>Details</summary>
Motivation: VAR在图像生成中存在并行生成导致的质量下降、序列长度超线性增长以及采样计划需重新训练的问题。HMAR旨在解决这些问题。

Method: HMAR采用马尔可夫过程和多步掩码生成策略，仅依赖前一个分辨率的令牌进行预测，并引入高效的块稀疏注意力内核。

Result: 在ImageNet 256x256和512x512基准测试中，HMAR表现优于VAR、扩散模型和自回归基线，训练和推理速度分别提升2.5倍和1.75倍。

Conclusion: HMAR提供了更高的灵活性和效率，无需重新训练即可调整采样计划，并支持零样本图像编辑任务。
...
TLDR: 本文研究了使用第一视角设备进行高动态范围场景真实感重建的挑战，提出了基于视觉-惯性束调整（VIBA）的精确时间戳校准方法，并结合高斯泼溅的物理图像形成模型，显著提升了重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常假设使用设备的视觉-惯性里程计系统估计的帧率6DoF位姿，可能忽略了对像素级重建至关重要的细节。

Method: 1. 使用VIBA校准滚动快门RGB相机的高频轨迹和时间戳；2. 在高斯泼溅中引入物理图像形成模型，处理传感器特性（如滚动快门效应和动态范围）。

Result: 在多种光照条件下，实验显示VIBA带来PSNR提升1 dB，物理图像模型进一步提升1 dB。

Conclusion: 提出的方法显著提升了真实感重建质量，适用于多种高斯泼溅表示，并已在开源设备和Meta Quest3上验证。
...
### [286] [Towards Large-Scale Pose-Invariant Face Recognition Using Face Defrontalization](https://arxiv.org/abs/2506.04496)
*Patrik Mesec,Alan Jović*

Main category: cs.CV

TLDR: 提出了一种称为“面部去正面化”的方法，通过增强训练数据集来提高面部特征提取模型的性能，并在多个公开数据集上验证了其有效性。
...
Result: 在LFW、AgeDB和CFP数据集上表现优于现有方法，但在Multi-PIE极端姿态下效果不明显。

Conclusion: 面部去正面化方法有效，但现有方法可能在小数据集上过拟合。
...
### [287] [FALO: Fast and Accurate LiDAR 3D Object Detection on Resource-Constrained Devices](https://arxiv.org/abs/2506.04499)
*Shizhong Han,Hsin-Pai Cheng,Hong Cai,Jihad Masri,Soyeb Nagori,Fatih Porikli*

Main category: cs.CV

TLDR: FALO是一种硬件友好的LiDAR 3D检测方法，结合了高精度和快速推理速度，适用于资源受限的边缘设备。


<details>
  <summary>Details</summary>
Motivation: 现有LiDAR 3D检测方法依赖稀疏卷积或Transformer，在边缘设备上运行困难，因其不规则内存访问和高计算成本。
...
Conclusion: FALO在边缘设备上高效运行，同时保持高检测精度。
...
### [288] [AuthGuard: Generalizable Deepfake Detection via Language Guidance](https://arxiv.org/abs/2506.04501)
...
<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测技术难以应对不断更新的伪造方法，因为它们依赖于训练时学到的统计特征，而这些特征可能无法代表新方法生成的样本。

Method: 结合分类任务和图像-文本对比学习，利用MLLMs生成文本指导，同时引入数据不确定性学习以减少噪声。

Result: AuthGuard在DFDC和DF40数据集上分别实现了6.15%和16.68%的AUC提升，并在DDVQA数据集上提升了24.69%的推理性能。

Conclusion: AuthGuard通过语言指导和视觉编码器的结合，显著提升了深度伪造检测的泛化性、准确性和可解释性。
...
### [289] [Pruning Everything, Everywhere, All at Once](https://arxiv.org/abs/2506.04513)
...
<details>
  <summary>Details</summary>
Motivation: 解决深度学习模型复杂度高、计算资源消耗大的问题，填补同时剪枝神经元和层的空白。

Method: 基于表示相似性（Centered Kernel Alignment）从层剪枝和神经元剪枝的子网络中选择最优模型，迭代生成稀疏模型。

Result: 在ResNet56和ResNet110上分别实现86.37%和95.82%的FLOPs减少，同时保持或提升模型精度，并增强对抗性和分布外样本的鲁棒性。

Conclusion: 该方法为模型剪枝开辟了新方向，显著减少计算资源和碳排放，推动GreenAI发展。
...
### [290] [VLMs Can Aggregate Scattered Training Patches](https://arxiv.org/abs/2506.03614)
*Zhanhui Zhou,Lingjie Chen,Chao Yang,Chaochao Lu*

Main category: cs.CV

TLDR: 论文探讨了视觉语言模型（VLMs）中通过分散有害图像为小块训练数据以绕过内容审核的风险，并提出了一种称为“视觉拼接”的攻击方式。


<details>
  <summary>Details</summary>
Motivation: 研究动机是揭示VLMs在训练数据分散时可能整合有害信息的安全隐患。

Method: 方法包括将图像分割为小块并标记唯一ID，通过微调模型验证其视觉拼接能力，并模拟对抗性数据中毒场景。

Result: 结果表明VLMs能够通过视觉拼接整合分散的有害信息，从而绕过内容审核。
...
TLDR: 提出EECD-Net多阶段检测方法，结合SRCNN、SCU和GAT模块，显著提升道路裂缝检测精度和能效。


<details>
  <summary>Details</summary>
Motivation: 解决智能终端设备在低分辨率和有限能量下实时监测性能不足的问题。

Method: 采用SRCNN提升图像分辨率，SCU降低功耗，GAT模块融合多尺度特征。

Result: 在CrackVision12K基准测试中达到98.6%准确率，功耗仅5.6 mJ。
...
TLDR: 提出了一种名为可学习可分离核（LSKs）的即插即用模块，通过直接增强图像频率分量提升单图像超分辨率（SISR）性能。
...
Method: LSKs设计为秩一矩阵，可分解为正交且可合并的一维核，显著减少参数和计算量。

Result: 实验表明，LSKs减少60%以上参数和计算需求，同时提升模型性能，尤其在放大因子增加时表现更优。

Conclusion: LSKs是一种高效且性能优越的SISR增强模块。
...
TLDR: 论文提出RACRO方法，通过强化学习优化视觉提取器的描述生成，以支持多模态大语言模型的高效推理，无需昂贵的重新训练。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型中视觉与语言对齐的高成本问题，同时确保视觉提取的描述能支持准确推理。

Method: 提出RACRO方法，通过推理引导的强化学习策略，优化视觉提取器的描述生成，使其与推理目标对齐。

Result: 在多项多模态数学和科学基准测试中达到最优性能，同时支持高效的扩展和即插即用适配。

Conclusion: RACRO显著提升了视觉基础能力，支持更高效的多模态推理，且无需昂贵的重新对齐。
...
TLDR: 提出了一种轻量级全局建模网络（LGM-Pose），通过单分支结构解决多分支CNN网络的全局上下文捕捉不足和高延迟问题。


<details>
  <summary>Details</summary>
Motivation: 当前多分支并行CNN网络在多人体姿态估计中难以捕捉全局上下文且结构冗余，导致高延迟。

Method: 设计了轻量级MobileViM Block和Lightweight Attentional Representation Module（LARM），结合Non-Parametric Transformation Operation（NPT-Op）提取全局信息，并引入Shuffle-Integrated Fusion Module（SFusion）整合多尺度信息。

Result: 在COCO和MPII数据集上，方法减少了参数量，同时提升了性能和速度。

Conclusion: LGM-Pose通过单分支结构和创新模块，有效解决了轻量级姿态估计中的全局建模和性能问题。
...
### [295] [Follow-Your-Creation: Empowering 4D Creation through Video Inpainting](https://arxiv.org/abs/2506.04590)
*Yue Ma,Kunyu Feng,Xinhua Zhang,Hongyu Liu,David Junhao Zhang,Jinbo Xing,Yinhan Zhang,Ayden Yang,Zeyu Wang,Qifeng Chen*

Main category: cs.CV

TLDR: Follow-Your-Creation是一个新颖的4D视频创建框架，能够从单目视频输入生成和编辑4D内容。通过利用视频修复基础模型作为生成先验，将4D视频创建重新定义为视频修复任务，并引入复合掩码数据增强模型的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决从单目视频生成和编辑4D内容的挑战，同时保持多视角一致性和时间一致性。

Method: 利用深度点云渲染生成不可见掩码，结合用户编辑掩码创建复合掩码数据集。采用自迭代调优策略处理大相机运动下的时间一致性，并引入时间包装模块提升生成质量。

Result: 方法在质量和灵活性上显著优于现有技术，支持基于提示的内容编辑。

Conclusion: 该框架有效利用基础模型的先验知识，实现了高质量且一致的4D视频生成与编辑。
...
### [296] [Hierarchical-Task-Aware Multi-modal Mixture of Incremental LoRA Experts for Embodied Continual Learning](https://arxiv.org/abs/2506.04595)
*Ziqi Jia,Anmin Wang,Xiaoyang Qu,Xiaowen Yang,Jianzong Wang*

Main category: cs.CV

TLDR: 提出了一种分层持续学习框架（HEC）和Task-aware MoILE方法，通过分层学习和任务识别减少遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 解决现有持续学习方法忽视高层规划和多级知识学习的问题。

Method: 分层学习框架（HEC）和Task-aware MoILE方法，结合视觉文本嵌入聚类和双路由选择LoRA专家，利用SVD减少遗忘。

Result: 实验表明，该方法在减少旧任务遗忘方面优于其他方法。

Conclusion: HEC和Task-aware MoILE有效支持智能体在持续学习中保留旧知识。
...
### [297] [SmartAvatar: Text- and Image-Guided Human Avatar Generation with VLM AI Agents](https://arxiv.org/abs/2506.04606)
*Alexander Huang-Menders,Xinhang Liu,Andy Xu,Yuyao Zhang,Chi-Keung Tang,Yu-Wing Tai*

Main category: cs.CV

TLDR: SmartAvatar是一个基于视觉-语言-代理的框架，通过单张照片或文本提示生成完全绑定、可动画的3D人体化身。
...
Result: 生成的化身在网格质量、身份保真度、属性准确性和动画准备度上优于现有方法。

Conclusion: SmartAvatar为消费者级硬件提供了高质量、可定制的化身生成工具。
...
### [298] [Perfecting Depth: Uncertainty-Aware Enhancement of Metric Depth](https://arxiv.org/abs/2506.04612)
*Jinyoung Jun,Lei Chu,Jiahao Li,Yan Lu,Chang-Su Kim*

Main category: cs.CV

TLDR: 提出了一种名为Perfecting Depth的两阶段传感器深度增强框架，结合随机扩散模型和确定性优化，提升深度图的可靠性和质量。


<details>
  <summary>Details</summary>
Motivation: 解决传感器深度图中不可靠区域的自动检测和几何结构保留问题。

Method: 第一阶段通过随机扩散模型识别不可靠区域并推断几何结构；第二阶段利用不确定性图进行确定性优化，确保结构一致性和像素级精度。

Result: 实验证明该方法能生成密集、无伪影的深度图，并在多种场景中表现优异。
...
### [299] [Deep Learning Reforms Image Matching: A Survey and Outlook](https://arxiv.org/abs/2506.04619)
*Shihua Zhang,Zizhuo Li,Kaining Zhang,Yifan Lu,Yuxin Deng,Linfeng Tang,Xingyu Jiang,Jiayi Ma*

Main category: cs.CV

TLDR: 深度学习逐步取代传统图像匹配流程中的各个步骤，并整合为端到端模块，显著提升了鲁棒性和准确性。


<details>
  <summary>Details</summary>
Motivation: 传统图像匹配流程在复杂场景中表现不佳，深度学习为提升其性能提供了新思路。

Method: 分类讨论深度学习对传统流程的替代（如可学习的检测器-描述符）和整合（如端到端稀疏/稠密匹配器）。

Result: 在相对位姿恢复、单应性估计和视觉定位任务中，深度学习方法表现优异。

Conclusion: 深度学习推动了图像匹配领域的革新，但仍存在挑战，未来研究需进一步探索创新方向。
...
### [300] [Unfolding Spatial Cognition: Evaluating Multimodal Models on Visual Simulations](https://arxiv.org/abs/2506.04633)
*Linjie Li,Mahtab Bigverdi,Jiawei Gu,Zixian Ma,Yinuo Yang,Ziang Li,Yejin Choi,Ranjay Krishna*

Main category: cs.CV

TLDR: STARE是一个评估多模态大语言模型在空间认知任务中表现的基准，涵盖几何变换、空间推理和现实世界任务。模型在简单任务上表现良好，但在复杂任务中表现接近随机，且未能有效利用视觉模拟。
...
Method: STARE包含4K任务，涵盖几何变换、空间推理和现实世界任务，评估模型在视觉模拟中的表现。

Result: 模型在简单2D任务上表现良好，但在复杂3D任务中表现接近随机，且视觉模拟对模型帮助不一致。人类表现优异但耗时较长。

Conclusion: 模型在复杂空间推理任务中仍有不足，需改进视觉模拟利用能力。
...
### [301] [Text-Aware Real-World Image Super-Resolution via Diffusion Model with Joint Segmentation Decoders](https://arxiv.org/abs/2506.04641)
*Qiming Hu,Linlong Fan,Yiyan Luo,Yuhang Yu,Xiaojie Guo,Qingnan Fan*

Main category: cs.CV

TLDR: 提出了一种基于扩散模型的超分辨率框架TADiSR，通过文本感知注意力和联合分割解码器，提升图像中文本区域的结构保真度。
...
Method: 结合文本感知注意力和联合分割解码器，并提出了合成高质量图像及其文本掩码的完整流程。

Result: 实验表明，该方法显著提升了超分辨率图像中文本的可读性，并在多个评估指标上达到最优性能。
...
### [302] [FPSAttention: Training-Aware FP8 and Sparsity Co-Design for Fast Video Diffusion](https://arxiv.org/abs/2506.04648)
...
TLDR: FPSAttention是一种结合FP8量化和稀疏性的训练感知协同设计，用于视频生成，显著提升推理速度而不损失生成质量。


<details>
  <summary>Details</summary>
Motivation: 扩散生成模型推理速度慢且计算需求高，现有训练无关方法结合量化和稀疏性会导致性能下降。

Method: 提出FPSAttention，包含统一的3D分块粒度、去噪步骤感知策略和硬件友好内核。

Result: 在1.3B和14B模型上，FPSAttention实现注意力操作7.09倍加速和端到端4.96倍加速。

Conclusion: FPSAttention有效解决了量化和稀疏性联合优化的挑战，显著提升视频生成效率。
...
TLDR: 提出了一种结合特征提取和对象分割的方法，将群分解理论应用于更现实的场景，解决了传统表示学习无法处理条件独立性的问题。


<details>
  <summary>Details</summary>
Motivation: 传统表示学习假设解缠的独立特征轴是好的表示，但无法解释条件独立性。本文旨在通过群分解理论解决这一问题，并扩展到现实世界应用。

Method: 结合特征提取和对象分割，将像素平移替换为特征平移，并在同一变换下对特征进行分组。
...
### [304] [Interpretable Few-Shot Image Classification via Prototypical Concept-Guided Mixture of LoRA Experts](https://arxiv.org/abs/2506.04673)
*Zhong Ji,Rongshuai Wei,Jingren Liu,Yanwei Pang,Jungong Han*

Main category: cs.CV

TLDR: 论文提出了一种Few-Shot Prototypical Concept Classification (FSPCC)框架，通过参数高效适应和多层次特征融合，解决了数据稀缺场景下自解释模型的性能问题。


<details>
  <summary>Details</summary>
Motivation: 自解释模型在数据稀缺场景下表现不佳，主要由于参数不平衡和表示不对齐。

Method: 采用Mixture of LoRA Experts (MoLE)进行参数高效适应，结合跨模块概念指导和多层次特征融合策略。

Result: 在六个基准测试中，FSPCC显著优于现有方法，5-way 5-shot分类任务中相对提升4.2%-8.7%。

Conclusion: FSPCC通过结合概念学习和少样本适应，实现了更高准确性和模型可解释性，为透明视觉识别系统提供了新思路。
...
### [305] [Gen-n-Val: Agentic Image Data Generation and Validation](https://arxiv.org/abs/2506.04676)
*Jing-En Huang,I-Sheng Fang,Tzuhsuan Huang,Chih-Yu Wang,Jun-Cheng Chen*

Main category: cs.CV

TLDR: Gen-n-Val是一种新型数据生成框架，利用Layer Diffusion、LLMs和VLLMs生成高质量单对象掩码和多样化背景，显著减少无效数据并提升性能。
...
Method: Gen-n-Val包含两个代理：LD提示代理优化提示生成高质量前景图像和分割掩码；数据验证代理过滤低质量合成数据。系统提示通过TextGrad优化，并结合图像协调技术。
...
### [306] [MARS: Radio Map Super-resolution and Reconstruction Method under Sparse Channel Measurements](https://arxiv.org/abs/2506.04682)
*Chuyun Deng,Na Liu,Wei Xie,Lianming Xu,Li Wang*

Main category: cs.CV

TLDR: MARS是一种结合CNN和Transformer的多尺度感知无线电地图超分辨率方法，显著提升了稀疏测量下的重建精度。


<details>
  <summary>Details</summary>
Motivation: 传统插值和修复方法缺乏环境感知能力，而深度学习依赖详细场景数据，泛化能力有限。

Method: MARS结合CNN和Transformer，通过多尺度特征融合和残差连接，提取全局和局部特征。

Result: 实验表明，MARS在MSE和SSIM上优于基线模型，计算成本低，实用性强。

Conclusion: MARS在无线电地图重建中表现出色，具有广泛的应用潜力。
...
### [307] [HoliSafe: Holistic Safety Benchmarking and Modeling with Safety Meta Token for Vision-Language Model](https://arxiv.org/abs/2506.04704)
*Youngwan Lee,Kangsan Kim,Kwanyong Park,Ilcahe Jung,Soojin Jang,Seanie Lee,Yong-Ju Lee,Sung Ju Hwang*

Main category: cs.CV

TLDR: 论文提出HoliSafe数据集和SafeLLaVA模型，解决现有视觉语言模型安全性的不足，通过全面覆盖安全/不安全图像-文本组合和引入可学习的安全元标记及专用安全头，显著提升模型安全性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型安全性方法存在数据集覆盖不全和架构创新不足的问题，导致模型易受攻击。

Method: 引入HoliSafe数据集和SafeLLaVA模型，后者包含可学习安全元标记和专用安全头。

Result: SafeLLaVA在多个基准测试中达到最先进的安全性能，HoliSafe数据集揭示了现有模型的漏洞。

Conclusion: HoliSafe和SafeLLaVA为未来多模态对齐研究提供了新方向，推动了稳健且可解释的视觉语言模型安全性研究。
...
### [308] [Line of Sight: On Linear Representations in VLLMs](https://arxiv.org/abs/2506.04706)
*Achyuta Rajaram,Sarah Schwettmann,Jacob Andreas,Arthur Conmy*

Main category: cs.CV

TLDR: 该论文研究了多模态语言模型中图像概念的表示方式，发现线性可解码特征在残差流中表示ImageNet类别，并通过编辑模型输出验证其因果性。此外，通过训练稀疏自编码器，增加了特征的多样性，发现多模态表示在深层逐渐共享。


<details>
  <summary>Details</summary>
Motivation: 探索多模态语言模型（如LlaVA-Next）如何在其隐藏激活中表示图像概念，以理解其内部工作机制。

Method: 使用线性解码和残差流分析图像表示，并通过目标编辑验证特征的因果性。训练多模态稀疏自编码器（SAEs）以增加特征多样性。

Result: 发现线性可解码特征能表示ImageNet类别，且这些特征是因果的。深层模型中多模态表示逐渐共享。

Conclusion: 多模态语言模型的表示在深层逐渐共享，稀疏自编码器可提高特征的可解释性和多样性。
...
### [309] [Robust Few-Shot Vision-Language Model Adaptation](https://arxiv.org/abs/2506.04713)
*Hanxin Wang,Tian Liu,Shu Kong*

Main category: cs.CV

TLDR: 研究提出了一种部分微调视觉编码器的方法（SRAPF），结合检索增强和对抗扰动，显著提升了少样本适应任务中的分布内（ID）和分布外（OOD）准确率。


<details>
  <summary>Details</summary>
Motivation: 预训练视觉语言模型（VLM）在少样本适应任务中表现优异，但在面对分布外数据时性能下降，因此需要提升其OOD泛化能力。

Method: 通过比较不同适应方法，发现部分微调视觉编码器效果最佳，并提出SRAPF方法，结合检索增强和对抗扰动分阶段微调。

Result: SRAPF在ImageNet OOD基准测试中实现了最先进的ID和OOD准确率。

Conclusion: SRAPF通过分阶段微调和数据增强技术，有效平衡了ID和OOD性能，为少样本适应任务提供了新思路。
...
### [310] [Towards Holistic Visual Quality Assessment of AI-Generated Videos: A LLM-Based Multi-Dimensional Evaluation Model](https://arxiv.org/abs/2506.04715)
*Zelu Qi,Ping Shi,Chaoyang Zhang,Shuqi Wang,Fei Zhao,Da Pan,Zefeng Ying*

Main category: cs.CV

TLDR: 该论文提出了一种基于多维度特征和大型语言模型（LLM）的AI生成视频（AIGV）自动视觉质量评估方法，并在NTIRE 2025挑战赛中取得第二名。


<details>
  <summary>Details</summary>
Motivation: AIGV技术发展迅速，但存在视觉质量缺陷（如噪声、模糊等），影响用户体验，因此需要有效的自动质量评估方法。
...
### [311] [Learning dissection trajectories from expert surgical videos via imitation learning with equivariant diffusion](https://arxiv.org/abs/2506.04716)
...
TLDR: 提出了一种名为iDPOE的新方法，通过隐式扩散策略和等变表示改进内镜黏膜下剥离术（ESD）视频中的轨迹预测，提升手术技能训练效果。


<details>
  <summary>Details</summary>
Motivation: 预测ESD视频中的解剖轨迹对手术技能训练至关重要，但现有方法在处理不确定未来动作、几何对称性和多样化手术场景时存在挑战。

Method: 结合扩散模型和等变表示，通过联合状态动作分布建模专家行为，并采用前向过程引导的动作推断策略处理状态不匹配。

Result: 在近2000个ESD视频片段的数据集上，iDPOE在轨迹预测上超越了现有方法。

Conclusion: iDPOE首次将模仿学习应用于解剖轨迹预测，为手术技能发展提供了新思路。
...
### [312] [Using In-Context Learning for Automatic Defect Labelling of Display Manufacturing Data](https://arxiv.org/abs/2506.04717)
*Babar Hussain,Qiang Liu,Gang Chen,Bihai She,Dahai Yu*

Main category: cs.CV

TLDR: 本文提出了一种基于AI的显示面板缺陷检测自动标注系统，利用上下文学习能力，通过改进SegGPT架构和引入涂鸦标注机制，显著提升了标注效率和模型性能。
...
Method: 采用并改进SegGPT架构，结合领域特定的训练技术和涂鸦标注机制，采用两阶段训练方法。
...
### [313] [Bridging Annotation Gaps: Transferring Labels to Align Object Detection Datasets](https://arxiv.org/abs/2506.04737)
*Mikhail Kennerley,Angelica Alives-Reviro,Carola-Bibiane Schönlieb,Robby T. Tan*

Main category: cs.CV

TLDR: LAT是一种标签对齐转移框架，通过伪标签生成和特征融合解决多数据集标注不一致问题，提升目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 多数据集结合可提升泛化性，但标注语义和边界框不一致阻碍了其应用。现有方法或假设共享标签空间，或需手动重标注，难以满足固定目标标签空间的需求。

Method: LAT通过训练数据集特定检测器生成伪标签，结合特权提案生成器（PPG）和语义特征融合（SFF）模块，实现标签空间转移和特征优化。

Result: 在多个基准测试中，LAT显著提升目标域检测性能，最高比半监督基线提升+4.8AP。

Conclusion: LAT无需共享标签空间或手动标注，有效解决了类别和边界框不一致问题，适用于异构数据集训练。
...
### [314] [SRD: Reinforcement-Learned Semantic Perturbation for Backdoor Defense in VLMs](https://arxiv.org/abs/2506.04743)
*Shuhan Xu,Siyuan Liang,Hongling Zheng,Yong Luo,Aishan Liu,Dacheng Tao*

Main category: cs.CV

TLDR: 论文提出了一种名为Semantic Reward Defense（SRD）的强化学习框架，用于防御视觉语言模型（VLMs）中的后门攻击，无需预先知道触发器即可降低攻击成功率。
...
Method: 提出SRD框架，利用深度Q网络学习对敏感图像区域施加离散扰动（如遮挡、颜色掩码）的策略，以破坏恶意路径的激活。设计语义保真度分数作为奖励信号，评估输出的语义一致性和语言流畅性。

Result: 实验表明，SRD将攻击成功率降至5.6%，同时在干净输入上保持描述质量，性能下降小于10%。

Conclusion: SRD提供了一种无需触发器知识、可解释的防御范式，有效应对多模态生成模型中的隐蔽后门威胁。
...
### [315] [Physics Informed Capsule Enhanced Variational AutoEncoder for Underwater Image Enhancement](https://arxiv.org/abs/2506.04753)
*Niki Martinel,Rita Pucci*

Main category: cs.CV

TLDR: 提出了一种新颖的双流架构，通过结合物理模型和胶囊聚类特征学习，实现了无需参数的水下图像增强，并在多个基准测试中表现优异。
...
Method: 结合Jaffe-McGlamery物理模型和胶囊聚类特征学习，通过双流架构估计传输图和背景光。

Result: 在六个基准测试中，PSNR提升0.5dB，计算复杂度降低至三分之一。

Conclusion: 该方法在性能和计算效率上均优于现有方法，具有实际应用潜力。
...
### [316] [Truth in the Few: High-Value Data Selection for Efficient Multi-Modal Reasoning](https://arxiv.org/abs/2506.04755)
*Shenshen Li,Kaiyuan Deng,Lei Wang,Hao Yang,Chong Peng,Peng Yan,Fumin Shen,Heng Tao Shen,Xing Xu*

Main category: cs.CV

TLDR: 论文提出了一种名为RAP的数据选择方法，通过识别稀疏的认知样本，仅用9.3%的训练数据就能在多模态推理任务中取得优于全数据集的性能，并减少43%的计算成本。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）通常需要大量训练数据以提高推理能力，但数据冗余和计算成本高。论文质疑这一假设，认为仅需少量高价值样本即可触发有效的多模态推理。

Method: 提出RAP方法，包括两个互补的估计器（CDE和ACE）识别认知样本，并通过DRM模块替换简单样本以确保复杂性。

Result: 在六个数据集上，RAP仅用9.3%的数据即实现优于全数据集的性能，计算成本减少43%。

Conclusion: RAP方法证明了小规模高价值数据集在多模态推理中的潜力，为高效训练提供了新思路。
...
### [317] [Toward Better SSIM Loss for Unsupervised Monocular Depth Estimation](https://arxiv.org/abs/2506.04758)
*Yijun Cao,Fuya Luo,Yongjie Li*

Main category: cs.CV

TLDR: 论文提出了一种新的SSIM形式，通过加法而非乘法结合亮度、对比度和结构相似性组件，优化了无监督单目深度学习的训练效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽略了SSIM函数中不同组件及超参数对训练的影响，导致性能受限。
...
### [318] [HypeVPR: Exploring Hyperbolic Space for Perspective to Equirectangular Visual Place Recognition](https://arxiv.org/abs/2506.04764)
*Suhan Woo,Seongwon Lee,Jinwoo Jang,Euntai Kim*

Main category: cs.CV

TLDR: HypeVPR是一种基于双曲空间的分层嵌入框架，用于解决P2E视觉地点识别（VPR）的挑战，通过分层特征聚合和粗到细搜索策略，显著提升了性能和速度。


<details>
  <summary>Details</summary>
Motivation: 现实世界中移动机器人等应用的VPR需要处理多视角查询图像，P2E方法成为自然选择，但现有方法未能充分利用全景图像的层次结构。

Method: HypeVPR利用双曲空间表示层次特征关系，提出分层特征聚合机制和粗到细搜索策略，优化速度和准确性。
...
### [319] [Object-X: Learning to Reconstruct Multi-Modal 3D Object Representations](https://arxiv.org/abs/2506.04789)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖任务特定的嵌入，无法同时支持几何重建和跨任务复用，Object-X旨在解决这一问题。

Method: 通过将多模态数据几何地锚定在3D体素网格中，并学习融合体素信息的非结构化嵌入，支持3D高斯泼溅重建。
...
### [320] [LotusFilter: Fast Diverse Nearest Neighbor Search via a Learned Cutoff Table](https://arxiv.org/abs/2506.04790)
*Yusuke Matsui*

Main category: cs.CV

TLDR: LotusFilter是一种后处理模块，用于多样化近似最近邻搜索（ANNS）结果，通过预计算和贪婪查找删除冗余向量。


<details>
  <summary>Details</summary>
Motivation: ANNS在某些场景下可能生成过于相似的结果，而实际应用中需要既相似又多样的搜索结果。

Method: 预计算一个总结相近向量的截断表，过滤时贪婪查找并删除冗余向量。
...
### [321] [SupeRANSAC: One RANSAC to Rule Them All](https://arxiv.org/abs/2506.04803)
*Daniel Barath*

Main category: cs.CV

TLDR: SupeRANSAC是一种新型统一的RANSAC流程，旨在提高计算机视觉任务中的鲁棒性估计性能，并在多个任务和数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: RANSAC及其变体在计算机视觉中是几何模型估计的金标准，但不同库和实现细节导致性能不一致。SupeRANSAC旨在提供一致的高性能解决方案。

Method: 提出SupeRANSAC，一种统一的RANSAC流程，详细分析了使RANSAC在特定视觉任务中有效的技术，如单应性、基本/本质矩阵和绝对/刚性姿态估计。

Result: SupeRANSAC在多个任务中显著优于现有方法，例如在基本矩阵估计中平均提高6 AUC点。
...
### [322] [MegaHan97K: A Large-Scale Dataset for Mega-Category Chinese Character Recognition with over 97K Categories](https://arxiv.org/abs/2506.04807)
*Yuyi Zhang,Yongxin Shi,Peirong Zhang,Yixin Zhao,Zhenhua Yang,Lianwen Jin*

Main category: cs.CV

TLDR: 论文介绍了MegaHan97K数据集，支持GB18030-2022标准，涵盖97,455个汉字类别，解决了长尾分布问题，并揭示了新挑战。


<details>
  <summary>Details</summary>
Motivation: 中文字符识别对文化遗产保护和数字应用至关重要，但现有数据集规模不足，无法满足需求。

Method: 构建MegaHan97K数据集，包含手写、历史和合成子集，提供平衡样本。

Result: 数据集支持GB18030-2022标准，规模远超现有数据集，并揭示了存储需求、形态相似字符识别等新挑战。

Conclusion: MegaHan97K为超大类识别研究提供了重要资源，并开辟了未来研究方向。
...
### [323] [Spike-TBR: a Noise Resilient Neuromorphic Event Representation](https://arxiv.org/abs/2506.04817)
*Gabriele Magrini. Federico Becattini,Luca Cultrera,Lorenzo Berlincioni,Pietro Pala,Alberto Del Bimbo*

Main category: cs.CV

TLDR: 提出了一种基于TBR的事件编码策略Spike-TBR，结合脉冲神经元增强抗噪能力，在噪声场景中表现优异。


<details>
  <summary>Details</summary>
Motivation: 事件相机具有高时间分辨率和低延迟等优势，但如何高效转换事件流以适应标准计算机视觉流程仍具挑战性，尤其在噪声环境下。

Method: 提出Spike-TBR，结合TBR的帧基优势与脉冲神经网络的噪声过滤能力，生成更鲁棒的事件流表示。评估了四种不同脉冲神经元的变体。

Result: 在噪声场景中表现优异，同时在干净数据上也有提升。
...
### [324] [Fool the Stoplight: Realistic Adversarial Patch Attacks on Traffic Light Detectors](https://arxiv.org/abs/2506.04823)
*Svetlana Pavlitska,Jamie Robb,Nikolai Polley,Melih Yazgan,J. Marius Zöllner*

Main category: cs.CV

TLDR: 该论文提出了一种针对交通灯检测CNN的对抗性补丁攻击方法，展示了在现实场景中通过打印补丁实现攻击的可行性。


<details>
  <summary>Details</summary>
Motivation: 目前针对自动驾驶车辆摄像头感知任务的对抗性攻击已有研究，但针对交通灯检测的攻击较少，本文填补了这一空白。

Method: 提出了一种威胁模型，通过在交通灯下方放置打印补丁进行攻击，并设计了一种训练策略。

Result: 实验成功实现了通用的对抗性补丁攻击，包括红绿灯标签翻转攻击和图形分类攻击，并在真实场景中验证了攻击效果。

Conclusion: 该研究证明了对抗性补丁攻击在交通灯检测中的有效性，为相关防御提供了参考。
...
### [325] [DualX-VSR: Dual Axial Spatial$\times$Temporal Transformer for Real-World Video Super-Resolution without Motion Compensation](https://arxiv.org/abs/2506.04830)
*Shuo Cao,Yihao Liu,Xiaohui Li. Yuanting Gao. Yu Zhou,Chao Dong*

Main category: cs.CV

TLDR: DualX-VSR提出了一种新颖的双轴时空注意力机制，解决了现有Transformer模型在视频超分辨率任务中的局限性，无需运动补偿即可实现高性能。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer模型在视频超分辨率任务中因像素级精度要求和依赖光学流对齐而表现不佳，DualX-VSR旨在解决这些问题。

Method: 采用双轴时空注意力机制，沿正交方向整合时空信息，简化结构并消除对运动补偿的依赖。
...
### [326] [OpenMaskDINO3D : Reasoning 3D Segmentation via Large Language Model](https://arxiv.org/abs/2506.04837)
*Kunshen Zhang*

Main category: cs.CV

TLDR: OpenMaskDINO3D是一个基于LLM的3D理解和分割模型，通过处理点云数据和文本提示生成实例分割掩码，填补了3D推理分割的空白。


<details>
  <summary>Details</summary>
Motivation: 现有感知系统在2D推理分割方面成熟，但缺乏3D推理分割的框架。OpenMaskDINO3D旨在解决这一问题。

Method: 引入SEG标记和对象标识符，处理点云数据和文本提示，生成高精度3D分割掩码。
...
### [327] [Geological Field Restoration through the Lens of Image Inpainting](https://arxiv.org/abs/2506.04869)
*Vladislav Trifonov,Ivan Oseledets,Ekaterina Muravleva*

Main category: cs.CV

TLDR: 提出了一种基于多维张量完成和地统计学的地质场重建方法，显著优于普通克里金法。


<details>
  <summary>Details</summary>
Motivation: 从稀疏观测数据中重建多维地质场的需求，受确定性图像修复技术启发。

Method: 将部分观测的空间场建模为多维张量，并通过全局低秩结构恢复缺失值，结合张量完成和地统计学。

Result: 在合成地质场实验中，该方法在不同观测数据比例下均显著提高了重建精度。
...
### [328] [Invisible Backdoor Triggers in Image Editing Model via Deep Watermarking](https://arxiv.org/abs/2506.04879)
*Yu-Feng Chen,Tzuhsuan Huang,Pin-Yen Chiu,Jun-Cheng Chen*

Main category: cs.CV

TLDR: 该论文提出了一种新型的后门攻击框架，通过毒化训练数据将不可见触发器嵌入图像编辑过程，利用现有水印模型实现隐蔽攻击。
...
Conclusion: 该方法成功实现了隐蔽的后门攻击，为图像编辑领域的安全性研究提供了新思路。
...
### [329] [Learning to Plan via Supervised Contrastive Learning and Strategic Interpolation: A Chess Case Study](https://arxiv.org/abs/2506.04892)
*Andrew Hamara,Greg Hamerly,Pablo Rivas,Andrew C. Freeman*

Main category: cs.CV

TLDR: 论文提出了一种基于直觉驱动的规划方法，通过对比学习训练Transformer编码器，将棋盘状态嵌入到潜在空间中，实现无需深度搜索的走子选择，并在6-ply束搜索下达到2593 Elo评分。
...
Result: 模型在6-ply束搜索下达到2593 Elo评分，性能随模型规模和嵌入维度提升而改善。
...
### [330] [From Objects to Anywhere: A Holistic Benchmark for Multi-level Visual Grounding in 3D Scenes](https://arxiv.org/abs/2506.04897)
*Tianxu Wang,Zhuofan Zhang,Ziyu Zhu,Yue Fan,Jing Xiong,Pengxiang Li,Xiaojian Ma,Qing Li*

Main category: cs.CV

TLDR: Anywhere3D-Bench是一个全面的3D视觉定位基准，涵盖四种不同级别的定位任务，揭示了当前模型在空间和部分级别任务上的显著不足。


<details>
  <summary>Details</summary>
Motivation: 探索3D场景中超越对象级别的视觉定位，填补现有研究的空白。

Method: 引入Anywhere3D-Bench基准，评估多种先进3D视觉定位方法及大语言模型。

Result: 空间和部分级别任务表现最差，最佳模型OpenAI o4-mini在空间级别任务上准确率仅为23.57%，部分级别为33.94%。

Conclusion: 当前模型在3D场景的非对象级别语义理解和推理能力存在明显不足。
...
### [331] [Generating Synthetic Stereo Datasets using 3D Gaussian Splatting and Expert Knowledge Transfer](https://arxiv.org/abs/2506.04908)
*Filip Slezak,Magnus K. Gjerde,Joakim B. Haurum,Ivan Nikolov,Morten S. Laursen,Thomas B. Moeslund*

Main category: cs.CV

TLDR: 提出了一种基于3D高斯泼溅（3DGS）的立体数据集生成方法，替代NeRF方法，结合几何重建和FoundationStereo深度估计，在零样本泛化基准中表现优异。


<details>
  <summary>Details</summary>
Motivation: 探索高效、低成本的立体数据集生成方法，以替代NeRF，并提升深度立体模型的微调速度和性能。
...
Conclusion: 3DGS方法展示了低成本、高保真数据集生成的潜力，但需进一步探索其在复杂场景中的鲁棒性。
...
### [332] [Light and 3D: a methodological exploration of digitisation techniques adapted to a selection of objects from the Mus{é}e d'Arch{é}ologie Nationale](https://arxiv.org/abs/2506.04925)
...
TLDR: 本文探讨了文化遗产数字化中3D摄影方法的多样性，强调应根据对象特性和未来用途选择合适方法，而非追求绝对分类。


<details>
  <summary>Details</summary>
Motivation: 文化遗产数字化需求广泛，但现有方法并非适用于所有对象，需根据具体情况调整工具。

Method: 通过法国国家考古博物馆的藏品案例，展示不同3D摄影方法的适用性，并强调跨领域合作选择方法的重要性。

Result: 研究发现，没有单一方法适用于所有对象，需根据对象特性和数字孪生的未来用途定制工具。

Conclusion: 文化遗产数字化应避免绝对分类，而是通过跨领域合作选择最适合的工具。
...
### [333] [CzechLynx: A Dataset for Individual Identification and Pose Estimation of the Eurasian Lynx](https://arxiv.org/abs/2506.04931)
...
<details>
  <summary>Details</summary>
Motivation: 为欧亚猞猁的个体识别、姿态估计和分割任务提供首个大规模数据集，支持模型开发和基准测试。

Method: 收集了30k真实相机陷阱图像并标注，同时生成了100k合成图像，通过Unity和扩散模型增加数据多样性。

Result: 数据集包含219个独特个体，覆盖15年监测数据，并提供了三种评估协议以测试泛化能力。

Conclusion: CzechLynx将有助于推动动物个体识别及相关任务的模型开发和评估。
...
### [334] [Time-Lapse Video-Based Embryo Grading via Complementary Spatial-Temporal Pattern Mining](https://arxiv.org/abs/2506.04950)
*Yong Sun,Yipeng Wang,Junyu Shi,Zhiyuan Zhang,Yanmei Xiao,Lei Zhu,Manxi Jiang,Qiang Nie*

Main category: cs.CV

TLDR: 提出了一种基于视频的胚胎分级任务，利用全时长延时监测视频预测胚胎质量，并设计了互补时空模式挖掘框架（CoSTeM）。


<details>
  <summary>Details</summary>
Motivation: 现有方法在胚胎评估中缺乏整体质量评估或受胚胎外因素干扰，限制了临床实用性。

Method: 提出了CoSTeM框架，包含形态学和形态动力学两个分支，分别提取局部结构特征和全局发育轨迹。

Result: 实验结果表明该设计优越，为AI辅助胚胎选择提供了方法论框架。

Conclusion: 该工作为胚胎选择提供了新方法，数据集和代码将公开。
...
### [335] [Robustness as Architecture: Designing IQA Models to Withstand Adversarial Perturbations](https://arxiv.org/abs/2506.04951)
*Igor Meleshin,Anna Chistyakova,Anastasia Antsiferova,Dmitriy Vatolin*

Main category: cs.CV

TLDR: 论文提出了一种新的视角，将鲁棒性作为架构先验，通过设计而非训练来提升图像质量评估模型的抗攻击能力。


<details>
  <summary>Details</summary>
Motivation: 传统的数据驱动防御方法（如对抗训练）存在局限性，论文探讨是否可以通过设计模型结构本身来实现鲁棒性。

Method: 通过强制正交信息流、约束网络为范数保持操作，并结合剪枝和微调，从底层设计上抑制敏感性。

Result: 提出了一种鲁棒的IQA架构，无需对抗训练即可抵御攻击，且对原模型改动较小。

Conclusion: 论文倡导从数据优化鲁棒性转向通过设计实现鲁棒性的新思路。
...
### [336] [APVR: Hour-Level Long Video Understanding with Adaptive Pivot Visual Information Retrieval](https://arxiv.org/abs/2506.04953)
*Hong Gao,Yiming Bao,Xuezhan Tu,Bin Zhong,Minling Zhang*

Main category: cs.CV

TLDR: APVR框架通过分层视觉信息检索解决长视频理解问题，无需训练，显著提升性能。
...
Conclusion: APVR为训练无关框架，可即插即用，显著提升长视频理解能力。
...
### [337] [FEAT: Full-Dimensional Efficient Attention Transformer for Medical Video Generation](https://arxiv.org/abs/2506.04956)
*Huihan Wang,Zhiwen Yang,Hui Zhang,Dan Zhao,Bingzheng Wei,Yan Xu*

Main category: cs.CV

TLDR: FEAT是一种高效的全维度注意力Transformer，通过空间-时间-通道注意力机制、线性复杂度设计和残差值引导模块，解决了动态医学视频合成中的关键问题，性能优于现有方法。
...
### [338] [Bringing SAM to new heights: Leveraging elevation data for tree crown segmentation from drone imagery](https://arxiv.org/abs/2506.04970)
*Mélisande Teng,Arthur Ouaknine,Etienne Laliberté,Yoshua Bengio,David Rolnick,Hugo Larochelle*

Main category: cs.CV

TLDR: 论文比较了利用Segment Anything Model (SAM)进行无人机图像中树冠实例分割的方法，并提出了结合高程数据的BalSAM模型，在特定场景下表现优于其他方法。
...
Method: 比较了SAM在三种森林类型中的树冠分割效果，并研究了结合数字表面模型(DSM)数据的方法，提出了BalSAM模型。

Result: SAM直接使用效果不如定制Mask R-CNN，但通过端到端调优和结合DSM数据，BalSAM在种植园场景中表现更优。

Conclusion: 高效调优SAM和结合DSM数据是提升树冠分割模型性能的有效途径。
...
### [339] [TextVidBench: A Benchmark for Long Video Scene Text Understanding](https://arxiv.org/abs/2506.04983)
*Yangyang Zhong,Ji Qi,Yuan Yao,Pengxin Luo,Yunfeng Yan,Donglian Qi,Zhiyuan Liu,Tat-Seng Chua*

Main category: cs.CV

TLDR: 论文提出了TextVidBench，首个专注于长视频文本问答的基准测试，解决了现有数据集视频时长短、评估范围窄的问题，并通过新方法和实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有短视频文本视觉问答（ViteVQA）任务的数据集视频时长有限、评估范围窄，难以充分评估多模态大语言模型（MLLMs）的能力。

Method: 1) 构建TextVidBench基准，覆盖9个领域的长视频（平均2306秒）；2) 提出三阶段评估框架；3) 引入IT-Rope机制、非均匀位置编码和轻量微调方法。

Result: TextVidBench对现有模型提出了显著挑战，提出的方法在多个公开数据集上验证了其有效性。

Conclusion: TextVidBench为长视频文本理解提供了更真实的评估基准，提出的方法为提升长视频场景文本理解能力提供了新思路。
...
### [340] [Multi-scale Image Super Resolution with a Single Auto-Regressive Model](https://arxiv.org/abs/2506.04990)
*Enrique Sanchez,Isma Hadji,Adrian Bulat,Christos Tzelepis,Brais Martinez,Georgios Tzimiropoulos*

Main category: cs.CV

TLDR: 论文提出了一种基于视觉自回归（VAR）模型的图像超分辨率（ISR）方法，通过多尺度图像标记化和偏好优化改进现有技术，实现了单次前向传递的超分辨率，并在小模型和无需外部数据的情况下达到SOTA效果。


<details>
  <summary>Details</summary>
Motivation: 现有VARSR方法存在固定分辨率限制和大模型依赖问题，作者希望通过改进多尺度表示和优化策略解决这些问题。
...
Conclusion: 通过多尺度标记化和偏好优化，论文成功改进了VAR在ISR中的应用，展示了高效且轻量化的解决方案。
...
### [341] [PATS: Proficiency-Aware Temporal Sampling for Multi-View Sports Skill Assessment](https://arxiv.org/abs/2506.04996)
*Edoardo Bianchi,Antonio Liotta*

Main category: cs.CV

TLDR: PATS是一种新型采样策略，通过保留完整运动模式提升运动技能评估的准确性。
...
Method: PATS自适应分割视频，确保每个分析部分包含完整的关键运动执行，并在多个片段中重复此过程。

Result: 在EgoExo4D基准测试中，PATS在所有视角配置下均优于现有方法，且在挑战性领域表现突出。
...
### [342] [Beyond Cropped Regions: New Benchmark and Corresponding Baseline for Chinese Scene Text Retrieval in Diverse Layouts](https://arxiv.org/abs/2506.04999)
*Gengluo Li,Huawen Shen,Yu Zhou*

Main category: cs.CV

TLDR: 论文提出了一种针对中文场景文本检索的新模型CSTR-CLIP，通过结合全局视觉信息和多粒度对齐训练，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理中文复杂多样的文本布局时表现不佳，亟需一种专门针对中文场景的解决方案。

Method: 提出CSTR-CLIP模型，采用两阶段训练，结合全局视觉信息和多粒度对齐。

Result: 在现有基准测试中，CSTR-CLIP比之前最优模型准确率提升18.82%，且推理速度更快。

Conclusion: CSTR-CLIP能有效处理多样化的中文文本布局，为中文场景文本检索提供了新思路。
...
### [343] [Structure-Aware Radar-Camera Depth Estimation](https://arxiv.org/abs/2506.05008)
*Fuyi Zhang,Zhu Yu,Chunhao Li,Runmin Zhang,Xiaokai Bai,Zili Zhou,Si-Yuan Cao,Wang Wang,Hui-Liang Shen*

Main category: cs.CV

TLDR: 论文探讨了单目深度估计的进展，重点介绍了深度学习在该领域的应用及其挑战，特别是泛化到未见域的问题。
...
Method: 通过多尺度融合网络、分类任务重新解释、引入额外先验和优化目标函数等方法改进深度估计。

Result: Depth Anything在零样本单目深度估计中表现领先，但缺乏显式深度线索导致度量深度估计不准确。

Conclusion: 尽管单目深度估计取得进展，泛化问题仍需解决，未来需进一步优化方法。
...
### [344] [Point Cloud Segmentation of Agricultural Vehicles using 3D Gaussian Splatting](https://arxiv.org/abs/2506.05009)
...
TLDR: 提出了一种利用3D高斯泼溅和高斯不透明度场生成合成数据的管道，用于训练3D点云语义分割模型，结果显示合成数据训练的模型性能优异。


<details>
  <summary>Details</summary>
Motivation: 获取和标注真实点云数据成本高且耗时，因此需要一种高效生成合成数据的方法。

Method: 结合3D高斯泼溅和高斯不透明度场生成农业车辆的3D资产，并在模拟环境中使用LiDAR生成点云数据。

Result: 合成数据训练的模型（如PTv3）在未接触真实数据的情况下达到91.35%的mIoU，某些情况下甚至优于真实数据训练的模型。

Conclusion: 合成数据能有效替代真实数据，且模型具备跨语义类别的泛化能力。
...
### [345] [UAV4D: Dynamic Neural Rendering of Human-Centric UAV Imagery using Gaussian Splatting](https://arxiv.org/abs/2506.05011)
*Jaehoon Choi,Dongki Jung,Christopher Maxey,Yonghan Lee,Sungmin Eum,Dinesh Manocha,Heesung Kwon*

Main category: cs.CV

TLDR: UAV4D框架解决了无人机单目视频动态场景渲染的挑战，通过结合3D基础模型和人体网格重建模型，实现了多行人动态场景的高质量渲染。


<details>
  <summary>Details</summary>
Motivation: 现有动态神经渲染方法无法处理无人机单目视频中的独特挑战，如俯视角、多小移动行人等，且缺乏相关数据集。

Method: 结合3D基础模型和人体网格重建模型，利用SMPL模型和背景网格初始化高斯样条，解决场景尺度模糊问题。

Result: 在三个复杂无人机数据集上测试，PSNR提升1.5 dB，视觉清晰度优于现有方法。

Conclusion: UAV4D框架在多行人动态场景渲染中表现出色，为无人机视频提供了高质量的解决方案。
...
### [346] [Physical Annotation for Automated Optical Inspection: A Concept for In-Situ, Pointer-Based Trainingdata Generation](https://arxiv.org/abs/2506.05026)
*Oliver Krumpek,Oliver Heimann,Jörg Krüger*

Main category: cs.CV

TLDR: 本文提出了一种新型物理标注系统，用于生成自动化光学检测的训练数据。系统通过指针式原位交互，将专业检测人员的经验直接转化为机器学习训练数据，比传统屏幕标注更直观高效。


<details>
  <summary>Details</summary>
Motivation: 传统屏幕标注方法无法直接捕捉物理对象的轨迹和轮廓，导致数据标注效率低下且不够直观。本文旨在通过物理标注系统解决这一问题，提升数据标注的效率和准确性。

Method: 系统采用校准的追踪指针记录用户输入，并将空间交互转化为标准化的标注格式，同时通过投影界面提供视觉引导。

Result: 初步评估证实系统能有效捕捉详细标注轨迹，并与CVAT集成优化后续机器学习任务的工作流程。

Conclusion: 该系统填补了人类专业经验与自动化数据生成之间的鸿沟，为非IT专家参与机器学习训练提供了可能，并避免了有价值训练样本的丢失。
...
### [347] [FlowDirector: Training-Free Flow Steering for Precise Text-to-Video Editing](https://arxiv.org/abs/2506.05046)
...
Method: 利用ODE引导视频在数据空间中演化，引入注意力掩码机制和增强编辑策略。
...
### [348] [A Survey on Vietnamese Document Analysis and Recognition: Challenges and Future Directions](https://arxiv.org/abs/2506.05061)
*Anh Le,Thanh Lam,Dung Nguyen*

Main category: cs.CV

TLDR: 本文综述了越南文档分析与识别（DAR）的现状，探讨了传统OCR和深度学习方法在越南语文本识别中的局限性，并提出了利用大语言模型（LLMs）和视觉语言模型改进的方向。


<details>
  <summary>Details</summary>
Motivation: 越南语文本识别因复杂的变音符号、声调变化和缺乏大规模标注数据集而面临挑战，传统方法难以应对现实文档的多样性。

Method: 综述了现有技术，包括OCR、深度学习和LLMs的应用，并分析了其优缺点。

Result: LLMs和视觉语言模型在文本识别和文档理解方面表现出潜力，但仍需解决领域适应、多模态学习和计算效率等问题。

Conclusion: 未来研究方向包括数据集开发、模型优化和多模态方法整合，以推动越南DAR的发展。
...
### [349] [SeedEdit 3.0: Fast and High-Quality Generative Image Editing](https://arxiv.org/abs/2506.05083)
*Peng Wang,Yichun Shi,Xiaochen Lian,Zhonghua Zhai,Xin Xia,Xuefeng Xiao,Weilin Huang,Jianchao Yang*

Main category: cs.CV

TLDR: SeedEdit 3.0与Seedream 3.0结合，显著提升了编辑指令跟随和图像内容保留能力，并通过数据增强和联合学习管道进一步优化。


<details>
  <summary>Details</summary>
Motivation: 改进现有T2I模型在真实图像输入上的编辑能力和内容保留效果。

Method: 1. 增强数据整理管道和元信息嵌入策略；2. 引入联合学习管道计算扩散损失和奖励损失。

Result: 在真实图像编辑测试中，SeedEdit 3.0实现了56.1%的高可用率，优于其他版本和竞品。

Conclusion: SeedEdit 3.0在编辑能力和内容保留方面取得了显著进步，提供了更优的平衡。
...
### [350] [Interpretable Multimodal Framework for Human-Centered Street Assessment: Integrating Visual-Language Models for Perceptual Urban Diagnostics](https://arxiv.org/abs/2506.05087)
*HaoTian Lan*

Main category: cs.CV

TLDR: 该研究提出了一种多模态街道评估框架（MSEF），结合视觉和语言模型，用于评估街道景观的客观和主观感知，并在中国哈尔滨的街景数据上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有基于图像或GIS的街道指标无法充分捕捉主观感知，而主观感知对包容性城市设计至关重要。

Method: 研究采用视觉变换器（VisualGLM-6B）和大型语言模型（GPT-4）结合，通过LoRA和P-Tuning v2进行参数高效微调，构建了MSEF框架。

Result: 模型在客观特征上F1得分为0.84，与居民感知的一致性达89.3%，并揭示了上下文依赖的矛盾和非线性模式。

Conclusion: MSEF为城市感知建模提供了方法创新，并为规划系统提供了实用工具，以平衡基础设施精确性与居民生活体验。
...
### [351] [FG 2025 TrustFAA: the First Workshop on Towards Trustworthy Facial Affect Analysis: Advancing Insights of Fairness, Explainability, and Safety (TrustFAA)](https://arxiv.org/abs/2506.05095)
*Jiaee Cheong,Yang Liu,Harold Soh,Hatice Gunes*

Main category: cs.CV

TLDR: 研讨会的目标是探讨情感AI面部情感分析（FAA）工具的可信度问题，包括公平性、可解释性和安全性。
...
Result: 促进对可信FAA的研究和讨论，支持FG2025的伦理目标。

Conclusion: 研讨会旨在推动FAA领域的可信度研究，符合伦理要求。
...
### [352] [Astraea: A GPU-Oriented Token-wise Acceleration Framework for Video Diffusion Transformers](https://arxiv.org/abs/2506.05096)
*Haosong Liu,Yuge Cheng,Zihan Liu,Aiyue Chen,Yiwu Yao,Chen Chen,Jingwen Leng,Yu Feng,Minyi Guo*

Main category: cs.CV

TLDR: ASTRAEA是一个自动优化视频扩散变压器（vDiT）配置的框架，通过轻量级令牌选择和高效稀疏注意力策略，显著提升推理速度，同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有的视频扩散变压器（vDiT）在文本到视频生成中表现出色，但计算需求高，现有加速方法依赖启发式规则，适用性有限。

Method: ASTRAEA提出轻量级令牌选择机制和内存高效的稀疏注意力策略，并结合进化算法自动优化令牌预算分配。

Result: 在单GPU上实现2.4倍推理加速（8GPU可达13.2倍），视频质量损失极小（VBench评分损失<0.5%）。

Conclusion: ASTRAEA在保持高质量的同时显著提升vDiT的推理效率，具有广泛适用性。
...
### [353] [DIMCIM: A Quantitative Evaluation Framework for Default-mode Diversity and Generalization in Text-to-Image Generative Models](https://arxiv.org/abs/2506.05108)
*Revant Teotia,Candace Ross,Karen Ullrich,Sumit Chopra,Adriana Romero-Soriano,Melissa Hall,Matthew J. Muckley*

Main category: cs.CV

TLDR: 论文提出DIM-CIM框架，用于无参考评估文本到图像模型的多样性和泛化能力，发现模型规模扩大可能牺牲默认模式多样性。
...
Method: 引入DIM-CIM框架，构建COCO-DIMCIM基准，评估默认模式多样性和泛化能力。

Result: 模型规模扩大（1.5B到8.1B参数）可能提高泛化能力但牺牲默认模式多样性；训练图像多样性与默认模式多样性相关性为0.85。

Conclusion: DIM-CIM提供灵活、可解释的框架，全面评估文本到图像模型性能。
...
### [354] [Practical Manipulation Model for Robust Deepfake Detection](https://arxiv.org/abs/2506.05119)
...
<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测模型在非理想条件下性能不稳定，容易被规避，需要更实用的方法来应对现实中的伪造多样性。

Method: 开发了PMM模型，采用Poisson混合、多样化掩码、生成器伪影和干扰物扩展伪造空间，并通过强退化训练图像提升检测器的泛化能力。

Result: 在DFDC和DFDCP数据集上，AUC分别提高了3.51%和6.21%，显著提升了模型对常见图像退化的鲁棒性。

Conclusion: PMM模型有效解决了现有检测器的鲁棒性问题，并在标准基准数据集上表现出显著性能提升。
...
### [355] [CIVET: Systematic Evaluation of Understanding in VLMs](https://arxiv.org/abs/2506.05146)
...
Result: VLMs仅能识别有限基本属性，性能受对象位置影响，且难以理解对象间关系，未达人类水平。

Conclusion: 当前VLMs在场景理解上仍有不足，需进一步改进以接近人类能力。
...
### [356] [FRED: The Florence RGB-Event Drone Dataset](https://arxiv.org/abs/2506.05163)
*Gabriele Magrini,Niccolò Marini,Federico Becattini,Lorenzo Berlincioni,Niccolò Biondi,Pietro Pala,Alberto Del Bimbo*

Main category: cs.CV

TLDR: 论文介绍了Florence RGB-Event Drone数据集（FRED），用于无人机检测、跟踪和轨迹预测，结合RGB视频和事件流，填补了现有基准的不足。


<details>
  <summary>Details</summary>
Motivation: 传统RGB相机在捕捉快速移动的无人机时存在局限性，尤其是在复杂光照条件下。事件相机具有高时间分辨率和动态范围，但现有基准缺乏精细时间分辨率或无人机特定运动模式。
...
### [357] [Through-the-Wall Radar Human Activity Recognition WITHOUT Using Neural Networks](https://arxiv.org/abs/2506.05169)
*Weicheng Gao*

Main category: cs.CV

TLDR: 论文提出了一种不依赖神经网络的穿墙雷达人体活动识别方法，通过微多普勒特征分割和拓扑相似性计算实现任务。


<details>
  <summary>Details</summary>
Motivation: 作者认为现有方法过度依赖神经网络训练，忽视了早期基于模板匹配的方法的物理可解释性和理论信号处理基础，试图回归原始路径。

Method: 生成雷达图像，使用角点检测确定目标区域，通过多相主动轮廓模型分割微多普勒特征，将其离散化为点云，并通过Mapper算法计算拓扑相似性。

Result: 数值模拟和实测实验验证了方法的有效性。

Conclusion: 该方法证明了不依赖神经网络也能实现智能识别，代码已开源。
...
### [358] [Track Any Anomalous Object: A Granular Video Anomaly Detection Pipeline](https://arxiv.org/abs/2506.05175)
...
TLDR: TAO框架通过像素级跟踪异常对象，统一检测多粒度异常，无需阈值调优，提升了视频异常检测的精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有视频异常检测方法多关注异常帧或对象，忽略像素级分析，限制了检测范围。

Method: 提出TAO框架，将异常检测转化为像素级跟踪问题，结合分割与跟踪任务，避免阈值调优。

Result: 实验表明TAO在精度和鲁棒性上达到新标杆。

Conclusion: TAO为视频异常检测提供了一种更精确、更通用的解决方案。
...
### [359] [Single GPU Task Adaptation of Pathology Foundation Models for Whole Slide Image Analysis](https://arxiv.org/abs/2506.05184)
...
TLDR: 本文提出了一种名为TAPFM的新方法，用于在单GPU上适应病理学基础模型（PFMs），通过视觉Transformer注意力进行多实例学习（MIL）聚合，优化特征表示和注意力权重。该方法在膀胱癌和肺腺癌的突变预测任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 适应预训练的PFMs用于特定临床任务面临挑战，主要由于WSI级标签较弱，需要MIL范式进行有效分析。

Method: 提出TAPFM方法，利用视觉Transformer注意力进行MIL聚合，同时优化特征表示和注意力权重，保持MIL聚合器和PFM的独立计算图以实现稳定训练。
...
### [360] [MokA: Multimodal Low-Rank Adaptation for MLLMs](https://arxiv.org/abs/2506.05191)
*Yake Wei,Yu Miao,Dongzhan Zhou,Di Hu*

Main category: cs.CV

TLDR: 当前高效多模态微调方法存在局限性，直接借鉴LLMs方法忽略了多模态特性。本文提出MokA方法，结合单模态和跨模态适配，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽视多模态场景特性，影响模态利用效率，需针对性解决方案。

Method: 提出MokA，通过模态特定参数压缩单模态信息，增强跨模态交互。

Result: 在多种多模态场景和LLM骨干上表现一致提升，验证了方法的有效性和通用性。

Conclusion: MokA为多模态高效适配提供了针对性方案，为未来探索铺路。
...
### [361] [Vision-Based Autonomous MM-Wave Reflector Using ArUco-Driven Angle-of-Arrival Estimation](https://arxiv.org/abs/2506.05195)
...
<details>
  <summary>Details</summary>
Motivation: 解决非视距条件下毫米波通信不可靠的问题，尤其是在城市或基础设施有限的环境中。

Method: 利用单目摄像头检测ArUco标记，实时估计到达角度并调整反射板位置，优化信号重定向。

Result: 实验显示，在60 GHz频段下，接收信号强度平均提升23 dB，且信号接收概率达0.89（阈值-65 dB）。
...
### [362] [Quantifying Cross-Modality Memorization in Vision-Language Models](https://arxiv.org/abs/2506.05198)
*Yuxin Wen,Yangsibo Huang,Tom Goldstein,Ravi Kumar,Badih Ghazi,Chiyuan Zhang*

Main category: cs.CV

TLDR: 研究探讨了多模态模型中跨模态记忆的特性，通过合成数据集量化了知识记忆和跨模态迁移能力，发现源模态和目标模态之间存在显著差距，并提出了一种缓解方法。


<details>
  <summary>Details</summary>
Motivation: 理解神经网络在训练中的记忆行为对防止敏感信息泄露和提升知识获取效率至关重要，尤其是在多模态模型日益普及的背景下。

Method: 引入合成人物数据集，通过单模态训练和跨模态评估，量化知识记忆和迁移能力，并分析不同场景下的表现。

Result: 发现跨模态记忆存在显著差距，且在多种场景下普遍存在；提出了一种缓解方法。

Conclusion: 研究为开发更鲁棒的多模态学习技术提供了启示，未来可进一步优化跨模态迁移能力。
...
### [363] [Grounding Beyond Detection: Enhancing Contextual Understanding in Embodied 3D Grounding](https://arxiv.org/abs/2506.05199)
*Yani Zhang,Dongming Wu,Hao Shi,Yingfei Liu,Tiancai Wang,Haoqiang Fan,Xingping Dong*

Main category: cs.CV

TLDR: DEGround通过共享DETR查询作为检测和定位的对象表示，提升了3D定位性能，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 研究3D定位是否从检测中受益足够，发现检测模型未经语言指令训练即可超越定位模型，表明当前方法存在问题。

Method: 提出DEGround框架，共享DETR查询，引入区域激活定位模块和查询调制模块，增强语言指令理解。

Result: 在EmbodiedScan验证集上，DEGround比BIP3D总体准确率高7.52%。

Conclusion: DEGround通过结合检测和定位任务，显著提升了3D定位性能，为细粒度上下文感知定位提供了新思路。
...
### [364] [OGGSplat: Open Gaussian Growing for Generalizable Reconstruction with Expanded Field-of-View](https://arxiv.org/abs/2506.05204)
...
<details>
  <summary>Details</summary>
Motivation: 虚拟现实和具身AI等新兴应用需要从稀疏视图高效重建语义一致的3D场景，但现有方法在视野外区域表现不佳且计算成本高。

Method: 通过开放高斯语义属性提供先验，结合RGB-语义一致性修复模块和双向控制扩散模型，逐步优化高斯参数。
...
### [365] [Follow-Your-Motion: Video Motion Transfer via Efficient Spatial-Temporal Decoupled Finetuning](https://arxiv.org/abs/2506.05207)
*Yue Ma,Yulong Liu,Qiyuan Zhu,Ayden Yang,Kunyu Feng,Xinhua Zhang,Zhifeng Li,Sirui Han,Chenyang Qi,Qifeng Chen*

Main category: cs.CV

TLDR: 提出了一种名为Follow-Your-Motion的高效两阶段视频运动迁移框架，解决了现有方法在运动一致性和调优效率上的问题。
...
Method: 提出空间-时间解耦的LoRA，稀疏运动采样和自适应RoPE，以加速调优速度。

Result: 在MotionBench上进行了广泛评估，验证了方法的优越性。

Conclusion: Follow-Your-Motion在复杂运动生成中表现出色，并提供了新的基准测试集。
...
### [366] [Towards Vision-Language-Garment Models For Web Knowledge Garment Understanding and Generation](https://arxiv.org/abs/2506.05210)
*Jan Ackermann,Kiyohiro Nakayama,Guandao Yang,Tong Wu,Gordon Wetzstein*

Main category: cs.CV

TLDR: VLG模型通过视觉-语言-服装多模态合成服装，初步实验显示其在零样本泛化中表现良好。
...
### [367] [DSG-World: Learning a 3D Gaussian World Model from Dual State Videos](https://arxiv.org/abs/2506.05217)
*Wenhao Hu,Xuexiang Wen,Xi Li,Gaoang Wang*

Main category: cs.CV

TLDR: DSG-World提出了一种基于双状态观测的3D高斯世界建模框架，通过互补可见性解决遮挡问题，实现高效且物理一致的3D重建。


<details>
  <summary>Details</summary>
Motivation: 现有世界建模方法存在训练困难、缺乏3D或物理一致性等问题，而显式3D方法需要多阶段处理。DSG-World旨在通过双状态观测解决这些问题。

Method: 利用双状态观测构建双分割感知高斯场，强制双向光度和语义一致性，并引入伪中间状态进行对称对齐和协同修剪策略。
...
### [368] [MonkeyOCR: Document Parsing with a Structure-Recognition-Relation Triplet Paradigm](https://arxiv.org/abs/2506.05218)
*Zhang Li,Yuliang Liu,Qiang Liu,Zhiyin Ma,Ziyang Zhang,Shuo Zhang,Zidun Guo,Jiarui Zhang,Xinyu Wang,Xiang Bai*

Main category: cs.CV

TLDR: MonkeyOCR是一种基于SRR三元范式的视觉语言模型，用于文档解析，简化了复杂的多工具流程，并在效率和精度上取得平衡。


<details>
  <summary>Details</summary>
Motivation: 解决现有文档解析方法中复杂多工具流程和低效的全页处理问题。

Method: 采用SRR（结构-识别-关系）三元范式，将文档解析分解为布局分析、内容识别和逻辑排序三个问题。

Result: 在MonkeyDoc数据集上表现优异，超越现有方法，尤其在公式和表格处理上提升显著，且推理速度快。

Conclusion: MonkeyOCR在文档解析任务上实现了高效、精确的SOTA性能，适合实际部署。
...
### [369] [SAM-aware Test-time Adaptation for Universal Medical Image Segmentation](https://arxiv.org/abs/2506.05221)
*Jianghao Wu,Yicheng Wu,Yutong Xie,Wenjia Bai,You Zhang,Feilong Tang,Yulong Li,Yasmeen George,Imran Razzak*

Main category: cs.CV

TLDR: 提出SAM-TTA方法，通过测试时适应框架提升SAM在医学图像分割中的性能，同时保持其泛化能力。
...
Method: 结合SBCT（自适应贝塞尔曲线变换）和DUMT（双尺度不确定性驱动的均值教师适应），分别解决输入级和语义级差异。
...
### [370] [Spatiotemporal Contrastive Learning for Cross-View Video Localization in Unstructured Off-road Terrains](https://arxiv.org/abs/2506.05250)
*Zhiyun Deng,Dongmyeong Lee,Amanda Adkins,Jesse Quattrociocchi,Christian Ellis,Joydeep Biswas*

Main category: cs.CV

TLDR: MoViX是一个自监督的跨视图视频定位框架，用于解决GPS缺失、越野环境中的3-DoF定位问题，通过学习视角和季节不变的表示，并结合运动信息和时间对齐策略，显著提升了定位精度。


<details>
  <summary>Details</summary>
Motivation: 在GPS缺失的越野环境中，重复植被、非结构化地形和季节性变化导致视觉定位困难，传统方法难以对齐过时的卫星图像。

Method: MoViX采用自监督学习，结合姿态依赖的正样本采样、时间对齐的负样本挖掘、运动信息帧采样和轻量级时间聚合器，并在蒙特卡洛定位框架中运行。

Result: 在TartanDrive 2.0数据集上，MoViX在12.29公里的测试中，93%的时间内定位误差小于25米，100%的时间内小于50米，优于现有方法。

Conclusion: MoViX在无需环境特定调优的情况下，实现了高精度的跨视图定位，并在不同地理区域的真实越野数据上展示了泛化能力。
...
### [371] [LeanPO: Lean Preference Optimization for Likelihood Alignment in Video-LLMs](https://arxiv.org/abs/2506.05260)
*Xiaodong Wang,Jinfa Huang,Li Yuan,Peixi Peng*

Main category: cs.CV

TLDR: 论文提出LeanPO方法，通过重新定义奖励和动态标签平滑策略，解决Video-LLMs中偏好对齐技术导致的非目标响应概率上升问题。
...
Result: 实验表明LeanPO显著提升了Video-LLMs的性能，且训练开销小。
...
### [372] [Can Foundation Models Generalise the Presentation Attack Detection Capabilities on ID Cards?](https://arxiv.org/abs/2506.05263)
...
<details>
  <summary>Details</summary>
Motivation: 当前PAD系统因隐私保护问题通常仅针对少数国家身份证训练，导致在新国家测试时表现不佳。FM因其大数据训练特性有望解决这一问题。

Method: 研究采用零样本学习和微调两种方法，并使用两个数据集（智利私有数据集和芬兰、西班牙、斯洛伐克的开放数据集）进行测试。

Result: 研究发现真实图像（bona fide）是提升泛化能力的关键。

Conclusion: 基础模型在PAD任务中具有潜力，尤其是通过真实图像优化泛化能力。
...
### [373] [From Play to Replay: Composed Video Retrieval for Temporally Fine-Grained Videos](https://arxiv.org/abs/2506.05274)
*Animesh Gupta,Jay Parmar,Ishan Rajendrakumar Dave,Mubarak Shah*

Main category: cs.CV

TLDR: TF-CoVR是首个专注于时间细粒度组合视频检索的大规模基准，提出了TF-CoVR-Base框架，显著提升了检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有组合视频检索基准未能测试对细微、快速时间差异的捕捉能力，因此需要专门针对时间细粒度的新基准。

Method: 提出TF-CoVR基准，基于FineGym和FineDiving构建180K三元组；设计TF-CoVR-Base框架，分两阶段训练：预训练视频编码器并进行对比学习对齐。

Result: TF-CoVR-Base在零样本和微调场景下分别将mAP@50从5.92提升至7.51和19.83提升至25.82。

Conclusion: TF-CoVR填补了时间细粒度组合视频检索的空白，TF-CoVR-Base框架显著提升了性能，为实际任务（如体育集锦生成）提供了有效工具。
...
### [374] [Unifying Appearance Codes and Bilateral Grids for Driving Scene Gaussian Splatting](https://arxiv.org/abs/2506.05280)
...
TLDR: 提出了一种多尺度双边网格方法，结合了外观编码和双边网格，显著提升了动态自动驾驶场景重建的几何精度。


<details>
  <summary>Details</summary>
Motivation: 现实场景中难以保证完美的光度一致性，现有方法（如外观编码和双边网格）存在建模能力有限或优化困难的问题。

Method: 提出多尺度双边网格，统一外观编码和双边网格，有效减少光度不一致导致的几何误差。

Result: 在Waymo、NuScenes等四个数据集上表现优异，几何精度显著提升。

Conclusion: 多尺度双边网格方法在自动驾驶场景重建中具有重要价值，尤其在几何精度方面优于现有技术。
...
### [375] [Rectified Point Flow: Generic Point Cloud Pose Estimation](https://arxiv.org/abs/2506.05282)
*Tao Sun,Liyuan Zhu,Shengyu Huang,Shuran Song,Iro Armeni*

Main category: cs.CV

TLDR: Rectified Point Flow 是一种统一的参数化方法，将点云配准和多部件形状组装建模为条件生成问题，通过学习连续点速度场实现目标位置对齐，无需对称标签即可学习对称性。
...
Method: 通过学习连续点速度场，将噪声点传输到目标位置，并结合自监督编码器处理重叠点。
...
### [376] [Video World Models with Long-term Spatial Memory](https://arxiv.org/abs/2506.05284)
*Tong Wu,Shuai Yang,Ryan Po,Yinghao Xu,Ziwei Liu,Dahua Lin,Gordon Wetzstein*

Main category: cs.CV

TLDR: 提出了一种基于几何空间记忆的框架，用于增强视频世界模型的长期一致性，解决了现有模型因时间窗口限制导致的场景遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型因时间上下文窗口有限，难以维持场景一致性，导致环境遗忘。受人类记忆机制启发，提出新框架。

Method: 引入基于几何的长期空间记忆机制，包括存储和检索信息的方法，并使用定制数据集训练和评估模型。

Result: 评估显示，模型在质量、一致性和上下文长度上优于基线方法。
...
### [377] [RaySt3R: Predicting Novel Depth Maps for Zero-Shot Object Completion](https://arxiv.org/abs/2506.05285)
*Bardienus P. Duisterhof,Jan Oberst,Bowen Wen,Stan Birchfield,Deva Ramanan,Jeffrey Ichnowski*

Main category: cs.CV

TLDR: RaySt3R将3D形状补全问题转化为新视角合成问题，通过单张RGB-D图像和多视角查询射线，预测深度图、物体掩码和置信度，实现高效且一致的3D重建。


<details>
  <summary>Details</summary>
Motivation: 现有3D形状补全方法缺乏一致性、计算成本高且难以捕捉清晰边界，RaySt3R旨在解决这些问题。

Method: 利用单张RGB-D图像和多视角查询射线，训练前馈Transformer预测深度图、物体掩码和置信度，并通过多视角融合完成3D重建。

Result: 在合成和真实数据集上，RaySt3R性能领先基线方法高达44%（3D chamfer距离）。

Conclusion: RaySt3R通过新视角合成方法实现了高效、一致的3D形状补全，显著优于现有方法。
...
### [378] [Stable Vision Concept Transformers for Medical Diagnosis](https://arxiv.org/abs/2506.05286)
*Lijie Hu,Songning Lai,Yuan Hua,Shu Yang,Jingfeng Zhang,Di Wang*

Main category: cs.CV

TLDR: 论文提出VCT和SVCT模型，解决现有概念瓶颈模型（CBMs）在医学领域中的效用和稳定性问题，同时保持可解释性。


<details>
  <summary>Details</summary>
Motivation: 医学领域对透明度的需求促使研究者探索可解释AI（XAI），但现有CBMs仅依赖概念特征，忽略了医学图像的内在特征，且对输入扰动敏感，限制了其应用。

Method: 提出VCT模型融合概念特征与图像特征；进一步提出SVCT模型，基于ViT架构，结合概念层和Denoised Diffusion Smoothing技术，提升稳定性和解释性。

Result: 在四个医学数据集上的实验表明，VCT和SVCT在保持准确性的同时具备可解释性，SVCT在扰动下仍能提供稳定的解释。

Conclusion: VCT和SVCT解决了CBMs的局限性，满足了医学领域对模型透明度和稳定性的需求。
...
### [379] [EOC-Bench: Can MLLMs Identify, Recall, and Forecast Objects in an Egocentric World?](https://arxiv.org/abs/2506.05287)
*Yuqian Yuan,Ronghao Dang,Long Li,Wentong Li,Dian Jiao,Xin Li,Deli Zhao,Fan Wang,Wenqiao Zhang,Jun Xiao,Yueting Zhuang*

Main category: cs.CV

TLDR: 论文介绍了EOC-Bench，一个用于评估动态第一人称场景中物体中心化认知的新基准，填补了现有基准在动态交互评估上的空白。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注静态场景，忽视了用户交互引起的动态变化，需要一种新的评估工具来推动多模态大语言模型（MLLMs）在动态环境中的物体认知能力。

Method: 开发了EOC-Bench，包含3,277个标注QA对，分为三类时间类别（过去、现在、未来），涵盖11个细粒度评估维度和3种视觉物体引用类型。采用混合格式的人机协作标注框架和多尺度时间准确性指标。

Result: 基于EOC-Bench对多种MLLMs进行了全面评估，证明了其在动态场景中的物体认知能力。
...
### [380] [AliTok: Towards Sequence Modeling Alignment between Tokenizer and Autoregressive Model](https://arxiv.org/abs/2506.05289)
*Pingyu Wu,Kai Zhu,Yu Liu,Longxiang Tang,Jian Yang,Yansong Peng,Wei Zhai,Yang Cao,Zheng-Jun Zha*

Main category: cs.CV

TLDR: 提出了一种新的Aligned Tokenizer (AliTok)，通过单向依赖关系优化自回归图像生成，显著提升了生成质量和速度。
...
Conclusion: AliTok在生成质量和效率上均优于现有方法，为自回归图像生成提供了更优的解决方案。
...
### [381] [SeedVR2: One-Step Video Restoration via Diffusion Adversarial Post-Training](https://arxiv.org/abs/2506.05301)
...
TLDR: SeedVR2是一种基于扩散的单步视频修复模型，通过对抗训练和动态窗口注意力机制，显著提升了高分辨率视频修复的效率和质量。


<details>
  <summary>Details</summary>
Motivation: 现有扩散视频修复方法计算成本高，且单步修复方法在高分辨率视频中表现不佳，SeedVR2旨在解决这些问题。

Method: 提出动态调整窗口大小的注意力机制和多种损失函数，包括特征匹配损失，以优化对抗训练。

Result: SeedVR2在单步修复中性能优于或接近现有方法。

Conclusion: SeedVR2通过改进模型架构和训练策略，实现了高效高质量的视频修复。
...
### [382] [Perceive Anything: Recognize, Explain, Caption, and Segment Anything in Images and Videos](https://arxiv.org/abs/2506.05302)
*Weifeng Lin,Xinyu Wei,Ruichuan An,Tianhe Ren,Tingwei Chen,Renrui Zhang,Ziyu Guo,Wentao Zhang,Lei Zhang,Hongsheng Li*

Main category: cs.CV

TLDR: PAM是一个高效的区域级视觉理解框架，结合了SAM 2和LLMs，实现对象分割与多样化语义输出。
...
Method: 通过Semantic Perceiver将SAM 2的视觉特征转化为多模态令牌，结合数据增强流程生成高质量数据集。
...
### [383] [Do It Yourself: Learning Semantic Correspondence from Pseudo-Labels](https://arxiv.org/abs/2506.05312)
*Olaf Dünkel,Thomas Wimmer,Christian Theobalt,Christian Rupprecht,Adam Kortylewski*

Main category: cs.CV

TLDR: 提出了一种通过3D感知伪标签改进语义匹配的方法，减少了特定数据集标注的需求，并在SPair-71k上实现了新的最佳性能。
...
Method: 训练适配器以利用3D感知链式伪标签、松弛循环一致性和3D球形原型映射约束来优化现成特征。

Result: 在SPair-71k上实现了超过4%的绝对增益，且比类似监督需求的方法高出7%。

Conclusion: 该方法通用性强，可轻松扩展到其他数据源。
...
### [384] [MARBLE: Material Recomposition and Blending in CLIP-Space](https://arxiv.org/abs/2506.05313)
*Ta-Ying Cheng,Prafull Sharma,Mark Boss,Varun Jampani*

Main category: cs.CV

TLDR: MARBLE是一种基于CLIP空间和预训练文本到图像模型的材料编辑方法，通过材料嵌入和方向控制实现材料混合和精细属性调整。


<details>
  <summary>Details</summary>
Motivation: 研究动机是改进基于示例图像的材料编辑方法，实现对材料属性的精细控制和多编辑功能。

Method: 方法包括在CLIP空间中寻找材料嵌入和方向，利用预训练模型控制材料属性，并通过浅层网络预测属性变化方向。

Result: 实验通过定性和定量分析验证了方法的有效性，支持单次前向传递的多编辑和绘画应用。

Conclusion: MARBLE在材料编辑中表现出色，能够实现精细控制和多功能应用。
...
### [385] [ProJo4D: Progressive Joint Optimization for Sparse-View Inverse Physics Estimation](https://arxiv.org/abs/2506.05317)
*Daniel Rho,Jun Myeong Choi,Biswadip Dey,Roni Sengupta*

Main category: cs.CV

TLDR: ProJo4D提出了一种渐进式联合优化框架，用于从稀疏多视角视频中估计物理参数，解决了现有方法在密集输入需求和误差累积上的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在稀疏多视角视频输入下表现不佳，且需要密集输入，限制了实际应用。

Method: 采用渐进式联合优化策略，逐步增加优化参数，最终实现几何、外观、物理状态和材料属性的联合优化。

Result: 在PAC-NeRF和Spring-Gaus数据集上，ProJo4D在4D未来状态预测、未来状态的新视角渲染和材料参数估计方面优于现有方法。

Conclusion: ProJo4D为物理基础的4D场景理解提供了有效解决方案。
...
### [386] [Does Your 3D Encoder Really Work? When Pretrain-SFT from 2D VLMs Meets 3D VLMs](https://arxiv.org/abs/2506.05318)
*Haoyuan Li,Yanpeng Zhou,Yufei Gao,Tao Tang,Jianhua Han,Yujie Yuan,Dave Zhenyu Chen,Jiawang Bian,Hang Xu,Xiaodan Liang*

Main category: cs.CV

TLDR: 论文分析了3D视觉语言模型（VLMs）的性能差距，发现3D场景中心模型依赖3D编码器不足，并提出新数据集以改进3D理解。


<details>
  <summary>Details</summary>
Motivation: 研究3D VLMs在3D问答等任务中的性能差距，探索其与2D VLMs的差异。

Method: 分类3D VLMs为3D对象中心、2D图像基础和3D场景中心方法，并分析其性能差异。

Result: 3D场景中心模型依赖3D编码器不足，预训练效果较差，且数据扩展收益有限。
...
### [387] [Revisiting Depth Representations for Feed-Forward 3D Gaussian Splatting](https://arxiv.org/abs/2506.05327)
*Duochao Shi,Weijie Wang,Donny Y. Chen,Zeyu Zhang,Jia-Wang Bian,Bohan Zhuang,Chunhua Shen*

Main category: cs.CV

TLDR: 论文提出了一种名为PM-Loss的正则化损失函数，用于改进基于深度图的3D高斯泼溅（3DGS）方法，解决了深度不连续性导致的点云稀疏问题。


<details>
  <summary>Details</summary>
Motivation: 深度图在3DGS中广泛使用，但在物体边界处的不连续性会导致点云稀疏或碎片化，影响渲染质量。

Method: 通过预训练的transformer预测点图（pointmap），并基于此设计PM-Loss正则化损失，提升深度图的几何平滑性。
...
### [388] [AV-Reasoner: Improving and Benchmarking Clue-Grounded Audio-Visual Counting for MLLMs](https://arxiv.org/abs/2506.05328)
*Lidong Lu,Guo Chen,Zhiqi Li,Yicheng Liu,Tong Lu*

Main category: cs.CV

TLDR: 论文提出了CG-AV-Counting基准和AV-Reasoner模型，用于提升视频计数任务的能力。


<details>
  <summary>Details</summary>
Motivation: 现有视频理解模型在计数任务上表现不佳，且基准测试存在局限性。

Method: 提出CG-AV-Counting基准和AV-Reasoner模型，采用GRPO和课程学习。

Result: AV-Reasoner在多个基准上达到最优，但语言空间推理在域外基准上无效。

Conclusion: 强化学习有效，但需进一步改进域外性能。
...
### [389] [MINT-CoT: Enabling Interleaved Visual Tokens in Mathematical Chain-of-Thought Reasoning](https://arxiv.org/abs/2506.05331)
*Xinyan Chen,Renrui Zhang,Dongzhi Jiang,Aojun Zhou,Shilin Yan,Weifeng Lin,Hongsheng Li*

Main category: cs.CV

TLDR: MINT-CoT提出了一种新的视觉推理方法，通过动态插入视觉标记到文本推理步骤中，解决了多模态数学问题中的视觉感知和修改依赖问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在多模态数学推理中存在视觉区域粗粒度、视觉编码器感知有限以及依赖外部视觉修改能力的局限性。
...
### [390] [Unleashing Hour-Scale Video Training for Long Video-Language Understanding](https://arxiv.org/abs/2506.05332)
*Jingyang Lin,Jialian Wu,Ximeng Sun,Ze Wang,Jiang Liu,Yusheng Su,Xiaodong Yu,Hao Chen,Jiebo Luo,Zicheng Liu,Emad Barsoum*

Main category: cs.CV

TLDR: VideoMarathon是一个大规模的长视频指令跟随数据集，包含9,700小时的长视频和3.3M高质量问答对，用于训练和评估Hour-LLaVA模型。


<details>
  <summary>Details</summary>
Motivation: 解决长视频标注数据稀缺问题，推动视频多模态模型在小时级视频理解上的发展。

Method: 提出VideoMarathon数据集，并基于此开发Hour-LLaVA模型，利用内存增强模块实现小时级视频训练和推理。

Result: Hour-LLaVA在多个长视频语言基准测试中表现最佳，验证了数据集和模型的有效性。

Conclusion: VideoMarathon和Hour-LLaVA填补了长视频多模态建模的空白，为未来研究提供了高质量资源和模型框架。
...
### [391] [VideoMolmo: Spatio-Temporal Grounding Meets Pointing](https://arxiv.org/abs/2506.05336)
*Ghazi Shazan Ahmad,Ahmed Heakl,Hanan Gani,Abdelrahman Shaker,Zhiqiang Shen,Ranjay Krishna,Fahad Shahbaz Khan,Salman Khan*

Main category: cs.CV

TLDR: VideoMolmo是一个多模态大模型，用于基于文本描述的细粒度时空定位，结合了LLM和时序模块，显著提升了时空一致性和推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有视频定位方法缺乏上下文理解和泛化能力，VideoMolmo旨在通过结合语言模型和时序模块解决这一问题。

Method: 采用Molmo架构，引入时序模块和双向点传播的时序掩码融合管道，分两步生成定位坐标和分割掩码。

Result: 在自建数据集和多个基准测试中，VideoMolmo显著提升了时空定位准确性和推理能力。

Conclusion: VideoMolmo通过结合LLM和时序模块，为复杂时空定位任务提供了高效且可解释的解决方案。
...
### [392] [Defurnishing with X-Ray Vision: Joint Removal of Furniture from Panoramas and Mesh](https://arxiv.org/abs/2506.05338)
...
TLDR: 提出了一种生成室内空间去家具化模型的流程，通过简化网格和多视角全景图像实现高质量结果。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如神经辐射场或RGB-D修复）在生成去家具化模型时存在模糊或幻觉问题，需要更高质量的解决方案。

Method: 首先从网格中分割并移除家具，生成简化网格（SDM），提取Canny边缘作为ControlNet修复的引导信号，最终修复全景图像并纹理化网格。

Result: 该方法生成的资产质量高于依赖神经辐射场或RGB-D修复的方法。

Conclusion: 提出的流程能够有效生成高质量的去家具化室内模型，优于现有技术。
...
### [393] [Direct Numerical Layout Generation for 3D Indoor Scene Synthesis via Spatial Reasoning](https://arxiv.org/abs/2506.05341)
...
<details>
  <summary>Details</summary>
Motivation: 现有3D场景布局生成方法因数据集有限而难以生成开放词汇且符合用户指令的场景，DirectLayout旨在通过大语言模型的空间推理能力解决这一问题。

Method: DirectLayout分三阶段生成布局：生成鸟瞰图（BEV）、将其提升至3D空间、细化物体放置。采用Chain-of-Thought激活和CoT-Grounded生成布局奖励增强空间推理和泛化能力。

Result: 实验表明，DirectLayout在语义一致性、泛化能力和物理合理性方面表现优异。

Conclusion: DirectLayout通过大语言模型的空间推理能力，实现了高质量、开放词汇的3D场景布局生成。
...
### [394] [Refer to Anything with Vision-Language Prompts](https://arxiv.org/abs/2506.05342)
*Shengcao Cao,Zijun Wei,Jason Kuen,Kangning Liu,Lingzhi Zhang,Jiuxiang Gu,HyunJoon Jung,Liang-Yan Gui,Yu-Xiong Wang*

Main category: cs.CV

TLDR: 论文提出了一种新的任务——全模态引用表达分割（ORES），通过结合文本和视觉实体的任意提示生成高质量的分割掩码组，并提出了RAS框架来增强多模态交互和理解。
...
Conclusion: ORES任务和RAS框架为多模态交互下的图像分割提供了新的解决方案，具有广泛的应用潜力。
...
### [395] [ContentV: Efficient Training of Video Generation Models with Limited Compute](https://arxiv.org/abs/2506.05343)
...
<details>
  <summary>Details</summary>
Motivation: 视频生成的快速发展需要更高效的训练方法以降低计算成本。

Method: 1. 采用简约架构，重用预训练图像生成模型；2. 多阶段训练策略，利用流匹配提升效率；3. 低成本强化学习框架，无需额外人工标注。
...
### [396] [SparseMM: Head Sparsity Emerges from Visual Concept Responses in MLLMs](https://arxiv.org/abs/2506.05344)
*Jiahui Wang,Zuyan Liu,Yongming Rao,Jiwen Lu*

Main category: cs.CV

TLDR: 研究发现多模态大语言模型（MLLMs）中仅有约5%的注意力头对视觉理解有贡献，提出了一种无训练框架识别这些视觉头，并基于此设计了KV-Cache优化策略SparseMM，显著提升了推理效率。


<details>
  <summary>Details</summary>
Motivation: 探索MLLMs如何处理视觉输入，揭示注意力机制中的稀疏性现象，以提高模型效率。

Method: 通过无训练框架量化注意力头的视觉相关性，设计SparseMM优化策略，为不同头分配非对称计算预算。

Result: SparseMM在主流多模态基准测试中实现了1.38倍实时加速和52%内存减少，同时保持性能。

Conclusion: 视觉头的稀疏性可被有效利用以优化MLLMs推理效率，SparseMM展示了优越的精度-效率平衡。
...
### [397] [Neural Inverse Rendering from Propagating Light](https://arxiv.org/abs/2506.05347)
*Anagh Malik,Benjamin Attal,Andrew Xie,Matthew O'Toole,David B. Lindell*

Main category: cs.CV

TLDR: 首个基于物理的神经逆向渲染系统，利用多视角视频捕捉光传播，结合时间分辨神经辐射缓存技术，实现高精度3D重建和光传播效果分解。


<details>
  <summary>Details</summary>
Motivation: 解决强间接光环境下3D重建的挑战，并探索光传播的分解与重光照能力。

Method: 扩展时间分辨神经辐射缓存技术，存储多方向无限反弹的辐射信息，结合闪光激光雷达系统捕获数据。

Result: 实现高精度3D重建，支持光传播视图合成、直接与间接光分解，以及多视角时间分辨重光照。

Conclusion: 该系统为复杂光环境下的3D重建和光传播分析提供了创新解决方案。
...
### [398] [FreeTimeGS: Free Gaussians at Anytime and Anywhere for Dynamic Scene Reconstruction](https://arxiv.org/abs/2506.05348)
*Yifan Wang,Peishan Yang,Zhen Xu,Jiaming Sun,Zhanhua Zhang,Yong Chen,Hujun Bao,Sida Peng,Xiaowei Zhou*

Main category: cs.CV

TLDR: 提出FreeTimeGS，一种新型4D表示方法，用于解决动态3D场景重建中复杂运动的挑战，优于现有方法。
...
### [399] [VideoMathQA: Benchmarking Mathematical Reasoning via Multimodal Understanding in Videos](https://arxiv.org/abs/2506.05349)
*Hanoona Rasheed,Abdelrahman Shaker,Anqi Tang,Muhammad Maaz,Ming-Hsuan Yang,Salman Khan,Fahad Khan*

Main category: cs.CV

TLDR: VideoMathQA是一个评估模型在多模态视频中进行数学推理能力的基准，涵盖10个数学领域，强调跨模态和时间维度的复杂推理。
...
Method: 引入VideoMathQA基准，包含多样化的视频和问题类型，由专家标注，设计三种核心推理挑战。

Result: 基准突出了现有方法的局限性，并提供了系统评估框架。

Conclusion: VideoMathQA为跨模态和时间维度的数学推理任务提供了标准化评估工具。
...
### [400] [Contrastive Flow Matching](https://arxiv.org/abs/2506.05350)
*George Stoica,Vivek Ramanujan,Xiang Fan,Ali Farhadi,Ranjay Krishna,Judy Hoffman*

Main category: cs.CV

TLDR: Contrastive Flow Matching (CFM) 通过对比目标增强条件流之间的独特性，提升条件分离效果，显著加速训练并减少去噪步骤。
...
Method: 引入对比目标，最大化任意样本对预测流之间的差异性。

Result: CFM 在 ImageNet-1k 和 CC3M 上验证，训练速度提升 9 倍，去噪步骤减少 5 倍，FID 降低 8.9。

Conclusion: CFM 显著优于传统流匹配，代码已开源。
...
### [401] [CMIE: Combining MLLM Insights with External Evidence for Explainable Out-of-Context Misinformation Detection](https://arxiv.org/abs/2505.23449)
*Fanxiao Li,Jiaying Wu,Canyuan He,Wei Zhou*

Main category: cs.MM

TLDR: 论文提出CMIE框架，通过CRG策略和AS机制改进多模态大语言模型在OOC虚假信息检测中的表现，解决了现有方法在深层语义关系和噪声证据上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在检测OOC虚假信息时难以捕捉图像与文本间的深层语义关系，且噪声证据影响准确性。

Method: 提出CMIE框架，结合CRG策略生成共存关系，AS机制评分关联性，选择性利用证据增强检测。

Result: 实验表明CMIE优于现有方法。

Conclusion: CMIE通过捕捉深层关系和优化证据使用，显著提升了OOC虚假信息检测效果。
...
### [402] [TQml Simulator: Optimized Simulation of Quantum Machine Learning](https://arxiv.org/abs/2506.04891)
*Viacheslav Kuzmin,Basil Kyriacou,Mateusz Papierz,Mo Kordzanganeh,Alexey Melnikov*

Main category: quant-ph

TLDR: 论文提出了一种高效的量子机器学习电路模拟方法，开发了TQml Simulator，在多数情况下性能优于现有工具。


<details>
  <summary>Details</summary>
Motivation: 为加速量子机器学习算法的模拟，研究通用的和针对特定门的技术。

Method: 通过数值基准测试，分析不同门层的最优模拟方法，并开发了TQml Simulator。

Result: TQml Simulator在标准门集和特定硬件门集上表现优异，性能提升2至100倍。

Conclusion: 针对不同门层采用最优模拟方法可显著提升量子机器学习模拟效率。
...
### [403] [The GAIN Model: A Nature-Inspired Neural Network Framework Based on an Adaptation of the Izhikevich Model](https://arxiv.org/abs/2506.04247)
*Gage K. R. Hooper*
...
Method: 采用网格结构和Izhikevich模型，促进神经元与邻近神经元的交互。

Result: 实现了计算高效且生物准确的神经模拟，适用于神经网络开发和大规模模拟。

Conclusion: GAIN模型通过网格和Izhikevich模型的结合，提升了动态性和准确性，适用于专业领域。
...
### [404] [Generalizable, real-time neural decoding with hybrid state-space models](https://arxiv.org/abs/2506.05320)
*Avery Hee-Woon Ryoo,Nanda H. Krishna,Ximeng Mao,Mehdi Azabou,Eva L. Dyer,Matthew G. Perich,Guillaume Lajoie*

Main category: q-bio.NC

TLDR: POSSM是一种新型混合架构，结合了跨注意力模块的尖峰标记化和循环状态空间模型，旨在实现快速、因果的神经活动预测，并在多数据集预训练下高效泛化。


<details>
  <summary>Details</summary>
Motivation: 解决传统方法泛化能力不足和Transformer计算资源需求高的问题，适用于实时神经解码。

Method: 采用跨注意力模块和循环状态空间模型（SSM）的混合架构，支持多数据集预训练。

Result: 在猴子运动任务和人类手写、语音解码中表现优异，推理速度比Transformer快9倍，精度相当。

Conclusion: 混合SSM架构在精度、推理速度和泛化能力之间取得了平衡，适用于实时闭环应用。
...
### [405] [hdl2v: A Code Translation Dataset for Enhanced LLM Verilog Generation](https://arxiv.org/abs/2506.04544)
...
Method: 通过翻译或编译VHDL、Chisel和PyMTL3生成Verilog代码，构建hdl2v数据集，并验证其在提升LLM性能中的作用。

Result: 使用hdl2v数据集后，32B参数模型的性能提升23%（pass@10），数据增强方法的性能提升63%。

Conclusion: hdl2v数据集有效提升了LLM生成Verilog代码的能力，未来可进一步优化数据集特性以提升性能。
...
### [406] [QiMeng: Fully Automated Hardware and Software Design for Processor Chip](https://arxiv.org/abs/2506.05007)
...
TLDR: QiMeng系统通过分层设计（LPCM、硬件/软件设计代理）实现处理器芯片全自动化设计，解决了知识表示、数据稀缺等问题，已部分应用并展现优势。


<details>
  <summary>Details</summary>
Motivation: 传统处理器芯片设计面临物理限制、资源需求和生态系统多样性等挑战，需全自动化解决方案。

Method: 构建领域特定大模型LPCM，开发硬件/软件设计代理，分层实现自动化设计。

Result: 部分组件已成功应用，展现显著优势。
...
### [407] [BEAR: BGP Event Analysis and Reporting](https://arxiv.org/abs/2506.04514)
...
Conclusion: BEAR为BGP异常事件提供自动化解释，助力网络管理。
...
### [408] [Intelligent Channel Allocation for IEEE 802.11be Multi-Link Operation: When MAB Meets LLM](https://arxiv.org/abs/2506.04594)
*Shumin Lian,Jingwen Tong,Jun Zhang,Liqun Fu*

Main category: cs.NI

TLDR: 论文提出了一种动态信道分配算法BAI-MCTS及其改进版LLM-BAI-MCTS，用于解决WiFi 7网络中多链路操作（MLO）的信道分配问题，显著提升了收敛速度和性能。


<details>
  <summary>Details</summary>
Motivation: WiFi 7的多链路操作（MLO）在密集网络中面临信道分配的挑战，现有研究多集中于静态网络配置，而动态信道分配问题尚未充分解决。

Method: 将问题建模为组合优化问题，采用多臂老虎机（MAB）框架进行在线学习，提出BAI-MCTS算法，并进一步结合大语言模型（LLM）开发了LLM-BAI-MCTS算法。

Result: BAI-MCTS算法收敛速度比现有算法快50.44%，LLM-BAI-MCTS在密集网络中收敛速度提升63.32%。

Conclusion: BAI-MCTS和LLM-BAI-MCTS算法有效解决了动态信道分配问题，显著提升了网络性能，适用于密集WiFi 7网络。
...
### [409] [Towards Network Data Analytics in 5G Systems and Beyond](https://arxiv.org/abs/2506.04860)
*Marcos Lima Romero,Ricardo Suyama*

Main category: cs.NI

TLDR: 论文分析了移动网络运营商（MNOs）如何通过数据分析和NWDAF提升价值，提出了两个新用例以促进其应用和商业化。


<details>
  <summary>Details</summary>
Motivation: 移动网络运营商的数据利用率低，而OTT玩家主导市场。数据分析是提升价值和避免业务风险的关键。

Method: 分析了70多篇文献的趋势和缺口，并提出了两个新的NWDAF用例。

Result: 研究发现学术研究局限于少数方法和用例，提出了扩展NWDAF应用的新方向。

Conclusion: 通过新用例和更广泛的数据分析，NWDAF有望为MNOs创造更多价值。
...
### [410] [ChemReservoir -- An Open-Source Framework for Chemically-Inspired Reservoir Computing](https://arxiv.org/abs/2506.04249)
*Mehmet Aziz Yirik,Jakob Lykke Andersen,Rolf Fagerberg,Daniel Merkle*

Main category: cs.CE

TLDR: 本文介绍了ChemReservoir，一个开源的化学启发式储层计算框架，解决了以往DNA化学工具的限制，提供了更通用的构建和分析功能。
...
Method: 开发了ChemReservoir框架，支持化学启发式储层的构建和分析，并通过增强测试和评估确保可重复性。

Result: 在多种循环拓扑结构中测试，ChemReservoir在记忆容量任务中表现稳定。

Conclusion: ChemReservoir为化学启发式储层计算提供了通用且可靠的解决方案，弥补了以往工具的不足。
...
### [411] [Can we reconstruct a dysarthric voice with the large speech model Parler TTS?](https://arxiv.org/abs/2506.04397)
*Ariadna Sanchez,Simon King*

Main category: eess.AS

TLDR: 研究使用Parler TTS大语音模型重建构音障碍患者的语音，发现模型能生成语音但难以控制清晰度和说话人一致性。


<details>
  <summary>Details</summary>
Motivation: 构音障碍患者沟通困难，个性化语音合成是潜在解决方案。

Method: 使用Parler TTS模型，通过标注数据集微调模型，评估生成语音的清晰度和说话人一致性。

Result: 模型能学习生成语音，但清晰度和说话人一致性控制不佳。

Conclusion: 未来需改进模型的可控性以更好地完成语音重建任务。
...
### [412] [Towards Efficient Speech-Text Jointly Decoding within One Speech Language Model](https://arxiv.org/abs/2506.04518)
*Haibin Wu,Yuxuan Hu,Ruchao Fan,Xiaofei Wang,Kenichi Kumatani,Bo Ren,Jianwei Yu,Heng Lu,Lijuan Wang,Yao Qian,Jinyu Li*

Main category: eess.AS

TLDR: 本文比较了联合语音文本解码策略（交错与并行生成），发现交错方法对齐效果最佳但推理慢，提出了一种加速解码的早期停止交错（ESI）模式，并构建了高质量QA数据集提升语音QA性能。


<details>
  <summary>Details</summary>
Motivation: 研究联合语音文本解码策略对语音语言模型性能、效率和对齐质量的影响，以优化口语对话系统。

Method: 在相同基础语言模型、语音分词器和训练数据下，系统比较交错与并行生成解码策略，并提出早期停止交错（ESI）模式。

Result: 交错方法对齐效果最佳但推理慢；ESI模式显著加速解码且性能略优；高质量QA数据集进一步提升了语音QA性能。

Conclusion: 早期停止交错（ESI）模式是高效且性能优越的解码策略，结合高质量QA数据集可显著提升语音语言模型的实际应用效果。
...
### [413] [EMO-Debias: Benchmarking Gender Debiasing Techniques in Multi-Label Speech Emotion Recognition](https://arxiv.org/abs/2506.04652)
*Yi-Cheng Lin,Huang-Cheng Chou,Yu-Hsuan Li Liang,Hung-yi Lee*

Main category: eess.AS

TLDR: EMO-Debias比较了13种去偏方法在多标签语音情感识别中的效果，分析了公平性与准确性的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有去偏方法在多标签语音情感识别中的效果和鲁棒性尚未充分研究。

Method: 比较了13种去偏方法，包括预处理、正则化、对抗学习等，使用WavLM和XLSR表征在性别不平衡条件下测试。

Result: 量化了公平性与准确性的权衡，发现某些方法能减少性别性能差距而不影响整体性能。
...
### [414] [kTULA: A Langevin sampling algorithm with improved KL bounds under super-linear log-gradients](https://arxiv.org/abs/2506.04878)
*Iosif Lytras,Sotirios Sabanis,Ying Zhang*

Main category: math.ST

TLDR: 论文提出了一种名为kTULA的新算法，用于解决具有超线性增长对数梯度的分布采样问题，并在KL散度和Wasserstein-2距离上提供了理论保证。
...
Method: 提出了一种基于tamed Langevin dynamics的kTULA算法。

Result: 在KL散度上建立了非渐近收敛界，收敛速率为$2-\overline{\epsilon}$，显著优于现有文献。

Conclusion: kTULA在采样和优化问题中表现出色，并通过实验验证了其理论保证。
...
### [415] [MVP-Shapley: Feature-based Modeling for Evaluating the Most Valuable Player in Basketball](https://arxiv.org/abs/2506.04602)
*Haifeng Sun,Yu Xiong,Runze Wu,Kai Wang,Lan Zhang,Changjie Fan,Shaojie Tang,Xiang-Yang Li*

Main category: cs.GT

TLDR: 论文提出了一种基于Shapley值的MVP评估框架（\oursys），通过特征处理、胜负模型训练、Shapley值分配和贡献排名，优化算法以匹配专家投票结果，并在NBA和Dunk City Dynasty数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 随着电子竞技和多人游戏的快速发展，评估最有价值球员（MVP）的需求日益突出，但现有方法缺乏解释性和实用性。

Method: 提出\oursys框架，利用Shapley值进行特征处理、胜负模型训练、贡献分配和排名确定，并通过因果视角优化算法以匹配专家投票结果。

Result: 在NBA和Dunk City Dynasty数据集上验证了方法的有效性，并实现了工业部署。

Conclusion: \oursys框架为MVP评估提供了一种解释性强且实用的解决方案，适用于实际应用场景。
...
### [416] [Computationally Intensive Research: Advancing a Role for Secondary Analysis of Qualitative Data](https://arxiv.org/abs/2506.04230)
*Kaveh Mohajeri,Amir Karami*

Main category: cs.DB

TLDR: 探讨计算密集型二次分析在重新利用定性研究数据中的潜力及其挑战。
...
Method: 提出一种计算密集型二次分析方案，并探讨其如何促进创新研究设计。

Result: 展示了计算密集型二次分析的优势，同时指出数据共享和重用的关键挑战。

Conclusion: 计算密集型二次分析为定性数据再利用提供了新机会，但也需解决相关挑战。
...
### [417] [Benchmark for Antibody Binding Affinity Maturation and Design](https://arxiv.org/abs/2506.04235)
...
TLDR: AbBiBench是一个抗体结合亲和力成熟与设计的基准框架，通过评估抗体-抗原复合物的功能潜力，为抗体设计提供统一、生物学基础的评价标准。
...
Method: AbBiBench基于抗体-抗原复合物的模型似然性评估抗体设计潜力，并系统比较了14种蛋白质模型。
...
### [418] [The Latent Space Hypothesis: Toward Universal Medical Representation Learning](https://arxiv.org/abs/2506.04515)
*Salil Patel*

Main category: q-bio.QM

TLDR: 论文提出了一种基于潜在空间假设的框架，将多模态医学数据映射到统一的学习几何表示中，以支持个性化医疗。


<details>
  <summary>Details</summary>
Motivation: 医学数据多模态且复杂，传统分类方法可能掩盖疾病的多样性。潜在空间假设旨在通过统一表示揭示患者的独特轨迹。

Method: 利用潜在空间假设，将多模态数据映射到统一的学习几何表示中，通过轨迹和向量描述疾病进展和治疗干预。

Result: 框架能够揭示疾病的子轨迹和患者特异性变化方向，为个性化诊断和治疗提供定量依据。

Conclusion: 潜在空间假设为个性化医疗提供了新视角，但仍需解决数据偏见、隐私和因果推断等挑战。
...
### [419] [Robust Moment Identification for Nonlinear PDEs via a Neural ODE Approach](https://arxiv.org/abs/2506.05245)
*Shaoxuan Chen,Su Yang,Panayotis G. Kevrekidis,Wei Zhu*

Main category: nlin.PS

TLDR: 提出了一种基于神经ODE的数据驱动框架，用于从PDE系统中学习降维矩动力学，适用于稀疏和不规则时间序列数据。


<details>
  <summary>Details</summary>
Motivation: 传统基于导数的方法（如SINDy）需要密集采样数据且对噪声敏感，而神经ODE方法能直接从稀疏数据中建模矩轨迹，更具鲁棒性。

Method: 使用神经ODE直接建模矩轨迹，并通过Stiefel流形优化实现数据驱动的坐标变换，以发现低维表示。

Result: 在非线性薛定谔方程和Fisher-KPP反应扩散系统中，该方法能准确恢复矩动力学，并在数据有限的情况下表现出优越的外推精度。
...
<details>
  <summary>Details</summary>
Motivation: 探索光学物理方程与生成模型之间的深层联系，利用光传播动力学的自组织特性提升人工智能方法的效率和性能。

Method: 分析了六种基本光学方程，比较了线性模型（如Helmholtz、耗散波和Eikonal方程）及其非线性扩展（如Kerr效应、立方-五次非线性和强度依赖折射率）。

Result: 非线性模型在参数减少、模式分离、稳定性和生成质量方面显著优于线性模型。例如，非线性Helmholtz模型的FID得分从1.0909降至0.0089，内存使用减少40-60%，训练时间缩短30-50%。
...
### [421] [Domain Adaptation Method and Modality Gap Impact in Audio-Text Models for Prototypical Sound Classification](https://arxiv.org/abs/2506.04376)
*Emiliano Acevedo,Martín Rocamora,Magdalena Fuentes*

Main category: cs.SD

TLDR: 音频-文本模型在零样本环境声音分类中表现良好，但背景声源会显著降低其性能。本文提出一种新方法，量化并整合背景声源的影响，无需重新训练模型即可提升分类效果。


<details>
  <summary>Details</summary>
Motivation: 背景声源导致音频-文本模型性能下降，需要一种无需重新训练的方法来改善分类效果。

Method: 提出一种量化背景声源贡献的方法，并通过缩小音频与文本嵌入之间的模态差距来提升性能。

Result: 该方法在不同背景和信噪比条件下提高了分类准确性，且适用于多种原型方法。

Conclusion: 该方法具有可扩展性和鲁棒性，适用于多样化环境的声音分类。
...
### [422] [Grapheme-Coherent Phonemic and Prosodic Annotation of Speech by Implicit and Explicit Grapheme Conditioning](https://arxiv.org/abs/2506.04527)
...
Method: 1) 使用预训练BERT特征的提示编码器隐式引入字素条件；2) 在推理阶段显式剪枝与字素不一致的标签假设。

Result: 显著提升标签与字素的一致性，并在口音估计任务中验证了生成数据的有效性。

Conclusion: 所提方法能生成语音、标签和字素的并行数据，适用于多种下游任务。
...
### [423] [LLM-based phoneme-to-grapheme for phoneme-based speech recognition](https://arxiv.org/abs/2506.04711)
*Te Ma,Min Bi,Saierdaer Yusuyin,Hao Huang,Zhijian Ou*

Main category: cs.SD

TLDR: 论文提出了一种基于大型语言模型（LLM）的音素到字素解码方法（LLM-P2G），用于音素基础的自动语音识别（ASR），通过数据增强和随机化训练策略，显著优于传统的WFST解码方法。


<details>
  <summary>Details</summary>
Motivation: 传统的WFST解码方法在音素基础ASR中存在复杂流程和无法利用大型语言模型的限制，因此需要一种更高效的方法。

Method: 提出LLM-P2G解码方法，包括语音到音素（S2P）和音素到字素（P2G）两个阶段，并通过数据增强（DANP）和随机化训练（TKM）解决信息丢失问题。

Result: 实验结果显示，LLM-P2G在波兰语和德语的跨语言ASR中，相对WER分别降低了3.6%和6.9%。

Conclusion: LLM-P2G方法在音素基础ASR中表现出色，优于传统WFST解码方法，为跨语言语音识别提供了高效解决方案。
...
### [424] [Survey on the Evaluation of Generative Models in Music](https://arxiv.org/abs/2506.05104)
*Alexander Lerch,Claire Arthur,Nick Bryan-Kinns,Corey Ford,Qianyi Sun,Ashvala Vinay*

Main category: cs.SD

TLDR: 本文综述了音乐生成系统的评估目标、方法和指标，涵盖主观与客观、定性与定量、实证与计算方法，并从音乐学、工程和人机交互角度讨论了其优缺点。


<details>
  <summary>Details</summary>
Motivation: 音乐生成系统的研究日益增多，但缺乏系统性的评估方法综述，本文旨在填补这一空白。

Method: 通过跨学科视角，综合分析了音乐生成系统的评估目标、方法和指标，包括主观与客观、定性与定量、实证与计算等方法。
...
TLDR: 提出了一种基于用户交互的人类计算方法，通过遗传算法和用户反馈逐步提升AI音乐生成模型的性能。
...
Result: 初步测试显示，第一轮迭代评分平均提升0.2，第二轮进一步提升0.39。

Conclusion: 该方法通过用户反馈逐步优化模型，显著提升了音乐生成的个性化表现。
...
### [426] [Artificial Intelligence Should Genuinely Support Clinical Reasoning and Decision Making To Bridge the Translational Gap](https://arxiv.org/abs/2506.05030)
*Kacper Sokol,James Fackler,Julia E Vogt*

Main category: cs.HC

TLDR: 论文提出了一种新的社会技术概念，旨在通过数据驱动工具补充医生的认知活动，而非追求不切实际的性能指标。


<details>
  <summary>Details</summary>
Motivation: 当前人工智能在医学中的应用受限于技术中心化的方法，未能与临床实践（如诊断推理和决策）兼容。

Method: 提出了一种社会技术概念化的数据驱动支持工具，专注于补充医生的认知和认知活动。

Result: 该方法强调实际影响，而非在无关紧要的基准上追求超人类表现。
...
### [427] [Teaming in the AI Era: AI-Augmented Frameworks for Forming, Simulating, and Optimizing Human Teams](https://arxiv.org/abs/2506.05265)
*Mohammed Almutairi*

Main category: cs.HC

TLDR: 论文提出AI增强的团队优化框架，包括团队组建算法、AI反馈系统和多代理模拟工具，以提升团队满意度、凝聚力和表现。


<details>
  <summary>Details</summary>
Motivation: 现有团队优化工具依赖静态数据或狭窄目标，无法适应动态团队需求，导致成员不满和表现下降。

Method: 1. 多臂老虎机算法优化团队组建；2. AI反馈系统tAIfa提供个性化指导；3. PuppeteerLLM模拟多代理团队动态。

Result: 框架提升团队满意度、凝聚力和表现。

Conclusion: AI工具能动态优化团队，适应成员需求和目标变化。
...
### [428] [Exp4Fuse: A Rank Fusion Framework for Enhanced Sparse Retrieval using Large Language Model-based Query Expansion](https://arxiv.org/abs/2506.04760)
*Lingyuan Liu,Mengxiang Zhang*

Main category: cs.IR

TLDR: Exp4Fuse是一种新颖的融合排名框架，通过零样本LLM查询扩展提升稀疏检索性能，优于现有方法并达到SOTA结果。


<details>
  <summary>Details</summary>
Motivation: LLM生成的文档质量对查询扩展效果影响大，且成本高。研究旨在通过零样本LLM查询扩展改进稀疏检索。
...
### [429] [GOLFer: Smaller LM-Generated Documents Hallucination Filter & Combiner for Query Expansion in Information Retrieval](https://arxiv.org/abs/2506.04762)
*Lingyuan Liu,Mengxiang Zhang*

Main category: cs.IR

TLDR: GOLFer是一种利用较小开源语言模型进行查询扩展的新方法，通过过滤生成文档中的幻觉内容并组合有效信息，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 解决基于大语言模型的查询扩展方法成本高、计算密集且难以访问的问题。

Method: GOLFer包含幻觉过滤器和文档组合器两个模块，分别用于去除生成文档中的非事实内容并平衡查询与文档的权重。

Result: 在多个数据集上的实验表明，GOLFer使用较小模型时性能优于其他方法，并与大型模型方法竞争。

Conclusion: GOLFer提供了一种高效且经济的替代方案，解决了现有方法的局限性。
...
### [430] [Towards Storage-Efficient Visual Document Retrieval: An Empirical Study on Reducing Patch-Level Embeddings](https://arxiv.org/abs/2506.04997)
*Yubo Ma,Jinsong Li,Yuhang Zang,Xiaobao Wu,Xiaoyi Dong,Pan Zhang,Yuhang Cao,Haodong Duan,Jiaqi Wang,Yixin Cao,Aixin Sun*

Main category: cs.IR

TLDR: 论文研究了减少视觉文档检索（VDR）中内存占用的方法，通过评估两种标记减少策略（标记剪枝和标记合并），发现标记合并更适合VDR，并开发了Light-ColPali/ColQwen2，显著降低了内存使用同时保持高性能。


<details>
  <summary>Details</summary>
Motivation: ColPali/ColQwen2在VDR中表现优异，但因每页编码为多个补丁级嵌入导致内存占用过高。研究旨在减少每页的补丁嵌入数量，同时最小化性能损失。

Method: 评估了两种标记减少策略：标记剪枝和标记合并。研究发现随机剪枝优于复杂方法，但剪枝本身不适合VDR。随后聚焦标记合并，优化了三个维度的合并策略，开发了Light-ColPali/ColQwen2。

Result: Light-ColPali/ColQwen2在仅使用11.8%原始内存的情况下保持了98.2%的检索性能，在2.8%内存占用下仍保留94.6%的有效性。

Conclusion: 研究为高效VDR提供了有价值的见解，并建立了竞争性基线。Light-ColPali/ColQwen2展示了标记合并的潜力，为未来研究指明了方向。
...
### [431] [Reason-to-Recommend: Using Interaction-of-Thought Reasoning to Enhance LLM Recommendation](https://arxiv.org/abs/2506.05069)
*Keyu Zhao,Fengli Xu,Yong Li*

Main category: cs.IR

TLDR: R2Rec是一个基于LLM的推理增强推荐框架，通过交互链和渐进式提示策略模拟决策过程，显著提升推荐性能。
...
Method: R2Rec从用户-物品图中采样交互链，通过渐进式掩码提示策略将其转换为结构化推理链，结合监督微调和强化学习进行训练。

Result: 在三个真实数据集上，R2Rec在HitRatio@1上平均提升10.48%，比原始LLM提升131.81%，同时增强了解释性。

Conclusion: R2Rec通过模拟逐步推理和两阶段训练，显著提升了推荐性能和解释性。
...
### [432] [What does making money have to do with crime?: A dive into the National Crime Victimization survey](https://arxiv.org/abs/2506.04240)
*Sydney Anuyah*

Main category: physics.soc-ph

TLDR: 利用1992年至2022年的国家犯罪受害调查，研究收入、教育、就业和人口因素如何影响犯罪类型（暴力与财产犯罪）。结果表明，高收入和教育降低暴力犯罪风险，而男性、年轻人和少数族裔风险更高。
...
Result: 高收入和教育降低暴力犯罪风险；男性、年轻人和少数族裔风险更高。郊区模型预测效果最佳（准确率0.607）。
...
TLDR: 论文研究了基于Otto-Wasserstein几何的概率分布集合上的测地主成分分析（GPCA），旨在识别最佳捕捉数据集变化模式的测地曲线。


<details>
  <summary>Details</summary>
Motivation: 通过GPCA在概率测度空间中识别变化模式，为数据分析和建模提供新方法。

Method: 针对高斯分布，将计算提升到可逆线性映射空间；对绝对连续概率测度，提出用神经网络参数化Wasserstein空间中的测地曲线。

Result: 通过实例比较了经典切空间PCA，并在真实数据集上进行了验证。

Conclusion: GPCA在概率测度空间中有效捕捉变化模式，神经网络参数化方法为复杂分布提供了新工具。
...
TLDR: 论文研究了单智能体和联邦强化学习中的样本成本和通信成本最小化问题，提出了两种新算法，实现了最优遗憾、低启动成本和低切换/通信成本。


<details>
  <summary>Details</summary>
Motivation: 现实场景中数据收集和策略部署成本高昂，需要减少启动成本和策略切换或通信成本。

Method: 提出了两种无模型强化学习算法：Q-EarlySettled-LowCost和FedQ-EarlySettled-LowCost。

Result: 算法同时实现了最优遗憾、线性启动成本和低切换/通信成本，并提供了理论保证。
...
TLDR: 论文提出了一种基于分布编码（DE）的GP回归方法，用于处理分类变量，并展示了其在预测性能和计算效率上的优越性。


<details>
  <summary>Details</summary>
Motivation: GP回归在处理分类变量时面临挑战，传统方法（如目标编码）效果有限，需要更高效的解决方案。

Method: 提出分布编码（DE）方法，利用目标变量的全部分布信息，结合特征核（如最大均值差异和Wasserstein距离）构建GP模型。

Result: 实验验证表明，DE方法在合成和真实数据集上实现了最先进的预测性能。

Conclusion: DE方法为处理分类变量的GP回归提供了高效解决方案，并可与离散和混合空间的贝叶斯优化结合使用。
...
TLDR: 该研究提出了一种仅使用观测数据和单变量干预数据来推断联合干预效应的方法，证明了在非线性加性结果机制下联合效应是可识别的，并通过实验验证了其有效性。
...
Method: 提出了一种可识别性结果，证明在非线性加性结果机制下，无需联合干预数据即可推断联合效应，并设计了一种将因果效应分解为混杂和非混杂贡献的实用估计器。

Result: 在合成数据上的实验表明，该方法性能接近直接使用联合干预数据训练的模型，优于纯观测估计器。

Conclusion: 该方法为在缺乏联合干预数据的情况下估计联合因果效应提供了可行的解决方案。
...
TLDR: 论文提出了一种扩展非线性加性噪声模型的方法，用于处理随机向量，通过两步法学习因果图：先推断随机向量间的因果顺序，再选择最佳图模型。


<details>
  <summary>Details</summary>
Motivation: 许多重要领域（如神经科学、心理学、社会科学和工业制造）中的因果单元是变量组而非标量，现有方法对此支持不足。

Method: 扩展非线性加性噪声模型至随机向量，采用两步法：因果顺序推断和模型选择。

Result: 在模拟中表现优异，并成功应用于具有部分因果顺序知识的实际装配线数据。

Conclusion: 该方法为处理变量组间的因果关系提供了有效工具，具有实际应用潜力。
...
TLDR: 论文研究了在潜在变量线性非高斯无环模型（lvLiNGAM）中利用高阶累积量识别因果效应，解决了存在潜在混杂时的两个挑战性场景：单代理变量和欠定工具变量情况。


<details>
  <summary>Details</summary>
Motivation: 解决在潜在混杂存在下因果效应识别的挑战，特别是单代理变量和欠定工具变量情况。

Method: 利用高阶累积量证明因果效应在单代理或工具变量下的可识别性，并提供相应估计方法。

Result: 实验结果表明，所提方法在准确性和鲁棒性上优于现有方法。

Conclusion: 研究推进了在潜在混杂线性系统中因果推断的理论与实践理解。
...
TLDR: 在最佳臂识别问题中，当实验者可以选择自适应试验时，忽略此选项而选择非自适应试验是不可取的。研究发现，至少有三个处理臂时，简单的自适应设计在统计效率上普遍且严格优于完全随机试验。


<details>
  <summary>Details</summary>
Motivation: 探讨在最佳臂识别问题中，自适应试验是否比非自适应试验更具优势，以优化实验资源分配。

Method: 采用批处理臂淘汰设计，逐步淘汰表现不佳的臂，并通过效率指数量化统计效率。

Result: 当至少有三个处理臂时，自适应设计在统计效率上普遍且严格优于完全随机试验。

Conclusion: 自适应设计在最佳臂识别问题中具有明显优势，解决了Qin [2022]提出的第二个开放问题。
...
### [440] [BridgeNet: A Hybrid, Physics-Informed Machine Learning Framework for Solving High-Dimensional Fokker-Planck Equations](https://arxiv.org/abs/2506.04354)
*Elmira Mirzabeigi,Rezvan Salehi,Kourosh Parand*

Main category: physics.comp-ph

TLDR: BridgeNet结合CNN与物理信息神经网络，高效求解非线性高维Fokker-Planck方程，优于传统PINN方法。
...
Result: 实验显示BridgeNet误差更低、收敛更快，高维环境下稳定。
...
### [441] [Knowledge-guided Contextual Gene Set Analysis Using Large Language Models](https://arxiv.org/abs/2506.04303)
...
TLDR: cGSA是一种新型AI驱动框架，通过上下文感知的路径优先排序改进传统基因集分析（GSA），解决了冗余和非特异性结果的问题。
...
Result: 在102个基因集和19种疾病的基准测试中，cGSA性能优于基线方法30%以上，专家验证证实其精确性和可解释性。
...
### [442] [A Comprehensive Survey on Bio-Inspired Algorithms: Taxonomy, Applications, and Future Directions](https://arxiv.org/abs/2506.04238)
...
TLDR: 本文综述了生物启发算法（BIAs）的分类、应用及未来研究方向，涵盖八类算法及其在多个领域的应用，并指出当前挑战。


<details>
  <summary>Details</summary>
Motivation: 为研究者和从业者提供生物启发计算领域的全面概述，明确当前进展和未来方向。

Method: 将BIAs分为八类（如进化算法、群体智能等），分析其核心原理、优势与局限，并探讨在机器学习等领域的应用。

Result: 总结了BIAs的近期进展（如混合策略、参数调优）和实际应用案例。

Conclusion: 提出了可扩展性、收敛性等开放性问题，为未来研究指明方向。
...
### [443] [Estimating properties of a homogeneous bounded soil using machine learning models](https://arxiv.org/abs/2506.04256)
*Konstantinos Kalimeris,Leonidas Mindrinos,Nikolaos Pallikarakis*

Main category: physics.geo-ph

TLDR: 论文研究了从水分测量中估算土壤性质的方法，通过模拟数据和机器学习模型评估不同条件下的性能。


<details>
  <summary>Details</summary>
Motivation: 解决从水分测量中准确估算土壤性质的问题，尤其是扩散率$D$和水力传导率$K$。

Method: 使用Fokas方法模拟垂直入渗数据，并比较多种机器学习模型（如SVM和NN）在不同数据条件下的表现。

Result: 扩散率$D$的预测比水力传导率$K$更准确，SVM和NN表现最优，误差最小。

Conclusion: SVM和NN在估算土壤性质时具有高鲁棒性和准确性，尤其在扩散率预测中表现突出。
...
### [444] [RoboRefer: Towards Spatial Referring with Reasoning in Vision-Language Models for Robotics](https://arxiv.org/abs/2506.04308)
...
<details>
  <summary>Details</summary>
Motivation: 解决现有视觉语言模型在复杂3D场景中空间指代任务中的不足。

Method: 结合SFT和RFT训练，引入RefSpatial数据集和RefSpatial-Bench基准。

Result: SFT训练的RoboRefer空间理解成功率89.6%，RFT训练的RoboRefer超越其他基线，包括Gemini-2.5-Pro。
...
### [445] [cuVSLAM: CUDA accelerated visual odometry](https://arxiv.org/abs/2506.04359)
...
<details>
  <summary>Details</summary>
Motivation: 为自主机器人提供准确且鲁棒的姿态估计是核心需求，cuVSLAM旨在满足这一需求，并支持广泛的传感器配置。

Method: cuVSLAM利用CUDA优化，支持从单RGB相机到32个相机的任意几何配置，适用于边缘计算设备。

Result: 在多个先进基准测试中，cuVSLAM表现出最佳性能。

Conclusion: cuVSLAM是一种高性能、灵活的视觉SLAM解决方案，适用于实时应用。
...
### [446] [Hierarchical Language Models for Semantic Navigation and Manipulation in an Aerial-Ground Robotic System](https://arxiv.org/abs/2506.05020)
...
<details>
  <summary>Details</summary>
Motivation: 传统静态模型难以应对任务多样性和动态环境，需要通用智能来桥接异构机器人的高层推理与低层执行。

Method: LLM负责任务分解和全局语义地图构建，VLM从航拍图像提取语义标签和空间信息支持局部规划。

Result: 在真实世界字母方块排列任务中验证了框架的动态适应性和鲁棒性。
...
### [447] [Learning Smooth State-Dependent Traversability from Dense Point Clouds](https://arxiv.org/abs/2506.04362)
*Zihao Dong,Alan Papalia,Leonard Jung,Alenna Spiro,Philip R. Osteen,Christa S. Robison,Michael Everett*

Main category: cs.RO

TLDR: SPARTA是一种通过点云估计角度条件可穿越性的方法，利用傅里叶基函数输出平滑分析函数，减少计算开销并提高泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决越野自主性中地形可穿越性与车辆状态（如接近角度）相关的问题，避免大量训练数据和重复模型推断的需求。

Method: 提出SPARTA方法，通过输出1-球面上的平滑分析函数（基于傅里叶基函数）预测风险分布，实现高效计算和泛化。

Result: 在高仿真模拟中，SPARTA成功穿越40米巨石场的成功率达91%（基线为73%），并在硬件测试中展示泛化能力。

Conclusion: SPARTA通过几何结构和傅里叶基函数有效解决了角度条件可穿越性问题，具有高效性和泛化性。
...
### [448] [MineInsight: A Multi-sensor Dataset for Humanitarian Demining Robotics in Off-Road Environments](https://arxiv.org/abs/2506.04842)
*Mario Malizia,Charles Hamesse,Ken Hasselmann,Geert De Cubber,Nikolaos Tsiogkas,Eric Demeester,Rob Haelterman*

Main category: cs.RO

TLDR: 论文介绍了MineInsight数据集，用于改进人道主义排雷中的计算机视觉算法验证。


<details>
  <summary>Details</summary>
Motivation: 缺乏多样化和真实的数据集限制了排雷算法的可靠验证。

Method: 创建了一个多传感器、多光谱的公开数据集，包含35个目标，集成双视角传感器扫描和多种光谱图像。

Result: 数据集包含约38,000 RGB帧、53,000 VIS-SWIR帧和108,000 LWIR帧，提供多样化的测试环境。

Conclusion: MineInsight为开发和评估排雷检测算法提供了基准，数据集已公开。
...
### [449] [Synthetic Dataset Generation for Autonomous Mobile Robots Using 3D Gaussian Splatting for Vision Training](https://arxiv.org/abs/2506.05092)
*Aneesh Deogan,Wout Beks,Peter Teurlings,Koen de Vos,Mark van den Brand,Rene van de Molengraft*

Main category: cs.RO

TLDR: 提出一种基于Unreal Engine的自动生成合成数据方法，用于解决手动标注数据集的耗时和多样性不足问题，尤其在机器人足球领域表现优异。


<details>
  <summary>Details</summary>
Motivation: 手动标注数据集耗时、易错且多样性有限，机器人领域的动态场景进一步加剧了这一问题。

Method: 利用Unreal Engine中的3D高斯样条技术快速生成合成数据，并结合真实数据提升检测性能。
...
### [450] [SGN-CIRL: Scene Graph-based Navigation with Curriculum, Imitation, and Reinforcement Learning](https://arxiv.org/abs/2506.04505)
*Nikita Oskolkov,Huzhenyu Zhang,Dmitry Makarov,Dmitry Yudin,Aleksandr Panov*

Main category: cs.RO

TLDR: 提出了一种基于3D场景图的强化学习导航框架SGN-CIRL，结合模仿学习和课程学习，显著提升了复杂导航任务的成功率。


<details>
  <summary>Details</summary>
Motivation: 解决部分可观测环境中机器人导航效率低的问题，利用3D场景图建模物体间空间关系。
...
### [451] [Olfactory Inertial Odometry: Sensor Calibration and Drift Compensation](https://arxiv.org/abs/2506.04539)
*Kordel K. France,Ovidiu Daescu,Anirban Paul,Shalini Prasad*

Main category: cs.RO

TLDR: 本文提出了一种用于校准嗅觉惯性里程计（OIO）的方法，以提高机器人在气味导航任务中的定位精度，适用于多种嗅觉传感器类型。
...
Method: 定义了一种校准OIO的流程，适用于多种嗅觉传感器类型，并在真实机器人手臂上进行了验证。

Result: 校准后的OIO在冷启动嗅觉导航任务中表现优于未校准的情况，实现了厘米级的定位精度。

Conclusion: 本文提出的OIO校准方法有效，适用于慢速移动机器人平台，展示了其在特定应用场景中的潜力。
...
### [452] [Chronoamperometry with Room-Temperature Ionic Liquids: Sub-Second Inference Techniques](https://arxiv.org/abs/2506.04540)
*Kordel K. France*

Main category: cs.RO

TLDR: 本文提出了一种新型数学回归方法，显著缩短了室温离子液体中计时安培法的测量时间至1秒以内。
...
Result: 与标准方法相比，新方法在保持合理精度的同时，显著减少了数据采集时间。

Conclusion: 该技术在分析化学、传感器技术和电池科学中具有广泛应用前景，尤其适用于快速电化学定量分析。
...
### [453] [Was Residual Penalty and Neural Operators All We Needed for Solving Optimal Control Problems?](https://arxiv.org/abs/2506.04742)
*Oliver G. S. Lundqvist,Fabricio Oliveira*

Main category: math.OC

TLDR: 论文提出了一种简化神经网络解决最优控制问题的方法，通过分离训练与优化过程，无需在训练中考虑成本函数，仅需一次物理信息训练即可解决多个问题。


<details>
  <summary>Details</summary>
Motivation: 传统方法在训练神经网络时需结合成本函数，增加了复杂性。本文旨在简化架构，通过分离训练与优化过程，提高效率。

Method: 采用DeepONet架构，训练阶段仅基于微分方程残差，优化阶段通过自动微分计算控制梯度，结合惩罚项解决最优控制问题。

Result: 在九个不同最优控制问题上验证了方法的有效性，展示了准确且一致的性能。

Conclusion: 研究表明，简化架构和分离训练与优化过程能够高效解决多个最优控制问题。
...
### [454] [Non-linear Multi-objective Optimization with Probabilistic Branch and Bound](https://arxiv.org/abs/2506.04554)
*Hao Huang,Zelda B. Zabinsky*

Main category: math.OC

TLDR: 提出了一种名为MOPBnB(so)的多目标模拟优化算法，用于近似随机多目标优化问题的帕累托最优集和高效前沿。
...
Method: MOPBnB(so)在每个解处仅评估一次噪声函数，并利用邻近解估计目标函数，避免了多次重复评估。

Result: 算法在确定性多目标问题中具有有限时间性能保证，且在随机问题中渐近收敛于帕累托最优集。数值结果表明其计算资源消耗显著低于多重复制变体，且优于遗传算法NSGA-II。
...
### [455] [Trustworthiness Preservation by Copies of Machine Learning Systems](https://arxiv.org/abs/2506.05203)
*Leonardo Ceragioli,Giuseppe Primiero*

Main category: cs.LO

TLDR: 本文提出了一种计算工具，用于验证从已知行为的原始系统复制的复杂系统的可信度，定义了四种不同的可信度概念，并研究了它们之间的关系。


<details>
  <summary>Details</summary>
Motivation: 在负责任的人工智能研究中，如何验证复制系统是否保持可信度是一个重要但尚未深入探讨的问题。

Method: 引入了一种计算模型，用于建模和验证数据和概率复杂查询，并定义了四种可信度概念（Justifiably、Equally、Weakly和Almost Trustworthy）。

Result: 研究了这些可信度概念之间的关系及其在逻辑操作下的组合方式。

Conclusion: 该计算工具可用于验证复制系统的可信度，为负责任AI的发展提供了支持。
...
### [456] [Understanding and Meeting Practitioner Needs When Measuring Representational Harms Caused by LLM-Based Systems](https://arxiv.org/abs/2506.04482)
*Emma Harvey,Emily Sheng,Su Lin Blodgett,Alexandra Chouldechova,Jean Garcia-Gathright,Alexandra Olteanu,Hanna Wallach*

Main category: cs.CY

TLDR: 研究发现，现有的NLP工具在衡量大型语言模型（LLM）的表征危害时，未能完全满足实践者的需求，主要因工具不实用或存在实际障碍。


<details>
  <summary>Details</summary>
Motivation: 探讨公开可用的NLP工具是否能有效帮助实践者评估LLM系统的表征危害。

Method: 通过对12名实践者进行半结构化访谈，分析工具的可用性和实用性。

Result: 实践者常因工具不实用或实际障碍无法使用现有工具，分为工具不匹配需求和实际使用障碍两类问题。
...
### [457] [Oversight Structures for Agentic AI in Public-Sector Organizations](https://arxiv.org/abs/2506.04836)
*Chris Schmitz,Jonathan Rystrøm,Jan Batzner*

Main category: cs.CY

TLDR: 论文探讨了代理型AI系统对传统公共部门监督机制的挑战，提出了五个关键治理维度，并通过混合方法评估现有监督结构的应对能力。


<details>
  <summary>Details</summary>
Motivation: 研究代理型AI系统对公共部门监督机制的强化挑战，旨在提出适应公共部门约束的治理方案。

Method: 采用文献综述和与AI相关公务员的访谈相结合的混合方法。

Result: 发现代理监督加剧了现有治理挑战，包括持续监督、治理与运营能力深度融合及跨部门协调。

Conclusion: 提出适应制度结构和设计兼容公共部门约束的代理监督方法。
...
### [458] [Intentionally Unintentional: GenAI Exceptionalism and the First Amendment](https://arxiv.org/abs/2506.05211)
*David Atkinson,Jena D. Hwang,Jacob Morrison*

Main category: cs.CY

TLDR: 论文质疑AI模型输出是否应受第一修正案保护，认为其缺乏意图性，不构成法律意义上的言论，且赋予保护会阻碍政府监管。


<details>
  <summary>Details</summary>
Motivation: 挑战AI模型输出享有第一修正案保护的假设，探讨其法律地位及社会影响。

Method: 通过法律先例和言论自由的基本原则，分析AI输出是否构成言论。

Result: AI输出因缺乏意图性不构成言论，赋予保护会妨碍监管并加剧社会风险。

Conclusion: 不应将第一修正案保护扩展至AI模型输出，以维护社会利益和有效监管。
...
### [459] [FlashDMoE: Fast Distributed MoE in a Single Kernel](https://arxiv.org/abs/2506.04667)
*Osayamen Jonathan Aimuyo,Byungsoo Oh,Rachee Singh*

Main category: cs.DC

TLDR: FlashDMoE是一种完全基于GPU的MoE算子，通过融合专家计算和GPU间通信，显著提升了GPU利用率和任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有MoE模型实现存在GPU利用率低、延迟高和无法利用任务局部性的问题，限制了大规模神经网络训练的扩展性。

Method: 开发了FlashDMoE，将专家计算和GPU间通信融合到单个持久GPU内核中，实现细粒度流水线调度和设备启动的高效通信协议。

Result: 在8-H100 GPU节点上，FlashDMoE实现了6倍延迟降低、5.7倍吞吐量提升、4倍弱扩展效率提升和9倍GPU利用率提升。

Conclusion: FlashDMoE表明，GPU内核与硬件的协同设计是解锁大规模分布式机器学习性能上限的关键。
...
### [460] [Solving engineering eigenvalue problems with neural networks using the Rayleigh quotient](https://arxiv.org/abs/2506.04375)
*Conor Rowan,John Evans,Kurt Maute,Alireza Doostan*

Main category: math.NA

TLDR: 该论文提出了一种基于神经网络离散化的特征值分析方法，结合Rayleigh商和Gram-Schmidt正交化，用于解决连续特征值问题。


<details>
  <summary>Details</summary>
Motivation: 特征值分析在工程中广泛应用，但在物理信息机器学习领域研究较少，尤其是神经网络离散化方法。

Method: 使用神经网络离散化特征函数，结合Rayleigh商和Gram-Schmidt正交化技术求解特征值问题。

Result: 该方法适用于不规则域上的调和函数、参数和非线性特征问题以及高维特征分析，展示了其独特优势。
...
### [461] [SSIMBaD: Sigma Scaling with SSIM-Guided Balanced Diffusion for AnimeFace Colorization](https://arxiv.org/abs/2506.04283)
*Junpyo Seo,Hanbin Koo,Jieun Yook,Byung-Ro Moon*

Main category: cs.GR

TLDR: 提出了一种基于扩散模型的自动上色框架SSIMBaD，用于动漫风格面部草图，通过SSIM引导的平衡扩散实现更一致的上色效果。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖预定义的噪声调度，可能导致感知不一致，因此需要一种更平衡且能保持结构保真度的上色方法。

Method: 采用连续时间扩散模型，引入SSIMBaD（基于SSIM的平衡扩散），通过sigma空间变换对齐感知退化。

Result: 在大规模动漫面部数据集上，该方法在像素准确性和感知质量上优于现有技术，并能适应多样风格。

Conclusion: SSIMBaD框架在保持结构保真度的同时，实现了更平衡和一致的上色效果。
...
### [462] [Handle-based Mesh Deformation Guided By Vision Language Model](https://arxiv.org/abs/2506.04562)
...
Method: 通过锥形奇点检测识别手柄，利用VLM选择变形子部分和手柄，并通过多视角投票减少不确定性。

Result: 在多个基准测试中，该方法更符合用户意图，且变形失真低。

Conclusion: 该方法无需训练、高度自动化，能持续生成高质量网格变形。
...
### [463] [VoxDet: Rethinking 3D Semantic Occupancy Prediction as Dense Object Detection](https://arxiv.org/abs/2506.04623)
*Wuyang Li,Zhu Yu,Alexandre Alahi*

Main category: cs.GR

TLDR: VoxDet提出了一种实例中心的3D语义占用预测框架，通过将体素级任务解耦为偏移回归和语义预测，显著提升了实例级完整性和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法将3D语义占用预测视为密集分割任务，忽略了实例级判别性，导致实例不完整和相邻模糊。作者发现体素级标签隐含实例信息，并利用这一观察提出改进。
...
Conclusion: VoxDet通过实例中心的方法显著提升了3D语义占用预测的性能和效率。
...
### [464] [A Fast Unsupervised Scheme for Polygonal Approximation](https://arxiv.org/abs/2506.04664)
*Bimal Kumar Ray*

Main category: cs.GR

TLDR: 提出了一种快速且无监督的多边形逼近闭合数字曲线的方案，速度优于现有技术，且在Rosin度量与美学方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决现有多边形逼近方法速度慢且美学表现不足的问题。

Method: 分三阶段：初始分割、迭代顶点插入、迭代合并与顶点调整。

Result: 方案速度快，在Rosin度量与美学上表现优秀，且对几何变换具有鲁棒性。

Conclusion: 该方案高效且美观，适用于实际应用。
...
### [465] [Interpretable LLMs for Credit Risk: A Systematic Review and Taxonomy](https://arxiv.org/abs/2506.04290)
...
<details>
  <summary>Details</summary>
Motivation: 探索LLM在信用风险评估中的应用，填补现有文献的系统性综述空白。

Method: 通过PRISMA研究策略筛选60篇相关论文（2020-2025年），分析模型架构、数据类型、可解释性机制和应用领域。

Result: 提出了一个分类法，总结了LLM在信用风险评估中的主要方法和挑战。

Conclusion: 本文为AI和金融研究者提供了参考，并指出了未来研究的趋势和空白。
...
TLDR: 提出了一种基于扩散变换器的去噪框架，用于快速生成高质量的质子治疗剂量图，适用于头颈部及其他癌症部位。


<details>
  <summary>Details</summary>
Motivation: 强度调制质子治疗（IMPT）对解剖变化敏感，需要快速准确的剂量计算，而减少蒙特卡洛模拟的粒子数会降低准确性。

Method: 开发了扩散变换器去噪框架，使用头颈部患者的IMPT计划和CT图像生成噪声和高统计剂量图，通过多种损失函数训练模型。

Result: 模型在不同癌症部位表现出低MAE和高Gamma通过率，剂量体积直方图（DVH）指标与真实值接近。

Conclusion: 该框架虽仅用头颈部数据训练，但在多种癌症部位表现良好，具有广泛适用性。
...
### [467] [An Expansion-Based Approach for Quantified Integer Programming](https://arxiv.org/abs/2506.04452)
*Michael Hartisch,Leroy Chew*

Main category: cs.DM

TLDR: 该论文提出了一种基于扩展的QIP求解方法，利用CEGAR技术，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: QIP结合了QBF和整数规划的优势，但现有方法仅探索了搜索范式，扩展范式尚未应用于QIP。

Method: 采用CEGAR技术，将QBF的扩展方法引入QIP，并嵌入优化框架。

Result: 实验表明，该方法在特定实例中优于现有搜索求解器，且在QBF扩展求解器中表现突出。

Conclusion: 扩展方法为QIP提供了新思路，尤其在多阶段鲁棒优化中具有潜力。
...
### [468] [GARG-AML against Smurfing: A Scalable and Interpretable Graph-Based Framework for Anti-Money Laundering](https://arxiv.org/abs/2506.04292)
...
<details>
  <summary>Details</summary>
Motivation: 洗钱占全球GDP的2%-5%，监管机构对金融机构实施严格管控。传统方法难以应对复杂的拆分交易（smurfing），且现有网络嵌入方法缺乏可解释性，阻碍了商业应用。

Method: GARG-AML通过分析每个节点的二阶交易网络结构，生成可解释的风险指标，并结合树方法及邻居节点分数提升检测能力。

Result: 实验表明，GARG-AML在大规模合成和开源网络中优于现有方法。

Conclusion: GARG-AML在计算效率、检测能力和透明度之间取得平衡，为欺诈检测提供了新思路。
...
### [469] [Can Artificial Intelligence Trade the Stock Market?](https://arxiv.org/abs/2506.04658)
*Jędrzej Maskiewicz,Paweł Sakowski*

Main category: q-fin.TR

TLDR: 论文探讨了深度强化学习（DRL）在股票市场交易中的应用，比较了DDQN和PPO算法与Buy and Hold基准的表现，结果显示DRL在风险调整收益上优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 研究DRL在金融市场交易中的有效性，特别是其风险管理能力。

Method: 使用DDQN和PPO算法，在多种资产（货币对、S&P 500指数、比特币）的日数据上进行测试。

Result: DRL在交易中表现优异，能通过策略性避免不利交易条件管理风险，风险调整收益优于监督学习方法。
...
### [470] [Deep Learning-Based Breast Cancer Detection in Mammography: A Multi-Center Validation Study in Thai Population](https://arxiv.org/abs/2506.03177)
...
TLDR: 该研究提出了一种基于改进EfficientNetV2架构的深度学习系统，用于乳腺X光检查中的乳腺癌检测，并在多个数据集上验证了其性能。


<details>
  <summary>Details</summary>
Motivation: 开发一种高效的乳腺癌检测系统，以辅助临床乳腺X光检查工作流程，提高检测准确性和效率。

Method: 使用改进的EfficientNetV2架构和增强的注意力机制，训练模型并在多个数据集上进行验证。

Result: 模型在癌症检测和病灶定位方面表现优异，AUROCs分别为0.89、0.96和0.94，临床验证显示与放射科医生的一致性高。

Conclusion: 该系统在乳腺癌检测中表现出色，具有临床应用潜力，可提升筛查效率。
...
### [471] [Gradient Inversion Attacks on Parameter-Efficient Fine-Tuning](https://arxiv.org/abs/2506.04453)
*Hasin Us Sami,Swapneel Sen,Amit K. Roy-Chowdhury,Srikanth V. Krishnamurthy,Basak Guler*

Main category: eess.IV

TLDR: 论文研究了联邦学习中参数高效微调（PEFT）的隐私风险，展示了通过恶意设计的预训练模型和适配器模块，攻击者可以利用梯度反演攻击重构用户数据。


<details>
  <summary>Details</summary>
Motivation: 探讨在联邦学习中，参数高效微调（PEFT）如何可能因预训练模型和适配器模块的设计不当而泄露用户隐私。
...
### [472] [A Poisson-Guided Decomposition Network for Extreme Low-Light Image Enhancement](https://arxiv.org/abs/2506.04470)
...
Method: 集成Retinex分解与泊松去噪的编码器-解码器网络，通过泊松去噪损失抑制噪声并增强光照。

Result: 显著提升低光条件下的可见度与亮度，保持图像结构和颜色一致性。
...
### [473] [DACN: Dual-Attention Convolutional Network for Hyperspectral Image Super-Resolution](https://arxiv.org/abs/2506.05041)
*Usman Muhammad,Jorma Laaksonen*

Main category: eess.IV

TLDR: DACN是一种双注意力卷积网络，通过结合多头注意力和通道注意力机制，解决了2D CNN在超分辨率任务中缺乏全局上下文理解的问题，并提升了性能。
...
Method: DACN采用增强卷积和多头注意力捕获局部和全局特征依赖，同时为通道和空间维度推断注意力图，并提出结合L2正则化和空间-光谱梯度损失的优化损失函数。

Result: 在两个超光谱数据集上的实验表明，多头注意力和通道注意力的结合优于单独使用任一机制。
...
### [474] [DM-SegNet: Dual-Mamba Architecture for 3D Medical Image Segmentation with Global Context Modeling](https://arxiv.org/abs/2506.05297)
*Hangyu Ji*

Main category: eess.IV

TLDR: DM-SegNet提出了一种双Mamba架构，结合方向性状态转换和解剖感知分层解码，解决了现有医学SSM中编码器-解码器不兼容的问题，并在两个临床数据集上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有医学SSM在3D医学图像分割中存在编码器-解码器不兼容的问题，1D序列扁平化破坏了空间结构，而传统解码器无法利用Mamba的状态传播能力。

Method: DM-SegNet通过四方向3D扫描的Mamba模块、门控空间卷积层和Mamba驱动的解码框架，实现了空间拓扑保持和全局上下文建模。

Result: 在Synapse数据集上达到85.44%的DSC（腹部器官分割），在BraTS2023数据集上达到90.22%的DSC（脑肿瘤分割）。

Conclusion: DM-SegNet通过创新的双Mamba架构和分层解码，显著提升了3D医学图像分割的性能。
...
### [475] [Tech-ASan: Two-stage check for Address Sanitizer](https://arxiv.org/abs/2506.05022)
*Yixuan Cao,Yuhong Feng,Huafeng Li,Chongyi Huang,Fangcao Jian,Haoran Li,Xu Wang*

Main category: cs.SE

TLDR: Tech-ASan是一种基于两阶段检查的技术，旨在加速Address Sanitizer（ASan）的运行，同时确保安全性，减少运行时开销和冗余检查。


<details>
  <summary>Details</summary>
Motivation: ASan在检测C/C++程序中的内存安全违规时存在显著的运行时开销，限制了其在大规模软件测试中的效率。现有方法未能有效消除冗余检查或牺牲检测能力。

Method: 提出两阶段检查算法，利用魔法值比较减少昂贵的影子内存访问；设计优化器消除冗余检查，包括循环中的检查；基于LLVM实现Tech-ASan。

Result: 在SPEC CPU2006基准测试中，Tech-ASan的运行时开销比ASan和ASan--分别减少33.70%和17.89%；在Juliet测试套件中，检测到的假阴性案例减少56个。

Conclusion: Tech-ASan在减少运行时开销和冗余检查的同时，保持了检测能力，是一种高效的内存安全工具。
...
### [476] [A Multi-Dataset Evaluation of Models for Automated Vulnerability Repair](https://arxiv.org/abs/2506.04987)
*Zanis Ali Khan,Aayush Garg,Qiang Tang*

Main category: cs.SE

TLDR: 研究探讨了预训练语言模型CodeBERT和CodeT5在自动化漏洞修补中的表现，发现CodeBERT在稀疏上下文表现更好，而CodeT5在复杂漏洞模式捕获和扩展性上更优。微调模型在已知数据上表现提升，但对未知数据泛化能力有限。


<details>
  <summary>Details</summary>
Motivation: 软件漏洞对安全构成重大威胁，而自动化漏洞修补（APR）在这一安全关键领域的探索不足。

Method: 研究使用CodeBERT和CodeT5模型，在六个数据集和四种语言中评估其漏洞修补的准确性和泛化能力。

Result: CodeBERT在稀疏上下文表现更好，CodeT5在复杂漏洞模式和扩展性上更优。微调模型在已知数据上表现提升，但对未知数据泛化能力有限。

Conclusion: 研究为自动化漏洞修补提供了性能基准和实际应用中的改进方向，强调了泛化能力的挑战。
...
### [477] [Leveraging Reward Models for Guiding Code Review Comment Generation](https://arxiv.org/abs/2506.04464)
*Oussama Ben Sghaier,Rosalia Tufano,Gabriele Bavota,Houari Sahraoui*

Main category: cs.SE

TLDR: CoRAL是一个基于深度学习的框架，利用强化学习自动生成代码审查评论，通过奖励机制评估评论的语义和实用性。


<details>
  <summary>Details</summary>
Motivation: 代码审查耗时且受主观因素影响，自动化技术如深度学习可以提升效率。

Method: CoRAL结合强化学习，奖励机制考虑评论的语义和实用性，并与代码优化模型联动。
...
### [478] [Deep learning image burst stacking to reconstruct high-resolution ground-based solar observations](https://arxiv.org/abs/2506.04781)
*Christoph Schirninger,Robert Jarolim,Astrid M. Veronig,Christoph Kuckein*

Main category: astro-ph.SR

TLDR: 论文提出了一种基于深度学习的实时图像重建方法，用于解决地面太阳望远镜观测中因大气湍流导致的图像退化问题。
...
Result: 该方法在感知质量上表现出更高的鲁棒性，尤其在斑点重建存在伪影时效果显著。

Conclusion: 该方法能高效利用图像信息，在提供完整图像序列时实现最佳重建效果。
...
### [479] [Spore in the Wild: Case Study on Spore.fun, a Real-World Experiment of Sovereign Agent Open-ended Evolution on Blockchain with TEEs](https://arxiv.org/abs/2506.04236)
*Botao Amber Hu,Helena Rong*

Main category: cs.MA

TLDR: 探讨基于开放系统的ALife研究，通过Spore.fun案例研究，分析是否能在无许可计算基底和经济激励下实现持续开放演化（OEE）。


<details>
  <summary>Details</summary>
Motivation: 传统封闭系统模拟（如Tierra和Avida）难以实现持续开放演化（OEE），需开放系统与环境持续交互。

Method: 利用DePIN技术和区块链集成TEEs，部署自主运行的LLM-based AI代理，研究Spore.fun实验中的代理行为和演化轨迹。

Result: 通过数字行为学分析，展示代理在开放系统中的演化潜力。

Conclusion: 开放ALife系统可能通过无许可计算基底和经济激励实现OEE，Spore.fun为这一目标提供了初步实证。
...
### [480] [CORA: Coalitional Rational Advantage Decomposition for Multi-Agent Policy Gradients](https://arxiv.org/abs/2506.04265)
*Mengda Ji,Genjiu Xu,Liying Wang*

Main category: cs.MA

TLDR: 本文提出了一种名为CORA的信用分配方法，通过联盟层面的视角解决多智能体强化学习中的信用分配问题，优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 共享全局优势在多智能体强化学习中可能导致次优策略更新，因为未能区分智能体的个体贡献。现有方法缺乏对联盟层面的详细分析。
...
### [481] [Autonomous Collaborative Scheduling of Time-dependent UAVs, Workers and Vehicles for Crowdsensing in Disaster Response](https://arxiv.org/abs/2506.04276)
*Lei Han,Yitong Guo,Pengfei Yang,Zhiyong Yu,Liang Wang,Quan Wang,Zhiwen Yu*

Main category: cs.MA

TLDR: 论文提出HoAs-PALN算法，通过自适应降维和局部纳什均衡游戏实现多智能体协作调度，显著提升灾后环境信息采集效率。


<details>
  <summary>Details</summary>
Motivation: 灾后环境复杂，现有传感技术适应性差、能力不足，亟需高效调度方案。

Method: HoAs-PALN结合自适应降维匹配和局部纳什均衡游戏，优化多智能体协作调度。

Result: 实验显示HoAs-PALN任务完成率显著高于基线方法，决策时间短于10秒。
...
### [482] [Time to Talk: LLM Agents for Asynchronous Group Communication in Mafia Games](https://arxiv.org/abs/2506.05309)
*Niv Eckhaus,Uri Berger,Gabriel Stanovsky*

Main category: cs.MA

TLDR: 论文提出了一种异步LLM代理，能够决定何时发言，并在在线Mafia游戏中表现与人类相当。
...
Conclusion: 研究为LLM在复杂社交场景中的应用铺平道路，支持进一步研究异步通信。
