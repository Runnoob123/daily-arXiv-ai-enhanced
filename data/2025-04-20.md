...
Motivation: 去中心化系统（如区块链）需要数学工具来建模和分析其行为，尤其是避免分叉等关键问题。

Method: 引入“半拓扑”概念，将参与者视为空间中的点，研究其“可行动联盟”的性质。

Result: 证明了一种数学意义：如果两个点的可行动联盟交集非空，则它们必须保持一致。这在分布式系统中是关键正确性属性。

Conclusion: 半拓扑为去中心化系统提供了一种新的数学建模工具，有助于分析和设计避免分叉等问题的系统。

Abstract: Blockchains and peer-to-peer systems are part of a trend towards computer
systems that are "radically decentralised", by which we mean that they 1) run
across many participants, 2) without central control, and 3) are such that
qualities 1 and 2 are essential to the system's intended use cases.
  We propose a notion of topological space, which we call a "semitopology", to
help us mathematically model such systems. We treat participants as points in a
space, which are organised into "actionable coalitions". An actionable
coalition is any set of participants who collectively have the resources to
collaborate (if they choose) to progress according to the system's rules,
without involving any other participants in the system.
  It turns out that much useful information about the system can be obtained
\emph{just} by viewing it as a semitopology and studying its actionable
coalitions. For example: we will prove a mathematical sense in which if every
actionable coalition of some point p has nonempty intersection with every
actionable coalition of another point q -- note that this is the negation of
the famous Hausdorff separation property from topology -- then p and q must
remain in agreement.
  This is of practical interest, because remaining in agreement is a key
correctness property in many distributed systems. For example in blockchain,
participants disagreeing is called "forking", and blockchain designers try hard
to avoid it.
  We provide an accessible introduction to: the technical context of
decentralised systems; why we build them and find them useful; how they
motivate the theory of semitopological spaces; and we sketch some basic
theorems and applications of the resulting mathematics.

</details>

### [2] [Provable Secure Steganography Based on Adaptive Dynamic Sampling](https://arxiv.org/abs/2504.12579)
*Kaiyi Pang*
...
<details>
  <summary>Details</summary>
Motivation: 当前可证明安全隐写（PSS）方法需要显式访问生成模型分布，限制了黑盒场景的实用性。

Method: 采用动态采样策略，允许生成模型在不干扰正常生成过程的情况下嵌入秘密消息。

Result: 在三个真实数据集和三个LLM上的评估表明，该方法在效率和容量上与现有白盒方法相当，且避免了隐写对模型输出的影响。

Conclusion: 提出的黑盒隐写方案解决了现有PSS方法的局限性，具有实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 解决同态加密环境下神经网络训练的扩展性和梯度消失问题，提升计算与内存效率。

Method: 用Sigmoid和BCE损失函数替代Softmax，改进数据编码方案Double Volley Revolver。

Result: 实现了非交互式单输出和多输出分类任务，解决了梯度消失问题，提升了训练效率。

Conclusion: 提出的方法扩展了同态加密在神经网络中的应用，并提供了开源实现。

Abstract: In this paper, we present the demonstration of training a four-layer neural
...
<details>
  <summary>Details</summary>
Motivation: 随着区块链技术的发展，智能合约的安全问题日益突出，恶意代码可能导致用户资产损失和系统崩溃。本文旨在通过机器学习方法提升智能合约恶意代码的检测效果。

Method: 采用基于智能合约文本的Opcode向量化方法，结合N-Gram和TF-IDF算法进行特征提取，并通过机器学习模型训练和比较不同方法的性能。

Result: 通过实验比较了直接量化Opcode与分类简化后再量化的方法，结果表明后者在检测效果上更优。

Conclusion: 基于Opcode分类简化的特征提取方法在智能合约恶意代码检测中表现更好，为未来研究提供了参考。

Abstract: With the booming development of blockchain technology, smart contracts have
...
<details>
  <summary>Details</summary>
Motivation: 研究分布式系统中对抗行为的潜在影响，特别是区块链网络的公平性攻击及其副作用。

Method: 通过分类和约束对抗行为，基于目标协议的假设（如故障模型、通信模型和BFT协议的容错阈值），构建仿真框架分析攻击效果。

Result: 仿真显示，即使受限的对手也能破坏客户端公平性和订单公平性，但提出的缓解机制能有效防御交易重排序攻击。

Conclusion: 本文扩展了先前研究，提出并验证了一种针对交易重排序攻击的防御机制，展示了其在不同条件下的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究仅从攻击者角度分析成本、损害或时间等指标，忽略了防御者的有限资源，导致系统视图不完整。

Method: 提出将攻击者和防御者指标编码为半环，采用自底向上方法分析树状ADTs，并将一般ADTs转换为二元决策图进行分析。

Result: 实验表明，两种方法均能有效处理数百个节点的ADTs。

Conclusion: 新框架弥补了现有研究的不足，为网络安全风险管理提供了更全面的视角。
...
<details>
  <summary>Details</summary>
Motivation: 随着Agentic AI的普及，模型能力提升迅速，但系统仍受限于数据孤岛，且集成需要难以扩展的自定义逻辑。MCP解决了这一问题，但其灵活性引入了新的安全风险。

Method: 提出MCP Guardian框架，通过多种安全机制（如认证、限流、日志记录等）加强MCP通信的安全性。

Result: 通过实际场景和实证测试，MCP Guardian有效抵御攻击，确保稳健监管且开销极小。

Conclusion: MCP Guardian为AI助手提供了安全、可扩展的数据访问，强调了深度防御方法在AI驱动环境中的重要性。
...
<details>
  <summary>Details</summary>
Motivation: EMV非接触支付的普及带来了便利，但也暴露了严重的安全挑战，需要系统性的漏洞分析和解决方案。

Method: 通过实验平台复制Visa和Mastercard协议的攻击，评估其可行性，并对协议进行详细分析和比较。

Result: 研究发现七种攻击向量，揭示了当前非接触支付的安全现状，并提出了漏洞和应对措施。

Conclusion: 研究为EMV非接触支付的安全性提供了深入见解，并提出了改进建议。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM的安全机制容易被绕过，亟需研究系统化的攻击方法以发现漏洞。

Method: 利用图结构表示恶意提示，通过AMR和RDF解析语义并进行转换，生成代码实现攻击。

Result: 实验显示该方法对主流商业LLM的成功率达87%，上下文框架和抽象化是绕过安全措施的关键。

Conclusion: 研究为开发更鲁棒的安全机制提供了理论和实践基础，揭示了语义攻击的潜在威胁。

Abstract: Large Language Models (LLMs) have been equipped with safety mechanisms to
...
Method: 提出HSDL方法分析专家协作模式，并设计CAEP算法剪枝低贡献专家。

Result: 协作模式与输入类型相关，CAEP算法平均提升模型性能2.5%。
...
Method: 修改LoRA框架，仅对调用aLoRA后的序列令牌进行权重适应，保留基础模型的KV缓存。

Result: aLoRA在保持与标准LoRA竞争性准确度的同时，显著提升推理效率。

Conclusion: aLoRA为构建高度专业化的模型（intrinsics）提供了高效解决方案。
...
### [12] [Standardization of Multi-Objective QUBOs](https://arxiv.org/abs/2504.12419)
...
Result: 实证表明，该方法能更平衡地找到解，且简化权重选择过程。

Conclusion: 该方法有效解决了多目标QUBO问题中的尺度不平衡问题，提升了优化效率。
...
<details>
  <summary>Details</summary>
Motivation: AI系统的不透明性对信任和接受度构成挑战，因此需要开发可解释的模型。

Method: 提出了一种系统方法，通过分析神经网络的分布式表示来识别符号组件，并将其映射到决策树中。

Result: 原型验证了从神经网络中提取符号表示的可行性，提高了AI系统的透明度和可问责性。
...
<details>
  <summary>Details</summary>
Motivation: 探索Moran特征向量空间滤波方法能否扩展到机器学习中，以处理空间效应。

Method: 生成合成数据集，测试不同空间权重矩阵的Moran特征向量，评估多种机器学习模型性能，并与仅使用坐标的模型对比。

Result: 基于坐标的模型表现优于特征向量方法，但在负自相关或网络自相关情况下特征向量仍有效。

Conclusion: Moran特征向量在特定空间自相关情况下仍具价值，但需根据场景选择方法。
...
<details>
  <summary>Details</summary>
Motivation: 确保机器学习模型在预测时不因性别、种族等受保护属性对边缘群体产生不利影响。

Method: 扩展梯度提升方法，结合分类/回归损失和最小-最大公平项，通过原始-对偶问题优化。

Result: 理论证明算法在温和条件下收敛，实验显示其能有效处理二元和子群公平性问题。

Conclusion: 提出的最小-最大原始-对偶梯度提升算法是一种灵活且强大的公平性优化方法。
...
<details>
  <summary>Details</summary>
Motivation: Mixture of Experts (MoE) 预训练因稀疏更新导致训练不稳定和性能不佳，需要改进。

Method: 提出Default MoE方法，用专家输出的指数移动平均替代缺失的激活，使路由器接收每个专家的信号。

Result: Default MoE在多种设置中优于标准TopK路由，且计算开销小。

Conclusion: Default MoE显著提升训练性能，无需额外计算成本。
...
<details>
  <summary>Details</summary>
Motivation: 传统联邦学习聚合方法依赖客户端报告的数据量，易受不诚实客户端影响，导致模型偏差。

Method: 提出FedDua方法，通过客户端上传的梯度预测其真实数据量，并集成到现有联邦学习算法中。

Result: 在三个基准数据集上，FedDua平均提升全局模型性能3.17%，优于四种流行聚合方法。

Conclusion: FedDua能有效解决数据量不准确问题，提升联邦学习模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有Transformer-based Gröbner基计算中数据集缺乏理论保证的问题，并提升训练效果。

Method: 提出并证明数据集生成算法的通用性，扩展算法以系统化生成理想生成器的数据集。

Result: 证明了生成的数据集足够通用，确保Transformer能学习多样化的Gröbner基。
...
<details>
  <summary>Details</summary>
Motivation: 基础模型规模增大导致微调在商用设备上内存带宽不足，现有方法计算成本高且负载不均衡。

Method: 提出D2FT框架，动态选择注意力模块，结合多背包优化策略优化分布式计算负载。

Result: D2FT降低40%计算成本和50%通信成本，准确率仅下降1%-2%；在LoRA技术上扩展后，准确率下降4%-6%。

Conclusion: D2FT高效解决了基础模型微调的计算和通信问题，适用于多种微调技术。
...
<details>
  <summary>Details</summary>
Motivation: RLHF已成为部署最新机器学习系统的重要工具，本书旨在为读者提供对其核心方法的入门介绍。

Method: 从RLHF的起源和理论基础入手，逐步介绍问题定义、数据收集、优化阶段（如奖励模型训练、强化学习算法等）。

Result: 详细解析了RLHF的各个优化阶段，并探讨了合成数据和评估等前沿问题。

Conclusion: 本书总结了RLHF的核心技术，并提出了该领域的未解决问题和未来研究方向。
...
<details>
  <summary>Details</summary>
Motivation: 对抗攻击可能导致自动驾驶车辆感知模块的分类错误，研究旨在探索更鲁棒的模型以应对此类攻击。

Method: 使用AlexNet和VGG-16作为特征提取器，构建HCQ-DL模型，并测试了1000多种量子电路，评估其在PGD、FGSA和GA攻击下的表现。

Result: HCQ-DL模型在无攻击场景下准确率超过95%，在GA和FGSA攻击下超过91%，优于C-DL模型；在PGD攻击下，HCQ-DL准确率为85%，而C-DL低于21%。

Conclusion: HCQ-DL模型在对抗攻击下表现显著优于经典模型，为自动驾驶感知模块提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 工程问题中机器学习模型需要适应不断更新的数据，但高计算成本使得从头训练不可行。持续学习能够在不遗忘旧知识的情况下学习新数据。

Method: 在五个工程数据集上应用多种CL方法，构建了九个新的工程CL基准，评估其抗遗忘和泛化能力。

Result: 初步结果显示，CL方法（如Replay策略）性能优于简单基线，接近从头训练的效果，同时训练时间减少近半。

Conclusion: 持续学习在工程设计中具有实际应用潜力，Replay策略尤其适合现实工程工作流程。
...
<details>
  <summary>Details</summary>
Motivation: 长上下文语言模型在推理时GPU内存需求高，部署困难，需要一种高效的内存优化方法。

Method: MOM将关键层分割为“小序列”，并与KV缓存卸载无缝集成。

Result: 实验显示MOM平均降低50%峰值内存使用，将最大上下文长度从155k扩展到455k，且不影响准确性。

Conclusion: MOM消除了预填充阶段的内存瓶颈，将研究重点转向解码阶段的KV缓存效率优化。
...
<details>
  <summary>Details</summary>
Motivation: VQNN的损失景观存在指数级增长的局部极小值，导致从梯度中恢复信息比经典神经网络更具挑战性。

Method: 结合梯度估计与有限差分法和自适应低通滤波的梯度反演方案，并进一步用卡尔曼滤波优化收敛效率。

Result: 实验表明，算法可以反演批量训练的数据，前提是VQNN模型足够过参数化。

Conclusion: 该方法为从VQNN梯度中恢复输入数据提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究扩散模型如何泛化到训练集之外，尽管其训练目标的最优解是训练分布的分值函数。

Method: 利用物理启发的路径积分方法，分析欠参数化和过参数化扩散模型学习到的分布。

Result: 扩散模型学习的分布填补了训练分布的“空白”，这是由训练目标噪声的协方差结构引起的。

Conclusion: 扩散模型的泛化能力源于训练目标的噪声特性及其与特征相关归纳偏置的相互作用。
...
<details>
  <summary>Details</summary>
Motivation: 在安全强化学习中，安全约束通常未知或难以明确指定，需要从稀疏标注数据中学习。

Method: 设计了安全模型进行信用分配，估计每个决策步骤对整体安全的影响，并提出了相应的安全强化学习算法。

Result: 实验结果表明，该方法能有效满足未知安全定义，并适用于多种连续控制任务。

Conclusion: 该方法通过学习安全模型，能够在未知安全约束下实现安全且高效的决策。
...
<details>
  <summary>Details</summary>
Motivation: 洪水是最常见的自然灾害，准确的洪水预测对早期预警系统至关重要。现有机器学习模型需要全球数据训练，但国家预报员难以调整模型以适应本地需求，且本地数据对提升性能有价值。

Method: 提出一种方法：先在大型全球数据集上预训练模型，然后在单个流域的本地数据上进行微调。

Result: 结果显示性能提升，尤其是在全局训练中表现不佳的流域。

Conclusion: 该方法为国家预报员提供了一种利用本地数据优化全球模型的路径，降低了机器学习水文预测系统的部署门槛。
...
<details>
  <summary>Details</summary>
Motivation: 解决 Hebbian 学习在 Hopfield 网络中的容量限制问题，并探索更高效的核方法。

Method: 提出使用 Kernel Ridge Regression (KRR) 作为替代方法，通过闭式解非迭代学习对偶变量。

Result: KRR 实现了与 KLR 相当的高存储容量（比例达 1.5）和噪声鲁棒性（从约 80% 的损坏模式中恢复），同时大幅减少训练时间。
...
<details>
  <summary>Details</summary>
Motivation: 策略梯度方法在利用上表现优异但探索不足，进化计算擅长探索但缺乏利用机制，EPO旨在结合两者优势。

Method: 提出EPO算法，整合神经进化和策略梯度方法，利用EC的探索能力和PG的利用能力。

Result: 在Atari Pong和Breakout上，EPO在策略质量和样本效率上均优于标准PG和EC方法。

Conclusion: EPO有效解决了需要探索和局部优化的任务，展示了其优越性。
...
<details>
  <summary>Details</summary>
Motivation: 现有OSSL方法在处理未标记数据时，可能排除、干扰或过度信任OOD样本，导致特征空间次优。

Method: MagMatch通过原型对比学习隔离OOD样本，使用ID-Selective Magnetic模块选择性对齐ID样本，并提出SMA损失动态调整未标记数据的对齐。

Result: 在多样数据集上，MagMatch在闭集分类准确率和OOD检测AUROC上显著优于现有方法，尤其是对未见OOD数据的泛化能力。

Conclusion: MagMatch通过自然隔离OOD样本和动态对齐ID样本，有效提升了开放集半监督学习的性能。
...
<details>
  <summary>Details</summary>
Motivation: 燃烧化学动力学模型推断因大ODE系统和时间尺度差异大而具有挑战性，机器学习方法受非线性和噪声数据限制。

Method: 通过引入物理知识（动力学和热力学定律）及元素守恒损失项，改进KAN-ODEs框架，提出ChemKANs。

Result: ChemKANs在稀疏和噪声数据下无过拟合，参数稀疏（344参数）且预测精度高，氢燃烧化学模拟速度提升2倍。

Conclusion: ChemKANs在燃烧物理和化学动力学中具有潜力，KAN-ODEs框架可扩展至更大规模问题。
...
<details>
  <summary>Details</summary>
Motivation: 传统决策树仅优化目标属性的信息增益，可能导致对受保护社会群体的不公平歧视。

Method: 提出GroupCART，优化目标属性和保护属性的熵，避免偏见，支持自定义权重。

Result: 实验显示GroupCART无需数据转换即可实现公平模型，性能损失小，支持灵活权衡。

Conclusion: 通过多任务公平感知学习，可有效减轻决策树模型的算法偏见。
...
Result: 在多种图数据集上表现显著提升，图同构表达能力强。

Conclusion: 简单修改即可提升Transformer在图学习中的性能，无需复杂架构。
...
Method: 提出Beta-Binomial分布模型、自适应停止策略和先验转移机制，整合为BetaConform框架。

Result: 在TruthfulQA数据集上，仅需10个样本，BetaConform能将误差控制在3.37%以内。

Conclusion: BetaConform为LLM集成判断提供了高效且理论保证的性能评估方法。
...
Method: 利用几何直观和信息投影，提出了一种简单计算“元依赖”的方法，并通过合成和真实数据验证。

Result: 提供了一种衡量条件独立性之间“元依赖”的方法，并实证验证了其有效性。

Conclusion: 通过几何和信息投影的方法，可以有效理解和计算条件独立性之间的“元依赖”关系。
...
<details>
  <summary>Details</summary>
Motivation: 现有SGD收敛分析通常依赖强假设和特定步长条件，限制了实际应用。本文旨在填补这一空白。

Method: 采用停止时间方法，分析SGD在非凸设置下的收敛性，步长条件为∑ε_t=∞且∑ε_t^p<∞（p>2）。

Result: 证明了SGD在几乎必然收敛和L2收敛下的结果，放宽了损失函数的全局Lipschitz连续性和随机梯度高阶矩的有界性假设。

Conclusion: 本文的理论结果更具普适性，提升了SGD在实际应用中的适用性。
...
TLDR: 本文综述了基于机器学习的基因调控网络（GRN）推断方法，重点探讨了深度学习技术的应用及其未来发展方向。


<details>
  <summary>Details</summary>
Motivation: 基因调控网络（GRN）是研究基因表达调控的关键系统，而现代计算生物学和高通量测序技术的发展为GRN推断提供了新机遇。本文旨在总结机器学习方法在GRN推断中的应用，并探讨其未来潜力。

Method: 综述了包括监督、无监督、半监督和对比学习在内的多种机器学习技术，用于分析大规模组学数据并揭示基因调控关系。

Result: 文章总结了当前GRN推断的常用数据集、评估指标及深度学习技术的应用效果。

Conclusion: 深度学习技术显著提升了GRN推断的性能，未来研究方向包括进一步优化算法和整合多源数据。
...
<details>
  <summary>Details</summary>
Motivation: GNN在材料发现中表现优异，但对域外数据的预测不可靠且难以识别，因此需要有效的UQ方法。

Method: 将DPOSE（一种轻量级UQ技术）集成到SchNet模型中，评估其在QM9、OC20和Gold Molecular Dynamics等数据集上的表现。

Result: DPOSE能有效区分域内和域外样本，对未观测到的分子和材料类别表现出更高的不确定性。

Conclusion: 轻量级UQ方法（如DPOSE）可提升GNN的鲁棒性，并为未来与主动学习策略的结合奠定基础。
...
<details>
  <summary>Details</summary>
Motivation: 在PU学习中，传统特征选择方法将未标记数据视为负样本，可能无法捕捉正样本的统计特性，导致性能不佳。

Method: FSCPU将特征选择问题建模为二元优化任务，目标函数明确结合了PU学习中的聚类假设。

Result: 在合成数据集和三个公开数据集上的实验表明，FSCPU在不同数据条件下表现优异，且在下游分类任务中具有竞争力。

Conclusion: FSCPU在PU学习场景中有效，即使聚类假设不完全成立，仍能取得良好效果。
...
Method: 通过多模态推理驱动的提示重写框架VLMGuard-R1，动态解析文本与图像的交互，生成更安全的提示，而无需修改模型核心参数。

Result: 在三个基准测试和五种VLMs上的实验表明，VLMGuard-R1显著提升了安全性，尤其在SIUO基准上平均安全性提高了43.59%。

Conclusion: VLMGuard-R1为视觉语言模型的安全对齐提供了一种有效且无需修改模型的新方向。
...
<details>
  <summary>Details</summary>
Motivation: 驾驶员对风险的感知影响其对自动驾驶系统（ADSs）的接受、信任和使用，但现有方法难以评估这种主观感知风险。

Method: 提出DSPR模型，采用卷积神经网络和双向长短期记忆网络（CNN-Bi-LSTM-TPA）结合半监督学习策略，预测驾驶员实时风险评分。

Result: DSPR模型预测准确率达87.91%，半监督策略提升20.12%，CNN-Bi-LSTM-TPA网络在四种LSTM结构中表现最佳。
...
Result: 在合成和真实数据实验中，MPOCtrL能有效检测数据与物理属性的非线性关系，尤其在代谢通量分析任务中表现优于现有方法。

Conclusion: MPOCtrL为数据稀缺或高维系统提供了一种有效的解决方案，扩展了PINN的应用范围。
...
<details>
  <summary>Details</summary>
Motivation: 探讨在多任务持续学习中，梯度下降如何影响分类器的收敛方向，以及任务顺序和对齐如何影响遗忘和知识转移。

Method: 通过固定迭代次数的梯度下降在多任务上顺序训练，分析任务顺序（循环/随机）对分类器方向收敛和遗忘的影响。

Result: 任务联合可分时，分类器方向收敛于联合最大间隔解；任务对齐影响遗忘和知识转移；任务不可分时，收敛于联合损失函数的最小值。

Conclusion: 任务顺序和对齐在多任务持续学习中起关键作用，梯度下降能有效收敛于联合解，任务对齐减少遗忘。
...
Method: 使用向量量化增强图自编码器，提出退火编码策略解决码本利用率低的问题，并引入分层码本解决码本空间稀疏性。

Result: 模型在自监督链接预测和节点分类任务中优于16种基线方法。

Conclusion: 向量量化显著提升了图自编码器的拓扑捕捉能力，提出的方法有效解决了码本利用率和稀疏性问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有LTSF模型过于复杂，而简单模型如线性模型或MLP表现更优，需重新组织核心思想以提升效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有多标签特征选择方法主要关注全局特征，忽视了标签的个性化需求，限制了性能。

Method: GPMFS首先利用标签相关性识别全局特征，然后通过阈值控制策略为每个标签补充个性化特征子集。

Result: 实验表明GPMFS在多个真实数据集上表现优异，兼具可解释性和鲁棒性。

Conclusion: GPMFS验证了个性化特征选择方法的必要性及其潜在应用价值。
...
TLDR: 本文提出了一种名为DEPOSITUM的去中心化非凸复合联邦学习算法，通过近似全局梯度和动量更新减少数据异构性和通信成本，理论分析显示其高效性。


<details>
  <summary>Details</summary>
Motivation: 传统联邦学习依赖服务器-客户端架构，而DNCFL问题尚未充分研究，需要解决非凸复合优化问题。

Method: 基于近端随机梯度跟踪，引入动量更新以减少随机梯度方差，支持本地更新降低通信成本。

Result: 理论证明DEPOSITUM达到ε-稳定点，迭代复杂度为O(1/ε²)，误差以O(1/T)速率下降，实验验证其有效性。

Conclusion: DEPOSITUM在非凸复合优化中表现优异，适用于实际神经网络训练。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLMs在图推理任务中表现不一，缺乏系统性评估方法，因此需要开发一个全面的基准框架。

Method: 通过分析图类型、序列化格式和提示方案，提出基于强化学习的动态选择方法。
...
<details>
  <summary>Details</summary>
Motivation: 稀疏神经网络从头训练（PaI）与密集到稀疏训练的性能差距是高效深度学习的障碍，需解决参数初始化问题。

Method: 提出Sign-In方法，动态重参数化以诱导符号翻转，弥补PaI的不足。

Result: 实验和理论表明Sign-In能提升PaI性能，但仍需进一步缩小与密集到稀疏训练的差距。

Conclusion: Sign-In是解决稀疏训练性能差距的有效方法，但仍有挑战需攻克。
...
<details>
  <summary>Details</summary>
Motivation: 解决群智能算法因配置和超参数不明确导致的低可靠性问题。

Method: 使用IOHxplainer工具分析不同通信拓扑对信息流、多样性和收敛速度的影响。

Result: 研究增强了PSO决策的可解释性，并提供了选择拓扑的实用建议。

Conclusion: 提升了群智能优化的透明性、鲁棒性和可信度。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在利用高频数据和策略性追踪捕捉短期市场波动，规避印度市场的卖空限制，实现超额收益。

Method: 基于NIFTY 50指数前50只股票的5分钟K线数据，结合技术指标和超参数（如追踪止损值），开发交易策略。

Result: PDT模型在12天测试期内实现1.3468%的收益，优于LSTM（0.1238%）和RNN（0.3096%），且均跑赢买入持有策略（亏损2.2508%）。

Conclusion: PDT模型在高频交易中表现优异，具有超越市场平均和风险无风险收益的潜力。
...
### [53] [FedX: Adaptive Model Decomposition and Quantization for IoT Federated Learning](https://arxiv.org/abs/2504.12849)
*Phung Lai,Xiaopeng Jiang,Hai Phan,Cristian Borcea,Khang Tran,An Chen,Vijaya Datta Mayyuri,Ruoming Jin*

Main category: cs.LG

TLDR: FedX是一种新型的联邦学习系统，通过自适应模型分解和量化技术，在资源受限的IoT设备上实现高效训练。


<details>
  <summary>Details</summary>
Motivation: 解决联邦学习在异构、资源受限的移动/IoT设备上模型效用与计算/通信开销之间的平衡问题。

Method: 将全局模型分解为不同子网络，并根据设备资源动态调整量化位数；量化操作在服务器端完成以减少设备计算负担。

Result: 实验表明，FedX显著提升了量化效率（8.43倍）、设备计算时间（1.5倍）和端到端训练时间（1.36倍）。

Conclusion: FedX通过结合模型量化与联邦学习，在保证模型收敛的同时，实现了资源受限设备上的高效训练。
...
### [54] [iHHO-SMOTe: A Cleansed Approach for Handling Outliers and Reducing Noise to Improve Imbalanced Data Classification](https://arxiv.org/abs/2504.12850)
*Khaled SH. Raslan,Almohammady S. Alsharkawy,K. R. Raslan*

Main category: cs.LG

TLDR: iHHO-SMOTe方法通过数据清洗和特征选择改进SMOTE，有效处理不平衡数据集中的噪声和异常值，显著提升分类性能。


<details>
  <summary>Details</summary>
Motivation: 解决不平衡数据集中噪声和异常值对SMOTE生成新样本的负面影响，提升分类器性能。

Method: 结合随机森林特征选择和DBSCAN异常值检测，先清洗数据，再用iHHO-SMOTe进行过采样。

Result: AUC超过0.99，G-means为0.99，F1-score持续高于0.967，表现优异。

Conclusion: iHHO-SMOTe通过噪声和异常值处理，成为处理不平衡数据集的强有力方法。
...
### [55] [A Client-level Assessment of Collaborative Backdoor Poisoning in Non-IID Federated Learning](https://arxiv.org/abs/2504.12875)
*Phung Lai,Guanxiong Liu,Hai Phan,Issa Khalil,Abdallah Khreishah,Xintao Wu*

Main category: cs.LG

TLDR: 论文揭示了联邦学习在非独立同分布数据下的新漏洞，提出了一种名为CollaPois的协作后门攻击方法，能够高效且隐蔽地放大攻击效果。


<details>
  <summary>Details</summary>
Motivation: 研究联邦学习在非独立同分布数据下的潜在安全风险，特别是针对模型投毒攻击的脆弱性。

Method: 开发了CollaPois攻击方法，通过分发预训练的带有特洛伊木马的模型，使被攻陷的客户端协作生成恶意梯度，放大攻击效果。

Result: CollaPois在多种基准数据集上表现出色，能够绕过现有防御机制，并在少量被攻陷客户端的情况下仍保持高效。

Conclusion: 非独立同分布数据加剧了联邦学习的脆弱性，CollaPois攻击方法为未来防御机制的设计提供了重要启示。
...
### [56] [Can Masked Autoencoders Also Listen to Birds?](https://arxiv.org/abs/2504.12880)
*Lukas Rauch,Ilyass Moummad,René Heinrich,Alexis Joly,Bernhard Sick,Christoph Scholz*

Main category: cs.LG

TLDR: Bird-MAE是一种针对鸟类声音分类的专用MAE模型，通过调整预训练和微调策略，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 通用音频模型（如Audio-MAE）在特定领域（如鸟类声音分类）表现不佳，而鸟类声音分类对环境健康评估至关重要。

Method: 提出Bird-MAE，基于BirdSet数据集进行预训练，并探索了预训练、微调及冻结表示的使用方法。此外，提出了原型探测方法。

Result: Bird-MAE在所有下游任务中达到最优性能，多标签分类性能显著提升。原型探测方法比线性探测性能提升37%。

Conclusion: Bird-MAE在鸟类声音分类任务中表现优异，原型探测方法有效缩小了与微调的性能差距。
...
### [57] [Mirror, Mirror of the Flow: How Does Regularization Shape Implicit Bias?](https://arxiv.org/abs/2504.12883)
*Tom Jacobs,Chao Zhou,Rebekka Burkholz*

Main category: cs.LG

TLDR: 论文研究了显式正则化（如权重衰减）与隐式偏差在过参数化模型中的相互作用，提出了动态调整权重衰减的策略以改善泛化性能。
...
Result: 通过实验验证了动态权重衰减策略（如训练中关闭权重衰减）可以改善泛化性能。

Conclusion: 显式正则化对隐式偏差有持久影响，动态调整正则化策略有助于优化模型性能。
...
### [58] [Exact Learning Dynamics of In-Context Learning in Linear Transformers and Its Application to Non-Linear Transformers](https://arxiv.org/abs/2504.12916)
*Nischal Mainali,Lucas Teixeira*

Main category: cs.LG

TLDR: 论文通过分析线性Transformer的SGD动态，揭示了上下文学习（ICL）的涌现机制，包括分阶段学习、固定点和非线性行为，并提出了理论工具解释ICL的突然涌现和延迟泛化现象。


<details>
  <summary>Details</summary>
Motivation: 理解Transformer模型在上下文学习（ICL）中的底层机制，目前仍不明确。

Method: 通过推导简化线性Transformer在回归任务中的闭式SGD动态，分析ICL的涌现特性。

Result: 揭示了分阶段学习、固定点和非线性行为等关键特性，并提出了理论工具解释ICL的突然涌现和延迟泛化。

Conclusion: 研究为ICL提供了精确的动态模型和理论分析工具，有助于理解复杂Transformer的训练机制。
...
### [59] [Sliced-Wasserstein Distance-based Data Selection](https://arxiv.org/abs/2504.12918)
*Julien Pallage,Antoine Lesage-Landry*

Main category: cs.LG

TLDR: 提出了一种基于切片-Wasserstein距离的无监督异常检测方法，用于机器学习中的训练数据选择，适用于关键领域如电力系统。


<details>
  <summary>Details</summary>
Motivation: 为关键领域的机器学习决策流程提供保守的数据选择和最优传输解释。

Method: 提出了两种高效近似方法：一种是处理低基数数据集表示，另一种是使用轻量级欧几里得距离近似。

Result: 在合成数据集上展示了过滤模式，并在训练数据选择上进行了数值基准测试。

Conclusion: 该方法首次应用于开源数据集，并作为预测基准的一部分。
...
### [60] [IdentiARAT: Toward Automated Identification of Individual ARAT Items from Wearable Sensors](https://arxiv.org/abs/2504.12921)
*Daniel Homm,Patrick Carqueville,Christian Eichhorn,Thomas Weikert,Thomas Menard,David A. Plecher,Chris Awai Easthope*

Main category: cs.LG

TLDR: 研究探讨了利用腕戴惯性传感器自动标记ARAT项目的潜力，通过IMU传感器和MiniROCKET分类技术，实现了对ARAT项目的快速可靠分类。


<details>
  <summary>Details</summary>
Motivation: ARAT测试存在主观性和耗时问题，研究旨在通过传感器技术实现自动化评估。

Method: 使用IMU传感器和MiniROCKET时间序列分类技术，测试不同预处理策略以优化分类效果。

Result: MiniROCKET能快速可靠分类ARAT项目，但在区分相似项目上仍有挑战。
...
### [61] [RL-PINNs: Reinforcement Learning-Driven Adaptive Sampling for Efficient Training of PINNs](https://arxiv.org/abs/2504.12949)
*Zhenao Song*

Main category: cs.LG

TLDR: RL-PINNs提出了一种基于强化学习的自适应采样框架，显著提升了PINNs在求解偏微分方程时的效率和精度。
...
Method: 将自适应采样建模为马尔可夫决策过程，通过强化学习代理动态选择最优训练点，并使用计算高效的函数变分替代梯度依赖的残差度量。

Result: RL-PINNs在多种PDE基准测试中显著优于现有方法，且采样开销极低，适用于高维和高阶问题。

Conclusion: RL-PINNs通过强化学习驱动的自适应采样，解决了传统方法的计算效率问题，同时提升了精度和可扩展性。
...
### [62] [Transferrable Surrogates in Expressive Neural Architecture Search Spaces](https://arxiv.org/abs/2504.12971)
*Shiwen Qin,Gabriela Kadlecová,Martin Pilát,Shay B. Cohen,Roman Neruda,Elliot J. Crowley,Jovita Lukasik,Linus Ericsson*

Main category: cs.LG

TLDR: 论文研究了在高度表达的NAS搜索空间中使用代理模型提升搜索效率的方法，展示了代理模型在预测架构性能、加速搜索和直接作为搜索目标方面的有效性。
...
Result: 代理模型能高效预测架构性能，加速搜索并提升最终性能。

Conclusion: 代理模型是提升NAS搜索效率的有效工具。
...
### [63] [A Virtual Machine for Arbitrary Low-Precision GPGPU Computation in LLM Serving](https://arxiv.org/abs/2504.12984)
*Yaoyao Ding,Bohan Hou,Xiao Zhang,Allan Lin,Tianqi Chen,Cody Yu Hao,Yida Wang,Gennady Pekhimenko*

Main category: cs.LG

TLDR: 提出了一种用于GPGPU计算的虚拟机，支持任意位宽的低精度数据类型，显著提升了性能。
...
Method: 设计了一个虚拟机，包含线程块级编程模型、分层内存空间、新代数布局系统，并支持多样低精度数据类型。

Result: 实验表明，该虚拟机在支持全谱低精度数据类型的同时，性能优于现有低精度核。

Conclusion: 该虚拟机在低精度计算中表现出色，性能优于现有方法。
...
### [64] [Why Ask One When You Can Ask $k$? Two-Stage Learning-to-Defer to a Set of Experts](https://arxiv.org/abs/2504.12988)
*Yannis Montreuil,Axel Carlier,Lai Xing Ng,Wei Tsang Ooi*

Main category: cs.LG

TLDR: 论文提出了Top-$k$ Learning-to-Defer（L2D）框架，扩展了传统的单代理延迟决策方法，允许将查询分配给$k$个最自信的代理，并进一步提出自适应扩展Top-$k(x)$ L2D，以优化每查询的代理数量。


<details>
  <summary>Details</summary>
Motivation: 现有单代理延迟决策方法在高风险场景中不足以利用集体专业知识，因此需要更灵活的决策框架。

Method: 提出Top-$k$ L2D和Top-$k(x)$ L2D框架，前者固定分配$k$个代理，后者自适应选择代理数量，并设计了新的代理损失函数。

Result: 证明了代理损失的贝叶斯一致性和收敛性，实验验证了框架在分类和回归任务中的有效性。

Conclusion: Top-$k$和Top-$k(x)$ L2D框架显著提升了决策系统的可靠性和灵活性，适用于高风险场景。
...
### [65] [Chain-of-Thought Prompting for Out-of-Distribution Samples: A Latent-Variable Study](https://arxiv.org/abs/2504.12991)
*Yu Wang,Fu-Chieh Chang,Pei-Yuan Wu*

Main category: cs.LG

TLDR: CoT提示通过分步推理提升LLM的上下文学习能力，但其在分布偏移下的泛化能力尚不明确。本文通过潜在变量框架研究了两种OOD场景，发现CoT在潜在变量相似时表现良好，但随差异增大性能下降。


<details>
  <summary>Details</summary>
Motivation: 探究CoT提示在分布偏移下的泛化能力，填补现有研究的空白。

Method: 扩展潜在变量框架，分析两种OOD场景：潜在变量排列组合和均匀缩放。

Result: CoT在潜在变量相似时泛化良好，但差异增大时性能下降。

Conclusion: 揭示了CoT在OOD条件下的局限性，为未来开发更稳健的推理策略提供了方向。
...
### [66] [Inference-friendly Graph Compression for Graph Neural Networks](https://arxiv.org/abs/2504.13034)
*Yangxin Fan,Haolai Che,Yinghui Wu*

Main category: cs.LG

TLDR: 该论文提出了一种名为IFGC的图压缩方案，旨在加速GNN的推理过程，通过压缩图数据保留推理结果，同时减少解压成本。
...
Result: 实验验证了IFGC在多种大规模图上的有效性和高效性。
...
### [67] [An All-Atom Generative Model for Designing Protein Complexes](https://arxiv.org/abs/2504.13075)
*Ruizhe Chen,Dongyu Xue,Xiangxin Zhou,Zaixiang Zheng,Xiangxiang Zeng,Quanquan Gu*

Main category: cs.LG

TLDR: APM是一种专为多链蛋白质建模设计的生成模型，能够精确建模链间相互作用并从头设计具有结合能力的蛋白质复合物。


<details>
  <summary>Details</summary>
Motivation: 多链蛋白质的研究和建模尚未深入，但对理解生物功能至关重要。

Method: APM整合原子级信息并利用多链蛋白质数据，支持折叠和逆折叠任务。

Result: APM在下游任务中表现优异，支持监督微调和零样本采样，达到最先进水平。
...
### [68] [An Empirically Grounded Identifiability Theory Will Accelerate Self-Supervised Learning Research](https://arxiv.org/abs/2504.13101)
*Patrik Reizinger,Randall Balestriero,David Klindt,Wieland Brendel*

Main category: cs.LG

TLDR: 论文提出了一种新的理论框架SITh，用于解释自监督学习（SSL）中表示收敛的现象，并指出了未来研究的三个关键方向。


<details>
  <summary>Details</summary>
Motivation: 当前的自监督学习方法虽然表现优异，但其理论解释不足，尤其是Platonic Representation Hypothesis（PRH）缺乏精确的理论支持。

Method: 通过结合Identifiability Theory（IT），提出了Singular Identifiability Theory（SITh）作为更广泛的理论框架，以解释SSL的整个流程。

Result: SITh能够揭示SSL中隐含的数据假设，并推动学习更具解释性和泛化性的表示。
...
### [69] [Uncertainty-Aware Trajectory Prediction via Rule-Regularized Heteroscedastic Deep Classification](https://arxiv.org/abs/2504.13111)
*Kumar Manas,Christian Schlauch,Adrian Paschke,Christian Wirth,Nadja Klein*

Main category: cs.LG

TLDR: SHIFT是一种结合校准不确定性建模和自动化规则提取的新框架，用于改进轨迹预测的分布外泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习轨迹预测模型在分布外泛化和数据不平衡方面表现不足，需要更鲁棒和校准的方法。

Method: SHIFT将轨迹预测重新定义为分类任务，使用异方差谱归一化高斯过程分离不确定性，并通过大型语言模型自动提取驾驶规则生成信息性先验。

Result: 在nuScenes数据集上，SHIFT在不确定性校准和位移指标上优于现有方法，尤其在复杂场景（如交叉路口）表现突出。

Conclusion: SHIFT通过结合不确定性建模和规则提取，显著提升了轨迹预测的鲁棒性和泛化能力。
...
### [70] [Hadamard product in deep learning: Introduction, Advances and Challenges](https://arxiv.org/abs/2504.13112)
*Grigorios G Chrysos,Yongtao Wu,Razvan Pascanu,Philip Torr,Volkan Cevher*

Main category: cs.LG

TLDR: 该论文首次系统分析了Hadamard积在深度学习中的核心作用，提出了其在四大领域中的应用，并展示了其在计算效率和表示能力之间的平衡优势。


<details>
  <summary>Details</summary>
Motivation: 尽管Hadamard积在深度学习中广泛应用，但其作为核心架构原语的系统性分析仍未被充分研究。

Method: 通过全面分类Hadamard积的应用，识别了其在四大领域中的作用，并分析了其计算优势和适用场景。

Result: Hadamard积在资源受限部署和多模态融合任务中表现出色，具有线性计算复杂度和非线性建模能力。

Conclusion: Hadamard积是一种多功能原语，为深度学习架构提供了计算效率与表示能力之间的平衡，是未来创新的重要基础。
...
### [71] [Quorum: Zero-Training Unsupervised Anomaly Detection using Quantum Autoencoders](https://arxiv.org/abs/2504.13113)
*Jason Zev Ludmir,Sophia Rebello,Jacob Ruiz,Tirthak Patel*

Main category: cs.LG

TLDR: Quorum是一个无需训练的量子异常检测框架，适用于无监督学习，解决了量子机器学习中梯度计算的难题。


<details>
  <summary>Details</summary>
Motivation: 检测关键异常事件和数据在多个行业中至关重要，但量子机器学习的训练仍具挑战性，尤其是在无监督学习中。

Method: 提出Quorum框架，专为无监督学习设计，无需训练即可运行。

Result: Quorum是首个无需训练的量子异常检测框架。

Conclusion: Quorum为解决量子机器学习中的异常检测问题提供了创新方案。
...
### [72] [Predicting BVD Re-emergence in Irish Cattle From Highly Imbalanced Herd-Level Data Using Machine Learning Algorithms](https://arxiv.org/abs/2504.13116)
...
TLDR: 爱尔兰成功实施了牛病毒性腹泻（BVD）根除计划，通过机器学习模型预测高风险畜群，随机森林和XGBoost表现最佳。


<details>
  <summary>Details</summary>
Motivation: 随着爱尔兰接近BVD自由状态，开发预测模型以针对性监测疾病再发风险变得重要。

Method: 评估多种机器学习算法（如二元分类和异常检测技术），模拟不同样本量和类别不平衡比例，结合重采样和评估指标。

Result: 随机森林和XGBoost表现最优，随机森林在2023年实际预测中正确识别219/250阳性畜群，减少一半检测量。

Conclusion: 随机森林模型在BVD预测中效果显著，为针对性监测提供了高效工具。
...
### [73] [Transfer Learning via Auxiliary Labels with Application to Cold-Hardiness Prediction](https://arxiv.org/abs/2504.13142)
*Kristen Goebel,Paola Pesantez-Cabrera,Markus Keller,Alan Fern*

Main category: cs.LG

TLDR: 论文提出了一种新的迁移学习框架TAL，利用物候数据提高无冷害数据作物的冷害预测准确性。


<details>
  <summary>Details</summary>
Motivation: 冷害对水果作物造成严重损害，但冷害数据稀缺，而农民通常拥有多年物候数据。TAL旨在利用这些数据提升预测。

Method: TAL框架通过源任务（有冷害和物候数据）迁移到目标任务（仅有物候数据），提出基于模型选择和平均的新方法。

Result: 在多个葡萄品种的真实数据上，TAL显著提升了无冷害数据时的预测准确性。

Conclusion: TAL为冷害预测提供了一种有效方法，尤其在缺乏直接数据时，具有实际应用价值。
...
### [74] [MIB: A Mechanistic Interpretability Benchmark](https://arxiv.org/abs/2504.13151)
*Aaron Mueller,Atticus Geiger,Sarah Wiegreffe,Dana Arad,Iván Arcuschin,Adam Belfki,Yik Siu Chan,Jaden Fiotto-Kaufman,Tal Haklay,Michael Hanna,Jing Huang,Rohan Gupta,Yaniv Nikankin,Hadas Orgad,Nikhil Prakash,Anja Reusch,Aruna Sankaranarayanan,Shun Shao,Alessandro Stolfo,Martin Tutek,Amir Zur,David Bau,Yonatan Belinkov*

Main category: cs.LG

TLDR: MIB是一个新的基准测试，用于评估机制解释性方法的有效性，包含两个任务轨道和多个模型，旨在比较不同方法在定位因果路径或变量上的表现。
...
Method: 提出MIB基准，包含电路定位和因果变量定位两个轨道，分别比较不同方法在定位模型组件或因果变量上的表现。

Result: 电路定位中，归因和掩码优化方法表现最佳；因果变量定位中，监督DAS方法最优，而SAE特征与标准神经元维度无显著差异。

Conclusion: MIB能有效比较方法，并证明该领域确实取得了进展。
...
### [75] [It's All Connected: A Journey Through Test-Time Memorization, Attentional Bias, Retention, and Online Optimization](https://arxiv.org/abs/2504.13173)
*Ali Behrouz,Meisam Razaviyayn,Peilin Zhong,Vahab Mirrokni*

Main category: cs.LG

TLDR: 论文提出了一种基于注意力偏见的神经网络框架Miras，通过重新定义关联记忆模块和遗忘机制，设计了三种新型序列模型，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 受人类注意力偏见启发，重新定义神经网络架构，提升基础模型的能力。

Method: 提出Miras框架，包含关联记忆架构、注意力偏见目标、保留门和记忆学习算法，并设计三种新模型。

Result: 实验表明Miras框架下的模型在语言建模、常识推理等任务中表现优异，超越Transformer和线性RNN。

Conclusion: Miras为深度学习架构设计提供了新思路，其灵活性和性能优于现有方法。
...
### [76] [Aligning Constraint Generation with Design Intent in Parametric CAD](https://arxiv.org/abs/2504.13178)
...
TLDR: 论文将语言模型的对齐技术应用于CAD模型中的工程草图约束生成，提出“设计对齐”问题，并通过对齐技术显著提升了约束生成的完整性和准确性。


<details>
  <summary>Details</summary>
Motivation: 当前生成CAD设计的方法难以确保模型输出与设计意图一致，即“设计对齐”问题，尤其是生成完全约束几何图形而不失真。

Method: 利用对齐技术训练现有约束生成模型，结合约束求解器的反馈，改进约束生成效果。

Result: 采用对齐技术后，93%的草图被完全约束，远高于基线方法的34%和无对齐的8.9%。

Conclusion: 该方法可推广至现有约束生成模型，并为语言与设计领域的对齐策略研究奠定了基础。
...
### [77] [Unmasking the Reality of PII Masking Models: Performance Gaps and the Call for Accountability](https://arxiv.org/abs/2504.12308)
*Devansh Singh,Sundaraparipurnan Narayanan*

Main category: cs.CL

TLDR: 论文探讨了隐私掩码技术及其依赖的NER方法的局限性，评估了PII检测模型的性能，并提出改进建议。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于揭示现有PII数据集和NER方法在隐私掩码中的不足，以及广泛使用的模型可能导致的隐私泄露风险。

Method: 通过构建包含16种PII类型的17K半合成句子数据集，并在五个NER检测维度及对抗环境下测试模型性能。

Result: 结果显示现有模型在隐私掩码中存在显著缺陷，可能导致隐私暴露。

Conclusion: 结论强调需改进模型性能评估方法，并在模型卡片中提供上下文披露。
...
### [78] [Learning Optimal Prompt Ensemble for Multi-source Visual Prompt Transfer](https://arxiv.org/abs/2504.12311)
*Enming Zhang,Liwen Cao,Yanru Wu,Zijie Zhao,Guan Wang,Yang Li*

Main category: cs.CL

TLDR: HGPrompt是一个自适应多源提示迁移框架，通过优化双重目标（可迁移性和稳定性）来提升提示调优的效果。


<details>
  <summary>Details</summary>
Motivation: 解决多源提示简单聚合导致的表示崩溃问题，以充分利用互补知识。

Method: 引入信息论指标评估提示特征的可迁移性，并提出梯度对齐正则化来稳定知识迁移。

Result: 在VTAB基准测试中达到最先进性能。
...
### [79] [Socrates or Smartypants: Testing Logic Reasoning Capabilities of Large Language Models with Logic Programming-based Test Oracles](https://arxiv.org/abs/2504.12312)
*Zihao Xu,Junchen Ding,Yiling Lou,Kun Zhang,Dong Gong,Yuekang Li*

Main category: cs.CL

TLDR: 论文介绍了SmartyPat-Bench和SmartyPat框架，用于评估LLMs的逻辑推理能力，解决了现有数据集的局限性，并通过自动化生成逻辑谬误数据提升了研究效率。


<details>
  <summary>Details</summary>
Motivation: 现有数据集过于简单或不自然，无法有效评估LLMs的逻辑推理能力，因此需要更自然、多样化的基准和自动化工具。

Method: 提出SmartyPat-Bench（基于Reddit的真实数据）和SmartyPat框架（利用Prolog和LLMs自动生成逻辑谬误）。

Result: SmartyPat生成的数据质量接近人类内容，实验揭示了LLMs在谬误检测和分类中的表现。

Conclusion: SmartyPat框架有效解决了数据局限性，并提供了对LLMs逻辑推理能力的深入洞察。
...
### [80] [Exploring the Impact of Personality Traits on Conversational Recommender Systems: A Simulation with Large Language Models](https://arxiv.org/abs/2504.12313)
*Xiaoyan Zhao,Yang Deng,Wenjie Wang,Hongzhan lin,Hong Cheng,Rui Zhang,See-Kiong Ng,Tat-Seng Chua*

Main category: cs.CL

TLDR: 论文提出了一种基于大语言模型（LLM）的人格感知用户模拟系统（PerCRS），用于研究人格特质如何影响对话推荐系统的结果。


<details>
  <summary>Details</summary>
Motivation: 人格特质对用户交互行为有显著影响，但现有对话推荐系统（CRSs）缺乏对其的系统研究。

Method: 设计了PerCRS系统，用户代理可定制人格特质和偏好，系统代理具备说服能力以模拟真实交互。采用多角度评估确保鲁棒性。

Result: 实验表明，LLM能生成符合指定人格特质的多样化用户响应，促使CRSs动态调整推荐策略。
...
### [81] [How to Detect and Defeat Molecular Mirage: A Metric-Driven Benchmark for Hallucination in LLM-based Molecular Comprehension](https://arxiv.org/abs/2504.12314)
*Hao Li,Liuzhenghao Lv,He Cao,Zijing Liu,Zhiyuan Yan,Yu Wang,Yonghong Tian,Yu Li,Li Yuan*

Main category: cs.CL

TLDR: 本文分析了大型语言模型（LLMs）在分子理解任务中的幻觉问题，提出了一种新的评估指标Mol-Hallu，并设计了后处理阶段HRPP以减少幻觉，实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有LLMs在分子理解任务中存在幻觉问题，导致药物设计和应用中的错误，亟需解决以提高模型可靠性。

Method: 提出Mol-Hallu指标量化幻觉程度，并设计HRPP后处理阶段以减少幻觉。

Result: 实验表明HRPP在多种LLMs中有效减少分子幻觉。

Conclusion: 研究为减少幻觉、提升LLMs在科学应用中的可靠性提供了重要见解。
...
### [82] [Capybara-OMNI: An Efficient Paradigm for Building Omni-Modal Language Models](https://arxiv.org/abs/2504.12315)
*Xingguang Ji,Jiakang Wang,Hongzhi Zhang,Jingyuan Zhang,Haonan Zhou,Chenxi Sun,Yahui Liu,Qi Wang,Fuzheng Zhang*

Main category: cs.CL

TLDR: Capybara-OMNI是一种轻量高效的多模态大语言模型（MLLM），支持文本、图像、视频和音频模态，并提供详细框架设计、数据构建和训练方法。


<details>
  <summary>Details</summary>
Motivation: 由于多模态数据对的创建和训练复杂且耗时，开发高效MLLM仍具挑战性。

Method: 提出Capybara-OMNI框架，详述其设计、数据构建和训练方法，并开发专属基准测试验证多模态理解能力。

Result: 模型在同类规模模型中表现优异，并进一步开发了聊天版本以增强交互能力。

Conclusion: 公开了模型权重、部分训练数据和推理代码，推动开源社区发展。
...
### [83] [Data Metabolism: An Efficient Data Design Schema For Vision Language Model](https://arxiv.org/abs/2504.12316)
...
TLDR: 论文提出了一种数据代谢框架，通过数据管理和迭代构建高效视觉语言模型（VLM），并展示了其模型Capybara-VL的优异性能。


<details>
  <summary>Details</summary>
Motivation: 数据管理对训练强大的视觉语言模型至关重要，但现有方法缺乏系统性和闭环优化。

Method: 提出数据代谢框架，包括数据管理和迭代闭环系统，并详细说明数据处理和用户特定数据飞轮构建方法。

Result: Capybara-VL在多项任务中表现优异，超越多个开源模型，甚至媲美专有模型。

Conclusion: 数据代谢框架能显著提升VLM性能，证明小型高效模型的潜力。
...
### [84] [ChatGPT as Linguistic Equalizer? Quantifying LLM-Driven Lexical Shifts in Academic Writing](https://arxiv.org/abs/2504.12317)
*Dingkang Lin,Naixuan Zhao,Dan Tian,Jiang Li*

Main category: cs.CL

TLDR: ChatGPT显著提升了非英语母语作者（NNES）在学术写作中的词汇复杂度，尤其在预印本、技术和生物领域以及低级别期刊中效果最明显。


<details>
  <summary>Details</summary>
Motivation: 研究ChatGPT是否能减少非英语母语作者在学术写作中的语言障碍，促进学术公平。

Method: 使用MTLD量化词汇复杂度，采用DID设计分析2.8万篇OpenAlex文章（2020-2024）的因果效应。

Result: ChatGPT显著提升了NNES作者摘要的词汇复杂度，尤其在特定领域和期刊中效果突出。

Conclusion: ChatGPT有助于减少语言差异，推动全球学术公平。
...
### [85] [Has the Creativity of Large-Language Models peaked? An analysis of inter- and intra-LLM variability](https://arxiv.org/abs/2504.12320)
*Jennifer Haase,Paul H. P. Hanel,Sebastian Pokutta*

Main category: cs.CL

TLDR: 研究发现，尽管LLMs在创意任务中表现优异，但其创意能力并未随时间提升，且输出存在显著波动。


<details>
  <summary>Details</summary>
Motivation: 探讨LLMs的创意能力是否随时间增强，以及其创意输出的稳定性。

Method: 评估14种LLMs（如GPT-4、Claude等）在DAT和AUT两项创意测试中的表现。

Result: LLMs创意能力未提升，GPT-4表现下降；部分模型优于人类平均水平，但极少达到人类顶尖水平。输出波动大。
...
### [86] [AttentionDefense: Leveraging System Prompt Attention for Explainable Defense Against Novel Jailbreaks](https://arxiv.org/abs/2504.12321)
*Charlotte Siska,Anush Sankaran*

Main category: cs.CL

TLDR: 论文提出了一种名为AttentionDefense的新方法，利用小型语言模型（SLM）的系统提示注意力来检测和解释对抗性提示，提供了一种更经济且可解释的防御策略。
...
Method: 通过SLM的系统提示注意力机制，设计了一种名为AttentionDefense的方法，用于检测和解释对抗性提示。该方法利用注意力机制而非文本嵌入语义来理解恶意输入。

Result: 实验表明，AttentionDefense在检测jailbreak方面优于基于文本嵌入的分类器和GPT-4零样本检测器，并在新型jailbreak数据集上表现稳健。

Conclusion: AttentionDefense是一种高效、可解释且经济的防御方法，兼具小型语言模型的计算需求和大型语言模型的检测性能。
...
### [87] [A Strategic Coordination Framework of Small LLMs Matches Large LLMs in Data Synthesis](https://arxiv.org/abs/2504.12322)
*Xin Gao,Qizhi Pei,Zinan Tang,Yu Li,Honglin Lin,Jiang Wu,Conghui He,Lijun Wu*

Main category: cs.CL

TLDR: 论文提出了一种名为GRA的框架，通过多个小型LLM协作模拟同行评审流程，实现高质量数据合成，挑战了依赖单一大型LLM的传统方法。


<details>
  <summary>Details</summary>
Motivation: 当前数据合成方法依赖大型LLM，存在高成本、低效率和潜在偏见问题，而小型LLM个体能力不足，因此需要一种协作框架来提升数据质量。

Method: GRA框架将多个小型LLM分配为Generator、Reviewer和Adjudicator角色，模拟同行评审流程，通过迭代优化生成高质量数据。

Result: 实验表明，GRA生成的数据质量与单一大型LLM（如Qwen-2.5-72B-Instruct）相当或更优。

Conclusion: GRA证明了小型LLM协作可以替代大型LLM，为高效、可持续的数据合成提供了新思路。
...
### [88] [The Other Side of the Coin: Exploring Fairness in Retrieval-Augmented Generation](https://arxiv.org/abs/2504.12323)
...
<details>
  <summary>Details</summary>
Motivation: 研究RAG框架对LLMs公平性的影响，特别是在小规模模型中的应用。

Method: 通过实验分析不同LLMs、检索器和检索源的影响，并提出FairFT（对齐检索器与LLMs公平性）和FairFilter（过滤偏见内容）两种方法。

Result: 小规模LLMs（<8B）在RAG框架中公平性问题更严重，提出的方法能有效改善公平性且保持性能。
...
### [89] [Cross-Document Cross-Lingual Natural Language Inference via RST-enhanced Graph Fusion and Interpretability Prediction](https://arxiv.org/abs/2504.12324)
*Mengying Yuan,Wangzi Xuan,Fei Li*

Main category: cs.CL

TLDR: 本文提出了一种新的跨文档跨语言自然语言推理（CDCL-NLI）范式，构建了一个高质量的数据集，并提出了一种结合RST增强图融合和可解释性预测的创新方法。实验表明该方法优于传统NLI模型和大型语言模型。


<details>
  <summary>Details</summary>
Motivation: 跨文档跨语言自然语言推理（CDCL-NLI）尚未被充分探索，本文旨在填补这一空白，扩展NLI能力至多文档、多语言场景。

Method: 提出了一种结合RST增强的RGAT进行跨文档上下文建模，以及基于词汇链的结构感知语义对齐机制。同时开发了EDU级别的归因框架以生成可解释性推理。

Result: 实验结果显示，该方法显著优于传统NLI模型（如DocNLI和R2F）和大型语言模型（如Llama3和GPT-4o）。

Conclusion: 本文为NLI研究提供了新方向，并激发了跨文档跨语言上下文理解、语义检索和可解释性推理的研究兴趣。
...
### [90] [LLMTaxo: Leveraging Large Language Models for Constructing Taxonomy of Factual Claims from Social Media](https://arxiv.org/abs/2504.12325)
*Haiqi Zhang,Zhengyuan Zhu,Zeyu Zhang,Chengkai Li*

Main category: cs.CL

TLDR: LLMTaxo利用大语言模型自动构建社交媒体事实声明的多粒度分类框架，通过实验验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 社交媒体内容爆炸式增长，分析在线讨论变得复杂，需要更高效的工具。

Method: 提出LLMTaxo框架，结合不同模型在三个数据集上实现多粒度主题生成，并设计专门评估指标。

Result: 实验表明LLMTaxo能有效分类事实声明，且某些模型在特定数据集上表现更优。

Conclusion: LLMTaxo为社交媒体内容分类提供了高效解决方案，模型选择需结合数据集特点。
...
### [91] [Reconstructing Sepsis Trajectories from Clinical Case Reports using LLMs: the Textual Time Series Corpus for Sepsis](https://arxiv.org/abs/2504.12326)
*Shahriar Noroozizadeh,Jeremy C. Weiss*

Main category: cs.CL

TLDR: 该研究构建了一个管道，利用大语言模型从临床病例报告中提取时间定位的发现，生成了一个关于Sepsis-3的开放访问文本时间序列语料库，并验证了其高恢复率和强时间排序能力。


<details>
  <summary>Details</summary>
Motivation: 临床病例报告和出院总结虽完整准确，但时间滞后；结构化数据流虽及时但不完整。研究旨在通过更完整且时间细粒度的数据训练模型。

Method: 构建管道，利用大语言模型对病例报告中的时间定位发现进行表型提取和标注，生成开放访问语料库，并与专家标注结果对比验证。

Result: 高恢复率（事件匹配率：O1-preview--0.755，Llama 3.3 70B Instruct--0.753）和强时间排序（一致性：O1-preview--0.932，Llama 3.3 70B Instruct--0.932）。

Conclusion: 研究展示了大语言模型在时间定位临床发现中的能力，但也指出了其在时间重建中的局限性，提出了多模态整合的改进方向。
...
### [92] [Word Embeddings Track Social Group Changes Across 70 Years in China](https://arxiv.org/abs/2504.12327)
*Yuxi Ma,Yongqian Peng,Yixin Zhu*

Main category: cs.CL

TLDR: 该研究首次大规模分析了中国官方媒体（1950-2019）中社会群体的语言表征，发现其与西方模式显著不同，尤其在性别和经济阶层方面表现出动态变化。


<details>
  <summary>Details</summary>
Motivation: 探讨革命性社会变革如何通过官方语言表征反映社会群体，填补非西方语境下计算社会科学的研究空白。

Method: 使用历时词嵌入方法，分析中国官方媒体在不同时间分辨率下的语言模式。

Result: 中国社会群体的语言表征与西方差异显著，性别和经济阶层的表征随历史变革剧烈变化，而种族、年龄和体型等刻板印象则相对稳定。

Conclusion: 官方话语通过语言编码社会结构，非西方视角对计算社会科学至关重要。
...
### [93] [A Comprehensive Survey of Reward Models: Taxonomy, Applications, Challenges, and Future](https://arxiv.org/abs/2504.12328)
*Jialun Zhong,Wei Shen,Yanzeng Li,Songyang Gao,Hua Lu,Yicheng Chen,Yang Zhang,Wei Zhou,Jinjie Gu,Lei Zou*

Main category: cs.CL

TLDR: 本文综述了奖励模型（RM）在增强大型语言模型（LLM）中的应用，涵盖偏好收集、奖励建模及使用方法，并探讨了当前挑战与未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 奖励模型（RM）作为人类偏好的代理，能够指导LLM的行为，但其研究尚不完善，本文旨在为初学者提供全面介绍并推动未来研究。

Method: 通过综述相关研究，从偏好收集、奖励建模和使用三个角度分析RM，并讨论其应用、评估基准及现存挑战。

Result: 总结了RM的研究现状，提出了未来可能的研究方向，并公开了相关资源。

Conclusion: 本文为RM领域提供了全面的入门指南，并指出了未来研究的潜在方向。
...
### [94] [Speculative Thinking: Enhancing Small-Model Reasoning with Large Model Guidance at Inference Time](https://arxiv.org/abs/2504.12329)
*Wang Yang,Xiang Yue,Vipin Chaudhary,Xiaotian Han*

Main category: cs.CL

TLDR: 提出了一种名为“Speculative Thinking”的无训练框架，通过大模型指导小模型在推理时提升性能，显著提高了准确率并缩短输出长度。


<details>
  <summary>Details</summary>
Motivation: 现有后训练方法成本高且输出冗长，需一种更高效的解决方案。

Method: 利用大模型在推理时指导小模型，基于结构分隔符和反射信号优化推理过程。

Result: 1.5B模型在MATH500上的准确率提升6.2%，输出长度减少15.7%；非推理模型Qwen-2.5-7B-Instruct准确率提升7.8%。

Conclusion: Speculative Thinking是一种高效且无需训练的方法，显著提升了推理模型的性能。
...
### [95] [HM-RAG: Hierarchical Multi-Agent Multimodal Retrieval Augmented Generation](https://arxiv.org/abs/2504.12330)
*Pei Liu,Xin Liu,Ruoyu Yao,Junming Liu,Siyuan Meng,Ding Wang,Jun Ma*

Main category: cs.CL

TLDR: HM-RAG是一种新型分层多代理多模态RAG框架，通过协作智能解决复杂查询，显著提升答案准确性和问题分类准确性。


<details>
  <summary>Details</summary>
Motivation: 传统单代理RAG在解决跨异构数据生态系统的复杂查询时存在局限性，需要更动态的知识合成方法。

Method: 采用三层架构：分解代理、多源检索代理和决策代理，结合文本、图关系和网络证据进行综合查询理解。

Result: 在ScienceQA和CrisisMMD基准测试中，答案准确性提升12.95%，问题分类准确性提升3.56%，并在零样本设置中达到最先进水平。
...
### [96] [Span-level Emotion-Cause-Category Triplet Extraction with Instruction Tuning LLMs and Data Augmentation](https://arxiv.org/abs/2504.12331)
*Xiangju Li,Dong Yang,Xiaogang Zhu,Faliang Huang,Peng Zhang,Zhongying Zhao*

Main category: cs.CL

TLDR: 该论文提出了一种基于大语言模型的细粒度方法，用于提取情感-原因-类别三元组，通过指令调优和数据增强技术显著提升了性能。
...
Method: 采用任务特定的三元组提取指令，利用低秩适应微调大语言模型，无需复杂任务特定架构，并通过提示生成高质量合成数据。

Result: 实验表明，该方法在情感-原因-类别三元组提取指标上至少提升了12.8%，优于现有基线方法。

Conclusion: 该方法有效且鲁棒，为情感原因分析研究提供了新方向，代码已开源。
...
### [97] [Can the capability of Large Language Models be described by human ability? A Meta Study](https://arxiv.org/abs/2504.12332)
*Mingrui Zan,Yunquan Zhang,Boyang Zhang,Fangming Liu,Daning Cheng*

Main category: cs.CL

TLDR: 论文通过评估80多个模型在37个基准测试中的表现，比较了LLMs与人类能力的相似性，发现部分能力可用人类指标描述，但相关性差异显著。


<details>
  <summary>Details</summary>
Motivation: 探讨LLMs能力是否真正接近人类能力，并量化其表现。

Method: 收集80多个模型在37个基准测试中的性能数据，分类为6种主要能力和11种子能力，并进行聚类分析。

Result: 1. 部分LLMs能力可用人类指标描述；2. 人类相关能力在LLMs中几乎无关；3. 能力随模型参数规模变化显著。

Conclusion: LLMs能力与人类能力存在部分重叠，但相关性及参数依赖性差异显著。
...
### [98] [Meta-Evaluating Local LLMs: Rethinking Performance Metrics for Serious Games](https://arxiv.org/abs/2504.12333)
*Andrés Isaza-Giraldo,Paulo Bala,Lucas Pereira*

Main category: cs.CL

TLDR: 研究探讨了五种小型LLM在评估游戏《En-join》中玩家回答时的可靠性，发现模型在敏感性和特异性之间存在权衡，需根据场景选择合适模型。


<details>
  <summary>Details</summary>
Motivation: 开放式回答在严肃游戏中的评估具有主观性，小型LLM的准确性和一致性尚不明确，需验证其可靠性。

Method: 使用传统二元分类指标（如准确率、真阳性率、真阴性率）系统比较五种小型LLM在不同评估场景下的表现。

Result: 部分模型在识别正确答案上表现优异，但其他模型存在假阳性或不一致问题，需权衡敏感性与特异性。

Conclusion: 研究强调需开发情境感知的评估框架，并谨慎选择模型，为AI评估工具的可信度提供了新见解。
...
### [99] [QM-ToT: A Medical Tree of Thoughts Reasoning Framework for Quantized Model](https://arxiv.org/abs/2504.12334)
*Zongxian Yang,Jiayu Qian,Zhi-An Huang,Kay Chen Tan*

Main category: cs.CL

TLDR: QM-ToT框架通过树状思维分解医学问题，显著提升量化模型在MedQAUSMLE数据集上的性能。
...
Method: 提出QM-ToT框架，结合树状思维分解和评估层，优化INT4量化模型。

Result: LLaMA2-70b准确率从34%提升至50%，LLaMA-3.1-8b从58.77%提升至69.49%；数据蒸馏方法效率提升86.27%。

Conclusion: QM-ToT首次展示树状思维在生物医学任务中的潜力，为资源有限环境下的高效部署奠定基础。
...
### [100] [You've Changed: Detecting Modification of Black-Box Large Language Models](https://arxiv.org/abs/2504.12335)
*Alden Dima,James Foulds,Shimei Pan,Philip Feldman*

Main category: cs.CL

TLDR: 提出了一种通过比较生成文本的语言和心理语言学特征分布来监控LLM变化的方法，使用统计测试检测模型变化，并验证了其在区分模型和检测提示注入攻击中的有效性。
...
Conclusion: 该方法为开发者提供了一种轻量级的LLM监控手段，避免了计算昂贵的基准评估。
...
### [101] ["It Listens Better Than My Therapist": Exploring Social Media Discourse on LLMs as Mental Health Tool](https://arxiv.org/abs/2504.12337)
*Anna-Carolina Haensch*

Main category: cs.CL

TLDR: 研究分析了用户如何将大型语言模型（LLMs）作为心理健康支持工具，通过分析TikTok评论发现，用户态度总体积极，但也存在隐私和专业性担忧。


<details>
  <summary>Details</summary>
Motivation: 探讨生成式AI聊天机器人（如ChatGPT）在非正式心理健康支持中的角色及其潜在影响。

Method: 分析10,000多条TikTok评论，采用分层编码模式和监督分类模型识别用户经验和主题。

Result: 约20%的评论反映个人使用，用户态度积极，但担忧隐私和缺乏专业监督。
...
### [102] [Paging Dr. GPT: Extracting Information from Clinical Notes to Enhance Patient Predictions](https://arxiv.org/abs/2504.12338)
*David Anderson,Michaela Anderson,Margret Bjarnadottir,Stephen Mahar,Shriyan Reyya*

Main category: cs.CL

TLDR: 研究探讨了利用GPT-4o-mini生成的临床问题答案结合患者出院摘要，提升患者死亡率预测的效果。结果表明，GPT模型优于传统表格数据模型，结合两者可显著提高预测性能。
...
Method: 使用MIMIC-IV Note数据集中的14,011例患者数据，将GPT回答作为逻辑回归模型的输入特征。

Result: GPT模型表现优于传统模型，结合两者使AUC平均提高5.1个百分点，高风险组的阳性预测值提升29.9%。
...
### [103] [GOAT-TTS: LLM-based Text-To-Speech Generation Optimized via A Dual-Branch Architecture](https://arxiv.org/abs/2504.12339)
*Yaodong Song,Hongjie Chen,Jie Lian,Yuxin Zhang,Guangmin Xia,Zehan Li,Genliang Zhao,Jian Kang,Yongxiang Li,Jie Li*

Main category: cs.CL

TLDR: GOAT-TTS是一种基于LLM的双分支架构TTS方法，解决了现有模型在声学特征丢失、依赖对齐数据及语言理解遗忘上的问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在TTS中存在声学特征丢失、依赖对齐数据及语言理解遗忘的三大问题。

Method: 提出双分支架构：模态对齐分支捕获连续声学嵌入，语音生成分支通过模块化微调预测语音标记。

Result: 实验表明GOAT-TTS性能接近SOTA，并验证了方言语音合成的有效性。

Conclusion: GOAT-TTS通过双分支设计平衡了声学特征与语言理解，为TTS提供了新思路。
...
### [104] [Streamlining Biomedical Research with Specialized LLMs](https://arxiv.org/abs/2504.12341)
...
TLDR: 提出了一种结合领域专用大语言模型与高级信息检索技术的新系统，提供全面且上下文感知的响应。


<details>
  <summary>Details</summary>
Motivation: 提升对话生成质量，支持多模态数据交互，优化生物医药领域的研究效率与决策速度。

Method: 集成大语言模型与信息检索技术，实现组件间无缝交互与输出交叉验证。

Result: 显著提高响应精度，提供实时高保真交互平台，加速研发决策。

Conclusion: 该系统为生物医药领域提供了高效的研究工具，显著提升了决策效率。
...
### [105] [Benchmarking Biopharmaceuticals Retrieval-Augmented Generation Evaluation](https://arxiv.org/abs/2504.12342)
*Hanmeng Zhong,Linqing Chen,Weilei Wang,Wentao Wu*

Main category: cs.CL

TLDR: 论文介绍了首个针对生物制药领域的检索增强生成评估基准（BRAGE），并提出了一种基于引用的分类方法评估LLMs的查询与参考理解能力（QRUC）。
...
Method: 提出BRAGE基准和基于引用的分类方法，评估LLMs的QRUC能力。
...
### [106] [Propaganda via AI? A Study on Semantic Backdoors in Large Language Models](https://arxiv.org/abs/2504.12344)
*Nay Myat Min,Long H. Pham,Yige Li,Jun Sun*

Main category: cs.CL

TLDR: 论文提出了一种针对大语言模型（LLMs）中语义后门攻击的黑盒检测框架RAVEN，通过语义熵和跨模型一致性分析，揭示了传统防御方法忽视的语义层面的隐蔽漏洞。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在多种任务中表现优异，但易受后门攻击，尤其是语义层面的隐蔽攻击。传统防御方法仅关注显式的词级异常，无法检测基于语义的触发机制。

Method: 研究首先在微调环境中验证了语义后门的可行性，随后提出了RAVEN框架，通过主题视角提示、双向蕴含聚类和跨模型比较来检测异常输出。

Result: 实验在多种LLM（如GPT-4o、Llama等）中发现了此前未检测到的语义后门，证明了概念级审计的必要性。

Conclusion: 研究揭示了语义后门的实际威胁，并提出了有效的检测方法，强调了部署语言模型时需进行概念级审计。
...
### [107] [Reimagining Urban Science: Scaling Causal Inference with Large Language Models](https://arxiv.org/abs/2504.12345)
*Yutong Xia,Ao Qu,Yunhan Zheng,Yihong Tang,Dingyi Zhuang,Yuxuan Liang,Cathy Wu,Roger Zimmermann,Jinhua Zhao*

Main category: cs.CL

TLDR: 论文提出AutoUrbanCI框架，利用大语言模型（LLMs）改进城市因果研究，解决假设生成、数据复杂性等问题，并强调人机协作与公平性。


<details>
  <summary>Details</summary>
Motivation: 城市因果研究面临假设生成效率低、数据复杂性和方法脆弱性等挑战，LLMs为改进提供了新机会。

Method: 提出AutoUrbanCI框架，包含四个模块化代理：假设生成、数据工程、实验设计与执行、结果解释与政策建议。

Result: 框架旨在提高研究的严谨性和透明度，推动人机协作，提升可重复性和包容性。

Conclusion: 呼吁采用AI增强的工作流程，作为扩展人类专业知识的工具，而非替代，以实现更包容的城市因果推理。
...
### [108] [Mathematical Capabilities of Large Language Models in Finnish Matriculation Examination](https://arxiv.org/abs/2504.12347)
*Mika Setälä,Pieta Sikström,Ville Heilala,Tommi Kärkkäinen*

Main category: cs.CL

TLDR: 大型语言模型（LLMs）在数学推理能力上快速进步，部分模型在芬兰高考中取得接近满分成绩。


<details>
  <summary>Details</summary>
Motivation: 评估LLMs在数学能力上的表现，探索其在教育评估中的潜力。

Method: 使用芬兰高考（高风险的数字化测试）对多种LLMs进行数学能力评估。

Result: 初期表现中等，后期显著提升，部分模型达到接近满分或满分成绩。

Conclusion: LLMs数学能力进步迅速，具备支持大规模教育评估的潜力。
...
### [109] [A Large-Language Model Framework for Relative Timeline Extraction from PubMed Case Reports](https://arxiv.org/abs/2504.12350)
*Jing Wang,Jeremy C Weiss*

Main category: cs.CL

TLDR: 论文提出了一种将病例报告转化为文本时间序列的系统，对比了人工和LLM标注的效果，发现LLM在事件召回和时间一致性上表现良好。
...
Method: 开发系统将病例报告转化为文本时间序列，对比人工和LLM标注的效果，评估LLM间一致性。
...
### [110] [Leveraging Large Language Models for Multi-Class and Multi-Label Detection of Drug Use and Overdose Symptoms on Social Media](https://arxiv.org/abs/2504.12355)
*Muhammad Ahmad,Muhammad Waqas,ldar Batyrshin,Grigori Sidorov*

Main category: cs.CL

TLDR: AI驱动的NLP框架利用社交媒体数据检测药物滥用和过量症状，准确率高达98%。


<details>
  <summary>Details</summary>
Motivation: 传统研究方法在药物滥用监测中存在局限，社交媒体提供实时数据。
...
### [111] [Replicating ReLM Results: Validating Large Language Models with ReLM](https://arxiv.org/abs/2504.12357)
*Reece Adamson,Erin Song*

Main category: cs.CL

TLDR: ReLM方法通过形式化语言评估LLMs的记忆、偏见和零样本性能，解决了现有方法慢、不精确或引入偏见的问题。


<details>
  <summary>Details</summary>
Motivation: 现有评估LLM行为的方法存在效率低、不精确或引入偏见的问题，而这些问题对LLM的实际应用至关重要。

Method: 使用形式化语言（ReLM）对LLMs进行评估和控制，重点关注记忆、偏见和零样本性能。

Result: 验证了ReLM方法的有效性，并扩展了其在机器学习系统领域的应用。

Conclusion: ReLM为LLM的评估提供了一种高效、精确且无偏见的方法，具有重要的实际应用价值。
...
### [112] [A Method for Handling Negative Similarities in Explainable Graph Spectral Clustering of Text Documents -- Extended Version](https://arxiv.org/abs/2504.12360)
*Mieczysław A. Kłopotek,Sławomir T. Wierzchoń,Bartłomiej Starosta,Dariusz Czerski,Piotr Borkowski*

Main category: cs.CL

TLDR: 论文研究了图谱聚类中负相似性问题，比较了6种解决方案，发现GloVe嵌入会导致归一化拉普拉斯方法失效，并提出修正方法提升精度。


<details>
  <summary>Details</summary>
Motivation: 研究文档嵌入（如doc2vec、GloVe等）与传统词向量空间不同导致的负相似性对图谱聚类的影响。

Method: 讨论了组合拉普拉斯和归一化拉普拉斯的解决方案，实验比较了6种方法。

Result: GloVe嵌入常导致归一化拉普拉斯方法失效，修正负相似性后精度提升，且适用于作者提出的解释方法。

Conclusion: 修正负相似性可提升图谱聚类精度，并扩展了解释方法的适用范围。
...
### [113] [Position: The Most Expensive Part of an LLM should be its Training Data](https://arxiv.org/abs/2504.12427)
*Nikhil Kandpal,Colin Raffel*

Main category: cs.CL

TLDR: 论文主张为LLM训练数据生产者提供补偿，估算其成本远高于模型训练费用。


<details>
  <summary>Details</summary>
Motivation: 揭示LLM训练数据背后被忽视的人力成本，呼吁公平补偿。

Method: 研究64个LLM（2016-2024），估算人工生产训练数据的成本。

Result: 训练数据成本是模型训练费用的10-1000倍，构成重大财务负担。

Conclusion: 强调未来需研究更公平的实践方式。
...
### [114] [On Linear Representations and Pretraining Data Frequency in Language Models](https://arxiv.org/abs/2504.12459)
*Jack Merullo,Noah A. Smith,Sarah Wiegreffe,Yanai Elazar*

Main category: cs.CL

TLDR: 研究预训练数据频率与语言模型线性表示的关系，发现线性表示的形成与数据频率高度相关，并提出一种预测预训练数据频率的方法。


<details>
  <summary>Details</summary>
Motivation: 理解预训练数据如何影响语言模型的线性表示，探索数据频率与模型行为的关系。

Method: 分析预训练数据频率与线性表示的关系，训练回归模型预测数据频率。

Result: 线性表示的形成与数据频率高度相关，回归模型能有效预测数据频率。

Conclusion: 线性表示强度可反映预训练数据特性，为改进模型行为提供新思路。
...
### [115] [SLURG: Investigating the Feasibility of Generating Synthetic Online Fallacious Discourse](https://arxiv.org/abs/2504.12466)
*Cal Blanco,Gavin Dsouza,Hugo Lin,Chelsey Rush*

Main category: cs.CL

TLDR: 论文探讨了社交媒体上操纵行为的自动检测中逻辑谬误的定义与推断，重点关注乌克兰-俄罗斯冲突相关论坛中的错误信息。提出了SLURG方法，利用大语言模型生成合成谬误评论，发现模型能复制真实数据的句法模式。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于现有自动谬误检测数据集多限于正式语言领域，而在线论坛语言多样且非标准化，需填补这一空白。

Method: 采用SLURG方法，利用DeepHermes-3-Mistral-24B等大语言模型生成合成谬误评论，并通过高质量少样本提示增强模型能力。

Result: 研究发现大语言模型能复制真实数据的句法模式，且少样本提示能提升模型对论坛语言多样性的模仿能力。
...
### [116] [Integrating Structural and Semantic Signals in Text-Attributed Graphs with BiGTex](https://arxiv.org/abs/2504.12474)
*Azadeh Beiranvand,Seyed Mehdi Vahidipour*

Main category: cs.CL

TLDR: BiGTex是一种双向图文本融合架构，结合GNN和LLM，通过双向注意力机制实现文本与结构的交互，在节点分类和链接预测任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决文本属性图（TAGs）中同时捕捉文本语义和结构依赖的挑战，弥补GNN和LLM各自的不足。

Method: 提出BiGTex架构，通过堆叠的图文本融合单元实现双向注意力机制，结合参数高效微调（LoRA）训练。

Result: 在五个基准数据集上实现节点分类和链接预测的最优性能，并通过消融实验验证了软提示和双向注意力的重要性。

Conclusion: BiGTex通过紧密集成GNN和LLM，有效解决了TAGs的表示学习问题，具有广泛的应用潜力。
...
### [117] [Can Pre-training Indicators Reliably Predict Fine-tuning Outcomes of LLMs?](https://arxiv.org/abs/2504.12491)
*Hansi Zeng,Kai Hui,Honglei Zhuang,Zhen Qin,Zhenrui Yue,Hamed Zamani,Dana Alon*

Main category: cs.CL

TLDR: 论文研究了预训练指标（如困惑度）在固定模型规模下对下游任务性能的预测能力不足的问题，提出了一种基于成对分类的方法来优化预训练检查点选择，并引入新的代理指标显著提升了预测准确性。


<details>
  <summary>Details</summary>
Motivation: 预训练指标（如困惑度）在模型规模扩展研究中表现良好，但在固定模型规模下对下游任务性能的预测能力不明确，影响了模型选择和开发的效率。

Method: 将预训练检查点选择任务定义为成对分类问题，构建了一个包含50种1B参数LLM变体的数据集，通过系统变化的预训练配置（如目标或数据）和下游任务评估，提出了新的无监督和有监督代理指标。

Result: 研究发现传统困惑度指标具有误导性，新提出的代理指标将相对性能预测错误率降低了50%以上。

Conclusion: 尽管任务复杂，新代理指标在特定场景中具有实际应用价值，为优化下游任务的预训练方案设计提供了更高效的方法。
...
### [118] [Accelerating Clinical NLP at Scale with a Hybrid Framework with Reduced GPU Demands: A Case Study in Dementia Identification](https://arxiv.org/abs/2504.12494)
*Jianlin Shi,Qiwei Gan,Elizabeth Hanchrow,Annie Bowles,John Stanley,Adam P. Bress,Jordana B. Cohen,Patrick R. Alba*

Main category: cs.CL

TLDR: 提出了一种结合规则过滤、SVM分类器和BERT模型的混合NLP框架，用于高效且准确地分析大规模临床文本，并在痴呆症识别案例中验证了其性能。
...
Result: 患者级别的精确度为0.90，召回率为0.84，F1分数为0.87，识别出的痴呆症病例是结构化数据方法的三倍多。
...
### [119] [Beyond Text: Characterizing Domain Expert Needs in Document Research](https://arxiv.org/abs/2504.12495)
*Sireesh Gururaja,Nupoor Gandhi,Jeremiah Milbauer,Emma Strubell*

Main category: cs.CL

TLDR: 研究探讨了NLP系统在文档处理任务中与专家实际需求的差距，呼吁NLP社区更关注文档的社会背景和个性化需求。


<details>
  <summary>Details</summary>
Motivation: 了解NLP系统是否能真正模拟专家处理文档的方式，以及现有技术与专家需求的匹配程度。

Method: 采访了16位跨两个领域的专家，分析其文档研究过程，并与当前NLP系统进行比较。

Result: 专家处理文档的方式具有独特性、迭代性，并依赖文档的社会背景；现有NLP方法中关注文档本身（而非仅文本）的更贴近专家需求。

Conclusion: 呼吁NLP社区开发更注重文档角色、个性化、迭代性和社会意识的工具。
...
### [120] [BrowseComp: A Simple Yet Challenging Benchmark for Browsing Agents](https://arxiv.org/abs/2504.12516)
...
TLDR: BrowseComp是一个用于评估网络浏览能力的简单但具有挑战性的基准测试，包含1,266个需要持久搜索复杂信息的问题。


<details>
  <summary>Details</summary>
Motivation: 为衡量代理在网络浏览中的持久性和创造力，提供一个易于使用且可验证的基准测试。

Method: 通过设计短答案、易验证的问题，模拟复杂信息搜索场景。

Result: BrowseComp成功提供了一个简单但有效的测试工具，可用于评估代理的网络浏览能力。

Conclusion: BrowseComp是一个有用的基准测试，尽管不完全模拟真实用户查询，但能有效衡量核心浏览能力。
...
### [121] [Evaluating the Diversity and Quality of LLM Generated Content](https://arxiv.org/abs/2504.12522)
*Alexander Shypula,Shuo Li,Botong Zhang,Vishakh Padmakumar,Kayo Yin,Osbert Bastani*

Main category: cs.CL

TLDR: 研究发现偏好调优技术（如RLHF）虽然减少了词汇和句法多样性，但提高了有效的语义多样性，揭示了形式多样性与内容多样性的区别。


<details>
  <summary>Details</summary>
Motivation: 解决偏好调优技术减少多样性的问题，提出衡量有效语义多样性的框架，以更好地反映大语言模型的实用性。

Method: 通过无需人工干预的开放式任务，比较偏好调优模型与基础模型在多样性和质量上的表现。

Result: 偏好调优模型在语义多样性上表现更好，但句法多样性降低；小模型在固定采样预算下更高效地生成独特内容。

Conclusion: 研究为需要高质量且多样化输出的应用提供了重要启示，揭示了模型规模与多样性之间的关系。
...
### [122] [Memorization vs. Reasoning: Updating LLMs with New Knowledge](https://arxiv.org/abs/2504.12523)
*Aochong Oliver Li,Tanya Goyal*

Main category: cs.CL

TLDR: 论文提出了一种名为KUP的知识更新评估框架和一种轻量级方法MCT，用于改进大型语言模型的知识更新能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要针对实体替换，未能全面反映复杂现实世界动态，因此需要更全面的知识更新评估和方法。

Method: 引入KUP自动管道模拟知识更新，并提出MCT方法，通过自生成记忆令牌在训练中强化知识记忆和推理。

Result: KUP基准测试极具挑战性，最佳CPT模型在间接探测（推理）中表现低于2%；MCT显著优于基线，直接探测（记忆）提升25.4%。

Conclusion: KUP和MCT为LLMs的知识更新提供了有效工具，显著提升了模型在记忆和推理任务中的表现。
...
### [123] [Memorization: A Close Look at Books](https://arxiv.org/abs/2504.12549)
*Iris Ma,Ian Domingo,Alberto Krone-Martins,Pierre Baldi,Cristina V. Lopes*

Main category: cs.CL

TLDR: 研究发现，通过Llama 3 70B模型和“前缀提示”技术，可以从500个初始标记中高相似度地重构整本书（如《爱丽丝梦游仙境》），但成功率与书籍的流行度相关。此外，指令调优后的Llama 3.1中缓解措施的失效与少数权重变化有关。


<details>
  <summary>Details</summary>
Motivation: 探讨大型语言模型（LLMs）能否从少量初始信息中提取整本书，并分析其与训练数据重复性的关系，同时研究指令调优对模型记忆提取的影响。

Method: 使用Llama 3 70B模型和“前缀提示”技术，通过自回归方式从500个初始标记中提取书籍内容，并分析提取率与书籍流行度的相关性。同时研究指令调优后的模型权重变化。

Result: 成功高相似度提取《爱丽丝梦游仙境》等书籍，但提取率与书籍流行度相关。指令调优后的模型缓解措施失效，且仅少数权重变化导致此现象。

Conclusion: 当前缓解策略存在局限性，指令调优对模型记忆提取有显著影响，需进一步研究如何优化模型以减少记忆泄露。
...
### [124] [ELAB: Extensive LLM Alignment Benchmark in Persian Language](https://arxiv.org/abs/2504.12553)
...
TLDR: 该论文提出了一个全面的评估框架，用于将波斯大型语言模型（LLMs）与关键伦理维度（如安全性、公平性和社会规范）对齐，填补了现有LLM评估框架在波斯语言和文化背景下的空白。


<details>
  <summary>Details</summary>
Motivation: 现有LLM评估框架未充分考虑波斯语言和文化背景，因此需要开发一个适应波斯语境的评估框架。

Method: 通过翻译现有数据集（如Anthropic Red Teaming数据）和创建新数据集（如ProhibiBench-fa、SafeBench-fa等），构建了三种波斯语基准数据。

Result: 建立了一个统一的波斯LLM评估框架，并公开了评估波斯LLMs在安全性、公平性和社会规范方面的排行榜。

Conclusion: 该研究为波斯LLMs的文化基础对齐评估提供了新方法，并推动了波斯语LLMs的伦理发展。
...
### [125] [CDF-RAG: Causal Dynamic Feedback for Adaptive Retrieval-Augmented Generation](https://arxiv.org/abs/2504.12560)
*Elahe Khatibi,Ziyu Wang,Amir M. Rahmani*

Main category: cs.CL

TLDR: CDF-RAG框架通过动态反馈和因果推理增强RAG，提升生成内容的因果一致性和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有RAG框架依赖语义相似性检索，难以区分真实因果关系与虚假关联，导致生成内容可能误导。

Method: CDF-RAG通过迭代查询优化、结构化因果图检索和多跳因果推理，验证因果路径以确保逻辑一致。

Result: 在四个数据集上验证，CDF-RAG显著提升了响应准确性和因果正确性。

Conclusion: CDF-RAG为知识密集型任务提供了更可靠的因果推理和解释能力。
...
### [126] [MetaSynth: Meta-Prompting-Driven Agentic Scaffolds for Diverse Synthetic Data Generation](https://arxiv.org/abs/2504.12563)
*Haris Riaz,Sourav Bhabesh,Vinayak Arannil,Miguel Ballesteros,Graham Horwood*

Main category: cs.CL

TLDR: MetaSynth方法通过元提示增强合成数据的多样性，成功将Mistral-7B-v0.3适配到金融和生物医学领域，性能显著提升且不影响通用任务能力。
...
Conclusion: MetaSynth生成的少量高多样性合成数据足以实现有效的领域适配。
...
### [127] [Identifying and Mitigating the Influence of the Prior Distribution in Large Language Models](https://arxiv.org/abs/2504.12585)
*Liyi Zhang,Veniamin Veselovsky,R. Thomas McCoy,Thomas L. Griffiths*

Main category: cs.CL

TLDR: LLMs有时因隐式先验分布而无法正确完成确定性任务，但研究发现它们实际具备所需信息，通过干预（如提示不依赖先验或微调特定网络层）可显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 探究LLMs在确定性任务中失败的原因，并验证其是否具备完成任务所需信息。

Method: 通过提示LLMs不依赖先验知识，并使用机制解释技术定位并微调与先验相关的网络层。

Result: 干预后LLMs在任务中表现显著提升，错误不再与先验相关，表明信息已存在于模型表示中。

Conclusion: 通过操纵LLMs对先验的依赖，可提升其在易产生幻觉任务中的性能。
...
### [128] [GeoSense: Evaluating Identification and Application of Geometric Principles in Multimodal Reasoning](https://arxiv.org/abs/2504.12597)
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试未能全面评估MLLMs在几何问题解决中的视觉理解和符号推理能力，需要填补这一空白。
...
### [129] [Towards Characterizing Subjectivity of Individuals through Modeling Value Conflicts and Trade-offs](https://arxiv.org/abs/2504.12633)
*Younghun Lee,Dan Goldwasser*

Main category: cs.CL

TLDR: 论文提出SOLAR框架，通过分析社交媒体文本中的价值冲突和权衡，推断个体的主观道德判断，提升推断效果。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs是否能捕捉个体层面的主观性，尤其是在社交媒体上个体的道德判断。

Method: 提出SOLAR框架，通过观察用户生成文本中的价值冲突和权衡，推断个体的主观立场。

Result: 实验表明SOLAR提升了推断效果，尤其在争议性情境中，并能解释个体的价值偏好。

Conclusion: SOLAR能有效捕捉个体主观性，为LLMs在主观决策任务中的应用提供支持。
...
### [130] [Scaling Instruction-Tuned LLMs to Million-Token Contexts via Hierarchical Synthetic Data Generation](https://arxiv.org/abs/2504.12637)
...
Result: 模型在1M tokens的上下文长度下，在RULER和InfiniteBench基准上表现良好，且通用任务性能稳健。

Conclusion: 该方法有效解决了长上下文数据稀缺问题，扩展了LLMs的上下文窗口能力。
...
### [131] [Persona-judge: Personalized Alignment of Large Language Models via Token-level Self-judgment](https://arxiv.org/abs/2504.12663)
*Xiaotian Zhang,Ruizhe Chen,Yang Feng,Zuozhu Liu*

Main category: cs.CL

TLDR: 提出了一种无需训练、基于判别式范式的个性化对齐方法Persona-judge，利用模型内在偏好判断能力，避免高计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖奖励信号和额外标注数据，难以适应多样化人类价值观且扩展性差。

Method: Persona-judge通过生成候选令牌并由另一偏好模型交叉验证，实现个性化对齐。

Result: 实验证明该方法具有高效性和扩展性。

Conclusion: Persona-judge为自适应个性化对齐提供了新思路。
...
### [132] [ACoRN: Noise-Robust Abstractive Compression in Retrieval-Augmented Language Models](https://arxiv.org/abs/2504.12673)
*Singon Kim,Gunho Jung,Seong-Whan Lee*

Main category: cs.CL

TLDR: ACoRN通过更细粒度的文档分类和两个新训练步骤，提升抽象压缩模型对噪声的鲁棒性，改善RAG中的信息保留。


<details>
  <summary>Details</summary>
Motivation: 检索到的文档常含无关或误导信息，抽象压缩模型易忽略关键内容，尤其在长上下文中。

Method: 1. 离线数据增强提升噪声鲁棒性；2. 微调模型以围绕关键信息生成摘要。

Result: ACoRN训练的T5-large提升EM和F1分数，保留答案字符串作为直接证据。

Conclusion: ACoRN在含大量降低准确性文档的数据集上表现优异，适用于实际场景。
...
### [133] [GRAIL: Gradient-Based Adaptive Unlearning for Privacy and Copyright in LLMs](https://arxiv.org/abs/2504.12681)
*Kun-Woo Kim,Ji-Hoon Park,Ju-Min Han,Seong-Whan Lee*

Main category: cs.CL

TLDR: GRAIL是一种基于梯度的多领域遗忘框架，用于精确移除大型语言模型中的敏感信息，同时保留关键知识。
...
Method: GRAIL利用多领域梯度信息区分遗忘和保留范围，并采用自适应参数定位策略。

Result: GRAIL在遗忘性能上与现有方法相当，知识保留成功率提升17%。

Conclusion: GRAIL为管理预训练语言模型中的敏感信息提供了新范式。
...
### [134] [Data-efficient LLM Fine-tuning for Code Generation](https://arxiv.org/abs/2504.12687)
*Weijie Lv,Xuan Xia,Sheng-Jun Huang*

Main category: cs.CL

TLDR: 提出了一种数据选择和动态打包策略，显著提升了代码生成LLM的训练效率和性能。


<details>
  <summary>Details</summary>
Motivation: 开源与闭源代码生成模型存在性能差距，传统方法生成大量合成数据导致训练低效。

Method: 通过数据复杂度优先选择和动态打包技术优化数据选择和tokenization。

Result: 在40%数据上训练，性能提升且训练时间、GPU内存消耗显著减少。
...
### [135] [Why and How LLMs Hallucinate: Connecting the Dots with Subsequence Associations](https://arxiv.org/abs/2504.12691)
*Yiyou Sun,Yu Gai,Lijie Chen,Abhilasha Ravichander,Yejin Choi,Dawn Song*

Main category: cs.CL

TLDR: 论文提出了一种子序列关联框架，用于系统追踪和理解大语言模型（LLMs）中的幻觉现象，并通过理论和实证分析验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLMs）常生成偏离事实或上下文的幻觉内容，诊断其成因复杂。本文旨在系统理解幻觉的产生机制。

Method: 提出子序列关联框架，通过分析解码器-仅变换器（decoder-only transformers）的子序列嵌入特性，设计了一种追踪算法，识别幻觉概率的因果子序列。
...
### [136] [KODIS: A Multicultural Dispute Resolution Dialogue Corpus](https://arxiv.org/abs/2504.12723)
*James Hale,Sushrita Rakshit,Kushal Chawla,Jeanne M. Brett,Jonathan Gratch*

Main category: cs.CL

TLDR: KODIS是一个包含来自75个国家数千个对话的双向争议解决语料库，旨在研究文化和冲突理论，支持愤怒表达导致冲突升级的理论，并揭示文化差异。


<details>
  <summary>Details</summary>
Motivation: 基于文化和冲突的理论模型，研究愤怒表达如何导致冲突升级以及文化差异对情感表达的影响。

Method: 通过专家设计的典型客户服务争议对话，收集包含丰富的情感、过程和结果数据。

Result: 初步分析支持愤怒表达导致冲突升级的理论，并揭示了情感表达中的文化差异。

Conclusion: KODIS语料库及其数据收集框架为研究文化和冲突提供了宝贵资源。
...
### [137] [Pandora: A Code-Driven Large Language Model Agent for Unified Reasoning Across Diverse Structured Knowledge](https://arxiv.org/abs/2504.12734)
...
TLDR: Pandora框架通过Python的Pandas API统一知识表示，结合LLM生成推理步骤和代码，提升跨任务知识迁移能力，性能优于现有统一框架。


<details>
  <summary>Details</summary>
Motivation: 现有USKR方法依赖任务特定策略或自定义表示，难以实现知识迁移或与LLM对齐，性能受限。

Method: 利用Python的Pandas API构建统一知识表示，LLM生成推理步骤和可执行代码，结合训练示例库。

Result: 在四个基准测试中，Pandora优于现有统一框架，与任务特定方法竞争。

Conclusion: Pandora通过统一表示和LLM结合，有效提升跨任务知识推理性能。
...
### [138] [Chinese-Vicuna: A Chinese Instruction-following Llama-based Model](https://arxiv.org/abs/2504.12737)
*Chenghao Fan,Zhenyi Lu,Jie Tian*

Main category: cs.CL

TLDR: Chinese-Vicuna是一个基于LLaMA架构的开源中文指令跟随模型，通过LoRA微调和4位量化（QLoRA）实现高效资源利用，支持低资源环境和领域特定任务。


<details>
  <summary>Details</summary>
Motivation: 解决中文指令跟随模型的资源高效性问题，并支持领域特定应用（如医疗和法律）。

Method: 使用LoRA微调LLaMA架构，结合混合数据集（BELLE和Guanaco）和4位量化（QLoRA），优化低资源环境下的性能。

Result: 在翻译、代码生成、领域特定问答等任务中表现优异，尤其在医疗任务和多轮对话中具有竞争力。

Conclusion: Chinese-Vicuna通过模块化设计和开源生态，为中文LLM应用提供了高效、灵活的解决方案。
...
### [139] [Out of Sight Out of Mind, Out of Sight Out of Mind: Measuring Bias in Language Models Against Overlooked Marginalized Groups in Regional Contexts](https://arxiv.org/abs/2504.12767)
*Fatma Elsafoury,David Hartmann*

Main category: cs.CL

TLDR: 该研究探讨了语言模型（LMs）对边缘群体的偏见问题，特别关注了阿拉伯国家、德国、英国和美国中的270个边缘群体，以及低资源语言和方言的影响。研究发现LMs对许多边缘群体表现出更高的偏见，但阿拉伯语LMs在宗教和种族方面对边缘和主导群体均表现出高偏见。此外，研究还发现对非二元性别、LGBTQIA+和黑人女性存在更高的交叉偏见。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注英语世界的偏见问题，而许多边缘群体和低资源语言被忽视。为了开发更具包容性的LMs，需要扩展对边缘群体和低资源语言的理解。

Method: 研究调查了23个LMs对270个边缘群体的偏见，并分析了低资源语言（如埃及阿拉伯语方言）对偏见测量的影响。

Result: LMs对许多边缘群体表现出更高的偏见，但阿拉伯语LMs在宗教和种族方面对边缘和主导群体均表现出高偏见。此外，对非二元性别、LGBTQIA+和黑人女性的交叉偏见更高。

Conclusion: 研究强调了扩展偏见研究范围的重要性，并指出当前偏见指标的局限性，特别是在低资源语言和方言中的应用。
...
### [140] [Enhancing the Geometric Problem-Solving Ability of Multimodal LLMs via Symbolic-Neural Integration](https://arxiv.org/abs/2504.12773)
*Yicheng Pan,Zhenrong Zhang,Pengfei Hu,Jiefeng Ma,Jun Du,Jianshu Zhang,Quan Liu,Jianqing Gao,Feng Ma*

Main category: cs.CL

TLDR: GeoGen是一个自动生成几何问题分步解答的流程，通过符号推理生成高质量数据，并训练GeoLogic模型以增强MLLMs的逻辑推理能力。


<details>
  <summary>Details</summary>
Motivation: 解决MLLMs在几何问题求解中因缺乏分步解答数据和推理幻觉导致的挑战。

Method: 提出GeoGen流程生成大规模高质量数据，并训练GeoLogic模型结合符号系统验证MLLM输出。

Result: 实验表明该方法显著提升MLLMs在几何推理任务中的性能。

Conclusion: 结合LLMs和符号系统的优势，为几何问题求解提供了更可靠和可解释的方法。
...
### [141] [Assesing LLMs in Art Contexts: Critique Generation and Theory of Mind Evaluation](https://arxiv.org/abs/2504.12805)
*Takaya Arita,Wenxian Zheng,Reiji Suzuki,Fuminori Akiba*

Main category: cs.CL

TLDR: 研究探讨了大型语言模型（LLMs）在艺术批评和心智理论（ToM）任务中的表现，发现通过精心设计的提示，LLMs能生成接近人类水平的艺术批评，并在复杂ToM任务中展现出差异化的能力。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在艺术相关任务中的表现，尤其是批评生成和心智推理能力，以评估其是否能在复杂情境中模拟人类理解。

Method: 结合Noel Carroll的评价框架与艺术批评理论，生成艺术批评；设计新的ToM任务测试LLMs在情感和道德情境中的推理能力。

Result: LLMs生成的艺术批评在风格和解释上接近人类；ToM任务表现因模型和任务复杂度而异，情感和模糊情境揭示明显差异。

Conclusion: LLMs在特定提示下能模拟理解行为，但其能力仍受限于认知复杂性，支持生成式AI悖论的部分观点。
...
### [142] [SMARTe: Slot-based Method for Accountable Relational Triple extraction](https://arxiv.org/abs/2504.12816)
*Xue Wen Tan,Stanley Kok*

Main category: cs.CL

TLDR: SMARTe是一种基于槽注意力机制的关系三元组提取方法，强调可解释性且性能不逊于现有最优模型。


<details>
  <summary>Details</summary>
Motivation: 现有方法注重性能优化但缺乏对模型内部机制的理解，且依赖复杂预处理导致系统不透明。

Method: SMARTe通过槽注意力机制将任务转化为集合预测问题，确保预测可追溯至槽表示和贡献词。

Result: 在NYT和WebNLG数据集上，SMARTe性能与最优模型相当，同时提供可解释性。

Conclusion: SMARTe在保持性能的同时提升了可解释性，未来研究可进一步探索其应用。
...
### [143] [Can LLMs reason over extended multilingual contexts? Towards long-context evaluation beyond retrieval and haystacks](https://arxiv.org/abs/2504.12845)
*Amey Hengle,Prasoon Bajpai,Soham Dan,Tanmoy Chakraborty*

Main category: cs.CL

TLDR: MLRBench是一个新的多语言长上下文推理基准测试，超越现有基准的表面检索任务，评估多跳推理、聚合和认知推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试过于关注检索能力，忽略了推理能力，且易受数据泄漏和短路问题影响。

Method: 提出MLRBench，包含多语言、抗泄漏、可扩展的任务设计，覆盖七种语言。

Result: 实验显示高低资源语言表现差距显著，LLMs在多语言环境下仅利用不到30%的宣称上下文长度。

Conclusion: MLRBench开源以促进多语言LLMs的评估与训练研究。
...
### [144] [ViClaim: A Multilingual Multilabel Dataset for Automatic Claim Detection in Videos](https://arxiv.org/abs/2504.12882)
...
<details>
  <summary>Details</summary>
Motivation: 视频内容作为传播和虚假信息的媒介日益重要，但现有研究主要关注书面文本，缺乏对视频转录中口语复杂性的分析工具。

Method: 构建了包含1,798个标注视频转录的ViClaim数据集，涵盖三种语言和六个主题，并开发了定制标注工具。使用多语言语言模型进行实验。

Result: 模型在交叉验证中表现良好（宏F1达0.896），但在未见主题上泛化能力有限。

Conclusion: ViClaim为视频虚假信息检测提供了坚实基础，揭示了视频转录中声明检测的复杂性。
...
### [145] [Are AI agents the new machine translation frontier? Challenges and opportunities of single- and multi-agent systems for multilingual digital communication](https://arxiv.org/abs/2504.12891)
*Vicent Briva-Iglesias*

Main category: cs.CL

TLDR: 本文探讨了单智能体和多智能体系统在机器翻译（MT）中的潜力，重点分析了多智能体系统在复杂场景下的优势，并通过法律翻译的试点研究验证了其可行性。
...
Method: 通过试点研究，采用多智能体系统（包括翻译、充分性审查、流畅性审查和最终编辑四个专业AI智能体）进行法律翻译。
...
### [146] [Information Gain-Guided Causal Intervention for Autonomous Debiasing Large Language Models](https://arxiv.org/abs/2504.12898)
*Zhouhao Sun,Xiao Ding,Li Du,Yunpeng Xu,Yixuan Ma,Yang Zhao,Bing Qin,Ting Liu*

Main category: cs.CL

TLDR: 提出了一种基于信息增益和因果干预的去偏框架（IGCIDB），用于提升大语言模型的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型（LLMs）在推理时可能仍受数据集偏差影响，导致泛化能力不足，现有去偏方法效果有限。

Method: 结合因果机制与信息理论，提出信息增益引导的因果干预去偏框架，自动平衡指令微调数据分布，并进行标准监督微调。

Result: 实验表明，IGCIDB能有效去偏，提升LLM在不同任务中的泛化能力。

Conclusion: IGCIDF框架为解决LLM的偏差问题提供了一种有效方法。
...
### [147] [Benchmarking Multi-National Value Alignment for Large Language Models](https://arxiv.org/abs/2504.12911)
...
TLDR: NaVAB是一个评估大语言模型（LLMs）与国家价值观对齐的基准，通过自动化流程解决现有方法的局限。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注伦理审查，忽视了国家价值观的多样性，且手动设计的问卷难以扩展。

Method: 提出NaVAB，包括国家价值观提取流程、指令标记建模、筛选冲突减少机制。

Result: 实验表明NaVAB能有效识别LLMs与国家价值观的不对齐情况，并可通过对齐技术减少冲突。

Conclusion: NaVAB为评估和改进LLMs与国家价值观对齐提供了实用工具。
...
### [148] [MAIN: Mutual Alignment Is Necessary for instruction tuning](https://arxiv.org/abs/2504.12913)
*Fanyi Yang,Jianfeng Liu,Xin Zhang,Haoyu Liu,Xixin Cao,Yuefeng Zhan,Hao Sun,Weiwei Deng,Feng Sun,Qi Zhang*

Main category: cs.CL

TLDR: 论文提出了一种名为MAIN的互对齐框架，通过确保指令与响应的对齐来提升大规模语言模型的性能。


<details>
  <summary>Details</summary>
Motivation: 当前的数据生成方法忽视了指令与响应之间的对齐，而高质量的对齐对模型性能至关重要。

Method: 提出了互对齐框架（MAIN），通过相互约束确保指令与响应的连贯性。

Result: 实验表明，基于MAIN微调的LLaMA和Mistral模型在多个基准测试中优于传统方法。

Conclusion: 指令与响应的对齐是提升大规模语言模型性能的关键因素。
...
### [149] [ConExion: Concept Extraction with Large Language Models](https://arxiv.org/abs/2504.12915)
*Ebrahim Norouzi,Sven Hertling,Harald Sack*

Main category: cs.CL

TLDR: 提出一种基于预训练大语言模型（LLMs）的概念提取方法，优于传统关键词提取方法，并在两个基准数据集上验证了其F1分数的提升。
...
Method: 利用预训练LLMs进行概念提取，并探索无监督提示的潜力。

Result: 在两个基准数据集上，F1分数优于现有技术。

Conclusion: LLMs在概念提取任务中表现有效，支持本体评估和学习。
...
### [150] [Are Retrials All You Need? Enhancing Large Language Model Reasoning Without Verbalized Feedback](https://arxiv.org/abs/2504.12951)
*Nearchos Potamitis,Akhil Arora*

Main category: cs.CL

TLDR: 论文提出了一种无需反馈的“重试”机制，通过让大语言模型（LLM）在识别错误答案后重新尝试解决问题，简化了推理框架。研究发现，这种简单方法常优于复杂方法。


<details>
  <summary>Details</summary>
Motivation: 当前基于提示的推理框架需要复杂的自我评估和反馈机制，导致计算成本显著增加。本文旨在探索一种更简单高效的替代方案。

Method: 引入“无反馈重试”机制，允许LLM在发现错误答案时直接重试，无需显式自我反思或反馈。

Result: 研究发现，简单的重试方法常优于复杂推理框架，表明复杂方法的计算成本可能不总是合理的。

Conclusion: 论文挑战了复杂推理策略必然更优的假设，展示了简单高效方法的潜力。
...
### [151] [Estimating Optimal Context Length for Hybrid Retrieval-augmented Multi-document Summarization](https://arxiv.org/abs/2504.12972)
*Adithya Pratapa,Teruko Mitamura*

Main category: cs.CL

TLDR: 提出了一种结合检索增强系统和长上下文窗口的混合方法，用于多文档摘要任务，通过估计最优检索长度提升性能。


<details>
  <summary>Details</summary>
Motivation: 解决长上下文模型在实际应用中效果不佳的问题，同时优化检索增强系统对检索长度选择的敏感性。

Method: 结合检索增强系统和长上下文模型，通过生成银参考估计最优检索长度，并在多文档摘要任务中验证。

Result: 方法在不同模型类别和规模上均有效，优于长上下文基准（如RULER和HELMET）。

Conclusion: 该方法能有效估计长上下文模型的最优检索长度，并适用于新类别模型。
...
### [152] [Sparks of Science: Hypothesis Generation Using Structured Paper Data](https://arxiv.org/abs/2504.12976)
*Charles O'Neill,Tirthankar Ghosal,Roberta Răileanu,Mike Walmsley,Thang Bui,Kevin Schawinski,Ioana Ciucă*

Main category: cs.CL

TLDR: HypoGen是一个包含5500个结构化问题-假设对的数据集，用于改进科学假设生成任务，通过Bit-Flip-Spark框架和推理链提升假设的新颖性和可行性。


<details>
  <summary>Details</summary>
Motivation: 当前基础模型在生成新颖且可行的科学假设方面存在不足，缺乏专门的数据集将科学假设生成任务形式化为自然语言生成任务。

Method: 提出HypoGen数据集，采用Bit-Flip-Spark框架和推理链，将假设生成任务建模为条件语言建模，并通过微调模型提升假设质量。

Result: 实验表明，使用HypoGen数据集微调的模型在假设的新颖性、可行性和整体质量上均有显著提升。

Conclusion: HypoGen为科学假设生成任务提供了有效的数据集和方法，推动了人工智能在科学创新中的应用。
...
### [153] [Accommodate Knowledge Conflicts in Retrieval-augmented LLMs: Towards Reliable Response Generation in the Wild](https://arxiv.org/abs/2504.12982)
*Jiatai Wang,Zhiwei Xu,Di Jin,Xuewen Yang,Tao Li*

Main category: cs.CL

TLDR: 论文提出Swin-VIB框架，通过信息瓶颈模型解决大语言模型（LLMs）在知识冲突中的不确定性，显著提升任务准确性。


<details>
  <summary>Details</summary>
Motivation: LLMs在信息检索中面临内部记忆与外部信息的冲突，导致响应不可靠和决策不确定性。

Method: 提出Swin-VIB框架，结合变分信息瓶颈模型，自适应增强检索信息并指导LLMs生成响应。

Result: 实验表明，Swin-VIB在单选择、开放QA和RAG任务中表现优异，单选择任务准确率提升至少7.54%。

Conclusion: Swin-VIB有效解决LLMs的知识冲突问题，提升响应可靠性。
...
### [154] [SHA256 at SemEval-2025 Task 4: Selective Amnesia -- Constrained Unlearning for Large Language Models via Knowledge Isolation](https://arxiv.org/abs/2504.12996)
*Saransh Agrawal,Kuan-Hao Huang*

Main category: cs.CL

TLDR: 论文提出了一种针对大型语言模型（LLMs）的两阶段目标遗忘方法，结合因果中介分析和分层优化，有效移除敏感信息同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: LLMs在训练中容易记忆敏感信息，现有遗忘方法难以选择性移除数据而不损害模型能力。

Method: 通过因果追踪实验识别关键层（0-5层），采用约束优化方法冻结上层并优化下层，使用联合损失函数。

Result: 在1B模型赛道中排名第二，保持88%基线MMLU准确率。

Conclusion: 因果导向的分层优化为高效精确的LLM遗忘提供了新范式，解决了AI系统中的数据隐私问题。
...
### [155] [ChatEXAONEPath: An Expert-level Multimodal Large Language Model for Histopathology Using Whole Slide Images](https://arxiv.org/abs/2504.13023)
*Sangwook Kim,Soonyoung Lee,Jongseong Jang*

Main category: cs.CL

TLDR: 本文提出了一种基于全切片图像（WSI）的专家级多模态大语言模型ChatEXAONEPath，用于病理学诊断，并通过检索式数据生成和AI评估协议展示了其潜力。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在病理学中因数据局限性难以全面理解临床背景，开发WSI级模型对提升可扩展性和适用性至关重要。

Method: 使用TCGA的10,094对WSI和病理报告构建检索式数据生成管道，并设计AI评估协议验证模型能力。

Result: ChatEXAONEPath在1,134对WSI和报告中诊断接受率达62.9%，能理解多种癌症类型的WSI和临床背景。

Conclusion: 该模型通过整合多模态信息，有望辅助临床医生全面理解复杂病理形态，提升癌症诊断能力。
...
TLDR: 提出了一种基于自检索增强的方面摘要生成框架，解决了传统方法在资源限制和通用性上的不足，同时优化了令牌使用并提升了性能。


<details>
  <summary>Details</summary>
Motivation: 传统摘要方法资源受限且通用性差，大语言模型虽无需训练但依赖提示工程且面临令牌限制和幻觉问题。

Method: 采用嵌入驱动的检索机制提取相关文本片段，删除无关内容，严格基于给定方面生成摘要。

Result: 在基准数据集上表现优异，有效缓解了令牌限制问题。

Conclusion: 提出的框架在性能和令牌优化上均优于现有方法。
...
### [157] [Accuracy is Not Agreement: Expert-Aligned Evaluation of Crash Narrative Classification Models](https://arxiv.org/abs/2504.13068)
*Sudesh Ramesh Bhagat,Ibne Farabi Shihab,Anuj Sharma*

Main category: cs.CL

TLDR: 研究发现，深度学习模型的技术准确性与专家一致性之间存在反直觉关系，而大语言模型（LLMs）在专家一致性上表现更好。
...
Conclusion: 建议在模型评估框架中加入专家一致性指标，并认为LLMs在事故分析中具有潜力。
...
### [158] [Retrieval-Augmented Generation with Conflicting Evidence](https://arxiv.org/abs/2504.13079)
*Han Wang,Archiki Prasad,Elias Stengel-Eskin,Mohit Bansal*

Main category: cs.CL

TLDR: 论文提出RAMDocs数据集和MADAM-RAG方法，用于同时处理用户查询的模糊性、多源冲突信息及噪声，显著提升了模型在模糊查询和抑制错误信息方面的性能。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强生成（RAG）系统通常孤立处理模糊查询或噪声信息，无法同时应对多源冲突。本文旨在联合解决这些问题。

Method: 提出RAMDocs数据集模拟复杂冲突场景，并设计MADAM-RAG多代理辩论框架，通过多轮辩论筛选有效答案并剔除错误信息。

Result: MADAM-RAG在模糊查询（AmbigDocs）和抑制错误信息（FaithEval）任务中分别提升11.40%和15.80%，但RAMDocs对现有基线仍具挑战性。
...
### [159] [LLMs Meet Finance: Fine-Tuning Foundation Models for the Open FinLLM Leaderboard](https://arxiv.org/abs/2504.13125)
...
<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型在金融任务中的应用潜力，并验证其性能提升的可能性。
...
### [160] [Energy-Based Reward Models for Robust Language Model Alignment](https://arxiv.org/abs/2504.13134)
*Anamika Lochab,Ruqi Zhang*

Main category: cs.CL

TLDR: EBRM是一种轻量级后处理框架，通过显式建模奖励分布提升奖励模型的鲁棒性和泛化能力，无需重新训练。
...
Result: 实验显示EBRM在鲁棒性和泛化性上显著提升，安全关键对齐任务提升5.97%。
...
### [161] [Syntactic and Semantic Control of Large Language Models via Sequential Monte Carlo](https://arxiv.org/abs/2504.13139)
...
TLDR: 该论文提出了一种基于序贯蒙特卡罗（SMC）的架构，用于控制语言模型生成，能够灵活地结合领域和问题特定的约束，并在生成过程中高效分配计算资源。
...
Method: 采用序贯蒙特卡罗（SMC）框架，动态调整计算资源并整合约束条件。

Result: 实验表明，该方法在多个领域（如Python代码生成、文本到SQL等）中，小型开源模型性能优于大模型和闭源模型。

Conclusion: 通过更好地近似后验分布，该方法显著提升了生成性能，并为用户提供了简单可编程的解决方案。
...
### [162] [CLIMB: CLustering-based Iterative Data Mixture Bootstrapping for Language Model Pre-training](https://arxiv.org/abs/2504.13161)
...
TLDR: CLIMB是一种自动化框架，通过聚类和迭代优化预训练数据混合，提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 预训练数据通常缺乏明确的领域划分，手动标注成本高，导致数据混合优化困难。

Method: CLIMB通过语义空间嵌入和聚类，结合代理模型和预测器迭代搜索最优数据混合。

Result: 在400B tokens上训练后，1B模型性能超过Llama-3.2-1B 2.0%，特定领域优化提升5%。

Conclusion: CLIMB和其数据集ClimbMix为预训练数据混合优化提供了高效解决方案。
...
### [163] [Interpretable AI-driven Guidelines for Type 2 Diabetes Treatment from Observational Data](https://arxiv.org/abs/2504.12417)
*Dewang Kumar Agarwal,Dimitris J. Bertsimas*

Main category: cs.AI

TLDR: AI模型优化2型糖尿病治疗方案，比医生方案更有效。


<details>
  <summary>Details</summary>
Motivation: 为2型糖尿病治疗提供数据支持的结构化指南。

Method: 使用机器学习优化数据，训练树模型生成治疗方案。

Result: 模型在BMC和Hartford数据上分别比医生方案多降低HbA1c 0.26%和0.13%。

Conclusion: AI方法优于当前实践，可改善患者结果。
...
### [164] [Towards Conversational AI for Human-Machine Collaborative MLOps](https://arxiv.org/abs/2504.12477)
*George Fatouros,Georgios Makridis,George Kousiouris,John Soldatos,Anargyros Tsadimas,Dimosthenis Kyriazis*

Main category: cs.AI

TLDR: 论文介绍了一种基于大语言模型的对话代理系统，旨在提升MLOps中的人机协作。通过模块化设计和多代理架构，系统简化了ML工作流的管理。
...
Method: 采用分层模块化设计，集成KFP代理、MinIO代理和RAG代理，通过自然语言交互实现ML工作流管理。

Result: 系统通过对话界面降低了MLOps的复杂性，提升了用户体验，适用于不同技术水平的用户。

Conclusion: 该对话式MLOps助手有效降低了使用门槛，同时保持了扩展性，为MLOps的普及提供了新思路。
...
### [165] [Agentic AI Optimisation (AAIO): what it is, how it works, why it matters, and how to deal with it](https://arxiv.org/abs/2504.12482)
*Luciano Floridi,Carlotta Buttaboni,Emmie Hine,Jessica Morley,Claudio Novelli,Tyler Schroder*

Main category: cs.AI

TLDR: 本文介绍了Agentic AI Optimisation (AAIO)，一种优化自主AI代理与在线平台交互的方法论，强调其重要性及潜在的社会、伦理和法律影响。


<details>
  <summary>Details</summary>
Motivation: 随着自主AI代理的出现，需要一种新的优化范式（AAIO）来确保其与平台的顺畅交互，类似于SEO对数字内容的影响。

Method: 文章提出AAIO作为方法论，探讨了网站优化与自主AI代理成功之间的相互依赖关系，并研究了其治理、伦理、法律和社会影响（GELSI）。

Result: AAIO能够创造良性循环，但需要主动的监管框架来减轻潜在负面影响。

Conclusion: AAIO是自主数字代理时代的基础设施，应确保其公平和包容的访问。
...
### [166] [Heuristic Recognition and Rapid Response to Unfamiliar Events Outside of Agent Design Scope](https://arxiv.org/abs/2504.12497)
*Robert E. Wray,Steven J. Jones,John E. Laird*

Main category: cs.AI

TLDR: 论文探讨了开放世界中智能体如何应对超出其设计范围的陌生情境，提出了一种结合元知识和元推理的新方法。


<details>
  <summary>Details</summary>
Motivation: 智能体在开放世界中常遇到超出其经验或模型的情境，且缺乏足够时间评估和应对。研究旨在解决这一问题。

Method: 结合领域通用的元知识（受人类认知启发的评估）和元推理，提出新方法。

Result: 该方法能快速适应陌生情境，满足开放世界智能体的性能需求。

Conclusion: 提出的方法为智能体在开放世界中的适应性提供了潜在解决方案。
...
### [167] [Is Trust Correlated With Explainability in AI? A Meta-Analysis](https://arxiv.org/abs/2504.12529)
*Zahra Atf,Peter R. Lewis*

Main category: cs.AI

TLDR: 研究表明AI系统的可解释性虽能提升用户信任，但效果有限，并非唯一因素。


<details>
  <summary>Details</summary>
Motivation: 探讨AI可解释性是否显著提升用户信任，并分析其社会技术影响。

Method: 采用元分析方法，整合90项研究数据。

Result: 发现可解释性与用户信任之间存在显著但适度的正相关。

Conclusion: 强调需关注AI的长期可信度，而非仅短期信任，尤其在医疗和司法等关键领域。
...
### [168] [ZeroSumEval: Scaling LLM Evaluation with Inter-Model Competition](https://arxiv.org/abs/2504.12562)
*Haidar Khan,Hisham A. Alyahya,Yazeed Alnumay,M Saiful Bari,Bülent Yener*

Main category: cs.AI

TLDR: ZeroSumEval是一种基于竞争的新型评估协议，通过零和游戏动态评估LLMs，避免传统方法的过拟合、高成本和偏见问题。


<details>
  <summary>Details</summary>
Motivation: 传统评估方法（如静态数据集、人工评估或模型评估）存在过拟合、高成本和偏见问题，需要一种更动态、抗饱和的评估方式。

Method: ZeroSumEval采用零和游戏框架，涵盖多种游戏类型（如安全挑战、经典游戏、知识测试和说服挑战），评估LLMs的战略推理、规划、知识应用和创造力。

Result: 实验表明，前沿模型（如GPT和Claude家族）在常见游戏和问答中表现良好，但在需要创造新颖问题的游戏中表现不佳，且无法可靠地互相破解。
...
### [169] [The Chronicles of Foundation AI for Forensics of Multi-Agent Provenance](https://arxiv.org/abs/2504.12612)
*Ching-Chun Chang,Isao Echizen*

Main category: cs.AI

TLDR: 研究提出了一种用于追踪多智能体生成内容来源的时序系统，通过符号化的时间戳记录实现事后归因，无需依赖内部状态或外部元信息。


<details>
  <summary>Details</summary>
Motivation: 随着AI向多智能体协作发展，生成内容的来源追踪变得复杂，需要一种方法来记录和归因集体创作中的贡献。

Method: 提出了一种基于符号化时间戳记录的系统，通过反馈循环更新生成历史，并与生成内容同步。

Result: 系统能够从内容本身追溯生成历史，实现多智能体协作中的来源追踪。

Conclusion: 研究为协作AI提供了一种可追溯的来源追踪方法，有助于构建负责任的AI生态系统。
...
### [170] [Embodied-R: Collaborative Framework for Activating Embodied Spatial Reasoning in Foundation Models via Reinforcement Learning](https://arxiv.org/abs/2504.12680)
*Baining Zhao,Ziyou Wang,Jianjie Fang,Chen Gao,Fanhang Man,Jinqiang Cui,Xin Wang,Xinlei Chen,Yong Li,Wenwu Zhu*

Main category: cs.AI

TLDR: Embodied-R框架结合视觉语言模型（VLM）和小型语言模型（LM），通过强化学习实现高效的空间推理能力，仅需少量训练样本即可达到先进水平。


<details>
  <summary>Details</summary>
Motivation: 研究预训练模型如何从视觉观察中获取高级空间推理能力。

Method: 结合VLM和LM，使用强化学习（RL）和新型奖励系统（考虑逻辑一致性），训练模型进行慢思考。

Result: 在5k样本训练后，3B参数的LM在空间推理任务上达到先进水平，并展现出系统性分析和上下文整合能力。

Conclusion: Embodied-R展示了高效的空间推理能力，并探讨了奖励设计、模型泛化等研究问题。
...
### [171] [WebLists: Extracting Structured Information From Complex Interactive Websites Using Executable LLM Agents](https://arxiv.org/abs/2504.12682)
...
TLDR: 论文提出了WebLists基准测试和BardeenAgent框架，用于解决大规模结构化数据提取任务，显著提升了性能和成本效益。
...
Method: 提出BardeenAgent框架，通过将代理执行转化为可重复程序，并利用HTML的规则结构生成通用CSS选择器。

Result: 在WebLists基准测试中，BardeenAgent的召回率达到66%，是现有最佳代理的两倍，且每行输出成本降低3倍。

Conclusion: BardeenAgent显著提升了大规模数据提取任务的性能，为实际应用提供了高效解决方案。
...
### [172] [InstructRAG: Leveraging Retrieval-Augmented Generation on Instruction Graphs for LLM-Based Task Planning](https://arxiv.org/abs/2504.13032)
*Zheng Wang,Shu Xian Teo,Jun Jie Chew,Wei Shi*

Main category: cs.AI

TLDR: InstructRAG提出了一种基于多智能体元强化学习的框架，通过结合检索增强生成（RAG）解决任务规划中的可扩展性和可迁移性问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于TAO过程的方法受限于LLM对复杂任务的知识不足，而RAG提供了利用外部数据库增强生成的机会。

Method: InstructRAG包含一个组织历史指令路径的图、一个用于扩展图覆盖的RL-Agent和一个用于任务泛化的ML-Agent，两者端到端训练。

Result: 在四个任务规划数据集上，InstructRAG显著提升性能，最高比现有方法提升19.2%。

Conclusion: InstructRAG通过多智能体框架有效解决了RAG在任务规划中的挑战，并展示了优越的适应性和性能。
...
### [173] [Exploring Expert Failures Improves LLM Agent Tuning](https://arxiv.org/abs/2504.13145)
*Li-Cheng Lan,Andrew Bai,Minhao Cheng,Ruochen Wang,Cho-Jui Hsieh,Tianyi Zhou*

Main category: cs.AI

TLDR: EEF方法通过利用专家失败轨迹中的有益行动，解决了复杂子任务，显著提升了LLM代理的性能。


<details>
  <summary>Details</summary>
Motivation: 发现专家失败轨迹中的有益行动能显著提升代理探索效率和关键技能获取，从而解决复杂子任务。

Method: 提出EEF方法，从失败轨迹中提取有益行动并整合到训练数据中，同时排除有害行动。

Result: EEF在WebShop中达到62%胜率，超越RFT和GPT-4，并在SciWorld中创下新高。

Conclusion: EEF通过利用专家失败信息，显著提升了LLM代理的性能，解决了复杂子任务。
...
### [174] [Antidistillation Sampling](https://arxiv.org/abs/2504.13146)
*Yash Savani,Asher Trockman,Zhili Feng,Avi Schwarzschild,Alexander Robey,Marc Finzi,J. Zico Kolter*

Main category: cs.AI

TLDR: 论文提出了一种名为“抗蒸馏采样”的方法，通过修改模型的下一词概率分布，毒化推理轨迹，从而在不影响模型性能的情况下限制蒸馏效果。
...
Method: 采用抗蒸馏采样策略，通过策略性地修改模型的下一词概率分布，毒化推理轨迹。

Result: 抗蒸馏采样显著降低了推理轨迹对蒸馏的有效性，同时保持了模型的实用性。

Conclusion: 抗蒸馏采样为模型所有者提供了一种有效限制蒸馏的方法，同时不影响模型性能。
...
### [175] [Readable Twins of Unreadable Models](https://arxiv.org/abs/2504.13150)
...
<details>
  <summary>Details</summary>
Motivation: 创建负责任的人工智能系统需要可解释性，而深度学习模型通常难以理解，因此需要一种方法将其转换为可读的形式。

Method: 基于物理对象的数字孪生概念，提出创建可读的不精确信息流模型（IIFM）来替代不可读的深度学习模型（DLM），并详细描述了转换过程。

Result: 通过手写数字MNIST数据集的图像识别分类模型示例验证了方法的可行性。

Conclusion: 该方法为深度学习模型的可解释性提供了一种新思路，有助于构建更负责任的人工智能系统。
...
### [176] [Sleep-time Compute: Beyond Inference Scaling at Test-time](https://arxiv.org/abs/2504.13171)
*Kevin Lin,Charlie Snell,Yu Wang,Charles Packer,Sarah Wooders,Ion Stoica,Joseph E. Gonzalez*

Main category: cs.AI

TLDR: 通过离线预计算（sleep-time compute）减少大语言模型（LLM）的测试时计算需求，显著降低延迟和成本。


<details>
  <summary>Details</summary>
Motivation: 测试时计算需求高导致延迟和成本问题，希望通过离线预计算优化。

Method: 引入sleep-time compute，预计算用户可能查询的内容，减少测试时计算。

Result: 在Stateful GSM-Symbolic和Stateful AIME任务中，测试时计算需求减少5倍，准确率提升13%-18%。

Conclusion: sleep-time compute能有效降低计算成本，提升性能，适用于可预测查询的场景。
...
### [177] [DMM: Building a Versatile Image Generation Model via Distillation-Based Model Merging](https://arxiv.org/abs/2504.12364)
*Tianhui Song,Weixin Feng,Shuai Wang,Xubin Li,Tiezheng Ge,Bo Zheng,Limin Wang*

Main category: cs.CV

TLDR: 提出了一种基于风格提示的图像生成方法（DMM），将多个文本到图像生成模型合并为一个多功能模型，解决了参数冗余和存储成本问题。


<details>
  <summary>Details</summary>
Motivation: 解决文本到图像生成模型中因大量专用模型导致的参数冗余和存储成本问题，并避免静态线性插值方法在风格混合中的不兼容性。

Method: 提出风格提示的图像生成管道，通过风格向量控制生成任意风格的图像，并基于此设计了一种基于分数蒸馏的模型合并范式（DMM）。

Result: 实验表明，DMM能有效整合多个教师模型的知识，实现可控的任意风格生成。

Conclusion: DMM为文本到图像生成任务中的模型合并提供了新的目标和评估协议，成功实现了多功能模型的紧凑重组。
...
### [178] [Geographical Context Matters: Bridging Fine and Coarse Spatial Information to Enhance Continental Land Cover Mapping](https://arxiv.org/abs/2504.12368)
...
### [179] [WORLDMEM: Long-term Consistent World Simulation with Memory](https://arxiv.org/abs/2504.12369)
*Zeqi Xiao,Yushi Lan,Yifan Zhou,Wenqi Ouyang,Shuai Yang,Yanhong Zeng,Xingang Pan*

Main category: cs.CV

TLDR: WorldMem通过引入记忆银行和注意力机制，解决了世界模拟中长期一致性问题，并支持动态场景建模。


<details>
  <summary>Details</summary>
Motivation: 现有世界模拟方法因时间窗口限制，难以保持长期一致性，尤其是3D空间一致性。

Method: 提出WorldMem框架，利用记忆银行存储记忆帧和状态（如姿态和时间戳），并通过注意力机制提取相关信息。

Result: 实验证明，WorldMem能准确重建场景，支持动态世界建模，适用于虚拟和真实场景。

Conclusion: WorldMem有效解决了长期一致性问题，并扩展了世界模拟的动态能力。
...
### [180] [InstantCharacter: Personalize Any Characters with a Scalable Diffusion Transformer Framework](https://arxiv.org/abs/2504.12395)
...
TLDR: InstantCharacter是一个基于扩散变换器的可扩展框架，用于字符定制，解决了现有方法的泛化能力差和图像质量低的问题。
...
Method: 提出InstantCharacter框架，采用扩散变换器，引入可扩展适配器和大型字符数据集。

Result: 实现了高保真、文本可控且字符一致的图像生成，性能优于现有方法。

Conclusion: InstantCharacter为字符驱动的图像生成设定了新基准。
...
### [181] [NTIRE 2025 Challenge on Event-Based Image Deblurring: Methods and Results](https://arxiv.org/abs/2504.12401)
...
TLDR: NTIRE 2025挑战赛聚焦事件驱动的图像去模糊，旨在通过事件和图像输入实现高质量去模糊，无计算复杂度限制，15支团队提交有效结果。


<details>
  <summary>Details</summary>
Motivation: 推动事件驱动视觉研究的进展，探索事件与图像结合的去模糊方法。

Method: 利用事件和图像作为输入，通过PSNR评估性能，无模型复杂度限制。

Result: 199人注册，15支团队提交有效结果，展示了当前事件驱动去模糊的技术水平。

Conclusion: 挑战赛为事件驱动视觉研究提供了新方向，有望推动该领域进一步发展。
...
TLDR: 提出了一种稀疏优化（SO）框架，通过动态调整少量参数解决视觉语言模型（VLM）在少样本适应中的过拟合和计算限制问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如低秩重参数化）在泛化和超参数调优上表现不佳，需要一种更高效的方法。

Method: 采用局部稀疏全局密度和局部随机全局重要性的双重范式，动态调整参数。

Result: 在11个数据集上，SO实现了最先进的少样本适应性能，同时降低了内存开销。

Conclusion: SO框架显著缓解了过拟合问题，在低数据环境下实现了稳定的模型适应。
...
### [183] [3D-PointZshotS: Geometry-Aware 3D Point Cloud Zero-Shot Semantic Segmentation Narrowing the Visual-Semantic Gap](https://arxiv.org/abs/2504.12442)
*Minmin Yang,Huantao Ren,Senem Velipasalar*

Main category: cs.CV

TLDR: 3D-PointZshotS是一个几何感知的零样本点云分割框架，通过潜在几何原型和自一致性损失提升特征生成和对齐能力，在多个数据集上表现优异。
...
Method: 引入潜在几何原型（LGPs）和自一致性损失，通过交叉注意力机制增强语义特征，并在共享空间中重新表示特征。

Result: 在ScanNet、SemanticKITTI和S3DIS数据集上，性能优于四个基线方法（谐波mIoU）。

Conclusion: 3D-PointZshotS通过几何感知和特征对齐，显著提升了零样本点云分割的效果。
...
### [184] [DG-MVP: 3D Domain Generalization via Multiple Views of Point Clouds for Classification](https://arxiv.org/abs/2504.12456)
*Huantao Ren,Minmin Yang,Senem Velipasalar*

Main category: cs.CV

TLDR: 提出了一种基于2D投影的3D点云领域泛化方法，解决了点云数据因遮挡和缺失点导致的领域偏移问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D点云分类依赖大规模标注数据，但LiDAR数据标注成本高，而CAD模型生成的点云数据与真实数据存在领域差异。

Method: 通过多视角2D投影缓解点云缺失问题，并采用卷积模型提取特征。

Result: 在PointDA-10和Sim-to-Real基准测试中表现优于基线方法，能有效从合成领域迁移到真实领域。

Conclusion: 该方法显著提升了3D点云在不同领域的泛化能力。
...
### [185] [AdaVid: Adaptive Video-Language Pretraining](https://arxiv.org/abs/2504.12513)
*Chaitanya Patel,Juan Carlos Niebles,Ehsan Adeli*

Main category: cs.CV

TLDR: AdaVid是一个动态调整计算资源的视频编码框架，通过自适应Transformer块和轻量级层次网络，在计算效率和性能之间取得平衡。


<details>
  <summary>Details</summary>
Motivation: 现有视频编码器计算需求高且仅能处理短片段，难以在边缘设备上部署。

Method: 引入自适应Transformer块和轻量级层次网络，动态调整隐藏层维度和处理长视频。

Result: AdaVid-EgoVLP在计算资源减半时性能与EgoVLP相当，资源相同时更优；在长视频任务中表现高效。

Conclusion: AdaVid为视频编码提供了高效灵活的解决方案，适用于资源受限场景。
...
### [186] [Event Quality Score (EQS): Assessing the Realism of Simulated Event Camera Streams via Distances in Latent Space](https://arxiv.org/abs/2504.12515)
*Kaustav Chanda,Aayush Atul Verma,Arpitsinh Vaghela,Yezhou Yang,Bharatesh Chakravarthi*

Main category: cs.CV

TLDR: 论文提出了一种名为事件质量分数（EQS）的指标，用于评估模拟事件相机数据的质量，以缩小模拟与真实数据的差距。


<details>
  <summary>Details</summary>
Motivation: 事件相机在深度学习中应用受限，主要由于高质量标记数据稀缺，而现有模拟数据难以准确反映真实事件相机数据。

Method: 利用RVT架构的激活特征，提出EQS作为模拟事件数据的质量评估指标。

Result: 在DSEC驾驶数据集上的实验表明，更高的EQS分数意味着模型在模拟数据训练后对真实数据的泛化能力更强。

Conclusion: EQS有助于开发更真实的事件相机模拟器，减少模拟与真实数据的差距。
...
### [187] [Decision-based AI Visual Navigation for Cardiac Ultrasounds](https://arxiv.org/abs/2504.12535)
*Andy Dimnaku,Dominic Yurk,Zhiyuan Gao,Arun Padmanabhan,Mandar Aras,Yaser Abu-Mostafa*

Main category: cs.CV

TLDR: 本文提出了一种基于AI的新型导航系统，用于辅助超声检查中的心脏下腔静脉（IVC）定位，适用于不同质量的超声设备。


<details>
  <summary>Details</summary>
Motivation: 传统心脏超声检查需要专业人员和高端设备，限制了其在医院外的应用。AI导航系统可帮助新手操作者获取标准化视图，扩展超声诊断的适用范围。

Method: 系统采用离线训练的决策模型，通过二分类判断IVC是否存在，并结合新型定位算法实时标注IVC空间位置。
...
### [188] [Set You Straight: Auto-Steering Denoising Trajectories to Sidestep Unwanted Concepts](https://arxiv.org/abs/2504.12782)
*Leyang Li,Shilin Lu,Yan Ren,Adams Wai-Kin Kong*

Main category: cs.CV

TLDR: ANT是一种新的微调框架，通过自动引导去噪轨迹避免有害内容生成，解决了现有概念擦除方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 确保文本到图像模型的伦理部署，需要防止生成有害或不适当内容。现有方法存在局限性，如锚点自由方法可能导致视觉伪影，锚点方法依赖启发式选择。

Method: ANT通过反转分类器自由引导的条件方向，提出轨迹感知目标，保护早期阶段得分函数场的完整性。单概念擦除使用增强权重显著性图，多概念擦除提供通用目标函数。

Result: 实验表明，ANT在单概念和多概念擦除中均达到最先进效果，生成高质量、安全的输出，且不损害生成保真度。

Conclusion: ANT提供了一种高效、无需启发式锚点选择的概念擦除方法，显著提升了文本到图像模型的伦理安全性。
...
### [189] [Post-Hurricane Debris Segmentation Using Fine-Tuned Foundational Vision Models](https://arxiv.org/abs/2504.12542)
*Kooshan Amini,Yuhao Liu,Jamie Ellen Padgett,Guha Balakrishnan,Ashok Veeraraghavan*

Main category: cs.CV

TLDR: 该研究通过微调预训练视觉模型，开发了一种通用的飓风碎片分割方法，使用小规模高质量数据集，并在未参与训练的数据上表现优异。
...
Method: 通过微调预训练视觉模型，利用约1,200张手动标注的飓风图像数据集，结合多标注者标签聚合和视觉提示工程。

Result: 模型fCLIPSeg在未参与训练的飓风Ida数据上达到Dice分数0.70，且在无碎片区域几乎无假阳性。

Conclusion: 该研究首次提出了一种仅需标准RGB图像的通用碎片分割模型，适用于快速大规模灾害评估。
...
### [190] [Privacy-Preserving Operating Room Workflow Analysis using Digital Twins](https://arxiv.org/abs/2504.12552)
...
TLDR: 提出了一种两阶段隐私保护手术室视频分析方法，通过生成数字孪生（DT）实现去标识化，并利用SafeOR模型进行事件检测，性能媲美原始RGB视频模型。


<details>
  <summary>Details</summary>
Motivation: 手术室工作流程优化需要自动识别事件，但隐私问题限制了计算机视觉的应用，因此需要隐私保护方法。

Method: 两阶段方法：首先生成去标识化的数字孪生（DT），然后使用SafeOR模型（融合分割掩码和深度图）进行事件检测。

Result: DT方法在事件检测上性能与原始RGB视频模型相当或更好。

Conclusion: 数字孪生支持隐私保护的工作流程分析，促进跨机构数据共享，并可能通过减少外观差异提升模型泛化能力。
...
### [191] [Contour Field based Elliptical Shape Prior for the Segment Anything Model](https://arxiv.org/abs/2504.12556)
*Xinyu Zhao,Jun Liu,Faqiang Wang,Li Cui,Yuping Duan*

Main category: cs.CV

TLDR: 提出了一种将椭圆形状先验信息集成到基于深度学习的SAM图像分割技术中的新方法，通过变分方法提升分割精度。
...
Result: 在特定图像数据集上实验表明，该方法优于原始SAM。

Conclusion: 通过变分方法集成椭圆先验，显著提升了SAM的分割精度。
...
### [192] [ArtistAuditor: Auditing Artist Style Pirate in Text-to-Image Generation Models](https://arxiv.org/abs/2504.13061)
*Linkang Du,Zheng Zhu,Min Chen,Zhou Su,Shouling Ji,Peng Cheng,Jiming Chen,Zhikun Zhang*

Main category: cs.CV

TLDR: 论文提出了一种名为ArtistAuditor的新方法，用于审核文本到图像生成模型是否使用了特定艺术家的作品进行微调，通过分析风格特征实现高效识别。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法（如扰动或水印）在艺术品或模型已发布后无法修改或重新训练时的版权侵权问题。

Method: 使用风格提取器获取多粒度风格表示，并通过训练判别器进行审核决策。

Result: 在六种模型和数据集组合上，AUC值超过0.937，验证了方法的有效性。

Conclusion: ArtistAuditor在真实场景中表现出色，为版权保护提供了实用工具。
...
### [193] [Parsimonious Dataset Construction for Laparoscopic Cholecystectomy Structure Segmentation](https://arxiv.org/abs/2504.12573)
*Yuning Zhou,Henry Badgery,Matthew Read,James Bailey,Catherine Davey*

Main category: cs.CV

TLDR: 论文提出了一种基于主动学习的手术视频帧选择方法，用于构建高质量且经济实惠的腹腔镜胆囊切除术数据集，以解决医学标注成本高的问题。


<details>
  <summary>Details</summary>
Motivation: 医学领域的标注成本高昂，阻碍了深度学习在相关领域的应用。本文旨在通过主动学习方法降低标注成本，同时提高数据集质量。

Method: 采用主动学习方法，通过深度神经网络（DNNs）从新收集的数据中识别最具信息量的帧，并将其纳入训练数据。评估了不同的数据信息量度量方法，发现深度特征距离在此任务中最有效。

Result: 实验表明，使用主动学习选择的一半数据训练的DNNs，在关键解剖结构和手术器械上的平均交并比（mIoU）为0.4349，接近使用完整数据集训练的DNNs的0.4374 mIoU。

Conclusion: 主动学习方法能够显著降低标注成本，同时保持模型性能，为医学图像分割任务提供了一种高效的数据集构建策略。
...
### [194] [Prompt-Driven and Training-Free Forgetting Approach and Dataset for Large Language Models](https://arxiv.org/abs/2504.12574)
*Zhenyu Yu,Mohd Yamani Inda Idris,Pei Wang*

Main category: cs.CV

TLDR: 提出了一种基于提示的分层编辑和无训练局部特征移除的自动数据集创建框架，用于扩散模型的选择性遗忘，并引入了Entangled评估指标。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像生成中的广泛应用增加了对隐私合规遗忘的需求，但现有方法难以在移除敏感信息的同时保持非敏感区域的连贯性。

Method: 通过基于提示的分层编辑和无训练局部特征移除构建ForgetMe数据集，并引入Entangled评估指标，支持配对和非配对数据的无监督评估。

Result: 在ForgetMe数据集上应用LoRA微调实现了选择性遗忘，验证了数据集和评估指标的有效性。

Conclusion: 该工作为隐私保护的生成AI提供了可扩展和适应性强的解决方案。
...
### [195] [CM3AE: A Unified RGB Frame and Event-Voxel/-Frame Pre-training Framework](https://arxiv.org/abs/2504.12576)
*Wentao Wu,Xiao Wang,Chenglong Li,Bo Jiang,Jin Tang,Bin Luo,Qi Liu*

Main category: cs.CV

TLDR: 提出了一种名为CM3AE的新型预训练框架，用于RGB-Event感知，通过多模态融合重建和对比学习策略，增强跨模态信息聚合能力。
...
Conclusion: CM3AE为基于事件和RGB-Event融合的下游任务提供了强大支持，代码和模型将开源。
...
### [196] [3DResT: A Strong Baseline for Semi-Supervised 3D Referring Expression Segmentation](https://arxiv.org/abs/2504.12599)
*Wenxin Chen,Mengxue Qu,Weitai Kang,Yan Yan,Yao Zhao,Yunchao Wei*

Main category: cs.CV

TLDR: 论文提出了一种半监督学习框架3DResT，通过TSCS和QDW方法高效利用伪标签，显著提升了3D-RES任务的性能。


<details>
  <summary>Details</summary>
Motivation: 3D-RES任务通常需要大量实例级标注，成本高昂。现有半监督学习方法在3D-RES中未能充分利用伪标签，限制了模型性能。

Method: 提出3DResT框架，包含TSCS（选择高质量伪标签）和QDW（动态加权低质量伪标签）两种设计。

Result: 实验表明，仅用1%标注数据，3DResT比全监督方法mIoU提升8.34点。

Conclusion: 3DResT通过高效利用伪标签，显著降低了标注成本并提升了性能。
...
### [197] [AdaQual-Diff: Diffusion-Based Image Restoration via Adaptive Quality Prompting](https://arxiv.org/abs/2504.12605)
*Xin Su,Chen Wu,Yu Zhang,Chen Lyu,Zhuoran Zheng*

Main category: cs.CV

TLDR: AdaQual-Diff是一种基于扩散的框架，通过将感知质量评估直接集成到生成修复过程中，解决了传统方法在复杂真实世界退化图像修复中的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖间接线索，难以适应复杂退化情况，无法准确捕捉感知质量缺陷。
...
### [198] [Robo-SGG: Exploiting Layout-Oriented Normalization and Restitution for Robust Scene Graph Generation](https://arxiv.org/abs/2504.12606)
*Changsheng Lv,Mengshi Qi,Zijian Fu,Huadong Ma*

Main category: cs.CV

TLDR: 提出Robo-SGG方法，利用布局信息增强场景图生成在损坏图像上的鲁棒性，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有SGG方法在损坏图像上性能下降，需解决域偏移问题。

Method: 采用实例归一化和布局导向恢复，设计布局嵌入编码器增强特征。

Result: 在VG-C数据集上，mR@50提升5.6%-8.0%，达到新SOTA。

Conclusion: Robo-SGG作为即插即用模块，显著提升SGG在损坏图像上的鲁棒性。
...
### [199] [SAM-Based Building Change Detection with Distribution-Aware Fourier Adaptation and Edge-Constrained Warping](https://arxiv.org/abs/2504.12619)
*Yun-Cheng Li,Sen Lei,Yi-Tao Zhao,Heng-Chao Li,Jun Li,Antonio Plaza*

Main category: cs.CV

TLDR: FAEWNet提出了一种基于SAM的网络，结合分布感知傅里叶适配和边缘约束变形，用于建筑物变化检测，解决了领域差距和噪声干扰问题。


<details>
  <summary>Details</summary>
Motivation: 建筑物变化检测在城市化、灾害评估和军事侦察中具有重要应用，但现有方法（如SAM）因领域差距和噪声干扰表现不佳。

Method: FAEWNet利用SAM编码器提取特征，引入分布感知傅里叶适配器聚合任务导向的变化信息，并设计流模块优化边缘提取和变化感知。

Result: 在LEVIR-CD、S2Looking和WHU-CD数据集上取得了最先进的结果。

Conclusion: FAEWNet通过创新适配器和流模块，显著提升了建筑物变化检测的准确性和边缘识别能力。
...
### [200] [Packing Input Frame Context in Next-Frame Prediction Models for Video Generation](https://arxiv.org/abs/2504.12626)
*Lvmin Zhang,Maneesh Agrawala*

Main category: cs.CV

TLDR: FramePack是一种神经网络结构，用于视频生成的下一帧预测，通过压缩输入帧固定上下文长度，提高训练效率，并提出抗漂移采样方法。


<details>
  <summary>Details</summary>
Motivation: 解决视频生成中因帧数多导致的上下文长度不固定和计算瓶颈问题，同时减少误差累积。

Method: 提出FramePack压缩输入帧固定上下文长度，使用抗漂移采样方法（逆时序生成帧），并支持对现有视频扩散模型的微调。

Result: 能够高效处理大量帧，训练批量显著提高，视觉质量因更平衡的扩散调度器而改善。

Conclusion: FramePack为视频生成提供了一种高效且质量优化的解决方案。
...
### [201] [RoPETR: Improving Temporal Camera-Only 3D Detection by Integrating Enhanced Rotary Position Embedding](https://arxiv.org/abs/2504.12643)
*Hang Ji,Tao Ni,Xufeng Huang,Tao Luo,Xin Zhan,Junbo Chen*

Main category: cs.CV

TLDR: 针对StreamPETR框架的改进，通过定制化的位置嵌入策略提升速度估计能力，在NuScenes测试集上达到70.86%的NDS，创下相机3D目标检测新标杆。


<details>
  <summary>Details</summary>
Motivation: StreamPETR在3D边界框检测中表现优异，但速度估计成为NuScenes数据集上的瓶颈，需针对性改进。

Method: 提出定制化的位置嵌入策略，增强时间建模能力。

Result: 在NuScenes测试集上，改进后的方法使用ViT-L主干网络，NDS达到70.86%，成为相机3D目标检测的新标杆。

Conclusion: 通过优化速度估计，显著提升了StreamPETR在NuScenes数据集上的性能。
...
### [202] [AdaptoVision: A Multi-Resolution Image Recognition Model for Robust and Scalable Classification](https://arxiv.org/abs/2504.12652)
*Md. Sanaullah Chowdhury Lameya Sabrin*

Main category: cs.CV

TLDR: AdaptoVision是一种新型CNN架构，通过优化结构和减少计算需求，在保持高精度的同时实现高效性能。


<details>
  <summary>Details</summary>
Motivation: 设计一种能够在计算复杂性和分类精度之间高效平衡的CNN架构，适用于资源受限环境。

Method: 采用增强残差单元、深度可分离卷积和分层跳跃连接，减少参数和计算需求。

Result: 在BreakHis数据集上达到SOTA，CIFAR-10和CIFAR-100上分别取得95.3%和85.77%的精度。

Conclusion: AdaptoVision结构高效，适合实时和资源受限场景，具有广泛部署潜力。
...
### [203] [Two Tasks, One Goal: Uniting Motion and Planning for Excellent End To End Autonomous Driving Performance](https://arxiv.org/abs/2504.12667)
*Lin Liu,Ziying Song,Hongyu Pan,Lei Yang,Caiyan Jia*

Main category: cs.CV

TLDR: TTOG是一种新颖的两阶段轨迹生成框架，通过统一规划和运动任务，解决了传统方法中两者分离的问题，并在nuScenes和Bench2Drive数据集上取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 传统自动驾驶方法将规划与运动任务解耦，忽略了从运动任务中学习分布外数据的潜在优势。统一这些任务面临共享上下文表示构建和其他车辆状态不可观测等挑战。

Method: 提出TTOG框架，首先生成多样化轨迹候选，第二阶段通过车辆状态信息优化候选轨迹。为解决周围车辆状态不可观测问题，采用自车数据训练的状态估计器，并引入ECSA增强场景表示泛化能力。

Result: 在nuScenes数据集上L2距离减少36.06%，在Bench2Drive数据集上驾驶分数提升22%，显著优于现有基线。

Conclusion: TTOG通过统一规划和运动任务，实现了端到端自动驾驶的先进性能，验证了其框架的有效性和泛化能力。
...
### [204] [Accurate Tracking of Arabidopsis Root Cortex Cell Nuclei in 3D Time-Lapse Microscopy Images Based on Genetic Algorithm](https://arxiv.org/abs/2504.12676)
*Yu Song,Tatsuaki Goh,Yinhao Li,Jiahua Dong,Shunsuke Miyashima,Yutaro Iwamoto,Yohei Kondo,Keiji Nakajima,Yen-wei Chen*

Main category: cs.CV

TLDR: 提出了一种基于遗传算法的细胞核追踪方法，解决了密集排列细胞跟踪不准确的问题。


<details>
  <summary>Details</summary>
Motivation: 解决现有细胞追踪软件在细胞密集排列时性能不足的问题。

Method: 采用遗传算法，结合拟南芥根细胞模式和空间关系，分粗到细两阶段进行核追踪。

Result: 在长时间活体成像数据上验证，仅需少量人工修正即可准确追踪核。
...
### [205] [TongUI: Building Generalized GUI Agents by Learning from Multimodal Web Tutorials](https://arxiv.org/abs/2504.12679)
*Bofei Zhang,Zirui Shang,Zhi Gao,Wang Zhang,Rui Xie,Xiaojian Ma,Tao Yuan,Xinxiao Wu,Song-Chun Zhu,Qing Li*

Main category: cs.CV

TLDR: TongUI框架通过从多模态网络教程中学习，构建通用GUI代理，解决了GUI代理开发中轨迹数据不足的问题。
...
Method: 爬取并处理在线GUI教程（如视频和文章）为轨迹数据，构建GUI-Net数据集，并基于此微调Qwen2.5-VL-3B/7B模型。

Result: TongUI代理在多个基准测试中表现优异，性能提升约10%。

Conclusion: GUI-Net数据集和TongUI框架的有效性得到验证，代码、数据集和模型将开源。
...
### [206] [HSS-IAD: A Heterogeneous Same-Sort Industrial Anomaly Detection Dataset](https://arxiv.org/abs/2504.12689)
*Qishan Wang,Shuyong Gao,Junjie Hu,Jiawen Yu,Xuan Tong,You Li,Wenqiang Zhang*

Main category: cs.CV

TLDR: 论文提出了HSS-IAD数据集，解决了现有工业异常检测数据集在多样性和真实性上的不足，并评估了流行方法在多类和单类设置下的表现。


<details>
  <summary>Details</summary>
Motivation: 现有工业异常检测数据集缺乏多样性和真实性，无法反映真实工厂条件，因此需要更贴近实际的数据集。

Method: 构建了HSS-IAD数据集，包含8,580张金属类工业零件图像，提供精确异常标注和前景图像用于合成异常生成。

Result: 评估了流行工业异常检测方法，展示了数据集在多类和单类设置下的潜力。

Conclusion: HSS-IAD数据集填补了现有数据集与真实工厂条件之间的差距，为多类无监督异常检测提供了更真实的测试平台。
...
### [207] [Collaborative Perception Datasets for Autonomous Driving: A Review](https://arxiv.org/abs/2504.12696)
*Naibang Wang,Deyong Shang,Yan Gong,Xiaoxi Hu,Ziying Song,Lei Yang,Yuhan Huang,Xiaoyu Wang,Jianli Lu*

Main category: cs.CV

TLDR: 该论文对自动驾驶中的协作感知数据集进行了首次全面综述，从多维度比较现有资源，并提出了未来研究方向。
...
Method: 从合作范式、数据来源、场景、传感器模态和支持任务等维度对数据集进行分类和比较分析。

Result: 提供了详细的比较分析，并指出了数据集可扩展性、多样性、领域适应等关键挑战。

Conclusion: 论文总结了现有协作感知数据集，提出了未来方向，并提供了在线资源库支持研究。
...
### [208] [Unsupervised Cross-Domain 3D Human Pose Estimation via Pseudo-Label-Guided Global Transforms](https://arxiv.org/abs/2504.12699)
*Jingjing Liu,Zhiyong Wang,Xinyu Fan,Amirhossein Dadashzadeh,Honghai Liu,Majid Mirmehdi*

Main category: cs.CV

TLDR: 提出了一种新框架，通过全局变换解决跨场景3D人体姿态估计中的域偏移问题，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有3D人体姿态估计方法在跨场景推断时因域偏移（如相机视角、位置等）导致性能下降，需解决此问题。

Method: 包括伪标签生成模块和全局变换模块，利用人体中心坐标系对齐不同域的姿势，并结合姿势增强器提升泛化能力。

Result: 在多个跨数据集基准测试中表现优于现有方法，甚至超过目标域训练模型。

Conclusion: 提出的框架有效解决了跨场景3D姿态估计中的域偏移问题，具有显著性能提升。
...
### [209] [SmartFreeEdit: Mask-Free Spatial-Aware Image Editing with Complex Instruction Understanding](https://arxiv.org/abs/2504.12704)
*Qianqian Sun,Jixiang Luo,Dell Zhang,Xuelong Li*

Main category: cs.CV

TLDR: SmartFreeEdit 是一种新型端到端框架，结合多模态大语言模型和超图增强修复架构，通过自然语言指令实现精确、无掩码的图像编辑。
...
Method: 引入区域感知标记和掩码嵌入范式，设计推理分割管道，以及超图增强修复模块。

Result: 在多个评估指标上超越现有方法，包括分割准确性、指令遵循和视觉质量保持。

Conclusion: SmartFreeEdit 解决了局部信息聚焦问题，提升了编辑图像的全局一致性。
...
### [210] [Self-Supervised Pre-training with Combined Datasets for 3D Perception in Autonomous Driving](https://arxiv.org/abs/2504.12709)
*Shumin Wang,Zhuoran Yang,Lidian Wang,Zhipeng Tang,Heng Li,Lehan Pan,Sha Zhang,Jie Peng,Jianmin Ji,Yanyong Zhang*

Main category: cs.CV

TLDR: 论文提出了一种利用大规模无标签数据预训练3D感知模型的自监督框架，结合提示适配器减少数据集偏差，显著提升下游任务性能。
...
Method: 提出自监督预训练框架，利用异构数据集的无标签数据学习3D表示，结合提示适配器进行领域适应。

Result: 模型在3D目标检测、BEV分割、3D目标跟踪和占用预测等任务中表现显著提升，且性能随数据量增加而稳定增长。

Conclusion: 展示了大规模数据预训练对自动驾驶3D感知模型的持续潜力，并将开源代码以促进社区研究。
...
### [211] [NTIRE 2025 Challenge on Day and Night Raindrop Removal for Dual-Focused Images: Methods and Results](https://arxiv.org/abs/2504.12711)
...
TLDR: NTIRE 2025挑战赛综述，聚焦于日夜间雨滴去除任务，使用多样化的Raindrop Clarity数据集，吸引了361名参与者，32支团队提交了SOTA解决方案。


<details>
  <summary>Details</summary>
Motivation: 建立一个新的强大基准，用于在不同光照和对焦条件下去除雨滴的任务。

Method: 收集并使用了多样化的Raindrop Clarity数据集，分为训练、验证和测试子集，吸引了361名参与者提交解决方案。

Result: 32支团队提交了有效解决方案，并在Raindrop Clarity数据集上实现了SOTA性能。

Conclusion: 挑战赛成功建立了雨滴去除任务的新基准，并展示了多样数据集的实际应用价值。
...
### [212] [Post-pre-training for Modality Alignment in Vision-Language Foundation Models](https://arxiv.org/abs/2504.12717)
*Shin'ya Yamaguchi,Dewei Feng,Sekitoshi Kanai,Kazuki Adachi,Daiki Chijiwa*

Main category: cs.CV

TLDR: CLIP-Refine是一种后预训练方法，通过随机特征对齐和混合对比蒸馏技术，在1个epoch的小数据集训练中减少模态间隙，提升零样本性能。


<details>
  <summary>Details</summary>
Motivation: CLIP模型在多模态特征空间中存在模态间隙，影响下游任务性能，现有方法成本高或性能下降。

Method: 提出CLIP-Refine，结合随机特征对齐（RaFA）和混合对比蒸馏（HyCD），在小数据集上快速对齐特征空间。

Result: 实验表明，CLIP-Refine有效减少模态间隙，提升零样本性能。

Conclusion: CLIP-Refine是一种高效的后预训练方法，无需牺牲零样本性能即可优化特征对齐。
...
### [213] [Mask Image Watermarking](https://arxiv.org/abs/2504.12739)
*Runyi Hu,Jie Zhang,Shiqian Zhao,Nils Lukas,Jiwei Li,Qing Guo,Han Qiu,Tianwei Zhang*

Main category: cs.CV

TLDR: MaskMark是一个高效灵活的图像水印框架，包含支持全局和局部水印的两种变体，通过简单的掩码机制和编码-失真-解码范式实现高性能水印嵌入与提取。
...
Method: 基于编码-失真-解码范式，MaskMark-D在解码阶段引入掩码机制支持局部提取，MaskMark-ED在编码阶段也加入掩码以增强局部鲁棒性。

Result: MaskMark在全局和局部水印提取、定位及多水印嵌入方面表现最优，计算成本仅为WAM的1/15。

Conclusion: MaskMark是一个高效、灵活且高性能的水印框架，适用于多种应用场景。
...
### [214] [Privacy Protection Against Personalized Text-to-Image Synthesis via Cross-image Consistency Constraints](https://arxiv.org/abs/2504.12747)
*Guanyu Wang,Kailong Wang,Yihao Huang,Mingyi Zhou,Zhang Qing cnwatcher,Geguang Pu,Li Li*

Main category: cs.CV

TLDR: 论文提出了一种跨图像反个性化（CAP）框架，通过增强图像间的风格一致性来提升隐私保护效果。


<details>
  <summary>Details</summary>
Motivation: 随着扩散模型和个性化技术的发展，公开图像可能被用于生成高度逼真的仿冒肖像，引发隐私问题。现有反个性化方法未充分利用多图像关系。

Method: 提出CAP框架，通过跨图像风格一致性增强扰动效果，并采用动态比率调整策略平衡损失影响。

Result: 在CelebHQ和VGGFace2基准测试中，CAP显著优于现有方法。

Conclusion: CAP为多图像隐私保护提供了更有效的解决方案。
...
### [215] [LAD-Reasoner: Tiny Multimodal Models are Good Reasoners for Logical Anomaly Detection](https://arxiv.org/abs/2504.12749)
*Weijia Li,Guanglei Chu,Jiong Chen,Guo-Sen Xie,Caifeng Shan,Fang Zhao*

Main category: cs.CV

TLDR: 论文提出了一种新的任务RLAD和框架LAD-Reasoner，通过逻辑推理扩展传统异常检测，使用小型多模态语言模型实现高效且可解释的检测。
...
Method: 采用两阶段训练范式：监督微调（SFT）和组相对策略优化（GRPO），结合检测准确性和输出结构质量作为奖励信号。

Result: 在MVTec LOCO AD数据集上，LAD-Reasoner性能媲美更大模型，且生成更简洁可解释的推理。

Conclusion: LAD-Reasoner减少了对大型模型和复杂管道的依赖，提供了透明可解释的逻辑异常检测方案。
...
### [216] [Stronger, Steadier & Superior: Geometric Consistency in Depth VFM Forges Domain Generalized Semantic Segmentation](https://arxiv.org/abs/2504.12753)
*Siyu Chen,Ting Han,Changshe Zhang,Xin Luo,Meiliu Wu,Guorong Cai,Jinhe Su*

Main category: cs.CV

TLDR: DepthForge框架通过整合视觉基础模型（VFMs）和深度信息，提升域泛化语义分割（DGSS）的性能，尤其在极端条件下表现优异。


<details>
  <summary>Details</summary>
Motivation: 视觉线索易受干扰，而几何信息（如深度）更稳定，因此研究如何结合深度信息与VFMs特征以提升几何一致性和泛化能力。

Method: 提出DepthForge框架，结合冻结的DINOv2/EVA02视觉特征与Depth Anything V2深度信息，通过深度感知可学习标记和深度细化解码器增强模型。

Result: 在多个DGSS设置和未见目标域数据集上，DepthForge性能显著优于其他方法，尤其在极端条件下表现突出。

Conclusion: DepthForge通过整合深度信息与VFMs特征，显著提升了域泛化语义分割的性能和泛化能力。
...
### [217] [EarthGPT-X: Enabling MLLMs to Flexibly and Comprehensively Understand Multi-Source Remote Sensing Imagery](https://arxiv.org/abs/2504.12795)
*Wei Zhang,Miaoxin Cai,Yaqian Ning,Tong Zhang,Yin Zhuang,He Chen,Jun Li,Xuerui Mao*

Main category: cs.CV

TLDR: EarthGPT-X是一个针对遥感图像的多模态大语言模型，通过视觉提示和灵活的交互能力，解决了自然空间模型在遥感领域的适应性问题。


<details>
  <summary>Details</summary>
Motivation: 遥感图像与自然图像差异大，现有模型适应性差且交互方式有限，限制了实际应用。

Method: 提出多模态内容整合方法、跨域单阶段融合训练策略和像素感知模块，统一了参考和定位任务。

Result: 实验显示EarthGPT-X在多粒度任务中表现优越，多模态交互灵活。

Conclusion: EarthGPT-X显著推动了遥感领域多模态大语言模型的发展。
...
### [218] [TSGS: Improving Gaussian Splatting for Transparent Surface Reconstruction via Normal and De-lighting Priors](https://arxiv.org/abs/2504.12799)
*Mingwei Li,Pu Pang,Hehe Fan,Hua Huang,Yi Yang*

Main category: cs.CV

TLDR: TSGS提出了一种新框架，通过分离几何学习和外观优化，解决了透明表面重建中的透明-深度困境，显著提升了几何精度和渲染效果。


<details>
  <summary>Details</summary>
Motivation: 透明表面重建在实验室机器人操作等任务中至关重要，但现有3D高斯泼溅（3DGS）方法因透明-深度困境导致几何精度不足。

Method: TSGS分两阶段：几何学习阶段使用抑制镜面反射的输入；外观优化阶段通过各向异性镜面建模提升视觉保真度，同时采用滑动窗口方法提取第一表面深度。

Result: 在TransLab数据集上，TSGS显著优于现有方法，减少了37.3%的Chamfer距离，提升了8.0%的F1分数。

Conclusion: TSGS在3DGS框架内实现了透明对象的精确几何重建和逼真渲染，代码和数据集将公开。
...
### [219] [Hybrid Dense-UNet201 Optimization for Pap Smear Image Segmentation Using Spider Monkey Optimization](https://arxiv.org/abs/2504.12807)
*Ach Khozaimi,Isnani Darti,Syaiful Anam,Wuryansari Muharini Kusumawinahyu*

Main category: cs.CV

TLDR: 提出了一种结合DenseNet201和U-Net的混合模型，并通过蜘蛛猴优化算法（SMO）进行优化，显著提升了宫颈细胞图像分割的性能。


<details>
  <summary>Details</summary>
Motivation: 传统分割模型在处理复杂的宫颈细胞结构和图像变化时表现不佳，需要更高效的方法。

Method: 采用Dense-UNet201模型，以DenseNet201作为编码器，并利用改进的SMO算法优化模型。

Result: 在SIPaKMeD数据集上，Dense-UNet201表现优于其他模型，准确率达96.16%，IoU为91.63%，Dice系数为95.63%。
...
### [220] [Saliency-Aware Diffusion Reconstruction for Effective Invisible Watermark Removal](https://arxiv.org/abs/2504.12809)
*Inzamamul Alam,Md Tanvir Islam,Simon S. Woo*

Main category: cs.CV

TLDR: 本文提出了一种名为SADRE的新框架，用于去除数字内容中的水印，结合了自适应噪声注入、区域特定扰动和先进的扩散重建技术，显著提升了水印去除的鲁棒性和图像质量。


<details>
  <summary>Details</summary>
Motivation: 随着数字内容的普及，现有水印嵌入技术缺乏鲁棒性，亟需更有效的水印去除方法。

Method: SADRE框架通过基于显著性掩码的自适应噪声注入和反向扩散过程，破坏水印的同时保留图像关键特征。

Result: 实验表明，SADRE在多种场景下均能有效去除水印，并在水印破坏与图像质量之间取得平衡，优于现有技术。

Conclusion: SADRE为水印去除设定了新标准，提供了灵活可靠的解决方案，适用于实际网络内容。
...
### [221] [TwoSquared: 4D Generation from 2D Image Pairs](https://arxiv.org/abs/2504.12825)
*Lu Sang,Zehranaz Canfes,Dongliang Cao,Riccardo Marin,Florian Bernard,Daniel Cremers*

Main category: cs.CV

TLDR: TwoSquared方法通过两步分解4D动态物体生成问题：1）基于高质量3D资产的图像到3D生成模块；2）物理启发的变形模块预测中间运动，仅需两张2D图像即可生成纹理和几何一致的4D序列。


<details>
  <summary>Details</summary>
Motivation: 尽管生成式AI取得显著进展，4D动态物体生成仍面临训练数据有限、计算需求高的挑战。TwoSquared旨在解决这一问题，仅需两张2D图像生成物理合理的4D序列。

Method: TwoSquared将问题分解为两步：1）基于现有3D生成模型的图像到3D生成；2）物理启发的变形模块预测中间运动。无需模板或类别先验知识，支持野外图像输入。

Result: 实验表明，TwoSquared仅需两张2D图像即可生成纹理和几何一致的4D序列。

Conclusion: TwoSquared通过两步分解方法，有效解决了4D动态物体生成的挑战，展示了其在生成一致4D序列方面的潜力。
...
### [222] [Image-Editing Specialists: An RLAIF Approach for Diffusion Models](https://arxiv.org/abs/2504.12833)
*Elior Benarous,Yilun Du,Heng Yang*

Main category: cs.CV

TLDR: 提出了一种基于指令的图像编辑扩散模型训练方法，通过强化学习框架实现与人类偏好的对齐，无需大量人工标注数据。


<details>
  <summary>Details</summary>
Motivation: 解决图像编辑中结构保持与语义对齐的挑战，简化用户操作，提高编辑的精确性和真实性。

Method: 采用在线强化学习框架，结合视觉提示，减少对长文本提示的依赖，仅需少量参考图像进行训练。

Result: 模型在复杂场景中实现精确、结构一致的编辑，同时保持高保真度，仅需10步训练即可完成复杂编辑。

Conclusion: 该方法在图像编辑和机器人仿真中展现出高效性和多功能性，显著提升了编辑质量和实用性。
...
### [223] [High-Fidelity Image Inpainting with Multimodal Guided GAN Inversion](https://arxiv.org/abs/2504.12844)
*Libo Zhang,Yongsheng Yu,Jiali Yao,Heng Fan*

Main category: cs.CV

TLDR: MMInvertFill是一种新型GAN反演方法，通过多模态引导编码器和F&W+潜在空间改进图像修复性能。


<details>
  <summary>Details</summary>
Motivation: 现有GAN反演方法忽视输入输出未掩码区域一致性，且忽略多模态辅助信息，导致性能下降。

Method: 提出多模态引导编码器和预调制模块，结合F&W+潜在空间和软更新均值潜在模块。

Result: 在六个数据集上定性和定量优于现有方法，支持域外图像修复。

Conclusion: MMInvertFill通过多模态和潜在空间优化，显著提升图像修复效果。
...
### [224] [Computer-Aided Design of Personalized Occlusal Positioning Splints Using Multimodal 3D Data](https://arxiv.org/abs/2504.12868)
*Agnieszka Anna Tomaka,Leszek Luchowski,Michał Tarnawski,Dariusz Pojda*

Main category: cs.CV

TLDR: 提出了一种基于数字化技术的咬合板设计与精度评估方法，结合临床需求与牙科实践。


<details>
  <summary>Details</summary>
Motivation: 现代数字技术在定制医疗器械设计中具有重要作用，尤其是用于治疗颌系统功能障碍的咬合板。本文旨在将临床概念与当前数字化牙科实践结合。

Method: 通过虚拟患者模型（基于口内扫描、CBCT、3D面部扫描和石膏模型数字化）生成3D咬合板，利用变换矩阵表示治疗性下颌位置变化，并通过虚拟压花解决表面冲突。

Result: 展示了如何通过临床工具和口内设备获取变换矩阵，并通过轮廓和表面偏差分析评估设计和打印咬合板的精度。

Conclusion: 该方法实现了可重复、个性化的咬合板制作，为诊断、多模态图像配准和咬合差异量化提供了新可能。
...
### [225] [SC3EF: A Joint Self-Correlation and Cross-Correspondence Estimation Framework for Visible and Thermal Image Registration](https://arxiv.org/abs/2504.12869)
*Xi Tong,Xing Luo,Jiangxin Yang,Yanpeng Cao*

Main category: cs.CV

TLDR: 提出了一种名为SC3EF的新框架，通过结合局部特征和全局上下文线索，解决了RGB-T图像配准的挑战。


<details>
  <summary>Details</summary>
Motivation: 多光谱成像在智能交通中至关重要，但RGB-T图像配准因模态差异大而困难。

Method: 设计了基于卷积-Transformer的流程，提取局部特征并编码全局相关性，结合分层光流估计解码器优化配准结果。

Result: 在RGB-T数据集上表现优于现有方法，并在大视差、遮挡等挑战性场景中展现泛化能力。
...
### [226] [Tree-NeRV: A Tree-Structured Neural Representation for Efficient Non-Uniform Video Encoding](https://arxiv.org/abs/2504.12899)
*Jiancheng Zhao,Yifan Zhan,Qingtian Zhu,Mingze Ma,Muyao Niu,Zunian Wan,Xiang Ji,Yinqiang Zheng*

Main category: cs.CV

TLDR: Tree-NeRV提出了一种基于二叉搜索树的非均匀采样方法，优化视频编码的效率和适应性。
...
Method: 采用二叉搜索树组织特征表示，引入优化驱动的非均匀采样策略。
...
### [227] [Second-order Optimization of Gaussian Splats with Importance Sampling](https://arxiv.org/abs/2504.12905)
*Hamza Pehlivan,Andrea Boscolo Camiletto,Lin Geng Foo,Marc Habermann,Christian Theobalt*

Main category: cs.CV

TLDR: 提出了一种基于Levenberg-Marquardt和共轭梯度的二阶优化策略，显著提升了3D高斯溅射的训练速度。


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射依赖一阶优化器（如Adam）导致训练时间长，需改进。

Method: 利用Jacobian的稀疏性，提出矩阵无关的GPU并行LM优化，结合采样策略和学习率启发式方法。

Result: 方法在低高斯数量时比Adam快6倍，标准LM快3倍，中高斯数量时仍具竞争力。

Conclusion: 二阶优化策略有效提升了3D高斯溅射的训练效率。
...
### [228] [Efficient Masked Image Compression with Position-Indexed Self-Attention](https://arxiv.org/abs/2504.12923)
*Chengjie Dai,Tiantian Song,Hui Tang,Fangdong Chen,Bowei Yang,Guanghua Song*

Main category: cs.CV

TLDR: 提出一种基于位置索引自注意力机制的图像压缩方法，仅编码和解码掩码图像的可见部分，显著降低计算成本。
...
### [229] [Disentangling Polysemantic Channels in Convolutional Neural Networks](https://arxiv.org/abs/2504.12939)
...
<details>
  <summary>Details</summary>
Motivation: CNN中的多义通道难以解释，阻碍了机制可解释性研究。

Method: 通过分析前一层的不同激活模式，重构CNN权重，解耦多义通道。

Result: 成功将多义通道分解为单义通道，提升了特征可视化的解释性。

Conclusion: 该方法有效增强了CNN的可解释性，为机制分析提供了新工具。
...
### [230] [Rethinking Temporal Fusion with a Unified Gradient Descent View for 3D Semantic Occupancy Prediction](https://arxiv.org/abs/2504.12959)
*Dubing Chen,Huan Zheng,Jin Fang,Xingping Dong,Xianfei Li,Wenlong Liao,Tao He,Pai Peng,Jianbing Shen*

Main category: cs.CV

TLDR: GDFusion是一种用于视觉3D语义占用预测的时间融合方法，通过关注时间线索和融合策略，显著提升了性能并降低了内存消耗。


<details>
  <summary>Details</summary>
Motivation: 探索VisionOcc框架中未充分研究的时间融合问题，关注时间线索和融合策略。

Method: 提出三种时间线索（场景一致性、运动校准、几何互补）和基于梯度下降的融合策略，重新解释RNN公式。

Result: 在nuScenes数据集上表现优异，Occ3D基准上mIoU提升1.4%-4.8%，内存消耗减少27%-72%。

Conclusion: GDFusion通过系统性时间线索和高效融合策略，显著提升了VisionOcc的性能和效率。
...
### [231] [Vision and Language Integration for Domain Generalization](https://arxiv.org/abs/2504.12966)
*Yanmei Wang,Xiyao Liu,Fupeng Chu,Zhi Han*

Main category: cs.CV

TLDR: VLCA结合语言和视觉空间，通过语义空间作为桥梁，解决领域泛化中图像特征空间不可靠的问题。


<details>
  <summary>Details</summary>
Motivation: 由于领域差距，图像特征空间难以找到可靠的共同特征，而语言具有更完整的语义表达元素。

Method: 利用语言空间的词向量距离捕捉语义关系，视觉空间通过低秩近似探索同类样本的共同模式，最后通过多模态空间对齐语言和视觉表示。
...
### [232] [MathPhys-Guided Coarse-to-Fine Anomaly Synthesis with SQE-Driven Bi-Level Optimization for Anomaly Detection](https://arxiv.org/abs/2504.12970)
*Long Qian,Bingke Zhu,Yingying Chen,Ming Tang,Jinqiao Wang*

Main category: cs.CV

TLDR: 论文提出了一种基于数学物理模型指导的合成异常检测方法，通过粗到细的优化策略和合成质量评估器（SQE），显著提升了异常检测的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 由于真实缺陷图像的稀缺性和不可预测性，现有合成方法生成的异常图像缺乏物理一致性，影响了模型的泛化能力。

Method: 采用数学物理模型指导合成异常，通过粗到细的两阶段优化（npcF和npcF++）提升异常的真实性，并结合SQE驱动的加权策略优化训练。

Result: 在MVTec AD、VisA和BTAD三个基准测试中，方法在图像和像素级别的AUROC指标上均达到最先进水平。

Conclusion: 提出的MaPhC2F和BiSQAD方法通过物理建模和优化策略，显著提升了合成异常的真实性和检测性能。
...
### [233] [Enhancing Cocoa Pod Disease Classification via Transfer Learning and Ensemble Methods: Toward Robust Predictive Modeling](https://arxiv.org/abs/2504.12992)
*Devina Anduyan,Nyza Cabillo,Navy Gultiano,Mark Phil Pacot*

Main category: cs.CV

TLDR: 该研究通过集成迁移学习和三种集成学习策略（Bagging、Boosting和Stacking），提出了一种用于可可豆荚病害分类的集成方法，其中Bagging表现最佳，测试准确率达100%。


<details>
  <summary>Details</summary>
Motivation: 提高可可豆荚病害分类的准确性和鲁棒性，以支持精准农业和自动化作物病害管理。

Method: 使用预训练的卷积神经网络（如VGG16、ResNet50等）作为基础学习器，结合Bagging、Boosting和Stacking三种集成策略，对三类病害（黑腐病、豆荚蛀虫和健康）进行分类。

Result: Bagging方法表现最佳，测试准确率为100%，优于Boosting（97%）和Stacking（92%）。

Conclusion: 迁移学习与集成技术的结合显著提升了模型的泛化能力和可靠性，为精准农业提供了有效解决方案。
...
### [234] [All-in-One Transferring Image Compression from Human Perception to Multi-Machine Perception](https://arxiv.org/abs/2504.12997)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常以单任务方式将学习到的图像压缩（LIC）模型适应下游任务，效率低且缺乏任务交互。
...
### [235] [Hierarchical Feature Learning for Medical Point Clouds via State Space Model](https://arxiv.org/abs/2504.13015)
*Guoqing Zhang,Jingyun Yang,Yang Li*

Main category: cs.CV

TLDR: 本文提出了一种基于状态空间模型（SSM）的分层特征学习框架，用于医学点云理解，并在新构建的大规模数据集MedPointS上验证了其优越性能。
...
Method: 通过最远点采样将输入下采样至多层级，结合KNN查询聚合多尺度结构信息，并引入坐标顺序和内外扫描策略优化SSM处理。

Result: 在MedPointS数据集上的实验表明，该方法在解剖分类、补全和分割任务中均表现优异。

Conclusion: 提出的SSM框架为医学点云分析提供了有效解决方案，且数据集和代码已公开。
...
### [236] [Pose and Facial Expression Transfer by using StyleGAN](https://arxiv.org/abs/2504.13021)
*Petr Jahoda,Jan Cech*

Main category: cs.CV

TLDR: 提出了一种将姿态和表情从源人脸图像迁移到目标人脸的方法，利用StyleGAN2的潜在空间生成输出，无需人工标注，支持随机身份合成。


<details>
  <summary>Details</summary>
Motivation: 实现人脸图像中姿态和表情的迁移，避免人工标注，提高效率。

Method: 使用两个编码器和一个映射网络将输入投影到StyleGAN2的潜在空间，自监督训练。

Result: 能够合成随机身份的可控姿态和表情，接近实时性能。

Conclusion: 该方法高效且无需标注，适用于人脸图像编辑。
...
### [237] [Riemannian Patch Assignment Gradient Flows](https://arxiv.org/abs/2504.13024)
*Daniel Gonzalez-Alvarado,Fabio Schlindwein,Jonas Cassel,Laura Steingruber,Stefania Petra,Christoph Schnörr*

Main category: cs.CV

TLDR: 提出了一种基于图结构的度量数据标签分配方法，通过动态交互和几何数值积分优化标签一致性。


<details>
  <summary>Details</summary>
Motivation: 解决图中度量数据标签分配的局部初始标签不一致问题。
...
### [238] [TTRD3: Texture Transfer Residual Denoising Dual Diffusion Model for Remote Sensing Image Super-Resolution](https://arxiv.org/abs/2504.13026)
*Yide Liu,Haijiang Sun,Xiaowen Zhang,Qiaoyuan Liu,Zhouchang Chen,Chongzhuo Xiao*

Main category: cs.CV

TLDR: 论文提出了一种名为TTRD3的模型，通过多尺度特征提取、纹理转移和双扩散模型解决了遥感图像超分辨率重建中的三大挑战。


<details>
  <summary>Details</summary>
Motivation: 现有方法在遥感图像超分辨率重建中存在多尺度特征提取困难、语义不一致性以及几何精度与视觉质量之间的权衡问题。

Method: TTRD3包含三个创新：多尺度特征聚合块（MFAB）、稀疏纹理转移引导模块（STTG）和残差去噪双扩散模型（RDDM）。

Result: 实验表明，TTRD3在多源遥感数据集上优于现有方法，LPIPS和FID指标分别提升了1.43%和3.67%。

Conclusion: TTRD3通过结合多尺度特征提取、纹理转移和双扩散模型，显著提升了遥感图像超分辨率重建的性能。
...
### [239] [Prototypes are Balanced Units for Efficient and Effective Partially Relevant Video Retrieval](https://arxiv.org/abs/2504.13035)
*WonJun Moon,Cheol-Ho Cho,Woojin Jun,Minho Shim,Taeoh Kim,Inwoong Lee,Dongyoon Wee,Jae-Pil Heo*

Main category: cs.CV

TLDR: 提出了一种原型PRVR框架，通过固定数量的原型编码视频多样上下文，结合跨模态和单模态重建任务，提升检索精度与效率。


<details>
  <summary>Details</summary>
Motivation: 在部分相关视频检索（PRVR）中，同时实现高精度和高效率具有挑战性，传统方法因多样上下文表示导致计算和内存成本增加。

Method: 设计原型PRVR框架，将视频上下文编码为固定数量原型；引入跨模态和单模态重建任务对齐文本与视频特征；使用视频混合技术优化原型对齐。

Result: 在TVR、ActivityNet-Captions和QVHighlights数据集上验证了方法的有效性，且未牺牲效率。

Conclusion: 原型PRVR框架通过原型编码和重建任务，成功平衡了检索精度与效率。
...
### [240] [Event-Enhanced Blurry Video Super-Resolution](https://arxiv.org/abs/2504.13042)
...
<details>
  <summary>Details</summary>
Motivation: 现有BVSR方法因缺乏运动信息和高频细节，难以在高分辨率下恢复清晰细节，导致伪影和抖动。

Method: 引入事件信号，提出互惠特征去模糊模块和混合可变形对齐模块，结合帧和事件信息优化特征去模糊和运动估计。

Result: 在合成和真实数据集上达到最优性能，真实数据上比FMA-Net准确度提升2.59 dB，速度提升7.28倍。

Conclusion: Ev-DeblurVSR通过事件信号显著提升了BVSR的性能和效率。
...
### [241] [Expert Kernel Generation Network Driven by Contextual Mapping for Hyperspectral Image Classification](https://arxiv.org/abs/2504.13045)
*Guandong Li,Mengxia Ye*

Main category: cs.CV

TLDR: EKGNet提出了一种基于改进3D-DenseNet的模型，通过上下文感知映射网络和动态核生成模块，解决了高光谱图像分类中的高维数据、地物稀疏分布和光谱冗余问题，提升了分类性能。


<details>
  <summary>Details</summary>
Motivation: 高光谱图像分类中，高维数据、地物稀疏分布和光谱冗余导致分类过拟合和泛化能力受限，需要一种更高效的模型。

Method: EKGNet结合上下文感知映射模块和动态核生成模块，前者生成卷积核组合权重，后者构建自适应专家卷积系统，动态处理不同区域。
...
### [242] [NoisyRollout: Reinforcing Visual Reasoning with Data Augmentation](https://arxiv.org/abs/2504.13055)
...
Conclusion: NoisyRollout通过增强探索能力和视觉感知，显著提升了视觉语言模型的性能。
...
### [243] [Imaging for All-Day Wearable Smart Glasses](https://arxiv.org/abs/2504.13060)
*Michael Goesele,Daniel Andersen,Yujia Chen,Simon Green,Eddy Ilg,Chao Li,Johnson Liu,Grace Kuo,Logan Wan,Richard Newcombe*

Main category: cs.CV

TLDR: 本文分析了智能眼镜成像的基本限制，并提出了一种分布式成像方法以减少相机模块尺寸。


<details>
  <summary>Details</summary>
Motivation: 智能眼镜需要全天佩戴，需满足体积、重量、时尚性和社会接受度等要求，这对成像解决方案提出了挑战。

Method: 系统分析智能眼镜成像空间，提出分布式成像方法，并通过合成数据和原型实验验证。

Result: 分布式成像方法相比传统单块设计能显著减小相机模块尺寸。
...
### [244] [EchoWorld: Learning Motion-Aware World Models for Echocardiography Probe Guidance](https://arxiv.org/abs/2504.13065)
*Yang Yue,Yulin Wang,Haojun Jiang,Pan Liu,Shiji Song,Gao Huang*

Main category: cs.CV

TLDR: EchoWorld是一个基于世界建模的运动感知超声探头引导框架，通过预训练和微调策略提升引导精度，显著减少误差。
...
Method: 提出EchoWorld框架，结合预训练（预测掩膜解剖区域和模拟探头调整效果）和微调（运动感知注意力机制整合历史数据）。

Result: 在超过100万张超声图像上验证，EchoWorld显著减少引导误差，优于现有视觉骨干和引导框架。

Conclusion: EchoWorld通过运动感知和世界建模有效提升超声探头引导精度，为AI辅助扫描提供实用解决方案。
...
### [245] [SkyReels-V2: Infinite-length Film Generative Model](https://arxiv.org/abs/2504.13074)
...
TLDR: SkyReels-V2是一个无限长度电影生成模型，结合多模态大语言模型、多阶段预训练、强化学习和扩散框架，解决了视频生成中的动态质量、时长和镜头感知问题。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型在动态质量、时长和镜头感知方面存在局限，难以实现真实长视频和专业电影风格生成。

Method: 结合多模态大语言模型和子专家模型设计视频结构表示，分四阶段训练：基础生成、运动强化学习、扩散框架优化和最终视觉质量提升。

Result: 实现了高质量、长时长且符合电影语法的视频生成。

Conclusion: SkyReels-V2为长视频和专业风格生成提供了有效解决方案。
...
### [246] [Effective Dual-Region Augmentation for Reduced Reliance on Large Amounts of Labeled Data](https://arxiv.org/abs/2504.13077)
...
<details>
  <summary>Details</summary>
Motivation: 解决传统方法对大规模标注数据的依赖问题，提升模型在跨域任务（如SFDA和ReID）中的表现。

Method: 通过对前景对象施加随机噪声扰动和背景区域的空间打乱，增加训练数据的多样性。

Result: 在PACS数据集上显著优于现有方法，同时在Market-1501和DukeMTMC-reID数据集上验证了ReID任务的有效性。

Conclusion: 该方法通过结构化数据增强，提供了一种减少标注数据依赖的可扩展解决方案。
...
### [247] [Enhancing Person-to-Person Virtual Try-On with Multi-Garment Virtual Try-Off](https://arxiv.org/abs/2504.13078)
*Riza Velioglu,Petra Bevandic,Robin Chan,Barbara Hammer*

Main category: cs.CV

TLDR: 论文介绍了TryOffDiff，一种基于扩散的VTOFF模型，用于从穿着服装的人中提取标准化服装图像，并在VTON中优化p2p-VTON效果。


<details>
  <summary>Details</summary>
Motivation: 解决虚拟试穿（VTON）和虚拟试脱（VTOFF）中的挑战，特别是从穿着服装的人中提取标准化服装图像，并优化p2p-VTON的效果。

Method: 基于潜在扩散框架和SigLIP图像条件，TryOffDiff捕捉服装的纹理、形状和图案特性，并结合类别特定嵌入实现多服装VTOFF。

Result: 在VITON-HD数据集上达到最佳效果，在DressCode数据集上表现优异，支持上装、下装和连衣裙。

Conclusion: TryOffDiff是首个多服装VTOFF模型，与VTON模型结合可减少p2p-VTON中不希望的属性转移（如肤色）。
...
### [248] [EventVAD: Training-Free Event-Aware Video Anomaly Detection](https://arxiv.org/abs/2504.13092)
...
TLDR: EventVAD是一种结合动态图架构和多模态大语言模型的事件感知视频异常检测框架，通过时间-事件推理实现高效检测。


<details>
  <summary>Details</summary>
Motivation: 监督方法需要大量训练数据且泛化能力有限，而训练免费方法在细粒度视觉转换和多样化事件定位上存在挑战。

Method: EventVAD采用动态时空图建模捕获事件特征，通过无监督统计特征检测事件边界，并利用分层提示策略指导MLLM推理。

Result: 在UCF-Crime和XD-Violence数据集上，EventVAD使用7B MLLM在训练免费设置下达到SOTA性能。

Conclusion: EventVAD通过结合动态图与MLLM，显著提升了视频异常检测的效果，尤其在无训练数据场景下表现优异。
...
### [249] [RF-DETR Object Detection vs YOLOv12 : A Study of Transformer-based and CNN-based Architectures for Single-Class and Multi-Class Greenfruit Detection in Complex Orchard Environments Under Label Ambiguity](https://arxiv.org/abs/2504.13099)
*Ranjan Sapkota,Rahul Harsha Cheppally,Ajay Sharda,Manoj Karkee*

Main category: cs.CV

TLDR: 比较RF-DETR和YOLOv12在复杂果园环境中检测绿色水果的性能，RF-DETR在全局上下文建模和复杂场景中表现更优，而YOLOv12在计算效率和边缘部署上更具优势。


<details>
  <summary>Details</summary>
Motivation: 研究旨在评估两种模型在复杂果园环境（标签模糊、遮挡、背景混合）中检测绿色水果的能力，为精准农业应用提供参考。

Method: 使用自定义数据集（单类和多类标注），比较RF-DETR（基于DINOv2和可变形注意力）和YOLOv12（基于CNN注意力）的性能。

Result: RF-DETR在单类检测中mAP50最高（0.9464），多类检测中mAP@50最高（0.8298）；YOLOv12在mAP@50:95和计算效率上表现更好。
...
### [250] [UniEdit-Flow: Unleashing Inversion and Editing in the Era of Flow Models](https://arxiv.org/abs/2504.13109)
*Guanlong Jiao,Biqing Huang,Kuan-Chieh Wang,Renjie Liao*

Main category: cs.CV

TLDR: 本文提出了一种基于预测-校正框架的流模型反演和编辑方法，包括Uni-Inv和Uni-Edit，解决了现有扩散模型方法在流模型中的无效性问题。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型的反演和编辑方法在流模型中效果不佳，流模型的直线非交叉轨迹特性为新型解决方案提供了机会。

Method: 提出Uni-Inv反演方法和Uni-Edit编辑方法，基于预测-校正框架，无需调参、模型无关且高效。

Result: 实验表明Uni-Inv和Uni-Edit在多种生成模型中表现优越且通用性强，即使在低成本设置下。
...
### [251] [Probing and Inducing Combinational Creativity in Vision-Language Models](https://arxiv.org/abs/2504.13120)
...
<details>
  <summary>Details</summary>
Motivation: 研究VLMs是否能够像人类一样通过组合现有概念生成新颖想法，而非仅依赖训练数据的模式匹配。

Method: 提出IEI框架，将创造性过程分解为三个层次，并使用CreativeMashup数据集进行验证。

Result: 在理解任务中，最佳VLMs已超越普通人表现，但不及专家；在生成任务中，IEI框架显著提升了VLMs的创造力。

Conclusion: 研究为评估人工创造力提供了理论基础，并为提升VLMs的创造性生成提供了实用指南。
...
### [252] [VistaDPO: Video Hierarchical Spatial-Temporal Direct Preference Optimization for Large Video Models](https://arxiv.org/abs/2504.13122)
*Haojian Huang,Haodong Chen,Shengqiong Wu,Meng Luo,Jinlan Fu,Xinya Du,Hanwang Zhang,Hao Fei*

Main category: cs.CV

TLDR: VistaDPO是一种新型框架，通过分层空间-时间直接偏好优化解决大型视频模型与人类直觉不对齐及视频幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 大型视频模型在视频理解中存在与人类直觉不对齐和视频幻觉问题，需要改进。

Method: VistaDPO在三个层次上优化文本-视频偏好对齐：实例级、时间级和感知级，并构建了VistaDPO-7k数据集。

Result: 实验表明VistaDPO显著提升了现有大型视频模型的性能，有效缓解了对齐和幻觉问题。

Conclusion: VistaDPO通过分层优化和数据集支持，显著改善了视频-语言对齐和模型性能。
...
### [253] [Low-hallucination Synthetic Captions for Large-Scale Vision-Language Model Pre-training](https://arxiv.org/abs/2504.13123)
*Xinsong Zhang,Yarong Zeng,Xinting Huang,Hu Hu,Runquan Xie,Han Hu,Zhanhui Kang*

Main category: cs.CV

TLDR: 该研究提出了一种可扩展的合成字幕生成技术，用于视觉语言模型预训练，展示了大规模低幻觉合成字幕的双重优势：替代真实数据并提升模型性能。
...
Method: 提出了一种生成高质量、低幻觉、知识丰富的合成字幕的新流程，采用连续DPO方法减少幻觉。

Result: 合成字幕在35个视觉语言任务中性能提升至少6.2%，在文本到图像领域显著降低FID分数。
...
### [254] [Science-T2I: Addressing Scientific Illusions in Image Synthesis](https://arxiv.org/abs/2504.13129)
...
### [255] [PCBEAR: Pose Concept Bottleneck for Explainable Action Recognition](https://arxiv.org/abs/2504.13140)
*Jongseo Lee,Wooil Lee,Gyeong-Moon Park,Seong Tae Kim,Jinwoo Choi*

Main category: cs.CV

TLDR: PCBEAR提出了一种基于人体姿态序列的概念瓶颈框架，用于可解释的动作识别，结合静态和动态姿态概念，实现了高性能和可解释性。


<details>
  <summary>Details</summary>
Motivation: 解决现有视频XAI方法难以捕捉动作动态和时间依赖性的问题，提升动作识别的透明度和可解释性。

Method: 利用人体骨架姿态序列，定义静态和动态姿态概念，通过聚类自动发现概念，无需人工标注。

Result: 在KTH、Penn-Action和HAA500数据集上验证了高性能分类和可解释的动理解释。

Conclusion: PCBEAR在保持高预测性能的同时，提供了人类可理解的模型推理过程，支持调试和改进模型行为。
...
### [256] [$\texttt{Complex-Edit}$: CoT-Like Instruction Generation for Complexity-Controllable Image Editing Benchmark](https://arxiv.org/abs/2504.13143)
*Siwei Yang,Mude Hui,Bingchen Zhao,Yuyin Zhou,Nataniel Ruiz,Cihang Xie*

Main category: cs.CV

TLDR: 论文介绍了$	exttt{Complex-Edit}$，一个用于评估基于指令的图像编辑模型的综合基准，揭示了开源模型与闭源模型之间的性能差距，以及复杂指令对模型表现的影响。


<details>
  <summary>Details</summary>
Motivation: 为了系统评估不同复杂度指令下的图像编辑模型性能，并揭示现有模型的局限性。

Method: 利用GPT-4o自动收集多样化的编辑指令，采用“Chain-of-Edit”流程生成复杂指令，并引入基于VLM的自动评估指标。

Result: 开源模型表现显著低于闭源模型；复杂指令影响关键元素保留和美学质量；分解复杂指令会降低性能；Best-of-N策略可提升结果；合成数据训练导致编辑结果更显人工痕迹。

Conclusion: $	exttt{Complex-Edit}$为图像编辑模型提供了系统评估工具，揭示了复杂指令和合成数据对模型性能的影响。
...
### [257] [St4RTrack: Simultaneous 4D Reconstruction and Tracking in the World](https://arxiv.org/abs/2504.13152)
*Haiwen Feng,Junyi Zhang,Qianqian Wang,Yufei Ye,Pengcheng Yu,Michael J. Black,Trevor Darrell,Angjoo Kanazawa*

Main category: cs.CV

TLDR: St4RTrack是一个同时进行动态3D重建和点跟踪的框架，通过预测点图和对帧进行链式处理，结合了3D重建与跟踪。


<details>
  <summary>Details</summary>
Motivation: 动态3D重建和点跟踪通常被视为独立任务，但它们有深层联系。本文旨在提出一个统一的框架来解决这两个问题。

Method: 通过预测一对帧的点图，并在同一世界坐标系中捕捉静态和动态场景几何，同时保持3D对应关系。采用重投影损失进行训练。

Result: 提出了一个新的基准测试，证明了该框架的有效性和高效性。

Conclusion: St4RTrack是一个数据驱动的统一框架，能够同时实现动态3D重建和点跟踪，且无需依赖4D地面真值监督。
...
### [258] [Training-Free Hierarchical Scene Understanding for Gaussian Splatting with Superpoint Graphs](https://arxiv.org/abs/2504.13153)
*Shaohui Dai,Yansong Qu,Zheyan Li,Xinyang Li,Shengchuan Zhang,Liujuan Cao*

Main category: cs.CV

TLDR: 提出一种无需训练的方法，通过构建超点图直接从高斯基元中实现3D场景的开放词汇理解，避免了多视图迭代训练，提高了效率和一致性。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要迭代优化2D语义特征图，导致效率低下和视图间3D语义不一致。

Method: 构建超点图分割场景为语义一致区域，设计重投影策略将2D语义特征提升到超点，避免多视图训练。

Result: 实现了最先进的开放词汇分割性能，语义场重建速度快30倍。

Conclusion: 该方法高效、一致，支持粗粒度和细粒度的开放词汇感知。
...
### [259] [AerialMegaDepth: Learning Aerial-Ground Reconstruction and View Synthesis](https://arxiv.org/abs/2504.13157)
*Khiem Vuong,Anurag Ghosh,Deva Ramanan,Srinivasa Narasimhan,Shubham Tulsiani*

Main category: cs.CV

TLDR: 论文提出了一种结合伪合成渲染和真实地面图像的方法，以解决航空-地面图像对中极端视角变化的问题，显著提升了重建和相机估计的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有学习方法难以处理航空-地面图像对的极端视角变化，主要原因是缺乏高质量、配准的训练数据集。

Method: 提出了一种可扩展框架，结合3D城市网格的伪合成渲染和真实地面图像，以弥补领域差距。

Result: 在真实世界的零样本任务中，该方法将相机旋转误差在5度以内的准确率从5%提升至56%，并改善了新视角合成等下游任务。

Conclusion: 该方法通过混合数据集显著提升了航空-地面任务的性能，展示了实际应用价值。
...
### [260] [Digital Twin Generation from Visual Data: A Survey](https://arxiv.org/abs/2504.13159)
...
TLDR: 本文综述了从视频生成数字孪生的最新进展，分析了多种方法及其优缺点，并探讨了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 数字孪生在机器人、媒体内容创作和设计等领域有广泛应用潜力，本文旨在全面概述相关技术及其实际应用。

Method: 分析了3D高斯泼溅、生成式修复、语义分割和基础模型等方法，并讨论了遮挡、光照变化和可扩展性等挑战。

Result: 总结了当前最先进的方法及其在实际应用中的潜力。

Conclusion: 本文为数字孪生技术的未来发展提供了方向，并强调了其跨领域应用的重要性。
...
### [261] [Personalized Text-to-Image Generation with Auto-Regressive Models](https://arxiv.org/abs/2504.13162)
*Kaiyue Sun,Xian Liu,Yao Teng,Xihui Liu*

Main category: cs.CV

TLDR: 本文探讨了自回归模型在个性化图像合成中的潜力，提出了一种两阶段训练策略，并在实验中证明了其与扩散模型相当的效果。
...
### [262] [ODHSR: Online Dense 3D Reconstruction of Humans and Scenes from Monocular Videos](https://arxiv.org/abs/2504.13167)
*Zetong Zhang,Manuel kaufmann,Lixin Xue,Jie Song,Martin R. Oswald*

Main category: cs.CV

TLDR: 提出了一种统一的框架，能够在线同时进行相机跟踪、人体姿态估计和人与场景重建，利用3D高斯泼溅技术高效学习高斯基元，并通过遮挡感知和几何先验提升重建质量。


<details>
  <summary>Details</summary>
Motivation: 解决现有神经渲染方法需要预校准相机和人体姿态以及长时间训练的问题，实现更高效的在线重建。

Method: 采用3D高斯泼溅技术，设计相机跟踪和人体姿态估计模块，结合人体变形模块和遮挡感知渲染，利用几何先验提升重建质量。

Result: 在EMDB和NeuMan数据集上，相机跟踪、人体姿态估计和新视角合成性能优于或与现有方法相当。

Conclusion: 提出的框架在效率和重建质量上表现优异，适用于在线人与场景重建任务。
...
### [263] [Generate, but Verify: Reducing Hallucination in Vision-Language Models with Retrospective Resampling](https://arxiv.org/abs/2504.13169)
*Tsung-Han Wu,Heekyung Lee,Jiaxin Ge,Joseph E. Gonzalez,Trevor Darrell,David M. Chan*

Main category: cs.CV

TLDR: REVERSE是一个统一的框架，通过结合幻觉感知训练和实时自验证，显著减少视觉语言模型（VLMs）中的幻觉现象。


<details>
  <summary>Details</summary>
Motivation: 现有方法（生成调整和后验验证）存在局限性，如依赖启发式或复杂的外部模型，无法动态修正幻觉。

Method: 利用包含130万半合成样本的数据集和推理时回顾重采样技术，实现幻觉检测和动态修正。

Result: 在CHAIR-MSCOCO和HaloQuest上分别优于现有方法12%和28%。

Conclusion: REVERSE在减少幻觉方面达到最先进水平，并提供数据集、模型和代码。
...
### [264] [IMAGGarment-1: Fine-Grained Garment Generation for Controllable Fashion Design](https://arxiv.org/abs/2504.13176)
...
Conclusion: IMAGGarment-1为服装生成提供了高效可控的解决方案，并发布了GarmentBench数据集支持相关研究。
...
### [265] [Single-Shot Shape and Reflectance with Spatial Polarization Multiplexing](https://arxiv.org/abs/2504.13177)
...
Method: 设计空间复用的偏振模式，通过量化AoLP值实现形状重建，同时利用局部区域的不同偏振光分离镜面和漫反射，用于BRDF估计。

Result: 实验验证了该方法能从单次偏振图像中恢复形状、穆勒矩阵和BRDF，并成功应用于动态表面。
...
### [266] [PerceptionLM: Open-Access Data and Models for Detailed Visual Understanding](https://arxiv.org/abs/2504.13180)
*Jang Hyun Cho,Andrea Madotto,Effrosyni Mavroudi,Triantafyllos Afouras,Tushar Nagarajan,Muhammad Maaz,Yale Song,Tengyu Ma,Shuming Hu,Suyog Jain,Miguel Martin,Huiyu Wang,Hanoona Rasheed,Peize Sun,Po-Yao Huang,Daniel Bolya,Nikhila Ravi,Shashank Jain,Tammy Stark,Shane Moon,Babak Damavandi,Vivian Lee,Andrew Westbury,Salman Khan,Philipp Krähenbühl,Piotr Dollár,Lorenzo Torresani,Kristen Grauman,Christoph Feichtenhofer*

Main category: cs.CV

TLDR: 研究提出了一种完全开放和可复现的感知语言模型（PLM）框架，用于透明研究图像和视频理解，避免了依赖闭源模型的蒸馏方法。
...
Method: 分析了标准训练流程，探索大规模合成数据，并发布280万人工标注的视频问答对和时空标注视频描述。

Result: 提出了PLM-VideoBench，用于评估视频理解任务，并公开了数据、训练方法、代码和模型。

Conclusion: 通过开放框架和数据，推动了透明和可复现的视频理解研究。
...
### [267] [Perception Encoder: The best visual embeddings are not at the output of the network](https://arxiv.org/abs/2504.13181)
...
TLDR: Perception Encoder (PE) 是一种通过视觉-语言学习训练的最先进编码器，适用于图像和视频理解。通过对比视觉-语言训练，PE 生成了强大的通用嵌入，适用于多种下游任务。
...
Method: PE 通过对比视觉-语言训练生成通用嵌入，并引入两种对齐方法（语言对齐和空间对齐）从网络中间层提取嵌入。

Result: PE 在多种任务上实现最先进性能，包括零样本分类、检索、问答及空间任务（如检测、深度估计和跟踪）。

Conclusion: PE 展示了对比视觉-语言训练的潜力，为多任务学习提供了高效解决方案，并开源了模型、代码和新数据集。
...
### [268] [A Two-Phase Perspective on Deep Learning Dynamics](https://arxiv.org/abs/2504.12700)
*Robert de Mello Koch,Animik Ghosh*

Main category: hep-th

TLDR: 论文提出深度学习分两阶段：快速拟合和缓慢压缩，支持了泛化延迟现象。


<details>
  <summary>Details</summary>
Motivation: 探讨深度学习训练中泛化延迟的共同时间结构，揭示压缩阶段的重要性。

Method: 通过实验验证三种现象（grokking、双下降、信息瓶颈）的时间结构一致性，使用互信息作为进展度量。

Result: 发现压缩阶段未被标准算法主动优化，可能延长，类比重整化群提出压缩是泛化关键。

Conclusion: 压缩阶段是深度学习泛化的关键，需优化训练算法以加速此过程。
...
### [269] [Cross-environment Cooperation Enables Zero-shot Multi-agent Coordination](https://arxiv.org/abs/2504.12714)
*Kunal Jha,Wilka Carvalho,Yancheng Liang,Simon S. Du,Max Kleiman-Weiner,Natasha Jaques*

Main category: cs.MA

TLDR: 论文提出了一种通过跨环境合作（CEC）训练通用协作代理的方法，以支持零样本协作（ZSC），并在与人类合作时表现优于基线。


<details>
  <summary>Details</summary>
Motivation: 解决现有模型在单一任务上训练后无法泛化到新任务或新伙伴的问题。

Method: 使用Jax-based程序生成器创建大量协调挑战，并通过跨环境合作（CEC）训练代理。

Result: CEC方法在定量和定性上优于基线，并能与不同伙伴有效协作。

Conclusion: 跨多场景学习鼓励代理发展通用规范，为设计无需人类数据的通用协作代理提供了新途径。
...
### [270] [The Athenian Academy: A Seven-Layer Architecture Model for Multi-Agent Systems](https://arxiv.org/abs/2504.12735)
*Lidong Zhai,Zhijie Qiu,Xizhong Guo,Jiaqi Li*

Main category: cs.MA

TLDR: 本文提出了一种名为“雅典学院”的多智能体七层框架，旨在系统解决AI艺术创作中多智能体系统的协作效率、角色分配、环境适应和任务并行等挑战。
...
Method: 将多智能体系统分为七层，包括多智能体协作、单智能体多角色扮演、单智能体多场景遍历、单智能体多能力化身等，并通过实验验证框架的有效性。
...
### [271] [Multi-Agent Reinforcement Learning Simulation for Environmental Policy Synthesis](https://arxiv.org/abs/2504.12777)
*James Rudd-Jones,Mirco Musolesi,María Pérez-Ortiz*

Main category: cs.MA

TLDR: 论文提出了一种结合多智能体强化学习（MARL）与气候模拟的框架，以解决气候政策制定中的非线性动态、异质代理和不确定性量化问题。
...
Result: 框架为气候政策探索提供了更先进的方法，同时指出了未来研究的局限性。

Conclusion: 该框架为气候政策制定提供了新工具，但仍需进一步解决可解释性和实用性等问题。
...
### [272] [QLLM: Do We Really Need a Mixing Network for Credit Assignment in Multi-Agent Reinforcement Learning?](https://arxiv.org/abs/2504.12961)
*Zhouyang Jiang,Bin Zhang,Airong Wei,Zhiwei Xu*

Main category: cs.MA

TLDR: 论文提出了一种基于大语言模型（LLM）的新算法QLLM，用于解决多智能体强化学习中的信用分配问题，通过TFCAF概念和coder-evaluator框架提升了性能。


<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习中的信用分配问题存在贡献分配不精确、可解释性差和可扩展性不足等挑战。

Method: 提出QLLM算法，利用LLM自动构建信用分配函数，引入TFCAF概念，并采用coder-evaluator框架生成和验证代码。
...
### [273] [Prototype-Guided Diffusion for Digital Pathology: Achieving Foundation Model Performance with Minimal Clinical Data](https://arxiv.org/abs/2504.12351)
*Ekaterina Redekop,Mara Pleasure,Vedrana Ivezic,Zichen Wang,Kimberly Flores,Anthony Sisk,William Speier,Corey Arnold*

Main category: cs.GR

TLDR: 该研究提出了一种原型引导的扩散模型，用于生成高质量合成病理数据，减少对真实患者样本的依赖，同时保持下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 探索如何减少对大规模真实病理数据集的依赖，同时保持模型性能。

Method: 使用原型引导的扩散模型生成合成病理数据，并通过自监督学习训练模型。

Result: 合成数据训练的模型性能与大规模真实数据集训练的模型相当或更好，且数据量减少60x-760x。

Conclusion: 生成式AI可高效创建病理训练数据，显著减少对临床数据集的依赖。
...
### [274] [UniPhys: Unified Planner and Controller with Diffusion for Flexible Physics-Based Character Control](https://arxiv.org/abs/2504.12540)
*Yan Wu,Korrawe Karunratanakul,Zhengyi Luo,Siyu Tang*

Main category: cs.GR

TLDR: UniPhys是一个基于扩散的行为克隆框架，将运动规划与控制统一为单一模型，生成自然且物理合理的角色运动。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法在长时程控制中因领域差距导致运动质量下降和需要任务特定微调的问题。

Method: 采用Diffusion Forcing范式训练，学习去噪运动历史并处理物理模拟器引入的差异。
...
### [275] [SOPHY: Generating Simulation-Ready Objects with Physical Materials](https://arxiv.org/abs/2504.12684)
*Junyi Cao,Evangelos Kalogerakis*

Main category: cs.GR

TLDR: SOPHY是一种生成模型，用于合成具有物理意识的3D形状，同时生成形状、纹理和材料属性，适用于动态环境。
...
Method: 引入带有详细物理材料属性的数据集和标注流程，联合建模形状和材料属性。

Result: 实验表明，联合建模提高了生成形状的真实性和保真度，支持文本驱动生成和单图像重建。

Conclusion: SOPHY为动态环境提供了物理感知的3D形状生成解决方案。
...
### [276] [3D-PNAS: 3D Industrial Surface Anomaly Synthesis with Perlin Noise](https://arxiv.org/abs/2504.12856)
*Yifeng Cheng,Juan Du*

Main category: cs.GR

TLDR: 提出了一种基于Perlin噪声和表面参数化的3D异常生成方法3D-PNAS，填补了3D异常生成领域的空白，并提供了代码库和可视化工具。


<details>
  <summary>Details</summary>
Motivation: 工业异常检测中真实缺陷样本稀缺，而3D异常生成技术尚未充分探索，限制了3D数据在工业质量检测中的应用。

Method: 通过将点云投影到2D平面，从Perlin噪声场采样多尺度噪声值，并沿法线方向扰动点云，生成逼真的3D表面异常。

Result: 实验表明，该方法能精细控制异常生成，适应不同物体表面特性，产生几何合理的异常。

Conclusion: 3D-PNAS为3D异常生成提供了有效工具，推动了工业质量检测的发展。
...
### [277] [ARAP-GS: Drag-driven As-Rigid-As-Possible 3D Gaussian Splatting Editing with Diffusion Prior](https://arxiv.org/abs/2504.12788)
*Xiao Han,Runze Tian,Yifei Tong,Fenggen Yu,Dingyao Liu,Yan Zhang*

Main category: cs.GR

TLDR: ARAP-GS是一种基于ARAP变形的拖拽驱动3D高斯溅射（3DGS）编辑框架，首次将ARAP直接应用于3D高斯，实现灵活编辑，并通过扩散先验保持视觉质量。


<details>
  <summary>Details</summary>
Motivation: 当前拖拽驱动编辑在3DGS中研究较少，且保持形状一致性和视觉连续性具有挑战性。

Method: 采用ARAP变形直接作用于3D高斯，结合扩散先验进行迭代优化以提升视觉质量。

Result: 实验显示ARAP-GS在多样3D场景中优于现有方法，且高效（单GPU 10-20分钟）。

Conclusion: ARAP-GS为3DGS拖拽编辑提供了有效且高效的解决方案。
...
### [278] [CAGE-GS: High-fidelity Cage Based 3D Gaussian Splatting Deformation](https://arxiv.org/abs/2504.12800)
*Yifei Tong,Runze Tian,Xiao Han,Dingyao Liu,Fenggen Yu,Yan Zhang*

Main category: cs.GR

TLDR: CAGE-GS是一种基于笼的3D高斯溅射变形方法，通过目标形状学习变形笼，保持纹理保真度。


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射（3DGS）作为真实场景的3D表示，用户友好变形需求增加，但保留细节和纹理保真度是挑战。

Method: 学习目标形状的变形笼，使用雅可比矩阵更新高斯协方差参数，确保纹理保真。

Result: 在公共数据集和新场景中，CAGE-GS在效率和变形质量上显著优于现有技术。

Conclusion: CAGE-GS提供了一种灵活且高效的3DGS变形方法，适用于多种目标形状表示。
...
### [279] [AAA-Gaussians: Anti-Aliased and Artifact-Free 3D Gaussian Rendering](https://arxiv.org/abs/2504.12811)
*Michael Steiner,Thomas Köhler,Lukas Radl,Felix Windisch,Dieter Schmalstieg,Markus Steinberger*

Main category: cs.GR

TLDR: 3D高斯泼溅（3DGS）在3D重建中表现优异，但仍存在走样、投影伪影和视角不一致等问题。本文提出通过全3D高斯评估改进3DGS，引入自适应3D平滑滤波器和稳定的视角空间边界方法，显著提升渲染质量。


<details>
  <summary>Details</summary>
Motivation: 3DGS因将高斯泼溅简化为2D实体而引发走样和伪影问题，需通过全3D评估解决。

Method: 引入自适应3D平滑滤波器、稳定的视角空间边界方法及基于屏幕空间平面的3D瓦片剔除技术。

Result: 在分布内和分布外视角评估中均达到最优质量，有效消除走样和伪影。

Conclusion: 全3D高斯评估显著提升3DGS的渲染质量，实现实时无伪影渲染。
...
### [280] [Real-time High-fidelity Gaussian Human Avatars with Position-based Interpolation of Spatially Distributed MLPs](https://arxiv.org/abs/2504.12909)
*Youyi Zhan,Tianjia Shao,Yin Yang,Kun Zhou*

Main category: cs.GR

TLDR: 提出一种新型高斯人体化身表示方法，通过空间分布的MLP和偏移基实现高保真姿态相关外观重建，同时保持实时渲染性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以在单MLP中捕捉姿态相关细节，或依赖计算密集型网络导致渲染性能下降。

Method: 使用空间分布的MLP和偏移基，通过插值和线性组合生成高斯属性，并用控制点约束高斯分布。

Result: 相比现有方法，实现了更高保真度的外观细节和显著更快的渲染速度。
...
### [281] [GSAC: Leveraging Gaussian Splatting for Photorealistic Avatar Creation with Unity Integration](https://arxiv.org/abs/2504.12999)
*Rendong Zhang,Alexandra Watkins,Nilanjan Sarkar*

Main category: cs.GR

TLDR: 提出了一种基于3D高斯泼溅（3DGS）的端到端虚拟化身创建流程，利用单目视频输入，实现高效、可扩展的逼真化身，并兼容Unity引擎。


<details>
  <summary>Details</summary>
Motivation: 现有虚拟化身创建技术成本高、耗时长且实用性有限，手动方法需要大量时间和专业知识，自动方法则缺乏效率和细节表现力。

Method: 结合单目视频输入和定制预处理，采用3D高斯泼溅技术，实现详细面部表情重建并嵌入到完整的骨骼绑定化身模型中。

Result: 实验验证了预处理流程的有效性，展示了高斯化身在Unity中的多功能性，证明了方法的可扩展性和实用性。

Conclusion: 该方法为VR/AR应用提供了一种高效、逼真的虚拟化身创建解决方案。
...
### [282] [CompGS++: Compressed Gaussian Splatting for Static and Dynamic Scene Representation](https://arxiv.org/abs/2504.13022)
*Xiangrui Liu,Xinju Wu,Shiqi Wang,Zhu Li,Sam Kwong*

Main category: cs.GR

TLDR: CompGS++提出了一种压缩高斯泼溅方法，通过消除冗余和优化预测模块，显著减小3D场景建模的数据体积，同时保持高精度。


<details>
  <summary>Details</summary>
Motivation: 高斯泼溅在3D场景建模中表现优异，但数据量大，难以适应现有互联网基础设施的传输需求，因此需要高效的压缩方法。

Method: 通过空间和时间预测模块消除原始数据冗余，并设计速率约束优化模块，联合最小化重建误差和速率消耗。

Result: 在多个基准数据集上，CompGS++显著优于现有方法，实现了高效的压缩和精确的场景建模。

Conclusion: CompGS++为未来逼真的3D沉浸式视觉通信应用提供了高效的压缩解决方案。
...
### [283] [HiScene: Creating Hierarchical 3D Scenes with Isometric View Generation](https://arxiv.org/abs/2504.13072)
...
Method: 采用分层方法，将场景视为等距视图下的层次化对象，结合视频扩散技术和形状先验注入，确保空间一致性。

Result: 实验表明，HiScene能生成更自然的对象布局和完整实例，适合交互应用。

Conclusion: HiScene通过分层框架和新技术，实现了高保真3D场景生成，提升了编辑灵活性和物理合理性。
...
### [284] [A Multi-task Learning Balanced Attention Convolutional Neural Network Model for Few-shot Underwater Acoustic Target Recognition](https://arxiv.org/abs/2504.13102)
*Wei Huang,Shumeng Sun,Junpeng Lu,Zhenpeng Xu,Zhengyang Xiu,Hao Zhang*

Main category: cs.SD

TLDR: 提出了一种多任务平衡通道注意力卷积神经网络（MT-BCA-CNN），用于解决水下声学目标识别中的样本稀缺和环境干扰问题，显著提升了分类性能。
...
### [285] [Algorithms for the Shortest Vector Problem in $2$-dimensional Lattices, Revisited](https://arxiv.org/abs/2504.12948)
*Lihao Zhao,Chengliang Tian,Jingguo Bi,Guangwu Xu,Jia Yu*

Main category: cs.CG

TLDR: 本文提出了一种高效的自适应格基约简算法CrossEuc和向量化算法HVec，进一步优化为HVecSBP，显著提升了二维格中最短向量问题（SVP）的求解效率。


<details>
  <summary>Details</summary>
Motivation: 二维格中的SVP问题在密码学和计算几何中具有实际意义，且能为高维格问题提供可扩展的解决方案。

Method: 提出新的约简基定义，开发自适应算法CrossEuc，并基于HVec算法优化为HVecSBP，避免了对输入基的预转换需求。

Result: HVecSBP在HNF输入基上效率提升至少13.5倍，对一般形式输入基直接应用效果更优。

Conclusion: HVecSBP算法在效率和通用性上优于现有方法，尤其适用于输入基线性相关性较低的情况。

Abstract: Efficiently solving the Shortest Vector Problem (SVP) in two-dimensional
...
### [286] [Design Topological Materials by Reinforcement Fine-Tuned Generative Model](https://arxiv.org/abs/2504.13048)
*Haosheng Xu,Dongheng Qian,Zhixuan Liu,Yadong Jiang,Jing Wang*

Main category: cond-mat.mtrl-sci

TLDR: 通过强化微调（ReFT）生成新型拓扑材料，成功发现大量新材料，如Ge$_2$Bi$_2$O$_6$，具有0.26 eV的全带隙。


<details>
  <summary>Details</summary>
Motivation: 传统方法在发现具有全带隙的拓扑绝缘体（TIs）和拓扑晶体绝缘体（TCIs）方面受限，需开发新方法生成新材料。

Method: 应用强化微调（ReFT）对预训练生成模型进行优化，以生成符合材料设计目标的拓扑材料。

Result: ReFT显著提升了模型生成TIs和TCIs的能力，且材料稳定性高，成功发现新材料如Ge$_2$Bi$_2$O$_6$。

Conclusion: ReFT是一种有效生成拓扑材料的方法，为未来材料设计提供了新思路。
...
### [287] [AUTONAV: A Toolfor Autonomous Navigation of Robots](https://arxiv.org/abs/2504.12318)
*Mir Md Sajid Sarwar,Sudip Samanta,Rajarshi Ray*

Main category: cs.RO

TLDR: AUTONAV是一个自动化机器人导航工具，用于地图构建、定位和路径规划，支持模块化算法集成，并在室内模拟场景中展示效果。


<details>
  <summary>Details</summary>
Motivation: 解决机器人自主导航中地图构建、定位和路径规划的自动化问题，提供模块化设计以方便算法比较。

Method: 采用模块化架构，集成多种算法完成地图构建、定位和路径规划任务。

Result: 在室内模拟场景中生成了地图和路径规划结果。

Conclusion: AUTONAV成功实现了机器人导航任务的自动化，并展示了模块化设计的灵活性。
...
### [288] [Crossing the Human-Robot Embodiment Gap with Sim-to-Real RL using One Human Demonstration](https://arxiv.org/abs/2504.12609)
*Tyler Ga Wei Lum,Olivia Y. Lee,C. Karen Liu,Jeannette Bohg*

Main category: cs.RO

TLDR: Human2Sim2Robot框架通过单段RGB-D视频训练机器人灵巧操作策略，利用强化学习跨越人-机器人形态差异，无需大规模数据或穿戴设备。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖大量演示或穿戴设备，难以扩展；视频数据易获取但缺乏动作标签且形态差异大。

Method: 从视频提取物体姿态轨迹和手部初始姿态，用于定义奖励函数和引导强化学习训练。
...
### [289] [Diffusion Based Robust LiDAR Place Recognition](https://arxiv.org/abs/2504.12412)
*Benjamin Krummenacher,Jonas Frey,Turcan Tuna,Olga Vysotska,Marco Hutter*

Main category: cs.RO

TLDR: 论文提出了一种基于LiDAR数据和扩散模型的机器人全局重定位方法，用于解决建筑工地中因重复特征和感知混淆导致的定位难题。


<details>
  <summary>Details</summary>
Motivation: 建筑工地中的重复特征（如平坦墙面）和相似布局导致机器人定位困难，需要一种可靠的全局重定位方法。

Method: 使用合成LiDAR点云训练扩散模型（基于PointNet++），模拟真实建筑网格中的LiDAR数据，生成多模态位置分布。

Result: 模型在五个真实数据集上平均定位精度为77%±2m，误差均值比基线方法低一半。

Conclusion: 该方法能有效应对感知混淆问题，提供多模态位置分布，适用于复杂建筑环境。
...
### [290] [Learning Transferable Friction Models and LuGre Identification via Physics Informed Neural Networks](https://arxiv.org/abs/2504.12441)
*Asutay Ozmen,João P. Hespanha,Katie Byl*

Main category: cs.RO

TLDR: 提出了一种物理信息摩擦估计框架，结合经典摩擦模型与可学习组件，仅需少量通用数据即可准确模拟动态摩擦特性，减少仿真与现实的差距。


<details>
  <summary>Details</summary>
Motivation: 机器人仿真中摩擦模型简化导致仿真与物理性能差异显著，需更准确且通用的解决方案。

Method: 结合物理一致性与可学习组件，利用少量噪声数据训练摩擦模型。

Result: 模型在小规模噪声数据下准确模拟动态摩擦，并可迁移至未训练系统。

Conclusion: 该方法为复杂任务提供可扩展且可解释的摩擦建模路径，缩小仿真与现实的差距。
...
### [291] [Trajectory Adaptation using Large Language Models](https://arxiv.org/abs/2504.12755)
*Anurag Maurya,Tashmoy Ghosh,Ravi Prakash*

Main category: cs.RO

TLDR: 提出一种基于语言的灵活框架，利用预训练LLM生成代码来调整机器人轨迹，适应复杂的人类指令，无需任务特定训练。


<details>
  <summary>Details</summary>
Motivation: 实现更直观和可扩展的人机交互，适应新情况下的机器人轨迹调整。
...
### [292] [Explainable Scene Understanding with Qualitative Representations and Graph Neural Networks](https://arxiv.org/abs/2504.12817)
*Nassim Belmecheri,Arnaud Gotlieb,Nadjib Lazaar,Helge Spieker*

Main category: cs.RO

TLDR: 本文提出了一种将图神经网络（GNN）与定性可解释图（QXG）结合的方法，用于自动驾驶中的场景理解，并在nuScenes数据集上验证了其优越性能。


<details>
  <summary>Details</summary>
Motivation: 场景理解是自动驾驶决策的基础，但现有方法仅分析对象对的单一关系链，忽略了场景的全局上下文。

Method: 提出了一种新型GNN架构，处理整个图结构以识别交通场景中的相关对象。

Result: 实验结果表明，该方法在nuScenes数据集上优于基线方法，能有效处理类别不平衡问题并考虑时空关系。
...
### [293] [RoboTwin: Dual-Arm Robot Benchmark with Generative Digital Twins](https://arxiv.org/abs/2504.13059)
...
Method: 通过单张2D图像生成多样化数字孪生对象，结合空间关系感知的代码生成框架，分解任务并生成精确机器人运动代码。
...
### [294] [UncAD: Towards Safe End-to-end Autonomous Driving via Online Map Uncertainty](https://arxiv.org/abs/2504.12826)
*Pengxuan Yang,Yupeng Zheng,Qichao Zhang,Kefei Zhu,Zebin Xing,Qiao Lin,Yun-Fu Liu,Zhiguo Su,Dongbin Zhao*

Main category: cs.RO

TLDR: UncAD提出了一种新的端到端自动驾驶范式，通过感知模块中的在线地图不确定性估计，提升规划安全性，减少碰撞率。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖确定性建模的在线地图，可能引入感知错误并影响规划安全。

Method: UncAD估计在线地图不确定性，指导多模态轨迹生成，并提出不确定性-碰撞感知的规划选择策略。

Result: 在nuScenes数据集上，UncAD仅增加1.9%参数，但碰撞率降低26%，可行驶区域冲突率降低42%。

Conclusion: UncAD通过不确定性建模显著提升了自动驾驶的安全性。
...
### [295] [Taccel: Scaling Up Vision-based Tactile Robotics via High-performance GPU Simulation](https://arxiv.org/abs/2504.12908)
*Yuyang Li,Wenxin Du,Chang Yu,Puhao Li,Zihang Zhao,Tengyu Liu,Chenfanfu Jiang,Yixin Zhu,Siyuan Huang*

Main category: cs.RO

TLDR: Taccel是一个高性能的触觉传感模拟平台，通过集成IPC和ABD技术，实现了快速且精确的物理模拟，支持大规模并行环境，显著加速触觉机器人研究。


<details>
  <summary>Details</summary>
Motivation: 触觉传感对机器人实现人类水平的操作能力至关重要，但现有VBTS传感器因复杂的物理特性和视觉信号处理需求，缺乏高效准确的模拟工具，限制了研究规模。

Method: Taccel平台结合IPC和ABD技术，模拟机器人、触觉传感器和物体，提供高精度物理模拟和真实触觉信号，支持灵活配置。

Result: Taccel实现了18倍于实时速度的模拟，并通过物体识别、抓取和操作任务验证了其精确性和从模拟到现实的迁移能力。

Conclusion: Taccel作为强大工具，通过大规模触觉传感模拟，加速机器人系统开发，有望改变机器人对物理环境的交互与理解方式。
...
### [296] [ViTa-Zero: Zero-shot Visuotactile Object 6D Pose Estimation](https://arxiv.org/abs/2504.13179)
*Hongyu Li,James Akl,Srinath Sridhar,Tye Brady,Taskin Padir*

Main category: cs.RO

TLDR: ViTa-Zero是一个零样本的视觉触觉姿态估计框架，通过物理约束和测试时优化提升性能。
...
### [297] [RUKA: Rethinking the Design of Humanoid Hands with Learning](https://arxiv.org/abs/2504.13165)
*Anya Zorin,Irmak Guzey,Billy Yan,Aadhithya Iyer,Lisa Kondrich,Nikhil X. Bhattasali,Lerrel Pinto*

Main category: cs.RO

TLDR: RUKA是一种紧凑、经济且功能强大的肌腱驱动仿人机械手，通过3D打印和现成组件制成，具有15个欠驱动自由度，支持多样化抓握。基于学习的方法解决了控制挑战，并通过运动捕捉数据建模。实验证明其优于其他机械手的可达性、耐用性和力量。


<details>
  <summary>Details</summary>
Motivation: 现有机械手设计在精度、紧凑性、强度和成本之间存在权衡，限制了灵巧操作的发展。学习型方法为重新思考这些权衡提供了机会。

Method: RUKA采用肌腱驱动设计，通过3D打印和现成组件制成，具有15个欠驱动自由度。利用MANUS手套的运动捕捉数据学习关节到执行器和指尖到执行器的模型。

Result: RUKA在可达性、耐用性和力量方面优于其他机械手，并能完成灵巧的远程操作任务。

Conclusion: RUKA的开源设计和学习型控制方法为灵巧机械手的发展提供了新方向。
...
### [298] [Deep Generative Model-Based Generation of Synthetic Individual-Specific Brain MRI Segmentations](https://arxiv.org/abs/2504.12352)
*Ruijie Wang,Luca Rossetto,Susan Mérillat,Christina Röcke,Mike Martin,Abraham Bernstein*

Main category: q-bio.NC

TLDR: 提出了一种新方法CSegSynth，利用易获取的人口统计、访谈和认知测试信息生成个体特异性脑MRI分割，无需详细脑结构信息。
...
Method: 使用新型深度生成模型CSegSynth，优于现有C-VAE、C-GAN和C-LDM模型。

Result: 生成的分割质量高，体积预测Pearson相关系数达0.80、0.82和0.70（WM、GM、CSF）。

Conclusion: CSegSynth能高效生成个体特异性脑MRI分割，为医学研究提供新工具。
...
### [299] [Universal Approximation with XL MIMO Systems: OTA Classification via Trainable Analog Combining](https://arxiv.org/abs/2504.12758)
*Kyriakos Stylianopoulos,George C. Alexandropoulos*

Main category: eess.SP

TLDR: XL-MIMO系统通过模拟组合器实现类似神经网络的通用函数逼近功能，结合ELM框架，实现无需数字处理的OTA边缘推理，性能接近深度学习但复杂度极低。


<details>
  <summary>Details</summary>
Motivation: 探索XL-MIMO系统在无线通信中的新应用，将其视为神经网络，简化边缘推理的复杂度。

Method: 将XL-MIMO信道系数视为隐藏层节点，接收端模拟组合器作为可训练输出层，结合ELM框架实现OTA推理。

Result: XL-MIMO-ELM实现快速训练和高效分类，性能接近深度学习但复杂度大幅降低。

Conclusion: XL-MIMO系统可作为神经网络应用于无线通信，为超低功耗设备提供高效解决方案。
...
### [300] [Specialized text classification: an approach to classifying Open Banking transactions](https://arxiv.org/abs/2504.12319)
*Duc Tuyen TA,Wajdi Ben Saad,Ji Young Oh*

Main category: cs.IR

TLDR: 本文提出了一种基于语言的开放式银行交易分类系统，专注于法语市场和法语文本，通过定制化方法提升性能。


<details>
  <summary>Details</summary>
Motivation: PSD2法规和开放式银行框架为银行和金融科技公司提供了新机会，但针对特定领域（如银行业）的文本处理仍未被充分解决。

Method: 系统包括数据收集、标注、预处理、建模和评估阶段，结合语言特定技术和领域知识。

Result: 相比通用方法，该系统在法语银行数据分类中表现出更高的性能和效率。

Conclusion: 定制化语言模型在特定领域（如法语银行数据）中具有显著优势。
...
### [301] [SimUSER: Simulating User Behavior with Large Language Models for Recommender System Evaluation](https://arxiv.org/abs/2504.12722)
*Nicolas Bougie,Narimasa Watanabe*

Main category: cs.IR

TLDR: SimUSER是一个模拟用户代理框架，用于解决推荐系统评估中离线指标与在线行为之间的差距问题。


<details>
  <summary>Details</summary>
Motivation: 由于真实用户数据的稀缺性和隐私问题，需要一种可信且成本效益高的用户代理来评估推荐系统性能。

Method: SimUSER通过从历史数据中识别自洽的用户角色，并配备人物、记忆、感知和大脑模块，模拟用户与推荐系统的交互。

Result: SimUSER在微观和宏观层面比现有方法更接近真实用户行为，并通过实验研究了缩略图、曝光效应和评论对用户参与的影响。

Conclusion: 基于离线A/B测试结果优化推荐系统参数，SimUSER在真实场景中提升了用户参与度。
...
### [302] [A Human-AI Comparative Analysis of Prompt Sensitivity in LLM-Based Relevance Judgment](https://arxiv.org/abs/2504.12408)
*Negar Arabzadeh,Charles L. A . Clarke*

Main category: cs.IR

TLDR: 该论文研究了大型语言模型（LLMs）在信息检索任务中作为相关性判断工具的鲁棒性和可靠性，重点关注提示敏感性对任务的影响。


<details>
  <summary>Details</summary>
Motivation: 评估LLMs在相关性判断任务中的表现，尤其是提示变化对结果的影响，以验证其是否接近人类标注的一致性。

Method: 收集了来自15名人类专家和15个LLMs的90个提示，筛选后使用72个提示对TREC数据集进行标注，并与官方人类标注进行比较。

Result: 通过Cohen's κ和成对一致性度量，分析了提示变化对LLMs与人类标注一致性的影响，并比较了人类和LLMs生成的提示。

Conclusion: 研究为LLM在相关性判断中的应用提供了数据支持，并揭示了提示设计的重要性。
...
### [303] [Benchmarking LLM-based Relevance Judgment Methods](https://arxiv.org/abs/2504.12558)
*Negar Arabzadeh,Charles L. A. Clarke*

Main category: cs.IR

TLDR: 本文系统比较了多种基于大语言模型（LLM）的相关性评估方法，包括二元判断、分级评估、成对偏好和基于关键片段的方法，并通过实验验证其与人类偏好的一致性。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注通过提示策略复现人类分级相关性判断，但对其他评估方法或全面比较研究探索有限。本文旨在填补这一空白。

Method: 比较了多种LLM相关性评估方法，包括二元判断、分级评估、成对偏好和两种基于关键片段的方法，并通过Kendall相关性和人类偏好进行验证。

Result: 实验基于TREC Deep Learning 2019-2021和ANTIQUE数据集，开源和商业模型（Llama3.2b和gpt-4o）生成的判断数据公开。

Conclusion: 研究提供了全面的LLM相关性评估方法比较，代码和数据公开，为未来研究提供参考。
...
### [304] [Towards Lossless Token Pruning in Late-Interaction Retrieval Models](https://arxiv.org/abs/2504.12778)
*Yuxuan Zong,Benjamin Piwowarski*

Main category: cs.IR

TLDR: 本文提出了一种基于正则化损失的方法，用于在神经信息检索模型中高效剪枝文档标记，同时保持检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有的剪枝方法（如启发式或统计方法）无法保证不影响检索分数，因此需要一种更原则性的方法。

Method: 引入了三种正则化损失和两种剪枝策略，以实现高剪枝率且不影响检索分数。

Result: 实验表明，该方法可以仅使用30%的标记，同时保持ColBERT的性能。

Conclusion: 该方法为神经信息检索模型提供了一种高效的剪枝解决方案。
...
### [305] [Building Russian Benchmark for Evaluation of Information Retrieval Models](https://arxiv.org/abs/2504.12879)
*Grigory Kovalev,Mikhail Tikhomirov,Evgeny Kozhevnikov,Max Kornilov,Natalia Loukachevitch*

Main category: cs.IR

TLDR: RusBEIR是一个用于俄语零样本信息检索模型评估的综合基准，包含17个多领域数据集，支持词法和神经模型的系统比较。


<details>
  <summary>Details</summary>
Motivation: 为俄语信息检索研究提供一个统一的评估框架，填补现有研究的空白。

Method: 整合了适应、翻译和新创建的数据集，比较了词法模型（如BM25）和神经模型（如mE5-large和BGE-M3）的性能。

Result: 词法模型在形态丰富语言中依赖预处理，BM25在全文检索中表现良好；神经模型在多数数据集上表现更优，但在长文档检索中受输入长度限制。
...
### [306] [FreshStack: Building Realistic Benchmarks for Evaluating Retrieval on Technical Documents](https://arxiv.org/abs/2504.13128)
*Nandan Thakur,Jimmy Lin,Sam Havens,Michael Carbin,Omar Khattab,Andrew Drozdov*

Main category: cs.IR

TLDR: FreshStack是一个可重复使用的框架，用于从社区问答自动构建信息检索（IR）评估基准，包含自动语料收集、问题答案片段生成和混合检索技术。
...
Method: 通过自动语料收集、片段生成和混合检索技术构建数据集，并应用于五个快速发展的主题。

Result: 现有检索模型在五个主题上表现显著低于理想方法，且重排序器在部分主题上未明显提升准确性。
...
### [307] [SemCORE: A Semantic-Enhanced Generative Cross-Modal Retrieval Framework with MLLMs](https://arxiv.org/abs/2504.13172)
*Haoxuan Li,Yi Bin,Yunshan Ma,Guoqing Wang,Yang Yang,See-Kiong Ng,Tat-Seng Chua*

Main category: cs.IR

TLDR: SemCORE是一个统一语义增强的生成跨模态检索框架，通过结构化自然语言标识符和生成语义验证策略，显著提升了生成跨模态检索的性能。


<details>
  <summary>Details</summary>
Motivation: 传统跨模态检索方法依赖嵌入相似性计算，而生成检索方法虽潜力巨大，但在标识符构建和生成过程中存在语义信息不足的问题。

Method: 提出SemCORE框架，包括结构化自然语言标识符（SID）和生成语义验证（GSV）策略，同时支持文本到图像和图像到文本的双向检索任务。

Result: 实验表明，SemCORE在基准数据集上显著优于现有方法，文本到图像检索的Recall@1平均提升8.65分。

Conclusion: SemCORE通过语义增强和生成策略，有效解决了生成跨模态检索中的语义不足问题，展现了优越性能。
...
### [308] [Corner Gradient Descent](https://arxiv.org/abs/2504.12519)
*Dmitry Yarotsky*

Main category: math.OC

TLDR: 论文研究了在无限维二次问题中，通过广义的带无限记忆的随机梯度下降（SGD）实现$O(t^{-2\zeta})$的收敛速率，超越了传统方法的限制。


<details>
  <summary>Details</summary>
Motivation: 在随机梯度下降（SGD）中，由于采样噪声，传统的Jacobi Heavy Ball方法会发散，导致无法达到$O(t^{-2\zeta})$的收敛速率。本文旨在解决这一问题。

Method: 通过将广义（S）GD算法与复平面中的轮廓关联，并利用具有外部角$\theta\pi$的轮廓加速收敛，同时优化$\theta$以平衡加速和噪声效应。

Result: 证明了最优速率由$\theta_{\max}=\min(2,\nu,\tfrac{2}{\zeta+1/\nu})$决定，并通过有限记忆算法高效近似理想算法。

Conclusion: 广义SGD方法在理论和实验中均表现出色，为随机优化问题提供了新的解决方案。
...
### [309] [On the asymptotic behaviour of stochastic processes, with applications to supermartingale convergence, Dvoretzky's approximation theorem, and stochastic quasi-Fejér monotonicity](https://arxiv.org/abs/2504.12922)
*Morenikeji Neri,Nicholas Pischke,Thomas Powell*

Main category: math.OC

TLDR: 论文证明了满足松弛超鞅条件的随机过程的渐进行为，提供了收敛速率的显式构造，并应用于随机逼近中的经典定理。


<details>
  <summary>Details</summary>
Motivation: 研究随机过程的渐进行为，为随机逼近理论提供定量分析工具。

Method: 通过构造显式收敛速率，分析松弛超鞅条件，并应用于随机逼近中的经典定理。

Result: 提出了收敛速率的显式构造，适用于多种随机逼近场景，包括线性速率。

Conclusion: 方法具有广泛适用性，可推广到更多随机逼近问题。
...
### [310] [TransST: Transfer Learning Embedded Spatial Factor Modeling of Spatial Transcriptomics Data](https://arxiv.org/abs/2504.12353)
*Shuo Shuo Liu,Shikun Wang,Yuxuan Chen,Anil K. Rustgi,Ming Yuan,Jianhua Hu*

Main category: q-bio.GN

TLDR: TransST是一种新型的迁移学习框架，用于提高空间转录组数据中细胞异质性的推断能力，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 空间转录组技术分辨率低且测序深度不足，难以可靠提取生物信号，需要新方法解决这一问题。

Method: TransST通过迁移学习利用外部细胞标记信息，自适应地推断目标数据的细胞异质性。

Result: 在乳腺癌等研究中，TransST成功识别出有生物学意义的细胞簇，并能区分其他方法无法分离的组织类型。
...
### [311] [A Survey on Archetypal Analysis](https://arxiv.org/abs/2504.12392)
*Aleix Alcacer,Irene Epifanio,Sebastian Mair,Morten Mørup*

Main category: stat.ME

TLDR: 本文概述了原型分析（AA）的方法、应用及未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 原型分析提供了一种直观且可解释的高维数据结构表示方法，但其非凸优化问题带来了挑战。

Method: 通过提取原型并将观测记录近似为这些原型的混合，实现特征提取和降维。

Result: AA在多个科学领域有广泛应用，但仍需解决非凸优化等问题。

Conclusion: 未来研究应关注AA的优化方法及其在新领域的应用。
...
### [312] [Cluster weighted models with multivariate skewed distributions for functional data](https://arxiv.org/abs/2504.12683)
*Cristina Anton,Roy Shivam Ram Shreshtth*

Main category: stat.ME

TLDR: 提出了一种基于功能线性回归模型和三种偏态多元分布的聚类方法funWeightClustSkew，扩展了funHDDC框架，并针对功能数据改进了有限维数据的偏态分布模型。


<details>
  <summary>Details</summary>
Motivation: 扩展功能数据的聚类方法，利用偏态分布更好地捕捉数据特征。

Method: 基于功能线性回归模型和三种偏态多元分布（方差-伽马、偏态-t、正态-逆高斯），构建EM算法估计参数。

Result: 在模拟数据和空气质量数据集上验证了方法的性能。

Conclusion: funWeightClustSkew能有效处理功能数据的聚类问题，尤其是偏态分布的数据。
...
### [313] [Enhanced Battery Capacity Estimation in Data-Limited Scenarios through Swarm Learning](https://arxiv.org/abs/2504.12444)
*Jiawei Zhang,Yu Zhang,Wei Xu,Yifei Zhang,Weiran Jiang,Qi Jiao,Yao Ren,Ziyou Song*

Main category: eess.SY

TLDR: 提出了一种基于群体学习的电池管理系统，在数据有限的情况下提升容量估计准确性，同时确保数据隐私和安全性。


<details>
  <summary>Details</summary>
Motivation: 数据驱动方法在电池管理中潜力巨大，但数据有限时性能不佳，且缺乏同时保障隐私和容错性的框架。

Method: 结合去中心化群体学习框架和基于可信权重的模型合并机制。

Result: 在四种数据有限场景下验证，群体学习显著提升估计准确性，性能接近大数据集中学习。

Conclusion: 群体学习为数据有限的电池管理提供了高效、隐私安全的解决方案。
...
### [314] [Mitigating LLM Hallucinations with Knowledge Graphs: A Case Study](https://arxiv.org/abs/2504.12422)
*Harry Li,Gabriel Appleby,Kenneth Alperin,Steven R Gomez,Ashley Suh*

Main category: cs.HC

TLDR: 论文研究了如何通过结合知识图谱（KG）减少大型语言模型（LLM）的幻觉问题，提出了LinkQ系统，并在实验中表现优于GPT-4，但仍需改进。


<details>
  <summary>Details</summary>
Motivation: 高风险的网络操作领域需要可信赖的AI方法，而LLM存在幻觉问题，因此研究如何通过KG增强LLM的可靠性。

Method: 开发了LinkQ系统，通过强制LLM在问答过程中查询KG获取真实数据，以减少幻觉。

Result: LinkQ在定量评估中表现优于GPT-4，但在某些问题类别上仍有困难，需进一步研究查询构建策略。

Conclusion: LinkQ展示了结合KG的潜力，但仍需改进，未来需探索更多查询策略以提升性能。
...
### [315] [Don't Just Translate, Agitate: Using Large Language Models as Devil's Advocates for AI Explanations](https://arxiv.org/abs/2504.12424)
*Ashley Suh,Kenneth Alperin,Harry Li,Steven R Gomez*

Main category: cs.HC

TLDR: 论文指出，当前XAI研究中使用LLM将可解释性技术输出翻译为自然语言的做法可能误导用户，建议LLM应扮演质疑者角色，促进用户批判性思考。


<details>
  <summary>Details</summary>
Motivation: 探讨LLM在XAI中翻译解释性输出的潜在问题，如用户过度依赖AI或误解解释，提出改进方向。

Method: 分析LLM在XAI中的翻译行为，提出LLM应作为质疑者，揭示模型局限性、偏见和不确定性。

Result: 发现单纯翻译XAI输出可能强化可解释性假象，而LLM的质疑角色可减少用户过度依赖。

Conclusion: 建议LLM在XAI中应主动质疑解释，而非简单翻译，以提升透明度和用户批判性思维。
...
### [316] [Co-Writing with AI, on Human Terms: Aligning Research with User Demands Across the Writing Process](https://arxiv.org/abs/2504.12488)
*Mohi Reza,Jeb Thomas-Mitchell,Peter Dushniku,Nathan Laundry,Joseph Jay Williams,Anastasia Kuzminykh*

Main category: cs.HC

TLDR: 论文探讨了生成式AI工具（如ChatGPT）对写作过程中作者代理感和所有权的影响，提出了四种AI写作支持的设计策略，并通过文献综述和访谈揭示了不同作者对AI干预的偏好差异。
...
Result: 研究发现作者对AI干预的偏好因写作过程阶段和写作目标而异，并提出了四种设计策略。

Conclusion: 研究为开发以人为本的AI协作写作工具提供了实用设计指导。
...
### [317] [Multimodal LLM Augmented Reasoning for Interpretable Visual Perception Analysis](https://arxiv.org/abs/2504.12511)
*Shravan Chaudhari,Trilokya Akula,Yoon Kim,Tom Blake*

Main category: cs.HC

TLDR: 该论文研究了多模态大语言模型（MLLMs）在视觉感知任务中的应用，提出了一种无需标注的分析框架，旨在评估MLLMs作为认知助手的实用性。


<details>
  <summary>Details</summary>
Motivation: 结合心理学和认知科学的原理，探索MLLMs在HCI任务中的解释性和实用性，以提升人类推理能力并揭示现有感知数据集的偏见。

Method: 利用心理学和认知科学的复杂性原则指导MLLMs解释视觉内容，提出了一种无需标注的分析框架。

Result: 论文为量化MLLMs的解释性提供了理论基础，并展示了其在视觉感知任务中的潜力。

Conclusion: 该研究为MLLMs在HCI和认知科学中的应用开辟了新方向，强调了其在解释性和偏见识别方面的价值。
...
### [318] [MobilePoser: Real-Time Full-Body Pose Estimation and 3D Human Translation from IMUs in Mobile Consumer Devices](https://arxiv.org/abs/2504.12492)
*Vasco Xu,Chenfeng Gao,Henry Hoffmann,Karan Ahuja*

Main category: cs.HC

TLDR: MobilePoser是一个实时系统，利用消费设备中已有的IMU子集进行全身姿态和全局平移估计，解决了传感器噪声和漂移带来的挑战。


<details>
  <summary>Details</summary>
Motivation: 随着运动捕捉技术向低精度IMU迁移，设备如手机、手表和耳机面临性能下降、时间一致性丢失和全局平移损失等问题。

Method: 采用多阶段深度神经网络进行运动姿态估计，随后通过基于物理的运动优化器，实现轻量级且高精度的结果。

Result: MobilePoser在精度上达到最新水平，同时保持轻量级。
...
### [319] [EmoVoice: LLM-based Emotional Text-To-Speech Model with Freestyle Text Prompting](https://arxiv.org/abs/2504.12867)
...
TLDR: EmoVoice是一种新型情感可控的TTS模型，利用LLMs实现细粒度自然语言情感控制，并通过并行输出音素和音频令牌增强内容一致性。
...
Result: 在英文EmoVoice-DB和中文Secap测试集上达到SOTA性能。

Conclusion: EmoVoice在情感控制TTS领域表现优异，并探索了多模态LLMs在情感评估中的应用。
...
### [320] [WaterFlow: Learning Fast & Robust Watermarks using Stable Diffusion](https://arxiv.org/abs/2504.12354)
*Vinay Shukla,Prachee Sharma,Ryan Rossi,Sungchul Kim,Tong Yu,Aditya Grover*

Main category: eess.IV

TLDR: WaterFlow (WF) 是一种基于学习潜在依赖水印的快速且鲁棒的视觉水印方法，解决了现有技术在计算速度和鲁棒性之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 当前水印技术在计算速度和鲁棒性之间存在不足，尤其是面对生成图像的快速崛起。

Method: 利用预训练的潜在扩散模型将图像编码到潜在空间，并通过可逆流层在傅里叶域中嵌入水印。

Result: WaterFlow 在鲁棒性上表现最优，并能有效防御组合攻击，验证了其在多个数据集上的性能。
...
### [321] [TUMLS: Trustful Fully Unsupervised Multi-Level Segmentation for Whole Slide Images of Histology](https://arxiv.org/abs/2504.12718)
*Walid Rehamnia,Alexandra Getmanskaya,Evgeniy Vasilyev,Vadim Turlapov*

Main category: eess.IV

TLDR: TUMLS是一种新型的无监督多级分割方法，通过自动编码器提取特征并基于不确定性选择代表性图像块，无需机器学习算法即可完成高分辨率核分割，显著提升数字病理学工作流程。


<details>
  <summary>Details</summary>
Motivation: 当前AI方法在组织病理学中面临标注成本高、计算需求大和缺乏不确定性估计等问题，阻碍了实际应用。

Method: TUMLS使用自动编码器从低分辨率数据中识别组织类型，基于不确定性选择代表性图像块，并在高分辨率空间进行无监督核分割。

Result: 在UPENN-GBM数据集上，自动编码器的MSE为0.0016；在MoNuSeg数据集上，核分割的F1分数为77.46%，Jaccard分数为63.35%，优于其他无监督方法。

Conclusion: TUMLS通过透明且高效的方法，显著推动了数字病理学的发展。
...
### [322] [Regist3R: Incremental Registration with Stereo Foundation Model](https://arxiv.org/abs/2504.12356)
*Sidun Liu,Wenyu Li,Peng Qiao,Yong Dou*

Main category: eess.IV

TLDR: Regist3R是一种新型立体基础模型，用于高效、可扩展的增量重建，解决了多视图3D重建中的计算成本和全局对齐误差问题。


<details>
  <summary>Details</summary>
Motivation: 多视图3D重建在计算机视觉中仍具挑战性，现有方法（如DUSt3R）在多视图场景中存在计算成本高和全局对齐误差累积的局限性。

Method: 提出Regist3R，采用增量重建范式，支持从无序多视图图像集合中进行大规模3D重建。

Result: 在公共数据集上，Regist3R性能与优化方法相当，计算效率显著提升，优于现有多视图重建模型。在倾斜航空数据集上表现优异，首次实现千视图级场景重建。

Conclusion: Regist3R展示了在大规模3D重建任务（如城市建模、航空测绘）中的实际应用潜力。
...
### [323] [Towards Cardiac MRI Foundation Models: Comprehensive Visual-Tabular Representations for Whole-Heart Assessment and Beyond](https://arxiv.org/abs/2504.13037)
*Yundi Zhang,Paul Hager,Che Liu,Suprosanna Shit,Chen Chen,Daniel Rueckert,Jiazhen Pan*

Main category: eess.IV

TLDR: ViTa是一个多模态框架，结合心脏磁共振成像（CMR）和患者健康数据，提供全面的心脏健康评估和疾病风险预测。


<details>
  <summary>Details</summary>
Motivation: CMR虽为心脏评估的金标准，但缺乏患者健康因素的整合，限制了全面评估。ViTa旨在填补这一空白。

Method: ViTa整合42,000名UK Biobank参与者的3D+T影像和患者数据，通过多模态融合生成共享潜在表征。

Result: ViTa支持多种下游任务，如心脏表型预测、分割和疾病分类，提供更全面的心脏健康分析。

Conclusion: ViTa为心脏健康提供了通用且个性化的理解，具有临床实用性和可扩展性。
...
### [324] [NTIRE 2025 Challenge on Short-form UGC Video Quality Assessment and Enhancement: Methods and Results](https://arxiv.org/abs/2504.13131)
...
TLDR: NTIRE 2025挑战赛聚焦短用户生成内容（UGC）视频质量评估与增强，包含高效视频质量评估（KVQ）和基于扩散的图像超分辨率（KwaiSR）两个赛道。


<details>
  <summary>Details</summary>
Motivation: 推动轻量级视频质量评估模型的发展，并提升短UGC平台的用户体验。

Method: Track 1优化VQA模型以减少计算成本；Track 2引入KwaiSR数据集，包含合成和真实图像对。

Result: 吸引了266名参与者，收到18份有效提交，推动了短UGC视频质量评估和图像超分辨率的研究。

Conclusion: 挑战赛成功促进了相关技术进步，并为短UGC平台提供了实用解决方案。
...
### [325] [Resonances in reflective Hamiltonian Monte Carlo](https://arxiv.org/abs/2504.12374)
*Namu Kroupa,Gábor Csányi,Will Handley*

Main category: stat.ML

TLDR: 高维中，反射式哈密顿蒙特卡洛在粒子群从狄拉克δ分布初始化且目标为均匀分布时，混合速度较慢。通过Sinkhorn散度量化的瞬时非均匀性，揭示了混合问题的机制。在球体和立方体中，集体运动在流体行为和离散化主导行为间转换，临界步长随维度呈幂律缩放。两种情况下粒子可能自发分离，导致密度共振和混合问题。此外，构建了低维模型重现高维问题特征，并与精确哈密顿粒子流对比，讨论了调参实践。


<details>
  <summary>Details</summary>
Motivation: 研究高维中反射式哈密顿蒙特卡洛在特定初始化条件下的混合问题，揭示其机制并提出改进方法。

Method: 使用Sinkhorn散度量化分布的非均匀性，分析球体和立方体中的集体运动行为，构建低维模型验证高维问题特征。

Result: 发现临界步长随维度呈幂律缩放，粒子可能自发分离导致混合问题，低维模型成功重现高维特征。

Conclusion: 揭示了混合问题的机制，提出了调参建议，为改进高维反射式哈密顿蒙特卡洛提供了理论支持。
...
### [326] [Robust and Scalable Variational Bayes](https://arxiv.org/abs/2504.12528)
*Carlos Misael Madrid Padilla,Shitao Fan,Lizhen Lin*

Main category: stat.ML

TLDR: 提出了一种鲁棒且可扩展的变分贝叶斯框架，通过子集划分和几何中位数聚合处理大数据集中的异常值。


<details>
  <summary>Details</summary>
Motivation: 解决大数据集中异常值和污染问题，提升变分贝叶斯方法的鲁棒性和可扩展性。

Method: 将数据集划分为子集，独立计算后验并应用变分近似，通过Wasserstein距离的几何中位数聚合。

Result: 提出的VM-Posterior保持了后验收缩性质，提供了鲁棒性保证，并在实验中表现出色。

Conclusion: VM-Posterior是一种适用于复杂污染数据集的可靠贝叶斯推断工具。
...
### [327] [Spectral Algorithms under Covariate Shift](https://arxiv.org/abs/2504.12625)
*Jun Fan,Zheng-Chu Guo,Lei Shi*

Main category: stat.ML

TLDR: 该论文研究了谱算法在分布偏移（协变量偏移）下的收敛行为，提出加权谱算法以解决无界密度比时的次优问题，并证明其最优收敛率。


<details>
  <summary>Details</summary>
Motivation: 探究谱算法在训练和测试数据分布不同的现实场景中的性能，特别是在协变量偏移下的表现。

Method: 在再生核希尔伯特空间框架下，分析谱算法的泛化误差，提出加权谱算法并引入权重裁剪技术。

Result: 加权谱算法在无界密度比情况下实现最优收敛率，解决了传统谱算法的次优问题。

Conclusion: 加权谱算法通过引入密度比信息和权重裁剪，显著提升了谱算法在分布偏移下的性能。
...
### [328] [When do Random Forests work?](https://arxiv.org/abs/2504.12860)
*C. Revelas,O. Boldea,B. J. M. Werker*

Main category: stat.ML

TLDR: 研究了随机森林中随机化分裂方向的有效性，发现随机化在低信噪比（SNR）下表现更好，但在某些数据特征下可能增加偏差。


<details>
  <summary>Details</summary>
Motivation: 探讨随机化在随机森林中的作用，尤其是在不同SNR和数据特征下的表现。

Method: 通过系统分析不同SNR下的样本外均方误差（MSE），并结合模拟研究验证随机化的效果。

Result: 随机化在低SNR下减少方差，但在某些情况下（如厚尾分布）可能增加偏差；相关协变量下随机化效果显著。

Conclusion: 随机化的效果不仅取决于SNR，还受数据特征影响；相关协变量对随机森林性能有重要影响。
...
### [329] [Propagation of Chaos in One-hidden-layer Neural Networks beyond Logarithmic Time](https://arxiv.org/abs/2504.13110)
*Margalit Glasgow,Denny Wu,Joan Bruna*

Main category: stat.ML

TLDR: 研究了多项式宽度神经网络与无限宽度网络在训练中的动态近似差距，并通过微分方程紧密界定了这一差距。


<details>
  <summary>Details</summary>
Motivation: 探讨神经网络宽度对动态行为的影响，特别是在特征学习任务中。

Method: 使用投影梯度下降和均值场动态，通过微分方程分析近似差距，并利用局部Hessian矩阵的性质。

Result: 证明了在自协调性质下，多项式数量的神经元足以近似均值场动态。

Conclusion: 多项式宽度的神经网络可以有效逼近无限宽度网络的动态行为。
...
### [330] [Boosting Reservoir Computing with Brain-inspired Adaptive Dynamics](https://arxiv.org/abs/2504.12480)
*Keshav Srinivasan,Dietmar Plenz,Michelle Girvan*

Main category: cs.NE

TLDR: 本文提出了一种自适应的储层计算机（RC）设计，通过动态调整兴奋-抑制（E-I）平衡来提高性能，减少对超参数调优的依赖。


<details>
  <summary>Details</summary>
Motivation: 储层计算机（RCs）虽然简化了训练过程，但对超参数敏感，且忽略了神经元动态中的兴奋-抑制平衡，这是大脑功能的关键。

Method: 引入自调节机制，局部调整E-I平衡以实现目标神经元放电率，并结合大脑启发的异质性。

Result: 自适应RC在记忆能力和时间序列预测等任务中性能提升高达130%，且减少了超参数调优的需求。
...
TLDR: 本文研究了匿名公共公告的逻辑形式化，探讨了其介于外部公告和代理公告之间的特性，并分析了背景知识如何揭示公告者身份。


<details>
  <summary>Details</summary>
Motivation: 研究匿名公共公告的逻辑特性，揭示其在特定背景下可能泄露公告者身份的现象。

Method: 首先分析无意图假设下的情况，逻辑可归约为认知逻辑；其次研究共同知识意图假设下的情况，涉及“安全”公告概念。

Result: 提出了形式化表达性结果和关键逻辑语言的公理完备性。

Conclusion: 匿名公共公告在意图假设下具有复杂性，与“安全”公告概念相关，研究成果为逻辑语言提供了理论基础。
...
### [332] [Attractor-merging Crises and Intermittency in Reservoir Computing](https://arxiv.org/abs/2504.12695)
*Tempei Kabayama,Motomasa Komuro,Yasuo Kuniyoshi,Kazuyuki Aihara,Kohei Nakajima*

Main category: nlin.CD

TLDR: 储层计算可通过调整全局参数在随机神经网络中引发吸引子合并危机及间歇性现象，其机制与相位空间结构相关，且具有普适性。


<details>
  <summary>Details</summary>
Motivation: 研究储层计算中随机神经网络吸引子行为的动态变化及其机制。

Method: 通过调整全局参数观察吸引子合并危机，分析相位空间结构以揭示机制。

Result: 发现吸引子合并危机伴随间歇性现象，且该现象在多种随机神经网络中普遍存在。

Conclusion: 储层计算中的吸引子合并危机是普适现象，与训练数据无关。
...
### [333] [The Dissipation Theory of Aging: A Quantitative Analysis Using a Cellular Aging Map](https://arxiv.org/abs/2504.13044)
*Farhan Khodaee,Rohola Zandie,Yufan Xia,Elazer R. Edelman*

Main category: q-bio.QM

TLDR: 提出基于动力系统的新衰老理论，通过数据驱动方法量化细胞水平变化，揭示衰老为生物系统中的耗散过程。


<details>
  <summary>Details</summary>
Motivation: 探索衰老的动力学本质，提供分子分辨率下的衰老变化测量框架。

Method: 结合遍历理论和基于Transformer的机器学习算法，分析基因表达数据，构建细胞衰老图谱（CAM）。

Result: 发现基因嵌入空间的分歧、非线性转变和熵变化模式，验证衰老为耗散过程。

Conclusion: 为衰老研究提供新视角和计算工具，支持分子水平的衰老动态分析。
...
### [334] [Themisto: Jupyter-Based Runtime Benchmark](https://arxiv.org/abs/2504.12365)
*Konstantin Grotov,Sergey Titov*

Main category: cs.SE

TLDR: 本文提出了一个基于Jupyter笔记本开发轨迹的基准测试，用于评估大型语言模型（LLMs）在利用运行时信息预测代码输出和生成代码方面的表现。研究发现当前LLMs在此类任务上表现不佳，并指出代码模型开发中一个被忽视的领域：运行时上下文的整合。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于探索LLMs在代码预测和生成任务中利用运行时信息的能力，填补当前研究中对此领域的不足。

Method: 方法是通过构建一个包含Jupyter笔记本开发轨迹的基准测试，评估LLMs在预测代码输出和生成代码时的表现。

Result: 结果显示当前LLMs在此类任务上表现不佳，表明运行时上下文的重要性被低估。

Conclusion: 结论指出，代码模型开发需要更多关注运行时上下文的整合，以提升模型性能。
...
### [335] [Code Copycat Conundrum: Demystifying Repetition in LLM-based Code Generation](https://arxiv.org/abs/2504.12608)
...
TLDR: 该论文研究了大型语言模型（LLM）生成代码中的重复问题，提出了一种基于规则的检测和缓解方法DeRep，显著提高了代码质量。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM在代码生成方面取得进展，但生成的代码存在重复问题，影响效率和可读性。

Method: 通过定量和定性分析19种先进代码LLM的重复现象，总结20种重复模式，并提出DeRep技术。

Result: DeRep在减少重复（平均提升91.3%、93.5%和79.9%）和提高代码质量（Pass@1提升208.3%）方面显著优于基线。

Conclusion: DeRep能有效缓解代码重复问题，并提升现有方法的性能。
...
### [336] [A Phenomenological Approach to Analyzing User Queries in IT Systems Using Heidegger's Fundamental Ontology](https://arxiv.org/abs/2504.12977)
*Maksim Vishnevskiy*

Main category: cs.SE

TLDR: 论文提出了一种基于海德格尔基础本体论的新型IT分析系统，通过区分存在者与存在，使用两种模态语言处理用户输入和内部分析，并通过现象学还原模块连接两者，以揭示更深层次的本体论模式。


<details>
  <summary>Details</summary>
Motivation: 现有系统仅局限于范畴分析，无法处理复杂交互中的逻辑陷阱（如隐喻），因此需要一种基于现象学存在分析的方法来提供更深入的洞察。

Method: 系统采用两种模态语言（存在者的范畴语言和存在的存在语言），通过现象学还原模块连接，分析用户查询并识别递归和自指结构。
...
### [337] [Post-processing improves accuracy of Artificial Intelligence weather forecasts](https://arxiv.org/abs/2504.12672)
...
TLDR: 统计后处理方法适用于AI天气预报模型，与传统NWP模型结合可提升预报技能。


<details>
  <summary>Details</summary>
Motivation: AI天气模型虽性能接近业务级，但仍存在系统偏差和可靠性问题，需验证现有后处理方法对其适用性。

Method: 测试IMPROVER统计后处理系统对ECMWF的AIFS模型的应用，并与传统NWP模型后处理结果对比。

Result: 后处理显著提升AIFS的准确性，与传统NWP效果相当；结合AI与NWP模型可进一步提高预报技能。

Conclusion: 现有NWP后处理方法可直接用于AI模型，便于气象中心低风险整合AI预报。
...
### [338] [Codes over Finite Ring $\mathbb{Z}_k$, MacWilliams Identity and Theta Function](https://arxiv.org/abs/2504.12604)
...
Abstract: In this paper, we study linear codes over $\mathbb{Z}_k$ based on lattices
...
### [339] [Predictive control of blast furnace temperature in steelmaking with hybrid depth-infused quantum neural networks](https://arxiv.org/abs/2504.12389)
*Nayoung Lee,Minsoo Shin,Asel Sagingalieva,Ayush Joshi Tripathi,Karan Pinto,Alexey Melnikov*

Main category: quant-ph

TLDR: 提出了一种结合混合量子机器学习和喷煤控制的新方法，显著提高了高炉温度预测和稳定性。


<details>
  <summary>Details</summary>
Motivation: 传统方法难以处理高炉温度的复杂非线性波动，需更精准的预测和稳定控制以优化钢铁生产效率。
...
### [340] [Featuremetric benchmarking: Quantum computer benchmarks based on circuit features](https://arxiv.org/abs/2504.12575)
*Timothy Proctor,Anh Tran,Xingxin Liu,Aditya Dhumuntarao,Stefan Seritan,Alaina Green,Norbert M Linke*

Main category: quant-ph

TLDR: 提出了一种基于量子电路特征的基准测试框架，用于更全面地评估多量子比特计算机的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试方法（如体积基准测试）无法充分捕捉量子计算机性能的多样性，需要更丰富的模型。

Method: 通过量化量子计算机在不同电路特征（如深度、宽度、双量子比特门密度等）下的表现，构建特征度量基准测试框架。

Result: 在IBM Q和IonQ系统（最多27量子比特）上验证了该框架的有效性，并展示了如何通过高斯过程回归生成性能摘要。

Conclusion: 特征度量基准测试框架提供了更丰富、更准确的性能模型，适用于体积基准测试等场景。
...
### [341] [Query Complexity of Classical and Quantum Channel Discrimination](https://arxiv.org/abs/2504.12989)
*Theshani Nuradha,Mark M. Wilde*

Main category: quant-ph

TLDR: 研究了量子信道区分的查询复杂度，分析了最小信道使用次数与错误概率的关系，并给出了经典信道和多信道区分的具体结果。


<details>
  <summary>Details</summary>
Motivation: 从信息论角度研究量子信道区分，目标是确定达到期望错误概率所需的最小信道使用次数。

Method: 通过分析二进制信道区分的查询复杂度，推导其对错误概率和对数信道保真度的依赖关系。

Result: 二进制信道区分的查询复杂度与错误概率的对数和信道保真度的负对数相关；经典信道区分的结果被精确刻画；多信道区分的查询复杂度与信道数量对数相关。

Conclusion: 研究提供了量子信道区分查询复杂度的理论框架，为实际应用提供了理论支持。
...
### [342] [Large Language Model-Based Knowledge Graph System Construction for Sustainable Development Goals: An AI-Based Speculative Design Perspective](https://arxiv.org/abs/2504.12309)
*Yi-De Lin,Guan-Ze Liao*

Main category: cs.CY

TLDR: 本研究开发了一个AI知识图谱系统，分析SDG目标间的关联，并提出潜在新目标。
...
Method: 结合官方SDG文本、Elsevier关键词数据集和TED演讲转录，使用AI推测设计、大语言模型和检索增强生成技术。

Result: 发现目标10与16关联性强，目标6覆盖少；知识图谱揭示新中心节点；提出6个潜在新目标。

Conclusion: AI推测框架为政策制定者提供新见解，并为未来多模态和跨系统SDG应用奠定基础。
...
### [343] [Towards an AI Observatory for the Nuclear Sector: A tool for anticipatory governance](https://arxiv.org/abs/2504.12358)
*Aditi Verma,Elizabeth Williams*

Main category: cs.CY

TLDR: 呼吁建立核能领域AI的预见性治理体系和全球AI观察站。


<details>
  <summary>Details</summary>
Motivation: AI在核能领域的广泛应用，但其安全、安保和保障影响尚未充分理解。

Method: 结合科技研究、公共政策和前瞻性研究，探讨核能AI观察站和预见性治理体系的框架。

Result: 提出了核能AI观察站和预见性治理体系的初步构想。

Conclusion: 需要建立全球性机制以预见和管理AI在核能领域的潜在风险。
...
### [344] [What do people expect from Artificial Intelligence? Public opinion on alignment in AI moderation from Germany and the United States](https://arxiv.org/abs/2504.12476)
*Andreas Jungherr,Adrian Rauchfleisch*

Main category: cs.CY

TLDR: 研究通过德美两国的调查，探讨公众对AI系统功能特征的期望，发现美国对AI对齐特征支持更高，而两国均更重视准确性和安全性。
...
Method: 在德国（n=1800）和美国（n=1756）进行问卷调查，分析公众对AI对齐功能的支持程度及其影响因素。

Result: 美国受访者对AI使用更多且支持度更高；两国均最重视准确性和安全性，但对公平和理想目标的支持较谨慎。

Conclusion: 研究强调公众态度对AI治理的重要性，需将规范性期望纳入理论和政策讨论。
...
### [345] [Knowledge Acquisition on Mass-shooting Events via LLMs for AI-Driven Justice](https://arxiv.org/abs/2504.12545)
*Benign John Ihugba,Afsana Nasrin,Ling Wu,Lin Li,Lijun Qian,Xishuang Dong*

Main category: cs.CY

TLDR: 论文提出首个用于大规模枪击事件知识获取的数据集，利用命名实体识别（NER）技术提取关键信息，如罪犯、受害者、地点和犯罪工具，并通过大型语言模型（LLM）实现高效信息提取。实验表明GPT-4o在NER任务中表现最佳。
...
Method: 应用命名实体识别（NER）技术，结合大型语言模型（LLM）的少样本提示方法，从新闻、警方报告和社交媒体中提取关键实体。
...
### [346] [How Large Language Models Are Changing MOOC Essay Answers: A Comparison of Pre- and Post-LLM Responses](https://arxiv.org/abs/2504.13038)
*Leo Leppänen,Lili Aunimo,Arto Hellas,Jukka K. Nurminen,Linda Mannila*

Main category: cs.CY

TLDR: 论文研究了ChatGPT发布后对学生在线教育的影响，分析了学生论文的长度、风格和内容变化。


<details>
  <summary>Details</summary>
Motivation: 探讨大型语言模型（LLMs）对在线教育的潜在影响，尤其是学术诚信和学习方式的变化。

Method: 通过分析一个关于AI伦理的大学MOOC课程中学生论文的多年度数据集，比较ChatGPT发布前后的变化。

Result: ChatGPT的发布与论文长度和风格的显著变化相关，但主题内容未发生根本性改变。

Conclusion: LLMs对在线教育产生了可量化的影响，但并未彻底改变学生的讨论主题。
...
### [347] [Tackling Social Bias against the Poor: A Dataset and Taxonomy on Aporophobia](https://arxiv.org/abs/2504.13085)
*Georgina Curto,Svetlana Kiritchenko,Muhammad Hammad Fahim Siddiqui,Isar Nejadgholi,Kathleen C. Fraser*

Main category: cs.CY

TLDR: 该论文旨在通过社交媒体数据识别和追踪针对贫困人群的偏见（aporophobia），为消除贫困政策提供支持。


<details>
  <summary>Details</summary>
Motivation: 贫困是联合国可持续发展目标的首要任务，但社会对贫困人群的偏见（aporophobia）阻碍了扶贫政策的制定和实施。

Method: 与政府和非营利组织合作，收集并标注英语推文，构建aporophobia分类法，并训练分类器进行自动检测。

Result: 建立了aporophobia的分类法，并揭示了社交媒体上自动检测aporophobia的主要挑战。
