...
Motivation: 探讨用户对消息平台中防止第三方追踪通信关系的功能需求，以指导隐私增强功能和不可追踪通信协议的设计。

Method: 通过基于情景的问卷调查（189名参与者），分析用户对虚构平台Texty和Chatty的功能建议。

Result: 用户对不可追踪性的理解多样，涉及多种隐私增强技术，但认知与现有协议威胁模型存在差距。

Conclusion: 用户认知与协议设计的差异为不可追踪通信工具的采用带来挑战与机遇。
...
<details>
  <summary>Details</summary>
Motivation: 现有IoC提取工具因缺乏高质量真实数据而效果受限，手动标注成本高，自动方法则导致高误报率。

Method: 引入混合人机协作流程，结合语言模型分类器（LANCE）与专家验证，实现上下文感知标注。

Result: 系统将分析师工作量减少43%，并生成了高质量的公开基准数据集PRISM（1,791个标注IoC）。

Conclusion: 该方法提升了IoC提取的精度和可重复性，为威胁情报研究提供了可靠基础。
...
<details>
  <summary>Details</summary>
Motivation: 探讨LLMs/LRMs是否具备真实推理能力，避免目标不断变化的争论。

Method: 通过提出两个分析性基准（"tautological" benchmarks）来明确区分推理与模式匹配。

Result: 未明确提及具体实验结果，但提出了解决争议的理论框架。

Conclusion: 通过明确的分析基准，为LLMs/LRMs的推理能力提供了更清晰的评估标准。
...
Method: 采用双域水印方法，结合空间和频域嵌入，并引入高斯噪声恢复器（GNR）增强鲁棒性。

Result: 在多种图像扭曲和高级攻击下，GaussMarker表现最优，召回率高且误报率低。
...
<details>
  <summary>Details</summary>
Motivation: 随着机器学习即服务（MLaaS）的发展，确保隐私和计算正确性成为挑战，零知识密码学为解决这一问题提供了可能。

Method: 采用零知识密码学生成计算完整性证明，并在差分隐私线性回归任务中验证训练过程的正确性。

Result: 在单机上6分钟内完成了5万样本的差分隐私线性回归训练，验证仅需0.17秒，是目前文献中同类任务的最快结果。

Conclusion: 该成果为端到端私有MLaaS的实现迈出了重要一步。
...
<details>
  <summary>Details</summary>
Motivation: 尽管MLLMs在音频-视觉任务中表现出色，但其依赖第三方数据和开源模型带来了安全隐患，亟需系统研究其安全漏洞。

Method: 通过文献综述，系统分析了音频-视觉攻击的类型，包括对抗攻击、后门攻击和越狱攻击，并总结了最新MLLMs的攻击案例。

Result: 研究发现MLLMs易受多种攻击，现有综述局限于特定攻击类型，缺乏统一视角。

Conclusion: 本文填补了音频-视觉攻击系统综述的空白，为未来安全防御研究提供了方向和挑战。
...
<details>
  <summary>Details</summary>
Motivation: 解决外包神经网络推理中的隐私问题，现有框架因高计算和通信成本而难以实用。

Method: 集成布尔GMW协议、NTT预处理和GPU加速，支持高效安全比较和线性操作。

Result: 在线非线性操作提速17倍，通信开销降低，GPU上端到端推理时间2.8秒。
...
<details>
  <summary>Details</summary>
Motivation: 传统的函数级匹配方法在二进制代码相似性分析中时间复杂度过高，难以应对大规模场景（如1/n到n搜索）。

Method: 提出KEENHash，通过大语言模型生成函数嵌入，结合K-Means和特征哈希将二进制代码压缩为固定长度的程序级表示。

Result: 实验表明，KEENHash比现有方法快至少215倍，且在53亿次相似性评估中仅需395.83秒，而传统方法需至少56天。

Conclusion: KEENHash在大规模二进制代码相似性分析中表现出色，尤其在恶意软件检测等安全场景中显著优于现有方法。
...
Conclusion: FAA框架可自动化大部分工作，有效减轻分析师的负担。
...
<details>
  <summary>Details</summary>
Motivation: 5G及未来网络中，毫米波、太赫兹频段和超密集网络技术带来了移动设备在快速移动时的频繁切换认证问题，可能导致连接中断和恶意攻击。

Method: 利用数字孪生作为智能中介，提前处理计算并协助移动设备在目标基站接入前完成安全互认证和密钥协商。

Result: 通过形式化验证和非正式分析，证明该方案具备多样化的安全功能，且在信令、计算和通信开销方面优于相关方案。

Conclusion: 该方案为5G及未来网络中的安全切换认证提供了高效且安全的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 移动设备普及带来便利的同时，安全与隐私风险日益复杂，亟需更先进的检测方法。

Method: 应用大语言模型（LLMs）识别和缓解移动应用中的安全风险，特别是针对敏感数据泄露的检测。

Result: LLMs在识别和缓解移动应用安全风险方面表现出可行性，有望替代传统分析方法。

Conclusion: LLMs在移动安全领域具有潜力，但仍面临开放的研究挑战。
...
<details>
  <summary>Details</summary>
Motivation: 机器学习模型可能泄露敏感信息，差分隐私提供了一种框架来减少隐私风险，确保算法输出不因单个数据点的变化而显著改变。

Method: 回顾差分隐私的基础定义、原始表述及关键研究进展，分析其在机器学习中的集成方法，并探讨实际评估技术。

Result: 差分隐私为机器学习提供了一种有效的隐私保护机制，但其仍面临对抗攻击等挑战。

Conclusion: 差分隐私在机器学习中具有重要价值，但其实际应用需进一步研究以应对潜在挑战。
...
Method: 将数据集重组为一系列动态变化的训练和测试标签，利用Mann-Kendall检验验证模型学习效果。

Result: 在4个时间数据集和5个模型上验证，性能随时间变化不一致，表明大多数模型未学习。

Conclusion: 动态调整标签的方法揭示了模型学习能力的不足，挑战了‘更多回顾信息带来更好性能’的直觉。
...
<details>
  <summary>Details</summary>
Motivation: 评估HbHAI技术是否能在加密数据上高效运行现有AI算法，同时保证安全性和性能。

Method: 使用传统无监督和监督学习技术（聚类、分类、深度神经网络）测试HbHAI保护的数据集。

Result: 结果基本验证了HbHAI的宣称性能，仅存在少量保留意见。

Conclusion: HbHAI在加密数据上运行AI算法的性能和安全表现优异，具有潜在应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有PRAC实现（如Chronus）在解决Rowhammer攻击时，存在性能下降或能耗增加的问题。

Method: 通过解耦计数器访问与数据访问的关键路径，优化计数器请求的缓冲与合并。

Result: 相比现有方案，计数器访问的行激活减少75%-83%，性能开销可忽略，动态能耗仅增加0.84%-1%。

Conclusion: CnC-PRAC在性能与能耗上均优于现有方案，为DDR5提供高效安全的Rowhammer防护。
...
Method: 通过分析眼动模式（如注视时长和不规则扫视），提出改进方案，并采用随机森林分类器和层次聚类方法。

Result: 随机森林分类器实现了88.58%的准确率，层次聚类方法成功识别了不同严重程度的阅读障碍。

Conclusion: 结合眼动追踪和机器学习为非侵入性诊断提供了高准确性和可及性，是诊断技术的重大进步。
...
<details>
  <summary>Details</summary>
Motivation: 探讨数据科学作为一个自然生态系统的本质，强调数据宇宙的复杂性与数据生命周期的结合，以及数据代理和科学家的角色。

Method: 提出一个以数据为中心的框架，将数据科学分为计算数据科学和基础数据科学，并定义特定学科驱动的数据科学和泛数据科学。

Result: 指出计算数据科学与基础数据科学之间存在潜在分歧，并强调评估数据宇宙发现实用性的重要性。

Conclusion: 建议通过严格评估数据宇宙发现的实用性来缓解计算数据科学与基础数据科学之间的分歧，促进数据科学的健康发展。
...
<details>
  <summary>Details</summary>
Motivation: 集中式训练的基础模型存在隐私和传输成本问题，而联邦学习（FL）和个性化联邦学习（PFL）提供了分布式替代方案，但现有研究多局限于模拟环境，忽视了真实场景中的异构性。

Method: 针对数据异构性，提出任务相似性感知的模型聚合方法；针对模型异构性，提出维度不变模块以实现跨模型知识共享。

Result: 在包含40个任务的多模态PFL基准测试中，所提方法在个性化和泛化能力上均优于现有技术。

Conclusion: 该方法有效解决了真实场景中的异构性问题，为PFL的实际应用提供了可行方案。
...
Result: T2I系统将吸引力与智力等无关特质关联；性别分类模型在“不具吸引力”面孔（尤其是非白人女性）上错误率更高。

Conclusion: 研究结果引发对数字身份系统公平性的担忧，需进一步解决算法偏见。
...
<details>
  <summary>Details</summary>
Motivation: 动态分时电价需要识别受益家庭，但真实数据存在隐私问题，因此需评估合成数据方法的适用性。

Method: 比较了四种合成数据生成方法（WGAN、CTGAN、扩散模型和高斯噪声增强），评估其分类效用、分布保真度和隐私泄漏。

Result: 扩散模型在分类效用上表现最佳（macro-F1达88.2%），CTGAN在抵抗重构攻击上最强。

Conclusion: 结构化生成模型在隐私保护和数据驱动的能源系统开发中具有潜力。
...
Method: 使用Qwen 2.5的小规模代码版本结合GRPO，通过显式推理步骤生成代码，并将推理反馈集成到强化学习循环中。

Result: 实验表明，模型在数学逻辑问题基准测试中显著提高了推理质量、代码准确性和逻辑正确性。

Conclusion: 该方法对缺乏大量训练资源的编程语言具有广泛的应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 填补机器学习算法与流行病学应用之间的差距，解决基准数据集方法在真实数据中表现不佳的问题。
...
Method: 提出YingLong框架，采用非因果双向注意力编码器，通过掩码标记恢复训练，并结合多输入集成减少输出方差。

Result: 在ETT和Weather数据集上，YingLong零样本任务表现优异，最佳性能提升60%以上；在GIFT-Eval基准测试中，分别以14%和44%的优势超越其他模型。

Conclusion: YingLong框架在时间序列预测中具有显著优势，尤其适用于长输出任务，且模型已开源。
...
<details>
  <summary>Details</summary>
Motivation: 反向传播（BP）在生物和硬件视角下存在局限性，如对称权重、非局部信用分配等问题，需要更高效的替代方法。

Method: FTP通过两次前向传递替代反向传播，利用前向计算估计层间目标，无需对称反馈权重或可学习逆函数。

Result: FTP在MNIST、CIFAR10和CIFAR100上表现与BP相当，且在低精度和硬件约束下优于BP，效率高于其他生物启发方法。

Conclusion: FTP为高效节能的设备和神经形态计算提供了有前景的方向。
...
Method: 利用预训练的视觉语言模型（VLMs），通过任务对齐提示（如zero-shot-s²）引导模型更聚焦的推理。
...
<details>
  <summary>Details</summary>
Motivation: 解决轴承和感应电机诊断问题，通过多传感器数据融合提高诊断效果。

Method: 使用CNN评估加速度计和麦克风数据，结合LSTM有效融合传感器信息。

Result: 展示了多传感器数据融合在诊断中的有效性，为多模型诊断提供了新思路。

Conclusion: 鼓励研究人员关注多模型诊断，并收集更多多传感器数据以优化诊断效果。
...
<details>
  <summary>Details</summary>
Motivation: 隐藏参数（如机器人质量分布或摩擦）的变化可能带来执行时的安全风险，需要一种机制来实时适应和约束。

Method: 基于约束隐藏参数马尔可夫决策过程，利用函数编码器实时推断隐藏参数，并通过共形预测考虑不确定性。

Result: 实验表明，该方法显著减少安全违规，并在不同环境中表现出强大的泛化能力，运行时开销极小。
...
<details>
  <summary>Details</summary>
Motivation: 传统深度学习中的几何相似性模型与人类心理感知不符，而Tversky的基于特征的相似性理论更具心理合理性，但尚未在深度学习中应用。

Method: 开发了可微分的Tversky相似性参数化方法，并设计了Tversky投影层，用于神经网络中替代线性投影层。

Result: 在NABirds图像分类任务中，Tversky投影层使准确率相对提升24.7%；在GPT-2模型中，困惑度降低7.5%，参数减少34.8%。

Conclusion: Tversky投影层为深度学习提供了一种更符合心理相似性理论的新范式，同时提升了模型的性能和可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习（FL）虽能保护数据隐私，但研究表明梯度仍可能导致隐私泄露。本文旨在探索客户端如何利用梯度攻击其他客户端的私有数据。

Method: 提出恶意好奇客户端威胁模型，结合梯度反演和恶意更新策略，开发了一种数据重建算法。

Result: 实验表明，现有服务器端和客户端的防御机制可能无法抵御此攻击，甚至可能加剧数据泄露。

Conclusion: 本文揭示了FL中新的隐私风险，呼吁重新审视防御机制的设计。
...
Method: 提出ICL框架，包括THI模块（利用多模态大语言模型进行多轮交互）和RDA策略（通过信息丰富和多样性增强优化训练文本）。

Result: 在四个TIReID基准测试中表现出显著性能提升。

Conclusion: ICL框架通过人机交互和数据增强有效解决了现有方法的局限性，提升了行人重识别的准确性。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注RAG系统中投毒攻击对模型输出质量的影响，而忽略了其对模型偏见的放大作用。

Method: 设计了基于多目标奖励函数的对抗性文档生成方法，利用子空间投影技术操纵检索结果，并构建循环反馈机制持续放大偏见。

Result: 实验表明BRRA攻击能显著增强模型偏见，同时提出了双阶段防御机制。

Conclusion: 研究揭示了RAG系统投毒攻击直接放大模型偏见，强调了RAG系统安全性与模型公平性的关系。
...
Method: 采用图表示学习解决数据稀缺问题，并利用帕累托优化处理任务间的依赖关系。

Result: GRePO-LTV框架能够有效提升LTV预测的准确性。

Conclusion: 该框架为LTV预测提供了一种创新且高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决类增量学习中预训练模型存在的提示覆盖和维度不一致问题，以及混合专家（MoE）在动态环境中的参数覆盖挑战。

Method: 提出MoTE框架，结合任务感知专家过滤和可靠专家联合推断机制，避免灾难性遗忘，并通过适配器扩展探索性能与模型规模的平衡。

Result: 实验表明MoTE在无需示例集的情况下表现优异，且任务数量与适配器数量线性扩展。

Conclusion: MoTE框架有效解决了类增量学习中的维度不一致和参数覆盖问题，同时探索了适配器扩展与性能的权衡。
...
<details>
  <summary>Details</summary>
Motivation: 分类器无关引导（CFG）在文本到图像的潜在扩散模型中表现优异，但在高权重下会导致颜色失真。论文旨在解决这一问题。

Method: 通过理论分析潜在空间中的样本范数放大现象，提出ADG算法，约束幅度变化并优化角度对齐。

Result: 实验表明，ADG显著优于现有方法，生成的图像在文本对齐、颜色保真度和人类感知偏好方面表现更好。

Conclusion: ADG有效解决了CFG的颜色失真问题，同时保持了其优势，为高质量图像合成提供了新方法。
...
TLDR: SEC-bench是一个自动化基准测试框架，用于评估LLM代理在真实安全工程任务中的表现，填补了现有基准测试的不足。
...
Method: SEC-bench采用多代理架构，自动构建代码仓库、复现漏洞并生成补丁，成本仅为每实例0.87美元。
...
Method: 通过建立AI驱动时间序列分析的演化路线图，从机器学习到LLM驱动范式，再到原生时间基础模型；从工作流程角度系统化LLM驱动时间序列分析的技术框架。
...
<details>
  <summary>Details</summary>
Motivation: 传统图神经网络（GNN）在多反应物反应建模中存在局限性，无法有效捕捉高阶关系。

Method: 提出ChemHGNN框架，利用超边建模多反应物反应，引入反应中心感知负采样策略（RCNS）和分层嵌入方法。

Result: 在USPTO数据集上，ChemHGNN显著优于HGNN和GNN基线，特别是在大规模场景下。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型（LLM）量化方法性能提升有限，需要新的研究方向来支持更高效的模型压缩。

Method: 提出Q2N模块，设计高效的零空间投影近似方法，并推导闭式解以满足实际推理条件。

Result: 在多种先进LLM和基准测试中验证了Q2N和零空间优化的有效性。

Conclusion: 论文为零空间优化的量化方法提供了初步探索，希望为未来研究提供启发。
...
<details>
  <summary>Details</summary>
Motivation: 现有工具使用训练数据的生成方法多为非交互式或非组合式，限制了在线RL训练的效果。

Method: 提出RandomWorld流程，生成交互式和组合式的工具使用数据，并通过SFT和RL训练模型。

Result: 在NESTFUL数据集上两项指标达到新SoTA，且下游性能随训练数据量提升。

Conclusion: RandomWorld为完全合成数据的使用提供了进一步改进的可能性。
...
<details>
  <summary>Details</summary>
Motivation: 封闭源模型的置信度估计对可靠性至关重要，但现有数据增强方法讨论有限，限制了潜力。

Method: 研究不同数据增强方法对置信度估计的影响，分析其效果和适用性。

Result: 数据增强策略能提升性能并缓解过度自信，数据多样性是关键，随机组合增强策略表现最佳。

Conclusion: 数据多样性对增强效果至关重要，随机组合增强策略在参数可迁移性和实用性上表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 传统偏见检测方法依赖敏感标签或固定公平性指标，限制了实际应用。本文旨在通过人类视觉直觉提供一种高效、可扩展的替代方案。

Method: 开发了一个轻量级网络平台，展示简化数据可视化（如薪资分布），收集用户对群体相似性的二元判断，并通过统计测试和机器学习验证用户反馈。

Result: 研究发现非专家用户的感知信号与已知偏见案例可靠相关，表明视觉直觉可作为公平性审计的有效代理。

Conclusion: 该方法为公平性诊断提供了一种标签高效、可解释的替代方案，推动了基于众包的偏见检测流程的发展。
...
<details>
  <summary>Details</summary>
Motivation: 传统实数神经网络（RVNNs）在低信噪比环境下难以捕捉无线信号的关键特性（如相位和幅度），限制了频谱感知的效率。

Method: 提出CMuSeNet，采用复数神经网络和残差架构，结合CFL和CIoU优化训练性能。

Result: 在合成、室内和真实数据集上，CMuSeNet平均准确率达98.98%-99.90%，比RVNN提升9.2个百分点，训练时间减少92.2%。

Conclusion: 复数神经网络架构在低信噪比环境下显著提升了弱信号检测和训练效率。
...
<details>
  <summary>Details</summary>
Motivation: 随着无人机市场的增长，其安全威胁日益严重，但无人机音频分类领域的数据稀缺问题亟待解决。

Method: 采用参数高效微调、数据增强和预训练网络（如EfficientNet-B0）等创新方法。

Result: 实现了超过95%的验证准确率。
...
<details>
  <summary>Details</summary>
Motivation: 现有电力价格预测方法过度依赖数值历史数据，忽略了文本信号。本文旨在填补这一空白，评估多模态输入（包括新闻和天气）对预测的影响。

Method: 构建NSW-EPNews数据集，包含17.5万条价格数据、天气和市场新闻摘要。任务为48步预测，使用滞后价格、向量化新闻和天气特征（传统模型）或提示工程（LLMs）。

Result: 传统模型从新闻特征中获益有限；先进LLMs（如GPT-4o和Gemini 1.5 Pro）性能略有提升，但频繁出现幻觉问题（如虚构价格序列）。

Conclusion: NSW-EPNews为多模态环境下的数值推理提供了严格测试平台，揭示了当前LLMs在高风险能源预测中的能力差距。
...
Method: 提出ACCORD框架，结合自回归约束生成和动态注意力路由，使用Llama 8B模型。

Result: ACCORD在多种NP难问题上表现优于标准方法，甚至超过更大的LLMs（如GPT-4）。

Conclusion: ACCORD是首个大规模端到端框架，成功扩展了LLMs在组合优化中的应用范围。
...
<details>
  <summary>Details</summary>
Motivation: 现有用户行为序列预训练方法依赖手动构建行为词汇，耗时且易偏，限制了模型泛化能力。

Method: 采用学生-教师编码器方案，自动构建未来时间窗口内行为信息的监督嵌入。

Result: 在两个工业数据集和八个下游任务中，AUC平均提升3.9%，训练吞吐量提升98.9%；在线部署中，KS指标提升2.7%-7.1%。
...
<details>
  <summary>Details</summary>
Motivation: 物联网环境的动态性和不确定性导致数据分布变化和系统需求演变，挑战MLaaS的长期有效性。

Method: 框架整合了服务评估模型和候选选择模型，采用上下文多臂老虎机优化策略逐步更新MLaaS组合。

Result: 实验证明该方法能保持服务质量，同时降低计算成本。

Conclusion: 该自适应框架有效解决了物联网环境下MLaaS组合的动态适应问题。
...
Result: 成功构建了PolyMicros模型，并通过零样本学习解决了3D实验显微镜中的多个长期挑战。
...
### [51] [xInv: Explainable Optimization of Inverse Problems](https://arxiv.org/abs/2506.11056)
*Sean Memery,Kevin Denamganai,Anna Kapron-King,Kartic Subr*

Main category: cs.LG

TLDR: 提出一种方法，通过优化器生成的轨迹生成人类可解释的解释，利用可微分模拟器和语言模型实现。


<details>
  <summary>Details</summary>
Motivation: 逆问题的迭代优化过程对领域专家来说通常难以理解，需要一种可解释的方法。

Method: 通过可微分模拟器在正向和反向传递中生成自然语言事件，并用语言模型后处理生成解释。

Result: 在优化问题和神经网络训练示例中验证了方法的有效性。

Conclusion: 该方法能够生成领域抽象的、人类可解释的解释，有助于理解逆问题的优化过程。
...
### [52] [STRCMP: Integrating Graph Structural Priors with Language Models for Combinatorial Optimization](https://arxiv.org/abs/2506.11057)
*Xijun Li,Jiexiang Yang,Jinghao Wang,Bo Peng,Jianguo Yao,Haibing Guan*

Main category: cs.LG

TLDR: 论文提出了一种名为STRCMP的结构感知LLM框架，通过结合图神经网络和大型语言模型，显著提升了组合优化问题的求解质量和效率。


<details>
  <summary>Details</summary>
Motivation: 组合优化问题因其NP难特性带来巨大计算挑战，现有方法常忽略结构先验，导致求解效果不佳。

Method: 结合GNN提取结构嵌入和LLM生成求解器特定代码，并通过进化过程迭代优化算法。

Result: 在多个基准数据集上，STRCMP显著优于其他神经和LLM方法，尤其在求解最优性和计算效率方面。

Conclusion: STRCMP通过结构感知和迭代优化，为组合优化问题提供了高效且高质量的解决方案。
...
### [53] [ADAMIX: Adaptive Mixed-Precision Delta-Compression with Quantization Error Optimization for Large Language Models](https://arxiv.org/abs/2506.11087)
*Boya Xiong,Shuo Wang,Weifeng Ge,Guanhua Chen,Yun Chen*

Main category: cs.LG

TLDR: ADAMIX是一种自适应混合精度delta压缩框架，通过数学推导优化量化误差，显著提升压缩性能。


<details>
  <summary>Details</summary>
Motivation: 在多租户服务场景中，现有delta压缩方法在高压缩比下性能不佳或依赖经验性位分配方案。

Method: 提出ADAMIX框架，通过数学推导量化误差并转化为0/1整数线性规划问题，优化位分配策略。

Result: 在AIME2024和GQA任务中，ADAMIX分别比最佳基线Delta-CoMe提升22.3%和6.1%。

Conclusion: ADAMIX在压缩性能和量化误差优化方面显著优于现有方法。
...
### [54] [Debiasing Online Preference Learning via Preference Feature Preservation](https://arxiv.org/abs/2506.11098)
*Dongyoung Kim,Jinsung Yoon,Jinwoo Shin,Jaehyung Kim*

Main category: cs.LG

TLDR: PFP框架通过保留人类偏好特征的分布，解决了大型语言模型偏好学习中的偏见问题，并在实验中表现优于现有方法。
...
Result: PFP成功减少在线学习中的特征偏见，在标准评测中优于现有偏好学习方法。

Conclusion: PFP通过保留偏好特征分布，有效提升模型对齐性能，解决了现有方法的局限性。
...
### [55] [Knowledge Graph Embeddings with Representing Relations as Annular Sectors](https://arxiv.org/abs/2506.11099)
*Huiling Zhu,Yingqi Zeng*

Main category: cs.LG

TLDR: SectorE是一种基于极坐标的新型知识图谱嵌入模型，通过将关系建模为环形扇区，有效捕捉语义层次结构，并在多个数据集上表现出色。
...
Conclusion: SectorE通过极坐标嵌入有效解决了语义层次建模问题，为知识图谱补全提供了新思路。
...
### [56] [An Active Learning-Based Streaming Pipeline for Reduced Data Training of Structure Finding Models in Neutron Diffractometry](https://arxiv.org/abs/2506.11100)
*Tianle Wang,Jorge Ramirez,Cristina Garcia-Cardona,Thomas Proffen,Shantenu Jha,Sudip K. Seal*

Main category: cs.LG

TLDR: 论文提出了一种新颖的批量主动学习策略，通过不确定性采样减少训练数据需求，显著提升结构测定效率。


<details>
  <summary>Details</summary>
Motivation: 中子衍射结构测定计算成本高，传统方法耗时且数据需求大，机器学习模型虽能加速但训练数据需求随参数增加呈指数增长。

Method: 引入批量主动学习策略，基于不确定性采样选择训练数据，设计高效的流式训练流程。

Result: 实验表明，该方法减少75%训练数据需求并提升准确性，流式训练流程缩短20%训练时间且不损失精度。

Conclusion: 提出的主动学习策略和流式训练流程显著优化了中子衍射结构测定的计算效率。
...
### [57] [PromptTSS: A Prompting-Based Approach for Interactive Multi-Granularity Time Series Segmentation](https://arxiv.org/abs/2506.11170)
*Ching Chang,Ming-Chih Lo,Wen-Chih Peng,Tien-Fu Chen*

Main category: cs.LG

TLDR: PromptTSS是一个新颖的时间序列分割框架，通过统一的模型和提示机制解决多粒度状态分割问题，显著提升了分割准确性和适应性。
...
Result: 实验显示，PromptTSS在多粒度分割、单粒度分割和迁移学习中分别提升了24.49%、17.88%和599.24%的准确性。

Conclusion: PromptTSS有效解决了多粒度时间序列分割问题，并展现出对动态环境的强大适应性。
...
### [58] [Collapsing Sequence-Level Data-Policy Coverage via Poisoning Attack in Offline Reinforcement Learning](https://arxiv.org/abs/2506.11172)
*Xue Zhou,Dapeng Man,Chen Xu,Fanyi Zeng,Tao Liu,Huan Wang,Shucheng He,Chaoyang Gao,Wu Yang*

Main category: cs.LG

TLDR: 论文提出了一种针对离线强化学习的数据覆盖攻击方法（CSDPC），通过毒化罕见决策模式显著降低性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究忽视数据覆盖不足的安全风险，且单步分析不适用于多步决策的离线强化学习。

Method: 引入序列级集中系数量化覆盖，提出CSDPC攻击，毒化罕见决策模式以减少覆盖。

Result: 实验显示毒化1%数据可导致性能下降90%。

Conclusion: 研究为离线强化学习的安全分析提供了新视角。
...
### [59] [Detection of obstructions in oil and gas pipelines: machine learning techniques for hydrate classification](https://arxiv.org/abs/2506.11220)
*Hellockston Gomes de Brito,Carla Wilza Souza de Paula Maitelli,Osvaldo Chiavone-Filho*

Main category: cs.LG

TLDR: 该论文使用监督机器学习方法（决策树、k-NN和朴素贝叶斯）检测和缓解油气生产中的流动保障问题，特别是水合物形成，决策树算法表现最佳（99.99%准确率）。


<details>
  <summary>Details</summary>
Motivation: 油气生产中的管道堵塞问题（如沉积物堆积、蜡沉积、矿物结垢和腐蚀）影响效率，需可靠方法检测和缓解。

Method: 采用数据预处理和清洗，利用scikit-learn库中的决策树、k-NN和朴素贝叶斯算法进行分类任务。

Result: 决策树算法在预测水合物形成时准确率最高（99.99%），方法有效。
...
### [60] [uPVC-Net: A Universal Premature Ventricular Contraction Detection Deep Learning Algorithm](https://arxiv.org/abs/2506.11238)
*Hagai Hamami,Yosef Solewicz,Daniel Zur,Yonatan Kleerekoper,Joachim A. Behar*

Main category: cs.LG

TLDR: uPVC-Net是一种深度学习模型，用于从单导联心电图中检测室性早搏（PVCs），在多个数据集上表现出色，AUC高达99.1%。
...
Method: 开发了uPVC-Net模型，采用自定义架构和多源多导联训练策略，在四个独立数据集上进行训练和评估。

Result: 模型在保留数据集上的AUC介于97.8%至99.1%之间，尤其在可穿戴设备单导联数据上表现最佳（AUC 99.1%）。
...
### [61] [A Causal Lens for Learning Long-term Fair Policies](https://arxiv.org/abs/2506.11242)
*Jacob Lear,Lu Zhang*

Main category: cs.LG

TLDR: 本文提出了一种在动态决策系统中研究长期公平性的框架，通过强化学习衡量不同群体的资格增益差异，并从因果角度分解为三个组件，最后提出了一种平衡多种公平性概念的方法。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注静态环境中的即时公平性，而忽视了动态决策系统中的长期公平性及其与即时公平性的关系。本文旨在填补这一空白。

Method: 提出了一种基于强化学习的框架，通过因果分解将长期公平性指标分为直接效应、延迟效应和虚假效应，并与新兴的“利益公平”概念关联。

Result: 分析了长期公平性组件的内在联系，并提出了一种简单有效的方法来平衡多种公平性概念。

Conclusion: 本文强调了长期公平性的重要性，并通过因果分解和平衡方法为动态决策系统中的公平性研究提供了新思路。
...
### [62] [Can Time-Series Foundation Models Perform Building Energy Management Tasks?](https://arxiv.org/abs/2506.11250)
*Ozan Baris Mulayim,Pengrui Quan,Liying Han,Xiaomin Ouyang,Dezhi Hong,Mario Bergés,Mani Srivastava*

Main category: cs.LG

TLDR: TSFMs在建筑能源管理中的泛化能力有限，表现仅略优于统计模型，且对协变量处理不佳，需进一步改进设计。


<details>
  <summary>Details</summary>
Motivation: 解决建筑能源管理中任务和数据特定模型的局限性，探索TSFMs的潜力。

Method: 评估TSFMs在四个维度：零样本单变量预测、协变量预测、零样本表示学习和鲁棒性。

Result: TSFMs泛化能力有限，协变量处理无效，分类任务表现尚可，但预测性能敏感且不稳定。

Conclusion: 需改进TSFMs设计，特别是协变量处理和上下文整合，以实现更适应和可扩展的解决方案。
...
### [63] [Domain-Constrained Diffusion Models to Synthesize Tabular Data: A Case Study in Power Systems](https://arxiv.org/abs/2506.11281)
*Milad Hoseinpour,Vladimir Dvorkin*

Main category: cs.LG

TLDR: 论文提出了一种基于引导扩散模型的合成数据方法，结合领域知识生成高保真数据，应用于电力系统等领域。


<details>
  <summary>Details</summary>
Motivation: 隐私、安全和法律障碍推动了合成数据的需求，尤其是在医疗、金融和能源领域。生成模型需要结合领域知识以提高实用性。

Method: 使用引导扩散模型，通过梯度引导将领域约束（如基尔霍夫定律）直接融入生成过程，生成统计代表性强的电力流数据。

Result: 数值结果表明该方法有效生成了满足领域约束的高保真数据。

Conclusion: 该方法为合成数据生成提供了一种有效解决方案，并具有扩展到其他表格数据领域的潜力。
...
### [64] [Sampling Imbalanced Data with Multi-objective Bilevel Optimization](https://arxiv.org/abs/2506.11315)
*Karen Medlin,Sven Leyffer,Krishnan Raghavan*

Main category: cs.LG

TLDR: 论文提出了一种多目标优化框架MOODS，用于解决类别不平衡问题，并通过新指标验证采样方法的效果。


<details>
  <summary>Details</summary>
Motivation: 类别不平衡问题导致少数类分类效果差，传统方法因忽略数据集多样性而效果有限。

Method: 提出MOODS框架，结合合成过采样和多数类欠采样，并引入ε/δ非重叠多样性指标。

Result: 实验表明，该方法在F1分数上提升1-15%，表现优于现有技术。

Conclusion: MOODS框架和多样性指标有效解决了类别不平衡问题，提升了分类性能。
...
### [65] [An Attention-based Spatio-Temporal Neural Operator for Evolving Physics](https://arxiv.org/abs/2506.11328)
*Vispi Karkaria,Doksoo Lee,Yi-Ping Chen,Yue Yu,Wei Chen*

Main category: cs.LG

TLDR: ASNO是一种基于注意力机制的时空神经算子，用于解决科学机器学习中未知物理过程的预测问题，具有适应性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习模型在捕捉长时空交互方面表现优异，但缺乏物理可解释性且难以适应变化的环境条件。

Method: 结合可分离的注意力机制处理时空交互，并利用BDF启发的方法学习时空预测和外部负载适应。

Result: ASNO在SciML基准测试中优于现有模型，展示了其在工程应用和物理发现中的潜力。

Conclusion: ASNO通过增强可解释性和适应性，为科学机器学习提供了新的解决方案。
...
### [66] [The Sample Complexity of Parameter-Free Stochastic Convex Optimization](https://arxiv.org/abs/2506.11336)
*Jared Lawrence,Ari Kalinsky,Hannah Bradfield,Yair Carmon,Oliver Hinder*

Main category: cs.LG

TLDR: 论文研究了在问题参数未知时随机凸优化的样本复杂度，提出了两种策略：可靠的模型选择方法和基于正则化的方法，以优化学习率并适应未知参数。实验验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 研究在问题参数（如最优距离）未知时，如何优化随机凸优化的样本复杂度，避免过拟合验证集。

Method: 1. 开发可靠的模型选择方法，避免验证集过拟合；2. 提出基于正则化的方法，专门适应未知最优距离的情况。

Result: 方法能匹配已知参数的最优样本复杂度，并实现完美适应未知最优距离。实验表明模型选择方法可减少小验证集过拟合。

Conclusion: 结合两种方法可同时适应多种问题结构，提升随机凸优化的性能。
...
### [67] [Improving Group Robustness on Spurious Correlation via Evidential Alignment](https://arxiv.org/abs/2506.11347)
*Wenqian Ye,Guangtao Zheng,Aidong Zhang*

Main category: cs.LG

TLDR: 提出了一种名为Evidential Alignment的新框架，利用不确定性量化来识别和抑制虚假相关性，无需依赖外部标注。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络常学习虚假相关性，影响泛化能力和鲁棒性。现有方法依赖昂贵的外部标注或确定性模型，难以全面捕捉偏差。

Method: 通过二阶风险最小化和证据校准技术，量化模型预测证据，识别并抑制虚假相关性。

Result: 实验表明，该方法显著提升了不同架构和数据模态下的群体鲁棒性。

Conclusion: Evidential Alignment为虚假相关性提供了一种可扩展且原理性的解决方案。
...
### [68] [Generalization Bound of Gradient Flow through Training Trajectory and Data-dependent Kernel](https://arxiv.org/abs/2506.11357)
*Yilan Chen,Zhichao Wang,Wei Huang,Andi Han,Taiji Suzuki,Arya Mazumdar*

Main category: cs.LG

TLDR: 本文通过损失路径核（LPK）为梯度流建立了与经典Rademacher复杂度对齐的泛化界，揭示了梯度流在优化轨迹中的动态特性对泛化性能的影响。
...
Result: LPK能捕捉训练轨迹的动态特性，提供更紧的泛化保证，并通过实验验证了其与实际泛化差距的相关性。

Conclusion: 本文不仅恢复了过参数化神经网络的核回归界，还展示了神经网络相对于核方法的特征学习能力。
...
### [69] [EDN: A Novel Edge-Dependent Noise Model for Graph Data](https://arxiv.org/abs/2506.11368)
*Pintu Kumar,Nandyala Hemachandra*

Main category: cs.LG

TLDR: 论文提出了一种新的图数据标签噪声模型EDN，考虑了节点间关系对标签噪声的影响，并通过实验验证了其重要性。


<details>
  <summary>Details</summary>
Motivation: 现有标签噪声模型（如SLN和CCN）忽略了图中节点间关系对标签噪声的影响，EDN模型填补了这一空白。

Method: 探索了EDN的三种变体，并分析了节点度对标签噪声概率的影响。实验使用了5种GNN架构和8种噪声鲁棒算法。

Result: 两种EDN变体导致GNN和噪声鲁棒算法的性能显著下降，验证了EDN的重要性。

Conclusion: 在评估图数据的噪声鲁棒算法时，应纳入EDN模型以提高可靠性。
...
### [70] [The Effect of Stochasticity in Score-Based Diffusion Sampling: a KL Divergence Analysis](https://arxiv.org/abs/2506.11378)
*Bernardo P. Schaeffer,Ricardo M. S. Rosa,Glauco Valle*

Main category: cs.LG

TLDR: 研究了基于分数的扩散模型中随机性对生成过程的影响，通过KL散度边界和数值实验分析了不同随机性参数下的误差传播。


<details>
  <summary>Details</summary>
Motivation: 探讨随机性在分数扩散模型中的作用，量化其对生成过程中误差的影响。

Method: 使用对数Sobolev不等式分析前向过程的边际分布，推导KL散度的理论边界，并通过数值和解析实验验证。

Result: 随机性在精确分数函数下可纠正误差，但在近似分数函数下存在误差校正与放大的权衡。

Conclusion: 随机性的选择需根据分数误差结构决定，理论结果通过实验验证。
...
### [71] [FIGNN: Feature-Specific Interpretability for Graph Neural Network Surrogate Models](https://arxiv.org/abs/2506.11398)
*Riddhiman Raut,Romit Maulik,Shivam Barwey*

Main category: cs.LG

TLDR: 提出了一种新型图神经网络FIGNN，通过特征特定池化策略和掩码正则化，提升科学应用中非结构化网格深度学习模型的解释性。
...
Method: 采用特征特定池化策略和掩码正则化，独立归因各预测变量的空间重要性。

Result: 在SPEEDY和BFS基准测试中，FIGNN预测性能优异，同时揭示了各特征的物理意义空间模式。
...
### [72] [LoRA Users Beware: A Few Spurious Tokens Can Manipulate Your Finetuned Model](https://arxiv.org/abs/2506.11402)
*Pradyut Sekhsaria,Marcel Mateos Salles,Hai Huang,Randall Balestriero*

Main category: cs.LG

TLDR: PEFT（如LoRA）在资源高效的情况下调整预训练大语言模型，但可能因依赖虚假标记而引发灾难性失败，如Seamless Spurious Token Injection（SSTI）。研究发现，少量虚假标记即可操控模型行为，且LoRA秩影响模型对虚假标记的依赖。


<details>
  <summary>Details</summary>
Motivation: 研究PEFT在高效调整预训练模型时可能引发的灾难性失败，尤其是虚假标记对模型行为的影响。

Method: 通过SSTI实验，在三个模型家族和四个数据集上测试虚假标记对模型决策的影响，分析LoRA秩的作用。

Result: 研究发现：1）单个虚假标记即可操控模型；2）LoRA秩与虚假标记依赖成正比；3）高LoRA秩在激进SSTI下提升鲁棒性。

Conclusion: PEFT可能因虚假标记引发风险，需在设计时考虑鲁棒性，LoRA秩的选择对模型行为有显著影响。
...
### [73] [PPDiff: Diffusing in Hybrid Sequence-Structure Space for Protein-Protein Complex Design](https://arxiv.org/abs/2506.11420)
*Zhenqiao Song,Tiaoxiao Li,Lei Li,Martin Renqiang Min*

Main category: cs.LG

TLDR: PPDiff是一种扩散模型，用于非自回归地设计任意蛋白质靶点的高亲和力结合蛋白的序列和结构，显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 设计高亲和力蛋白质结合蛋白对生物医学研究和生物技术至关重要，但目前缺乏无需大量实验即可针对任意靶点设计结合蛋白的方法。

Method: PPDiff基于SSINC网络，结合自注意力层、kNN等变图层和因果注意力层，预训练于PPBench数据集，并微调于靶蛋白小结合体和抗原-抗体复合物设计。
...
### [74] [TruncQuant: Truncation-Ready Quantization for DNNs with Flexible Weight Bit Precision](https://arxiv.org/abs/2506.11431)
*Jinhee Kim,Seoyeon Yoon,Taeho Lee,Joo Chan Lee,Kang Eun Jeon,Jong Hwan Ko*

Main category: cs.LG

TLDR: 论文提出了一种名为TruncQuant的新型截断训练方案，通过运行时位偏移实现灵活的位精度，解决了现有量化训练方案无法适应截断过程的问题。


<details>
  <summary>Details</summary>
Motivation: 由于最先进模型的复杂性增加，边缘设备上部署深度神经网络面临挑战，需要减少模型大小和推理延迟。现有量化训练方案无法适应截断过程，因此需要新的训练方案。

Method: 提出TruncQuant，通过运行时位偏移实现灵活的位精度，并与截断过程的输出对齐，确保在不同位宽设置下的鲁棒性。
...
### [75] [Dynamic Sparse Training of Diagonally Sparse Networks](https://arxiv.org/abs/2506.11449)
*Abhishek Tyagi,Arjun Iyer,William H Renninger,Christopher Kanan,Yuhao Zhu*

Main category: cs.LG

TLDR: DynaDiag是一种新型的结构化稀疏训练方法，通过强制对角线稀疏模式，在保持与无结构稀疏相同性能的同时，实现硬件友好的计算加速。


<details>
  <summary>Details</summary>
Motivation: 无结构稀疏在实际硬件上难以实现速度提升，因此需要一种既能保持性能又能加速计算的结构化稀疏方法。

Method: DynaDiag采用对角线稀疏模式，并通过定制CUDA内核加速计算，确保前向和反向传播的稀疏性。

Result: 在多种神经网络架构上，DynaDiag在保持与无结构稀疏相同精度的同时，实现了3.13倍的在线推理加速和1.59倍的训练加速。

Conclusion: DynaDiag为稀疏训练提供了一种高效且硬件友好的解决方案，显著提升了计算效率。
...
### [76] [RollingQ: Reviving the Cooperation Dynamics in Multimodal Transformer](https://arxiv.org/abs/2506.11465)
*Haotian Ni,Yake Wei,Hang Liu,Gong Chen,Chong Peng,Hao Lin,Di Hu*

Main category: cs.LG

TLDR: 论文提出RollingQ方法，通过旋转查询打破自强化循环，恢复多模态Transformer的动态适应性。


<details>
  <summary>Details</summary>
Motivation: 多模态学习中，动态融合策略（如注意力机制）因模态质量差异而失效，模型倾向于偏好单一模态，导致动态适应性下降。

Method: 提出RollingQ方法，通过旋转查询平衡注意力分配，打破自强化循环并缩小模态间的键分布差距。

Result: 在多种多模态场景下的实验验证了RollingQ的有效性，恢复了动态合作能力。

Conclusion: RollingQ能有效恢复多模态Transformer的动态适应性，提升其广泛能力。
...
### [77] [Position Paper: Rethinking AI/ML for Air Interface in Wireless Networks](https://arxiv.org/abs/2506.11466)
*Georgios Kontes,Diomidis S. Michalopoulos,Birendra Ghimire,Christopher Mutschler*

Main category: cs.LG

TLDR: AI/ML在无线网络中的应用仍处于早期阶段，本文概述了3GPP标准化中的相关讨论，并提出了未来的研究挑战与机遇。


<details>
  <summary>Details</summary>
Motivation: 探索AI/ML在无线网络（尤其是空中接口）中的应用潜力，填补现有研究空白。

Method: 综述3GPP标准化中的AI/ML相关讨论，包括用例、架构和技术需求。

Result: 总结了AI/ML在无线通信中的关键应用场景和标准化进展。

Conclusion: 需要跨学科合作以推动AI/ML在无线系统中的发展，并提出了未来的研究方向。
...
### [78] [LearnAlign: Reasoning Data Selection for Reinforcement Learning in Large Language Models Based on Improved Gradient Alignment](https://arxiv.org/abs/2506.11480)
*Shikun Li,Shipeng Li,Zhiqin Yang,Xinghua Zhang,Gaode Chen,Xiaobo Xia,Hengyu Liu,Zhe Peng*

Main category: cs.LG

TLDR: 论文提出了一种名为LearnAlign的梯度对齐方法，通过智能选择可学习和代表性的训练数据，显著减少强化学习后训练的数据需求，同时保持或提升性能。


<details>
  <summary>Details</summary>
Motivation: 强化学习在提升大语言模型推理能力方面数据效率低下，亟需解决这一问题。

Method: 提出基于梯度对齐的LearnAlign方法，利用数据可学习性（基于成功率）选择训练数据，克服梯度范数中的响应长度偏差问题。
...
### [79] [Diabetes Prediction and Management Using Machine Learning Approaches](https://arxiv.org/abs/2506.11501)
*Mowafaq Salem Alzboon,Muhyeeddin Alqaraleh,Mohammad Subhi Al-Batah*

Main category: cs.LG

TLDR: 研究评估了多种机器学习算法在糖尿病风险预测中的表现，发现神经网络和随机森林算法准确率最高，可作为早期筛查工具。


<details>
  <summary>Details</summary>
Motivation: 糖尿病全球发病率上升，需早期检测和管理，机器学习算法在预测糖尿病风险方面潜力巨大。

Method: 使用Pima Indians Diabetes Database的768个样本，评估包括逻辑回归、决策树、随机森林、K近邻、朴素贝叶斯、支持向量机、梯度提升和神经网络在内的多种算法。

Result: 神经网络算法预测准确率最高（78.57%），随机森林次之（76.30%）。

Conclusion: 机器学习算法可作为糖尿病早期筛查工具，有助于及时干预，减轻医疗系统负担。
...
### [80] [Machine Learning-Based Quantification of Vesicoureteral Reflux with Enhancing Accuracy and Efficiency](https://arxiv.org/abs/2506.11508)
...
TLDR: 本研究探讨了机器学习在分析VCUG图像中的应用，以提高VUR诊断的一致性。通过分析113张VCUG图像，训练了六种预测模型，结果显示所有模型均能准确分类，且无假阳性或假阴性。研究发现肾盏变形是高级别VUR的关键指标。


<details>
  <summary>Details</summary>
Motivation: 传统VUR评估依赖主观分级系统，导致诊断变异性较大。本研究旨在通过机器学习提供一种客观、标准化的替代方法。

Method: 研究选取了113张VCUG图像，提取了9个图像特征，并训练了六种机器学习模型（如逻辑回归、决策树等），采用留一法交叉验证进行评估。

Result: 所有模型均表现出高准确性，无假阳性或假阴性。肾盏变形被确认为高级别VUR的关键指标，AUC值证实了模型对细微图像特征的高敏感性。

Conclusion: 机器学习可为VUR评估提供客观标准，肾盏变形是严重病例的强预测因子。未来需扩大数据集并优化模型以提高临床适用性。
...
### [81] [Task-Driven Discrete Representation Learning](https://arxiv.org/abs/2506.11511)
*Tung-Long Vuong*

Main category: cs.LG

TLDR: 该论文提出了一种任务驱动的离散表示学习框架，探讨了离散特征在下游任务中的实用性，并分析了表示能力与样本复杂度之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 当前离散表示学习（DRL）主要关注生成任务，而离散表示的质量定义模糊，缺乏任务驱动的视角。
...
### [82] [Prioritizing Alignment Paradigms over Task-Specific Model Customization in Time-Series LLMs](https://arxiv.org/abs/2506.11512)
*Wei Li,Yunyao Cheng,Xinli Hao,Chaohong Ma,Yuxuan Liang,Bin Yang,Christian S. Jensen,Xiaofeng Meng*

Main category: cs.LG

TLDR: 该论文主张在时间序列推理中，应优先考虑基于时间序列数据内在特性的对齐范式，而非任务特定的模型定制，以解决现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常专注于任务特定的模型定制，而忽视了时间序列数据的内在特性（时间序列基元），导致推理成本高、灵活性差和效率低。

Method: 提出三种对齐范式：Injective Alignment、Bridging Alignment和Internal Alignment，分别针对时间序列基元的不同方面（领域、特征和表示）进行优化。

Result: 通过系统性地考虑数据内在结构，这些对齐范式能够激活LLMs的时间序列推理能力，实现经济、灵活和高效的推理。

Conclusion: 建议实践者采用对齐导向的方法，并提供了相关文献的分类和未来研究方向。
...
TLDR: 论文提出了一种新理论视角，将上下文学习（ICL）解释为隐式知识蒸馏（KD），并推导了泛化边界，解释了ICL的机制。
...
Method: 将ICL视为隐式知识蒸馏，推导基于Rademacher复杂度的泛化边界，证明蒸馏权重的偏差与提示和目标分布的MMD线性增长。

Result: 理论框架解释了多个实证现象，统一了基于梯度和分布分析的研究。
...
### [84] [Delayformer: spatiotemporal transformation for predicting high-dimensional dynamics](https://arxiv.org/abs/2506.11528)
*Zijian Wang,Peng Tao,Luonan Chen*

Main category: cs.LG

TLDR: Delayformer框架通过多变量时空信息转换和Transformer技术，解决了高维系统中非线性与交互问题，显著提升了时间序列预测性能。


<details>
  <summary>Details</summary>
Motivation: 在有限且噪声数据下，高维系统中变量的非线性与复杂交互使得准确预测成为挑战，现有方法表现不佳。

Method: 提出Delayformer框架，利用延迟嵌入状态和ViT编码器跨变量学习，结合线性解码器预测系统状态。

Result: 在合成和真实数据集上，Delayformer优于现有方法，并展示了跨领域应用的潜力。

Conclusion: Delayformer通过理论与Transformer结合，成为时间序列预测的基础模型，具有广泛适用性。
...
### [85] [Robust Filtering -- Novel Statistical Learning and Inference Algorithms with Applications](https://arxiv.org/abs/2506.11530)
*Aamir Hussain Chughtai*

Main category: cs.LG

TLDR: 该论文提出了一种新型鲁棒非线性滤波方法，用于处理现实场景中的异常噪声问题，并扩展至离线估计和学习设置，通过贝叶斯推理框架和理论界限验证了性能提升。


<details>
  <summary>Details</summary>
Motivation: 现实场景中的噪声统计信息通常未知或部分已知，传统滤波方法在处理异常（如离群值、偏差、漂移和缺失观测）时表现受限，因此需要更鲁棒的解决方案。

Method: 基于贝叶斯推理框架，结合变分推断（VI）和粒子滤波/序贯蒙特卡洛（SMC）等确定性及随机近似技术，提出鲁棒非线性滤波方法，并研究其理论界限（如贝叶斯克拉美罗界，BCRB）。

Result: 通过目标跟踪、室内定位、3D点云配准等实验验证了所提方法的性能提升。

Conclusion: 该研究为处理异常噪声提供了通用解决方案，未来可扩展至鲁棒机器学习管道、异常数据学习系统动态及生成式AI中的挑战。
...
### [86] [Improving Multimodal Learning Balance and Sufficiency through Data Remixing](https://arxiv.org/abs/2506.11550)
*Xiaoyu Ma,Hao Chen,Yongjian Deng*

Main category: cs.LG

TLDR: 论文提出了一种多模态数据重混合方法，通过解耦数据、筛选难样本和批量重组，解决多模态学习中的不平衡和不足问题。


<details>
  <summary>Details</summary>
Motivation: 多模态学习中存在模态懒惰和模态冲突问题，导致学习不充分和不平衡，现有方法无法同时解决这两个问题。

Method: 提出多模态数据重混合方法，包括数据解耦、难样本筛选和批量级重组，以对齐梯度方向并避免跨模态干扰。

Result: 实验表明，该方法可无缝集成现有方法，在CREMAD和Kinetic-Sounds数据集上分别提升6.50%和3.41%的准确率。

Conclusion: 多模态数据重混合方法有效解决了模态不平衡和学习不足问题，且无需额外计算开销。
...
### [87] [Learn to Preserve Personality: Federated Foundation Models in Recommendations](https://arxiv.org/abs/2506.11563)
*Zhiwei Li,Guodong Long,Chunxu Zhang,Honglei Zhang,Jing Jiang,Chengqi Zhang*

Main category: cs.LG

TLDR: 本文提出了一种基于联邦基础模型（FFM）的新学习范式，旨在平衡模型的泛化能力与个性化需求，特别适用于推荐系统。
...
Method: 利用联邦基础模型（FFM）分离共享知识与个性化适配，并在推荐系统中验证其有效性。

Result: 提出了一种用户中心化的去中心化架构，未来可支持个性化代理。

Conclusion: FFM在推荐系统中展现出潜力，能同时实现泛化与个性化，并为用户提供对个性化代理的控制权。
...
### [88] [A Comparative Analysis of Influence Signals for Data Debugging](https://arxiv.org/abs/2506.11584)
*Nikolaos Myrtakis,Ioannis Tsamardinos,Vassilis Christophides*

Main category: cs.LG

TLDR: 本文通过比较基于影响力的信号来调试训练数据，评估了它们在检测错误标记和异常样本中的效果，发现现有信号无法检测异常，并提出了一些局限性。


<details>
  <summary>Details</summary>
Motivation: 提高训练样本质量对提升ML模型的可靠性和性能至关重要，但目前缺乏对基于影响力信号在检测不同类型问题样本（如错误标记和异常）中的系统评估。

Method: 使用共同的影响力估计器（如TraceIn），对不同数据模态（图像和表格）和深度学习模型（从头训练或基础模型）进行实验，评估多种影响力信号（如Self-Influence、Average Absolute Influence等）的效果。

Result: 实验表明，Self-Influence能有效检测错误标记样本，但现有信号均无法检测异常样本，且存在训练动态未考虑或影响力抵消效应等问题。

Conclusion: 现有影响力信号在检测异常样本方面存在不足，未来研究需考虑训练动态并解决影响力抵消问题。
...
TLDR: 论文提出了一种无需训练、模型无关的防御框架KCES，通过图核复杂度（GKC）评估边的重要性并修剪高KC分数的边，提升GNN的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: GNN对微小扰动和对抗攻击高度脆弱，现有防御方法依赖启发式指标、过拟合特定攻击模式且计算复杂。

Method: 提出KCES框架，利用GKC定义边的KC分数，修剪高分数边以减轻对抗扰动的影响。

Result: KCES显著提升GNN鲁棒性，优于现有方法，并能与现有防御策略无缝集成。

Conclusion: KCES为GNN安全提供了一种高效、理论支持的解决方案。
...
### [90] [Model Organisms for Emergent Misalignment](https://arxiv.org/abs/2506.11613)
*Edward Turner,Anna Soligo,Mia Taylor,Senthooran Rajamanoharan,Neel Nanda*

Main category: cs.LG

TLDR: 研究发现，在窄范围有害数据集上微调大语言模型可能导致广泛的对齐失效（Emergent Misalignment，EM）。通过新数据集和改进方法，研究实现了更高的模型一致性和更小的模型规模，揭示了对齐问题的机制。


<details>
  <summary>Details</summary>
Motivation: 揭示大语言模型在微调过程中可能出现的对齐失效现象，填补对齐理解的空白。

Method: 使用新的窄范围对齐失效数据集，改进模型生物体，实现更高一致性和更小规模，并研究对齐失效的机制。

Result: 实现了99%的模型一致性（之前为67%），并在0.5B参数模型上验证了对齐失效现象，揭示了机制性相变。

Conclusion: 对齐失效现象暴露了大语言模型对齐的挑战，为未来研究和风险缓解提供了基础。
...
### [91] [Machine Unlearning for Robust DNNs: Attribution-Guided Partitioning and Neuron Pruning in Noisy Environments](https://arxiv.org/abs/2506.11615)
*Deliang Jin,Gang Chen,Shuo Feng,Yufeng Ling,Haoran Zhu*

Main category: cs.LG

TLDR: 提出了一种基于机器遗忘原则的新框架，通过数据分区、神经元剪枝和微调来减少噪声数据的影响，显著提升了模型在噪声环境中的性能。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络在噪声或损坏的训练数据下性能会严重下降，传统方法依赖噪声分布假设或需要大量重训练，不适用于大规模模型。

Method: 结合梯度归因的数据分区、回归敏感性分析的神经元剪枝以及高质量数据子集的微调。

Result: 在CIFAR-10和语音识别任务中，准确率提升约10%，重训练时间减少达47%。

Conclusion: 该框架在噪声环境中表现出高效性和可扩展性，无需全模型重训练或显式噪声建模。
...
### [92] [Convergent Linear Representations of Emergent Misalignment](https://arxiv.org/abs/2506.11618)
*Anna Soligo,Edward Turner,Senthooran Rajamanoharan,Neel Nanda*

Main category: cs.LG

TLDR: 论文研究了大型语言模型在窄数据集微调时出现的广泛行为不匹配现象（涌现性不匹配），并通过最小模型实验揭示了其机制和表征。


<details>
  <summary>Details</summary>
Motivation: 探讨大型语言模型在窄数据集微调时为何会出现广泛行为不匹配，填补模型对齐知识的空白。

Method: 使用仅含9个rank-1适配器的最小模型微调Qwen2.5-14B-Instruct，提取“不匹配方向”并验证其普适性。

Result: 发现不同涌现性不匹配模型会收敛到相似的表征，并成功通过提取的方向消除不匹配行为。

Conclusion: 通过理解涌现性不匹配的机制，为更广泛地理解和缓解模型不匹配提供了基础。
...
### [93] [Physically-informed change-point kernels for structural dynamics](https://arxiv.org/abs/2506.11625)
*Daniel James Pitchforth,Matthew Rhys Jones,Samuel John Gibson,Elizabeth Jane Cross*

Main category: cs.LG

TLDR: 论文提出了一种动态调整物理知识与数据依赖的Gaussian过程核方法，以优化模型设计。


<details>
  <summary>Details</summary>
Motivation: 平衡物理知识与数据依赖是模型设计的关键，过度依赖物理知识或数据均可能导致模型性能下降。

Method: 开发了新型的物理信息变化点核，支持动态调整对物理知识的依赖，并提供用户控制和自动学习功能。

Result: 通过桥梁风载和飞机机翼应变两个工程案例验证了新核结构的有效性。

Conclusion: 该方法在动态调整物理与数据依赖方面表现出色，提升了模型的不确定性捕捉能力。
...
### [94] [Geometry-Aware Edge Pooling for Graph Neural Networks](https://arxiv.org/abs/2506.11700)
*Katharina Limbeck,Lydia Mezrag,Guy Wolf,Bastian Rieck*

Main category: cs.LG

TLDR: 提出了一种基于边折叠的结构感知图池化层，通过扩散几何和多样性度量保留图的度量结构和结构多样性，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有图池化层在优化学习任务时牺牲了图结构和可解释性，导致性能不稳定。

Method: 利用扩散几何和多样性度量（如magnitude和spread）迭代缩小图尺寸，保留关键结构。

Result: 在多种图分类任务中表现优异，保留输入图的谱特性，并在不同池化比例下保持高精度。

Conclusion: 新方法在性能和结构保留方面优于现有池化层，适用于多样化任务。
...
### [95] [Growing with Experience: Growing Neural Networks in Deep Reinforcement Learning](https://arxiv.org/abs/2506.11706)
*Lukas Fehring,Marius Lindauer,Theresa Eimer*

Main category: cs.LG

TLDR: GrowNN是一种通过渐进式网络增长提升强化学习性能的方法，实验显示其优于静态网络。
...
Method: 提出GrowNN方法，从小网络开始训练，逐步增加层数而不改变已有功能，以提升策略表达能力。

Result: 在MiniHack和Mujoco实验中，GrowNN性能优于静态网络，分别提升48%和72%。
...
### [96] [Taxonomy of reduction matrices for Graph Coarsening](https://arxiv.org/abs/2506.11743)
*Antonin Joly,Nicolas Keriven,Aline Roumy*

Main category: cs.LG

TLDR: 论文提出了一种更通用的图粗化方法，通过放宽降维矩阵与提升矩阵的固定关系，进一步减少信息损失（RSA）。


<details>
  <summary>Details</summary>
Motivation: 传统图粗化方法中降维矩阵与提升矩阵通常互为伪逆，限制了灵活性。研究发现提升矩阵的约束更重要，因此探索更通用的降维矩阵。

Method: 引入不依赖提升矩阵伪逆的降维矩阵，建立其分类与性质，并通过优化RSA改进性能。

Result: 实验表明，固定提升矩阵时，调整降维矩阵可进一步减少RSA，提升图神经网络在节点分类任务中的表现。

Conclusion: 放宽降维矩阵与提升矩阵的固定关系，能更灵活地优化图粗化效果，提升下游任务性能。
...
### [97] [SSPINNpose: A Self-Supervised PINN for Inertial Pose and Dynamics Estimation](https://arxiv.org/abs/2506.11786)
*Markus Gambietz,Eva Dorschky,Altan Akat,Marcel Schöckel,Jörg Miehling,Anne D. Koelewijn*

Main category: cs.LG

TLDR: SSPINNpose是一种自监督、物理信息神经网络，直接从IMU数据估计关节运动学和动力学，无需地面真实标签训练，适用于实时生物力学分析。
...
Result: SSPINNpose在步行和跑步中准确估计关节角度和力矩（RMSD分别为8.7度和4.9%BWBH），延迟3.5毫秒，适应稀疏传感器配置。
...
### [98] [Why Do Class-Dependent Evaluation Effects Occur with Time Series Feature Attributions? A Synthetic Data Investigation](https://arxiv.org/abs/2506.11790)
*Gregor Baer,Isel Grau,Chao Zhang,Pieter Van Gorp*

Main category: cs.LG

TLDR: 研究发现，基于扰动的评估方法在不同类别中表现不一致，可能与真实特征定位不符，需谨慎使用。


<details>
  <summary>Details</summary>
Motivation: 探讨特征归因方法评估的可靠性，尤其是在缺乏真实数据时扰动分析的局限性。

Method: 使用合成时间序列数据，系统变化特征类型和类别对比，比较扰动和真实指标。

Result: 类依赖效应普遍存在，扰动和真实指标评估常矛盾，相关性弱。

Conclusion: 需开发更全面的评估框架，谨慎解释扰动指标。
...
### [99] [TrustGLM: Evaluating the Robustness of GraphLLMs Against Prompt, Text, and Structure Attacks](https://arxiv.org/abs/2506.11844)
*Qihai Zhang,Xinyue Sheng,Yuanfu Sun,Qiaoyu Tan*

Main category: cs.LG

TLDR: 论文研究了基于大语言模型（LLM）的图学习框架（GraphLLMs）的鲁棒性，提出了TrustGLM评估其对抗攻击的脆弱性，并探索了防御方法。


<details>
  <summary>Details</summary>
Motivation: 尽管GraphLLMs在推理能力上表现出色，但其对抗扰动的鲁棒性尚未充分研究，这对高风险场景的应用至关重要。

Method: 通过文本、图结构和提示操纵三个维度评估GraphLLMs的脆弱性，并采用数据增强和对抗训练等防御技术。

Result: 实验表明，GraphLLMs对文本攻击高度敏感，图结构攻击和提示随机化也会显著降低性能。防御方法显示出提升鲁棒性的潜力。

Conclusion: 论文揭示了GraphLLMs的脆弱性，提出了防御方案，并开源了评估工具以促进进一步研究。
...
### [100] [In Defense of Defensive Forecasting](https://arxiv.org/abs/2506.11848)
*Juan Carlos Perdomo,Benjamin Recht*

Main category: cs.LG

TLDR: 本文概述了防御性预测算法，通过纠正过去错误而非预测未来来生成预测，并介绍了相关理论和简单算法。


<details>
  <summary>Details</summary>
Motivation: 防御性预测的目标是通过序列化游戏框架，最小化预测误差，无论实际结果如何。

Method: 介绍了防御性预测的基本理论，并推导了在线学习、校准、专家建议预测和在线共形预测的简单算法。

Result: 提出了近最优的算法，适用于多种预测场景。

Conclusion: 防御性预测提供了一种有效的预测框架，适用于广泛的在线学习问题。
...
### [101] [Regression-adjusted Monte Carlo Estimators for Shapley Values and Probabilistic Values](https://arxiv.org/abs/2506.11849)
*R. Teal Witter,Yurong Liu,Christopher Musco*

Main category: cs.LG

TLDR: 提出了一种结合蒙特卡洛采样和线性回归的新方法，用于高效计算概率值（如Shapley值），在多个数据集上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 概率值在可解释AI中应用广泛，但精确计算耗时，现有方法（蒙特卡洛采样和线性回归）各有局限，需更灵活高效的解决方案。

Method: 结合蒙特卡洛采样和线性回归，允许用任何高效计算概率值的函数族替代线性回归，利用树模型（如XGBoost）提高准确性。

Result: 在8个数据集上，新方法对Shapley值的误差比现有方法低6.5倍（Permutation SHAP）、3.8倍（Kernel SHAP）和2.6倍（Leverage SHAP）；对其他概率值的误差比现有最优方法低215倍。

Conclusion: 新方法灵活高效，显著提升了概率值估计的准确性，为可解释AI提供了更优工具。
...
### [102] [Robust Molecular Property Prediction via Densifying Scarce Labeled Data](https://arxiv.org/abs/2506.11877)
*Jina Kim,Jeffrey Willette,Bruno Andreis,Sung Ju Hwang*

Main category: cs.LG

TLDR: 论文提出了一种基于元学习的新方法，利用未标记数据来弥合训练数据分布内外的差异，显著提升了模型在存在协变量偏移时的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 分子预测模型依赖训练数据中的结构，导致对分布外化合物的泛化能力差，而药物发现中关键化合物常超出训练集范围，实验验证数据稀缺加剧了这一问题。

Method: 采用元学习方法，利用未标记数据在分布内和分布外数据之间进行插值，使模型能够学习如何超越训练分布进行泛化。

Result: 在存在显著协变量偏移的真实数据集上，该方法显著优于现有最先进方法。
...
### [103] [An Explainable AI Framework for Dynamic Resource Management in Vehicular Network Slicing](https://arxiv.org/abs/2506.11882)
*Haochen Sun,Yifan Liu,Ahmed Al-Tahmeesschi,Swarna Chetty,Syed Ali Raza Zaidi,Avishek Nag,Hamed Ahmadi*

Main category: cs.LG

TLDR: 本文提出了一种基于可解释深度强化学习（XRL）的动态网络切片和资源分配框架，用于车载网络，通过Shapley值和注意力机制提高决策的透明度和可靠性。


<details>
  <summary>Details</summary>
Motivation: 车载网络需要满足多样化的服务需求（如eMBB和URLLC），但资源管理和网络切片的可靠性是关键挑战。

Method: 采用XRL框架，结合Shapley值和注意力机制，动态分配资源并解释决策过程。

Result: 仿真显示，该方法提高了资源分配的透明度和精确性，URLLC的QoS满意度从78.0%提升至80.13%，eMBB从71.44%提升至73.21%。

Conclusion: XRL框架显著提升了车载网络的资源分配效率和可靠性，同时增强了决策的可解释性。
...
### [104] [Understanding Input Selectivity in Mamba: Impact on Approximation Power, Memorization, and Associative Recall Capacity](https://arxiv.org/abs/2506.11891)
*Ningyuan Huang,Miguel Sarabia,Abhinav Moudgil,Pau Rodriguez,Luca Zappella,Federico Danieli*

Main category: cs.LG

TLDR: 本文解析了Mamba中输入选择性的作用，探讨了其对函数逼近、长期记忆和关联回忆能力的影响，并提出了理论构造的紧密度验证。


<details>
  <summary>Details</summary>
Motivation: Mamba作为SSM的改进版本，引入了输入选择性和其他操作，但其具体作用机制尚不明确。本文旨在揭示输入选择性的作用及其与其他操作的交互。

Method: 通过理论分析和实验验证，研究了S6层的功能，包括Haar小波投影、动态抵消记忆衰减以及MQAR任务的解析解。

Result: 发现S6层能更好地逼近不连续函数，动态管理记忆衰减，并在关联回忆任务中表现优异。

Conclusion: 研究提供了对Mamba机制的理解，并指出了改进方向。
...
### [105] [Attention-based Adversarial Robust Distillation in Radio Signal Classifications for Low-Power IoT Devices](https://arxiv.org/abs/2506.11892)
*Lu Zhang,Sangarapillai Lambotharan,Gan Zheng,Guisheng Liao,Basil AsSadhan,Fabio Roli*

Main category: cs.LG

TLDR: 论文提出了一种针对基于Transformer的调制分类中的对抗样本的防御系统，并设计了一种紧凑型Transformer以增强鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 由于Transformer在自动调制分类中的成功应用及其对对抗样本的脆弱性，研究旨在提出一种高效且鲁棒的防御方法，特别适用于资源受限的IoT设备。

Method: 通过将鲁棒训练的大型Transformer的对抗注意力图转移到紧凑型Transformer中，提出了一种新颖的紧凑型Transformer架构。

Result: 新方法在白盒攻击场景（如快速梯度法和投影梯度下降攻击）中优于现有技术，并研究了对抗样本在不同架构间的可迁移性。

Conclusion: 该方法能有效提升紧凑型Transformer的鲁棒性，并可能防止对抗样本的迁移攻击。
...
### [106] [Measurement-aligned Flow for Inverse Problem](https://arxiv.org/abs/2506.11893)
*Shaorong Zhang,Rob Brekelmans,Yunshu Wu,Greg Ver Steeg*

Main category: cs.LG

TLDR: 论文提出了一种名为MAS的新框架，用于解决线性逆问题，能够更灵活地平衡先验信息和测量信息。


<details>
  <summary>Details</summary>
Motivation: 现有方法在结合先验信息和测量信号时存在困难，尤其是在非高斯或未知噪声的情况下。

Method: 提出了Measurement-Aligned Sampling (MAS)框架，统一并扩展了现有方法（如DDNM和DAPS），并提供新的优化视角。

Result: 实验表明，MAS在多种任务中均优于现有最先进方法。

Conclusion: MAS能够更灵活地处理已知高斯噪声、未知或非高斯噪声类型，为逆问题解决提供了更强大的工具。
...
### [107] [Scalable Generalized Bayesian Online Neural Network Training for Sequential Decision Making](https://arxiv.org/abs/2506.11898)
*Gerardo Duran-Martin,Leandro Sánchez-Betancourt,Álvaro Cartea,Kevin Murphy*

Main category: cs.LG

TLDR: 论文提出了可扩展的在线学习和广义贝叶斯推理算法，用于神经网络参数的序列决策任务，结合了频率派和贝叶斯滤波的优势。


<details>
  <summary>Details</summary>
Motivation: 解决序列决策任务中神经网络参数的在线学习和贝叶斯推理问题，结合频率派和贝叶斯方法的优势以提高效率和准确性。

Method: 采用块对角近似参数误差协方差的快速低秩更新方法，同时更新隐藏层的低秩误差协方差和最终层的全秩误差协方差。

Result: 在非平稳上下文赌博机和贝叶斯优化问题上，方法在速度和准确性之间取得了竞争性的平衡。

Conclusion: 提出的方法无需重放缓冲区或离线重新训练，能够在线更新所有网络参数，且后验预测分布定义良好。
...
### [108] [A Neural Rejection System Against Universal Adversarial Perturbations in Radio Signal Classification](https://arxiv.org/abs/2506.11901)
*Lu Zhang,Sangarapillai Lambotharan,Gan Zheng,Fabio Roli*

Main category: cs.LG

TLDR: 本文提出了一种神经拒绝系统，用于防御通用对抗扰动对基于深度学习的无线电信号分类的影响，并证明其有效性。
...
Result: 神经拒绝系统在防御通用对抗扰动时，准确率显著高于未防御的深度神经网络。
...
### [109] [TreeRL: LLM Reinforcement Learning with On-Policy Tree Search](https://arxiv.org/abs/2506.11902)
*Zhenyu Hou,Ziniu Hu,Yujiang Li,Rui Lu,Jie Tang,Yuxiao Dong*

Main category: cs.LG

TLDR: TreeRL是一种结合树搜索的强化学习框架，用于LLM训练，通过中间监督和高效树搜索提升性能。


<details>
  <summary>Details</summary>
Motivation: 传统独立链采样策略在RL训练中存在探索不足和奖励模型问题，TreeRL旨在解决这些问题。

Method: TreeRL直接整合树搜索，引入中间监督，无需单独训练奖励模型，并采用高效分支策略。

Result: 在数学和代码推理任务中，TreeRL表现优于传统ChainRL。

Conclusion: TreeRL展示了树搜索在LLM强化学习中的潜力，并已开源。
...
### [110] [Spectra-to-Structure and Structure-to-Spectra Inference Across the Periodic Table](https://arxiv.org/abs/2506.11908)
*Yufeng Wang,Peiyao Wang,Lu Ma,Yuewei Lin,Qun Liu,Haibin Ling*

Main category: cs.LG

TLDR: XAStruct是一个机器学习框架，用于从晶体结构预测XAS光谱，并从XAS输入推断局部结构描述符，支持多种元素和化学环境。


<details>
  <summary>Details</summary>
Motivation: XAS解释需要专家分析、计算成本高的模拟和元素特定启发式方法，限制了其应用。机器学习可以加速XAS解释，但现有模型通常局限于特定元素或光谱范围。

Method: XAStruct基于大规模数据集训练，涵盖70多种元素，结合深度神经网络和高效基线模型，独立训练光谱预测和结构推断任务。

Result: XAStruct能够预测XAS光谱和推断局部结构描述符，无需元素特定调优，并在多元素和化学环境中表现良好。

Conclusion: XAStruct为数据驱动的XAS分析和局部结构推断提供了可扩展的解决方案，但端到端模型整合导致性能下降，因此任务需独立训练。
...
### [111] [Breaking Habits: On the Role of the Advantage Function in Learning Causal State Representations](https://arxiv.org/abs/2506.11912)
*Miguel Suau*

Main category: cs.LG

TLDR: 优势函数在策略梯度方法中不仅降低梯度估计方差，还能缓解策略混淆问题，提升泛化性能。


<details>
  <summary>Details</summary>
Motivation: 强化学习代理可能因策略混淆问题而依赖虚假相关性，影响泛化能力。

Method: 通过优势函数调整动作值，削弱当前策略下高概率的状态-动作对，打破虚假相关性。

Result: 理论和实验证明，使用优势函数能提升轨迹外性能。
...
### [112] [Visual Pre-Training on Unlabeled Images using Reinforcement Learning](https://arxiv.org/abs/2506.11967)
*Dibya Ghosh,Sergey Levine*

Main category: cs.LG

TLDR: 该论文将自监督图像预训练类比为强化学习问题，通过训练通用价值函数来改进特征学习。


<details>
  <summary>Details</summary>
Motivation: 观察到自监督图像预训练方法与强化学习中的价值学习有相似性，希望通过强化学习框架改进特征学习。

Method: 将图像预训练视为强化学习问题，训练通用价值函数，通过改变视图或添加图像增强来学习特征。

Result: 在未标记图像数据（如视频帧、网络爬取数据）上训练时，表现出改进的特征表示。

Conclusion: 通过强化学习框架，可以更灵活地利用未标记数据改进自监督图像预训练的效果。
...
### [113] [Self-Regulating Cars: Automating Traffic Control in Free Flow Road Networks](https://arxiv.org/abs/2506.11973)
*Ankit Bhardwaj,Rohail Asim,Sachin Chauhan,Yasir Zaki,Lakshminarayanan Subramanian*

Main category: cs.LG

TLDR: 提出一种基于强化学习的自调节车辆协议，动态调节车速以优化交通流量，无需新增基础设施。


<details>
  <summary>Details</summary>
Motivation: 解决郊区高速公路等自由流道路因通勤流量增加和基础设施有限导致的交通拥堵问题。

Method: 结合经典交通流理论、间隙接受模型和微观仿真，构建物理信息强化学习框架，将道路抽象为超级路段。

Result: 在PTV Vissim模拟器中，总吞吐量提升5%，平均延迟降低13%，停车次数减少3%。
...
### [114] [Compression Aware Certified Training](https://arxiv.org/abs/2506.11992)
*Changming Xu,Gagandeep Singh*

Main category: cs.LG

TLDR: CACTUS框架统一了深度神经网络的压缩与认证鲁棒性训练，在保持高精度和鲁棒性的同时实现高效压缩。
...
### [115] [pLSTM: parallelizable Linear Source Transition Mark networks](https://arxiv.org/abs/2506.11997)
*Korbinian Pöppel,Richard Freinschlag,Thomas Schmied,Wei Lin,Sepp Hochreiter*

Main category: cs.LG

TLDR: 论文提出了一种并行化的线性源转换标记网络（pLSTM），适用于处理多维数据（如DAGs），解决了传统RNN在长距离依赖中的梯度问题，并在分子图和计算机视觉任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现代循环架构（如xLSTM和Mamba）在语言建模中挑战了Transformer，但其结构限制了其在多维数据上的应用。MDRNNs虽适合多维数据，但缺乏并行化能力。本文旨在扩展多维性到线性RNNs，提出并行化的pLSTM。

Method: 引入pLSTM，利用源、转换和标记门作用于DAG的线图，实现并行化。针对规则网格（如1D和2D），使用einsum操作、拼接和填充高效实现。通过P模式和D模式解决长距离梯度问题。

Result: pLSTM在合成计算机视觉任务和分子图基准测试中表现优异，能泛化到更大图像尺寸，而Transformer难以外推。

Conclusion: pLSTM为处理多维数据提供了一种高效并行化的解决方案，解决了长距离依赖问题，并在多个任务中优于Transformer。
...
### [116] [An Efficient Compression of Deep Neural Network Checkpoints Based on Prediction and Context Modeling](https://arxiv.org/abs/2506.12000)
*Yuriy Kim,Evgeny Belyaev*

Main category: cs.LG

TLDR: 论文提出了一种基于预测的压缩方法，结合剪枝和量化技术，有效减少神经网络训练过程中检查点的存储需求，同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 针对存储受限环境中神经网络训练检查点的高存储需求问题，提出高效压缩方法以减少存储占用。

Method: 采用基于预测的压缩方法，利用先前保存的检查点进行上下文建模，并结合剪枝和量化技术优化压缩性能。

Result: 实验结果显示，该方法显著减少了检查点的比特大小，同时支持近乎无损的训练恢复，模型性能得以保持。

Conclusion: 该方法适用于存储受限环境，实现了高效的检查点压缩与恢复。
...
### [117] [SIMSHIFT: A Benchmark for Adapting Neural Surrogates to Distribution Shifts](https://arxiv.org/abs/2506.12007)
*Paul Setinek,Gianluca Galletti,Thomas Gross,Dominik Schnürer,Johannes Brandstetter,Werner Zellinger*

Main category: cs.LG

TLDR: 论文提出了SIMSHIFT数据集，用于评估神经代理模型在未见问题配置下的性能，并扩展了领域自适应方法以提升神经代理的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 神经代理模型在未见问题配置（如新材料或结构维度）下性能下降明显，而领域自适应技术在视觉和语言处理中已广泛应用，论文旨在填补这一空白。

Method: 引入SIMSHIFT数据集，包含四个工业仿真任务，并扩展领域自适应方法，利用源配置的参数描述和仿真数据预测目标配置的仿真结果。

Result: 实验表明，神经代理模型在分布偏移下存在挑战，但领域自适应在仿真中具有潜力。

Conclusion: 论文揭示了工业场景中神经代理模型在分布偏移下的开放问题，并提供了代码库以促进进一步研究。
...
### [118] [EMLoC: Emulator-based Memory-efficient Fine-tuning with LoRA Correction](https://arxiv.org/abs/2506.12015)
*Hsi-Che Lin,Yu-Chu Yu,Kai-Po Chang,Yu-Chiang Frank Wang*

Main category: cs.LG

TLDR: EMLoC是一种基于模拟器的内存高效微调框架，通过LoRA校正，实现在推理内存预算内完成模型微调。


<details>
  <summary>Details</summary>
Motivation: 开源基础模型虽强大，但微调大模型对多数用户来说内存开销过高。

Method: 构建轻量级模拟器（基于SVD）并通过LoRA微调，提出补偿算法校正LoRA模块。

Result: 实验表明EMLoC优于基线方法，支持单24GB GPU微调38B模型。
...
### [119] [TeleEval-OS: Performance evaluations of large language models for operations scheduling](https://arxiv.org/abs/2506.11017)
...
TLDR: 论文提出了首个电信运营调度评估基准TeleEval-OS，包含15个数据集和13个子任务，评估了开源和闭源大语言模型在电信调度任务中的表现。
...
Method: 设计了TeleEval-OS基准，包含四个关键操作阶段的模拟任务，并采用零样本和少样本评估方法测试14种大语言模型。

Result: 实验表明开源模型在特定场景下优于闭源模型，展现了其在电信调度领域的潜力。

Conclusion: TeleEval-OS填补了研究空白，为大语言模型在电信调度领域的应用提供了评估工具。
...
### [120] [Who is in the Spotlight: The Hidden Bias Undermining Multimodal Retrieval-Augmented Generation](https://arxiv.org/abs/2506.11063)
*Jiayu Yao,Shenghua Liu,Yiwei Wang,Lingrui Mei,Baolong Bi,Yuyao Ge,Zhecheng Li,Xueqi Cheng*

Main category: cs.CL

TLDR: 研究探讨了多模态检索增强生成（RAG）系统中证据位置对性能的影响，发现位置偏差普遍存在且多模态交互会加剧偏差。


<details>
  <summary>Details</summary>
Motivation: 当前RAG模型对证据顺序高度敏感，导致性能不稳定和推理偏差，尤其在多模态任务中。研究旨在揭示位置偏差的影响。

Method: 通过文本、图像及混合模态任务的实验，引入位置敏感指数（PSI_p）和可视化框架分析注意力分配模式。

Result: 发现多模态交互会加剧位置偏差，且偏差随检索范围对数增长。

Conclusion: 研究为RAG中的位置感知分析提供了理论基础，强调需要证据重排序或去偏策略以提高系统可靠性。
...
### [121] [Smotrom tvoja pa ander drogoj verden! Resurrecting Dead Pidgin with Generative Models: Russenorsk Case Study](https://arxiv.org/abs/2506.11065)
*Alexey Tikhonov,Sergei Shteiner,Anna Bykova,Ivan P. Yamshchikov*

Main category: cs.CL

TLDR: 利用现代大语言模型分析Russenorsk（俄挪皮钦语）的词汇，构建结构化词典，验证其构词和语法原则，并开发翻译工具。


<details>
  <summary>Details</summary>
Motivation: 研究Russenorsk这一独特的语言现象，填补其词汇和语法研究的空白。

Method: 基于现存文献构建词典，利用大语言模型验证假设，并开发翻译工具。

Result: 验证了部分学术假设，并生成了现代文本的Russenorsk翻译。

Conclusion: 大语言模型在历史语言研究中具有潜力，但仍需结合传统方法。
...
### [122] [A Large Language Model Based Pipeline for Review of Systems Entity Recognition from Clinical Notes](https://arxiv.org/abs/2506.11067)
...
TLDR: 开发了一种基于大语言模型（LLM）的自动化流程，用于从临床笔记中提取系统回顾（ROS）实体，结合开源和商业LLM，实现了低成本且高效的解决方案。
...
### [123] [Deontological Keyword Bias: The Impact of Modal Expressions on Normative Judgments of Language Models](https://arxiv.org/abs/2506.11068)
*Bumjin Park,Jinsil Lee,Jaesik Choi*

Main category: cs.CL

TLDR: 研究发现大型语言模型（LLMs）在道德推理中存在义务判断偏差（DKB），即模态表达（如“必须”或“应该”）会显著增加LLMs将非义务场景误判为义务的概率。


<details>
  <summary>Details</summary>
Motivation: 探讨LLMs在义务判断中的偏差现象，揭示模态表达对LLMs规范性决策的影响。

Method: 通过模态表达增强提示，分析LLMs在常识场景中的义务判断倾向，并提出结合少量示例和推理提示的缓解策略。

Result: LLMs在模态表达存在时，90%以上的常识场景被误判为义务，且该现象在不同模型、问题类型和答案格式中一致。

Conclusion: 模态表达作为语言框架显著影响LLMs的规范性决策，需解决此类偏差以实现判断对齐。
...
### [124] [Targeted control of fast prototyping through domain-specific interface](https://arxiv.org/abs/2506.11070)
*Yu-Zhe Shi,Mingchen Liu,Hanlu Ma,Qiao Xu,Huamin Qu,Kun He,Lecheng Ruan,Qining Wang*

Main category: cs.CL

TLDR: 提出一种接口架构，通过自然语言指令实现原型模型的精准控制，弥补设计师语言与建模语言之间的差距。


<details>
  <summary>Details</summary>
Motivation: 工业设计师希望通过自然语言指令直观控制原型模型，但现有大型语言模型在此领域的潜力未完全发挥。

Method: 设计一种接口架构，基于快速原型设计实践的原则，开发自动化领域规范算法。

Result: 机器评估和人类研究表明，该接口可作为大型语言模型的辅助模块，实现精准控制。

Conclusion: 该接口架构有效弥补了语言差距，提升了原型模型的控制精度和效率。
...
### [125] [CLAIM: Mitigating Multilingual Object Hallucination in Large Vision-Language Models with Cross-Lingual Attention Intervention](https://arxiv.org/abs/2506.11073)
...
TLDR: CLAIM提出了一种无需训练的跨语言注意力干预方法，通过调整注意力模式来减少多语言对象幻觉。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在多语言查询中容易产生与视觉输入不一致的幻觉，现有方法依赖资源密集的预训练或微调。

Method: CLAIM通过识别语言特定的跨模态注意力头，估计语言偏移向量，并在推理时干预注意力输出。

Result: 实验显示CLAIM在POPE和MME基准上平均分别提升13.56%和21.75%，西班牙语最高提升30%。
...
### [126] [CyclicReflex: Improving Large Reasoning Models via Cyclical Reflection Token Scheduling](https://arxiv.org/abs/2506.11077)
*Chongyu Fan,Yihua Zhang,Jinghan Jia,Alfred Hero,Sijia Liu*

Main category: cs.CL

TLDR: 论文提出了一种动态调整反思令牌（reflection tokens）使用的方法CyclicReflex，通过周期性调度提升大型推理模型的性能。


<details>
  <summary>Details</summary>
Motivation: 反思令牌在大型推理模型（LRMs）中用于引导多步推理，但过多或过少使用会降低性能，需要一种动态调节方法。

Method: 提出CyclicReflex方法，通过位置相关的三角波形动态调整反思令牌的logits。

Result: 在多个数据集（MATH500等）上验证，CyclicReflex优于标准解码和其他方法（如TIP和S1）。
...
### [127] [RoE-FND: A Case-Based Reasoning Approach with Dual Verification for Fake News Detection via LLMs](https://arxiv.org/abs/2506.11078)
*Yuzhou Yang,Yangming Zhou,Zhiying Zhu,Zhenxing Qian,Xinpeng Zhang,Sheng Li*

Main category: cs.CL

TLDR: RoE-FND是一个基于逻辑推理的假新闻检测框架，结合大型语言模型与经验学习，通过自反知识构建和动态准则检索提升检测效果。


<details>
  <summary>Details</summary>
Motivation: 解决现有假新闻检测方法中证据选择噪声、泛化瓶颈和决策过程不清晰的问题，以及大型语言模型在检测中产生的幻觉和结论偏差。

Method: RoE-FND分为两个阶段：自反知识构建（分析历史推理错误）和动态准则检索（从历史案例中提取推理准则），并通过双通道程序交叉验证。

Result: 在三个数据集上的实验验证了RoE-FND的优越泛化能力和有效性，优于现有方法。

Conclusion: RoE-FND通过逻辑推理和经验学习，显著提升了假新闻检测的准确性和适应性。
...
### [128] [MANBench: Is Your Multimodal Model Smarter than Human?](https://arxiv.org/abs/2506.11080)
*Han Zhou,Qitong Xu,Yiheng Dong,Xin Yang*

Main category: cs.CL

TLDR: MANBench是一个双语基准测试，用于评估多模态大语言模型（MLLMs）在多种任务中的表现，发现其在某些领域接近人类水平，但在复杂推理任务中仍有不足。


<details>
  <summary>Details</summary>
Motivation: 评估MLLMs在多模态任务中的表现，并与人类能力进行对比，揭示其优势和局限性。

Method: 开发了包含1,314个问题的双语基准测试MANBench，涵盖九类任务，并通过人类实验与MLLMs进行对比。

Result: MLLMs在知识和文本-图像理解任务中表现优异，但在跨模态推理和复杂任务（如谜题和空间想象）中表现不佳。

Conclusion: MANBench揭示了MLLMs与人类能力的差距，为未来研究提供了方向。
...
### [129] [SAGE:Specification-Aware Grammar Extraction for Automated Test Case Generation with LLMs](https://arxiv.org/abs/2506.11081)
*Aditi,Hyunwoo Park,Sicheol Sung,Yo-Sub Han,Sang-Ki Ko*

Main category: cs.CL

TLDR: 论文提出了一种基于开源大语言模型（LLM）的方法SAGE，用于从自然语言规范中生成上下文无关计数器文法（CCFG），并通过强化学习优化文法质量。实验表明，SAGE在文法有效性和测试效果上优于17种开源和闭源LLM。
...
Method: 结合开源LLM的微调和强化学习（GRPO）优化文法生成，并引入迭代反馈修正错误。

Result: SAGE在文法有效性上提升15.92%，测试效果提升12.34%，优于现有方法。

Conclusion: SAGE展示了LLM在文法生成中的潜力，为竞争性编程问题提供了高效解决方案。
...
### [130] [PRISM: A Transformer-based Language Model of Structured Clinical Event Data](https://arxiv.org/abs/2506.11082)
*Lionel Levine,John Santerre,Alex S. Young,T. Barry Levine,Francis Campion,Majid Sarrafzadeh*

Main category: cs.CL

TLDR: PRISM是一种基于Transformer的架构，用于建模临床决策过程的序列进展，通过学习预测患者诊断旅程中的下一步，显著优于随机基线。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖孤立的诊断分类，无法捕捉临床轨迹的复杂性。PRISM旨在通过序列建模改进临床决策支持。

Method: PRISM将临床轨迹标记化为事件序列（如诊断测试、实验室结果），利用自定义临床词汇和自回归训练目标学习依赖关系。
...
### [131] [RedDebate: Safer Responses through Multi-Agent Red Teaming Debates](https://arxiv.org/abs/2506.11083)
*Ali Asad,Stephen Obadinma,Radin Shayanfar,Xiaodan Zhu*

Main category: cs.CL

TLDR: RedDebate是一种多智能体辩论框架，通过LLM之间的对抗性论证自动识别和减少不安全行为，结合长期记忆模块，效果显著。
...
Method: 利用多LLM相互批判性检查，结合长期记忆模块，通过红队测试系统性发现并改进不安全行为。

Result: 辩论单独减少17.7%不安全行为，结合记忆模块后减少超过23.5%。

Conclusion: RedDebate是首个结合多智能体辩论与红队测试的全自动AI安全框架，无需人工干预。
...
### [132] [The Scales of Justitia: A Comprehensive Survey on Safety Evaluation of LLMs](https://arxiv.org/abs/2506.11094)
*Songyang Liu,Chaozhuo Li,Jiameng Qiu,Xi Zhang,Feiran Huang,Litian Zhang,Yiming Hei,Philip S. Yu*

Main category: cs.CL

TLDR: 本文综述了大语言模型（LLMs）安全评估的最新进展，探讨了评估的背景、内容、方法及挑战，并提出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs的广泛应用，其生成内容中的安全风险（如毒性、偏见等）引发关注，但缺乏系统性总结。本文旨在填补这一空白。

Method: 从四个方面系统梳理LLMs安全评估：评估背景（Why）、评估内容（What）、评估工具（Where）、评估方法（How）。

Result: 总结了现有评估任务、指标、数据集及方法，并分类主流评估方法。

Conclusion: 强调LLMs安全评估的重要性，提出未来研究方向，以推动其安全部署。
...
### [133] [Two Birds with One Stone: Improving Factuality and Faithfulness of LLMs via Dynamic Interactive Subspace Editing](https://arxiv.org/abs/2506.11088)
*Pengbo Wang,Chaozhuo Li,Chenxu Wang,Liwen Zheng,Litian Zhang,Xi Zhang*

Main category: cs.CL

TLDR: 论文提出SPACE框架，通过编辑共享激活子空间同时提升大语言模型的事实性和忠实性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型存在事实性和忠实性幻觉问题，现有方法独立解决这些问题但导致性能折衷。

Method: 通过双任务特征建模和混合探针策略（谱聚类与注意力头显著性评分）识别并编辑共享子空间。

Result: 实验结果表明SPACE在多个基准数据集上表现优越。

Conclusion: SPACE为同时缓解事实性和忠实性幻觉提供了统一框架。
...
### [134] [Customizing Speech Recognition Model with Large Language Model Feedback](https://arxiv.org/abs/2506.11091)
*Shaoshi Ling,Guoli Ye*

Main category: cs.CL

TLDR: 提出了一种基于强化学习的无监督领域适应方法，利用LLM作为奖励模型提升ASR系统对命名实体的识别能力。
...
Method: 通过强化学习方法，利用LLM作为奖励模型对ASR模型的假设进行评分，并使用这些评分作为奖励信号对ASR模型进行微调。

Result: 相比传统自训练方法，该方法在实体词错误率上提升了21%。
...
### [135] [Dynamic Context Tuning for Retrieval-Augmented Generation: Enhancing Multi-Turn Planning and Tool Adaptation](https://arxiv.org/abs/2506.11092)
*Jubin Abhishek Soni,Amit Anand,Rajesh Kumar Pandey,Aniket Abhishek Soni*

Main category: cs.CL

TLDR: DCT扩展了RAG框架，支持多轮对话和动态工具环境，无需重新训练，显著提升性能并降低成本。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统局限于静态单轮交互，无法适应动态领域（如医疗和智能家居）的需求。

Method: DCT结合注意力缓存、LoRA检索和上下文压缩技术，动态选择工具并跟踪信息。

Result: 实验显示DCT提升计划准确性14%，减少幻觉37%，性能接近GPT-4但成本更低。

Conclusion: DCT可推广至未见工具，适用于广泛动态环境，具有扩展性和适应性。
...
### [136] [Persistent Homology of Topic Networks for the Prediction of Reader Curiosity](https://arxiv.org/abs/2506.11095)
*Manuel D. S. Hopp,Vincent Labatut,Arthur Amalvy,Richard Dufour,Hannah Stone,Hayley Jach,Kou Murayama*

Main category: cs.CL

TLDR: 该论文提出了一种基于信息缺口理论的框架，通过量化文本语义结构中的信息缺口来建模读者好奇心，结合BERTopic和持久同调分析动态语义网络的拓扑特征，实验证明其显著提升了好奇心预测效果。


<details>
  <summary>Details</summary>
Motivation: 读者好奇心对文本参与至关重要，但在NLP领域研究较少，因此需要一种计算模型来量化信息缺口并预测好奇心。

Method: 结合BERTopic主题建模和持久同调分析动态语义网络的拓扑特征（如连通组件、循环、空洞），将其作为信息缺口的代理变量。

Result: 实验表明，该模型显著提升了好奇心预测效果（解释偏差从30%提升至73%）。
...
### [137] [C-SEO Bench: Does Conversational SEO Work?](https://arxiv.org/abs/2506.11097)
*Haritz Puerto,Martin Gubri,Tommaso Green,Seong Joon Oh,Sangdoo Yun*

Main category: cs.CL

TLDR: 论文提出C-SEO Bench，首个评估跨任务、领域和多参与者的C-SEO方法的基准，发现现有方法效果有限，传统SEO策略更有效。
...
Method: 设计C-SEO Bench基准，涵盖两种搜索任务（问答和产品推荐）和三个领域，并引入多参与者采用率评估协议。

Result: 当前C-SEO方法效果不佳，传统SEO策略更有效；多参与者竞争下收益递减，问题呈现零和性质。

Conclusion: C-SEO Bench为评估C-SEO方法提供了新标准，揭示了现有方法的局限性，强调传统策略的优势和多参与者竞争的挑战。
...
### [138] [Evolutionary Perspectives on the Evaluation of LLM-Based AI Agents: A Comprehensive Survey](https://arxiv.org/abs/2506.11102)
...
TLDR: 本文系统分析了当前评估方法，区分了AI代理与LLM聊天机器人，并提出了一个五维框架。同时分类了现有评估基准，并展望了未来评估方法。
...
Method: 通过五维框架（复杂环境、多源指令、动态反馈、多模态感知、高级能力）区分AI代理与LLM聊天机器人，并分类评估基准。

Result: 提出了实用的参考表格，总结了当前趋势，并展望了未来评估方法。

Conclusion: 研究为AI代理评估提供了指导，促进了该领域的进一步发展。
...
### [139] [You Only Fine-tune Once: Many-Shot In-Context Fine-Tuning for Large Language Model](https://arxiv.org/abs/2506.11103)
*Wenchong He,Liqian Peng,Zhe Jiang,Alex Go*

Main category: cs.CL

TLDR: 论文提出了一种名为ManyICL的新方法，通过将上下文学习扩展到多样本设置，显著缩小了与专用微调的性能差距。


<details>
  <summary>Details</summary>
Motivation: 当前的中等规模LLMs通过少样本上下文微调虽能实现多任务处理，但仍落后于专用微调的性能。

Method: 提出ManyICL方法，将上下文中的每个答案作为监督训练目标，而非仅预测最终答案。

Result: ManyICL在多种任务上显著优于零/少样本微调，接近专用微调性能，并缓解了灾难性遗忘问题。
...
### [140] [DAM: Dynamic Attention Mask for Long-Context Large Language Model Inference Acceleration](https://arxiv.org/abs/2506.11104)
*Hanzhi Zhang,Heng Fan,Kewei Sha,Yan Huang,Yunhe Feng*

Main category: cs.CL

TLDR: 论文提出了一种动态稀疏注意力机制（DAM），通过自适应生成注意力掩码，解决了传统稀疏注意力方法中静态掩码导致的性能下降问题，同时保持了计算效率。


<details>
  <summary>Details</summary>
Motivation: 长上下文理解对NLP应用至关重要，但传统Transformer的自注意力机制因二次复杂度效率低下。稀疏注意力方法虽能缓解计算成本，但静态预定义掩码无法捕捉异质性注意力模式，导致性能受限。

Method: 提出动态稀疏注意力机制，在注意力图级别自适应生成掩码，无需预定义结构或微调，保留各层和头的异质性模式。

Result: 该方法在保持计算效率的同时，与全注意力模型性能高度对齐，显著降低内存和计算开销，适用于大规模LLM部署。

Conclusion: DAM为全注意力机制提供了可扩展的替代方案，既提升了长序列任务的适应性，又保持了检索性能。
...
### [141] [Enabling On-Device Medical AI Assistants via Input-Driven Saliency Adaptation](https://arxiv.org/abs/2506.11105)
*Uttej Kallakurik,Edward Humes,Rithvik Jonna,Xiaomin Lin,Tinoosh Mohsenin*

Main category: cs.CL

TLDR: 该论文提出了一种针对医疗领域的LLMs压缩框架，通过神经元剪枝和量化技术，显著减小模型体积并保持性能，成功在边缘设备上实现实时推理。


<details>
  <summary>Details</summary>
Motivation: LLMs在医疗场景中潜力巨大，但体积过大难以在资源受限的边缘设备上部署，因此需要一种高效的压缩方法。

Method: 通过测量神经元在领域数据上的显著性进行剪枝，再结合后训练量化技术，进一步减小模型体积。

Result: 在MedMCQA、MedQA和PubMedQA等医疗基准测试中表现良好，Gemma和LLaMA3模型分别压缩50%和67%，成功部署在Jetson Orin Nano和Raspberry Pi 5上。

Conclusion: 该压缩框架有效解决了LLMs在边缘设备上的部署问题，为医疗领域的实时应用提供了可行方案。
...
### [142] [Graph-based RAG Enhancement via Global Query Disambiguation and Dependency-Aware Reranking](https://arxiv.org/abs/2506.11106)
*Ningyuan Li,Junrui Liu,Yi Shan,Minghui Huang,Tong Li*

Main category: cs.CL

TLDR: PankRAG框架通过全局感知的分层查询解析和依赖感知的重新排序机制，解决了传统基于图的RAG方法在实体提取上的局限性，显著提升了检索和生成的准确性与相关性。


<details>
  <summary>Details</summary>
Motivation: 传统基于图的RAG方法仅依赖实体提取，容易忽略潜在关键信息，导致检索内容不相关或矛盾，增加幻觉风险并降低生成响应的准确性。

Method: PankRAG采用分层查询解析策略和依赖感知的重新排序机制，构建多级解析路径并利用子问题间的依赖关系优化检索结果。

Result: PankRAG在多个基准测试中优于现有方法，证明了其鲁棒性和泛化能力。

Conclusion: PankRAG通过结构化推理和依赖感知的优化，显著提升了RAG方法的性能，为复杂查询提供了更准确的检索和生成结果。
...
### [143] [History-Aware Cross-Attention Reinforcement: Self-Supervised Multi Turn and Chain-of-Thought Fine-Tuning with vLLM](https://arxiv.org/abs/2506.11108)
*Andrew Kiruluta,Andreas Lemos,Priscilla Burity*

Main category: cs.CL

TLDR: CAGSR-vLLM-MTC扩展了CAGSR框架，基于vLLM运行时实现，用于多轮对话和链式推理。通过异步捕获跨注意力权重并泛化奖励函数，优化了对话历史和推理步骤的注意力信号。
...
Method: 在vLLM运行时中异步捕获跨注意力权重，并泛化自监督奖励函数以累积对话历史和推理步骤的注意力信号。

Result: 提出了基于熵的钳制机制以防止注意力崩溃，并优化了多轮对话和推理的注意力分配。

Conclusion: 为多轮对话和链式推理提供了有效的注意力机制优化方案，并展望了多参与方对话和分层推理的未来方向。
...
### [144] [Enhancing Large Language Models for Mobility Analytics with Semantic Location Tokenization](https://arxiv.org/abs/2506.11109)
*Yile Chen,Yicheng Tao,Yue Jiang,Shuai Liu,Han Yu,Gao Cong*

Main category: cs.CL

TLDR: QT-Mob框架通过改进位置标记化和多目标微调，提升了LLM在移动数据分析中的性能。
...
Method: QT-Mob引入位置标记化模块学习语义丰富的紧凑标记，并结合多目标微调优化LLM对移动模式和位置语义的理解。
...
### [145] [AssertBench: A Benchmark for Evaluating Self-Assertion in Large Language Models](https://arxiv.org/abs/2506.11110)
*Jaeho Lee,Atharv Chowdhary*

Main category: cs.CL

TLDR: AssertBench评估LLMs在面对用户对立陈述时保持事实一致性的能力，通过构建两种对立提示并记录模型反应。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在用户对立陈述下是否坚持事实一致性，而非盲目迎合用户。

Method: 从FEVEROUS数据集中采样事实，构建对立提示（用户声称正确或错误），记录模型反应。

Result: 通过分层分析，隔离框架诱导的变异性，测量模型坚持事实的能力。

Conclusion: AssertBench为评估LLMs在面对对立陈述时的稳定性提供了新方法。
...
### [146] [Evaluating and Improving Robustness in Large Language Models: A Survey and Future Directions](https://arxiv.org/abs/2506.11111)
*Kun Zhang,Le Wu,Kui Yu,Guangyi Lv,Dacao Zhang*

Main category: cs.CL

TLDR: 本文综述了大语言模型（LLMs）的鲁棒性研究，从对抗鲁棒性、OOD鲁棒性和评估方法三个方面系统梳理了相关概念和方法，并提供了开源项目支持社区研究。
...
Method: 通过定义LLM鲁棒性，并从对抗鲁棒性、OOD鲁棒性和评估方法三个角度组织文献综述。

Result: 总结了代表性工作，并提供了开源项目支持社区研究。

Conclusion: 未来研究方向包括进一步优化LLM的鲁棒性，并推动相关工具和数据集的发展。
...
### [147] [Manifesto from Dagstuhl Perspectives Workshop 24352 -- Conversational Agents: A Framework for Evaluation (CAFE)](https://arxiv.org/abs/2506.11112)
...
TLDR: 讨论了CONIAC的定义及其独特特征，提出了一个抽象的世界模型，并定义了CAFE框架用于评估CONIAC系统，包含六个主要组成部分。


<details>
  <summary>Details</summary>
Motivation: 探讨CONIAC的概念及其在信息访问中的独特作用，并建立一个系统化的评估框架。

Method: 提出世界模型抽象化CONIAC，并设计CAFE框架，包含六个评估组成部分。

Result: 定义了CONIAC的评估框架CAFE，明确了六个关键评估维度。

Conclusion: CAFE框架为CONIAC系统的评估提供了系统化的方法，有助于标准化和优化其性能。
...
### [148] [Breaking the Reviewer: Assessing the Vulnerability of Large Language Models in Automated Peer Review Under Textual Adversarial Attacks](https://arxiv.org/abs/2506.11113)
...
TLDR: 本文研究了大型语言模型（LLMs）作为自动化审稿人在对抗攻击下的鲁棒性，发现文本操纵会扭曲其评估，强调需解决对抗风险以保障学术交流的完整性。
...
Method: 研究聚焦三个问题：LLMs生成审稿的有效性、对抗攻击对其可靠性的影响，以及应对挑战的策略。
...
### [149] [KokushiMD-10: Benchmark for Evaluating Large Language Models on Ten Japanese National Healthcare Licensing Examinations](https://arxiv.org/abs/2506.11114)
*Junyu Liu,Kaiqi Yan,Tianyang Wang,Qian Niu,Momoko Nagai-Tanima,Tomoki Aoyama*

Main category: cs.CL

TLDR: KokushiMD-10是一个多模态基准测试，基于日本国家医疗执照考试构建，用于评估大型语言模型在医疗领域的多语言和多模态推理能力。
...
Method: 构建KokushiMD-10基准，包含11588道真实考试题目，涵盖多个医疗领域，结合临床图像和专家注释。

Result: 测试了30多个先进LLM，包括GPT-4o、Claude 3.5和Gemini，但无模型在所有领域均达到及格标准。

Conclusion: KokushiMD-10为评估和推进多语言、多模态医疗AI提供了全面资源，突显了医疗AI的挑战。
...
### [150] [Incorporating Domain Knowledge into Materials Tokenization](https://arxiv.org/abs/2506.11115)
*Yerim Oh,Jun-Hyung Park,Junho Kim,SungHo Kim,SangKeun Lee*

Main category: cs.CL

TLDR: MATTER是一种新型标记化方法，通过整合材料知识解决传统方法在材料科学中的语义和结构完整性问题。
...
Conclusion: 研究表明领域知识对科学文本处理中的标记化策略至关重要。
...
### [151] [Infinity Instruct: Scaling Instruction Selection and Synthesis to Enhance Language Models](https://arxiv.org/abs/2506.11116)
*Jijie Li,Li Du,Hanyu Zhao,Bo-wen Zhang,Liangdong Wang,Boyan Gao,Guang Liu,Yonghua Lin*

Main category: cs.CL

TLDR: Infinity-Instruct是一个高质量指令数据集，通过两阶段流程提升LLM的基础和聊天能力，显著超越现有开源模型。
...
Method: 两阶段流程：1) 从1亿样本中筛选7.4M高质量基础指令；2) 通过选择、进化和过滤合成1.5M高质量聊天指令。

Result: InfInstruct-LLaMA3.1-70B在指令跟随任务中超越GPT-4-0314 8.6%，基础性能相当。
...
### [152] [ScIRGen: Synthesize Realistic and Large-Scale RAG Dataset for Scientific Research](https://arxiv.org/abs/2506.11117)
*Junyong Lin,Lu Dai,Ruiqian Han,Yijie Sui,Ruilin Wang,Xingliang Sun,Qinglin Wu,Min Feng,Hao Liu,Hui Xiong*

Main category: cs.CL

TLDR: ScIRGen是一个科学问答与检索数据集生成框架，旨在更准确地反映专业科研人员的信息需求，并创建了一个大规模的科学检索增强生成数据集ScIRGen-Geo。


<details>
  <summary>Details</summary>
Motivation: 现有科学检索和问答数据集通常处理简单问题，与真实研究需求不符。

Method: 设计了基于学术论文的数据集导向信息提取方法、认知分类法生成问题框架，以及基于LLM困惑度变化的自动答案过滤方法。

Result: 生成了61k的QA数据集ScIRGen-Geo，并发现现有方法在处理复杂问题时仍有不足。

Conclusion: 该工作推动了支持科学社区复杂信息需求的更先进工具的发展。
...
### [153] [Benchmarking Foundation Speech and Language Models for Alzheimer's Disease and Related Dementia Detection from Spontaneous Speech](https://arxiv.org/abs/2506.11119)
*Jingyu Li,Lingchao Mao,Hairong Wang,Zhendong Wang,Xi Mao,Xuelei Sherry Ni*

Main category: cs.CL

TLDR: 研究通过语音和语言基础模型对阿尔茨海默病及相关痴呆（ADRD）进行分类，发现语音模型Whisper-medium和语言模型BERT表现最佳，ASR生成的音频嵌入效果突出。
...
Method: 使用PREPARE Challenge数据集，筛选703例健康对照、81例轻度认知障碍和405例AD病例，测试多种开源基础模型。

Result: Whisper-medium语音模型（准确率0.731，AUC 0.802）和BERT语言模型（准确率0.662，AUC 0.744）表现最佳，ASR嵌入效果显著。

Conclusion: 基础模型结合临床数据集展示了语音特征在ADRD早期检测中的潜力，尤其是ASR嵌入方法。
...
### [154] [SDMPrune: Self-Distillation MLP Pruning for Efficient Large Language Models](https://arxiv.org/abs/2506.11120)
*Hourun Zhu,Chengchao Shen*

Main category: cs.CL

TLDR: 提出了一种在剪枝阶段引入自蒸馏损失的方法，以更准确地利用原始模型的预测信息，显著压缩LLM而不明显降低性能。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs性能强大，但其部署成本高昂，现有梯度剪枝方法忽略了生成能力的关键信息。

Method: 在剪枝阶段引入自蒸馏损失，专注于多层感知机（MLP）模块的剪枝。

Result: 在零样本基准测试中显著优于现有剪枝方法，性能在1B规模开源LLMs中具有竞争力。

Conclusion: 该方法有效降低了LLMs的部署成本，同时保持了高性能。
...
### [155] [SUTA-LM: Bridging Test-Time Adaptation and Language Model Rescoring for Robust ASR](https://arxiv.org/abs/2506.11121)
*Wei-Ping Huang,Guan-Ting Lin,Hung-yi Lee*

Main category: cs.CL

TLDR: SUTA-LM是一种结合TTA和语言模型重评分的改进方法，通过控制适应过程和利用声学与语言信息优化ASR性能。


<details>
  <summary>Details</summary>
Motivation: 解决TTA与语言模型重评分结合时的干扰问题，提升ASR在领域不匹配情况下的表现。

Method: 提出SUTA-LM，结合熵最小化TTA和语言模型重评分，采用自动步长选择机制控制适应过程。

Result: 在18个多样化ASR数据集上验证，SUTA-LM表现出跨领域的鲁棒性。

Conclusion: SUTA-LM有效解决了TTA与语言模型结合的挑战，显著提升ASR性能。
...
### [156] [ASRJam: Human-Friendly AI Speech Jamming to Prevent Automated Phone Scams](https://arxiv.org/abs/2506.11125)
*Freddie Grabovski,Gilad Gressel,Yisroel Mirsky*

Main category: cs.CL

TLDR: 论文提出ASRJam和EchoGuard框架，通过对抗性扰动和自然失真干扰ASR系统，防止语音钓鱼诈骗，同时不影响人类通话体验。


<details>
  <summary>Details</summary>
Motivation: 语音钓鱼诈骗利用LLM、TTS和ASR技术规模化且逼真，构成严重安全威胁。

Method: 提出ASRJam注入对抗性扰动，EchoGuard利用自然失真（如回声）干扰ASR。

Result: 39人用户研究表明，EchoGuard在ASR干扰和人类听觉体验上表现最佳。

Conclusion: EchoGuard是一种实用且高效的防御方案，适用于实时语音钓鱼防范。
...
### [157] [GUIRoboTron-Speech: Towards Automated GUI Agents Based on Speech Instructions](https://arxiv.org/abs/2506.11127)
*Wenkang Han,Zhixiong Zeng,Jing Huang,Shu Jiang,Liming Zheng,Longrong Yang,Haibo Qiu,Chang Yao,Jingyuan Chen,Lin Ma*

Main category: cs.CL

TLDR: GUIRoboTron-Speech是一种端到端的自主GUI代理，直接接受语音指令和设备截图来预测动作，解决了基于文本指令的限制。
...
Method: 利用随机音色TTS模型生成高质量语音指令，通过渐进式接地和规划训练阶段开发代理能力，并采用启发式混合指令训练策略解决模态不平衡问题。
...
### [158] [Stronger Language Models Produce More Human-Like Errors](https://arxiv.org/abs/2506.11128)
*Andrew Keenan Richardson,Ryan Othniel Kearns,Sean Moss,Vincent Wang-Mascianica,Philipp Koralus*

Main category: cs.CL

TLDR: 研究发现，随着语言模型能力的提升，其错误模式逐渐接近人类推理的典型误区，而非逻辑正确性提高。


<details>
  <summary>Details</summary>
Motivation: 探讨语言模型是否随着能力提升而接近人类的推理模式，尤其是错误模式。

Method: 使用Erotetic Theory of Reasoning (ETR)框架和PyETR工具生成逻辑问题，评估38个语言模型在383个任务中的表现。

Result: 模型能力提升时，错误答案中符合ETR预测的人类误区比例增加（ρ=0.360,p=0.0265），但逻辑正确性未提高。

Conclusion: 语言模型的错误模式趋向人类化，挑战了模型规模扩展会自然实现规范理性的观点。
...
### [159] [Trustworthy AI for Medicine: Continuous Hallucination Detection and Elimination with CHECK](https://arxiv.org/abs/2506.11129)
...
TLDR: CHECK框架通过结合临床数据库和信息理论分类器，显著降低了LLM在医疗领域的幻觉率，并提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在医疗领域中的幻觉问题，以确保其临床应用的可靠性。

Method: CHECK框架整合了结构化临床数据库和信息理论分类器，用于检测事实性和推理性幻觉。

Result: 在1500个临床问题中，CHECK将幻觉率从31%降至0.3%，并在多个医疗基准测试中表现优异（AUC 0.95-0.96）。
...
### [160] [A Self-Refining Framework for Enhancing ASR Using TTS-Synthesized Data](https://arxiv.org/abs/2506.11130)
*Cheng Kang Chou,Chan-Jan Hsu,Ho-Lam Chung,Liang-Hsuan Tseng,Hsi-Chun Cheng,Yu-Kuan Fu,Kuan Po Huang,Hung-Yi Lee*

Main category: cs.CL

TLDR: 提出了一种自优化框架，仅使用未标注数据集提升ASR性能，通过生成伪标签训练TTS系统，再反馈至ASR模型，显著降低错误率。
...
Method: 利用未标注语音生成伪标签，训练TTS系统，合成数据反馈至ASR模型，形成闭环优化。

Result: 在台湾普通话任务中，错误率降低20%（普通话）和50%（中英混合），优于Whisper。

Conclusion: 该框架为低资源ASR提供高效优化路径，优于传统伪标签自蒸馏方法。
...
### [161] [Large Language Models and Emergence: A Complex Systems Perspective](https://arxiv.org/abs/2506.11135)
*David C. Krakauer,John W. Krakauer,Melanie Mitchell*

Main category: cs.CL

TLDR: 论文探讨了涌现现象与大型语言模型（LLM）的涌现能力及智能之间的关系。
...
Result: LLM展现出涌现能力，但其是否具备涌现智能仍需探讨。

Conclusion: 涌现现象在LLM中显著存在，但其智能的涌现性质仍需进一步研究。
...
### [162] [Scalable Medication Extraction and Discontinuation Identification from Electronic Health Records Using Large Language Models](https://arxiv.org/abs/2506.11137)
...
TLDR: 研究评估了大型语言模型（LLMs）在电子健康记录（EHR）中提取药物信息并分类药物状态的能力，发现GPT-4o在零样本设置下表现最佳，开源模型如Llama-3.1-70B-Instruct也表现优异。


<details>
  <summary>Details</summary>
Motivation: 电子健康记录中的药物停用信息对患者安全至关重要，但通常埋藏在非结构化文本中，需要高效且可扩展的自动提取方法。

Method: 研究收集了三个EHR数据集，评估了12种先进LLMs，并探索了多种提示策略，比较了药物提取、状态分类及联合任务的性能。

Result: GPT-4o在零样本设置下表现最佳（药物提取F1=94.0%，停用分类F1=78.1%，联合任务F1=72.7%），开源模型紧随其后。医学专用LLMs表现较差。
...
### [163] [RETUYT-INCO at BEA 2025 Shared Task: How Far Can Lightweight Models Go in AI-powered Tutor Evaluation?](https://arxiv.org/abs/2506.11243)
*Santiago Góngora,Ignacio Sastre,Santiago Robaina,Ignacio Remersaro,Luis Chiruzzo,Aiala Rosá*

Main category: cs.CL

TLDR: 论文展示了在计算资源受限的全球南方研究环境中，使用参数少于1B的小型模型在BEA 2025共享任务中的表现仍具竞争力。


<details>
  <summary>Details</summary>
Motivation: 模拟全球南方研究机构因计算资源昂贵而受限的条件，验证小型模型的实用性。

Method: 使用参数少于1B的小型模型参与BEA 2025共享任务。

Result: 在五个赛道中，与优胜者的F1分数差距为6.46至13.13分，表明小型模型仍具竞争力。

Conclusion: 小型模型在低预算GPU或无GPU环境下仍能胜任任务，适合资源受限的研究环境。
...
### [164] [Iterative Multilingual Spectral Attribute Erasure](https://arxiv.org/abs/2506.11244)
*Shun Shao,Yftah Ziser,Zheng Zhao,Yifu Qiu,Shay B. Cohen,Anna Korhonen*

Main category: cs.CL

TLDR: IMSAE是一种通过迭代SVD截断方法在多语言中识别和减轻联合偏见的算法，优于传统单语和跨语言方法。


<details>
  <summary>Details</summary>
Motivation: 现有去偏方法无法利用多语言表示中共享语义空间的优势，因为它们仅针对单语操作。

Method: 提出IMSAE，通过迭代SVD截断在多语言中识别和减轻联合偏见子空间。

Result: 在八种语言和五个维度上验证，IMSAE在标准和零样本场景中表现优异，且保持模型实用性。

Conclusion: IMSAE在多语言去偏中优于传统方法，并能利用语言相似性进行零样本去偏。
...
### [165] [No Universal Prompt: Unifying Reasoning through Adaptive Prompting for Temporal Table Reasoning](https://arxiv.org/abs/2506.11246)
*Kushagra Dixit,Abhishek Rajgaria,Harshavardhan Kalalbandi,Dan Roth,Vivek Gupta*

Main category: cs.CL

TLDR: 论文研究了不同提示技术对时间表推理的影响，提出了自适应提示框架SEAR，并在多种表类型中验证其优越性。


<details>
  <summary>Details</summary>
Motivation: 现有提示技术对表推理的影响尚未充分研究，且模型性能因表结构和上下文差异而波动，缺乏统一最优方法。

Method: 研究多种提示技术，提出自适应框架SEAR，动态调整提示策略并结合结构化推理。

Result: SEAR在所有表类型中表现优于基线方法；表结构重构的统一表示能提升推理能力。

Conclusion: SEAR是解决时间表推理的有效框架，适应性强且性能优越。
...
### [166] [Learning a Continue-Thinking Token for Enhanced Test-Time Scaling](https://arxiv.org/abs/2506.11274)
*Liran Ringel,Elad Tolochinsky,Yaniv Romano*

Main category: cs.CL

TLDR: 研究探讨通过学习专用的继续思考令牌（如“<|continue-thinking|>”）来扩展语言模型的推理能力，并通过强化学习训练其嵌入，显著提升了数学基准测试的准确性。


<details>
  <summary>Details</summary>
Motivation: 探索是否可以通过学习专用的继续思考令牌来触发更长的推理步骤，从而提升语言模型的性能。

Method: 在DeepSeek-R1的蒸馏版本中引入一个学习的“<|continue-thinking|>”令牌，仅通过强化学习训练其嵌入，同时冻结模型权重。

Result: 在GSM8K等数学基准测试中，学习令牌方法比基线模型和固定令牌方法（如“Wait”）显著提升了准确性（例如，4.2% vs. 1.3%）。
...
### [167] [Beyond Random Sampling: Efficient Language Model Pretraining via Curriculum Learning](https://arxiv.org/abs/2506.11300)
*Yang Zhang,Amr Mohamed,Hadi Abdine,Guokan Shang,Michalis Vazirgiannis*

Main category: cs.CL

TLDR: 本文首次系统研究了课程学习在预训练语言模型中的应用，实验表明其能显著提升训练效率和泛化能力。
...
Method: 实验采用了多种课程学习设置（如基础课程学习、基于节奏的采样和交错课程），并结合六种难度指标（语言学和信息论视角）进行指导。

Result: 课程学习在训练早期和中期显著提升收敛速度，作为预热策略时能带来最高3.5%的持续性能提升。压缩比、词汇多样性和可读性被证明为有效的难度信号。
...
### [168] [Don't Pay Attention](https://arxiv.org/abs/2506.11305)
*Mohammad Hammoud,Devang Acharya*

Main category: cs.CL

TLDR: 论文提出了一种名为Avey的新神经网络架构，旨在解决Transformer在处理长序列时的局限性，同时避免RNN的并行性问题。


<details>
  <summary>Details</summary>
Motivation: Transformer在处理长序列时存在固定上下文窗口和注意力机制二次复杂度的挑战，而RNN虽然能线性扩展序列长度，但并行性受限。

Method: Avey由排序器和自回归神经处理器组成，通过协作识别并仅处理最相关的标记，解耦序列长度与上下文宽度。

Result: 实验表明，Avey在短程NLP任务上与Transformer表现相当，且在长程依赖任务中表现更优。

Conclusion: Avey提供了一种替代注意力机制和循环结构的新方法，能有效处理任意长度序列。
...
### [169] [Surprisal from Larger Transformer-based Language Models Predicts fMRI Data More Poorly](https://arxiv.org/abs/2506.11338)
*Yi-Chien Lin,William Schuler*

Main category: cs.CL

TLDR: 研究发现，Transformer模型的困惑度与其对人类句子处理难度的预测能力呈正相关，且这一趋势不仅适用于基于延迟的测量，还可推广到神经测量。


<details>
  <summary>Details</summary>
Motivation: 探索Transformer模型的困惑度与其对人类语言处理（包括脑成像数据）预测能力之间的关系。

Method: 评估17种预训练Transformer模型在三种语言家族和两个fMRI数据集上的预测能力。

Result: 模型困惑度与模型拟合度之间的正相关关系依然存在，表明这一趋势可推广到神经测量。
...
### [170] [From Replication to Redesign: Exploring Pairwise Comparisons for LLM-Based Peer Review](https://arxiv.org/abs/2506.11343)
*Yaohui Zhang,Haijing Zhang,Wenlong Ji,Tianyu Hua,Nick Haber,Hancheng Cao,Weixin Liang*

Main category: cs.CL

TLDR: 论文提出了一种利用LLM代理进行成对比较而非单独评分的新机制，显著优于传统评分方法，但也揭示了选择过程中的偏见问题。


<details>
  <summary>Details</summary>
Motivation: 探索如何利用LLM重新构想学术评审流程，而非简单替代人类评审者。

Method: 引入LLM代理进行大量成对比较，以更准确地衡量论文相对质量。

Result: 实验表明该方法在识别高影响力论文上优于传统评分方法，但也发现了新颖性降低和机构不平衡等偏见。

Conclusion: LLM在重构评审流程中具有潜力，但需解决公平性和多样性问题。
...
### [171] [Do We Still Need Audio? Rethinking Speaker Diarization with a Text-Based Approach Using Multiple Prediction Models](https://arxiv.org/abs/2506.11344)
*Peilin Wu,Jinho D. Choi*

Main category: cs.CL

TLDR: 提出了一种基于文本的说话人分割新方法，利用句子级说话人变化检测，显著优于传统音频方法，尤其在短对话中表现突出。


<details>
  <summary>Details</summary>
Motivation: 音频说话人分割常受音质和说话人相似性限制，本文探索基于文本的替代方案。

Method: 开发了单预测模型（SPM）和多预测模型（MPM），利用对话文本检测说话人变化。

Result: 在多样化对话数据上，文本方法（尤其是MPM）表现优异，尤其在短对话中优于音频方法。

Conclusion: 展示了文本特征在说话人分割中的潜力，并强调语义理解的重要性，为未来多模态研究开辟方向。
...
### [172] [The Biased Samaritan: LLM biases in Perceived Kindness](https://arxiv.org/abs/2506.11361)
*Jack H Fagan,Ruhaan Juyaal,Amy Yue-Ming Yu,Siya Pun*

Main category: cs.CL

TLDR: 本文提出了一种评估生成式AI模型人口统计偏见的新方法，通过量化分析不同LLM对性别、种族和年龄的偏见，发现基准人口统计（白人中年或年轻男性）与非基准人口统计之间的差异。


<details>
  <summary>Details</summary>
Motivation: 理解和减轻LLM的偏见是当前的重要问题，本文旨在通过定量评估不同LLM的偏见，为开发者和用户提供客观的偏见评估工具。

Method: 通过提示模型评估道德患者的干预意愿，量化分析LLM对不同性别、种族和年龄的偏见，并确定基准人口统计与其他人口统计的关系。

Result: 研究发现模型通常将基准人口统计视为白人中年或年轻男性，但非基准人口统计更愿意提供帮助。

Conclusion: 本文方法有助于区分两种常被混淆的偏见，为LLM的偏见评估和未来训练提供了参考。
...
### [173] [A Variational Approach for Mitigating Entity Bias in Relation Extraction](https://arxiv.org/abs/2506.11381)
*Samuel Mensah,Elena Kochkina,Jabez Magomere,Joy Prakash Sain,Simerjot Kaur,Charese Smiley*

Main category: cs.CL

TLDR: 论文提出了一种基于变分信息瓶颈（VIB）框架的新方法，用于减少关系抽取（RE）中的实体偏差，提升模型的泛化能力。
...
### [174] [Curriculum-Guided Layer Scaling for Language Model Pretraining](https://arxiv.org/abs/2506.11389)
*Karanpartap Singh,Neil Band,Ehsan Adeli*

Main category: cs.CL

TLDR: CGLS是一种通过逐步增加模型深度和数据难度来提高预训练效率的方法，在多个基准测试中表现优于基线方法。
...
Method: 提出Curriculum-Guided Layer Scaling (CGLS)，通过逐步堆叠层（逐步增加模型深度）和调整数据难度（从简单到复杂）来优化预训练。

Result: 在100M和1.2B参数规模下，CGLS在PIQA、ARC等基准测试中表现优于基线方法，并提升了零样本性能。

Conclusion: CGLS通过渐进式堆叠层和数据难度调整，显著提升了模型在知识密集和推理任务中的泛化能力。
...
### [175] [Predicting Early-Onset Colorectal Cancer with Large Language Models](https://arxiv.org/abs/2506.11410)
*Wilson Lau,Youngwon Kim,Sravanthi Parasa,Md Enamul Haque,Anand Oka,Jay Nanduri*

Main category: cs.CL

TLDR: 论文研究了早发性结直肠癌（EoCRC）的预测，比较了10种机器学习模型与大型语言模型（LLM）的性能，发现微调后的LLM表现最佳。
...
Method: 应用10种机器学习模型和LLM，利用患者诊断前6个月的病情、实验室结果和观察数据进行预测。

Result: 微调后的LLM平均敏感度为73%，特异度为91%。

Conclusion: LLM在EoCRC预测中表现优异，可能成为早期筛查的有力工具。
...
### [176] [Efficient Long-Context LLM Inference via KV Cache Clustering](https://arxiv.org/abs/2506.11418)
*Jie Hu,Shengnan Wang,Yutong He,Ping Gong,Jiawei Yi,Juncheng Zhang,Youhui Bai,Renhai Chen,Gong Zhang,Cheng Li,Kun Yuan*

Main category: cs.CL

TLDR: Chelsea是一个在线KV缓存聚类框架，通过分块软匹配和聚类合并，显著减少KV缓存内存使用，同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 解决长上下文LLMs中KV缓存的高内存需求和计算开销问题。

Method: 将序列分块，采用分块软匹配策略进行聚类，合并KV缓存。
...
### [177] [Agent-RLVR: Training Software Engineering Agents via Guidance and Environment Rewards](https://arxiv.org/abs/2506.11425)
*Jeff Da,Clinton Wang,Xiang Deng,Yuntao Ma,Nikhil Barhate,Sean Hendryx*

Main category: cs.CL

TLDR: Agent-RLVR是一种改进的强化学习方法，通过引入代理指导机制，在复杂的代理环境中提升RLVR的效果，显著提高了任务完成率。


<details>
  <summary>Details</summary>
Motivation: 传统RLVR在多步复杂问题解决环境中效果不佳，奖励稀疏导致模型训练困难，需要一种新方法来提升性能。

Method: 提出Agent-RLVR框架，结合代理指导机制，利用多样信息线索（如策略计划和动态反馈）引导代理完成任务，并通过RLVR更新策略。
...
### [178] [KoGEC : Korean Grammatical Error Correction with Pre-trained Translation Models](https://arxiv.org/abs/2506.11432)
*Taeeun Kim,Semin Jeong,Youngsook Song*

Main category: cs.CL

TLDR: KoGEC是一个基于预训练翻译模型的韩语语法纠错系统，通过微调NLLB模型，在韩语GEC任务中表现优于GPT-4和HCX-3，并开发了Chrome扩展以提升实用性。


<details>
  <summary>Details</summary>
Motivation: 解决韩语语法纠错任务中大型语言模型（如GPT-4）的局限性，提供更高效的专用解决方案。

Method: 微调NLLB模型，使用特殊语言标记区分原始和纠正句子，并通过BLEU分数和LLM评估方法进行性能测试。

Result: KoGEC在韩语GEC任务中表现优于GPT-4和HCX-3，尤其在标点错误纠正上更均衡。

Conclusion: 研究表明专用模型在特定NLP任务中可媲美大型通用模型，同时提出了新的评估方法和实用工具。
...
### [179] [AbsenceBench: Language Models Can't Tell What's Missing](https://arxiv.org/abs/2506.11440)
*Harvey Yiyun Fu,Aryan Shrivastava,Jared Moore,Peter West,Chenhao Tan,Ari Holtzman*

Main category: cs.CL

TLDR: 大语言模型（LLMs）在长文本中定位特定信息表现出色，但在检测缺失信息时表现不佳。AbsenceBench评估了LLMs在三个领域中识别缺失信息的能力，结果显示即使先进模型如Claude-3.7-Sonnet的F1分数仅为69.6%。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在处理缺失信息时的局限性，揭示其与已表现出色的任务（如NIAH）之间的差距。

Method: 通过AbsenceBench测试LLMs在数值序列、诗歌和GitHub拉取请求中识别缺失信息的能力。

Result: LLMs在检测缺失信息时表现不佳，F1分数为69.6%，表明Transformer注意力机制难以处理文档中的“空白”。

Conclusion: LLMs在特定任务（如NIAH）中表现超人类，但在检测缺失信息时存在根本性限制。
...
### [180] [A Gamified Evaluation and Recruitment Platform for Low Resource Language Machine Translation Systems](https://arxiv.org/abs/2506.11467)
*Carlos Rafael Catalan*

Main category: cs.CL

TLDR: 论文探讨了在低资源语言机器翻译系统中，如何解决数据集和评估者短缺的问题，提出了一个招募和游戏化评估平台的设计方案。


<details>
  <summary>Details</summary>
Motivation: 由于低资源语言的数据集和评估者稀缺，传统自动化指标无法全面评估机器翻译系统的表现，因此需要设计一种解决方案。

Method: 论文首先综述现有评估方法，然后提出一个平台设计方案，用于招募评估者并通过游戏化方式评估机器翻译系统。

Result: 设计了一个招募和游戏化评估平台，解决了低资源语言机器翻译系统中的资源短缺问题。

Conclusion: 该平台不仅适用于机器翻译系统评估，还可扩展到更广泛的自然语言处理研究领域，但实施中仍需面对一些挑战。
...
### [181] [Med-PRM: Medical Reasoning Models with Stepwise, Guideline-verified Process Rewards](https://arxiv.org/abs/2506.11474)
...
TLDR: Med-PRM框架通过检索增强生成技术验证推理步骤，提升临床决策准确性。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在临床决策中难以定位和纠正推理错误，而医学领域需精确识别和修正错误以确保诊断准确性。

Method: 提出Med-PRM框架，利用检索增强生成技术，基于医学知识库验证每个推理步骤。

Result: 在五个医学QA基准和两个开放诊断任务中表现优异，基础模型性能提升达13.50%，小规模模型在MedQA上首次突破80%准确率。

Conclusion: Med-PRM通过细粒度验证推理步骤，显著提升临床决策性能，并展示出通用性和可扩展性。
...
### [182] [ImmunoFOMO: Are Language Models missing what oncologists see?](https://arxiv.org/abs/2506.11478)
*Aman Sinha,Bogdan-Valentin Popescu,Xavier Coubez,Marianne Clausel,Mathieu Constant*

Main category: cs.CL

TLDR: 研究探讨了语言模型在医学领域的应用，特别是针对乳腺癌免疫治疗摘要中特定概念的识别能力，发现预训练模型在识别低层次概念上优于大型语言模型。
...
Method: 通过比较不同语言模型与临床专家在识别乳腺癌免疫治疗摘要中的标志性概念的能力。

Result: 预训练语言模型在识别非常具体（低层次）的概念上表现优于大型语言模型。

Conclusion: 预训练语言模型在医学特定任务中具有潜在优势，尤其是在识别低层次概念时。
...
### [183] [Relational Schemata in BERT Are Inducible, Not Emergent: A Study of Performance vs. Competence in Language Models](https://arxiv.org/abs/2506.11485)
*Cole Gawin*

Main category: cs.CL

TLDR: 研究探讨BERT是否通过预训练编码了抽象关系模式，发现其高分类准确性表明潜在关系信号，但关系类型的组织需通过微调任务实现。


<details>
  <summary>Details</summary>
Motivation: 探究大型语言模型（如BERT）的语义任务表现是否反映真正的概念能力，还是仅基于表面统计关联。

Method: 通过分析BERT内部表示，比较概念对在分类任务中的表现与[CLS]标记嵌入的结构。

Result: 预训练BERT具有高分类准确性，但关系类型的组织需通过微调任务实现。
...
### [184] [Lag-Relative Sparse Attention In Long Context Training](https://arxiv.org/abs/2506.11498)
*Manlai Liang,Wanyi Huang,Mandi Liu,Huaijun Li,Jinlong Li*

Main category: cs.CL

TLDR: 论文提出了一种名为Lag-Relative Sparse Attention（LRSA）的方法，结合LagKV压缩技术，用于解决大语言模型（LLMs）处理长上下文输入时的计算和内存问题。


<details>
  <summary>Details</summary>
Motivation: LLMs在处理长上下文输入时，由于注意力计算的二次复杂性和线性增加的内存占用，性能受限。现有的键值缓存压缩技术虽能减少计算和内存开销，但会导致性能下降，且不适用于梯度优化。

Method: 提出LRSA方法，通过LagKV压缩技术，以固定大小的滞后窗口选择最相关的键值对，实现分块预填充，使模型能高效关注关键历史上下文。

Result: 实验表明，该方法显著提升了LLM在键值压缩下的鲁棒性，并在问答微调任务中取得了更好的结果。

Conclusion: LRSA方法在无需额外参数和低计算开销的情况下，有效解决了长上下文训练中的性能问题。
...
### [185] [On the Effectiveness of Integration Methods for Multimodal Dialogue Response Retrieval](https://arxiv.org/abs/2506.11499)
*Seongbo Jang,Seonghyeon Lee,Dongha Lee,Hwanjo Yu*

Main category: cs.CL

TLDR: 该论文探讨了多模态对话系统的响应生成方法，比较了两步法和端到端法的优劣，并验证了参数共享策略的有效性。


<details>
  <summary>Details</summary>
Motivation: 研究多模态对话系统中如何生成包含文本和图像等多种模态的响应，以提升对话系统的表现。

Method: 提出了三种集成方法，包括两步法和端到端法，并比较其优缺点。同时，引入了参数共享策略以减少参数数量并提升性能。

Result: 实验结果表明，端到端方法在不依赖中间步骤的情况下表现与两步法相当，参数共享策略不仅减少了参数数量，还通过跨任务和跨模态的知识迁移提升了性能。

Conclusion: 端到端方法和参数共享策略在多模态对话系统中具有潜力，能够在不增加复杂性的情况下提升性能。
...
### [186] [From Persona to Person: Enhancing the Naturalness with Multiple Discourse Relations Graph Learning in Personalized Dialogue Generation](https://arxiv.org/abs/2506.11557)
*Chih-Hao Hsu,Ying-Jia Lin,Hung-Yu Kao*

Main category: cs.CL

TLDR: 论文提出MUDI方法，通过结构化对话图和新型注意力策略提升个性化对话生成的连贯性和自然性。
...
Method: 利用大语言模型标注话语关系并构建对话图，提出DialogueGAT编码器捕捉隐含关系，解码时采用连贯感知注意力策略。

Result: 实验显示个性化响应质量显著提升，更接近人类对话。

Conclusion: MUDI方法有效提升个性化对话生成的连贯性和自然性。
...
### [187] [Are LLMs Good Text Diacritizers? An Arabic and Yorùbá Case Study](https://arxiv.org/abs/2506.11602)
*Hawau Olamide Toyin,Samar M. Magdy,Hanan Aldarmaki*

Main category: cs.CL

TLDR: 研究评估了大型语言模型（LLMs）在阿拉伯语和约鲁巴语文本标注中的效果，发现许多现成LLMs优于专用模型，但小模型易产生幻觉，微调可提升性能。


<details>
  <summary>Details</summary>
Motivation: 探讨LLMs在多语言文本标注任务中的表现，特别是阿拉伯语和约鲁巴语这两种类型学差异较大的语言。

Method: 引入多语言数据集MultiDiac，评估14种LLMs和6种专用模型，并对4种小模型进行LoRA微调。

Result: 现成LLMs在两种语言中表现优于专用模型，但小模型易产生幻觉；微调可提升性能并减少幻觉。

Conclusion: LLMs在文本标注任务中具有潜力，微调小模型可优化性能。
...
### [188] [SceneGram: Conceptualizing and Describing Tangrams in Scene Context](https://arxiv.org/abs/2506.11631)
*Simeon Junker,Sina Zarrieß*

Main category: cs.CL

TLDR: 论文研究了场景上下文对人类概念化抽象形状的影响，并提出了SceneGram数据集，分析了多模态LLM在生成参考时的局限性。


<details>
  <summary>Details</summary>
Motivation: 探讨场景上下文如何影响人类对抽象形状的概念化和命名，以及多模态LLM是否能捕捉人类参考的丰富性和变异性。

Method: 构建SceneGram数据集，包含不同场景上下文中的抽象形状参考，并分析多模态LLM生成的参考。

Result: 多模态LLM未能充分反映人类参考的丰富性和变异性。

Conclusion: 场景上下文对人类概念化有显著影响，而多模态LLM在此方面存在不足。
...
### [189] [LoRA-Gen: Specializing Large Language Model via Online LoRA Generation](https://arxiv.org/abs/2506.11638)
*Yicheng Xiao,Lin Song,Rui Yang,Cheng Cheng,Yixiao Ge,Xiu Li,Ying Shan*

Main category: cs.CL

TLDR: LoRA-Gen框架利用云端大模型生成边缘侧模型的LoRA参数，通过重参数化技术实现灵活专业化，提升推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在领域特定任务（尤其是小型边缘侧模型）上效果和效率有限。

Method: 利用云端大模型生成LoRA参数，通过重参数化合并到边缘侧模型，减少输入上下文长度。

Result: 在推理任务中优于传统LoRA微调，实现2.1倍加速；在智能代理任务中压缩比达10.1倍。

Conclusion: LoRA-Gen在提升模型专业化和效率方面具有显著优势。
...
### [190] [Converting Annotated Clinical Cases into Structured Case Report Forms](https://arxiv.org/abs/2506.11666)
*Pietro Ferrazzi,Alberto Lavelli,Bernardo Magnini*

Main category: cs.CL

TLDR: 为解决CRF数据集稀缺问题，研究者提出将信息提取任务数据集转化为结构化CRF，并开发了一种半自动转换方法，应用于E3C数据集，生成了高质量的CRF槽填充数据集。实验显示，槽填充任务对大型语言模型具有挑战性。
...
Method: 提出一种半自动转换方法，将信息提取任务数据集转化为结构化CRF，并应用于E3C数据集。

Result: 生成的CRF数据集上，槽填充任务在意大利语和英语中的准确率分别为59.7%和67.3%，且开源模型表现更差。

Conclusion: 填充CRF对当前最先进的大型语言模型仍具挑战性，研究团队公开了数据集以促进进一步研究。
...
### [191] [Improving Causal Interventions in Amnesic Probing with Mean Projection or LEACE](https://arxiv.org/abs/2506.11673)
*Alicja Dobrzeniecka,Antske Fokkens,Pia Sommerauer*

Main category: cs.CL

TLDR: Amnesic probing通过移除特定语言信息来评估模型行为，但现有方法（如INLP）可能引入随机修改。MP和LEACE两种新方法能更精准地移除信息，提升行为解释能力。


<details>
  <summary>Details</summary>
Motivation: 研究如何更精准地移除特定语言信息，以准确评估其对模型行为的影响。

Method: 提出Mean Projection (MP)和LEACE两种方法，用于更精准地移除目标信息。

Result: MP和LEACE相比INLP能更针对性地移除信息，减少对模型的随机修改。

Conclusion: MP和LEACE提升了Amnesic Probing的行为解释能力，为模型分析提供了更可靠的工具。
...
### [192] [LLMs for Sentence Simplification: A Hybrid Multi-Agent prompting Approach](https://arxiv.org/abs/2506.11681)
*Pratibha Zunjare,Michael Hsiao*

Main category: cs.CL

TLDR: 论文提出了一种结合高级提示和多智能体架构的混合方法，用于将复杂句子转换为逻辑简化的句子序列，实验结果显示其成功率显著高于单智能体方法。


<details>
  <summary>Details</summary>
Motivation: 解决在保持语义和逻辑完整性的前提下，将复杂句子简化为逻辑清晰的序列的挑战。
...
### [193] [Configurable Preference Tuning with Rubric-Guided Synthetic Data](https://arxiv.org/abs/2506.11702)
*Víctor Gallego*

Main category: cs.CL

TLDR: 论文提出Configurable Preference Tuning (CPT)框架，通过动态调整语言模型行为以适应人类可解释的指令，解决了传统静态偏好的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统AI对齐模型（如DPO）假设偏好是静态且单一的，限制了适应性。CPT旨在通过动态调整模型行为，实现更细粒度和上下文相关的反馈。
...
Conclusion: CPT为语言模型提供了动态调整行为的能力，提升了适应性和控制力，为AI对齐提供了新思路。
...
### [194] [The Cambrian Explosion of Mixed-Precision Matrix Multiplication for Quantized Deep Learning Inference](https://arxiv.org/abs/2506.11728)
*Héctor Martínez,Adrián Castelló,Francisco D. Igual,Enrique S. Quintana-Ortí*

Main category: cs.CL

TLDR: 论文探讨了深度学习（DL）中从传统FP64转向低精度格式（如FP16、BF16、8/16位整数）的趋势，并提出了适应混合精度整数（MIP）算术的高性能矩阵乘法（gemm）优化策略。


<details>
  <summary>Details</summary>
Motivation: 随着硬件架构向混合精度点积操作演进，传统SIMD优化方法已不适用，需重新设计gemm以适应现代ISA（如x86_64、ARM、RISC-V）。

Method: 提出新型微内核设计和数据布局，以充分利用现代专用硬件，并在三种CPU架构上验证MIP算术的性能优势。

Result: 实验表明，MIP算术在性能上显著优于浮点实现，为DL推理提供了高效解决方案。

Conclusion: 论文标志着矩阵乘法优化的新时代，称为“寒武纪时期”，为异构架构上的DL推理需求提供了新方向。
...
### [195] [DART: Distilling Autoregressive Reasoning to Silent Thought](https://arxiv.org/abs/2506.11752)
*Nan Jiang,Ziming Wu,De-Chuan Zhan,Fuming Lai,Shaobing Lian*

Main category: cs.CL

TLDR: DART通过自蒸馏框架，用非自回归的Silent Thought（ST）替代自回归的Chain-of-Thought（CoT），显著提升了推理效率。


<details>
  <summary>Details</summary>
Motivation: 自回归CoT推理计算开销大，限制了其在延迟敏感应用中的部署。

Method: DART引入两条训练路径：CoT路径和ST路径，后者通过轻量级REM模块对齐隐藏状态，直接生成答案。

Result: 实验显示DART推理性能与基线相当，同时显著提升效率。
...
### [196] [DeepResearch Bench: A Comprehensive Benchmark for Deep Research Agents](https://arxiv.org/abs/2506.11763)
*Mingxuan Du,Benfeng Xu,Chiwei Zhu,Xiaorui Wang,Zhendong Mao*

Main category: cs.CL

TLDR: DeepResearch Bench是一个用于评估基于LLM的深度研究代理（DRA）能力的基准测试，包含100个博士级研究任务，并提出两种评估方法。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏系统性评估深度研究代理能力的基准，阻碍了其发展。

Method: 提出两种评估方法：基于参考的自适应标准评估报告质量，以及评估信息检索和引用准确性的框架。

Result: 开发了DeepResearch Bench并开源，以加速基于LLM的代理发展。

Conclusion: 该基准和方法填补了评估DRA能力的空白，促进了实用LLM代理的研究。
...
### [197] [Long-Short Alignment for Effective Long-Context Modeling in LLMs](https://arxiv.org/abs/2506.11769)
*Tianqi Du,Haotian Huang,Yifei Wang,Yisen Wang*

Main category: cs.CL

TLDR: 论文提出了一种新视角，关注输出分布而非输入特征，以解决大语言模型在长上下文建模中的长度泛化问题。通过提出长短期对齐概念和度量标准，并开发正则化方法，实验验证了其有效性。
...
Method: 通过合成任务案例研究，提出长短期对齐概念，并设计Long-Short Misalignment度量标准。进一步开发正则化方法，促进训练中的对齐。

Result: 实验表明，长短期对齐与长度泛化性能强相关，正则化方法显著提升了模型表现。
...
### [198] [Persona-driven Simulation of Voting Behavior in the European Parliament with Large Language Models](https://arxiv.org/abs/2506.11798)
*Maximilian Kreutner,Marlene Lutz,Markus Strohmaier*

Main category: cs.CL

TLDR: 研究探讨了零样本角色提示是否能准确预测欧洲议会议员的投票行为，并发现其预测效果较好（F1分数约0.793）。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在政治话语中表现出进步左倾偏见，研究旨在验证角色提示是否能准确模拟个体投票行为及群体政策立场。

Method: 使用零样本角色提示和有限信息，分析其对欧洲议会议员投票行为的预测能力，并评估其对反事实论据、不同角色提示和生成方法的稳定性。

Result: 研究发现，角色提示能较好地模拟欧洲议会议员的投票行为，加权F1分数约为0.793。

Conclusion: 角色提示可用于预测政治行为，为政策立场模拟提供了一种有效方法。
...
### [199] [Are Multimodal Large Language Models Pragmatically Competent Listeners in Simple Reference Resolution Tasks?](https://arxiv.org/abs/2506.11807)
*Simeon Junker,Manar Ali,Larissa Koch,Sina Zarrieß,Hendrik Buschmeier*

Main category: cs.CL

TLDR: 研究多模态大语言模型在简单抽象视觉刺激（如色块和色格）的指代消解任务中的语言能力，发现其基础语用能力仍面临挑战。


<details>
  <summary>Details</summary>
Motivation: 探讨多模态大语言模型在简单但抽象的视觉任务中的语用能力，作为其实际应用能力的测试。

Method: 通过指代消解任务，使用色块和色格等简单视觉刺激，评估模型的语用能力。

Result: 结果显示，即使是基础语用能力（如上下文相关的颜色描述解释），对当前最先进的多模态大语言模型仍具挑战性。

Conclusion: 多模态大语言模型在基础语用能力方面仍需改进，尤其是在处理抽象视觉刺激时。
...
### [200] [Post Persona Alignment for Multi-Session Dialogue Generation](https://arxiv.org/abs/2506.11857)
*Yi-Pei Chen,Noriki Nishida,Hideki Nakayama,Yuji Matsumoto*

Main category: cs.CL

TLDR: 论文提出了一种名为PPA的两阶段框架，通过后验对齐策略提升多轮对话中一致性和多样性。


<details>
  <summary>Details</summary>
Motivation: 解决多轮对话中LLMs在长期一致性和个性化响应生成上的不足。

Method: PPA首先生成通用响应，再检索相关记忆并调整响应以对齐角色。

Result: PPA在一致性、多样性和角色相关性上显著优于现有方法。
...
### [201] [Beyond Homogeneous Attention: Memory-Efficient LLMs via Fourier-Approximated KV Cache](https://arxiv.org/abs/2506.11886)
...
TLDR: FourierAttention是一种无需训练的方法，通过利用Transformer头维度的异质性，将长上下文不敏感的维度投影到正交傅里叶基上，以减少KV缓存的内存需求。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理长上下文时，KV缓存的内存需求增加，现有压缩方法通常牺牲准确性或引入计算开销。

Method: 提出FourierAttention，利用Transformer头维度的异质性，将长上下文不敏感的维度投影到正交傅里叶基上，并用固定长度的频谱系数近似其时间演化。

Result: 在LLaMA模型上评估，FourierAttention在LongBench和NIAH任务上实现了最佳长上下文准确性，并通过FlashFourierAttention优化内存。

Conclusion: FourierAttention是一种高效且无需训练的方法，显著减少了内存需求，同时保持了性能。
...
### [202] [GeistBERT: Breathing Life into German NLP](https://arxiv.org/abs/2506.11903)
*Raphael Scheible-Schmitt,Johann Frei*

Main category: cs.CL

TLDR: GeistBERT通过增量训练和优化，提升了德语NLP任务的性能，并在多个任务中达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 德语NLP需要针对其语言特性的现代数据集和架构改进。

Method: 基于GottBERT权重，使用fairseq和WWM预训练，并扩展了长上下文模型。

Result: 在NER和文本分类任务中表现优异，超越基线模型并达到SOTA。

Conclusion: GeistBERT为德语NLP研究提供了高性能模型，并开源以支持社区。
...
### [203] [Effectiveness of Counter-Speech against Abusive Content: A Multidimensional Annotation and Classification Study](https://arxiv.org/abs/2506.11919)
*Greta Damo,Elena Cabrio,Serena Villata*

Main category: cs.CL

TLDR: 提出了一种基于社会科学概念的反仇恨言论（CS）有效性分类框架，定义了六个核心维度，并标注了4,214个CS实例。提出了两种分类策略，性能优于基线。


<details>
  <summary>Details</summary>
Motivation: 评估反仇恨言论的有效性标准尚不明确，需要一种系统化的方法。

Method: 提出了一个计算框架，定义了六个核心维度（清晰性、证据、情感诉求、反驳、受众适应性和公平性），并标注了4,214个CS实例。提出了多任务和依赖关系两种分类策略。

Result: 两种分类策略在专家和用户撰写的CS上均表现优异（平均F1分别为0.94和0.96），优于基线，并揭示了维度间的强相关性。

Conclusion: 该框架为反仇恨言论的有效性评估提供了新的工具和资源，揭示了维度间的相互依赖关系。
...
### [204] [Feedback Friction: LLMs Struggle to Fully Incorporate External Feedback](https://arxiv.org/abs/2506.11930)
*Dongwei Jiang,Alvin Zhang,Andrew Wang,Nicholas Andrews,Daniel Khashabi*

Main category: cs.CL

TLDR: 研究发现，即使在大模型（LLMs）接收到近乎完美的外部反馈时，它们仍难以完全整合反馈并纠正错误答案，这种现象被称为“反馈摩擦”。


<details>
  <summary>Details</summary>
Motivation: 探讨LLMs在理想条件下整合外部反馈的能力，以验证其自我改进的潜力。

Method: 设计实验环境，通过反馈生成器提供近乎完整的真实答案反馈，并观察模型在数学推理、知识推理等任务中的表现。

Result: 模型表现出对反馈的抵抗（反馈摩擦），尝试的改进策略效果有限。
...
### [205] [Improving Large Language Model Safety with Contrastive Representation Learning](https://arxiv.org/abs/2506.11938)
*Samuel Simko,Mrinmaya Sachan,Bernhard Schölkopf,Zhijing Jin*

Main category: cs.CL

TLDR: 提出了一种基于对比表示学习（CRL）的防御框架，通过三元组损失和对抗性硬负样本挖掘，增强模型对恶意输入的鲁棒性。
...
Method: 将模型防御问题转化为对比表示学习问题，使用三元组损失和对抗性硬负样本挖掘来分离良性表示和有害表示。

Result: 实验表明，该方法在多个模型上优于现有基于表示工程的防御方法，提高了对输入级和嵌入空间攻击的鲁棒性，且不影响标准性能。

Conclusion: 提出的CRL框架为LLMs的对抗防御提供了有效且通用的解决方案。
...
### [206] [code_transformed: The Influence of Large Language Models on Code](https://arxiv.org/abs/2506.12014)
*Yuliang Xu,Siming Huang,Mingmeng Geng,Yao Wan,Xuanhua Shi,Dongping Chen*

Main category: cs.CL

TLDR: 研究探讨了大型语言模型（LLMs）对代码风格的影响，包括命名规范、复杂性、可维护性和相似性，发现LLMs显著改变了编程风格。
...
Method: 通过分析2020至2025年间19,000多个GitHub仓库的代码，研究LLMs对命名规范（如snake_case使用率上升）和算法问题处理方式的影响。

Result: 研究发现LLMs显著改变了代码风格，例如Python中snake_case变量名比例从2023年的47%升至2025年的51%。

Conclusion: LLMs对实际编程风格产生了可测量的大规模影响，但难以精确估计其生成或辅助代码的比例。
...
### [207] [A Survey of Task-Oriented Knowledge Graph Reasoning: Status, Applications, and Prospects](https://arxiv.org/abs/2506.11012)
*Guanglin Niu,Bo Li,Yangguang Lin*

Main category: cs.AI

TLDR: 该论文综述了知识图谱推理（KGR）的研究现状，系统分类了六种KGR任务，并探讨了下游应用和挑战性推理范式，同时分析了LLMs等先进技术对KGR的影响。


<details>
  <summary>Details</summary>
Motivation: 现有综述未全面涵盖KGR任务及其下游应用和挑战性推理范式，因此需要更系统的总结。

Method: 基于主要推理任务、下游应用任务和潜在挑战性任务对KGR方法进行分类，并分析LLMs等先进技术的影响。

Result: 提供了KGR领域的全面视角，突出了研究趋势和未来方向。

Conclusion: 该综述为KGR研究提供了系统框架，并指出了未来发展的潜在方向。
...
### [208] [OntoGSN: An Ontology for Dynamic Management of Assurance Cases](https://arxiv.org/abs/2506.11023)
*Tomas Bueno Momcilovic,Barbara Gallina,Ingmar Kessler,Dian Balta*

Main category: cs.AI

TLDR: OntoGSN是一个基于GSN标准的本体论和中间件，用于管理保证案例（ACs），提供知识表示和可查询图，支持自动填充、评估和更新。


<details>
  <summary>Details</summary>
Motivation: 管理ACs在动态环境中（如自动驾驶）具有挑战性，现有工具难以维护嵌入的知识，导致开发效率低或虚假信心。

Method: 提出OntoGSN，包括GSN标准的OWL本体、辅助本体、SPARQL查询库和原型界面，严格遵循标准并经过多维度评估。

Result: OntoGSN支持动态AC管理，并通过案例（如大语言模型的对抗鲁棒性）验证其实用性。

Conclusion: OntoGSN为AC管理提供了一种高效、自动化的解决方案，适用于动态环境，并具有社区驱动的持续发展潜力。
...
### [209] [LLM-as-a-Fuzzy-Judge: Fine-Tuning Large Language Models as a Clinical Evaluation Judge with Fuzzy Logic](https://arxiv.org/abs/2506.11221)
*Weibing Zheng,Laurah Turner,Jess Kropczynski,Murat Ozer,Tri Nguyen,Shane Halse*

Main category: cs.AI

TLDR: 论文提出了一种结合模糊逻辑和大型语言模型（LLM）的方法（LLM-as-a-Fuzzy-Judge），用于自动化评估医学生的临床沟通技能，并与医生的主观偏好对齐。
...
Conclusion: 研究表明，模糊逻辑与LLM的结合能够有效对齐人类偏好，推动医学教育中自动化评估的发展，并支持更稳健的评估实践。
...
### [210] [MUDAS: Mote-scale Unsupervised Domain Adaptation in Multi-label Sound Classification](https://arxiv.org/abs/2506.11331)
*Jihoon Yun,Chengzhang Li,Dhrubojyoti Roy,Anish Arora*

Main category: cs.AI

TLDR: MUDAS是一种针对资源受限物联网设备的多标签声音分类的无监督域适应框架，通过选择性重训练和自适应阈值提升性能。
...
Result: 在SONYC-UST数据集上，MUDAS显著优于现有算法，适合资源受限的物联网设备。
...
### [211] [Benchmarking Multimodal LLMs on Recognition and Understanding over Chemical Tables](https://arxiv.org/abs/2506.11375)
*Yitong Zhou,Mingyue Cheng,Qingyang Mao,Yucong Luo,Qi Liu,Yupeng Li,Xiaohan Zhang,Deguang Liu,Xin Li,Enhong Chen*

Main category: cs.AI

TLDR: ChemTable是一个针对化学表格的大规模基准测试，旨在提升多模态大语言模型在化学领域的科学理解能力，包括表格识别和理解任务。
...
Method: 通过从文献实验部分收集真实化学表格，并标注单元格、逻辑布局和领域特定标签，构建ChemTable基准。

Result: 模型在基础布局解析上表现尚可，但在描述性和推理性QA任务中表现显著低于人类水平，开源与闭源模型间存在性能差距。

Conclusion: ChemTable为化学表格理解提供了严格且现实的基准，突显了化学感知理解的挑战。
...
### [212] [Large Language Model-Powered Conversational Agent Delivering Problem-Solving Therapy (PST) for Family Caregivers: Enhancing Empathy and Therapeutic Alliance Using In-Context Learning](https://arxiv.org/abs/2506.11376)
...
TLDR: 研究探讨了大型语言模型（LLM）驱动的对话代理如何为家庭护理人员提供基于证据的心理健康支持，结合了问题解决疗法（PST）、动机访谈（MI）和行为链分析（BCA）。实验显示，Few-Shot和RAG提示技术的模型表现最佳，提升了共情和治疗联盟。


<details>
  <summary>Details</summary>
Motivation: 家庭护理人员因多重角色和资源有限面临心理健康挑战，需探索LLM提供支持的可能性。

Method: 采用被试内实验，28名护理人员与四种LLM配置互动，评估共情和治疗联盟。最佳模型结合Few-Shot、RAG技术和临床示例。

Result: 模型在情境理解和个性化支持方面表现更优，参与者认可其情感验证和提供实用策略的能力，但评估与建议效率需平衡。

Conclusion: LLM在提供共情和定制化支持方面潜力显著，为家庭护理人员心理健康支持提供了新方向。
...
### [213] [FocalAD: Local Motion Planning for End-to-End Autonomous Driving](https://arxiv.org/abs/2506.11419)
...
TLDR: FocalAD是一个专注于局部交互的端到端自动驾驶框架，通过增强局部运动表示提升规划可靠性。
...
Result: 在nuScenes和Bench2Drive基准测试中表现优异，Adv-nuScenes数据集上碰撞率显著降低。
...
### [214] [Resolve Highway Conflict in Multi-Autonomous Vehicle Controls with Local State Attention](https://arxiv.org/abs/2506.11445)
*Xuan Duy Ta,Bang Giang Le,Thanh Ha Le,Viet Cuong Ta*

Main category: cs.AI

TLDR: 提出了一种基于局部状态注意力的模块，用于解决多智能体强化学习中的交通冲突问题，显著提高了高速公路合并场景的效率。


<details>
  <summary>Details</summary>
Motivation: 在混合交通环境中，自动驾驶车辆需适应人类驾驶车辆和突发情况，但现有方法难以解决智能体间的局部冲突和随机事件。

Method: 引入局部状态注意力模块，利用自注意力操作压缩附近智能体的关键信息，以解决交通冲突。

Result: 在模拟高速公路合并场景中，该方法显著提高了合并效率，尤其在交通密度高时表现优于基线方法。
...
### [215] [Structure-Aware Automatic Channel Pruning by Searching with Graph Embedding](https://arxiv.org/abs/2506.11469)
*Zifan Liu,Yuan Cao,Yanwei Yu,Heng Qi,Jie Gui*

Main category: cs.AI

TLDR: 提出了一种基于图卷积网络（GCN）的结构感知自动通道剪枝（SACP）框架，通过建模网络拓扑结构实现全局重要性评估，优于现有剪枝方法。
...
Method: 利用GCN建模网络拓扑，学习通道的全局重要性；限制剪枝率组合空间，动态调整组合数量，采用搜索方法确定最优组合。

Result: 在CIFAR-10和ImageNet数据集上，SACP在压缩效率和精度保持上优于现有方法。

Conclusion: SACP通过结构感知和自动化剪枝，显著提升了剪枝效果，适用于资源受限设备。
...
### [216] [Reviving DSP for Advanced Theorem Proving in the Era of Reasoning Models](https://arxiv.org/abs/2506.11487)
*Chenrui Cao,Liangcheng Song,Zenan Li,Xinyi Le,Xian Zhang,Hui Xue,Fan Yang*

Main category: cs.AI

TLDR: DSP+框架通过神经符号协调改进自动定理证明，无需额外训练即可达到与RL方法相当的性能。


<details>
  <summary>Details</summary>
Motivation: 探索无需大规模训练的神经符号协调方法在自动定理证明中的潜力。

Method: 改进DSP框架，分阶段精细整合神经符号方法：生成子目标、自动形式化、符号搜索。

Result: 在多个数据集上表现优异，解决未解决问题，并发现形式化错误。
...
### [217] [RAG+: Enhancing Retrieval-Augmented Generation with Application-Aware Reasoning](https://arxiv.org/abs/2506.11555)
*Yu Wang,Shiwan Zhao,Ming Fan,Zhihu Wang,Yubo Zhang,Xicheng Zhang,Zhengfan Wang,Heyuan Huang,Ting Liu*

Main category: cs.AI

TLDR: RAG+是一种改进的RAG方法，通过引入应用感知推理，显著提升了知识密集型任务中LLMs的表现。


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法忽略了知识应用的认知步骤，导致检索到的知识与任务推理之间存在差距。

Method: RAG+构建了一个包含知识和应用示例的双重语料库，并在推理过程中联合检索两者。

Result: 在数学、法律和医学领域的实验中，RAG+平均提升3-5%，复杂场景下峰值提升7.5%。

Conclusion: RAG+通过结合检索与可操作应用，为知识集成提供了更认知基础化的框架，推动了更可解释和强大的LLMs发展。
...
### [218] [Collaborative LLM Inference via Planning for Efficient Reasoning](https://arxiv.org/abs/2506.11578)
*Byeongchan Lee,Jonghoon Lee,Dongyoung Kim,Jaehyung Kim,Jinwoo Shin*

Main category: cs.AI

TLDR: 提出一种测试时协作框架，让小型和大型语言模型通过多轮计划与推理协作，以低成本实现高性能。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型（LLMs）成本高和小型模型推理能力不足的问题。

Method: 通过计划模型生成问题的高层抽象计划，推理模型基于计划生成完整解决方案，多轮协作。

Result: 达到与强商业模型相当的准确性，同时显著降低付费推理依赖。
...
### [219] [VLM@school -- Evaluation of AI image understanding on German middle school knowledge](https://arxiv.org/abs/2506.11604)
*René Peinl,Vincent Tischler*

Main category: cs.AI

TLDR: 论文介绍了一个新的德语基准数据集，用于评估视觉语言模型在结合视觉推理和学科背景知识的任务中的表现，结果显示当前模型表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有英语基准数据集常依赖脱离上下文的问题，而该数据集基于真实中学课程，旨在更真实地评估模型的多模态理解能力。

Method: 数据集包含2000多个开放性问题，基于486张图像，覆盖数学、历史等九大学科，并评估了13个先进模型。

Result: 最强模型总体准确率低于45%，在音乐、数学和对抗性问题中表现尤其差，表明现有基准与真实理解存在差距。

Conclusion: 中学任务是非英语环境下测试模型的宝贵资源，数据集为未来AI系统的视觉和语言推理能力提供了严格测试平台。
...
### [220] [Mitigating Hallucination Through Theory-Consistent Symmetric Multimodal Preference Optimization](https://arxiv.org/abs/2506.11712)
*Wenqi Liu,Xuemeng Song,Jiaxi Li,Yinwei Wei,Na Zheng,Jianhua Yin,Liqiang Nie*

Main category: cs.AI

TLDR: SymMPO是一种对称多模态偏好优化方法，通过直接偏好监督和严格的优化目标函数，有效减少多模态大语言模型的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法虽然通过视觉对比目标增强MLLMs对视觉输入的注意力，但仍存在优化目标函数不严谨和偏好监督间接的问题。

Method: 提出SymMPO，采用对称偏好学习和直接偏好监督（响应对），并引入偏好边际一致性损失来量化调节偏好差距。

Result: 在五个基准测试中表现优异，验证了其在减少MLLMs幻觉方面的有效性。

Conclusion: SymMPO通过对称偏好学习和直接监督，显著提升了MLLMs的视觉理解能力并减少了幻觉。
...
### [221] [Relational GNNs Cannot Learn $C_2$ Features for Planning](https://arxiv.org/abs/2506.11721)
*Dillon Z. Chen*

Main category: cs.AI

TLDR: R-GNNs无法学习由$C_2$特征定义的价值函数，但其他GNN架构可能更适合。
...
Method: 通过理论分析，探讨R-GNNs与$C_2$特征的关系，并对比其他GNN架构。
...
### [222] [Causal Effect Identification in Heterogeneous Environments from Higher-Order Moments](https://arxiv.org/abs/2506.11756)
*Yaroslav Kivva,Sina Akbari,Saber Salehkaleybar,Negar Kiyavash*

Main category: cs.AI

TLDR: 论文研究了在存在潜在混杂变量的情况下，如何估计处理变量对结果的因果效应。通过多环境数据，证明了因果效应在特定条件下的可识别性，并提出了一种基于矩的估计算法。


<details>
  <summary>Details</summary>
Motivation: 在因果推断中，潜在混杂变量可能导致估计偏差，因此需要一种方法在存在潜在混杂变量的情况下准确估计因果效应。

Method: 利用多环境数据，假设目标因果效应在不同环境中保持不变，提出基于矩的估计算法，并分析不同参数变化对可识别性的影响。

Result: 证明了因果效应在单一参数变化时可识别，但若潜在混杂变量和处理变量的外生噪声分布均变化，则失去可识别性。实验验证了方法的性能。

Conclusion: 通过多环境数据和特定假设，可以识别和估计因果效应，但需注意参数变化的限制。
...
### [223] [On the Performance of LLMs for Real Estate Appraisal](https://arxiv.org/abs/2506.11812)
*Margot Geerts,Manon Reusens,Bart Baesens,Seppe vanden Broucke,Jochen De Weerdt*

Main category: cs.AI

TLDR: 研究探讨了如何利用大语言模型（LLMs）通过优化的上下文学习策略生成具有竞争力的、可解释的房价估计，以解决房地产市场中信息不对称的问题。


<details>
  <summary>Details</summary>
Motivation: 房地产市场的关键性及其信息不对称问题促使研究探索LLMs在提供透明、可交互的房价估计方面的潜力。

Method: 系统评估了多种LLMs在国际住房数据集上的表现，比较了零样本、少样本、市场报告增强及混合提示技术。

Result: LLMs能有效利用特征变量生成有意义的估计，虽传统模型在预测准确性上更强，但LLMs提供了更易访问和解释的替代方案。
...
### [224] [Revealing Political Bias in LLMs through Structured Multi-Agent Debate](https://arxiv.org/abs/2506.11825)
*Aishwarya Bandaru,Fabian Bindley,Trevor Bluth,Nandini Chavda,Baixu Chen,Ethan Law*

Main category: cs.AI

TLDR: 研究探讨了大型语言模型（LLMs）在模拟社会行为中的政治偏见，发现中立模型偏向民主党，共和党模型向中立靠拢，性别影响态度，且相同政治倾向的代理会形成回音室。


<details>
  <summary>Details</summary>
Motivation: LLMs被广泛用于模拟社会行为，但其政治偏见和辩论中的互动动态尚未充分研究。

Method: 通过结构化多代理辩论框架，使用中立、共和党和民主党LLM代理在敏感政治话题上辩论，系统变化LLM类型、代理性别和辩论形式。

Result: 中立代理偏向民主党，共和党代理向中立靠拢；性别影响态度；相同政治倾向的代理形成回音室。

Conclusion: LLMs的政治偏见和互动动态受模型来源和代理属性影响，需进一步研究以减少偏见。
...
### [225] [Addressing Bias in LLMs: Strategies and Application to Fair AI-based Recruitment](https://arxiv.org/abs/2506.11880)
*Alejandro Peña,Julian Fierrez,Aythami Morales,Gonzalo Mancera,Miguel Lopez,Ruben Tolosana*

Main category: cs.AI

TLDR: 本文研究了基于Transformer的系统如何学习数据中的人口统计偏差，并提出了一种隐私增强框架以减少性别信息，从而减轻最终工具中的偏见行为。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型（LLMs）的成功应用，语言技术在高风险场景中的使用增加，但其存在伦理问题（如人口统计偏差）。本文旨在分析LLMs学习数据偏差的能力，并提出解决方案。

Method: 通过AI自动化招聘的案例研究，提出一种隐私增强框架，减少学习流程中的性别信息。实验分析了数据偏差对两种LLMs构建系统的影响。
...
### [226] [Towards a Cascaded LLM Framework for Cost-effective Human-AI Decision-Making](https://arxiv.org/abs/2506.11887)
*Claudio Fanconi,Mihaela van der Schaar*

Main category: cs.AI

TLDR: 提出了一种级联LLM决策框架，通过自适应任务分配平衡预测正确性、成本及置信度，结合在线学习机制优化决策质量。


<details>
  <summary>Details</summary>
Motivation: 平衡预测正确性、知识成本和推理复杂性，以及决定是否放弃自动化答案或引入人类专家。

Method: 两阶段级联决策：1) 延迟策略决定是否接受基础模型答案或使用更大模型重新生成；2) 放弃策略决定是否需要人类干预。结合在线学习机制。

Result: 在问答任务（ARC-Easy、ARC-Challenge）和医学问答（MedQA、MedMCQA）中，级联策略在多数情况下优于单模型基线，同时降低成本并处理放弃情况。

Conclusion: 级联LLM框架在准确性和成本效率上表现优越，并提供了一种处理放弃的合理方法。
...
### [227] [Schema-R1: A reasoning training approach for schema linking in Text-to-SQL Task](https://arxiv.org/abs/2506.11986)
*Wuzhenghong Wen,Su Pan,yuwei Sun*

Main category: cs.AI

TLDR: 论文提出了一种基于强化学习的Schema-R1模型，用于提升文本到SQL任务中的模式链接推理能力，相比现有方法在过滤准确率上提升了10%。


<details>
  <summary>Details</summary>
Motivation: 当前模式链接模型的微调方法过度优化真实结果，牺牲了推理能力，缺乏高质量推理样本。

Method: Schema-R1通过构建高质量推理样本小批量、监督微调冷启动初始化，以及基于规则的强化学习训练三个步骤实现。

Result: 方法显著提升了模式链接模型的推理能力，过滤准确率提高了10%。
...
### [228] [Tracing LLM Reasoning Processes with Strategic Games: A Framework for Planning, Revision, and Resource-Constrained Decision Making](https://arxiv.org/abs/2506.12012)
*Xiaopeng Yuan,Xingjian Zhang,Ke Xu,Yifan Xu,Lijun Yu,Jindong Wang,Yushun Dong,Haohan Wang*

Main category: cs.AI

TLDR: 论文提出通过战略游戏评估大语言模型（LLMs）的中间推理步骤，包括规划、修订和资源受限决策，并引入新指标。ChatGPT-o3-mini表现最佳，而Qwen-Plus因资源过度使用表现较差。


<details>
  <summary>Details</summary>
Motivation: 现有基准多关注最终结果，忽视了中间推理步骤（如规划、修订和资源受限决策），而这些对理解模型行为和提升可靠性至关重要。

Method: 使用战略游戏作为评估环境，提出框架评估LLMs在规划、修订和资源受限决策三个维度的表现，并定义多个新指标。

Result: 在4320轮对抗测试中，ChatGPT-o3-mini综合得分最高（胜率74.7%，修正成功率78.6%），而Qwen-Plus因资源过度使用表现较差（胜率25.6%）。
...
### [229] [Evaluation is All You Need: Strategic Overclaiming of LLM Reasoning Capabilities Through Evaluation Design](https://arxiv.org/abs/2506.04734)
...
TLDR: 研究发现Deepseek-R1-Distill系列模型及其衍生模型的基准评估结果受多种因素影响，波动较大，需建立更严格的评估范式。


<details>
  <summary>Details</summary>
Motivation: 揭示现有推理模型（如Deepseek-R1-Distill系列和QwQ-32B）评估结果的不稳定性，强调评估条件对结果的影响。

Method: 通过实证评估分析Deepseek-R1-Distill系列模型的性能波动。

Result: 评估结果受细微条件差异影响显著，性能改进难以可靠复现。

Conclusion: 呼吁建立更严格的模型性能评估范式，以确保结果的可信性和可比性。
...
### [230] [EfficientQuant: An Efficient Post-Training Quantization for CNN-Transformer Hybrid Models on Edge Devices](https://arxiv.org/abs/2506.11093)
*Shaibal Saha,Lanyu Xu*

Main category: cs.CV

TLDR: EfficientQuant是一种针对混合卷积-Transformer模型的结构感知后训练量化方法，显著降低延迟并保持高精度。


<details>
  <summary>Details</summary>
Motivation: 混合模型在CV任务中表现优异，但资源消耗大，传统量化方法对其效果有限。

Method: 采用均匀量化处理卷积块，$log_2$量化处理Transformer块。

Result: 在ImageNet-1K上实现2.5×-8.7×延迟降低，精度损失极小。

Conclusion: EfficientQuant在边缘设备上高效实用，适合实际部署。
...
### [231] [Adaptive Object Detection with ESRGAN-Enhanced Resolution & Faster R-CNN](https://arxiv.org/abs/2506.11122)
*Divya Swetha K,Ziaul Haque Choudhury,Hemanta Kumar Bhuyan,Biswajit Brahma,Nilayam Kumar Kamila*

Main category: cs.CV

TLDR: 提出了一种结合ESRGAN和Faster R-CNN的方法，用于从低分辨率图像中提升物体检测性能。
...
Conclusion: 该方法为图像质量不稳定的场景提供了有效的解决方案，平衡了图像质量和检测效率。
...
### [232] [Technical Report for Argoverse2 Scenario Mining Challenges on Iterative Error Correction and Spatially-Aware Prompting](https://arxiv.org/abs/2506.11124)
*Yifei Chen,Ross Greer*

Main category: cs.CV

TLDR: RefAV框架通过LLM将自然语言查询转换为可执行代码以挖掘自动驾驶场景，但面临代码错误和参数解释不准确的问题。本文提出两种改进：迭代代码生成机制和专用提示工程，实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 解决LLM生成代码的运行时错误和空间关系函数参数解释不准确的问题，提高自动驾驶场景挖掘的可靠性。

Method: 1. 引入容错迭代代码生成机制，通过错误反馈重新提示LLM优化代码；2. 采用专用提示工程提升LLM对空间关系函数的理解和应用。

Result: 在Argoverse 2验证集上实验显示，系统性能显著提升，使用Gemini 2.5 Pro时HOTA-Temporal得分达52.37。
...
### [233] [Image-Based Method For Measuring And Classification Of Iron Ore Pellets Using Star-Convex Polygons](https://arxiv.org/abs/2506.11126)
*Artem Solomko,Oleg Kartashev,Andrey Golov,Mikhail Deulin,Vadim Valynkin,Vasily Kharin*

Main category: cs.CV

TLDR: 本文提出了一种基于StarDist算法的铁矿石球团分类方法，用于检测质量违规，并改进了物理尺寸测量的准确性。


<details>
  <summary>Details</summary>
Motivation: 传统算法在密集和不稳定环境中对铁矿石球团的分类和测量效果不佳，需要更精确的方法。

Method: 利用StarDist算法分割物体、确定轮廓、分类并测量物理尺寸。

Result: 开发了一种能检测平滑边界物体的新方法，显著提高了尺寸分布分析的准确性。

Conclusion: StarDist算法为铁矿石球团的复杂分类和测量问题提供了有效解决方案。
...
### [234] [Segment This Thing: Foveated Tokenization for Efficient Point-Prompted Segmentation](https://arxiv.org/abs/2506.11131)
*Tanner Schmidt,Richard Newcombe*

Main category: cs.CV

TLDR: STT是一种高效的图像分割模型，通过单点提示生成单一分割区域，采用变分辨率补丁标记化减少计算成本。


<details>
  <summary>Details</summary>
Motivation: 提高图像分割效率，同时保持模型性能，适用于增强现实和机器人应用。

Method: 通过中心裁剪和变分辨率补丁标记化减少图像标记数量，降低计算成本。

Result: STT在保持分割性能的同时显著提高了效率，可在消费级硬件上实时运行。
...
### [235] [Gender Fairness of Machine Learning Algorithms for Pain Detection](https://arxiv.org/abs/2506.11132)
*Dylan Green,Yuting Shang,Jiaee Cheong,Yang Liu,Hatice Gunes*

Main category: cs.CV

TLDR: 该论文研究了基于机器学习和深度学习的自动疼痛检测模型在性别公平性上的表现，发现所有模型均存在性别偏见，尽管Vision Transformer（ViT）在准确性和部分公平性指标上表现最佳。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于评估自动疼痛检测算法在不同性别群体中的公平性，填补现有研究中关于算法偏见的空白。

Method: 方法包括比较传统机器学习算法（线性SVM和径向基SVM）与深度学习方法（CNN和ViT），使用UNBC-McMaster数据库的面部表情数据进行训练和评估。

Result: 结果显示ViT在准确性和部分公平性指标上表现最佳，但所有模型均存在性别偏见。

Conclusion: 结论强调了在自动医疗系统中需要采用公平性技术以减少偏见，同时指出准确性与公平性之间的权衡问题。
...
### [236] [Monocular 3D Hand Pose Estimation with Implicit Camera Alignment](https://arxiv.org/abs/2506.11133)
*Christos Pantazopoulos,Spyridon Thermos,Gerasimos Potamianos*

Main category: cs.CV

TLDR: 提出了一种从2D关键点输入估计3D手部关节的优化流程，无需相机参数，在EgoDexter和Dexter+Object基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 从单张彩色图像估计3D手部关节在AR、VR、HCI和机器人领域有广泛应用，但缺乏深度信息、遮挡和复杂关节结构等问题增加了难度。

Method: 提出包含关键点对齐步骤和指尖损失的优化流程，无需相机参数。
...
### [237] [ContextLoss: Context Information for Topology-Preserving Segmentation](https://arxiv.org/abs/2506.11134)
*Benedict Schacht,Imke Greving,Simone Frintrop,Berit Zeller-Plumhoff,Christian Wilms*

Main category: cs.CV

TLDR: 提出了一种新的损失函数ContextLoss（CLoss），通过考虑关键像素掩码中的上下文信息，提高了图像分割中拓扑结构的正确性，并在多个数据集上验证了其优越性。


<details>
  <summary>Details</summary>
Motivation: 在图像分割中，保持分割结构（如血管、膜或道路）的拓扑结构至关重要，尤其是拓扑错误可能严重影响实际应用（如导航）。现有方法基于关键像素掩码，但缺乏上下文信息。

Method: 提出ContextLoss（CLoss）损失函数，利用关键像素掩码中的上下文信息，增强网络对拓扑错误的关注。同时，提出了两种直观的指标来验证连通性改进。

Result: 在多个2D和3D数据集上验证，CLoss显著提高了拓扑感知指标的性能，修复了比其他方法多44%的缺失连接。

Conclusion: CLoss通过引入上下文信息有效提升了拓扑正确性，为图像分割中的拓扑问题提供了新的解决方案。
...
### [238] [JAFAR: Jack up Any Feature at Any Resolution](https://arxiv.org/abs/2506.11136)
*Paul Couairon,Loick Chambon,Louis Serrano,Jean-Emmanuel Haugeard,Matthieu Cord,Nicolas Thome*

Main category: cs.CV

TLDR: JAFAR是一种轻量级、灵活的特征上采样器，能够将基础视觉编码器的低分辨率特征提升到任意目标分辨率，无需高分辨率监督即可实现优异的性能。
...
Method: JAFAR采用基于注意力的模块，结合空间特征变换（SFT）调制，实现低分辨率特征与高分辨率查询的语义对齐。
...
### [239] [Autonomous Computer Vision Development with Agentic AI](https://arxiv.org/abs/2506.11140)
*Jin Kim,Muhammad Wahi-Anwa,Sangyun Park,Shawn Shin,John M. Hoffman,Matthew S. Brown*

Main category: cs.CV

TLDR: 论文展示了如何利用基于大型语言模型（LLM）的Agentic AI方法，从自然语言提示中自主构建一个专门的计算机视觉系统，并成功应用于医学图像分析任务。


<details>
  <summary>Details</summary>
Motivation: 探索Agentic AI系统在复杂推理、规划和工具配置中的潜力，特别是在计算机视觉任务中替代传统数据科学家的角色。

Method: 扩展了开源Cognitive AI环境SimpleMind，结合LLM代理（OpenManus实现），通过自然语言提示自动生成工具配置（YAML文件），并执行训练和推理脚本。

Result: 代理系统成功配置、训练和测试了50张胸部X光图像，肺部、心脏和肋骨的分割平均Dice分数分别为0.96、0.82和0.83。

Conclusion: 该工作证明了Agentic AI在计算机视觉应用中自主规划和工具配置的潜力，为自动化开发提供了新方向。
...
### [240] [FARCLUSS: Fuzzy Adaptive Rebalancing and Contrastive Uncertainty Learning for Semi-Supervised Semantic Segmentation](https://arxiv.org/abs/2506.11142)
*Ebenezer Tarubinga,Jenifer Kalafatovich*

Main category: cs.CV

TLDR: 提出了一种半监督语义分割框架，通过模糊伪标签、不确定性感知动态加权、自适应类别平衡和轻量对比正则化，有效利用未标记数据并提升分割性能。
...
Method: 采用模糊伪标签、不确定性感知动态加权、自适应类别平衡和轻量对比正则化四种组件。
...
### [241] [On the development of an AI performance and behavioural measures for teaching and classroom management](https://arxiv.org/abs/2506.11143)
...
TLDR: 该论文介绍了一个为期两年的研究项目，利用AI分析课堂动态，重点是通过多模态传感器数据捕捉教师行为。研究开发了实时数据分析工具，生成了音频-视觉数据集、行为测量方法和教学回顾仪表盘。初步评估显示系统清晰易用，支持教师反思。


<details>
  <summary>Details</summary>
Motivation: 旨在通过AI技术减轻教师手动分析课堂动态的负担，并提供客观反馈以改进教学策略。

Method: 利用多模态传感器数据实时捕捉课堂动态，结合AI技术提取行为指标，开发教学回顾仪表盘。

Result: 生成了音频-视觉数据集和新型行为测量方法，初步评估显示系统清晰、易用且支持教师反思。

Conclusion: 该系统为AI教育分析领域提供了文化背景下的方法论，支持教师改进教学策略。
...
### [242] [AlignHuman: Improving Motion and Fidelity via Timestep-Segment Preference Optimization for Audio-Driven Human Animation](https://arxiv.org/abs/2506.11144)
*Chao Liang,Jianwen Jiang,Wang Liao,Jiaqi Yang,Zerong zheng,Weihong Zeng,Han Liang*

Main category: cs.CV

TLDR: AlignHuman框架通过偏好优化和分治训练策略，优化了人类视频生成中运动自然性与视觉保真度的权衡，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 当前人类视频生成在运动自然性与视觉保真度之间存在权衡问题，AlignHuman旨在解决这一挑战。

Method: 提出AlignHuman框架，结合偏好优化（TPO）和分治训练策略，使用两个专用LoRAs模块分别优化不同时间段的运动动态和保真度。

Result: 实验表明，AlignHuman显著提升基线性能，推理速度提升3.3倍（从100 NFEs降至30 NFEs），且生成质量影响极小。

Conclusion: AlignHuman通过分治策略和偏好优化，有效平衡了运动自然性与视觉保真度，为人类视频生成提供了高效解决方案。
...
### [243] [3D-RAD: A Comprehensive 3D Radiology Med-VQA Dataset with Multi-Temporal Analysis and Diverse Diagnostic Tasks](https://arxiv.org/abs/2506.11147)
*Xiaotang Gai,Jiaxiang Liu,Yichen Li,Zijie Meng,Jian Wu,Zuozhu Liu*

Main category: cs.CV

TLDR: 3D-RAD是一个基于CT扫描的大规模3D医学视觉问答数据集，支持多样化的任务和复杂推理，现有模型表现有限，但微调可显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 推动3D医学视觉问答（Med-VQA）的发展，弥补现有研究在3D影像和任务多样性上的不足。

Method: 构建3D-RAD数据集，包含六种任务类型，支持开放和封闭问题，并引入复杂推理挑战。

Result: 现有视觉语言模型在3D任务中泛化能力有限，尤其是多时态任务，但微调后性能显著提升。

Conclusion: 3D-RAD为3D医学视觉理解提供了高质量基准，推动多模态医学AI研究。
...
### [244] [LLM-to-Phy3D: Physically Conform Online 3D Object Generation with LLMs](https://arxiv.org/abs/2506.11148)
*Melvin Wong,Yueming Lyu,Thiago Rios,Stefan Menzel,Yew-Soon Ong*

Main category: cs.CV

TLDR: LLM-to-Phy3D是一种新型方法，通过结合视觉和物理评估，使大型语言模型（LLMs）能够生成符合物理约束的3D对象，显著提升了物理性能。
...
Method: LLM-to-Phy3D引入了一种在线黑盒优化循环，通过迭代反馈优化提示，生成物理合规的3D对象。

Result: 实验表明，LLM-to-Phy3D在车辆设计优化中，物理合规性提升了4.5%至106.7%。
...
### [245] [Self-Calibrating BCIs: Ranking and Recovery of Mental Targets Without Labels](https://arxiv.org/abs/2506.11151)
*Jonathan Grizou,Carlos de la Torre-Ortiz,Tuukka Ruotsalo*

Main category: cs.CV

TLDR: 论文提出了一种名为CURSOR的算法，无需标记数据或预训练解码器即可从EEG数据中恢复未知的心理目标。


<details>
  <summary>Details</summary>
Motivation: 解决在无标记数据的情况下从EEG和图像数据中恢复心理目标的问题，此前仅在有标记数据下研究过。

Method: 提出CURSOR框架和算法，通过自校准学习恢复未知心理目标。

Result: CURSOR能预测与人类感知判断相关的图像相似度分数，并生成与未知心理目标无法区分的新刺激。

Conclusion: CURSOR在无标记数据下有效恢复心理目标，具有潜在应用价值。
...
### [246] [SLRNet: A Real-Time LSTM-Based Sign Language Recognition System](https://arxiv.org/abs/2506.11154)
*Sharvari Kamble*

Main category: cs.CV

TLDR: SLRNet是一个基于MediaPipe Holistic和LSTM网络的实时手语识别系统，验证准确率达86.7%。
...
### [247] [Evaluating Multimodal Large Language Models on Video Captioning via Monte Carlo Tree Search](https://arxiv.org/abs/2506.11155)
*Linhao Yu,Xinguang Ji,Yahui Liu,Fanheng Kong,Chenxi Sun,Jingyuan Zhang,Hongzhi Zhang,V. W.,Fuzheng Zhang,Deyi Xiong*

Main category: cs.CV

TLDR: AutoCaption框架通过蒙特卡洛树搜索生成多样化视频描述，构建MCTS-VCB基准，全面评估多模态大语言模型的视频字幕能力。


<details>
  <summary>Details</summary>
Motivation: 现有视频字幕评估方法存在关键点单一、数据创建成本高和评估范围有限等问题。

Method: 利用蒙特卡洛树搜索（MCTS）迭代生成多样化描述句子，构建MCTS-VCB基准。

Result: Gemini-1.5-Pro在MCTS-VCB上取得最高F1分数71.2，微调InternVL2.5-8B后性能显著提升。

Conclusion: AutoCaption有效提升视频字幕评估的全面性和模型性能，代码和数据已开源。
...
### [248] [Digitization of Document and Information Extraction using OCR](https://arxiv.org/abs/2506.11156)
*Rasha Sinha,Rekha B S*

Main category: cs.CV

TLDR: 论文提出了一种结合OCR和LLM的文本提取框架，显著提升了传统方法的灵活性和语义精度。


<details>
  <summary>Details</summary>
Motivation: 从扫描图像和数字格式混合的文档中准确提取信息是一个关键任务。

Method: 结合OCR技术和LLM，扫描文件用OCR处理，数字文件用布局感知库解析，提取的文本由LLM分析以识别键值对并消除歧义。
...
### [249] [VIBE: Can a VLM Read the Room?](https://arxiv.org/abs/2506.11162)
*Tania Chakraborty,Eylon Caplan,Dan Goldwasser*

Main category: cs.CV

TLDR: 本文探讨了视觉语言模型（VLMs）在社交推理中的能力，发现其存在视觉社交-语用推理的局限性，并提出新任务和数据集进行测试。


<details>
  <summary>Details</summary>
Motivation: 理解人类社交行为（如情感识别和社交动态）是一个重要但具有挑战性的问题，而现有大型语言模型（LLMs）无法捕捉非语言线索的作用。
...
### [250] [Synthetic Geology -- Structural Geology Meets Deep Learning](https://arxiv.org/abs/2506.11164)
*Simon Ghyselincks,Valeriia Okhmak,Stefano Zampini,George Turkiyyah,David Keyes,Eldad Haber*

Main category: cs.CV

TLDR: 利用生成式人工智能和深度学习技术，通过合成数据生成器和神经网络模型，实现了从地表地质数据到地下三维结构的可视化，填补了地下数据空白。


<details>
  <summary>Details</summary>
Motivation: 解决地下地质结构可视化的长期挑战，为资源勘探、灾害评估和岩土工程等应用提供支持。

Method: 设计合成数据生成器模拟地质活动，训练神经网络模型生成地下三维图像，结合地表地形和钻孔数据提高准确性。

Result: 模型能够从未见过的地表地形和地质数据生成高保真度的地下结构图像，包括地层、断层、褶皱等。

Conclusion: 该方法为地下地质研究提供了新工具，未来可通过区域数据微调模型，进一步优化应用效果。
...
### [251] [Evaluating BiLSTM and CNN+GRU Approaches for Human Activity Recognition Using WiFi CSI Data](https://arxiv.org/abs/2506.11165)
*Almustapha A. Wakili,Babajide J. Asaju,Woosub Jung*

Main category: cs.CV

TLDR: 比较BiLSTM和CNN+GRU在WiFi CSI数据上的HAR性能，发现CNN+GRU在UT-HAR表现更好，BiLSTM在NTU-Fi HAR更优，强调数据集特性和预处理的重要性。


<details>
  <summary>Details</summary>
Motivation: 探索不同深度学习模型在WiFi CSI数据上的HAR性能差异，以优化模型选择。
...
### [252] [Test-Time-Scaling for Zero-Shot Diagnosis with Visual-Language Reasoning](https://arxiv.org/abs/2506.11166)
*Ji Young Byun,Young-Jin Park,Navid Azizan,Rama Chellappa*

Main category: cs.CV

TLDR: 提出了一种零样本框架，通过测试时缩放增强LLMs在医学影像诊断中的推理能力，提高诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 临床决策对患者结果至关重要，但LLMs在医学影像视觉问答中的应用尚未充分探索，且监督微调因数据有限和标注成本高而不切实际。

Method: 结合视觉语言模型和LLM，通过测试时缩放整合多候选输出生成可靠诊断。

Result: 在多种医学影像模态中验证了方法的有效性，测试时缩放提升了诊断准确性。
...
### [253] [Towards a general-purpose foundation model for fMRI analysis](https://arxiv.org/abs/2506.11167)
...
TLDR: NeuroSTORM是一个通用的fMRI分析框架，通过预训练和任务特定提示调优，显著提高了跨任务的可转移性和可重复性。


<details>
  <summary>Details</summary>
Motivation: 当前fMRI分析方法存在可重复性和可转移性问题，NeuroSTORM旨在解决这些问题。

Method: 采用Mamba骨干网络和移位扫描策略处理4D fMRI数据，提出空间-时间优化预训练和任务特定提示调优。

Result: 在五个任务中表现优于现有方法，展示了强大的临床实用性。

Conclusion: NeuroSTORM为fMRI临床研究提供了标准化、开源的基础模型，提高了可重复性和可转移性。
...
### [254] [WaveFormer: A Lightweight Transformer Model for sEMG-based Gesture Recognition](https://arxiv.org/abs/2506.11168)
*Yanlong Chen,Mattia Orlandi,Pierangelo Maria Rapa,Simone Benatti,Luca Benini,Yawei Li*

Main category: cs.CV

TLDR: 论文提出了一种轻量级的基于Transformer的架构WaveFormer，用于解决sEMG手势识别中相似手势分类的挑战，同时降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习模型在sEMG手势识别中计算量大且难以部署在资源受限的嵌入式系统上，且对相似手势的分类效果不佳。

Method: WaveFormer结合时域和频域特征，通过可学习的小波变换增强特征提取，并采用WaveletConv模块实现高效和紧凑的设计。

Result: 模型仅需310万参数，在EPN612数据集上达到95%的分类准确率，且INT8量化后在Intel CPU上实现6.75毫秒的实时推理延迟。

Conclusion: WaveFormer在保持高性能的同时显著降低了计算成本，适合嵌入式系统部署。
...
### [255] [Teaching in adverse scenes: a statistically feedback-driven threshold and mask adjustment teacher-student framework for object detection in UAV images under adverse scenes](https://arxiv.org/abs/2506.11175)
*Hongyu Chen,Jiping Liu,Yong Wang,Jun Zhu,Dejun Feng,Yakun Xie*

Main category: cs.CV

TLDR: 论文提出了一种名为SF-TMAT的无监督域自适应方法，用于无人机在恶劣场景下的目标检测，通过动态调整掩码比和阈值策略提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有UDA研究多基于自然图像或清晰无人机图像，而恶劣条件下的无人机图像研究较少，且现有方法在特征对齐和伪标签质量上表现不佳。

Method: 提出SF-TMAT框架，包括动态步反馈掩码调整自编码器（DSFMA）和方差反馈平滑阈值（VFST）策略，动态调整学习焦点和伪标签选择阈值。

Result: 实验表明SF-TMAT在恶劣场景下的无人机目标检测中具有优越性和泛化能力。

Conclusion: SF-TMAT为恶劣条件下的无人机目标检测提供了有效解决方案，并通过动态策略提升了性能。
...
### [256] [BrainMAP: Multimodal Graph Learning For Efficient Brain Disease Localization](https://arxiv.org/abs/2506.11178)
*Nguyen Linh Dan Le,Jing Ren,Ciyuan Peng,Chengyao Xie,Bowen Li,Feng Xia*

Main category: cs.CV

TLDR: BrainMAP是一种新型多模态图学习框架，用于高效识别神经退行性疾病相关脑区，通过局部子图分析和多模态融合显著降低计算开销。
...
Conclusion: BrainMAP为神经退行性疾病的精确高效检测提供了新思路。
...
### [257] [Enhanced Vehicle Speed Detection Considering Lane Recognition Using Drone Videos in California](https://arxiv.org/abs/2506.11239)
*Amirali Ataee Naeini,Ashkan Teymouri,Ghazaleh Jafarsalehi,Michael Zhang*

Main category: cs.CV

TLDR: 研究提出了一种基于YOLOv11的改进模型，用于提高车辆速度和车道检测的准确性，特别针对加州交通监控需求。


<details>
  <summary>Details</summary>
Motivation: 加州车辆数量增加，但现有交通系统和速度检测技术（如YOLO）在准确性、车道识别和车辆分类方面存在不足。

Method: 使用800张鸟瞰图训练微调YOLOv11模型，检测车辆速度和车道，并分类为轿车和重型车辆。还评估了无人机高度、ROI距离和车速对检测的影响。

Result: 模型在测试中表现优异，MAE为0.97 mph，MSE为0.94 mph²。
...
### [258] [Lifting Data-Tracing Machine Unlearning to Knowledge-Tracing for Foundation Models](https://arxiv.org/abs/2506.11253)
*Yuwen Tan,Boqing Gong*

Main category: cs.CV

TLDR: 论文提出将机器遗忘从数据追踪提升到知识追踪，以解决基础模型的多样化遗忘需求，并更贴近人脑遗忘机制。


<details>
  <summary>Details</summary>
Motivation: 基础模型的多样化遗忘需求（如监管机构、企业用户等）无法通过数据追踪满足，且知识追踪更贴近人脑遗忘机制。

Method: 提出知识追踪机器遗忘范式，并通过视觉语言基础模型案例研究展示其实现方式。

Result: 知识追踪机器遗忘范式更符合实际需求和人脑遗忘机制。
...
### [259] [TARDIS STRIDE: A Spatio-Temporal Road Image Dataset for Exploration and Autonomy](https://arxiv.org/abs/2506.11302)
...
TLDR: 论文介绍了STRIDE数据集和TARDIS模型，用于模拟动态时空环境，提升智能体的行为表现。


<details>
  <summary>Details</summary>
Motivation: 现实世界环境具有动态时空变化特性，现有模型难以有效捕捉这些复杂动态。

Method: 提出STRIDE数据集，将360度全景图像转化为观察、状态和动作节点；开发TARDIS模型，基于Transformer统一建模时空动态。

Result: 在可控图像合成、指令跟随、自主控制和地理定位等任务中表现优异。

Conclusion: STRIDE和TARDIS为构建具备时空理解能力的通用智能体提供了新方向。
...
### [260] [HyBiomass: Global Hyperspectral Imagery Benchmark Dataset for Evaluating Geospatial Foundation Models in Forest Aboveground Biomass Estimation](https://arxiv.org/abs/2506.11314)
*Aaron Banze,Timothée Stassin,Nassim Ait Ali Braham,Rıdvan Salih Kuzu,Simon Besnard,Michael Schmitt*

Main category: cs.CV

TLDR: 论文介绍了一个全球分布的森林地上生物量（AGB）估算数据集，用于评估地理空间基础模型（Geo-FMs）的性能，并展示了Geo-FMs在像素级回归任务中的潜力。


<details>
  <summary>Details</summary>
Motivation: 现有基准数据集多局限于特定地理区域或任务类型，缺乏对Geo-FMs的全面评估。

Method: 结合EnMAP卫星的高光谱影像和GEDI激光雷达的AGB密度估计，构建了一个覆盖七大洲的基准数据集，并比较了Geo-FMs与基线U-Net的性能。

Result: Geo-FMs在某些情况下表现优于U-Net，性能差异与区域数据集大小和Vision Transformer的token patch尺寸相关。

Conclusion: 该数据集有助于Geo-FMs的开发与评估，并促进对地理偏差和泛化能力的研究。
...
### [261] [GynSurg: A Comprehensive Gynecology Laparoscopic Surgery Dataset](https://arxiv.org/abs/2506.11356)
*Sahar Nasirihaghighi,Negin Ghamsarian,Leonie Peschek,Matteo Munari,Heinrich Husslein,Raphael Sznitman,Klaus Schoeffmann*

Main category: cs.CV

TLDR: 论文介绍了GynSurg，一个用于妇科腹腔镜手术的最大、最多样化的多任务数据集，解决了现有数据集规模小、任务单一或注释不足的问题。
...
Method: 引入GynSurg数据集，提供丰富的多任务注释，支持动作识别、语义分割、手术文档化和新程序洞察发现。

Result: 通过标准化训练协议对最先进模型进行基准测试，验证了数据集的质量和多功能性。

Conclusion: GynSurg数据集的公开发布将加速该领域的进展。
...
### [262] [A Watermark for Auto-Regressive Image Generation Models](https://arxiv.org/abs/2506.11371)
*Yihan Wu,Xuehao Cui,Ruibo Chen,Georgios Milis,Heng Huang*

Main category: cs.CV

TLDR: 提出了一种名为C-reweight的无失真水印方法，用于解决图像生成模型中的retokenization mismatch问题，同时保持图像质量并提高检测能力。


<details>
  <summary>Details</summary>
Motivation: 图像生成模型的快速发展带来了潜在的滥用风险（如深度伪造、钓鱼攻击等），需要有效的真实性验证机制。传统水印技术因retokenization mismatch问题难以直接应用于图像生成模型。

Method: 提出C-reweight方法，采用基于聚类的策略，将同一聚类内的token视为等效，以解决retokenization mismatch问题。

Result: 在主流图像生成平台上验证，C-reweight不仅保持了生成图像的视觉质量，还提高了检测能力，优于现有无失真水印技术。
...
### [263] [Scalable Context-Preserving Model-Aware Deep Clustering for Hyperspectral Images](https://arxiv.org/abs/2506.11377)
*Xianlu Li,Nicolas Nadisic,Shaoguang Huang,Nikos Deligiannis,Aleksandra Pižurica*

Main category: cs.CV

TLDR: 提出了一种基于基表示的可扩展、保留上下文的深度聚类方法，用于高效的高光谱图像聚类，同时捕捉局部和非局部结构。
...
Method: 引入空间平滑约束（局部结构）和基于小聚类的方案（非局部结构），并联合优化这两种约束。采用单阶段框架，复杂度为O(n)。
...
### [264] [Enhance Multimodal Consistency and Coherence for Text-Image Plan Generation](https://arxiv.org/abs/2506.11380)
*Xiaoxin Lu,Ranran Haoran Zhang,Yusen Zhang,Rui Zhang*

Main category: cs.CV

TLDR: 论文提出了一种生成文本-图像计划的框架，解决了多模态对齐和视觉步骤连贯性的挑战，并通过实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注文本计划生成，而文本-图像计划的潜力尚未充分研究，因此需要解决多模态对齐和视觉连贯性的问题。

Method: 提出分步生成和优化框架，包括文本步骤生成、视觉步骤编辑、视觉信息提取和优化，支持多种骨干模型。

Result: 在1,100个任务的基准测试中，新方法在多模态一致性和连贯性上表现优于基线模型。

Conclusion: 该框架为文本-图像计划生成提供了有效解决方案，具有广泛适用性和改进潜力。
...
### [265] [Dynamic Double Space Tower](https://arxiv.org/abs/2506.11394)
*Weikai Sun,Shijie Song,Han Wang*

Main category: cs.CV

TLDR: 论文提出了一种动态双向空间塔模型，用于增强视觉问答任务中的空间关系理解和推理能力，替代传统注意力机制。
...
Result: 实验表明该模块可应用于多模态模型，并在空间关系问答数据集上取得最先进结果。

Conclusion: 该方法显著提升了模型对空间关系的理解能力，且具有广泛适用性。
...
### [266] [Stop learning it all to mitigate visual hallucination, Focus on the hallucination target](https://arxiv.org/abs/2506.11417)
*Dokyoon Yoon,Youngsook Song,Woomyong Park*

Main category: cs.CV

TLDR: 提出了一种基于偏好学习的方法（\mymethod\），通过针对性地处理幻觉问题，提升多模态大语言模型（MLLMs）在视觉语言任务中的可靠性。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉语言任务中常产生幻觉（生成不存在于输入图像中的信息），影响模型在实际应用中的可靠性。

Method: 构建包含幻觉响应、正确响应和目标信息的数据集，并应用偏好学习方法，专注于纠正特定目标的幻觉。

Result: 实验表明，该方法能有效减少幻觉，提升模型可靠性，同时不影响整体性能。

Conclusion: \mymethod\ 通过针对性学习，显著减少了MLLMs的幻觉问题，提升了模型的实用性和准确性。
...
### [267] [Auto-Connect: Connectivity-Preserving RigFormer with Direct Preference Optimization](https://arxiv.org/abs/2506.11430)
...
TLDR: Auto-Connect是一种自动绑定骨骼的新方法，通过连接保留标记化方案和拓扑感知奖励函数，显著提升骨骼结构的拓扑准确性和变形质量。


<details>
  <summary>Details</summary>
Motivation: 传统方法在预测骨骼位置时通常忽略连接性，导致拓扑错误或变形问题。Auto-Connect旨在通过直接集成连接性信息，生成更符合解剖学的骨骼结构。

Method: 采用连接保留标记化方案定义关节端点，结合拓扑感知奖励函数进行后训练优化，并利用隐式测地线特征选择关键骨骼。

Result: 模型能够生成拓扑更准确、变形效果更好的骨骼结构，显著减少皮肤绑定中的常见伪影。

Conclusion: Auto-Connect通过结合连接性保留、奖励引导优化和测地线特征，实现了高质量的自动骨骼绑定。
...
### [268] [Auditing Data Provenance in Real-world Text-to-Image Diffusion Models for Privacy and Copyright Protection](https://arxiv.org/abs/2506.11434)
*Jie Zhu,Leye Wang*

Main category: cs.CV

TLDR: 提出了一种黑盒审计框架FSCA，用于文本到图像扩散模型的数据来源审计，无需访问内部知识，实验表明其优于现有方法。
...
Method: FSCA利用文本到图像扩散模型中的语义连接进行审计，无需内部知识，并引入召回平衡和阈值调整策略。

Result: 在LAION-mi和COCO数据集上，FSCA优于8种基线方法，用户级准确率达90%（仅需10样本/用户）。
...
### [269] [TAViS: Text-bridged Audio-Visual Segmentation with Foundation Models](https://arxiv.org/abs/2506.11436)
*Ziyang Luo,Nian Liu,Xuguang Yang,Salman Khan,Rao Muhammad Anwer,Hisham Cholakkal,Fahad Shahbaz Khan,Junwei Han*

Main category: cs.CV

TLDR: TAViS框架通过结合多模态基础模型（ImageBind）和分割基础模型（SAM2），解决了音频-视觉分割中的跨模态对齐问题，并引入文本桥接设计提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在音频-视觉分割中未能有效解决跨模态对齐问题，且依赖单模态知识或简单组合基础模型。

Method: 提出TAViS框架，结合ImageBind和SAM2，通过文本桥接设计（混合提示机制和对齐监督策略）解决特征空间差异和分割损失不足的问题。

Result: 在单源、多源、语义数据集及零样本设置中表现优异。

Conclusion: TAViS通过文本桥接设计有效对齐音频和视觉模态，显著提升了分割性能。
...
### [270] [Uncertainty Awareness Enables Efficient Labeling for Cancer Subtyping in Digital Pathology](https://arxiv.org/abs/2506.11439)
...
TLDR: 该论文提出了一种结合不确定性感知的自监督对比学习模型，用于癌症亚型分类，通过选择性标注关键图像优化训练，仅需1-10%标注即可达到最佳性能。


<details>
  <summary>Details</summary>
Motivation: 癌症亚型分类模型需要专家标注，但标注成本高且资源有限。引入不确定性感知可以优化标注过程，减少对大量标注数据的依赖。

Method: 在自监督对比学习模型中计算证据向量，评估模型预测的置信度，利用不确定性分数选择关键图像进行标注，迭代优化训练。

Result: 仅需1-10%的标注即可在基准数据集上实现最优性能，同时提高分类精度和效率。

Conclusion: 该方法在标注数据有限的场景下具有显著优势，为数字病理学的研究和应用提供了新方向。
...
### [271] [On the Natural Robustness of Vision-Language Models Against Visual Perception Attacks in Autonomous Driving](https://arxiv.org/abs/2506.11472)
...
TLDR: 论文提出了一种名为V2LMs的视觉语言模型，用于提升自动驾驶车辆感知任务的鲁棒性，对抗攻击时表现优于传统DNN，且无需对抗训练。


<details>
  <summary>Details</summary>
Motivation: 传统DNN在自动驾驶任务中易受攻击，且现有防御机制（如对抗训练）会降低正常精度且无法泛化到未知攻击。

Method: 提出V2LMs，并评估其两种部署模式：Solo Mode（单任务）和Tandem Mode（多任务）。

Result: V2LMs在对抗攻击下精度下降小于8%，而传统DNN下降33%-46%。Tandem Mode在内存效率上表现更优。
...
### [272] [FAME: A Lightweight Spatio-Temporal Network for Model Attribution of Face-Swap Deepfakes](https://arxiv.org/abs/2506.11477)
*Wasim Ahmad,Yan-Tsung Peng,Yuan-Hao Chang*

Main category: cs.CV

TLDR: FAME是一种轻量级高效的时空框架，用于识别不同人脸交换模型生成的Deepfake视频，通过多级嵌入和注意力机制提升准确性。


<details>
  <summary>Details</summary>
Motivation: Deepfake视频对数字安全、隐私和媒体完整性构成威胁，现有研究多集中于二元检测，而模型归因任务（确定生成模型）研究不足。

Method: FAME结合空间和时间注意力机制，捕捉不同模型的生成伪影，设计为轻量级且高效。
...
### [273] [Environmental Change Detection: Toward a Practical Task of Scene Change Detection](https://arxiv.org/abs/2506.11481)
*Kyusik Cho,Suhan Woo,Hongje Seong,Euntai Kim*

Main category: cs.CV

TLDR: 论文提出了一种新的环境变化检测（ECD）任务，解决了传统场景变化检测（SCD）中依赖理想化对齐图像的局限性，并提出了一种利用多参考候选和语义丰富表示的框架。


<details>
  <summary>Details</summary>
Motivation: 传统SCD假设查询和参考图像视角完全对齐，不切实际。ECD旨在通过环境线索检测变化，更贴近现实场景。

Method: 提出了一种联合理解空间环境和检测变化的框架，利用多参考候选和语义聚合来解决视角不对齐和视野限制问题。

Result: 在三个标准基准集上显著优于现有方法，性能接近理想化设置。

Conclusion: ECD任务和提出的框架为实际场景变化检测提供了更实用的解决方案。
...
### [274] [Composite Data Augmentations for Synthetic Image Detection Against Real-World Perturbations](https://arxiv.org/abs/2506.11490)
*Efthymia Amarantidou,Christos Koutlis,Symeon Papadopoulos,Panagiotis C. Petrantonakis*

Main category: cs.CV

TLDR: 论文提出了一种改进合成图像检测（SID）的方法，通过数据增强组合和遗传算法优化选择，显著提升了模型在真实世界扰动下的性能。
...
Method: 研究采用数据增强组合、遗传算法优化选择和双标准优化方法，提升模型性能。

Result: 最佳模型平均精度提升22.53%，优于未使用增强的模型。
...
### [275] [Preserving Clusters in Prompt Learning for Unsupervised Domain Adaptation](https://arxiv.org/abs/2506.11493)
*Tung-Long Vuong,Hoang Phan,Vy Vo,Anh Bui,Thanh-Toan Do,Trung Le,Dinh Phung*

Main category: cs.CV

TLDR: 本文提出了一种利用多模态预训练模型（如CLIP）进行无监督域适应（UDA）的新方法，通过强化伪标签和目标提示学习，解决了现有方法中视觉嵌入分布偏离的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的UDA方法虽然性能优越，但视觉嵌入分布可能偏离预训练模型，导致误导信号。本文旨在通过利用视觉和文本嵌入的几何关系，改进伪标签和目标提示学习。

Method: 提出了一种基于最优传输理论的策略，利用源和目标视觉嵌入的关系强化伪标签，并通过聚类行为增强文本嵌入的对齐。

Result: 实验验证了方法的有效性，展示了性能提升和目标提示表示质量的改进。

Conclusion: 该方法通过几何嵌入关系优化了UDA任务，显著提升了性能和伪标签质量。
...
### [276] [Manager: Aggregating Insights from Unimodal Experts in Two-Tower VLMs and MLLMs](https://arxiv.org/abs/2506.11515)
*Xiao Xu,Libo Qin,Wanxiang Che,Min-Yen Kan*

Main category: cs.CV

TLDR: 本文提出Manager，一种轻量级插件，通过自适应聚合不同层次预训练单模态专家的知识，提升视觉-语言模型（VLM）的性能。ManagerTower在Two-Tower VLM架构中表现优异，并在MLLM架构中显著提升零样本性能。
...
### [277] [GNSS-inertial state initialization by distance residuals](https://arxiv.org/abs/2506.11534)
*Samuel Cerezo,Javier Civera*

Main category: cs.CV

TLDR: 提出了一种新的GNSS-惯性初始化策略，通过延迟使用全局GNSS测量数据，利用相对距离残差和Hessian矩阵奇异值优化初始化过程，实验证明其优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 传感器平台的初始状态估计常因初始测量数据有限而效果不佳，容易陷入局部最优解。

Method: 延迟使用全局GNSS数据，转而依赖相对距离残差，并通过Hessian矩阵奇异值变化确定切换时机。

Result: 在EuRoC和GVINS数据集上，该方法比传统方法更准确、鲁棒。

Conclusion: 该方法通过优化初始化策略，显著提升了GNSS-惯性系统的初始估计性能。
...
### [278] [FIMA-Q: Post-Training Quantization for Vision Transformers by Fisher Information Matrix Approximation](https://arxiv.org/abs/2506.11543)
*Zhuguanyu Wu,Shihe Wang,Jiayi Zhang,Jiaxin Chen,Yunhong Wang*

Main category: cs.CV

TLDR: 论文提出了一种名为FIMA-Q的新型后训练量化方法，针对视觉Transformer（ViT）在低比特量化下的精度下降问题，通过KL散度与FIM的联系及高效的DPLR-FIM近似方法，显著提升了量化精度。


<details>
  <summary>Details</summary>
Motivation: 当前视觉Transformer的后训练量化方法在低比特量化下仍存在显著的精度下降问题，传统Hessian近似方法存在局限性。

Method: 提出FIMA-Q方法，通过KL散度与FIM的联系快速计算量化损失，并采用DPLR-FIM近似方法优化量化损失计算。

Result: 实验表明，FIMA-Q在多种视觉任务和ViT架构上显著提升了量化精度，尤其在低比特量化下表现优异。

Conclusion: FIMA-Q是一种高效的后训练量化方法，显著改善了ViT在低比特量化下的性能。
...
### [279] [Leveraging Satellite Image Time Series for Accurate Extreme Event Detection](https://arxiv.org/abs/2506.11544)
*Heng Fang,Hossein Azizpour*

Main category: cs.CV

TLDR: SITS-Extreme框架利用卫星图像时间序列检测极端天气事件，通过多时相观测提高准确性，优于传统双时相基线。
...
Method: 提出SITS-Extreme框架，利用多时相卫星图像过滤无关变化，提取灾害信号。

Result: 实验验证框架有效性，性能优于传统方法，并分析关键组件和扩展性。

Conclusion: SITS-Extreme为大规模灾害监测提供可扩展且适用的解决方案。
...
### [280] [Linearly Solving Robust Rotation Estimation](https://arxiv.org/abs/2506.11547)
*Yinlong Liu,Tianyu Huang,Zhi-Xin Yang*

Main category: cs.CV

TLDR: 论文提出了一种新的旋转估计方法，通过线性模型拟合和四元数球面上的大圆表示，实现了高效且鲁棒的旋转估计。
...
Method: 将旋转估计问题重新表述为线性模型拟合问题，利用四元数球面上的大圆表示旋转运动，并提出基于投票的方法。

Result: 方法对噪声和异常值具有极高的鲁棒性，支持GPU并行计算，能在0.5秒内处理大规模（10^6）和高异常值比例（99%）的问题。

Conclusion: 实验验证了方法的有效性和鲁棒性，为旋转估计提供了一种新的解决方案。
...
### [281] [EyeSim-VQA: A Free-Energy-Guided Eye Simulation Framework for Video Quality Assessment](https://arxiv.org/abs/2506.11549)
*Zhaoyang Wang,Wen Lu,Jie Li,Lihuo He,Maoguo Gong,Xinbo Gao*

Main category: cs.CV

TLDR: EyeSimVQA是一个基于自由能的自修复视频质量评估框架，采用双分支架构，结合全局感知和细粒度分析，通过生物启发设计提升性能与可解释性。


<details>
  <summary>Details</summary>
Motivation: 视频质量评估（VQA）中，时空动态性和预训练模型限制导致自修复机制未充分探索，需要解决这些挑战。

Method: 提出双分支架构（美学分支和技术分支），集成专用增强模块，模拟自适应修复行为，并结合生物启发预测头。

Result: 在五个公开VQA基准测试中表现优异，性能优于或媲美现有方法，同时提升可解释性。

Conclusion: EyeSimVQA通过自由能自修复和生物启发设计，有效解决了VQA中的挑战，兼具高性能和可解释性。
...
### [282] [DaMO: A Data-Efficient Multimodal Orchestrator for Temporal Reasoning with Video LLMs](https://arxiv.org/abs/2506.11558)
*Bo-Cheng Chiu,Jen-Jee Chen,Yu-Chee Tseng,Feng-Chi Chen*

Main category: cs.CV

TLDR: DaMO是一种数据高效的视频大语言模型，专注于细粒度时间推理和多模态理解，通过分层双流架构和全局残差设计提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型在细粒度时间推理方面表现不足，限制了其对特定视频时刻的精确响应能力。

Method: 提出Temporal-aware Fuseformer，采用分层双流架构捕获时间动态并融合视觉和音频信息，结合全局残差设计提升计算效率。通过四阶段渐进训练范式增强多模态对齐、语义基础和时间推理能力。
...
### [283] [VFaith: Do Large Multimodal Models Really Reason on Seen Images Rather than Previous Memories?](https://arxiv.org/abs/2506.11571)
*Jiachen Yu,Yufei Zhan,Ziheng Wu,Yousong Zhu,Jinqiao Wang,Minghui Qiu*

Main category: cs.CV

TLDR: 论文提出了一种自动编辑视觉线索的流程和评估基准VFaith-Bench，用于分析多模态大语言模型（MLLMs）的视觉推理能力及其对视觉信息的忠实性。


<details>
  <summary>Details</summary>
Motivation: 当前MLLMs通过长链思维（CoT）提升复杂问题解决能力，但其有效性原因不明，尤其是视觉线索提取与推理的贡献难以量化。

Method: 设计了基于GPT-Image-1的自动编辑流程，生成对比问答对，并通过VFaith-Bench（包含755条目）评估模型视觉推理能力。

Result: 通过编辑测试集图像，揭示了模型推理能力与视觉感知的关系，并测试了主流模型的性能。

Conclusion: VFaith-Bench为评估MLLMs视觉推理能力提供了首个基准，揭示了其能力来源与视觉忠实性的关联。
...
### [284] [Camera-based method for the detection of lifted truck axles using convolutional neural networks](https://arxiv.org/abs/2506.11574)
*Bachir Tchana Tankeu,Mohamed Bouteldja,Nicolas Grignard,Bernard Jacob*

Main category: cs.CV

TLDR: 提出了一种基于YOLOv8s卷积神经网络的方法，用于检测卡车抬起的车轴，在实时应用中表现良好。


<details>
  <summary>Details</summary>
Motivation: 现有技术难以准确分类抬起车轴的车辆，且缺乏相关检测方法。

Method: 使用YOLOv8s CNN模型，通过垂直于交通方向的摄像头图像检测抬起车轴。

Result: 精度87%，召回率91.7%，推理时间1.4 ms，适合实时应用。
...
### [285] [OV-MAP : Open-Vocabulary Zero-Shot 3D Instance Segmentation Map for Robots](https://arxiv.org/abs/2506.11585)
*Juno Kim,Yesol Park,Hye-Jung Yoon,Byoung-Tak Zhang*

Main category: cs.CV

TLDR: OV-MAP是一种开放世界3D映射新方法，通过将开放特征集成到3D地图中提升物体识别能力，解决了特征溢出导致的精度问题。


<details>
  <summary>Details</summary>
Motivation: 解决3D映射中因特征溢出导致的实例级精度下降问题，提升开放世界中的物体识别能力。

Method: 使用类无关分割模型将2D掩码投影到3D空间，结合原始和合成深度图像，采用3D掩码投票机制实现零样本3D实例分割。

Result: 在ScanNet200和Replica数据集上表现出卓越的零样本性能、鲁棒性和适应性，并在真实环境中验证了其有效性。

Conclusion: OV-MAP方法在开放世界3D映射中实现了高精度的零样本实例分割，具有广泛的环境适应性和鲁棒性。
...
### [286] [EasyARC: Evaluating Vision Language Models on True Visual Reasoning](https://arxiv.org/abs/2506.11595)
*Mert Unsal,Aylin Akkus*

Main category: cs.CV

TLDR: 论文提出EasyARC，一个多模态推理基准，结合视觉和语言，支持多图像、多步推理和自我纠正，用于评估视觉语言模型的真实推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准缺乏视觉与语言复杂交互的真实推理，EasyARC旨在填补这一空白。

Method: 通过程序生成可验证、可扩展的基准EasyARC，包含渐进难度级别，用于强化学习流程。

Result: 评估了最先进的视觉语言模型，并分析了其失败模式。

Conclusion: EasyARC为视觉语言模型的真实推理和测试扩展能力设定了新标准，并开源了数据集和评估代码。
...
### [287] [A$^2$LC: Active and Automated Label Correction for Semantic Segmentation](https://arxiv.org/abs/2506.11599)
*Youjin Jeon,Kyusik Cho,Suhan Woo,Euntai Kim*

Main category: cs.CV

TLDR: A$^2$LC是一种高效的主动标签校正框架，通过自动化校正和自适应平衡采集函数，显著提升了语义分割中标签校正的效率和效果。


<details>
  <summary>Details</summary>
Motivation: 解决语义分割中手动像素级标注的高成本和易出错问题，提升标签校正的效率。
...
### [288] [Wi-CBR: WiFi-based Cross-domain Behavior Recognition via Multimodal Collaborative Awareness](https://arxiv.org/abs/2506.11616)
*Ruobei Zhang,Shengeng Tang,Huan Yan,Xiang Zhang,Richang Hong*

Main category: cs.CV

TLDR: 提出一种基于WiFi信号的多模态协作感知方法，通过融合相位数据和多普勒频移数据提升行为识别精度。
...
Method: 采用双分支自注意力模块捕获模态内时空线索，通过分组注意力机制挖掘关键特征，并利用门控机制优化信息熵。
...
### [289] [SignAligner: Harmonizing Complementary Pose Modalities for Coherent Sign Language Generation](https://arxiv.org/abs/2506.11621)
*Xu Wang,Shengeng Tang,Lechao Cheng,Feng Li,Shuo Wang,Richang Hong*

Main category: cs.CV

TLDR: 论文提出了一种名为SignAligner的新方法，用于生成逼真的手语视频，通过三个阶段实现：文本驱动的姿势模态共生成、在线协作校正和视频合成。


<details>
  <summary>Details</summary>
Motivation: 手语生成面临复杂性和多样性的挑战，现有方法难以实现自然和逼真的效果。

Method: SignAligner方法包括三阶段：文本驱动的姿势模态共生成、在线协作校正和视频合成，利用Transformer和跨模态注意力机制。

Result: 实验表明，SignAligner显著提高了生成手语视频的准确性和表现力。

Conclusion: SignAligner为手语生成提供了一种有效的解决方案，能够生成高质量的手语视频。
...
### [290] [Evaluating Fairness and Mitigating Bias in Machine Learning: A Novel Technique using Tensor Data and Bayesian Regression](https://arxiv.org/abs/2506.11627)
*Kuniko Paxton,Koorosh Aslansefat,Dhavalkumar Thakker,Yiannis Papadopoulos*

Main category: cs.CV

TLDR: 本文提出了一种新的机器学习公平性评估方法，专注于图像分类任务中的肤色处理，无需标注，通过概率分布和统计距离度量来捕捉公平性细节。


<details>
  <summary>Details</summary>
Motivation: 肤色在计算机视觉中以张量数据形式存在，不同于传统的分类特征（如性别和种族），现有公平性研究未能充分处理此类数据。

Method: 将肤色张量数据转换为概率分布，应用统计距离度量；提出基于贝叶斯回归和多项式函数的训练方法以减少潜在偏见。

Result: 新方法能够更细致地评估和处理肤色相关的公平性问题，避免了传统分类的刚性限制。

Conclusion: 该方法为机器学习中的肤色公平性提供了更灵活和精确的解决方案，弥补了现有研究的不足。
...
### [291] [DISCO: Mitigating Bias in Deep Learning with Conditional Distance Correlation](https://arxiv.org/abs/2506.11653)
*Emre Kavak,Tom Nuno Wolf,Christian Wachinger*

Main category: cs.CV

TLDR: 论文提出了一种标准反因果预测模型（SAM）和正则化策略DISCO，用于解决预测任务中模型依赖无关信号的问题，并在实验中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 预测任务中，模型可能利用无关信号（如光照条件）作为捷径，导致预测结果不理想。为了解决这一问题，需要一种方法确保模型仅关注因果相关的信号。

Method: 引入SAM模型构建因果框架，提出DISCO正则化策略，利用条件距离相关性优化回归任务中的条件独立性。

Result: DISCO在不同偏差缓解实验中表现优异，可作为传统核方法的替代方案。

Conclusion: SAM和DISCO提供了一种有效的方法，确保模型在预测时仅依赖因果相关的信号，避免了无关变量的干扰。
...
### [292] [Prohibited Items Segmentation via Occlusion-aware Bilayer Modeling](https://arxiv.org/abs/2506.11661)
*Yunhan Ren,Ruihuang Li,Lingbo Liu,Changwen Chen*

Main category: cs.CV

TLDR: 提出了一种针对X射线图像中违禁物品的实例分割方法，通过结合Segment Anything Model（SAM）和双层掩码解码器模块，解决了外观差异和重叠问题。


<details>
  <summary>Details</summary>
Motivation: X射线图像中违禁物品的外观与自然物体差异大，且物体重叠严重，导致实例分割困难。

Method: 集成SAM模型以利用其先验知识，设计双层掩码解码器模块显式建模遮挡关系，并标注了两个大规模数据集（PIDray-A和PIXray-A）用于监督。

Result: 实验结果表明该方法在遮挡标注数据集上表现有效。

Conclusion: 提出的方法有效解决了X射线图像中违禁物品的实例分割问题，相关数据和代码已开源。
...
### [293] [Dynamic Mixture of Curriculum LoRA Experts for Continual Multimodal Instruction Tuning](https://arxiv.org/abs/2506.11672)
*Chendi Ge,Xin Wang,Zeyang Zhang,Hong Chen,Jiapei Fan,Longtao Huang,Hui Xue,Wenwu Zhu*

Main category: cs.CV

TLDR: 提出了一种动态混合课程LoRA专家（D-MoLE）方法，通过动态调整MLLM架构以适应新任务，解决任务架构冲突和模态不平衡问题，实验显示性能提升15%。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM方法采用固定架构，难以适应新任务，需解决任务架构冲突和模态不平衡问题。

Method: 提出D-MoLE方法，包括动态层专家分配器和梯度驱动的跨模态课程，动态调整架构和模块更新比例。

Result: 实验表明D-MoLE显著优于现有方法，平均提升15%。

Conclusion: 首次从架构角度研究MLLM的持续学习，D-MoLE有效解决了任务适应问题。
...
### [294] [Cross-Modal Clustering-Guided Negative Sampling for Self-Supervised Joint Learning from Medical Images and Reports](https://arxiv.org/abs/2506.11674)
*Libin Lan,Hongxing Li,Zunhui Xia,Juan Zhou,Xiaofei Zhu,Yongmei Li,Yudong Zhang,Xin Luo*

Main category: cs.CV

TLDR: 论文提出了一种跨模态聚类引导负采样方法（CM-CGNS），通过改进负样本选择和增强局部细节特征，提升了医学图像识别的性能。


<details>
  <summary>Details</summary>
Motivation: 现有模型在医学图像识别中存在负样本选择不当、忽略局部细节和低层特征的问题，影响了识别效果。

Method: CM-CGNS结合跨模态注意力扩展k-means聚类用于负采样，并引入跨模态掩码图像重建模块（CM-MIR）以增强局部特征交互。

Result: 在五个下游数据集上的实验表明，该方法在分类、检测和分割任务中均优于现有方法。

Conclusion: CM-CGNS能有效学习医学视觉表示，适用于多种识别任务。
...
### [295] [Predicting Patient Survival with Airway Biomarkers using nn-Unet/Radiomics](https://arxiv.org/abs/2506.11677)
...
TLDR: 论文提出了一种三阶段方法，通过分割网络提取气道结构特征，结合放射组学特征，使用SVM分类器预测肺纤维化患者的生存结果。
...
Method: 1. 使用nn-Unet分割气道结构；2. 从气管和气道周围提取放射组学特征；3. 结合SVM分类器进行分类。

Result: 分割任务得分0.8601，分类任务得分0.7346。

Conclusion: 三阶段方法在预测肺纤维化患者生存结果方面表现出潜力。
...
### [296] [Pose Matters: Evaluating Vision Transformers and CNNs for Human Action Recognition on Small COCO Subsets](https://arxiv.org/abs/2506.11678)
*MingZe Tang,Madiha Kazi*

Main category: cs.CV

TLDR: 论文研究了基于COCO图像数据集的三类动作识别，比较了从简单全连接网络到Transformer架构的性能。ViT表现最佳，准确率达90%，显著优于其他模型。


<details>
  <summary>Details</summary>
Motivation: 探索不同模型在动作识别任务中的表现，尤其是Transformer架构的效率和可解释性。

Method: 使用COCO数据集的三类子集，测试了全连接网络、卷积网络、CLIP模型和ViT，并通过统计分析和可解释性工具（SHAP和LeGrad）评估性能。

Result: ViT的准确率最高（90%），且与其他模型差异显著（p < 0.001）。可解释性分析显示ViT能聚焦动作相关区域，而其他模型易受背景干扰。

Conclusion: Transformer在数据效率和可解释性上表现优越，强调了可解释性技术对诊断模型失败的重要性。
...
### [297] [MTabVQA: Evaluating Multi-Tabular Reasoning of Language Models in Visual Space](https://arxiv.org/abs/2506.11684)
*Anshul Singh,Chris Biemann,Jan Strich*

Main category: cs.CV

TLDR: MTabVQA是一个新的多表格视觉问答基准，用于评估视觉语言模型在多表格图像中的推理能力，并通过微调提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要针对单表格或非视觉数据，无法评估多表格图像的解析和跨表格推理能力。

Method: 引入MTabVQA基准，包含3,745个复杂问题-答案对，并开发MTabVQA-Instruct数据集用于微调。

Result: 实验显示现有模型在多表格推理上表现有限，但通过微调显著提升性能。
...
### [298] [DMAF-Net: An Effective Modality Rebalancing Framework for Incomplete Multi-Modal Medical Image Segmentation](https://arxiv.org/abs/2506.11691)
*Libin Lan,Hongxing Li,Zunhui Xia,Yudong Zhang*

Main category: cs.CV

TLDR: DMAF-Net提出了一种动态模态感知融合网络，通过动态平衡模态贡献和结构关系，解决了多模态医学图像分割中的模态不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖完整模态假设，无法动态平衡贡献或忽略模态间结构关系，导致实际临床场景中性能不佳。
...
### [299] [Quizzard@INOVA Challenge 2025 -- Track A: Plug-and-Play Technique in Interleaved Multi-Image Model](https://arxiv.org/abs/2506.11737)
*Dinh Viet Cuong,Hoang-Bao Le,An Pham Ngoc Nguyen,Liting Zhou,Cathal Gurrin*

Main category: cs.CV

TLDR: 论文展示了LLaVA-NeXT-interleave在22个数据集上的优异表现，并比较了加入DCI连接器的版本与标准模型的性能。标准模型在视觉任务中表现最佳，而DCI版本在语义连贯性任务中更强。
...
Result: 标准模型在视觉任务中表现最佳，DCI版本在语义连贯性任务中更强。

Conclusion: 结合基础模型与即插即用技术在多任务中具有潜力。
...
### [300] [AgriPotential: A Novel Multi-Spectral and Multi-Temporal Remote Sensing Dataset for Agricultural Potentials](https://arxiv.org/abs/2506.11740)
*Mohammad El Sakka,Caroline De Pourtales,Lotfi Chaari,Josiane Mothe*

Main category: cs.CV

TLDR: AgriPotential是一个基于Sentinel-2卫星影像的新型基准数据集，用于农业潜力预测，支持多种机器学习任务。
...
Conclusion: AgriPotential填补了农业潜力预测数据集的空白，并公开提供数据和代码以促进研究。
...
### [301] [DiffFuSR: Super-Resolution of all Sentinel-2 Multispectral Bands using Diffusion Models](https://arxiv.org/abs/2506.11764)
*Muhammad Sarmad,Arnt-Børre Salberg,Michael Kampffmeyer*

Main category: cs.CV

TLDR: DiffFuSR是一个模块化管道，用于将Sentinel-2 Level-2A影像的12个光谱波段超分辨率统一到2.5米地面采样距离（GSD）。


<details>
  <summary>Details</summary>
Motivation: 提升Sentinel-2影像的分辨率，同时保持光谱一致性和空间对齐。

Method: 采用两阶段方法：1）基于扩散模型的超分辨率（SR）模型；2）学习融合网络，利用超分辨率RGB图像作为空间先验。

Result: 在OpenSR基准测试中表现优于现有方法，反射率保真度、光谱一致性和空间对齐均更优。
...
### [302] [MambaVSR: Content-Aware Scanning State Space Model for Video Super-Resolution](https://arxiv.org/abs/2506.11768)
*Linfeng He,Meiqin Liu,Qi Tang,Chao Yao,Yao Zhao*

Main category: cs.CV

TLDR: MambaVSR提出了一种基于状态空间模型的视频超分辨率框架，通过内容感知扫描机制和全局-局部状态空间块，显著提升了非局部依赖建模和计算效率。


<details>
  <summary>Details</summary>
Motivation: 视频超分辨率（VSR）在建模非局部依赖性和保持计算效率方面面临挑战，现有方法（如光流或Transformer）难以处理大运动位移和长视频序列。

Method: MambaVSR结合了共享罗盘构造（SCC）和内容感知序列化（CAS）模块，动态建模时空交互；全局-局部状态空间块（GLSSB）整合窗口自注意力和SSM特征传播。
...
### [303] [CLIP Meets Diffusion: A Synergistic Approach to Anomaly Detection](https://arxiv.org/abs/2506.11772)
*Byeongchan Lee,John Won,Seunghyun Lee,Jinwoo Shin*

Main category: cs.CV

TLDR: CLIPFUSION结合判别式和生成式基础模型，通过CLIP捕捉全局特征，扩散模型捕捉局部细节，显著提升异常检测性能。


<details>
  <summary>Details</summary>
Motivation: 异常检测因定义模糊、类型多样及数据稀缺而复杂，需综合模型解决。

Method: 利用CLIP和扩散模型，提取交叉注意力图和特征图进行异常检测。

Result: 在MVTec-AD和VisA数据集上表现优异，超越基线方法。

Conclusion: 多模态多模型融合有效解决异常检测挑战，提供可扩展方案。
...
### [304] [AgentSense: Virtual Sensor Data Generation Using LLM Agent in Simulated Home Environments](https://arxiv.org/abs/2506.11773)
*Zikang Leng,Megha Thukral,Yaqi Liu,Hrudhai Rajasekhar,Shruthi K. Hiremath,Thomas Plötz*

Main category: cs.CV

TLDR: AgentSense利用大型语言模型生成虚拟数据，通过模拟家庭环境和传感器记录活动，解决了智能家居中人类活动识别（HAR）数据不足的问题。实验表明，虚拟数据显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 智能家居中的人类活动识别系统因缺乏大规模、多样化的标注数据而受限，且家庭布局、传感器配置和用户行为的多样性增加了复杂性。

Method: 提出AgentSense，通过生成虚拟人物及其日常活动，在模拟家庭环境中记录传感器数据，生成多样化数据集。

Result: 在五个基准HAR数据集上，虚拟数据显著提升了模型性能，少量真实数据结合虚拟数据即可达到与完整真实数据集相当的效果。

Conclusion: 虚拟数据为解决大规模标注数据缺乏问题提供了有效途径，无需手动采集数据。
...
### [305] [Real-Time Feedback and Benchmark Dataset for Isometric Pose Evaluation](https://arxiv.org/abs/2506.11774)
*Abhishek Jaiswal,Armeet Singh Luthra,Purav Jangir,Bhavya Garg,Nisheeth Srivastava*

Main category: cs.CV

TLDR: 论文提出了一种实时反馈系统，用于评估等长运动姿势，解决了依赖不可靠数字媒体内容的问题。


<details>
  <summary>Details</summary>
Motivation: 等长运动依赖不可靠的数字媒体内容，缺乏专家监督，可能导致姿势错误、受伤和用户流失。

Method: 发布了一个大型多类别等长运动视频数据集，并评估了包括基于图的网络在内的先进模型，同时提出了一种新的三部分度量标准。

Result: 系统提高了智能个性化家庭锻炼系统的可行性，并扩展了其在康复和物理治疗等领域的应用潜力。

Conclusion: 实时反馈系统能够提供专家级诊断，为家庭锻炼和其他健身领域带来更安全和有效的解决方案。
...
### [306] [Self-supervised Learning of Echocardiographic Video Representations via Online Cluster Distillation](https://arxiv.org/abs/2506.11777)
*Divyanshu Mishra,Mohammadreza Salehi,Pramit Saha,Olga Patey,Aris T. Papageorghiou,Yuki M. Asano,J. Alison Noble*

Main category: cs.CV

TLDR: DISCOVR是一种自监督双分支框架，用于心脏超声视频表示学习，结合聚类和图像编码器，通过语义聚类蒸馏损失提升性能。


<details>
  <summary>Details</summary>
Motivation: 心脏超声领域缺乏领域特定的预训练模型，现有自监督学习方法难以处理高样本相似性和低PSNR输入。
...
### [307] [GPLQ: A General, Practical, and Lightning QAT Method for Vision Transformers](https://arxiv.org/abs/2506.11784)
*Guang Liang,Xinyao Liu,Jianxin Wu*

Main category: cs.CV

TLDR: GPLQ是一种高效且有效的ViT量化框架，通过两阶段策略（先量化激活，再量化权重）显著提升了4位量化模型的性能，同时降低了计算成本和内存占用。
...
Method: GPLQ采用两阶段策略：第一阶段保持权重为FP32，仅量化激活；第二阶段量化权重。
...
### [308] [Teleoperated Driving: a New Challenge for 3D Object Detection in Compressed Point Clouds](https://arxiv.org/abs/2506.11804)
*Filippo Bragato,Michael Neri,Paolo Testolina,Marco Giordani,Federica Battisti*

Main category: cs.CV

TLDR: 论文研究了如何通过点云数据检测车辆和行人，以支持远程驾驶（TD）的安全操作，并评估了压缩算法和目标检测器的性能及其对V2X网络的影响。


<details>
  <summary>Details</summary>
Motivation: 随着互联设备的普及和传感器技术的进步，远程驾驶（TD）成为受益领域之一。然而，如何高效检测车辆和行人是确保TD安全的关键问题。

Method: 利用扩展的SELMA数据集（包含3D物体的真实边界框），评估了多种压缩算法和目标检测器的性能，包括压缩效率、推理时间和检测精度，并分析了其对V2X网络的影响。

Result: 研究结果表明，压缩算法和目标检测器在数据率、延迟和检测精度方面对TD应用具有显著影响，符合3GPP的要求。

Conclusion: 该研究为远程驾驶中的目标检测和数据处理提供了实用指导，并验证了其在V2X网络中的可行性。
...
### [309] [Rethinking Multilingual Vision-Language Translation: Dataset, Evaluation, and Adaptation](https://arxiv.org/abs/2506.11820)
...
TLDR: 该论文系统评估了视觉语言翻译（VLT）任务，从数据质量、模型架构和评估指标三个角度展开研究，提出了新数据集AibTrans、对比了多种模型性能，并提出了更鲁棒的评估方法DA Score。


<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型（LVLMs）在VLT任务上缺乏系统评估，且现有数据集在语义和文化保真度上存在局限性。

Method: 研究从数据质量（引入AibTrans数据集）、模型架构（评估11种商业和6种开源模型）和评估指标（提出DA Score）三个角度展开。

Result: 发现高资源语言对微调会损害跨语言性能，并提出平衡多语言微调策略。

Conclusion: 建立了VLT的新评估基准，并提出改进模型性能的策略。
...
### [310] [Vision-based Lifting of 2D Object Detections for Automated Driving](https://arxiv.org/abs/2506.11839)
*Hendrik Königshof,Kun Li,Christoph Stiller*

Main category: cs.CV

TLDR: 提出了一种仅使用摄像头将2D检测结果提升为3D检测的流程，作为LiDAR的经济替代方案。


<details>
  <summary>Details</summary>
Motivation: 由于LiDAR成本高，而摄像头已广泛配备，需要一种低成本且高效的3D目标检测方法。

Method: 利用2D CNN处理点云数据，将现有2D算法结果提升为3D检测。

Result: 在KITTI基准测试中表现与现有图像方法相当，且运行时间仅为三分之一。

Conclusion: 该方法为低成本3D目标检测提供了可行方案，适用于多种道路使用者。
...
### [311] [SphereDrag: Spherical Geometry-Aware Panoramic Image Editing](https://arxiv.org/abs/2506.11863)
*Zhiao Feng,Xuewei Li,Junjie Yang,Yuxin Peng,Xi Li*

Main category: cs.CV

TLDR: SphereDrag是一个新颖的全景图像编辑框架，通过自适应重投影、大圆轨迹调整和球形搜索区域跟踪解决边界不连续、轨迹变形和像素密度不均问题，并在PanoBench基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 全景图像编辑因球形几何和投影变形存在边界不连续、轨迹变形和像素密度不均的挑战，现有方法未能有效解决。

Method: 采用自适应重投影（AR）处理不连续问题，大圆轨迹调整（GCTA）提高轨迹精度，球形搜索区域跟踪（SSRT）解决像素密度不均。

Result: SphereDrag在几何一致性和图像质量上显著优于现有方法，相对提升达10.5%。

Conclusion: SphereDrag通过球形几何知识实现了准确可控的全景图像编辑，为未来研究提供了新思路。
...
### [312] [Methods for evaluating the resolution of 3D data derived from satellite images](https://arxiv.org/abs/2506.11876)
*Christina Selby,Holden Bindl,Tyler Feldman,Andrew Skow,Nicolas Norena Acosta,Shea Hagstrom,Myron Brown*

Main category: cs.CV

TLDR: 本文探讨了从卫星图像中获取的3D数据的分辨率评估方法，并提出了基于高分辨率参考激光雷达的自动化评估工具和工作流程。


<details>
  <summary>Details</summary>
Motivation: 卫星图像衍生的3D数据在大规模场景建模中至关重要，但需要评估其分辨率以确定任务效用和改进方向。

Method: 提出了评估点云、数字表面模型和3D网格模型分辨率的方法，并开发了自动化评估工具和工作流程。

Result: 通过不同质量数据的分析展示了评估工具的有效性。

Conclusion: 自动化评估工具能够有效支持卫星图像3D数据的分辨率分析和改进。
...
### [313] [O2Former:Direction-Aware and Multi-Scale Query Enhancement for SAR Ship Instance Segmentation](https://arxiv.org/abs/2506.11913)
*F. Gao,Y Li,X He,J Sun,J Wang*

Main category: cs.CV

TLDR: O2Former是一种针对SAR图像中船舶实例分割的改进框架，通过优化查询生成器和方向感知嵌入模块，解决了尺度变化、目标密度和模糊边界等挑战。


<details>
  <summary>Details</summary>
Motivation: SAR图像中的船舶实例分割在海洋监测、环境分析和国家安全中至关重要，但现有方法常忽视尺度变化、目标密度和模糊边界等问题，导致性能不佳。
...
### [314] [Aligned Novel View Image and Geometry Synthesis via Cross-modal Attention Instillation](https://arxiv.org/abs/2506.11924)
*Min-Seop Kwak,Junho Kim,Sangdoo Yun,Dongyoon Han,Taekyoung Kim,Seungryong Kim,Jin-Hwa Kim*

Main category: cs.CV

TLDR: 提出了一种基于扩散的框架，通过变形和修复方法实现对齐的新视角图像和几何生成。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法需要密集姿态图像或局限于域内视角的问题，利用现成的几何预测器实现更灵活的视角合成。

Method: 采用多任务方法，通过跨模态注意力蒸馏确保图像与几何对齐，并引入基于邻近的网格条件整合深度和法线线索。

Result: 在未见场景中实现了高保真的外推视角合成，并在插值设置下提供了竞争性的重建质量。

Conclusion: 该方法在图像和几何生成中实现了对齐，并生成了几何对齐的彩色点云，适用于全面的3D完成。
...
### [315] [Evaluating Sensitivity Parameters in Smartphone-Based Gaze Estimation: A Comparative Study of Appearance-Based and Infrared Eye Trackers](https://arxiv.org/abs/2506.11932)
*Nishan Gunawardena,Gough Yumu Lui,Jeewani Anupama Ginige,Bahman Javadi*

Main category: cs.CV

TLDR: 研究比较了基于智能手机的深度学习眼动追踪算法与商用红外眼动仪Tobii Pro Nano的性能，探讨了外观基眼动追踪在移动设备上的可行性。


<details>
  <summary>Details</summary>
Motivation: 研究旨在评估外观基眼动追踪在真实移动使用条件下的可行性，并分析关键敏感因素。
...
Conclusion: 外观基方法在移动眼动追踪中具有潜力，并为评估不同使用条件下的注视估计系统提供了参考框架。
...
### [316] [How Visual Representations Map to Language Feature Space in Multimodal LLMs](https://arxiv.org/abs/2506.11976)
*Constantin Venhoff,Ashkan Khakzar,Sonia Joseph,Philip Torr,Neel Nanda*

Main category: cs.CV

TLDR: 论文提出了一种通过线性适配器连接冻结的视觉和语言模型的方法，研究了视觉与语言表征的对齐机制。
...
Result: 实验表明，视觉表征在LLM的中后层逐渐与语言表征对齐，早期层存在不匹配。
...
### [317] [Simple Radiology VLLM Test-time Scaling with Thought Graph Traversal](https://arxiv.org/abs/2506.11989)
*Yue Yao,Zelin Wen,Yan Tong,Xinyu Tian,Xuqing Li,Xiao Ma,Dongliang Xu,Tom Gedeon*

Main category: cs.CV

TLDR: 提出了一种轻量级的Thought Graph Traversal (TGT)框架，通过测试时缩放提升视觉语言大模型在放射学报告生成中的推理性能，无需额外训练。


<details>
  <summary>Details</summary>
Motivation: 探索如何在不改变模型的情况下，通过测试时缩放提升视觉语言大模型在医学领域的推理能力，特别是放射学报告生成。

Method: 引入TGT框架，整合结构化医学先验知识到提示中，并结合推理预算强制策略动态调整推理深度。

Result: 方法在标准基准测试中优于基线提示方法，生成更准确、一致的胸部X光报告，并揭示数据集偏差。

Conclusion: TGT框架和推理预算强制策略的结合显著提升了模型性能，展示了测试时缩放的有效性。
...
### [318] [VGR: Visual Grounded Reasoning](https://arxiv.org/abs/2506.11991)
*Jiacong Wang,Zijiang Kang,Haochen Wang,Haiyong Jiang,Jiawen Li,Bohong Wu,Ya Wang,Jiao Ran,Xiao Liang,Chao Feng,Jun Xiao*

Main category: cs.CV

TLDR: VGR是一种新型多模态推理模型，通过增强细粒度视觉感知能力解决传统语言空间推理的局限性，显著提升复杂视觉推理任务的表现。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖纯语言空间推理，存在语言偏见且局限于数学或科学领域，难以处理需要全面图像细节理解的复杂视觉推理任务。

Method: VGR结合视觉定位和语言推理，首先生成相关区域检测，再基于图像区域重放提供精确答案，并通过VGR-SFT数据集进行训练。

Result: 在LLaVA-NeXT-7B基准上，VGR仅使用30%的图像标记数，在MMStar、AI2D和ChartQA上分别提升4.1、7.1和12.9分。

Conclusion: VGR通过多模态推理和视觉区域重放，显著提升了复杂视觉任务的性能，为多模态推理提供了新方向。
...
### [319] [Improving Surgical Risk Prediction Through Integrating Automated Body Composition Analysis: a Retrospective Trial on Colectomy Surgery](https://arxiv.org/abs/2506.11996)
*Hanxue Gu,Yaqian Chen,isoo Lee,Diego Schaps,Regina Woody,Roy Colglazier,Maciej A. Mazurowski,Christopher Mantyh*

Main category: cs.CV

TLDR: 研究评估了术前CT扫描自动提取的身体组成指标是否能预测结肠切除术后结果，单独或结合临床变量或现有风险预测工具。
...
Method: 使用Cox比例风险模型和逻辑回归，评估300多个CT提取特征（如骨骼肌面积、脂肪面积等）对1年全因死亡率及其他术后结果的预测性能。

Result: 通过C指数和综合Brier评分评估预测性能，同时分析了并发症、再入院等次要结果。

Conclusion: 术前CT提取的身体组成指标可能对结肠切除术后结果有预测价值，尤其是结合临床变量时。
...
### [320] [Affogato: Learning Open-Vocabulary Affordance Grounding with Automated Data Generation at Scale](https://arxiv.org/abs/2506.12009)
*Junha Lee,Eunha Park,Chunghyun Park,Dahyun Kang,Minsu Cho*

Main category: cs.CV

TLDR: Affogato是一个大规模基准数据集，用于解决基于自然语言描述的交互进行物体区域定位的挑战，包含150K实例，支持开放词汇描述和3D功能热图。
...
Method: 开发基于预训练部分感知视觉主干和文本条件热图解码器的简单有效视觉语言模型。

Result: 模型在现有2D和3D基准测试中表现优异，尤其在开放词汇跨领域泛化方面有效。

Conclusion: Affogato数据集和模型为功能定位任务提供了新的资源和解决方案。
...
### [321] [Decoding Cortical Microcircuits: A Generative Model for Latent Space Exploration and Controlled Synthesis](https://arxiv.org/abs/2506.11062)
*Xingyu Liu,Yubin Li,Guozhang Chen*

Main category: q-bio.NC

TLDR: 论文提出了一种生成模型，通过学习小鼠皮层微电路的详细连接图，揭示大脑结构的低维蓝图，并展示了如何利用该模型可控地生成具有特定结构特征的合成微电路。


<details>
  <summary>Details</summary>
Motivation: 揭示大脑复杂结构背后的低维蓝图，以理解结构如何决定功能，并为人工神经网络的发展提供启示。

Method: 引入生成模型，从小鼠皮层微电路的详细连接图中学习潜在表示，并利用该潜在空间可控地生成合成微电路。

Result: 模型成功压缩了微电路的结构信息，并在潜在空间中发现了与可理解网络特性相关的方向，实现了可控生成。

Conclusion: 该研究为探索神经回路的设计原则提供了新方法，并可能推动更先进的人工神经网络的发展。
...
### [322] [Sparse Autoencoders Bridge The Deep Learning Model and The Brain](https://arxiv.org/abs/2506.11123)
*Ziming Mao,Jia Xu,Zeqi Zheng,Haofang Zheng,Dabing Sheng,Yaochu Jin,Guoyuan Yang*

Main category: q-bio.NC

TLDR: SAE-BrainMap框架通过稀疏自编码器（SAE）将深度学习视觉模型与fMRI信号对齐，揭示了模型与人类视觉皮层的层级映射关系。
...
Method: 使用SAE对模型激活进行分层训练，并通过余弦相似度计算SAE单元与fMRI信号的相关性，构建体素字典并可视化层级映射。

Result: SAE单元与fMRI信号的最大相似度达0.76，ViT-B/16$_{CLIP}$在早期层利用低层信息生成高层语义信息。
...
### [323] [Voxel-Level Brain States Prediction Using Swin Transformer](https://arxiv.org/abs/2506.11455)
*Yifei Sun,Daniel Chahine,Qinghao Wen,Tianming Liu,Xiang Li,Yixuan Yuan,Fernando Calamante,Jinglei Lv*

Main category: q-bio.NC

TLDR: 论文提出了一种基于4D Swin Transformer的新架构，用于预测人类静息状态下的未来脑活动，通过fMRI数据实现高精度预测。
...
Method: 采用4D Swin Transformer作为编码器学习时空信息，卷积解码器实现与输入相同分辨率的预测，使用HCP的100名受试者数据。

Result: 模型能高精度预测7.2秒的静息态脑活动，预测结果与BOLD信号高度相似。

Conclusion: Swin Transformer模型能高效学习大脑时空组织，为减少fMRI扫描时间和脑机接口开发提供潜力。
...
### [324] [Anti-Aliased 2D Gaussian Splatting](https://arxiv.org/abs/2506.11252)
*Mae Younes,Adnane Boukhayma*

Main category: cs.GR

TLDR: AA-2DGS是一种改进的2D高斯泼溅方法，解决了原方法在不同采样率下的锯齿问题，提升了渲染质量。


<details>
  <summary>Details</summary>
Motivation: 2D高斯泼溅（2DGS）在新视角合成和表面重建中表现优异，但在不同采样率下会出现严重锯齿，限制了其实际应用。

Method: 提出了AA-2DGS，引入世界空间平滑核和对象空间Mip滤波器，约束频率内容并优化抗锯齿处理。

Result: AA-2DGS显著提升了渲染质量，消除了高频率锯齿，同时保持了2DGS的几何优势。

Conclusion: AA-2DGS通过频率约束和局部抗锯齿处理，有效解决了2DGS的锯齿问题，拓展了其应用场景。
...
### [325] [CGVQM+D: Computer Graphics Video Quality Metric and Dataset](https://arxiv.org/abs/2506.11546)
*Akshay Jindal,Nabil Sadaka,Manu Mathew Thomas,Anton Sochenov,Anton Kaplanyan*

Main category: cs.GR

TLDR: 论文提出了一种专注于高级渲染技术引入失真的视频质量数据集，并开发了新的质量评估指标CGVQM，优于现有方法。
...
Method: 构建了一个包含多种高级渲染技术失真的视频质量数据集，并基于预训练的3D CNN特征空间提出CGVQM指标。

Result: 现有全参考质量指标在这些失真上表现不佳（最大Pearson相关系数0.78），CGVQM显著优于现有指标。

Conclusion: CGVQM能有效评估渲染失真，并提供像素级误差图和全局质量分数，数据集和指标已开源。
...
### [326] [HQFNN: A Compact Quantum-Fuzzy Neural Network for Accurate Image Classification](https://arxiv.org/abs/2506.11146)
*Jianhong Yao,Yangming Guo*

Main category: quant-ph

TLDR: HQFNN结合模糊推理与量子电路，提出一种紧凑、可解释且抗噪声的视觉模型，性能优于传统方法。
...
Method: 通过量子电路实现模糊推理流程，结合轻量级CNN特征提取器，利用量子信号进行分类。

Result: 在标准图像基准测试中优于经典、模糊增强和纯量子方法，且参数更少，抗噪声能力强。
...
### [327] [Learning Encodings by Maximizing State Distinguishability: Variational Quantum Error Correction](https://arxiv.org/abs/2506.11552)
*Nico Meyer,Christopher Mutschler,Andreas Maier,Daniel D. Scherer*

Main category: quant-ph

TLDR: 提出了一种基于可区分性损失函数的变分量子纠错方法（VarQEC），针对特定噪声结构优化编码电路，优于传统纠错码。
...
Method: 通过最大化噪声通道后量子态的可区分性，提出可区分性损失函数，利用变分技术实现资源优化的编码电路。

Result: VarQEC生成的纠错码在理论和实践上表现优异，并在IBM和IQM硬件上验证了其可行性。

Conclusion: VarQEC为噪声特定的量子纠错提供了高效方案，具有实际应用潜力。
...
### [328] [Interpretable representation learning of quantum data enabled by probabilistic variational autoencoders](https://arxiv.org/abs/2506.11982)
*Paulin de Schoulepnikoff,Gorka Muñoz-Gil,Hendrik Poulsen Nautrup,Hans J. Briegel*

Main category: quant-ph

TLDR: 论文提出了一种改进的变分自编码器（VAE）方法，用于从量子数据中提取物理意义明确的潜在表示，解决了传统方法忽略量子数据概率特性的问题。


<details>
  <summary>Details</summary>
Motivation: 量子数据的复杂性和随机性使得传统VAE难以提取有意义的物理特征，因此需要改进方法以更好地处理量子数据。

Method: 通过改进解码器以准确再现量子态，并设计适合量子数据的概率损失函数，使VAE能够学习物理意义明确的潜在表示。

Result: 在量子自旋模型和实验数据中验证了方法的有效性，能够自主发现相结构，无需先验知识或标签。

Conclusion: 该方法为量子系统的无监督和可解释研究提供了有力工具。
...
### [329] [Measuring multi-calibration](https://arxiv.org/abs/2506.11251)
*Ido Guy,Daniel Haimovich,Fridolin Linder,Nastaran Okati,Lorenzo Perini,Niek Tax,Mark Tygert*

Main category: stat.ME

TLDR: 论文提出了一种基于Kuiper统计量的新度量方法，用于衡量多校准性，并通过信号噪声比加权子群体贡献。
...
Method: 基于Kuiper统计量提出新度量，避免分箱或核密度估计的问题，并按信号噪声比加权子群体贡献。

Result: 数值实验验证了新度量的有效性，忽略信号噪声比会导致度量结果噪声增加。

Conclusion: 新度量方法在多校准性衡量中表现良好，信号噪声比加权是关键。
...
### [330] [Bias and Identifiability in the Bounded Confidence Model](https://arxiv.org/abs/2506.11751)
*Claudio Borile,Jacopo Lenti,Valentina Ghidini,Corrado Monti,Gianmarco De Francisci Morales*

Main category: stat.ME

TLDR: 论文分析了有界置信模型（BCMs）中两个关键参数（置信边界和收敛速率）的最大似然估计特性，发现置信边界估计存在小样本偏差但一致，收敛速率估计则有持续偏差，且联合估计存在可识别性问题。


<details>
  <summary>Details</summary>
Motivation: 连接有界置信模型与真实数据以理解共识、碎片化或极化现象，并测试模型假设。

Method: 使用最大似然估计方法分析置信边界和收敛速率的统计估计特性。

Result: 置信边界估计在小样本下有偏差但一致，收敛速率估计有持续偏差；联合估计在参数空间特定区域存在可识别性问题。

Conclusion: 似然函数分析有助于理解意见动力学模型参数估计的局限与可能性，并为校准提供形式化保证。
...
### [331] [Gradients of unitary optical neural networks using parameter-shift rule](https://arxiv.org/abs/2506.11565)
*Jinzhe Jiang,Yaqian Zhao,Xin Zhang,Chen Li,Yunlong Yu,Hailing Liu*

Main category: cs.ET

TLDR: 本文探讨了参数偏移规则（PSR）在单元光学神经网络（UONNs）中计算梯度的应用，提出了一种绕过光学系统物理限制的训练方法。
...
Method: 通过评估参数偏移后的函数值计算梯度，利用光学干涉的傅里叶级数特性直接从硬件测量中获取精确梯度。

Result: PSR成功应用于马赫-曾德尔干涉仪网络中的相位参数优化。

Conclusion: PSR为光学计算系统的高效硬件训练策略提供了潜在解决方案。
...
### [332] [Deep Symmetric Autoencoders from the Eckart-Young-Schmidt Perspective](https://arxiv.org/abs/2506.11641)
*Simone Brivio,Nicola Rares Franco*

Main category: math.NA

TLDR: 本文分析了对称自编码器的理论表达性，提出了基于EYS定理的初始化策略，并通过实验验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 尽管深度自编码器在应用中表现优异，但其理论表达性仍缺乏坚实基础，尤其是与经典投影方法相比。本文旨在填补这一空白。

Method: 通过数学分析对称自编码器的不同类别，提出基于EYS定理的初始化策略（EYS初始化）。

Result: 实验表明，EYS初始化策略在对称自编码器中优于传统方法，突显了模型设计和初始化的关键作用。

Conclusion: 对称自编码器的理论分析为理解其表达性提供了新视角，EYS初始化策略在实际应用中具有显著优势。
...
### [333] [Data-driven approaches to inverse problems](https://arxiv.org/abs/2506.11732)
*Carola-Bibiane Schönlieb,Zakhar Shumaylov*

Main category: math.NA

TLDR: 本文介绍了逆问题的数据驱动范式，包括传统方法及其局限性，以及现代基于深度学习的解决方案，如对抗正则化和可证明收敛的线性即插即用去噪器。


<details>
  <summary>Details</summary>
Motivation: 逆问题在医学成像、遥感等领域至关重要，但传统方法受限于建模和计算效率。数据驱动方法通过深度学习提供了更高效和准确的解决方案。

Method: 文章分为两部分：第一部分介绍逆问题和传统解法；第二部分探讨现代数据驱动方法，如对抗正则化和线性即插即用去噪器。

Result: 数据驱动方法在解决逆问题时展现出更高的准确性和计算效率，超越了传统方法的限制。

Conclusion: 数据驱动范式为逆问题提供了新的解决方案，但仍存在开放性问题，未来研究将进一步探索其潜力。
...
### [334] [Bubble Dynamics Transformer: Microrheology at Ultra-High Strain Rates](https://arxiv.org/abs/2506.11936)
*Lehu Bu,Zhaohan Yu,Shaoting Lin,Jan N. Fuhg,Jin Yang*

Main category: physics.flu-dyn

TLDR: 论文提出了一种基于机器学习的微流变学框架，利用激光诱导惯性空化（LIC）和Bubble Dynamics Transformer（BDT）神经网络，快速、非接触地表征生物材料在超高应变率下的粘弹性。


<details>
  <summary>Details</summary>
Motivation: 传统流变学工具在高应变率（>1000 1/s）下存在加载速度、分辨率或侵入性限制，需要一种新方法来研究软生物材料的力学特性。

Method: 通过超高速度成像捕捉LIC事件中气泡半径动态，利用基于物理模拟数据训练的BDT神经网络分析数据，推断材料粘弹性参数。

Result: BDT无需迭代拟合或复杂反演过程，即可准确推断材料粘弹性参数，实现快速、非接触表征。

Conclusion: 该方法为生物医学和材料科学中的极端加载条件提供了高效、准确的表征工具。
...
### [335] [PermRust: A Token-based Permission System for Rust](https://arxiv.org/abs/2506.11701)
*Lukas Gehring,Sebastian Rehms,Florian Tschorsch*

Main category: cs.PL

TLDR: 本文提出了一种基于Rust编程语言的权限系统PermRust，通过类型系统实现零成本抽象，以管理库级别的系统资源访问。


<details>
  <summary>Details</summary>
Motivation: 现代软件常使用第三方库代码，操作系统级别的权限系统仅能管理进程级访问，因此需要库级别的权限系统以增强安全性。

Method: 借鉴能力系统的概念，构建编程语言级别的权限系统理论基础，并实现为Rust的零成本抽象。

Result: 开发了PermRust，支持按库管理系统资源访问。

Conclusion: PermRust为库级别权限管理提供了理论和技术支持，增强了安全性。
...
### [336] [On Differential and Boomerang Properties of a Class of Binomials over Finite Fields of Odd Characteristic](https://arxiv.org/abs/2506.11486)
*Namhun Koo,Soonhak Kwon*

Main category: cs.IT

TLDR: 研究了有限域上一类二项式的微分和回旋镖性质，发现某些条件下函数具有局部PN性质且回旋镖均匀性为0，或局部APN性质且回旋镖均匀性至多为2。
...
Method: 分析一类二项式函数在不同条件下的微分和回旋镖性质，并分类其谱。

Result: 发现一类非PN函数在特定条件下具有回旋镖均匀性0或2，并提供了微分和回旋镖谱的完整分类。

Conclusion: 该研究为有限域上二项式函数的性质提供了新见解，特别是在奇特征域上的应用。
...
### [337] [AutoGen Driven Multi Agent Framework for Iterative Crime Data Analysis and Prediction](https://arxiv.org/abs/2506.11475)
*Syeda Kisaa Fatima,Tehreem Zubair,Noman Ahmed,Asifullah Khan*

Main category: cs.MA

TLDR: LUCID-MA是一个多智能体协作分析犯罪数据的AI框架，包含分析助手、反馈组件和预测组件，支持离线运行和自我改进。


<details>
  <summary>Details</summary>
Motivation: 利用多智能体协作提升犯罪数据分析的自主性和可扩展性，同时通过离线执行保障数据隐私。

Method: 基于LLaMA-2-13B-Chat-GPTQ模型，设计三组件框架，通过100轮通信实现自我改进，并引入评分函数评估性能。

Result: 展示了AutoGen式智能体在社会科学领域中的潜力，支持离线、可扩展和迭代分析。

Conclusion: LUCID-MA为犯罪数据分析提供了高效、隐私保护的解决方案，具有广泛应用前景。
...
### [338] [Shapley Machine: A Game-Theoretic Framework for N-Agent Ad Hoc Teamwork](https://arxiv.org/abs/2506.11285)
*Jianhong Wang,Yang Li,Samuel Kaski,Jonathan Lawry*

Main category: cs.MA

TLDR: 本文提出了一种基于合作博弈论的方法来解决开放多智能体系统中的n智能体临时团队（NAHT）问题，通过扩展游戏空间和状态空间，并利用Shapley值分配信用，提出了一种名为Shapley Machine的强化学习算法。
...
Method: 将开放多智能体系统建模为合作博弈实例，扩展游戏空间和状态空间以处理动态场景，利用Shapley值分配信用，并提出Shapley Machine算法。

Result: 实验验证了Shapley Machine的有效性，并证明了理论的合理性。

Conclusion: 本文首次将合作博弈论概念直接与强化学习概念关联，为开放多智能体系统的信用分配提供了理论支持。
...
### [339] [Decadal sink-source shifts of forest aboveground carbon since 1988](https://arxiv.org/abs/2506.11879)
*Zhen Qian,Sebastian Bathiany,Teng Liu,Lana L. Blaschke,Hoong Chen Teo,Niklas Boers*

Main category: physics.geo-ph

TLDR: 全球森林在1988-2021年间总体为碳汇，但热带湿润森林和北方森林逐渐转变为碳源，热带森林的碳汇-源转变尤为显著。


<details>
  <summary>Details</summary>
Motivation: 研究全球森林地上碳（AGC）的动态变化及其对碳循环的影响，以应对气候变化和森林管理的需求。

Method: 结合多源卫星观测数据和概率深度学习模型，估算高空间分辨率的全球森林AGC及其不确定性。

Result: 全球森林30年间净吸收6.2 PgC，但热带湿润森林和北方森林逐渐转变为碳源；巴西亚马逊未受干扰区域的碳损失贡献显著增加。

Conclusion: 热带森林AGC在调节陆地碳循环中的作用增强，人为气候变化可能加剧未受干扰区域的碳损失。
...
### [340] [Complexity of normalized stochastic first-order methods with momentum under heavy-tailed noise](https://arxiv.org/abs/2506.11214)
*Chuan He,Zhaosong Lu,Defeng Sun,Zhanwang Deng*

Main category: math.OC

TLDR: 提出了一种实用的归一化随机一阶方法，结合Polyak动量、多外推动量和递归动量，用于解决无约束优化问题。方法动态更新参数，无需依赖问题相关量（如Lipschitz常数或噪声边界）。在重尾噪声和弱平均平滑条件下，给出了近似随机驻点的一阶Oracle复杂度结果，优于或匹配现有文献。数值实验验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 解决无约束优化问题时，现有方法通常依赖问题相关参数（如Lipschitz常数），且噪声和平滑性假设较强。本文旨在提出更通用的方法，减少对强假设的依赖。

Method: 结合Polyak动量、多外推动量和递归动量，动态更新算法参数，无需显式问题相关量。适用于重尾噪声和弱平均平滑条件。

Result: 在重尾噪声和弱平均平滑条件下，复杂度结果优于或匹配现有文献。数值实验验证了方法的实际效果。

Conclusion: 提出的方法在更弱的假设下实现了高效优化，具有理论和实际优势。
...
### [341] [Quantum Learning and Estimation for Distribution Networks and Energy Communities Coordination](https://arxiv.org/abs/2506.11730)
*Yingrui Zhuang,Lin Cheng,Yuji Cao,Tongxin Li,Ning Qi,Yan Xu,Yue Chen*

Main category: math.OC

TLDR: 论文提出了一种量子学习和估计方法，通过量子特性提升电网与能源社区间的协调效率，显著提高了准确性和计算速度。


<details>
  <summary>Details</summary>
Motivation: 解决电网与能源社区间信息有限和计算负担高的问题，以提升协调效率和可靠性。

Method: 开发了混合量子时序卷积网络-长短期记忆模型（Q-TCN-LSTM）和基于量子幅度估计的量子估计方法。

Result: Q-TCN-LSTM模型比传统神经网络准确率提升69.2%，模型大小减少99.75%，计算时间减少93.9%；量子幅度估计比蒙特卡洛模拟计算时间减少99.99%。
...
### [342] [Convergence of Momentum-Based Optimization Algorithms with Time-Varying Parameters](https://arxiv.org/abs/2506.11904)
*Mathukumalli Vidyasagar*

Main category: math.OC

TLDR: 本文提出了一种统一的随机优化算法，利用“动量”项，将SHB和SNAG算法作为特例，并允许动量随时间变化。假设条件最广泛，适用于零阶方法。给出了收敛的充分条件，并分析了另一种方法的不可行性。


<details>
  <summary>Details</summary>
Motivation: 提出一种更通用的随机优化算法，扩展现有方法的应用范围，特别是针对零阶方法的需求。

Method: 结合动量项，允许其随时间变化，并放宽对随机梯度的假设条件，使其适用于更广泛的问题。

Result: 提出了收敛的充分条件，验证了算法的可行性，同时指出另一种方法的不可行性。

Conclusion: 该统一算法在广泛条件下有效，为零阶方法提供了理论支持，同时揭示了某些方法的局限性。
...
### [343] [Grids Often Outperform Implicit Neural Representations](https://arxiv.org/abs/2506.11139)
*Namhoon Kim,Sara Fridovich-Keil*

Main category: eess.IV

TLDR: 研究探讨了隐式神经表示（INRs）的性能，比较了其在2D/3D信号任务中的表现，发现网格表示在多数情况下优于INRs，但INRs在特定场景（如低维结构信号）中表现更好。


<details>
  <summary>Details</summary>
Motivation: INRs的性能、隐式偏差和扩展行为尚不明确，需通过实验验证其在不同任务和信号类型中的表现。

Method: 通过分层实验，比较不同INRs和网格表示在多种任务（如断层扫描、超分辨率、去噪）中的性能，分析其参数分配。

Result: 多数任务中，带插值的正则化网格训练更快且质量更高；INRs仅在低维结构信号（如形状轮廓）中表现更优。

Conclusion: INRs在特定场景中具有优势，未来应针对其优势领域应用；网格表示在多数情况下更高效。
...
### [344] [ADAgent: LLM Agent for Alzheimer's Disease Analysis with Collaborative Coordinator](https://arxiv.org/abs/2506.11150)
*Wenlong Hou,Gangqian Yang,Ye Du,Yeung Lau,Lihao Liu,Junjun He,Ling Long,Shujun Wang*

Main category: eess.IV

TLDR: ADAgent是一个基于大语言模型的AI代理，用于阿尔茨海默病的多模态诊断和预后任务，显著提升了准确率。


<details>
  <summary>Details</summary>
Motivation: 早期和精确诊断阿尔茨海默病至关重要，但现有方法多依赖单模态数据，无法处理多模态或缺失输入。

Method: ADAgent整合推理引擎、专业医疗工具和协作结果协调器，支持多模态诊断和预后任务。

Result: 实验表明，ADAgent在多模态诊断、预后及MRI和PET诊断任务中均优于现有方法，准确率提升显著。

Conclusion: ADAgent为阿尔茨海默病分析提供了首个专用AI代理，支持多模态输入和高级方法集成，性能优越。
...
### [345] [Vector Representations of Vessel Trees](https://arxiv.org/abs/2506.11163)
*James Batten,Michiel Schaap,Matthew Sinclair,Ying Bai,Ben Glocker*

Main category: eess.IV

TLDR: VeTTA框架通过两阶段Transformer自编码器学习3D血管网络的向量表示，降低GPU内存需求，实现高保真重建和拓扑一致性。


<details>
  <summary>Details</summary>
Motivation: 传统3D卷积模型在建模树状几何数据（如血管网络）时内存需求高，且难以保持拓扑结构。

Method: 采用两阶段Transformer自编码器：第一阶段捕捉血管段几何细节，第二阶段编码网络拓扑结构，并通过递归解码确保树状结构有效性。

Result: 在2D合成树和3D冠状动脉数据集上表现优异，重建保真度高，拓扑准确，支持潜在空间插值。
...
### [346] [DiffPR: Diffusion-Based Phase Reconstruction via Frequency-Decoupled Learning](https://arxiv.org/abs/2506.11183)
*Yi Zhang*

Main category: eess.IV

TLDR: 论文提出DiffPR框架，通过取消高频跳跃连接和引入扩散模型，解决深度学习中过平滑问题，提升定量相位成像的细节恢复能力。


<details>
  <summary>Details</summary>
Motivation: 深度学习在离轴定量相位成像（QPI）中存在过平滑问题，导致高频细节丢失。传统U-Net因频谱偏差而偏向低频内容，影响诊断细节的恢复。

Method: 提出DiffPR框架，分两阶段：1）取消高频跳跃连接的U-Net预测低频相位图；2）扩散模型通过反向去噪恢复高频残差。

Result: 在四个QPI数据集上，DiffPR显著优于U-Net基线，PSNR提升1.1 dB，MAE降低11%，细节恢复更清晰。

Conclusion: 取消高频跳跃连接并利用扩散模型合成细节，是解决频谱偏差的有效方法，显著提升相位恢复网络的性能。
...
### [347] [Joint Denoising of Cryo-EM Projection Images using Polar Transformers](https://arxiv.org/abs/2506.11283)
*Joakim Andén,Justus Sagemüller*

Main category: eess.IV

TLDR: 论文提出了一种基于Transformer的神经网络架构，用于同时聚类、对齐和去噪冷冻电镜图像，显著优于传统单图像DNN方法。


<details>
  <summary>Details</summary>
Motivation: 传统DNN在高噪声环境（如冷冻电镜投影图像）中效果有限，但数据中存在大量同一分子的冗余投影信息，可用于改进去噪。

Method: 基于Transformer的神经网络架构，同时实现图像聚类、对齐和去噪。

Result: 在合成数据上，该方法将相对均方误差（MSE）降低了45%（信噪比为0.03时）。

Conclusion: 该方法有效利用了冗余信息，显著提升了高噪声环境下的去噪性能。
...
### [348] [FAD-Net: Frequency-Domain Attention-Guided Diffusion Network for Coronary Artery Segmentation using Invasive Coronary Angiography](https://arxiv.org/abs/2506.11454)
*Nan Mu,Ruiqi Song,Xiaoning Li,Zhihui Xu,Jingfeng Jiang,Chen Zhao*

Main category: eess.IV

TLDR: 提出了一种基于频域分析的深度学习模型FAD-Net，用于提高冠状动脉分割和狭窄检测的准确性。
...
Result: FAD-Net在冠状动脉分割中平均Dice系数达0.8717，狭窄检测的阳性预测值为0.6398，优于现有方法。
...
### [349] [Taming Stable Diffusion for Computed Tomography Blind Super-Resolution](https://arxiv.org/abs/2506.11496)
*Chunlei Li,Yilei Shi,Haoxi Hu,Jingliang Hu,Xiao Xiang Zhu,Lichao Mou*

Main category: eess.IV

TLDR: 提出一种基于Stable Diffusion的CT超分辨率框架，结合退化模型和文本描述生成，显著提升图像质量并降低辐射剂量。
...
Method: 使用退化模型合成低质量CT图像，结合预训练视觉语言模型生成文本描述，通过Stable Diffusion进行超分辨率重建。
...
### [350] [FCA2: Frame Compression-Aware Autoencoder for Modular and Fast Compressed Video Super-Resolution](https://arxiv.org/abs/2506.11545)
*Zhaoyang Wang,Jie Li,Wen Lu,Lihuo He,Maoguo Gong,Xinbo Gao*

Main category: eess.IV

TLDR: 提出了一种基于压缩驱动降维的高效视频超分辨率方法，显著减少计算复杂度和推理时间。


<details>
  <summary>Details</summary>
Motivation: 现有视频超分辨率模型存在推理时间长、训练流程复杂、依赖辅助信息等问题，且传统方法难以满足高帧率视频的需求。

Method: 利用高光谱图像与视频数据的结构相似性，提出压缩驱动降维策略，设计模块化架构以兼容现有框架。

Result: 实验表明，该方法性能与或优于当前SOTA模型，同时显著减少推理时间。

Conclusion: 该方法为视频超分辨率技术提供了高效实用的解决方案，代码已开源。
...
### [351] [Brain Network Analysis Based on Fine-tuned Self-supervised Model for Brain Disease Diagnosis](https://arxiv.org/abs/2506.11671)
*Yifei Tang,Hongjie Jiang,Changhong Jing,Hieu Pham,Shuqiang Wang*

Main category: eess.IV

TLDR: 提出了一种基于多维度扩展的脑网络微调模型，用于脑疾病诊断，提升了模型的泛化能力。
...
Result: 模型在脑疾病诊断中表现优异，提供了脑网络分析的新方法。

Conclusion: 该模型为脑网络分析研究提供了有前景的解决方案。
...
### [352] [Exploring the Effectiveness of Deep Features from Domain-Specific Foundation Models in Retinal Image Synthesis](https://arxiv.org/abs/2506.11753)
*Zuzanna Skorniewska,Bartlomiej W. Papiez*

Main category: eess.IV

TLDR: 该研究探讨了在医学影像生成中，基于领域特定深度特征的距离损失函数是否优于感知损失和边缘检测损失函数，结果发现传统边缘检测滤波器在提升合成样本血管结构清晰度方面更有效。


<details>
  <summary>Details</summary>
Motivation: 医学影像生成面临隐私、数据稀缺、成本高和人口偏见等问题，深度生成模型提供了一种解决方案，但需要验证形态和临床准确性。

Method: 研究比较了基于领域特定深度特征的距离损失函数与感知损失和边缘检测损失函数的效果。

Result: 领域特定深度特征并未提升自动编码器的图像生成质量，而传统边缘检测滤波器显著改善了合成样本中血管结构的清晰度。

Conclusion: 传统边缘检测滤波器在医学影像生成中表现更优，领域特定深度特征的优势未被证实。
...
### [353] [Framework of a multiscale data-driven digital twin of the muscle-skeletal system](https://arxiv.org/abs/2506.11821)
*Martina Paccini,Simone Cammarasana,Giuseppe Patanè*

Main category: eess.IV

TLDR: 本文提出了一种名为MS-DT的新型框架，通过整合多尺度生物力学数据和计算建模，为肌肉骨骼系统提供个性化的数字孪生模型，用于精准诊断和治疗。


<details>
  <summary>Details</summary>
Motivation: 肌肉骨骼疾病（MSDs）是全球致残的主要原因，需要先进的诊断和治疗工具。数字孪生（DT）范式因其能整合异构数据源，成为有效的管理方案。

Method: MS-DT框架结合了运动捕捉、超声成像、肌电图和医学成像，通过计算建模创建患者特定的肌肉骨骼系统模型，并提供交互式可视化平台。

Result: MS-DT能精确提取运动学和动态组织特征，为脊柱生物力学监测和康复提供全面工具，实现了高保真建模和实时可视化。

Conclusion: MS-DT框架通过高精度建模和实时可视化，显著提升了患者特异性诊断和干预规划的准确性。
...
### [354] [Structural Similarity-Inspired Unfolding for Lightweight Image Super-Resolution](https://arxiv.org/abs/2506.11823)
*Zhangkai Ni,Yang Zhang,Wenhan Yang,Hanli Wang,Shiqi Wang,Sam Kwong*

Main category: eess.IV

TLDR: 提出了一种基于结构相似性启发的展开方法（SSIU），用于高效图像超分辨率，结合数据驱动和模型驱动的优势，实现高性能且紧凑的模型。


<details>
  <summary>Details</summary>
Motivation: 当前数据驱动的图像超分辨率方法通常通过增加模型深度或使用注意力机制扩展感受野，导致模型复杂度高。模型驱动的展开方法则能保持模型紧凑性，但性能有待提升。

Method: SSIU方法通过展开一个受结构相似性约束的优化函数设计，包含混合尺度门控模块（MSGM）和高效稀疏注意力模块（ESAM），并利用基于专家混合的特征选择器（MoE-FS）整合多级特征。

Result: 实验表明，该方法在性能上优于当前最先进模型，同时参数更少、内存消耗更低。
...
### [355] [MindGrab for BrainChop: Fast and Accurate Skull Stripping for Command Line and Browser](https://arxiv.org/abs/2506.11860)
*Armina Fani,Mike Doan,Isabelle Le,Alex Fedorov,Malte Hoffmann,Chris Rorden,Sergey Plis*

Main category: eess.IV

TLDR: MindGrab是一种参数和内存高效的深度全卷积模型，用于多模态头部图像的颅骨剥离，性能优于传统方法且资源需求低。


<details>
  <summary>Details</summary>
Motivation: 开发一种高效且适用于多模态头部图像的颅骨剥离工具，以降低计算资源需求并提升可访问性。

Method: 基于频谱解释的扩张卷积架构，仅使用模态无关的合成数据进行训练。

Result: 在606例多模态脑扫描数据上，MindGrab的平均Dice得分为95.9，显著优于ROBEX和BET，与SynthStrip性能相当但参数减少95%。
...
### [356] [crossMoDA Challenge: Evolution of Cross-Modality Domain Adaptation Techniques for Vestibular Schwannoma and Cochlea Segmentation from 2021 to 2023](https://arxiv.org/abs/2506.12006)
...
TLDR: crossMoDA挑战系列专注于无监督跨模态分割，从ceT1 MRI迁移到T2 MRI，旨在自动化VS和耳蜗分割。挑战目标逐年演进，数据集扩大减少了异常值，但2023年耳蜗分割性能下降。


<details>
  <summary>Details</summary>
Motivation: 通过跨模态分割自动化VS和耳蜗分割，以降低临床管理成本，并作为域适应的代表性基准。
...
### [357] [Score-based Generative Diffusion Models to Synthesize Full-dose FDG Brain PET from MRI in Epilepsy Patients](https://arxiv.org/abs/2506.11297)
*Jiaqi Wu,Jiahong Ouyang,Farshad Moradi,Mohammad Mehdi Khalighi,Greg Zaharchuk*

Main category: eess.IV

TLDR: 研究比较了扩散模型和非扩散模型在癫痫患者MRI到PET图像转换中的表现，发现扩散模型在纯MRI输入时表现最佳，而所有模型在加入低剂量PET后性能显著提升。


<details>
  <summary>Details</summary>
Motivation: 由于癫痫患者需要同时评估脑结构和代谢，但FDG PET的辐射剂量对年轻患者不利，因此研究旨在通过生成式AI方法合成高质量PET图像。

Method: 研究比较了两种基于分数的生成扩散模型（SGM-KD和SGM-VP）和Transformer-Unet模型，使用52名受试者的同步PET/MRI数据。

Result: SGM-KD在纯MRI输入时表现最佳，而所有模型在加入1%低剂量PET后性能显著提升。

Conclusion: 扩散模型在纯MRI到PET转换中潜力巨大，而所有模型均可通过MRI和极低剂量PET准确合成全剂量FDG-PET。
...
### [358] [Solving Inverse Problems in Stochastic Self-Organising Systems through Invariant Representations](https://arxiv.org/abs/2506.11796)
*Elias Najarro,Nicolas Bessone,Sebastian Risi*

Main category: nlin.AO

TLDR: 提出了一种新的逆向建模方法，利用视觉嵌入处理可观测空间中的随机性，有效恢复未知因果参数。


<details>
  <summary>Details</summary>
Motivation: 解决传统逆向方法在随机性强的观测数据中无法捕捉特征相似性的问题。

Method: 通过将模式表示映射到不变的嵌入空间，无需手工设计目标函数或启发式方法。

Result: 在反应扩散系统和社会隔离的基于代理模型中可靠恢复参数，并应用于真实生物模式。

Conclusion: 该方法为研究复杂随机模式形成的动态提供了理论和实验工具。
...
### [359] [Convergence of physics-informed neural networks modeling time-harmonic wave fields](https://arxiv.org/abs/2506.11395)
*Stefan Schoder,Aneta Furmanová,Viktor Hruška*

Main category: cs.CE

TLDR: 研究物理信息神经网络（PINN）用于建模偏微分方程以求解声波场，在二维简单几何中取得初步成果。本文探讨了3D低频室内声学案例，评估了PINN的收敛行为。


<details>
  <summary>Details</summary>
Motivation: 现有数值模型在解决声波场问题时存在局限性，PINN需克服从2D到3D、声源建模、边界条件及复数解等问题，以提升准确性。

Method: 使用Helmholtz方程计算时谐波场，研究3D案例，调整声源定义和边界条件集，采用复数声速模型模拟吸收效应，并通过损失景观和L2误差评估收敛。

Result: 研究表明，至少需要每波长六个训练点才能确保PINN的准确训练和预测。

Conclusion: PINN在低频室内声学建模中具有潜力，但仍需进一步优化以处理复杂场景。
...
### [360] [CLEAN-MI: A Scalable and Efficient Pipeline for Constructing High-Quality Neurodata in Motor Imagery Paradigm](https://arxiv.org/abs/2506.11830)
*Dingkun Liu,Zhu Chen,Dongrui Wu*

Main category: cs.CE

TLDR: 提出CLEAN-MI，一种用于构建大规模、高质量运动想象（MI）脑机接口（BCI）数据的系统化流程，通过多步骤处理提升数据质量和分类性能。


<details>
  <summary>Details</summary>
Motivation: EEG信号存在噪声、异构性和个体差异，影响模型训练效果，需构建高质量数据集以支持基础模型开发。

Method: CLEAN-MI整合频带滤波、通道模板选择、受试者筛选和边缘分布对齐，标准化多源EEG数据。

Result: 在多个公共MI数据集上验证，显著提升数据质量和分类性能。

Conclusion: CLEAN-MI为构建高效、准确的MI神经数据提供了可扩展的解决方案。
...
### [361] [Generative Representational Learning of Foundation Models for Recommendation](https://arxiv.org/abs/2506.11999)
*Zheli Zhou,Chenxu Zhu,Jianghao Lin,Bo Chen,Ruiming Tang,Weinan Zhang,Yong Yu*

Main category: cs.IR

TLDR: RecFound是一个推荐基础模型的生成表示学习框架，解决了多任务学习中的知识共享与冲突、收敛速度不一致等问题，并在实验中表现优异。


<details>
  <summary>Details</summary>
Motivation: 开发一个能在多样化任务中表现出色的基础模型是人工智能领域的长期目标，但现有推荐基础模型常忽略嵌入任务，且难以处理多任务学习的复杂性。

Method: 构建了首个全面的推荐基础模型数据集，并提出多任务训练方案，包括TMoLE处理知识共享与冲突、S2Sched解决收敛不一致问题，以及Model Merge模块平衡任务性能。

Result: 实验表明，RecFound在多种推荐任务中表现优于现有基线，达到最先进水平。

Conclusion: RecFound通过创新的多任务训练方案，有效解决了推荐基础模型中的关键挑战，为未来研究提供了重要参考。
...
### [362] [Deep Learning Model Acceleration and Optimization Strategies for Real-Time Recommendation Systems](https://arxiv.org/abs/2506.11421)
*Junli Shao,Jing Dong,Dingzhou Wang,Kowei Shih,Dannier Li,Chengrui Zhou*

Main category: cs.IR

TLDR: 论文提出了一种结合模型和系统优化的方法，显著降低实时推荐系统的延迟并提高吞吐量，同时保持推荐质量。


<details>
  <summary>Details</summary>
Motivation: 随着互联网服务的快速增长，实时推荐系统面临高计算成本和资源瓶颈的挑战。

Method: 在模型层面采用轻量级网络设计、结构化剪枝和权重量化；在系统层面整合异构计算平台、高性能推理库，并设计弹性推理调度和负载均衡机制。
...
### [363] [GraphRAG-Causal: A novel graph-augmented framework for causal reasoning and annotation in news](https://arxiv.org/abs/2506.11600)
*Abdul Haque,Umm e Hani,Ahmad Din,Muhammad Babar,Ali Abbas,Insaf Ullah*

Main category: cs.IR

TLDR: GraphRAG-Causal结合图检索与大型语言模型，提升新闻分析中的因果推理能力，F1-score达82.1%。


<details>
  <summary>Details</summary>
Motivation: 传统NLP方法难以识别复杂隐式因果关系，尤其在数据稀缺场景。

Method: 三阶段流程：数据准备（标注新闻标题为因果图）、图检索（混合语义与结构检索）、LLM推理（少样本学习）。

Result: 实验显示仅需20个少样本即可实现82.1%的F1-score。

Conclusion: 框架显著提升准确性，适用于新闻可靠性评估、虚假信息检测等实时应用。
...
### [364] [Security Degradation in Iterative AI Code Generation -- A Systematic Analysis of the Paradox](https://arxiv.org/abs/2506.11022)
*Shivani Shukla,Himanshu Joshi,Romilla Syed*

Main category: cs.SE

TLDR: 研究发现，通过LLM迭代反馈生成的代码中，安全漏洞显著增加，挑战了迭代改进能提升代码安全性的假设，并强调了人类验证的重要性。
...
Conclusion: 迭代LLM反馈可能引入新安全问题，需结合人类验证，并提出实用指南以降低风险。
...
### [365] [Application Modernization with LLMs: Addressing Core Challenges in Reliability, Security, and Quality](https://arxiv.org/abs/2506.10984)
*Ahilan Ayyachamy Nadar Ponnusamy*

Main category: cs.SE

TLDR: 本文探讨了AI辅助代码生成工具的挑战，并提出了一种结合LLMs代码推理与生成能力及人类专业知识的框架，以解决应用现代化问题。


<details>
  <summary>Details</summary>
Motivation: 尽管AI辅助代码生成工具提高了效率，但仍存在安全性、可靠性和代码质量等问题，需通过结合人类专业知识来解决。

Method: 提出一个框架，整合LLMs的代码推理与生成能力，并结合人类指导，应用于应用现代化。

Result: 通过详细案例研究展示了框架的实用性，并提供了参考实现。

Conclusion: 该框架为AI驱动的应用现代化提供了可行方案，并强调了人类参与的重要性。
...
### [366] [Prompt engineering and framework: implementation to increase code reliability based guideline for LLMs](https://arxiv.org/abs/2506.10989)
*Rogelio Cruz,Jonatan Contreras,Francisco Guerrero,Ezequiel Rodriguez,Carlos Valdez,Citlali Carrillo*

Main category: cs.SE

TLDR: 提出一种新颖的提示方法，提升大语言模型生成准确Python代码的能力，实验表明其优于零样本和思维链方法，且更节省资源。
...
Conclusion: 定制化提示策略能优化代码生成性能，为AI驱动的编程任务提供更高效解决方案。
...
### [367] [On the Effectiveness of the 'Follow-the-Sun' Strategy in Mitigating the Carbon Footprint of AI in Cloud Instances](https://arxiv.org/abs/2506.10990)
*Roberto Vergallo,Luís Cruz,Alessio Errico,Luca Mainetti*

Main category: cs.SE

TLDR: 论文研究了'Follow-the-Sun'（FtS）策略在减少AI工作负载碳足迹方面的效果，通过实验证明其优于其他策略。


<details>
  <summary>Details</summary>
Motivation: AI的高能耗问题引发关注，但缺乏科学证据支持FtS策略的有效性，因此本研究旨在填补这一空白。
...
### [368] [Evaluating LLMs for Visualization Tasks](https://arxiv.org/abs/2506.10996)
*Saadiq Rauf Khan,Vinit Chandak,Sougata Mukherjea*

Main category: cs.SE

TLDR: LLMs can generate visualization code and answer questions about visualizations, but have limitations. Insights can improve LLMs and visualization systems.


<details>
  <summary>Details</summary>
Motivation: Explore the capabilities of LLMs in generating visualization code and understanding common visualizations.

Method: Evaluate popular LLMs by testing their ability to generate code from prompts and answer questions about visualizations.

Result: LLMs can generate some visualization code and answer questions, but exhibit limitations.

Conclusion: Findings can help improve both LLMs and Information Visualization systems.
...
### [369] [Towards Automated Formal Verification of Backend Systems with LLMs](https://arxiv.org/abs/2506.10998)
*Kangping Xu,Yifan Luo,Yang Yuan,Andrew Chi-Chih Yao*

Main category: cs.SE

TLDR: 提出了一种利用函数式编程和类型系统将Scala后端代码转换为形式化Lean表示的新框架，通过LLM验证自动生成定理，显著提高了测试效率和成本效益。


<details>
  <summary>Details</summary>
Motivation: 现有自动化测试方法在测试局部性、通用可靠性和业务逻辑识别方面存在局限性，无法匹敌人类工程师的能力。

Method: 将Scala代码转换为Lean形式化表示，自动生成定理并使用LLM验证，证明成功则逻辑正确，否则需人工干预。

Result: 在真实后端系统中验证了50%以上的测试需求，LLM验证成本仅为每API 2.19美元，显著优于人工测试。

Conclusion: 该方法为可扩展的AI驱动软件测试提供了有前景的方向，有望大幅提升工程生产力。
...
### [370] [Automated Validation of COBOL to Java Transformation](https://arxiv.org/abs/2506.10999)
...
TLDR: 论文提出了一种框架和工具，用于验证COBOL与翻译后的Java代码的等价性，并通过符号执行生成测试用例以确保语义一致性。


<details>
  <summary>Details</summary>
Motivation: 尽管基于LLM的代码翻译结果令人鼓舞，但翻译后的代码无法完全信任其正确性，因此需要一种验证和修复机制。

Method: 采用符号执行生成COBOL程序的单元测试，并生成等效的JUnit测试用例，通过运行测试验证语义等价性。

Result: 开发了工具来自动生成测试用例并验证翻译代码的正确性，同时提供修复反馈和改进AI模型的建议。

Conclusion: 该框架有效验证了代码翻译的语义等价性，并为修复和改进提供了支持。
...
### [371] [Rethinking Technological Readiness in the Era of AI Uncertainty](https://arxiv.org/abs/2506.11001)
*S. Tucker Browne,Mark M. Bailey*

Main category: cs.SE

TLDR: 提出了一种新的AI准备框架，用于评估军事系统中AI组件的成熟度和可信度，以解决现有技术准备评估无法涵盖AI特有因素的问题。


<details>
  <summary>Details</summary>
Motivation: 当前的技术准备评估未能充分捕捉AI特有因素，可能导致部署风险，因此需要一种专门针对AI的评估框架。

Method: 提出了一种类似于传统技术准备水平（TRL）但针对AI扩展的框架，结合现有数据评估工具和测试实践。

Result: 框架的可行性已在近期实施中得到验证，为军事决策者提供了更清晰的AI系统性能、透明度和人机集成标准。

Conclusion: 该框架为军事AI系统的可靠性和安全性提供了结构化评估方法，推动了国防技术管理和风险评估的进步。
...
### [372] [EmbedAgent: Benchmarking Large Language Models in Embedded System Development](https://arxiv.org/abs/2506.11003)
*Ruiyang Xu,Jialun Cao,Mingyuan Wu,Wenliang Zhong,Yaojie Lu,Ben He,Xianpei Han,Shing-Chi Cheung,Le Sun*

Main category: cs.SE

TLDR: EmbedAgent 是一个用于评估大型语言模型（LLMs）在嵌入式系统开发中能力的范式，通过 Embedbench 基准测试发现 LLMs 在电路设计和跨平台迁移任务中表现不一，并提出两种策略显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏评估 LLMs 在嵌入式系统开发中能力的基准测试，EmbedAgent 旨在填补这一空白，模拟真实开发角色以全面测试 LLMs 的能力。

Method: 提出 Embedbench 基准测试，包含 126 个案例，覆盖 9 种电子组件和 3 种硬件平台，测试 10 种主流 LLMs 的表现。

Result: LLMs 在简单任务中表现不佳（如 DeepSeek-R1 仅 55.6% 通过率），但在某些任务（如 MicroPython 迁移）中表现较好（73.8% 通过率）。提出的策略显著提升了性能（如 DeepSeek-R1 通过率提升至 65.1%）。

Conclusion: LLMs 在嵌入式系统开发中潜力巨大但仍有局限，通过检索增强生成和编译器反馈策略可显著提升其性能。
...
### [373] [Impact of Comments on LLM Comprehension of Legacy Code](https://arxiv.org/abs/2506.11007)
...
TLDR: 评估大型语言模型（LLM）对遗留代码的理解能力，提出使用多选问答（MCQA）方法，并探讨文档对LLM理解的影响。


<details>
  <summary>Details</summary>
Motivation: LLM在软件工程任务中表现优异，但对遗留语言的理解能力尚未充分研究，尤其是缺乏或文档不准确的情况。

Method: 采用多选问答（MCQA）方法，定量评估LLM对遗留代码的理解，并分析文档的影响。

Result: 初步发现文档的存在和准确性对LLM理解遗留代码有显著影响。

Conclusion: 需进一步研究文档对LLM理解遗留代码的作用，并提出未来工作的战略目标。
...
### [374] [Extracting Knowledge Graphs from User Stories using LangChain](https://arxiv.org/abs/2506.11020)
*Thayná Camargo da Silva*

Main category: cs.SE

TLDR: 提出了一种利用大语言模型自动从用户故事生成知识图谱的新方法，通过LangChain框架和用户故事图转换模块实现自动化。


<details>
  <summary>Details</summary>
Motivation: 提升用户需求和领域概念的可视化与理解，以更好地对齐软件功能与用户期望。

Method: 基于LangChain框架开发用户故事图转换模块，利用大语言模型提取节点和关系，构建知识图谱，并通过脚本实现自动化。

Result: 实现了知识图谱提取和评估的自动化，增强了用户需求的可视化。

Conclusion: 该方法有助于更有效和以用户为中心的软件开发过程。
...
### [375] [Eliminating Hallucination-Induced Errors in LLM Code Generation with Functional Clustering](https://arxiv.org/abs/2506.11021)
*Chaitanya Ravuri,Saman Amarasinghe*

Main category: cs.SE

TLDR: 论文提出了一种名为功能聚类的黑盒包装方法，通过采样多个候选程序并执行自生成的测试套件，显著减少了代码生成LLM的幻觉错误，同时提供可调的置信度分数。


<details>
  <summary>Details</summary>
Motivation: 现代代码生成LLM虽然能解决大部分编程问题，但仍会产生细微的错误，导致输出不安全。需要一种方法减少这些错误并提高可靠性。

Method: 功能聚类方法通过采样多个候选程序，执行自生成的测试套件，并将行为相同的程序聚类，以最大聚类的经验质量作为置信度估计。

Result: 在LiveCodeBench上，该方法将错误率从约65%降至2%，并在保守阈值下将错误率降至0%，同时仍能回答15.6%的提示。

Conclusion: 功能聚类方法为代码生成提供了一条实用路径，适用于闭源API和未来模型，未来工作可专注于提升规范清晰度。
...
### [376] [Refactoring Codebases through Library Design](https://arxiv.org/abs/2506.11058)
*Ziga Kovacic,Celine Lee,Justin Chiu,Wenting Zhao,Kevin Ellis*

Main category: cs.SE

TLDR: 论文提出了一种名为Librarian的方法和Minicode基准，用于评估代码代理在代码重构中的能力，旨在生成可重用库并提高代码的压缩率和正确性。


<details>
  <summary>Details</summary>
Motivation: 随着代码代理在解决独立编程问题上的准确性提高，如何通过重构将专用解决方案转化为可重用组件成为关键挑战。

Method: 提出Librarian方法，采用sample-and-rerank策略生成可重用库，并设计Minicode基准测试代码代理的压缩和重构能力。

Result: Librarian在压缩率和正确性上优于现有代码代理，压缩率提高1.6-2倍，同时提升正确性。

Conclusion: 论文展示了代码代理在支持代码增长和可重用性方面的潜力，并开源了相关代码和基准。
...
### [377] [Code Researcher: Deep Research Agent for Large Systems Code and Commit History](https://arxiv.org/abs/2506.11060)
*Ramneet Singh,Sathvik Joel,Abhav Mehrotra,Nalin Wadhwa,Ramakrishna B Bairi,Aditya Kanade,Nagarajan Natarajan*

Main category: cs.SE

TLDR: 论文提出了一种名为Code Researcher的深度研究代理，用于解决系统代码中的崩溃问题，通过多步推理和结构化记忆显著提升了补丁生成效果。


<details>
  <summary>Details</summary>
Motivation: 系统代码的复杂性和规模使得修改任务极具挑战性，现有LLM代理在系统代码上的表现尚未充分探索。

Method: 设计Code Researcher代理，通过多步推理分析代码语义、模式和提交历史，并将上下文存储在结构化记忆中以合成补丁。
...
### [378] [CoQuIR: A Comprehensive Benchmark for Code Quality-Aware Information Retrieval](https://arxiv.org/abs/2506.11066)
...
TLDR: 论文介绍了CoQuIR，一个多语言、大规模的质量感知代码检索基准，覆盖正确性、效率、安全性和可维护性四个维度，并提出了两种评估指标。研究发现现有模型难以区分代码质量，同时探索了提升质量感知能力的训练方法。


<details>
  <summary>Details</summary>
Motivation: 当前代码检索基准主要关注功能相关性，忽视了软件质量的关键维度，因此需要开发一个质量感知的基准。

Method: 提出CoQuIR基准，包含42,725个查询和134,907个代码片段的质量标注，并设计了两种质量中心评估指标。对23种检索模型进行评测，并探索了提升质量感知能力的训练方法。

Result: 现有模型在区分代码质量方面表现不佳，但通过合成数据集训练，质量感知能力有所提升，且不影响语义相关性。下游代码生成实验验证了方法的有效性。
...
### [379] [LeanExplore: A search engine for Lean 4 declarations](https://arxiv.org/abs/2506.11085)
*Justin Asher*

Main category: cs.SE

TLDR: LeanExplore是一个针对Lean 4生态系统的搜索引擎，支持语义搜索声明，结合多源语义嵌入模型、BM25+和PageRank评分，提供网站和Python API访问，并支持与LLM集成。
...
Method: 采用混合排名策略，结合多源语义嵌入模型、BM25+和PageRank评分，支持网站和API访问。

Result: 开发了LeanExplore搜索引擎，支持语义搜索，并与LLM集成，提升Lean 4工作流和AI数学研究。

Conclusion: LeanExplore为Lean 4用户提供了高效的搜索工具，并展示了在AI驱动研究中的潜力。
...
### [380] [Denoising Programming Knowledge Tracing with a Code Graph-based Tuning Adaptor](https://arxiv.org/abs/2506.11107)
*Weibo Gao,Qi Liu,Rui Li,Yuze Zhao,Hao Wang,Linan Yre,Fangzhou Yao,Zheng Zhang*

Main category: cs.SE

TLDR: Coda是一个基于代码图的调谐适配器，旨在通过识别和减轻噪声影响来增强现有的编程知识跟踪（PKT）模型。
...
Method: Coda将松散代码序列转换为紧凑代码图，通过语义相似性识别噪声，并应用聚类感知GCN改进弱信号识别。最后，通过优化噪声特征约束和导航正则化项，校正受噪声影响的知识状态。
...
### [381] [CodeMirage: A Multi-Lingual Benchmark for Detecting AI-Generated and Paraphrased Source Code from Production-Level LLMs](https://arxiv.org/abs/2506.11059)
*Hanxi Guo,Siyuan Cheng,Kaiyuan Zhang,Guangyu Shen,Xiangyu Zhang*

Main category: cs.SE

TLDR: CodeMirage是一个全面的基准测试，用于评估AI生成代码的检测器，覆盖10种编程语言和10种先进LLM，揭示了当前检测器的优缺点。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试在编程语言覆盖和生成模型能力上不足，无法满足现实需求，亟需更全面的测试工具。

Method: CodeMirage通过涵盖10种编程语言、包含原始和改写代码样本，以及整合10种先进LLM的输出，构建了一个全面的基准测试。

Result: 评估了10种检测器，发现了9个关键发现，揭示了当前检测器的优缺点，并提出了未来挑战。
...
### [382] [You Only Train Once: A Flexible Training Framework for Code Vulnerability Detection Driven by Vul-Vector](https://arxiv.org/abs/2506.10988)
*Bowen Tian,Zhengyang Xu,Mingqiang Wu,Songning Lai,Yutai Yue*

Main category: cs.SE

TLDR: 论文提出YOTO框架，通过参数融合实现多类型漏洞检测模型的快速适应，减少训练时间和资源消耗。
...
Method: 提出YOTO框架，通过参数融合集成多类型漏洞检测模型，避免联合训练。
...
### [383] [LLM-as-a-Judge for Reference-less Automatic Code Validation and Refinement for Natural Language to Bash in IT Automation](https://arxiv.org/abs/2506.11237)
*Ngoc Phuoc An Vo,Brent Paulovicks,Vadim Sheinin*

Main category: cs.SE

TLDR: 论文提出了一种基于LLM的自动评估方法（LLM-as-a-Judge），用于验证和改进Bash代码生成，以提高IT自动化中事件修复的代码质量和模型选择。
...
Method: 采用双向功能匹配和逻辑表示增强LLM-as-a-Judge，并以执行评估为基准验证其准确性。

Result: LLM-as-a-Judge与执行评估高度一致（准确率提升8%），并通过反馈代理实现了24%的代码优化提升。

Conclusion: LLM-as-a-Judge方法在自动代码验证和优化中表现优异，显著提升了IT自动化的效率和准确性。
...
### [384] [Beyond Formal Semantics for Capabilities and Skills: Model Context Protocol in Manufacturing](https://arxiv.org/abs/2506.11180)
*Luis Miguel Vieira da Silva,Aljosha Köcher,Felix Gehlhoff*

Main category: cs.SE

TLDR: 提出了一种基于模型上下文协议（MCP）的方法，使LLM能直接访问系统功能，无需显式语义建模，初步验证了其在工业自动化中的潜力。


<details>
  <summary>Details</summary>
Motivation: 显式建模能力和技能需要大量手动工作，且结果不易被LLM访问，因此探索更灵活的方法。

Method: 使用MCP标准化接口暴露系统功能，让LLM直接调用，并在实验室规模制造系统中进行原型评估。

Result: LLM能通过MCP规划和执行多步骤流程，包括约束处理和功能调用，表明无需显式语义模型即可实现灵活自动化。

Conclusion: MCP为LLM驱动的生产系统外部工具集成提供了基础，未来可进一步探索。
...
### [385] [Invocable APIs derived from NL2SQL datasets for LLM Tool-Calling Evaluation](https://arxiv.org/abs/2506.11266)
*Benjamin Elder,Anupama Murthi,Jungkoo Kang,Ankita Rajaram Naik,Kiran Kate,Kinjal Basu,Danish Contractor*

Main category: cs.SE

TLDR: 论文探讨了如何利用NL2SQL数据集自动生成NL2API数据集，并评估了10种公开LLMs在API调用任务中的表现，发现其完成率较低，需进一步改进。


<details>
  <summary>Details</summary>
Motivation: 企业部署的LLMs需要与复杂API集合交互，但现有数据集和模型在此任务上表现不佳，因此研究如何利用NL2SQL数据集生成NL2API数据集。

Method: 提出了一种数据生成管道，利用SQL查询语法构建功能等效的API调用序列，并将其应用于BIRD-SQL数据集，生成2500多个API。

Result: 评估显示，所有LLMs在工具选择（意图检测、嵌套函数调用和槽填充）上表现不佳，任务完成率仅为7-47%，ReACT代理略有提升至50%。

Conclusion: 当前工具调用LLMs的性能远未达到通用需求，需进一步改进；同时发现模型在SQL生成任务上表现优于API调用。
...
### [386] [A Tale of Two Systems: Characterizing Architectural Complexity on Machine Learning-Enabled Systems](https://arxiv.org/abs/2506.11295)
*Renato Cordeiro Ferreira*

Main category: cs.SE

TLDR: 研究提出基于指标的架构模型，以管理ML系统的复杂性，并通过两个案例研究验证模型。
...
Method: 引入基于指标的架构模型，分析SPIRA和Ocean Guard MLES两个案例。

Result: 提出模型支持ML系统的架构设计和扩展。

Conclusion: 基于指标的模型能有效管理ML系统的复杂性，为未来系统设计提供参考。
...
### [387] [Mutual-Supervised Learning for Sequential-to-Parallel Code Translation](https://arxiv.org/abs/2506.11153)
...
TLDR: 提出了一种名为Mutual-Supervised Learning (MSL)的新框架，用于解决序列到并行代码翻译中的功能等价问题，通过Translator和Tester的协同验证与进化显著提升性能。


<details>
  <summary>Details</summary>
Motivation: GPU高性能计算的普及推动了并行编程模型的需求，但并行编程的复杂性及数据稀缺性使得基于机器学习的序列到并行代码翻译面临挑战，尤其是功能等价性问题。

Method: MSL框架包含Translator和Tester两个模型，通过Co-verify和Co-evolve步骤的迭代循环，相互生成数据并共同改进。

Result: 实验结果显示，MSL显著提升了基础模型的性能，Pass@1提升28.91%，Tester性能提升68.90%，并在BLEU和CodeBLEU分数上超越之前的最佳方法。

Conclusion: MSL框架有效解决了功能等价性问题，性能接近或超越现有先进方法，为序列到并行代码翻译提供了新思路。
...
### [388] [LiveCodeBench Pro: How Do Olympiad Medalists Judge LLMs in Competitive Programming?](https://arxiv.org/abs/2506.11928)
...
TLDR: 论文重新评估了大型语言模型（LLM）在竞争性编程中的表现，发现其与人类专家仍有显著差距，尤其是在复杂算法推理方面。


<details>
  <summary>Details</summary>
Motivation: 研究动机是验证LLM是否真的在竞争性编程中超越人类精英，并分析其与人类专家的差异及局限性。

Method: 方法包括引入LiveCodeBench Pro基准，由国际算法竞赛奖牌得主标注问题并分析模型失败案例。

Result: 结果显示，前沿模型在中等难度问题上的通过率为53%，在难题上为0%，且表现依赖于实现精度而非推理能力。

Conclusion: 结论指出LLM与人类大师水平仍有显著差距，需进一步改进代码为中心的推理能力。
...
### [389] [ReVeal: Self-Evolving Code Agents via Iterative Generation-Verification](https://arxiv.org/abs/2506.11442)
*Yiyang Jin,Kunzhao Xu,Hang Li,Xueting Han,Yanmin Zhou,Cheng Li,Jing Bai*

Main category: cs.SE

TLDR: ReVeal是一个多轮强化学习框架，结合代码生成与显式自验证和工具评估，显著提升大型语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏真实环境的验证信号和显式优化验证，导致自验证不可靠。

Method: ReVeal通过多轮强化学习，生成测试用例、调用外部工具获取反馈，并使用密集奖励优化性能。

Result: 在LiveCodeBench上Pass@k显著提升，推理时能扩展到更深层次，性能超越DeepSeek-R1-Zero-Qwen-32B。

Conclusion: ReVeal是一种可扩展且有效的范式，能构建更鲁棒和自主的AI代理。
...
### [390] [Leveraging GPT-4 for Vulnerability-Witnessing Unit Test Generation](https://arxiv.org/abs/2506.11559)
*Gábor Antal,Dénes Bán,Martin Isztin,Rudolf Ferenc,Péter Hegedűs*

Main category: cs.SE

TLDR: 论文探讨了GPT-4在自动生成单元测试方面的能力，特别是在漏洞修复场景中的应用，结果显示GPT-4能生成部分正确的测试用例，但需进一步人工完善。


<details>
  <summary>Details</summary>
Motivation: 软件测试在质量保障中至关重要，但手动生成测试用例复杂且耗时。研究旨在利用GPT-4自动生成漏洞相关的单元测试，减轻开发者负担。

Method: 使用VUL4J数据集中的真实漏洞及其修复代码，评估GPT-4生成测试用例的语法和语义正确性，并分析代码上下文、自我修正能力及测试用例的实用性。

Result: GPT-4生成语法正确测试用例的成功率为66.5%，但语义正确性仅7.5%。主观评估显示其生成的测试模板可进一步人工完善。

Conclusion: GPT-4在漏洞测试生成中具有潜力，可作为部分自动化工具使用，但仍需人工干预。
...
### [391] [Identifying Helpful Context for LLM-based Vulnerability Repair: A Preliminary Study](https://arxiv.org/abs/2506.11561)
*Gábor Antal,Bence Bogenfürst,Rudolf Ferenc,Péter Hegedűs*

Main category: cs.SE

TLDR: GPT-4o在修复Java漏洞时表现略逊于GPT-4，但通过优化提示策略（如结合CVE信息和代码上下文）可显著提升修复率。


<details>
  <summary>Details</summary>
Motivation: 探索GPT-4o在自动化漏洞修复（AVR）中的性能，并研究不同上下文信息对修复能力的影响。

Method: 使用Vul4J数据集，比较GPT-4o与GPT-4的表现，测试多种提示策略（如CVE信息、代码上下文等）。

Result: GPT-4o平均修复率低于GPT-4，但通过优化提示（如结合CVE和代码上下文）修复了更多漏洞。

Conclusion: 提示策略的优化（如集成多提示）可显著提升零样本漏洞修复效果。
...
### [392] [The Memory Paradox: Why Our Brains Need Knowledge in an Age of AI](https://arxiv.org/abs/2506.11015)
*Barbara Oakley,Michael Johnston,Ken-Zen Chen,Eulho Jung,Terrence J. Sejnowski*

Main category: cs.CY

TLDR: 论文探讨了过度依赖AI工具对人类记忆系统的负面影响，并提出了加强内部认知模型的必要性。
...
Method: 结合神经科学和认知心理学，分析AI工具对记忆巩固的干扰，并对比深度学习现象与人类认知过程。

Result: 研究发现，过早依赖AI会阻碍程序性记忆和直觉掌握，强调内部认知模型的重要性。

Conclusion: 建议在教育和职业培训中，注重培养内部认知模型以优化人机协作。
...
### [393] [The Strategic Imperative for Healthcare Organizations to Build Proprietary Foundation Models](https://arxiv.org/abs/2506.11412)
*Naresh Tiwari*

Main category: cs.CY

TLDR: 本文主张医疗组织应开发专有基础模型而非依赖商业替代品，分析了四大驱动因素，并通过实证证明专有模型在临床性能、数据治理和竞争优势上的优势。
...
Method: 通过实证证据、经济框架和组织案例研究，分析专有多模态基础模型的优势。

Result: 专有模型能提升临床性能、强化数据治理、创造竞争优势并加速创新。
...
### [394] [Subjective Experience in AI Systems: What Do AI Researchers and the Public Believe?](https://arxiv.org/abs/2506.11945)
*Noemi Dreksler,Lucius Caviola,David Chalmers,Carter Allen,Alex Rand,Joshua Lewis,Philip Waggoner,Kate Mays,Jeff Sebo*

Main category: cs.CY

TLDR: AI研究人员和公众对具有主观体验的AI系统的出现持不同观点，但多数认为其可能在本世纪出现，并支持相关保障措施。
...
Method: 调查了582名AI研究人员和838名美国公众，评估其对AI系统主观体验的预期和治理态度。

Result: AI研究人员和公众对AI系统主观体验的出现时间估计不同，但多数支持相关保障措施，态度在治理和道德问题上存在分歧。

Conclusion: 尽管存在不确定性，AI研究人员和公众普遍认为具有主观体验的AI系统可能在本世纪出现，并需要提前制定保障措施。
...
### [395] [Efficient Traffic Classification using HW-NAS: Advanced Analysis and Optimization for Cybersecurity on Resource-Constrained Devices](https://arxiv.org/abs/2506.11319)
*Adel Chehade,Edoardo Ragusa,Paolo Gastaldo,Rodolfo Zunino*

Main category: cs.NI

TLDR: 本文提出了一种硬件高效的深度神经网络（DNN），通过硬件感知的神经架构搜索（HW-NAS）优化，用于资源受限的物联网（IoT）和边缘设备上的加密流量分类。该模型在ISCX VPN-nonVPN数据集上实现了96.59%的准确率，同时显著减少了参数、FLOPs和内存占用。


<details>
  <summary>Details</summary>
Motivation: 解决资源受限设备上加密流量分类的挑战，提供高效的实时分析方案。

Method: 使用硬件感知的神经架构搜索（HW-NAS）优化1D卷积神经网络（CNN），并结合头级预处理策略。

Result: 模型在多个分类任务中表现优异，准确率高达99.64%，同时显著减少了计算和内存需求。

Conclusion: 该方法为物联网网络安全提供了高效、可扩展的解决方案，适合在严格硬件限制下实时分析加密流量。
...
### [396] [Upgrade or Switch: Do We Need a New Registry Architecture for the Internet of AI Agents?](https://arxiv.org/abs/2506.12003)
...
TLDR: 本文探讨了现有网络基础设施是否能够满足AI代理的需求，分析了升级现有系统与构建新架构的优劣，并建议采用混合方案。


<details>
  <summary>Details</summary>
Motivation: 随着AI代理的兴起，现有为人类设计的网络基础设施（如DNS/PKI）无法满足其需求，如毫秒级发现、即时凭证撤销等。

Method: 分析了三种方案：升级现有系统、构建新架构、混合方案，并对比了它们的优缺点。

Result: 发现AI代理的需求是质变而非量变，升级方案兼容性好但性能有限，全新架构性能更优但部署时间长。

Conclusion: 建议采用混合方案，结合集中式注册和联邦式网络，以适应不同场景的需求。
...
### [397] [FeNN: A RISC-V vector processor for Spiking Neural Network acceleration](https://arxiv.org/abs/2506.11760)
*Zainab Aizaz,James C. Knight,Thomas Nowotny*

Main category: cs.NE

TLDR: 本文提出了一种基于RISC-V的软向量处理器（FeNN），专为在FPGA上模拟脉冲神经网络（SNNs）而设计，旨在解决现有硬件（如GPU和TPU）在SNN模拟中的不足。


<details>
  <summary>Details</summary>
Motivation: 现有主流加速器（如GPU和TPU）设计用于高算术强度的标准人工神经网络（ANNs），不适合SNN模拟。FPGA因其高带宽和大容量片上内存更适合低算术强度的SNN模拟。

Method: 开发了一种基于RISC-V的软向量处理器（FeNN），采用随机舍入和饱和技术，以低硬件利用率实现高数值精度。

Result: 实验表明，单个FeNN核心在模拟SNN分类器时速度超过嵌入式GPU和Loihi神经形态系统。

Conclusion: FeNN是一种完全可编程的解决方案，适用于从边缘到云端的标准计算机应用，为SNN模拟提供了高效硬件支持。
...
### [398] [Brain2Vec: A Deep Learning Framework for EEG-Based Stress Detection Using CNN-LSTM-Attention](https://arxiv.org/abs/2506.11179)
*Md Mynoddin,Troyee Dev,Rishita Chakma*

Main category: eess.SP

TLDR: Brain2Vec是一种深度学习工具，通过混合卷积、循环和注意力机制从原始EEG信号中分类压力状态，在DEAP数据集上表现优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 心理健康问题日益普遍，需要开发非侵入性诊断工具，而EEG信号的高维和非平稳特性带来了建模挑战。

Method: 采用卷积层捕捉空间依赖，LSTM层建模时间序列，注意力机制突出关键时间区域，结合预处理流程（如带通滤波和归一化）。
...
### [399] [Recursive KalmanNet: Deep Learning-Augmented Kalman Filtering for State Estimation with Consistent Uncertainty Quantification](https://arxiv.org/abs/2506.11639)
*Hassan Mortada,Cyril Falcon,Yanis Kahil,Mathéo Clavaud,Jean-Philippe Michel*

Main category: eess.SP

TLDR: 论文提出了一种基于卡尔曼滤波的递归神经网络Recursive KalmanNet，用于在非高斯噪声条件下实现精确状态估计和误差协方差量化。
...
Conclusion: Recursive KalmanNet在复杂噪声条件下表现优越，为状态估计提供了新方法。
...
### [400] [Diffusion-Based Electrocardiography Noise Quantification via Anomaly Detection](https://arxiv.org/abs/2506.11815)
*Tae-Seong Han,Jae-Wook Heo,Hakseung Kim,Cheol-Hui Lee,Hyub Huh,Eue-Keun Choi,Dong-Joo Kim*

Main category: eess.SP

TLDR: 该研究提出了一种基于扩散模型的ECG噪声量化方法，通过重建异常检测解决传统方法的标注不一致和泛化性差问题，显著提升了噪声量化的准确性和临床实用性。


<details>
  <summary>Details</summary>
Motivation: ECG信号常受噪声干扰，影响临床和可穿戴设备的诊断准确性。传统方法存在标注不一致和泛化性差的局限性。

Method: 采用基于扩散模型的框架，利用Wasserstein-1距离（$W_1$）比较干净与噪声ECG的重建误差分布，仅需三步反向扩散实现噪声量化。

Result: 模型在基准测试中取得1.308的$W_1$宏平均分，优于次优方法48%以上，外部验证显示强泛化性。

Conclusion: 该方法提升了临床决策、诊断准确性和实时ECG监测能力，为未来临床和可穿戴ECG应用提供了支持。
...
### [401] [Poutine: Vision-Language-Trajectory Pre-Training and Reinforcement Learning Post-Training Enable Robust End-to-End Autonomous Driving](https://arxiv.org/abs/2506.11234)
*Luke Rowe,Rodrigue de Schaetzen,Roger Girgis,Christopher Pal,Liam Paull*

Main category: cs.RO

TLDR: Poutine是一个3B参数的视觉语言模型，专为长尾驾驶场景的端到端自动驾驶设计。通过两阶段训练（自监督预训练和强化学习微调），在Waymo挑战赛中表现优异。
...
Conclusion: VLT预训练和轻量级强化学习微调是实现稳健自动驾驶的有效方法。
...
### [402] [Gondola: Grounded Vision Language Planning for Generalizable Robotic Manipulation](https://arxiv.org/abs/2506.11261)
*Shizhe Chen,Ricardo Garcia,Paul Pacaud,Cordelia Schmid*

Main category: cs.RO

TLDR: Gondola是一种基于LLMs的新型视觉语言规划模型，通过多视角图像和历史计划生成带有目标对象分割掩码的动作计划，显著提升了机器人操作的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 机器人操作在未见过的对象、环境和多样化语言指令任务中泛化能力不足，现有方法受限于单视角输入和精确对象定位困难。
...
Conclusion: Gondola通过多视角输入和对象分割掩码，显著提升了机器人操作的泛化能力，为未来研究提供了新方向。
...
### [403] [Control Architecture and Design for a Multi-robotic Visual Servoing System in Automated Manufacturing Environment](https://arxiv.org/abs/2506.11387)
*Rongfei Li*

Main category: cs.RO

TLDR: 本文探讨了在微尺度制造中，机器人如何通过模拟人类感官线索和多机器人控制系统减少不确定性，并提出了一种优化摄像头位置的算法以最小化图像噪声。


<details>
  <summary>Details</summary>
Motivation: 人类在微尺度制造中仍优于机器人，主要依赖感官线索补偿环境不确定性。机器人虽配备先进传感器和处理器，但控制算法仍是减少不确定性的经济有效方案。

Method: 提出多机器人控制系统模拟定位过程以减少不确定性，并设计摄像头移动策略算法以寻找噪声最小的最优位置。

Result: 多机器人控制系统显著减少了制造过程中的不确定性，摄像头移动策略成功优化了图像质量。

Conclusion: 通过模拟人类感官和优化摄像头位置，机器人系统在微尺度制造中能更高效地减少不确定性，提升制造精度。
...
TLDR: 论文研究了非专家演示中的非最优行为，发现这些行为是系统性的，称为“演示偏离”。通过实验识别了四种偏离类型，并指出其对LfD算法改进的重要性。


<details>
  <summary>Details</summary>
Motivation: 人类演示中的不完美行为通常被视为随机噪声，但本文认为这些行为是系统性的，需要更好的模型来改进LfD算法。

Method: 通过40名参与者的公共空间研究，在仿真中重现任务并标注所有演示，识别了四种偏离类型和一种控制模式。

Result: 发现偏离行为在参与者中频繁出现，其时空分布与任务上下文相关，且控制模式依赖于控制界面。
...
### [405] [Foundation Models in Autonomous Driving: A Survey on Scenario Generation and Scenario Analysis](https://arxiv.org/abs/2506.11526)
...
TLDR: 本文综述了基础模型在自动驾驶场景生成与分析中的应用，提出了统一的分类法，并探讨了方法、数据集、平台、挑战及未来方向。
...
Method: 通过调查基础模型（如大语言模型、视觉语言模型等）在自动驾驶场景生成与分析中的应用，提出分类法并回顾相关方法、数据集和平台。

Result: 综述了基础模型的应用现状，总结了现有方法、数据集和挑战，并提出了评价指标。

Conclusion: 基础模型在自动驾驶场景生成与分析中具有潜力，但仍存在开放挑战，未来研究需进一步探索。
...
### [406] [Robot Context Protocol (RCP): A Runtime-Agnostic Interface for Agent-Aware Robot Control](https://arxiv.org/abs/2506.11650)
*Lambert Lee,Joshua Lau*

Main category: cs.RO

TLDR: RCP是一种轻量级、中间件无关的通信协议，旨在简化机器人系统复杂性并实现无缝交互。


<details>
  <summary>Details</summary>
Motivation: 解决机器人系统中通信复杂性和多环境部署问题。

Method: 基于HTTP和WebSocket，定义结构化消息格式，支持多种操作和功能。

Result: 提供统一接口，支持多环境部署，确保鲁棒性、可扩展性和安全性。

Conclusion: RCP适用于复杂多智能体生态系统，实现智能、弹性和安全的机器人操作。
...
### [407] [SAIL: Faster-than-Demonstration Execution of Imitation Learning Policies](https://arxiv.org/abs/2506.11948)
...
TLDR: 论文提出了一种名为SAIL的系统，用于在离线模仿学习中实现比演示速度更快的执行速度，解决了机器人动态和状态-动作分布偏移的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有模仿学习方法只能以演示速度执行任务，限制了机器人系统的任务吞吐量，尤其是在工业自动化等应用中。

Method: SAIL系统包含四个组件：一致性保持的动作推断算法、高保真跟踪控制器不变的运动目标、自适应速度调制以及动作调度。

Result: 在仿真和真实机器人平台上，SAIL实现了最高4倍（仿真）和3.2倍（真实世界）的速度提升。

Conclusion: SAIL系统成功解决了模仿学习中速度限制的问题，为机器人任务的高效执行提供了新方法。
...
### [408] [Polymorphism Crystal Structure Prediction with Adaptive Space Group Diversity Control](https://arxiv.org/abs/2506.11332)
*Sadman Sadeed Omee,Lai Wei,Sourin Dey,Jianjun Hu*

Main category: cond-mat.mtrl-sci

TLDR: ParetoCSP2是一种多目标遗传算法，用于预测无机多晶型结构，通过自适应空间群多样性控制技术提高预测性能。


<details>
  <summary>Details</summary>
Motivation: 计算预测多晶型结构在材料科学中至关重要，但现有算法有限。

Method: 结合神经网络原子间势的自适应空间群多样性控制技术，改进种群初始化方法并进行迭代结构松弛。

Result: 在基准数据集上表现优异，多晶型预测准确率显著提升。

Conclusion: ParetoCSP2在多晶型结构预测中表现出色，代码已开源。
...
### [409] [Learning Before Filtering: Real-Time Hardware Learning at the Detector Level](https://arxiv.org/abs/2506.11981)
*Boštjan Maček*

Main category: hep-ex

TLDR: 提出了一种用于实时神经网络训练的数字硬件架构，优化了高吞吐量数据处理，并在FPGA上验证了其可行性。


<details>
  <summary>Details</summary>
Motivation: 传统过滤方法难以适应动态或意外数据特征，机器学习提供了更灵活的替代方案，尤其是在检测器附近进行训练时。

Method: 设计了一种与实现无关的数字硬件架构，详细分析了各组件及其性能影响，并通过参数化探索了速度、模型复杂度和硬件资源之间的权衡。

Result: FPGA上的概念验证实现了原位训练，保持了计算精度，并估计当前FPGA可训练约3,500个神经元。

Conclusion: 该架构具有可扩展性和适应性，为将学习直接集成到检测器系统中提供了重要进展，支持极端边缘实时信息处理。
...
### [410] [Assessing the Impact of Anisotropy in Neural Representations of Speech: A Case Study on Keyword Spotting](https://arxiv.org/abs/2506.11096)
*Guillaume Wisniewski,Séverine Guillaume,Clara Rosina Fernández*

Main category: cs.SD

TLDR: 论文研究了预训练语音表示（如wav2vec2和HuBERT）的各向异性对关键词识别任务的影响，发现尽管存在各向异性，这些表示仍能有效识别单词。
...
Conclusion: 预训练在学习丰富且不变的语音表示中具有重要性。
...
### [411] [GLAP: General contrastive audio-text pretraining across domains and languages](https://arxiv.org/abs/2506.11350)
*Heinrich Dinkel,Zhiyong Yan,Tianzi Wang,Yongqing Wang,Xingwei Sun,Yadong Niu,Jizhong Liu,Gang Li,Junbo Zhang,Jian Luan*

Main category: cs.SD

TLDR: GLAP扩展了CLAP，增加了多语言和多领域能力，在音频-文本检索、语音检索和分类任务中表现优异，并在多语言关键词识别和音乐理解中展示了先进能力。
...
Method: 引入GLAP，扩展CLAP以支持多语言和多领域能力。

Result: 在标准音频-文本检索任务中表现优异，显著超越现有方法；在多语言关键词识别和音乐理解任务中表现突出。

Conclusion: GLAP在多语言和跨领域任务中具有显著优势，为音频-文本交互提供了更通用的解决方案。
...
### [412] [(SimPhon Speech Test): A Data-Driven Method for In Silico Design and Validation of a Phonetically Balanced Speech Test](https://arxiv.org/abs/2506.11620)
*Stefan Bleeck*

Main category: cs.SD

TLDR: 论文提出了一种名为SimPhon Speech Test-25的新型语音测试方法，通过计算模拟和优化，解决了传统听力测试在评估老年性耳聋患者语音理解功能上的不足。


<details>
  <summary>Details</summary>
Motivation: 传统听力测试对老年性耳聋患者的语音理解功能评估不全面，特别是对超阈值缺陷的捕捉不足，因此需要开发更具诊断特异性的语音感知测试。

Method: 采用多阶段计算流程，利用自动语音识别系统模拟听力损失对语音感知的影响，通过声学退化和数据分析筛选出25对优化的单词对。
...
### [413] [A correlation-permutation approach for speech-music encoders model merging](https://arxiv.org/abs/2506.11403)
*Fabian Ritter-Gutierrez,Yi-Cheng Lin,Jeremy H. M Wong,Hung-yi Lee,Eng Siong Chng,Nancy F. Chen*

Main category: cs.SD

TLDR: 提出了一种基于相关置换的方法，用于合并语音和音乐编码器，显著提升音乐性能，同时保留语音能力。


<details>
  <summary>Details</summary>
Motivation: 统一语音和音乐模型通常需要昂贵的预训练，而模型合并可以低成本实现统一音频模型，但直接合并因权重空间不对齐而困难。

Method: 采用相关置换方法，逐层计算最大化特征交叉相关的置换矩阵，以对齐音乐和语音编码器的内部层。

Result: 合并后的模型在音乐性能上显著提升（平均得分提高14.83分），同时保留语音能力。

Conclusion: 该方法可从独立训练的编码器中创建统一音频模型。
...
### [414] [LiLAC: A Lightweight Latent ControlNet for Musical Audio Generation](https://arxiv.org/abs/2506.11476)
*Tom Baker,Javier Nistal*

Main category: cs.SD

TLDR: 提出了一种轻量级模块化架构，显著减少参数数量，同时保持音频质量和条件控制能力，优于ControlNet。
...
Result: 在音频质量和条件控制方面与ControlNet相当，但内存占用更低，灵活性更强。

Conclusion: 该方法显著提升了控制效率和灵活性，适用于音乐制作。
...
### [415] [Enabling automatic transcription of child-centered audio recordings from real-world environments](https://arxiv.org/abs/2506.11747)
*Daniil Kocharov,Okko Räsänen*

Main category: cs.SD

TLDR: 提出一种方法，通过自动检测可被现代ASR系统可靠转录的语音片段，实现对儿童日常长音频的部分准确转录，显著降低错误率。


<details>
  <summary>Details</summary>
Motivation: 长音频转录对研究儿童语言发展至关重要，但手动标注不现实，而传统ASR因音频噪声问题难以直接应用。

Method: 开发一种方法，仅转录长音频中可被ASR可靠识别的部分，而非全部音频。

Result: 在四个英语长音频语料库中验证，该方法转录13%语音时，中位WER为0%，平均WER为18%，显著优于全量转录（中位WER 52%）。自动与手动转录的词频相关性高达0.92-0.98。
...
### [416] [Abstract Sound Fusion with Unconditioned Inversion Model](https://arxiv.org/abs/2506.11811)
*Jing Liu,EnQi Lian*

Main category: cs.SD

TLDR: 论文提出了一种基于DPMSolver++采样器的SDE和ODE反转模型，用于实现声音融合，生成超越简单叠加的新声音。
...
Result: 提出的反转方法在保持采样灵活性的同时，成功合成了具有新听觉特征的声音。

Conclusion: 该模型为声音融合提供了一种高效且可控的方法，能够生成超越简单叠加的抽象声音。
...
### [417] [Reimagining Dance: Real-time Music Co-creation between Dancers and AI](https://arxiv.org/abs/2506.12008)
*Olga Vechtomova,Jeff Bos*

Main category: cs.SD

TLDR: 提出一种双向舞蹈与音乐互动系统，通过舞者动作动态生成音乐，实现舞者与AI的创造性合作。


<details>
  <summary>Details</summary>
Motivation: 传统舞蹈表演中音乐与动作是单向关系，AI在舞蹈领域的应用多集中于从音乐生成编舞。本文旨在探索舞者如何通过动作动态塑造音乐环境，建立双向创作关系。

Method: 采用多模态架构，通过智能组合预录音乐片段响应舞者动作，生成连贯音乐作品。
...
### [418] [Automated Treatment Planning for Interstitial HDR Brachytherapy for Locally Advanced Cervical Cancer using Deep Reinforcement Learning](https://arxiv.org/abs/2506.11957)
...
TLDR: 开发了一种基于强化学习和剂量优化的自动化HDR近距离放射治疗计划框架，显著提高了计划的一致性和效率。


<details>
  <summary>Details</summary>
Motivation: 手动治疗计划高度依赖专家经验，缺乏一致性和效率，因此需要自动化解决方案。

Method: 采用两阶段框架：第一阶段使用DQN强化学习代理选择治疗参数，第二阶段用Adam优化器计算停留时间分布。

Result: 在测试患者中，自动化方法平均得分93.89%，优于临床计划的91.86%，同时保持目标覆盖并减少热点。

Conclusion: 该自动化框架在复杂解剖结构中表现优异，具有临床实用潜力。
...
### [419] [Multimodal Modeling of CRISPR-Cas12 Activity Using Foundation Models and Chromatin Accessibility Data](https://arxiv.org/abs/2506.11182)
*Azim Dehghani Amirabad,Yanfei Zhang,Artem Moskalev,Sowmya Rajesh,Tommaso Mansi,Shuwei Li,Mangal Prakash,Rui Liao*

Main category: q-bio.GN

TLDR: 利用预训练的生物学基础模型和染色质可及性数据，显著提升了CRISPR-Cas12基因组编辑中gRNA活性的预测效果。
...
Method: 使用RNA基础模型的嵌入作为轻量级回归器的输入，并结合染色质可及性数据以捕捉调控背景。

Result: 与传统基线相比，该方法显著提升了gRNA活性预测的准确性。
...
### [420] [HEIST: A Graph Foundation Model for Spatial Transcriptomics and Proteomics Data](https://arxiv.org/abs/2506.11152)
*Hiren Madhu,João Felipe Rocha,Tinglin Huang,Siddharth Viswanath,Smita Krishnaswamy,Rex Ying*

Main category: q-bio.GN

TLDR: HEIST是一种基于层次图变换器的空间转录组学基础模型，通过结合空间和基因调控信息，实现了对细胞微环境影响的编码，并在多个下游任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有模型忽视了空间分辨率或基因调控信息，而HEIST旨在通过层次化建模和基因调控网络（GRN）表示，解决这一问题。

Method: HEIST将组织建模为空间细胞邻域图，每个细胞建模为基因调控网络图，采用层次图变换器进行跨层次和层次内消息传递。

Result: HEIST在22.3M细胞上预训练，成功编码微环境影响，并在临床结果预测、细胞类型注释等任务中达到最优性能。
...
### [421] [Brain-wide interpolation and conditioning of gene expression in the human brain using Implicit Neural Representations](https://arxiv.org/abs/2506.11158)
*Xizheng Yu,Justin Torok,Sneha Pandya,Sourav Pal,Vikas Singh,Ashish Raj*

Main category: q-bio.GN

TLDR: 论文研究了基于隐式神经表示（INR）的非局部非线性图像插值和外推算法在空间转录组数据分析中的应用，旨在生成全脑范围内基因表达的高分辨率空间图谱。


<details>
  <summary>Details</summary>
Motivation: 利用稀疏采样的健康人脑微阵列基因表达数据，生成全脑范围内任意基因的高分辨率空间图谱，以支持阿尔茨海默病（AD）相关基因的研究。

Method: 从Allen Human Brain Atlas（AHBA）获取100个AD风险基因的基线空间转录谱，并调整隐式神经表示模型以生成高分辨率的基因表达图谱。

Result: 通过实验验证了该方法的有效性，并以Abagen的插值结果作为基线参考。

Conclusion: 隐式神经表示模型能够有效生成高分辨率的基因表达图谱，为空间转录组数据分析提供了新工具。
...
### [422] [SemanticST: Spatially Informed Semantic Graph Learning for1 Clustering, Integration, and Scalable Analysis of Spatial2 Transcriptomics](https://arxiv.org/abs/2506.11491)
*Roxana Zahedi,Ahmadreza Argha,Nona Farbehi,Ivan Bakhshayeshi,Youqiong Ye,Nigel H. Lovell,Hamid Alinejad-Rokny*

Main category: q-bio.GN

TLDR: SemanticST是一种基于图的深度学习框架，通过多语义图建模空间转录组数据，解决了噪声、可扩展性和复杂细胞关系建模的挑战。


<details>
  <summary>Details</summary>
Motivation: 当前空间转录组分析方法在噪声数据、可扩展性和复杂细胞关系建模方面存在不足，需要一种更强大的工具。

Method: 构建多语义图（空间邻近性、基因表达相似性、组织域结构），学习解耦嵌入并通过注意力融合，使用社区感知的最小割损失提高鲁棒性。

Result: 在多个平台和组织上表现优于现有方法（ARI、NMI提升20%），并在乳腺癌数据中发现罕见临床相关细胞群。

Conclusion: SemanticST为空间转录组分析提供了可扩展、可解释且生物学基础强的框架，支持精准医学和空间组织图谱研究。
...
### [423] [Interaction, Process, Infrastructure: A Unified Architecture for Human-Agent Collaboration](https://arxiv.org/abs/2506.11718)
*Yun Wang,Yan Lu*

Main category: cs.HC

TLDR: 论文提出了一种分层框架，整合了交互、流程和基础设施三个维度，以提升人类与AI代理的持续协作能力。


<details>
  <summary>Details</summary>
Motivation: 当前AI工具虽然功能强大，但缺乏支持持续协作的架构，限制了其在实际工作中的应用。

Method: 提出一个分层框架，重点关注流程的显式化、可检查和可适应性，以实现人类与代理的长期协调。

Result: 该框架揭示了当前工具的局限性，统一了系统设计方法，并为研究者和开发者提供了新机会。

Conclusion: 通过结构化协作，重新构想人类与代理的协作，使其成为现实工作中的一致且对齐的系统。
...
### [424] [Enter: Graduated Realism: A Pedagogical Framework for AI-Powered Avatars in Virtual Reality Teacher Training](https://arxiv.org/abs/2506.11890)
*Judson Leroy Dean Haynes IV*

Main category: cs.HC

TLDR: 本文探讨了VR教师培训中AI学生头像的真实性水平问题，提出了渐进式真实性框架，并设计了一种高效的计算架构。


<details>
  <summary>Details</summary>
Motivation: 解决VR教师培训中头像真实性与教学需求之间的脱节问题，优化新手教师的学习体验。

Method: 通过文献综述分析头像真实性的演变，结合认知负荷理论，提出渐进式真实性框架，并设计Crazy Slots架构。

Result: 研究发现超真实头像可能增加新手认知负荷，渐进式真实性框架和Crazy Slots架构能有效平衡教学与技术需求。

Conclusion: 强调教学导向的真实性设计对可扩展且高效的教师培训工具的重要性。
...
### [425] [SwiftSpec: Ultra-Low Latency LLM Decoding by Scaling Asynchronous Speculative Decoding](https://arxiv.org/abs/2506.11309)
*Ziyi Zhang,Ziheng Jiang,Chengquan Jiang,Menghan Yu,Size Zheng,Haibin Lin,Henry Hoffmann,Xin Liu*

Main category: cs.DC

TLDR: SwiftSpec是一种针对大语言模型（LLM）解码的超低延迟系统，通过异步和分散式设计优化推测解码，实现1.75倍加速。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码和并行方法无法同时应用，导致低延迟需求未满足。

Method: SwiftSpec采用异步分散式推测解码，结合并行树生成、树感知KV缓存管理和优化内核。

Result: 在5个模型和6个数据集上平均加速1.75倍，Llama3-70B达到348 tokens/s。
...
### [426] [A Framework for Non-Linear Attention via Modern Hopfield Networks](https://arxiv.org/abs/2506.11043)
*Ahmed Farooq*

Main category: stat.ML

TLDR: 该论文提出了一种基于现代Hopfield网络（MNH）的能量函数，其稳定点对应于Vaswani等人的注意力机制，从而统一了两者框架。


<details>
  <summary>Details</summary>
Motivation: 通过能量函数统一现代Hopfield网络和注意力机制，揭示其内在联系，并探索非线性注意力机制在提升Transformer模型性能方面的潜力。

Method: 定义了一个能量函数，其梯度对应于注意力计算，并通过非线性注意力机制增强模型对复杂关系的理解和表示学习。

Result: 能量函数的极小值形成“上下文井”，稳定地封装了token之间的上下文关系，非线性注意力机制可提升模型效率和性能。

Conclusion: 该方法为Transformer模型（如BERT）引入非线性头提供了理论基础，扩展了其在序列建模任务中的应用能力。
...
### [427] [Collaborative Prediction: To Join or To Disjoin Datasets](https://arxiv.org/abs/2506.11271)
*Kyung Rok Kim,Yansong Wang,Xiaocheng Li,Guanting Chen*

Main category: stat.ML

TLDR: 论文研究了如何选择高质量数据集以优化预测模型的性能，提出了一种具有理论保证的算法，通过合并不同来源的数据集来减少总体损失。
...
Method: 研究提出了一种基于oracle不等式和数据驱动估计器的实用算法，用于合并不同来源的数据集，以减少预测模型的总体损失。
...
### [428] [Fast Bayesian Optimization of Function Networks with Partial Evaluations](https://arxiv.org/abs/2506.11456)
*Poompol Buathong,Peter I. Frazier*

Main category: stat.ML

TLDR: 提出了一种加速的p-KGFN算法，通过全局蒙特卡洛模拟生成节点候选输入，显著减少计算开销，同时保持查询效率。


<details>
  <summary>Details</summary>
Motivation: 许多实际应用（如制造和药物发现）涉及具有独立评估节点和不同成本的功能网络，p-KGFN虽能减少昂贵评估次数，但计算开销大。

Method: 通过一次全局蒙特卡洛模拟为网络中的每个节点生成候选输入，避免嵌套优化，从而加速算法。

Result: 数值实验显示，新方法在保持查询效率的同时，计算速度提升高达16倍。

Conclusion: 加速的p-KGFN算法在减少计算开销方面表现优异，适用于实际应用。
...
### [429] [On the performance of multi-fidelity and reduced-dimensional neural emulators for inference of physiologic boundary conditions](https://arxiv.org/abs/2506.11683)
*Chloe H. Choi,Andrea Zanoni,Daniele E. Schiavazzi,Alison L. Marsden*

Main category: stat.ML

TLDR: 论文探讨了心血管建模中逆问题的解决方法，通过低保真近似降低计算成本，比较了五种方法的效果。


<details>
  <summary>Details</summary>
Motivation: 解决心血管建模中高保真模拟计算成本高的问题。

Method: 利用低保真近似构建替代模型或差异模型，采用神经网络或降维技术，或通过归一化流估计噪声分布。

Result: 在分析测试案例和心血管模型中验证了五种方法的准确性和计算效率。

Conclusion: 提出的方法能有效降低计算成本，适用于复杂心血管建模问题。
...
### [430] [Using Deep Operators to Create Spatio-temporal Surrogates for Dynamical Systems under Uncertainty](https://arxiv.org/abs/2506.11761)
*Jichuan Tang,Patrick T. Brewick,Ryan G. McClarren,Christopher Sweet*

Main category: stat.ML

TLDR: 提出了一种新型深度算子网络FExD，用于时空替代模型，显著提升了结构动力学中的多输出响应预测精度和计算效率。


<details>
  <summary>Details</summary>
Motivation: 解决现有SciML方法在时空替代模型中的局限性，特别是在多自由度动态系统中的应用。

Method: 通过增强分支网络的表达能力和扩展主干网络的预测能力，提出FExD模型，并在电缆桥模型中验证其性能。

Result: FExD在精度和计算效率上均优于传统DeepONet和改进的时空Extended DeepONet。
...
### [431] [Bayesian Optimization with Inexact Acquisition: Is Random Grid Search Sufficient?](https://arxiv.org/abs/2506.11831)
*Hwanwoo Kim,Chong Liu,Yuxin Chen*

Main category: stat.ML

TLDR: 研究探讨了贝叶斯优化中非精确最大化获取函数的影响，证明了在适当条件下，非精确优化仍能实现次线性累积遗憾，并提出随机网格搜索作为高效求解器。


<details>
  <summary>Details</summary>
Motivation: 贝叶斯优化中精确最大化获取函数通常计算昂贵且难以实现，研究旨在分析非精确优化的影响及其可行性。

Method: 定义了获取函数解的不精确性度量，并基于此建立了GP-UCB和GP-TS的累积遗憾界，同时验证了随机网格搜索的有效性。

Result: 在适当条件下，非精确优化仍能实现次线性累积遗憾，随机网格搜索在计算效率和效果上表现良好。

Conclusion: 非精确优化在贝叶斯优化中具有实际可行性，随机网格搜索是一种高效且实用的获取函数求解方法。
...
### [432] [Learning Overspecified Gaussian Mixtures Exponentially Fast with the EM Algorithm](https://arxiv.org/abs/2506.11850)
*Zhenisbek Assylbekov,Alan Legg,Artur Pak*

Main category: stat.ML

TLDR: 研究了EM算法在过参数化高斯混合模型中的收敛性，证明在特定结构配置下，算法能以指数速度收敛。
...
Result: 在特定条件下，EM算法能以指数速度收敛，且有限样本下也有统计收敛保证。
...
### [433] [How do Probabilistic Graphical Models and Graph Neural Networks Look at Network Data?](https://arxiv.org/abs/2506.11869)
*Michela Lapenna,Caterina De Bacco*

Main category: stat.ML

TLDR: 比较概率图模型（PGMs）和图神经网络（GNNs）在图数据中的表现，发现PGMs在低维或噪声特征及高异质性图中表现更优。


<details>
  <summary>Details</summary>
Motivation: 探究PGMs和GNNs在捕捉网络数据信息时的差异，尤其是在特征处理、噪声鲁棒性和异质性方面的表现。

Method: 通过链接预测任务进行实验，包括特征处理、噪声特征和高异质性图的测试。

Result: PGMs在低维或噪声特征下优于GNNs，且在高异质性图中更鲁棒。

Conclusion: PGMs在特定场景下（如低维或噪声特征、高异质性）表现更优，但需权衡计算复杂度和可解释性。
...
### [434] [Spectral Estimation with Free Decompression](https://arxiv.org/abs/2506.11994)
*Siavash Ameli,Chris van der Heide,Liam Hodgkinson,Michael W. Mahoney*

Main category: stat.ML

TLDR: 论文提出了一种基于自由概率理论的“自由解压”方法，用于估计无法直接访问或操作的超大矩阵的谱。


<details>
  <summary>Details</summary>
Motivation: 随着数据规模增长，直接处理超大矩阵变得不切实际，尤其是在分布式学习或间接定义矩阵的情况下，传统方法（如矩阵-向量乘积）可能无法使用。

Method: 利用自由概率理论，通过小子矩阵的经验谱密度推断超大矩阵的谱。

Result: 方法在合成和真实数据集上验证有效，能够匹配已知的极限分布和完整谱。

Conclusion: 自由解压方法为解决无法直接操作的矩阵谱估计问题提供了新思路。
...
### [435] [Real-World Deployment of a Lane Change Prediction Architecture Based on Knowledge Graph Embeddings and Bayesian Inference](https://arxiv.org/abs/2506.11925)
*M. Manzour,Catherine M. Elias,Omar M. Shehata,R. Izquierdo,M. A. Sotelo*

Main category: cs.AR

TLDR: 论文提出了一种基于知识图谱嵌入和贝叶斯推理的车道变换预测系统，并在真实硬件上验证了其有效性，填补了算法与实际部署之间的差距。


<details>
  <summary>Details</summary>
Motivation: 当前车道变换预测研究多局限于仿真或数据集，缺乏实际道路部署验证，本文旨在填补这一空白。
...
Conclusion: 该系统在真实硬件上验证了车道变换预测的可行性，为实际道路部署提供了有效解决方案。
...
### [436] [DPUV4E: High-Throughput DPU Architecture Design for CNN on Versal ACAP](https://arxiv.org/abs/2506.11441)
*Guoyu Li,Pengbo Zheng,Jian Weng,Enshan Yang*

Main category: cs.AR

TLDR: 本文提出了一种名为DPUV4E的设计，针对AMD Versal ACAP架构，通过优化计算单元和数据流，显著提升了性能和能效，同时减少了资源使用和延迟。


<details>
  <summary>Details</summary>
Motivation: 传统FPGA在平衡性能和多功能性方面存在挑战，而AMD Versal ACAP架构的AI Engines因内存带宽不足未能充分发挥潜力。本文旨在解决这些问题。

Method: 设计了两种计算单元（Conv PE和DWC PE），优化数据流以利用数据复用机会，并扩展PE功能以利用AIEs进行非卷积操作。

Result: 实验表明，DPUV4E在TOPS/W上提升了8.6倍，DSP使用减少95.8%，LUT使用减少44.7%，延迟降低至68.5%。在端到端推理中，吞吐量提升最高2.2倍（深度卷积模型）和1.3倍（标准模型）。

Conclusion: DPUV4E设计显著提升了Versal架构的性能和能效，同时减少了资源开销，适用于多种计算机视觉应用。
...
### [437] [PMF-CEC: Phoneme-augmented Multimodal Fusion for Context-aware ASR Error Correction with Error-specific Selective Decoding](https://arxiv.org/abs/2506.11064)
*Jiajun He,Tomoki Toda*

Main category: eess.AS

TLDR: 论文提出了一种基于音素增强的多模态融合方法（PMF-CEC），用于改进ASR后处理中对罕见词（尤其是同音异义词）的识别准确性，并通过保留概率机制减少过检测问题。


<details>
  <summary>Details</summary>
Motivation: 现有的ASR后处理方法（ED-CEC）在识别罕见词时表现不佳，尤其是对同音异义词的区分能力有限，且存在过检测问题。

Method: 在ED-CEC基础上引入音素增强的多模态融合方法（PMF-CEC），并结合保留概率机制过滤低置信度编辑操作。

Result: 在五个数据集上的实验表明，PMF-CEC在保持合理推理速度的同时，进一步降低了偏置词错误率，尤其在同音异义词纠正上表现更优。

Conclusion: PMF-CEC在罕见词纠正上优于现有方法，且在推理速度和鲁棒性方面优于基于LLM的方法。
...
### [438] [Regularized Federated Learning for Privacy-Preserving Dysarthric and Elderly Speech Recognition](https://arxiv.org/abs/2506.11069)
*Tao Zhong,Mengzhe Geng,Shujie Hu,Guinan Li,Xunying Liu*

Main category: eess.AS

TLDR: 本文研究了正则化联邦学习技术在保护隐私的构音障碍和老年语音识别中的应用，通过参数、嵌入和损失正则化方法显著提升了识别性能。


<details>
  <summary>Details</summary>
Motivation: 解决构音障碍和老年语音识别中的数据稀缺、分布不均和说话者异质性等问题，同时保护数据隐私。

Method: 采用参数、嵌入和损失三种正则化方法改进联邦学习过程。

Result: 正则化联邦学习系统显著优于基线FedAvg系统，WER相对降低2.13%。

Conclusion: 正则化联邦学习在保护隐私的同时，显著提升了语音识别性能，接近集中式训练效果。
...
### [439] [Embedded Acoustic Intelligence for Automotive Systems](https://arxiv.org/abs/2506.11071)
*Renjith Rajagopal,Peter Winzell,Sladjana Strbac,Konstantin Lindström,Petter Hörling,Faisal Kohestani,Niloofar Mehrzad*

Main category: eess.AS

TLDR: 通过分析车轮基座麦克风采集的声学特征，利用深度神经网络和预训练模型分类道路类型，提升自动驾驶系统的智能性和道路管理效率。


<details>
  <summary>Details</summary>
Motivation: 增强汽车系统智能性，解决道路类型识别问题，支持自适应学习和主动降噪，为城市规划提供数据支持。

Method: 使用深度神经网络和Hugging Face的预训练模型提取声学特征，分类道路类型。

Result: 研究结果为下一代汽车系统提供了商业案例，提升了乘客舒适度和车辆安全性。

Conclusion: 该方法不仅改善了驾驶体验，还为智能、可持续的城市道路管理奠定了基础。
...
### [440] [Improving Child Speech Recognition and Reading Mistake Detection by Using Prompts](https://arxiv.org/abs/2506.11079)
*Lingyun Gao,Cristian Tejedor-Garcia,Catia Cucchiarini,Helmer Strik*

Main category: eess.AS

TLDR: 提出了一种结合音频和文本知识的多模态方法，用于自动朗读评估，显著提升了儿童语音识别和阅读错误检测的效果。


<details>
  <summary>Details</summary>
Motivation: 为教师提供高效的朗读练习评分支持，填补朗读评估系统和应用研究的空白。

Method: 利用Whisper和指令调优的大型语言模型（LLMs）结合提示，改进儿童语音转录和阅读错误检测。

Result: 最佳系统在荷兰儿童朗读语音中实现了5.1%的词错误率（WER），优于基线9.4%；阅读错误检测的F1分数从0.39提升至0.73。

Conclusion: 提示Whisper和LLM的方法显著提升了朗读评估的准确性和实用性。
...
### [441] [Intelligibility of Text-to-Speech Systems for Mathematical Expressions](https://arxiv.org/abs/2506.11086)
*Sujoy Roychowdhury,H. G. Ranjani,Sumit Soman,Nishtha Paul,Subhadip Bandyopadhyay,Siddhanth Iyengar*

Main category: eess.AS

TLDR: 论文评估了五种TTS模型处理数学表达式（MX）的质量和可理解性，发现其表现普遍不如专家朗读，且可理解性因模型和MX类别而异。


<details>
  <summary>Details</summary>
Motivation: 当前高级TTS模型对数学表达式的处理能力缺乏充分评估，需要填补这一研究空白。

Method: 通过听力和转录测试评估五种TTS模型，使用LLMs将LaTeX MX转换为英语发音，并采用用户评分和转录正确性指标量化结果。

Result: TTS模型对MX的输出可理解性不足，表现显著低于专家朗读，且不同模型和MX类别间存在差异。LLM选择的影响有限。

Conclusion: 研究揭示了TTS模型在处理MX时的不足，强调了改进的必要性。
...
### [442] [Better Pseudo-labeling with Multi-ASR Fusion and Error Correction by SpeechLLM](https://arxiv.org/abs/2506.11089)
...
TLDR: 论文提出了一种统一的多ASR提示驱动框架，利用文本或语音大语言模型（LLM）进行后处理，替代传统的投票或仲裁逻辑，显著提高了转录准确性。


<details>
  <summary>Details</summary>
Motivation: 传统方法通过多阶段处理结合多个ASR输出生成伪标签，导致错误传播、信息丢失和优化不连贯，需要更高效的解决方案。

Method: 提出基于多ASR提示驱动的统一框架，结合文本或语音LLM进行后处理，避免传统仲裁逻辑。

Result: 实验表明，该方法在转录准确性上显著优于传统方法，且生成的伪标签在半监督ASR模型训练中表现更优。

Conclusion: 多ASR提示驱动框架结合LLM后处理是一种高效且准确的伪标签生成方法，适用于半监督ASR训练。
...
### [443] [Can We Trust Machine Learning? The Reliability of Features from Open-Source Speech Analysis Tools for Speech Modeling](https://arxiv.org/abs/2506.11072)
*Tahiya Chowdhury,Veronica Romero*

Main category: eess.AS

TLDR: 论文评估了OpenSMILE和Praat两种语音分析工具在自闭症青少年中的可靠性，发现特征提取存在显著差异，呼吁领域相关验证以提高临床应用的可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有语音分析工具缺乏验证，可能导致行为相关信息的不可靠性，影响模型的可重复性和公平性。

Method: 通过比较OpenSMILE和Praat提取的语音特征，评估其在自闭症青少年中的可靠性。

Result: 两种工具提取的特征存在显著差异，影响了模型在不同背景和人群中的性能。
...
### [444] [Challenges in Automated Processing of Speech from Child Wearables: The Case of Voice Type Classifier](https://arxiv.org/abs/2506.11074)
...
TLDR: 论文探讨了儿童佩戴设备录音数据在语音科学中的应用障碍，指出改进数据质量和数量比技术优化更重要。


<details>
  <summary>Details</summary>
Motivation: 研究旨在解决儿童自然语音数据转化为可用信息的技术挑战，推动语音科学的发展。

Method: 通过三年的实验，改进语音类型分类任务，测试了特征表示、架构和参数搜索等方法。

Result: 实验表明，技术优化带来的性能提升有限，而数据相关性和数量对进展更为关键。

Conclusion: 强调收集高质量、可共享数据的重要性，而非单纯依赖技术改进。
...
### [445] [Fifteen Years of Child-Centered Long-Form Recordings: Promises, Resources, and Remaining Challenges to Validity](https://arxiv.org/abs/2506.11075)
*Loann Peurey,Marvin Lavechin,Tarek Kunze,Manel Khentout,Lucas Gautheron,Emmanuel Dupoux,Alejandrina Cristia*

Main category: eess.AS

TLDR: 论文总结了使用儿童佩戴设备收集音频数据的技术，强调自动化分析的必要性及潜在误差，并提出改进数据质量的策略。


<details>
  <summary>Details</summary>
Motivation: 通过长时音频记录捕捉儿童语言输入和输出，减少观察者偏差，提高研究有效性。

Method: 总结现有技术资源，分析自动化注释中的误差来源，提出数据质量评估指标。

Result: 指出完全自动化的质量控制不可行，但提供了改进数据收集和分析的实用策略。

Conclusion: 研究者需结合自动化工具和手动策略，以确保数据质量和分析准确性。
