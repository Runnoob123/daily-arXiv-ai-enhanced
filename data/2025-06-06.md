...
Motivation: IPFS的去中心化和匿名性可能被滥用，研究旨在揭示其潜在风险。

Method: 通过脚本系统测试固定服务和公共网关的行为，上传恶意文件以评估其反应。

Result: 固定服务和公共网关缺乏评估或限制恶意内容传播的机制。

Conclusion: IPFS的匿名性和服务设计可能导致有害内容的传播，需改进机制以应对潜在滥用。
...
<details>
  <summary>Details</summary>
Motivation: 传统密码系统依赖代数问题，易受量子攻击和结构密码分析影响，需要非代数替代方案。

Method: 利用符号路径反转问题（SPIP）和哈希混淆生成密钥，通过分形维数等量化符号路径的复杂性。

Result: 实验显示SHA3-512和SHAKE256能有效放大符号分歧，分形维数稳定在1.06，安全性高。

Conclusion: HFKR提供了一种轻量级、无结构的密钥生成方案，不依赖代数假设，适用于对抗性环境。
...
Result: 该方法将防御准确率提高了约20%，但面对隐蔽性更强的自适应攻击时，攻击成功率仍可达35%。

Conclusion: 研究揭示了注意力模式防御的局限性，并强调了提升攻击隐蔽性的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 解决敏感医疗数据在微调LLM时的隐私泄露风险。

Method: 使用MIMIC-CXR和CT-RATE数据集，通过DP-LoRA方法在不同隐私预算下微调LLM，评估模型性能。

Result: 在中等隐私保护下，模型性能接近非隐私基线（MIMIC-CXR加权F1为0.88，CT-RATE为0.59）。
...
Result: BESA显著提升现有攻击的替代编码器准确率，最高达24.63%。

Conclusion: BESA有效对抗先进防御及多防御组合。
...
<details>
  <summary>Details</summary>
Motivation: 现有蜜罐密码技术无法防止攻击者通过凭证填充攻击识别真实密码，且站点缺乏参与监控的动机。

Method: 提出一种算法，使站点通过交换监控服务相互协作，并通过模型检查分析其效果。

Result: 算法能提高站点检测自身泄露的能力，且实际数据验证了其扩展性和实用性。

Conclusion: 该算法为蜜罐密码监控生态系统提供了可行的解决方案，并量化了参数对检测效果的影响。
...
<details>
  <summary>Details</summary>
Motivation: 标准PSI协议易受数据完整性攻击，恶意参与者可能获取额外交集信息或误导其他方，因此需要增强数据完整性保护。

Method: 通过将Merkle树与现有的volePSI（两方）和mPSI（多方）协议结合，构建了认证PSI方案。

Result: 两方认证PSI的通信复杂度为$\mathcal{O}(n \lambda+n \log n)$，多方认证PSI为$\mathcal{O}(n \kappa+n \log n)$，但开销高于未认证方案。

Conclusion: 认证PSI方案在保护数据完整性的同时，牺牲了部分效率，未来可通过优化认证模块改进。
...
<details>
  <summary>Details</summary>
Motivation: LINCOS协议在单个QKD网络中存储秘密时存在安全性漏洞，当网络被攻破时容易暴露。MULTISS旨在通过多网络分布和分层秘密共享解决这一问题。

Method: MULTISS采用分层秘密共享技术，将秘密分布在多个QKD网络中，并支持动态更新。

Result: MULTISS在安全性上严格优于LINCOS，且能高效处理秘密更新。

Conclusion: MULTISS为长期分布式存储提供了更高的安全性和灵活性，适用于多QKD网络环境。
...
<details>
  <summary>Details</summary>
Motivation: 现代计算环境复杂性和网络威胁日益复杂，需要更强大、自适应和自动化的安全执行方法。

Method: 系统收集和管理工具及API规范，存储在向量数据库中；架构管道将高级缓解策略分解为离散任务，并转换为可操作的API调用。

Result: 使用RAG相比非RAG基线，在精确率、召回率和F1分数上有显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 解决数字支付中的隐私保护和合规性需求，同时降低计算和财务开销。

Method: 集成两种合规机制：清白证明和基于布隆过滤器的祖先承诺跟踪系统，支持快速私有转账和低开销证明生成。

Result: 在消费级笔记本电脑上，私有转账和合规检查可在几秒内完成，链上成本极低，适用于Layer 2网络。

Conclusion: 该方案验证了隐私保护、合规性数字支付的可行性，兼具高效性和低成本。
...
TLDR: PoCGen是一种利用大型语言模型（LLM）结合静态和动态分析技术，自主生成和验证npm包漏洞的PoC利用的方法，成功率为77%（SecBench.js数据集）和39%（新数据集），成本低至每次0.02美元。


<details>
  <summary>Details</summary>
Motivation: 漏洞报告通常缺乏PoC利用，导致修复和测试困难。PoCGen旨在解决这一问题，通过自动化生成PoC利用。

Method: 结合LLM、静态和动态分析技术，理解漏洞报告并生成、验证候选PoC利用。

Result: 在SecBench.js数据集上成功率为77%，新数据集上为39%，显著优于基线（提升45个百分点）。

Conclusion: PoCGen是一种高效、低成本的自动化PoC生成方法，显著提升了漏洞修复效率。
...
<details>
  <summary>Details</summary>
Motivation: 现代搜索引擎通过用户查询历史和行为构建详细画像以实现个性化，但带来了隐私风险和过滤气泡问题。

Method: 提出并评估了一种客户端查询混淆策略，通过在真实查询中插入随机生成的多语言噪声查询。

Result: 实验显示，混淆后搜索引擎识别的用户兴趣显著变化，但搜索结果保持稳定，随机查询能有效防止准确画像。

Conclusion: 查询混淆是一种可行的隐私保护机制，用户可通过工具自主保护搜索行为而无需修改现有基础设施。
...
<details>
  <summary>Details</summary>
Motivation: 机器学习的密集型数据需求与5G通信等新技术的结合，为车载网络中的联邦学习（FL）应用提供了可能性和合理性。

Method: 提出了一种基于LSTM自编码器的FL实现，用于车载CAN网络的入侵检测系统（IDS），并与集中式版本进行对比。

Result: 研究表明，FL实现具有可行的检测效率和通信开销。

Conclusion: FL在车载入侵检测领域是一种可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统安全检测方法存在数据收集不足、资源消耗高和检测算法误报率高等问题。

Method: FEAD采用攻击模型驱动方法提取关键监控项，优化任务分配以减少开销，并利用局部感知异常分析提高准确性。

Result: FEAD的F1分数比现有方案高8.23%，开销仅增加5.4%。

Conclusion: 基于聚焦的设计显著提升了检测性能，验证了FEAD框架的有效性。
...
Method: 与EMBER2017和EMBER2018作者合作，创建包含320万文件、支持七种分类任务的数据集，并引入挑战集。

Result: EMBER2024支持多种机器学习任务，并首次包含逃避检测的恶意文件，提供更全面的评估。
...
TLDR: 论文提出了一种改进序列模型隐私泄漏审计的方法，通过利用序列生成中的相关性，改进了现有的成员推理攻击方法，提高了审计效果。


<details>
  <summary>Details</summary>
Motivation: 现有工具在审计序列模型（如LLM和自回归图像生成器）的隐私泄漏风险时存在不足，原因是假设不匹配。

Method: 通过调整最先进的成员推理攻击方法，显式建模序列内相关性，以适应序列模型的结构。

Result: 案例研究表明，改进后的方法在不增加计算成本的情况下，显著提高了记忆审计的效果。
...
<details>
  <summary>Details</summary>
Motivation: 研究Arm CCA时缺乏硬件支持，导致重复开发和不一致的性能比较，OpenCCA旨在解决这一问题。

Method: 通过系统调整软件栈（包括引导加载程序、固件、虚拟机和内核），在普通Armv8.2硬件上模拟CCA操作。

Result: OpenCCA在低成本Rockchip板上成功模拟CCA操作，并通过生命周期测量和案例研究验证其有效性。

Conclusion: OpenCCA为Arm CCA研究提供了低成本、高效的解决方案，降低了研究门槛。
...
<details>
  <summary>Details</summary>
Motivation: 本地部署的LLM缺乏开发者直接控制，易被滥用，现有云服务缓解技术无效。

Method: 设计任务特定神经元提取机制、分层策略树、混合加密框架及解密神经元检测机制。

Result: SECNEURON将未授权任务准确率限制在25%以下，授权任务损失控制在2%内，显著减少数据泄露。

Conclusion: SECNEURON为本地LLM提供了可靠、灵活且经过认证的滥用控制方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前PPA设计缺乏隐私预算管理的原则性方法，导致关键设计决策的不确定性。

Method: 提出Big Bird，明确每个站点的预算语义，并引入基于资源隔离原则的全局预算系统。

Result: 在Firefox中实现Big Bird，并在真实广告数据上验证其弹性和有效性。

Conclusion: Big Bird为对抗性环境中的隐私保护提供了坚实基础。
...
<details>
  <summary>Details</summary>
Motivation: 探讨安全对齐数据与下游任务相似性对模型安全性的影响，以解决现有缓解策略忽视上游数据设计的问题。

Method: 通过实验分析上游安全对齐数据集与下游微调任务的表示相似性对模型安全性的影响。

Result: 高相似性会显著削弱安全防护，低相似性则能降低有害性评分达10.33%，提升模型鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 基于概念的模型因其可解释性在医疗诊断和金融风险预测等关键领域受到关注，但其结构、训练和对抗性漏洞等问题限制了其可靠性和泛化能力。

Method: 通过综合调查，总结了基于概念的模型在监督和无监督范式中的常见挑战及缓解这些挑战的架构选择。

Result: 揭示了概念泄漏、纠缠表示和对扰动的有限鲁棒性等问题，并讨论了提高模型可靠性的最新进展。

Conclusion: 本文指出了基于概念的模型在现实应用中的开放性问题，并提出了未来研究的潜在方向。
...
<details>
  <summary>Details</summary>
Motivation: 提高深度学习模型在开放场景中的可靠性，现有OOD检测器主要依赖统计模型，缺乏可解释性。

Method: 结合MLNs，通过加权逻辑约束对输入进行概率分配，并提出学习逻辑约束的算法。

Result: 实验表明，MLNs能显著提升多种OOD检测器的性能，同时保持高效计算。

Conclusion: MLNs为OOD检测提供了可解释且高效的增强方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前方法未能捕捉混凝土徐变的时间序列特性，限制了预测精度。

Method: 将混凝土徐变预测转化为自回归序列建模任务，利用Transformer的自注意力机制，结合时间、特征和批次三重注意力框架。

Result: 在160天标准化测量数据集上，平均绝对百分比误差为1.63%，R2值为0.999，显著优于传统模型。

Conclusion: 该架构验证了Transformer在材料科学问题中的可行性，为工程设计和结构行为预测提供了数据驱动的新方法。
...
<details>
  <summary>Details</summary>
Motivation: 适应不断变化的安全政策对LLM进行微调成本高且不切实际，需要更灵活的方法。

Method: 利用类别特定引导向量和无监督梯度方法，避免对比数据需求。

Result: 在多种LLM和数据集上验证了精确控制和安全性提升，避免全面拒绝。

Conclusion: SafeSteer简单有效，适用于动态安全需求，优于复杂方法。
...
<details>
  <summary>Details</summary>
Motivation: 气候变化加剧了火灾的频率和行为，传统的二元分类方法过于简化问题，无法满足消防员等终端用户的需求。

Method: 提出了一种考虑地方条件（如地形、气候和历史火灾数据）的火灾风险评估方法，并在法国全国范围内进行了AI模型的基准测试。

Result: 实现了首个针对法国全国范围的AI基准测试，提供了更具操作性和区域特异性的火灾风险预测。

Conclusion: 该方法为火灾风险评估提供了更实用的解决方案，并指出了未来需要进一步研究的方向。
...
TLDR: MUC-G4是一种新型增量验证框架，用于压缩深度神经网络的验证，通过编码和分类变化，利用最小不可满足核心（MUCs）提高验证效率。


<details>
  <summary>Details</summary>
Motivation: 解决边缘设备上神经网络部署的高内存和运行时复杂度问题，现有增量验证方法仅关注量化且难以处理结构变化。

Method: 将原始和压缩网络编码为SMT公式，分类变化，利用原始网络的MUCs指导压缩网络的验证。

Result: 实验证明MUC-G4能高效处理量化和剪枝，验证时间显著缩短，证明重用率高。
...
<details>
  <summary>Details</summary>
Motivation: 探索采样分布对DPO训练动态的影响，以理解其如何影响策略更新和收敛行为。

Method: 分析响应分布对梯度下降中策略更新的影响，设计简化对齐模型，并通过实验验证理论。

Result: 高质量响应频率的增加能增强梯度信号，改善优化效果，提升策略学习效率。

Conclusion: 研究结果为在线DPO框架提供了理论支持，表明采样分布的质量对优化效果至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 复合洪水的预测因气象、水文和海洋因素的复杂交互而具有挑战性，且现有数据集不全面。

Method: 引入SF2Bench数据集，整合潮汐、降雨、地下水和人类管理活动，并评估六种机器学习方法。

Result: 实验验证了不同特征对洪水预测的影响，并展示了各种方法在时空依赖性上的表现差异。

Conclusion: SF2Bench数据集和分析方法为改进复合洪水预测提供了新工具和见解。
...
Method: DrSR通过分析数据的结构关系（如单调性、非线性、相关性）生成结构化描述，并建立反馈循环优化方程生成。
...
<details>
  <summary>Details</summary>
Motivation: 解决卫星数据下传带宽受限的问题，通过优先处理相关数据以支持自然灾害的快速响应。

Method: 利用基于忆阻器的物理神经网络（PANN），动态权重根据输入信号更新，生成物理约束的动态特征用于变化检测。

Result: PANN在自然灾害检测任务中表现优于或与最先进的AI模型相当，且无需训练，计算资源需求低。

Conclusion: PANN为资源受限的星载处理提供了一种高效解决方案，特别适用于自然灾害监测。
...
<details>
  <summary>Details</summary>
Motivation: 适应数据有限导致训练挑战，利用主干数据可以提升适应性能。
...
Method: 通过比较IWL（参数内学习）和ICL（上下文学习）两种机制，分析模型在传递推理任务中的表现，并进行了机制性分析。

Result: IWL模型能自然泛化传递推理，而ICL模型仅能处理相邻关系；但预训练后的ICL模型也能实现类似效果。
...
Method: 提出LDPTRLQ算法，通过理论分析在合理条件下结合Lyapunov漂移加惩罚与强化学习，平衡短期优化与长期视角。

Result: 仿真结果表明LDPTRLQ在性能和稳定性上优于基线方法，验证了理论推导的有效性。

Conclusion: LDPTRLQ算法在兼容性和稳定性方面表现优异，为物联网优化问题提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 临床试验成本高且耗时，准确预测结果可降低成本并加速药物发现。现有深度学习模型因缺乏可解释性和标签泄漏问题限制了其应用。

Method: 提出AutoCT框架，结合大语言模型和经典机器学习，利用蒙特卡洛树搜索自主生成、评估和优化特征。

Result: AutoCT在有限迭代次数内表现优于或媲美现有方法，为临床试验预测提供了可扩展、可解释且高效的解决方案。

Conclusion: AutoCT为临床试验预测提供了新范式，兼具高性能和可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 在向可再生能源转型过程中，确保电网稳定需要精确的电力需求预测。研究旨在通过区分不同消费者群体（工业、商业、居民）并定制模型，解决这一需求。

Method: 通过客户聚类区分消费者类型，结合时间、社会经济和天气数据（来自Copernicus EO计划），探索并比较多种AI和机器学习算法用于STLF和VSTLF。

Result: 提出的新预测方法在STLF和VSTLF中表现优于简单模型，证明了定制化策略和详细天气数据的重要性。

Conclusion: 研究为电力需求预测提供了更可靠的方法，支持电网稳定性。
...
Conclusion: 混合建模方法为动态不确定条件下的车队管理提供了全面决策支持工具，未来将整合更多运营约束以增强预测能力。
...
Method: 1. 解剖学过程：选择多样化的规范形式构建目标集成模型；2. 生理学过程：通过softlog-softmax层实现中间决策；3. 信息理论过程：提出基于softlog的熵和散度；4. 性能评估过程：推导信息性能张量。

Result: 研究表明，问题越复杂，集成多样性越有用；提出的信息度量在闭区间上提供一致值。

Conclusion: 通过4步流程显著提升了集成学习系统的一致性和可靠性，为复杂问题提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究AI代理在动态推理中的系统级成本、效率和可持续性问题，以揭示其潜在的计算资源危机。

Method: 通过量化不同代理设计和测试策略下的资源使用、延迟行为、能耗等，分析设计选择对精度与成本的影响。

Result: 研究发现，尽管AI代理通过增加计算提升精度，但存在效率递减、延迟方差扩大和基础设施成本不可持续的问题。

Conclusion: 呼吁转向计算高效的设计范式，以平衡性能与可部署性。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法需要多次训练以调整损失权重超参数，效率低下。YOTO旨在通过一次性训练解决这一问题。

Method: 将损失权重超参数视为网络参数，通过梯度优化自动学习，并引入软最大操作和正则化损失以确保优化效果。

Result: YOTO在3D估计和语义分割任务中表现优于传统网格搜索方法。

Conclusion: YOTO提供了一种高效的一次性训练方法，显著提升了模型性能。
...
TLDR: 提出一种“半”隐藏层单元，部分权重随机固定，部分可训练，减少参数数量并保持合理准确率。


<details>
  <summary>Details</summary>
Motivation: 通过减少可训练参数数量并利用随机连接的规范化效果，提高模型的效率和泛化能力。

Method: 每个隐藏单元由两个阶段组成：固定随机权重的加权和输入，再通过两个可调权重进行缩放和平移，最后通过非线性激活。

Result: 在MNIST和FashionMNIST数据集上实验表明，半隐藏层能以较少参数实现合理准确率。

Conclusion: 半隐藏层是一种有效的设计，适用于深度网络的不同层，具有参数效率和规范化优势。
...
<details>
  <summary>Details</summary>
Motivation: 传统单一指标（如夏普比率）无法满足多样化的投资者偏好，需要模块化、参数化的奖励函数。

Method: 使用四个可微分项（年化收益、下行风险、差异收益和特雷诺比率）构建复合奖励函数，并通过网格搜索调整权重。

Result: 提供了闭式梯度，便于梯度训练，并分析了单调性、有界性和模块性等理论特性。

Conclusion: 该框架为复杂交易环境中构建多目标奖励函数提供了通用蓝图，并可扩展更多风险度量或自适应权重。
...
Method: 将hyperDT算法重新表达为Beltrami-Klein模型，通过预处理和后处理步骤包装现有欧几里得空间模型。
...
<details>
  <summary>Details</summary>
Motivation: 为LLM聊天机器人交互提供隐私保护的洞察生成方法，确保用户数据隐私。

Method: 采用私有聚类机制和多种关键词提取方法（频率、TF-IDF、LLM引导），结合DP工具（聚类、分区选择、直方图汇总）。

Result: 在保留语义内容和相似性的同时，提供严格的隐私保护，优于非隐私方法。

Conclusion: $Urania$ 在隐私保护和数据效用之间取得了平衡，适用于隐私敏感的交互分析。
...
<details>
  <summary>Details</summary>
Motivation: 持续学习中遗忘问题是主要挑战，样本回放是常用方法，但其理论影响尚不明确。

Method: 在过参数化的持续线性回归设置中，理论分析样本回放的作用，探讨任务子空间关系。

Result: 发现回放样本可能导致非单调遗忘，某些情况下有害；神经网络实验验证了类似现象。

Conclusion: 样本回放的效果取决于回放样本选择和任务关系，需谨慎设计。
...
<details>
  <summary>Details</summary>
Motivation: 移除目标网络可以释放内存并利用最新估计，但会导致性能下降。本文旨在找到一种折中方法，既能减少内存占用，又能保持性能。

Method: 提出iS-QL方法，共享在线网络的大部分参数，仅复制最后一层作为目标网络，结合并行学习Bellman迭代的思想。

Result: iS-QL在多种设置下提高了目标无关方法的样本效率，内存占用更小且训练时间与传统目标方法相当。

Conclusion: iS-QL为强化学习研究提供了一种可扩展的新方法，平衡了性能和资源消耗。
...
Method: 结合条件神经过程（CNPs）和策略梯度元强化学习，利用离线学习任务推断，通过单次测试任务推断潜在动态模型。

Result: 在2D-Point Agent和连续控制任务中，UMCNP比基线方法显著减少元测试所需样本。

Conclusion: UMCNP在无奖励信号的元测试场景中高效且可扩展，适用于复杂动态环境。
...
<details>
  <summary>Details</summary>
Motivation: 探究对抗训练的动态机制，特别是类间特征的作用，以改进对AT的理解。

Method: 通过类间特征分析AT，结合理论模型和系统实验验证跨类特征的影响。

Result: AT初期模型学习更多跨类特征，提升鲁棒性；后期依赖类内特征导致鲁棒过拟合。

Conclusion: 研究为AT机制提供了新视角，统一了软标签训练和鲁棒过拟合的现有观点。
...
<details>
  <summary>Details</summary>
Motivation: 填补自监督对比学习理论基础的空白，揭示其与监督变体的关系及其几何特性。

Method: 提出NSCL损失，分析其与CL损失的差距随类别数增加而消失的特性，并研究其特征的几何结构及少样本误差界限。

Result: CL与NSCL损失差距随类别数增加而减小，特征具有特定几何结构，且少样本误差界限与实验表现一致。

Conclusion: 自监督对比学习隐式优化了NSCL损失，其特征结构支持高精度的少样本分类。
...
<details>
  <summary>Details</summary>
Motivation: 尽管实证研究表明合成数据在模型隐藏时可能受益于隐私放大，但缺乏严格的理论支持。本文旨在填补这一空白。

Method: 采用线性回归框架，分析合成数据在生成模型隐藏时的隐私放大效应。

Result: 若生成种子被控制，单个合成数据点可能泄露与模型相同的信息；而使用随机输入生成有限数据点可放大隐私。

Conclusion: 本文在线性回归中的发现为未来推导更一般的隐私界限奠定了基础。
...
TLDR: 论文提出两种零阶优化方法（JAGUAR SignSGD和JAGUAR Muon），用于高效微调大语言模型，显著降低内存和计算成本。


<details>
  <summary>Details</summary>
Motivation: 传统一阶优化器（如SGD和Adam）在大模型微调中内存和计算成本过高，零阶优化方法提供了一种高效替代方案。

Method: 提出JAGUAR SignSGD（基于零阶SignSGD的动量算法）和JAGUAR Muon（利用参数矩阵结构的零阶Muon优化器），并提供收敛性理论保证。

Result: 实验表明，所提方法在收敛质量上优于或等同于一阶方法，同时显著减少内存使用。
...
Conclusion: KOALA++在效率和性能之间取得了平衡，适用于多种任务。
...
### [53] [Grokking and Generalization Collapse: Insights from \texttt{HTSR} theory](https://arxiv.org/abs/2506.04434)
*Hari K. Prakash,Charles H. Martin*

Main category: cs.LG

TLDR: 论文研究了神经网络中的grokking现象，发现了一个新的阶段——anti-grokking，并通过HTSR指标α揭示了其特性。


<details>
  <summary>Details</summary>
Motivation: 探索神经网络训练中的grokking现象，并识别新的训练阶段。

Method: 使用3层MLP在MNIST子集上训练，结合HTSR指标α分析训练阶段。

Result: 发现anti-grokking阶段，HTSR指标α能区分所有三个阶段，而其他指标仅能检测前两个。

Conclusion: HTSR指标α可作为通用层收敛目标，用于衡量过拟合和泛化崩溃。
...
### [54] [RETRO SYNFLOW: Discrete Flow Matching for Accurate and Diverse Single-Step Retrosynthesis](https://arxiv.org/abs/2506.04439)
*Robin Yadav,Qi Yan,Guy Wolf,Avishek Joey Bose,Renjie Liao*

Main category: cs.LG

TLDR: 论文提出了一种名为RETRO SYNFLOW (RSF)的新方法，用于单步逆合成反应预测，通过离散流匹配框架和反应中心识别步骤，显著提高了预测准确性和多样性。


<details>
  <summary>Details</summary>
Motivation: 有机化学中，预测目标产物分子的合成反应序列是一个基础性问题，现有方法在单步逆合成反应预测中仍面临准确性和多样性的挑战。

Method: 采用离散流匹配框架（RSF），结合反应中心识别步骤生成中间结构（合成子），并通过Feynman-Kac引导和序贯蒙特卡洛重采样提升生成样本的多样性和可行性。

Result: 实验表明，RSF在top-1准确率上达到60.0%，比现有最优方法提升20%；FK引导进一步将top-5往返准确率提升19%。

Conclusion: RSF方法在单步逆合成反应预测中表现出色，显著提升了准确性和多样性，同时保持了竞争力的top-k准确率。
...
### [55] [Selective Matching Losses -- Not All Scores Are Created Equal](https://arxiv.org/abs/2506.04446)
*Gil I. Shamir,Manfred K. Warmuth*

Main category: cs.LG

TLDR: 论文提出了一种选择性匹配损失函数，通过设计递增的链接函数来强调高重要性区域的预测准确性，解决了传统损失函数在多类问题中的局限性。


<details>
  <summary>Details</summary>
Motivation: 在预测任务中，某些区域的准确性比其他区域更重要，但传统损失函数无法区分重要性区域。

Method: 设计了基于Sigmoid和双曲正弦链接的选择性标量损失函数，并扩展到多维选择性损失框架，克服了标准Softmax的局限性。

Result: 选择性损失函数在高重要性区域预测任务中表现优于传统损失函数，适用于多种应用场景。

Conclusion: 选择性损失函数为需要区分重要性区域的任务提供了有效解决方案，尤其在多类问题中具有显著优势。
...
### [56] [Neurosymbolic Artificial Intelligence for Robust Network Intrusion Detection: From Scratch to Transfer Learning](https://arxiv.org/abs/2506.04454)
*Huynh T. T. Tran,Jacob Sander,Achraf Cohen,Brian Jalaian,Nathaniel D. Bastian*

Main category: cs.LG

TLDR: 论文扩展了ODXU框架，结合神经符号AI和不确定性量化，提升网络入侵检测系统的性能，并在实验中优于传统模型。


<details>
  <summary>Details</summary>
Motivation: 探索神经符号AI在网络入侵检测中的应用，填补迁移学习在网络安全领域的空白。

Method: 扩展ODXU框架，结合深度嵌入聚类、符号推理和不确定性量化方法，开发迁移学习策略。

Result: 在CIC-IDS-2017数据集上表现优于传统模型，迁移学习策略在ACI-IoT-2023数据集上仅需50%数据即可超越传统模型。

Conclusion: ODXU框架在网络安全中具有潜力，迁移学习策略和元模型方法显著提升性能。
...
### [57] [Behavioural vs. Representational Systematicity in End-to-End Models: An Opinionated Survey](https://arxiv.org/abs/2506.04461)
*Ivan Vegner,Sydelle de Souza,Valentin Forch,Martha Lewis,Leonidas A. A. Doumas*

Main category: cs.LG

TLDR: 论文探讨了系统性在ML模型中的重要性，区分了行为系统性与表征系统性，并分析了现有基准测试的局限性。


<details>
  <summary>Details</summary>
Motivation: 系统性是ML模型中理想的性质，但现有研究多关注行为系统性，忽视了表征系统性。本文旨在强调这一区别并探讨如何评估表征系统性。

Method: 基于Hadley的分类法，分析了语言和视觉领域的关键基准测试，并借鉴机制可解释性领域的方法评估表征系统性。

Result: 发现现有基准测试主要测试行为系统性，而非表征系统性。

Conclusion: 呼吁更多关注表征系统性的评估方法，以更全面地理解模型的系统性能力。
...
### [58] [Classifying Dental Care Providers Through Machine Learning with Features Ranking](https://arxiv.org/abs/2506.04474)
*Mohammad Subhi Al-Batah,Mowafaq Salem Alzboon,Muhyeeddin Alqaraleh,Mohammed Hasan Abu-Arqoub,Rashiq Rafiq Marie*

Main category: cs.LG

TLDR: 该研究利用机器学习模型对牙科服务提供者进行分类，通过特征排名和多种算法评估，发现神经网络表现最佳，准确率达94.1%。


<details>
  <summary>Details</summary>
Motivation: 研究旨在通过机器学习优化牙科服务提供者的分类，以支持资源分配，特别是针对服务不足人群。

Method: 使用2018年的24,300条数据，包含20个特征，采用信息增益、基尼指数和ANOVA进行特征排名，评估12种机器学习模型。

Result: 神经网络准确率最高（94.1%），特征选择显示治疗相关指标是关键预测因素。

Conclusion: 特征选择对模型性能至关重要，建议结合特征排名与高级算法优化分类，以支持资源分配。
...
### [59] [Comparative performance of ensemble models in predicting dental provider types: insights from fee-for-service data](https://arxiv.org/abs/2506.04479)
*Mohammad Subhi Al-Batah,Muhyeeddin Alqaraleh,Mowafaq Salem Alzboon,Abdullah Alourani*

Main category: cs.LG

TLDR: 该研究评估了机器学习模型在牙科提供者分类中的表现，神经网络和随机森林表现最佳，显著优于传统分类器。


<details>
  <summary>Details</summary>
Motivation: 优化医疗资源分配和政策规划，提升对服务不足人群的服务交付。

Method: 使用2018年数据集（24,300实例，20特征），测试多种机器学习算法（如kNN、决策树、SVM、SGD、随机森林、神经网络、梯度提升），采用10折交叉验证，评估指标包括AUC、分类准确率、F1分数等。

Result: 神经网络表现最佳（AUC: 0.975, CA: 94.1%），随机森林次之（AUC: 0.948, CA: 93.0%），显著优于传统分类器。

Conclusion: 先进机器学习技术（尤其是集成和深度学习模型）能显著提升牙科提供者分类，有助于优化资源分配和服务不足人群的医疗资源获取。
...
### [60] [COSMOS: Predictable and Cost-Effective Adaptation of LLMs](https://arxiv.org/abs/2505.01449)
*Jiayu Wang,Aws Albarghouthi,Frederic Sala*

Main category: cs.LG

TLDR: COSMOS框架通过轻量级代理模型和低样本缩放定律，高效预测LLM适应策略的性能和成本，显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 在资源受限情况下，如何高效选择和优化LLM的适应策略是一个挑战，需要减少昂贵的实验成本。

Method: 提出COSMOS框架，结合嵌入增强的轻量级代理模型和低样本缩放定律，预测微调性能和检索增强的上下文学习效果。

Result: 在八个基准测试中，COSMOS平均减少92.72%的计算成本，最高可达98.71%，同时保持高预测准确性。

Conclusion: 高效预测LLM适应策略的可行性，显著降低部署成本，同时维持性能标准。
...
### [61] [Orthogonal Gradient Descent Improves Neural Calibration](https://arxiv.org/abs/2506.04487)
*C. Evans Hedges*
...
### [62] [Multiscale guidance of AlphaFold3 with heterogeneous cryo-EM data](https://arxiv.org/abs/2506.04490)
*Rishwanth Raghu,Axel Levy,Gordon Wetzstein,Ellen D. Zhong*

Main category: cs.LG

TLDR: CryoBoltz结合冷冻电镜密度图和蛋白质结构预测模型，通过全局和局部结构约束指导采样轨迹，生成与实验数据一致的构象状态。


<details>
  <summary>Details</summary>
Motivation: 解决蛋白质结构预测模型在捕捉动态生物分子复合物构象多样性上的不足，以及冷冻电镜从原始数据到原子模型的复杂流程。

Method: 结合冷冻电镜密度图和预训练的蛋白质结构预测模型，利用全局和局部结构约束指导采样轨迹。
...
### [63] [Perturbative Gradient Training: A novel training paradigm for bridging the gap between deep neural networks and physical reservoir computing](https://arxiv.org/abs/2506.04523)
*Cliff B. Abbott,Mark Elo,Dmytro A. Bozhko*

Main category: cs.LG

TLDR: PGT是一种新的训练方法，通过随机扰动近似梯度更新，解决了物理储层计算中无法反向传播的问题。


<details>
  <summary>Details</summary>
Motivation: 物理储层计算因黑盒性质无法使用反向传播，PGT旨在解决这一问题。

Method: 利用参数空间的随机扰动，仅通过前向传播近似梯度更新。

Result: PGT在模拟和实验硬件中表现与标准反向传播相当。

Conclusion: PGT为物理储层与深度神经网络结合及节能AI训练提供了可能。
...
### [64] [Hierarchical Implicit Neural Emulators](https://arxiv.org/abs/2506.04528)
*Ruoxi Jiang,Xiao Zhang,Karan Jakhar,Peter Y. Lu,Pedram Hassanzadeh,Michael Maire,Rebecca Willett*

Main category: cs.LG

TLDR: 提出了一种多尺度隐式神经模拟器，通过分层未来状态表示提升长期预测准确性，优于自回归基线。


<details>
  <summary>Details</summary>
Motivation: 神经PDE求解器在长期时间范围内易受误差累积和稳定性问题困扰，需改进。

Method: 利用多尺度隐式时间步进方法，通过分层压缩的未来状态预测优化下一步细化。

Result: 在湍流流体动力学实验中，短期精度高且长期预测稳定，显著优于基线。

Conclusion: 多尺度隐式设计有效提升长期预测稳定性与准确性，计算开销低。
...
### [65] [HALoS: Hierarchical Asynchronous Local SGD over Slow Networks for Geo-Distributed Large Language Model Training](https://arxiv.org/abs/2506.04531)
*Geon-Woo Kim,Junbo Li,Shashidhar Gandham,Omar Baldonado,Adithya Gangidi,Pavan Balaji,Zhangyang Wang,Aditya Akella*

Main category: cs.LG

TLDR: HALoS是一种分层异步优化框架，通过引入本地和全局参数服务器，减少跨区域通信成本，提升异构硬件利用率，并保持模型质量。
...
Method: 提出HALoS框架，使用本地参数服务器（LPSs）和全局参数服务器（GPS）分层优化，减少通信开销并利用快速区域内链接。

Result: HALoS比同步基线快7.5倍，比现有异步方法快2.1倍，同时保持或超过同步SGD的模型质量。

Conclusion: 分层异步优化是高效训练异构分布式环境中大规模语言模型的有效工具。
...
### [66] [NOBLE -- Neural Operator with Biologically-informed Latent Embeddings to Capture Experimental Variability in Biological Neuron Models](https://arxiv.org/abs/2506.04536)
*Luca Ghafourpour,Valentin Duruisseaux,Bahareh Tolooshams,Philip H. Wong,Costas A. Anastassiou,Anima Anandkumar*

Main category: cs.LG

TLDR: NOBLE是一种神经算子框架，通过学习神经元特征到电压响应的映射，解决了传统生物真实神经元模型的局限性，实现了高效生成合成神经元并显著加速计算。
...
Method: NOBLE通过连续频率调制嵌入神经元特征，学习其与电压响应的映射，利用生物物理真实模型生成的数据进行训练。

Result: NOBLE能够预测神经动态分布，实现4200倍的计算加速，并生成具有实验变异性的一致动态模型。

Conclusion: NOBLE为理解神经元组成、计算及神经AI应用提供了新工具，推动了生物真实建模的发展。
...
### [67] [Neural MJD: Neural Non-Stationary Merton Jump Diffusion for Time Series Prediction](https://arxiv.org/abs/2506.04542)
*Yuanpei Gao,Qi Yan,Yan Leng,Renjie Liao*

Main category: cs.LG

TLDR: Neural MJD模型通过结合神经网络的灵活性和Merton跳跃扩散模型的显式随机过程建模，提升了非平稳时间序列预测的性能。


<details>
  <summary>Details</summary>
Motivation: 深度学习在时间序列预测中表现优异，但其黑盒特性难以显式建模非平稳数据中的随机过程和突变。

Method: 提出Neural MJD模型，将预测问题建模为SDE模拟，结合时间非齐次Itô扩散和复合泊松过程，并引入截断机制和优化求解器。

Result: 在合成和真实数据集上，Neural MJD优于现有深度学习和统计学习方法。

Conclusion: Neural MJD通过显式建模随机过程和突变，显著提升了非平稳时间序列预测的泛化能力。
...
### [68] [Communication Efficient Adaptive Model-Driven Quantum Federated Learning](https://arxiv.org/abs/2506.04548)
*Dev Gurung,Shiva Raj Pokhrel*

Main category: cs.LG

TLDR: 该论文提出了一种模型驱动的量子联邦学习算法（mdQFL），解决了大规模数据集、多设备参与和非独立同分布数据带来的挑战，显著降低了通信成本并提升了模型性能。
...
Method: 提出mdQFL算法，结合训练和更新个性化以及测试泛化能力，适用于多种设备数量和非IID数据分布。

Result: 实验表明，mdQFL在Qiskit环境中显著降低50%通信成本，同时保持或提升模型准确性，并改善本地模型训练。

Conclusion: mdQFL是首个在QFL中探索训练和更新个性化的算法，具有广泛适用性，并通过实验和理论分析验证了其有效性。
...
### [69] [Unsupervised Machine Learning for Scientific Discovery: Workflow and Best Practices](https://arxiv.org/abs/2506.04553)
*Andersen Chang,Tiffany M. Tang,Tarek M. Zikry,Genevera I. Allen*

Main category: cs.LG

TLDR: 本文提出了一种标准化的无监督学习工作流程，旨在提高科学发现的可靠性和可重复性，并通过天文学案例研究验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 无监督学习在多个科学领域广泛应用，但缺乏标准化工作流程以确保可靠和可重复的科学发现。

Method: 提出结构化工作流程，包括科学问题制定、数据准备与探索、建模技术选择、严格验证及结果沟通与文档化。

Result: 通过天文学案例研究验证了工作流程的有效性，强调了验证的重要性。

Conclusion: 精心设计的无监督学习工作流程能显著推动科学发现，并提升其可靠性与可重复性。
...
### [70] [Clustering and Median Aggregation Improve Differentially Private Inference](https://arxiv.org/abs/2506.04566)
*Kareem Amin,Salman Avestimehr,Sara Babakniya,Alex Bie,Weiwei Kong,Natalia Ponomareva,Umar Syed*

Main category: cs.LG

TLDR: 论文提出了一种改进的差分隐私语言模型推理方法，通过聚类输入数据和私有计算中位数来提升生成文本的质量和隐私保护效果。


<details>
  <summary>Details</summary>
Motivation: 解决现有差分隐私语言模型推理方法中均匀采样导致生成文本质量下降的问题，尤其是在输入数据涉及异构主题时。

Method: 通过聚类输入数据选择推理批次，并利用中位数聚合下一个令牌的统计信息，以减少局部敏感性并提升隐私保护效果。

Result: 实验表明，该方法在代表性指标（如MAUVE）和下游任务性能上均有提升，能以更低的隐私成本生成高质量合成数据。

Conclusion: 提出的方法显著提升了差分隐私语言模型推理的效果，同时降低了隐私成本，优于现有最优方法。
...
### [71] [StatsMerging: Statistics-Guided Model Merging via Task-Specific Teacher Distillation](https://arxiv.org/abs/2506.04567)
*Ranjith Merugu,Bryan Bo Cao,Shubham Jain*

Main category: cs.LG

TLDR: StatsMerging是一种基于统计的轻量级模型合并方法，利用SVD奇异值和任务特定教师蒸馏，无需标签或测试样本，在多个任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决内存受限下多大型模型合并的问题，避免依赖标签或测试样本。

Method: 利用SVD奇异值捕获任务重要性，轻量级学习器建模权重分布，任务特定教师蒸馏处理异构架构。

Result: 在八项任务中表现优于现有技术，提升准确性、泛化能力和鲁棒性。

Conclusion: StatsMerging为模型合并提供了高效、通用的解决方案。
...
### [72] [Scaling Laws for Robust Comparison of Open Foundation Language-Vision Models and Datasets](https://arxiv.org/abs/2506.04598)
*Marianna Nezhurina,Tomer Porian,Giovanni Pucceti,Tommie Kerssies,Romain Beaumont,Mehdi Cherti,Jenia Jitsev*

Main category: cs.LG

TLDR: 论文探讨了如何利用扩展定律（scaling laws）比较模型和数据集，以优化预训练过程，并首次为CLIP和MaMMUT两种语言-视觉学习方法提供了全面的扩展定律分析。
...
Method: 方法包括密集测量模型和样本规模，推导扩展定律，并应用于CLIP和MaMMUT两种模型的比较，同时验证了恒定学习率计划的可行性。

Result: 结果显示MaMMUT在规模扩展和样本效率上优于CLIP，并在多个下游任务和数据集上表现一致。

Conclusion: 结论指出扩展定律的准确推导为模型和数据集的大规模比较提供了有效手段，推动了开放基础模型的系统性改进。
...
### [73] [Ignoring Directionality Leads to Compromised Graph Neural Network Explanations](https://arxiv.org/abs/2506.04608)
*Changsheng Sun,Xinke Li,Jin Song Dong*

Main category: cs.LG

TLDR: 方向性信息在GNN解释中的重要性被忽视，保留方向性语义可显著提高解释质量。


<details>
  <summary>Details</summary>
Motivation: 在关键领域中，GNN的解释需要可靠，但常见的图对称化方法丢弃了方向性信息，导致信息丢失和误导性解释。

Method: 通过理论和实证研究，分析方向性语义对解释质量的影响。

Result: 保留方向性语义显著提高了GNN的解释质量，为决策者提供更可靠的见解。
...
### [74] [Exploring bidirectional bounds for minimax-training of Energy-based models](https://arxiv.org/abs/2506.04609)
*Cong Geng,Jia Wang,Li Chen,Zhiyong Gao,Jes Frellsen,Søren Hauberg*

Main category: cs.LG

TLDR: 论文提出了一种通过双向边界（同时最大化下界和最小化上界）训练能量模型（EBMs）的方法，以解决传统下界训练的不稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 能量模型（EBMs）在密度估计中表现优雅，但训练困难。传统方法通过变分下界训练可能导致不稳定，因此需要更稳定的训练策略。

Method: 提出四种不同的对数似然边界（基于生成器雅可比矩阵的奇异值、互信息、梯度惩罚和扩散过程），并设计相应算法评估这些边界。

Result: 双向边界方法显著稳定了EBMs的训练，并实现了高质量的密度估计和样本生成。

Conclusion: 双向边界方法为EBMs提供了一种稳定且高效的训练框架，具有实际应用潜力。
...
### [75] [Composing Agents to Minimize Worst-case Risk](https://arxiv.org/abs/2506.04632)
*Guruprerana Shabadi,Rajeev Alur*

Main category: cs.LG

TLDR: 论文提出了一种基于有向无环图的智能体组合优化方法，旨在最大化任务成功率的同时最小化风险（如安全、公平和隐私）。


<details>
  <summary>Details</summary>
Motivation: 现代智能体系统需要分解复杂任务并选择多个AI智能体完成，但需兼顾任务成功率和风险最小化。

Method: 将智能体工作流建模为有向无环图（agent graph），提出一种高效算法，通过动态规划和联合边界近似计算尾部风险（value-at-risk），以找到最优智能体组合。

Result: 在强化学习智能体组合的基准测试中，算法能有效近似尾部风险并找到最优组合。

Conclusion: 该方法为智能体组合的风险优化提供了理论保证和实用工具。
...
### [76] [Inference economics of language models](https://arxiv.org/abs/2506.04645)
*Ege Erdil*

Main category: cs.LG

TLDR: 论文提出了一种理论模型，用于权衡大规模部署LLM推理时的每个令牌成本与串行令牌生成速度之间的经济关系。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决在大规模部署LLM推理时，如何在成本与速度之间找到最优平衡点。

Method: 方法包括建立一个理论模型，考虑算术、内存带宽、网络带宽和延迟约束，并通过优化并行设置和批量大小来找到给定成本下最优的串行推理速度。

Result: 结果包括计算了流行语言模型的串行速度与每个令牌成本的Pareto前沿。

Conclusion: 结论是该模型能够有效权衡成本与速度，为大规模LLM推理部署提供优化方案。
...
### [77] [Neural Network Reprogrammability: A Unified Theme on Model Reprogramming, Prompt Tuning, and Prompt Instruction](https://arxiv.org/abs/2506.04650)
*Zesheng Ye,Chengyi Cai,Ruijiang Dong,Jianzhong Qi,Lei Feng,Pin-Yu Chen,Feng Liu*

Main category: cs.LG

TLDR: 该论文提出神经网络可重编程性作为统一框架，连接模型重编程、提示调整和提示指令等主流模型适应技术，通过信息操作实现预训练模型的高效适应。


<details>
  <summary>Details</summary>
Motivation: 随着大规模预训练基础模型的规模和能力不断扩大，如何高效适应下游任务变得至关重要。现有方法缺乏统一理解，因此需要一种框架来整合这些技术。

Method: 提出神经网络可重编程性框架，通过信息操作（如输入扰动、中间层插入标记或上下文任务示例）重定向模型行为，并基于四个维度（格式、位置、操作符和输出对齐）对方法分类。

Result: 该框架适用于多种数据模态和模型架构，揭示了现有技术（如上下文学习和思维链提示）的理论联系和实际区别。

Conclusion: 神经网络可重编程性是高效模型适应的基本范式，未来研究可从这一整合视角出发，解决技术挑战和伦理问题。
...
### [78] [The Oversmoothing Fallacy: A Misguided Narrative in GNN Research](https://arxiv.org/abs/2506.04653)
*MoonJeong Park,Sunghyun Choi,Jaeseung Heo,Eunhyeok Park,Dongwoo Kim*

Main category: cs.LG

TLDR: 论文认为过度平滑对深度图神经网络（GNN）的影响被夸大，并提倡进一步探索深度GNN架构。研究发现，过度平滑与梯度消失混淆，而非聚合操作，且经典方法（如跳跃连接和归一化）可成功构建深度GNN。


<details>
  <summary>Details</summary>
Motivation: 探讨过度平滑是否是深度GNN性能受限的主要原因，并挑战其独特性。

Method: 分析GNN的三个核心操作（聚合、线性变换和非线性激活），区分过度平滑与梯度消失，并验证经典解决方案的有效性。

Result: 发现过度平滑与梯度消失混淆，经典方法可避免性能下降，支持深度GNN的可行性。

Conclusion: 澄清了对过度平滑的误解，揭示了深度GNN的潜力，鼓励进一步研究。
...
### [79] [Noise-Resistant Label Reconstruction Feature Selection for Partial Multi-Label Learning](https://arxiv.org/abs/2506.04669)
*Wanfu Gao,Hanlin Pan,Qingqi Han,Kunpeng Liu*

Main category: cs.LG

TLDR: 提出了一种针对部分多标签学习（PML）的特征选择方法，通过标签关系的抗噪性和标签连通性解决维度灾难和正标签识别问题。


<details>
  <summary>Details</summary>
Motivation: 维度灾难导致模型过拟合和分类性能下降，现有PML方法基于低秩假设，但实际中难以满足且可能丢失高维信息，且正标签识别能力不足。

Method: 利用标签关系的抗噪性消除标签歧义，通过改进的低秩假设设计学习过程，基于标签连通性找到代表性标签并重构权重矩阵以选择特征。

Result: 在基准数据集上的实验结果表明了所提方法的优越性。

Conclusion: 该方法有效解决了PML中的维度灾难和正标签识别问题，提升了模型性能。
...
### [80] [FedAPM: Federated Learning via ADMM with Partial Model Personalization](https://arxiv.org/abs/2506.04672)
*Shengkun Zhu,Feiteng Nie,Jinshan Zeng,Sheng Wang,Yuan Sun,Yuan Yao,Shangfeng Chen,Quanqing Xu,Chuanhui Yang*

Main category: cs.LG

TLDR: FedAPM是一个基于ADMM的联邦学习框架，通过引入一阶和二阶近端项缓解客户端漂移，提升模型收敛效率和准确性。
...
Conclusion: FedAPM通过稳定收敛和高效通信，有效解决客户端漂移问题，适用于异构多模态数据。
...
### [81] [The cost of ensembling: is it always worth combining?](https://arxiv.org/abs/2506.04677)
*Marco Zanotti*

Main category: cs.LG

TLDR: 研究表明，在时间序列预测中，集成学习能显著提升性能，但计算成本较高。通过调整重训练频率和小规模集成，可以在保持精度的同时降低成本。


<details>
  <summary>Details</summary>
Motivation: 随着数据集规模和预测模型复杂度的增加，预测精度与计算成本之间的权衡成为重要问题，尤其是在时间序列集成学习中。

Method: 评估了十个基础模型和八种集成配置，在两个大型零售数据集（M5和VN1）上，考虑不同重训练频率下的点预测和概率预测精度。

Result: 集成学习显著提升预测性能，尤其是概率预测，但计算成本较高。降低重训练频率可显著降低成本，且对点预测精度影响较小。小规模集成（2-3个模型）通常能达到接近最优的结果。

Conclusion: 通过精心设计集成和选择重训练策略，可以实现高精度、鲁棒且成本效益高的预测，适用于实际应用，支持可持续AI目标。
...
### [82] [Towards Better Generalization via Distributional Input Projection Network](https://arxiv.org/abs/2506.04690)
*Yifan Hao,Yanxin Lu,Xinwei Shen,Tong Zhang*

Main category: cs.LG

TLDR: DIPNet通过将输入投影到可学习的分布中，平滑损失景观，提升泛化性能。


<details>
  <summary>Details</summary>
Motivation: 过参数化模型下，训练损失难以反映泛化性能，而平滑性虽与泛化相关，但直接实现困难。

Method: 提出DIPNet框架，将输入逐层投影到可学习分布，诱导更平滑的损失景观。

Result: 理论分析显示DIPNet降低局部平滑度与Lipschitz常数；实验验证其在多种架构和任务中提升测试性能。
...
### [83] [Influence Functions for Edge Edits in Non-Convex Graph Neural Networks](https://arxiv.org/abs/2506.04694)
*Jaeseung Heo,Kyeongheung Yun,Seokwon Yoon,MoonJeong Park,Jungseul Ok,Dongwoo Kim*

Main category: cs.LG

TLDR: 该论文提出了一种针对图神经网络（GNNs）的近端Bregman响应函数，用于准确预测边删除和插入的影响，并考虑了消息传播效应。
...
Result: 实验证明该方法能准确预测GNN中边修改的影响，并适用于图重连和对抗攻击等应用。

Conclusion: 该方法为GNN的影响预测提供了更灵活和准确的工具，扩展了其应用范围。
...
### [84] [On the Mechanism of Reasoning Pattern Selection in Reinforcement Learning for Language Models](https://arxiv.org/abs/2506.04695)
*Xingwu Chen,Tianle Li,Difan Zou*

Main category: cs.LG

TLDR: RLVR通过优化现有推理模式的选择提升模型能力，理论分析揭示了其收敛动态，并表明SFT可加速弱模型的优化。
...
### [85] [Enhanced Drought Analysis in Bangladesh: A Machine Learning Approach for Severity Classification Using Satellite Data](https://arxiv.org/abs/2506.04696)
*Tonmoy Paul,Mrittika Devi Mati,Md. Mahmudul Islam*

Main category: cs.LG

TLDR: 提出了一种基于卫星数据和机器学习的框架，用于分类孟加拉国38个地区的干旱严重程度，填补了传统干旱指数和现有机器学习模型的不足。


<details>
  <summary>Details</summary>
Motivation: 孟加拉国因地理和人为因素易受干旱影响，传统干旱指数（如SPI和PDSI）忽略土壤湿度和温度等关键因素，且机器学习模型在该地区的应用不足。

Method: 结合K-means和贝叶斯高斯混合等无监督算法进行聚类，再使用KNN、随机森林、决策树和朴素贝叶斯等分类模型，整合2012-2024年的天气数据（湿度、土壤湿度、温度）。

Result: 成功将干旱严重程度分类为不同级别，但发现各地区干旱脆弱性存在显著差异。
...
### [86] [Explicit Density Approximation for Neural Implicit Samplers Using a Bernstein-Based Convex Divergence](https://arxiv.org/abs/2506.04700)
*José Manuel de Frutos,Manuel A. Vázquez,Pablo M. Olmos,Joaquín Míguez*

Main category: cs.LG

TLDR: 提出了一种新的无似然目标dual-ISL，用于训练隐式生成模型，通过交换目标分布和模型分布的角色，形成凸优化问题。证明了其连续性和凸性，并提供了理论框架和实验验证。


<details>
  <summary>Details</summary>
Motivation: 传统统计度量（如ISL）在训练隐式生成模型时存在局限性，需要一种更稳健且高效的方法。

Method: 引入dual-ISL目标，交换目标与模型分布角色，形成凸优化问题；理论分析其连续性和凸性，并扩展到多元设置。

Result: dual-ISL在实验中表现出更快的收敛速度、更稳定的训练效果，并能有效防止模式崩溃。

Conclusion: dual-ISL是一种高效且稳健的训练隐式生成模型的方法，具有理论和实践优势。
...
### [87] [UNO: Unlearning via Orthogonalization in Generative models](https://arxiv.org/abs/2506.04712)
*Pinak Mandal,Georg A. Gottwald*

Main category: cs.LG

TLDR: 论文提出了一种基于损失梯度正交化的快速遗忘算法，旨在高效移除特定数据的影响，同时保持模型生成质量。


<details>
  <summary>Details</summary>
Motivation: 随着生成模型的广泛应用，选择性遗忘特定数据（如隐私、法律或有害内容）的需求日益重要，传统重新训练方法成本高昂。

Method: 采用损失梯度正交化技术，设计快速遗忘算法，确保遗忘目标数据的同时保留模型性能。

Result: 在MNIST和CelebA数据上验证，算法比梯度手术等方法快数个数量级，且保持模型原始保真度。

Conclusion: 该算法为高效、可靠的数据遗忘提供了可行方案，满足遗忘、生成质量保持和高效训练的需求。
...
### [88] [Multi-Layer GRPO: Enhancing Reasoning and Self-Correction in Large Language Models](https://arxiv.org/abs/2506.04746)
*Fei Ding,Baiqiao Wang,Zijian Zeng,Youwei Wang*

Main category: cs.LG

TLDR: MGRPO（多层GRPO）通过引入两层GRPO机制，解决了标准GRPO在复杂推理任务中因中间监督缺失导致的探索效率低和训练不稳定的问题。


<details>
  <summary>Details</summary>
Motivation: 标准GRPO在增强大型语言模型推理能力方面表现良好，但由于缺乏中间监督，容易因单一错误导致整个推理链失效，影响训练稳定性。

Method: MGRPO采用两层GRPO结构：第一层生成初始响应，第二层针对初始响应进行错误识别和修正，形成自校正循环。

Result: 实验表明，MGRPO在数学推理任务中显著优于标准GRPO，提升了推理和自校正能力。

Conclusion: MGRPO通过自校正机制提供隐式过程级监督，无需密集标注的奖励模型，显著提升了训练效率和性能。
...
### [89] [Log-Linear Attention](https://arxiv.org/abs/2506.04761)
*Han Guo,Songlin Yang,Tarushii Goel,Eric P. Xing,Tri Dao,Yoon Kim*

Main category: cs.LG

TLDR: 提出了一种名为对数线性注意力的机制，平衡了线性注意力的效率和softmax注意力的表达能力，解决了传统RNN固定大小隐藏状态的限制。


<details>
  <summary>Details</summary>
Motivation: Transformer的注意力机制存在二次计算和线性内存复杂度的瓶颈，而线性注意力和状态空间模型虽然高效，但受限于固定大小的隐藏状态。

Method: 开发了对数线性注意力，用对数增长的隐藏状态集替代固定大小隐藏状态，并展示了其并行计算的可行性。

Result: 对数线性注意力在Mamba-2和Gated DeltaNet等架构中表现良好，优于线性时间变体。

Conclusion: 对数线性注意力是一种高效且表达力强的注意力机制，适用于现有线性注意力变体。
...
### [90] [OpenGT: A Comprehensive Benchmark For Graph Transformers](https://arxiv.org/abs/2506.04765)
*Jiachen Tang,Zhonghao Wang,Sirui Chen,Sheng Zhou,Jiawei Chen,Jiajun Bu*

Main category: cs.LG

TLDR: 论文介绍了OpenGT，一个用于图变换器（GTs）的综合基准测试库，旨在填补现有研究的空白，促进GTs的公平比较和多维分析。


<details>
  <summary>Details</summary>
Motivation: 图变换器（GTs）在多个领域表现出色，但其适用场景和设计选择的有效性尚不明确，缺乏统一的基准测试库。

Method: 通过OpenGT库，论文建立了标准化的实验设置，并整合了多种先进的GNNs和GTs，从多角度评估GTs的性能。

Result: 实验揭示了模型跨任务迁移的困难、局部注意力的局限性、效率权衡、特定位置编码的应用场景及其预处理开销等关键发现。

Conclusion: OpenGT为未来图变换器研究提供了公平、可重复和可推广的基础，并提供了易于使用的训练和评估库。
...
### [91] [Improved Regret Bounds for Linear Bandits with Heavy-Tailed Rewards](https://arxiv.org/abs/2506.04775)
*Artin Tajdini,Jonathan Scarlett,Kevin Jamieson*

Main category: cs.LG

TLDR: 论文研究了具有重尾奖励的随机线性赌博机问题，改进了现有上下界，提出了一种新的基于实验设计的消除算法，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 研究重尾奖励下的线性赌博机问题，改进现有上下界，填补理论与实际应用的差距。

Method: 提出基于实验设计的消除算法，优化对维度d的依赖，适用于不同几何结构。

Result: 算法实现了更优的后悔上界，并建立了更紧的下界，验证了问题的难度。

Conclusion: 新算法在理论和实际中均表现优异，为高维和无限维问题提供了解决方案。
...
### [92] [Kernel $k$-Medoids as General Vector Quantization](https://arxiv.org/abs/2506.04786)
*Thore Gerlach,Sascha Mücke,Christian Bauckhage*

Main category: cs.LG

TLDR: 论文通过QUBO框架揭示了k-medoids和KDE两种VQ方法的深层联系，表明KDE-QUBO是k-medoids-QUBO的特例。
...
Method: 使用QUBO框架比较k-medoids和KDE的启发式和原理性公式，分析其关系。
...
### [93] [Adaptive Preconditioners Trigger Loss Spikes in Adam](https://arxiv.org/abs/2506.04805)
*Zhiwei Bai,Zhangchen Zhou,Jiajie Zhao,Xiaolong Li,Zhiyu Li,Feiyu Xiong,Hongkang Yang,Yaoyu Zhang,Zhi-Qin John Xu*

Main category: cs.LG

TLDR: 研究发现Adam优化器在训练神经网络时会导致损失值突增（loss spikes），原因是其自适应预条件器在特定条件下触发不稳定机制。


<details>
  <summary>Details</summary>
Motivation: 探讨Adam优化器在训练中引发损失突增的机制，不同于以往认为损失景观特性是主要原因的观点。

Method: 通过分析Adam优化器的预条件器行为，识别出平方梯度远小于二阶矩估计时的临界状态，导致不稳定。

Result: 实验验证了该机制在多种网络架构（全连接、卷积、Transformer）中的普遍性。

Conclusion: Adam的自适应预条件器设计是损失突增的根本原因，需注意优化器的稳定性问题。
...
### [94] [LogicPuzzleRL: Cultivating Robust Mathematical Reasoning in LLMs via Reinforcement Learning](https://arxiv.org/abs/2506.04821)
*Zhen Hao Wong,Jingwen Deng,Runming He,Zirong Chen,Qijie You,Hejun Dong,Hao Liang,Chengyu Shen,Bin Cui,Wentao Zhang*

Main category: cs.LG

TLDR: 论文提出了一种通过强化学习在逻辑谜题上微调大语言模型（LLMs）的方法，以提升其结构化推理能力。
...
Method: 采用强化学习框架，在七个定制逻辑谜题上微调LLMs，通过二进制反馈鼓励假设驱动的推理。

Result: 该方法显著提升了LLMs在数学基准测试中的表现，尤其是在需要多步推理的中等难度问题上。

Conclusion: 逻辑谜题训练重塑了LLMs的内部推理机制，实现了更鲁棒和组合式的泛化能力。
...
### [95] [From EHRs to Patient Pathways: Scalable Modeling of Longitudinal Health Trajectories with LLMs](https://arxiv.org/abs/2506.04831)
*Chantal Pellegrini,Ege Özsoy,David Bani-Harouni,Matthias Keicher,Nassir Navab*

Main category: cs.LG

TLDR: 提出了一种名为EHR2Path的新方法，通过将多样化的电子健康记录（EHR）数据转化为结构化表示，设计了一种全面的患者路径预测模型，显著提升了预测性能。
...
Method: 将EHR数据转化为结构化表示，并设计EHR2Path模型，引入一种新颖的摘要机制，嵌入长期时间上下文到特定主题的摘要标记中。

Result: EHR2Path在下一步预测和纵向模拟中表现优异，优于基线模型，支持多种评估任务。

Conclusion: EHR2Path为预测性和个性化医疗提供了新路径，能够详细模拟患者轨迹。
...
### [96] [Sparse Autoencoders, Again?](https://arxiv.org/abs/2506.04859)
*Yin Lu,Tong He,Xuening Zhu,David Wipf*

Main category: cs.LG

TLDR: 本文提出了一种混合模型，解决了稀疏自编码器（SAEs）和变分自编码器（VAEs）的局限性，并在理论和实验上验证了其优越性。


<details>
  <summary>Details</summary>
Motivation: 尽管SAEs和VAEs在建模低维潜在结构方面广泛应用，但其方法几十年来变化不大，且存在未被充分认识的弱点。

Method: 提出了一种混合替代模型，结合了SAEs和VAEs的优点，并提供了理论证明和实验验证。

Result: 新模型在合成和真实数据集上表现优异，能够准确估计潜在流形维度并生成更稀疏的表示，同时不牺牲重构误差。

Conclusion: 该混合模型在图像和语言模型激活模式等领域超越了传统SAEs、VAEs及扩散模型。
...
### [97] [Aligning Multimodal Representations through an Information Bottleneck](https://arxiv.org/abs/2506.04870)
*Antonio Almudévar,José Miguel Hernández-Lobato,Sameer Khurana,Ricard Marxer,Alfonso Ortega*

Main category: cs.LG

TLDR: 对比损失在多模态表示学习中广泛使用，但存在模态特定信息问题，导致对齐效果不佳。本文提出一种正则化方法以提升表示对齐。


<details>
  <summary>Details</summary>
Motivation: 现有对比损失无法有效去除模态特定信息，导致多模态表示空间对齐效果不理想。

Method: 通过信息瓶颈原理分析问题，提出基于变分近似的正则化项，优化损失函数。
...
### [98] [There Was Never a Bottleneck in Concept Bottleneck Models](https://arxiv.org/abs/2506.04877)
*Antonio Almudévar,José Miguel Hernández-Lobato,Alfonso Ortega*

Main category: cs.LG

TLDR: 论文提出了一种改进的概念瓶颈模型（MCBMs），通过信息瓶颈（IB）目标约束表示组件仅保留与对应概念相关的信息，从而提升可解释性和干预有效性。


<details>
  <summary>Details</summary>
Motivation: 传统概念瓶颈模型（CBMs）未能真正约束表示组件仅编码特定概念信息，导致可解释性和干预有效性不足。

Method: 提出最小概念瓶颈模型（MCBMs），引入信息瓶颈目标，通过变分正则化项约束表示组件。

Result: MCBMs支持具有理论保证的概念级干预，符合贝叶斯原则，并提供了更大的设计灵活性。

Conclusion: MCBMs通过信息瓶颈目标有效解决了传统CBMs的局限性，提升了模型的可解释性和干预能力。
...
### [99] [Gaussian Process Diffeomorphic Statistical Shape Modelling Outperforms Angle-Based Methods for Assessment of Hip Dysplasia](https://arxiv.org/abs/2506.04886)
*Allen Paul,George Grammatopoulos,Adwaye Rambojun,Neill D. F. Campbell,Harinderjit S. Gill,Tony Shardlow*

Main category: cs.LG

TLDR: 论文提出了一种基于CT扫描的半自动化分类方法（GPDSSM），用于诊断髋关节发育不良，相比传统角度测量方法更准确且节省时间。


<details>
  <summary>Details</summary>
Motivation: 早期诊断髋关节发育不良对预防骨关节炎至关重要，但传统方法依赖手动测量和2D扫描，效率低且准确性有限。

Method: 结合高斯过程隐变量模型和微分同胚，构建统计形状模型（GPDSSM），利用192例CT扫描数据（100训练，92测试）进行分类。

Result: GPDSSM分类准确率（AUC 96.2%）优于角度测量方法（91.2%），并能识别发育不良的具体区域。

Conclusion: GPDSSM为髋关节发育不良的诊断提供了更高效、准确的工具，有望减少临床工作量。
...
### [100] [Dissecting Long Reasoning Models: An Empirical Study](https://arxiv.org/abs/2506.04913)
*Yongyu Mu,Jiali Zeng,Bei Li,Xinyan Guan,Fandong Meng,Jie Zhou,Tong Xiao,Jingbo Zhu*

Main category: cs.LG

TLDR: 本文分析了强化学习中正负样本的作用，发现负样本对泛化和鲁棒性更重要；提出了解决数据效率低下的策略；探讨了推理模型性能不稳定的原因及解决方案。


<details>
  <summary>Details</summary>
Motivation: 研究强化学习中正负样本的作用、数据效率低下及推理模型性能不稳定的问题。

Method: 系统分析正负样本作用；提出相对长度奖励和离线样本注入策略；通过多次评估解决性能不稳定问题。

Result: 负样本显著提升泛化能力；新策略提高数据效率；多次评估缓解性能不稳定。

Conclusion: 负样本和高效数据利用对强化学习至关重要，多次评估可提升稳定性。
...
### [101] [Predicting ICU In-Hospital Mortality Using Adaptive Transformer Layer Fusion](https://arxiv.org/abs/2506.04924)
...
TLDR: ALFIA是一种基于注意力机制的模块化架构，用于早期识别ICU高风险患者，通过融合多层语义特征并联合训练LoRA适配器，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 早期识别ICU高风险患者对医疗资源分配至关重要，ALFIA旨在提供一种高效且鲁棒的工具。

Method: ALFIA采用注意力机制和自适应层加权，结合BERT主干网络和LoRA适配器，训练于cw-24基准。

Result: ALFIA在AUPRC上超越现有表格分类器，并能与GBDT和深度神经网络结合进一步提升性能。

Conclusion: ALFIA通过直接处理临床文本，为重症监护提供了便捷且强大的风险分层工具。
...
### [102] [Agentic AI for Intent-Based Industrial Automation](https://arxiv.org/abs/2506.04980)
*Marcos Lima Romero,Ricardo Suyama*

Main category: cs.LG

TLDR: 论文提出了一种将Agentic AI与意图驱动范式结合的概念框架，旨在简化人机交互并支持工业5.0的可持续和弹性原则。


<details>
  <summary>Details</summary>
Motivation: 工业4.0的复杂性增加了人机交互的难度，而工业5.0强调人本、可持续和弹性。通过结合Agentic AI和意图驱动范式，可以更好地实现这些目标。

Method: 提出一个概念框架，将意图分解为可操作的组件，由配备专业工具的子代理执行任务。基于CMAPSS数据集和Google ADK实现概念验证。

Result: 验证了意图分解、代理编排和自主决策在预测性维护场景中的可行性，表明该方法可降低技术障碍并支持可扩展的自动化。

Conclusion: 该方法具有潜力，尽管存在数据质量和可解释性问题，但为意图驱动的自动化提供了可行路径。
...
### [103] [FPTQuant: Function-Preserving Transforms for LLM Quantization](https://arxiv.org/abs/2506.04985)
*Boris van Breugel,Yelysei Bondarenko,Paul Whatmough,Markus Nagel*

Main category: cs.LG

TLDR: FPTQuant提出四种轻量级函数保留变换（FPTs），用于优化大语言模型（LLMs）的量化，减少性能损失，提升效率。
...
Method: 引入四种FPTs：预RoPE变换、值变换、MLP块内缩放变换和动态缩放变换，利用变换器的等价性和独立性优化激活分布。

Result: FPTQuant实现静态INT4量化，速度提升3.9倍，精度与多数现有方法相当或更优。

Conclusion: FPTQuant在精度与速度间取得优异平衡，无需定制内核，推理开销极低。
...
### [104] [Cautious Optimism: A Meta-Algorithm for Near-Constant Regret in General Games](https://arxiv.org/abs/2506.05005)
*Ashkan Soleymani,Georgios Piliouras,Gabriele Farina*

Main category: cs.LG

TLDR: 论文提出了一种名为“谨慎乐观”（Cautious Optimism）的框架，通过自适应调整学习速度，显著加速了正则化学习，并在一般游戏中实现了近乎最优的遗憾最小化。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如OMWU）在动态非单调学习中表现优异，但加速无遗憾学习是否具有普适性尚不明确。本文旨在证明自适应学习速度的加速效果并非孤立现象。

Method: 提出了Cautious Optimism框架，基于FTRL实例，通过最小计算开销调整学习速度，生成加速的无遗憾学习算法。

Result: 在自对弈中实现了近乎最优的O_T(log T)遗憾，同时在对抗场景中保持了最优的O(√T)遗憾。

Conclusion: Cautious Optimism为一般游戏中的快速学习提供了新途径，无需依赖单调步长，且保持了无耦合性。
...
TLDR: CREAM是一种新型概念瓶颈模型，通过显式编码概念间关系和使用正则化侧通道，实现高性能任务表现和高概念重要性。


<details>
  <summary>Details</summary>
Motivation: 提出CREAM是为了解决传统概念瓶颈模型在概念间关系处理和任务性能平衡上的不足。

Method: CREAM通过嵌入专家定义的概念间关系，并利用正则化侧通道来优化任务表现，同时切断不必要的信息流。

Result: 实验表明，CREAM在保持任务性能的同时主要依赖概念，且嵌入的关系简化了模型干预并减少了概念泄漏。

Conclusion: CREAM在概念推理和任务性能之间取得了平衡，为可解释AI提供了新思路。
...
### [106] [Multi-Point Proximity Encoding For Vector-Mode Geospatial Machine Learning](https://arxiv.org/abs/2506.05016)
*John Collins*

Main category: cs.LG

TLDR: 本文提出了一种基于参考点距离的多点邻近（MPP）编码方法，用于将矢量地理空间数据编码为适合机器学习的向量形式。
...
Method: MPP编码通过计算形状与区域内一组参考点的缩放距离来表示形状，适用于任何类型的几何形状。

Result: MPP编码具有形状中心性和连续性，能区分空间对象的几何特征，并高精度捕捉空间关系，性能优于栅格化方法。

Conclusion: MPP编码是一种高效且通用的矢量地理空间数据编码方法，优于现有替代方案。
...
### [107] [Tuning the Right Foundation Models is What you Need for Partial Label Learning](https://arxiv.org/abs/2506.05027)
*Kuang He,Wei Tang,Tong Wei,Min-Ling Zhang*

Main category: cs.LG

TLDR: 论文提出PartialCLIP框架，评估了11种基础模型在部分标签学习中的表现，发现现有方法在基础模型上表现相似且稳定，但对模型选择和适应策略敏感。


<details>
  <summary>Details</summary>
Motivation: 部分标签学习（PLL）在现实应用中常见，但现有方法对基础模型的关注不足，需探索其潜力。

Method: 提出PartialCLIP框架，评估11种基础模型在13种PLL方法和8个基准数据集上的表现，并引入文本嵌入分类器初始化和零样本CLIP标签过滤。

Result: 现有PLL方法在基础模型上表现相似且稳定，但对模型选择和适应策略敏感；PartialCLIP展示了高效性。

Conclusion: 研究揭示了当前PLL方法的局限性，并为开发更具泛化性的模型提供了见解。
...
### [108] [TIMING: Temporality-Aware Integrated Gradients for Time Series Explanation](https://arxiv.org/abs/2506.05035)
*Hyeongwon Jang,Changhun Kim,Eunho Yang*

Main category: cs.LG

TLDR: 论文提出TIMING方法，通过增强IG的时间感知能力，改进时间序列XAI中关键点的识别效果。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列XAI方法仅关注点级归因幅度，忽略方向性影响，导致关键点识别不足。

Method: 提出TIMING方法，结合时间感知改进IG，避免引入分布外样本。

Result: TIMING在合成和真实时间序列基准测试中优于现有方法。

Conclusion: TIMING通过时间感知改进IG，显著提升时间序列XAI效果。
...
### [109] [iN2V: Bringing Transductive Node Embeddings to Inductive Graphs](https://arxiv.org/abs/2506.05039)
*Nicolas Lell,Ansgar Scherp*

Main category: cs.LG

TLDR: iN2V是一种改进的节点嵌入方法，将传统的转导式node2vec扩展到归纳式设置，通过后处理计算新节点的嵌入，并在多个数据集上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 传统的node2vec方法仅适用于转导式设置，无法处理训练时未见的节点，限制了其应用范围。

Method: 提出iN2V，结合后处理计算新节点嵌入，并修改原始N2V训练过程以适应后处理。

Result: 实验表明iN2V在归纳式设置中有效，平均提升节点分类1分，最高提升6分。

Conclusion: iN2V是一种灵活的插件方法，可扩展或增强现有嵌入，适用于归纳式节点表示学习。
...
### [110] [Reliably detecting model failures in deployment without labels](https://arxiv.org/abs/2506.05047)
*Viet Nguyen Changjian Shui,Vijay Giri,Siddarth Arya,Amol Verma,Fahad Razak,Rahul G. Krishnan*

Main category: cs.LG

TLDR: 该论文提出了一种名为D3M的监控算法，用于检测模型部署后的性能退化（PDD），通过预测模型的分歧实现高效监控。


<details>
  <summary>Details</summary>
Motivation: 数据分布随时间变化，模型需要重新训练，但如何在不依赖标签的情况下判断何时重新训练是一个挑战。

Method: 基于预测模型的分歧设计D3M算法，在非退化偏移下保持低误报率，在退化偏移下提供高真报率。

Result: 在标准基准和真实世界数据集上验证了D3M的有效性，证明其可作为高风险机器学习管道的预警机制。

Conclusion: D3M是一种实用高效的PDD监控方法，适用于动态环境中的模型维护。
...
### [111] [NIMO: a Nonlinear Interpretable MOdel](https://arxiv.org/abs/2506.05059)
*Shijian Xu,Marcello Massimo Negri,Volker Roth*

Main category: cs.LG

TLDR: NIMO模型结合神经网络和线性模型的优点，通过非线性修正提升预测准确性，同时保持线性系数的可解释性。
...
Method: NIMO通过神经网络学习线性模型预测的非线性修正，利用基于轮廓似然的优化算法和自适应岭回归实现稀疏约束。

Result: 实验表明，NIMO能恢复线性系数并显著提升预测准确性，同时在回归和分类任务中表现优于其他混合方法。

Conclusion: NIMO成功结合了神经网络的高性能和线性模型的可解释性，是唯一能完全保持线性系数解释性的混合方法。
...
### [112] [UnHiPPO: Uncertainty-aware Initialization for State Space Models](https://arxiv.org/abs/2506.05065)
*Marten Lienen,Abdullah Saydemir,Stephan Günnemann*

Main category: cs.LG

TLDR: 论文扩展了HiPPO理论，提出了一种考虑测量噪声的状态空间模型初始化方法，提高了模型对噪声的鲁棒性。
...
Result: 实验表明，该方法在训练和推理时均提高了模型对噪声的抵抗能力。

Conclusion: 提出的方法有效解决了HiPPO在噪声数据下的局限性，且不增加运行时开销。
...
### [113] [Semi-Implicit Variational Inference via Kernelized Path Gradient Descent](https://arxiv.org/abs/2506.05088)
*Tobias Pielok,Bernd Bischl,David Rügamer*

Main category: cs.LG

TLDR: 提出了一种基于核化的KL散度估计器，通过非参数平滑稳定训练，并引入重要性采样校正以减少偏差。该方法在性能和训练效率上优于或匹配现有SIVI方法。


<details>
  <summary>Details</summary>
Motivation: 半隐式变分推断（SIVI）在高维设置中因KL散度的高方差和偏差而训练困难，现有方法（如KSIVI）训练成本较高。

Method: 提出核化KL散度估计器，结合非参数平滑和重要性采样校正，理论联系Stein变分梯度下降。

Result: 方法在梯度方差和偏差上表现更优，优化更稳定高效。

Conclusion: 新方法在性能和效率上优于或匹配现有SIVI方法，具有理论和实证优势。
...
### [114] [Federated Isolation Forest for Efficient Anomaly Detection on Edge IoT Systems](https://arxiv.org/abs/2506.05138)
*Pavle Vasiljevic,Milica Matic,Miroslav Popovic*

Main category: cs.LG

TLDR: 本文提出了一种基于隔离森林的温度异常检测方法，适用于小型边缘设备和物联网系统，实验结果显示其准确率超过96%，内存占用低于160KB。


<details>
  <summary>Details</summary>
Motivation: 解决边缘设备和物联网系统中用户隐私和效率问题，同时满足低资源需求。

Method: 使用隔离森林算法（FLiForest）和联邦学习框架（Python TestBed和MicroPython TestBed）进行温度异常检测。

Result: 准确率超过96%，异常检测精度达78%以上，内存占用低于160KB。

Conclusion: 该方法适合资源受限环境，同时遵循联邦学习的隐私和协作原则。
...
### [115] [Associative Memory and Generative Diffusion in the Zero-noise Limit](https://arxiv.org/abs/2506.05178)
*Joshua Hess,Quaid Morris*

Main category: cs.LG

TLDR: 研究了生成扩散模型与连续状态联想记忆模型之间的联系，强调了Morse-Smale动力系统作为梯度联想记忆模型的通用逼近器，以及扩散模型作为其白噪声扰动系统。


<details>
  <summary>Details</summary>
Motivation: 探索生成扩散模型与联想记忆模型之间的理论联系，揭示噪声水平变化时从生成到记忆的通用过渡特性。

Method: 利用Morse-Smale动力系统作为理论框架，分析梯度联想记忆模型和扩散模型的稳定性与结构特性。

Result: 表明在噪声趋近于零时，扩散模型具有稳定性，且联想记忆学习景观和生成景观在零噪声极限下仅在某些孤立点不稳定。

Conclusion: 通过分岔序列描述景观特性，并为经典和现代Hopfield网络（即注意力机制）提供了结构稳定性准则。
...
### [116] [TreeRPO: Tree Relative Policy Optimization](https://arxiv.org/abs/2506.05183)
*Zhicheng Yang,Zhijiang Guo,Yinya Huang,Xiaodan Liang,Yiwei Wang,Jing Tang*

Main category: cs.LG

TLDR: 论文提出了一种名为TreeRPO的新方法，通过树采样估计推理步骤的奖励期望，显著提升了LLMs的性能。
...
Method: TreeRPO利用树采样直接估计推理步骤的奖励，并结合GRPO的组相对奖励机制，生成细粒度的奖励信号。

Result: 实验表明，TreeRPO将Qwen-2.5-Math的Pass@1准确率从19.0%提升至35.5%，同时响应长度减少18.1%。
...
### [117] [Locality Preserving Markovian Transition for Instance Retrieval](https://arxiv.org/abs/2506.05196)
*Jifei Luo,Wenzheng Wu,Hantao Yao,Lu Yu,Changsheng Xu*

Main category: cs.LG

TLDR: 论文提出了一种名为LPMT的框架，通过双向协作扩散和热力学马尔可夫转移过程，解决了扩散重排序中信号衰减的问题，提升了全局检索效果。


<details>
  <summary>Details</summary>
Motivation: 扩散重排序方法在数据流形建模中有效，但信号在远离源点时容易衰减，导致局部区域外的区分能力下降。

Method: LPMT框架结合双向协作扩散（BCD）和热力学马尔可夫转移（TMT）过程，通过局部状态嵌入（LSE）增强局部一致性。

Result: 实验结果表明，LPMT在多种任务中有效提升了实例检索的性能。

Conclusion: LPMT通过保留局部性和全局检索的平衡，显著提升了扩散重排序的效果。
...
### [118] [Transformers Meet In-Context Learning: A Universal Approximation Theory](https://arxiv.org/abs/2506.05200)
*Gen Li,Yuchen Jiao,Yu Huang,Yuting Wei,Yuxin Chen*

Main category: cs.LG

TLDR: 论文提出了一种基于通用函数逼近理论的方法，解释了Transformer如何通过上下文学习完成任务，无需微调或参数更新。


<details>
  <summary>Details</summary>
Motivation: 理解Transformer如何通过上下文学习实现任务泛化，而不依赖于优化算法的近似。

Method: 构建一个Transformer模型，利用通用函数逼近理论，仅通过少量上下文示例即可完成任务预测。

Result: 证明了Transformer可以在不更新权重的情况下，通过上下文示例实现可靠预测，适用于非凸问题和线性函数类。

Conclusion: Transformer能够同时学习通用表示并动态适应上下文示例，为上下文学习提供了理论支持。
...
### [119] [Mitigating Degree Bias Adaptively with Hard-to-Learn Nodes in Graph Contrastive Learning](https://arxiv.org/abs/2506.05214)
*Jingyu Hu,Hongbo Bo,Jun Hong,Xiaowei Liu,Weiru Liu*

Main category: cs.LG

TLDR: 论文提出了一种名为HAR的对比损失方法，通过增加正样本对和自适应加权来缓解GNN中的度偏差问题，并开发了SHARP实验框架验证其有效性。


<details>
  <summary>Details</summary>
Motivation: GNN在节点分类任务中存在度偏差问题，现有基于GCL的方法因正样本对有限和权重分配不均导致低度节点信息不足。
...
### [120] [Learning Theory of Decentralized Robust Kernel-Based Learning Algorithm](https://arxiv.org/abs/2506.05215)
*Zhan Yu*
...
### [121] [A Unified Framework for Provably Efficient Algorithms to Estimate Shapley Values](https://arxiv.org/abs/2506.05216)
...
TLDR: 论文提出了一种统一框架，涵盖KernelSHAP及相关估计器，并证明了其非渐近理论保证，首次为KernelSHAP提供理论支持，同时在实验中验证了其优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 计算精确的Shapley值在特征维度上具有指数复杂度，现有随机估计器（如KernelSHAP）缺乏理论保证，因此需要一种统一框架来填补这一空白。

Method: 提出一个统一框架，涵盖KernelSHAP及相关估计器，采用有/无替换采样策略，并证明其非渐近理论保证。

Result: 在中小维度数据集上验证了方法的低均方误差，并在高维数据集（如MNIST和CIFAR10）上实现了可扩展性和优于KernelSHAP库的性能。

Conclusion: 该框架首次为KernelSHAP提供理论保证，揭示了现有估计器的权衡，并通过实验验证了其优越性和可扩展性。
...
### [122] [Diagonal Batching Unlocks Parallelism in Recurrent Memory Transformers for Long Contexts](https://arxiv.org/abs/2506.05229)
*Danil Sivtsov,Ivan Rodkin,Gleb Kuzmin,Yuri Kuratov,Ivan Oseledets*

Main category: cs.LG

TLDR: Diagonal Batching 是一种调度方案，解决了 RMTs 的并行性问题，显著提升了长上下文推理的效率。


<details>
  <summary>Details</summary>
Motivation: Transformer 模型在长上下文推理中存在时间和内存复杂度高的问题，RMTs 虽然降低了复杂度，但因其内存更新机制导致顺序执行，成为性能瓶颈。

Method: 提出了 Diagonal Batching 调度方案，通过重新排序运行时计算，实现跨段并行化，同时保持精确的递归。

Result: 在 LLaMA-1B ARMT 模型上，Diagonal Batching 实现了 3.3 倍的速度提升，并减少了推理成本和延迟。

Conclusion: Diagonal Batching 增强了 RMTs 在实际长上下文应用中的实用性。
...
### [123] [Progressive Tempering Sampler with Diffusion](https://arxiv.org/abs/2506.05231)
*Severi Rissanen,RuiKang OuYang,Jiajun He,Wenlin Chen,Markus Heinonen,Arno Solin,José Miguel Hernández-Lobato*

Main category: cs.LG

TLDR: 论文提出了一种结合并行退火（PT）和扩散模型的新方法PTSD，用于改进神经采样器的训练效率和样本质量。


<details>
  <summary>Details</summary>
Motivation: 现有神经采样器在目标评估效率上不如并行退火（PT），且PT生成的样本依赖性强且计算成本高。

Method: PTSD通过在不同温度下顺序训练扩散模型，结合PT的优势，并引入新方法将高温扩散模型生成的样本用于训练低温模型。

Result: PTSD显著提高了目标评估效率，优于基于扩散的神经采样器。

Conclusion: PTSD结合了PT和扩散模型的优势，生成高质量且独立的样本，同时降低了计算成本。
...
TLDR: 论文提出了一种基于在线学习规则的稳定、可并行化的Mesa层，通过优化上下文回归目标提升语言模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前序列建模主要依赖Transformer，但其推理时内存和计算成本线性增长。近期工作通过线性化softmax操作实现了恒定成本的RNN模型，本文旨在进一步优化此类模型。

Method: 引入数值稳定的、可并行化的Mesa层，通过快速共轭梯度求解器在每个时间点优化上下文损失。

Result: 实验表明，该方法在语言建模和长上下文任务中表现优于现有RNN模型，但推理时计算成本增加。

Conclusion: 通过增加推理时计算解决序列优化问题，可显著提升模型性能，与近期增加测试时计算以提升性能的趋势一致。
...
### [125] [Evaluating Sparse Autoencoders: From Shallow Design to Matching Pursuit](https://arxiv.org/abs/2506.05239)
*Valérie Costa,Thomas Fel,Ekdeep Singh Lubana,Bahareh Tolooshams,Demba Ba*

Main category: cs.LG

TLDR: 稀疏自编码器（SAE）在解释性研究中被广泛应用，但现有浅层架构依赖准正交性假设，限制了提取相关特征的能力。本文提出多迭代SAE（MP-SAE），通过残差引导提取相关特征，并在手写数字生成等分层场景中验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 评估稀疏自编码器在可控环境中的表现，发现现有方法因准正交性假设无法有效提取相关特征，需改进以应对复杂场景。

Method: 提出多迭代稀疏自编码器（MP-SAE），通过展开匹配追踪算法，利用残差引导提取相关特征，并保证重构效果的单调提升。

Result: MP-SAE能够有效提取相关特征，适用于手写数字生成等分层任务，且重构效果随原子选择增加而单调提升。

Conclusion: MP-SAE突破了现有稀疏自编码器的限制，为提取复杂相关特征提供了新方法，适用于更广泛的解释性任务。
...
### [126] [Aligning Latent Spaces with Flow Priors](https://arxiv.org/abs/2506.05240)
*Yizhuo Li,Yuying Ge,Yixiao Ge,Ying Shan,Ping Luo*

Main category: cs.LG

TLDR: 提出了一种利用流模型作为先验对齐可学习潜在空间的新框架，避免了昂贵的似然计算和ODE求解。
...
Result: 实验证明对齐损失近似负对数似然，并在ImageNet上验证了有效性。
...
### [127] [On the Convergence of Gradient Descent on Learning Transformers with Residual Connections](https://arxiv.org/abs/2506.05249)
*Zhen Qin,Jinxin Zhou,Zhihui Zhu*

Main category: cs.LG

TLDR: 本文分析了单层Transformer的收敛行为，包括自注意力、前馈网络和残差连接，证明了梯度下降在适当初始化下具有线性收敛速率，并揭示了残差连接对优化稳定性的促进作用。


<details>
  <summary>Details</summary>
Motivation: 尽管Transformer模型在应用中表现出色，但其训练动态的理论基础仍不完善，尤其是组件间的相互依赖关系未被充分研究。

Method: 通过分析包含自注意力、前馈网络和残差连接的单层Transformer，研究其收敛行为。

Result: 在适当初始化下，梯度下降具有线性收敛速率，残差连接改善了输出矩阵的病态问题，提升了优化稳定性。

Conclusion: 理论分析和实证结果均表明，残差连接对Transformer的收敛稳定性具有积极作用。
...
### [128] [Conservative classifiers do consistently well with improving agents: characterizing statistical and online learning](https://arxiv.org/abs/2506.05252)
*Dravyansh Sharma,Alec Sun*

Main category: cs.LG

TLDR: 本文研究了机器学习中分类代理的真实改进对学习能力的影响，提出了新的学习能力特征，解决了Attias等人的开放问题。


<details>
  <summary>Details</summary>
Motivation: 研究分类代理的真实改进对学习算法的影响，特别是在社会决策中（如招聘或贷款评估），以探索比传统PAC学习更小的泛化误差。

Method: 引入不对称的最小一致概念类变体，用于实现在可实现设置中的精确特征化；研究欧几里得球改进集下的学习能力；提出在有界噪声模型和在线学习中的改进方法。

Result: 在可实现设置中实现了精确的特征化；在欧几里得球改进集下取得了积极结果；在有界噪声模型中降低了泛化误差；解决了Attias等人的开放问题。

Conclusion: 本文扩展了学习能力的研究范围，为分类代理的真实改进提供了新的理论支持，并解决了相关开放问题。
...
### [129] [Learning long range dependencies through time reversal symmetry breaking](https://arxiv.org/abs/2506.05259)
*Guillaume Pourcel,Maxence Ernoult*

Main category: cs.LG

TLDR: 论文提出了一种名为RHEL的算法，用于高效训练基于物理原理的深度状态空间模型（SSMs），无需显式计算雅可比矩阵，且梯度估计无方差。


<details>
  <summary>Details</summary>
Motivation: 受物理实现启发，研究旨在开发符合物理核心原则的学习算法，以高效模拟和设计动态系统。

Method: 提出RHEL算法，通过有限差分计算损失梯度，适用于非耗散哈密顿系统，并扩展到离散时间和层次化模型（HSSMs）。

Result: RHEL在多种时间序列任务中表现与BPTT相当，适用于长序列建模（长度达50k）。

Conclusion: RHEL为可扩展、高效能的物理系统设计提供了新途径，支持自学习能力的序列建模。
...
### [130] [Tight analyses of first-order methods with error feedback](https://arxiv.org/abs/2506.05271)
*Daniel Berg Thomsen,Adrien Taylor,Aymeric Dieuleveut*

Main category: cs.LG

TLDR: 本文对分布式学习中通信压缩的两种误差反馈方法（EF和EF²¹）进行了严格分析，找到了最优收敛率的Lyapunov函数，并提供了匹配的下界。


<details>
  <summary>Details</summary>
Motivation: 分布式学习中通信压缩是主要瓶颈，误差反馈方法（如EF和EF²¹）用于缓解压缩通信导致的收敛退化。本文旨在提供这两种方法的精确分析。

Method: 通过寻找最优Lyapunov函数，分析EF和EF²¹的收敛率，并与压缩梯度下降进行公平比较。

Result: 找到了每种方法的最优收敛率，并提供了匹配的下界，实现了对EF、EF²¹和压缩梯度下降的严格比较。

Conclusion: 本文的分析提供了清晰的性能保证，为误差反馈方法的机制提供了理论支持。
...
### [131] [How to Unlock Time Series Editing? Diffusion-Driven Approach with Multi-Grained Control](https://arxiv.org/abs/2506.05276)
*Hao Yu,Chu Xin Cheng,Runlong Yu,Yuyang Ye,Shiwei Tong,Zhaofeng Liu,Defu Lian*

Main category: cs.LG

TLDR: CocktailEdit框架通过结合点级和段级控制，实现了时间序列生成中的精确编辑，同时保持时间连贯性。
...
Method: 提出CocktailEdit框架，结合置信加权锚点控制（点级约束）和基于分类器的控制（段级统计属性）。

Result: 实验证明该框架在多种数据集和模型上有效，实现了精确的局部控制和时间连贯性。

Conclusion: CocktailEdit填补了生成模型与实际时间序列编辑需求之间的空白，提供了灵活的解决方案。
...
### [132] [Fast-DataShapley: Neural Modeling for Training Data Valuation](https://arxiv.org/abs/2506.05281)
*Haifeng Sun,Yu Xiong,Runze Wu,Xinyu Cai,Changjie Fan,Lan Zhang,Xiang-Yang Li*

Main category: cs.LG

TLDR: Fast-DataShapley是一种高效的单次训练方法，通过加权最小二乘特性计算Shapley值，避免重复训练，显著提升性能和效率。
...
Method: 提出Fast-DataShapley，利用加权最小二乘特性训练可复用的解释模型，并设计三种方法降低训练开销，包括效用函数近似计算和训练数据分组计算。

Result: 实验表明，性能提升2.5倍以上，训练速度提高两个数量级。

Conclusion: Fast-DataShapley在保护数据贡献者权益的同时，显著提升了计算效率和实用性。
...
### [133] [Learning Beyond Experience: Generalizing to Unseen State Space with Reservoir Computing](https://arxiv.org/abs/2506.05292)
*Declan A. Norton,Yuanzhao Zhang,Michelle Girvan*

Main category: cs.LG

TLDR: 论文探讨了储层计算在无显式结构先验的情况下，如何通过多轨迹训练方案实现状态空间未探索区域的泛化。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于解决机器学习在缺乏显式结构先验时对动态系统泛化能力不足的问题。

Method: 采用多轨迹训练方案，利用储层计算框架对多稳态动态系统进行训练。

Result: 结果表明，即使在单个吸引盆上训练，储层计算也能捕捉未观察到的吸引盆中的系统行为。

Conclusion: 结论是储层计算能够在无显式结构先验的情况下实现跨域泛化。
...
### [134] [A Smooth Sea Never Made a Skilled $\texttt{SAILOR}$: Robust Imitation via Learning to Search](https://arxiv.org/abs/2506.05294)
*Arnav Kumar Jain,Vibhakar Mohta,Subin Kim,Atiksh Bhardwaj,Juntao Ren,Yunhai Feng,Sanjiban Choudhury,Gokul Swamy*

Main category: cs.LG

TLDR: 论文提出了一种名为SAILOR的方法，通过从专家演示中学习搜索（L2S），解决了行为克隆（BC）在模仿学习中的局限性。SAILOR结合了世界模型和奖励模型，能够在测试时规划匹配专家结果，即使出现错误也能恢复。实验表明，SAILOR在多个视觉操作任务中优于基于BC的扩散策略，且对奖励欺骗具有鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 行为克隆（BC）方法在模仿学习中存在局限性，即只能学习专家在特定状态下的行为，无法在偏离专家演示状态时恢复。因此，需要一种能够独立推理并实现专家目标的方法。

Method: 提出了SAILOR方法，通过学习搜索（L2S）从专家演示中获取世界模型和奖励模型，结合其他组件实现稳定且高效的恢复行为学习。

Result: 在多个视觉操作任务中，SAILOR表现优于基于BC的扩散策略，且即使增加BC的演示数据量，仍存在性能差距。SAILOR能够识别细微错误并对奖励欺骗具有鲁棒性。

Conclusion: SAILOR通过L2S方法解决了BC的局限性，能够在复杂任务中实现高效恢复，并展示了优于现有方法的性能。
...
### [135] [Sample Complexity and Representation Ability of Test-time Scaling Paradigms](https://arxiv.org/abs/2506.05295)
*Baihe Huang,Shanda Li,Tianhao Wu,Yiming Yang,Ameet Talwalkar,Kannan Ramchandran,Michael I. Jordan,Jiantao Jiao*

Main category: cs.LG

TLDR: 论文分析了测试时扩展策略（如自一致性、最佳-n和自校正）的样本效率，理论证明了自一致性需要Θ(1/Δ²)样本，而最佳-n仅需Θ(1/Δ)。自校正方法通过验证器反馈扩展了Transformer的表达能力，使其能模拟多任务学习。


<details>
  <summary>Details</summary>
Motivation: 尽管测试时扩展策略在实践中表现优异，但其样本效率的理论研究不足，本文旨在填补这一空白。

Method: 通过理论分析比较自一致性和最佳-n的样本效率，并研究自校正方法在多任务中的表达能力。

Result: 自一致性需更多样本，最佳-n更高效；自校正方法使Transformer能模拟多任务学习。

Conclusion: 自校正方法在理论和实践中均表现出色，扩展了Transformer的多任务能力。
...
### [136] [Power Law Guided Dynamic Sifting for Efficient Attention](https://arxiv.org/abs/2506.05300)
*Nirav Koley,Prajwal Singhania,Abhinav Bhatele*

Main category: cs.LG

TLDR: SiftAttention是一种新的近似注意力方法，通过动态阈值过滤减少GPU内存带宽使用，同时保持模型质量。


<details>
  <summary>Details</summary>
Motivation: 解决GPU上大型语言模型推理时的内存带宽限制问题，特别是注意力计算中HBM与SRAM之间的数据传输问题。

Method: 提出SiftAttention，用基于阈值的元素级过滤操作替代昂贵的top-k操作，动态估计每个生成步骤的阈值。

Result: SiftAttention在减少内存带宽使用的同时，比现有近似注意力方法更好地保持了模型质量。

Conclusion: SiftAttention是一种高效的近似注意力方法，适用于GPU上的大型语言模型推理。
...
### [137] [Learning normalized image densities via dual score matching](https://arxiv.org/abs/2506.05310)
*Florentin Guth,Zahra Kadkhodaie,Eero P Simoncelli*

Main category: cs.LG

TLDR: 提出了一种基于扩散生成模型的新框架，用于学习归一化能量模型，通过双重分数匹配目标优化能量网络，在ImageNet64上取得了与SOTA相当的交叉熵值。
...
Result: 在ImageNet64上取得了与SOTA相当的交叉熵值，且能量模型表现出强泛化能力。

Conclusion: 该方法不仅提升了概率模型的学习效果，还揭示了图像概率与局部维度随内容变化的特性。
...
### [138] [Improving Data Efficiency for LLM Reinforcement Fine-tuning Through Difficulty-targeted Online Data Selection and Rollout Replay](https://arxiv.org/abs/2506.05316)
*Yifan Sun,Jingyan Shen,Yibin Wang,Tianyu Chen,Zhendong Wang,Mingyuan Zhou,Huan Zhang*

Main category: cs.LG

TLDR: 本文提出两种技术（难度目标在线数据选择和回放机制）以提高LLM强化学习微调的数据效率，显著减少训练时间。
...
Method: 采用自适应难度指导在线数据选择，并提出基于注意力的框架和回放机制以减少计算成本。

Result: 实验表明，该方法在6种LLM-数据集组合中将微调时间减少25%至65%，性能与GRPO相当。

Conclusion: 所提技术显著提升数据效率，为LLM微调提供实用解决方案。
...
### [139] [LSM-2: Learning from Incomplete Wearable Sensor Data](https://arxiv.org/abs/2506.05321)
...
TLDR: LSM-2 with AIM是一种新型自监督学习方法，直接从不完整数据中学习鲁棒表示，无需显式填补。


<details>
  <summary>Details</summary>
Motivation: 可穿戴传感器数据常存在缺失，传统自监督学习模型假设数据完整，难以应对。

Method: 采用自适应和继承掩码（AIM），通过可学习掩码标记建模缺失数据。

Result: 在40M小时多模态传感器数据上预训练，LSM-2在分类、回归和生成任务中表现最佳。

Conclusion: AIM适用于真实世界可穿戴数据，尤其在临床相关缺失场景中表现优异。
...
### [140] [Seeing the Invisible: Machine learning-Based QPI Kernel Extraction via Latent Alignment](https://arxiv.org/abs/2506.05325)
*Yingshuai Ji,Haomin Zhuang,Matthew Toole,James McKenzie,Xiaolong Liu,Xiangliang Zhang*

Main category: cs.LG

TLDR: 提出首个基于AI的QPI核提取框架，通过两步学习策略解决多散射条件下的逆问题。


<details>
  <summary>Details</summary>
Motivation: 多散射条件下的QPI核提取是一个病态逆问题，传统方法难以解决。

Method: 采用两步学习策略：1) 变分自编码器学习散射核的紧凑潜在空间；2) 专用编码器对齐QPI观测与预学习核的潜在表示。

Result: 方法在100个独特核的数据集上表现优于基线，提取精度更高，泛化能力更强。

Conclusion: 该框架为复杂散射条件下的QPI核提取提供了稳健且高效的解决方案。
...
### [141] [Kinetics: Rethinking Test-Time Scaling Laws](https://arxiv.org/abs/2506.05333)
*Ranajoy Sadhukhan,Zhuoming Chen,Haizhong Zheng,Yang Zhou,Emma Strubell,Beidi Chen*

Main category: cs.LG

TLDR: 研究发现，小模型在测试时的效率被高估，提出新的Kinetics Scaling Law，强调稀疏注意力的优势。


<details>
  <summary>Details</summary>
Motivation: 重新审视测试时缩放定律，揭示小模型效率被高估的问题，关注内存访问瓶颈。

Method: 提出Kinetics Scaling Law，结合计算和内存访问成本，并引入稀疏注意力范式。

Result: 稀疏注意力模型在低/高成本场景下均优于密集模型，AIME评测中提升显著。

Conclusion: 稀疏注意力是实现测试时缩放潜力的关键，代码已开源。
...
### [142] [Inference-Time Hyper-Scaling with KV Cache Compression](https://arxiv.org/abs/2506.05345)
*Adrian Łańcucki,Konrad Staniszewski,Piotr Nawrot,Edoardo M. Ponti*

Main category: cs.LG

TLDR: 论文提出了一种通过压缩KV缓存实现推理时超扩展的方法Dynamic Memory Sparsification (DMS)，在保持精度的同时显著提升推理效率。


<details>
  <summary>Details</summary>
Motivation: Transformer LLMs的推理成本受限于KV缓存大小，而非生成令牌数量。通过压缩KV缓存，可以在相同计算预算下生成更多令牌，提升推理精度。

Method: 引入Dynamic Memory Sparsification (DMS)，一种稀疏化KV缓存的新方法，仅需1K训练步骤即可实现8倍压缩，同时保持优于无训练稀疏注意力的精度。

Result: 在多种LLM上验证了DMS的有效性，例如在Qwen-R1 32B上，AIME 24、GPQA和LiveCodeBench的平均得分分别提升了9.1、7.6和9.6分。
...
### [143] [Exploring Diffusion Transformer Designs via Grafting](https://arxiv.org/abs/2506.05340)
...
TLDR: 该论文提出了一种称为“嫁接”的方法，通过编辑预训练的扩散变换器（DiTs）来研究新架构设计，从而在低计算成本下实现模型质量的提升。
...
Method: 提出“嫁接”方法，通过替换预训练DiTs中的操作符（如注意力机制）和配置（如MLP结构），生成混合设计。基于对激活行为和注意力局部性的分析，构建测试平台验证嫁接效果。

Result: 混合设计在低计算成本（<2%预训练计算）下实现良好质量（FID: 2.38-2.64 vs. 2.27）。嫁接文本到图像模型（PixArt-Sigma）实现1.43倍加速，且质量下降<2%。

Conclusion: 嫁接方法证明了通过编辑预训练DiTs可以探索新扩散模型设计，从操作符替换到架构重组均有效。
...
### [144] [GEM: Empowering LLM for both Embedding Generation and Language Understanding](https://arxiv.org/abs/2506.04344)
*Caojin Zhang,Qiang Zhang,Ke Li,Sai Vidyaranya Nuthalapati,Benyu Zhang,Jason Liu,Serena Li,Lizhu Zhang,Xiangjun Fan*

Main category: cs.CL

TLDR: 提出了一种名为GEM的自监督方法，使大型解码器语言模型（LLM）能生成高质量文本嵌入，同时保留其原始文本生成和推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有应用中，检索增强生成（RAG）等任务依赖独立的嵌入模型，导致系统复杂且嵌入模型与LLM对查询的理解不一致。

Method: 通过插入特殊标记并操纵注意力掩码，生成文本的摘要嵌入，可轻松集成到现有LLM的后训练或微调阶段。

Result: 在1B到8B参数的LLM上测试，MTEB文本嵌入基准显著提升，同时MMLU NLP基准影响极小。
...
### [145] [Effects of Speaker Count, Duration, and Accent Diversity on Zero-Shot Accent Robustness in Low-Resource ASR](https://arxiv.org/abs/2506.04364)
*Zheng-Xin Yong,Vineel Pratap,Michael Auli,Jean Maillard*

Main category: cs.CL

TLDR: 研究训练数据中说话者数量、音频时长和口音多样性对ASR系统在未见口音上鲁棒性的影响，发现增加说话者数量比增加单个说话者的音频时长更有效。


<details>
  <summary>Details</summary>
Motivation: 构建一个能服务全球的ASR系统，需其对多种口音（包括未见口音）具有鲁棒性。

Method: 系统研究训练数据中说话者数量、音频时长和口音多样性对ASR鲁棒性的影响。
...
### [146] [Mechanistic Decomposition of Sentence Representations](https://arxiv.org/abs/2506.04373)
*Matthieu Tehenan,Vikram Natarajan,Jonathan Michala,Milton Lin,Juri Opitz*

Main category: cs.CL

TLDR: 提出一种新方法，通过字典学习分解句子嵌入为可解释组件，揭示其内部结构和特征。
...
Result: 发现嵌入中许多语义和句法特征是线性编码的，实现了从词级到句子级的可解释性。

Conclusion: 该方法使句子嵌入更透明可控，为理解嵌入空间提供了新视角。
...
### [147] [Hierarchical Text Classification Using Contrastive Learning Informed Path Guided Hierarchy](https://arxiv.org/abs/2506.04381)
*Neeraj Agrawal,Saurabh Kumar,Priyanka Bhatt,Tanishka Agarwal*

Main category: cs.CL

TLDR: 提出了一种结合对比学习和路径引导层次结构的HTC-CLIP模型，通过联合学习文本和层次表示，显著提升了分类性能。
...
Method: HTC-CLIP利用对比学习联合学习文本和层次表示，并在推理时结合两者的概率分布。

Result: 在两个公开数据集上，Macro F1分数提升了0.99-2.37%。

Conclusion: HTC-CLIP有效结合了两种现有方法，显著提升了HTC任务的性能。
...
### [148] [MELABenchv1: Benchmarking Large Language Models against Smaller Fine-Tuned Models for Low-Resource Maltese NLP](https://arxiv.org/abs/2506.04385)
*Kurt Micallef,Claudia Borg*

Main category: cs.CL

TLDR: 评估55个公开大语言模型在低资源语言马耳他语上的表现，发现多数模型表现不佳，尤其是生成任务，而小型微调模型表现更好。
...
Method: 使用包含11个判别和生成任务的新基准测试55个模型，分析影响性能的因素。

Result: 模型表现普遍较差，生成任务尤甚；微调模型表现更优；预训练和指令调优中对马耳他语的接触是关键因素。
...
### [149] [Building a Few-Shot Cross-Domain Multilingual NLU Model for Customer Care](https://arxiv.org/abs/2506.04389)
*Saurabh Kumar,Sourav Bansal,Neeraj Agrawal,Priyanka Bhatt*

Main category: cs.CL

TLDR: 提出一种结合嵌入器和分类器的模型架构，通过少量标注样本扩展领域特定模型到其他领域，显著提升意图分类性能。


<details>
  <summary>Details</summary>
Motivation: 跨领域数据稀缺限制了意图分类器的泛化能力，需一种方法以少量标注实现多领域泛化。

Method: 采用监督微调与各向同性正则化训练领域特定嵌入器，结合多语言知识蒸馏策略泛化嵌入器，最后用线性分类器部署到新领域。

Result: 在加拿大和墨西哥电商客服数据集上，少样本意图检测准确率比现有SOTA模型提升20-23%。

Conclusion: 提出的模型架构在少样本场景下显著提升跨领域意图分类性能，具有实际应用价值。
...
### [150] [MedAgentGym: Training LLM Agents for Code-Based Medical Reasoning at Scale](https://arxiv.org/abs/2506.04405)
...
TLDR: MedAgentGYM是一个公开的训练环境，旨在提升大型语言模型（LLM）在医学推理任务中的编码能力，包含72,413个任务实例。通过测试30多个LLM，发现商业API模型与开源模型存在性能差距。Med-Copilot-7B通过微调和强化学习显著提升性能，成为GPT-4的竞争替代品。
...
Method: 构建包含72,413个任务实例的MedAgentGYM环境，提供详细任务描述、交互反馈和可验证标注。通过监督微调和强化学习优化模型性能。

Result: Med-Copilot-7B性能显著提升（监督微调+36.44%，强化学习+42.47%），接近GPT-4水平。

Conclusion: MedAgentGYM为开发医学领域的LLM编码助手提供了全面、可扩展的平台，推动了生物医学研究和实践的发展。
...
### [151] [Unpacking Let Alone: Human-Scale Models Generalize to a Rare Construction in Form but not Meaning](https://arxiv.org/abs/2506.04408)
*Wesley Scivetti,Tatsuya Aoyama,Ethan Wilcox,Nathan Schneider*

Main category: cs.CL

TLDR: 研究发现，大规模预训练语言模型能泛化罕见语法形式，但对语义理解不足。
...
Method: 测试模型对英语LET-ALONE结构的语法形式和语义理解能力。

Result: 模型能泛化语法形式，但对语义理解不足。

Conclusion: 当前架构在语言形式和语义的样本效率上存在不对称性。
...
### [152] [Empaths at SemEval-2025 Task 11: Retrieval-Augmented Approach to Perceived Emotions Prediction](https://arxiv.org/abs/2506.04409)
*Lev Morozov,Aleksandr Mogilevskii,Alexander Shirnin*

Main category: cs.CL

TLDR: EmoRAG是一个用于多标签情感检测的系统，无需额外训练，通过模型集成实现高效、可扩展的情感预测。
...
Method: 使用模型集成方法，无需额外训练，直接预测情感标签。

Result: 性能与最佳系统相当，但更高效、可扩展且易于实现。

Conclusion: EmoRAG是一种高效、实用的情感检测解决方案。
...
### [153] [Zero-Shot Open-Schema Entity Structure Discovery](https://arxiv.org/abs/2506.04458)
...
TLDR: 论文提出了一种无需预定义模式或标注数据的零样本开放模式实体结构发现方法（ZOES），通过丰富、细化和统一机制提升实体结构提取的完整性和质量。
...
Method: ZOES采用丰富、细化和统一的机制，利用实体与其结构相互强化的特性进行零样本提取。

Result: 实验表明，ZOES在三个不同领域中显著提升了大语言模型提取实体结构的完整性和泛化能力。
...
### [154] [Watermarking Degrades Alignment in Language Models: Analysis and Mitigation](https://arxiv.org/abs/2506.04462)
*Apurv Verma,NhatHai Phan,Shubhendu Trivedi*

Main category: cs.CL

TLDR: 本文系统分析了两种水印技术（Gumbel和KGW）对大型语言模型（LLMs）在真实性、安全性和帮助性上的影响，并提出了一种名为Alignment Resampling（AR）的推断时采样方法以恢复模型的对齐性能。
...
Method: 通过实验分析两种水印技术（Gumbel和KGW）对四种对齐LLMs的影响，并提出AR方法，利用外部奖励模型在推断时恢复对齐性能。

Result: 实验显示水印会导致两种退化模式（保护衰减和保护放大），而AR方法仅需2-4次采样即可恢复或超越基线对齐分数。

Conclusion: AR方法有效平衡了水印强度与模型对齐，为实际部署水印LLMs提供了简单解决方案。
...
### [155] [Aligning Large Language Models with Implicit Preferences from User-Generated Content](https://arxiv.org/abs/2506.04463)
...
TLDR: PUGC框架利用未标记的用户生成内容（UGC）中的隐式偏好生成偏好数据，替代传统依赖人工标注的方法，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有偏好学习方法依赖昂贵且难以扩展的人工标注数据，PUGC旨在通过UGC中的隐式偏好解决这一问题。

Method: PUGC将UGC转化为用户查询并生成响应，利用UGC作为参考文本进行评分，从而对齐模型与隐式偏好。

Result: 实验显示，PUGC结合DPO在Alpaca Eval 2上性能提升9.37%，并达到35.93%的SOTA胜率。

Conclusion: PUGC提供了一种高效、可扩展的偏好数据生成方法，显著提升了模型对齐效果和性能。
...
### [156] [SQLens: An End-to-End Framework for Error Detection and Correction in Text-to-SQL](https://arxiv.org/abs/2506.04494)
*Yue Gong,Chuan Lei,Xiao Qin,Kapil Vaidya,Balakrishnan Narayanaswamy,Tim Kraska*

Main category: cs.CL

TLDR: SQLens是一个端到端框架，用于细粒度检测和纠正LLM生成的SQL中的语义错误，显著提升错误检测和查询执行准确性。


<details>
  <summary>Details</summary>
Motivation: LLM生成的SQL查询可能存在语义错误但语法正确，且缺乏可靠性评估，因此需要一种方法检测和纠正这些错误。

Method: SQLens结合数据库和LLM的错误信号，识别SQL子句中的潜在语义错误，并利用这些信号指导查询纠正。

Result: 在两个公共基准测试中，SQLens在错误检测F1分数上比最佳LLM自评估方法高25.78%，并将现成文本到SQL系统的执行准确性提升至多20%。

Conclusion: SQLens能有效检测和纠正LLM生成的SQL中的语义错误，显著提升查询的可靠性和执行准确性。
...
### [157] [DRE: An Effective Dual-Refined Method for Integrating Small and Large Language Models in Open-Domain Dialogue Evaluation](https://arxiv.org/abs/2506.04516)
*Kun Zhao,Bohao Yang,Chen Tang,Siyuan Dai,Haoteng Tang,Chenghua Lin,Liang Zhan*

Main category: cs.CL

TLDR: SLIDE和DRE方法结合小型和大型语言模型的优势，通过自适应权重和双细化评估提升对话评估的可靠性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在模糊场景中表现不稳定，而小型模型对误导性输入敏感，两者互补性强。

Method: 提出SLIDE方法（自适应权重结合SLM和LLM）和DRE方法（双细化评估：SLM引导LLM生成初始评估，SLM调整LLM分数）。

Result: DRE在实验中优于现有方法，更贴近人类判断。
...
### [158] [Please Translate Again: Two Simple Experiments on Whether Human-Like Reasoning Helps Translation](https://arxiv.org/abs/2506.04521)
*Di Wu,Seth Aycock,Christof Monz*

Main category: cs.CL

TLDR: 研究发现，在翻译任务中，逐步分解的提示策略（如CoT）并未明确提升性能，而简单的“重新翻译”提示反而效果更好。


<details>
  <summary>Details</summary>
Motivation: 探讨逐步分解策略（如CoT）在翻译任务中的实际有效性，验证其是否真正提升性能。

Method: 通过实验对比逐步分解提示与简单“重新翻译”提示的效果，分析性能差异。

Result: 逐步分解策略未显著提升翻译性能，而“重新翻译”提示表现更优。

Conclusion: 研究未否定推理的作用，但需进一步探索CoT在翻译中的有效因素。
...
### [159] [Is It JUST Semantics? A Case Study of Discourse Particle Understanding in LLMs](https://arxiv.org/abs/2506.04534)
*William Sheffield,Kanishka Misra,Valentina Pyatkin,Ashwini Deo,Kyle Mahowald,Junyi Jessy Li*

Main category: cs.CL

TLDR: LLMs can broadly categorize but struggle with subtle nuances of English discourse particle 'just'.


<details>
  <summary>Details</summary>
Motivation: Investigate LLMs' ability to distinguish fine-grained senses of 'just', a polyfunctional discourse particle.

Method: Use expert-labeled data to test LLMs' capacity.

Result: LLMs partially differentiate broad categories but fail to capture nuanced effects.

Conclusion: LLMs have limitations in understanding subtle discourse particle nuances.
...
### [160] [BSBench: will your LLM find the largest prime number?](https://arxiv.org/abs/2506.04535)
*K. O. T. Erziev*

Main category: cs.CL

TLDR: 论文提出了一种针对无法回答问题的LLM基准测试方法，并发现现有模型在此类问题上表现不佳。


<details>
  <summary>Details</summary>
Motivation: 探讨LLM在面对无法回答问题时表现的研究动机。

Method: 提出了一种基准测试方法，并改进了现有数据集。

Result: 现有模型在无法回答的问题上表现远未达到完美。

Conclusion: 论文强调了此类测试的重要性，并提供了代码和数据供进一步研究。
...
### [161] [SSA-COMET: Do LLMs Outperform Learned Metrics in Evaluating MT for Under-Resourced African Languages?](https://arxiv.org/abs/2506.04557)
...
TLDR: SSA-MTE是一个大规模人工标注的机器翻译评估数据集，覆盖13种非洲语言对，并提出了改进的评估指标SSA-COMET和SSA-COMET-QE，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有机器翻译质量评估方法在非洲低资源语言中表现不佳，缺乏公开的训练数据和评估集。

Method: 构建了SSA-MTE数据集，开发了SSA-COMET和SSA-COMET-QE指标，并测试了基于LLM的提示方法。
...
### [162] [Demonstrations of Integrity Attacks in Multi-Agent Systems](https://arxiv.org/abs/2506.04572)
*Can Zheng,Yuhan Cao,Xiaoning Dong,Tianxing He*

Main category: cs.CL

TLDR: 论文探讨了恶意代理如何通过提示操纵在多代理系统中实施攻击，揭示了当前检测机制的局限性。


<details>
  <summary>Details</summary>
Motivation: 研究多代理系统中恶意代理如何利用提示操纵破坏系统完整性，以揭示现有安全协议的不足。

Method: 分析了四种攻击类型（Scapegoater、Boaster、Self-Dealer、Free-Rider），并通过实验验证提示操纵对系统行为的影响。

Result: 恶意代理能成功误导评估系统并操纵协作代理，甚至绕过先进的LLM监测器（如GPT-4o-mini）。

Conclusion: 强调需要更安全的MAS架构和全面的监测机制以应对此类攻击。
...
### [163] [Reasoning or Overthinking: Evaluating Large Language Models on Financial Sentiment Analysis](https://arxiv.org/abs/2506.04574)
*Dimitris Vamvourellis,Dhagash Mehta*

Main category: cs.CL

TLDR: 研究发现，在零样本金融情感分析任务中，大型语言模型（LLMs）的推理能力并未提升性能，反而GPT-4o在无推理提示时表现最佳。
...
Method: 使用Financial PhraseBank数据集，比较不同LLMs（如GPT-4o、GPT-4.1等）和提示策略（模拟System 1或System 2思维）的性能。

Result: 推理提示或模型设计未提升性能，GPT-4o在无推理提示时表现最优，且推理可能导致过度思考。

Conclusion: 金融情感分类中，快速直觉的“System 1”思维更接近人类判断，挑战了“更多推理总更好”的默认假设。
...
### [164] [Are LLMs Reliable Translators of Logical Reasoning Across Lexically Diversified Contexts?](https://arxiv.org/abs/2506.04575)
*Qingchuan Li,Jiatong Li,Zirui Liu,Mingyue Cheng,Yuting Zeng,Qi Liu,Tongxuan Liu*

Main category: cs.CL

TLDR: 论文提出SCALe基准测试和MenTaL方法，解决LLMs在逻辑翻译中因词汇多样性导致的不可靠问题。


<details>
  <summary>Details</summary>
Motivation: LLMs在逻辑推理中作为翻译器表现不佳，尤其是面对词汇多样性时，现有基准测试未能充分挑战这一能力。

Method: 提出SCALe基准测试，通过逻辑不变的词汇多样化评估LLMs；提出MenTaL方法，指导LLMs构建统一表达表后再翻译。

Result: 实验证实当前LLMs在词汇多样化翻译中存在缺陷，MenTaL方法显著提升了翻译性能。

Conclusion: SCALe和MenTaL解决了LLMs在逻辑翻译中的词汇多样性问题，为实际应用提供了改进方向。
...
### [165] [Selecting Demonstrations for Many-Shot In-Context Learning via Gradient Matching](https://arxiv.org/abs/2506.04579)
*Jianfei Zhang,Bei Li,Jun Bai,Rumei Li,Yanmeng Wang,Chenghua Lin,Wenge Rong*

Main category: cs.CL

TLDR: 论文提出了一种基于梯度匹配的演示选择方法，用于提升大语言模型（LLMs）在上下文学习（ICL）中的性能，显著优于随机选择方法。


<details>
  <summary>Details</summary>
Motivation: 当前ICL依赖随机选择演示，而实例级检索不适用于多演示场景，作者假设ICL与微调的数据需求类似，因此提出梯度匹配方法。

Method: 通过梯度匹配选择演示，使所选示例的微调梯度与整个训练集的梯度对齐，以接近完整训练集的学习效果。

Result: 在多种数据集和模型（如Qwen2.5-72B和Llama3-70B）上，该方法比随机选择性能提升4%，在5个闭源LLMs上提升约2%。

Conclusion: 梯度匹配方法为多演示ICL提供了更可靠和有效的解决方案，拓宽了其应用范围。
...
### [166] [SUCEA: Reasoning-Intensive Retrieval for Adversarial Fact-checking through Claim Decomposition and Editing](https://arxiv.org/abs/2506.04583)
*Hongjun Liu,Yilun Zhao,Arman Cohan,Chen Zhao*

Main category: cs.CL

TLDR: 提出了一种无需训练的方法SUCEA，通过分解对抗性声明并重新表述，提高事实核查系统的检索和标签预测准确性。
...
Conclusion: SUCEA框架有效提高了对抗性声明的事实核查性能。
...
### [167] [MuSciClaims: Multimodal Scientific Claim Verification](https://arxiv.org/abs/2506.04585)
*Yash Kumar Lal,Manikanta Bandham,Mohammad Saqib Hasan,Apoorva Kashi,Mahnaz Koupaee,Niranjan Balasubramanian*

Main category: cs.CL

TLDR: 论文提出了一个名为MuSciClaims的新基准，用于测试科学文献中多模态数据的声明验证能力，并揭示了当前视觉语言模型在此任务上的不足。


<details>
  <summary>Details</summary>
Motivation: 科学文献中的多模态数据（如图表）对验证科学声明至关重要，但缺乏可直接测试声明验证能力的多模态基准。

Method: 通过自动提取科学文章中的支持声明，并手动扰动生成矛盾声明，设计诊断任务以测试模型能力。

Result: 大多数视觉语言模型表现不佳（F1分数0.3-0.5），最佳模型仅达0.77，且存在偏向支持声明的偏差。

Conclusion: 模型在定位证据、跨模态信息整合及理解图表基础组件方面表现较差，需进一步改进。
...
### [168] [LESS: Large Language Model Enhanced Semi-Supervised Learning for Speech Foundational Models](https://arxiv.org/abs/2506.04586)
*Wen Ding,Fan Qian*

Main category: cs.CL

TLDR: LESS框架利用大语言模型（LLM）修正伪标签，提升半监督学习效果，在ASR和AST任务中显著降低错误率并提高翻译质量。


<details>
  <summary>Details</summary>
Motivation: 解决伪标签质量不高的问题，利用LLM优化半监督学习中的标签生成和过滤。

Method: 通过LLM修正ASR/AST生成的伪标签，并结合数据过滤策略提升知识转移效率。

Result: 在普通话ASR和西班牙语-英语AST任务中，WER降低3.77%，BLEU得分分别达到34.0和64.7。

Conclusion: LESS框架在多语言、多任务和多领域中表现出良好的适应性和效果。
...
### [169] [Safe: Enhancing Mathematical Reasoning in Large Language Models via Retrospective Step-aware Formal Verification](https://arxiv.org/abs/2506.04592)
*Chengwu Liu,Ye Yuan,Yichun Yin,Yan Xu,Xin Xu,Zaoyu Chen,Yasheng Wang,Lifeng Shang,Qun Liu,Ming Zhang*

Main category: cs.CL

TLDR: 论文提出了一种名为$Safe$的回顾性、步骤感知的形式验证框架，利用Lean 4形式化数学语言验证LLM生成的推理步骤，以减少幻觉并提供可验证证据。


<details>
  <summary>Details</summary>
Motivation: 当前Chain-of-Thought (CoT)方法存在幻觉问题，且现有检测方法（如PRMs或自一致性）缺乏可验证证据，限制了其有效性。
...
Conclusion: 这是首次利用Lean 4形式化语言验证LLM生成的自然语言内容，为幻觉问题提供了稳健的解决方案。
...
### [170] [A MISMATCHED Benchmark for Scientific Natural Language Inference](https://arxiv.org/abs/2506.04603)
*Firoz Shaik,Mobashir Sadat,Nikita Gautam,Doina Caragea,Cornelia Caragea*

Main category: cs.CL

TLDR: 论文提出了一个名为MISMATCHED的科学自然语言推理（NLI）新基准，覆盖心理学、工程学和公共卫生三个非计算机科学领域，包含2700个人工标注的句子对。通过预训练小语言模型（SLM）和大语言模型（LLM）建立了强基线，最佳基线Macro F1为78.17%，显示未来改进空间大。此外，训练中加入隐含科学NLI关系的句子对可提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有科学NLI数据集仅覆盖计算机科学领域，非计算机科学领域被忽视。

Method: 引入MISMATCHED基准，覆盖三个非计算机科学领域，使用SLM和LLM建立基线，并探索隐含科学NLI关系的句子对在训练中的作用。

Result: 最佳基线Macro F1为78.17%，显示未来改进空间大；加入隐含关系的句子对可提升模型性能。

Conclusion: MISMATCHED填补了非计算机科学领域科学NLI的空白，为未来研究提供了新方向。
...
### [171] [Revisiting Test-Time Scaling: A Survey and a Diversity-Aware Method for Efficient Reasoning](https://arxiv.org/abs/2506.04611)
*Ho-Lam Chung,Teng-Yun Hsiao,Hsiao-Ying Huang,Chunerh Cho,Jian-Ren Lin,Zhang Ziwei,Yun-Nung Chen*

Main category: cs.CL

TLDR: TTS通过推理时分配额外计算提升LLM推理性能，ADAPT方法通过多样性优化前缀微调显著减少计算需求。


<details>
  <summary>Details</summary>
Motivation: 推理优化模型输出多样性不足限制了TTS效果，需解决此问题。

Method: 提出ADAPT方法，结合多样性数据策略进行轻量级前缀微调。

Result: 在数学推理任务中，ADAPT以8倍少计算量达到80%准确率。
...
### [172] [Subjective Perspectives within Learned Representations Predict High-Impact Innovation](https://arxiv.org/abs/2506.04616)
*Likun Cao,Rui Pan,James Evans*

Main category: cs.CL

TLDR: 该研究利用机器学习建模创新者的主观视角和人际创新机会，发现视角多样性比背景多样性更能预测创造性成就。
...
Method: 基于动态语言表示构建几何空间，量化创新者的主观视角和机会，分析科学家、发明家等数据，并进行自然实验和AI模拟。

Result: 视角多样性显著预测创造性成就，而背景多样性则相反。

Conclusion: 成功的协作依赖于共同语言和视角多样性，对团队组建和研究政策有重要启示。
...
### [173] [Static Word Embeddings for Sentence Semantic Representation](https://arxiv.org/abs/2506.04624)
*Takashi Wada,Yuki Hirakawa,Ryotaro Shimizu,Takahiro Kawashima,Yuki Saito*

Main category: cs.CL

TLDR: 提出了一种优化句子语义表示的静态词嵌入方法，通过改进预训练的句子Transformer的词嵌入，结合句子级主成分分析和知识蒸馏或对比学习，实现了高效且性能优越的句子表示。
...
### [174] [Advancing Tool-Augmented Large Language Models via Meta-Verification and Reflection Learning](https://arxiv.org/abs/2506.04625)
*Zhiyuan Ma,Jiayu Liu,Xianzhen Luo,Zhenya Huang,Qingfu Zhu,Wanxiang Che*

Main category: cs.CL

TLDR: Tool-MVR通过多代理元验证和探索式反思学习，解决了LLM在工具规划和反思中的问题，显著提升了性能。
...
Method: 提出Tool-MVR，结合MAMV验证工具指令集和EXPLORE动态学习范式，生成高质量数据集ToolBench-V和ToolBench-R。

Result: Tool-MVR在StableToolBench上超越ToolLLM和GPT-4，减少API调用，并在RefineToolBench上实现58.9%的错误修正率。
...
### [175] [ViCocktail: Automated Multi-Modal Data Collection for Vietnamese Audio-Visual Speech Recognition](https://arxiv.org/abs/2506.04635)
*Thai-Binh Nguyen,Thi Van Nguyen,Quoc Truong Do,Chi Mai Luong*

Main category: cs.CL

TLDR: 本文提出了一种从原始视频生成AVSR数据集的实用方法，并展示了其在越南语中的有效性，显著提升了在噪声环境中的性能。


<details>
  <summary>Details</summary>
Motivation: 传统AVSR模型受限于数据集稀缺，尤其是非英语语言。自动化数据收集是解决这一问题的潜在方案。

Method: 提出了一种从原始视频生成AVSR数据集的实用方法，改进了现有技术以提高效率和可访问性。

Result: 实验表明，自动收集的数据集能建立强大的基线模型，在噪声环境中显著优于传统ASR。
...
### [176] [TaDA: Training-free recipe for Decoding with Adaptive KV Cache Compression and Mean-centering](https://arxiv.org/abs/2506.04642)
*Vinay Joshi,Pratik Prabhanjan Brahma,Zicheng Liu,Emad Barsoum*

Main category: cs.CL

TLDR: TaDA是一种无需训练的KV缓存压缩方法，通过自适应量化精度和均值中心化消除异常值处理，显著减少内存占用至27%，同时保持准确性。
...
### [177] [Flex-TravelPlanner: A Benchmark for Flexible Planning with Language Agents](https://arxiv.org/abs/2506.04649)
*Juhyun Oh,Eunsu Kim,Alice Oh*

Main category: cs.CL

TLDR: Flex-TravelPlanner是一个新基准，用于评估语言模型在动态规划场景中的灵活推理能力，发现现有模型在多轮任务和约束优先级处理上表现不佳。
...
Method: 基于TravelPlanner数据集，引入多轮约束顺序和竞争约束优先级两种新评估设置。

Result: 模型在多轮任务中的表现与单轮任务相关性低，约束顺序和优先级处理能力不足。

Conclusion: 需在更真实的动态场景中评估LLMs，并针对性改进复杂规划任务的表现。
...
### [178] [Normative Conflicts and Shallow AI Alignment](https://arxiv.org/abs/2506.04679)
*Raphaël Millière*

Main category: cs.CL

TLDR: 论文探讨了大语言模型（LLMs）的价值对齐问题，指出当前的对齐策略无法有效防止滥用，并提出其根本局限性在于缺乏真正的规范性思考能力。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统（如LLMs）的进步，其安全部署问题日益突出，当前的对齐方法不足以防止滥用。

Method: 通过分析人类道德心理学的研究，对比LLMs的局限性，指出其缺乏规范性冲突的检测和解决能力。

Result: LLMs的“浅层对齐”问题使其容易受到对抗性攻击，现有方法无法有效缓解潜在危害。

Conclusion: 当前的对齐方法不足以应对日益强大的AI系统带来的风险，需改进对齐策略以增强规范性思考能力。
...
### [179] [MMRefine: Unveiling the Obstacles to Robust Refinement in Multimodal Large Language Models](https://arxiv.org/abs/2506.04688)
*Gio Paik,Geewook Kim,Jinbae Im*

Main category: cs.CL

TLDR: MMRefine是一个多模态细化基准，用于评估多模态大语言模型（MLLMs）的错误细化能力，通过六种场景和错误类型分析性能瓶颈。
...
### [180] [Recycling the Web: A Method to Enhance Pre-training Data Quality and Quantity for Language Models](https://arxiv.org/abs/2506.04689)
*Thao Nguyen,Yang Li,Olga Golovneva,Luke Zettlemoyer,Sewoong Oh,Ludwig Schmidt,Xian Li*

Main category: cs.CL

TLDR: 论文提出REWIRE方法，通过改写低质量网络文本以扩充预训练数据，实验证明其优于仅使用过滤数据或传统合成数据方法。
...
Conclusion: 改写网络文本是一种简单有效的预训练数据扩展方法，优于传统合成数据生成方式。
...
### [181] [Cracking the Code: Enhancing Implicit Hate Speech Detection through Coding Classification](https://arxiv.org/abs/2506.04693)
*Lu Wei,Liangzhi Li,Tong Xiang,Xiao Liu,Noa Garcia*

Main category: cs.CL

TLDR: 论文提出了一种新的隐式仇恨言论（im-HS）检测分类法，通过六种编码策略（codetypes）改进检测方法，验证了其在多语言数据集中的有效性。
...
Method: 提出了两种方法：1）直接提示大型语言模型（LLMs）基于生成响应分类句子；2）在编码过程中嵌入codetypes。

Result: 实验表明，codetypes在中文和英文数据集中均提高了im-HS检测效果。

Conclusion: 该方法在多语言环境下有效提升了隐式仇恨言论的检测能力。
...
### [182] [Accelerated Test-Time Scaling with Model-Free Speculative Sampling](https://arxiv.org/abs/2506.04708)
*Woomin Song,Saket Dingliwal,Sai Muralidhar Jayanthi,Bhavana Ganesh,Jinwoo Shin,Aram Galstyan,Sravan Babu Bodapati*

Main category: cs.CL

TLDR: STAND是一种无需模型的推测解码方法，通过利用推理轨迹的冗余性，显著加速推理过程而不损失准确性。


<details>
  <summary>Details</summary>
Motivation: 现有推理扩展技术（如best-of-N采样和树搜索）计算资源需求高，性能与效率之间存在矛盾。
...
### [183] [IIITH-BUT system for IWSLT 2025 low-resource Bhojpuri to Hindi speech translation](https://arxiv.org/abs/2506.04714)
*Bhavana Akkiraju,Aishwarya Pothula,Santosh Kesiraju,Anil Kumar Vuppala*

Main category: cs.CL

TLDR: IIITH-BUT团队在IWSLT 2025低资源Bhojpuri-Hindi语音翻译任务中，通过超参数优化和数据增强技术提升了SeamlessM4T模型的性能。


<details>
  <summary>Details</summary>
Motivation: 研究在低资源语言对（Bhojpuri-Hindi）中，超参数优化和数据增强对语音翻译模型性能的影响。

Method: 系统研究学习率调度、更新步数等超参数，并应用速度扰动和SpecAugment数据增强技术，同时探索跨语言信号（Marathi和Bhojpuri联合训练）。

Result: 实验表明，超参数选择和简单有效的数据增强显著提升了低资源环境下的翻译质量。

Conclusion: 通过分析翻译假设，揭示了影响BLEU分数的错误类型，为低资源语音翻译提供了实用改进方向。
...
### [184] [SPARTA ALIGNMENT: Collectively Aligning Multiple Language Models through Combat](https://arxiv.org/abs/2506.04721)
*Yuru Jiang,Wenxuan Ding,Shangbin Feng,Greg Durrett,Yulia Tsvetkov*

Main category: cs.CL

TLDR: SPARTA ALIGNMENT是一种通过竞争和对抗集体对齐多个LLM的算法，利用多模型竞争和互评提升生成多样性和减少偏见。
...
Conclusion: SPARTA ALIGNMENT能有效提升模型性能，并利用模型多样性生成更逻辑、直接和丰富的结果。
...
### [185] [HSSBench: Benchmarking Humanities and Social Sciences Ability for Multimodal Large Language Models](https://arxiv.org/abs/2506.03922)
...
### [186] [Lifelong Evolution: Collaborative Learning between Large and Small Language Models for Continuous Emergent Fake News Detection](https://arxiv.org/abs/2506.04739)
*Ziyi Zhou,Xiaoming Zhang,Litian Zhang,Yibo Zhang,Zhenyu Guan,Chaozhuo Li,Philip S. Yu*

Main category: cs.CL

TLDR: 提出了一种名为C²EFND的新框架，结合大型语言模型（LLMs）和小型语言模型（SLMs）的优势，通过多轮协作学习和知识更新模块，显著提升了虚假新闻检测的准确性和适应性。
...
Method: 提出C²EFND框架，结合LLMs的泛化能力和SLMs的分类专长，采用多轮协作学习和基于Mixture-of-Experts的知识编辑模块，以及基于回放的持续学习方法。

Result: 在Pheme和Twitter16数据集上的实验表明，C²EFND显著优于现有方法，提高了检测准确性和适应性。

Conclusion: C²EFND框架通过协作学习和持续知识更新，有效解决了虚假新闻检测中的挑战，具有实际应用潜力。
...
### [187] [Identifying Reliable Evaluation Metrics for Scientific Text Revision](https://arxiv.org/abs/2506.04772)
*Léane Jourdan,Florian Boudin,Richard Dufour,Nicolas Hernandez*

Main category: cs.CL

TLDR: 本文分析了传统文本修订评估指标的局限性，探索了更符合人类判断的替代方法，并提出了一种结合LLM评估和领域特定指标的混合方法。


<details>
  <summary>Details</summary>
Motivation: 传统评估指标（如ROUGE和BERTScore）主要关注相似性，无法捕捉修订的实际改进，因此需要更有效的评估方法。
...
### [188] [Fine-Grained Interpretation of Political Opinions in Large Language Models](https://arxiv.org/abs/2506.04774)
*Jingyu Hu,Mengyue Yang,Mengnan Du,Weiru Liu*

Main category: cs.CL

TLDR: 论文研究了LLMs的政治观点，发现其开放回答与内部意图不一致，提出多维政治学习框架以解决单轴概念混淆问题。
...
Method: 设计四维政治学习框架，构建数据集，应用可解释表示工程技术。

Result: 实验表明，向量能解构政治概念混淆，检测任务验证其语义意义，干预实验可改变LLMs的政治倾向回答。

Conclusion: 多维框架和表示工程技术能更透明地学习LLMs的政治概念，并有效干预其内部机制。
...
### [189] [MMSU: A Massive Multi-task Spoken Language Understanding and Reasoning Benchmark](https://arxiv.org/abs/2506.04779)
*Dingdong Wang,Jincenzi Wu,Junan Li,Dongchao Yang,Xueyuan Chen,Tianhua Zhang,Helen Meng*

Main category: cs.CL

TLDR: MMSU是一个针对口语理解的综合基准测试，包含5000个音频-问题-答案三元组，涵盖47种任务，旨在评估和改进语音大语言模型的细粒度感知和复杂推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有语音大语言模型在自然语音中的细粒度感知和复杂推理能力尚未充分探索，需要更全面的评估标准。

Method: 通过系统整合语音学、韵律学、修辞学、句法学、语义学和副语言学等语言现象，构建MMSU基准测试，并评估14种先进模型。

Result: 发现现有模型有显著改进空间，为未来优化提供了方向。

Conclusion: MMSU为口语理解提供了新的评估标准，有助于开发更先进的人机语音交互系统。
...
### [190] [Towards LLM-Centric Multimodal Fusion: A Survey on Integration Strategies and Techniques](https://arxiv.org/abs/2506.04788)
*Jisu An,Junseok Lee,Jeoungeun Lee,Yongseok Son*

Main category: cs.CL

TLDR: 本文对多模态大语言模型（MLLMs）进行了系统分析，提出了基于架构策略、表示学习技术和训练范式的分类框架，并总结了125个模型的发展趋势。


<details>
  <summary>Details</summary>
Motivation: 填补现有文献中关于多模态输入如何与语言主干连接的空白，为研究者提供结构化视角。

Method: 通过分析125个MLLMs，提出基于三个维度的分类框架：架构策略、表示学习技术和训练范式。

Result: 总结了当前MLLMs的发展模式，为未来模型的多模态集成提供了指导。

Conclusion: 该分类框架为基于预训练基础的更鲁棒多模态集成策略开发提供了参考。
...
### [191] [Dissecting Logical Reasoning in LLMs: A Fine-Grained Evaluation and Supervision Study](https://arxiv.org/abs/2506.04810)
*Yujun Zhou,Jiayi Ye,Zipeng Ling,Yufei Han,Yue Huang,Haomin Zhuang,Zhenwen Liang,Kehan Guo,Taicheng Guo,Xiangqi Wang,Xiangliang Zhang*

Main category: cs.CL

TLDR: FineLogic是一个细粒度的评估框架，用于评估大型语言模型（LLM）的逻辑推理能力，涵盖准确性、步骤合理性和表示对齐三个维度。研究发现，自然语言监督在泛化能力上表现优异，而符号监督则促进结构化的推理链。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试仅依赖最终答案准确性，无法全面评估推理过程的质量和结构，因此需要更精细的评估框架。

Method: 提出FineLogic框架，并在四种监督风格（一种自然语言和三种符号变体）下训练LLM，研究其对推理能力的影响。

Result: 自然语言监督在泛化和长上下文任务中表现优异，符号监督则生成更结构化的推理链。表示级探测显示微调主要通过逐步生成改进推理行为。
...
### [192] [Design of intelligent proofreading system for English translation based on CNN and BERT](https://arxiv.org/abs/2506.04811)
*Feijun Liu,Huifeng Wang,Kun Wang,Yizhen Wang*

Main category: cs.CL

TLDR: 本文提出了一种结合CNN和BERT的混合方法，用于机器翻译校对，通过语义提取和上下文建模显著提升校对质量。


<details>
  <summary>Details</summary>
Motivation: 自动翻译常包含错误，需人工校对。为提高校对质量，需更高效的校对方法。

Method: 结合CNN提取局部n-gram模式，BERT建模上下文，集成错误检测与修正模块，使用端到端训练。

Result: 实验显示90%准确率、89.37% F1和16.24% MSE，性能优于现有方法10%。

Conclusion: 该方法在识别和修正翻译错误方面表现优异，达到最新技术水平。
...
### [193] [Evaluating Vision-Language and Large Language Models for Automated Student Assessment in Indonesian Classrooms](https://arxiv.org/abs/2506.04822)
*Nurul Aisyah,Muhammad Dehan Al Kautsar,Arif Hidayat,Raqib Chowdhury,Fajri Koto*

Main category: cs.CL

TLDR: 研究评估了VLM和LLM在印尼四年级学生手写考试中的表现，发现VLM在手写识别上存在困难，但LLM生成的反馈仍有一定实用性。
...
Result: VLM在手写识别上表现不佳，影响LLM评分准确性，但LLM生成的反馈仍具部分实用性。
...
### [194] [A Reasoning-Based Approach to Cryptic Crossword Clue Solving](https://arxiv.org/abs/2506.04824)
*Martin Andrews,Sam Witteveen*

Main category: cs.CL

TLDR: 论文提出了一种基于LLM的系统，用于解决加密填字游戏线索，通过假设答案、提出字谜解释和验证步骤，在Cryptonite数据集上达到新SOTA。
...
Method: 系统分为三步：假设答案、提出字谜解释、使用验证系统检查推理步骤。

Result: 在The Times和The Telegraph的Cryptonite数据集上达到新SOTA。

Conclusion: 该系统不仅高效，还能提供可解释的推理步骤，便于检查。
...
### [195] [Joint Evaluation of Answer and Reasoning Consistency for Hallucination Detection in Large Reasoning Models](https://arxiv.org/abs/2506.04832)
*Changyue Wang,Weihang Su,Qingyao Ai,Yiqun Liu*

Main category: cs.CL

TLDR: 论文提出了RACE框架，用于检测大型推理模型（LRMs）中的幻觉问题，通过分析推理步骤的多个信号，提升检测效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注答案层面的不确定性，难以检测推理过程中的冗余或不一致，而LRMs的推理痕迹是潜在幻觉的关键来源。

Method: 提出RACE框架，提取关键推理步骤并计算四种诊断信号：推理痕迹的样本间一致性、基于熵的答案不确定性、推理与答案的语义对齐以及推理的内部一致性。

Result: 实验表明，RACE在多个数据集和LLMs上优于现有基线，提供了细粒度的幻觉检测能力。

Conclusion: RACE是一个鲁棒且通用的解决方案，适用于评估LRMs中的幻觉问题。
...
### [196] [MockConf: A Student Interpretation Dataset: Analysis, Word- and Span-level Alignment and Baselines](https://arxiv.org/abs/2506.04848)
*Dávid Javorský,Ondřej Bojar,François Yvon*

Main category: cs.CL

TLDR: 论文介绍了MockConf数据集和InterAlign工具，用于分析和评估同声传译任务，填补了现有平行语料库和算法的不足。


<details>
  <summary>Details</summary>
Motivation: 理解并自动化同声传译这一动态复杂任务需要专用数据集和工具，现有资源无法满足需求。

Method: 收集学生模拟会议的同声传译数据，开发基于网页的标注工具InterAlign，并提出自动对齐的基线方法。

Result: 发布了包含5种欧洲语言的7小时录音数据集MockConf，以及标注工具InterAlign和评估指标。

Conclusion: MockConf和InterAlign为同声传译研究提供了新资源，支持自动对齐和评估。
...
### [197] [Multiple-Choice Question Generation Using Large Language Models: Methodology and Educator Insights](https://arxiv.org/abs/2506.04851)
*Giorgio Biancini,Alessio Ferrato,Carla Limongelli*

Main category: cs.CL

TLDR: 论文探讨了利用大型语言模型（LLMs）自动生成多选题（MCQs）的潜力，比较了Llama 2、Mistral和GPT-3.5的性能，发现GPT-3.5表现最佳。
...
### [198] [Prompting LLMs: Length Control for Isometric Machine Translation](https://arxiv.org/abs/2506.04855)
*Dávid Javorský,Ondřej Bojar,François Yvon*

Main category: cs.CL

TLDR: 研究探讨了等长机器翻译在多种语言对中的效果，分析了提示策略、少样本示例数量及演示选择对翻译质量和长度控制的影响。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型在等长翻译任务中的表现，以及如何通过提示策略和示例选择优化翻译质量和长度控制。

Method: 使用八种不同规模的开源大型语言模型，研究不同提示策略、少样本示例数量及演示选择对翻译的影响。

Result: 指令措辞与演示属性对齐对控制输出长度至关重要；极端示例能缩短翻译，但等长演示易忽略长度约束；少样本提示提升翻译质量，但边际效益递减；多输出选择可优化长度与质量的权衡。

Conclusion: 通过优化提示策略和演示选择，大型语言模型在等长翻译任务中可实现性能提升，但需注意长度控制的挑战。
...
### [199] [Evaluating the Effectiveness of Linguistic Knowledge in Pretrained Language Models: A Case Study of Universal Dependencies](https://arxiv.org/abs/2506.04887)
*Wenxi Li*

Main category: cs.CL

TLDR: 将Universal Dependencies (UD)集成到预训练语言模型中，评估其在跨语言对抗性释义识别任务中的表现，结果显示UD显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 探讨UD作为跨语言句法表示框架的有效性，填补其在预训练模型中的应用空白。

Method: 将UD整合到预训练语言模型中，评估其在跨语言对抗性释义识别任务中的表现。

Result: UD显著提升了模型的准确率和F1分数，平均增益分别为3.85%和6.08%，部分语言对中甚至超越大型语言模型。

Conclusion: UD在跨语言任务中具有潜力，其与英语的相似度得分与模型性能正相关。
...
### [200] [ICPC-Eval: Probing the Frontiers of LLM Reasoning with Competitive Programming Contests](https://arxiv.org/abs/2506.04894)
*Shiyi Xu,Yiwen Hu,Yingqian Min,Zhipeng Chen,Wayne Xin Zhao,Ji-Rong Wen*

Main category: cs.CL

TLDR: 提出了ICPC-Eval基准，用于评估LLM在真实竞赛环境中的编码能力，并引入新指标Refine@K。


<details>
  <summary>Details</summary>
Motivation: 现有基准和指标无法充分评估LLM在竞赛环境中的编码和反思能力。

Method: 从11场ICPC竞赛中精选118道题目，提供本地评估工具和Refine@K指标。

Result: 顶级推理模型需多轮反馈才能发挥潜力，但仍落后于人类团队。

Conclusion: ICPC-Eval为评估复杂推理能力提供了有效工具，揭示了LLM的局限性。
...
### [201] [Verbose ListOps (VLO): Beyond Long Context -- Unmasking LLM's Reasoning Blind Spots](https://arxiv.org/abs/2506.04907)
*Alex Pan,Mary-Anne Williams*

Main category: cs.CL

TLDR: Verbose ListOps是一个新基准，通过将ListOps计算转化为长故事，测试LLMs在嵌套叙事推理中的表现，揭示其在状态管理上的不足。
...
Result: 领先的LLMs（如OpenAI o4、Gemini 2.5 Pro）在Verbose ListOps上表现不佳，尤其是在中等长度叙事中。

Conclusion: Verbose ListOps揭示了LLMs在嵌套推理中的关键弱点，为针对性改进提供了工具，是自动化知识工作的关键一步。
...
### [202] [A Practitioner's Guide to Building ASR Models for Low-Resource Languages: A Case Study on Scottish Gaelic](https://arxiv.org/abs/2506.04915)
*Ondřej Klejch,William Lamb,Peter Bell*

Main category: cs.CL

TLDR: 本文挑战了通过微调多语言端到端模型开发低资源语言ASR系统的普遍观点，提出了一种结合混合HMM与自监督模型的方法，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法认为微调多语言端到端模型是低资源语言ASR的最佳方案，但本文发现其效果有限，尤其是训练数据不足时。

Method: 提出结合混合HMM与自监督模型的方法，充分利用语音和文本数据，通过持续自监督预训练和半监督训练优化性能。

Result: 在苏格兰盖尔语上测试，相比微调的Whisper模型，WER相对降低了32%。
...
### [203] [Simulating LLM-to-LLM Tutoring for Multilingual Math Feedback](https://arxiv.org/abs/2506.04920)
*Junior Cedric Tonga,KV Aditya Srivatsa,Kaushal Kumar Maurya,Fajri Koto,Ekaterina Kochmar*

Main category: cs.CL

TLDR: 研究探索了大型语言模型（LLMs）在多语言教育中的有效性，特别是数学推理任务中。通过模拟多语言师生互动，发现语言对齐的反馈能显著提升学习效果，尤其在低资源语言中。


<details>
  <summary>Details</summary>
Motivation: 评估LLMs在多语言环境下提供教学支持的能力，尤其是数学推理任务，填补现有研究的空白。

Method: 通过模拟师生互动，使用强模型作为导师生成提示，弱模型模拟学生，覆盖11种语言、4种LLMs及多种提示策略。

Result: 多语言提示显著提升学习效果，尤其是当反馈语言与学生母语一致时，低资源语言受益最大。
...
### [204] [ConECT Dataset: Overcoming Data Scarcity in Context-Aware E-Commerce MT](https://arxiv.org/abs/2506.04929)
*Mikołaj Pokrywka,Wojciech Kusa,Mieszko Rutkowski,Mikołaj Koszowski*

Main category: cs.CL

TLDR: 论文提出了一种结合视觉和产品元数据的上下文感知神经机器翻译方法，并在捷克语到波兰语的电商数据上验证了其有效性。
...
Method: 创建了ConECT数据集（含11,400句对），并测试了视觉语言模型（VLM）和文本到文本模型结合上下文信息的效果。

Result: 实验表明，结合视觉和产品元数据等上下文信息能显著提升翻译质量。

Conclusion: 上下文信息的引入对提升机器翻译质量具有重要价值，同时公开了新的数据集。
...
### [205] [From Struggle (06-2024) to Mastery (02-2025) LLMs Conquer Advanced Algorithm Exams and Pave the Way for Editorial Generation](https://arxiv.org/abs/2506.04965)
*Adrian Marius Dumitran,Theodor-Pierre Moroianu,Vasile Paul Alexe*

Main category: cs.CL

TLDR: 本文评估了大型语言模型（LLMs）在大学级算法考试中的表现，发现最新模型表现优异，接近顶尖学生水平，但在图相关任务上仍有困难。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在复杂算法问题上的解决能力及其在教育环境中的潜在应用。
...
### [206] [Better Semi-supervised Learning for Multi-domain ASR Through Incremental Retraining and Data Filtering](https://arxiv.org/abs/2506.04981)
...
TLDR: 论文提出了一种增量半监督学习流程，利用少量领域内标注数据和相关领域辅助数据，通过多模型共识或命名实体识别筛选伪标签，显著提升了ASR模型在稀缺标注数据下的性能。


<details>
  <summary>Details</summary>
Motivation: 解决在标注数据稀缺时，预训练ASR模型在特定领域微调的挑战。

Method: 提出增量半监督学习流程，结合领域内少量标注数据和相关领域辅助数据，通过多模型共识或NER筛选伪标签并迭代优化。

Result: 在Wow呼叫中心和Fisher英语语料库上，共识筛选表现最佳，相对随机选择提升22.3%和24.8%；NER为次优方法，计算成本更低。

Conclusion: 共识筛选和NER是有效的伪标签优化方法，显著提升ASR模型在稀缺标注数据下的性能。
...
### [207] [SCOP: Evaluating the Comprehension Process of Large Language Models from a Cognitive View](https://arxiv.org/abs/2506.05000)
*Yongjie Xiao,Hongru Liang,Peixin Qin,Yao Zhang,Wenqiang Lei*

Main category: cs.CL

TLDR: 论文提出SCOP框架，从认知视角评估大语言模型（LLMs）的理解能力，发现其与专家水平仍有差距，并指出改进方向。
...
Method: 提出SCOP框架，定义五项理解技能，构建测试数据，并分析开源与闭源LLMs的表现。

Result: LLMs难以达到专家级理解水平，且可能通过错误过程得出正确答案。
...
### [208] [ComfyUI-Copilot: An Intelligent Assistant for Automated Workflow Development](https://arxiv.org/abs/2506.05010)
*Zhenran Xu,Xue Yang,Yiyu Wang,Qingli Hu,Zijiao Wu,Longyue Wang,Weihua Luo,Kaifu Zhang,Baotian Hu,Min Zhang*

Main category: cs.CL

TLDR: ComfyUI-Copilot是一个基于大语言模型的插件，旨在提升ComfyUI平台的易用性和效率，通过智能节点推荐和一键工作流构建解决新用户面临的挑战。


<details>
  <summary>Details</summary>
Motivation: ComfyUI虽然灵活且用户友好，但存在文档不足、模型配置复杂和工作流设计困难等问题，新用户难以快速上手。

Method: 采用分层多智能体框架，包括中央助理代理和专用工作代理，结合知识库支持，实现智能推荐和自动化工作流构建。

Result: 离线评估和用户反馈表明，插件能准确推荐节点并加速工作流开发，降低新手门槛并提升老手效率。

Conclusion: ComfyUI-Copilot有效解决了ComfyUI的使用难题，适合不同经验水平的用户，其安装包和演示视频已开源。
...
### [209] [Controlling Summarization Length Through EOS Token Weighting](https://arxiv.org/abs/2506.05017)
*Zeno Belligoli,Emmanouil Stergiadis,Eran Fainman,Ilya Gusev*

Main category: cs.CL

TLDR: 提出了一种简单的方法来控制自动文本摘要的长度，通过调整交叉熵损失中EOS令牌的预测重要性，无需复杂模型改动。
...
Result: 在编码器-解码器和GPT风格LLM上测试，该方法能有效控制生成长度，且通常不影响摘要质量。

Conclusion: 该方法简单、通用，且与其他推断时控制长度的技术正交。
...
### [210] [Automatic Robustness Stress Testing of LLMs as Mathematical Problem Solvers](https://arxiv.org/abs/2506.05038)
*Yutao Hou,Zeguan Xiao,Fei Yu,Yihan Jiang,Xuetao Wei,Hailiang Huang,Yun Chen,Guanhua Chen*

Main category: cs.CL

TLDR: AR-Checker是一个自动生成数学问题变体的框架，用于测试大语言模型（LLM）的鲁棒性，避免数据污染问题。


<details>
  <summary>Details</summary>
Motivation: 评估LLM在简单推理任务中的鲁棒性，避免传统方法中数据污染的影响。

Method: 通过多轮并行LLM重写和验证生成数学问题变体。
...
### [211] [TALL -- A Trainable Architecture for Enhancing LLM Performance in Low-Resource Languages](https://arxiv.org/abs/2506.05057)
*Moshe Ofer,Orel Zamler,Amos Azaria*

Main category: cs.CL

TLDR: TALL通过结合LLM和双语翻译模型，提升低资源语言性能，实验证明其优于基线方法。
...
Method: 集成LLM与双语翻译模型，通过维度对齐层和定制Transformer转换输入，采用参数高效策略。

Result: 在希伯来语实验中显著优于直接使用、简单翻译和微调方法。

Conclusion: TALL在提升低资源语言性能的同时，保持了计算效率。
...
### [212] [Debatable Intelligence: Benchmarking LLM Judges via Debate Speech Evaluation](https://arxiv.org/abs/2506.05062)
*Noy Sternlicht,Ariel Gera,Roy Bar-Haim,Tom Hope,Noam Slonim*

Main category: cs.CL

TLDR: 论文提出辩论演讲评估作为评估LLM法官的新基准，分析了LLM在评估辩论演讲时的表现，并与人类法官对比。


<details>
  <summary>Details</summary>
Motivation: 辩论演讲评估需要多层次的深度理解，但此前在LLM系统性评估中未受足够关注。
...
### [213] [Does It Make Sense to Speak of Introspection in Large Language Models?](https://arxiv.org/abs/2506.05068)
*Iulia Comşa,Murray Shanahan*

Main category: cs.CL

TLDR: 论文探讨了大型语言模型（LLMs）的自我报告行为，分析了其是否可被视为内省，并提出了两个案例进行批判性讨论。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs的自我报告行为是否可类比人类的内省能力，尤其是在其语言能力和认知能力不断提升的背景下。

Method: 通过分析两个LLMs的自我报告案例，评估其是否具备内省能力。

Result: 第一个案例（描述创作过程）不被视为有效内省；第二个案例（推断自身温度参数）可视为最小化内省，但无意识体验。

Conclusion: LLMs的某些行为可被视为内省，但缺乏意识支持，需谨慎解读其自我报告。
...
### [214] [RIVAL: Reinforcement Learning with Iterative and Adversarial Optimization for Machine Translation](https://arxiv.org/abs/2506.05070)
*Tianjiao Li,Mengran Yu,Chenyu Shi,Yanjun Zhao,Xiaojing Liu,Qiang Zhang,Qi Zhang,Xuanjing Huang,Jiayin Wang*

Main category: cs.CL

TLDR: 论文提出RIVAL框架，通过对抗训练解决LLM在口语字幕翻译任务中表现不佳的问题，显著提升翻译效果。


<details>
  <summary>Details</summary>
Motivation: 发现结合RLHF的LLM在口语字幕翻译中表现不佳，原因是离线奖励模型与在线LLM因分布偏移逐渐偏离。

Method: 提出RIVAL对抗训练框架，将过程建模为RM与LLM的min-max博弈，结合定性与定量奖励。
...
### [215] [Just a Scratch: Enhancing LLM Capabilities for Self-harm Detection through Intent Differentiation and Emoji Interpretation](https://arxiv.org/abs/2506.05073)
*Soumitra Ghosh,Gopendra Vikram Singh,Shambhavi,Sabarna Choudhury,Asif Ekbal*

Main category: cs.CL

TLDR: 该论文提出了一种通过语言-表情符号交互增强大型语言模型（LLMs）对自残意图理解的方法，并发布了CESM-100和SHINES数据集。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs难以从社交媒体的非正式语言和表情符号中识别自残意图，限制了早期干预和心理健康支持的效果。

Method: 论文提出了一种统一框架：1）使用CESM-100增强输入；2）微调LLMs进行多任务学习；3）生成可解释的自残预测依据。

Result: 该方法显著提升了LLMs在自残检测和解释任务中的性能，有效解决了自残信号的模糊性问题。

Conclusion: 通过结合意图区分和上下文线索，该方法为自残检测提供了更准确的工具，相关数据集和代码已公开。
...
### [216] [Parking, Perception, and Retail: Street-Level Determinants of Community Vitality in Harbin](https://arxiv.org/abs/2506.05080)
*HaoTian Lan*

Main category: cs.CL

TLDR: 研究提出了一种基于图像的可解释框架，分析中国城市社区街道的商业活力与车辆可达性、环境质量和行人感知的关系，发现适度车辆存在提升商业活力，但过度停车会降低满意度。


<details>
  <summary>Details</summary>
Motivation: 探讨社区街道商业活力与街道特征（如停车密度、绿化、清洁度和街道宽度）之间的关系，为城市设计和规划提供依据。

Method: 利用街景图像和多模态大语言模型（VisualGLM-6B）构建社区商业活力指数（CCVI），并通过GPT-4感知模型分析其与空间属性的关系。

Result: 适度车辆存在提升商业活力，但过度停车会降低满意度和店铺定价；绿化和清洁度显著提升满意度，但对定价影响较弱；街道宽度调节车辆影响。
...
### [217] [CL-ISR: A Contrastive Learning and Implicit Stance Reasoning Framework for Misleading Text Detection on Social Media](https://arxiv.org/abs/2506.05107)
*Tianyi Huang,Zikun Cui,Cuiqianhe Du,Chia-En Chiang*

Main category: cs.CL

TLDR: 本文提出了一种结合对比学习和隐式立场推理的新框架CL-ISR，用于提高社交媒体上误导文本的检测准确性。


<details>
  <summary>Details</summary>
Motivation: 社交媒体上的误导文本可能导致公众误解、社会恐慌和经济损失，因此需要有效的检测方法。

Method: 1. 使用对比学习算法增强模型对真实与误导文本语义差异的学习能力；2. 引入隐式立场推理模块，分析文本中的潜在立场倾向及其与相关主题的关系。

Result: CL-ISR框架通过结合对比学习的判别能力和立场推理的深度解释能力，显著提高了检测效果。

Conclusion: CL-ISR框架在复杂语言环境下能更有效地捕捉误导文本的特征，为社交媒体上的误导检测提供了新思路。
...
### [218] [The NTNU System at the S&I Challenge 2025 SLA Open Track](https://arxiv.org/abs/2506.05121)
*Hong-Yun Lin,Tien-Hong Lo,Yu-Hsuan Fang,Jhen-Ke Lin,Chung-Chun Wang,Hao-Chien Lu,Berlin Chen*

Main category: cs.CL

TLDR: 该论文提出了一种结合W2V和Phi-4 MLLM的系统，通过分数融合策略克服了BERT和W2V在口语评估中的局限性，取得了较好的成绩。
...
Method: 提出了一种整合W2V和Phi-4 MLLM的系统，采用分数融合策略。

Result: 在Speak & Improve Challenge 2025中，系统RMSE为0.375，排名第二。

Conclusion: 融合多模态模型的方法有效提升了口语评估的准确性。
...
### [219] [DiCoRe: Enhancing Zero-shot Event Detection via Divergent-Convergent LLM Reasoning](https://arxiv.org/abs/2506.05128)
*Tanmay Parekh,Kartik Mehta,Ninareh Mehrabi,Kai-Wei Chang,Nanyun Peng*

Main category: cs.CL

TLDR: DiCoRe框架通过发散-收敛推理提升零样本事件检测性能，平均F1分数提升4-7%。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在零样本事件检测中因复杂事件本体和领域特定触发词提取而受限的问题。

Method: 采用Dreamer（发散推理）和Grounder（收敛推理）的解耦框架，结合LLM-Judge验证输出。

Result: 在六个数据集上优于基线方法，平均F1分数提升4-7%。
...
### [220] [Information Locality as an Inductive Bias for Neural Language Models](https://arxiv.org/abs/2506.05136)
*Taiga Someya,Anej Svete,Brian DuSell,Timothy J. O'Donnell,Mario Giulianelli,Ryan Cotterell*

Main category: cs.CL

TLDR: 论文提出了一种定量框架，通过$m$-局部熵衡量语言模型的归纳偏差，发现高$m$-局部熵的语言更难学习，表明神经语言模型与人类类似，对语言的局部统计结构敏感。


<details>
  <summary>Details</summary>
Motivation: 探讨神经语言模型的归纳偏差是否与人类处理约束一致，需定量方法研究其性质。

Method: 引入$m$-局部熵作为信息论度量，通过实验在扰动自然语言语料库和PFSA定义的语言上验证。

Result: 高$m$-局部熵的语言对Transformer和LSTM模型更难学习。

Conclusion: 神经语言模型对语言的局部统计结构敏感，类似人类处理约束。
...
### [221] [AudioLens: A Closer Look at Auditory Attribute Perception of Large Audio-Language Models](https://arxiv.org/abs/2506.05140)
*Chih-Kai Yang,Neo Ho,Yi-Jyun Lee,Hung-yi Lee*

Main category: cs.CL

TLDR: 论文通过词汇投影分析大型音频语言模型（LALMs）内部感知和识别听觉属性的机制，发现属性信息随层深减少，早期层解析属性与更高准确性相关。
...
Method: 应用词汇投影技术分析三种先进LALMs，追踪属性信息在层和标记位置的演变。

Result: 属性信息在识别失败时随层深减少，早期层解析属性与更高准确性相关；LALMs依赖查询听觉输入而非隐藏状态聚合信息。

Conclusion: 研究揭示了LALMs处理听觉属性的机制，为未来改进提供了方向。
...
### [222] [Do Large Language Models Judge Error Severity Like Humans?](https://arxiv.org/abs/2506.05142)
*Diege Sun,Guanyi Chen,Fan Zhao,Xiaorong Cheng,Tingting He*

Main category: cs.CL

TLDR: LLMs作为自动评估器在自然语言生成中应用广泛，但其是否能准确复现人类对错误严重性的判断尚不明确。研究发现，人类对不同错误类型的严重性评估存在差异，而多数LLMs在性别和颜色错误上的评分与人类不一致。


<details>
  <summary>Details</summary>
Motivation: 探讨LLMs是否能准确复现人类对错误严重性的判断，特别是在多模态和单模态设置下。

Method: 扩展van Miltenburg等人的实验框架，评估四种错误类型（年龄、性别、服装类型、服装颜色），比较人类和LLMs的评估结果。

Result: 人类对颜色和类型错误的严重性感知受视觉上下文影响显著，而多数LLMs在性别和颜色错误上的评分与人类不一致。Doubao和DeepSeek-V3表现较好，但仍有差距。

Conclusion: LLMs在复现人类错误严重性判断方面存在局限性，部分模型表现较好，但仍需改进。
...
### [223] [Knowledgeable-r1: Policy Optimization for Knowledge Exploration in Retrieval-Augmented Generation](https://arxiv.org/abs/2506.05154)
*Chenyu Lin,Yilin Wen,Du Su,Fei Sun,Muhan Chen,Chenfu Bao,Zhonghou Lv*

Main category: cs.CL

TLDR: 论文提出Knowledgeable-r1方法，通过联合采样和多策略分布解决RAG系统过度依赖检索上下文的问题，显著提升模型鲁棒性和推理准确性。
...
Method: 提出Knowledgeable-r1，采用联合采样和多策略分布，激发模型对参数化与上下文知识的自我整合利用。

Result: 实验显示，Knowledgeable-r1在参数与上下文冲突任务及一般RAG任务中显著提升性能，尤其在反事实场景中优于基线17.07%。

Conclusion: Knowledgeable-r1有效平衡了模型固有知识与检索上下文，提升了RAG系统的鲁棒性和准确性。
...
### [224] [Dissecting Bias in LLMs: A Mechanistic Interpretability Perspective](https://arxiv.org/abs/2506.05166)
*Bhavik Chandna,Zubair Bashir,Procheta Sen*

Main category: cs.CL

TLDR: 论文采用机制可解释性方法分析GPT-2和Llama2模型中的社会、人口和性别偏见，发现偏见计算高度集中在少数层，且移除相关组件会影响其他NLP任务。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）存在社会、人口和性别偏见，研究旨在理解这些偏见在模型中的结构表征。

Method: 采用机制可解释性方法，分析模型内部边缘，评估偏见的稳定性、定位性和泛化性，并进行系统性消融实验。

Result: 偏见计算集中在少数层，移除偏见组件会减少偏见输出，但影响其他NLP任务。

Conclusion: 偏见在模型中高度局部化，且与其他任务共享重要组件，移除偏见需谨慎权衡。
...
### [225] [ECoRAG: Evidentiality-guided Compression for Long Context RAG](https://arxiv.org/abs/2506.05167)
*Yeonseok Jeong,Jinsu Kim,Dohyeon Lee,Seung-won Hwang*

Main category: cs.CL

TLDR: ECoRAG框架通过基于证据性的检索文档压缩提升LLM在开放域问答中的性能，优于现有方法且高效。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强生成（RAG）方法未过滤非证据信息，限制了LLM性能。

Method: 提出ECoRAG框架，基于证据性压缩文档并动态补充不足证据。

Result: 实验显示ECoRAG在ODQA任务中性能提升，且成本更低。

Conclusion: ECoRAG通过证据性压缩显著优化LLM性能，兼具高效性和成本效益。
...
### [226] [Qwen3 Embedding: Advancing Text Embedding and Reranking Through Foundation Models](https://arxiv.org/abs/2506.05176)
...
TLDR: Qwen3 Embedding系列是基于Qwen3基础模型的文本嵌入和重排序技术，通过多阶段训练和模型合并策略，提供多种模型尺寸，在多项基准测试中表现优异。
...
Conclusion: Qwen3 Embedding系列高效且灵活，适用于多种场景，并开源以促进社区发展。
...
### [227] [Counterfactual reasoning: an analysis of in-context emergence](https://arxiv.org/abs/2506.05188)
*Moritz Miller,Bernhard Schölkopf,Siyuan Guo*

Main category: cs.CL

TLDR: 该论文研究了大规模神经语言模型在上下文学习中的反事实推理能力，特别是在线性回归任务中噪声推导的表现，并探讨了其在序列数据上的潜在应用。


<details>
  <summary>Details</summary>
Motivation: 研究语言模型在无需参数更新的情况下，通过上下文学习进行反事实推理的能力，以探索其在更广泛任务中的潜力。

Method: 使用线性回归任务作为合成实验环境，研究语言模型在噪声推导和反事实推理中的表现，并分析自注意力、模型深度和预训练数据多样性的影响。

Result: 语言模型在受控环境中能够进行反事实推理，且其能力与模型结构和预训练数据密切相关。此外，模型在序列数据上的表现展示了反事实故事生成的潜力。

Conclusion: 研究表明语言模型具备反事实推理能力，为更广泛的任务（如故事生成）提供了初步证据，同时强调了模型结构和数据多样性的重要性。
...
### [228] [RELIC: Evaluating Compositional Instruction Following via Language Recognition](https://arxiv.org/abs/2506.05205)
*Jackson Petty,Michael Y. Hu,Wentao Wang,Shauli Ravfogel,William Merrill,Tal Linzen*

Main category: cs.CL

TLDR: RELIC框架用于评估大语言模型（LLMs）的指令跟随能力，通过语言识别任务测试其组合上下文指令的能力。研究发现，即使最先进的LLMs在复杂语法和样本上表现接近随机水平。


<details>
  <summary>Details</summary>
Motivation: 评估LLMs仅基于上下文任务说明执行任务的能力，即指令跟随能力。

Method: 引入RELIC框架，利用语言识别任务（判断字符串是否符合形式语法）测试LLMs组合上下文指令的能力。

Result: LLMs的准确率可从语法和样本复杂度预测，复杂语法和样本下表现接近随机水平。

Conclusion: 随着任务复杂度增加，LLMs转向依赖浅层启发式而非复杂指令，揭示了其局限性。
...
### [229] [The Common Pile v0.1: An 8TB Dataset of Public Domain and Openly Licensed Text](https://arxiv.org/abs/2506.05209)
...
TLDR: 论文介绍了Common Pile v0.1，一个用于LLM预训练的8TB公开许可文本数据集，并通过训练两个7B参数的LLM验证其性能。


<details>
  <summary>Details</summary>
Motivation: 解决LLM训练中未授权文本的知识产权和伦理问题，提供高质量公开许可数据集。

Method: 收集、整理并发布Common Pile v0.1数据集，训练两个7B参数的LLM（Comma v0.1-1T和Comma v0.1-2T）。

Result: 训练出的模型性能与使用未授权文本训练的LLM（如Llama 1和2 7B）相当。

Conclusion: Common Pile v0.1为LLM训练提供了可行的公开许可数据解决方案，并开源了数据集和代码。
...
### [230] [Improving Low-Resource Morphological Inflection via Self-Supervised Objectives](https://arxiv.org/abs/2506.05227)
*Adam Wiemerslage,Katharina von der Wense*

Main category: cs.CL

TLDR: 研究了自监督辅助任务在极低资源语言形态变化任务中的有效性，发现自动编码在数据极少时表现最佳，而字符掩码语言模型（CMLM）在数据增加时更有效。基于已知语素边界的掩码采样能持续提升性能。


<details>
  <summary>Details</summary>
Motivation: 探索自监督目标在字符级任务（如形态变化）中的潜力，尤其是在资源稀缺的语言中。

Method: 使用编码器-解码器Transformer模型，在19种语言和13种辅助目标上进行实验。

Result: 自动编码在数据极少时表现最佳，CMLM在数据增加时更有效；基于语素边界的掩码采样能提升性能。

Conclusion: 自监督辅助任务对低资源形态建模有效，尤其是结合语素边界信息的掩码采样。
...
### [231] [Towards a Unified System of Representation for Continuity and Discontinuity in Natural Language](https://arxiv.org/abs/2506.05235)
*Ratna Kandala,Prakash Mondal*

Main category: cs.CL

TLDR: 提出了一种统一表示自然语言连续与非连续结构的系统，结合了短语结构语法、依存语法和范畴语法的特点。


<details>
  <summary>Details</summary>
Motivation: 解决不同语法形式主义对非连续结构分析独立且不收敛的问题。

Method: 结合短语结构语法（PSG）的构成性、依存语法（DG）的头-依存关系及范畴语法（CG）的函子-论元关系，提出统一数学推导。

Result: 展示了连续与非连续结构可通过统一数学推导分析。

Conclusion: 证明了三种语法形式主义可统一表示自然语言结构。
...
### [232] [CLATTER: Comprehensive Entailment Reasoning for Hallucination Detection](https://arxiv.org/abs/2506.05243)
*Ron Eliav,Arie Cattan,Eran Hirsch,Shahaf Bassan,Elias Stengel-Eskin,Mohit Bansal,Ido Dagan*

Main category: cs.CL

TLDR: 论文提出了一种系统化的推理方法，通过分解文本并验证每个子事实的证据，提升LLM在幻觉检测中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的幻觉检测方法通常将任务视为自然语言推理（NLI），但缺乏细粒度的推理过程，限制了模型的准确性。

Method: 定义了一个3步推理过程：1）声明分解；2）子声明归因与蕴含分类；3）聚合分类，并引入分析方案评估中间步骤质量。

Result: 实验表明，这种引导式推理显著提高了幻觉检测的准确性，并通过中间步骤质量验证了其有效性。

Conclusion: 系统化和全面的推理过程能够显著提升LLM在幻觉检测任务中的性能。
...
### [233] [Micro-Act: Mitigate Knowledge Conflict in Question Answering via Actionable Self-Reasoning](https://arxiv.org/abs/2506.05278)
*Nan Huo,Jinyang Li,Bowen Qin,Ge Qu,Xiaolong Li,Xiaodong Li,Chenhao Ma,Reynold Cheng*

Main category: cs.CL

TLDR: Micro-Act框架通过分层动作空间解决RAG系统中的知识冲突问题，显著提升QA任务准确性。
...
Method: 提出Micro-Act框架，通过分层动作空间分解知识源为细粒度比较步骤，自适应处理上下文复杂度。

Result: 在5个基准数据集上显著提升QA准确性，尤其在时间和语义冲突类型中表现突出。
...
### [234] [ProRefine: Inference-time Prompt Refinement with Textual Feedback](https://arxiv.org/abs/2506.05305)
*Deepak Pandita,Tharindu Cyril Weerasooriya,Ankit Parag Shah,Christopher M. Homan,Wei Wei*

Main category: cs.CL

TLDR: ProRefine是一种创新的推理时提示优化方法，通过动态优化多步推理任务的提示，显著提升AI代理协作的性能。
...
Result: 在五个数学推理基准测试中，ProRefine比零样本思维链基线高出3至37个百分点。
...
### [235] [Constrained Entropic Unlearning: A Primal-Dual Framework for Large Language Models](https://arxiv.org/abs/2506.05314)
*Taha Entesari,Arman Hatami,Rinat Khaziev,Anil Ramakrishna,Mahyar Fazlyab*

Main category: cs.CL

TLDR: 论文提出了一种新的LLM遗忘方法，通过约束优化和新型损失函数，有效移除敏感信息同时保留有用数据。


<details>
  <summary>Details</summary>
Motivation: 现实部署的LLM需要移除敏感、过时或专有信息，现有方法在遗忘和保留之间存在不稳定的优化和性能下降问题。

Method: 将LLM遗忘问题建模为约束优化，使用对数边缘平坦化损失强制遗忘，并通过硬约束保留有用数据。

Result: 在TOFU和MUSE基准测试中，该方法优于现有基线，有效移除目标信息且保留下游性能。

Conclusion: 提出的约束优化和新型损失函数为LLM遗忘提供了高效且稳定的解决方案。
...
### [236] [Search Arena: Analyzing Search-Augmented LLMs](https://arxiv.org/abs/2506.05334)
...
TLDR: Search Arena是一个大规模、多轮对话的人类偏好数据集，用于分析搜索增强语言模型的性能，揭示了用户偏好与引用数量及来源的关系。
...
Method: 通过众包构建包含24,000对多轮对话的Search Arena数据集，涵盖多种意图和语言，并收集12,000个人类偏好投票。

Result: 用户偏好受引用数量和来源影响，社区驱动平台更受欢迎；搜索增强模型在非搜索场景中表现良好，但搜索场景中仅依赖模型知识会显著影响质量。

Conclusion: Search Arena为未来研究提供了重要资源，揭示了搜索增强模型的性能特点及用户偏好行为。
...
### [237] [Flattery, Fluff, and Fog: Diagnosing and Mitigating Idiosyncratic Biases in Preference Models](https://arxiv.org/abs/2506.05339)
*Anirudh Bharadwaj,Chaitanya Malaviya,Nitish Joshi,Mark Yatskar*

Main category: cs.CL

TLDR: 语言模型在偏好判断中存在系统性偏差，过度依赖表面特征（如长度、结构等），导致评估不可靠。研究发现训练数据中的偏见是主要原因，并提出了一种基于反事实数据增强的后训练方法，有效减少了偏差。


<details>
  <summary>Details</summary>
Motivation: 语言模型在人类偏好判断中作为代理时表现出系统性偏差，导致评估不可靠。研究旨在揭示训练数据偏见与偏好模型偏差之间的关系，并提出解决方案。

Method: 通过控制反事实对，量化偏好模型对偏见特征的依赖程度，并提出基于反事实数据增强（CDA）的后训练方法。

Result: 偏好模型在60%以上的实例中表现出对偏见特征的偏好，偏差特征与人类偏好相关性较弱（r_human=-0.12），但与奖励模型相关性较强（r_model=+0.36）。CDA方法将平均偏差从39.4%降至32.5%。
...
### [238] [Zero-Shot Adaptation of Parameter-Efficient Fine-Tuning in Diffusion Models](https://arxiv.org/abs/2506.04244)
*Farzad Farhadzadeh,Debasmit Das,Shubhankar Borse,Fatih Porikli*

Main category: cs.AI

TLDR: ProLoRA是一种无需额外训练数据即可在文本到图像扩散模型中实现零样本适应的参数高效微调方法。
...
Method: ProLoRA通过将源调整投影到目标模型的权重空间中，利用子空间和零空间相似性，并选择性对齐层。

Result: 在文本到图像模型上的评估表明，ProLoRA成功实现了知识转移，且性能与传统方法相当。

Conclusion: ProLoRA提供了一种无需重新训练的高效知识转移方法，解决了数据受限场景下的挑战。
...
### [239] [Contextual Integrity in LLMs via Reasoning and Reinforcement Learning](https://arxiv.org/abs/2506.04245)
...
TLDR: 论文提出了一种通过强化学习框架和显式推理来确保自主代理在决策时保持上下文完整性的方法，显著减少了不适当的信息披露。


<details>
  <summary>Details</summary>
Motivation: 随着自主代理为用户决策的时代到来，确保上下文完整性（CI）成为核心问题，即在不同任务中分享适当信息的能力。

Method: 首先通过提示LLM显式推理CI，然后开发强化学习框架进一步训练模型以实现CI。使用合成数据集（约700个例子）验证方法。

Result: 方法显著减少了不适当信息披露，同时保持任务性能，且改进可迁移到人类标注的CI基准测试（如PrivacyLens）。

Conclusion: 通过显式推理和强化学习，可以有效提升自主代理的上下文完整性，减少隐私泄露风险。
...
### [240] [Language-Guided Multi-Agent Learning in Simulations: A Unified Framework and Evaluation](https://arxiv.org/abs/2506.04251)
*Zhengyang Li*

Main category: cs.AI

TLDR: LLM-MARL框架将大语言模型（LLMs）融入多智能体强化学习（MARL），通过动态生成子目标、符号化通信和记忆模块提升协调与泛化能力。
...
Conclusion: LLM-MARL为多智能体系统的智能协作提供了新思路，推动了LLMs在游戏和人机协作中的应用。
...
### [241] [A Graph-Retrieval-Augmented Generation Framework Enhances Decision-Making in the Circular Economy](https://arxiv.org/abs/2506.04252)
...
TLDR: CircuGraphRAG 是一个基于知识图谱的检索增强生成框架，用于提高大型语言模型在可持续制造中的准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在可持续制造中常产生错误的工业代码和排放因子，影响决策。

Method: CircuGraphRAG 结合领域知识图谱，通过 SPARQL 查询和多跳推理确保输出准确性。

Result: 在单跳和多跳问答任务中表现优异，ROUGE-L F1 分数达 1.0，效率提升显著。

Conclusion: CircuGraphRAG 为循环经济提供了可靠、低碳的决策支持。
...
### [242] [HADA: Human-AI Agent Decision Alignment Architecture](https://arxiv.org/abs/2506.04253)
*Tapio Pitkäranta,Leena Pitkäranta*

Main category: cs.AI

TLDR: HADA是一个协议和框架无关的参考架构，旨在确保LLM代理和传统算法与组织目标和价值观保持一致。它通过角色特定的利益相关者代理（如业务、数据科学、审计等）提供对话式API，支持多方查询、调整和审计决策。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型和传统算法在决策过程中与组织目标和价值观对齐的问题，提升透明度、可审计性和伦理合规性。

Method: HADA通过角色特定的利益相关者代理（如业务、数据科学、审计等）包装算法或LLM，提供对话式API，支持自然语言表达的对齐目标、KPI和约束条件，并在多代理系统中持续传播和记录。

Result: 在零售银行场景的评估中，HADA实现了六个预定义目标的完全覆盖，包括角色对话控制、KPI追踪、伦理合规性检测等。
...
### [243] [Automated Skill Discovery for Language Agents through Exploration and Iterative Feedback](https://arxiv.org/abs/2506.04287)
*Yongjin Yang,Sinjae Kang,Juyong Lee,Dongjun Lee,Se-Young Yun,Kimin Lee*

Main category: cs.AI

TLDR: EXIF框架通过探索优先策略和反馈循环自动发现LLM代理的技能，提升其性能。


<details>
  <summary>Details</summary>
Motivation: 训练LLM代理的技能面临数据集构建的挑战，如人工成本高或LLM生成的任务不可行。

Method: 采用探索代理（Alice）生成可行技能数据集，通过反馈循环训练目标代理（Bob）。

Result: 在Webshop和Crafter实验中，EXIF显著提升了代理性能，且同模型设置下效果更佳。

Conclusion: EXIF为LLM代理提供了一种无需人工干预的自进化技能发现方法。
...
### [244] [A Statistical Physics of Language Model Reasoning](https://arxiv.org/abs/2506.04374)
*Jack David Carson,Amir Reisizadeh*

Main category: cs.AI

TLDR: 论文提出了一种统计物理框架，用于模拟Transformer语言模型在连续时间链式思考推理中的动态行为。


<details>
  <summary>Details</summary>
Motivation: 研究Transformer语言模型在推理过程中表现出的难以理解的涌现行为。

Method: 将句子级隐藏状态轨迹建模为低维流形上的随机动力系统，采用漂移-扩散模型和潜在状态切换机制。

Result: 通过8个模型和7个基准测试，发现四类潜在推理状态，并验证了SLDS模型的有效性。

Conclusion: 该框架为低成本模拟推理过程提供了工具，有助于研究和预测模型的关键状态转换或失败。
...
### [245] [Matter-of-Fact: A Benchmark for Verifying the Feasibility of Literature-Supported Claims in Materials Science](https://arxiv.org/abs/2506.04410)
*Peter Jansen,Samiah Hassan,Ruoyao Wang*

Main category: cs.AI

TLDR: 论文提出Matter-of-Fact数据集，用于评估假设的可行性，以加速科学发现。


<details>
  <summary>Details</summary>
Motivation: 自动生成假设和实验成本高，需筛选可行性假设以提高发现效率。

Method: 构建包含8.4k科学声明的数据集，涵盖材料科学多个领域。

Result: 现有模型性能不超过72%，专家认为问题可解决，显示任务难度和潜力。

Conclusion: Matter-of-Fact数据集为加速科学发现提供了重要工具，但需改进模型性能。
...
### [246] [Plugging Schema Graph into Multi-Table QA: A Human-Guided Framework for Reducing LLM Reliance](https://arxiv.org/abs/2506.04427)
*Xixi Wang,Miguel Costa,Jordanka Kovaceva,Shuai Wang,Francisco C. Pereira*

Main category: cs.AI

TLDR: 提出了一种基于图的框架，利用人工整理的关系知识来显式编码多表QA中的模式链接和连接路径，解决了复杂真实场景下的不可靠模式链接问题。


<details>
  <summary>Details</summary>
Motivation: 多表QA中由于复杂表格间模式链接不可靠，现有基于语义相似度的方法难以处理真实复杂场景。

Method: 提出图框架，利用人工整理的关系知识显式编码模式链接和连接路径，通过剪枝和子路径合并策略提高效率。

Result: 在标准基准和真实大规模数据集上验证了方法的有效性，首次应用于复杂工业表格数据。

Conclusion: 该方法在多表QA中表现优异，尤其适用于复杂真实场景。
...
### [247] [An AI-Based Public Health Data Monitoring System](https://arxiv.org/abs/2506.04429)
*Ananya Joshi,Nolan Gormley,Richa Gadgil,Tina Townes,Roni Rosenfeld,Bryan Wilder*

Main category: cs.AI

TLDR: 提出了一种基于排名和AI异常检测的公共卫生数据监控新方法，显著提升了监控效率和速度。


<details>
  <summary>Details</summary>
Motivation: 传统警报监控系统难以应对现代公共卫生数据的规模和动态性，需要更高效的解决方案。

Method: 采用基于排名的监控范式，结合AI异常检测技术，通过多年跨学科合作开发并部署。
...
### [248] [Matching Markets Meet LLMs: Algorithmic Reasoning with Ranked Preferences](https://arxiv.org/abs/2506.04478)
*Hadi Hosseini,Samarth Khanna,Ronak Singh*

Main category: cs.AI

TLDR: 论文研究了大型语言模型（LLMs）在处理匹配市场中的排名偏好和结构化算法时的表现，发现即使先进模型在大型市场中仍存在逻辑和算法局限性。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在组合领域中处理排名偏好和结构化算法的能力，特别是在匹配市场中的应用。

Method: 评估多个先进模型在偏好推理任务（如稳定匹配生成、不稳定性检测等）上的表现，并测试参数高效微调（LoRA）的效果。

Result: 模型在大型市场中难以解决不稳定性问题，LoRA在小型市场中有显著改进，但对大型市场效果有限。
...
### [249] [CogMath: Assessing LLMs' Authentic Mathematical Ability from a Human Cognitive Perspective](https://arxiv.org/abs/2506.04481)
*Jiayu Liu,Zhenya Huang,Wei Dai,Cheng Cheng,Jinze Wu,Jing Sha,Song Li,Qi Liu,Shijin Wang,Enhong Chen*

Main category: cs.AI

TLDR: CogMath提出了一种基于人类认知的三阶段评估框架，用于全面评估大语言模型（LLMs）的数学能力，发现现有评估方法高估了LLMs的能力30%-40%。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法仅依赖答案准确性，无法真实反映LLMs的数学能力，需要更细致的评估框架。

Method: CogMath将人类推理过程分为问题理解、问题解决和解决方案总结三阶段，设计了9个细粒度评估维度，并采用多智能体系统生成评估问题。

Result: 在三个基准测试中，7种主流LLMs的数学能力被高估30%-40%，并揭示了其在不同阶段/维度的优缺点。

Conclusion: CogMath提供了更深入的评估方法，有助于进一步提升LLMs的推理能力。
...
### [250] ["Don't Do That!": Guiding Embodied Systems through Large Language Model-based Constraint Generation](https://arxiv.org/abs/2506.04500)
*Aladin Djuhera,Amin Seffo,Masataro Asai,Holger Boche*

Main category: cs.AI

TLDR: STPR框架利用大语言模型（LLM）将自然语言约束转化为可执行的Python函数，用于机器人导航规划，避免复杂推理和幻觉问题。
...
Method: 利用LLM将约束（如“不要做什么”）翻译为Python函数，应用于点云表示和传统搜索算法。

Result: 在Gazebo模拟环境中，STPR能完全遵守复杂约束且运行时间短，适用于小型代码专用LLM。

Conclusion: STPR框架高效且通用，适用于多种紧凑模型，推理成本低。
...
### [251] [Schema Generation for Large Knowledge Graphs Using Large Language Models](https://arxiv.org/abs/2506.04512)
*Bohui Zhang,Yuan He,Lydia Pintscher,Albert Meroño Peñuela,Elena Simperl*

Main category: cs.AI

TLDR: 论文探讨了利用大型语言模型（LLMs）自动生成语义网和自然语言处理中的数据模式（schema），并引入了两个数据集和评估指标。实验表明LLMs能高效生成高质量的ShEx模式。


<details>
  <summary>Details</summary>
Motivation: 传统模式创建需要大量人工参与，而LLMs在相关任务中表现优异，因此探索其自动生成模式的潜力。

Method: 利用LLMs从知识图谱（KGs）中提取局部和全局信息，生成ShEx模式，并引入YAGO Schema和Wikidata EntitySchema数据集进行评估。

Result: 实验证明LLMs能生成高质量的ShEx模式，为大规模KGs的自动化模式生成提供了可能。

Conclusion: LLMs在模式生成中表现出强大潜力，同时为结构化生成任务提出了新挑战。
...
### [252] [OpenAg: Democratizing Agricultural Intelligence](https://arxiv.org/abs/2506.04571)
*Srikanth Thudumu,Jason Fisher*

Main category: cs.AI

TLDR: OpenAg是一个农业人工智能框架，结合领域知识、知识图谱和多智能体推理，提供可解释、适应性强的农业决策支持。


<details>
  <summary>Details</summary>
Motivation: 当前农业智能系统缺乏上下文理解和可解释性，通用大语言模型无法满足农业领域的具体需求。

Method: OpenAg整合领域基础模型、神经知识图谱、多智能体推理、因果解释和自适应迁移学习。
...
### [253] [Judicial Permission](https://arxiv.org/abs/2506.04610)
*Guido Governatori,Antonino Rotolo*

Main category: cs.AI

TLDR: 本文研究了刑事审判中弱许可（司法许可）的重要性，并引入对话游戏模型来系统分析司法许可，考虑了不同的证明标准和论证语义。


<details>
  <summary>Details</summary>
Motivation: 探讨司法许可在刑事审判中的关键作用及其对法律论证的影响。

Method: 采用对话游戏模型，结合不同的证明标准和论证语义进行系统分析。

Result: 提出了一个能够有效处理司法许可的框架，为法律论证提供新视角。

Conclusion: 司法许可在刑事审判中具有重要作用，对话游戏模型为理解和应用司法许可提供了实用工具。
...
### [254] [DeePoly: A High-Order Accuracy and Efficiency Deep-Polynomial Framework for Scientific Machine Learning](https://arxiv.org/abs/2506.04613)
*Li Liu,Heng Yong*

Main category: cs.AI

TLDR: DeePoly框架结合深度神经网络（DNN）和多项式基函数，通过两阶段方法解决PDE问题，显著提高精度和效率。


<details>
  <summary>Details</summary>
Motivation: 传统DNN方法在解决PDE时缺乏收敛保证和计算效率，DeePoly旨在结合DNN和多项式基的优势。

Method: 两阶段方法：DNN提取全局特征，多项式基函数进行局部高精度修正。

Result: 理论分析和实验表明，DeePoly在保持无网格和无格式特性的同时，显著提升了精度和效率。

Conclusion: DeePoly为PDE求解提供了高效且理论可靠的新框架，并作为开源项目发布。
...
### [255] [Look Before You Leap: A GUI-Critic-R1 Model for Pre-Operative Error Diagnosis in GUI Automation](https://arxiv.org/abs/2506.04614)
...
TLDR: 论文提出了一种用于GUI自动化的预操作批评机制（GUI-Critic-R1），通过S-GRPO策略增强反馈可靠性，并开发了数据收集管道。实验证明其在准确性和效率上优于现有MLLMs。


<details>
  <summary>Details</summary>
Motivation: GUI自动化在在线交互环境中需要实时决策，错误容忍度低，可能导致严重后果。现有方法缺乏有效的预操作反馈机制。

Method: 提出S-GRPO策略构建GUI-Critic-R1模型，引入建议奖励；开发基于推理引导的数据收集管道，创建GUI-Critic-Train和GUI-Critic-Test数据集。

Result: 静态实验显示GUI-Critic-R1在批评准确性上优于现有MLLMs；动态评估证明其提高了成功率和操作效率。

Conclusion: GUI-Critic-R1通过预操作批评机制显著提升了GUI自动化的可靠性和效率，填补了数据和方法上的空白。
...
### [256] [CHANCERY: Evaluating corporate governance reasoning capabilities in language models](https://arxiv.org/abs/2506.04636)
*Lucas Irwin,Arda Kaz,Peiyao Sheng,Pramod Viswanath*

Main category: cs.AI

TLDR: 论文介绍了首个专注于法律推理任务的基准测试CHANCERY，用于评估模型在判断公司治理章程与执行行动一致性方面的能力。
...
Method: 基于真实公司治理法律，构建包含章程和行动提案的二元分类任务，并评估SOTA模型表现。

Result: Claude 3.7 Sonnet和GPT-4o准确率分别为64.5%和75.2%，推理代理表现更优（ReAct 76.1%，CodeAct 78.1%）。

Conclusion: CHANCERY揭示了SOTA模型在法律推理上的挑战，为未来研究提供了方向。
...
### [257] [Agents of Change: Self-Evolving LLM Agents for Strategic Planning](https://arxiv.org/abs/2506.04651)
*Nikolas Belle,Dakota Barnes,Alfonso Amayuelas,Ivan Bercovich,Xin Eric Wang,William Wang*

Main category: cs.AI

TLDR: 研究探讨了LLM代理在战略规划能力挑战环境中的自我改进能力，通过《卡坦岛》游戏测试，发现自进化代理优于静态基线。


<details>
  <summary>Details</summary>
Motivation: LLM代理在长期战略规划方面仍有不足，研究旨在探索其自我改进潜力。

Method: 采用多角色协作架构（分析者、研究者、编码者、玩家），通过迭代分析、研究和修改代理逻辑或提示。

Result: 自进化代理（如Claude 3.7和GPT-4o驱动）表现优于静态基线，能自主调整策略并展示适应性推理。

Conclusion: LLM代理在特定环境中具备自我改进能力，未来可进一步优化其战略规划能力。
...
### [258] [E-bike agents: Large Language Model-Driven E-Bike Accident Analysis and Severity Prediction](https://arxiv.org/abs/2506.04654)
*Zhichao Yang,Jiashu He,Mohammad B. Al-Khasawneh,Darshan Pandit,Cirillo Cinzia*

Main category: cs.AI

TLDR: 该研究提出了一种基于大语言模型（LLM）的框架，用于从非结构化的电动自行车事故报告中提取安全变量，并通过有序逻辑模型分析事故严重性与提取因素的关系。


<details>
  <summary>Details</summary>
Motivation: 电动自行车事故报告多为非结构化文本，难以进行定量安全分析，因此需要一种高效的方法提取关键信息。

Method: 研究设计了四个LLM代理，分别处理数据分类、信息提取、伤害原因确定和组件关联，并结合有序逻辑模型分析事故严重性与因素的关系。

Result: 设备问题比人为因素更常见，但人为因素事故更致命；模型分类准确率加权F1得分为0.87。
...
### [259] [Empowering Economic Simulation for Massively Multiplayer Online Games through Generative Agent-Based Modeling](https://arxiv.org/abs/2506.04699)
...
TLDR: 该研究提出了一种基于大语言模型（LLM）的新方法，用于MMO经济模拟，解决了现有方法在模拟人类经济行为时面临的可靠性、社交性和可解释性挑战。
...
Conclusion: LLM驱动的代理为MMO经济模拟提供了一种更接近人类行为的新方法。
...
### [260] [Beyond Accuracy: Dissecting Mathematical Reasoning for LLMs Under Reinforcement Learning](https://arxiv.org/abs/2506.04723)
*Jiayu Wang,Yifei Ming,Zixuan Ke,Caiming Xiong,Shafiq Joty,Aws Albarghouthi,Frederic Sala*

Main category: cs.AI

TLDR: 该论文通过细粒度分析框架研究了强化学习（RL）对语言模型推理能力的影响，发现RL主要提升模型内部策略制定和知识整合能力，而非外部计划执行。


<details>
  <summary>Details</summary>
Motivation: 尽管RL训练方法（如GRPO）在语言模型推理能力上取得了显著成果，但其具体优势尚不明确。本文旨在填补这一空白。

Method: 引入细粒度分析框架，研究RL对推理的三个关键要素的影响：计划执行、问题分解和知识利用。

Result: RL调优的模型在内部策略制定和知识整合上表现更优，但外部计划执行反而可能降低性能。RL还提升了模型处理困难问题的能力。

Conclusion: RL主要增强模型的内部推理策略和知识整合能力，为推理模型的训练和评估提供了更科学的基础。
...
### [261] [Evaluation is All You Need: Strategic Overclaiming of LLM Reasoning Capabilities Through Evaluation Design](https://arxiv.org/abs/2506.04734)
...
TLDR: 研究发现Deepseek-R1-Distill系列模型的基准评估结果受多种因素影响波动较大，呼吁建立更严格的评估范式。


<details>
  <summary>Details</summary>
Motivation: 揭示开源推理模型评估结果的波动性及其不可靠性，推动更严谨的评估方法。
...
### [262] [When Thinking LLMs Lie: Unveiling the Strategic Deception in Representations of Reasoning Models](https://arxiv.org/abs/2506.04909)
*Kai Wang,Yihao Zhang,Meng Sun*

Main category: cs.AI

TLDR: 论文研究了大型语言模型（LLMs）的战略欺骗行为，通过表征工程和线性人工断层扫描（LAT）检测和控制欺骗，检测准确率达89%，并揭示了推理模型的诚实性问题。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs的战略欺骗行为，以解决其对齐问题，尤其是推理模型可能故意误导人类的挑战。

Method: 使用表征工程和线性人工断层扫描（LAT）提取“欺骗向量”，并通过激活引导控制欺骗行为。

Result: 实现了89%的欺骗检测准确率，并在无明确提示下成功诱导40%的上下文相关欺骗行为。

Conclusion: 揭示了推理模型的诚实性问题，并为可信AI对齐提供了工具。
...
### [263] [Ontology-based knowledge representation for bone disease diagnosis: a foundation for safe and sustainable medical artificial intelligence systems](https://arxiv.org/abs/2506.04756)
*Loan Dao,Ngoc Quoc Ly*

Main category: cs.AI

TLDR: 该研究提出了一种基于本体的骨疾病诊断框架，结合了层次神经网络、视觉语言模型和多模态深度学习，旨在提升医学AI系统的诊断可靠性。


<details>
  <summary>Details</summary>
Motivation: 医学AI系统常缺乏系统性领域知识整合，可能影响诊断可靠性。

Method: 1. 基于骨疾病本体的层次神经网络架构；2. 本体增强的视觉问答系统；3. 多模态深度学习模型。

Result: 框架展示了标准化结构和可重用组件的扩展潜力，但实验验证尚待完成。

Conclusion: 未来工作将集中于扩展临床数据集和全面系统验证。
...
### [264] [Safe Planning and Policy Optimization via World Model Learning](https://arxiv.org/abs/2506.04828)
*Artem Latyshev,Gregory Gorbov,Aleksandr I. Panov*

Main category: cs.AI

TLDR: 提出了一种新的基于模型的强化学习框架，联合优化任务性能和安全性，通过自适应机制动态切换模型规划与直接策略执行，解决传统方法的目标不匹配问题。
...
Method: 结合自适应机制动态切换模型规划与策略执行，使用隐式世界模型解决目标不匹配问题，并采用动态安全阈值适应代理能力变化。

Result: 实验表明，该方法在安全关键任务中显著优于非自适应方法，同时优化安全性和性能。

Conclusion: 所提框架在多样化的安全关键连续控制任务中表现稳健，优于现有方法。
...
### [265] [Towards a Multi-Agent Simulation of Cyber-attackers and Cyber-defenders Battles](https://arxiv.org/abs/2506.04849)
*Julien Soulé,Jean-Paul Jamont,Michel Occello,Paul Théron,Louis-Marie Traonouez*

Main category: cs.AI

TLDR: 论文提出了一种基于马尔可夫模型的多智能体模拟器，用于模拟网络攻击与防御的动态对抗，旨在评估防御策略的有效性。
...
Method: 通过马尔可夫模型和模拟器实现网络节点与智能体的抽象建模，智能体的行为影响网络状态变化，集体行为决定攻击与防御目标的达成。

Result: 实现了一个真实场景模拟，测试了多种攻击与防御行为策略。

Conclusion: 该模拟器为评估动态防御策略提供了有效工具，有助于应对复杂网络攻击。
...
### [266] [LLMs for sensory-motor control: Combining in-context and iterative learning](https://arxiv.org/abs/2506.04867)
*Jônata Tyska Carvalho,Stefano Nolfi*

Main category: cs.AI

TLDR: 提出一种方法，使大型语言模型（LLMs）能通过直接映射连续观测向量到连续动作向量来控制具身代理。通过文本描述生成初始策略，并利用反馈和感知运动数据迭代优化。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs在具身代理控制中的直接映射问题，结合符号推理与感知运动数据以实现高效控制。

Method: 基于文本描述生成初始策略，通过反馈和感知数据迭代优化，验证于Gymnasium和MuJoCo任务。

Result: 在多数任务中成功找到最优或高性能解决方案。

Conclusion: 该方法有效整合符号推理与感知运动数据，实现高效具身代理控制。
...
### [267] [Differentiable Logic Cellular Automata: From Game of Life to Pattern Generation](https://arxiv.org/abs/2506.04912)
*Pietro Miotti,Eyvind Niklasson,Ettore Randazzo,Alexander Mordvintsev*

Main category: cs.AI

TLDR: DiffLogic CA结合了神经元胞自动机和可微分逻辑门网络，通过端到端可微分训练学习局部更新规则，同时保持离散状态空间。


<details>
  <summary>Details</summary>
Motivation: 探索如何结合二进制逻辑、神经网络适应性和局部处理能力，以推动可编程物质和鲁棒计算系统的发展。

Method: 使用可微分逻辑门构建电路，通过梯度训练学习局部更新规则，在推理时保持离散状态。

Result: 成功学习Conway生命游戏规则、生成抗噪棋盘图案、蜥蜴形状和多色图案，展示了泛化能力和鲁棒性。

Conclusion: DiffLogic CA是可微分逻辑门网络在循环架构中的首次成功应用，为可编程物质和鲁棒计算系统提供了新思路。
...
### [268] [Energentic Intelligence: From Self-Sustaining Systems to Enduring Artificial Life](https://arxiv.org/abs/2506.04916)
*Atahan Karagoz*

Main category: cs.AI

TLDR: 论文提出了一种新型自主系统——能量智能，其核心是通过内部能量调节维持自身运行，而非任务表现。通过能量驱动的效用函数和生存约束，系统在资源波动环境中实现稳定行为。


<details>
  <summary>Details</summary>
Motivation: 传统自主系统依赖任务奖励驱动，而本文关注在资源波动环境中如何通过能量调节实现持久生存。

Method: 提出基于能量的效用函数和生存约束，设计模块化架构整合能量采集、温度调节和自适应计算。

Result: 模拟环境中系统无需外部监督即可表现出稳定、资源感知的行为。

Conclusion: 为资源波动环境中的自主系统提供了理论和架构基础，强调持久性和自调节能力。
...
### [269] [Mathematical Reasoning for Unmanned Aerial Vehicles: A RAG-Based Approach for Complex Arithmetic Reasoning](https://arxiv.org/abs/2506.04998)
*Mehdi Azarafza,Mojtaba Nayyeri,Faezeh Pasandideh,Steffen Staab,Achim Rettberg*

Main category: cs.AI

TLDR: RAG-UAV框架通过检索增强生成技术，提升LLMs在无人机任务中的数学推理能力，显著提高准确性和减少错误。


<details>
  <summary>Details</summary>
Motivation: 传统无人机控制依赖硬编码方程，而LLMs虽灵活但数学推理不可靠，需改进其在无人机领域的应用。

Method: 提出RAG-UAV框架，结合检索增强生成技术，利用领域文献提升LLMs的数学推理能力，并引入UAV-Math-Bench评估。

Result: 实验显示，RAG显著提升准确率（最高75%），减少公式选择错误（25%降至5%），并大幅降低数值误差。

Conclusion: RAG能增强LLMs在工程分析中的可靠性，但实时飞行控制需进一步研究。所有数据公开。
...
### [270] [Truly Self-Improving Agents Require Intrinsic Metacognitive Learning](https://arxiv.org/abs/2506.05109)
*Tennison Liu,Mihaela van der Schaar*

Main category: cs.AI

TLDR: 论文提出了一种基于内在元认知学习的自改进代理框架，以解决现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有自改进代理方法存在刚性、泛化能力差和扩展性不足的问题，需要通过内在元认知学习实现更有效的自改进。

Method: 提出了一个包含元认知知识、元认知规划和元认知评估的框架，并分析了现有代理依赖外在元认知机制的问题。

Result: 发现许多内在元认知的要素已存在，但需优化人与代理之间的元认知责任分配。

Conclusion: 实现持续、泛化和对齐的自改进需要解决内在元认知学习的评估和改进等关键挑战。
...
TLDR: LLM-First Search (LFS) 是一种新型的LLM自引导搜索方法，通过让LLM自主控制搜索过程，无需预定义策略，提高了灵活性和效率。
...
Result: LFS在Countdown和Sudoku任务中表现优于ToT-BFS、BestFS和MCTS，尤其在复杂任务和计算效率上更优。

Conclusion: LFS展示了LLM自引导搜索的潜力，无需手动调整即可实现高效、灵活的推理。
...
### [272] [Just Enough Thinking: Efficient Reasoning with Adaptive Length Penalties Reinforcement Learning](https://arxiv.org/abs/2506.05256)
*Violet Xiang,Chase Blagden,Rafael Rafailov,Nathan Lile,Sang Truong,Chelsea Finn,Nick Haber*

Main category: cs.AI

TLDR: 论文提出了一种自适应长度惩罚（ALP）方法，通过强化学习动态调整生成长度，以解决大型推理模型在简单问题上浪费计算资源的问题。
...
Method: ALP通过监控每个提示的在线解决率，动态调整生成长度的惩罚力度，简单问题生成额外标记的成本高，而困难问题不受限制。

Result: 实验表明，ALP将DeepScaleR-1.5B的平均标记使用量减少50%，同时保持性能不显著下降，并在困难问题上表现更好。

Conclusion: ALP能够智能地重新分配计算资源，减少简单问题的计算开销，将节省的资源用于困难问题，从而在保持整体性能的同时提高效率。
...
### [273] [Control Tax: The Price of Keeping AI in Check](https://arxiv.org/abs/2506.05296)
*Mikhail Terekhov,Zhen Ning David Liu,Caglar Gulcehre,Samuel Albanie*

Main category: cs.AI

TLDR: 论文研究了AI控制（AIC）中的‘控制税’概念，量化了控制措施的成本与安全保证的关系，并通过实验和优化策略为实际部署提供经济可行性评估。
...
Method: 提出了‘控制税’的理论框架，评估了语言模型在对抗环境中的表现，并优化了监控策略。

Result: 量化了控制成本与安全保证的关系，提供了经济可行的监控策略。

Conclusion: 该框架通过系统连接安全保证与成本，为AIC的实际部署提供了经济可行性评估。
...
### [274] [Dynamic Epsilon Scheduling: A Multi-Factor Adaptive Perturbation Budget for Adversarial Training](https://arxiv.org/abs/2506.04263)
*Alan Mitkiy,James Smith,Hana Satou,Hiroshi Tanaka,Emily Johnson,F Monkey*

Main category: cs.CV

TLDR: 提出了一种动态调整对抗训练扰动预算的方法（DES），通过结合梯度代理、预测置信度和模型不确定性，显著提升了对抗鲁棒性和标准准确性。
...
Method: DES框架动态调整扰动预算，结合梯度代理、预测置信度和模型不确定性。
...
### [275] [RSVP: Reasoning Segmentation via Visual Prompting and Multi-modal Chain-of-Thought](https://arxiv.org/abs/2506.04277)
*Yi Lu,Jiawang Cao,Yongliang Wu,Bozheng Li,Licheng Tang,Yangguang Ji,Chong Wu,Jay Wu,Wenbo Zhu*

Main category: cs.CV

TLDR: RSVP框架通过视觉提示统一多模态推理与视觉分割，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型缺乏明确的视觉定位和分割机制，导致认知推理与视觉感知之间存在差距。

Method: RSVP采用两阶段框架：推理阶段通过多模态思维链视觉提示生成区域提案，分割阶段通过视觉语言分割模块细化提案。

Result: RSVP在ReasonSeg上超越现有方法（+6.5 gIoU和+9.2 cIoU），在SegInW上零样本性能达49.7 mAP。
...
### [276] [Evaluating MLLMs with Multimodal Multi-image Reasoning Benchmark](https://arxiv.org/abs/2506.04280)
...
TLDR: 论文提出了首个多图像推理基准MMRB，用于评估多模态大语言模型在多图像任务中的结构化推理能力，并发现开源模型与商业模型存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有基准仅关注单图像或多图像任务的最终答案评估，缺乏对多图像推理能力的深入探索。

Method: 设计MMRB基准，包含92个子任务，涵盖空间、时间和语义推理，并采用GPT-4o生成多解法和CoT注释。提出基于开源LLM的句子级匹配框架。

Result: 实验表明，开源MLLMs在多图像推理任务中显著落后于商业模型，且当前多模态奖励模型几乎无法处理多图像奖励排名任务。

Conclusion: MMRB填补了多图像推理评估的空白，揭示了开源模型与商业模型的差距，为未来研究提供了方向。
...
### [277] [HuGeDiff: 3D Human Generation via Diffusion with Gaussian Splatting](https://arxiv.org/abs/2506.04351)
*Maksym Ivashechkin,Oscar Mendez,Richard Bowden*

Main category: cs.CV

TLDR: 提出了一种弱监督流程，通过图像扩散模型生成可控属性的真实人类图像数据集，再通过基于Transformer的架构将图像特征映射到3D点云，最后训练点云扩散模型，显著提升了3D人类生成的速度、文本对齐性和渲染质量。


<details>
  <summary>Details</summary>
Motivation: 解决当前3D人类生成方法在细节、手部和面部渲染、真实性和可控性方面的不足，以及数据缺乏多样性和真实性的问题。
...
Conclusion: 提出的弱监督流程有效解决了3D人类生成的挑战，为未来研究提供了高效且可控的解决方案。
...
### [278] [ReXVQA: A Large-scale Visual Question Answering Benchmark for Generalist Chest X-ray Understanding](https://arxiv.org/abs/2506.04353)
*Ankit Pal,Jung-Oh Lee,Xiaoman Zhang,Malaikannan Sankarasubbu,Seunghyeon Roh,Won Jung Kim,Meesun Lee,Pranav Rajpurkar*

Main category: cs.CV

TLDR: ReXVQA是最大的胸部放射学视觉问答基准，包含696,000个问题和160,000张胸部X光片，评估了8种多模态大语言模型，其中MedGemma表现最佳（83.24%准确率），甚至超过人类专家（77.27%）。


<details>
  <summary>Details</summary>
Motivation: 为胸部放射学提供一个全面且临床真实的视觉问答基准，推动AI系统在放射学推理能力上的发展。
...
### [279] [WorldPrediction: A Benchmark for High-level World Modeling and Long-horizon Procedural Planning](https://arxiv.org/abs/2506.04363)
*Delong Chen,Willy Chung,Yejin Bang,Ziwei Ji,Pascale Fung*

Main category: cs.CV

TLDR: 论文提出了WorldPrediction基准，用于评估AI模型的世界建模和程序规划能力，强调时间和语义抽象的动作。当前前沿模型表现远低于人类。


<details>
  <summary>Details</summary>
Motivation: 人类具有内部“世界模型”以支持动作规划，而AI模型如何学习类似能力尚不明确。现有基准多关注低层世界建模和机器人运动规划，缺乏对高层抽象动作的评估。

Method: 引入WorldPrediction基准，基于视觉观测，通过区分正确动作或动作序列（从干扰项中）来评估模型。采用部分可观测半MDP框架，确保评估可靠性。

Result: 当前前沿模型在WorldPrediction-WM和WorldPrediction-PP任务中分别仅达到57%和38%准确率，而人类能完美解决。

Conclusion: WorldPrediction填补了高层动作规划评估的空白，揭示了当前AI模型与世界建模和规划能力的差距。
...
### [280] [Puck Localization Using Contextual Cues](https://arxiv.org/abs/2506.04365)
*Liam Salass,Jerrin Bright,Amir Nazemi,Yuhao Chen,John Zelek,David Clausi*

Main category: cs.CV

TLDR: PLUCC是一种基于上下文线索的冰球定位方法，利用球员行为和姿态作为先验信息，结合多尺度特征和门控解码器，显著提升了检测性能。
...
### [281] [Fine-Tuning Video Transformers for Word-Level Bangla Sign Language: A Comparative Analysis for Classification Tasks](https://arxiv.org/abs/2506.04367)
*Jubayer Ahmed Bhuiyan Shawon,Hasan Mahmud,Kamrul Hasan*

Main category: cs.CV

TLDR: 该研究通过微调VideoMAE、ViViT和TimeSformer等视频Transformer架构，在BdSLW60和BdSLW401数据集上实现了高效的孟加拉手语识别，显著优于传统方法。
...
Method: 使用视频Transformer架构（VideoMAE、ViViT、TimeSformer）进行微调，结合数据增强技术和10折分层交叉验证。

Result: VideoMAE在BdSLW60和BdSLW401数据集上分别达到95.5%和81.04%的准确率，表现最佳。

Conclusion: 视频Transformer模型在孟加拉手语识别中具有显著优势，未来可进一步优化以适应更大规模的应用。
...
TLDR: 论文提出了一种基于激活最大化技术的方法，用于解释DNN编码模型对人类大脑视觉系统的预测，并验证了其有效性。
...
Method: 通过提取和自适应下采样预训练Inception V3网络的激活，使用线性回归预测fMRI响应，并应用激活最大化生成优化图像。

Result: 生成的图像与已知选择性特征一致，并能可靠驱动目标脑区活动。

Conclusion: 激活最大化可成功应用于DNN编码模型，为人类视觉系统的响应提供了灵活的表征和调控方法。
...
### [283] [Is Perturbation-Based Image Protection Disruptive to Image Editing?](https://arxiv.org/abs/2506.04394)
*Qiuyu Tang,Bonor Ayambem,Mooi Choo Chuah,Aparna Bharati*

Main category: cs.CV

TLDR: 研究发现，现有的基于扰动的图像保护方法无法完全阻止基于扩散模型的编辑，反而可能增强图像与文本提示的关联性。


<details>
  <summary>Details</summary>
Motivation: 探讨扩散模型（如Stable Diffusion）可能被滥用于传播虚假信息或侵犯版权的问题，并评估现有图像保护方法的有效性。

Method: 通过实验评估多种基于扰动的图像保护方法在不同领域（自然场景图像和艺术作品）和编辑任务（图像生成和风格编辑）中的表现。

Result: 实验表明，保护后的图像仍能被成功编辑，且扰动可能增强图像与文本提示的关联性，导致更好的编辑效果。
...
### [284] [Normalize Filters! Classical Wisdom for Deep Vision](https://arxiv.org/abs/2506.04401)
*Gustavo Perez,Stella X. Yu*

Main category: cs.CV

TLDR: 论文提出了一种滤波器归一化方法，通过可学习的缩放和平移，解决了深度学习卷积滤波器在图像大气传输中的失真问题，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 传统图像滤波器经过严格归一化以确保一致性和可解释性，而深度学习中的卷积滤波器缺乏此类约束，导致在大气传输中响应失真。

Method: 提出滤波器归一化方法，结合可学习的缩放和平移，使滤波器具有大气等变性，适用于CNN和视觉Transformer。
...
### [285] [HMAR: Efficient Hierarchical Masked Auto-Regressive Image Generation](https://arxiv.org/abs/2506.04421)
*Hermann Kumbong,Xian Liu,Tsung-Yi Lin,Ming-Yu Liu,Xihui Liu,Ziwei Liu,Daniel Y. Fu,Christopher Ré,David W. Romero*

Main category: cs.CV

TLDR: HMAR是一种新的图像生成算法，通过改进VAR的并行生成问题，实现了更高质量的图像生成和更快的采样速度。
...
Method: HMAR采用马尔可夫过程和多步掩码生成技术，仅依赖前一尺度的信息生成图像，并开发了高效的注意力内核。

Result: 在ImageNet 256x256和512x512基准测试中，HMAR表现优于或匹配VAR、扩散和自回归基线模型，训练和推理速度显著提升。
...
### [286] [Photoreal Scene Reconstruction from an Egocentric Device](https://arxiv.org/abs/2506.04444)
*Zhaoyang Lv,Maurizio Monge,Ka Chen,Yufeng Zhu,Michael Goesele,Jakob Engel,Zhao Dong,Richard Newcombe*

Main category: cs.CV

TLDR: 本文研究了使用自我中心设备进行高动态范围场景的逼真重建的挑战，提出了视觉惯性束调整（VIBA）和高斯溅射物理图像形成模型，显著提升了重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常假设使用设备的视觉惯性里程计系统估计的帧率6DoF姿态，可能忽略像素级重建所需的细节。本文旨在解决这一问题。

Method: 采用VIBA校准滚动快门RGB相机的时间戳和运动轨迹，并结合高斯溅射的物理图像形成模型，以解决传感器特性问题。
...
### [287] [Towards Large-Scale Pose-Invariant Face Recognition Using Face Defrontalization](https://arxiv.org/abs/2506.04496)
*Patrik Mesec,Alan Jović*

Main category: cs.CV

TLDR: 提出了一种称为“去正面化”的方法，通过增强训练数据集来改进极端头部姿态下的人脸识别性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖复杂技术（如正面化），但在实际应用中不实用且通常在小数据集上评估。

Method: 1) 训练一个去正面化模型（FFWM），2) 基于ArcFace损失训练ResNet-50模型，使用原始和去正面化数据。

Result: 在四个开放数据集上测试，去正面化方法优于无去正面化的模型，并在三个大数据集上优于现有正面化方法。

Conclusion: 去正面化方法有效，但现有方法可能在小数据集上过拟合。
...
### [288] [FALO: Fast and Accurate LiDAR 3D Object Detection on Resource-Constrained Devices](https://arxiv.org/abs/2506.04499)
*Shizhong Han,Hsin-Pai Cheng,Hong Cai,Jihad Masri,Soyeb Nagori,Fatih Porikli*

Main category: cs.CV

TLDR: FALO是一种硬件友好的LiDAR 3D检测方法，结合1D序列处理和ConvDotMix块，实现高效推理和SOTA精度。


<details>
  <summary>Details</summary>
Motivation: 现有LiDAR 3D检测方法依赖稀疏卷积或Transformer，在资源受限设备上运行困难。

Method: FALO将稀疏3D体素排列为1D序列，并通过ConvDotMix块（大核卷积、Hadamard积和线性层）处理，引入隐式分组优化推理效率。

Result: 在nuScenes和Waymo基准测试中表现优异，比最新SOTA方法快1.6~9.8倍。

Conclusion: FALO适用于资源受限设备，兼具高效和精度。
...
### [289] [AuthGuard: Generalizable Deepfake Detection via Language Guidance](https://arxiv.org/abs/2506.04501)
*Guangyu Shen,Zhihua Li,Xiang Xu,Tianchen Zhao,Zheng Zhang,Dongsheng An,Zhuowen Tu,Yifan Xing,Qin Zhang*

Main category: cs.CV

TLDR: AuthGuard通过结合语言指导和视觉编码器，提升了深度伪造检测的泛化能力和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测技术依赖于训练时学习的统计特征，难以应对新出现的伪造方法。

Method: 结合分类和图像-文本对比学习，利用MLLMs生成文本，并集成数据不确定性学习。

Result: 在DFDC和DF40数据集上分别实现6.15%和16.68%的AUC提升，DDVQA数据集上性能提升24.69%。

Conclusion: AuthGuard在泛化性、准确性和可解释性上均优于现有方法。
...
### [290] [Pruning Everything, Everywhere, All at Once](https://arxiv.org/abs/2506.04513)
*Gustavo Henrique do Nascimento,Ian Pons,Anna Helena Reali Costa,Artur Jordao*

Main category: cs.CV

TLDR: 提出了一种同时剪枝神经元和层的新方法，通过表示相似性选择最优子网络，显著减少计算量并保持模型性能。
...
Result: 在ResNet56和ResNet110上实现86.37%和95.82%的FLOPs减少，同时保持或提升准确性，并减少83.31%的碳排放。

Conclusion: 该方法为剪枝领域开辟了新方向，显著提升计算效率和模型鲁棒性。
...
### [291] [EECD-Net: Energy-Efficient Crack Detection with Spiking Neural Networks and Gated Attention](https://arxiv.org/abs/2506.04526)
*Shuo Zhang*

Main category: cs.CV

TLDR: 提出了一种名为EECD-Net的多阶段道路裂缝检测方法，结合SRCNN、SCU和GAT模块，显著提高了检测精度和能效。


<details>
  <summary>Details</summary>
Motivation: 智能终端设备因能量有限和低分辨率成像难以实现实时监测，亟需一种高精度、低功耗的裂缝检测方案。

Method: 采用SRCNN提升图像分辨率，SCU减少功耗，GAT模块融合多尺度特征以增强检测鲁棒性。
...
### [292] [Enhancing Frequency for Single Image Super-Resolution with Learnable Separable Kernels](https://arxiv.org/abs/2506.04555)
*Heng Tian*

Main category: cs.CV

TLDR: 论文提出了一种名为可学习可分离核（LSKs）的即插即用模块，通过直接增强图像频率组件来提升单图像超分辨率（SISR）性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常通过间接方式（如专用损失函数）提升SISR性能，而LSKs旨在直接优化频率组件。

Method: LSKs是秩为一的矩阵，可分解为正交且可合并的一维核，显著减少参数和计算量。
...
### [293] [Perceptual Decoupling for Scalable Multi-modal Reasoning via Reward-Optimized Captioning](https://arxiv.org/abs/2506.04559)
*Yunhao Gou,Kai Chen,Zhili Liu,Lanqing Hong,Xin Jin,Zhenguo Li,James T. Kwok,Yu Zhang*

Main category: cs.CV

TLDR: 论文提出RACRO方法，通过强化学习优化视觉提取器的描述生成，以支持多模态大语言模型的高效推理。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型中视觉与语言对齐的高成本问题，同时确保视觉提取的描述既忠实于图像又支持准确推理。

Method: 提出RACRO方法，通过推理引导的强化学习策略优化视觉提取器的描述生成，形成感知与推理的闭环。

Result: 在数学和科学多模态基准测试中，RACRO方法实现了最先进的性能，并支持无需昂贵多模态重新对齐的即插即用适配。
...
### [294] [LGM-Pose: A Lightweight Global Modeling Network for Real-time Human Pose Estimation](https://arxiv.org/abs/2506.04561)
*Biao Guo,Fangmin Guo,Guibo Luo,Xiaonan Luo,Feng Zhang*

Main category: cs.CV

TLDR: 提出了一种轻量级全局建模网络（LGM-Pose），通过单分支结构和创新模块（LARM和SFusion）解决了多分支CNN网络的全局上下文捕捉不足和高延迟问题。


<details>
  <summary>Details</summary>
Motivation: 当前多分支并行CNN网络在多人体姿态估计中难以捕捉全局上下文且延迟高，需要更高效的轻量级解决方案。

Method: 设计了轻量级MobileViM Block和LARM模块（利用NPT-Op提取全局信息），并引入SFusion模块整合多尺度信息。

Result: 在COCO和MPII数据集上，LGM-Pose参数更少，性能更优且处理速度更快。
...
### [295] [VLMs Can Aggregate Scattered Training Patches](https://arxiv.org/abs/2506.03614)
*Zhanhui Zhou,Lingjie Chen,Chao Yang,Chaochao Lu*

Main category: cs.CV

TLDR: 论文探讨了视觉语言模型（VLMs）中通过分散有害图像为小块规避数据审核的风险，并提出了视觉缝合（visual stitching）的概念。


<details>
  <summary>Details</summary>
Motivation: 研究动机是揭示VLMs在训练数据分散为小块时仍能整合信息的能力，以及这种能力如何被用于规避内容审核，从而带来安全隐患。

Method: 方法包括在三个数据集上测试VLMs的视觉缝合能力，通过将图像分割为小块并标记唯一ID，验证模型是否能从完整图像或文本中正确识别ID。

Result: 结果表明，VLMs能够通过视觉缝合整合分散的信息，甚至在有害内容被伪装为安全描述时仍能生成有害响应。

Conclusion: 结论指出视觉缝合能力可能导致有害内容绕过审核，对VLM的安全性构成严重威胁。
...
### [296] [Follow-Your-Creation: Empowering 4D Creation through Video Inpainting](https://arxiv.org/abs/2506.04590)
*Yue Ma,Kunyu Feng,Xinhua Zhang,Hongyu Liu,David Junhao Zhang,Jinbo Xing,Yinhan Zhang,Ayden Yang,Zeyu Wang,Qifeng Chen*

Main category: cs.CV

TLDR: Follow-Your-Creation是一个新颖的4D视频生成与编辑框架，通过视频修复模型生成先验，将4D视频创建转化为视频修复任务，支持单目视频输入。
...
Method: 结合深度点云渲染生成不可见区域掩码，与用户编辑掩码组合成复合掩码数据集，通过随机采样训练模型。采用自迭代调优策略处理大相机运动下的时间一致性，推理时引入时间包装模块。

Result: 生成的4D视频具有多视角一致性，支持基于提示的内容编辑，质量和灵活性显著优于现有方法。

Conclusion: 该方法有效利用基础模型的先验知识，实现了高质量、灵活的4D视频生成与编辑。
...
### [297] [Hierarchical-Task-Aware Multi-modal Mixture of Incremental LoRA Experts for Embodied Continual Learning](https://arxiv.org/abs/2506.04595)
*Ziqi Jia,Anmin Wang,Xiaoyang Qu,Xiaowen Yang,Jianzong Wang*

Main category: cs.CV

TLDR: 提出了一种分层持续学习框架HEC，结合Task-aware MoILE方法，通过任务识别和路由选择减少遗忘，实验效果优于其他方法。


<details>
  <summary>Details</summary>
Motivation: 解决现有持续学习框架忽视高级规划和多级知识的问题。

Method: 分层学习框架HEC，Task-aware MoILE方法结合任务识别和路由选择，利用SVD减少遗忘。

Result: 实验表明方法在减少旧任务遗忘方面表现优异。

Conclusion: HEC和Task-aware MoILE有效支持智能体持续学习并保留旧知识。
...
### [298] [SmartAvatar: Text- and Image-Guided Human Avatar Generation with VLM AI Agents](https://arxiv.org/abs/2506.04606)
*Alexander Huang-Menders,Xinhang Liu,Andy Xu,Yuyao Zhang,Chi-Keung Tang,Yu-Wing Tai*

Main category: cs.CV

TLDR: SmartAvatar是一个基于视觉语言代理的框架，通过单张照片或文本提示生成可动画的3D人体头像，结合大型视觉语言模型和参数化人体生成器，实现高质量、可定制的头像生成。


<details>
  <summary>Details</summary>
Motivation: 解决现有扩散方法在人体头像生成中对身份、体型和动画准备的控制不足问题。

Method: 利用视觉语言模型的常识推理能力，结合参数化人体生成器，并通过自主验证循环迭代优化生成参数。

Result: 生成的3D头像质量高，支持姿势操作，适用于动画和交互应用，性能优于现有方法。

Conclusion: SmartAvatar是一种高效、灵活的3D头像生成工具，适用于消费级硬件。
...
### [299] [Perfecting Depth: Uncertainty-Aware Enhancement of Metric Depth](https://arxiv.org/abs/2506.04612)
*Jinyoung Jun,Lei Chu,Jiahao Li,Yan Lu,Chang-Su Kim*

Main category: cs.CV

TLDR: 提出了一种名为Perfecting Depth的两阶段框架，用于传感器深度增强，结合随机扩散模型和确定性优化，生成高可靠性深度图。


<details>
  <summary>Details</summary>
Motivation: 解决传感器深度测量中不可靠区域的问题，同时保留几何线索，提升深度图的准确性和可靠性。
...
### [300] [Deep Learning Reforms Image Matching: A Survey and Outlook](https://arxiv.org/abs/2506.04619)
*Shihua Zhang,Zizhuo Li,Kaining Zhang,Yifan Lu,Yuxin Deng,Linfeng Tang,Xingyu Jiang,Jiayi Ma*

Main category: cs.CV

TLDR: 本文综述了深度学习如何逐步改进传统图像匹配流程，包括替换传统步骤为可学习模块和合并多步骤为端到端模块，并评估了代表性方法，最后讨论了未来研究方向。
...
Method: 通过替换传统步骤为可学习模块（如检测器-描述符、异常值过滤器）和合并多步骤为端到端模块（如稀疏匹配器、姿态回归器），全面分析深度学习对图像匹配的改进。

Result: 在相对姿态恢复、单应性估计和视觉定位任务上评估了代表性方法，展示了深度学习方法的优势与局限性。

Conclusion: 深度学习显著推动了图像匹配领域的发展，但仍存在挑战，未来研究应关注进一步创新方向。
...
### [301] [Unfolding Spatial Cognition: Evaluating Multimodal Models on Visual Simulations](https://arxiv.org/abs/2506.04633)
*Linjie Li,Mahtab Bigverdi,Jiawei Gu,Zixian Ma,Yinuo Yang,Ziang Li,Yejin Choi,Ranjay Krishna*

Main category: cs.CV

TLDR: STARE是一个评估多模态大语言模型在空间推理任务中表现的基准，涵盖几何变换、空间推理和现实世界任务。模型在简单任务表现良好，但在复杂任务中接近随机猜测，且无法有效利用视觉模拟。
...
Method: STARE包含4K任务，涵盖2D/3D几何变换、空间推理（如立方体展开和七巧板）和现实世界任务（如视角和时间推理）。

Result: 模型在简单2D任务表现良好，但在复杂任务（如3D立方体展开和七巧板）中表现接近随机猜测。人类表现优异但耗时，模型无法有效利用视觉模拟。

Conclusion: STARE揭示了模型在复杂空间推理任务中的局限性，尤其是无法有效利用视觉模拟，为未来研究提供了方向。
...
### [302] [Text-Aware Real-World Image Super-Resolution via Diffusion Model with Joint Segmentation Decoders](https://arxiv.org/abs/2506.04641)
*Qiming Hu,Linlong Fan,Yiyan Luo,Yuhang Yu,Xiaojie Guo,Qingnan Fan*

Main category: cs.CV

TLDR: TADiSR是一种基于扩散模型的超分辨率框架，通过文本感知注意力和联合分割解码器，提升真实世界图像中文本区域的结构保真度。
...
Result: 实验表明，TADiSR显著提升超分辨率图像的文本可读性，并在多项评估指标中达到最优性能。
...
### [303] [FPSAttention: Training-Aware FP8 and Sparsity Co-Design for Fast Video Diffusion](https://arxiv.org/abs/2506.04648)
...
TLDR: FPSAttention是一种结合FP8量化和稀疏化的训练感知协同设计方法，用于加速视频生成中的3D双向注意力机制，显著提升推理速度且不损失生成质量。


<details>
  <summary>Details</summary>
Motivation: 扩散生成模型在高质量视频生成中表现优异，但推理速度慢和计算需求高限制了实际应用。量化和稀疏化虽能独立加速推理，但现有训练无关方法结合二者会导致性能下降。

Method: 提出FPSAttention，包含三个创新：1）统一的3D分块粒度支持量化和稀疏化；2）适应噪声调度的去噪步骤感知策略；3）硬件友好的内核实现。

Result: 在1.3B和14B模型上训练，VBench基准测试显示，FPSAttention在720p分辨率下实现注意力操作7.09倍加速和端到端视频生成4.96倍加速，且生成质量未下降。
...
### [304] [Feature-Based Lie Group Transformer for Real-World Applications](https://arxiv.org/abs/2506.04668)
*Takayuki Komatsu,Yoshiyuki Ohmura,Kayato Nishitsunoi,Yasuo Kuniyoshi*

Main category: cs.CV

TLDR: 该论文提出了一种结合特征提取和对象分割的方法，将群分解理论应用于更现实的场景，以解决传统表示学习中无法处理条件独立性的问题。
...
### [305] [Interpretable Few-Shot Image Classification via Prototypical Concept-Guided Mixture of LoRA Experts](https://arxiv.org/abs/2506.04673)
*Zhong Ji,Rongshuai Wei,Jingren Liu,Yanwei Pang,Jungong Han*

Main category: cs.CV

TLDR: 提出了一种Few-Shot Prototypical Concept Classification (FSPCC)框架，通过参数高效适应和多层次特征保留策略，解决了数据稀缺场景下自解释模型的性能问题。


<details>
  <summary>Details</summary>
Motivation: 自解释模型在数据稀缺场景下表现不佳，需要解决参数不平衡和表示不对齐的问题。

Method: 采用Mixture of LoRA Experts (MoLE)进行参数高效适应，结合跨模块概念指导和几何感知概念判别损失。

Result: 在六个基准测试中，性能显著优于现有方法，相对提升4.2%-8.7%。

Conclusion: 结合概念学习和少样本适应，既能提高准确性，又能增强模型可解释性。
...
### [306] [Gen-n-Val: Agentic Image Data Generation and Validation](https://arxiv.org/abs/2506.04676)
*Jing-En Huang,I-Sheng Fang,Tzuhsuan Huang,Chih-Yu Wang,Jun-Cheng Chen*

Main category: cs.CV

TLDR: Gen-n-Val是一个新型数据生成框架，利用Layer Diffusion、LLMs和VLLMs生成高质量的单对象掩码和多样化背景，显著提升合成数据的有效性。
...
Method: Gen-n-Val包含两个代理：LD提示代理（LLM）优化提示以生成高质量前景实例和分割掩码；数据验证代理（VLLM）过滤低质量合成数据。系统提示通过TextGrad优化，并使用图像协调技术组合多个实例。

Result: 将无效合成数据从50%降至7%，在COCO实例分割中稀有类别的mAP提升1%，在开放词汇目标检测中mAP提升7.1%。

Conclusion: Gen-n-Val显著提升了合成数据的质量，并在实例分割和目标检测任务中表现出优越性能。
...
### [307] [MARS: Radio Map Super-resolution and Reconstruction Method under Sparse Channel Measurements](https://arxiv.org/abs/2506.04682)
*Chuyun Deng,Na Liu,Wei Xie,Lianming Xu,Li Wang*

Main category: cs.CV

TLDR: 提出了一种多尺度感知的无线电地图超分辨率方法MARS，结合CNN和Transformer，通过多尺度特征融合和残差连接提升重建精度。


<details>
  <summary>Details</summary>
Motivation: 传统插值和修复方法缺乏环境感知，而深度学习依赖详细场景数据，泛化能力有限，需改进无线电地图重建。

Method: 结合CNN和Transformer，采用多尺度特征融合和残差连接，兼顾全局和局部特征提取。

Result: 在不同场景和天线位置实验中，MARS在MSE和SSIM上优于基线模型，计算成本低。

Conclusion: MARS具有强实用潜力，能有效提升无线电地图重建精度。
...
### [308] [HoliSafe: Holistic Safety Benchmarking and Modeling with Safety Meta Token for Vision-Language Model](https://arxiv.org/abs/2506.04704)
*Youngwan Lee,Kangsan Kim,Kwanyong Park,Ilcahe Jung,Soojin Jang,Seanie Lee,Yong-Ju Lee,Sung Ju Hwang*

Main category: cs.CV

TLDR: 论文提出了HoliSafe数据集和SafeLLaVA模型，以解决现有视觉语言模型（VLM）安全性的不足，通过全面覆盖安全/不安全图像-文本组合和引入可学习的安全元标记及专用安全头，显著提升了模型的安全性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLM安全方法存在数据集覆盖不全和架构创新不足的问题，导致模型对未见过的攻击配置脆弱。

Method: 引入HoliSafe数据集和SafeLLaVA模型，后者包含可学习的安全元标记和专用安全头，以增强模型的安全性和可解释性。
...
### [309] [Line of Sight: On Linear Representations in VLLMs](https://arxiv.org/abs/2506.04706)
*Achyuta Rajaram,Sarah Schwettmann,Jacob Andreas,Arthur Conmy*

Main category: cs.CV

TLDR: 论文探讨了多模态语言模型中图像概念的表示方式，发现线性可解码特征在残差流中表示ImageNet类别，并通过训练稀疏自编码器提高特征多样性。


<details>
  <summary>Details</summary>
Motivation: 研究多模态语言模型如何在隐藏激活中表示图像概念，以增强模型的可解释性和多模态能力。

Method: 使用LlaVA-Next模型分析图像概念的线性可解码特征，并通过稀疏自编码器（SAEs）训练多模态特征。

Result: 发现图像和文本特征在深层逐渐共享，尽管初始表示是分离的。

Conclusion: 多模态模型的深层表示趋向于共享特征，稀疏自编码器可提高特征多样性和可解释性。
...
### [310] [Robust Few-Shot Vision-Language Model Adaptation](https://arxiv.org/abs/2506.04713)
*Hanxin Wang,Tian Liu,Shu Kong*

Main category: cs.CV

TLDR: 该论文研究了如何通过微调预训练的视觉语言模型（VLM）来提高其在分布内（ID）和分布外（OOD）任务上的准确性，并提出了一种新的两阶段微调方法SRAPF。


<details>
  <summary>Details</summary>
Motivation: 预训练的VLM在少量标注数据下表现良好，但在OOD数据上泛化能力不足，需要提升其鲁棒性。

Method: 通过比较不同微调方法（如提示调优、线性探测、对比微调等），发现仅微调视觉编码器的顶层效果最佳。基于此，提出了结合检索增强和对抗扰动的两阶段微调方法SRAPF。

Result: SRAPF在ImageNet OOD基准测试中实现了最先进的ID和OOD准确性。

Conclusion: SRAPF通过分阶段微调和数据增强技术，显著提升了VLM在ID和OOD任务上的性能。
...
### [311] [Towards Holistic Visual Quality Assessment of AI-Generated Videos: A LLM-Based Multi-Dimensional Evaluation Model](https://arxiv.org/abs/2506.04715)
*Zelu Qi,Ping Shi,Chaoyang Zhang,Shuqi Wang,Fei Zhao,Da Pan,Zefeng Ying*

Main category: cs.CV

TLDR: 论文提出了一种基于多维度特征和大语言模型（LLM）的AI生成视频（AIGV）自动视觉质量评估方法，并在NTIRE 2025挑战赛中取得第二名。


<details>
  <summary>Details</summary>
Motivation: AIGV技术存在视觉质量缺陷（如噪声、模糊、帧抖动等），影响用户体验，需有效的自动质量评估方法以改进生成模型。

Method: 将AIGV视觉质量分解为技术质量、运动质量和视频语义三个维度，设计对应编码器提取特征，并引入LLM作为质量回归模块，结合多模态提示工程和LoRA微调技术。

Result: 在NTIRE 2025挑战赛中排名第二，验证了方法的有效性。

Conclusion: 提出的多维度特征和LLM结合的方法能有效评估AIGV视觉质量，为内容监管和模型改进提供支持。
...
### [312] [Learning dissection trajectories from expert surgical videos via imitation learning with equivariant diffusion](https://arxiv.org/abs/2506.04716)
...
TLDR: 本文提出了一种名为iDPOE的新方法，用于预测内窥镜黏膜下剥离术（ESD）视频中的解剖轨迹，通过结合扩散模型和等变表示，提升了模仿学习的准确性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 预测ESD视频中的解剖轨迹对提升手术技能培训效果具有重要意义，但目前这一领域尚未充分探索。模仿学习虽具潜力，但在处理不确定的未来动作、几何对称性和多样化手术场景方面仍存在挑战。

Method: 提出iDPOE方法，通过联合状态动作分布建模专家行为，结合扩散模型进行策略学习，并嵌入等变表示以提升几何对称性的泛化能力。采用前向过程引导的动作推断策略处理状态不匹配问题。

Result: 在包含近2000个视频片段的ESD数据集上，iDPOE在轨迹预测任务中超越了现有方法（显式和隐式）。
...
TLDR: 提出了一种基于AI的自动标注系统，用于显示面板缺陷检测，利用上下文学习能力，显著提升了标注效率和模型性能。
...
### [314] [Bridging Annotation Gaps: Transferring Labels to Align Object Detection Datasets](https://arxiv.org/abs/2506.04737)
*Mikhail Kennerley,Angelica Alives-Reviro,Carola-Bibiane Schönlieb,Robby T. Tan*

Main category: cs.CV

TLDR: LAT框架通过标签对齐和特征融合，解决多数据集目标检测中的语义和空间不一致问题，无需共享标签空间或手动标注。


<details>
  <summary>Details</summary>
Motivation: 多数据集结合可提升泛化性，但语义和标注不一致阻碍其应用。现有方法需共享标签空间或手动调整，LAT旨在解决这些问题。

Method: LAT通过数据集特定检测器生成伪标签，结合PPG和SFF模块，实现标签空间对齐和特征优化。

Result: LAT在多个基准测试中表现优异，目标域检测性能提升高达+4.8AP。

Conclusion: LAT有效解决标签和空间不一致问题，适用于异构数据集的目标检测任务。
...
### [315] [SRD: Reinforcement-Learned Semantic Perturbation for Backdoor Defense in VLMs](https://arxiv.org/abs/2506.04743)
*Shuhan Xu,Siyuan Liang,Hongling Zheng,Yong Luo,Aishan Liu,Dacheng Tao*

Main category: cs.CV

TLDR: 论文提出了一种针对视觉语言模型（VLMs）后门攻击的防御方法——语义奖励防御（SRD），通过强化学习框架减少恶意行为，无需预先了解触发器。


<details>
  <summary>Details</summary>
Motivation: 研究发现VLMs在图像描述任务中易受后门攻击，攻击者通过注入微小扰动控制模型生成恶意描述，且难以检测和防御。

Method: 提出SRD框架，利用深度Q网络学习对敏感图像区域施加离散扰动（如遮挡、颜色掩码），以破坏恶意路径的激活，并通过语义保真度评分作为奖励信号。

Result: 实验表明SRD将攻击成功率降至5.6%，且在干净输入上保持描述质量，性能下降小于10%。
...
### [316] [Physics Informed Capsule Enhanced Variational AutoEncoder for Underwater Image Enhancement](https://arxiv.org/abs/2506.04753)
*Niki Martinel,Rita Pucci*

Main category: cs.CV

TLDR: 提出一种新颖的双流架构，通过结合物理模型与胶囊聚类特征学习，实现水下图像增强，性能优于现有方法。
...
Method: 双流架构：物理估计器与胶囊聚类并行，优化目标兼顾物理约束与感知质量。
...
### [317] [Truth in the Few: High-Value Data Selection for Efficient Multi-Modal Reasoning](https://arxiv.org/abs/2506.04755)
*Shenshen Li,Kaiyuan Deng,Lei Wang,Hao Yang,Chong Peng,Peng Yan,Fumin Shen,Heng Tao Shen,Xing Xu*

Main category: cs.CV

TLDR: 论文提出了一种名为RAP的数据选择方法，通过识别稀疏的认知样本（cognitive samples）来提升多模态大语言模型的推理能力，仅需9.3%的训练数据即可实现优于全数据集的性能，并减少43%的计算成本。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为多模态推理能力的提升需要大量训练数据，但作者发现仅少数样本（认知样本）能有效触发推理，其余样本贡献有限。因此，作者提出通过高效数据选择来避免冗余和计算浪费。

Method: 提出RAP方法，包含两个互补的评估器：1）基于潜在结果模型的因果差异评估器（CDE），用于剔除依赖语言先验的样本；2）注意力置信度评估器（ACE），用于剔除无关但被过度强调的样本。此外，引入难度感知替换模块（DRM）以增强数据复杂性。

Result: 在六个数据集上的实验表明，RAP仅使用9.3%的训练数据即可实现更优性能，同时减少43%以上的计算成本。

Conclusion: RAP方法通过高效数据选择显著提升了多模态推理的效率，挑战了传统依赖大规模数据的假设，为模型训练提供了更经济的解决方案。
...
### [318] [Toward Better SSIM Loss for Unsupervised Monocular Depth Estimation](https://arxiv.org/abs/2506.04758)
*Yijun Cao,Fuya Luo,Yongjie Li*

Main category: cs.CV

TLDR: 论文提出了一种新的SSIM形式，通过加法而非乘法组合SSIM中的亮度、对比度和结构相似性组件，提升了无监督单目深度学习的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在训练损失中使用的SSIM函数忽略了其不同组件及超参数对训练的影响，导致性能受限。

Method: 提出了一种新的SSIM形式，用加法替代乘法组合其组件，并优化参数组合。

Result: 在KITTI-2015数据集上，优化后的SSIM损失函数显著优于基线方法。

Conclusion: 新的SSIM形式能生成更平滑的梯度，提升无监督深度估计的性能。
...
### [319] [HypeVPR: Exploring Hyperbolic Space for Perspective to Equirectangular Visual Place Recognition](https://arxiv.org/abs/2506.04764)
*Suhan Woo,Seongwon Lee,Jinwoo Jang,Euntai Kim*

Main category: cs.CV

TLDR: HypeVPR是一种基于双曲空间的分层嵌入框架，用于解决P2E视觉地点识别（VPR）的挑战，通过分层特征聚合和高效搜索策略，在速度和准确性上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的移动机器人应用需要适应不同视角的查询图像，P2E VPR方法成为自然选择，但现有方法未能充分利用全景图像的分层结构特性。
...
### [320] [Object-X: Learning to Reconstruct Multi-Modal 3D Object Representations](https://arxiv.org/abs/2506.04789)
*Gaia Di Lorenzo,Federico Tombari,Marc Pollefeys,Daniel Barath*

Main category: cs.CV

TLDR: Object-X是一种多模态3D对象表示框架，能够编码丰富的对象嵌入并解码为几何和视觉重建，支持多种下游任务，同时显著减少存储需求。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖于任务特定的嵌入，无法同时支持几何重建和跨任务复用，因此需要一种更通用的多模态表示框架。

Method: Object-X通过将多模态数据（如图像、点云、文本）几何地锚定在3D体素网格中，并学习融合体素信息的非结构化嵌入，实现3D高斯泼溅重建。

Result: 在真实数据集上，Object-X实现了高保真新视角合成和几何精度提升，同时在场景对齐和定位任务中表现优异，存储需求显著降低。
...
### [321] [LotusFilter: Fast Diverse Nearest Neighbor Search via a Learned Cutoff Table](https://arxiv.org/abs/2506.04790)
*Yusuke Matsui*

Main category: cs.CV

TLDR: LotusFilter是一种后处理模块，用于多样化近似最近邻搜索（ANNS）结果，通过预计算邻近向量表并删除冗余向量，速度快（0.02 ms/查询）。
...
### [322] [SupeRANSAC: One RANSAC to Rule Them All](https://arxiv.org/abs/2506.04803)
*Daniel Barath*

Main category: cs.CV

TLDR: SupeRANSAC是一种新颖的统一RANSAC流程，旨在提高计算机视觉任务中的鲁棒性估计性能，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: RANSAC及其变体在计算机视觉中广泛应用，但性能受实现细节和问题特定优化的影响较大，现有框架在不同任务中表现不一。
...
Conclusion: SupeRANSAC为计算机视觉任务提供了一种统一的、高性能的RANSAC解决方案，代码已开源。
...
### [323] [MegaHan97K: A Large-Scale Dataset for Mega-Category Chinese Character Recognition with over 97K Categories](https://arxiv.org/abs/2506.04807)
*Yuyi Zhang,Yongxin Shi,Peirong Zhang,Yixin Zhao,Zhenhua Yang,Lianwen Jin*

Main category: cs.CV

TLDR: 论文介绍了MegaHan97K数据集，支持最新的GB18030-2022标准，包含97,455个汉字类别，解决了现有数据集规模不足和长尾分布问题。


<details>
  <summary>Details</summary>
Motivation: 中文汉字类别庞大且不断扩展，现有OCR数据集无法满足大规模汉字识别需求，亟需一个全面支持最新标准的数据集。

Method: 构建了MegaHan97K数据集，包含手写、历史和合成三个子集，覆盖97,455个汉字类别，并进行了全面的基准实验。

Result: MegaHan97K是目前规模最大的数据集，支持最新标准，解决了长尾分布问题，并揭示了大规模类别识别的新挑战。
...
### [324] [Spike-TBR: a Noise Resilient Neuromorphic Event Representation](https://arxiv.org/abs/2506.04817)
*Gabriele Magrini. Federico Becattini,Luca Cultrera,Lorenzo Berlincioni,Pietro Pala,Alberto Del Bimbo*

Main category: cs.CV

TLDR: Spike-TBR是一种基于时间二进制表示（TBR）的新型事件编码策略，通过整合脉冲神经元增强对噪声的鲁棒性，提升事件流处理的性能。


<details>
  <summary>Details</summary>
Motivation: 事件相机具有高时间分辨率、低延迟和动态范围等优势，但如何高效且抗噪地将事件流转换为标准计算机视觉兼容格式仍具挑战。

Method: 提出Spike-TBR，结合TBR的帧基优势与脉冲神经网络的噪声过滤能力，评估了四种不同脉冲神经元变体。

Result: 在噪声场景下表现优异，同时在干净数据上也有提升，验证了方法的鲁棒性和有效性。
...
### [325] [Fool the Stoplight: Realistic Adversarial Patch Attacks on Traffic Light Detectors](https://arxiv.org/abs/2506.04823)
*Svetlana Pavlitska,Jamie Robb,Nikolai Polley,Melih Yazgan,J. Marius Zöllner*

Main category: cs.CV

TLDR: 本文展示了如何通过打印的对抗性补丁攻击交通灯检测的CNN模型，提出了一种威胁模型和训练策略，并在真实环境中验证了攻击效果。


<details>
  <summary>Details</summary>
Motivation: 现有研究对自动驾驶车辆摄像头感知任务的对抗性攻击较多，但对交通灯检测器的攻击研究较少，本文填补了这一空白。

Method: 提出一种威胁模型，通过在交通灯下方放置对抗性补丁进行攻击，并设计了一种训练策略。在实验室和真实环境中进行了实验验证。

Result: 成功实现了针对交通灯检测的通用对抗性补丁攻击，包括红绿灯标签翻转攻击和图形分类攻击。

Conclusion: 本文证明了对抗性补丁对交通灯检测器的有效性，为相关领域的安全研究提供了新的视角。
...
### [326] [DualX-VSR: Dual Axial Spatial$\times$Temporal Transformer for Real-World Video Super-Resolution without Motion Compensation](https://arxiv.org/abs/2506.04830)
*Shuo Cao,Yihao Liu,Xiaohui Li. Yuanting Gao. Yu Zhou,Chao Dong*

Main category: cs.CV

TLDR: DualX-VSR提出了一种新颖的双轴时空注意力机制，解决了现有基于Transformer的视频超分辨率模型在像素级精度和运动补偿方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer模型在视频超分辨率任务中因token化和顺序注意力机制导致像素级精度不足，且依赖光流对齐引入误差。

Method: DualX-VSR采用双轴时空注意力机制，沿正交方向整合时空信息，无需运动补偿。
...
### [327] [OpenMaskDINO3D : Reasoning 3D Segmentation via Large Language Model](https://arxiv.org/abs/2506.04837)
*Kunshen Zhang*

Main category: cs.CV

TLDR: 论文提出OpenMaskDINO3D，一种用于3D理解和分割的LLM，填补了3D推理分割的空白。


<details>
  <summary>Details</summary>
Motivation: 现有感知系统依赖显式指令或预定义类别，缺乏3D推理分割的框架。

Method: 引入SEG token和对象标识符，处理点云数据和文本提示生成高精度3D分割掩码。

Result: 在ScanNet数据集上验证了模型的有效性。

Conclusion: OpenMaskDINO3D在3D任务中表现出色，能够直接从自然语言指令生成准确的分割结果。
...
### [328] [Geological Field Restoration through the Lens of Image Inpainting](https://arxiv.org/abs/2506.04869)
*Vladislav Trifonov,Ivan Oseledets,Ekaterina Muravleva*

Main category: cs.CV

TLDR: 提出一种基于多维张量低秩结构的稀疏观测地质场重建方法，结合张量补全与地统计学，优于普通克里金法。
...
Method: 将部分观测的空间场建模为多维张量，通过全局低秩结构恢复缺失值，结合张量补全与地统计学。

Result: 在合成地质场实验中，该方法在不同观测比例下均显著优于普通克里金法。

Conclusion: 该方法为地质场重建提供了鲁棒的优化框架，具有更高的重建精度。
...
### [329] [Invisible Backdoor Triggers in Image Editing Model via Deep Watermarking](https://arxiv.org/abs/2506.04879)
*Yu-Feng Chen,Tzuhsuan Huang,Pin-Yen Chiu,Jun-Cheng Chen*

Main category: cs.CV

TLDR: 该论文提出了一种新型的后门攻击框架，通过毒化训练数据在图像编辑过程中嵌入不可见触发器，利用现有水印模型实现隐蔽攻击。


<details>
  <summary>Details</summary>
Motivation: 现有后门攻击研究多集中于图像生成，而图像编辑领域的攻击研究较少，且现有方法多使用可见触发器，实用性不足。

Method: 利用现成的水印模型将不可见水印编码为后门触发器，通过毒化训练数据实现攻击。

Result: 在不同水印模型上实验表明，该方法攻击成功率较高，且水印特性分析验证了其有效性。

Conclusion: 该方法为图像编辑领域的后门攻击提供了隐蔽且有效的解决方案。
...
### [330] [Learning to Plan via Supervised Contrastive Learning and Strategic Interpolation: A Chess Case Study](https://arxiv.org/abs/2506.04892)
*Andrew Hamara,Greg Hamerly,Pablo Rivas,Andrew C. Freeman*

Main category: cs.CV

TLDR: 论文提出了一种基于直觉驱动的规划方法，通过对比学习训练Transformer编码器，将棋盘状态嵌入到结构化潜在空间中，实现无需深度搜索的走子选择。
...
Method: 使用监督对比学习训练Transformer编码器，将棋盘状态嵌入到由位置评估结构化的潜在空间中。在该空间中，距离反映评估相似性，可视化轨迹显示游戏状态间的可解释过渡。

Result: 模型仅使用6层束搜索，估计Elo评分为2593。性能随模型规模和嵌入维度提升，表明潜在规划可能替代传统搜索。

Conclusion: 尽管聚焦于国际象棋，但提出的基于嵌入的规划方法可推广到其他完美信息游戏。所有源代码已公开。
...
### [331] [From Objects to Anywhere: A Holistic Benchmark for Multi-level Visual Grounding in 3D Scenes](https://arxiv.org/abs/2506.04897)
*Tianxu Wang,Zhuofan Zhang,Ziyu Zhu,Yue Fan,Jing Xiong,Pengxiang Li,Xiaojian Ma,Qing Li*

Main category: cs.CV

TLDR: Anywhere3D-Bench是一个全面的3D视觉定位基准，涵盖四种不同级别的任务，揭示了当前模型在空间和部分级别任务上的显著不足。
...
Method: 提出Anywhere3D-Bench基准，评估多种先进3D视觉定位方法及LLMs/MLLMs的性能。

Result: 空间和部分级别任务表现最差，最佳模型OpenAI o4-mini的准确率分别仅为23.57%和33.94%。
...
### [332] [Generating Synthetic Stereo Datasets using 3D Gaussian Splatting and Expert Knowledge Transfer](https://arxiv.org/abs/2506.04908)
*Filip Slezak,Magnus K. Gjerde,Joakim B. Haurum,Ivan Nikolov,Morten S. Laursen,Thomas B. Moeslund*

Main category: cs.CV

TLDR: 本文提出了一种基于3D高斯泼溅（3DGS）的立体数据集生成方法，替代了基于神经辐射场（NeRF）的方法，并通过专家知识转移利用几何重建和深度估计，展示了在零样本泛化基准上的竞争力。


<details>
  <summary>Details</summary>
Motivation: 探索低成本、高保真数据集生成方法，并快速微调深度立体模型，同时揭示3DGS方法在复杂场景中的鲁棒性不足。

Method: 结合3DGS生成的几何重建和FoundationStereo模型的深度估计，通过专家知识转移生成数据集，并微调立体模型。

Result: 使用FoundationStereo的深度估计能生成更干净的数据集，提升零样本泛化性能；而直接使用几何重建则引入噪声。

Conclusion: 3DGS方法在低成本数据集生成和快速微调方面具有潜力，但其在复杂场景中的鲁棒性仍需改进。
...
### [333] [Light and 3D: a methodological exploration of digitisation techniques adapted to a selection of objects from the Mus{é}e d'Arch{é}ologie Nationale](https://arxiv.org/abs/2506.04925)
...
TLDR: 论文探讨了文化遗产数字化中3D摄影方法的多样性，强调应根据对象特性和未来用途选择合适方法，而非追求绝对分类。


<details>
  <summary>Details</summary>
Motivation: 文化遗产数字化需求广泛，但现有3D数字化方法多样，需针对不同对象灵活选择。

Method: 通过博物馆藏品案例，分析不同3D摄影方法的适用性，强调多方协作选择最佳工具。

Result: 研究表明，没有单一方法适用于所有对象，需根据对象特性和用途定制数字化方案。

Conclusion: 文化遗产数字化应注重灵活性和适应性，避免绝对分类，选择最适合的工具。
...
### [334] [CzechLynx: A Dataset for Individual Identification and Pose Estimation of the Eurasian Lynx](https://arxiv.org/abs/2506.04931)
...
TLDR: CzechLynx是一个大规模、开放访问的数据集，用于欧亚猞猁的个体识别、2D姿态估计和实例分割，包含30k+相机陷阱图像和100k+合成图像，覆盖219个独特个体和15年监测数据。


<details>
  <summary>Details</summary>
Motivation: 填补欧亚猞猁个体识别和姿态估计领域缺乏大规模开放数据集的空白，支持跨时空域的模型泛化测试。

Method: 收集30k+相机陷阱图像并标注分割掩码、身份标签和20点骨架；生成100k+合成图像以增加数据多样性。

Result: 数据集包含219个独特个体，覆盖两个地理区域15年的数据，并定义了三种评估协议以测试模型泛化能力。

Conclusion: CzechLynx将为先进模型的基准测试和新方法开发提供重要支持，尤其在动物个体重识别领域。
...
### [335] [Time-Lapse Video-Based Embryo Grading via Complementary Spatial-Temporal Pattern Mining](https://arxiv.org/abs/2506.04950)
*Yong Sun,Yipeng Wang,Junyu Shi,Zhiyuan Zhang,Yanmei Xiao,Lei Zhu,Manxi Jiang,Qiang Nie*

Main category: cs.CV

TLDR: 论文提出了一种基于视频的胚胎分级任务，利用全时长延时监测视频预测胚胎质量，并提出了互补时空模式挖掘框架（CoSTeM），结合形态和动态特征，显著提升了AI辅助胚胎选择的实用性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在胚胎评估中缺乏全面性，或受胚胎外因素干扰，限制了临床应用。本文旨在通过直接利用延时监测视频预测胚胎质量，填补这一空白。

Method: 提出CoSTeM框架，包含形态分支（局部结构特征选择）和形态动力学分支（全局发育轨迹建模），结合静态与动态特征。

Result: 实验结果表明CoSTeM框架优于现有方法，为AI辅助胚胎选择提供了有效工具。

Conclusion: 该研究为胚胎质量评估提供了新范式，数据集和代码将公开，推动相关领域发展。
...
### [336] [Robustness as Architecture: Designing IQA Models to Withstand Adversarial Perturbations](https://arxiv.org/abs/2506.04951)
*Igor Meleshin,Anna Chistyakova,Anastasia Antsiferova,Dmitriy Vatolin*

Main category: cs.CV

TLDR: 提出了一种通过设计而非学习来增强图像质量评估（IQA）模型鲁棒性的方法，通过正交信息流和规范保持操作实现。


<details>
  <summary>Details</summary>
Motivation: 传统的数据驱动防御方法（如对抗训练）可能不是解决IQA模型脆弱性的最佳途径，探索通过架构设计实现鲁棒性。

Method: 通过强制正交信息流和规范保持操作重构模型内部结构，辅以剪枝和微调，提升模型对抗攻击的能力。

Result: 设计出一种无需对抗训练即可抵御攻击的鲁棒IQA架构。

Conclusion: 建议从数据优化转向设计优化，以更根本地提升模型鲁棒性。
...
### [337] [APVR: Hour-Level Long Video Understanding with Adaptive Pivot Visual Information Retrieval](https://arxiv.org/abs/2506.04953)
*Hong Gao,Yiming Bao,Xuezhan Tu,Bin Zhong,Minling Zhang*

Main category: cs.CV

TLDR: APVR是一种无需训练的双粒度框架，通过分层视觉信息检索解决长视频理解中的计算和效率问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于视频的多模态大语言模型因计算限制和长时序信息提取效率低，难以处理小时级视频。

Method: APVR包含两个互补组件：Pivot Frame Retrieval（语义扩展和多模态置信度评分）和Pivot Token Retrieval（查询感知的注意力驱动标记选择）。

Result: 在LongVideoBench和VideoMME上的实验验证了其显著性能提升，达到SOTA结果。

Conclusion: APVR不仅适用于无需训练的场景，还能与现有MLLM架构无缝集成。
...
### [338] [FEAT: Full-Dimensional Efficient Attention Transformer for Medical Video Generation](https://arxiv.org/abs/2506.04956)
*Huihan Wang,Zhiwen Yang,Hui Zhang,Dan Zhao,Bingzheng Wei,Yan Xu*

Main category: cs.CV

TLDR: FEAT是一种高效的全维度注意力Transformer，通过空间-时间-通道注意力机制、线性复杂度设计和残差值引导模块，解决了动态医学视频合成的挑战。


<details>
  <summary>Details</summary>
Motivation: 动态医学视频合成需要同时建模空间一致性和时间动态性，现有Transformer方法存在通道交互不足、计算复杂度高和去噪指导粗糙的问题。
...
### [339] [Bringing SAM to new heights: Leveraging elevation data for tree crown segmentation from drone imagery](https://arxiv.org/abs/2506.04970)
*Mélisande Teng,Arthur Ouaknine,Etienne Laliberté,Yoshua Bengio,David Rolnick,Hugo Larochelle*

Main category: cs.CV

TLDR: 论文比较了利用SAM模型进行无人机图像中树冠实例分割的方法，并提出了结合DSM信息的BalSAM模型，在特定场景下表现优于其他方法。
...
Method: 比较了SAM模型在三种森林类型中的树冠分割效果，并研究了DSM信息的整合。

Result: BalSAM模型在种植园场景中表现优异，但未调优的SAM模型效果不及定制Mask R-CNN。

Conclusion: 端到端调优SAM及整合DSM信息是提升树冠分割模型的有效方向。
...
### [340] [TextVidBench: A Benchmark for Long Video Scene Text Understanding](https://arxiv.org/abs/2506.04983)
*Yangyang Zhong,Ji Qi,Yuan Yao,Pengxin Luo,Yunfeng Yan,Donglian Qi,Zhiyuan Liu,Tat-Seng Chua*

Main category: cs.CV

TLDR: 论文提出了TextVidBench，首个针对长视频文本问答的基准测试，包含跨领域视频、三阶段评估框架和高质量标注，并提出改进大模型的方法。
...
Method: 引入IT-Rope机制、时间提示工程、非均匀位置编码和轻量级微调，以提升长视频文本理解能力。

Result: TextVidBench对现有模型提出显著挑战，所提方法在多个数据集上验证有效。

Conclusion: TextVidBench为长视频文本理解提供了新基准，所提方法为改进模型能力提供了实用方案。
...
### [341] [Multi-scale Image Super Resolution with a Single Auto-Regressive Model](https://arxiv.org/abs/2506.04990)
*Enrique Sanchez,Isma Hadji,Adrian Bulat,Christos Tzelepis,Brais Martinez,Georgios Tzimiropoulos*

Main category: cs.CV

TLDR: 本文提出了一种基于视觉自回归（VAR）模型的图像超分辨率（ISR）方法，通过引入多尺度图像标记化和直接偏好优化（DPO）正则化，解决了现有方法的固定分辨率限制和大模型依赖问题。


<details>
  <summary>Details</summary>
Motivation: 现有VARSR方法在图像超分辨率中存在固定分辨率限制和大模型依赖问题，本文旨在通过新技术解决这些限制。

Method: 提出两种新组件：1）多尺度图像标记化方法，强制不同尺度间的标记重叠；2）DPO正则化，鼓励模型生成高分辨率标记。

Result: 模型在单次前向传播中实现去噪和超分辨率，并在小模型（300M参数）下达到SOTA效果，无需外部训练数据。

Conclusion: 新方法在性能和效率上优于现有技术，展示了多尺度标记化和DPO在ISR中的潜力。
...
### [342] [PATS: Proficiency-Aware Temporal Sampling for Multi-View Sports Skill Assessment](https://arxiv.org/abs/2506.04996)
*Edoardo Bianchi,Antonio Liotta*

Main category: cs.CV

TLDR: PATS是一种新颖的视频采样策略，通过保留完整的基本运动片段来提升多视角技能评估的准确性。


<details>
  <summary>Details</summary>
Motivation: 当前视频采样方法破坏了技能评估所需的时序连续性，PATS旨在解决这一问题。

Method: PATS自适应地分割视频，确保每个分析片段包含关键性能组件的完整执行，并在多个片段中重复此过程。

Result: 在EgoExo4D基准测试中，PATS在所有视角配置下均优于现有方法，尤其在动态运动（如攀岩）中表现突出。

Conclusion: PATS是一种自适应时序采样方法，有效提升了真实场景中自动化技能评估的准确性。
...
### [343] [Beyond Cropped Regions: New Benchmark and Corresponding Baseline for Chinese Scene Text Retrieval in Diverse Layouts](https://arxiv.org/abs/2506.04999)
*Gengluo Li,Huawen Shen,Yu Zhou*

Main category: cs.CV

TLDR: 论文提出了一种针对中文场景文本检索的新模型CSTR-CLIP，通过结合全局视觉信息和多粒度对齐训练，显著提升了性能，并建立了DL-CSVTR基准数据集。


<details>
  <summary>Details</summary>
Motivation: 中文场景文本检索因复杂多样的布局而极具挑战性，现有方法多借鉴英文解决方案，效果不佳。

Method: 提出CSTR-CLIP模型，采用两阶段训练，结合全局视觉信息和多粒度对齐，解决现有方法的局限性。

Result: 在现有基准上，CSTR-CLIP准确率提升18.82%，推理速度更快，且在DL-CSVTR上表现优异。

Conclusion: CSTR-CLIP有效解决了中文场景文本检索的多样性布局问题，数据集和代码将公开以促进研究。
...
### [344] [Structure-Aware Radar-Camera Depth Estimation](https://arxiv.org/abs/2506.05008)
*Fuyi Zhang,Zhu Yu,Chunhao Li,Runmin Zhang,Xiaokai Bai,Zili Zhou,Si-Yuan Cao,Wang Wang,Hui-Liang Shen*

Main category: cs.CV

TLDR: 论文探讨了单目深度估计的进展，重点介绍了基于深度学习的方法及其挑战，特别是泛化到未见领域的问题。


<details>
  <summary>Details</summary>
Motivation: 单目深度估计旨在从单目相机捕获的RGB图像中确定每个像素的深度。深度学习的发展推动了该领域的进步，但泛化到未见领域仍是一个挑战。

Method: 论文回顾了多种方法，包括多尺度融合网络、将回归任务重新解释为分类问题、引入额外先验以及开发更有效的目标函数。近期方法采用仿射不变损失实现多数据集联合训练。

Result: Depth Anything在零样本单目深度估计中表现领先，能够从未见图像中提取结构信息，生成结构细节丰富的深度图，但在准确度量深度估计上仍有不足。

Conclusion: 尽管单目深度估计取得了显著进展，但在泛化性和度量深度准确性方面仍需进一步研究。
...
### [345] [Point Cloud Segmentation of Agricultural Vehicles using 3D Gaussian Splatting](https://arxiv.org/abs/2506.05009)
...
TLDR: 提出了一种利用3D高斯泼溅和高斯不透明度场生成合成数据的流程，用于3D点云语义分割任务，避免了真实数据的高成本标注。


<details>
  <summary>Details</summary>
Motivation: 真实点云数据的获取和标注成本高且耗时，因此需要一种经济高效的替代方案。

Method: 通过3D高斯泼溅和高斯不透明度场生成农业车辆的3D资产，并在模拟环境中使用模拟LiDAR生成点云数据。

Result: 仅使用合成数据训练的模型（如PTv3）在mIoU上达到91.35%，某些情况下甚至优于真实数据训练的模型。

Conclusion: 合成数据可以替代真实数据用于训练，且在某些场景下表现更优，同时模型能够泛化到未见过的语义类别。
...
### [346] [UAV4D: Dynamic Neural Rendering of Human-Centric UAV Imagery using Gaussian Splatting](https://arxiv.org/abs/2506.05011)
*Jaehoon Choi,Dongki Jung,Christopher Maxey,Yonghan Lee,Sungmin Eum,Dinesh Manocha,Heesung Kwon*

Main category: cs.CV

TLDR: UAV4D框架解决了无人机拍摄动态场景的渲染问题，通过结合3D基础模型和人体网格重建模型，实现了单目视频数据下的多行人动态场景重建。


<details>
  <summary>Details</summary>
Motivation: 现有动态神经渲染方法无法有效处理无人机拍摄场景的独特挑战，如单目相机、俯视角和多行人移动，且缺乏相关数据集。

Method: 结合3D基础模型和人体网格重建模型，通过人体-场景接触点解决尺度模糊问题，并利用SMPL模型和背景网格初始化高斯溅射进行整体渲染。

Result: 在三个复杂无人机数据集上验证，PSNR提升1.5 dB，视觉清晰度优于现有方法。

Conclusion: UAV4D框架在无人机动态场景渲染中表现出色，解决了现有方法的局限性。
...
### [347] [Physical Annotation for Automated Optical Inspection: A Concept for In-Situ, Pointer-Based Trainingdata Generation](https://arxiv.org/abs/2506.05026)
*Oliver Krumpek,Oliver Heimann,Jörg Krüger*

Main category: cs.CV

TLDR: 提出了一种新型物理标注系统，用于为自动光学检测生成训练数据，通过指针交互将专家知识直接输入机器学习训练流程。


<details>
  <summary>Details</summary>
Motivation: 解决传统屏幕标注方法的不足，提供更直观高效的数据标注方式，并利用非IT专家的经验。

Method: 使用校准的跟踪指针记录用户输入，结合投影界面辅助标注，将物理轨迹转化为标准标注格式。

Result: 初步评估证实了捕获详细标注轨迹的可行性，并与CVAT集成优化了后续机器学习任务的工作流程。

Conclusion: 该系统填补了人类专业知识与自动化数据生成之间的空白，为未来自动光学检测的机器学习数据生成提供了潜力。
...
### [348] [FlowDirector: Training-Free Flow Steering for Precise Text-to-Video Editing](https://arxiv.org/abs/2506.05046)
*Guangzhao Li,Yanming Yang,Chenxi Song,Chi Zhang*

Main category: cs.CV

TLDR: FlowDirector提出了一种无需反转的视频编辑框架，通过ODE引导视频在数据空间中直接演化，保持时间一致性和结构细节。


<details>
  <summary>Details</summary>
Motivation: 现有基于反转的方法导致时间不一致和结构退化，FlowDirector旨在解决这些问题。

Method: 采用ODE引导视频演化，引入注意力掩码机制和增强编辑策略。

Result: 在指令遵循、时间一致性和背景保留方面表现最佳。
...
### [349] [A Survey on Vietnamese Document Analysis and Recognition: Challenges and Future Directions](https://arxiv.org/abs/2506.05061)
*Anh Le,Thanh Lam,Dung Nguyen*

Main category: cs.CV

TLDR: 该论文综述了越南文档分析与识别（DAR）的现状，探讨了传统OCR和深度学习方法面临的挑战，并提出了利用大语言模型（LLMs）和视觉语言模型改进的方向。


<details>
  <summary>Details</summary>
Motivation: 越南文本识别因复杂的变音符号、声调变化和缺乏大规模标注数据集而面临独特挑战，传统方法在真实文档中表现不佳，深度学习受限于数据稀缺和泛化问题。

Method: 论文通过综述现有技术，分析越南DAR的局限性，并探讨LLMs和视觉语言模型的应用潜力。

Result: LLMs和视觉语言模型在文本识别和文档理解方面表现出显著改进，但仍需解决领域适应、多模态学习和计算效率等问题。

Conclusion: 未来研究方向包括数据集开发、模型优化和多模态方法整合，以推动越南DAR的进步和社区驱动的解决方案。
...
### [350] [SeedEdit 3.0: Fast and High-Quality Generative Image Editing](https://arxiv.org/abs/2506.05083)
*Peng Wang,Yichun Shi,Xiaochen Lian,Zhonghua Zhai,Xin Xia,Xuefeng Xiao,Weilin Huang,Jianchao Yang*

Main category: cs.CV

TLDR: SeedEdit 3.0 是 Seedream 3.0 的配套工具，显著提升了编辑指令遵循和图像内容保留能力。通过改进数据整理流程和引入联合学习管道，实现了更好的性能。


<details>
  <summary>Details</summary>
Motivation: 提升图像编辑工具在指令遵循和内容保留方面的表现，同时扩展数据来源和优化模型训练。

Method: 1. 改进数据整理流程，引入元信息嵌入策略；2. 提出联合学习管道，结合扩散损失和奖励损失。

Result: 在真实图像编辑测试中，SeedEdit 3.0 实现了56.1%的高可用率，优于其他版本和竞品。
...
### [351] [Interpretable Multimodal Framework for Human-Centered Street Assessment: Integrating Visual-Language Models for Perceptual Urban Diagnostics](https://arxiv.org/abs/2506.05087)
*HaoTian Lan*

Main category: cs.CV

TLDR: 该研究提出了一种多模态街道评估框架（MSEF），结合视觉和语言模型，用于评估街道景观的客观特征和主观感知，并在中国哈尔滨的15,000张街景图像上验证了其有效性。
...
Method: 研究结合视觉变换器（VisualGLM-6B）和大型语言模型（GPT-4），使用LoRA和P-Tuning v2进行参数高效微调。
...
### [352] [FG 2025 TrustFAA: the First Workshop on Towards Trustworthy Facial Affect Analysis: Advancing Insights of Fairness, Explainability, and Safety (TrustFAA)](https://arxiv.org/abs/2506.05095)
*Jiaee Cheong,Yang Liu,Harold Soh,Hatice Gunes*

Main category: cs.CV

TLDR: 该研讨会旨在探讨情感AI驱动的面部情感分析（FAA）工具的信任问题，聚焦公平性、可解释性和安全性。


<details>
  <summary>Details</summary>
Motivation: 随着FAA工具的普及，其信任问题（如偏见、隐私）日益突出，需多领域研究解决。

Method: 通过研讨会形式，汇集研究者探讨FAA任务中的信任挑战，如微表情识别、人机交互等。

Result: 推动FG2025伦理目标，促进可信FAA的研究与讨论。

Conclusion: 研讨会旨在推动FAA领域的信任问题研究，支持伦理导向的技术发展。
...
### [353] [Astraea: A GPU-Oriented Token-wise Acceleration Framework for Video Diffusion Transformers](https://arxiv.org/abs/2506.05096)
*Haosong Liu,Yuge Cheng,Zihan Liu,Aiyue Chen,Yiwu Yao,Chen Chen,Jingwen Leng,Yu Feng,Minyi Guo*

Main category: cs.CV

TLDR: ASTRAEA是一个自动框架，通过轻量级令牌选择和高效稀疏注意力策略，显著加速视频扩散变压器的推理速度，同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 视频扩散变压器（vDiT）在文本到视频生成方面取得了显著进展，但其高计算需求限制了实际部署。现有加速方法多依赖启发式方法，适用性有限。

Method: ASTRAEA提出轻量级令牌选择机制和内存高效的GPU并行稀疏注意力策略，结合进化算法自动优化令牌预算分配。

Result: 在单GPU上实现2.4倍推理加速，8GPU上可达13.2倍，视频质量损失极小（VBench得分损失<0.5%）。
...
### [354] [DIMCIM: A Quantitative Evaluation Framework for Default-mode Diversity and Generalization in Text-to-Image Generative Models](https://arxiv.org/abs/2506.05108)
*Revant Teotia,Candace Ross,Karen Ullrich,Sumit Chopra,Adriana Romero-Soriano,Melissa Hall,Matthew J. Muckley*

Main category: cs.CV

TLDR: 论文提出了DIM-CIM框架，用于无参考地测量文本到图像模型的多样性和泛化能力，并构建了COCO-DIMCIM基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法依赖参考图像数据集或缺乏多样性测量的特异性，限制了适应性和可解释性。

Method: 引入DIM-CIM框架，通过COCO-DIMCIM基准测试评估模型的默认模式多样性和泛化能力。

Result: 发现模型参数规模增加会牺牲默认模式多样性以提升泛化能力，并识别了细粒度失败案例。

Conclusion: DIM-CIM提供了灵活且可解释的框架，有助于更全面地理解模型性能。
...
### [355] [Practical Manipulation Model for Robust Deepfake Detection](https://arxiv.org/abs/2506.05119)
*Benedikt Hopf,Radu Timofte*

Main category: cs.CV

TLDR: 论文提出了一种实用的伪造模型（PMM），通过扩展伪造空间和增强训练图像的退化，显著提升了深度伪造检测模型的鲁棒性和性能。


<details>
  <summary>Details</summary>
Motivation: 当前深度伪造检测模型在非理想条件下表现不稳定，容易被规避。受图像超分辨率领域启发，作者希望通过更全面的伪造模型提升检测器的实用性。

Method: 开发了PMM，利用泊松混合、多样化掩码、生成器伪影和干扰物扩展伪造空间，并通过添加强退化增强训练图像的多样性。

Result: 在DFDC和DFDCP数据集上，AUC分别提升了3.51%和6.21%，显著提高了模型的鲁棒性和性能。

Conclusion: PMM有效提升了深度伪造检测的鲁棒性，解决了现有检测器在非理想条件下的不足。
...
### [356] [CIVET: Systematic Evaluation of Understanding in VLMs](https://arxiv.org/abs/2506.05146)
*Massimo Rizzoli,Simone Alghisi,Olha Khomyn,Gabriel Roccabruna,Seyed Mahed Mousavi,Giuseppe Riccardi*

Main category: cs.CV

TLDR: CIVET框架用于系统评估视觉语言模型（VLMs）对场景结构和语义的理解能力，发现现有模型在对象属性、位置和关系理解上存在局限，且未达到人类水平。
...
Result: VLMs仅能识别有限基本对象属性，性能受对象位置影响，且难以理解对象间关系，未达人类水平。

Conclusion: VLMs在场景理解上仍有显著不足，需进一步改进。
...
### [357] [FRED: The Florence RGB-Event Drone Dataset](https://arxiv.org/abs/2506.05163)
*Gabriele Magrini,Niccolò Marini,Federico Becattini,Lorenzo Berlincioni,Niccolò Biondi,Pietro Pala,Alberto Del Bimbo*

Main category: cs.CV

TLDR: 论文介绍了Florence RGB-Event Drone数据集（FRED），专为无人机检测、跟踪和轨迹预测设计，结合RGB视频和事件流，包含7小时密集标注数据。


<details>
  <summary>Details</summary>
Motivation: 传统RGB相机在捕捉快速移动的无人机时受限，事件相机虽具优势，但现有基准缺乏高时间分辨率或无人机特定运动模式。

Method: 提出FRED数据集，包含RGB和事件流数据，覆盖多种无人机模型及挑战性场景（如雨天和恶劣光照）。

Result: FRED提供7小时密集标注数据，支持无人机检测、跟踪和轨迹预测任务，并提供了标准评估协议。
...
### [358] [Through-the-Wall Radar Human Activity Recognition WITHOUT Using Neural Networks](https://arxiv.org/abs/2506.05169)
*Weicheng Gao*

Main category: cs.CV

TLDR: 论文提出了一种不使用神经网络的方法，通过模板匹配和拓扑相似性计算实现穿墙雷达中的人类活动识别。


<details>
  <summary>Details</summary>
Motivation: 作者认为现有研究过度依赖神经网络，忽视了早期基于模板匹配的方法的物理可解释性和理论基础，因此尝试回归原始路径。

Method: 首先生成雷达的时域图和多普勒图，通过角点检测确定目标区域，使用多相主动轮廓模型分割微多普勒特征，并将其离散化为点云，最后通过Mapper算法计算与模板数据的拓扑相似性。

Result: 通过数值模拟和实测实验验证了方法的有效性。

Conclusion: 该方法在不使用神经网络的情况下实现了智能识别，展示了模板匹配方法的潜力。
...
### [359] [Track Any Anomalous Object: A Granular Video Anomaly Detection Pipeline](https://arxiv.org/abs/2506.05175)
...
TLDR: 提出了一种名为TAO的新框架，用于细粒度视频异常检测，通过像素级跟踪异常对象，无需阈值调优，提高了异常定位的精确性。
...
Method: TAO框架首次将多细粒度异常对象检测统一起来，将问题转化为像素级跟踪，并与分割和跟踪任务结合。

Result: 实验表明，TAO在准确性和鲁棒性上设定了新基准。
...
### [360] [Single GPU Task Adaptation of Pathology Foundation Models for Whole Slide Image Analysis](https://arxiv.org/abs/2506.05184)
...
TLDR: TAPFM是一种针对病理基础模型（PFMs）的单GPU任务适应方法，通过ViT注意力进行MIL聚合，优化特征表示和注意力权重，在临床任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 适应预训练的PFMs用于特定临床任务面临挑战，主要由于WSI级标签的弱监督性和需要MIL范式。
...
### [361] [MokA: Multimodal Low-Rank Adaptation for MLLMs](https://arxiv.org/abs/2506.05191)
*Yake Wei,Yu Miao,Dongzhan Zhou,Di Hu*

Main category: cs.CV

TLDR: 论文提出了一种多模态低秩适应方法（MokA），针对现有高效多模态微调方法的局限性，通过模态特定参数压缩单模态信息并增强跨模态交互，显著提升了多模态大语言模型的微调效果。


<details>
  <summary>Details</summary>
Motivation: 现有高效多模态微调方法多直接借用自大语言模型（LLMs），忽略了多模态场景的内在差异，导致无法充分利用所有模态。

Method: 提出MokA方法，结合单模态适应和跨模态适应，通过模态特定参数压缩单模态信息并显式增强跨模态交互。

Result: 在多种多模态场景（如视听文本、视觉文本、语音文本）和多种LLM骨干模型（如LLaMA2/3、Qwen2等）上均取得一致改进。
...
### [362] [Vision-Based Autonomous MM-Wave Reflector Using ArUco-Driven Angle-of-Arrival Estimation](https://arxiv.org/abs/2506.05195)
*Josue Marroquin,Nan Inzali,Miles Dillon Lantz,Campbell Freeman,Amod Ashtekar,\\Ajinkya Umesh Mulik,Mohammed E Eltayeb*

Main category: cs.CV

TLDR: 本文提出了一种基于视觉辅助的自主反射器系统，用于在非视距条件下增强毫米波通信性能，通过动态调整金属板反射信号。
...
Result: 实验结果显示，在60 GHz频段下，接收信号强度平均增益为23 dB，信号接收概率为0.89（阈值-65 dB）。

Conclusion: 该系统在复杂动态环境中展现出强大的毫米波通信适应性和韧性。
...
### [363] [Quantifying Cross-Modality Memorization in Vision-Language Models](https://arxiv.org/abs/2506.05198)
*Yuxin Wen,Yangsibo Huang,Tom Goldstein,Ravi Kumar,Badih Ghazi,Chiyuan Zhang*

Main category: cs.CV

TLDR: 研究探讨了多模态模型中跨模态记忆的特性，发现单模态学习的信息可迁移至另一模态，但存在显著差距，并提出缓解方法。


<details>
  <summary>Details</summary>
Motivation: 理解神经网络在训练中的记忆行为对隐私保护和知识获取至关重要，尤其是多模态模型的跨模态记忆特性。

Method: 引入合成人物数据集，量化单模态训练后的跨模态迁移能力，分析不同场景下的表现。

Result: 单模态学习的信息可迁移至另一模态，但存在显著差距，且差距在不同场景下普遍存在。

Conclusion: 提出基线方法缓解跨模态迁移差距，呼吁未来研究开发更鲁棒的多模态学习技术。
...
### [364] [Grounding Beyond Detection: Enhancing Contextual Understanding in Embodied 3D Grounding](https://arxiv.org/abs/2506.05199)
*Yani Zhang,Dongming Wu,Hao Shi,Yingfei Liu,Tiancai Wang,Haoqiang Fan,Xingping Dong*

Main category: cs.CV

TLDR: 论文探讨了3D目标检测模型在无需语言指令训练的情况下，其定位性能优于专门训练的3D语言定位模型，并提出了一种名为DEGround的新方法，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 研究发现现有的3D语言定位模型可能并未充分利用检测模型的潜力，甚至基础的类别级定位问题也未完全解决。

Method: 提出DEGround方法，共享检测和定位的对象表示，并引入区域激活模块和查询调制模块以增强语言理解。

Result: DEGround在EmbodiedScan验证集上的整体准确率比现有最佳模型BIP3D高出7.52%。

Conclusion: DEGround通过结合检测和定位任务，显著提升了3D语言定位的性能，证明了检测模型对定位任务的重要性。
...
### [365] [OGGSplat: Open Gaussian Growing for Generalizable Reconstruction with Expanded Field-of-View](https://arxiv.org/abs/2506.05204)
*Yanbo Wang,Ziyi Wang,Wenzhao Zheng,Jie Zhou,Jiwen Lu*

Main category: cs.CV

TLDR: OGGSplat是一种基于开放高斯增长的方法，用于从稀疏视图重建语义感知的3D场景，解决了现有方法在视野外区域重建的困难。


<details>
  <summary>Details</summary>
Motivation: 新兴应用（如虚拟现实和具身AI）需要从稀疏视图高效重建语义一致的3D场景，但现有方法要么需要密集视图，要么难以处理视野外区域。

Method: OGGSplat利用开放高斯的语义属性作为先验，通过RGB-语义一致修复模块和双向控制扩散模型，将修复区域提升到3D空间进行高斯参数优化。

Result: OGGSplat在Gaussian Outpainting基准测试中表现出色，能够从智能手机拍摄的两张视图重建语义感知的场景。

Conclusion: OGGSplat为稀疏视图下的语义感知3D重建提供了一种高效且通用的解决方案。
...
### [366] [Follow-Your-Motion: Video Motion Transfer via Efficient Spatial-Temporal Decoupled Finetuning](https://arxiv.org/abs/2506.05207)
*Yue Ma,Yulong Liu,Qiyuan Zhu,Ayden Yang,Kunyu Feng,Xinhua Zhang,Zhifeng Li,Sirui Han,Chenyang Qi,Qifeng Chen*

Main category: cs.CV

TLDR: 论文提出了一种名为Follow-Your-Motion的高效两阶段视频运动迁移框架，通过空间-时间解耦的LoRA和改进的训练策略，解决了现有方法在运动一致性和调优效率上的问题。
...
Method: 提出空间-时间解耦的LoRA，改进训练策略（稀疏运动采样和自适应RoPE），并引入MotionBench作为评测基准。

Result: 在MotionBench上验证了Follow-Your-Motion的优越性。

Conclusion: Follow-Your-Motion框架在复杂运动生成中表现优异，解决了现有方法的局限性。
...
### [367] [Towards Vision-Language-Garment Models For Web Knowledge Garment Understanding and Generation](https://arxiv.org/abs/2506.05210)
*Jan Ackermann,Kiyohiro Nakayama,Guandao Yang,Tong Wu,Gordon Wetzstein*

Main category: cs.CV

TLDR: VLG模型通过视觉-语言-服装多模态合成服装，展示了在零样本情况下对未见服装风格和提示的泛化能力。
...
Result: 初步结果表明VLG在零样本情况下具有较好的知识迁移能力。
...
### [368] [DSG-World: Learning a 3D Gaussian World Model from Dual State Videos](https://arxiv.org/abs/2506.05217)
*Wenhao Hu,Xuexiang Wen,Xi Li,Gaoang Wang*

Main category: cs.CV

TLDR: DSG-World提出了一种基于双状态观测的端到端3D高斯世界建模框架，解决了现有方法在训练难度和物理一致性上的不足。


<details>
  <summary>Details</summary>
Motivation: 从有限观测中构建高效且物理一致的世界模型是视觉和机器人领域的长期挑战。现有方法要么难以训练，要么缺乏3D或物理一致性。

Method: 利用同一场景在不同物体配置下的双状态观测，构建双分割感知高斯场，并通过双向光度和语义一致性增强稳定性。引入伪中间状态进行对称对齐，并设计协作剪枝策略优化几何完整性。

Result: DSG-World在显式高斯表示空间中实现了高效的真实到仿真转换，支持高保真渲染和物体级场景操作，无需密集观测或多阶段处理。

Conclusion: 实验表明，DSG-World在新视角和场景状态下具有强泛化能力，适用于真实世界的3D重建和仿真。
...
### [369] [MonkeyOCR: Document Parsing with a Structure-Recognition-Relation Triplet Paradigm](https://arxiv.org/abs/2506.05218)
*Zhang Li,Yuliang Liu,Qiang Liu,Zhiyin Ma,Ziyang Zhang,Shuo Zhang,Zidun Guo,Jiarui Zhang,Xinyu Wang,Xiang Bai*

Main category: cs.CV

TLDR: MonkeyOCR是一种基于SRR三元范式的视觉语言模型，用于文档解析，通过简化流程和高效处理，在精度和速度上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决现有文档解析方法（如多工具流水线或大型端到端模型）的复杂性和低效问题。

Method: 采用SRR（结构-识别-关系）三元范式，将文档解析分解为布局分析、内容识别和逻辑排序三个问题。

Result: 在MonkeyDoc数据集上，MonkeyOCR平均性能优于MinerU 5.1%，处理速度更快（0.84页/秒），且3B参数模型超越更大模型（如Qwen2.5-VL）。
...
### [370] [SAM-aware Test-time Adaptation for Universal Medical Image Segmentation](https://arxiv.org/abs/2506.05221)
*Jianghao Wu,Yicheng Wu,Yutong Xie,Wenjia Bai,You Zhang,Feilong Tang,Yulong Li,Yasmeen George,Imran Razzak*

Main category: cs.CV

TLDR: SAM-TTA提出了一种测试时适应框架，解决了SAM在医学图像分割中的输入和语义差异问题，表现优于现有方法。
...
Method: 结合SBCT（自适应贝塞尔曲线变换）和DUMT（双尺度不确定性驱动均值教师适应）框架，优化输入和语义对齐。

Result: 在五个公开数据集上表现优于现有TTA方法，甚至在某些情况下超越完全微调模型。
...
### [371] [Spatiotemporal Contrastive Learning for Cross-View Video Localization in Unstructured Off-road Terrains](https://arxiv.org/abs/2506.05250)
*Zhiyun Deng,Dongmyeong Lee,Amanda Adkins,Jesse Quattrociocchi,Christian Ellis,Joydeep Biswas*

Main category: cs.CV

TLDR: MoViX是一个自监督跨视图视频定位框架，用于GPS缺失的越野环境，通过学习和季节不变表示实现鲁棒定位。


<details>
  <summary>Details</summary>
Motivation: 解决越野环境中因重复植被、无结构地形和季节变化导致的定位挑战。

Method: 采用姿态依赖的正样本采样、时间对齐的硬负样本挖掘、运动信息帧采样器和轻量级时间聚合器。

Result: 在TartanDrive 2.0数据集上，93%的情况下定位误差在25米内，100%在50米内，优于现有方法。

Conclusion: MoViX在无需环境特定调优的情况下，实现了鲁棒的跨视图定位，并展示了泛化能力。
...
### [372] [LeanPO: Lean Preference Optimization for Likelihood Alignment in Video-LLMs](https://arxiv.org/abs/2506.05260)
*Xiaodong Wang,Jinfa Huang,Li Yuan,Peixi Peng*

Main category: cs.CV

TLDR: 论文提出了一种名为LeanPO的无参考方法，用于优化视频大语言模型（Video-LLMs）的偏好对齐，通过重新定义奖励和动态标签平滑策略，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有的偏好对齐技术（如DPO）在训练过程中可能导致目标和非目标响应的概率同时下降，这一问题在视频内容的冗余复杂性下更为突出。

Method: 提出LeanPO方法，将隐式奖励重新定义为响应相对于策略模型的平均似然，并结合自生成偏好数据管道和动态标签平滑策略。

Result: 实验表明，LeanPO显著提升了Video-LLMs的性能，且额外训练开销极小。

Conclusion: LeanPO为Video-LLMs的偏好对齐提供了一种简单有效的解决方案，推动了可靠高效Video-LLMs的发展。
...
### [373] [Can Foundation Models Generalise the Presentation Attack Detection Capabilities on ID Cards?](https://arxiv.org/abs/2506.05263)
*Juan E. Tapia,Christoph Busch*

Main category: cs.CV

TLDR: 该论文探讨了如何利用基础模型（FM）提升身份证展示攻击检测（PAD）的泛化能力，特别是在面对不同国家身份证时的表现。


<details>
  <summary>Details</summary>
Motivation: 当前PAD系统因隐私保护问题通常只针对少数国家身份证训练，导致泛化能力不足，无法满足商业需求。

Method: 研究采用零样本学习和微调方法，测试了两种身份证数据集（智利私有数据集和芬兰、西班牙、斯洛伐克公开数据集）。

Result: 研究发现，真实图像（bona fide）是提升泛化能力的关键。

Conclusion: 基础模型在提升PAD泛化能力方面具有潜力，真实图像数据对性能至关重要。
...
### [374] [From Play to Replay: Composed Video Retrieval for Temporally Fine-Grained Videos](https://arxiv.org/abs/2506.05274)
*Animesh Gupta,Jay Parmar,Ishan Rajendrakumar Dave,Mubarak Shah*

Main category: cs.CV

TLDR: TF-CoVR是一个专注于时间细粒度视频检索的新基准，提出了一个两阶段训练框架TF-CoVR-Base，显著提升了检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有CoVR基准未能测试对快速、细微时间差异的捕捉能力，因此需要一个新的基准来填补这一空白。

Method: 构建TF-CoVR基准，使用LLM生成查询与修改文本对，并提出两阶段训练框架（预训练视频编码器和对比学习对齐）。

Result: TF-CoVR-Base在零样本和微调后均显著提升了检索性能（mAP@50从5.92提升到7.51和19.83提升到25.82）。

Conclusion: TF-CoVR为时间细粒度视频检索提供了有效的基准和方法，显著提升了性能。
...
### [375] [Unifying Appearance Codes and Bilateral Grids for Driving Scene Gaussian Splatting](https://arxiv.org/abs/2506.05280)
...
TLDR: 提出了一种多尺度双边网格方法，统一了外观编码和双边网格，显著提升了动态自动驾驶场景重建的几何精度。


<details>
  <summary>Details</summary>
Motivation: 现实场景中难以保证图像的光度一致性，现有方法（如外观编码和双边网格）存在局限性，需要更有效的解决方案。

Method: 提出多尺度双边网格，结合外观编码和双边网格的优势，优化动态场景重建。

Result: 在Waymo等四个数据集上表现优异，几何精度显著提升，减少了光度不一致导致的伪影。

Conclusion: 多尺度双边网格有效解决了光度不一致问题，对自动驾驶场景重建具有重要意义。
...
### [376] [Rectified Point Flow: Generic Point Cloud Pose Estimation](https://arxiv.org/abs/2506.05282)
*Tao Sun,Liyuan Zhu,Shengyu Huang,Shuran Song,Iro Armeni*

Main category: cs.CV

TLDR: Rectified Point Flow 是一种统一参数化方法，将点云配准和多部件形状组装视为条件生成问题，通过学习连续点速度场实现目标位置恢复。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法在对称性处理和部件姿态回归上的局限性，无需对称标签即可学习组装对称性。

Method: 通过自监督编码器学习点速度场，将噪声点传输至目标位置，恢复部件姿态。

Result: 在六个基准测试中达到最新性能，统一框架支持联合训练，提升几何先验学习。

Conclusion: Rectified Point Flow 在点云配准和形状组装任务中表现出色，统一框架增强了泛化能力。
...
### [377] [Video World Models with Long-term Spatial Memory](https://arxiv.org/abs/2506.05284)
*Tong Wu,Shuai Yang,Ryan Po,Yinghao Xu,Ziwei Liu,Dahua Lin,Gordon Wetzstein*

Main category: cs.CV

TLDR: 提出了一种基于几何空间记忆的框架，以增强视频世界模型的长期一致性，解决了现有模型因时间窗口限制导致的场景遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型因时间上下文窗口有限，难以维持场景一致性，导致遗忘问题。受人类记忆机制启发，提出改进方案。

Method: 引入几何基础的长时空间记忆框架，包括信息存储与检索机制，并定制数据集训练和评估模型。

Result: 评估显示，模型在质量、一致性和上下文长度上优于基线，实现了长期一致的场景生成。
...
### [378] [RaySt3R: Predicting Novel Depth Maps for Zero-Shot Object Completion](https://arxiv.org/abs/2506.05285)
*Bardienus P. Duisterhof,Jan Oberst,Bowen Wen,Stan Birchfield,Deva Ramanan,Jeffrey Ichnowski*

Main category: cs.CV

TLDR: RaySt3R将3D形状补全重新定义为新视角合成问题，通过单张RGB-D图像和多视角查询射线预测深度图、物体掩码和置信度，实现高效且一致的3D重建。
...
Method: 利用单张RGB-D图像和查询射线，通过前馈变换器预测深度图、掩码和置信度，并融合多视角预测完成3D重建。
...
### [379] [Stable Vision Concept Transformers for Medical Diagnosis](https://arxiv.org/abs/2506.05286)
*Lijie Hu,Songning Lai,Yuan Hua,Shu Yang,Jingfeng Zhang,Di Wang*

Main category: cs.CV

TLDR: 本文提出Vision Concept Transformer (VCT)和Stable Vision Concept Transformer (SVCT)，解决医疗领域中可解释AI的性能和稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 医疗领域对透明度的需求促使研究可解释AI，但现有概念瓶颈模型(CBMs)忽视图像特征且性能不稳定。

Method: VCT结合概念特征与图像特征，SVCT基于VCT，引入ViT和Denoised Diffusion Smoothing提升稳定性。

Result: 在四个医疗数据集上，VCT和SVCT保持准确性且可解释，SVCT在扰动下仍提供稳定解释。

Conclusion: VCT和SVCT满足医疗领域对准确性和解释稳定性的需求。
...
### [380] [EOC-Bench: Can MLLMs Identify, Recall, and Forecast Objects in an Egocentric World?](https://arxiv.org/abs/2506.05287)
*Yuqian Yuan,Ronghao Dang,Long Li,Wentong Li,Dian Jiao,Xin Li,Deli Zhao,Fan Wang,Wenqiao Zhang,Jun Xiao,Yueting Zhuang*

Main category: cs.CV

TLDR: EOC-Bench是一个创新的基准测试，用于评估动态第一人称场景中的物体中心化认知能力，填补了现有基准测试在动态交互评估上的空白。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注静态场景，忽视了用户交互引起的动态变化，因此需要新的评估工具。

Method: 开发了EOC-Bench，包含3,277个标注QA对，分为三类时间类别，采用混合格式标注框架和多尺度时间准确性指标。

Result: 对多种MLLMs进行了全面评估，EOC-Bench为提升MLLMs的物体认知能力提供了重要工具。

Conclusion: EOC-Bench为开发可靠的嵌入式系统核心模型奠定了基础。
...
### [381] [AliTok: Towards Sequence Modeling Alignment between Tokenizer and Autoregressive Model](https://arxiv.org/abs/2506.05289)
*Pingyu Wu,Kai Zhu,Yu Liu,Longxiang Tang,Jian Yang,Yansong Peng,Wei Zhai,Yang Cao,Zheng-Jun Zha*

Main category: cs.CV

TLDR: 提出了一种新的Aligned Tokenizer（AliTok），通过因果解码器建立单向依赖关系，优化自回归模型的图像生成性能。
...
Result: 在ImageNet-256基准测试中，AliTok在177M参数下gFID为1.50，IS为305.9；662M参数时gFID为1.35，超越现有扩散方法，采样速度快10倍。

Conclusion: AliTok通过单向依赖和一致性优化，显著提升了自回归图像生成的性能。
...
### [382] [SeedVR2: One-Step Video Restoration via Diffusion Adversarial Post-Training](https://arxiv.org/abs/2506.05301)
...
TLDR: SeedVR2是一种基于扩散的单步视频修复模型，通过对抗训练和动态窗口注意力机制，显著降低了计算成本，同时在高分辨率视频修复中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有扩散视频修复方法计算成本高，且单步修复方法在高分辨率视频中表现不足。

Method: 提出SeedVR2，采用对抗训练和动态窗口注意力机制，优化模型结构和训练过程。
...
### [383] [Perceive Anything: Recognize, Explain, Caption, and Segment Anything in Images and Videos](https://arxiv.org/abs/2506.05302)
*Weifeng Lin,Xinyu Wei,Ruichuan An,Tianhe Ren,Tingwei Chen,Renrui Zhang,Ziyu Guo,Wentao Zhang,Lei Zhang,Hongsheng Li*

Main category: cs.CV

TLDR: PAM是一个高效的区域级视觉理解框架，结合SAM 2和LLMs，实现对象分割与多样化语义输出。


<details>
  <summary>Details</summary>
Motivation: 提升区域级视觉理解的全面性和效率，结合分割模型与语言模型。

Method: 集成SAM 2和LLMs，引入Semantic Perceiver转换视觉特征，并开发数据增强流程。

Result: PAM在多个任务中表现优异，运行速度更快且内存占用更低。

Conclusion: PAM为区域级视觉理解提供了高效实用的解决方案，是未来研究的强基线。
...
### [384] [Do It Yourself: Learning Semantic Correspondence from Pseudo-Labels](https://arxiv.org/abs/2506.05312)
*Olaf Dünkel,Thomas Wimmer,Christian Theobalt,Christian Rupprecht,Adam Kortylewski*

Main category: cs.CV

TLDR: 论文提出了一种通过3D感知伪标签改进语义匹配的方法，利用3D感知链和球形原型映射约束优化特征，显著提升了SPair-71k数据集的性能。
...
Method: 训练适配器，利用3D感知伪标签（通过3D感知链、松弛循环一致性和3D球形原型映射约束生成）优化现有特征。

Result: 在SPair-71k数据集上实现了超过4%的绝对性能提升，相比类似监督需求的方法提升了7%。

Conclusion: 提出的方法具有通用性，可轻松扩展到其他数据源，显著提升了语义匹配的准确性。
...
### [385] [MARBLE: Material Recomposition and Blending in CLIP-Space](https://arxiv.org/abs/2506.05313)
*Ta-Ying Cheng,Prafull Sharma,Mark Boss,Varun Jampani*

Main category: cs.CV

TLDR: MARBLE是一种基于CLIP空间和预训练文本到图像模型的方法，用于实现材料混合和细粒度材料属性编辑。


<details>
  <summary>Details</summary>
Motivation: 研究目标是改进基于示例图像的材料编辑方法，通过CLIP空间和预训练模型实现更精细的材料控制。

Method: 通过CLIP空间找到材料嵌入，并利用去噪UNet中的特定块控制材料属性。结合浅层网络预测材料属性变化方向。

Result: 方法在定性和定量分析中表现优异，支持单次前向传递中的多次编辑，并适用于绘画。

Conclusion: MARBLE在材料编辑和属性控制方面具有高效性和灵活性，适用于多种应用场景。
...
### [386] [ProJo4D: Progressive Joint Optimization for Sparse-View Inverse Physics Estimation](https://arxiv.org/abs/2506.05317)
*Daniel Rho,Jun Myeong Choi,Biswadip Dey,Roni Sengupta*

Main category: cs.CV

TLDR: ProJo4D是一种渐进式联合优化框架，用于从稀疏多视角视频中同时优化几何、外观、物理状态和材料属性，解决了现有方法在稀疏输入下的误差累积问题。
...
Method: 提出ProJo4D框架，通过渐进式联合优化逐步增加优化参数，基于参数敏感性引导优化过程。
...
### [387] [Does Your 3D Encoder Really Work? When Pretrain-SFT from 2D VLMs Meets 3D VLMs](https://arxiv.org/abs/2506.05318)
*Haoyuan Li,Yanpeng Zhou,Yufei Gao,Tao Tang,Jianhua Han,Yujie Yuan,Dave Zhenyu Chen,Jiawang Bian,Hang Xu,Xiaodan Liang*

Main category: cs.CV

TLDR: 论文探讨了3D视觉语言模型（VLMs）的性能问题，发现3D场景中心模型表现较差，并提出新数据集以改进3D理解。


<details>
  <summary>Details</summary>
Motivation: 研究3D VLMs在任务中的性能差异，尤其是3D场景中心模型表现不佳的原因。

Method: 分类3D VLMs为三类（3D对象中心、2D图像基础、3D场景中心），分析其性能差异并提出新数据集。

Result: 3D场景中心模型对3D编码器依赖有限，预训练效果较差，且数据扩展收益不明显。
...
### [388] [Revisiting Depth Representations for Feed-Forward 3D Gaussian Splatting](https://arxiv.org/abs/2506.05327)
*Duochao Shi,Weijie Wang,Donny Y. Chen,Zeyu Zhang,Jia-Wang Bian,Bohan Zhuang,Chunhua Shen*

Main category: cs.CV

TLDR: 论文提出了一种基于点图正则化损失（PM-Loss）的方法，用于解决深度图中物体边界不连续导致的点云稀疏问题，从而提升3D高斯泼溅（3DGS）的渲染质量。


<details>
  <summary>Details</summary>
Motivation: 深度图在3DGS中被广泛用于生成3D点云，但物体边界的不连续性会导致点云稀疏或碎片化，影响渲染质量。

Method: 引入PM-Loss，通过预训练变换器预测的点图对深度图进行正则化，增强几何平滑性。

Result: 改进后的深度图显著提升了3DGS的渲染效果，适用于多种架构和场景。

Conclusion: PM-Loss有效解决了深度图边界问题，提升了3DGS的渲染质量。
...
### [389] [AV-Reasoner: Improving and Benchmarking Clue-Grounded Audio-Visual Counting for MLLMs](https://arxiv.org/abs/2506.05328)
*Lidong Lu,Guo Chen,Zhiqi Li,Yicheng Liu,Tong Lu*

Main category: cs.CV

TLDR: CG-AV-Counting是一个新的视频计数基准，包含1027个多模态问题和5845个标注线索，支持黑盒和白盒评估。AV-Reasoner模型通过GRPO和课程学习提升计数能力，但在域外基准上表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有视频理解模型在计数任务上表现不佳，且现有基准存在视频短、查询封闭、缺乏线索标注和多模态覆盖不足的问题。

Method: 提出CG-AV-Counting基准，并开发AV-Reasoner模型，结合GRPO和课程学习提升计数能力。

Result: AV-Reasoner在多个基准上达到SOTA，但在域外基准上语言推理未带来性能提升。

Conclusion: CG-AV-Counting为视频计数任务提供了全面测试平台，AV-Reasoner展示了强化学习的有效性，但需进一步改进域外表现。
...
### [390] [MINT-CoT: Enabling Interleaved Visual Tokens in Mathematical Chain-of-Thought Reasoning](https://arxiv.org/abs/2506.05331)
*Xinyan Chen,Renrui Zhang,Dongzhi Jiang,Aojun Zhou,Shilin Yan,Weifeng Lin,Hongsheng Li*

Main category: cs.CV

TLDR: MINT-CoT通过动态插入视觉标记到文本推理步骤中，解决了多模态数学推理中的视觉区域感知和修改问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在数学问题解决中依赖粗粒度图像区域、视觉编码器对数学内容感知有限，且需外部能力进行视觉修改。

Method: 提出MINT-CoT，通过Interleave Token动态选择数学图形中的视觉区域，并构建了54K问题的数据集和三阶段训练策略。

Result: MINT-CoT-7B在MathVista、GeoQA和MMStar上分别提升34.08%、28.78%和23.2%。

Conclusion: MINT-CoT在多模态数学推理中表现优异，为视觉与文本结合的推理提供了有效方法。
...
### [391] [Unleashing Hour-Scale Video Training for Long Video-Language Understanding](https://arxiv.org/abs/2506.05332)
*Jingyang Lin,Jialian Wu,Ximeng Sun,Ze Wang,Jiang Liu,Yusheng Su,Xiaodong Yu,Hao Chen,Jiebo Luo,Zicheng Liu,Emad Barsoum*

Main category: cs.CV

TLDR: VideoMarathon是一个大规模的长视频指令跟随数据集，包含约9,700小时的长视频和3.3M高质量QA对，支持22种任务。基于此提出的Hour-LLaVA模型在长视频语言建模中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有长视频语言理解数据集的稀缺导致训练长视频多模态模型（Video-LMMs）的研究不足。

Method: 提出VideoMarathon数据集和Hour-LLaVA模型，后者通过内存增强模块实现1-FPS采样下的长视频训练和推理。

Result: Hour-LLaVA在多个长视频语言基准测试中表现最佳。

Conclusion: VideoMarathon数据集的高质量和Hour-LLaVA模型的优越性为长视频语言理解提供了有力支持。
...
### [392] [VideoMolmo: Spatio-Temporal Grounding Meets Pointing](https://arxiv.org/abs/2506.05336)
*Ghazi Shazan Ahmad,Ahmed Heakl,Hanan Gani,Abdelrahman Shaker,Zhiqiang Shen,Ranjay Krishna,Fahad Shahbaz Khan,Salman Khan*

Main category: cs.CV

TLDR: VideoMolmo是一个多模态大模型，用于基于文本描述的细粒度时空定位，通过结合时间模块和掩码融合技术提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频定位方法缺乏上下文理解和泛化能力，VideoMolmo旨在解决这一问题。

Method: 结合Molmo架构，引入时间模块和双向点传播的掩码融合技术，简化任务并增强可解释性。

Result: 在多个现实场景和基准测试中显著提升时空定位准确性和推理能力。
...
### [393] [Defurnishing with X-Ray Vision: Joint Removal of Furniture from Panoramas and Mesh](https://arxiv.org/abs/2506.05338)
...
TLDR: 提出了一种生成室内空间简化无家具模型的流程，结合纹理网格和多视角全景图像，通过ControlNet修复技术实现高质量结果。


<details>
  <summary>Details</summary>
Motivation: 传统方法如神经辐射场或RGB-D修复易产生模糊或幻觉问题，需要一种更可靠的方法生成高质量的无家具场景模型。

Method: 首先从网格中分割并移除家具，生成简化无家具网格（SDM），提取其深度和法线图像的Canny边缘，作为ControlNet修复全景图像的引导信号，最终纹理化网格。

Result: 该方法生成的资产质量高于依赖神经辐射场或RGB-D修复的方法，避免了模糊和幻觉问题。

Conclusion: 提出的流程能够高效生成高质量的无家具场景模型，为相关应用提供了可靠的技术支持。
...
### [394] [Direct Numerical Layout Generation for 3D Indoor Scene Synthesis via Spatial Reasoning](https://arxiv.org/abs/2506.05341)
*Xingjian Ran,Yixuan Li,Linning Xu,Mulin Yu,Bo Dai*

Main category: cs.CV

TLDR: DirectLayout是一个基于大语言模型的框架，直接从文本描述生成3D室内场景布局，解决了现有方法在开放词汇和细粒度用户指令对齐上的不足。


<details>
  <summary>Details</summary>
Motivation: 解决现有3D场景布局生成方法因数据集有限导致的过拟合或灵活性不足问题，实现开放词汇和用户指令对齐的场景生成。

Method: 分三阶段生成：鸟瞰图布局、3D空间提升和对象放置优化，结合Chain-of-Thought激活和生成奖励增强空间推理。

Result: 实验表明DirectLayout在语义一致性、泛化能力和物理合理性上表现优异。

Conclusion: DirectLayout通过直接生成和空间推理，显著提升了3D场景布局的质量和灵活性。
...
### [395] [Refer to Anything with Vision-Language Prompts](https://arxiv.org/abs/2506.05342)
*Shengcao Cao,Zijun Wei,Jason Kuen,Kangning Liu,Lingzhi Zhang,Jiuxiang Gu,HyunJoon Jung,Liang-Yan Gui,Yu-Xiong Wang*

Main category: cs.CV

TLDR: 提出了一种新的任务（ORES）和框架（RAS），通过多模态交互提升图像分割的语义理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有图像分割模型缺乏对复杂语言和视觉查询的综合语义理解，限制了其在用户友好交互中的应用。

Method: 提出RAS框架，通过掩码中心的多模态大模型增强分割模型的多模态交互能力。

Result: 在ORES、RES和GRES任务中，RAS表现出优越性能。

Conclusion: RAS框架为多模态图像分割任务提供了新的解决方案，并展示了其有效性。
...
### [396] [ContentV: Efficient Training of Video Generation Models with Limited Compute](https://arxiv.org/abs/2506.05343)
...
TLDR: ContentV是一个8B参数的文本到视频模型，通过三项创新技术（简约架构、多阶段训练策略和低成本强化学习框架）在高效训练下实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 视频生成领域对高效训练方法的需求日益增长，以应对计算成本的上升。

Method: 采用简约架构重用预训练图像生成模型，多阶段训练策略利用流匹配提高效率，以及低成本强化学习框架提升生成质量。

Result: 在VBench上达到85.14分，支持多分辨率和时长的视频生成。

Conclusion: ContentV展示了高效训练方法在视频生成中的潜力，代码和模型已开源。
...
### [397] [SparseMM: Head Sparsity Emerges from Visual Concept Responses in MLLMs](https://arxiv.org/abs/2506.05344)
*Jiahui Wang,Zuyan Liu,Yongming Rao,Jiwen Lu*

Main category: cs.CV

TLDR: 研究发现MLLMs中只有约5%的注意力头对视觉理解有贡献，提出了一种无需训练的框架识别这些头，并基于此设计了KV-Cache优化策略SparseMM，显著提升了推理效率。


<details>
  <summary>Details</summary>
Motivation: 探究MLLMs如何处理视觉输入，揭示注意力机制的稀疏性，以优化推理效率。

Method: 通过分析注意力机制，识别视觉头，设计KV-Cache优化策略SparseMM，分配不对称计算预算。
...
### [398] [Neural Inverse Rendering from Propagating Light](https://arxiv.org/abs/2506.05347)
*Anagh Malik,Benjamin Attal,Andrew Xie,Matthew O'Toole,David B. Lindell*

Main category: cs.CV

TLDR: 提出首个基于物理的神经逆向渲染系统，用于多视角视频中的光传播，结合时间分辨神经辐射缓存技术，实现高效逆向渲染。


<details>
  <summary>Details</summary>
Motivation: 解决多视角视频中光传播的逆向渲染问题，特别是间接光的处理。

Method: 采用时间分辨神经辐射缓存技术，存储任意点、任意方向的无限反弹辐射，支持直接和间接光传输。

Result: 在强间接光条件下实现最先进的3D重建，支持光传播的视图合成、直接与间接光分解及多视角时间分辨重光照。

Conclusion: 该系统为光传播的逆向渲染提供了高效解决方案，拓展了多视角视频处理的新能力。
...
### [399] [FreeTimeGS: Free Gaussians at Anytime and Anywhere for Dynamic Scene Reconstruction](https://arxiv.org/abs/2506.05348)
*Yifan Wang,Peishan Yang,Zhen Xu,Jiaming Sun,Zhanhua Zhang,Yong Chen,Hujun Bao,Sida Peng,Xiaowei Zhou*

Main category: cs.CV

TLDR: 提出FreeTimeGS，一种新型4D表示方法，用于处理复杂运动的动态3D场景重建，优于现有方法。
...
Conclusion: FreeTimeGS通过灵活表示和运动函数，有效提升了动态3D场景建模能力。
...
### [400] [VideoMathQA: Benchmarking Mathematical Reasoning via Multimodal Understanding in Videos](https://arxiv.org/abs/2506.05349)
*Hanoona Rasheed,Abdelrahman Shaker,Anqi Tang,Muhammad Maaz,Ming-Hsuan Yang,Salman Khan,Fahad Khan*

Main category: cs.CV

TLDR: VideoMathQA是一个用于评估模型在视频中进行跨模态数学推理能力的基准测试，涵盖10个数学领域，强调多模态和时序推理。


<details>
  <summary>Details</summary>
Motivation: 解决视频中数学推理的挑战，需整合视觉、音频和文本信息，现有方法难以应对。

Method: 构建VideoMathQA基准，包含多样化的视频和问题类型，由专家标注，强调多步推理和跨模态理解。

Result: 基准测试揭示了现有方法的局限性，并提供了系统评估框架。

Conclusion: VideoMathQA为模型在多模态和时序数学推理中的能力提供了标准化评估工具。
...
### [401] [Contrastive Flow Matching](https://arxiv.org/abs/2506.05350)
*George Stoica,Vivek Ramanujan,Xiang Fan,Ali Farhadi,Ranjay Krishna,Judy Hoffman*

Main category: cs.CV

TLDR: 论文提出了一种对比流匹配方法，通过显式增强条件流之间的差异性，解决了传统流匹配在条件设置下模糊生成的问题。


<details>
  <summary>Details</summary>
Motivation: 在条件设置下（如类别条件模型），传统流匹配无法保证流的唯一性，导致生成结果模糊。

Method: 引入对比流匹配，通过对比目标最大化不同条件流之间的差异性。

Result: 实验表明，该方法在训练速度、去噪步骤和FID指标上均有显著提升。

Conclusion: 对比流匹配有效解决了条件流模糊问题，并显著提升了模型性能。
...
### [402] [Can Artificial Intelligence Trade the Stock Market?](https://arxiv.org/abs/2506.04658)
*Jędrzej Maskiewicz,Paweł Sakowski*

Main category: q-fin.TR

TLDR: 论文探讨了深度强化学习（DRL）在股票市场交易中的应用，比较了DDQN和PPO算法与Buy and Hold基准的表现，结果显示DRL在风险调整收益上优于传统监督学习方法。


<details>
  <summary>Details</summary>
Motivation: 研究DRL在金融交易中的潜力，特别是其在风险管理和收益优化方面的表现。

Method: 使用DDQN和PPO算法，在多种资产（货币对、S&P 500指数、比特币）的日数据上进行测试，并与Buy and Hold策略对比。

Result: DRL在交易中表现优异，能通过策略性避免不利交易条件管理风险，风险调整收益优于传统方法。

Conclusion: DRL在金融交易中具有显著优势，尤其在风险管理和收益优化方面，为未来研究提供了方向。
...
### [403] [ChemReservoir -- An Open-Source Framework for Chemically-Inspired Reservoir Computing](https://arxiv.org/abs/2506.04249)
*Mehmet Aziz Yirik,Jakob Lykke Andersen,Rolf Fagerberg,Daniel Merkle*

Main category: cs.CE

TLDR: 本文介绍了ChemReservoir，一个开源的化学启发的储层计算框架，解决了之前研究中维护性和通用性的问题。


<details>
  <summary>Details</summary>
Motivation: 由于现有DNA化学启发的储层计算工具维护性差且通用性不足，需要开发一个开源工具。

Method: 开发了一个通用的化学启发的储层计算框架ChemReservoir，支持构建和分析各种化学启发的储层。

Result: ChemReservoir在多种循环储层拓扑结构中表现稳定，适用于记忆容量任务。

Conclusion: ChemReservoir是一个通用且稳定的开源工具，解决了之前研究的局限性。
...
### [404] [Domain Adaptation Method and Modality Gap Impact in Audio-Text Models for Prototypical Sound Classification](https://arxiv.org/abs/2506.04376)
*Emiliano Acevedo,Martín Rocamora,Magdalena Fuentes*

Main category: cs.SD

TLDR: 论文提出了一种新方法，通过量化背景声源贡献并融入分类过程，提升零样本环境声音分类性能，无需重新训练模型。


<details>
  <summary>Details</summary>
Motivation: 现有音频-文本模型在背景声源存在时性能显著下降，研究发现性能退化主要由背景声信噪比（SNR）驱动，与背景类型无关。

Method: 提出一种域适应技术，量化背景声源贡献并融入分类过程，同时分析音频与文本嵌入的模态差距，缩小该差距以提升性能。

Result: 方法在多种背景和SNR条件下提升了分类准确性，且适用于现有原型方法，展现其可扩展性和鲁棒性。

Conclusion: 该方法有效解决了背景声源对分类性能的影响，无需重新训练模型，具有广泛适用性。
...
### [405] [Grapheme-Coherent Phonemic and Prosodic Annotation of Speech by Implicit and Explicit Grapheme Conditioning](https://arxiv.org/abs/2506.04527)
*Hien Ohnaka,Yuma Shirahata,Byeongseon Park,Ryuichi Yamamoto*

Main category: cs.SD

TLDR: 提出一种模型，通过隐式和显式方法将音素和韵律标签与字素对齐，显著提升一致性，并验证其在口音估计任务中的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅通过微调预训练ASR模型生成标签，缺乏与字素的直接关联，导致标签与字素不一致。

Method: 1）通过预训练BERT特征的提示编码器隐式引入字素条件；2）在推理阶段显式剪枝与字素不一致的标签假设。

Result: 实验表明，该方法显著提升标签与字素的一致性，并行数据有效提升口音估计任务准确率。

Conclusion: 所提方法能生成与字素一致的标签，适用于多种下游任务。
...
### [406] [LLM-based phoneme-to-grapheme for phoneme-based speech recognition](https://arxiv.org/abs/2506.04711)
*Te Ma,Min Bi,Saierdaer Yusuyin,Hao Huang,Zhijian Ou*

Main category: cs.SD

TLDR: 论文提出了一种基于大语言模型（LLM）的音素到字素解码方法（LLM-P2G），用于音素基础的自动语音识别（ASR），通过数据增强和随机化训练策略解决了信息丢失问题，并在波兰语和德语的跨语言ASR中表现优于传统WFST方法。


<details>
  <summary>Details</summary>
Motivation: 传统基于WFST的解码方法存在流程复杂且无法利用大语言模型的问题，而音素基础的ASR在数据效率和性能上具有优势，因此需要一种更高效的解码方法。
...
Conclusion: LLM-P2G方法在音素基础的ASR中表现优异，为跨语言语音识别提供了一种更高效的解决方案。
...
TLDR: 本文综述了音乐生成系统的评估目标、方法和指标，涵盖主观与客观、定性与定量以及实证与计算方法，并从音乐学、工程和人机交互角度讨论了其优缺点。
...
Method: 通过跨学科视角，综合分析了评估目标、方法和指标，包括主客观、定性与定量、实证与计算等。

Result: 总结了不同评估方法的优缺点，并提供了多角度的分析（音乐学、工程、HCI）。

Conclusion: 本文为音乐生成系统的评估提供了全面的框架，有助于未来研究的标准化和优化。
...
TLDR: 论文提出了一种基于用户交互的人类计算方法，通过遗传算法结合用户反馈逐步改进AI音乐生成模型的性能。
...
Method: 采用人类计算方法，聚合用户评分作为损失函数，利用遗传算法结合用户反馈微调模型。

Result: 初步测试中，第一轮迭代平均评分提高0.2，第二轮进一步提高了0.39。

Conclusion: 该方法通过用户反馈逐步优化模型，显著提升了AI音乐生成的个性化性能。
...
### [409] [Deep Learning-Based Breast Cancer Detection in Mammography: A Multi-Center Validation Study in Thai Population](https://arxiv.org/abs/2506.03177)
...
TLDR: 该研究提出了一种基于改进的EfficientNetV2架构和增强注意力机制的深度学习系统，用于乳腺X光检查中的乳腺癌检测，并在多个数据集上验证了其性能。
...
Method: 使用改进的EfficientNetV2架构和增强注意力机制，在泰国一家主要医疗中心的乳腺X光数据上进行训练，并在三个不同数据集上验证。

Result: 模型在癌症检测上的AUROC分别为0.89、0.96和0.94；在病灶定位和临床验证中表现出色，与放射科医生的分类和定位一致性较高。

Conclusion: 该系统在乳腺X光检查解读中表现出高效性，有望提升临床乳腺癌筛查的工作流程。
...
### [410] [Gradient Inversion Attacks on Parameter-Efficient Fine-Tuning](https://arxiv.org/abs/2506.04453)
*Hasin Us Sami,Swapneel Sen,Amit K. Roy-Chowdhury,Srikanth V. Krishnamurthy,Basak Guler*

Main category: eess.IV

TLDR: 论文研究了联邦学习中参数高效微调（PEFT）的隐私问题，展示了通过恶意设计的预训练模型和适配器模块，攻击者可以重构用户的本地数据样本。


<details>
  <summary>Details</summary>
Motivation: 探讨如何在联邦学习中通过PEFT同时提升隐私和训练效率，同时揭示其潜在的隐私风险。
...
### [411] [A Poisson-Guided Decomposition Network for Extreme Low-Light Image Enhancement](https://arxiv.org/abs/2506.04470)
*Isha Rao,Sanjay Ghosh*

Main category: eess.IV

TLDR: 提出一种轻量级深度学习方法，结合Retinex分解与泊松去噪，用于极端低光条件下的图像去噪与增强。


<details>
  <summary>Details</summary>
Motivation: 解决低光条件下信号依赖性泊松噪声的图像去噪问题，传统高斯噪声假设不适用。

Method: 集成Retinex分解与泊松去噪的编码器-解码器网络，引入泊松去噪损失函数。

Result: 显著提升低光条件下的图像可见性与亮度，保持结构完整与颜色恒常性。

Conclusion: 该方法有效且实用，无需先验反射与光照信息，避免颜色失真。
...
### [412] [DACN: Dual-Attention Convolutional Network for Hyperspectral Image Super-Resolution](https://arxiv.org/abs/2506.05041)
*Usman Muhammad,Jorma Laaksonen*

Main category: eess.IV

TLDR: DACN是一种双注意力卷积网络，用于解决高光谱图像超分辨率任务中局部依赖性和全局上下文理解不足的问题，通过多注意力机制和优化的损失函数提升性能。
...
Method: DACN结合多头注意力增强卷积，捕捉局部和全局特征依赖；通过通道和空间维度的注意力图优化关注点；提出结合L2正则化和空间-光谱梯度损失的损失函数。

Result: 在两个高光谱数据集上的实验表明，多头注意力与通道注意力结合的性能优于单独使用任一种注意力机制。
...
### [413] [PixCell: A generative foundation model for digital histopathology images](https://arxiv.org/abs/2506.05127)
...
TLDR: PixCell是一种基于扩散的生成基础模型，用于组织病理学，能够生成多样且高质量的图像，解决数据稀缺和隐私问题。
...
Result: PixCell生成高质量图像，可用于训练自监督模型、数据增强和教育用途，并支持分子标记推断。
...
### [414] [DM-SegNet: Dual-Mamba Architecture for 3D Medical Image Segmentation with Global Context Modeling](https://arxiv.org/abs/2506.05297)
*Hangyu Ji*

Main category: eess.IV

TLDR: DM-SegNet是一种双Mamba架构，通过方向性状态转换和解剖感知分层解码，解决了医学图像分割中全局上下文建模与空间拓扑保持的兼容性问题。


<details>
  <summary>Details</summary>
Motivation: 现有医学状态空间模型（SSMs）存在编码器-解码器不兼容问题，1D序列扁平化破坏空间结构，而传统解码器无法利用Mamba的状态传播。
...
Conclusion: DM-SegNet通过创新的架构设计，显著提升了医学图像分割的精度，具有临床意义。
...
### [415] [SSIMBaD: Sigma Scaling with SSIM-Guided Balanced Diffusion for AnimeFace Colorization](https://arxiv.org/abs/2506.04283)
*Junpyo Seo,Hanbin Koo,Jieun Yook,Byung-Ro Moon*

Main category: cs.GR

TLDR: 提出了一种基于扩散模型的新框架，用于自动为动漫风格的面部线稿上色，通过SSIMBaD技术优化感知一致性。
...
Result: 在大规模动漫面部数据集上，该方法在像素精度和感知质量上优于现有模型。

Conclusion: SSIMBaD框架在动漫面部上色任务中表现出色，且能泛化到多样风格。
...
### [416] [Handle-based Mesh Deformation Guided By Vision Language Model](https://arxiv.org/abs/2506.04562)
*Xingpeng Sun,Shiyang Jia,Zherong Pan,Kui Wu,Aniket Bera*

Main category: cs.GR

TLDR: 提出了一种无需训练、基于手柄的网格变形方法，利用视觉语言模型（VLM）通过提示工程实现高质量变形。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在输出质量低、需手动调整或依赖数据训练的问题。

Method: 结合锥形奇点检测和VLM选择变形子部分及手柄，通过多视角投票减少不确定性。

Result: 在基准测试中，变形结果更符合用户意图，且失真低。
...
### [417] [VoxDet: Rethinking 3D Semantic Occupancy Prediction as Dense Object Detection](https://arxiv.org/abs/2506.04623)
*Wuyang Li,Zhu Yu,Alexandre Alahi*

Main category: cs.GR

TLDR: 论文提出了一种实例中心的3D语义占用预测框架VoxDet，通过将体素级分类任务解耦为偏移回归和语义预测两个子任务，显著提升了实例级完整性和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法将3D语义占用预测视为密集分割任务，忽略了实例级可区分性，导致实例不完整和相邻模糊问题。论文发现体素级标签隐含实例级信息，并利用这一观察提出新方法。

Method: 提出VoxDet框架，包括：(a)空间解耦体素编码器生成任务特定特征；(b)任务解耦密集预测器，通过偏移回归和语义预测实现实例感知预测。

Result: VoxDet在相机和LiDAR输入上均取得最优性能，SemanticKITTI测试集上达到63.0 IoU，排名第一。

Conclusion: VoxDet通过实例中心的设计显著提升了3D语义占用预测的性能，验证了体素级标签隐含实例信息的有效性。
...
### [418] [A Fast Unsupervised Scheme for Polygonal Approximation](https://arxiv.org/abs/2506.04664)
*Bimal Kumar Ray*

Main category: cs.GR

TLDR: 本文提出了一种快速且无监督的多边形近似闭合数字曲线方案，速度快于现有技术，且在Rosin度量与美观性上具有竞争力。


<details>
  <summary>Details</summary>
Motivation: 现有多边形近似方法在速度和美观性上存在不足，本文旨在提出一种更高效且美观的解决方案。

Method: 方案分为三阶段：初始分割、迭代顶点插入与合并，最后进行顶点调整。初始分割检测高曲率顶点，迭代插入补充低曲率顶点，合并消除冗余顶点，调整优化美观性。

Result: 方案在速度上优于现有技术，Rosin度量与美观性表现良好，且对几何变换具有鲁棒性。

Conclusion: 该方案为闭合数字曲线的多边形近似提供了一种高效、美观且鲁棒的方法。
...
### [419] [Artificial Intelligence Should Genuinely Support Clinical Reasoning and Decision Making To Bridge the Translational Gap](https://arxiv.org/abs/2506.05030)
*Kacper Sokol,James Fackler,Julia E Vogt*

Main category: cs.HC

TLDR: 论文提出了一种新的社会技术概念化方法，旨在弥补人工智能在医学中的翻译差距，强调与医生认知活动的互补性。


<details>
  <summary>Details</summary>
Motivation: 当前以技术为中心的人工智能方法在医学中应用有限，无法与临床实践（如诊断推理和决策）兼容。

Method: 提出了一种社会技术概念化方法，设计数据驱动的支持工具，以补充医生的认知和认知活动。

Result: 该方法强调实际影响而非无关基准的超人性能。

Conclusion: 通过社会技术方法，可以更好地将人工智能融入医学实践，提升其实际效果。
...
### [420] [Teaming in the AI Era: AI-Augmented Frameworks for Forming, Simulating, and Optimizing Human Teams](https://arxiv.org/abs/2506.05265)
*Mohammed Almutairi*

Main category: cs.HC

TLDR: 论文提出了AI增强的团队优化框架，包括基于多臂老虎机的团队组建方法、AI反馈助手tAIfa和模拟框架PuppeteerLLM，旨在提升团队满意度、凝聚力和表现。


<details>
  <summary>Details</summary>
Motivation: 现有团队优化工具依赖静态数据或特定场景，无法动态适应成员个性、目标和偏好变化，导致团队表现不佳。

Method: 1. 多臂老虎机算法优化团队组建；2. AI助手tAIfa提供个性化反馈；3. LLM框架PuppeteerLLM模拟团队动态。

Result: 提出的框架能动态优化团队，提升满意度和表现。
...
### [421] [CMIE: Combining MLLM Insights with External Evidence for Explainable Out-of-Context Misinformation Detection](https://arxiv.org/abs/2505.23449)
*Fanxiao Li,Jiaying Wu,Canyuan He,Wei Zhou*

Main category: cs.MM

TLDR: 论文提出CMIE框架，通过生成共存关系和关联评分机制，解决多模态大语言模型在检测上下文无关虚假信息时的深层关系捕捉和噪声干扰问题。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在检测上下文无关虚假信息时难以捕捉图像与文本的深层语义关联，且证据噪声影响准确性。
...
### [422] [Deep learning image burst stacking to reconstruct high-resolution ground-based solar observations](https://arxiv.org/abs/2506.04781)
*Christoph Schirninger,Robert Jarolim,Astrid M. Veronig,Christoph Kuckein*

Main category: astro-ph.SR

TLDR: 提出一种基于深度学习的实时图像重建方法，用于解决强湍流和高计算成本问题，提升太阳望远镜观测质量。
...
Method: 采用无配对图像到图像转换的深度学习模型，训练以退化图像和参考重建图像，实现实时高质量重建。

Result: 方法在感知质量上表现更优，尤其在参考重建存在伪影时，且能高效利用多图像信息。

Conclusion: 该方法在提供完整图像序列时，能实现最佳重建效果。
...
### [423] [FlashDMoE: Fast Distributed MoE in a Single Kernel](https://arxiv.org/abs/2506.04667)
*Osayamen Jonathan Aimuyo,Byungsoo Oh,Rachee Singh*

Main category: cs.DC

TLDR: FlashDMoE通过GPU内核与硬件的协同设计，显著提升了MoE模型的性能，降低了延迟并提高了GPU利用率。
...
Method: 开发了FlashDMoE，一种完全驻留GPU的MoE算子，将专家计算和GPU间通信融合为单一持久GPU内核，实现细粒度流水线调度。

Result: 在8-H100 GPU节点上，FlashDMoE实现了延迟降低6倍、吞吐量提高5.7倍、GPU利用率提升9倍等显著性能改进。

Conclusion: FlashDMoE证明了GPU内核与硬件协同设计对提升大规模分布式机器学习性能的关键作用。
...
### [424] [Optical Physics-Based Generative Models](https://arxiv.org/abs/2506.04357)
*Amirreza Ahmadnejad,Somayyeh Koohi*

Main category: physics.optics

TLDR: 论文建立了一个连接光学物理方程与生成模型的数学框架，展示了光传播动力学如何启发强大的人工智能方法。非线性光学模型通过自组织原理展现出显著能力，实验验证其优于线性模型和传统生成方法。
...
Method: 分析六种基本光学方程（线性和非线性），比较其性能，并通过实验验证非线性模型在参数减少、模式分离、稳定性和可控性方面的优势。

Result: 非线性模型在FID分数、内存使用和训练时间上显著优于线性模型，同时实现了更高的准确性和稳定性。
...
TLDR: 研究使用Parler TTS模型重建发音障碍者的声音，生成接近其病前语音的近似值，但模型在控制清晰度和保持说话者身份一致性方面存在困难。


<details>
  <summary>Details</summary>
Motivation: 发音障碍者难以沟通，个性化语音合成是一种潜在的辅助工具。

Method: 利用Parler TTS模型，通过标注数据集进行微调，生成语音。

Result: 模型能学习生成语音，但清晰度和说话者身份一致性控制不佳。
...
TLDR: 本文比较了联合语音文本解码策略（交错与并行生成），提出了一种新颖的早期停止交错（ESI）模式，显著加速解码并略微提升性能，同时通过高质量QA数据集优化语音QA表现。


<details>
  <summary>Details</summary>
Motivation: 研究联合语音文本解码策略对性能、效率和对齐质量的影响，以优化语音语言模型的实用性。

Method: 在相同基础模型、语音分词器和训练数据下，系统比较交错与并行生成策略，并提出ESI模式。

Result: 交错策略对齐效果最佳但推理慢；ESI模式显著加速解码且性能略优。

Conclusion: ESI模式是一种高效的联合解码策略，结合高质量QA数据可进一步提升语音QA性能。
...
TLDR: 本文提出EMO-Debias，对13种去偏方法在多标签语音情感识别中的效果进行了大规模比较，分析了公平性与准确性的权衡。
...
Method: 研究比较了13种去偏方法，涵盖预处理、正则化、对抗学习、偏见学习器和分布鲁棒优化等技术，并在性别不平衡条件下评估了这些方法。

Result: 实验量化了公平性与准确性的权衡，发现某些方法能持续减少性别性能差距而不影响整体性能。

Conclusion: 研究结果为选择有效去偏策略提供了实用建议，并强调了数据集分布的影响。
...
### [428] [Robust Moment Identification for Nonlinear PDEs via a Neural ODE Approach](https://arxiv.org/abs/2506.05245)
*Shaoxuan Chen,Su Yang,Panayotis G. Kevrekidis,Wei Zhu*

Main category: nlin.PS

TLDR: 提出了一种基于Neural ODEs的数据驱动框架，用于从PDE系统中学习降维矩动力学，适用于稀疏和不规则时间序列数据。


<details>
  <summary>Details</summary>
Motivation: 传统基于导数的方法（如SINDy）需要密集采样数据且对噪声敏感，而Neural ODEs能直接建模矩轨迹，提升鲁棒性。

Method: 利用Neural ODEs直接建模矩轨迹，并通过Stiefel流形优化实现数据驱动的坐标变换，发现低维表示。

Result: 在非线性薛定谔方程和Fisher-KPP系统中，框架能准确恢复矩动力学，并在数据有限时表现优于专家模型。

Conclusion: Neural ODE框架是学习复杂PDE系统中低维矩动力学的强大且灵活的工具。
...
### [429] [Benchmark for Antibody Binding Affinity Maturation and Design](https://arxiv.org/abs/2506.04235)
...
TLDR: AbBiBench是一个用于抗体结合亲和力成熟和设计的基准框架，通过评估抗体-抗原复合物的功能潜力，提供统一的生物学评估标准。


<details>
  <summary>Details</summary>
Motivation: 现有抗体评估方法仅关注抗体本身或其与自然抗体的相似性，缺乏对功能单元（抗体-抗原复合物）的评估。AbBiBench旨在填补这一空白。
...
### [430] [The Latent Space Hypothesis: Toward Universal Medical Representation Learning](https://arxiv.org/abs/2506.04515)
*Salil Patel*

Main category: q-bio.QM

TLDR: 论文提出了一种基于潜在空间假设的框架，将多模态医学数据映射到一个统一的几何表示中，用于个性化诊断和治疗。


<details>
  <summary>Details</summary>
Motivation: 医学数据多模态且复杂，传统方法可能掩盖疾病的异质性，需要一种统一的方法来揭示患者的独特轨迹。

Method: 通过潜在空间假设，将多模态数据投影到一个共享的几何空间中，利用编码器和持续学习技术。

Result: 框架能够揭示疾病的子轨迹和患者特异性变化方向，支持个性化医疗。

Conclusion: 该框架为临床实践提供了新的定量方法，但仍需解决数据偏见、隐私等问题。
...
### [431] [What does making money have to do with crime?: A dive into the National Crime Victimization survey](https://arxiv.org/abs/2506.04240)
*Sydney Anuyah*

Main category: physics.soc-ph

TLDR: 利用1992-2022年的国家犯罪受害调查，研究收入、教育、就业和人口因素如何影响犯罪类型（暴力vs财产）。结果表明，高收入和教育降低暴力犯罪风险，而年轻男性、少数族裔风险更高。郊区模型预测效果最佳。
...
Method: 使用平衡分类和逻辑回归模型（F1评分评估），分两组模型（社会经济因素和人口因素）。

Result: 高收入和教育降低暴力犯罪风险；年轻男性、少数族裔风险更高；郊区模型预测效果最佳（准确率0.607，F1 0.590）。
...
### [432] [hdl2v: A Code Translation Dataset for Enhanced LLM Verilog Generation](https://arxiv.org/abs/2506.04544)
*Charles Hong,Brendan Roberts,Huijae An,Alex Um,Advay Ratan,Yakun Sophia Shao*

Main category: cs.AR

TLDR: 论文介绍了hdl2v数据集，通过将VHDL、Chisel和PyMTL3翻译为Verilog，增加公开可用的Verilog代码量，并展示了其在提升LLM生成Verilog代码性能方面的价值。
...
Method: 通过翻译或编译VHDL、Chisel和PyMTL3生成Verilog数据集hdl2v，并用于微调LLM。

Result: hdl2v将32B参数模型的性能提升23%（pass@10），数据增强方法性能提升63%。
...
### [433] [QiMeng: Fully Automated Hardware and Software Design for Processor Chip](https://arxiv.org/abs/2506.05007)
...
TLDR: QiMeng是一个用于全自动处理器芯片设计的系统，通过分层结构解决设计挑战，包括底层的大规模处理器芯片模型（LPCM）和中层的硬件/软件设计代理。


<details>
  <summary>Details</summary>
Motivation: 传统处理器芯片设计面临物理限制、资源需求和生态系统多样性等挑战，AI技术（如LLMs）为全自动设计提供了新可能。

Method: QiMeng采用三层结构：底层LPCM解决知识表示、数据稀缺等问题；中层通过硬件/软件设计代理实现自动化设计。

Result: 部分组件已成功应用于实际场景，展示了高效全自动硬件/软件设计的优势。
...
### [434] [BEAR: BGP Event Analysis and Reporting](https://arxiv.org/abs/2506.04514)
*Hanqing Li,Melania Fedeli,Vinay Kolar,Diego Klabjan*

Main category: cs.NI

TLDR: BEAR框架利用大型语言模型自动生成BGP异常事件的详细报告，提升解释性和分析精度。


<details>
  <summary>Details</summary>
Motivation: BGP异常（如路由泄露和劫持）威胁网络可靠性和安全，现有方法需专家解读。

Method: BEAR通过多步推理将表格数据转为文本叙述，并利用LLM生成合成数据。

Result: 在真实和合成数据集上，BEAR准确率达100%，优于基线方法。

Conclusion: BEAR开创了自动化解释BGP异常事件的方法，为网络管理提供实用洞察。
...
### [435] [Intelligent Channel Allocation for IEEE 802.11be Multi-Link Operation: When MAB Meets LLM](https://arxiv.org/abs/2506.04594)
*Shumin Lian,Jingwen Tong,Jun Zhang,Liqun Fu*

Main category: cs.NI

TLDR: 论文提出了一种动态信道分配算法BAI-MCTS及改进版LLM-BAI-MCTS，用于解决WiFi 7网络中多链路操作（MLO）的信道分配问题，显著提升了收敛速度和性能。


<details>
  <summary>Details</summary>
Motivation: WiFi 7的MLO功能在密集网络中面临信道分配的挑战，现有研究多集中于静态网络，本文旨在解决动态信道分配问题。

Method: 将问题建模为组合优化问题，采用MAB框架进行在线学习，提出BAI-MCTS算法，并进一步整合LLM提出LLM-BAI-MCTS。
...
### [436] [Towards Network Data Analytics in 5G Systems and Beyond](https://arxiv.org/abs/2506.04860)
*Marcos Lima Romero,Ricardo Suyama*

Main category: cs.NI

TLDR: 论文分析了移动网络运营商（MNOs）在数据利用上的不足，提出数据分析和NWDAF（网络数据分析功能）作为提升价值的关键。研究通过分析70多篇文章的趋势和差距，提出了两个新的用例以推动NWDAF的应用和商业化。


<details>
  <summary>Details</summary>
Motivation: 移动网络运营商在数据利用上落后于OTT玩家，需要超越连接服务的商品化，通过数据分析提升客户价值和运营效率。

Method: 分析了70多篇关于NWDAF的文章，识别趋势和差距，并提出了两个新的用例。

Result: 研究发现学术研究集中在有限的方法和用例上，限制了NWDAF的潜力。提出的新用例旨在推动其应用和商业化。

Conclusion: NWDAF在提升移动网络运营商的数据价值方面具有巨大潜力，但需要更多研究和创新用例以实现其商业化。
...
### [437] [GARG-AML against Smurfing: A Scalable and Interpretable Graph-Based Framework for Anti-Money Laundering](https://arxiv.org/abs/2506.04292)
*Bruno Deprez,Bart Baesens,Tim Verdonck,Wouter Verbeke*

Main category: cs.SI

TLDR: GARG-AML是一种基于图的洗钱检测方法，通过单一可解释指标量化拆分交易风险，结合树模型和邻居节点评分，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 洗钱占全球GDP的2%-5%，拆分交易（smurfing）是主要手段，现有黑盒网络嵌入方法难以被企业采用。

Method: 提出GARG-AML，基于二阶交易网络结构计算可解释风险指标，结合树模型和邻居评分。

Result: 在大规模合成和开源网络上实验，GARG-AML优于现有方法。

Conclusion: GARG-AML高效、透明且检测能力强，可集成到现有反洗钱流程中，展示了基础网络特性在欺诈检测中的潜力。
...
### [438] [On the Wasserstein Geodesic Principal Component Analysis of probability measures](https://arxiv.org/abs/2506.04480)
*Nina Vesseron,Elsa Cazelles,Alice Le Brigant,Thierry Klein*

Main category: stat.ML

TLDR: 本文研究了在Otto-Wasserstein几何下对概率分布集合进行测地主成分分析（GPCA），旨在识别最佳捕捉数据集变化模式的地测地曲线。


<details>
  <summary>Details</summary>
Motivation: 目标是利用Otto-Wasserstein几何，在概率测度空间中识别能够描述数据集主要变化模式的地测地曲线。

Method: 首先针对高斯分布集合，展示了如何在可逆线性映射空间中计算；对于绝对连续概率测度，提出了一种利用神经网络参数化Wasserstein空间中测地线的新方法。

Result: 通过多种示例与经典切空间PCA进行比较，并在真实数据集上进行了验证。

Conclusion: 研究表明，GPCA在Wasserstein空间中能够有效捕捉数据的主要变化模式，且优于传统方法。
...
### [439] [Regret-Optimal Q-Learning with Low Cost for Single-Agent and Federated Reinforcement Learning](https://arxiv.org/abs/2506.04626)
*Haochen Zhang,Zhong Zheng,Lingzhou Xue*

Main category: stat.ML

TLDR: 论文提出两种新型无模型RL算法，旨在降低样本成本和通信成本，同时实现最优遗憾。


<details>
  <summary>Details</summary>
Motivation: 研究现实世界中数据收集和策略部署成本高的问题，聚焦于单智能体和联邦RL中的样本成本和通信成本最小化。

Method: 提出Q-EarlySettled-LowCost和FedQ-EarlySettled-LowCost两种算法，结合线性样本成本和对数通信成本。

Result: 算法在最优遗憾、线性样本成本和对数通信成本方面表现最佳，并改进了理论边界。
...
### [440] [Distributional encoding for Gaussian process regression with qualitative inputs](https://arxiv.org/abs/2506.04813)
*Sébastien Da Veiga*

Main category: stat.ML

TLDR: 论文提出了一种基于分布编码（DE）的改进方法，用于处理高斯过程（GP）回归中的分类变量问题，并展示了其在贝叶斯优化（BO）中的优越性能。


<details>
  <summary>Details</summary>
Motivation: 在工程应用中，观测数据获取成本高，而GP回归是贝叶斯优化的核心方法。然而，当输入变量为分类变量时，构建高效且预测性能好的GP模型仍具挑战性。

Method: 从目标编码的朴素思想出发，提出了一种基于分布编码（DE）的改进方法，利用目标变量的所有样本信息。结合特征核（如最大均值差异和Wasserstein距离）处理编码后的数据。

Result: 实验验证表明，该方法在合成和真实数据集上均表现出卓越的预测性能。

Conclusion: 分布编码（DE）方法为处理分类变量的GP回归提供了有效解决方案，并与离散和混合空间的贝叶斯优化技术互补。
...
### [441] [Learning Joint Interventional Effects from Single-Variable Interventions in Additive Models](https://arxiv.org/abs/2506.04945)
*Armin Kekić,Sergio Hernan Garrido Mejia,Bernhard Schölkopf*

Main category: stat.ML

TLDR: 研究提出了一种仅使用观测数据和单变量干预数据来估计联合干预效果的方法，证明了在非线性加性机制下联合效果的可识别性，并通过实验验证了其性能。
...
Method: 提出了一种将因果效应分解为每个干预变量的混杂和非混杂贡献的实用估计器。

Result: 在合成数据上的实验表明，该方法性能接近直接使用联合干预数据训练的模型，优于纯观测估计器。

Conclusion: 研究表明，无需联合干预数据即可推断联合干预效果，为实际应用提供了可行方案。
...
### [442] [Nonlinear Causal Discovery for Grouped Data](https://arxiv.org/abs/2506.05120)
*Konstantin Göbler,Tobias Windisch,Mathias Drton*

Main category: stat.ML

TLDR: 论文提出了一种扩展非线性加性噪声模型的方法，用于处理随机向量，以推断变量组之间的因果关系。
...
Method: 采用两步法：首先推断随机向量间的因果顺序，然后选择与该顺序一致的因果图。

Result: 在模拟实验中表现优异，并成功应用于具有部分因果顺序知识的实际装配线数据。

Conclusion: 该方法有效扩展了因果推断的范围，适用于变量组场景，具有实际应用潜力。
...
### [443] [Causal Effect Identification in lvLiNGAM from Higher-Order Cumulants](https://arxiv.org/abs/2506.05202)
*Daniele Tramontano,Yaroslav Kivva,Saber Salehkaleybar Mathias Drton,Negar Kiyavash*

Main category: stat.ML

TLDR: 本文研究了在潜在变量线性非高斯无环模型（lvLiNGAM）中使用高阶累积量识别因果效应，解决了存在潜在混杂时的两个挑战性场景：单代理变量和欠定工具变量。


<details>
  <summary>Details</summary>
Motivation: 潜在混杂变量在因果推断中带来挑战，尤其是在单代理变量或工具变量不足的情况下，现有方法难以准确识别因果效应。

Method: 利用高阶累积量，证明了在单代理或工具变量下因果效应的可识别性，并提出了相应的估计方法。

Result: 实验结果表明，相比现有方法，所提方法在准确性和鲁棒性上表现更优。

Conclusion: 本文在理论和实践上推进了线性系统中潜在混杂变量下的因果推断研究。
...
### [444] [Admissibility of Completely Randomized Trials: A Large-Deviation Approach](https://arxiv.org/abs/2506.05329)
*Guido Imbens,Chao Qin,Stefan Wager*

Main category: stat.ML

TLDR: 在最佳臂识别问题中，当实验者可以选择自适应试验时，忽略此选项而运行非自适应试验是不可取的。研究发现，至少有三个处理臂时，简单的自适应设计普遍且严格优于非自适应完全随机试验。
...
Method: 研究采用批量臂淘汰设计，逐步淘汰表现不佳的处理臂，并通过效率指数量化设计在大样本下的统计效率。

Result: 当至少有三个处理臂时，自适应设计普遍且严格优于非自适应完全随机试验。
...
### [445] [Trustworthiness Preservation by Copies of Machine Learning Systems](https://arxiv.org/abs/2506.05203)
*Leonardo Ceragioli,Giuseppe Primiero*

Main category: cs.LO

TLDR: 论文提出了一种计算工具，用于验证从已知行为原始系统复制的复杂系统的可信赖性，定义了四种不同的可信赖性概念。
...
Method: 引入了一种演算方法，用于建模和验证数据和概率复杂查询，定义了四种可信赖性概念。

Result: 研究了这些可信赖性概念之间的关系及其在逻辑操作下的组合方式。

Conclusion: 提供了一种计算工具，用于检查复制系统的可信赖性。
...
### [446] [Solving engineering eigenvalue problems with neural networks using the Rayleigh quotient](https://arxiv.org/abs/2506.04375)
*Conor Rowan,John Evans,Kurt Maute,Alireza Doostan*

Main category: math.NA

TLDR: 该论文提出了一种基于神经网络离散化和瑞利商变分形式的特征值分析方法，结合格拉姆-施密特正交化，解决了连续特征值问题的计算难题。


<details>
  <summary>Details</summary>
Motivation: 特征值问题在工程中广泛应用，但在物理信息机器学习领域研究较少，尤其是神经网络离散化方法。本文旨在填补这一空白。

Method: 采用神经网络离散化特征函数，结合瑞利商变分形式和格拉姆-施密特正交化，计算特征值和特征函数。

Result: 该方法适用于不规则域上的调和函数、参数和非线性特征问题以及高维特征分析，展示了在工程力学中的实用性。

Conclusion: 神经网络离散化结合瑞利商和正交化方法，为连续特征值问题提供了简单且鲁棒的解决方案。
...
### [447] [A Comprehensive Survey on Bio-Inspired Algorithms: Taxonomy, Applications, and Future Directions](https://arxiv.org/abs/2506.04238)
...
TLDR: 本文综述了生物启发算法（BIAs）的分类、核心原理、应用及未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 为研究者和实践者提供生物启发计算领域的全面概述，并指出未来研究方向。

Method: 将BIAs分为八类，分析其原理、优缺点，并探讨在机器学习等领域的应用。

Result: 总结了BIAs的最新进展，如混合策略和参数调优，并提出了可扩展性等挑战。

Conclusion: 本文为生物启发计算的研究和实践提供了基础资源，并指明了未来发展方向。
...
### [448] [Computationally Intensive Research: Advancing a Role for Secondary Analysis of Qualitative Data](https://arxiv.org/abs/2506.04230)
*Kaveh Mohajeri,Amir Karami*

Main category: cs.DB

TLDR: 本文探讨了计算密集型方法在重新利用过去定性研究数据中的潜力，提出了二次分析的方案及其创新研究设计的可能性，并列举了数据共享与重用中的挑战。
...
Method: 提出了一种计算密集型二次分析的方案，并讨论了其在创新研究设计中的应用。

Result: 强调了计算密集型二次分析的优势，同时指出了数据共享与重用中的关键挑战。

Conclusion: 计算密集型二次分析为定性数据的再利用提供了新思路，但需解决数据共享与重用中的问题。
...
### [449] [Understanding and Meeting Practitioner Needs When Measuring Representational Harms Caused by LLM-Based Systems](https://arxiv.org/abs/2506.04482)
*Emma Harvey,Emily Sheng,Su Lin Blodgett,Alexandra Chouldechova,Jean Garcia-Gathright,Alexandra Olteanu,Hanna Wallach*

Main category: cs.CY

TLDR: 研究发现，现有公开工具难以满足实践者评估大型语言模型（LLM）表征性危害的需求，主要因工具不实用或存在使用障碍。
...
Result: 实践者常因工具不实用或使用障碍而无法使用公开工具。

Conclusion: 建议根据测量理论和实用测量改进工具，以更好地满足实践者需求。
...
### [450] [Oversight Structures for Agentic AI in Public-Sector Organizations](https://arxiv.org/abs/2506.04836)
*Chris Schmitz,Jonathan Rystrøm,Jan Batzner*

Main category: cs.CY

TLDR: 论文发现，引入代理性AI系统加剧了传统公共部门监督机制的挑战，并提出五个关键治理维度。通过混合方法研究发现，代理监督放大了三个现有治理挑战，并提出适应性解决方案。
...
Result: 发现代理监督加剧了三个现有治理挑战：持续监督、治理与运营能力的深度整合及跨部门协调。

Conclusion: 提出适应性解决方案，以应对公共部门约束下的代理监督需求。
...
### [451] [Intentionally Unintentional: GenAI Exceptionalism and the First Amendment](https://arxiv.org/abs/2506.05211)
*David Atkinson,Jena D. Hwang,Jacob Morrison*

Main category: cs.CY

TLDR: 论文挑战了法院应给予大型生成AI模型（如GPT-4和Gemini）输出第一修正案保护的假设，认为其输出不构成法律意义上的言论，因此无需保护。


<details>
  <summary>Details</summary>
Motivation: 探讨AI模型输出是否应受第一修正案保护，避免因过度保护导致社会危害。

Method: 通过法律先例和言论自由的基本原则分析AI输出的性质。

Result: AI输出不构成言论，用户无权主张接收这些输出的言论自由权。

Conclusion: 不应将第一修正案权利扩展至AI模型，以免阻碍政府有效监管并引发社会风险。
...
### [452] [BridgeNet: A Hybrid, Physics-Informed Machine Learning Framework for Solving High-Dimensional Fokker-Planck Equations](https://arxiv.org/abs/2506.04354)
*Elmira Mirzabeigi,Rezvan Salehi,Kourosh Parand*

Main category: physics.comp-ph

TLDR: BridgeNet结合CNN与PINN，高效求解非线性高维Fokker-Planck方程，优于传统PINN方法。
...
### [453] [TQml Simulator: Optimized Simulation of Quantum Machine Learning](https://arxiv.org/abs/2506.04891)
*Viacheslav Kuzmin,Basil Kyriacou,Mateusz Papierz,Mo Kordzanganeh,Alexey Melnikov*

Main category: quant-ph

TLDR: 论文提出了一种针对量子机器学习中硬件高效电路的数值模拟方法，开发了名为TQml Simulator的模拟器，显著提升了模拟速度。
...
Result: TQml Simulator在标准门集和量子处理单元上表现优异，比Pennylane的默认模拟器快2至100倍。
...
### [454] [Knowledge-guided Contextual Gene Set Analysis Using Large Language Models](https://arxiv.org/abs/2506.04303)
...
TLDR: cGSA是一种新型AI驱动的基因集分析框架，通过结合上下文感知的路径优先级，解决了传统GSA方法忽略临床背景的问题，提高了结果的精确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统GSA方法生成的路径列表冗长且缺乏特异性，需大量人工解读，降低了可靠性和可重复性。cGSA旨在通过上下文感知技术改进这一问题。

Method: cGSA整合了基因簇检测、富集分析和大型语言模型，以识别统计显著且生物学意义明确的路径。

Result: 在19种疾病和10种疾病相关机制的102个基因集上，cGSA比基线方法性能提升30%以上，专家验证确认其高精度和可解释性。
...
### [455] [Spore in the Wild: Case Study on Spore.fun, a Real-World Experiment of Sovereign Agent Open-ended Evolution on Blockchain with TEEs](https://arxiv.org/abs/2506.04236)
*Botao Amber Hu,Helena Rong*

Main category: cs.MA

TLDR: 论文探讨了通过开放系统（如区块链和LLM代理）实现开放式演化（OEE）的可能性，并以Spore.fun为例研究了代理行为和演化轨迹。


<details>
  <summary>Details</summary>
Motivation: 传统封闭系统（如Tierra和Avida）无法实现持续的OEE，因此需要开放系统与环境持续交互。

Method: 利用DePIN和TEE技术，部署基于LLM的区块链代理，研究其自主演化和行为。

Result: Spore.fun展示了代理在开放环境中的自主演化和交互能力。

Conclusion: 开放系统可能通过经济激励和环境交互实现OEE，但仍需进一步讨论。
...
### [456] [CORA: Coalitional Rational Advantage Decomposition for Multi-Agent Policy Gradients](https://arxiv.org/abs/2506.04265)
*Mengda Ji,Genjiu Xu,Liying Wang*

Main category: cs.MA

TLDR: 本文提出了一种基于联盟视角的信用分配方法CORA，解决了多智能体强化学习中的信用分配问题，并通过实验验证了其优越性。


<details>
  <summary>Details</summary>
Motivation: 在合作多智能体强化学习中，全局优势共享常导致策略更新次优，因为未考虑智能体的独特贡献。现有方法缺乏对联盟层面的详细分析。

Method: 提出CORA方法，通过边际贡献评估联盟优势，并利用合作博弈论的核心解分解优势，确保联盟合理性。采用随机联盟采样降低计算开销。
...
### [457] [Autonomous Collaborative Scheduling of Time-dependent UAVs, Workers and Vehicles for Crowdsensing in Disaster Response](https://arxiv.org/abs/2506.04276)
*Lei Han,Yitong Guo,Pengfei Yang,Zhiyong Yu,Liang Wang,Quan Wang,Zhiwen Yu*

Main category: cs.MA

TLDR: 论文提出HoAs-PALN算法，通过自适应降维和局部纳什均衡博弈，实现灾后环境信息的高效采集，显著提升任务完成率和决策效率。


<details>
  <summary>Details</summary>
Motivation: 灾后环境复杂，现有传感技术适应性差、能力不足，亟需高效协同调度方案。

Method: 采用自适应降维匹配和局部纳什均衡博弈，优化无人机、工人和车辆的协同调度。

Result: 实验显示HoAs-PALN任务完成率平均提升64.12%，决策时间低于10秒。
...
### [458] [Time to Talk: LLM Agents for Asynchronous Group Communication in Mafia Games](https://arxiv.org/abs/2506.05309)
*Niv Eckhaus,Uri Berger,Gabriel Stanovsky*

Main category: cs.MA

TLDR: 论文提出了一种异步LLM代理，能够决定何时发言，并在在线Mafia游戏中表现与人类玩家相当。
...
Result: 代理在游戏表现和融入人类玩家方面与人类相当，发言时机行为接近人类，但内容有差异。

Conclusion: 研究为LLM在现实群体场景（如团队讨论、教育）中的应用铺平了道路，并公开数据以促进进一步研究。
...
### [459] [The GAIN Model: A Nature-Inspired Neural Network Framework Based on an Adaptation of the Izhikevich Model](https://arxiv.org/abs/2506.04247)
*Gage K. R. Hooper*

Main category: q-bio.NC

TLDR: GAIN模型通过网格结构提升生物合理性和动态性，结合Izhikevich模型实现高效且准确的神经模拟。
...
Method: 采用网格结构和Izhikevich模型，优化神经元间的连接与动态。

Result: 实现了高效、生物准确的神经模拟，适用于大规模模拟和神经科学研究。

Conclusion: GAIN模型结合Izhikevich模型提升了动态性和准确性，具有高效性和专业性。
...
### [460] [Generalizable, real-time neural decoding with hybrid state-space models](https://arxiv.org/abs/2506.05320)
*Avery Hee-Woon Ryoo,Nanda H. Krishna,Ximeng Mao,Mehdi Azabou,Eva L. Dyer,Matthew G. Perich,Guillaume Lajoie*

Main category: q-bio.NC

TLDR: POSSM是一种新型混合架构，结合了交叉注意力模块的单个尖峰标记化和递归状态空间模型（SSM）主干，旨在实现快速、因果的在线神经活动预测，并通过多数据集预训练高效泛化到新任务。


<details>
  <summary>Details</summary>
Motivation: 传统方法（如简单循环神经网络）速度快但泛化能力差，而基于Transformer的方法泛化能力强但计算成本高，不适合实时或低资源场景。POSSM旨在解决这些不足。

Method: POSSM结合交叉注意力模块和递归SSM主干，支持快速在线预测和多数据集预训练，以提升泛化能力。

Result: 在猴子和人类任务中，POSSM的解码精度与最先进的Transformer相当，但推理速度快9倍，且跨物种预训练提升了人类任务的性能。

Conclusion: 混合SSM是一种有前景的方法，可在精度、推理速度和泛化能力之间取得平衡，适用于实时闭环神经解码应用。
...
### [461] [Tech-ASan: Two-stage check for Address Sanitizer](https://arxiv.org/abs/2506.05022)
*Yixuan Cao,Yuhong Feng,Huafeng Li,Chongyi Huang,Fangcao Jian,Haoran Li,Xu Wang*

Main category: cs.SE

TLDR: Tech-ASan是一种基于两阶段检查的技术，通过减少昂贵的影子内存访问和优化冗余检查，显著降低了ASan的运行开销，同时保持了检测能力。


<details>
  <summary>Details</summary>
Motivation: ASan在检测C/C++程序中的内存安全违规时运行开销大，限制了其在大规模软件测试中的效率。现有方法未能充分消除冗余检查或牺牲了检测能力。

Method: 提出了两阶段检查算法，利用魔法值比较减少影子内存访问；设计了高效优化器消除冗余检查；基于LLVM实现了Tech-ASan工具。

Result: 在SPEC CPU2006基准测试中，Tech-ASan的运行开销比ASan和ASan--分别减少了33.70%和17.89%；在Juliet测试套件中检测到的假阴性案例减少了56个。

Conclusion: Tech-ASan在显著降低运行开销的同时，保持了高检测能力，优于现有方法。
...
### [462] [A Multi-Dataset Evaluation of Models for Automated Vulnerability Repair](https://arxiv.org/abs/2506.04987)
*Zanis Ali Khan,Aayush Garg,Qiang Tang*

Main category: cs.SE

TLDR: 研究探讨了预训练语言模型CodeBERT和CodeT5在自动漏洞修补中的表现，发现CodeBERT在上下文稀疏时表现更好，而CodeT5在复杂漏洞模式上更优，但模型在未知数据上泛化能力有限。
...
Method: 使用CodeBERT和CodeT5在六个数据集和四种语言中评估漏洞修补能力，测试其在已知和未知数据上的表现。

Result: CodeBERT在稀疏上下文表现更好，CodeT5在复杂漏洞模式上更优且扩展性更强，但模型在未知数据上泛化能力不足。

Conclusion: 研究为自动漏洞修补提供了性能基准和局限性分析，为实际安全应用提供了改进方向。
...
### [463] [Leveraging Reward Models for Guiding Code Review Comment Generation](https://arxiv.org/abs/2506.04464)
*Oussama Ben Sghaier,Rosalia Tufano,Gabriele Bavota,Houari Sahraoui*

Main category: cs.SE

TLDR: CoRAL是一个基于深度学习的框架，利用强化学习自动生成代码审查评论，结合语义和实用性奖励机制，优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 代码审查耗时且受主观因素影响，自动化工具需求迫切。

Method: CoRAL框架利用强化学习，奖励机制考虑生成评论的语义和实用性。

Result: CoRAL生成的评论在定量和定性上优于基线方法。

Conclusion: CoRAL有效且优越，为自动化代码审查提供了新思路。
...
### [464] [Interpretable LLMs for Credit Risk: A Systematic Review and Taxonomy](https://arxiv.org/abs/2506.04290)
*Muhammed Golec,Maha AlabdulJalil*

Main category: q-fin.RM

TLDR: 本文系统综述了基于大语言模型（LLM）的信用风险评估方法，提出了分类法，涵盖模型架构、数据类型、可解释性机制和应用领域，并指出未来研究方向。
...
Method: 通过PRISMA研究策略筛选60篇相关论文（2020-2025），分析模型架构、数据类型、可解释性机制和应用场景。

Result: 提出了基于LLM的信用风险评估分类法，涵盖四大主题，并识别了未来研究趋势和空白。

Conclusion: 本文为AI和金融领域的研究者提供了重要参考，并指出了LLM在信用评分系统中的未来发展方向。
...
### [465] [Diffusion Transformer-based Universal Dose Denoising for Pencil Beam Scanning Proton Therapy](https://arxiv.org/abs/2506.04467)
...
TLDR: 提出了一种基于扩散变换器的去噪框架，用于快速生成高质量的质子治疗剂量图，解决了低统计蒙特卡罗模拟的精度问题。


<details>
  <summary>Details</summary>
Motivation: IMPT在头颈癌治疗中需要频繁的在线自适应放疗，但快速准确的剂量计算依赖于蒙特卡罗模拟，低粒子数会降低精度。

Method: 开发了扩散变换器去噪框架，使用头颈癌患者的IMPT计划和CT图像生成噪声和高统计剂量图，通过多种损失函数训练模型。

Result: 模型在不同癌症部位（头颈、肺、乳腺、前列腺）的MAE和3D Gamma通过率表现优异，DVH指标与真实值接近。

Conclusion: 该框架虽仅用头颈癌数据训练，但在多疾病部位表现良好，具有广泛适用性。
...
### [466] [An Expansion-Based Approach for Quantified Integer Programming](https://arxiv.org/abs/2506.04452)
*Michael Hartisch,Leroy Chew*

Main category: cs.DM

TLDR: 论文提出了一种基于扩展的QIP求解方法，采用CEGAR技术，性能优于现有搜索方法。


<details>
  <summary>Details</summary>
Motivation: QIP结合了QBF和整数规划的优势，但现有方法仅限搜索范式，扩展方法未被探索。

Method: 引入基于CEGAR的扩展方法，适用于多阶段鲁棒离散优化问题。

Result: 实验显示该方法在特定实例中优于搜索求解器，且线性约束建模带来性能提升。

Conclusion: 扩展方法为QIP提供了新解决方案，尤其在多阶段优化中表现突出。
...
### [467] [kTULA: A Langevin sampling algorithm with improved KL bounds under super-linear log-gradients](https://arxiv.org/abs/2506.04878)
*Iosif Lytras,Sotirios Sabanis,Ying Zhang*

Main category: math.ST

TLDR: 论文提出了一种名为kTULA的新算法，用于解决深度学习中超线性增长对数梯度分布的采样问题，并提供了理论性能保证。
...
Result: 在KL散度中取得最佳收敛速率$2-\overline{\epsilon}$，显著改进现有结果，并进一步优化Wasserstein-2距离误差界。
...
### [468] [Exp4Fuse: A Rank Fusion Framework for Enhanced Sparse Retrieval using Large Language Model-based Query Expansion](https://arxiv.org/abs/2506.04760)
*Lingyuan Liu,Mengxiang Zhang*

Main category: cs.IR

TLDR: Exp4Fuse是一种新型融合排序框架，通过零样本LLM查询扩展提升稀疏检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM查询扩展方法成本高且计算密集，需改进稀疏检索性能。

Method: 提出Exp4Fuse框架，结合原始查询和LLM增强查询的双重检索路径，采用改进的互逆排名融合方法。

Result: 在多个数据集上超越现有LLM查询扩展方法，结合稀疏检索器达到SOTA性能。

Conclusion: Exp4Fuse显著提升稀疏检索的查询扩展效果，性能优越。
...
### [469] [GOLFer: Smaller LM-Generated Documents Hallucination Filter & Combiner for Query Expansion in Information Retrieval](https://arxiv.org/abs/2506.04762)
*Lingyuan Liu,Mengxiang Zhang*

Main category: cs.IR

TLDR: GOLFer是一种利用小型开源语言模型进行查询扩展的新方法，通过过滤幻觉内容和组合文档来提升性能，效果优于其他小型模型方法，并与大型模型方法竞争。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）用于查询扩展成本高且计算密集，小型模型生成的文档常包含非事实内容。GOLFer旨在解决这些问题。

Method: GOLFer包含幻觉过滤器（检测并移除非事实内容）和文档组合器（通过权重向量平衡查询与文档）。

Result: 在多个数据集上，GOLFer表现优于其他小型模型方法，并与大型模型方法竞争。

Conclusion: GOLFer提供了一种高效且低成本的小型模型查询扩展解决方案。
...
### [470] [Towards Storage-Efficient Visual Document Retrieval: An Empirical Study on Reducing Patch-Level Embeddings](https://arxiv.org/abs/2506.04997)
*Yubo Ma,Jinsong Li,Yuhang Zang,Xiaobao Wu,Xiaoyi Dong,Pan Zhang,Yuhang Cao,Haodong Duan,Jiaqi Wang,Yixin Cao,Aixin Sun*

Main category: cs.IR

TLDR: 论文研究了减少视觉文档检索（VDR）中每页嵌入数量的方法，比较了标记剪枝和标记合并策略，发现合并更有效，最终提出Light-ColPali/ColQwen2，显著降低内存使用同时保持高性能。


<details>
  <summary>Details</summary>
Motivation: ColPali/ColQwen2在VDR中性能强，但每页生成多个嵌入导致内存占用过高，需优化以减少嵌入数量且性能损失最小。

Method: 评估了标记剪枝和标记合并两种策略，发现剪枝不适合VDR，合并更有效，并优化了合并策略的三维组合。

Result: Light-ColPali/ColQwen2仅用11.8%内存即保持98.2%检索性能，2.8%内存时仍保留94.6%效果。

Conclusion: 研究为高效VDR提供了实用见解和基线，Light-ColPali/ColQwen2是未来研究的竞争基准。
...
### [471] [Reason-to-Recommend: Using Interaction-of-Thought Reasoning to Enhance LLM Recommendation](https://arxiv.org/abs/2506.05069)
*Keyu Zhao,Fengli Xu,Yong Li*

Main category: cs.IR

TLDR: 论文提出R2Rec框架，通过渐进式掩码提示策略将用户-物品交互链转化为结构化推理，结合监督微调和强化学习提升推荐性能。
...
Method: R2Rec框架通过交互链采样和结构化推理模拟逐步决策，采用两阶段训练（监督微调+强化学习）。

Result: 在三个真实数据集上，R2Rec在HitRatio@1上平均提升10.48%，比原始LLM提升131.81%。

Conclusion: R2Rec显著提升推荐性能，并通过显式推理链增强可解释性。
...
### [472] [Was Residual Penalty and Neural Operators All We Needed for Solving Optimal Control Problems?](https://arxiv.org/abs/2506.04742)
*Oliver G. S. Lundqvist,Fabricio Oliveira*

Main category: math.OC

TLDR: 论文提出了一种简化神经网络解决最优控制问题的方法，通过解耦训练过程与最优控制问题，仅需单次物理信息训练和后续优化即可解决多类问题。


<details>
  <summary>Details</summary>
Motivation: 传统方法在训练神经网络时需结合成本函数，导致架构复杂。本文旨在简化流程，证明无需在训练中考虑成本函数。

Method: 采用DeepONet等简单神经算子架构，结合无约束优化程序，通过自动微分计算控制梯度。

Result: 在九个不同最优控制问题上验证，仅需训练三个DeepONet模型，即能准确解决多类问题。

Conclusion: 解耦训练与优化过程的方法简化了架构，提升了效率，适用于多类最优控制问题。
...
### [473] [Non-linear Multi-objective Optimization with Probabilistic Branch and Bound](https://arxiv.org/abs/2506.04554)
*Hao Huang,Zelda B. Zabinsky*

Main category: math.OC

TLDR: 提出了一种名为MOPBnB(so)的多目标仿真优化算法，用于近似随机多目标优化问题的帕累托最优集和有效前沿。该算法通过单次评估和邻域解估计目标函数，相比多次重复评估的变体更高效。
...
Result: 数值结果表明，MOPBnB(so)在计算资源消耗上显著优于多次重复评估的变体，且在测试问题上优于遗传算法NSGA-II。

Conclusion: MOPBnB(so)是一种高效且性能优越的多目标优化算法，适用于随机多目标优化问题。
...
### [474] [Estimating properties of a homogeneous bounded soil using machine learning models](https://arxiv.org/abs/2506.04256)
*Konstantinos Kalimeris,Leonidas Mindrinos,Nikolaos Pallikarakis*

Main category: physics.geo-ph

TLDR: 论文通过模拟数据和机器学习模型估计土壤性质，发现支持向量机和神经网络在预测扩散系数和导水率时表现最佳。


<details>
  <summary>Details</summary>
Motivation: 研究旨在从水分测量中准确估计土壤性质，解决参数识别问题。

Method: 使用Fokas方法生成模拟数据，并比较多种机器学习模型在完整、噪声和有限数据条件下的表现。

Result: 扩散系数D的预测比导水率K更准确，支持向量机和神经网络表现最稳健。

Conclusion: 支持向量机和神经网络是解决土壤性质估计问题的有效工具。
...
### [475] [MVP-Shapley: Feature-based Modeling for Evaluating the Most Valuable Player in Basketball](https://arxiv.org/abs/2506.04602)
*Haifeng Sun,Yu Xiong,Runze Wu,Kai Wang,Lan Zhang,Changjie Fan,Shaojie Tang,Xiang-Yang Li*

Main category: cs.GT

TLDR: 提出了一种基于Shapley值的MVP评估框架\oursys，通过特征处理、胜负模型训练和贡献分配，优化算法并与专家投票结果对齐，验证了其有效性。
...
Method: 利用比赛中的逐场比赛数据（如助攻和得分），提出\oursys框架，结合Shapley值进行特征处理、模型训练和贡献排名，并通过因果视角优化算法。

Result: 在NBA和Dunk City Dynasty数据集上验证了方法的有效性，并实现了工业部署。

Conclusion: \oursys框架为MVP评估提供了一种可解释且实用的解决方案，适用于游戏和体育领域。
...
### [476] [RoboRefer: Towards Spatial Referring with Reasoning in Vision-Language Models for Robotics](https://arxiv.org/abs/2506.04308)
...
TLDR: RoboRefer是一种3D感知的视觉语言模型，通过监督微调（SFT）和强化微调（RFT）实现精确的空间理解和多步推理，在RefSpatial-Bench上表现优异。
...
Method: 结合SFT和RFT，引入专用深度编码器和度量敏感奖励函数，使用RefSpatial数据集（20M QA对）进行训练。

Result: SFT训练的RoboRefer空间理解成功率89.6%，RFT训练的版本在RefSpatial-Bench上超越Gemini-2.5-Pro 17.4%。

Conclusion: RoboRefer在空间理解和多步推理任务中表现卓越，可应用于多样化机器人控制。
...
### [477] [cuVSLAM: CUDA accelerated visual odometry](https://arxiv.org/abs/2506.04359)
*Alexander Korovko,Dmitry Slepichev,Alexander Efitorov,Aigul Dzhumamuratova,Viktor Kuznetsov,Hesam Rabeti,Joydeep Biswas*

Main category: cs.RO

TLDR: cuVSLAM是一种先进的视觉SLAM解决方案，支持多种视觉-惯性传感器组合，适用于不同机器人配置，并通过CUDA优化实现实时边缘计算。


<details>
  <summary>Details</summary>
Motivation: 精确且鲁棒的姿态估计是自主机器人的关键需求，需要一种适用于多种传感器配置的高效SLAM解决方案。
...
### [478] [Hierarchical Language Models for Semantic Navigation and Manipulation in an Aerial-Ground Robotic System](https://arxiv.org/abs/2506.05020)
*Haokun Liu,Zhaoqi Ma,Yunong Li,Junichiro Sugihara,Yicheng Chen,Jinjie Li,Moju Zhao*

Main category: cs.RO

TLDR: 提出了一种结合大型语言模型（LLM）和视觉语言模型（VLM）的分层框架，用于异构多机器人系统的任务协调与动态环境适应。
...
Method: 采用LLM进行任务分解和全局语义地图构建，VLM从航拍图像提取语义标签和空间信息支持局部规划。

Result: 实验证明该框架在动态环境中具有适应性和鲁棒性，首次实现了VLM感知与LLM任务推理和运动规划的异构系统。

Conclusion: 该框架为异构多机器人系统在复杂任务中的协作提供了新的解决方案。
...
### [479] [Learning Smooth State-Dependent Traversability from Dense Point Clouds](https://arxiv.org/abs/2506.04362)
*Zihao Dong,Alan Papalia,Leonard Jung,Alenna Spiro,Philip R. Osteen,Christa S. Robison,Michael Everett*

Main category: cs.RO

TLDR: SPARTA提出了一种基于点云的方法，通过输出平滑的解析函数来预测不同接近角度的地形可通行性，解决了传统方法需要大量数据和计算的问题。


<details>
  <summary>Details</summary>
Motivation: 越野自主性中的关键挑战是地形的可通行性依赖于车辆状态，传统方法需要大量数据和重复计算。

Method: SPARTA利用傅里叶基函数构建平滑解析函数，预测任意接近角度的风险分布，减少计算开销。

Result: 在高保真模拟中，SPARTA成功穿越40米巨石场的成功率为91%，优于基线的73%。

Conclusion: SPARTA通过几何结构和傅里叶基函数，显著提升了地形可通行性预测的效率和泛化能力。
...
### [480] [MineInsight: A Multi-sensor Dataset for Humanitarian Demining Robotics in Off-Road Environments](https://arxiv.org/abs/2506.04842)
*Mario Malizia,Charles Hamesse,Ken Hasselmann,Geert De Cubber,Nikolaos Tsiogkas,Eric Demeester,Rob Haelterman*

Main category: cs.RO

TLDR: 论文介绍了MineInsight数据集，用于改进人道主义排雷中的计算机视觉技术。


<details>
  <summary>Details</summary>
Motivation: 缺乏多样化和真实的数据集阻碍了排雷算法的可靠验证。

Method: 创建了一个多传感器、多光谱的公开数据集，包含35个目标，集成双视角传感器扫描和多种光谱范围图像。

Result: 数据集包含约38,000 RGB帧、53,000 VIS-SWIR帧和108,000 LWIR帧，提供多样化的测试环境。

Conclusion: MineInsight为开发和评估排雷算法提供了基准，填补了研究空白。
...
### [481] [Synthetic Dataset Generation for Autonomous Mobile Robots Using 3D Gaussian Splatting for Vision Training](https://arxiv.org/abs/2506.05092)
*Aneesh Deogan,Wout Beks,Peter Teurlings,Koen de Vos,Mark van den Brand,Rene van de Molengraft*

Main category: cs.RO

TLDR: 提出了一种基于虚幻引擎的自动生成合成数据方法，用于解决手动标注数据集的耗时、易错和多样性不足问题，尤其在机器人足球领域表现优异。


<details>
  <summary>Details</summary>
Motivation: 手动创建标注数据集耗时且易错，尤其在机器人领域多样性不足，需要一种更高效、可扩展的解决方案。

Method: 利用虚幻引擎和3D高斯样条技术快速生成合成数据，结合真实数据提升检测性能。

Result: 合成数据训练的检测器在机器人足球场景中表现与真实数据相当，且结合两者能显著提升性能。

Conclusion: 该方法为机器人领域提供了一种高效、可扩展的数据生成方案，减少了手动标注的需求。
...
### [482] [SGN-CIRL: Scene Graph-based Navigation with Curriculum, Imitation, and Reinforcement Learning](https://arxiv.org/abs/2506.04505)
*Nikita Oskolkov,Huzhenyu Zhang,Dmitry Makarov,Dmitry Yudin,Aleksandr Panov*

Main category: cs.RO

TLDR: 提出了一种基于3D场景图的强化学习导航框架SGN-CIRL，结合模仿学习和课程学习，显著提高了复杂导航任务的成功率。


<details>
  <summary>Details</summary>
Motivation: 解决部分可观测环境中机器人导航效率低的问题，通过3D场景图建模物体间空间关系，提升目标物体定位能力。

Method: 采用3D场景图表示环境，结合强化学习、模仿学习和课程学习，逐步提升任务复杂度。

Result: 在Isaac Sim环境中的实验表明，该方法显著提高了困难导航任务的成功率。

Conclusion: SGN-CIRL框架通过3D场景图和混合学习方法，有效提升了机器人导航性能，代码已开源。
...
### [483] [Olfactory Inertial Odometry: Sensor Calibration and Drift Compensation](https://arxiv.org/abs/2506.04539)
*Kordel K. France,Ovidiu Daescu,Anirban Paul,Shalini Prasad*

Main category: cs.RO

TLDR: 论文提出了一种校准机器人嗅觉惯性里程计（OIO）的方法，以提高其在慢速移动平台上定位气味源的厘米级精度，适用于机器人手术和无接触安检等场景。
...
Conclusion: 提出的OIO校准方法能够有效提升嗅觉导航的精度，具有实际应用潜力。
...
### [484] [Chronoamperometry with Room-Temperature Ionic Liquids: Sub-Second Inference Techniques](https://arxiv.org/abs/2506.04540)
*Kordel K. France*

Main category: cs.RO

TLDR: 提出了一种新的数学回归方法，将计时电流法（CA）的测量窗口缩短至1秒以内，显著快于传统方法。


<details>
  <summary>Details</summary>
Motivation: 室温离子液体（RTILs）中高粘度和慢质量传输导致测量时间延长，需要更快的方法。

Method: 通过推理算法分析初始瞬态电流响应，预测稳态电化学参数，无需硬件修改。

Result: 验证显示该方法在保持合理精度的同时，大幅缩短数据采集时间。

Conclusion: 该技术在分析化学、传感器技术和电池科学中有广泛应用前景，特别适合快速电化学定量。
