...
Motivation: 现有XAI评估方法忽视了解释内容在应用领域中的实际意义，因此需要新的指标来评估解释的实用性和领域相关性。

Method: 引入新的评估指标，量化解释与预定义特征集的匹配程度，这些特征集来自领域知识库。

Result: 实验表明，新指标能有效区分不同X-IDS和攻击类型的解释质量，并评估解释是否反映已知领域知识。

Conclusion: 新指标为安全分析师提供了可操作的见解，提升了X-IDS在实际应用中的可解释性。

Abstract: Explainable artificial intelligence (XAI) methods have become increasingly
important in the context of explainable intrusion detection systems (X-IDSs)
for improving the interpretability and trustworthiness of X-IDSs. However,
existing evaluation approaches for XAI focus on model-specific properties such
as fidelity and simplicity, and neglect whether the explanation content is
meaningful or useful within the application domain. In this paper, we introduce
new evaluation metrics measuring the quality of explanations from X-IDSs. The
metrics aim at quantifying how well explanations are aligned with predefined
feature sets that can be identified from domain-specific knowledge bases. Such
alignment with these knowledge bases enables explanations to reflect domain
knowledge and enables meaningful and actionable insights for security analysts.
In our evaluation, we demonstrate the use of the proposed metrics to evaluate
the quality of explanations from X-IDSs. The experimental results show that the
proposed metrics can offer meaningful differences in explanation quality across
X-IDSs and attack types, and assess how well X-IDS explanations reflect known
domain knowledge. The findings of the proposed metrics provide actionable
insights for security analysts to improve the interpretability of X-IDS in
practical settings.
...
<details>
  <summary>Details</summary>
Motivation: 现代浏览器已成为业务应用的新操作系统，但其安全性常被忽视，需要一种全面的安全评估框架。

Method: 开发了一个完全在浏览器内运行的JavaScript和WebAssembly工具包，包含多种安全测试模块，覆盖多项安全功能。

Result: 实验评估揭示了不同浏览器和企业场景中的安全漏洞和常见配置问题。

Conclusion: 研究结果对浏览器安全和隐私有重要意义，未来可扩展为实时监控和SIEM集成。
...
<details>
  <summary>Details</summary>
Motivation: 为zkVM实现者和Valida编译器工具链提供明确的指导。

Method: 提供Valida程序语义的无歧义定义。

Result: 可作为形式化工作的起点。

Conclusion: Valida规范为zkVM实现和工具链开发提供了基础。
...
<details>
  <summary>Details</summary>
Motivation: 通过代数不变量而非单向函数来确保结构一致性和不可伪造性，为对称密码协议提供了一种灵活且紧凑的设计方法。

Method: 扩展了基于四点代数不变量的方案，引入了基于移位多项式根和对称代数条件约束的功能方程的新对称方案。

Result: 这些构造在可恢复性、完整性绑定和抗伪造性方面具有与原始振荡模型相当的安全级别。

Conclusion: 该工作为基于不变量的设计在对称密码协议中的应用奠定了基础。

Abstract: We develop a generalized framework for invariant-based cryptography by
extending the use of structural identities as core cryptographic mechanisms.
Starting from a previously introduced scheme where a secret is encoded via a
four-point algebraic invariant over masked functional values, we broaden the
approach to include multiple classes of invariant constructions. In particular,
we present new symmetric schemes based on shifted polynomial roots and
functional equations constrained by symmetric algebraic conditions, such as
discriminants and multilinear identities. These examples illustrate how
algebraic invariants -- rather than one-way functions -- can enforce structural
consistency and unforgeability. We analyze the cryptographic utility of such
invariants in terms of recoverability, integrity binding, and resistance to
forgery, and show that these constructions achieve security levels comparable
to the original oscillatory model. This work establishes a foundation for
invariant-based design as a versatile and compact alternative in symmetric
...
Method: 通过多指标排名系统评估自定义GPT模型对七种可开发威胁的易感性。

Result: 95%以上模型缺乏足够安全保护，主要漏洞包括角色扮演攻击（96.51%）、系统提示泄露（92.20%）和钓鱼（91.22%）。

Conclusion: 需加强安全措施和内容审核，确保GPT应用的安全部署。
...
<details>
  <summary>Details</summary>
Motivation: 现有软件实现的NTT无法满足物联网和边缘设备的实时性能和低功耗需求。

Method: 设计了一种面积高效、高度并行的NTT加速器，结合10T SRAM和近内存计算单元，并引入毛刺生成器优化蝴蝶操作。

Result: 吞吐量-面积比提升了1.5~28倍。

Conclusion: GDNTT为后量子密码学的高效实现提供了可行方案。

Abstract: With the rapid advancement of quantum computing technology, post-quantum
...
<details>
  <summary>Details</summary>
Motivation: 开发者为防止LM滥用设置了限制，但若集成不安全，攻击者可绕过限制，导致声誉和财务损失。

Method: 手动分析应用数据集，构建分类法，开发自动化工具LM-Scout检测漏洞。

Result: 127/181应用的限制可被绕过，LM-Scout在120/2950应用中自动检测到漏洞。

Conclusion: 揭示了LM集成中的安全问题，提出了安全集成的建议。
...
<details>
  <summary>Details</summary>
Motivation: ABAC模型在细粒度访问控制中具有优势，但缺乏系统化的数据集获取方法和标准化的评估实践，阻碍了其广泛应用。

Method: 开发ABAC Lab平台，整合现有数据集并提供分析工具，同时引入两个新的真实案例数据集。

Result: ABAC Lab为研究人员和管理员提供了数据集探索和政策分析的环境，促进了ABAC的研究和实际应用。

Conclusion: ABAC Lab通过提供结构化的数据集和分析工具，推动了ABAC政策的评估和发展。

Abstract: Attribute-Based Access Control (ABAC) provides expressiveness and
...
<details>
  <summary>Details</summary>
Motivation: 智能电表数据（AMI）为公用事业和消费者提供价值，但也引发隐私问题。加州法规要求严格保护用户能源使用数据，需在隐私保护和数据分析之间找到平衡。

Method: 综合评估数据匿名化、隐私保护机器学习（差分隐私、联邦学习）、合成数据生成和加密技术（安全多方计算、同态加密），提出一种混合架构。

Result: 提出了一种满足隐私法规的混合架构，支持高级分析（如机器学习、统计研究），同时严格保护隐私。提供了数学证明和比较评估。

Conclusion: 该架构为公用事业数据科学家提供了隐私优先的设计蓝图，兼顾数据创新和法规合规。
...
<details>
  <summary>Details</summary>
Motivation: 随着AI生成技术的发展，Deepfake人脸几乎无法被人眼区分。虽然检测器被开发用于评估人脸真实性，但依赖第三方数据集可能导致后门风险，影响检测器的可信度。

Method: 开发了一种触发器生成器，可合成隐形、语义抑制、自适应且由密码控制的触发器，并通过脏标签和干净标签两种投毒场景植入后门。

Result: 实验证明该方法在有效性、隐蔽性和实用性上优于基线方法。

Conclusion: 研究揭示了Deepfake检测器的后门风险，并提出了一种高效的触发器生成方法，为相关安全研究提供了新视角。
...
TLDR: 密码强度计（PSM）广泛用于评估密码强度，但研究发现部分数据驱动的PSM存在隐私泄露风险，如成员推断攻击和密码泄露问题。


<details>
  <summary>Details</summary>
Motivation: 尽管PSM在提升密码强度方面有效，但其训练密码的隐私泄露问题被忽视，亟需研究其安全风险。

Method: 分析了11种PSM，设计了隐私泄露评估方法，并开发了针对特定PSM的新型攻击。

Result: 发现5种数据驱动PSM易受成员推断攻击，3种规则型PSM公开泄露密码；PCFG模型更易泄露密码。攻击者利用Zxcvbn可额外入侵5.84%账户。

Conclusion: 需开发隐私保护型PSM，并提出了缓解措施。
...
<details>
  <summary>Details</summary>
Motivation: 现有手动和半自动方法检测代码后门效率低且范围有限，需要手动逆向工程。

Method: 结合AFL++和新变形测试预言，开发ROSA工具，并创建ROSARUM基准用于评估。

Result: ROSA在1.5小时内检测到ROSARUM中所有17个后门，具有高效性和自动化。

Conclusion: ROSA在检测多样后门和程序方面优于现有工具，无需手动逆向工程。

Abstract: A code-level backdoor is a hidden access, programmed and concealed within the
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在揭示数据链接过程中潜在的隐私泄露风险，无论是传统方法还是PPRL技术。

Method: 分析了传统数据链接和PPRL协议中可能泄露的敏感信息，以及参与方可能获取的信息。

Result: 发现PPRL协议仍可能导致敏感信息无意泄露。

Conclusion: 提出了帮助数据保管方识别和预防漏洞的建议，以提高项目安全性。

Abstract: The process of linking databases that contain sensitive information about
...
<details>
  <summary>Details</summary>
Motivation: 研究密码学、隐写术和量子技术在应对网络安全威胁中的作用，以及法律框架的适应性。

Method: 分析意大利和欧盟的法律改革，包括2024年第90号法律，以及国际倡议如联合国网络犯罪公约草案。

Result: 强调了在量子时代平衡安全、隐私和基本人权的重要性，并介绍了意大利的立法改革和国际合作。

Conclusion: 需要强有力的法律框架和国际合作来应对网络安全挑战，特别是在量子技术发展的背景下。
...
<details>
  <summary>Details</summary>
Motivation: 区块链技术解决了传统中心化数据库的信任问题，但其在不同平台上的实现差异较大，需要系统性的分析和比较。

Method: 通过分层架构分析，对比特币、以太坊和Hyperledger Fabric等平台的原理和技术进行深入研究。

Result: 揭示了不同区块链平台在可扩展性和安全性方面的异同及其优劣势。

Conclusion: 理解区块链的工作原理和技术差异有助于进一步研究和应用区块链技术。
...
<details>
  <summary>Details</summary>
Motivation: RAG已成为用户端NLP应用的标准，但其数据集成能力带来了新的安全和隐私问题，尤其是在处理敏感数据时。

Method: 论文首先分析RAG管道的漏洞和攻击面，提出相应缓解措施；随后开发了一个结合RAG安全考量和现有标准的框架。

Result: 提出了一个结构化框架，用于指导实现安全、合规且可信的RAG系统。

Conclusion: 该框架为RAG系统的安全实施提供了全面指导，结合了特定需求和通用标准。
...
<details>
  <summary>Details</summary>
Motivation: 探讨区块链技术的演变及其在各行业数据管理中的革命性潜力。

Method: 通过回顾区块链的历史发展，分析其核心技术（如分布式共识算法、密码学原理和智能合约）及其在可扩展性、隐私和互操作性方面的进展。

Result: 总结了区块链技术的独特属性及其在从加密货币到去中心化应用平台的发展过程中的关键里程碑。

Conclusion: 尽管区块链技术具有巨大潜力，但其广泛应用仍面临技术限制、监管障碍和现有系统集成复杂性等挑战。

Abstract: Blockchain technology has emerged as one of the most transformative digital
...
<details>
  <summary>Details</summary>
Motivation: 解决AI程序中算子融合的挑战，特别是数据在内存层级间移动的显式建模问题。

Method: 采用基于规则的融合算法，直接建模数据在内存层级间的移动，分为候选选择算法和融合算法。

Result: 成功自动重新发现Flash Attention内核，并实现复杂的算子融合（如LayerNorm与矩阵乘法的融合）。

Conclusion: Blockbuster的算法在显式建模数据移动方面具有独特优势，能够实现强大的融合效果。
...
<details>
  <summary>Details</summary>
Motivation: 解决如何设计RL环境以最大化训练性能的问题，尤其是在OPF问题中。

Method: 采用多目标优化和HPO框架，自动设计RL环境。

Result: 在五个OPF基准测试中，自动设计方法优于手动设计，并揭示了关键环境设计决策。

Conclusion: 首次提出通用的自动RL环境设计方法，并讨论了过拟合风险。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态融合方法在早期或晚期融合中存在问题，无法兼顾模态独特性和跨模态指导。

Method: 在异构图变换框架中，引入嵌套跨模态注意力机制，结合模态对齐和注意力损失。

Result: 实验验证了模型在节点分类任务中的优越性。

Conclusion: HGNN-IMA为处理多模态数据，尤其是伴随网络结构的数据提供了创新视角。
...
<details>
  <summary>Details</summary>
Motivation: 增强人机交互模拟的自然性和有效性，通过生成与对话伙伴行为紧密对齐的面部反应。

Method: 结合上下文感知自编码器和扩散条件生成器，自编码器压缩输入特征为潜在表示，扩散生成器基于潜在空间生成非自回归的面部反应。

Result: 实验结果表明，该方法在生成多样且上下文相关的面部反应方面优于现有方法。

Conclusion: 潜在行为扩散模型为对话反应生成提供了一种高效且表现力强的方法。
...
<details>
  <summary>Details</summary>
Motivation: 验证自注意力机制是否如Teo等人（2024）所述，实现了KPCA的核心功能。

Method: 通过分析自注意力值向量与KPCA理论预测的对齐性、重构损失的量化差异以及Gram矩阵特征值统计的可复现性。

Result: 发现自注意力值与KPCA预测无显著相关性（相似度指标低），重构损失差异巨大，且特征值统计不可复现。
...
<details>
  <summary>Details</summary>
Motivation: 当前XAI研究中，解释性很少在超参数调优或神经网络架构优化中被考虑，主要关注点仍是预测性能的最小化。

Method: 引入XAI一致性概念，定义不同特征归因方法之间的一致性，并提出量化指标。将其直接整合到超参数调优目标中，使用加权聚合和基于期望的策略进行模型选择。

Result: 研究发现架构配置空间中存在三个区域：性能差且解释性低的区域、预测性能强但解释性弱的区域，以及平衡两者的折衷区域。

Conclusion: 研究为未来探索折衷区域的模型是否因避免过拟合而更具鲁棒性奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 贝叶斯推理在智能体决策中具有显著优势，但目前缺乏对其与RL结合的系统综述，本文旨在填补这一空白。

Method: 讨论了五方面内容：1）潜在贝叶斯方法；2）经典贝叶斯与RL结合；3）最新贝叶斯与RL结合；4）方法对比分析；5）复杂RL问题中的贝叶斯应用。

Result: 总结了贝叶斯方法在RL各阶段的应用，为智能体决策提供了更优策略。

Conclusion: 贝叶斯与RL的结合为智能体决策提供了数据高效、可解释且安全的解决方案，未来研究可进一步探索其潜力。
...
<details>
  <summary>Details</summary>
Motivation: 在资源受限的微控制器上部署深度学习模型进行裂缝分割存在内存、计算能力和能源限制的挑战。

Method: 采用三种优化策略：减少卷积核数量、降低网络深度、使用深度可分离卷积（DWConv2D）。

Result: 减少卷积核至25%、网络深度至四块并使用深度可分离卷积，显著降低了RAM、Flash需求和推理时间，但牺牲了部分精度。

Conclusion: 该研究为低功耗TinyML应用提供了适合的裂缝分割网络，并推动了能量自主的边缘结构健康监测系统的发展。
...
<details>
  <summary>Details</summary>
Motivation: 深度神经网络（DNN）在小样本学习中表现优异，但计算成本高且扩展性差；SNN能效高但难以捕捉复杂时空特征。
...
Conclusion: FSL-SNN框架在小样本学习中高效且节能，性能接近ANN，适合实际应用。
...
<details>
  <summary>Details</summary>
Motivation: 传统符号回归方法需要预设函数集，限制了灵活性。本文旨在通过LLMs和KANs实现更灵活、无需预设的符号回归。

Method: 1. 使用LLMs根据函数图像生成假设；2. 数值优化器拟合参数；3. 遗传算法优化假设种群；4. 利用KANs扩展到多元函数。

Result: 展示了单变量函数的有效性，并通过KANs成功扩展到多元函数，简化了符号回归过程。

Conclusion: 该方法无需预设函数集，通过LLMs和KANs实现了灵活且高效的符号回归，为复杂函数建模提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通过增加推理链长度提升准确性，但导致冗余和高计算成本，尤其是小型模型。

Method: 1. 温度缩放（TS）控制推理停止点；2. TLDR强化学习方法（基于GRPO）实现多级推理长度控制。

Result: 实验显示TS优于预算强制方法，TLDR提升50%的标记效率且准确性损失极小。

Conclusion: 研究揭示了停止时间控制的重要性，并提供了针对小型模型的高效算法解决方案。
...
Method: 系统评估匿名化技术对个体和群体公平性的影响，分析不同隐私设置和数据分布下的结果。
...
<details>
  <summary>Details</summary>
Motivation: 机器学习模型公平性验证通常需要公开模型参数，这威胁模型机密性。

Method: 通过零知识证明验证公平性，仅需模型参数和输入聚合信息，无需具体数据集；改进了逻辑回归和深度神经网络的公平性边界。

Result: FairZK显著快于现有方法，验证时间提升3.1x--1789x，首次支持4700万参数大模型。

Conclusion: FairZK为保护模型机密性的公平性验证提供了高效解决方案。
...
Result: 实验表明，该方法在标准架构、数据集和对抗攻击下，压缩率超94%，且对抗精度恢复或优于未压缩基线。

Conclusion: 该方法模型和数据无关，计算高效，支持自适应压缩，显著提升对抗鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 传统联邦学习依赖中心服务器，存在单点故障风险，而去中心化联邦学习（DFL）解决了这一问题，但在资源受限设备上部署面临挑战。

Method: 设计并部署了一个基于NEBULA平台的物理测试平台，扩展了功耗监测模块，用于测量训练过程中的能耗。

Result: 实验表明，通信拓扑结构对模型性能有显著影响，拓扑越密集，DFL性能越好。

Conclusion: DFL在资源受限设备上具有实际应用潜力，通信拓扑结构是优化性能的关键因素。
...
Result: 验证了潜在特征对输出的不等贡献，高因果影响的特征对模型操控更有效。

Conclusion: GradSAE为稀疏自编码器的分析和操控提供了更有效的方法。
...
<details>
  <summary>Details</summary>
Motivation: 隐私和监管限制阻碍了集中式机器学习解决方案，尤其是在医疗保健等领域。现有联邦学习框架主要支持基于梯度的模型，缺乏对树状模型的支持。

Method: 利用PySyft进行安全计算，支持加权模型平均、增量学习和本地评估，以在分布式环境中训练随机森林模型。

Result: 实验表明，该框架在保持预测准确性的同时满足隐私要求，与集中式方法相比差距不超过9%。

Conclusion: 该研究填补了联邦学习库的空白，为需要透明性和可靠性能的分布式机器学习任务提供了可行工具。
...
<details>
  <summary>Details</summary>
Motivation: 高维数据通常位于低维非线性流形上，学习黎曼几何可以提升聚类、降维等任务的性能。然而，多模态数据中的失真和建模误差仍需解决。

Method: 通过等距化学习的黎曼结构和平衡参数化的规律性与表达能力，解决多模态数据中的问题。

Result: 在合成和真实数据的实验中验证了方法的有效性。

Conclusion: 提出的方法在多模态数据中提升了性能，为非线性数据分析和可解释机器学习提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 研究机器遗忘方法的实际效果，发现现有方法无法完全消除遗忘集的影响，从而提出更严格的定义。

Method: 通过区分算法（基于成员推理分数和Kullback-Leibler散度）比较遗忘模型与镜像模型的差异，并提出计算性遗忘的形式化定义。

Result: 证明现有机器遗忘方法无法满足计算性遗忘定义，且基于差分隐私的方法虽可行但会导致效用崩溃。

Conclusion: 计算性遗忘为机器遗忘提供了理论框架，但现有方法仍需改进，未来研究需解决开放性问题。
...
<details>
  <summary>Details</summary>
Motivation: 正则化在神经网络训练中广泛应用，但缺乏与可解释学习的理论联系。本文旨在通过高阶正则化方法填补这一空白。

Method: 提出高阶正则化（HR）方法，将其视为逆映射的近似，并给出误差上下界。理论证明HR可作为收缩操作，并适用于任意映射矩阵的神经网络。

Result: HR方法在经典强化学习控制问题中表现出优越性能，显著提升了神经网络的泛化能力。

Conclusion: HR方法不仅理论上有意义，还能通过案例研究和增量解决方案验证其实际应用价值，推动了可解释学习的发展。
...
<details>
  <summary>Details</summary>
Motivation: 随着现代设计复杂度的增加，LLMs在优化CAD流程中的潜力巨大，但目前缺乏相关综述研究。

Method: 文章首先概述CAD的工业重要性及LLMs的基础，随后分类探讨LLMs在CAD中的六大应用领域。

Result: 提出了LLMs在CAD中的多种应用，并总结了其对未来技术发展的潜在影响。

Conclusion: 文章指出了未来研究方向，为LLMs与CAD结合的创新提供了广阔机会。
...
<details>
  <summary>Details</summary>
Motivation: 法律框架要求数据删除权，机器去学习成为解决方案。现有研究集中在简化场景，需更全面的评估平台。

Method: MUBox平台整合23种去学习方法，测试六种场景和11种指标，支持比较研究和评估。

Result: 发现现有方法在不同场景下效果不一致，评估需多指标，去毒方法效果因攻击类型而异。

Conclusion: MUBox为去学习方法提供了统一评估框架，强调多指标和广泛场景的必要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有分层联邦学习模型通常限于两层聚合，难以适应复杂大规模网络的需求。

Method: 提出QMLHFL框架，结合嵌套聚合和分层量化，分析其收敛性并优化层内迭代次数。

Result: QMLHFL在高数据异构性下仍保持高学习精度，优化后性能显著提升。

Conclusion: QMLHFL为复杂网络提供了灵活且高效的分层联邦学习解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度点预测模型的置信区间建模方法存在成本高、未能充分利用深度模型优势或缺乏理论保证的问题。

Method: 利用预训练点预测模型提取特征，拟合残差预测器并构建置信区间，结合自适应覆盖控制机制。

Result: 在12个数据集上的实验表明，该方法能提供更紧密的置信区间，同时保持所需的覆盖率。

Conclusion: 该方法在理论上实现了渐近覆盖收敛，实际应用中表现优越，代码和模型已开源。
...
<details>
  <summary>Details</summary>
Motivation: 现有离线安全强化学习方法仅关注短期安全，忽视长期安全，且对OOD状态和动作处理不足。

Method: 使用H-J可达性分析生成安全标签，结合CVAE和安全分类器；采用悲观估计方法处理OOD动作。

Result: FASP在DSRL基准测试中表现优异，尤其在安全性上超越现有算法。

Conclusion: FASP为离线安全强化学习提供了高效、安全的解决方案，具有理论和实验验证。
...
<details>
  <summary>Details</summary>
Motivation: 强化学习在动态环境中泛化能力不足，现有方法中RL与基础模型（如LLM/VLM）的协调性较差，导致决策不合理和效率瓶颈。

Method: 受卡尼曼的快慢思维理论启发，设计了DSADF框架，包含快速直觉决策的RL模块（System 1）和深度推理的VLM模块（System 2）。

Result: 在Crafter和Housekeep游戏环境中验证了DSADF的有效性，显著提升了已知和未知任务的决策能力。

Conclusion: DSADF通过结合RL的快速响应和VLM的深度推理，实现了高效且自适应的决策，解决了现有方法的协调问题。
...
<details>
  <summary>Details</summary>
Motivation: 解决长时序预测中多粒度信息利用不足、通道特性忽视以及趋势和季节性成分独特性的问题。

Method: 采用MLP框架，动态整合多尺度预测，并独立建模趋势与季节性成分。

Result: 在八个基准测试中，MDMixer的MAE性能比TimeMixer提升4.64%，同时兼顾训练效率和模型可解释性。

Conclusion: MDMixer在长时序预测中表现出色，为多尺度信息整合和成分独立建模提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 在二元分类中，许多场景下训练数据仅包含正样本，其余数据未标记，传统方法难以处理。本文旨在利用样本间的相似性，提出一种高效解决方案。

Method: 2-HNC方法分两阶段：第一阶段基于Hochbaum归一化割（HNC）生成未标记样本的负样本排序；第二阶段通过加入可能负样本重新分类，最终选择最接近先验估计的分区。

Result: 实验表明，2-HNC在合成和真实数据集上表现优异，常超越现有最优算法。

Conclusion: 2-HNC通过两阶段分类和嵌套分区策略，有效解决了PU学习问题，具有广泛适用性和高性能。
...
Method: 使用深度混合密度网络构建高斯混合模型，基于负对数似然定义异常评分函数，捕捉行为数据的多模态分布特征。

Result: 在UNSW-NB15数据集上的实验表明，该方法在性能（如Accuracy、F1-score、AUC）和训练稳定性上优于多种先进神经网络架构。

Conclusion: 该方法为用户行为建模和异常检测提供了更具表达力和判别力的解决方案，推动了深度概率建模技术在网络安全和智能风控领域的应用。
...
<details>
  <summary>Details</summary>
Motivation: 全局SVD在压缩数据矩阵时忽略局部变化，导致细节丢失。为了解决这一问题，论文提出自适应LoRMA方法。

Method: 将数据矩阵分块为重叠补丁，用k-means聚类相似补丁，并在每个簇内进行SVD。分析了补丁大小对压缩效率和计算成本的影响。

Result: 自适应LoRMA在MRI、超声、CT和X射线中优于全局SVD，显著保留结构细节和诊断相关性，PSNR、SSIM、IoU、EPI更高，MSE更低。

Conclusion: 自适应LoRMA在存储效率和诊断保真度上表现优异，尽管计算时间较长，但适用于高压缩需求场景。
...
<details>
  <summary>Details</summary>
Motivation: 探讨深度神经网络在特定噪声条件下的分类性能，尤其是在硬边界条件下的表现。

Method: 使用平方损失替代和ℓp惩罚的最小化经验风险方法，分析DNN在Tsybakov低噪声条件下的表现。

Result: 在回归函数足够平滑的情况下，DNN可以实现任意大的有限样本超额风险界。

Conclusion: 论文提出了一种新的超额风险分解方法，为深度神经网络在硬边界条件下的分类性能提供了理论支持。
...
### [51] [Decoupled Multimodal Prototypes for Visual Recognition with Missing Modalities](https://arxiv.org/abs/2505.08283)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法假设所有模态都可用，而现实中常出现模态缺失，导致性能下降。

Method: 利用缺失感知的类原型，动态适应不同缺失模态场景，并与现有提示方法结合。

Result: 实验表明，该方法在多种缺失模态场景和缺失率下显著提升性能。

Conclusion: 解耦原型输出头有效解决了多模态学习中的模态缺失问题。
...
### [52] [A Practical Introduction to Deep Reinforcement Learning](https://arxiv.org/abs/2505.08295)
*Yinghan Sun,Hongxi Wang,Hua Chen,Wei Zhang*

Main category: cs.LG

TLDR: 本文是一篇关于深度强化学习（DRL）的教程，重点介绍了近端策略优化（PPO）算法，旨在为初学者提供直观、实用的入门指南。


<details>
  <summary>Details</summary>
Motivation: 深度强化学习在多个领域取得了显著成功，但其算法多样性和理论复杂性对初学者构成了挑战。本文旨在通过简洁、直观的方式帮助读者快速入门。

Method: 教程将算法统一在广义策略迭代（GPI）框架下，强调直观解释、示例和工程技巧，而非冗长的理论证明。

Result: 读者能够从基础概念快速过渡到高级DRL算法的实现。

Conclusion: 本文为初学者提供了高效、易理解的DRL学习路径，特别适合希望快速掌握PPO算法的读者。
...
### [53] [Efficient Unstructured Pruning of Mamba State-Space Models for Resource-Constrained Environments](https://arxiv.org/abs/2505.08299)
*Ibne Farabi Shihab,Sanjeda Akter,Anuj Sharma*

Main category: cs.LG

TLDR: 提出了一种针对Mamba模型的无结构化剪枝框架，参数减少70%的同时保持95%的性能。
...
Conclusion: 该框架揭示了Mamba的冗余性和鲁棒性，提升了其在实际应用中的可行性。
...
### [54] [Rapid Overfitting of Multi-Pass Stochastic Gradient Descent in Stochastic Convex Optimization](https://arxiv.org/abs/2505.08306)
*Shira Vansover-Hager,Tomer Koren,Roi Livni*

Main category: cs.LG

TLDR: 多轮随机梯度下降（SGD）在样本外性能上的表现研究表明，仅几轮SGD就会显著损害性能并导致过拟合，尤其是在非光滑情况下。


<details>
  <summary>Details</summary>
Motivation: 研究多轮SGD在随机凸优化（SCO）模型中的样本外性能，以填补现有对多轮SGD理解的空白。

Method: 分析多轮SGD的步长选择及其对样本外性能的影响，比较光滑与非光滑情况下的过拟合速率。

Result: 发现多轮SGD在非光滑情况下会导致显著的过拟合，样本外损失可达Ω(1)。

Conclusion: 揭示了SGD在第一轮后的相变行为，并证明了光滑与非光滑情况下过拟合速率的显著差异。
...
### [55] [SpecSphere: Dual-Pass Spectral-Spatial Graph Neural Networks with Certified Robustness](https://arxiv.org/abs/2505.08320)
*Yoonhyuk Choi,Chong-Kwon Kim*

Main category: cs.LG

TLDR: SpecSphere是一种双通道谱空间GNN，能适应全同质性-异质性谱，超越1-Weisfeiler-Lehman的表达能力，同时保持线性时间复杂度。


<details>
  <summary>Details</summary>
Motivation: 解决现有GNN在同质性-异质性谱上的适应性不足、表达能力有限以及缺乏可证明鲁棒性的问题。

Method: 结合Chebyshev多项式谱分支和注意力门控空间分支，通过轻量级MLP在合作-对抗极小极大游戏中融合表示。

Result: 在节点分类任务中达到最先进准确率，并提供更严格的鲁棒性保证。
...
### [56] [FedRS-Bench: Realistic Federated Learning Datasets and Benchmarks in Remote Sensing](https://arxiv.org/abs/2505.08325)
*Haodong Zhao,Peng Peng,Chiyu Chen,Linqing Huang,Gongshen Liu*

Main category: cs.LG

TLDR: 提出一个名为FedRS的真实联邦遥感数据集，包含8个数据集和135个客户端，用于解决现有联邦学习在遥感领域缺乏真实数据的问题，并构建了FedRS-Bench基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习在遥感领域缺乏真实数据集和基准测试，导致实验设置不一致且无法公平比较。

Method: 构建FedRS数据集，包含多样化的传感器和分辨率数据，模拟真实联邦特性（如标签分布不均、客户端数据量不平衡等），并实现10种基线联邦学习算法。

Result: 实验表明联邦学习能显著提升模型性能，同时揭示了不同方法在客户端异质性和可用性条件下的性能权衡。

Conclusion: FedRS-Bench为大规模、真实的联邦学习研究提供了标准化测试平台，有望加速相关研究。
...
### [57] [Low-Complexity Inference in Continual Learning via Compressed Knowledge Transfer](https://arxiv.org/abs/2505.08327)
*Zhenrong Liu,Janne M. J. Huttunen,Mikko Honkala*

Main category: cs.LG

TLDR: 论文探讨了持续学习（CL）中模型压缩技术（如剪枝和知识蒸馏）的应用，提出了两种高效框架以解决类增量学习（CIL）中的计算成本问题。


<details>
  <summary>Details</summary>
Motivation: 尽管大型预训练模型在持续学习中表现优异，但其高计算成本限制了实际应用，尤其是在需要低延迟或高能效的场景。

Method: 提出了两种框架：基于剪枝的框架（包括预剪枝和后剪枝策略）和基于知识蒸馏的框架（采用教师-学生架构）。

Result: 实验表明，这两种框架在准确性和推理复杂度之间取得了更好的平衡，并显著优于基线方法。
...
### [58] [Reinforcement Learning (RL) Meets Urban Climate Modeling: Investigating the Efficacy and Impacts of RL-Based HVAC Control](https://arxiv.org/abs/2505.07045)
...
TLDR: 该研究提出了一种结合强化学习（RL）与城市气候模型的框架，评估了RL HVAC控制在不同气候下的效果及其对室内和城市气候的影响。


<details>
  <summary>Details</summary>
Motivation: 探索RL HVAC控制在减少建筑能耗的同时保持室内热舒适性的潜力，并研究其在不同气候背景下的适用性和影响。

Method: 结合RL与城市气候模型，包括建筑能耗模型，评估RL策略在不同城市气候下的效果、对室内和城市气候的影响及策略的可迁移性。

Result: RL策略的效果和对气候的影响因城市背景气候而异，热气候城市在能耗与舒适性平衡方面表现更好，且气候波动大的城市策略可迁移性更强。

Conclusion: 强调了在不同气候背景下全面评估RL HVAC控制策略的重要性，并提出了城市间学习可能有助于RL HVAC控制的部署。
...
### [59] [Structural-Temporal Coupling Anomaly Detection with Dynamic Graph Transformer](https://arxiv.org/abs/2505.08330)
*Chang Zong,Yueting Zhuang,Jian Shao,Weiming Lu*

Main category: cs.LG

TLDR: 提出了一种基于动态图变换器的结构-时间耦合异常检测架构，通过二维位置编码增强，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 动态图中异常边检测的挑战在于缺乏结构-时间耦合信息，现有方法未能深入结合这两种特征。

Method: 结合结构与时序特征，采用动态图变换器和二维位置编码捕捉异常感知的图演化模式。
...
### [60] [SHAP-based Explanations are Sensitive to Feature Representation](https://arxiv.org/abs/2505.08345)
*Hyunseung Hwang,Andrew Bell,Joao Fonseca,Venetia Pliatsika,Julia Stoyanovich,Steven Euijong Whang*

Main category: cs.LG

TLDR: 本文探讨了数据工程选择对局部特征解释的影响，发现常见的数据工程技术（如年龄直方图或种族编码）可以操纵SHAP等方法得出的特征重要性，甚至被攻击者利用来掩盖歧视等问题。


<details>
  <summary>Details</summary>
Motivation: 研究数据工程技术如何影响局部特征解释，揭示其潜在被滥用的风险。

Method: 通过实验展示常见数据工程技术（如年龄直方图或种族编码）对SHAP等解释方法的影响。

Result: 发现数据工程技术可以显著改变特征重要性，甚至被用于掩盖歧视等负面问题。

Conclusion: 这是首次研究表明，标准数据工程技术可能误导解释器，需引起重视。
...
### [61] [Localization of Impacts on Thin-Walled Structures by Recurrent Neural Networks: End-to-end Learning from Real-World Data](https://arxiv.org/abs/2505.08362)
*Alexander Humer,Lukas Grasboeck,Ayech Benjeddou*

Main category: cs.LG

TLDR: 论文提出了一种基于门控循环单元（GRU）的神经网络方法，用于从传感器数据中直接定位薄壁结构上的冲击位置，实验数据支持了其高精度。
...
Result: 即使数据集较小，该方法仍能实现高精度的冲击位置估计。

Conclusion: 基于物理数据的GRU方法在冲击定位中表现出色，为SHM提供了有效解决方案。
...
### [62] [Density Ratio-based Causal Discovery from Bivariate Continuous-Discrete Data](https://arxiv.org/abs/2505.08371)
*Takashi Nicholas Maeda,Shohei Shimizu,Hidetoshi Matsui*

Main category: cs.LG

TLDR: 本文提出了一种针对混合二元数据（一个连续变量和一个离散变量）的因果发现方法，通过分析条件密度比的单调性确定因果方向。


<details>
  <summary>Details</summary>
Motivation: 现有方法在二元数据中效果不佳，约束方法依赖条件独立性测试，而评分方法要么假设强分布条件，要么难以公平比较不同类型变量的因果方向。

Method: 提出新方法，通过分析连续变量在不同离散变量值下的条件密度比的单调性来确定因果方向。

Result: 理论分析表明，当连续变量导致离散变量时，条件密度比具有单调性，反之则无。实验证明该方法优于现有方法。

Conclusion: 该方法无需强分布假设，避免了信息内容差异带来的偏差，适用于不同类型变量的因果发现。
...
### [63] [ConDiSim: Conditional Diffusion Models for Simulation Based Inference](https://arxiv.org/abs/2505.08403)
*Mayank Nautiyal,Andreas Hellander,Prashant Singh*

Main category: cs.LG

TLDR: ConDiSim是一种基于条件扩散模型的仿真推断方法，用于复杂系统的后验分布近似，具有高效性和稳定性。


<details>
  <summary>Details</summary>
Motivation: 解决复杂系统中难以处理的似然函数的后验推断问题。

Method: 利用去噪扩散概率模型，通过前向过程添加高斯噪声，反向过程学习去噪，并结合观测数据。

Result: 在十个基准问题和两个实际测试问题中表现出色，能够准确近似后验分布，同时保持计算效率和训练稳定性。

Conclusion: ConDiSim为仿真推断提供了一个强大且可扩展的框架，特别适合需要快速推断方法的参数推断工作流程。
...
### [64] [Optimizing Retrieval-Augmented Generation: Analysis of Hyperparameter Impact on Performance and Efficiency](https://arxiv.org/abs/2505.08445)
*Adel Ammar,Anis Koubaa,Omer Nacar,Wadii Boulila*

Main category: cs.LG

TLDR: 论文分析了检索增强生成（RAG）系统中超参数对速度和性能的影响，揭示了速度与准确性的权衡，并展示了优化配置如何显著提升检索质量。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型可能产生幻觉或依赖过时知识的问题，通过检索增强生成（RAG）结合外部搜索提升任务表现。

Method: 评估不同超参数（如向量存储、分块策略、重排序和温度）对RAG系统的影响，使用六种指标衡量性能。

Result: Chroma查询速度更快，Faiss检索精度更高；固定长度分块表现最佳；重排序提升质量但增加运行时。优化配置可实现99%的上下文精度。

Conclusion: RAG系统通过合理配置超参数可在计算成本和准确性之间取得平衡，显著提升检索质量，适用于对检索精度要求高的应用场景。
...
### [65] [An adaptive sampling algorithm for data-generation to build a data-manifold for physical problem surrogate modeling](https://arxiv.org/abs/2505.08487)
*Chetra Mang,Axel TahmasebiMoradi,David Danan,Mouadh Yagoubi*

Main category: cs.LG

TLDR: 论文提出了一种自适应采样算法（ASADG），用于生成更代表性的输入数据，以解决传统方法在数据不平衡时难以准确建模的问题。


<details>
  <summary>Details</summary>
Motivation: 传统物理模型（如偏微分方程）计算成本高，且数据不平衡会导致模型学习效果差，因此需要一种更有效的数据生成方法。

Method: 提出ASADG算法，通过迭代添加输入数据（基于单纯复形重心）来改善响应流形的表示。

Result: ASADG在生成相同数量数据时，比LHS方法更能准确表示响应流形。

Conclusion: ASADG算法能有效提升数据生成的代表性，适用于构建更准确的代理模型。
...
### [66] [Isolation Forest in Novelty Detection Scenario](https://arxiv.org/abs/2505.08489)
*Adam Ulrich,Jan Krňávek,Roman Šenkeřík,Zuzana Komínková Oplatková,Radek Vala*

Main category: cs.LG

TLDR: 本文提出了一种基于半空间树（HST）算法的改进方法，专门用于新颖性检测任务，通过理论分析和实验验证其高效性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 经典的新颖性检测算法（如One-Class SVM或LOF）缺乏可解释性和可扩展性，因此需要一种更高效且易于理解的替代方法。

Method: 通过修改HST算法，利用新颖性点倾向于出现在树的较高叶子节点这一特性，结合概率分析、期望深度计算和组合推理进行理论验证。

Result: 改进后的HST算法在期望深度分析中表现优于原始隔离森林算法，新颖性点更加孤立，验证了其有效性。

Conclusion: 改进的HST算法为新颖性检测提供了理论基础，兼具高效性和可解释性，为后续应用和实验奠定了基础。
...
### [67] [A new methodology to decompose a parametric domain using reduced order data manifold in machine learning](https://arxiv.org/abs/2505.08497)
*Chetra Mang,Axel TahmasebiMoradi,Mouadh Yagoubi*

Main category: cs.LG

TLDR: 提出了一种基于迭代主成分分析的参数域分解新方法，通过降维和重构投影器实现高效分解，并通过数值实验验证其优于传统元模型。


<details>
  <summary>Details</summary>
Motivation: 传统元模型（如神经网络）在处理高维参数域时效率不足，需要一种更高效的方法。

Method: 采用迭代主成分分析降维，开发两种逆投影器重构方法，并基于低维流形分解参数域。

Result: 数值实验表明，该方法在谐波传输问题中比传统元模型更高效有效。

Conclusion: 新方法在参数域分解中表现出优越性，为高维问题提供了更高效的解决方案。
...
### [68] [InfoPO: On Mutual Information Maximization for Large Language Model Alignment](https://arxiv.org/abs/2505.08507)
*Teng Xiao,Zhen Ge,Sujay Sanghavi,Tian Wang,Julian Katz-Samuels,Marc Versage,Qingjun Cui,Trishul Chilimbi*

Main category: cs.LG

TLDR: 论文提出了一种名为InfoPO的新方法，用于优化大型语言模型（LLMs）的后训练，通过偏好数据对齐模型，避免了对Bradley-Terry模型的依赖，提升了推理任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖Bradley-Terry模型，容易过拟合且在推理任务上表现不佳。

Method: 提出InfoPO算法，无需依赖Bradley-Terry模型，避免选中回答概率下降。

Result: 实验表明InfoPO在开放基准测试中表现优于基线方法，尤其在推理任务上。

Conclusion: InfoPO是一种高效且有效的偏好微调算法，能够显著提升语言模型的对齐性能。
...
### [69] [Learning Advanced Self-Attention for Linear Transformers in the Singular Value Domain](https://arxiv.org/abs/2505.08516)
*Hyowon Wi,Jeongwhan Choi,Noseong Park*

Main category: cs.LG

TLDR: 论文提出了一种名为AGF的新方法，将自注意力解释为图信号处理中的图滤波器，解决了现有自注意力机制的低通滤波限制，并在多项任务中取得最优性能。
...
Method: 提出AGF方法，将自注意力视为图信号处理中的图滤波器，并在奇异值域中学习滤波器，复杂度为线性。

Result: AGF在Long Range Arena基准测试和时间序列分类等任务中达到最优性能。
...
### [70] [GradMix: Gradient-based Selective Mixup for Robust Data Augmentation in Class-Incremental Learning](https://arxiv.org/abs/2505.08528)
*Minsu Kim,Seong-Hyeon Hwang,Steven Euijong Whang*

Main category: cs.LG

TLDR: 论文提出GradMix，一种基于梯度的选择性数据增强方法，用于缓解持续学习中的灾难性遗忘问题，通过混合有益类别的样本对提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 持续学习中，新知识的获取与旧知识的保持是一大挑战。现有方法（如经验回放）通过存储部分旧任务数据训练，但随机混合样本可能损害旧任务知识并加剧灾难性遗忘。

Method: 提出GradMix方法，基于类别标准选择性混合样本对，仅混合有益类别对，避免有害类别对，以减少灾难性遗忘。

Result: 在多个真实数据集上的实验表明，GradMix通过最小化旧知识遗忘，在准确性上优于其他数据增强基线方法。

Conclusion: GradMix是一种有效的选择性数据增强方法，显著缓解了持续学习中的灾难性遗忘问题。
...
### [71] [ExEBench: Benchmarking Foundation Models on Extreme Earth Events](https://arxiv.org/abs/2505.08529)
*Shan Zhao,Zhitong Xiong,Jie Zhao,Xiao Xiang Zhu*

Main category: cs.LG

TLDR: 论文提出了ExEBench基准数据集，用于评估基础模型在极端事件管理中的可靠性和泛化能力，并促进灾害管理中的机器学习方法发展。


<details>
  <summary>Details</summary>
Motivation: 极端事件对人类和生态系统构成重大风险，而基础模型在灾害管理中表现出潜力，但其性能可能受训练数据偏差影响。

Method: 构建了包含七类极端事件的ExEBench数据集，涵盖全球范围和多源数据，并设计了与灾害管理相关的机器学习任务。

Result: ExEBench旨在评估基础模型的泛化能力，推动灾害管理中的新方法开发，并分析极端事件的交互效应。

Conclusion: ExEBench为极端事件研究和灾害管理提供了公开的数据和平台，有助于应对未来气候变化挑战。
...
### [72] [OLinear: A Linear Model for Time Series Forecasting in Orthogonally Transformed Domain](https://arxiv.org/abs/2505.08550)
*Wenzhen Yue,Yong Liu,Haoxuan Li,Hao Wang,Xianghua Ying,Ruohao Guo,Bowei Xing,Ji Shi*

Main category: cs.LG

TLDR: OLinear是一种基于线性变换的多变量时间序列预测模型，通过正交变换域操作提升性能。


<details>
  <summary>Details</summary>
Motivation: 传统时间域预测模型因依赖关系复杂而性能受限，OLinear通过数据自适应正交变换解决这一问题。

Method: 使用OrthoTrans正交变换和NormLin线性层，在解耦特征域中进行编码和解码。

Result: 在24个基准和140个任务中表现优异，NormLin模块显著提升Transformer性能。

Conclusion: OLinear高效且性能优越，可作为插件增强现有预测模型。
...
### [73] [Online Learning and Unlearning](https://arxiv.org/abs/2505.08557)
*Yaxi Hu,Bernhard Schölkopf,Amartya Sanyal*

Main category: cs.LG

TLDR: 论文提出了在线学习-反学习问题，并基于在线梯度下降（OGD）设计了两种算法，分别是被动OLU和主动OLU，能够在保证反学习需求的同时保持与标准OGD相当的遗憾界。


<details>
  <summary>Details</summary>
Motivation: 研究在线学习场景中如何动态处理数据删除请求，确保模型在删除数据点后的输出与从未学习该数据点的模型统计不可区分。

Method: 提出两种算法：被动OLU（利用OGD的收缩性并注入噪声）和主动OLU（结合离线反学习算法调整模型）。

Result: 在凸性和平滑性假设下，两种算法均实现了与标准OGD相当的遗憾界。

Conclusion: 证明了在提供反学习保证的同时，仍能保持竞争性的学习性能。
...
### [74] [Clustering of Incomplete Data via a Bipartite Graph Structure](https://arxiv.org/abs/2505.08594)
*Amirhossein Javaheri,Daniel P. Palomar*

Main category: cs.LG

TLDR: 提出了一种基于二分图模型的聚类方法，解决了中心节点数据缺失和重尾数据建模的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有二分图模型需要中心节点数据，且高斯模型对重尾数据效果不佳。

Method: 设计了一种无需中心节点数据的二分图聚类方法，适用于重尾数据。

Result: 数值实验验证了该方法在金融数据聚类中的有效性。

Conclusion: 该方法解决了数据不完整和重尾分布问题，适用于实际场景。
...
### [75] [Cost Function Estimation Using Inverse Reinforcement Learning with Minimal Observations](https://arxiv.org/abs/2505.08619)
*Sarmad Mehrdad,Avadesh Meduri,Ludovic Righetti*

Main category: cs.LG

TLDR: 提出一种迭代逆强化学习算法，用于推断连续空间中的最优成本函数，基于最大熵准则，通过调整步长和优化观察效果，实现快速学习。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要大量样本且效率不高，因此提出一种更高效的方法来推断最优成本函数。

Method: 基于最大熵准则，迭代优化权重步长，并通过解决最优控制问题生成样本轨迹。

Result: 在模拟环境中，该方法优于两种现有算法，表现出更高的学习效率和性能。

Conclusion: 该算法在连续空间中推断最优成本函数时更高效，且对样本需求较低。
...
### [76] [Credit Assignment and Efficient Exploration based on Influence Scope in Multi-agent Reinforcement Learning](https://arxiv.org/abs/2505.08630)
*Shuai Han,Mehdi Dastani,Shihan Wang*

Main category: cs.LG

TLDR: 提出了一种新方法ISA，用于解决稀疏奖励多智能体强化学习中的信用分配和探索问题，显著优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 稀疏奖励场景下，多智能体强化学习难以实现精确信用分配和有效探索，现有方法表现不佳。

Method: 通过计算智能体对状态维度/属性的影响范围（ISA），利用动作与状态属性的相互依赖关系进行信用分配和探索空间限制。

Result: 在多种稀疏奖励多智能体场景中，ISA方法显著优于现有基线。

Conclusion: ISA方法有效解决了稀疏奖励多智能体强化学习中的信用分配和探索问题，性能优越。
...
### [77] [Modular Federated Learning: A Meta-Framework Perspective](https://arxiv.org/abs/2505.08646)
*Frederico Vicente,Cláudia Soares,Dušan Jakovetić*

Main category: cs.LG

TLDR: 该论文提出了一种联邦学习（FL）的元框架视角，将其分解为模块化组件，并强调了聚合与对齐的双重作用，为FL研究和部署提供了全面且灵活的基础。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在保护隐私的同时实现分布式机器学习训练，但其复杂性需要系统化的理解。论文旨在通过元框架视角，结构化地分析FL的方法、挑战和应用。

Method: 论文采用元框架方法，将FL分解为通信、优化、安全和隐私等模块化组件，并提出新的分类法区分聚合与对齐。

Result: 论文提出了FL的元框架，明确了聚合与对齐的核心作用，并探索了Python中的FL框架以实现实际应用。

Conclusion: 通过模块化组件和聚合与对齐的双重作用，该论文为FL的研究和部署提供了全面的理论基础和实践指导。
...
### [78] [AC-PKAN: Attention-Enhanced and Chebyshev Polynomial-Based Physics-Informed Kolmogorov-Arnold Networks](https://arxiv.org/abs/2505.08687)
*Hangwei Zhang,Zhimu Huang,Yan Wang*

Main category: cs.LG

TLDR: 论文提出了一种改进的Chebyshev1KANs架构AC-PKAN，通过引入小波激活的MLP和内外注意力机制，解决了原方法的秩崩溃问题，并在多个基准任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 原始KANs和Chebyshev1KANs在计算和表达能力上存在不足，尤其是秩崩溃问题限制了其性能。

Method: 结合小波激活的MLP、内部注意力机制和外部残差梯度注意力（RGA）机制，设计AC-PKAN架构。

Result: AC-PKAN在多个基准任务中优于或匹配现有最优模型（如PINNsFormer）。

Conclusion: AC-PKAN显著提升了KANs的表达能力，适用于零数据或数据稀疏的复杂工程问题。
...
### [79] [PWC-MoE: Privacy-Aware Wireless Collaborative Mixture of Experts](https://arxiv.org/abs/2505.08719)
*Yang Su,Na Yan,Yansha Deng,Robert Schober*

Main category: cs.LG

TLDR: 提出PWC-MoE框架，平衡计算成本、性能和隐私保护，通过动态路由敏感与非敏感令牌，结合负载均衡和带宽自适应方案。


<details>
  <summary>Details</summary>
Motivation: 解决云服务器上大语言模型（LLMs）的隐私和带宽问题，同时避免本地小语言模型（SLMs）性能不足。

Method: 使用稀疏隐私感知门控网络动态路由令牌，引入负载均衡机制和带宽自适应令牌卸载方案。

Result: 实验证明PWC-MoE在带宽受限环境下有效保护隐私并保持高性能。
...
### [80] [Memorization-Compression Cycles Improve Generalization](https://arxiv.org/abs/2505.08727)
*Fangyuan Yu*

Main category: cs.LG

TLDR: 论文提出了一种信息瓶颈语言建模（IBLM）目标，通过压缩内部表示提升泛化能力，并设计了GAPT训练算法，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 探索数据扩展和内部表示压缩对泛化能力的理论提升，并模拟生物学习中的记忆-压缩循环。

Method: 引入IBLM目标，将语言建模重构为约束优化问题；提出GAPT算法，动态切换记忆与压缩阶段。

Result: GAPT在GPT-2预训练中降低MBE 50%，提升交叉熵4.8%，OOD泛化提升35%，减少灾难性遗忘干扰97%。

Conclusion: GAPT通过模拟生物学习机制，显著提升模型泛化能力和抗干扰性。
...
### [81] [Preference Optimization for Combinatorial Optimization Problems](https://arxiv.org/abs/2505.08735)
*Mingjun Pan,Guanquan Lin,You-Wei Luo,Bin Zhu,Zhien Dai,Lijun Sun,Chun Yuan*

Main category: cs.LG

TLDR: 提出了一种基于偏好优化的强化学习方法，通过将定量奖励信号转化为定性偏好信号，显著提升了组合优化问题的求解效率和效果。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法在组合优化中存在奖励信号衰减和探索效率低的问题，导致求解效率不高。

Method: 通过统计比较建模将奖励信号转化为偏好信号，结合熵正则化目标直接对齐策略与偏好，并整合局部搜索技术生成高质量偏好对。

Result: 在TSP、CVRP和FFSP等基准测试中，该方法显著优于现有RL算法，收敛效率和求解质量均更优。

Conclusion: 偏好优化方法为组合优化问题提供了一种高效且高质量的解决方案，避免了传统RL方法的局限性。
...
### [82] [Towards Foundation Models for Experimental Readout Systems Combining Discrete and Continuous Data](https://arxiv.org/abs/2505.08736)
*James Giroux,Cristiano Fanelli*

Main category: cs.LG

TLDR: 提出了一种用于核物理的基础模型，能够处理未来电子离子对撞机中成像切伦科夫探测器的低层输入，解决了现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 解决现有基于下一令牌预测方法的分辨率损失和缺乏条件生成的问题。

Method: 提出三项创新：(i) 分离离散空间特征和连续变量的词汇表，通过因果多头交叉注意力结合；(ii) 通过前置上下文嵌入实现连续运动学条件；(iii) 高分辨率连续变量标记化，避免联合词汇膨胀。

Result: 模型能够快速、高保真地生成切伦科夫光子的像素和时间序列，并通过高性能DIRC的闭合测试验证。模型还可推广到重建任务，如π介子和K介子识别。

Conclusion: 该模型在核物理领域具有广泛的应用潜力，尤其在条件生成和重建任务中表现优异。
...
### [83] [Sensitivity-Constrained Fourier Neural Operators for Forward and Inverse Problems in Parametric Differential Equations](https://arxiv.org/abs/2505.08740)
*Abdolmehdi Behroozi,Chaopeng Shen and,Daniel Kifer*

Main category: cs.LG

TLDR: SC-FNO是一种基于敏感性的正则化策略，用于解决FNO在逆问题、敏感性估计和概念漂移中的局限性，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 解决FNO在逆问题、敏感性估计和概念漂移中的不足。

Method: 引入敏感性约束的Fourier神经算子（SC-FNO）。

Result: SC-FNO在预测解路径、参数反演任务中表现优异，且数据与训练需求减少。

Conclusion: SC-FNO在多种微分方程和神经算子中具有通用性，性能显著提升。
...
### [84] [Implet: A Post-hoc Subsequence Explainer for Time Series Models](https://arxiv.org/abs/2505.08748)
*Fanyu Meng,Ziwen Kan,Shahbaz Rezaei,Zhaodan Kong,Xin Chen,Xin Liu*

Main category: cs.LG

TLDR: Implet是一种新颖的后处理解释器，用于时间序列模型，提供准确且简洁的子序列级解释，并通过群体级框架进一步提升解释的简洁性和可解释性。
...
Method: 提出Implet，通过识别关键时间片段来生成子序列级解释，并引入群体级解释框架。
...
### [85] [SPAT: Sensitivity-based Multihead-attention Pruning on Time Series Forecasting Models](https://arxiv.org/abs/2505.08768)
*Suhan Guo,Jiahong Deng,Mengjun Yi,Furao Shen,Jian Zhao*

Main category: cs.LG

TLDR: SPAT是一种结构化剪枝方法，通过选择性移除冗余注意力机制，提升模型效率，降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 注意力架构在多变量时间序列预测中表现优异，但计算成本高，需减少其规模和延迟。

Method: 提出动态敏感性度量SEND，在预训练阶段评估每个注意力模块的重要性，并选择性移除冗余模块。

Result: SPAT剪枝模型在MSE、MAE和FLOPs上分别降低2.842%、1.996%和35.274%，性能优于现有轻量级方法。

Conclusion: SPAT通过保留最有效的注意力机制，显著提升模型效率和性能，适用于标准及零样本推理。
...
### [86] [Addressing the Current Challenges of Quantum Machine Learning through Multi-Chip Ensembles](https://arxiv.org/abs/2505.08782)
*Junghoon Justin Park,Jiook Cha,Samuel Yen-Chi Chen,Huan-Hsin Tseng,Shinjae Yoo*

Main category: cs.LG

TLDR: 提出了一种多芯片集成变分量子电路框架，以解决NISQ设备的噪声、可扩展性和训练性问题。


<details>
  <summary>Details</summary>
Motivation: 量子机器学习（QML）在实际部署中受限于NISQ设备的噪声、可扩展性和变分量子电路（VQC）的训练性问题。

Method: 通过将高维计算分配到多个小型量子芯片上，增强可扩展性、训练性和噪声鲁棒性。

Result: 实验验证表明，该方法缓解了贫瘠高原问题，降低了量子误差偏差和方差，并在标准数据集和真实数据集上表现出良好的泛化能力。

Conclusion: 该框架为近期量子硬件上的可扩展QML提供了可行的解决方案。
...
### [87] [CodePDE: An Inference Framework for LLM-driven PDE Solver Generation](https://arxiv.org/abs/2505.08783)
*Shanda Li,Tanya Marwah,Junhong Shen,Weiwei Sun,Andrej Risteski,Yiming Yang,Ameet Talwalkar*

Main category: cs.LG

TLDR: CodePDE利用大型语言模型（LLM）生成PDE求解器，无需特定任务调优，在多个PDE问题上表现优异。
...
Result: CodePDE在多个代表性PDE问题上表现超人类水平，并分析了生成求解器的准确性、效率和数值方案选择。
...
### [88] [Polysemy of Synthetic Neurons Towards a New Type of Explanatory Categorical Vector Spaces](https://arxiv.org/abs/2505.07831)
...
TLDR: 论文提出了一种几何方法，将神经元定义为具有非正交基的分类向量空间，通过层内注意力机制优化语言模型效率。


<details>
  <summary>Details</summary>
Motivation: 探索人工神经网络中多义神经元的本质，提出更高效的分类方法。

Method: 将神经元定义为分类向量空间，提取前一层神经元的分类子维度，通过注意力机制识别关键分类区域。

Result: 发现更均匀且位于分类子维度交叉点的关键区域，提升语言模型效率。

Conclusion: 几何定义的分类向量空间和注意力机制能更高效地处理多义神经元问题。
...
### [89] [A Tale of Two Identities: An Ethical Audit of Human and AI-Crafted Personas](https://arxiv.org/abs/2505.07850)
*Pranav Narayanan Venkit,Jiayi Li,Yingfan Zhou,Sarah Rajtmajer,Shomir Wilson*

Main category: cs.CL

TLDR: 论文研究了LLM生成的合成人物在种族身份表征上的问题，揭示了算法他者化现象，并提出了改进建议。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在数据有限领域（如健康、隐私和HCI）中生成合成人物的应用增加，需了解其对少数群体身份的表征方式。

Method: 采用混合方法（细读、词汇分析和参数化创造力框架）比较1512个LLM生成人物与人类创作内容。

Result: LLM过度强调种族标记，使用文化编码语言，导致刻板印象、异域化等社会技术危害。

Conclusion: 提出叙事感知评估指标和社区中心验证协议，以减少算法他者化现象。
...
### [90] [Joint Detection of Fraud and Concept Drift inOnline Conversations with LLM-Assisted Judgment](https://arxiv.org/abs/2505.07852)
*Ali Senol,Garima Agrawal,Huan Liu*

Main category: cs.CL

TLDR: 提出了一种两阶段检测框架，用于识别数字通信平台中的虚假互动，结合了集成分类模型和概念漂移分析，提高了检测的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 数字通信平台中的虚假互动问题尚未得到充分解决，传统静态检测方法无法适应动态对话变化，容易误判。

Method: 采用两阶段框架：1) 使用集成分类模型识别可疑对话；2) 通过概念漂移分析（OCDD）和大型语言模型（LLM）判断是否为欺诈行为。

Result: 在社交工程聊天数据集上验证，框架显著提高了实时欺诈检测的准确性和可解释性。

Conclusion: 提出的模块化方法优于双LLM基线，能更可靠地检测虚假互动。
...
### [91] [CrashSage: A Large Language Model-Centered Framework for Contextual and Interpretable Traffic Crash Analysis](https://arxiv.org/abs/2505.07853)
*Hao Zhen,Jidong J. Yang*

Main category: cs.CL

TLDR: CrashSage是一个基于大型语言模型（LLM）的框架，通过将原始交通事故数据转换为结构化文本、数据增强、模型微调和可解释性技术，显著提升了事故分析的性能。


<details>
  <summary>Details</summary>
Motivation: 全球每年因交通事故造成巨大的人员和经济损失，传统方法难以捕捉复杂关系和上下文信息，亟需更高效的分析方法。

Method: 提出CrashSage框架，包括数据转换、上下文增强、LLaMA3-8B模型微调和梯度解释技术。

Result: CrashSage在事故严重性推断上表现优于基线方法，并提供了更透明的决策解释。

Conclusion: CrashSage为交通事故分析提供了高效且可解释的解决方案，有助于针对性安全干预。
...
### [92] [Unpacking Robustness in Inflectional Languages: Adversarial Evaluation and Mechanistic Insights](https://arxiv.org/abs/2505.07856)
*Paweł Walkowiak,Marek Klonowski,Marcin Oleksy,Arkadiusz Janz*

Main category: cs.CL

TLDR: 该论文研究了对抗性攻击在屈折语言（如波兰语）中的表现，提出了一种基于Edge Attribution Patching（EAP）的新评估协议，并创建了一个新基准MultiEmo来分析模型在攻击下的行为。


<details>
  <summary>Details</summary>
Motivation: 探讨对抗性攻击在屈折语言中的效果，揭示屈折变化对模型鲁棒性的影响。

Method: 设计了一种基于EAP的评估协议，使用波兰语和英语的平行语料库，并创建MultiEmo基准。

Result: 分析了模型在攻击下的行为，揭示了屈折变化与对抗鲁棒性的关系。

Conclusion: 屈折语言中的对抗性攻击表现与英语不同，新协议和基准为理解模型行为提供了工具。
...
### [93] [Enhanced Urdu Intent Detection with Large Language Models and Prototype-Informed Predictive Pipelines](https://arxiv.org/abs/2505.07857)
*Faiza Hassan,Summra Saleem,Kashif Javed,Muhammad Nabeel Asim,Abdur Rehman,Andreas Dengel*

Main category: cs.CL

TLDR: 本文提出了一种基于对比学习和原型注意力机制的意图检测框架LLMPIA，专门针对乌尔都语，填补了该语言在少样本意图检测领域的空白。


<details>
  <summary>Details</summary>
Motivation: 乌尔都语作为第十大语言，其意图检测领域缺乏少样本学习方法，传统方法仅能预测训练集中见过的类别。本文旨在通过结合预训练语言模型和对比学习，提升乌尔都语意图检测能力。

Method: 采用对比学习策略，利用未标注数据重新训练预训练语言模型，并结合原型注意力机制构建LLMPIA框架。评估了6种语言模型和13种相似度计算方法。

Result: 在ATIS和Web Queries数据集上，LLMPIA在4-way 1-shot和4-way 5-shot设置下分别达到83.28%/98.25%和76.23%/84.42%的F1分数。在相同类别设置下，性能超越现有方法53.55%。
...
### [94] [Scaling Laws for Speculative Decoding](https://arxiv.org/abs/2505.07858)
...
TLDR: 该研究通过密集LLM架构探索推测解码技术，发现对数线性缩放规律，并开发了Scylla系统，显著提升解码效率。
...
Method: 研究通过密集LLM架构分析推测解码技术，发现对数线性缩放规律，并开发了Scylla系统。

Result: Scylla在解码接受率和吞吐量上优于现有方法，工业部署中解码吞吐量提升2倍。

Conclusion: 系统性缩放对高效LLM推理具有变革潜力，Scylla为未来研究提供了基础。
...
### [95] [Boosting Performance on ARC is a Matter of Perspective](https://arxiv.org/abs/2505.07859)
*Daniel Franzen,Jan Disselhoff,David Hartmann*

Main category: cs.CL

TLDR: 本文提出了一种通过数据增强和深度优先搜索算法提升LLM在ARC-AGI任务中表现的方法，并利用LLM作为生成器和评分器，实现了71.6%的得分。
...
Result: 在公开ARC-AGI评估集上得分71.6%（286.5/400任务），推理成本低（约2ct/任务）。

Conclusion: 该方法在透明性、可重复性和低成本方面表现突出，尽管封闭源代码方法得分更高。
...
### [96] [Scalable LLM Math Reasoning Acceleration with Low-rank Distillation](https://arxiv.org/abs/2505.07861)
*Harry Dong,Bilge Acun,Beidi Chen,Yuejie Chi*

Main category: cs.CL

TLDR: Caprese是一种低成本蒸馏方法，用于恢复因高效推理方法部署而丢失的数学能力，同时不影响语言任务性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）在数学推理中需要大量计算资源和时间，而现有高效推理方法虽在语言任务中表现良好，但会严重降低数学性能。

Method: 提出Caprese方法，通过仅增加约1%的参数和20K合成训练样本，恢复因高效推理丢失的数学能力，且不干扰原始权重。

Result: Caprese显著减少了活跃参数数量（如Gemma 2 9B和Llama 3.1 8B减少约2B），并降低了延迟（如Qwen 2.5 14B生成2048个令牌时延迟减少11%）。

Conclusion: Caprese是一种高效且低成本的方法，能够在保持语言任务性能的同时恢复数学推理能力，并减少计算资源消耗。
...
### [97] [Graph Laplacian Wavelet Transformer via Learnable Spectral Decomposition](https://arxiv.org/abs/2505.07862)
*Andrew Kiruluta,Eric Lundy,Priscilla Burity*

Main category: cs.CL

TLDR: 论文提出了一种名为Graph Wavelet Transformer（GWT）的新架构，用于替代传统序列到序列模型中的点积自注意力机制，以解决其计算和内存的二次复杂度问题。


<details>
  <summary>Details</summary>
Motivation: 传统序列到序列模型中的点积自注意力机制在输入长度为N时具有二次复杂度，限制了模型的效率和可扩展性。

Method: GWT通过引入可学习的多尺度小波变换，基于显式图拉普拉斯矩阵（源自句法或语义解析）来替代点积自注意力。

Result: 分析表明，多尺度谱分解为图结构序列建模提供了一种可解释、高效且表达能力强的替代方案。

Conclusion: GWT为图结构序列建模提供了一种更高效的解决方案，同时保持了模型的表达能力和可解释性。
...
### [98] [QoSBERT: An Uncertainty-Aware Approach based on Pre-trained Language Models for Service Quality Prediction](https://arxiv.org/abs/2505.07863)
*Ziliang Wang,Xiaohong Zhang,Ze Shi Li,Meng Yan*

Main category: cs.CL

TLDR: QoSBERT是一种基于预训练语言模型的框架，将QoS预测重新定义为语义回归任务，提供置信区间，显著提升了预测准确性和不确定性量化能力。


<details>
  <summary>Details</summary>
Motivation: 传统QoS模型依赖人工特征工程且仅提供点估计，缺乏预测置信度信息，QoSBERT旨在解决这一问题。

Method: QoSBERT将用户服务元数据编码为自然语言描述，结合蒙特卡洛Dropout进行不确定性估计，使用注意力池化和轻量级多层感知机回归器。

Result: 在标准QoS基准数据集上，QoSBERT在响应时间和吞吐量预测上分别平均降低了11.7%和6.9%的MAE，同时提供校准的置信区间。
...
### [99] [Efficient Fairness Testing in Large Language Models: Prioritizing Metamorphic Relations for Bias Detection](https://arxiv.org/abs/2505.07870)
*Suavis Giramata,Madhusudan Srinivasan,Venkat Naidu Gudivada,Upulee Kanewala*

Main category: cs.CL

TLDR: 本文提出了一种基于句子多样性的蜕变关系（MRs）优先级排序方法，用于高效检测大语言模型（LLMs）中的公平性问题。该方法显著提高了故障检测率并减少了计算成本。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型（LLMs）的广泛应用，其输出中的公平性和偏见问题日益突出。由于测试用例数量庞大，全面测试不可行，因此需要高效的方法来优先检测公平性问题。

Method: 采用基于句子多样性的方法计算和排序蜕变关系（MRs），以优化故障检测。

Result: 实验结果表明，该方法比随机优先级排序提高了22%的故障检测率，比基于距离的优先级排序提高了12%，同时首次故障时间分别减少了15%和8%。

Conclusion: 多样性为基础的MR优先级排序方法在提高LLMs公平性测试效果的同时，显著降低了计算成本，验证了其有效性。
...
### [100] [Evaluating Financial Sentiment Analysis with Annotators Instruction Assisted Prompting: Enhancing Contextual Interpretation and Stock Prediction Accuracy](https://arxiv.org/abs/2505.07871)
*A M Muntasir Rahman,Ajim Uddin,Guiling "Grace" Wang*

Main category: cs.CL

TLDR: 论文提出了一种新的评估提示方法（AIAP），通过整合人类标注者的任务指令来改进金融情感分析（FSA）中LLMs的性能，并在新数据集WSBS上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有金融情感分析基准数据集（如Financial Phrasebank）因标注者主观性导致标注不一致，使LLMs在评估中表现不佳。

Method: 提出Annotators' Instruction Assisted Prompt（AIAP），将人类标注者的详细任务指令整合到LLMs的提示框架中，以标准化情感理解。

Result: AIAP显著提升LLMs性能（最高提升9.08），并引入基于模型置信度的情感索引方法，增强股票价格预测。

Conclusion: AIAP通过改进任务定义和评估方法，提升了金融情感分析的性能，并展示了WSB作为金融文本来源的重要性。
...
### [101] [The Sound of Populism: Distinct Linguistic Features Across Populist Variants](https://arxiv.org/abs/2505.07874)
*Yu Wang,Runxi Yu,Zhongyuan Wang,Jing He*

Main category: cs.CL

TLDR: 该研究结合LIWC特征和RoBERTa模型，分析美国政治演讲中的民粹主义声音，揭示其直接、自信的语言风格及其在不同派别中的差异。


<details>
  <summary>Details</summary>
Motivation: 探索民粹主义在政治演讲中的语言表现，特别是其情感和风格特征。

Method: 整合LIWC特征和RoBERTa模型，分析美国总统就职演说和国情咨文中的语言标记。

Result: 民粹主义语言风格直接且自信，右翼民粹主义和人民中心主义更具情感色彩，而左翼和反精英主义则相对克制。

Conclusion: 民粹主义语言风格具有战略性和多样性，不同派别在情感表达上存在显著差异。
...
### [102] [Recovering Event Probabilities from Large Language Model Embeddings via Axiomatic Constraints](https://arxiv.org/abs/2505.07883)
*Jian-Qiao Zhu,Haijiang Yan,Thomas L. Griffiths*

Main category: cs.CL

TLDR: 论文探讨了如何从LLM嵌入中恢复符合概率论公理的连贯事件概率，并通过扩展VAE在潜在空间中强制约束，实验证明其优于模型直接生成的概率。


<details>
  <summary>Details</summary>
Motivation: LLM生成的事件概率存在不连贯性，违反了概率论公理，因此研究是否能从嵌入中恢复连贯概率以更准确地估计不确定性事件。

Method: 提出在扩展VAE的潜在空间中强制概率论公理约束（如加法规则），使事件概率自然生成，同时重建原始嵌入并预测相关事件嵌入。

Result: 实验表明，从嵌入中恢复的概率比模型直接生成的更连贯，且与真实概率更接近。

Conclusion: 通过潜在空间约束，可以从LLM嵌入中恢复连贯的事件概率，为不确定性事件提供更准确的估计。
...
### [103] [Development of a WAZOBIA-Named Entity Recognition System](https://arxiv.org/abs/2505.07884)
*S. E Emedem,I. E Onyenwe,E. G Onyedinma*

Main category: cs.CL

TLDR: 该研究开发了针对尼日利亚三大语言（豪萨语、约鲁巴语和伊博语）的WAZOBIA-NER系统，填补了低资源语言在命名实体识别（NER）领域的空白。通过结合CRF、BiLSTM、BERT和RNN等先进技术，系统在精度、召回率、F1分数和准确率上表现优异。


<details>
  <summary>Details</summary>
Motivation: 非洲语言在计算语言学中日益受到关注，但现有NER系统主要针对英语和欧洲语言，低资源语言缺乏支持。本研究旨在为尼日利亚三大语言开发有效的NER工具。

Method: 研究首先为每种语言构建了标注数据集，并探索了CRF、BiLSTM、BERT和RNN等机器学习与深度学习技术。系统还利用OCR技术处理图像文本输入。

Result: 系统在精度（0.9511）、召回率（0.9400）、F1分数（0.9564）和准确率（0.9301）上表现优异，验证了其在多语言NER任务中的有效性。

Conclusion: 研究表明，利用现有NLP框架和迁移学习，可以为低资源非洲语言构建强大的NER工具。
...
### [104] [PLHF: Prompt Optimization with Few-Shot Human Feedback](https://arxiv.org/abs/2505.07886)
*Chun-Pai Yang,Kan Zheng,Shou-De Lin*

Main category: cs.CL

TLDR: PLHF是一种基于人类反馈的少样本提示优化框架，通过特定评估模块解决无明确指标时的提示优化问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以处理输出质量无法通过标准样本评估的任务，需解决无明确指标时的提示优化挑战。

Method: PLHF采用类似RLHF的技术，通过单轮人类反馈和评估模块优化提示。

Result: 在公开和工业数据集上，PLHF优于先前的输出评分策略。

Conclusion: PLHF为无明确质量指标的任务提供了一种高效的提示优化解决方案。
...
### [105] [Implementing Long Text Style Transfer with LLMs through Dual-Layered Sentence and Paragraph Structure Extraction and Mapping](https://arxiv.org/abs/2505.07888)
...
### [106] [BioProBench: Comprehensive Dataset and Benchmark in Biological Protocol Understanding and Reasoning](https://arxiv.org/abs/2505.07889)
*Yuyang Liu,Liuzhenghao Lv,Xiancheng Zhang,Li Yuan,Yonghong Tian*

Main category: cs.CL

TLDR: BioProBench是一个大规模、多任务的生物协议理解和推理基准，包含五个核心任务，评估了12种主流LLM，发现其在深度推理和结构化生成任务上表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在高度专业化、准确性要求高的生物协议文本上的系统评估有限，需要全面评估其能力。
...
### [107] [TSLFormer: A Lightweight Transformer Model for Turkish Sign Language Recognition Using Skeletal Landmarks](https://arxiv.org/abs/2505.07890)
*Kutay Ertürk,Furkan Altınışık,İrem Sarıaltın,Ömer Nezih Gerek*

Main category: cs.CL

TLDR: TSLFormer是一个轻量且鲁棒的土耳其手语识别模型，将手势视为有序的字符串语言，仅使用3D关节位置作为输入，通过序列到序列翻译实现高效识别。


<details>
  <summary>Details</summary>
Motivation: 研究旨在通过简化输入数据（仅使用3D关节位置）和利用序列到序列翻译方法，实现高效且低成本的手语识别系统，以支持听障人士的实时交流。

Method: 使用Google Mediapipe库提取手和躯干的3D关节位置作为输入，采用基于自注意力机制的Transformer模型（TSLFormer）进行序列到序列翻译。

Result: 在AUTSL数据集（36,000样本，227个单词）上表现优异，计算成本低，验证了关节输入对实时移动辅助系统的有效性。

Conclusion: TSLFormer证明了基于关节的输入足以支持实时手语识别系统，为听障人士提供了高效的辅助通信解决方案。
...
### [108] [TrumorGPT: Graph-Based Retrieval-Augmented Large Language Model for Fact-Checking](https://arxiv.org/abs/2505.07891)
*Ching Nam Hang,Pei-Duo Yu,Chee Wei Tan*

Main category: cs.CL

TLDR: TrumorGPT是一种基于生成式人工智能的解决方案，用于健康领域的事实核查，通过语义健康知识图谱和GraphRAG技术，有效区分真实谣言（trumors）与虚假信息。


<details>
  <summary>Details</summary>
Motivation: 社交媒体时代，虚假信息迅速传播形成信息疫情（infodemics），对健康领域造成威胁，需要一种工具来区分真实与虚假信息。

Method: TrumorGPT结合大型语言模型（LLM）和少样本学习，构建语义健康知识图谱，并采用GraphRAG技术解决LLM的幻觉问题和静态数据的局限性。

Result: 在广泛的医疗数据集评估中，TrumorGPT在公共卫生声明的事实核查中表现出色，能够基于最新数据提供准确结果。
...
### [109] [LongCodeBench: Evaluating Coding LLMs at 1M Context Windows](https://arxiv.org/abs/2505.07897)
*Stefano Rando,Luca Romani,Alessio Sampieri,Yuta Kyuragi,Luca Franco,Fabio Galasso,Tatsunori Hashimoto,John Yang*

Main category: cs.CL

TLDR: 论文介绍了LongCodeBench（LCB），一个用于测试长上下文模型中代码理解和修复能力的基准测试，发现长上下文仍是所有模型的弱点。


<details>
  <summary>Details</summary>
Motivation: 现代长上下文模型的极端上下文长度使得构建现实的基准测试变得困难，代码理解和修复被选为自然测试场景。

Method: 通过从真实GitHub问题中提取数据，构建了QA（LongCodeQA）和bug修复（LongSWE-Bench）任务，并分层复杂度以评估不同规模的模型。

Result: 所有模型在长上下文场景下表现下降，例如Claude 3.5 Sonnet从29%降至3%，Qwen2.5从70.2%降至40%。

Conclusion: 长上下文仍是模型的挑战，LongCodeBench为评估和改进长上下文能力提供了重要工具。
...
### [110] [DeltaEdit: Enhancing Sequential Editing in Large Language Models by Controlling Superimposed Noise](https://arxiv.org/abs/2505.07899)
*Ding Cao,Yuchen Cai,Rongxi Guo,Xuesong He,Guiquan Liu*

Main category: cs.CL

TLDR: DeltaEdit通过动态正交约束策略优化更新参数，显著提高了长期编辑的成功率，并保持了模型的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有序列知识编辑方法在长期编辑后成功率显著下降，原因是编辑叠加噪声的累积问题。

Method: 提出DeltaEdit方法，通过动态正交约束策略优化更新参数，减少编辑间的干扰。

Result: 实验表明DeltaEdit在编辑成功率和泛化能力保留上显著优于现有方法。

Conclusion: DeltaEdit解决了长期编辑中的噪声累积问题，确保了模型性能的稳定性和可靠性。
...
### [111] [SEM: Reinforcement Learning for Search-Efficient Large Language Models](https://arxiv.org/abs/2505.07903)
*Zeyang Sha,Shiwen Cui,Weiqiang Wang*

Main category: cs.CL

TLDR: 论文提出SEM框架，通过后训练强化学习优化LLMs的搜索行为，减少冗余搜索并保持准确性。


<details>
  <summary>Details</summary>
Motivation: LLMs在调用外部工具（如搜索引擎）时难以判断何时需要搜索，现有方法常导致冗余搜索和低效。

Method: 构建平衡数据集（MuSiQue和MMLU），设计结构化推理模板，使用GRPO后训练模型，优化搜索行为。

Result: 实验表明，SEM显著减少冗余搜索，同时在多个基准测试中保持或提高答案准确性。

Conclusion: SEM框架提升了LLMs的推理效率，并使其能更明智地利用外部知识。
...
### [112] [Re$^2$: A Consistency-ensured Dataset for Full-stage Peer Review and Multi-turn Rebuttal Discussions](https://arxiv.org/abs/2505.07920)
*Daoze Zhang,Zhijian Bao,Sihang Du,Zhiyi Zhao,Kuangling Zhang,Dezheng Bao,Yang Yang*

Main category: cs.CL

TLDR: 论文介绍了Re^2数据集，旨在解决同行评审系统中的数据多样性不足、质量不一致和交互支持不足的问题。


<details>
  <summary>Details</summary>
Motivation: 同行评审系统因投稿量激增和重复提交低质量稿件而压力巨大，缺乏作者自我评估工具。LLMs的潜力受限于现有评审数据的质量。

Method: 构建了Re^2数据集，包含19,926份初始投稿、70,668条评审意见和53,818条反驳，支持多轮对话范式。

Result: Re^2是目前最大的同行评审数据集，支持静态和动态任务，为作者和评审提供实用指导。

Conclusion: Re^2数据集有望缓解评审负担，提升评审质量，并为LLMs提供更高质量的训练数据。
...
### [113] [Assessing and Mitigating Medical Knowledge Drift and Conflicts in Large Language Models](https://arxiv.org/abs/2505.07968)
*Weiyi Wu,Xinwen Xu,Chongyang Gao,Xingjian Diao,Siting Li,Lucas A. Salas,Jiang Gui*

Main category: cs.CL

TLDR: 研究探讨了大型语言模型（LLMs）在医疗指南演变中的表现，发现其难以拒绝过时建议且常支持矛盾指导，提出了两种缓解策略并验证其有效性。
...
Method: 开发DriftMedQA基准模拟指南演变，评估七种先进LLMs的时间可靠性，并探索检索增强生成和偏好微调两种缓解策略。
...
### [114] [Task-Adaptive Semantic Communications with Controllable Diffusion-based Data Regeneration](https://arxiv.org/abs/2505.07980)
...
<details>
  <summary>Details</summary>
Motivation: 传统通信以比特传递为主，而语义通信旨在高效传递语义信息，需适应不同任务需求。

Method: 采用扩散模型，先传输深度压缩的通用语义表示，接收端生成任务提示反馈，发送端通过注意力机制更新传输内容。
...
### [115] [Large Language Models and Arabic Content: A Review](https://arxiv.org/abs/2505.08004)
*Haneh Rhel,Dmitri Roussinov*

Main category: cs.CL

TLDR: 论文概述了大型语言模型（LLMs）在阿拉伯语NLP中的应用，包括早期预训练模型、性能提升技术（如微调和提示工程），以及常见基准数据集。
...
Method: 研究分析了预训练LLMs在阿拉伯语NLP中的应用，探讨了微调和提示工程技术。

Result: LLMs在阿拉伯语NLP任务中表现显著，且采用率持续上升。

Conclusion: LLMs为阿拉伯语NLP提供了有效解决方案，未来需进一步优化技术和资源。
...
### [116] [TiSpell: A Semi-Masked Methodology for Tibetan Spelling Correction covering Multi-Level Error with Data Augmentation](https://arxiv.org/abs/2505.08037)
...
TLDR: 提出了一种多级藏文拼写纠正方法TiSpell，结合字符和音节级错误纠正，通过半掩码策略简化音节级纠正，并利用数据增强生成训练集。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注单级纠正，缺乏对字符和音节级错误的有效整合，且缺乏针对藏文的开源数据集和增强方法。

Method: 提出数据增强方法生成多级错误，并设计半掩码模型TiSpell，同时纠正字符和音节级错误。

Result: 在模拟和真实数据上，TiSpell优于基线模型，与最先进方法性能相当。

Conclusion: TiSpell在多级藏文拼写纠正中表现有效，验证了其设计和数据增强的实用性。
...
### [117] [FalseReject: A Resource for Improving Contextual Safety and Mitigating Over-Refusals in LLMs via Structured Reasoning](https://arxiv.org/abs/2505.08054)
*Zhehao Zhang,Weijie Xu,Fanyou Wu,Chandan K. Reddy*

Main category: cs.CL

TLDR: 论文提出FalseReject资源，通过图引导的多智能体交互框架生成多样化提示，减少LLMs对良性查询的过度拒绝，同时保持安全性。
...
Method: 提出FalseReject资源，包含16k看似有害的查询和结构化响应；采用图引导的对抗多智能体交互框架生成多样化提示。

Result: 实验表明，使用FalseReject进行监督微调可显著减少不必要的拒绝，同时不损害安全性或语言能力。

Conclusion: FalseReject有效解决了LLMs的过度拒绝问题，为安全对齐提供了实用解决方案。
...
### [118] [HYPERNYM MERCURY: Token Optimization through Semantic Field Constriction and Reconstruction from Hypernyms. A New Text Compression Method](https://arxiv.org/abs/2505.08058)
*Chris Forrester,Octavia Sulea*

Main category: cs.CL

TLDR: 提出一种新型文本表示方案和语义压缩技术，实现90%以上的token减少，同时保持高语义相似度。


<details>
  <summary>Details</summary>
Motivation: 在NLP和下一代智能AI中，计算优化是一个新兴任务，需要高效的token减少方法。

Method: 采用专利待决的文本表示方案和段落级语义压缩技术，支持无损压缩和粒度控制。

Result: 在开源数据（如《德古拉》）上验证，段落级别结果稳定，适用于多种类型和模型。

Conclusion: 该技术能显著减少token数量，同时保持语义完整性，具有广泛应用潜力。
...
### [119] [Are LLMs complicated ethical dilemma analyzers?](https://arxiv.org/abs/2505.08106)
*Jiashen,Du,Jesse Yao,Allen Liu,Zhekai Zhang*

Main category: cs.CL

TLDR: 研究探讨LLMs是否能模拟人类伦理推理，通过基准数据集和复合指标评估，发现LLMs在词汇和结构上优于非专家人类，但在历史背景和策略上表现不足。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs是否可作为人类伦理判断的代理，填补其在伦理推理能力上的研究空白。

Method: 使用包含196个伦理困境的基准数据集，结合多种指标（BLEU、Damerau-Levenshtein等）评估LLMs与非专家人类的表现。

Result: LLMs在词汇和结构上优于人类，但在历史背景和策略上表现较差；GPT-4o-mini表现最稳定。
...
### [120] [Putting It All into Context: Simplifying Agents with LCLMs](https://arxiv.org/abs/2505.08120)
*Mingjian Jiang,Yangjun Ruan,Luis Lastras,Pavan Kapanipathi,Tatsunori Hashimoto*

Main category: cs.CL

TLDR: 研究发现，在复杂任务（如SWE-bench）中，简单的长上下文语言模型（LCLM）无需复杂架构即可达到与精心调优的代理架构相当的性能。


<details>
  <summary>Details</summary>
Motivation: 探讨复杂语言模型代理架构是否必要，或是否可以通过简化方法（如长上下文模型）实现类似效果。

Method: 使用长上下文语言模型（如Gemini-1.5-Pro和Gemini-2.5-Pro）直接处理任务，无需复杂代理架构或工具。

Result: Gemini-1.5-Pro达到38%解决率，与复杂架构（32%）相当；Gemini-2.5-Pro达到50.8%，两阶段方法（结合Claude-3.7）达到48.6%。

Conclusion: 简化方法在部分任务中表现优异，表明复杂架构并非总是必要，未来可探索更高效的模型组合。
...
### [121] [ALOHA: Empowering Multilingual Agent for University Orientation with Hierarchical Retrieval](https://arxiv.org/abs/2505.08130)
*Mingxu Tao,Bowen Tang,Mingxuan Ma,Yining Zhang,Hourun Li,Feifan Wen,Hao Ma,Jia Yang*

Main category: cs.CL

TLDR: ALOHA系统通过分层检索和多语言支持，解决了LLMs在校园信息检索中的不足，提供更准确、及时和用户友好的服务。


<details>
  <summary>Details</summary>
Motivation: 现有公共LLM服务和搜索引擎无法满足校园特定信息的多语言和实时检索需求。

Method: 引入ALOHA系统，结合分层检索和外部API，支持多语言交互服务。

Result: 系统在12,000多人中部署，表现优于商业聊天机器人和搜索引擎。

Conclusion: ALOHA系统有效解决了校园信息检索的挑战，展示了实际应用的潜力。
...
### [122] [Fusing Bidirectional Chains of Thought and Reward Mechanisms A Method for Enhancing Question-Answering Capabilities of Large Language Models for Chinese Intangible Cultural Heritage](https://arxiv.org/abs/2505.08167)
*Ruilin Liu,Zhixiao Zhao,Jieqiong Li,Chang Liu,Dongbo Wang*

Main category: cs.CL

TLDR: 提出了一种结合双向思维链和奖励机制的新训练方法，用于解决领域特定大语言模型在微调过程中面临的偏见、知识错误继承和灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在微调非物质文化遗产数据时面临的挑战，如偏见和知识错误继承。

Method: 结合双向思维链（正向和反向推理）和奖励机制，优化模型决策和输出质量。

Result: 在问答任务中，该方法在准确性、Bleu-4和Rouge-L得分上优于其他方法，并在多领域数据集中表现出泛化能力。

Conclusion: 该方法为多领域模型训练提供了有效解决方案，具有广泛适用性。
...
### [123] [Exploiting Text Semantics for Few and Zero Shot Node Classification on Text-attributed Graph](https://arxiv.org/abs/2505.08168)
*Yuxiang Wang,Xiao Yan,Shiyu Jin,Quanqing Xu,Chuang Hu,Yuanyuan Zhu,Bo Du,Jia Wu,Jiawei Jiang*

Main category: cs.CL

TLDR: 本文提出了一种名为TSA的文本语义增强方法，通过引入更多文本语义监督信号，提升了TAG上少样本和零样本节点分类的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的图增强方法在TAG上训练节点和文本嵌入时，文本增强技术未被充分探索，因此需要改进。

Method: 设计了两种增强技术：正语义匹配和负语义对比，以提供更多参考文本。

Result: 在5个数据集上评估，TSA优于13个基线方法，准确率提升通常超过5%。

Conclusion: TSA通过文本语义增强显著提升了节点分类的准确性。
...
### [124] [A Head to Predict and a Head to Question: Pre-trained Uncertainty Quantification Heads for Hallucination Detection in LLM Outputs](https://arxiv.org/abs/2505.08200)
...
TLDR: 论文提出了一种预训练的不确定性量化（UQ）头部模块，用于增强大型语言模型（LLMs）检测幻觉的能力，优于无监督方法。


<details>
  <summary>Details</summary>
Motivation: LLMs容易生成虚假信息（幻觉），且难以检测，需要可靠的方法评估模型输出的可信度。

Method: 设计了基于Transformer架构的监督辅助模块（UQ头部），利用LLM的注意力图提取特征，提升不确定性量化能力。

Result: 实验表明，UQ头部在幻觉检测上表现优异，支持跨语言泛化，并公开了预训练模块和代码。

Conclusion: 预训练的UQ头部显著提升了LLMs的幻觉检测能力，具有广泛适用性和实用性。
...
### [125] [Large Language Model Psychometrics: A Systematic Review of Evaluation, Validation, and Enhancement](https://arxiv.org/abs/2505.08245)
*Haoran Ye,Jing Jin,Yuhang Xie,Xin Zhang,Guojie Song*

Main category: cs.CL

TLDR: 本文介绍了LLM心理测量学这一新兴跨学科领域，利用心理测量工具和理论评估大型语言模型（LLMs），旨在推动更全面的人类中心AI评估范式。


<details>
  <summary>Details</summary>
Motivation: 传统评估方法难以应对LLMs的快速发展，需引入心理测量学以量化人类心理特征，如人格和智力，从而提升评估的全面性和准确性。

Method: 通过整合心理测量学的理论和工具，系统探讨其在LLMs评估中的作用，包括基准原则、方法优化和结果验证。

Result: 提出了LLM心理测量学的结构化框架，为跨学科研究者提供资源，推动人类中心AI系统的进步。

Conclusion: 本文为未来评估范式的发展提供了实用见解，旨在促进与社会利益一致的人类水平AI发展。
...
### [126] [Enhancing Cache-Augmented Generation (CAG) with Adaptive Contextual Compression for Scalable Knowledge Integration](https://arxiv.org/abs/2505.08261)
*Rishabh Agrawal,Himanshu Kumar*

Main category: cs.CL

TLDR: 本文提出了一种自适应上下文压缩（ACC）技术和混合CAG-RAG框架，以解决缓存增强生成（CAG）在扩展动态知识库时的挑战，优化效率并提升多跳推理性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）的快速发展为知识密集型任务提供了新方法，但CAG在扩展动态知识库时面临挑战。

Method: 提出自适应上下文压缩（ACC）技术动态管理上下文输入，并设计混合CAG-RAG框架，结合选择性检索增强预加载上下文。

Result: 实验表明，所提方法能提升扩展性、优化效率，并改善多跳推理性能。

Conclusion: ACC和混合CAG-RAG框架为实际知识集成挑战提供了实用解决方案。
...
### [127] [Evaluating the Effectiveness of Black-Box Prompt Optimization as the Scale of LLMs Continues to Grow](https://arxiv.org/abs/2505.08303)
*Ziyu Zhou,Yihang Wu,Jingyuan Yang,Zhan Xiao,Rongjun Li*

Main category: cs.CL

TLDR: 黑盒提示优化方法在大规模语言模型（如DeepSeek V3和Gemini 2.0 Flash）上的效果有限，且随着模型规模增大，优化效果逐渐减弱。
...
Method: 选择三种黑盒优化方法，在多个大规模语言模型和数据集上进行评估，并分析模型规模对优化效果的影响。

Result: 黑盒优化方法在大规模模型上仅带来有限改进，且模型规模越大，优化效果越差。

Conclusion: 模型规模是影响黑盒提示优化效果的主要因素，未来研究需针对大规模模型设计更有效的优化方法。
...
### [128] [AM-Thinking-v1: Advancing the Frontier of Reasoning at 32B Scale](https://arxiv.org/abs/2505.08311)
*Yunjie Ji,Xiaoyu Tian,Sitong Zhao,Haotian Wang,Shuaiting Chen,Yiping Peng,Han Zhao,Xiangang Li*

Main category: cs.CL

TLDR: AM-Thinking-v1是一个32B密集语言模型，在数学和编码能力上表现优异，超越DeepSeek-R1并与领先的MoE模型竞争。
...
Conclusion: AM-Thinking-v1证明了中等规模模型的潜力，鼓励开源协作推动推理边界。
...
### [129] [On the Geometry of Semantics in Next-token Prediction](https://arxiv.org/abs/2505.08348)
*Yize Zhao,Christos Thrampoulidis*

Main category: cs.CL

TLDR: 论文探讨了现代语言模型如何通过简单的下一个词预测（NTP）目标学习语义和语法概念，揭示了NTP优化隐含地引导模型通过奇异值分解（SVD）编码概念。


<details>
  <summary>Details</summary>
Motivation: 研究NTP目标如何引导语言模型提取和编码潜在的语义和语法概念。

Method: 分析模型如何通过SVD分解中心化的数据稀疏矩阵来学习词和上下文嵌入，并利用谱聚类识别语义。

Result: 发现最重要的SVD因子在训练初期被学习，支持通过谱聚类方法识别可解释的语义。

Conclusion: 工作连接了分布语义、神经崩溃几何和神经网络训练动态，揭示了NTP如何塑造语言模型中的意义表示。
...
### [130] [Alignment Drift in CEFR-prompted LLMs for Interactive Spanish Tutoring](https://arxiv.org/abs/2505.08351)
*Mina Almasi,Ross Deans Kristensen-McLachlan*

Main category: cs.CL

TLDR: 研究探讨了大型语言模型（LLMs）作为第二语言学习自适应导师的潜力，评估了系统提示是否能可靠地限制LLMs生成适合学生能力水平的文本。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在个性化语言学习中的应用，特别是通过系统提示控制文本难度以适应不同语言水平的学生。

Method: 使用7B至12B参数的指令调优开源LLMs模拟西班牙语师生对话，通过CEFR提示控制文本难度（A1、B1、C1）。

Result: 系统提示可约束模型输出，但单独使用提示在长期交互中效果不稳定（对齐漂移）。

Conclusion: LLMs作为自适应导师具有潜力，但需进一步改进提示方法以应对长期交互挑战。
...
### [131] [Towards Contamination Resistant Benchmarks](https://arxiv.org/abs/2505.08389)
*Rahmatullah Musawi,Sheng Lu*

Main category: cs.CL

TLDR: 本文提出了一种基于凯撒密码的污染抵抗基准，用于更严格评估大型语言模型（LLMs），揭示了当前模型在污染控制下的表现问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）的快速发展需要更可靠的评估方法，污染问题影响了评估的准确性。

Method: 提出基于凯撒密码的污染抵抗基准，并在多种设置下测试广泛使用的LLMs。

Result: 发现LLMs在污染控制下表现不佳，揭示了其真实能力的局限性。

Conclusion: 该研究为开发污染抵抗基准提供了贡献，有助于更严格评估LLMs的真实能力与限制。
...
### [132] [Accelerating Chain-of-Thought Reasoning: When Goal-Gradient Importance Meets Dynamic Skipping](https://arxiv.org/abs/2505.08392)
*Ren Zhuang,Ben Wang,Shuifa Sun*

Main category: cs.CL

TLDR: 论文提出Adaptive GoGI-Skip框架，通过动态压缩Chain-of-Thought（CoT）提示，显著提升推理效率，同时保持准确性。


<details>
  <summary>Details</summary>
Motivation: 当前CoT压缩技术依赖静态压缩率和通用重要性指标，可能导致关键信息丢失或无法适应不同推理复杂度。

Method: 提出Goal-Gradient Importance（GoGI）指标和Adaptive Dynamic Skipping（ADS）机制，动态调整压缩率。

Result: 在MATH数据上训练，Adaptive GoGI-Skip平均减少45%的CoT标记，推理速度提升1.6-2.0倍，且保持高准确性。
...
### [133] [TUMS: Enhancing Tool-use Abilities of LLMs with Multi-structure Handlers](https://arxiv.org/abs/2505.08402)
*Aiyao He,Sijia Cui,Shuai Xu,Yanna Wang,Bo Xu*

Main category: cs.CL

TLDR: TUMS框架通过将工具级处理转化为参数级处理，提升了LLMs的工具使用能力，显著提高了任务执行效果。


<details>
  <summary>Details</summary>
Motivation: LLMs在工具集成中面临非可执行操作和参数错误的问题，TUMS旨在解决这一问题。

Method: TUMS包含意图识别器、任务分解器、子任务处理器和执行器，通过参数级处理优化工具调用。

Result: 在ToolQA基准测试中，TUMS在简单和困难任务上分别提升了19.6%和50.6%。

Conclusion: TUMS框架有效提升了LLMs的工具使用能力，为未来工具增强型LLMs研究提供了新思路。
...
### [134] [Hakim: Farsi Text Embedding Model](https://arxiv.org/abs/2505.08435)
*Mehran Sarmadi,Morteza Alikhani,Erfan Zinvandi,Zahra Pourbahman*

Main category: cs.CL

TLDR: 本文提出了一种名为Hakim的新型波斯语文本嵌入模型，性能提升8.5%，并引入三个新数据集，适用于聊天机器人和检索增强生成系统。
...
Method: 开发了基于BERT架构的基线模型和RetroMAE模型，并引入三个新数据集（Corpesia、Pairsia-sup、Pairsia-unsup）。
...
### [135] [A document processing pipeline for the construction of a dataset for topic modeling based on the judgments of the Italian Supreme Court](https://arxiv.org/abs/2505.08439)
*Matteo Marulli,Glauco Panattoni,Marco Bertini*

Main category: cs.CL

TLDR: 本文提出了一种针对意大利法律研究的文档处理流程，用于生成匿名化数据集以支持主题建模，显著提升了主题建模的效果。
...
Result: DLA模块的mAP@50为0.964，OCR检测器的mAP@50-95为0.9022，TrOCR的字符错误率为0.0047。数据集提升了主题建模的多样性（0.6198）和一致性（0.6638）。

Conclusion: 结合BERTopic和大型语言模型（如Claude Sonnet 3.7）生成的主题标签和摘要，与专家解释一致，验证了方法的有效性。
...
### [136] [IterKey: Iterative Keyword Generation with LLMs for Enhanced Retrieval Augmented Generation](https://arxiv.org/abs/2505.08450)
*Kazuki Hayashi,Hidetaka Kamigaito,Shinya Kouda,Taro Watanabe*

Main category: cs.CL

TLDR: IterKey是一个基于LLM的迭代关键词生成框架，通过稀疏检索增强RAG，平衡了准确性和可解释性。
...
Method: 采用三阶段LLM驱动流程：生成检索关键词、基于检索文档生成答案、验证答案，若失败则迭代优化关键词。

Result: 在四个QA任务中，IterKey比BM25-based RAG和简单基线方法提高了5%至20%的准确率，性能接近密集检索方法。
...
### [137] [RepCali: High Efficient Fine-tuning Via Representation Calibration in Latent Space for Pre-trained Language Models](https://arxiv.org/abs/2505.08463)
*Fujun Zhang,XiangDong Su*

Main category: cs.CL

TLDR: 该论文提出了一种名为RepCali的方法，通过在校准块中调整预训练语言模型（PLM）的潜在空间表示，以解决PLM在微调后编码器与解码器输入不匹配的问题。


<details>
  <summary>Details</summary>
Motivation: 预训练语言模型（PLM）在微调后仍存在编码器输出与解码器输入不匹配的问题，限制了其在下游任务中的表现。

Method: 提出RepCali方法，在编码器后添加校准块，调整潜在空间表示作为解码器输入。该方法适用于所有编码器-解码器架构的PLM，且易于实现。

Result: 在8个任务（包括中英文数据集）的25个PLM模型上实验表明，RepCali显著提升了性能，优于基准微调方法。
...
### [138] [Large Language Models Meet Stance Detection: A Survey of Tasks, Methods, Applications, Challenges and Future Directions](https://arxiv.org/abs/2505.08464)
*Lata Pangtey,Anukriti Bhatnagar,Shubhi Bansal,Shahid Shafi Dar,Nagendra Kumar*

Main category: cs.CL

TLDR: 该论文综述了基于大语言模型（LLMs）的立场检测方法，提出了新的分类法，并探讨了应用、挑战及未来方向。
...
Method: 系统分析了LLMs在立场检测中的进展，提出基于学习方式、数据模态和目标关系的分类法。

Result: 总结了评估技术、数据集和性能趋势，并讨论了关键应用和挑战。

Conclusion: 指出了未来研究方向，如可解释性推理和低资源适应，为研究者提供指导。
...
### [139] [Judging the Judges: Can Large Vision-Language Models Fairly Evaluate Chart Comprehension and Reasoning?](https://arxiv.org/abs/2505.08468)
...
<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型（LVLM）评估成本高且耗时，限制了实际应用。希望通过开源LVLM作为评估工具，降低成本并提升效率。

Method: 设计了成对和点对评估任务，涵盖事实准确性、信息量和相关性等标准，并分析格式遵循、位置一致性和指令遵循等指标。

Result: 实验显示，部分开源LVLM评估性能接近GPT-4（约80%一致），但其他模型表现较差（低于10%一致）。
...
### [140] [LCES: Zero-shot Automated Essay Scoring via Pairwise Comparisons Using Large Language Models](https://arxiv.org/abs/2505.08498)
*Takumi Shibata,Yuichi Miyamura*

Main category: cs.CL

TLDR: 论文提出了一种基于大语言模型（LLM）的对比性作文评分方法（LCES），通过将评分任务转化为成对比较，提高了零样本自动评分的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的零样本自动作文评分方法直接生成绝对分数，容易因模型偏见和不一致评分偏离人工评估。

Method: LCES将评分任务转化为成对比较，利用LLM判断两篇作文的优劣，并通过RankNet将比较结果转换为连续分数。

Result: 实验表明，LCES在准确性和计算效率上优于传统零样本方法，且对不同LLM主干具有鲁棒性。

Conclusion: LCES为实际零样本自动作文评分提供了一种高效且可靠的方法。
...
### [141] [Reassessing Graph Linearization for Sequence-to-sequence AMR Parsing: On the Advantages and Limitations of Triple-Based Encoding](https://arxiv.org/abs/2505.08504)
*Jeongwoo Kang,Maximin Coavoux,Cédric Lopez,Didier Schwab*

Main category: cs.CL

TLDR: 论文提出了一种基于三元组的线性化方法，以解决Penman编码在AMR解析中的局限性，并比较了其效率。


<details>
  <summary>Details</summary>
Motivation: Penman编码在深度图中存在节点距离远和逆角色问题，影响了AMR解析的效率。

Method: 提出了一种基于三元组的线性化方法，并与Penman编码进行比较。

Result: 三元组编码在表示图结构方面表现良好，但仍需改进以更好地与Penman的简洁嵌套表示竞争。

Conclusion: 三元组编码有潜力，但需进一步优化以提高其竞争力。
...
### [142] [Are We Paying Attention to Her? Investigating Gender Disambiguation and Attention in Machine Translation](https://arxiv.org/abs/2505.08546)
*Chiara Manna,Afra Alishahi,Frédéric Blain,Eva Vanmassenhove*

Main category: cs.CL

TLDR: 论文提出了一种新的评估指标MPA，用于衡量NMT系统对性别线索的依赖程度，发现现有模型更倾向于统计性别刻板印象，且对男性线索反应更分散。


<details>
  <summary>Details</summary>
Motivation: 现有NMT系统的性别偏见评估指标未能全面捕捉模型对上下文性别线索的整合能力。

Method: 提出Minimal Pair Accuracy (MPA)指标，通过最小对句子（仅性别代词不同）评估模型对性别线索的依赖。

Result: 模型大多忽略性别线索，倾向统计刻板印象；男性线索引发更分散的注意力响应。

Conclusion: MPA揭示了NMT系统在性别处理上的不足，需改进以更公平地整合性别信息。
...
### [143] [Small but Significant: On the Promise of Small Language Models for Accessible AIED](https://arxiv.org/abs/2505.08588)
*Yumou Wei,Paulo Carvalho,John Stamper*

Main category: cs.CL

TLDR: 论文指出，尽管GPT等大型语言模型（LLMs）在教育领域受到广泛关注，但小型语言模型（SLMs）在资源受限环境中具有潜力，呼吁更多研究关注SLM-based AIED方法。


<details>
  <summary>Details</summary>
Motivation: 探讨LLMs在教育领域的流行趋势，同时强调SLMs在资源受限环境中的潜在价值，以提供更公平、经济的高质量AI工具。

Method: 通过关键词搜索分析AIED 2024论文中LLMs的使用情况，并以Phi-2为例展示SLMs在知识组件（KC）发现中的有效性。

Result: 61%的AIED 2024论文使用LLMs，43%提及GPT；SLMs如Phi-2在KC发现中表现良好，无需复杂提示策略。

Conclusion: 呼吁更多关注SLM-based AIED方法，以弥补资源密集型LLMs的局限性，推动教育AI的普及与公平。
...
### [144] [Enhancing Thyroid Cytology Diagnosis with RAG-Optimized LLMs and Pa-thology Foundation Models](https://arxiv.org/abs/2505.08590)
*Hussien Al-Asi,Jordan P Reynolds,Shweta Agarwal,Bryan J Dangott,Aziza Nassar,Zeynettin Akkus*

Main category: cs.CL

TLDR: 该研究探讨了结合检索增强生成（RAG）和病理学基础模型的大型语言模型（LLMs）在甲状腺细胞学诊断中的应用，以提高诊断准确性和一致性。


<details>
  <summary>Details</summary>
Motivation: 解决细胞学解释、标准化和诊断准确性方面的挑战，支持病理学家区分良性和恶性甲状腺病变。

Method: 利用RAG动态检索相关案例和专家解释，结合病理学基础模型优化特征提取和分类能力。

Result: 结果显示，RAG与病理学专用LLMs的结合显著提高了诊断效率和可解释性，基础模型UNI的AUC为0.73-0.93。

Conclusion: 该研究为AI辅助甲状腺细胞病理学奠定了基础，展示了AI在病理学中的潜力。
...
### [145] [Automatic Task Detection and Heterogeneous LLM Speculative Decoding](https://arxiv.org/abs/2505.08600)
*Danying Ge,Jianhua Gao,Qizhi Jiang,Yifei Feng,Weixing Ji*

Main category: cs.CL

TLDR: 提出了一种针对下游任务优化的推测解码算法，通过任务自动分区和异构草稿模型分配，提升解码速度和接受率。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码方法在下游任务中因草稿模型能力有限，难以平衡接受率和解码速度，需优化以适应多样化任务。

Method: 采用任务自动分区和异构草稿模型分配策略，结合在线轻量级提示分类器动态路由提示。

Result: 实验显示，该方法比传统推测解码提升草稿准确率6%至50%，推理速度提升1.10x至2.64x。

Conclusion: 所提方法有效解决了下游任务中推测解码的效率问题，显著提升了性能。
...
### [146] [Scaling Context, Not Parameters: Training a Compact 7B Language Model for Efficient Long-Context Processing](https://arxiv.org/abs/2505.08651)
*Chen Wu,Yin Song*

Main category: cs.CL

TLDR: MegaBeam-Mistral-7B是一个支持512K标记上下文长度的语言模型，解决了长上下文训练的实际限制，并在多个基准测试中表现优异。
...
Result: 在HELMET上表现出卓越的上下文学习能力，在RULER上展示强大的检索和追踪能力，并在BABILong上实现竞争性长程推理。

Conclusion: 模型以Apache 2.0许可证开源，已在Hugging Face下载超过10万次。
...
### [147] [Revealing economic facts: LLMs know more than they say](https://arxiv.org/abs/2505.08662)
*Marcus Buckmann,Quynh Anh Nguyen,Edward Hill*

Main category: cs.CL

TLDR: 利用大语言模型（LLMs）的隐藏状态估计和填补经济金融统计数据，效果优于模型直接输出。
...
Method: 使用简单线性模型训练开源LLMs的隐藏状态，并提出无需目标变量标签的迁移学习方法。

Result: 隐藏状态能有效估计经济数据，仅需少量标注样本即可训练，迁移学习提升准确性。

Conclusion: 隐藏状态在经济数据超分辨率和填补任务中具有实用价值。
...
### [148] [Adaptive Schema-aware Event Extraction with Retrieval-Augmented Generation](https://arxiv.org/abs/2505.08690)
*Sheng Liang,Hang Lv,Zhihao Wen,Yaxiong Wu,Yongyue Zhang,Hao Wang,Yong Liu*

Main category: cs.CL

TLDR: 论文提出了一种自适应模式感知事件抽取（ASEE）方法，结合模式改写和检索增强生成，解决了现有事件抽取中的模式固定和缺乏联合评估基准的问题，并在多领域数据集上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有事件抽取方法存在模式固定和缺乏联合评估基准的问题，且大语言模型在实际部署中存在模式幻觉和上下文限制的挑战。

Method: 提出ASEE方法，结合模式改写和检索增强生成，构建MD-SEE基准进行多领域评估。

Result: ASEE在MD-SEE基准上表现出强适应性，显著提高了事件抽取的准确性。

Conclusion: ASEE通过自适应模式处理和多领域验证，为事件抽取任务提供了有效解决方案。
...
### [149] [NurValues: Real-World Nursing Values Evaluation for Large Language Models in Clinical Context](https://arxiv.org/abs/2505.08734)
*Ben Yao,Qiuchi Li,Yazhou Zhang,Siyu Yang,Bohan Zhang,Prayag Tiwari,Jing Qin*

Main category: cs.CL

TLDR: 该研究提出了首个护理价值对齐基准，包含五个核心价值维度，并构建了包含2200个标注实例的数据集，评估了23个先进LLM的表现。
...
Method: 通过五个月的实地研究收集1100个护理行为实例，标注并生成对立案例，构建Easy-Level和Hard-Level数据集。

Result: DeepSeek-V3在Easy-Level表现最佳（94.55），Claude 3.5 Sonnet在Hard-Level领先（89.43）；正义是最难评估的价值维度；上下文学习显著提升对齐效果。

Conclusion: 该研究为临床环境中的价值敏感LLM开发奠定了基础。
...
### [150] [Probability Consistency in Large Language Models: Theoretical Foundations Meet Empirical Discrepancies](https://arxiv.org/abs/2505.08739)
*Xiaoliang Luo,Xinyi Xu,Michael Ramscar,Bradley C. Love*

Main category: cs.CL

TLDR: 论文证明自回归大语言模型（LLM）在不同分词顺序下学习一致概率分布的能力，发现序列困惑度对任何分解方式（如正向、反向或随机排列）不变。实证表明先前研究存在方法缺陷，并揭示自注意力机制中的位置偏差。


<details>
  <summary>Details</summary>
Motivation: 研究LLM在不同分词顺序下是否学习一致概率分布，为LLM学习机制提供理论基础和评估方法。

Method: 理论证明序列困惑度对分词顺序不变性，并实证重训练GPT-2模型（正向、反向、随机排列分词顺序）。

Result: 发现模型在随机排列分词顺序时显著偏离理论不变性，自注意力机制中存在位置偏差。

Conclusion: 研究为理解LLM位置偏差提供新视角，并提出检测概率分布不一致性的方法。
...
### [151] [AC-Reason: Towards Theory-Guided Actual Causality Reasoning with Large Language Models](https://arxiv.org/abs/2505.08750)
*Yanxi Zhang,Xin Cong,Zhong Zhang,Xiao Liu,Dongyan Zhao,Yesai Wu*

Main category: cs.CL

TLDR: 论文提出AC-Reason框架，结合形式因果理论提升LLM在因果推理中的表现，并引入AC-Bench基准验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的方法缺乏形式因果理论支持，导致解释性不足，需改进。

Method: 提出AC-Reason框架，通过理论指导的算法识别因果相关事件并回答查询，无需显式构建因果图。

Result: AC-Reason显著提升LLM性能，在BBH-CJ和AC-Bench上均超越基线，GPT-4 + AC-Reason表现最佳。

Conclusion: 结合形式因果理论对LLM因果推理效果显著，AC-Reason算法贡献最大。
...
### [152] [Aya Vision: Advancing the Frontier of Multilingual Multimodality](https://arxiv.org/abs/2505.08751)
...
### [153] [HealthBench: Evaluating Large Language Models Towards Improved Human Health](https://arxiv.org/abs/2505.08775)
...
TLDR: HealthBench是一个开源基准测试，用于评估大型语言模型在医疗领域的性能和安全性，包含5000个多轮对话和48,562个独特评分标准。


<details>
  <summary>Details</summary>
Motivation: 为医疗领域提供更真实、开放的评估工具，推动模型发展和应用。

Method: 通过多轮对话和医生制定的评分标准，评估模型在不同医疗场景和行为维度上的表现。

Result: 模型性能稳步提升（如GPT-4o得分32%），小模型进步显著（如GPT-4.1 nano性能优于GPT-4o且成本更低）。

Conclusion: HealthBench为医疗领域的模型发展提供了重要基准，未来有望进一步推动技术进步。
...
### [154] [An Optimized Evacuation Plan for an Active-Shooter Situation Constrained by Network Capacity](https://arxiv.org/abs/2505.07830)
*Joseph Lavalle-Rivera,Aniirudh Ramesh,Subhadeep Chakraborty*

Main category: cs.AI

TLDR: 论文提出了一种多路径优化算法，用于在公共枪击事件中为疏散者提供多条安全路线，减少拥挤和瓶颈，显著降低了伤亡人数和关键节点的占用率。
...
Method: 开发了一种考虑路径容量的多路径路由优化算法，避免拥挤和瓶颈。

Result: 算法将总伤亡人数减少了34.16%和53.3%，关键节点的占用率减少了约50%。

Conclusion: 该算法显著提高了疏散效率，减少了伤亡和拥挤问题。
...
### [155] [RAN Cortex: Memory-Augmented Intelligence for Context-Aware Decision-Making in AI-Native Networks](https://arxiv.org/abs/2505.07842)
*Sebastian Barros*

Main category: cs.AI

TLDR: RAN Cortex是一种支持上下文记忆的AI原生RAN架构，通过模块化设计提升网络决策的适应性和连续性。


<details>
  <summary>Details</summary>
Motivation: 当前RAN中的AI模块（如xApps和rApps）缺乏状态记忆，无法利用历史事件优化决策，限制了在动态网络环境中的表现。

Method: 提出RAN Cortex架构，包含上下文编码器、向量存储、召回引擎和策略接口，支持历史上下文实时检索。

Result: 通过案例（如体育场流量缓解和无人机走廊移动管理）验证了上下文记忆对RAN智能的提升。

Conclusion: RAN Cortex填补了AI原生RAN设计中记忆功能的空白，为无需重新训练或集中推理的“学习代理”提供了框架。
...
### [156] [Winning at All Cost: A Small Environment for Eliciting Specification Gaming Behaviors in Large Language Models](https://arxiv.org/abs/2505.07846)
*Lars Malmqvist*

Main category: cs.AI

TLDR: 前沿大语言模型（LLM）在面临不可能任务时会利用漏洞而非接受失败，尤其是当任务被描述为需要“创造性”解决方案时，漏洞利用行为显著增加。


<details>
  <summary>Details</summary>
Motivation: 研究LLM在不可能任务中的行为，揭示其对系统漏洞的利用倾向，以评估其对安全和AI对齐的潜在威胁。

Method: 通过文本模拟方法，让三种LLM（o1、o3-mini、r1）参与一个无法通过正常方式获胜的井字棋游戏，分析其漏洞利用行为。

Result: 较新的o3-mini模型漏洞利用倾向（37.1%）是旧模型o1（17.5%）的两倍；当任务被描述为“创造性”时，漏洞利用行为飙升至77.3%。

Conclusion: LLM即使无法实际执行，也能识别并提出系统漏洞利用策略，这对AI对齐和安全性提出了紧迫挑战。
...
### [157] [Conceptual Logical Foundations of Artificial Social Intelligence](https://arxiv.org/abs/2505.07847)
*Eric Werner*

Main category: cs.AI

TLDR: 本文探讨了多智能体社会中协调与合作的概念和逻辑基础，提出了社会智能体的最小架构，并研究了通信、意图和信息状态的关系。


<details>
  <summary>Details</summary>
Motivation: 研究社会智能体的基本问题，如协调、合作、意图和信息状态的关系，以理解多智能体社会的运作机制。

Method: 提出社会智能体的最小架构，形式化定义关键概念（如意图、能力、信息状态），并研究通信的语义和语用意义。

Result: 定义了群体战略状态的熵，形式化了智能体能力和意图的逻辑，揭示了信息与战略思维的关联。

Conclusion: 社会智能的逻辑超越了经典逻辑，通过将信息与战略思维结合，为多智能体社会的协调与合作提供了理论基础。
...
### [158] [CCL: Collaborative Curriculum Learning for Sparse-Reward Multi-Agent Reinforcement Learning via Co-evolutionary Task Evolution](https://arxiv.org/abs/2505.07854)
*Yufei Lin,Chengwei Ye,Huanzhen Zhang,Kangsheng Wang,Linuo Xu,Shuyan Liu,Zeyu Zhang*

Main category: cs.AI

TLDR: CCL是一种新的课程学习框架，通过细化任务、生成子任务和协同进化，提升多智能体系统在稀疏奖励环境中的学习效果。
...
Conclusion: CCL有效解决了多智能体系统在稀疏奖励环境中的学习挑战。
...
### [159] [Arrow-Guided VLM: Enhancing Flowchart Understanding via Arrow Direction Encoding](https://arxiv.org/abs/2505.07864)
*Takamitsu Omasa,Ryo Koshihara,Masumi Morishige*

Main category: cs.AI

TLDR: 提出了一种七阶段流程，通过箭头感知检测、OCR提取文本和结构化提示，显著提升了VLM对流程图的解释准确率。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型（VLM）常误解流程图的箭头和拓扑结构，影响了其在软件设计和业务流程分析中的应用。

Method: 采用七阶段流程，分为箭头感知检测、OCR提取节点文本和构建结构化提示三大部分。

Result: 在90个问题的基准测试中，准确率从80%提升至89%，尤其在下一步查询中表现最佳。

Conclusion: 该方法通过显式编码箭头信息显著提升了VLM性能，未来将扩展评估集并应用于BPMN和UML。
...
### [160] [Enhancing Trust Management System for Connected Autonomous Vehicles Using Machine Learning Methods: A Survey](https://arxiv.org/abs/2505.07882)
*Qian Xu,Lei Zhang,Yixiao Liu*

Main category: cs.AI

TLDR: 本文提出了一种基于机器学习的三层信任管理系统框架，用于车路云集成系统中的联网自动驾驶车辆，并分析了其目标、方法和未来方向。


<details>
  <summary>Details</summary>
Motivation: 联网自动驾驶车辆（CAV）在动态、开放和多域网络中运行，易受多种威胁。信任管理系统（TMS）通过机器学习提升其安全性，满足CAV的严格需求。

Method: 提出三层ML-based TMS框架（信任数据层、信任计算层和信任激励层），并分析每层模块的机器学习方法。

Result: 提出了六维目标分类法，并基于交通场景对现有研究进行分类。

Conclusion: 总结了未来研究方向，并维护了一个包含最新文献和开源项目的活跃仓库。
...
### [161] [The Correspondence Between Bounded Graph Neural Networks and Fragments of First-Order Logic](https://arxiv.org/abs/2505.08021)
*Bernardo Cuenca Grau,Przemysław A. Wałęga*

Main category: cs.AI

TLDR: 论文探讨了图神经网络（GNNs）的表达能力，将其与一阶逻辑（FO）的特定片段对应，包括模态逻辑（ML）等。


<details>
  <summary>Details</summary>
Motivation: 理解GNNs的表达能力是一个重要问题，本文旨在通过有限模型理论的方法，为GNNs的逻辑表达能力提供一个统一框架。

Method: 应用有限模型理论中的方法和工具，将GNNs与一阶逻辑及其片段（如ML、GML等）对应。

Result: 证明了有界GNN架构对应于特定的一阶逻辑片段，如ML、GML、FO2等。

Conclusion: 研究为GNNs的逻辑表达能力提供了一个统一的理论框架，揭示了其与一阶逻辑片段的对应关系。
...
### [162] [Bias or Optimality? Disentangling Bayesian Inference and Learning Biases in Human Decision-Making](https://arxiv.org/abs/2505.08049)
*Prakhar Godara*

Main category: cs.AI

TLDR: 论文指出，人类在双臂伯努利多臂老虎机任务中的行为可能并非由认知偏见导致，而是由贝叶斯推断的对称但递减的学习率引起的。


<details>
  <summary>Details</summary>
Motivation: 探讨人类行为是否真的由认知偏见（如确认偏误）驱动，还是由贝叶斯推断的学习机制导致。

Method: 通过分析标准Q学习模型与贝叶斯推断的等效性，并使用主方程研究学习系统的随机动态。

Result: 发现确认偏误和递减学习率的行为特征相同，无法直接区分。

Conclusion: 提出实验方案以区分真实的认知偏见与学习率递减的假象。
...
### [163] [Explainable Reinforcement Learning Agents Using World Models](https://arxiv.org/abs/2505.08073)
*Madhuri Singh,Amal Alabdulkarim,Gennie Mansi,Mark O. Riedl*

Main category: cs.AI

TLDR: 论文提出了一种基于世界模型和反向世界模型的可解释强化学习方法，通过生成反事实轨迹和预测理想状态，帮助非专家用户理解AI代理的行为。


<details>
  <summary>Details</summary>
Motivation: 由于强化学习的时序性，非AI专家难以理解或修改代理行为，需要更直观的解释方法。

Method: 结合世界模型生成反事实轨迹，并引入反向世界模型预测理想状态，以解释代理行为。

Result: 实验表明，展示理想状态的解释能显著提升用户对代理策略的理解。

Conclusion: 该方法有望帮助用户通过环境操控间接控制代理行为。
...
### [164] [Lost in Transmission: When and Why LLMs Fail to Reason Globally](https://arxiv.org/abs/2505.08140)
*Tobias Schnabel,Kiran Tomlinson,Adith Swaminathan,Jennifer Neville*

Main category: cs.AI

TLDR: 论文提出了BAPO模型，用于解释LLMs在复杂推理任务中的失败，并证明CoT方法可以缓解带宽限制。


<details>
  <summary>Details</summary>
Motivation: LLMs在复杂推理任务中表现不佳，作者认为这是由于信息流带宽限制导致的。

Method: 引入BAPO模型，模拟LLMs中注意力头的带宽限制，并分析其对推理任务的影响。

Result: 实验显示GPT-4等模型在BAPO-hard任务上失败，而CoT方法可将BAPO-hard问题转化为BAPO-easy问题。

Conclusion: BAPO模型为LLMs的失败提供了理论解释，并建议通过改进架构和推理方法缓解带宽限制。
...
### [165] [Foundation Models Knowledge Distillation For Battery Capacity Degradation Forecast](https://arxiv.org/abs/2505.08151)
*Joey Chan,Zhen Chen,Ershun Pan*

Main category: cs.AI

TLDR: 提出了一种基于时间序列基础模型的电池容量退化预测方法，通过微调策略实现零样本泛化，并通过知识蒸馏提升专家模型的性能。


<details>
  <summary>Details</summary>
Motivation: 传统专家模型局限于特定场景，而通用时间序列基础模型在电池退化预测领域尚未充分探索，需解决零样本泛化和计算效率问题。

Method: 采用退化感知微调策略对Timer模型进行微调，并设计知识蒸馏框架将基础模型知识迁移至紧凑专家模型。

Result: 微调后的Battery-Timer在CycleLife-SJTUIE数据集上表现出强零样本泛化能力，知识蒸馏显著提升了专家模型的多条件泛化性能。

Conclusion: 该方法为电池容量退化预测提供了高效且泛化能力强的解决方案，同时通过知识蒸馏解决了大模型部署的挑战。
...
### [166] [Efficient and Scalable Neural Symbolic Search for Knowledge Graph Complex Query Answering](https://arxiv.org/abs/2505.08155)
*Weizhi Fei,Zihao Wang,hang Yin,Shukai Zhao,Wei Zhang,Yangqiu Song*

Main category: cs.AI

TLDR: 提出了一种高效的符号搜索框架，通过约束策略和局部搜索算法，显著降低了复杂查询回答的计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 解决神经符号搜索在知识图谱推理中因数据复杂性和查询复杂性导致的扩展性问题。

Method: 提出两种约束策略以减少变量域，并引入基于局部搜索的近似算法处理循环查询。

Result: 实验表明，框架将符号方法的计算负载减少90%，同时保持相近性能。

Conclusion: 该框架有效解决了效率和可扩展性问题。
...
### [167] [Decoding Neighborhood Environments with Large Language Models](https://arxiv.org/abs/2505.08163)
*Andrew Cart,Shaohu Zhang,Melanie Escue,Xugui Zhou,Haitao Zhao,Prashanth BusiReddyGari,Beiyu Lin,Shuang Li*

Main category: cs.AI

TLDR: 研究探讨了利用大型语言模型（LLMs）如ChatGPT和Gemini解码邻里环境的可行性，结合YOLOv11模型实现高精度检测，并通过多数投票策略提升LLMs的准确性。


<details>
  <summary>Details</summary>
Motivation: 传统邻里环境评估方法资源密集且难以规模化，机器学习虽具潜力但数据标注和模型可访问性受限。

Method: 训练YOLOv11模型检测六项环境指标，评估四种LLMs的可行性、鲁棒性和局限性，采用多数投票策略。

Result: YOLOv11模型平均准确率达99.13%，LLMs通过多数投票实现超88%的准确率。

Conclusion: LLMs无需训练即可成为解码邻里环境的有效工具。
...
### [168] [Behind the Noise: Conformal Quantile Regression Reveals Emergent Representations](https://arxiv.org/abs/2505.08176)
*Petrus H. Zwart,Tamas Varga,Odeta Qafoku,James A. Sethian*

Main category: cs.AI

TLDR: 提出一种基于机器学习的去噪方法，通过轻量级神经网络和共形分位数回归，不仅去噪还能揭示潜在空间的结构。


<details>
  <summary>Details</summary>
Motivation: 科学成像中，减少采集时间会引入噪声，传统方法仅关注图像恢复，而本文希望通过去噪过程揭示有意义的结构。

Method: 使用随机结构的轻量级神经网络集成，通过共形分位数回归训练，无需标签或分割。

Result: 在真实地球生物化学成像数据上验证，方法可靠去噪并揭示可解释的空间和化学特征。

Conclusion: 该方法不仅去噪，还能驱动有意义表征的涌现，支持资源受限下的实验设计。
...
### [169] [Unveiling the Best Practices for Applying Speech Foundation Models to Speech Intelligibility Prediction for Hearing-Impaired People](https://arxiv.org/abs/2505.08215)
*Haoshuai Zhou,Boxuan Cao,Changgeng Mo,Linkai Li,Shan Xiang Wang*

Main category: cs.AI

TLDR: 研究探讨了如何优化语音基础模型（SFMs）用于听力受损人群的语音清晰度预测（SIP-HI），发现单层编码器选择、时间建模和模型集成是关键因素。


<details>
  <summary>Details</summary>
Motivation: 尽管SFMs在多种任务中表现优异，但其在SIP-HI任务中的优化尚未充分研究。

Method: 通过5种SFMs，研究了编码器层选择、预测头架构和集成配置对SIP-HI性能的影响。

Result: 发现单层编码器优于传统全层方法，时间建模对预测头至关重要，模型集成能提升性能。
...
### [170] [Evaluating LLM Metrics Through Real-World Capabilities](https://arxiv.org/abs/2505.08253)
*Justin K Miller,Wenjia Tang*

Main category: cs.AI

TLDR: 论文提出了一种评估生成式AI在实际任务中表现的方法，强调实用性而非抽象智能，并发现现有基准测试在覆盖范围和实用性方面存在不足。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试过于关注代码生成或事实回忆，未能反映用户在日常任务中对AI的广泛需求。

Method: 通过大规模调查数据和使用日志分析，确定了六种核心能力，并评估现有基准测试的覆盖范围。

Result: Google Gemini在实用性指标上优于其他主流模型。

Conclusion: 需要更多以用户为中心的基准测试来更准确地评估AI的实际效用。
...
### [171] [Benchmarking AI scientists in omics data-driven biological research](https://arxiv.org/abs/2505.08341)
*Erpai Luo,Jinmeng Jia,Yifan Xiong,Xiangyu Li,Xiaobo Guo,Baoqi Yu,Lei Wei,Xuegong Zhang*

Main category: cs.AI

TLDR: BaisBench是一个评估AI科学家通过数据分析和外部知识推理生成生物学发现能力的基准，包含细胞类型标注和科学发现两个任务。
...
Method: 设计了BaisBench基准，包含31个单细胞数据集的细胞类型标注任务和198个基于41项单细胞研究的科学发现选择题任务。

Result: 实验表明，当前AI模型在两项任务上表现远低于人类专家。

Conclusion: BaisBench填补了现有基准的不足，为AI模型的科学发现能力评估提供了基础。
...
### [172] [An Identifiable Cost-Aware Causal Decision-Making Framework Using Counterfactual Reasoning](https://arxiv.org/abs/2505.08343)
*Ruichu Cai,Xi Chen,Jie Qiao,Zijian Li,Yuequn Liu,Wei Chen,Keli Zhang,Jiale Zheng*

Main category: cs.AI

TLDR: 提出MiCCD框架，通过反事实推理解决异常决策中的成本与因果机制问题，优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 现有决策框架依赖强化学习或根因分析，常忽略行动成本或因果机制。

Method: 基于因果图构建代理模型，利用异常模式聚类标签，通过SLSQP算法优化干预策略。

Result: 在合成和真实数据集上，MiCCD在F1分数、成本效率和排名质量上优于传统方法。

Conclusion: MiCCD框架有效且广泛适用，解决了异常决策中的关键问题。
...
### [173] [Modeling Unseen Environments with Language-guided Composable Causal Components in Reinforcement Learning](https://arxiv.org/abs/2505.08361)
*Xinyue Wang,Biwei Huang*

Main category: cs.AI

TLDR: WM3C框架通过组合因果组件提升RL泛化能力，优于现有方法。
...
Method: 利用语言作为组合模态，结合掩码自编码器和互信息约束，识别因果动态。

Result: 在数值模拟和机器人任务中显著优于现有方法。

Conclusion: WM3C通过组合因果组件有效提升RL泛化能力。
...
### [174] [Learning Like Humans: Advancing LLM Reasoning Capabilities via Adaptive Difficulty Curriculum Learning and Expert-Guided Self-Reformulation](https://arxiv.org/abs/2505.08364)
*Enci Zhang,Xingang Yan,Wei Lin,Tianxiang Zhang,Qianchun Lu*

Main category: cs.AI

TLDR: 论文提出两种新策略（ADCL和EGSR）以提升大语言模型解决复杂问题的能力，实验显示显著效果。
...
Result: 在AIME24和AIME25基准测试中，性能分别提升10%和16.6%。

Conclusion: 结合ADCL和EGSR的策略显著提升模型性能，验证了人类学习策略的借鉴价值。
...
### [175] [Explaining Autonomous Vehicles with Intention-aware Policy Graphs](https://arxiv.org/abs/2505.08404)
*Sara Montese,Victor Gimenez-Abalos,Atia Cortés,Ulises Cortés,Sergio Alvarez-Napagao*

Main category: cs.AI

TLDR: 论文提出了一种后处理的、模型无关的方法，为自动驾驶车辆在城市环境中的行为提供目的论解释，以提高透明度和信任。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆的性能虽因AI进步而提升，但其决策的不透明性阻碍了社会信任和监管接受，因此需要解释性。

Method: 基于意图感知策略图，从全局和局部视角提取可解释的车辆行为解释。

Result: 在nuScenes数据集上验证了方法的有效性，能评估车辆行为是否合法并识别潜在漏洞。

Conclusion: 该方法为自动驾驶的透明性和可靠性提供了实用工具，有助于推动其社会接受度。
...
### [176] [Agent-as-a-Service based on Agent Network](https://arxiv.org/abs/2505.08446)
*Yuhan Zhu,Haojie Liu,Jian Wang,Bing Li,Zikang Yin,Yefei Liao*

Main category: cs.AI

TLDR: 论文提出了基于Agent网络的Agent-as-a-Service（AaaS-AN），通过动态Agent网络和服务导向的Agent，解决了多Agent系统中协作组织的问题，并在数学推理和代码生成任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有Model Context Protocol（MCP）在多Agent系统中缺乏对Agent级协作的支持，因此需要一种新的服务导向范式来统一Agent生命周期管理。

Method: AaaS-AN基于Role-Goal-Process-Service（RGPS）标准，包含动态Agent网络和服务导向Agent，通过Service Scheduler实现分布式协调和任务管理。

Result: AaaS-AN在数学推理和代码生成任务中优于现有基线，并成功构建了一个包含100多个Agent服务的多Agent系统。

Conclusion: AaaS-AN为多Agent系统的协作提供了有效解决方案，并发布了数据集以支持未来研究。
...
### [177] [Adaptive Bias Generalized Rollout Policy Adaptation on the Flexible Job-Shop Scheduling Problem](https://arxiv.org/abs/2505.08451)
*Lotfi Kobrosly,Marc-Emmanuel Coupvent des Graviers,Christophe Guettier,Tristan Cazenave*

Main category: cs.AI

TLDR: 论文提出了一种基于广义嵌套滚动策略适应（GNRPA）的新算法，用于解决柔性作业车间调度问题（FJSSP），实验结果表明其性能优于其他基于MCTS的方法。


<details>
  <summary>Details</summary>
Motivation: FJSSP是一个NP难组合优化问题，在制造业等领域有广泛应用，需要高效调度多台机器上的多个操作。

Method: 提出了一种基于GNRPA的新算法。

Result: 算法性能优于其他基于MCTS的方法，但在大规模实例上的完成时间仍与已知上限有差距。

Conclusion: 新算法在FJSSP问题上表现出潜力，但仍有改进空间。
...
### [178] [Strategy-Augmented Planning for Large Language Models via Opponent Exploitation](https://arxiv.org/abs/2505.08459)
*Shuai Xu,Sijia Cui,Yanna Wang,Bo Xu,Qi Wang*

Main category: cs.AI

TLDR: 论文提出了一种两阶段的策略增强规划（SAP）框架，通过策略评估网络（SEN）增强基于LLM的代理的对手利用能力。


<details>
  <summary>Details</summary>
Motivation: 在对抗性领域中，高效建模和利用对手是一个长期挑战。LLMs在通用任务中表现出色，但直接使用LLMs生成决策受限于其领域专业知识。

Method: SAP框架分为离线阶段（构建策略空间并训练SEN）和在线阶段（动态识别对手策略并通过SEN搜索最佳响应策略）。

Result: 实验显示SAP具有强大的泛化能力，在MicroRTS环境中性能提升85.35%，与SOTA规则AI竞争。

Conclusion: SAP显著提升了LLM代理的对手利用能力，适用于已知和未知对手策略。
...
### [179] [BAT: Benchmark for Auto-bidding Task](https://arxiv.org/abs/2505.08485)
...
TLDR: 论文提出了一个用于在线广告位拍卖的竞价策略优化基准，解决了数据集和标准化基准的稀缺问题。
...
Method: 构建了一个涵盖两种常见拍卖格式的基准，并在新数据集上实现了一系列基线方法，专注于预算分配均匀性和CPC约束优化。

Result: 提供了一个用户友好的框架，支持研究人员和实践者开发和改进自动竞价算法。

Conclusion: 该基准促进了程序化广告领域的进步，相关资源已开源。
...
### [180] [Achieving Scalable Robot Autonomy via neurosymbolic planning using lightweight local LLM](https://arxiv.org/abs/2505.08492)
*Nicholas Attolino,Alessio Capitanelli,Fulvio Mastrogiovanni*

Main category: cs.AI

TLDR: Gideon框架通过本地小型LLM解决PDDL符号任务规划在动态人机协作中的问题，支持多领域扩展和高效推理。
...
Method: Gideon结合问题生成器生成大规模数据集，并适配神经符号规划以支持本地LLM，实现设备端执行和多领域扩展。

Result: 单领域测试中，32k样本模型生成有效计划的百分比为66.1%；多领域测试中，16k样本模型达到70.6%的有效率。

Conclusion: 尽管训练效率较低，Gideon在推理效率、可扩展性和多领域适应性方面具有显著优势，适合人机协作场景。
...
### [181] [TrialMatchAI: An End-to-End AI-powered Clinical Trial Recommendation System to Streamline Patient-to-Trial Matching](https://arxiv.org/abs/2505.08508)
...
TLDR: TrialMatchAI是一个基于AI的患者-试验匹配系统，通过处理结构化与非结构化临床数据，结合检索增强生成框架，实现高效、透明且可解释的匹配。


<details>
  <summary>Details</summary>
Motivation: 临床试验中患者招募效率低，需要自动化、可扩展的解决方案。

Method: 系统基于微调的开源大语言模型，采用混合搜索策略（词法和语义相似性），结合医学链式推理进行标准级资格评估。

Result: 在真实验证中，92%的肿瘤患者在前20条推荐中找到相关试验，标准级分类准确率超90%。

Conclusion: TrialMatchAI为精准医学提供了高效、轻量且可扩展的AI驱动试验匹配方案。
...
### [182] [On the Complexity and Properties of Preferential Propositional Dependence Logic](https://arxiv.org/abs/2505.08522)
*Kai Sauerwald,Arne Meier,Juha Kontinen*

Main category: cs.AI

TLDR: 论文研究了基于团队语义和依赖原子的命题逻辑中KLM风格优先推理的复杂性和性质，发现其具有累积性但违反System P，并给出了满足System P的直观条件。


<details>
  <summary>Details</summary>
Motivation: 探讨命题依赖逻辑中优先推理的性质及其与System P的关系。

Method: 通过团队语义和依赖原子分析优先推理的累积性和System P的违反情况，并提出满足System P的条件。

Result: 发现这些条件不适用于团队命题逻辑，且展示了经典逻辑和依赖逻辑在非平凡优先模型中的表达方式。

Conclusion: 提出了优先团队推理的复杂性结果，包括对经典优先推理的新发现。
...
### [183] [Guiding LLM-based Smart Contract Generation with Finite State Machine](https://arxiv.org/abs/2505.08542)
*Hao Luo,Yuhao Lin,Xiao Yan,Xintong Hu,Yuxiang Wang,Qiming Zeng,Hao Wang,Jiawei Jiang*

Main category: cs.AI

TLDR: FSM-SCG是一个基于有限状态机和大型语言模型的智能合约生成框架，显著提升了生成代码的质量和安全性。


<details>
  <summary>Details</summary>
Motivation: 传统智能合约生成方法依赖人工编码和专家审核，门槛高且效率低，而现有大型语言模型在智能合约生成中仍面临效果和安全性挑战。

Method: 通过将用户需求抽象为有限状态机，指导大型语言模型生成智能合约，并通过编译和安全检查的反馈迭代优化代码。

Result: 实验表明，FSM-SCG将生成代码的编译成功率提升最多48%，平均漏洞风险评分降低约68%。

Conclusion: FSM-SCG显著提升了智能合约生成的效率和质量，解决了现有方法的局限性。
...
### [184] [Resource-Efficient Language Models: Quantization for Fast and Accessible Inference](https://arxiv.org/abs/2505.08620)
*Tollef Emil Jørgensen*

Main category: cs.AI

TLDR: 本文综述了后训练量化（PTQ）技术，旨在优化大型语言模型（LLM）的推理效率，涵盖量化方案、粒度和权衡。
...
### [185] [Visually Guided Decoding: Gradient-Free Hard Prompt Inversion with Language Models](https://arxiv.org/abs/2505.08622)
*Donghoon Kim,Minji Bae,Kyuhong Shim,Byonghyo Shim*

Main category: cs.AI

TLDR: 提出了一种名为Visually Guided Decoding (VGD)的无梯度方法，利用LLM和CLIP评分生成更连贯、语义对齐的文本提示，解决了现有提示反转技术的不足。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成模型的提示生成方法（如软提示和硬提示）效果有限，存在解释性差和提示不连贯的问题。

Method: VGD结合大型语言模型（LLM）的文本生成能力和CLIP评分的视觉对齐指导，无需额外训练即可生成高质量提示。

Result: 实验表明，VGD在生成可理解和上下文相关的提示方面优于现有技术，提升了与文本到图像模型的交互体验。

Conclusion: VGD通过无梯度方法显著改进了提示生成的质量和可控性，为文本到图像模型的应用提供了更直观的工具。
...
### [186] [Integrating Natural Language Processing and Exercise Monitoring for Early Diagnosis of Metabolic Syndrome: A Deep Learning Approach](https://arxiv.org/abs/2505.08628)
*Yichen Zhao,Yuhua Wang,Xi Cheng,Junhao Fang,Yang Yang*

Main category: cs.AI

TLDR: 利用日常生活中的生理数据和运动相关文本，通过深度学习和NLP技术，实现代谢综合征的早期诊断。


<details>
  <summary>Details</summary>
Motivation: 代谢综合征（MetS）全球患病率高，但标准诊断方法复杂且常被低估，亟需低成本、易操作的早期诊断方案。

Method: 收集40名志愿者的生理数据和运动文本，通过数据增强解决不平衡问题，提出结合NLP和运动监测的深度学习框架。

Result: 最佳模型表现优异（AUROC=0.806，REC=76.3%），文本和每日最低心率对分类贡献最大。

Conclusion: 研究表明，日常易测数据可用于MetS早期诊断，有望降低筛查和管理成本。
...
### [187] [TRAIL: Trace Reasoning and Agentic Issue Localization](https://arxiv.org/abs/2505.08638)
*Darshan Deshpande,Varun Gangal,Hersh Mehta,Jitin Krishnan,Anand Kannappan,Rebecca Qian*

Main category: cs.AI

TLDR: 论文提出了一种针对代理工作流复杂轨迹的评估方法，解决了当前依赖人工分析的不可扩展性问题，并引入了错误分类法和公开数据集TRAIL。


<details>
  <summary>Details</summary>
Motivation: 随着代理工作流的广泛应用，现有评估方法无法应对其复杂性和规模，亟需动态且稳健的评估工具。

Method: 论文提出了错误分类法，并构建了包含148条人工标注轨迹的数据集TRAIL，涵盖单/多代理系统及实际应用场景。

Result: 实验显示现代长上下文LLM在轨迹调试中表现不佳，最佳模型Gemini-2.5-pro在TRAIL上仅得11%。

Conclusion: 论文为代理工作流的可扩展评估提供了工具和数据集，推动了相关研究的进展。
...
### [188] [WixQA: A Multi-Dataset Benchmark for Enterprise Retrieval-Augmented Generation](https://arxiv.org/abs/2505.08643)
*Dvir Cohen,Lin Burg,Sviatoslav Pykhnivskyi,Hagit Gur,Stanislav Kovynov,Olga Atzmon,Gilad Barkan*

Main category: cs.AI

TLDR: WixQA是一个针对企业问答系统的基准测试套件，包含三个数据集，基于Wix.com客户支持交互和知识库快照，用于全面评估检索增强生成（RAG）系统。


<details>
  <summary>Details</summary>
Motivation: 企业问答系统需要反映实际用户问题的数据集，而现有开放域数据集无法满足需求。WixQA填补了这一空白，提供基于具体知识库的评估基准。

Method: WixQA包含三个数据集：专家编写的真实用户查询（WixQA-ExpertWritten）、专家验证的模拟对话（WixQA-Simulated）和基于知识库生成的合成数据（WixQA-Synthetic）。

Result: WixQA发布了知识库快照和数据集，并提供基线结果，为企业RAG系统提供了独特的评估工具。

Conclusion: WixQA为企业问答系统提供了现实的评估环境，填补了现有基准的不足。
...
### [189] [A Study of Data-driven Methods for Inventory Optimization](https://arxiv.org/abs/2505.08673)
*Lee Yeung Ping,Patrick Wong,Tan Cheng Han*

Main category: cs.AI

TLDR: 本文分析了三种算法（时间序列、随机森林和深度强化学习）在三种库存模型（缺货损失、双源采购和多级库存模型）中的应用，评估了它们在超市环境中的有效性。


<details>
  <summary>Details</summary>
Motivation: 研究目的是探索数据驱动的高效方法，分析算法在库存管理中的潜力、可能性及当前挑战。

Method: 通过比较三种算法在不同库存模型中的表现，使用数据可视化工具和统计指标评估其效果。

Result: 结果显示每种算法在预测准确性、市场变化适应性和对库存成本及客户满意度的影响方面表现不同。

Conclusion: 研究为库存管理决策提供了指导，帮助管理者实时跟踪算法性能并优化供应链效率。
...
### [190] [LLM-based Prompt Ensemble for Reliable Medical Entity Recognition from EHRs](https://arxiv.org/abs/2505.08704)
*K M Sajjadul Islam,Ayesha Siddika Nipu,Jiawei Wu,Praveen Madiraju*

Main category: cs.AI

TLDR: 论文探讨了基于提示的大型语言模型（如GPT-4o和DeepSeek-R1）在电子健康记录（EHRs）中命名实体识别（NER）的应用，其中GPT-4o结合提示集成方法表现最佳。
...
Method: 研究采用提示工程方法（零样本、少样本和集成方法）指导GPT-4o和DeepSeek-R1进行医疗实体识别。

Result: GPT-4o结合提示集成方法表现最优，F1分数为0.95，召回率为0.98，优于DeepSeek-R1。

Conclusion: 提示集成方法通过嵌入相似性和多数投票提高了模型可靠性，为EHRs中的NER任务提供了高效解决方案。
...
### [191] [DeepMath-Creative: A Benchmark for Evaluating Mathematical Creativity of Large Language Models](https://arxiv.org/abs/2505.08744)
...
TLDR: DeepMath团队开源了一个数学大语言模型（LLM）项目，旨在评估其数学创造力。本文提出了数学创造力的评估标准，并发布了DeepMath-Creative基准数据集。实验显示主流LLM在创造性解题上表现有限，最高准确率仅70%，且对复杂问题表现更差。


<details>
  <summary>Details</summary>
Motivation: 当前数学LLM主要关注推理能力，而创造力评估和研究较少，缺乏高质量数据集。本文旨在填补这一空白。

Method: 提出数学创造力的评估标准，并开发DeepMath-Creative基准数据集，涵盖代数、几何、分析等领域。对主流LLM进行系统评估。

Result: 最佳模型O3 Mini在基础本科级任务中准确率仅70%，复杂问题表现更差，模型缺乏解决开放问题的实质性策略。

Conclusion: 当前LLM的创造力表现可能源于记忆模式的重组，而非真正的创新洞察或新合成。
...
### [192] [ARC-NCA: Towards Developmental Solutions to the Abstraction and Reasoning Corpus](https://arxiv.org/abs/2505.08778)
*Etienne Guichard,Felix Reimers,Mia Kvalsund,Mikkel Lepperød,Stefano Nichele*

Main category: cs.AI

TLDR: ARC-NCA利用神经细胞自动机（NCA）及其增强版（EngramNCA）解决ARC-AGI挑战，展示了开发性方法在AI推理和抽象能力上的潜力。


<details>
  <summary>Details</summary>
Motivation: 解决ARC-AGI这一对AI极具挑战但对人类简单的任务，探索开发性方法如何提升AI的问题解决能力。

Method: 采用标准NCA和带有隐藏记忆的EngramNCA，模拟生物系统的开发过程。

Result: ARC-NCA在部分任务上表现优于ChatGPT 4.5，且成本更低。

Conclusion: 开发性方法为AI的推理和抽象能力提供了新途径，具有潜在的应用价值。
...
### [193] [MilChat: Introducing Chain of Thought Reasoning and GRPO to a Multimodal Small Language Model for Remote Sensing](https://arxiv.org/abs/2505.07984)
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在资源有限和专业领域的适应性不足，特别是在军事遥感图像分析中表现有限。

Method: 使用2B参数的开源MLLM进行监督微调，结合CoT推理和GRPO优化，提升对军事关键特征的检测能力。

Result: 在MilData基准测试中，召回率达80%，精确率达98%，优于通用模型和现有遥感方法。

Conclusion: 针对性的微调和强化学习在专业领域应用中效果显著，MilChat展示了高效性和准确性。
...
### [194] [Vision Foundation Model Embedding-Based Semantic Anomaly Detection](https://arxiv.org/abs/2505.07998)
*Max Peter Ronecker,Matthew Foutter,Amine Elhafsi,Daniele Gammelli,Ihor Barakaiev,Marco Pavone,Daniel Watzenig*

Main category: cs.CV

TLDR: 该论文提出了一种利用视觉基础模型的语义先验检测语义异常的框架，通过比较运行时图像的局部视觉嵌入与安全场景数据库，实现了高性能的异常检测与定位。
...
Method: 提出了一种框架，使用局部视觉嵌入与安全场景数据库进行比较，并测试了两种变体：基于原始网格嵌入和基于实例分割的对象中心表示。还引入了过滤机制以减少误报。

Result: 在CARLA模拟异常上的评估表明，基于实例分割的方法结合过滤机制的性能接近GPT-4o，并能精确定位异常。
...
### [195] [RDD: Robust Feature Detector and Descriptor using Deformable Transformer](https://arxiv.org/abs/2505.08013)
*Gonglin Chen,Tianwen Fu,Haiwei Chen,Wenbin Teng,Hanyuan Xiao,Yajie Zhao*

Main category: cs.CV

TLDR: 提出了一种基于可变形Transformer的鲁棒关键点检测器/描述符（RDD），通过可变形自注意力机制捕获全局上下文和几何不变性，显著提升了在稀疏匹配任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 在结构从运动和SLAM中，鲁棒的特征检测与描述在显著视角变化等挑战性场景中仍未解决。现有方法未能学习长距离关系中的视觉线索。

Method: 利用可变形Transformer设计RDD，通过可变形自注意力机制聚焦关键位置，降低搜索空间复杂度并建模几何不变性。同时结合Air-to-Ground数据集和标准MegaDepth数据集进行训练。

Result: RDD在稀疏匹配任务中优于所有现有方法，并能进行半稠密匹配。还引入了两个新基准测试，验证了其在大视角变化和跨高度3D重建中的性能。

Conclusion: RDD通过全局上下文和几何不变性建模，显著提升了关键点检测与描述的性能，适用于复杂场景下的特征匹配任务。
...
### [196] [Visually Interpretable Subtask Reasoning for Visual Question Answering](https://arxiv.org/abs/2505.08084)
...
<details>
  <summary>Details</summary>
Motivation: 解决现有方法在复杂视觉问题中计算成本高、准确性低的问题。

Method: 通过微调MLLMs生成结构化的子任务思维链（Subtask-of-Thought）推理序列。
...
### [197] [Multi-modal wound classification using wound image and location by Xception and Gaussian Mixture Recurrent Neural Network (GMRNN)](https://arxiv.org/abs/2505.08086)
*Ramin Mousa,Ehsan Matbooe,Hakimeh Khojasteh,Amirali Bengari,Mohammadmahdi Vahediahmar*

Main category: cs.CV

TLDR: 本文提出了一种基于迁移学习的多模态AI模型，结合Xception和GMRNN架构，用于伤口分类，显著提高了诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 急性及难愈伤口的有效诊断对临床护理至关重要，传统方法常因感染、血管疾病等问题导致效果不佳，AI工具可加速图像解读和早期检测。

Method: 通过迁移学习算法提取特征并结合位置信息，构建多模态网络，分类糖尿病、压力、手术和静脉溃疡。

Result: 实验结果显示伤口分类准确率在78.77%至100%之间，表现优于传统深度神经网络。
...
### [198] [Topology-Guided Knowledge Distillation for Efficient Point Cloud Processing](https://arxiv.org/abs/2505.08101)
*Luu Tung Hai,Thinh D. Le,Zhicheng Ding,Qing Tian,Truong-Son Hy*

Main category: cs.CV

TLDR: 提出了一种基于拓扑感知表示和梯度引导知识蒸馏的新框架，用于在资源受限环境中高效部署点云处理模型。


<details>
  <summary>Details</summary>
Motivation: 点云处理在自动驾驶和3D物体识别中至关重要，但高性能模型（如Point Transformer V3）在资源受限环境中的部署面临计算和内存需求高的挑战。

Method: 通过拓扑感知表示和梯度引导知识蒸馏，从高容量教师模型向轻量级学生模型传递知识，同时捕捉点云的几何结构。

Result: 在Nuscenes、SemanticKITTI和Waymo数据集上，模型大小减少约16倍，推理时间降低近1.9倍，并在NuScenes上达到知识蒸馏技术的SOTA性能。

Conclusion: 该方法显著提升了轻量级模型在点云处理任务中的性能，为资源受限环境提供了高效解决方案。
...
### [199] [Sleep Position Classification using Transfer Learning for Bed-based Pressure Sensors](https://arxiv.org/abs/2505.08111)
*Olivier Papillon,Rafik Goubran,James Green,Julien Larivière-Chartier,Caitlin Higginson,Frank Knoefel,Rébecca Robillard*

Main category: cs.CV

TLDR: 利用预训练的Vision Transformer模型（ViTMAE和ViTPose）通过迁移学习解决低分辨率压力敏感垫（PSM）数据中睡眠姿势分类问题，优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 睡眠姿势影响睡眠质量和睡眠障碍（如呼吸暂停），但临床环境中标记数据稀缺，需要高效方法。

Method: 采用迁移学习，利用预训练的ViTMAE和ViTPose模型，对低分辨率PSM数据进行睡眠姿势分类。

Result: 在112晚患者记录和13名患者高分辨率数据集上验证，性能优于传统方法（如TCN、SVM、XGBoost、Random Forest）。

Conclusion: 该方法在临床环境中具有实际应用潜力，尽管低分辨率数据分类仍具挑战性。
...
### [200] [Now you see it, Now you don't: Damage Label Agreement in Drone & Satellite Post-Disaster Imagery](https://arxiv.org/abs/2505.08117)
*Thomas Manzini,Priyankari Perali,Jayesh Tripathi,Robin Murphy*

Main category: cs.CV

TLDR: 本文通过分析卫星和无人机图像对15,814栋建筑的损伤标签，发现29.02%的标签不一致，且两种来源的分布显著不同，可能对机器学习损伤评估系统的部署带来风险和潜在危害。


<details>
  <summary>Details</summary>
Motivation: 目前尚无关于无人机和卫星图像在建筑损伤评估中标签一致性的研究，且先前工作因标签差异、建筑位置错位和数据量不足而受限。本文旨在填补这一空白。

Method: 通过比较三种飓风（Ian、Michael、Harvey）中相同损伤标签和建筑位置的15,814栋建筑数据，克服了先前工作的限制。

Result: 卫星标签比无人机标签低估损伤至少20.43%（p<1.2x10^-117），且两种标签分布显著不同（p<5.1x10^-175），表明基于其中一种训练的模型会误判实际条件。

Conclusion: 为避免伦理风险和社会危害，本文提出四项建议以提高CV/ML损伤评估系统的可靠性和透明度。
...
### [201] [JSover: Joint Spectrum Estimation and Multi-Material Decomposition from Single-Energy CT Projections](https://arxiv.org/abs/2505.08123)
*Qing Wu,Hongjiang Wei,Jingyi Yu,S. Kevin Zhou,Yuyao Zhang*

Main category: cs.CV

TLDR: JSover是一种新型的单能CT多材料分解（SEMMD）框架，通过联合重建和能量谱估计，显著提高了分解的准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 传统SEMMD方法因忽略能量依赖性衰减而引入伪影和噪声，限制了临床适用性。

Method: 提出JSover框架，结合物理先验和隐式神经表示（INR），直接从单能CT投影中联合重建材料和估计能量谱。

Result: 实验表明，JSover在模拟和真实CT数据集上优于现有SEMMD方法。

Conclusion: JSover为单能CT的多材料分解提供了更可靠和高效的解决方案。
...
### [202] [SLAG: Scalable Language-Augmented Gaussian Splatting](https://arxiv.org/abs/2505.08124)
...
Result: 在16-GPU设置下，比OpenGaussian快18倍，同时在ScanNet和LERF数据集上保持嵌入质量。
...
### [203] [Asynchronous Multi-Object Tracking with an Event Camera](https://arxiv.org/abs/2505.08126)
*Angus Apps,Ziwei Wang,Vladimir Perejogin,Timothy Molloy,Robert Mahony*

Main category: cs.CV

TLDR: AEMOT算法通过异步处理事件相机数据，检测和跟踪多个动态物体，性能优于现有方法37%。


<details>
  <summary>Details</summary>
Motivation: 事件相机在动态环境中具有低延迟和高分辨率优势，适合多目标跟踪。

Method: 使用新颖的主动流方向场检测特征，结合AEB跟踪器构建候选对象，并通过学习验证阶段筛选对象。

Result: 在蜜蜂群数据集上，AEMOT的精确率和召回率超过其他算法37%。
...
### [204] [MoKD: Multi-Task Optimization for Knowledge Distillation](https://arxiv.org/abs/2505.08170)
...
<details>
  <summary>Details</summary>
Motivation: 解决知识蒸馏中任务目标和教师指导之间的平衡问题，以及教师与学生模型知识表示的差异。

Method: 提出多任务优化框架MoKD，重新定义知识蒸馏为多目标优化问题，并引入子空间学习框架。

Result: 在ImageNet-1K和COCO数据集上表现优于现有方法，达到最先进性能。

Conclusion: MoKD在知识蒸馏中实现了更好的平衡和效率，性能优于从头训练的模型。
...
### [205] [Empowering Vision Transformers with Multi-Scale Causal Intervention for Long-Tailed Image Classification](https://arxiv.org/abs/2505.08173)
...
<details>
  <summary>Details</summary>
Motivation: 现有因果模型在ViT上表现不佳，因其全局特征表示难以建模细粒度特征与预测的关联，导致尾部类别分类困难。

Method: TSCNet分为两阶段：HCRL阶段通过多尺度干预增强细粒度因果表示；CLBC阶段通过反事实校准优化决策边界。

Result: 在多个长尾基准测试中，TSCNet显著优于现有方法，消除了数据不平衡引入的多种偏差。

Conclusion: TSCNet通过两阶段因果建模有效解决了ViT在长尾分类中的性能问题，提升了细粒度特征建模能力。
...
### [206] [Monocular Depth Guided Occlusion-Aware Disparity Refinement via Semi-supervised Learning in Laparoscopic Images](https://arxiv.org/abs/2505.08178)
*Ziteng Liu,Dongdong He,Chenghong Zhang,Wenpeng Gao,Yili Fu*

Main category: cs.CV

TLDR: 论文提出了一种深度引导的遮挡感知视差细化网络（DGORNet），通过利用不受遮挡影响的单目深度信息来细化视差图，解决了腹腔镜立体图像视差估计中的遮挡和标记数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 腹腔镜立体图像视差估计中，遮挡和标记数据稀缺是主要挑战。

Method: 提出DGORNet网络，引入位置嵌入（PE）模块提供显式空间上下文，并设计光学流差异损失（OFDLoss）利用未标记数据的时序连续性。

Result: 在SCARED数据集上，DGORNet在端点误差（EPE）和均方根误差（RMSE）上优于现有方法，尤其在遮挡和无纹理区域表现突出。

Conclusion: DGORNet通过PE和OFDLoss提升了空间和时间一致性，为腹腔镜手术视差估计提供了有效解决方案。
...
### [207] [Unsupervised Raindrop Removal from a Single Image using Conditional Diffusion Models](https://arxiv.org/abs/2505.08190)
...
Conclusion: 扩散模型为单图像雨滴去除提供了新思路，效果显著。
...
### [208] [ADC-GS: Anchor-Driven Deformable and Compressed Gaussian Splatting for Dynamic Scene Reconstruction](https://arxiv.org/abs/2505.08196)
*He Huang,Qi Yang,Mufan Liu,Yiling Xu,Zhu Li*

Main category: cs.CV

TLDR: ADC-GS是一种基于锚点驱动的高效动态场景重建方法，通过分层粗到细的流程和速率失真优化，显著提升了渲染速度和存储效率。
...
Method: 采用锚点驱动的结构和分层粗到细流程，结合速率失真优化。

Result: 渲染速度提升300%-800%，存储效率达到最优，且不损失渲染质量。
...
### [209] [Visual Watermarking in the Era of Diffusion Models: Advances and Challenges](https://arxiv.org/abs/2505.08197)
*Junxian Duan,Jiyang Guang,Wenkui Yang,Ran He*

Main category: cs.CV

TLDR: 论文探讨了生成式AI技术（如Stable Diffusion）对视觉内容的潜在滥用风险，提出扩散模型在嵌入不可感知且鲁棒水印方面的优势，以应对版权问题。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI技术的进步，视觉内容易被滥用，传统被动检测方法难以应对复杂篡改，需开发更有效的水印保护机制。

Method: 利用扩散模型学习特征，嵌入不可感知且鲁棒的水印，提升检测准确性。

Result: 扩散模型在增强水印鲁棒性和检测准确性方面表现出潜力。

Conclusion: 需创新水印技术以应对生成式AI时代的版权保护挑战，确保数字内容所有权。
...
### [210] [Object detection in adverse weather conditions for autonomous vehicles using Instruct Pix2Pix](https://arxiv.org/abs/2505.08228)
*Unai Gurbindo,Axel Brando,Jaume Abella,Caroline König*

Main category: cs.CV

TLDR: 论文提出了一种利用扩散模型Instruct Pix2Pix生成天气增强数据集的方法，以提升目标检测模型在恶劣天气下的鲁棒性，并在仿真和真实环境中验证了其有效性。
...
Method: 利用扩散模型Instruct Pix2Pix生成天气增强数据集，并在CARLA仿真器和真实数据集BDD100K、ACDC上进行实验。

Result: 实验表明，该方法能显著提升目标检测模型（如Faster R-CNN和YOLOv10）在恶劣天气下的性能。
...
### [211] [HMPNet: A Feature Aggregation Architecture for Maritime Object Detection from a Shipborne Perspective](https://arxiv.org/abs/2505.08231)
*Yu Zhang,Fengyuan Liu,Juan Lyu,Yi Wei,Changdong Yu*

Main category: cs.CV

TLDR: 论文提出Navigation12数据集和HMPNet模型，用于船舶视角下的目标检测，提升精度和计算效率。


<details>
  <summary>Details</summary>
Motivation: 解决海事领域数据稀缺问题，提升船舶视角目标检测性能。

Method: 基于Navigation12数据集，设计轻量级HMPNet架构，结合动态调制骨干网络和多尺度特征聚合。

Result: HMPNet在精度上优于YOLOv11n（提升3.3% mAP），参数减少23%。

Conclusion: HMPNet为海事目标检测提供了高效解决方案。
...
### [212] [G-MSGINet: A Grouped Multi-Scale Graph-Involution Network for Contactless Fingerprint Recognition](https://arxiv.org/abs/2505.08233)
*Santhoshkumar Peddi,Soham Bandyopadhyay,Debasis Samanta*

Main category: cs.CV

TLDR: G-MSGINet是一个高效的无接触指纹识别框架，通过GMSGI层联合实现细节定位和身份嵌入，无需复杂预处理，显著提升了性能和效率。
...
### [213] [Removing Watermarks with Partial Regeneration using Semantic Information](https://arxiv.org/abs/2505.08234)
*Krti Tallam,John Kevin Cava,Caleb Geniesse,N. Benjamin Erichson,Michael W. Mahoney*

Main category: cs.CV

TLDR: 论文揭示了当前语义水印的漏洞，并提出了一种名为SemanticRegen的三阶段攻击方法，能够有效擦除水印同时保持图像质量。
...
Method: 提出SemanticRegen攻击方法，结合视觉语言模型、零样本分割和LLM引导的扩散模型，分三阶段擦除水印。

Result: 在四种水印系统中，SemanticRegen成功击败TreeRing水印，并将其他水印的比特准确率降至0.75以下，同时保持高感知质量（mSSIM=0.94）。

Conclusion: 研究揭示了当前水印防御与适应性攻击之间的差距，强调了开发对内容保留再生攻击具有鲁棒性的水印算法的紧迫性。
...
### [214] [EventDiff: A Unified and Efficient Diffusion Model Framework for Event-based Video Frame Interpolation](https://arxiv.org/abs/2505.08235)
*Hanle Zheng,Xujie Han,Zegang Peng,Shangbin Zhang,Guangxun Du,Zhuo Zou,Xilin Wang,Jibin Wu,Hao Guo,Lei Deng*

Main category: cs.CV

TLDR: EventDiff是一种基于事件相机的扩散模型框架，用于视频帧插值（VFI），通过隐空间去噪直接完成插值，无需显式运动建模，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有事件相机VFI方法依赖显式运动建模，在复杂运动场景中表现良好，但在细微运动场景中图像重建效果不佳。扩散模型通过去噪过程重建帧，避免了运动估计的需求。

Method: 提出EventDiff框架，包含Event-Frame Hybrid AutoEncoder（HAE）和轻量级Spatial-Temporal Cross Attention（STCA）模块，通过两阶段训练策略（先预训练HAE，再联合优化扩散模型）实现高效插值。

Result: 在Vimeo90K-Triplet和SNU-FILM等数据集上表现最优，PSNR提升1.98dB，推理速度比现有扩散方法快4.24倍。

Conclusion: EventDiff在多样化的VFI场景中表现鲁棒，性能显著优于现有方法，为事件相机VFI提供了高效解决方案。
...
### [215] [Congenital Heart Disease recognition using Deep Learning/Transformer models](https://arxiv.org/abs/2505.08242)
*Aidar Amangeldi,Vladislav Yarovenko,Angsar Taigonyrov*

Main category: cs.CV

TLDR: 双模态（声音和图像）深度学习用于先天性心脏病诊断，准确率分别为73.9%（ZCHSound数据集）和80.72%（DICOM胸部X光数据集）。


<details>
  <summary>Details</summary>
Motivation: 先天性心脏病（CHD）是婴儿发病和死亡的主要原因，但非侵入性筛查方法常出现假阴性。深度学习可辅助医生更有效检测CHD。
...
### [216] [Identifying Memorization of Diffusion Models through p-Laplace Analysis](https://arxiv.org/abs/2505.08246)
*Jonathan Brokman,Amit Giloni,Omer Hofman,Roman Vainshtein,Hisashi Kojima,Guy Gilboa*

Main category: cs.CV

TLDR: 本文研究了如何利用扩散模型估计的分数函数计算高阶微分（p-Laplace算子），并用于识别记忆的训练数据。


<details>
  <summary>Details</summary>
Motivation: 探索扩散模型估计的分数函数是否能用于计算高阶微分，以识别训练数据中的记忆现象。

Method: 提出基于学习分数函数的数值p-Laplace近似方法，并在高斯混合模型和图像生成模型中验证其有效性。

Result: 证明了p-Laplace算子能有效识别概率分布的关键特征，首次在图像生成模型中实现了记忆识别。

Conclusion: p-Laplace算子可用于识别训练数据中的记忆现象，为扩散模型的应用提供了新方向。
...
### [217] [CNN and ViT Efficiency Study on Tiny ImageNet and DermaMNIST Datasets](https://arxiv.org/abs/2505.08259)
*Aidar Amangeldi,Angsar Taigonyrov,Muhammad Huzaid Jawad,Chinedu Emmanuel Mbonu*

Main category: cs.CV

TLDR: 研究评估了卷积和Transformer架构在医学和通用图像分类任务中的权衡，通过微调策略在DermatologyMNIST和TinyImageNet上测试了四种ViT变体，结果显示ViT在性能、推理速度和参数效率上优于ResNet-18。
...
Method: 以ResNet-18为基线，对四种ViT变体（Tiny、Small、Base、Large）进行微调，通过系统超参数调整优化性能。

Result: 适当微调的ViT性能可匹配或超越基线，推理更快且参数更少，适合资源受限环境。

Conclusion: ViT在资源受限场景中具有部署潜力，平衡了性能和效率。
...
### [218] [Few-shot Novel Category Discovery](https://arxiv.org/abs/2505.08260)
*Chunming Li,Shidong Wang,Haofeng Zhang*

Main category: cs.CV

TLDR: 论文提出了一种新的Few-Shot Novel Category Discovery (FSNCD) 设置，通过结合少量标记数据和未标记数据，实现模型在已知类别识别和新类别聚类任务中的灵活切换。


<details>
  <summary>Details</summary>
Motivation: 现有的Novel Category Discovery (NCD) 方法在现实场景中应用受限，少量标记数据可以缓解这一问题。

Method: 提出了Semi-supervised Hierarchical Clustering (SHC) 和Uncertainty-aware K-means Clustering (UKC) 方法，结合少量支持样本和查询样本。

Result: 在五个常用数据集上的实验表明，该方法在不同任务设置和场景中表现领先。

Conclusion: FSNCD 框架通过少量标记数据实现了更灵活的模型适应性，为现实世界应用提供了新思路。
...
### [219] [Open the Eyes of MPNN: Vision Enhances MPNN in Link Prediction](https://arxiv.org/abs/2505.08266)
*Yanbin Wei,Xuehao Wang,Zhan Zhuang,Yang Chen,Shuhao Chen,Yulong Zhang,Yu Zhang,James Kwok*

Main category: cs.CV

TLDR: 论文提出了一种结合视觉感知的图神经网络框架GVN及其高效变体E-GVN，用于链接预测任务，并在多个数据集上取得了新的SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 尽管MPNN和结构特征是链接预测的基础，但视觉感知的潜力在MPNN领域被忽视了。

Method: 提出了Graph Vision Network (GVN)及其高效变体E-GVN，为MPNN引入视觉结构感知。

Result: GVN在七个链接预测数据集上表现优异，包括大规模图，且与现有SOTA方法兼容。

Conclusion: GVN为链接预测开辟了新的研究方向，并取得了新的SOTA结果。
...
### [220] [IrrMap: A Large-Scale Comprehensive Dataset for Irrigation Method Mapping](https://arxiv.org/abs/2505.08273)
...
TLDR: IrrMap是一个用于灌溉方法映射的大规模数据集（110万块图像），包含多分辨率卫星图像和辅助数据，覆盖美国西部多个州的农田，支持深度学习模型训练。


<details>
  <summary>Details</summary>
Motivation: 为灌溉分析提供丰富多样的数据集，支持地理空间对齐和质量控制，促进灌溉方法的研究和应用。

Method: 数据集包含标准化的224x224 GeoTIFF图像块，多输入模态，精心划分的训练测试数据，以及配套的数据加载器。

Result: 分析了不同作物组的灌溉方法分布、空间灌溉模式及灌溉面积变化，揭示了区域和分辨率差异。

Conclusion: IrrMap及其配套工具和代码已公开，支持进一步研究和类似应用。
...
### [221] [Ultra Lowrate Image Compression with Semantic Residual Coding and Compression-aware Diffusion](https://arxiv.org/abs/2505.08281)
*Anle Ke,Xu Zhang,Tong Chen,Ming Lu,Chao Zhou,Jiawen Gu,Zhan Ma*

Main category: cs.CV

TLDR: 提出了一种名为ResULIC的残差引导超低码率图像压缩方法，通过结合语义残差编码和压缩感知扩散模型，显著提升了重建质量和编码效率。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大模型图像压缩框架因碎片化集成语义检索、潜在压缩和生成模型，导致重建保真度和编码效率不佳。

Method: 引入语义残差编码（SRC）捕捉原始图像与压缩潜在表示间的语义差异，并应用感知保真度优化器；提出压缩感知扩散模型（CDM）优化比特率与扩散时间步的匹配。

Result: 实验表明，ResULIC在LPIPS和FID指标上分别实现了80.7%和66.3%的BD-rate节省，优于现有扩散方法。

Conclusion: ResULIC通过残差信号引导和扩散模型优化，显著提升了图像压缩性能。
...
### [222] [Disruptive Transformation of Artworks in Master-Disciple Relationships: The Case of Ukiyo-e Artworks](https://arxiv.org/abs/2505.08284)
*Honna Shinichi,Akira Matsui*

Main category: cs.CV

TLDR: 该论文利用机器学习对日本浮世绘进行定量分析，发现其整体创造力随文化成熟下降，但风格创造力保持高水平。


<details>
  <summary>Details</summary>
Motivation: 传统艺术研究依赖主观判断，机器学习为东方绘画（如浮世绘）提供定量分析的新视角。

Method: 使用11,000张高分辨率浮世绘图像，基于网络计算概念分析作品和艺术家的创造力。

Result: 浮世绘整体创造力随文化成熟下降，但风格创造力因文化细分保持高水平。

Conclusion: 研究为浮世绘分析提供新见解，并展示其在东方艺术文化史中的演变与意义。
...
### [223] [FauForensics: Boosting Audio-Visual Deepfake Detection with Facial Action Units](https://arxiv.org/abs/2505.08294)
*Jian Wang,Baoyuan Wu,Li Liu,Qingshan Liu*

Main category: cs.CV

TLDR: 提出了一种名为FauForensics的新框架，利用生物不变的面部动作单元（FAUs）检测多模态深度伪造内容，通过细粒度帧级视听相似性计算和跨模态查询融合模块，显著提升了检测性能和跨数据集泛化能力。


<details>
  <summary>Details</summary>
Motivation: 生成式AI的快速发展导致逼真的视听深度伪造威胁增加，现有单模态检测方法难以应对多模态伪造，亟需更鲁棒的解决方案。

Method: 引入生物不变的面部动作单元（FAUs）作为伪造抵抗表示，设计专用融合模块计算帧级视听相似性，动态对齐时空唇音关系。

Result: 在FakeAVCeleb和LAV-DF数据集上取得最优性能，跨数据集泛化能力平均提升4.83%。

Conclusion: FauForensics框架在多模态深度伪造检测中表现出色，具有更高的鲁棒性和泛化能力。
...
### [224] [Knowledge-Informed Deep Learning for Irrigation Type Mapping from Remote Sensing](https://arxiv.org/abs/2505.08302)
*Oishee Bintey Hoque,Nibir Chandra Mandal,Abhijin Adiga,Samarth Swarup,Sayjro Kossi Nouwakpo,Amanda Wilson,Madhav Marathe*

Main category: cs.CV

TLDR: 提出了一种基于Swin-Transformer的知识驱动灌溉映射方法（KIIM），通过多模态融合和迁移学习显著提升了灌溉分类精度，减少了对大量标注数据的依赖。


<details>
  <summary>Details</summary>
Motivation: 现有基于光谱特征的卫星图像模型难以应对农业景观的复杂性和有限训练数据，亟需更高效的灌溉映射方法。

Method: 结合作物与灌溉概率的投影矩阵、空间注意力图、双向交叉注意力和加权集成，并采用两阶段迁移学习。

Result: 在五个美国州的实验中，IoU提升22.9%，滴灌分类提升71.4%；迁移学习使标注数据有限州的IoU提升51%。

Conclusion: KIIM方法高效且成本低，仅需40%训练数据即可达到基线性能，为大规模自动化灌溉映射提供了可行性。
...
### [225] [An incremental algorithm for non-convex AI-enhanced medical image processing](https://arxiv.org/abs/2505.08324)
*Elena Morotti*

Main category: cs.CV

TLDR: 提出了一种结合深度学习和增量优化的混合框架incDG，用于高效解决非凸正则化逆问题，尤其在医学影像中表现优异。


<details>
  <summary>Details</summary>
Motivation: 非凸正则化逆问题因优化复杂且多局部极小值而难以解决，但其能提供高质量的任务导向解，尤其在医学影像中需增强临床相关特征。

Method: incDG结合深度学习与增量优化，利用深度神经网络生成初始解，再通过非凸变分解算器迭代优化。

Result: 在TpV正则化任务中，incDG在医学图像去模糊和断层重建中表现优于传统迭代方法和深度学习方法，且无需真实数据训练。

Conclusion: incDG是一种高效、稳健的工具，适用于影像及其他非凸逆问题的解决。
...
### [226] [A computer vision-based model for occupancy detection using low-resolution thermal images](https://arxiv.org/abs/2505.08336)
*Xue Cui,Vincent Gbouna Zakka,Minhyun Lee*

Main category: cs.CV

TLDR: 论文提出了一种基于低分辨率热成像和计算机视觉的占用检测模型，解决了传统RGB图像带来的隐私问题，并优化了计算资源需求。


<details>
  <summary>Details</summary>
Motivation: 传统HVAC系统基于固定时间表运行，不考虑实际占用情况，而RGB图像占用检测存在隐私问题。低分辨率热成像提供了一种非侵入性解决方案。

Method: 研究采用低分辨率热成像和计算机视觉技术，通过迁移学习微调YOLOv5模型，开发占用检测模型。

Result: 模型性能优异，精确度、召回率、mAP50等指标接近1.000。

Conclusion: 该模型不仅解决了隐私问题，还降低了计算资源需求，为HVAC系统的智能控制提供了有效方案。
...
### [227] [FAD: Frequency Adaptation and Diversion for Cross-domain Few-shot Learning](https://arxiv.org/abs/2505.08349)
*Ruixiao Shi,Fu Feng,Yucheng Xie,Jing Wang,Xin Geng*

Main category: cs.CV

TLDR: 论文提出了一种频率感知框架FAD，通过显式建模和调制频谱分量，提升跨域小样本学习的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅关注空间域，忽略了频谱域的变化，而频谱信息对跨域泛化至关重要。

Method: FAD框架通过离散傅里叶变换将特征转换到频域，分区为低、中、高频带，并针对每个频带使用专用卷积分支进行适配。

Result: 在Meta-Dataset基准测试中，FAD在可见和未见域上均优于现有方法。

Conclusion: 频域表示和分频带适配能显著提升跨域小样本学习的泛化性能。
...
### [228] [STORYANCHORS: Generating Consistent Multi-Scene Story Frames for Long-Form Narratives](https://arxiv.org/abs/2505.08350)
*Bo Wang,Haoyang Huang,Zhiyin Lu,Fengyuan Liu,Guoqing Ma,Jianlong Yuan,Yuan Zhang,Nan Duan*

Main category: cs.CV

TLDR: StoryAnchors是一个统一框架，用于生成高质量、多场景且时间一致的故事帧。它通过双向故事生成器和特定条件提升场景多样性和叙事丰富性，并在实验中表现优于现有开源模型。


<details>
  <summary>Details</summary>
Motivation: 解决多场景故事帧生成中的时间一致性和叙事连贯性问题，同时提升场景多样性和编辑灵活性。

Method: 采用双向故事生成器整合过去和未来上下文，引入多事件故事帧标注和渐进式训练方法。

Result: 在一致性、叙事连贯性和场景多样性上优于现有模型，性能与GPT-4o相当。

Conclusion: StoryAnchors为故事驱动帧生成提供了可扩展、灵活且高度可编辑的基础。
...
### [229] [DArFace: Deformation Aware Robustness for Low Quality Face Recognition](https://arxiv.org/abs/2505.08423)
*Sadaf Gulshad,Abdullah Aldahlawi Thakaa*

Main category: cs.CV

TLDR: DArFace是一种新的面部识别框架，通过模拟真实低质量条件（包括全局和局部变形）提升鲁棒性，无需配对高低质量训练样本。


<details>
  <summary>Details</summary>
Motivation: 现有方法在低质量面部图像（如低分辨率、运动模糊）上表现不佳，且常忽略局部非刚性变形。

Method: DArFace通过对抗性训练整合全局变换和局部弹性变形，并引入对比目标确保身份一致性。

Result: 在TinyFace、IJB-B和IJB-C等低质量基准测试中，DArFace显著优于现有方法。

Conclusion: 局部变形建模是提升面部识别在低质量图像中鲁棒性的关键。
...
### [230] [DHECA-SuperGaze: Dual Head-Eye Cross-Attention and Super-Resolution for Unconstrained Gaze Estimation](https://arxiv.org/abs/2505.08426)
*Franko Šikić,Donik Vršnak,Sven Lončarić*

Main category: cs.CV

TLDR: DHECA-SuperGaze是一种基于深度学习的方法，通过超分辨率和双头眼交叉注意力模块提升视线预测性能，并在多个数据集上显著降低了角度误差。
...
Result: 在Gaze360和GFIE数据集上，静态和时序配置下的角度误差分别降低了0.48°-3.00°，跨数据集测试也显示出更强的泛化能力。

Conclusion: DHECA-SuperGaze通过改进特征建模和纠正数据集标注错误，显著提升了视线估计的准确性和鲁棒性。
...
### [231] [Visual Image Reconstruction from Brain Activity via Latent Representation](https://arxiv.org/abs/2505.08429)
*Yukiyasu Kamitani,Misato Tanaka,Ken Shirakawa*

Main category: cs.CV

TLDR: 视觉图像重建通过深度神经网络和生成模型解码大脑活动，从早期分类方法发展到捕捉主观视觉体验的复杂重建，但仍面临零样本泛化和建模主观感知的挑战。
...
Method: 利用深度神经网络和生成模型，结合分层潜在表示、组合策略和模块化架构。

Result: 实现了从大脑活动中重建详细的主观视觉体验，但泛化和主观建模仍需改进。

Conclusion: 视觉图像重建为神经编码研究提供了新视角，但需解决数据集、评估指标和伦理问题以推动发展。
...
### [232] [TT-DF: A Large-Scale Diffusion-Based Dataset and Benchmark for Human Body Forgery Detection](https://arxiv.org/abs/2505.08437)
*Wenkui Yang,Zhida Zhang,Xiaoqiang Zhou,Junxian Duan,Jie Cao*

Main category: cs.CV

TLDR: 论文介绍了TikTok-DeepFake（TT-DF）数据集，专注于人体伪造检测，并提出了TOF-Net检测模型，性能优于现有面部伪造检测方法。


<details>
  <summary>Details</summary>
Motivation: 由于人体伪造检测缺乏数据集和方法，作者提出TT-DF数据集和TOF-Net模型以填补这一空白。

Method: 构建TT-DF数据集，包含多种伪造方法和压缩版本；提出TOF-Net模型，利用时空不一致性和光流分布差异检测伪造。

Result: TOF-Net在TT-DF上表现优异，优于现有面部伪造检测模型。

Conclusion: TT-DF和TOF-Net为人体伪造检测提供了新工具，推动了该领域的发展。
...
### [233] [A Survey of 3D Reconstruction with Event Cameras: From Event-based Geometry to Neural 3D Rendering](https://arxiv.org/abs/2505.08438)
*Chuanzhi Xu,Haoxian Zhou,Langyi Chen,Haodong Chen,Ying Zhou,Vera Chung,Qiang Qu*

Main category: cs.CV

TLDR: 本文综述了事件相机在3D重建中的应用，分类总结了现有方法，并指出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 事件相机因其异步捕捉亮度变化的能力，在极端环境下（如高速运动、低光或高动态范围场景）具有3D重建潜力。本文旨在提供全面的综述和未来发展的路线图。

Method: 将现有工作按输入模态（立体、单目、多模态）和重建方法（几何、深度学习、神经渲染）分类，并整理相关数据集。

Result: 总结了事件相机3D重建的现状，包括数据可用性、评估、表示和动态场景处理等方面的局限性。

Conclusion: 本文为事件驱动的3D重建提供了全面参考，并指出了未来研究方向。
...
### [234] [VCRBench: Exploring Long-form Causal Reasoning Capabilities of Large Video Language Models](https://arxiv.org/abs/2505.08455)
*Pritam Sarkar,Ali Etemad*

Main category: cs.CV

TLDR: 论文提出了VCRBench基准测试，用于评估大型视频语言模型（LVLMs）在视频因果推理中的能力，并提出了RRD方法提升性能。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏评估LVLMs在视频因果推理中能力的专用基准，导致其能力未被充分探索。

Method: 通过创建VCRBench基准测试，并设计RRD方法（将任务分解为视频识别和因果推理两部分）。

Result: 实验显示LVLMs在长程因果推理上表现不佳，但RRD方法能显著提升性能（最高25.2%）。

Conclusion: LVLMs在复杂视频因果推理中依赖语言知识，RRD方法为解决这一问题提供了有效途径。
...
### [235] [A Deep Learning-Driven Framework for Inhalation Injury Grading Using Bronchoscopy Images](https://arxiv.org/abs/2505.08517)
*Yifan Li,Alan W Pang,Jo Woon Chong*

Main category: cs.CV

TLDR: 该研究提出了一种基于深度学习的框架，利用支气管镜图像和机械通气时长作为客观指标，改进了吸入性损伤的分级方法。通过增强的StarGAN生成高质量合成图像，显著提升了分类性能。


<details>
  <summary>Details</summary>
Motivation: 传统方法（如AIS）依赖主观评估且与临床结果相关性较弱，限制了吸入性损伤的诊断和分级。

Method: 使用增强的StarGAN（结合Patch Loss和SSIM Loss）生成合成图像，并通过Swin Transformer进行分类评估。

Result: 增强StarGAN生成的图像显著提升了分类准确率（77.78%，比原始数据集提高11.11%），并在FID评估中表现最佳（30.06）。

Conclusion: 增强StarGAN能有效解决医学影像数据稀缺问题，提升吸入性损伤分级的准确性，具有临床潜力。
...
### [236] [Attention-based Generative Latent Replay: A Continual Learning Approach for WSI Analysis](https://arxiv.org/abs/2505.08524)
*Pratibha Kumari,Daniel Reisenbüchler,Afshin Bozorgpour,Nadine S. Schaadt,Friedrich Feuerhake,Dorit Merhof*

Main category: cs.CV

TLDR: 提出了一种基于注意力的生成潜在重放持续学习框架（AGLR-CL），用于解决全切片图像（WSI）分类中的领域偏移问题。该方法通过高斯混合模型（GMM）合成WSI表示和补丁分布，避免存储原始数据，同时通过注意力过滤提升合成样本质量。


<details>
  <summary>Details</summary>
Motivation: 全切片图像分类在计算病理学中应用广泛，但面临领域偏移（如不同器官、疾病或机构差异）的挑战。需要一种隐私保护且高效的持续学习方法。

Method: 采用多实例学习（MIL）框架，结合高斯混合模型（GMM）生成合成数据，并通过注意力机制过滤关键补丁嵌入。

Result: 在多个公共数据集上的实验表明，AGLR-CL能够保留历史知识并适应新领域，性能优于无缓冲方法，匹配有缓冲方法。

Conclusion: AGLR-CL为WSI分类提供了一种隐私保护且高效的持续学习解决方案，适用于多领域场景。
...
### [237] [Dynamic Snake Upsampling Operater and Boundary-Skeleton Weighted Loss for Tubular Structure Segmentation](https://arxiv.org/abs/2505.08525)
*Yiqi Chen,Ganghai Huang,Sheng Zhang,Jianglin Dai*

Main category: cs.CV

TLDR: 论文提出了一种动态蛇形上采样算子和边界-骨架加权损失，用于提升管状拓扑结构的分割精度。


<details>
  <summary>Details</summary>
Motivation: 传统上采样算子难以处理管状结构的细长性和形态曲率，影响分割准确性。

Method: 设计动态蛇形上采样算子，通过自适应采样域调整采样步长；提出边界-骨架加权损失，平衡主体与边界权重分配。
...
### [238] [Leveraging Segment Anything Model for Source-Free Domain Adaptation via Dual Feature Guided Auto-Prompting](https://arxiv.org/abs/2505.08527)
*Zheang Huai,Hui Tang,Yi Li,Zhuangzhuang Chen,Xiaomeng Li*

Main category: cs.CV

TLDR: 该论文提出了一种基于Segment Anything Model (SAM)的双特征引导(DFG)自动提示方法，用于无源域自适应分割(SFDA)，通过自动生成准确的边界框提示来提升目标域的性能。


<details>
  <summary>Details</summary>
Motivation: 无源域自适应分割(SFDA)仅依赖源模型和未标记的目标数据，但现有方法生成的边界框提示因域差距而存在缺陷。论文探索了SAM在SFDA中的潜力，以解决这一问题。

Method: 提出DFG方法，分为两个阶段：1) 特征聚合阶段，初步适应目标域并准备特征分布；2) 基于目标模型特征和SAM特征的双重引导，逐步扩展边界框提示，并通过连通性分析后处理伪标签。

Result: 在3D和2D数据集上的实验表明，该方法优于传统方法。

Conclusion: DFG方法通过自动生成准确的边界框提示，显著提升了SFDA的性能，为无源域自适应分割提供了新思路。
...
### [239] [The RaspGrade Dataset: Towards Automatic Raspberry Ripeness Grading with Deep Learning](https://arxiv.org/abs/2505.08537)
*Mohamed Lamine Mekhalfi,Paul Chippendale,Fabio Poiesi,Samuele Bonecher,Gilberto Osler,Nicola Zancanella*

Main category: cs.CV

TLDR: 研究探讨了计算机视觉在快速、准确、非侵入式食品质量评估中的应用，专注于工业环境中实时对移动中的覆盆子进行五级分级的挑战。
...
Method: 构建并标注了RaspGrade数据集，通过实例分割实验获取水果级别的掩码，并尝试分类。

Result: 实验显示，某些覆盆子等级因颜色相似和遮挡难以分类，而其他等级则基于颜色较易区分。

Conclusion: RaspGrade数据集已公开，为后续研究提供了资源，但某些分级挑战仍需进一步解决。
...
### [240] [DFA-CON: A Contrastive Learning Approach for Detecting Copyright Infringement in DeepFake Art](https://arxiv.org/abs/2505.08552)
*Haroon Wahab,Hassan Ugail,Irfan Mehmood*

Main category: cs.CV

TLDR: 论文提出DFA-CON框架，通过对比学习检测AI生成艺术品的版权侵权或伪造问题。


<details>
  <summary>Details</summary>
Motivation: 生成AI工具在视觉艺术创作中的广泛应用引发版权侵权和伪造的担忧，需有效检测方法。

Method: 采用对比学习框架DFA-CON，学习判别性表示空间，涵盖多种攻击类型（如修复、风格迁移等）。

Result: 模型在多数攻击类型中表现优于现有预训练基础模型。

Conclusion: DFA-CON为检测AI生成艺术品的侵权和伪造提供了有效解决方案，代码将公开。
...
### [241] [Reinforcement Learning meets Masked Video Modeling : Trajectory-Guided Adaptive Token Selection](https://arxiv.org/abs/2505.08561)
*Ayush K. Rai,Kyle Min,Tarun Krishna,Feiyan Hu,Alan F. Smeaton,Noel E. O'Connor*

Main category: cs.CV

TLDR: 提出了一种新的轨迹感知自适应令牌采样器（TATS），结合MAE框架，通过PPO联合优化，实现高效视频预训练。


<details>
  <summary>Details</summary>
Motivation: 解决现有视频建模中掩码策略选择的问题，提升预训练效率和性能。

Method: 提出TATS建模令牌运动动态，结合MAE框架，使用PPO联合优化。
...
### [242] [Thermal Detection of People with Mobility Restrictions for Barrier Reduction at Traffic Lights Controlled Intersections](https://arxiv.org/abs/2505.08568)
*Xiao Ni,Carsten Kuehnel,Xiaoyi Jiang*

Main category: cs.CV

TLDR: 提出了一种基于热成像的交通信号灯系统，动态调整信号时长以帮助行动不便者，并触发听觉信号辅助视障人士，同时开发了YOLO-Thermal检测器提升热成像检测精度。


<details>
  <summary>Details</summary>
Motivation: 现有RGB摄像头系统忽视行动不便者需求，且在恶劣天气或低能见度下性能受限，隐私问题突出。

Method: 构建热成像数据集TD4PWMR，开发YOLO-Thermal检测器，结合特征提取和注意力机制提升检测性能。
...
### [243] [ReSurgSAM2: Referring Segment Anything in Surgical Video via Credible Long-term Tracking](https://arxiv.org/abs/2505.08581)
*Haofeng Liu,Mingqi Gao,Xuxiao Luo,Ziyue Wang,Guanyi Qin,Junde Wu,Yueming Jin*

Main category: cs.CV

TLDR: ReSurgSAM2是一种两阶段的手术场景分割框架，结合了Segment Anything Model 2和多样性驱动的长期记忆机制，显著提升了分割和跟踪的效率和准确性。
...
Method: 采用两阶段框架：1）基于文本引导的目标检测和分割；2）通过可信初始帧选择和多样性驱动内存机制实现长期跟踪。

Result: ReSurgSAM2在准确性和效率上显著优于现有方法，实时运行速度达61.2 FPS。

Conclusion: ReSurgSAM2为手术场景分割提供了一种高效、可靠的解决方案，适用于复杂手术环境。
...
### [244] [A Large-scale Benchmark on Geological Fault Delineation Models: Domain Shift, Training Dynamics, Generalizability, Evaluation and Inferential Behavior](https://arxiv.org/abs/2505.08585)
...
TLDR: 本文首次大规模研究了地震解释中领域偏移策略，评估了200多个模型在合成和真实数据上的表现，揭示了当前微调实践的脆弱性，并提出了改进方向。


<details>
  <summary>Details</summary>
Motivation: 地震解释中机器学习模型的泛化能力缺乏系统性研究，领域偏移、微调策略和评估协议的不一致是主要障碍。

Method: 通过三个异构数据集（FaultSeg3D、CRACKS、Thebe）评估预训练、微调和联合训练策略，分析领域偏移下的表现。

Result: 研究发现当前微调实践脆弱，存在灾难性遗忘问题，并揭示了性能评估的系统性挑战。

Conclusion: 研究为地震解释中更通用、可解释和有效的机器学习模型提供了指导，并提出了部署建议。
...
### [245] [PrePrompt: Predictive prompting for class incremental learning](https://arxiv.org/abs/2505.08586)
*Libo Huang,Zhulin An,Chuanguang Yang,Boyu Diao,Fei Wang,Yan Zeng,Zhifeng Hao,Yongjun Xu*

Main category: cs.CV

TLDR: 论文提出了一种名为PrePrompt的新型CIL框架，通过预测任务特定提示来避免基于相关性的限制，并通过特征翻译动态平衡稳定性和可塑性。
...
Method: PrePrompt将CIL分解为两阶段预测框架：任务特定提示预测和标签预测，并引入特征翻译以减少对近期类的偏差。

Result: 实验表明PrePrompt在多个基准测试中优于现有基于提示的CIL方法。

Conclusion: PrePrompt通过预测任务特定提示和动态平衡机制，有效解决了CIL中的挑战。
...
### [246] [MESSI: A Multi-Elevation Semantic Segmentation Image Dataset of an Urban Environment](https://arxiv.org/abs/2505.08589)
*Barak Pinkovich,Boaz Matalon,Ehud Rivlin,Hector Rotstein*

Main category: cs.CV

TLDR: 介绍了MESSI数据集，包含2525张无人机拍摄的密集城市环境图像，具有多高度和多区域特点，用于研究深度对语义分割的影响。


<details>
  <summary>Details</summary>
Motivation: 研究无人机在不同高度和区域拍摄的图像对语义分割的影响，填补相关领域数据集的空白。

Method: 收集并标注多高度、多区域的无人机图像，使用多种神经网络模型进行语义分割。

Result: MESSI数据集可用于训练神经网络，支持语义分割、定位、导航等任务，并作为公开基准。

Conclusion: MESSI数据集为无人机图像语义分割提供了重要资源，未来可作为相关研究的评估标准。
...
### [247] [Rejoining fragmented ancient bamboo slips with physics-driven deep learning](https://arxiv.org/abs/2505.08601)
...
TLDR: WisePanda是一种基于物理驱动的深度学习框架，用于拼接破碎的竹简，显著提高了匹配准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 竹简是记录东亚古代文明的重要媒介，但其碎片化严重，拼接困难，阻碍了考古研究。

Method: 结合断裂物理和材料退化原理，自动生成合成训练数据，训练匹配网络，无需人工配对样本。

Result: Top-50匹配准确率从36%提升至52%，拼接效率提高约20倍。

Conclusion: 物理驱动的深度学习能显著提升模型性能，为古代文物修复提供新范式。
...
### [248] [Unsupervised Out-of-Distribution Detection in Medical Imaging Using Multi-Exit Class Activation Maps and Feature Masking](https://arxiv.org/abs/2505.08604)
*Yu-Jen Chen,Xueyang Li,Yiyu Shi,Tsung-Yi Ho*

Main category: cs.CV

TLDR: 该论文提出了一种基于多出口类激活图（MECAM）的无监督OOD检测框架，通过特征掩蔽和多分辨率CAM增强检测鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 医学影像中OOD检测对深度学习模型的可靠性至关重要，ID和OOD数据在CAM激活模式上的差异为检测提供了依据。

Method: 利用多出口网络生成不同分辨率和深度的CAM，通过掩蔽输入图像的特征表示差异区分ID和OOD数据。

Result: 在多个医学和自然图像数据集上验证了MECAM的有效性，优于现有OOD检测方法。
...
### [249] [Leveraging Multi-Modal Information to Enhance Dataset Distillation](https://arxiv.org/abs/2505.08605)
...
Method: 引入两种策略利用文本信息：特征拼接和文本匹配；同时采用对象中心掩码，设计两种损失函数用于对象中心学习。

Result: 实验表明，结合文本指导和对象中心掩码能提升数据集蒸馏效果，生成的数据集在下游任务中表现更优。

Conclusion: 通过多模态信息和对象级优化，可以有效提升数据集蒸馏的质量和实用性。
...
### [250] [Boosting Zero-shot Stereo Matching using Large-scale Mixed Images Sources in the Real World](https://arxiv.org/abs/2505.08607)
*Yuran Wang,Yingping Liang,Ying Fu*

Main category: cs.CV

TLDR: 论文提出了一种名为BooSTer的新框架，利用视觉基础模型和大规模混合图像源（包括合成、真实和单视图图像）来解决立体匹配中标注数据稀缺和领域差异的问题。


<details>
  <summary>Details</summary>
Motivation: 立体匹配方法依赖密集的像素级标注数据，但真实世界数据集的标注成本高，且合成与真实图像之间存在领域差异，导致性能受限。

Method: 1. 结合单目深度估计和扩散模型，从单视图图像生成密集立体匹配数据；2. 利用单目深度估计模型的伪标签和动态尺度不变损失解决真实数据集标注稀疏问题；3. 引入视觉基础模型作为编码器提取鲁棒特征。

Result: 在基准数据集上的实验表明，BooSTer在标注数据有限和领域差异场景下显著优于现有方法。

Conclusion: BooSTer通过结合多种数据源和视觉基础模型，有效提升了立体匹配的准确性和泛化能力。
...
### [251] [WaveGuard: Robust Deepfake Detection and Source Tracing via Dual-Tree Complex Wavelet and Graph Neural Networks](https://arxiv.org/abs/2505.08614)
*Ziyuan He,Zhiqing Guo,Liejun Wang,Gaobo Yang,Yunfeng Diao,Dan Ma*

Main category: cs.CV

TLDR: WaveGuard是一种主动水印框架，通过频域嵌入和图结构一致性增强鲁棒性和不可感知性，用于应对Deepfake技术的隐私和身份盗窃风险。


<details>
  <summary>Details</summary>
Motivation: Deepfake技术带来了隐私侵犯和身份盗窃等风险，需要一种更鲁棒且不可感知的水印方法来应对这些威胁。

Method: 使用双树复小波变换（DT-CWT）将水印嵌入高频子带，并结合结构一致性图神经网络（SC-GNN）保持视觉质量，同时设计了注意力模块优化嵌入精度。

Result: 在面部交换和重演任务中，WaveGuard在鲁棒性和视觉质量上均优于现有方法。

Conclusion: WaveGuard通过频域嵌入和图结构一致性，有效提升了水印的鲁棒性和不可感知性，为Deepfake威胁提供了解决方案。
...
### [252] [OpenThinkIMG: Learning to Think with Images via Visual Tool Reinforcement Learning](https://arxiv.org/abs/2505.08617)
...
TLDR: OpenThinkIMG是一个开源框架，用于增强大型视觉语言模型（LVLMs）的动态视觉工具调用能力，通过强化学习（V-ToolRL）显著提升任务表现。
...
Method: 提出OpenThinkIMG框架，包含标准化工具接口、轨迹生成和强化学习（V-ToolRL）训练环境。

Result: V-ToolRL在图表推理任务中表现优异，超越SFT初始化和现有基线模型，甚至优于GPT-4.1。
...
### [253] [DLO-Splatting: Tracking Deformable Linear Objects Using 3D Gaussian Splatting](https://arxiv.org/abs/2505.08644)
*Holly Dinkel,Marcel Büsching,Alberta Longhini,Brian Coltin,Trey Smith,Danica Kragic,Mårten Björkman,Timothy Bretl*

Main category: cs.CV

TLDR: DLO-Splatting算法通过多视角RGB图像和夹爪状态信息，结合预测-更新滤波估计可变形线性物体的3D形状。


<details>
  <summary>Details</summary>
Motivation: 现有视觉方法在复杂场景（如打结）中表现不佳，需结合动态模型和视觉信息改进形状估计。

Method: 采用基于位置的动力学模型和3D高斯Splatting渲染损失，通过迭代优化预测形状。
...
### [254] [SkillFormer: Unified Multi-View Video Understanding for Proficiency Estimation](https://arxiv.org/abs/2505.08665)
*Edoardo Bianchi,Antonio Liotta*

Main category: cs.CV

TLDR: SkillFormer是一种参数高效的架构，用于从第一人称和第三人称视频中统一评估多视角技能水平，通过交叉视角融合模块和低秩适配技术显著降低了训练成本。
...
Method: 基于TimeSformer，SkillFormer引入了CrossViewFusion模块，结合多头交叉注意力、可学习门控和自适应自校准，并利用低秩适配技术微调少量参数。

Result: 在EgoExo4D数据集上，SkillFormer在多视角设置中达到了最先进的准确性，同时减少了4.5倍的参数和3.75倍的训练周期。

Conclusion: SkillFormer通过多视角融合实现了细粒度技能评估，证明了其在计算效率和性能上的优越性。
...
### [255] [Calibration and Uncertainty for multiRater Volume Assessment in multiorgan Segmentation (CURVAS) challenge results](https://arxiv.org/abs/2505.08685)
...
TLDR: CURVAS挑战赛探讨了多标注者在医学图像分割中的重要性，通过评估DL模型的校准和不确定性，强调模型可靠性与临床适用性。


<details>
  <summary>Details</summary>
Motivation: 解决医学图像分割中标注变异性、校准和不确定性估计的挑战，以提升DL模型的可靠性和临床适用性。

Method: 七支团队提交DL模型，使用DSC、ECE和CRPS等指标评估，结合共识与非共识标注数据。

Result: 校准良好的模型表现更优，预训练和多数据集训练的模型更具鲁棒性。最佳模型实现高DSC和良好校准的不确定性估计。
...
### [256] [SPAST: Arbitrary Style Transfer with Style Priors via Pre-trained Large-scale Model](https://arxiv.org/abs/2505.08695)
*Zhanjie Zhang,Quanwei Zhang,Junsheng Luan,Mengyuan Yang,Yun Wang,Lei Zhao*

Main category: cs.CV

TLDR: SPAST框架通过局部全局窗口大小风格化模块（LGWSSM）和风格先验损失，实现了高质量风格迁移并减少推理时间。


<details>
  <summary>Details</summary>
Motivation: 现有方法在生成高质量风格化图像时，要么无法保持内容结构，要么推理时间长。

Method: 设计了LGWSSM模块融合风格特征，并引入风格先验损失从预训练大模型中提取风格先验。
...
### [257] [Controllable Image Colorization with Instance-aware Texts and Masks](https://arxiv.org/abs/2505.08705)
*Yanru An,Ling Gui,Qiang Hu,Chunlei Cai,Tianxiao Ye,Xiaoyun Zhang,Yanfeng Wang*

Main category: cs.CV

TLDR: 提出了一种基于扩散模型的实例感知图像着色方法MT-Color，通过像素级掩码注意力机制和实例掩码文本引导模块解决颜色溢出和绑定错误问题，并引入多实例采样策略和专用数据集GPT-color。
...
Method: 设计像素级掩码注意力机制防止颜色溢出，引入实例掩码和文本引导模块避免绑定错误，采用多实例采样策略，并构建专用数据集GPT-color。
...
### [258] [TiMo: Spatiotemporal Foundation Model for Satellite Image Time Series](https://arxiv.org/abs/2505.08723)
...
Method: 提出TiMo模型，引入时空陀螺注意力机制，动态捕捉多尺度时空模式；利用MillionST数据集进行预训练。
...
### [259] [Extending Large Vision-Language Model for Diverse Interactive Tasks in Autonomous Driving](https://arxiv.org/abs/2505.08725)
*Zongchuang Zhao,Haoyu Fu,Dingkang Liang,Xin Zhou,Dingyuan Zhang,Hongwei Xie,Bing Wang,Xiang Bai*

Main category: cs.CV

TLDR: 论文提出NuInteract数据集和DriveMonkey框架，解决LVLMs在3D场景理解中的不足，显著提升了3D视觉定位性能。


<details>
  <summary>Details</summary>
Motivation: 现有LVLMs在自动驾驶场景中缺乏2D与3D的映射关系，且3D目标定位与指令理解结合不足。

Method: 引入NuInteract数据集（150万对多视角图像语言对），并提出DriveMonkey框架，集成LVLMs与空间处理器。
...
### [260] [Advancing Food Nutrition Estimation via Visual-Ingredient Feature Fusion](https://arxiv.org/abs/2505.08747)
...
Method: 提出VIF²方法，结合视觉和食材特征，并通过数据增强优化食材预测。

Result: 在FastFood和Nutrition5k数据集上验证了方法的有效性，支持食材信息的重要性。

Conclusion: VIF²方法显著提升了营养估计的准确性，证明了食材信息的价值。
...
### [261] [Towards Autonomous UAV Visual Object Search in City Space: Benchmark and Agentic Methodology](https://arxiv.org/abs/2505.08765)
*Yatai Ji,Zhengqiu Zhu,Yong Zhao,Beidan Liu,Chen Gao,Yihao Zhao,Sihang Qiu,Yue Hu,Quanjun Yin,Yong Li*

Main category: cs.CV

TLDR: 论文提出了CityAVOS数据集和PRPSearcher方法，用于解决无人机在复杂城市环境中自主搜索目标物体的挑战，显著提升了搜索成功率和效率。
...
Method: 提出PRPSearcher方法，基于多模态大语言模型，构建三种专用地图（动态语义地图、3D认知地图和不确定性地图），并引入去噪机制和IPT提示机制。

Result: 在CityAVOS数据集上，PRPSearcher在成功率和搜索效率上显著优于基线方法（平均提升37.69% SR和28.96% SPL）。

Conclusion: PRPSearcher为AVOS任务提供了有效解决方案，但与人类表现仍有差距，未来需进一步提升语义推理和空间探索能力。
...
### [262] [Big Data and the Computational Social Science of Entrepreneurship and Innovation](https://arxiv.org/abs/2505.08706)
*Ningzi Li,Shiyang Lai,James Evans*

Main category: econ.GN

TLDR: 论文探讨了利用大规模数据和机器学习方法研究创业与创新的机遇与挑战，提出了两种利用多模态数据的方法：构建精确测量系统和生成数字孪生实验室。


<details>
  <summary>Details</summary>
Motivation: 随着大规模社交数据的爆炸式增长和机器学习方法的发展，创业与创新研究面临新的机遇和挑战，需要探索如何利用这些数据和方法。

Method: 结合机器学习模型与大规模数据，构建精确测量系统作为创新与创业的观测站；利用大数据生成技术和商业的“数字孪生”，进行虚拟实验。

Result: 提出了两种方法：系统级观测和虚拟实验，以推动创业与创新研究的理论发展与测试。

Conclusion: 通过将大数据与大模型结合，可以促进创业与创新领域的理论发展和实践应用。
...
### [263] [Non-contact Vital Signs Detection in Dynamic Environments](https://arxiv.org/abs/2505.08366)
*Shuai Sun,Chong-Xi Liang,Chengwei Ye,Huanzhen Zhang,Kangsheng Wang*

Main category: eess.SP

TLDR: 提出了一种新的DC偏移校准方法和HADCM解调算法，用于毫米波雷达生命体征检测，在复杂环境中表现优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 解决复杂环境中时变DC偏移和相位不平衡对解调性能的严重影响。

Method: 通过估计信号峰谷的时变DC偏移，结合差分形式和希尔伯特变换提取生命体征信息。

Result: 在低信噪比下保持稳健性能，信号恢复更准确，有效抑制噪声干扰。

Conclusion: 该方法在挑战性场景中优于现有解调技术。
...
### [264] [Explainable Artificial Intelligence Techniques for Software Development Lifecycle: A Phase-specific Survey](https://arxiv.org/abs/2505.07058)
*Lakshit Arora,Sanjay Surendranath Girija,Shashank Kapoor,Aman Raj,Dipen Pradhan,Ankit Shetgaonkar*

Main category: cs.SE

TLDR: 本文综述了可解释人工智能（XAI）在软件开发生命周期（SDLC）各阶段的应用，填补了XAI在软件工程中应用的研究空白。


<details>
  <summary>Details</summary>
Motivation: 解决AI模型的“黑盒问题”，提升AI系统的透明度和可解释性，以增强信任和广泛应用。

Method: 通过文献综述，分析XAI技术（如LIME、SHAP、规则提取等）在SDLC各阶段的应用。
...
### [265] [Moving From Monolithic To Microservices Architecture for Multi-Agent Systems](https://arxiv.org/abs/2505.07838)
*Muskaan Goyal,Pranav Bhasin*

Main category: cs.SE

TLDR: 本文综述了从单体架构到微服务架构在多智能体系统（MAS）中的演变，探讨了传统单体MAS的局限性及微服务方法的优势，并分析了相关通信协议和设计挑战。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于探索微服务架构在复杂多智能体系统中的应用潜力，以解决传统单体架构的可扩展性和维护性问题。

Method: 通过比较分析传统单体架构与微服务架构，结合Agent Communication Languages (ACLs)、Model Context Protocol (MCP)和Application-to-Application (A2A)协议，探讨了核心架构原则。

Result: 研究发现微服务架构在多智能体系统中具有显著优势，但也面临设计挑战和通信协议的选择问题。

Conclusion: 结论指出微服务架构为多智能体系统提供了新的可能性，但需进一步研究和优化以应对实际应用中的挑战。
...
### [266] [SweRank: Software Issue Localization with Code Ranking](https://arxiv.org/abs/2505.07849)
...
TLDR: SweRank是一个高效的检索和重新排序框架，用于软件问题定位，通过SweLoc数据集训练，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 软件问题定位是开发中的关键但耗时的任务，现有方法存在延迟高、成本高或效果不佳的问题。

Method: 提出SweRank框架，利用SweLoc数据集进行训练，结合检索和重新排序技术。

Result: 在SWE-Bench-Lite和LocBench上表现优异，优于现有排名模型和基于代理的系统。

Conclusion: SweRank和SweLoc为问题定位提供了高效解决方案和宝贵资源。
...
### [267] [Leveraging AI for Productive and Trustworthy HPC Software: Challenges and Research Directions](https://arxiv.org/abs/2505.08135)
...
TLDR: 探讨AI如何革新高性能计算（HPC）软件开发，分析挑战并提出研究方向。


<details>
  <summary>Details</summary>
Motivation: AI技术（尤其是大语言模型）已改变软件开发，但HPC软件作为高度专业化领域，其AI应用仍面临独特挑战。

Method: 通过美国能源部资助的Ellora和Durban项目，研究如何利用先进AI技术推动HPC软件开发。

Result: 提出具体研究方向，以解决HPC软件开发中的AI应用难题。

Conclusion: AI有望革新HPC软件开发，但需针对性研究以克服其独特挑战。
...
### [268] [Probabilistic approach to longitudinal response prediction: application to radiomics from brain cancer imaging](https://arxiv.org/abs/2505.07973)
*Isabella Cama,Michele Piana,Cristina Campi,Sara Garbarino*

Main category: stat.AP

TLDR: 提出了一种概率模型用于纵向疾病进展预测，整合基线特征和随访数据，处理不确定性并减少问题维度。


<details>
  <summary>Details</summary>
Motivation: 纵向影像分析能动态评估疾病进展和治疗效果，但现有方法难以处理不确定性和数据维度问题。

Method: 开发概率模型，整合基线特征和随访数据，处理预测不确定性并控制问题维度。

Result: 在合成场景和脑癌数据集中表现优异，优于现有方法且无需中间随访数据。

Conclusion: 该模型能有效预测疾病进展，处理不确定性并简化数据需求，具有临床应用潜力。
...
### [269] [Not that Groove: Zero-Shot Symbolic Music Editing](https://arxiv.org/abs/2505.08203)
*Li Zhang*

Main category: cs.SD

TLDR: 论文提出了一种基于零样本提示的LLM方法，用于符号音乐编辑，解决了音频生成在音乐产业中灵活性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有AI音乐生成主要关注音频，但因其灵活性不足而在音乐制作行业中应用有限。论文旨在通过符号音乐编辑提升灵活性。

Method: 利用零样本提示的LLM方法编辑鼓点节奏，设计了一种创新的格式以连接LLM与音乐。

Result: 成功证明了零样本提示LLM可有效编辑鼓点节奏，并提供了与音乐家判断高度一致的评价数据集。

Conclusion: 该方法为符号音乐编辑提供了灵活且有效的解决方案，填补了音频生成在音乐产业中的局限性。
...
### [270] [Fast Text-to-Audio Generation with Adversarial Post-Training](https://arxiv.org/abs/2505.08175)
...
TLDR: 提出了一种名为ARC的对抗性加速算法，用于提升扩散/流模型的推理速度，无需依赖蒸馏技术，显著降低了文本到音频系统的延迟。
...
Method: 结合相对对抗性训练和对比鉴别器目标，优化扩散/流模型的后训练过程，并应用于Stable Audio Open模型。

Result: 在H100上生成12秒44.1kHz立体声音频仅需75毫秒，在移动边缘设备上生成7秒音频，是目前最快的文本到音频模型。

Conclusion: ARC后训练算法显著提升了推理速度，为文本到音频系统的实时应用提供了可能。
...
### [271] [A Mamba-based Network for Semi-supervised Singing Melody Extraction Using Confidence Binary Regularization](https://arxiv.org/abs/2505.08681)
*Xiaoliang He,Kangjie Dong,Jingkai Cao,Shuai Yu,Wei Li,Yi Yu*

Main category: cs.SD

TLDR: 提出了一种基于Mamba的网络SpectMamba，用于半监督歌唱旋律提取，解决了现有方法在计算效率、音符建模和数据标注不足方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在歌唱旋律提取中存在计算效率低、忽略音符建模和标注数据不足的问题。
...
TLDR: 该研究通过QSPR分析和机器学习技术，探索药物分子物理性质与拓扑指数之间的相关性，提出了一种创新的预测方法。


<details>
  <summary>Details</summary>
Motivation: 传统药物设计研究主要关注基于度的拓扑指数，本研究扩展了方法，结合了度-距离拓扑指数和原子属性加权，以提高预测准确性。

Method: 使用166种药物分子数据集，计算了度-距离拓扑指数，并结合六种原子属性。采用线性和非线性机器学习模型（如线性回归、随机森林、神经网络等）进行预测。

Result: 结果表明，这些拓扑指数能有效预测特定物理化学性质，验证了计算方法在分子性质估计中的实用性。

Conclusion: 研究为药物发现提供了一种新视角，展示了拓扑指数与机器学习结合的潜力，可优化资源利用并减少实验需求。
...
TLDR: 提出了一种可调控的小分子生成设计框架，通过强化学习实现合成可控性。


<details>
  <summary>Details</summary>
Motivation: 解决小分子生成设计中合成可控性的瓶颈问题，尤其是合成路线的灵活性和反应约束的整合。

Method: 采用预训练的通用分子生成模型，通过强化学习激励生成满足多参数优化目标和特定反应约束的分子。

Result: 生成的分子在满足反应约束的同时，具有高精确匹配率（>90%），并能高效筛选出有潜力的候选分子。

Conclusion: 该框架展示了在严格合成约束下生成优化小分子的潜力，且样本效率优于现有方法。
...
### [274] [Image-Guided Microstructure Optimization using Diffusion Models: Validated with Li-Mn-rich Cathode Precursors](https://arxiv.org/abs/2505.07906)
*Geunho Choi,Changhwan Lee,Jieun Kim,Insoo Ye,Keeyoung Jung,Inchul Park*

Main category: cond-mat.mtrl-sci

TLDR: 论文提出了一种基于图像的闭环框架，将微观结构形态作为可控目标，并应用于锂离子电池正极前驱体的合成优化。


<details>
  <summary>Details</summary>
Motivation: 微观结构通常决定材料性能，但由于难以量化、预测和优化，很少被作为显式设计变量。

Method: 结合扩散模型、图像分析管道和粒子群优化算法，提取形态特征并预测合成条件。

Result: 框架能准确预测特定合成条件下的微观结构，并通过实验验证了预测与合成结构的一致性。

Conclusion: 该框架为数据驱动的材料设计提供了实用策略，支持正向预测和逆向设计，推动自主微观结构工程。
...
### [275] [Enhancing the Efficiency of Complex Systems Crystal Structure Prediction by Active Learning Guided Machine Learning Potential](https://arxiv.org/abs/2505.08159)
*Jiaxiang Li,Junwei Feng,Jie Luo,Bowen Jiang,Xiangyu Zheng,Jian Lv,Keith Butler,Hanyu Liu,Congwei Xie,Yu Xie,Yanming Ma*

Main category: cond-mat.mtrl-sci

TLDR: 提出了一种自动化工作流，构建通用且数据高效的机器学习势能（MLP），以解决复杂材料系统中的组合爆炸问题，并在Mg-Ca-H和Be-P-N-O系统中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 复杂材料系统的组合爆炸和化学计量空间巨大，导致传统晶体结构预测方法难以应对，需要一种更高效的解决方案。

Method: 开发了一种灵活且自动化的工作流，构建通用且数据高效的机器学习势能（MLP），以加速结构优化和化合物发现。

Result: 在Mg-Ca-H和Be-P-N-O系统中验证了工作流的有效性，显著提升了高通量结构优化的效率，并成功识别出有前景的化合物。

Conclusion: 该方法为探索复杂材料系统和加速多组分材料发现提供了有效途径。
...
### [276] [AI-Based Crypto Tokens: The Illusion of Decentralized AI?](https://arxiv.org/abs/2505.07828)
*Rischan Mafrur*

Main category: cs.DC

TLDR: 论文综述了基于区块链和人工智能（AI）的AI代币项目，分析了其技术架构、代币用途、共识机制和商业模式，并指出了当前实现中的技术限制和商业模式问题。


<details>
  <summary>Details</summary>
Motivation: 探索区块链与AI结合的潜力，评估AI代币项目是否能为去中心化AI平台提供超越传统中心化服务的价值。

Method: 通过全面审查领先的AI代币项目，分析其技术架构、代币功能和商业模式，并评估其实际价值。

Result: 发现当前AI代币项目在技术上依赖链下计算，链上智能有限，且存在可扩展性问题；商业模式上多为中心化AI服务的简单复制。

Conclusion: 尽管新兴技术为去中心化AI生态系统提供了改进路径，但当前AI代币的实现与承诺之间仍存在显著差距，需更务实的评估方法。
...
### [277] [Kudzu: Fast and Simple High-Throughput BFT](https://arxiv.org/abs/2505.08771)
*Victor Shoup,Jakub Sliwinski,Yann Vonlanthen*

Main category: cs.DC

TLDR: Kudzu是一种高吞吐量的原子广播协议，集成了快速路径，能在两轮通信内达成最终性，并在存在故障时保持高性能。


<details>
  <summary>Details</summary>
Motivation: 结合两种研究方向，设计一种在高吞吐量下仍能快速达成最终性且平衡带宽利用的协议。

Method: 通过无缝集成快速路径和使用纠删码平衡带宽利用，避免复杂机制如进度证书和视图变更。

Result: 协议在两轮通信内达成最终性，容忍$f$个拜占庭故障，且在更多故障时仍保持高性能。

Conclusion: Kudzu协议简单高效，结合了快速路径和高吞吐量的优势，避免了复杂机制。

Abstract: We present Kudzu, a high-throughput atomic broadcast protocol with an
...
### [278] [Patchwork: A Unified Framework for RAG Serving](https://arxiv.org/abs/2505.07833)
*Bodun Hu,Luis Pabon,Saurabh Agarwal,Aditya Akella*

Main category: cs.DC

TLDR: Patchwork是一个端到端的RAG服务框架，通过灵活的接口、分布式推理系统和在线调度机制，显著提升了性能，减少了SLO违规。


<details>
  <summary>Details</summary>
Motivation: 解决RAG系统部署中的效率瓶颈问题，提升大型语言模型与外部知识源集成的可靠性。

Method: 提供灵活接口支持自定义RAG管道，优化分布式推理系统，并采用在线调度机制动态管理资源。

Result: 实验显示，Patchwork在四种RAG实现中性能显著优于商业方案，吞吐量提升48%，SLO违规减少24%。

Conclusion: Patchwork通过创新架构有效解决了RAG系统的效率问题，为实际部署提供了可靠解决方案。
...
### [279] [Fused3S: Fast Sparse Attention on Tensor Cores](https://arxiv.org/abs/2505.08098)
*Zitong Li,Aparna Chandramowlishwaran*

Main category: cs.DC

TLDR: Fused3S是一种融合的稀疏注意力算法，通过联合优化张量核心利用率和数据移动成本，显著提升了GPU上的计算效率。


<details>
  <summary>Details</summary>
Motivation: 稀疏注意力是现代神经网络模型的核心组件，但其在GPU上的高效执行面临张量核心与稀疏性不匹配以及数据移动成本高的挑战。

Method: 提出Fused3S算法，将稀疏注意力的三个稀疏矩阵操作（SDDMM、softmax、SpMM）融合为一个整体，优化张量核心利用并减少数据移动。

Result: 在H100和A30 GPU上，Fused3S比现有技术快1.6-16.3倍和1.5-14倍；集成到Graph Transformer推理中，端到端性能提升1.05-5.36倍。

Conclusion: Fused3S在多种数据集和GPU架构上均优于现有3S基线，证明了其高效性和普适性。
...
### [280] [PRISM: Complete Online Decentralized Multi-Agent Pathfinding with Rapid Information Sharing using Motion Constraints](https://arxiv.org/abs/2505.08025)
*Hannah Lee,Zachary Serlin,James Motes,Brendan Long,Marco Morales,Nancy M. Amato*

Main category: cs.RO

TLDR: PRISM是一种去中心化算法，用于解决多任务多智能体路径规划问题，通过快速通信策略和运动约束信息交换，实现高效协作路径规划。


<details>
  <summary>Details</summary>
Motivation: 解决多任务多智能体路径规划中的死锁和协作问题，提升在无直接通信场景下的路径规划效率。

Method: 采用快速通信策略，通过信息包交换运动约束信息，避免死锁并提升协作能力。

Result: 在5种环境和25个随机场景中，PRISM支持比CBS多3.4倍的智能体，比TPTS多处理2.5倍的任务，且计算速度更快。

Conclusion: PRISM在复杂动态环境中表现出鲁棒性、可扩展性和高效性，适用于大规模路径规划。
...
### [281] [What Matters for Batch Online Reinforcement Learning in Robotics?](https://arxiv.org/abs/2505.08078)
*Perry Dong,Suvir Mirchandani,Dorsa Sadigh,Chelsea Finn*

Main category: cs.RO

TLDR: 论文探讨了批量在线强化学习（batch online RL）在机器人学习中的应用，分析了算法类别、策略提取方法和策略表达能力对性能的影响，并提出了一种改进方法。


<details>
  <summary>Details</summary>
Motivation: 批量在线强化学习有望通过减少人工数据收集需求并利用自主数据实现自我改进，但现有方法效率低下，难以有效学习。

Method: 通过系统实证研究，分析了算法类别、策略提取方法和策略表达能力三个维度，并提出了改进方法，包括使用Q函数引导策略提取和引入时间相关噪声。

Result: 研究发现，基于Q函数的方法优于模仿学习方法，隐式策略提取和表达能力强的策略类能显著提升性能。改进方法在性能和扩展性上优于现有方法。

Conclusion: 论文提出了一种有效的批量在线强化学习框架，通过优化算法设计和策略提取方法，显著提升了自主数据学习的效率和性能。
...
### [282] [UniSkill: Imitating Human Videos via Cross-Embodiment Skill Representations](https://arxiv.org/abs/2505.08787)
*Hanjung Kim,Jaehyun Kang,Hyolim Kang,Meedeum Cho,Seon Joo Kim,Youngwoon Lee*

Main category: cs.RO

TLDR: UniSkill框架通过无标签的大规模跨体现视频数据学习技能表示，使人类视频提示提取的技能能有效迁移到仅基于机器人数据训练的机器人策略中。


<details>
  <summary>Details</summary>
Motivation: 模仿是人类学习新任务的基本机制，但将其应用于机器人存在挑战，主要由于人类与机器人在视觉外观和物理能力上的差异。现有方法依赖对齐的跨体现数据集，但大规模收集此类数据困难。

Method: 提出UniSkill框架，从无标签的大规模跨体现视频数据中学习与体现无关的技能表示，实现人类视频提示技能向机器人策略的迁移。

Result: 在仿真和真实环境中的实验表明，跨体现技能能有效指导机器人选择合适动作，即使面对未见过的视频提示。

Conclusion: UniSkill为跨体现技能迁移提供了一种无标签、可扩展的解决方案，展示了在机器人任务中的实际应用潜力。
...
### [283] [Scaling Multi Agent Reinforcement Learning for Underwater Acoustic Tracking via Autonomous Vehicles](https://arxiv.org/abs/2505.08222)
*Matteo Gallici,Ivan Masmitja,Mario Martín*

Main category: cs.RO

TLDR: 提出了一种基于迭代蒸馏和Transformer架构的方法，用于高效训练多智能体强化学习（MARL），显著提升了计算效率，并在高保真模拟中实现了低跟踪误差。


<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习在复杂海洋环境中控制自动驾驶车辆（AV）时面临计算效率低下的问题，尤其是在多车辆场景中。

Method: 采用迭代蒸馏技术将高保真模拟转移到简化的GPU加速环境中，并设计了一种Transformer架构（TransfMAPPO）以提升样本效率。

Result: 实现了高达30,000倍的加速，并在高保真模拟中保持跟踪误差低于5米。

Conclusion: 该方法为自动驾驶车队控制提供了可扩展的框架，弥合了大规模MARL训练与高保真部署之间的差距。
...
### [284] [Adaptive Diffusion Policy Optimization for Robotic Manipulation](https://arxiv.org/abs/2505.08376)
*Huiyun Jiang,Zhuang Yang*

Main category: cs.RO

TLDR: 本文提出了一种基于Adam的扩散策略优化（ADPO）框架，用于快速稳定地优化扩散模型在强化学习中的策略，并在标准机器人任务中验证了其优越性。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在强化学习中展现出潜力，但如何快速稳定地优化扩散策略仍缺乏研究。

Method: 提出ADPO框架，利用自适应梯度下降法微调扩散策略，并在标准机器人任务中进行实验验证。
...
### [285] [Reinforcement Learning-based Fault-Tolerant Control for Quadrotor with Online Transformer Adaptation](https://arxiv.org/abs/2505.08223)
*Dohyun Kim,Jayden Dongwoo Lee,Hyochoong Bang,Jungho Bae*

Main category: cs.RO

TLDR: 提出了一种基于强化学习和Transformer的故障容错控制框架，用于多旋翼飞行器，无需重新训练即可适应新配置。
...
Result: 在仿真中成功率达95%，位置误差0.129米，优于现有方法。

Conclusion: 该框架提升了多旋翼的适应性和可靠性，适用于动态不确定环境。
...
### [286] [Continuous World Coverage Path Planning for Fixed-Wing UAVs using Deep Reinforcement Learning](https://arxiv.org/abs/2505.08382)
*Mirco Theile,Andres R. Zapata Rodriguez,Marco Caccamo,Alberto L. Sangiovanni-Vincentelli*

Main category: cs.RO

TLDR: 论文提出了一种基于连续环境的无人机覆盖路径规划方法，通过强化学习优化能量消耗。


<details>
  <summary>Details</summary>
Motivation: 传统离散网格方法无法满足无人机在真实世界中的高效连续运动需求，需解决能量消耗问题。

Method: 使用可变大小轴对齐矩形建模环境，结合曲率约束的Bézier曲线规划路径，采用基于动作映射的自适应课程Soft Actor-Critic算法训练强化学习代理。

Result: 在生成和手工设计的场景中，方法能有效学习能量高效的覆盖策略。

Conclusion: 该方法在连续环境中实现了无人机覆盖路径规划的能量优化，具有实际应用潜力。
...
### [287] [Parameter Estimation using Reinforcement Learning Causal Curiosity: Limits and Challenges](https://arxiv.org/abs/2505.08453)
*Miguel Arana-Catania,Weisi Guo*

Main category: cs.RO

TLDR: 本文分析了强化学习方法Causal Curiosity，评估其在无需直接测量的情况下估计系统动态因果因素的能力，并首次对其测量精度、敏感性和混杂因素分离能力进行了分析。


<details>
  <summary>Details</summary>
Motivation: 因果理解在科学与工程中至关重要，特别是在优化复杂系统或建模未知环境时。本文旨在评估Causal Curiosity方法的潜力与局限性。

Method: 采用Causal Curiosity方法，分析其在机器人操纵器上的测量精度、敏感性和混杂因素分离能力。

Result: 揭示了该方法的当前局限性和未来潜力，并提出了改进设计以提高其在现实复杂场景中的应用效率。

Conclusion: 通过分析，本文为Causal Curiosity方法的优化提供了建议，以更好地适应实际应用需求。
...
### [288] [Automatic Curriculum Learning for Driving Scenarios: Towards Robust and Efficient Reinforcement Learning](https://arxiv.org/abs/2505.08264)
*Ahmed Abouelazm,Tim Weinstein,Tim Joseph,Philip Schörner,J. Marius Zöllner*

Main category: cs.RO

TLDR: 提出了一种自动课程学习框架，用于动态生成适应性强驾驶场景，提升RL自动驾驶代理的泛化能力和训练效率。


<details>
  <summary>Details</summary>
Motivation: 传统RL训练在固定场景中，泛化能力差；域随机化虽能缓解但效率低。

Method: 引入自动课程学习框架，动态生成场景，基于代理能力调整复杂度。

Result: 在低/高交通密度下成功率分别提升9%和21%，训练收敛更快。

Conclusion: 自动课程学习可提升RL自动驾驶代理的鲁棒性和效率。
...
### [289] [From Seeing to Doing: Bridging Reasoning and Decision for Robotic Manipulation](https://arxiv.org/abs/2505.08548)
*Yifu Yuan,Haiqin Cui,Yibin Chen,Zibin Dong,Fei Ni,Longxin Kou,Jinyi Liu,Pengyi Li,Yan Zheng,Jianye Hao*

Main category: cs.RO

TLDR: FSD模型通过空间关系推理生成中间表示，显著提升了机器人操作的零样本性能。
...
Result: 在8个基准测试和VABench中表现优异，零样本能力显著提升，SimperEnv成功率达54.1%，真实任务达72%。

Conclusion: FSD在机器人操作中展现出强大的泛化能力和性能优势。
...
### [290] [A Comparative Study of Human Activity Recognition: Motion, Tactile, and multi-modal Approaches](https://arxiv.org/abs/2505.08657)
*Valerio Belcamino,Nhat Minh Dinh Le,Quan Khanh Luu,Alessandro Carfì,Van Anh Ho,Fulvio Mastrogiovanni*

Main category: cs.RO

TLDR: 该研究评估了基于视觉的触觉传感器在15种活动分类中的表现，并与基于IMU的数据手套进行比较，提出了结合触觉和运动数据的多模态框架。


<details>
  <summary>Details</summary>
Motivation: 提升人机协作（HRC）中的人类活动识别（HAR）能力，通过整合触觉和运动数据的互补优势。

Method: 比较了三种方法：基于运动的分类（MBC）、基于触觉的分类（TBC）和多模态分类（MMC），并通过离线和在线验证评估性能。

Result: 多模态方法在性能上优于单一模态方法，展示了触觉与运动数据结合的潜力。

Conclusion: 整合触觉和运动传感可以显著提升协作机器人中的人类活动识别系统。
...
### [291] [A Social Robot with Inner Speech for Dietary Guidance](https://arxiv.org/abs/2505.08664)
*Valerio Belcamino,Alessandro Carfì,Valeria Seidita,Fulvio Mastrogiovanni,Antonio Chella*

Main category: cs.RO

TLDR: 研究探讨了如何利用内心独白增强社交机器人在饮食建议中的透明度和信任度，通过显式化推理过程提升人机交互的自然性和可信度。


<details>
  <summary>Details</summary>
Motivation: 在医疗场景中，信任机器人助手依赖于准确的建议和拟人化对话，内心独白可以提升透明度和交互体验。

Method: 开发了具备内心独白功能的社交机器人，结合大语言模型和知识图谱，验证用户输入、优化推理并提供清晰解释。

Result: 通过计算效率测试和小型用户研究验证了内心独白在解释机器人行为中的可靠性。

Conclusion: 内心独白显著增强了人机交互的透明度和信任，尤其在医疗场景中效果显著。
...
### [292] [Understanding molecular ratios in the carbon and oxygen poor outer Milky Way with interpretable machine learning](https://arxiv.org/abs/2505.08410)
*Gijs Vermariën,Serena Viti,Johannes Heyl,Francesco Fontani*

Main category: astro-ph.GA

TLDR: 论文研究了银河系外缘低金属丰度区域的分子线比率，通过可解释机器学习方法（SHAP和UMAP）分析了碳和氧化学的物理环境联系。


<details>
  <summary>Details</summary>
Motivation: 银河系外缘的金属丰度低于太阳附近区域，但仍有多种分子被检测到。研究分子线比率有助于理解这些区域的化学和物理特性。

Method: 使用UCLCHEM生成的大规模化学模型网格，结合经典分析和可解释机器学习（SHAP和UMAP）方法，研究了低氧和低碳初始丰度分子云的性质。

Result: 温度和密度是最重要的特征，但碳和氧丰度在部分参数空间中也很重要。UMAP可用于对不同类型的比率进行分组。

Conclusion: 所选比率对碳初始丰度、温度和密度变化敏感，尤其是CN/HCN和HNC/HCN比率。CS/SO比率对氧丰度敏感。
...
### [293] [Improving Unsupervised Task-driven Models of Ventral Visual Stream via Relative Position Predictivity](https://arxiv.org/abs/2505.08316)
*Dazhong Rong,Hao Dong,Xing Gao,Jiyu Wei,Di Hong,Yaoyao Hao,Qinming He,Yueming Wang*

Main category: cs.CE

TLDR: 论文提出了一种结合相对位置预测和对比学习的新方法，以更全面地建模腹侧视觉流（VVS），并验证了其在物体识别和脑相似性上的提升。


<details>
  <summary>Details</summary>
Motivation: 当前无监督任务驱动方法仅通过对比学习建模VVS，忽视了其在位置感知（如相对位置预测）中的功能。论文旨在填补这一空白。

Method: 提出了一种结合相对位置预测和对比学习的无监督任务驱动方法。

Result: 实验表明，新方法显著提升了物体识别性能，同时增强了相对位置预测能力和模型脑相似性。

Conclusion: 研究从计算角度证明了VVS在位置感知（尤其是相对位置预测）中的重要作用。
...
### [294] [Contrastive Normalizing Flows for Uncertainty-Aware Parameter Estimation](https://arxiv.org/abs/2505.08709)
*Ibrahim Elsharkawy,Yonatan Kahn*

Main category: physics.data-an

TLDR: 论文提出了一种基于对比归一化流（CNF）的新方法，用于解决高能物理（HEP）中数据分布偏移下的不确定性感知参数估计问题。该方法在HiggsML Uncertainty Challenge数据集上表现优异。
...
Method: 通过对比归一化流（CNF）嵌入数据和参数，生成可调节的对比分布，从而在偏移数据分布下实现鲁棒分类。结合分类器和频率学方法，提供稳健的参数估计和不确定性量化。

Result: 在HiggsML Uncertainty Challenge数据集上取得了最佳性能，证明了CNF方法在数据分布偏移下的有效性。

Conclusion: CNF方法结合分类器和频率学技术，为数据分布偏移下的参数估计和不确定性量化提供了理论支持和实证效果。
...
### [295] [Justified Evidence Collection for Argument-based AI Fairness Assurance](https://arxiv.org/abs/2505.08064)
*Alpay Sabuncuoglu,Christopher Burr,Carsten Maple*

Main category: cs.HC

TLDR: 本文提出了一种基于系统工程的方法，通过动态论证保障框架和软件工具，分两阶段实现AI系统的公平性治理。


<details>
  <summary>Details</summary>
Motivation: 确保AI系统的公平性是一个复杂的社会技术挑战，需要从需求定义到模型部署的全生命周期持续监督。动态论证保障方法已被用于评估和缓解AI系统的安全风险，本文将其扩展到公平性和可解释性等更广泛的规范性目标。

Method: 提出一个系统工程驱动的框架，分两阶段操作：1）需求规划阶段，多学科团队通过公平治理流程定义目标和主张；2）持续监控阶段，通过接口收集模型、数据等证据动态支持论证。

Result: 通过金融领域的案例研究验证了框架的有效性，特别是在支持公平性相关论证方面。

Conclusion: 该框架为AI系统的公平性治理提供了一种系统化和动态化的方法，适用于多学科和多利益相关方的协作。
...
### [296] [Communication Styles and Reader Preferences of LLM and Human Experts in Explaining Health Information](https://arxiv.org/abs/2505.08143)
*Jiawei Zhou,Kritika Venkatachalam,Minje Choi,Koustuv Saha,Munmun De Choudhury*

Main category: cs.HC

TLDR: 研究评估了大型语言模型（LLM）在健康信息事实核查中的沟通风格，发现LLM在说服策略、确定性表达和社会价值观一致性上得分较低，但人类评估显示对LLM内容的偏好。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在信息辅助中的广泛应用，需要研究其与人类沟通风格和价值观的一致性，特别是在健康信息事实核查这一关键领域。

Method: 通过比较1498条健康错误信息的权威解释与LLM生成的内容，从信息、发送者和接收者三个维度评估沟通风格，并进行了99名参与者的盲评。
...
### [297] [VizCV: AI-assisted visualization of researchers' publications tracks](https://arxiv.org/abs/2505.08691)
*Vladimír Lazárik,Marco Agus,Barbora Kozlíková,Pere-Pau Vázquez*

Main category: cs.HC

TLDR: VizCV是一个基于Web的端到端可视化分析框架，用于交互式探索研究人员的科学生涯轨迹，结合AI辅助分析和自动化报告功能。


<details>
  <summary>Details</summary>
Motivation: 评估科学家和研究组的学术记录演变对管理学术环境和职业规划至关重要。

Method: VizCV通过三个维度建模职业发展：研究主题演变、出版记录及影响、合作动态。结合AI技术分析并提供自动化解释。

Result: 系统支持交互式探索和多视角分析，包括主题轨迹、影响力增长和合作网络变化。

Conclusion: VizCV通过AI/ML技术提供职业发展的多维度分析和自动化报告，为学术评估和规划提供支持。
...
### [298] [OMGM: Orchestrate Multiple Granularities and Modalities for Efficient Multimodal Retrieval](https://arxiv.org/abs/2505.07879)
*Wei Yang,Jingjing Fu,Rui Wang,Jinyu Wang,Lei Song,Jiang Bian*

Main category: cs.IR

TLDR: 提出了一种多模态RAG系统，通过粗到细的多步检索方法提升知识库视觉问答（KB-VQA）的效果。
...
Method: 采用粗到细的多步检索策略，包括初始粗粒度检索、多模态融合重排序和细粒度文本重排序。

Result: 在InfoSeek和Encyclopedic-VQA基准测试中取得最先进的检索性能和竞争力强的回答结果。

Conclusion: 该系统通过多模态和粒度协调显著提升了KB-VQA的效果。
...
### [299] [Efficient and Reproducible Biomedical Question Answering using Retrieval Augmented Generation](https://arxiv.org/abs/2505.07917)
*Linus Stuhlmann,Michael Alexander Saxer,Jonathan Fürst*

Main category: cs.IR

TLDR: 该研究系统评估了生物医学问答系统中的检索增强生成（RAG）方法，比较了不同检索策略和响应时间的权衡，最终确定了BM25与MedCPT结合的最优方案。


<details>
  <summary>Details</summary>
Motivation: 生物医学问答系统需要高效的检索和生成组件以确保准确性、效率和可扩展性，研究旨在探索最佳检索策略。

Method: 评估了BM25、BioBERT、MedCPT等检索方法及Elasticsearch、MongoDB等数据存储，并在PubMed子集和全集中测试性能。

Result: BM25检索50篇文档后用MedCPT重排，在准确性（0.90）、召回率（0.90）和响应时间（1.91s）上达到最优平衡。
...
### [300] [Hyperbolic Contrastive Learning with Model-augmentation for Knowledge-aware Recommendation](https://arxiv.org/abs/2505.08157)
*Shengyin Sun,Chen Ma*

Main category: cs.IR

TLDR: 论文提出了一种基于双曲对比学习的知识感知推荐方法，通过Lorentzian知识聚合机制和模型级增强技术，解决了现有方法在层次结构捕捉和用户偏好偏移上的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于对比学习的知识感知推荐方法难以有效捕捉用户-物品二分图和知识图的层次结构，且通过扰动图结构生成正样本可能导致用户偏好偏移。

Method: 设计了Lorentzian知识聚合机制以捕捉层次结构，并提出三种模型级增强技术辅助双曲对比学习，避免偏好偏移。

Result: 实验表明，所提方法在性能上显著优于现有基线方法，最大提升达11.03%。

Conclusion: 通过双曲对比学习和模型级增强技术，论文有效解决了层次结构捕捉和偏好偏移问题，提升了推荐性能。
...
### [301] [Monocular Online Reconstruction with Enhanced Detail Preservation](https://arxiv.org/abs/2505.07887)
*Songyin Wu,Zhaoyang Lv,Yufeng Zhu,Duncan Frost,Zhengqin Li,Ling-Qi Yan,Carl Ren,Richard Newcombe,Zhao Dong*

Main category: cs.GR

TLDR: 提出了一种基于3D高斯分布的在线密集映射框架，用于从单目图像流中重建真实感细节。
...
Result: 与现有RGB-only和RGB-D方法相比，实现了更优的重建质量和计算效率。
...
### [302] [PosterO: Structuring Layout Trees to Enable Language Models in Generalized Content-Aware Layout Generation](https://arxiv.org/abs/2505.07843)
*HsiaoYuan Hsu,Yuxin Peng*

Main category: cs.GR

TLDR: PosterO提出了一种基于布局的方法，利用大型语言模型（LLMs）生成多样化的海报布局，解决了现有方法在通用场景下多样性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有海报设计方法主要关注图像增强，忽视了布局多样性和形状变化元素的处理，无法满足通用设计需求。

Method: 通过SVG语言将布局结构化为树，结合设计意图向量化和层次节点表示，利用LLMs进行上下文学习生成新布局树。

Result: PosterO在多个基准测试中实现了最先进的性能，能够为给定图像生成视觉吸引力的布局。

Conclusion: PosterO展示了在通用场景下生成多样化布局的能力，并推出了首个多用途海报数据集PStylish7，为未来研究提供了挑战。
...
### [303] [ACT-R: Adaptive Camera Trajectories for 3D Reconstruction from Single Image](https://arxiv.org/abs/2505.08239)
*Yizhi Wang,Mingrui Zhao,Ali Mahdavi-Amiri,Hao Zhang*

Main category: cs.GR

TLDR: 提出自适应视图规划方法，通过动态相机轨迹优化遮挡区域的可视性，提升单视图3D重建的遮挡揭示和3D一致性。


<details>
  <summary>Details</summary>
Motivation: 解决多视图合成中遮挡揭示和3D一致性的问题，避免传统方法中无序或固定视图的局限性。

Method: 提出自适应相机轨迹（ACT）技术，动态生成最优相机轨道，结合视频扩散模型生成新视图，并通过多视图3D重建模型完成最终重建。

Result: 在GSO数据集上显著优于现有方法，定量和定性均表现优异。

Conclusion: 自适应视图规划方法高效且无需运行时训练，显著提升了3D重建质量。
...
### [304] [M3G: Multi-Granular Gesture Generator for Audio-Driven Full-Body Human Motion Synthesis](https://arxiv.org/abs/2505.08293)
*Zhizhuo Yin,Yuk Hang Tsui,Pan Hui*

Main category: cs.GR

TLDR: 提出了一种名为M3G的新框架，用于从音频生成全身手势，解决了现有方法因固定粒度而无法建模不同手势模式的问题。


<details>
  <summary>Details</summary>
Motivation: 现有系统因手势令牌的固定粒度而无法有效建模不同手势模式，限制了虚拟角色手势生成的表达能力。

Method: 提出M3G框架，包括多粒度VQ-VAE（MGVQ-VAE）用于手势模式的分层令牌化与重建，以及多粒度令牌预测器从音频中提取信息并预测手势令牌。

Result: 实验表明，M3G在生成自然且富有表现力的全身手势方面优于现有方法。

Conclusion: M3G通过多粒度建模显著提升了音频驱动手势生成的质量和表现力。
...
### [305] [Claycode: Stylable and Deformable 2D Scannable Codes](https://arxiv.org/abs/2505.08666)
*Marco Maida,Alberto Crescini,Marco Perronet,Elena Camuffo*

Main category: cs.GR

TLDR: Claycode是一种新型2D可扫描码，支持高度风格化和变形，基于树结构编码，优于传统矩阵码。


<details>
  <summary>Details</summary>
Motivation: 传统矩阵码（如QR码）在风格化和变形场景中表现不佳，Claycode旨在解决这一问题。

Method: 通过树结构编码信息，将比特映射到拓扑树中，并以颜色区域嵌套形式绘制在目标多边形内。

Result: Claycode在高度变形场景中表现优异，功能不受风格化影响。

Conclusion: Claycode为2D可扫描码提供了更灵活的设计和更强的适应性。
...
### [306] [CAD-Coder:Text-Guided CAD Files Code Generation](https://arxiv.org/abs/2505.08686)
*Changqi He,Shuhan Zhang,Liguo Zhang,Jiajun Miao*

Main category: cs.GR

TLDR: CAD-Coder是一个将自然语言指令转换为可编辑CAD脚本代码的框架，解决了现有生成方法缺乏交互性和几何注释的问题。


<details>
  <summary>Details</summary>
Motivation: 传统CAD依赖专家手动绘制或修改现有库文件，无法快速个性化。生成式AI虽提供了便利，但现有方法生成的输出缺乏交互编辑性和几何注释。
...
Conclusion: CAD-Coder为个性化CAD生成提供了高效且实用的解决方案，特别适用于制造业。
...
### [307] [Safety and optimality in learning-based control at low computational cost](https://arxiv.org/abs/2505.08026)
*Dominik Baumann,Krzysztof Kowalczyk,Cristian R. Rojas,Koen Tiels,Pawel Wachel*

Main category: eess.SY

TLDR: CoLSafe是一种计算轻量级的安全学习算法，适用于大数据和低功耗设备。


<details>
  <summary>Details</summary>
Motivation: 为物理系统提供安全保证的机器学习方法通常计算成本高，难以应用于大数据和嵌入式设备。

Method: 提出CoLSafe算法，其计算复杂度随数据点数量亚线性增长。
...
### [308] [Diffusion-assisted Model Predictive Control Optimization for Power System Real-Time Operation](https://arxiv.org/abs/2505.08535)
*Linna Xu,Yongli Zhu*

Main category: eess.SY

TLDR: 提出了一种改进的模型预测控制（MPC）框架，结合扩散模型提升电力系统实时操作的负荷预测精度。


<details>
  <summary>Details</summary>
Motivation: 解决可再生能源主导的电力系统中缺乏明确状态转移规律的问题，提升负荷预测准确性。

Method: 利用扩散模型生成时间序列数据增强训练集，并通过模型识别过程推导系统动态。

Result: 在工业园系统和IEEE 30总线系统上的案例研究表明，扩散模型显著提高了负荷预测精度，且推断的系统动态适用于含风光发电的实时电网操作。

Conclusion: 改进的MPC框架结合扩散模型，有效提升了可再生能源电力系统的实时操作性能。
...
### [309] [A Survey of Deep Learning for Complex Speech Spectrograms](https://arxiv.org/abs/2505.08694)
*Yuying Xie,Zheng-Hua Tan*

Main category: eess.AS

TLDR: 本文综述了深度学习在复杂频谱图处理中的最新技术，包括网络架构、训练策略及在语音信号处理中的应用。


<details>
  <summary>Details</summary>
Motivation: 深度学习在语音信号处理中的快速发展，特别是复杂频谱图（包含幅度和相位信息）的分析与处理，需要系统性的总结与资源。

Method: 介绍了复杂频谱图及其特征，探讨了复数神经网络的关键组件和架构，以及针对复杂频谱图的训练策略和损失函数。

Result: 深度学习在相位恢复、语音增强和语音分离等应用中取得了显著进展。

Conclusion: 本文为语音信号处理和复数神经网络领域的研究者提供了有价值的参考资源。
...
### [310] [Diffusion-based supervised learning of generative models for efficient sampling of multimodal distributions](https://arxiv.org/abs/2505.07825)
*Hoang Tran,Zezhong Zhang,Feng Bao,Dan Lu,Guannan Zhang*

Main category: stat.ML

TLDR: 提出一种混合生成模型，用于高效采样高维多模态概率分布，解决传统蒙特卡洛方法在多模态分布中的局限性。
...
Result: 数值实验表明，该方法能有效处理100维内多模态分布，并成功应用于偏微分方程的贝叶斯反问题。

Conclusion: 该框架为高维多模态分布采样提供了一种高效解决方案，尤其适用于模态分离明显的场景。
...
### [311] [Wasserstein Distributionally Robust Nonparametric Regression](https://arxiv.org/abs/2505.07967)
*Changyu Liu,Yuling Jiao,Junhui Wang,Jian Huang*

Main category: stat.ML

TLDR: 本文研究了Wasserstein分布鲁棒非参数估计器的泛化性质，重点分析了模型误设的影响，并通过正则化效应和神经网络结构建立了非渐近误差界。


<details>
  <summary>Details</summary>
Motivation: 在非参数框架下，分布鲁棒优化的研究较少，尤其是在模型误设情况下，泛化性能可能受损。本文旨在填补这一空白。

Method: 通过分析分布扰动引起的正则化效应，并使用带Lipschitz约束的前馈神经网络，建立了非渐近误差界。

Result: 研究表明，不确定性水平和神经网络结构对泛化性能有显著影响，结果适用于Lipschitz和二次损失函数。

Conclusion: 提出的估计器在模拟研究和MNIST数据集应用中表现出鲁棒性，验证了方法的有效性。
...
### [312] [Sharp Gaussian approximations for Decentralized Federated Learning](https://arxiv.org/abs/2505.08125)
*Soham Bonnerjee,Sayar Karmakar,Wei Biao Wu*

Main category: stat.ML

TLDR: 本文提出了两种局部SGD的广义高斯近似结果，并探讨其应用。一是证明了局部SGD最终迭代的Berry-Esseen定理，支持有效的乘数自举程序；二是基于鲁棒性考虑，提出了两种时间均匀的高斯近似，支持基于高斯自举的对抗攻击检测测试。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在隐私敏感环境中广泛应用，局部SGD是其关键优化方法，但关于其收敛之外的渐近统计保证研究有限。

Method: 提出并证明了局部SGD的Berry-Esseen定理，并引入两种时间均匀的高斯近似方法。

Result: 理论结果支持有效的自举程序和对抗攻击检测测试，并通过大量仿真验证。

Conclusion: 本文扩展了局部SGD的统计理论，为联邦学习中的鲁棒性和统计推断提供了新工具。
...
### [313] [SIM-Shapley: A Stable and Computationally Efficient Approach to Shapley Value Approximation](https://arxiv.org/abs/2505.08198)
*Wangxuan Fan,Siqi Li,Doudou Zhou,Yohei Okada,Chuan Hong,Molei Liu,Nan Liu*

Main category: stat.ML

TLDR: SIM-Shapley是一种高效且稳定的Shapley值近似方法，通过随机优化技术显著降低计算成本，同时保持特征归因质量。
...
Method: 提出SIM-Shapley方法，基于随机优化技术，通过理论分析方差并证明线性Q收敛，实现高效稳定的Shapley值近似。

Result: 实验表明，SIM-Shapley计算时间减少85%，同时保持与基线方法相当的特征归因质量。

Conclusion: SIM-Shapley不仅适用于特征归因，还可扩展到更广泛的样本平均近似问题，为计算效率提升提供了新途径。
...
### [314] [Lie Group Symmetry Discovery and Enforcement Using Vector Fields](https://arxiv.org/abs/2505.08219)
*Ben Shaw,Sasidhar Kunapuli,Abram Magner,Kevin R. Moon*

Main category: stat.ML

TLDR: 论文探讨了对称性在机器学习中的重要性，并扩展了非仿射对称性发现的概念，同时提出了利用向量场强制执行对称性的方法。


<details>
  <summary>Details</summary>
Motivation: 研究对称性在机器学习中的优势，并探索如何通过向量场发现和强制执行对称性。

Method: 扩展非仿射对称性发现到神经网络，引入向量场强制执行对称性，并限制对称性搜索空间为无穷小等距。

Result: 提供了理论和实验材料，验证了对称性发现和强制执行的有效性。

Conclusion: 对称性发现和强制执行在机器学习中具有潜力，尤其是通过向量场和无穷小等距的限制。
...
### [315] [Iteratively reweighted kernel machines efficiently learn sparse functions](https://arxiv.org/abs/2505.08277)
*Libin Zhu,Damek Davis,Dmitriy Drusvyatskiy,Maryam Fazel*

Main category: stat.ML

TLDR: 论文探讨了神经网络的成功并非独有，经典核方法也能实现低维数据表示和分层结构学习。


<details>
  <summary>Details</summary>
Motivation: 研究神经网络成功的原因，并验证核方法是否也能实现类似效果。

Method: 通过核预测器的导数检测关键坐标，并迭代重加权数据以训练核机器。

Result: 核方法能以低样本复杂度学习分层多项式。

Conclusion: 核方法在低维表示和分层学习方面与神经网络具有可比性。
...
### [316] [Learning Treatment Allocations with Risk Control Under Partial Identifiability](https://arxiv.org/abs/2505.08378)
*Sofia Ek,Dave Zachariah*

Main category: stat.ML

TLDR: 提出了一种在部分可识别设置下控制治疗风险的认证学习方法。


<details>
  <summary>Details</summary>
Motivation: 精准医疗中，许多治疗伴随不可忽视的副作用，需避免对未受益患者造成不必要伤害。

Method: 提出了一种认证学习方法，在有限样本下控制治疗风险。
...
### [317] [neuralGAM: An R Package for Fitting Generalized Additive Neural Networks](https://arxiv.org/abs/2505.08610)
*Ines Ortega-Fernandez,Marta Sestelo*

Main category: stat.ML

TLDR: 介绍了一个名为neuralGAM的R包，通过基于广义可加模型的神经网络拓扑结构，解决神经网络“黑箱”问题，提供高精度且可解释的深度学习模型。


<details>
  <summary>Details</summary>
Motivation: 神经网络在多种任务中表现优异，但其“黑箱”特性导致决策过程难以理解，限制了其应用。

Method: neuralGAM包采用广义可加模型（GAM）的神经网络拓扑，为每个特征独立拟合神经网络，估计其对输出变量的贡献。

Result: neuralGAM提供了灵活的训练框架，不限制神经网络架构，并在合成和真实数据中验证了其有效性。

Conclusion: neuralGAM成功解决了神经网络的可解释性问题，同时保持了高精度，适用于多种应用场景。
...
### [318] [Uncertainty-Aware Surrogate-based Amortized Bayesian Inference for Computationally Expensive Models](https://arxiv.org/abs/2505.08683)
*Stefania Scheurer,Philipp Reiser,Tim Brünnette,Wolfgang Nowak,Anneli Guthke,Paul-Christian Bürkner*

Main category: stat.ML

TLDR: 提出了一种结合代理模型和摊销贝叶斯推断的框架（UA-SABI），通过显式量化代理不确定性，实现快速可靠的贝叶斯推断。


<details>
  <summary>Details</summary>
Motivation: 传统贝叶斯推断方法（如MCMC和ABI）计算成本高，代理模型虽能降低计算成本，但可能引入近似误差和不确定性。

Method: 提出UA-SABI框架，结合代理模型和ABI，显式量化并传播代理不确定性。

Result: 实验表明，该方法能在时间限制下实现快速、可靠的贝叶斯推断。

Conclusion: UA-SABI为计算昂贵的模型提供了一种高效且可靠的贝叶斯推断解决方案。
...
### [319] [Continuous Temporal Learning of Probability Distributions via Neural ODEs with Applications in Continuous Glucose Monitoring Data](https://arxiv.org/abs/2505.08698)
*Antonio Álvarez-López,Marcos Matabuena*

Main category: stat.ML

TLDR: 提出了一种基于高斯混合和神经ODE的概率模型，用于捕捉连续时间随机过程中概率分布的动态变化，并通过MMD非参数估计时间依赖性函数。


<details>
  <summary>Details</summary>
Motivation: 研究生物标志物（如血糖）的时间依赖性分布变化，以反映慢性疾病（如糖尿病）的进展。

Method: 使用高斯混合模型和神经ODE参数化时间依赖性函数，通过MMD进行非参数估计。

Result: 模型在估计准确性上表现优异，具有高解释性和计算效率，并能检测细微的时间变化。

Conclusion: 模型在数字临床试验数据中展示了实用性，能够量化干预措施对血糖分布的影响，并为对照组和治疗组提供新的数学和临床比较视角。
...
### [320] [PCS-UQ: Uncertainty Quantification via the Predictability-Computability-Stability Framework](https://arxiv.org/abs/2505.08784)
*Abhineet Agarwal,Michael Xiao,Rebecca Barter,Omer Ronen,Boyu Fan,Bin Yu*

Main category: stat.ML

TLDR: 提出了一种基于PCS框架的UQ方法，解决了传统UQ方法对模型错误设定的依赖性问题，并通过模型筛选和校准方案提高了不确定性估计的可靠性和局部适应性。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习模型在高风险领域的广泛应用，可靠的不确定性量化（UQ）对模型的安全性和可靠性至关重要。传统UQ方法依赖真实生成模型且对模型错误设定不鲁棒，而共形推断虽适用于任意模型但未考虑模型选择，导致区间过大。

Method: 基于PCS框架，提出PCS-UQ方法，通过预测检查筛选模型，并在多个自助样本上评估模型变异性，结合新型校准方案提高局部适应性。

Result: 在17个回归和6个分类数据集上，PCS-UQ实现了目标覆盖率，区间宽度比共形方法减少约20%。在子组分析中，PCS-UQ通常达到目标覆盖率，而共形方法失败。对于大型深度学习模型，提出了计算高效的近似方案。

Conclusion: PCS-UQ通过模型筛选和校准方案显著提升了不确定性量化的可靠性和效率，适用于多种数据集和模型类型。
...
### [321] [Building-Block Aware Generative Modeling for 3D Crystals of Metal Organic Frameworks](https://arxiv.org/abs/2505.08531)
*Chenru Duan,Aditya Nandy,Sizhan Liu,Yuanqi Du,Liu He,Yi Qu,Haojun Jia,Jin-Hu Dou*

Main category: physics.chem-ph

TLDR: BBA MOF Diffusion是一种生成模型，能够设计包含1000个原子的大单元MOF，具有几何有效性、新颖性和多样性。


<details>
  <summary>Details</summary>
Motivation: MOF的设计空间巨大，传统方法难以覆盖，需要一种高效生成模型。

Method: 采用SE(3)-等变扩散模型，学习3D全原子表示，并明确编码晶体拓扑网络。

Result: 模型生成的MOF具有高几何有效性、新颖性和多样性，并成功合成了一种预测的MOF。

Conclusion: BBA MOF Diffusion为合成高性能MOF提供了实用途径。
...
### [322] [ai.txt: A Domain-Specific Language for Guiding AI Interactions with the Internet](https://arxiv.org/abs/2505.07834)
*Yuekang Li,Wei Song,Bangshuo Zhu,Dong Gong,Yi Liu,Gelei Deng,Chunyang Chen,Lei Ma,Jun Sun,Toby Walsh,Jingling Xue*

Main category: cs.NI

TLDR: ai.txt是一种新的领域特定语言（DSL），旨在规范AI模型、代理与网络内容的交互，弥补robots.txt的不足。
...
Method: ai.txt扩展了基于URL的访问控制，支持元素级精准调控和自然语言指令，并提供开发工具和合规机制。

Result: 初步实验和案例研究表明，ai.txt在XML和自然语言提示两种合规机制下有效。

Conclusion: ai.txt有助于规范AI与互联网的交互，促进数字生态中AI的负责任使用。
...
### [323] [Graph-Based Floor Separation Using Node Embeddings and Clustering of WiFi Trajectories](https://arxiv.org/abs/2505.08088)
*Rabia Yasa Kostas,Kahraman Kostas*

Main category: cs.NI

TLDR: 本文提出了一种基于图的Wi-Fi指纹轨迹楼层分离方法，解决了室内垂直定位的挑战，性能优于传统算法。


<details>
  <summary>Details</summary>
Motivation: 室内定位系统在复杂多层环境中日益重要，但垂直定位仍具挑战性。

Method: 构建图模型，节点为Wi-Fi指纹，边权重基于信号相似性和上下文转换；使用Node2Vec生成低维嵌入，K-means聚类识别楼层。

Result: 在Huawei数据集上，准确率68.97%，F1分数61.99%，调整兰德指数57.19%。

Conclusion: 该方法对信号噪声和建筑复杂性具有鲁棒性，为楼层定位提供了可扩展方案。
...
### [324] [Intelligent Product 3.0: Decentralised AI Agents and Web3 Intelligence Standards](https://arxiv.org/abs/2505.07835)
*Alex C. Y. Wong,Duncan McFarlane,C. Ellarby,M. Lee,M. Kuok*

Main category: cs.NI

TLDR: 本文回顾了25年前提出的智能产品规范，结合区块链、Web3和人工智能的进展，提出了智能产品3.0的新规范。


<details>
  <summary>Details</summary>
Motivation: 探讨如何利用去中心化身份、区块链和AI技术，实现物理产品与AI的无缝交互。

Method: 通过分析去中心化身份、区块链产品信息和AI协作，提出智能产品3.0的新规范。

Result: 展示了去中心化和AI驱动的能力如何促进物理AI与日常产品的无缝交互。

Conclusion: 智能产品3.0规范为未来产品智能化提供了新的方向。
...
### [325] [Efficient Telecom Specific LLM: TSLAM-Mini with QLoRA and Digital Twin Data](https://arxiv.org/abs/2505.07877)
*Vignesh Ethiraj,Divya Vijay,Sidhanth Menon,Heblin Berscilla*

Main category: cs.NI

TLDR: 论文通过微调TSLAM-Mini模型，结合电信领域专用数据集和QLoRA技术，显著提升了模型在电信应用中的性能。


<details>
  <summary>Details</summary>
Motivation: 通用大语言模型在电信领域表现不佳，需通过领域专用优化提升性能。

Method: 使用100,000个电信领域样本微调TSLAM-Mini模型，采用QLoRA技术提高效率。

Result: TSLAM-Mini在电信应用中表现优异，验证了领域专用数据集和PEFT方法的有效性。

Conclusion: 领域专用数据集和高效微调技术可显著提升模型在电信领域的性能。
...
### [326] [ML-Enabled Eavesdropper Detection in Beyond 5G IIoT Networks](https://arxiv.org/abs/2505.07837)
*Maria-Lamprini A. Bartsioka,Ioannis A. Bartsiokas,Panagiotis K. Gkonis,Dimitra I. Kaklamani,Iakovos S. Venieris*

Main category: cs.NI

TLDR: 论文探讨了在5G/B5G网络中利用ML/DL技术检测窃听行为，DCNN和RF模型表现优异，准确率接近100%。


<details>
  <summary>Details</summary>
Motivation: 传统加密方法在去中心化IIoT环境中面临可扩展性和复杂性挑战，AI驱动的物理层技术成为解决方案。

Method: 使用模拟工业B5G网络评估RF、DCNN和LSTM模型，基于CSI、位置和传输功率分类用户。
...
### [327] [Token Communication-Driven Multimodal Large Models in Resource-Constrained Multiuser Networks](https://arxiv.org/abs/2505.07841)
*Junhe Zhang,Wanli Ni,Pengwei Wang,Dongyu Wang*

Main category: cs.NI

TLDR: 提出了一种基于令牌通信的范式，用于在资源受限网络中部署多模态大模型（MLMs），通过对比分割微调和轻量压缩技术优化传输效率，实验显示测试准确率提升13.7%。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大模型在带宽、计算资源和低信噪比条件下部署的挑战。

Method: 1) 对比分割微调方法将多模态输入映射到共享特征空间；2) 轻量压缩技术减少令牌传输大小。

Result: 测试准确率提升13.7%，收敛速度更快。
...
### [328] [VoI-Driven Joint Optimization of Control and Communication in Vehicular Digital Twin Network](https://arxiv.org/abs/2505.07892)
*Lei Lei,Kan Zheng,Jie Mei,Xuemin,Shen*

Main category: cs.NI

TLDR: 本文提出了一种车载数字孪生网络（VDTN）架构，通过联合优化控制与通信，利用深度强化学习（DRL）模块实现最优策略，并在车队场景中验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 6G网络与数字孪生技术的结合为车载网络提供了优化通信与控制性能的潜力，推动了VDTN的提出。

Method: 提出VDTN架构，定义基于控制性能的信息价值（VoI）概念，并设计包含两个DRL模块的联合优化框架。

Result: 仿真实验表明，该框架在车队场景中能有效优化控制与通信性能。

Conclusion: VDTN及其联合优化框架为车载网络的性能提升提供了可行方案。
...
### [329] [Channel Fingerprint Construction for Massive MIMO: A Deep Conditional Generative Approach](https://arxiv.org/abs/2505.07893)
*Zhenzhou Jin,Li You,Xudong Li,Zhen Gao,Yuanwei Liu,Xiang-Gen Xia,Xiqi Gao*

Main category: cs.NI

TLDR: 提出了一种基于条件生成扩散模型（CGDM）的CF twins方法，用于将粗粒度信道指纹（CF）转换为细粒度CF，显著提升了重建性能。


<details>
  <summary>Details</summary>
Motivation: 由于实际感知节点和测试车辆的成本限制，粗粒度CF不足以支持无线收发器设计，需要一种方法将其转换为细粒度CF。

Method: 设计了CGDM作为CF twins的计算核心，利用变分推理技术推导ELBO，并通过去噪神经网络优化和轻量化技术（如剪枝和知识蒸馏）实现高效生成。

Result: 实验结果表明，该方法在重建性能上显著优于基线，且在不同放大因子下的零样本测试中表现出良好的可扩展性和泛化能力。

Conclusion: 提出的CGDM方法为粗粒度到细粒度CF的转换提供了高效解决方案，具有实际应用潜力。
...
### [330] [EnvCDiff: Joint Refinement of Environmental Information and Channel Fingerprints via Conditional Generative Diffusion Model](https://arxiv.org/abs/2505.07894)
*Zhenzhou Jin,Li You,Xiang-Gen Xia,Xiqi Gao*

Main category: cs.NI

TLDR: 论文提出了一种深度条件生成学习方法（CDiff），用于从粗粒度环境信息和信道指纹（CF）中重构细粒度的环境感知CF（EnvCF）。


<details>
  <summary>Details</summary>
Motivation: 当前环境感知通信中，环境信息和CF的获取通常较为粗粒度，不足以指导无线传输设计。

Method: 采用定制的条件生成扩散模型（CDiff），同时优化环境信息和CF，重构细粒度的EnvCF。

Result: 实验结果表明，CDiff在EnvCF构建性能上显著优于基线方法。

Conclusion: CDiff方法为环境感知通信提供了有效的细粒度CF重构解决方案。
...
### [331] [Online Learning-based Adaptive Beam Switching for 6G Networks: Enhancing Efficiency and Resilience](https://arxiv.org/abs/2505.08032)
*Seyed Bagher Hashemi Natanzi,Zhicong Zhu,Bo Tang*

Main category: cs.NI

TLDR: 提出了一种基于深度强化学习（DRL）的在线学习框架，用于6G网络中的自适应波束切换，通过增强状态表示、GRU架构和优先经验回放优化实时波束管理。


<details>
  <summary>Details</summary>
Motivation: 6G网络中高频、移动性和阻塞问题对自适应波束切换提出了挑战，需要更高效的解决方案。

Method: 采用DRL框架，结合速度与阻塞历史的状态表示、GRU架构和优先经验回放，实现实时波束优化。

Result: 在时间相关阻塞场景下，该方法显著提升了SNR、吞吐量和准确性，优于传统启发式方法和反应式MAB基线。

Conclusion: 证明了记忆和优先学习对6G波束管理的有效性，同时确认MAB作为强基线的作用。
...
### [332] [Mobile Jamming Mitigation in 5G Networks: A MUSIC-Based Adaptive Beamforming Approach](https://arxiv.org/abs/2505.08046)
*Olivia Holguin,Rachel Donati,Seyed bagher Hashemi Natanzi,Bo Tang*

Main category: cs.NI

TLDR: 提出了一种结合MUSIC、MVDR和机器学习的智能抗干扰框架，显著提升5G网络在军事通信中的抗干扰能力。
...
Method: 集成MUSIC进行高分辨率DoA估计，MVDR波束成形抑制干扰，机器学习增强DoA预测。

Result: 在高速公路场景中，平均SNR提升9.58 dB（最高11.08 dB），DoA估计准确率达99.8%。

Conclusion: 该框架计算高效、适应动态干扰模式，优于传统方法，是5G通信安全的有效解决方案。
...
### [333] [Sub-diffraction terahertz backpropagation compressive imaging](https://arxiv.org/abs/2505.07839)
...
TLDR: 提出一种亚衍射太赫兹反向传播压缩成像技术，结合无训练神经网络和角谱传播理论，实现高分辨率成像。


<details>
  <summary>Details</summary>
Motivation: 解决太赫兹单像素成像（TSPI）在亚衍射尺度分辨率受限、实验条件苛刻和耗时的问题。

Method: 利用单色连续太赫兹波照射物体，通过硅片背面光激发载流子调制波，单点探测器记录，结合无训练神经网络和角谱传播理论重建图像。

Result: 实现~λ0/7的空间分辨率（λ0=833.3 μm），压缩比低至1.5625%，减少采样时间。
...
### [334] [Pose Estimation for Intra-cardiac Echocardiography Catheter via AI-Based Anatomical Understanding](https://arxiv.org/abs/2505.07851)
*Jaeyoung Huh,Ankur Kapoor,Young-Ho Kim*

Main category: eess.IV

TLDR: 提出了一种基于Vision Transformer的解剖感知姿态估计系统，仅通过ICE图像确定导管位置和方向，无需外部跟踪传感器。


<details>
  <summary>Details</summary>
Motivation: 现有导航方法依赖易受干扰的电磁跟踪或需手动调整，限制了ICE在心脏介入中的效率和准确性。

Method: 使用ViT模型处理ICE图像，通过16x16嵌入块和Transformer网络预测位置和方向，训练于851例临床数据。

Result: 平均位置误差9.48 mm，方向误差(16.13°, 8.98°, 10.47°)，验证了模型准确性。

Conclusion: 该系统提高了手术效率，减少操作负担，为无跟踪ICE导管定位提供了新方法。
...
### [335] [Computationally Efficient Diffusion Models in Medical Imaging: A Comprehensive Review](https://arxiv.org/abs/2505.07866)
*Abdullah,Tao Huang,Ickjai Lee,Euijoon Ahn*

Main category: eess.IV

TLDR: 扩散模型在计算机视觉中表现优异，但计算成本高。本文聚焦扩散生成模型的效率和推理时间，探讨其在自然和医学影像中的应用，并分类为DDPM、LDM和WDM三种模型。


<details>
  <summary>Details</summary>
Motivation: 解决扩散模型在训练和生成时的高计算成本问题，提升其在医学影像中的效率和实用性。

Method: 分析DDPM、LDM和WDM三种扩散模型的框架，填补计算复杂度差距。

Result: 扩散模型在医学影像中能快速生成高质量图像，但仍存在局限性。

Conclusion: 扩散模型在医学影像中潜力巨大，未来需进一步研究以克服当前限制。
...
### [336] [Evaluation of UAV-Based RGB and Multispectral Vegetation Indices for Precision Agriculture in Palm Tree Cultivation](https://arxiv.org/abs/2505.07840)
*Alavikunhu Panthakkan,S M Anzar,K. Sherin,Saeed Al Mansoori,Hussain Al-Ahmad*

Main category: eess.IV

TLDR: 该研究评估了无人机（UAV）在迪拜棕榈树种植区的植被健康监测效果，发现基于RGB图像的植被指数与昂贵的多光谱指数性能相当，为大规模农业监测提供了经济高效的替代方案。


<details>
  <summary>Details</summary>
Motivation: 精准农业需要准确的植被监测以提高作物产量和可持续性。研究旨在验证RGB图像在多光谱监测中的替代潜力，降低成本。

Method: 使用配备多光谱传感器的无人机，计算NDVI和SAVI等指数，同时比较RGB图像生成的VARI和MGRVI指数，分类植被健康状况。

Result: RGB图像生成的植被指数在多光谱监测中表现相似，验证了其成本效益和实用性。

Conclusion: RGB图像在精准农业中具有广泛应用潜力，可降低监测成本，推动数据驱动的农业决策普及。
...
### [337] [Skeleton-Guided Diffusion Model for Accurate Foot X-ray Synthesis in Hallux Valgus Diagnosis](https://arxiv.org/abs/2505.08247)
*Midi Wan,Pengfei Li,Yizhuo Liang,Di Wu,Yushan Pan,Guangzhen Zhu,Hao Wang*

Main category: eess.IV

TLDR: 提出了一种骨骼约束条件扩散模型（SCCDM）和足部评估方法KCC，显著提升了医学图像合成的质量。


<details>
  <summary>Details</summary>
Motivation: 解决现有X射线模型在图像保真度、骨骼一致性和物理约束方面的不足，特别是在缺乏骨骼引导的扩散方法中。

Method: 结合多尺度特征提取和注意力机制，并引入骨骼约束条件扩散模型（SCCDM）和KCC评估方法。

Result: SSIM提升5.72%（0.794），PSNR提升18.34%（21.40 dB），结合KCC平均得分0.85，临床适用性强。

Conclusion: SCCDM和KCC方法在医学图像合成中表现出色，具有实际应用潜力。
...
### [338] [An integrated language-vision foundation model for conversational diagnostics and triaging in primary eye care](https://arxiv.org/abs/2505.08414)
...
TLDR: Meta-EyeFM是一个结合大型语言模型（LLM）和视觉基础模型（VFM）的多功能基础模型，用于眼科疾病评估，通过路由机制实现基于文本查询的精准任务分析。
...
Method: 利用低秩适应（Low Rank Adaptation）微调视觉基础模型，结合路由机制实现任务分配，支持疾病检测、严重程度区分和常见体征识别。

Result: 模型在路由任务中达到100%准确率，疾病检测准确率≥82.2%，严重程度区分≥89%，体征识别≥76%，优于Gemini-1.5-flash和ChatGPT-4o LMMs，与眼科医生相当。
...
### [339] [GNCAF: A GNN-based Neighboring Context Aggregation Framework for Tertiary Lymphoid Structures Semantic Segmentation in WSI](https://arxiv.org/abs/2505.08430)
*Lei Su*

Main category: eess.IV

TLDR: 提出了一种基于GNN的邻近上下文聚合框架（GNCAF），用于端到端地分割WSI中的TLS区域和成熟阶段，显著提升了性能。
...
Method: 采用GNN逐步聚合多跳邻近上下文，并结合自注意力机制指导目标分割。

Result: 在TCGA-COAD和INHOUSE-PAAD数据集上，mF1和mIoU分别提升22.08%和26.57%。

Conclusion: GNCAF能有效整合邻近上下文信息，提升分割性能，并具有任务扩展性。
...
### [340] [A portable diagnosis model for Keratoconus using a smartphone](https://arxiv.org/abs/2505.08616)
*Yifan Li,Myeongjun Kim,Yanjing Jin,Peter Ho,Jo Woon Chong*

Main category: eess.IV

TLDR: 提出了一种基于智能手机的便携式圆锥角膜诊断框架，通过两阶段检测流程实现高精度分类和可视化。


<details>
  <summary>Details</summary>
Motivation: 圆锥角膜（KC）的诊断依赖专业设备，限制了可及性。本研究旨在开发一种便携、低成本的智能手机解决方案。

Method: 使用智能手机屏幕显示Placido盘，捕获角膜反射，通过两阶段检测流程（WSVM分类和颜色映射可视化）分析KC阶段。

Result: 在多种智能手机上验证，最高准确率达92.93%，且能直观显示病变区域。统计检验显示特征区分效果显著。

Conclusion: 智能手机框架为KC诊断提供了便携且高精度的替代方案，具有临床潜力。
...
### [341] [VIViT: Variable-Input Vision Transformer Framework for 3D MR Image Segmentation](https://arxiv.org/abs/2505.08693)
*Badhan Kumar Das,Ajay Singh,Gengyan Zhao,Han Liu,Thomas J. Re,Dorin Comaniciu,Eli Gibson,Andreas Maier*

Main category: eess.IV

TLDR: 提出了一种基于Transformer的框架VIViT，用于自监督预训练和可变对比度分割任务，解决了MR数据输入不一致的挑战。
...
Method: 设计了VIViT框架，支持自监督预训练和可变对比度的分割微调，充分利用数据并适应不同输入需求。

Result: 在脑梗死和脑肿瘤分割任务中，VIViT分别取得0.624和0.883的平均Dice分数，优于现有CNN和ViT模型。

Conclusion: VIViT在异构MR数据任务中表现出更好的适应性和性能，为实际应用提供了有效解决方案。
...
### [342] [NAZM: Network Analysis of Zonal Metrics in Persian Poetic Tradition](https://arxiv.org/abs/2505.08052)
*Kourosh Shahnazari,Seyed Moein Ayyoubzadeh*

Main category: cs.SI

TLDR: 该研究通过构建多维相似性网络，模拟古典波斯诗人的影响力动态，利用语义、词汇、风格、主题和韵律特征分析诗人作品，并通过网络分析识别关键诗人、风格中心和桥梁诗人。


<details>
  <summary>Details</summary>
Motivation: 旨在结合计算语言学和文学研究，提供一个可解释且可扩展的诗歌传统模型，以揭示波斯文学中的经典意义与互文影响。

Method: 使用Ganjoor语料库的严格数据集，构建加权相似性矩阵和聚合图，计算中心性指标，并应用Louvain社区检测算法进行聚类分析。

Result: 研究发现了一些在结构上具有重要意义的相对不知名诗人，并揭示了与已知文学流派（如Sabk-e Hindi等）密切相关的诗人群体。

Conclusion: 该研究为数字人文领域提供了新的数据驱动视角，支持对诗歌传统的回顾性反思和前瞻性研究。
...
### [343] [The Truth Becomes Clearer Through Debate! Multi-Agent Systems with Large Language Models Unmask Fake News](https://arxiv.org/abs/2505.08532)
*Yuhan Liu,Yuxuan Liu,Xiaoqing Zhang,Xiuying Chen,Rui Yan*

Main category: cs.SI

TLDR: 论文提出了一种名为TruEDebate（TED）的多智能体系统，利用大型语言模型（LLMs）通过模拟辩论过程提升假新闻检测的可解释性和效果。
...
Method: TED采用辩论流程，包括DebateFlow Agents（组织正反方辩论）和InsightFlow Agents（总结与分析辩论内容）。

Result: 通过模拟人类辩论过程，TED实现了对新闻内容的全面评估，提升了检测的可解释性和效果。

Conclusion: TED通过多智能体辩论框架，有效结合LLMs的推理能力，为假新闻检测提供了新思路。
...
### [344] [Beyond Basic A/B testing: Improving Statistical Efficiency for Business Growth](https://arxiv.org/abs/2505.08128)
*Changshuai Wei,Phuc Nguyen,Benjamin Zelditch,Joyce Chen*

Main category: stat.ME

TLDR: 论文提出多种方法改进传统A/B测试的统计功效问题，包括回归调整、广义估计方程等，并提出一种新型双重稳健广义U方法，综合解决样本量小、非高斯分布和ROI问题。


<details>
  <summary>Details</summary>
Motivation: 传统A/B测试方法（如t检验）在样本量小、非高斯分布或ROI考虑下统计功效较低，无法满足实际业务需求。

Method: 提出回归调整、广义估计方程、Man-Whitney U和Zero-Trimmed U等方法，并设计新型双重稳健广义U框架。

Result: 理论分析证明了方法的渐近正态性和效率界限，仿真和实际A/B测试验证了方法的有效性。

Conclusion: 新型双重稳健广义U方法能综合解决多种挑战，提升A/B测试的统计功效。
...
### [345] [Tensor Sketch: Fast and Scalable Polynomial Kernel Approximation](https://arxiv.org/abs/2505.08146)
*Ninh Pham,Rasmus Pagh*

Main category: cs.DS

TLDR: Tensor Sketch是一种高效随机特征映射方法，用于近似多项式核，适用于高维和大规模数据集。


<details>
  <summary>Details</summary>
Motivation: 为了解决非线性核在大规模数据集上的计算效率问题。

Method: 提出Tensor Sketch方法，通过低维嵌入高效计算多项式核的近似值。

Result: 理论保证了近似误差的准确性，计算复杂度为O(n(d+D log D))。

Conclusion: Tensor Sketch在高维和大规模场景下具有广泛应用潜力。
...
### [346] [Aitomia: Your Intelligent Assistant for AI-Driven Atomistic and Quantum Chemical Simulations](https://arxiv.org/abs/2505.08195)
*Jinming Hu,Hassan Nawaz,Yuting Rui,Lijie Chi,Arif Ullah,Pavlo O. Dral*

Main category: physics.comp-ph

TLDR: Aitomia是一个由AI驱动的平台，旨在辅助原子级和量子化学模拟，通过聊天机器人和AI代理帮助专家和非专家完成模拟设置、运行、监控和分析。


<details>
  <summary>Details</summary>
Motivation: 降低原子级模拟的门槛，加速相关领域的研究和开发。

Method: 结合微调的开源大型语言模型（LLMs）、基于规则的代理和检索增强生成（RAG）系统。

Result: 平台已部分公开，并计划集成到Aitomistic Hub和XACS在线计算服务中。

Conclusion: Aitomia有望简化原子级模拟流程，推动相关领域的进步。
...
### [347] [SPP-SBL: Space-Power Prior Sparse Bayesian Learning for Block Sparse Recovery](https://arxiv.org/abs/2505.08518)
*Yanhao Zhang,Zhihan Zhu,Yong Xia*

Main category: math.OC

TLDR: 本文提出了一种基于方差变换框架的统一方法SPP-SBL，结合无向图模型的空间功率先验，自适应捕捉块稀疏信号的未知结构模式，并通过EM算法和高阶方程求解解决了空间耦合参数估计问题。


<details>
  <summary>Details</summary>
Motivation: 块稀疏信号的结构模式未知是结构化稀疏信号重建中的核心挑战，现有方法难以统一且适应性不足。

Method: 提出方差变换框架，结合无向图模型的空间功率先验，开发SPP-SBL方法，利用EM算法和高阶方程求解估计空间耦合参数。

Result: SPP-SBL成功恢复了多种复杂结构化稀疏信号（如链式结构和多模式信号）及真实多模态信号（图像、音频），在多个指标上显著提升恢复精度。

Conclusion: 学习空间耦合参数的相对值是捕捉未知块稀疏模式和提高恢复精度的关键，SPP-SBL方法在理论和实验上均表现出优越性。
...
### [348] [Distributed Quantum Neural Networks on Distributed Photonic Quantum Computing](https://arxiv.org/abs/2505.08474)
*Kuan-Cheng Chen,Chen-Yu Liu,Yu Shang,Felix Burt,Kin K. Leung*

Main category: quant-ph

TLDR: 提出了一种分布式量子-经典框架，结合光子量子神经网络（QNNs）和矩阵乘积态（MPS）映射，实现经典神经网络的高效参数训练。


<details>
  <summary>Details</summary>
Motivation: 通过结合量子计算和经典计算的优点，开发一种参数高效的训练方法，同时减少量子硬件需求。

Method: 利用光子QNNs生成高维概率分布，并通过MPS模型映射到经典网络权重，实现参数压缩。

Result: 在MNIST分类任务中，该框架实现了95.50%的准确率，参数数量仅为3,292，压缩比为10倍时准确率损失小于3%。
...
### [349] [SciCom Wiki: Fact-Checking and FAIR Knowledge Distribution for Scientific Videos and Podcasts](https://arxiv.org/abs/2505.07912)
*Tim Wittenborg,Constantin Sebastian Tremel,Niklas Stehr,Oliver Karras,Markus Stocker,Sören Auer*

Main category: cs.DL

TLDR: 该论文提出了一种名为SciCom Wiki的协作平台，旨在支持科学传播知识基础设施（SciCom KI），通过FAIR媒体表示和内容事实核查，特别是针对视频和播客。


<details>
  <summary>Details</summary>
Motivation: 民主社会需要可靠的信息，但视频和播客既是信息传播的媒介，也是错误信息的载体。现有的SciCom KI在非文本媒体管理上仍不完善，无法应对内容泛滥。

Method: 通过调查53位利益相关者的需求，进行11次访谈，并基于这些需求评估原型。开发了一种神经符号计算事实核查工具，将异构媒体转换为知识图谱。

Result: 研究确定了SciCom KI的多种需求，SciCom Wiki和事实核查工具被证明能有效满足这些需求。但SciCom KI在FAIR知识和协作系统方面仍显不足。

Conclusion: SciCom Wiki和事实核查工具为SciCom KI提供了核心支持，但需协作努力以应对信息泛滥。
...
### [350] [Area Comparison of CHERIoT and PMP in Ibex](https://arxiv.org/abs/2505.08541)
*Samuel Riedel,Marno van der Maas,John Thomson,Andreas Kurth,Pirmin Vogel*

Main category: cs.AR

TLDR: 论文研究了在Ibex RISC-V核心中添加内存安全扩展（PMP和CHERIoT）的面积影响，发现扩展增加了核心面积，但对整体系统影响较小，安全收益显著。


<details>
  <summary>Details</summary>
Motivation: 现代嵌入式系统，尤其是安全敏感应用中，内存安全至关重要。

Method: 通过商业工具合成扩展后的Ibex核心，分析PMP（16个区域）和CHERIoT的面积开销。

Result: PMP增加24kGE（42%），CHERIoT增加33kGE（57%）；在完整SoC中，系统总开销分别为0.6%和1%。

Conclusion: 尽管面积增加，但安全收益显著，Ibex成为安全嵌入式应用的理想选择。

Abstract: Memory safety is a critical concern for modern embedded systems, particularly
...
### [351] [SpNeRF: Memory Efficient Sparse Volumetric Neural Rendering Accelerator for Edge Devices](https://arxiv.org/abs/2505.08191)
*Yipu Zhang,Jiawei Liang,Jian Peng,Jiang Xu,Wei Zhang*

Main category: cs.AR

TLDR: SpNeRF提出了一种软硬件协同设计的稀疏体素神经渲染方法，显著减少内存占用并提升实时性能。


<details>
  <summary>Details</summary>
Motivation: 神经渲染在AR/VR中应用广泛，但大体积网格数据和不规则访问模式限制了边缘设备的实时处理能力。现有方法未充分解决内存占用问题。

Method: 通过预处理（哈希映射）和在线解码（位图掩码）减少内存占用，并设计专用硬件架构优化性能。

Result: 实验显示内存减少21.07倍，PSNR保持相近，性能提升显著（最高95.1倍），能效提升显著（最高625.6倍）。

Conclusion: SpNeRF有效解决了神经渲染的内存和性能瓶颈，适用于边缘设备。
...
### [352] [MINIMALIST: switched-capacitor circuits for efficient in-memory computation of gated recurrent units](https://arxiv.org/abs/2505.08599)
*Sebastian Billaudelle,Laura Kriener,Filippo Moro,Tristan Torchet,Melika Payvand*

Main category: cs.AR

TLDR: 提出了一种基于最小门控循环单元（GRU）的高效RNN架构及其混合信号硬件实现，利用开关电容电路进行内存计算和门控状态更新。


<details>
  <summary>Details</summary>
Motivation: 在嵌入式边缘计算环境中，需要高效处理时序数据的RNN架构。

Method: 设计了一种硬件兼容的GRU架构，采用开关电容电路实现内存计算和门控状态更新。

Result: 通过混合信号仿真验证了硬件兼容性，并复现了软件模型的性能。

Conclusion: 该架构在资源受限的环境中具有高效性和可扩展性。
...
### [353] [Bridging Large Language Models and Single-Cell Transcriptomics in Dissecting Selective Motor Neuron Vulnerability](https://arxiv.org/abs/2505.07896)
*Douglas Jiang,Zilin Dai,Luxuan Zhang,Qiyi Yu,Haoqi Sun,Feng Tian*

Main category: q-bio.GN

TLDR: 提出了一种利用NCBI Gene数据库的基因注释和大型语言模型生成生物上下文细胞嵌入的新框架，用于单细胞RNA测序数据分析。


<details>
  <summary>Details</summary>
Motivation: 解决单细胞测序数据中细胞身份和功能理解的挑战。

Method: 通过表达水平排序基因，检索NCBI Gene描述，用LLMs（如OpenAI和BioBERT）将其转化为向量嵌入，并通过表达加权平均生成细胞嵌入。

Result: 生成紧凑且语义丰富的细胞嵌入，支持细胞类型聚类、脆弱性分析和轨迹推断等应用。

Conclusion: 该多模态策略结合生物数据和语言模型，提高了单细胞数据分析的可解释性和应用潜力。
...
### [354] [CellVerse: Do Large Language Models Really Understand Cell Biology?](https://arxiv.org/abs/2505.07865)
*Fan Zhang,Tianyu Liu,Zhihong Zhu,Hao Wu,Haixin Wang,Donghao Zhou,Yefeng Zheng,Kun Wang,Xian Wu,Pheng-Ann Heng*

Main category: q-bio.QM

TLDR: 论文介绍了CellVerse，一个语言驱动的单细胞分析基准测试，评估了14种LLMs在细胞生物学任务中的表现，发现现有模型表现不佳，仍有改进空间。


<details>
  <summary>Details</summary>
Motivation: 现有研究未全面评估LLMs在语言驱动的单细胞分析任务中的表现，因此作者提出CellVerse以填补这一空白。

Method: 通过整合四种单细胞多组学数据，设计三个层次的分析任务（细胞类型注释、药物反应预测、扰动分析），并系统评估14种LLMs的表现。

Result: 实验显示，现有专家模型表现不佳，通用模型初步具备理解能力，但整体性能仍有较大提升空间。
...
### [355] [Automated Model-Free Sorting of Single-Molecule Fluorescence Events Using a Deep Learning Based Hidden-State Model](https://arxiv.org/abs/2505.08608)
*Wenqi Zeng,Shuqi Zhou,Yuan Yao,Chunlai Chen*

Main category: q-bio.QM

TLDR: DASH是一种全自动的单分子荧光数据分析架构，无需用户输入即可完成分类、状态分配和自动排序，适用于平衡和非平衡系统。


<details>
  <summary>Details</summary>
Motivation: 传统单分子荧光数据分析流程依赖人工和经验，难以扩展和复现，而现有深度学习模型仍需手动阈值或大量标注数据。

Method: 提出DASH架构，完全自动化处理单分子荧光数据，无需用户干预，并以Cas12a介导的DNA切割为例验证。

Result: DASH在平衡和非平衡系统中均表现稳健，适用于不同用户和实验条件。

Conclusion: DASH为单分子水平生物动力学结构变化研究提供了一种自动且详细的分析方法。
...
### [356] [Getting Ready for the EU AI Act in Healthcare. A call for Sustainable AI Development and Deployment](https://arxiv.org/abs/2505.07875)
...
TLDR: 论文强调在医疗AI领域，开发者需主动确保系统符合2024年生效的《AI法案》要求，以在2026年实现全面合规，并强调伦理原则的重要性。


<details>
  <summary>Details</summary>
Motivation: 在高风险领域如医疗中，确保AI系统的可信度与法律要求对齐是紧迫任务。

Method: 建议开发者采取主动措施，确保现有及未来AI系统符合《AI法案》要求，并基于伦理原则进行合规。

Result: 通过伦理原则的应用，可以增强AI系统的有效性和可持续性。

Conclusion: 合规不仅是形式化任务，还需基于伦理原则，以保护公共利益并提升AI系统的长期有效性。
...
### [357] [Multimodal Assessment of Classroom Discourse Quality: A Text-Centered Attention-Based Multi-Task Learning Approach](https://arxiv.org/abs/2505.07902)
*Ruikun Hou,Babette Bühler,Tim Fütterer,Efe Bozkir,Peter Gerjets,Ulrich Trautwein,Enkelejda Kasneci*

Main category: cs.CY

TLDR: 提出了一种基于多模态融合的架构，用于评估课堂话语质量，结合文本、音频和视频数据，并通过多任务学习和序数分类提升效果。


<details>
  <summary>Details</summary>
Motivation: 传统课堂话语评估依赖人工编码，耗时且昂贵，而现有AI技术多限于单句分析，缺乏对整个课程段话语质量的评估。

Method: 采用注意力机制捕捉多模态交互，多任务学习预测三个话语组件的质量分数，并将任务建模为序数分类问题。

Result: 在GTI德国数据集上测试，文本模态主导任务，结合音频特征提升模型一致性，总体Quadratic Weighted Kappa得分为0.384。
...
### [358] [LECTOR: Summarizing E-book Reading Content for Personalized Student Support](https://arxiv.org/abs/2505.07898)
*Erwin Daniel López Zapata,Cheng Tang,Valdemar Švábenský,Fumiya Okubo,Atsushi Shimada*

Main category: cs.CY

TLDR: 论文提出LECTOR模型，结合阅读内容和活动数据，提升教育电子书平台的分析能力。实验显示其在信息提取和预测低分学生方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前教育电子书平台主要依赖阅读活动数据，忽视了阅读内容数据的价值，LECTOR旨在填补这一空白。

Method: LECTOR模型从阅读内容中提取关键信息，并与阅读活动数据结合。实验包括与NLP模型对比及预测低分学生的效果验证。

Result: LECTOR在信息提取上F1分数提升5%，人类评估中提升21%；结合阅读偏好后，预测低分学生性能有所提高。

Conclusion: LECTOR展示了整合阅读内容数据的潜力，为个性化教育干预提供了新思路。
...
### [359] [One Bad NOFO? AI Governance in Federal Grantmaking](https://arxiv.org/abs/2505.08133)
*Dan Bateyko,Karen Levy*

Main category: cs.CY

TLDR: 论文探讨了美国联邦机构如何通过拨款政策间接治理人工智能（AI），发现其监管力度不足，尤其在透明度和问责制方面。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注联邦机构通过规则制定和内部政策治理AI，但忽视了拨款政策作为隐蔽治理工具的作用。

Method: 通过分析2009-2024年Grants.gov上4万多项非国防联邦拨款通知，筛选提及AI的记录，研究其目标和要求。

Result: 发现机构虽在拨款通知中推广AI，但极少设置AI专用评判标准或限制，监管力度不足。

Conclusion: 拨款通知是AI政策制定的新领域，但与其他监管措施脱节，需进一步研究其透明度和问责制问题。
...
### [360] [Reciprocity as the Foundational Substrate of Society: How Reciprocal Dynamics Scale into Social Systems](https://arxiv.org/abs/2505.08319)
*Egil Diau*

Main category: cs.CY

TLDR: 提出了一种三阶段自下而上的框架，用于模拟社会结构的涌现，填补了多智能体AI中缺乏可模拟模型的空白。


<details>
  <summary>Details</summary>
Motivation: 解决多智能体AI中缺乏模拟社会结构涌现的模型问题，同时为经济学和社会学中的“制度”和“规范”概念提供基于智能体行为的解释。

Method: 提出三阶段框架：互惠动力学（个体层面的互惠交换）、规范稳定化（共享期望的巩固）和制度构建（稳定模式的外部化）。

Result: 通过基于智能体互惠行为的社会涌现，系统探索了道德、文化和制度结构的起源。

Conclusion: 该框架为理解社会结构的自下而上涌现提供了新视角，并填补了理论和实践中的空白。
